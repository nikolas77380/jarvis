...
TMP_ROOT=$(dj_test_tmproot dj-captain-hold)
...
EOF
  ...
  dj_fake_exit0 "$fakebin" tmux treehouse no-mistakes gh gh-axi
  printf '%s\n' "$home"
}
...
run_bearings() {  # <home>
  local home=$1
  PATH="$home/fakebin:$PATH" DJ_HOME="$home" DJ_BEARINGS_NOW=2026-07-14T12:00:00Z \
    "$BEARINGS" --json
}
...
run_teardown() {  # <home> <id>
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id"
}
...
run_captain() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/dj-captain-hold.sh" "$@"
}
...
run_shim() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/dj-decision-hold.sh" "$@"
}
...
EOF
  ...
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$home/state" "$home/state/$id.status" \
  ...
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/dj-classify-lib.sh" "$home/state/$id.status")
...
test_answer_records_and_closes() {
  ...
  assert_contains "$show" "Resolution recorded by dj-captain-hold" "the answered task lost the decision record"
...
test_deferral_leaves_captains_call_until_due() {
  ...
  snap=$(PATH="$home/fakebin:$PATH" DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" \
    DJ_DATA_OVERRIDE="$home/data" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_PROJECTS_OVERRIDE="$home/projects" DJ_SNAPSHOT_NOW=2026-07-14T12:00:00Z \
    "$ROOT/bin/dj-fleet-snapshot.sh" --json) || fail "fleet snapshot failed"
  printf '%s' "$snap" | jq -e '
  ...
  snap=$(PATH="$home/fakebin:$PATH" DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" \
    DJ_DATA_OVERRIDE="$home/data" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_PROJECTS_OVERRIDE="$home/projects" DJ_SNAPSHOT_NOW=2026-08-01T12:00:00Z \
    "$ROOT/bin/dj-fleet-snapshot.sh" --json) || fail "fleet snapshot failed at the due date"
  printf '%s' "$snap" | jq -e '
...
EOF
  ...
  dj_write_secondmate_meta "$parent/state/sample-mate.meta" "$mate" \
    "jarvis:dj-sample-mate" sample
...
EOF
  ...
  cat > "$home/adapter-root/bin/dj-procevent-fixturechan.sh" <<SH
#!/usr/bin/env bash
...
exit 2
SH
  chmod +x "$home/adapter-root/bin/dj-procevent-fixturechan.sh"
  run_captain "$home" bind fixture-src >/dev/null \
    || fail "could not bind the fixture channel"
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$home/adapter-root" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/dj-procevent.sh" register fixturechan fixture-src -- cat "$result" >/dev/null \
    || fail "could not register the fixture channel source"
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$home/adapter-root" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/dj-procevent.sh" start fixture-src >/dev/null 2>&1
  assert_absent "$home/state/procevent-inbox/fixture-src.1.handled" \
    "feeding a captain answer retired the notification jarvis still needs"
...
SH
  ...
  : > "$home/send.log"
  env PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_SEND_LOG="$home/send.log" DJ_SEND_SETTLE=0 \
    "$ROOT/bin/dj-send.sh" "$id" --resolve-key chat-choice "go with option A" >/dev/null 2>&1 \
    || fail "an answer to a transferred legacy decision was refused by the chat channel"
  # The answer rides dj-send's durable inbox plane: the record carries the
  ...
  env PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_SEND_LOG="$home/send.log" DJ_SEND_SETTLE=0 \
    "$ROOT/bin/dj-send.sh" "$id" --resolve-key sample-chat-followup "take the second option" >/dev/null 2>&1 \
  ...
  if env PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_SEND_LOG="$home/send.log" DJ_SEND_SETTLE=0 \
    "$ROOT/bin/dj-send.sh" "$id" --resolve-key sample-chat-followup "again" \
...
run_drain() {  # <home>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    ...
    DJ_DATA_OVERRIDE="$home/data" DJ_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/dj-wake-drain.sh" 2>/dev/null
}
...
# is where jarvis actually looks.
...
EOF
  ...
  printf '%s\n' "$drain" | grep -F 'dj-captain-hold.sh answer' >/dev/null \
