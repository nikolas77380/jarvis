#!/usr/bin/env bash
# dj-send remote-secondmate delivery: the remote inbox leg.
#
# A remote steer is delivered by durable record, never by typing its payload:
# dj-send crosses dj-on.sh to the host-local leg (dj-remote-secondmate-control.sh
...
# the real dj-send + dj-on executables with a stubbed ssh transport that
# EXECUTES the real host-local leg against a seeded remote-home fixture (the
# DJ_SSH_BIN seam tests/dj-on.test.sh proves preserves exit status), and pin:
...
#      (executed remotely, then ssh exit 255) makes dj-send retry the same
...
TMP_ROOT=$(dj_test_tmproot dj-send-remote-delivery)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd)

# Stub tmux for the local typed-plane legs: logs literal typed text to
# DJ_SEND_LOG. The default composer reads empty (clean submit);
# DJ_FAKE_TMUX_PENDING=1 keeps a proven pending composer with no busy footer,
...
# fails with DJ_FAKE_SSH_RC (emitting DJ_FAKE_SSH_STDERR as the remote
# stderr), or decodes the entrypoint argv and executes the REAL host-local
# command against the decoded remote home - with DJ_FAKE_SSH_AMBIGUOUS=1
...
case "${1:-}" in
  ...
  display-message)
    ...
    printf 'fakepane\n'; exit 0 ;;
  capture-pane)
    if [ "${DJ_FAKE_TMUX_PENDING:-0}" = 1 ]; then
      printf '╭────────────╮\n│ > steer    │\n╰────────────╯\n'
    else
      printf '╭────╮\n│    │\n╰────╯\n'
    fi
...
set -u
cat > /dev/null
count=$(cat "$DJ_SSH_COUNT" 2>/dev/null || echo 0)
count=$((count + 1))
printf '%s\n' "$count" > "$DJ_SSH_COUNT"
printf '%s\n' "$*" >> "$DJ_SSH_LOG"
if [ "${DJ_FAKE_SSH_AFTER_AMBIGUOUS_RC:-0}" -ne 0 ] && [ "$count" -gt 1 ]; then
  exit "$DJ_FAKE_SSH_AFTER_AMBIGUOUS_RC"
fi
if [ "${DJ_FAKE_SSH_RC:-0}" -ne 0 ]; then
  [ -z "${DJ_FAKE_SSH_STDERR:-}" ] || printf '%s\n' "$DJ_FAKE_SSH_STDERR" >&2
  exit "${DJ_FAKE_SSH_RC}"
fi
while [ "$#" -gt 0 ]; do
  case "$1" in -o) shift 2 ;; --) shift; break ;; *) exit 90 ;; esac
done
shift 2  # host, dj-remote-entrypoint.sh
home_b64=$3
argv_b64=$4
...
cmd=${rargs[0]}
rc=0
env DJ_HOME="$remote_home" DJ_ROOT_OVERRIDE="$DJ_REMOTE_CODE_ROOT" \
  "$DJ_REMOTE_CODE_ROOT/bin/$cmd" "${rargs[@]:1}" || rc=$?
if [ "${DJ_FAKE_SSH_AMBIGUOUS:-0}" = 1 ] \
  || { [ "${DJ_FAKE_SSH_AFTER_AMBIGUOUS_RC:-0}" -ne 0 ] && [ "$count" -eq 1 ]; }; then
  exit 255
fi
exit "$rc"
SH
...
# writes into: identity marker, Jarvis-checkout shape, and a parent-route
# endpoint record on Herdr in the dedicated dj-remote session.
setup_remote_secondmate_home() {  # <name> -> echoes remote home dir
  local rh="$TMP_ROOT/$1-rhome"
  mkdir -p "$rh/state/parent-route" "$rh/bin"
  printf 'rsm\n' > "$rh/.dj-secondmate-home"
  printf '# remote secondmate home fixture\n' > "$rh/AGENTS.md"
  dj_write_meta "$rh/state/parent-route/rsm.meta" \
    "window=dj-remote:p1" \
    "worktree=-" \
    "project=-" \
    "backend=herdr" \
...
# dj-on.sh resolves the ssh route from, pointing at <remote-home>.
setup_remote_parent_home() {  # <name> <remote-home> -> echoes home dir
  local home
  home=$(setup_home "$1")
  mkdir -p "$home/data"
  dj_write_meta "$home/state/rsm.meta" \
    "window=dj-remote:p1" \
    "endpoint_task_id=rsm" \
    "harness=claude" \
    "kind=secondmate" \
...
drain_out() {  # <home>
  DJ_STATE_OVERRIDE="$1/state" "$DRAIN" 2>/dev/null
}
...
send_env() {  # <fakebin> <parent-home> <ssh-log> [extra env...] -- <cmd...>
  ...
  shift 3
  env PATH="$fb:$PATH" \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" DJ_SEND_SETTLE=0 \
    DJ_SSH_BIN="$fb/fake-ssh" DJ_SSH_LOG="$ssh_log" \
    DJ_SSH_COUNT="$ssh_log.count" DJ_REMOTE_CODE_ROOT="$ROOT" \
    "$@"
}
...
test_remote_steer_lands_in_remote_inbox() {
  ...
  assert_grep 'dj-remote-entrypoint.sh' "$ssh_log" "the steer should cross the remote transport"
  ...
  case "$body" in
    *"$DJ_FROMFIRST_MARK"*) : ;;
    *) fail "the remote record must carry the from-jarvis marker: $body" ;;
  esac
  ...
  pass "dj-send remote: a text steer lands as a durable remote inbox record and exits 0 at enqueue"
}
...
test_remote_rerun_is_idempotent() {
  ...
  # 255 (completion unknown). dj-send retries the identical leg once; the
  # idempotent remote write must land both executions on the same record.
  rc=0
  (
    cd "$TMP_ROOT" || exit 1
    send_env "$fb" "${home#"$TMP_ROOT/"}" "$ssh_log" DJ_FAKE_SSH_AMBIGUOUS=1 \
  ...
  [ "$(cat "$ssh_log.count")" = 2 ] \
    || fail "dj-send must retry the identical remote leg exactly once after ssh 255, got $(cat "$ssh_log.count") attempts"
  ...
  corr=$(dj_pending_reply_get "$pend" corr_id)
  printf -v quoted '%q' "$home"
  expected_cmd="DJ_HOME=$quoted DJ_PENDING_REPLY_EXISTING_CORR=$corr"
  ...
  dj_pending_reply_set "$pend" phase delivery_unknown \
  ...
  (
    unset DJ_HOME
    env PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_SEND_SETTLE=0 \
      DJ_SSH_BIN="$fb/fake-ssh" DJ_SSH_LOG="$ssh_log" \
      DJ_SSH_COUNT="$ssh_log.count" DJ_REMOTE_CODE_ROOT="$ROOT" \
      bash -c "$resend_cmd"
  ) >"$dir/resend.out" 2>"$dir/resend.err" || rc=$?
  expect_code 0 "$rc" "the printed correlation-reusing resend must succeed without an inherited DJ_HOME"
  ...
  [ -n "$(dj_pending_reply_get "$pend" delivered_epoch)" ] \
    || fail "the successful resend did not confirm pending-reply delivery: $(cat "$pend")"
  [ "$(dj_pending_reply_get "$pend" phase)" = awaiting_report ] \
    || fail "the successful resend did not restore awaiting_report: $(cat "$pend")"
  pass "dj-send remote: the printed correlation-reusing resend deduplicates onto the same record"
}
...
test_remote_retry_failure_preserves_ambiguous_expectation() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" DJ_FAKE_SSH_AFTER_AMBIGUOUS_RC=1 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "dj-send remote: a failed retry cannot erase an earlier ambiguous delivery"
}
...
test_remote_send_revalidates_after_retirement_lock() {
  ...
  DJ_STATE_OVERRIDE="$rhome/state/parent-route" bash -c '
    . "$1"
    dj_task_inbox_lock_acquire "$2" || exit 91
    : > "$3"
    while [ ! -e "$4" ]; do sleep 0.05; done
    rm -f "$5"
    dj_lock_release "$2"
  ' _ "$ROOT/bin/dj-task-inbox-lib.sh" "$lock" "$ready" "$release" "$meta" &
  holder_pid=$!
  while [ ! -e "$ready" ]; do kill -0 "$holder_pid" 2>/dev/null || fail "metadata-lock holder exited early"; sleep 0.05; done

  rc=0
  env DJ_HOME="$rhome" DJ_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/dj-remote-secondmate-control.sh" send rsm "retirement-race steer" \
    >"$dir/out" 2>"$dir/err" &
  sender_pid=$!
  sleep 0.2
  : > "$release"
...
test_remote_send_revalidates_parent_route_after_retirement_lock() {
  ...
  ready="$dir/lock-ready"
  release="$dir/release-lock"
  DJ_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"
    dj_task_inbox_lock_acquire "$2" || exit 91
    : > "$3"
    while [ ! -e "$4" ]; do sleep 0.05; done
  ...
  pass "dj-send remote: enqueue revalidates the parent route under its metadata lock"
}
...
test_remote_resolve_key_closes_at_enqueue() {
  ...
  pass "dj-send remote: a --resolve-key answer closes its decision at enqueue"
}
...
test_remote_slash_rides_inbox() {
  ...
  pass "dj-send remote: every remote text, harness-native included, rides the inbox"
}
...
test_remote_real_failure_still_fails() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" DJ_FAKE_SSH_RC=1 \
    DJ_FAKE_SSH_STDERR='error: remote secondmate rsm endpoint metadata is invalid; refusing access until it is explicitly migrated' \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "dj-send remote: a real remote failure still fails loudly with the remote diagnostics"
}
...
test_remote_exit3_no_longer_delivered() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" DJ_FAKE_SSH_RC=3 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "dj-send remote: exit 3 from the remote leg is a failure, never a delivery claim"
}
...
test_remote_transport_loss_preserves_expectation() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" DJ_FAKE_SSH_RC=255 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  [ "$rc" -ne 0 ] || fail "an unknown-completion transport loss must exit nonzero"
  [ "$(cat "$ssh_log.count")" = 2 ] \
    || fail "dj-send must retry the identical remote leg once on ssh 255, got $(cat "$ssh_log.count") attempts"
  assert_contains "$err" "Only the correlation-reusing resend below is idempotent" \
  ...
  pass "dj-send remote: ssh 255 fails with resend-safe guidance and preserves the expectation"
}
...
test_local_secondmate_pending_keeps_expectation_armed() {
  ...
  dj_write_meta "$home/state/lsm.meta" \
    "window=sess:dj-lsm" "harness=claude" "kind=secondmate" "mode=secondmate" "home=$home/sm"
  ...
  : > "$log"
  rc=0
  env PATH="$fb:$PATH" DJ_FAKE_TMUX_PENDING=1 \
  ...
  [ "$(dj_pending_reply_get "$rec" phase)" = awaiting_report ] \
    || fail "the surviving expectation must stay armed, got $(dj_pending_reply_get "$rec" phase)"
  ...
  dj_pending_reply_try_resolve "$home/state" "$corr" \
    || fail "a correlated report must still resolve the preserved expectation"
  pass "dj-send local: an unconfirmed secondmate send keeps its reply expectation armed"
}
...
test_local_pending_reports_delivered_unconfirmed() {
  ...
  pass "dj-send local: an unconfirmed submit exits 3 with an honest non-error report"
}
...
test_local_pending_does_not_close_resolve_key() {
  ...
  dj_write_meta "$home/state/t2.meta" "window=sess:dj-t2" "kind=ship"
  ...
  pass "dj-send local: an unconfirmed submit still never closes a --resolve-key decision"
}
...
echo "all dj-send-remote-delivery tests passed"
