#!/usr/bin/env bash
# tests/dj-daemon.test.sh - supervise-daemon classifiers, the captain-relevant
...
# dj-send typed-plane swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in dj-afk-inject-e2e and dj-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/dj-supervise-daemon.sh"
AFK_START="$ROOT/bin/dj-afk-start.sh"
...
if [ -z "${DJ_TEST_DAEMON_SOURCED:-}" ]; then
  export DJ_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/dj-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(dj_test_tmproot dj-daemon-tests)
DJ_DAEMON_PRIMARY_HARNESS=claude
export DJ_DAEMON_PRIMARY_HARNESS
...
test_afk_start_refuses_when_flag_cannot_be_written() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" DJ_SUPERVISOR_BACKEND=unsupported "$AFK_START" 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "dj-afk-start.sh should fail when state/.afk cannot be written"
  assert_not_contains "$out" "starting supervise daemon" "dj-afk-start.sh continued into daemon startup after .afk write failure"
  assert_absent "$state/.supervise-daemon.log" "dj-afk-start.sh started the daemon after .afk write failure"
  pass "dj-afk-start.sh fails before daemon startup when the afk flag cannot be written"
}
...
test_afk_start_ignores_stale_pidfile_without_lock() {
  ...
  [ "$status" -ne 0 ] || fail "dj-afk-start.sh should attempt daemon startup instead of trusting a pidfile-only live pid"
  assert_contains "$out" "starting supervise daemon" "dj-afk-start.sh did not attempt daemon startup"
  assert_contains "$out" "does not support supervisor backend 'unsupported'" "daemon startup did not reach backend validation"
  assert_not_contains "$out" "daemon already running" "dj-afk-start.sh trusted a stale pidfile-only live pid"
  pass "dj-afk-start.sh ignores stale pidfile-only live pids"
}
...
test_afk_start_reclaims_stale_daemon_lock_reused_pid() {
  ...
  [ "$status" -ne 0 ] || fail "dj-afk-start.sh should attempt daemon startup after rejecting a reused-pid lock"
  assert_contains "$out" "starting supervise daemon" "dj-afk-start.sh did not attempt daemon startup after rejecting the stale lock"
  ...
  assert_not_contains "$out" "daemon already running" "dj-afk-start.sh trusted a stale daemon lock with a reused pid"
  assert_not_contains "$out" "another dj-supervise-daemon is already running" "daemon singleton lock still trusted the reused pid"
  pass "dj-afk-start.sh reclaims stale daemon locks whose live pid identity no longer matches"
}

test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-dj-home)
  home="$dir/jarvis-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored DJ_HOME: $out"

  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored DJ_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by DJ_HOME"
}
...
test_classify_routine_signal_self() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
            ...
            "ready in branch dj/t1" "merged"; do
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(DJ_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:dj-qux-w4" "$state"
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:dj-qux-w4")" | tr ':/.' '___')
...
test_stale_diagnostic_wedge_survives_busy_housekeeping() {
  ...
  for case_name in working prior-terminal paused; do
    ...
    dj_write_meta "$state/$task.meta" "window=$win" "backend=tmux"
    case "$case_name" in
    ...
    (
      kill() { printf 'kill %s\n' "$*" >> "$action_log"; }
      dj_backend_send_text_submit() { printf 'interrupt %s\n' "$*" >> "$action_log"; }
      LOG="$dir/daemon.log" DJ_STATE_OVERRIDE="$state" handle_wake "$reason" "$state"
      PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$win" DJ_FAKE_TMUX_CAPTURE="$pane" \
        DJ_STATE_OVERRIDE="$state" DJ_ESCALATE_BATCH_SECS=999999 housekeeping "$state"
    )
...
test_stale_terminal_escalates() {
  ...
  state="$dir/state"
  printf 'done: ready in branch dj/t1\n' > "$state/fin-t5.status"
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-fin-t5" "$state")
  case "$out" in escalate\|*) ;; *) fail "terminal stale did not escalate: $out" ;; esac
  dj_write_meta "$state/herdr-t5.meta" "window=default:w1:p2" "backend=herdr"
  printf 'done: ready in branch dj/herdr\n' > "$state/herdr-t5.status"
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "default:w1:p2" "$state")
...
test_stale_paused_classifies_pause() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-held-w9" "$state")
...
test_stale_captain_held_classifies_pause() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-held-w9h" "$state")
...
test_handle_wake_paused_records_pause_marker() {
  ...
  DJ_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_handle_wake_paused_signal_records_pause_marker() {
  ...
  state="$dir/state"
  win="sess:dj-held-w10-signal"
  printf 'window=%s\nkind=ship\n' "$win" > "$state/held-w10-signal.meta"
  printf 'paused: awaiting the vendor rate-limit reset\n' > "$state/held-w10-signal.status"
  key=$(printf '%s' "held-w10-signal" | tr ':/.' '___')
  date +%s > "$state/.subsuper-stale-$key"
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-signal.status" "$state"
...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  ...
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-terminal.status" "$state"
...
test_housekeeping_migrates_watcher_unpaused_marker_to_clear() {
  ...
  state="$dir/state"
  win="sess:dj-held-w10-migrate-unpaused"
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  win="sess:dj-held-w11"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w11.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w11" | tr ':/.' '___')
  echo $(( $(date +%s) - 5000 )) > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$win" DJ_FAKE_TMUX_CAPTURE="$pane" \
    DJ_STATE_OVERRIDE="$state" DJ_PAUSE_RESURFACE_SECS=240 housekeeping "$state"
...
# dj-classify-lib.sh's cadence comment is explicit that a forgotten hold cannot rot
...
test_housekeeping_captain_held_resurfaces_and_resets() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:dj-held-w11h"; pane="$dir/pane.txt"
...
test_housekeeping_paused_resumed_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:dj-held-w12"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w12.status"
  printf 'Working...\n' > "$pane"
  dj_write_meta "$state/held-w12.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$state" held-w12)
  "$ROOT/bin/dj-busy-event.sh" apply "$state" held-w12 busy --gen "$gen" \
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:dj-held-w13"; pane="$dir/pane.txt"
...
test_housekeeping_captain_held_resolved_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:dj-held-w13h"; pane="$dir/pane.txt"
...
test_housekeeping_stale_marker_transitions_to_pause() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:dj-held-w14"; pane="$dir/pane.txt"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$win" DJ_FAKE_TMUX_CAPTURE="$pane" \
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_captain_held_stale_marker_transitions_to_pause() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:dj-held-w14h"; pane="$dir/pane.txt"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:dj-held-w15"; pane="$dir/pane.txt"
  printf 'working: upstream landed, resuming\n' > "$state/held-w15.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w15" | tr ':/.' '___')
  date +%s > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$win" DJ_FAKE_TMUX_CAPTURE="$pane" \
    DJ_STATE_OVERRIDE="$state" DJ_PAUSE_RESURFACE_SECS=999999 housekeeping "$state"
...
test_housekeeping_resumed_stale_cleared() {
  ...
  # record (bin/dj-busy-lib.sh), not through the pane's rendered footer.
  dj_write_meta "$state/res-w6.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$state" res-w6)
  "$ROOT/bin/dj-busy-event.sh" apply "$state" res-w6 busy --gen "$gen" \
    --source pi-ext --event agent-start
...
test_housekeeping_herdr_persistent_stale_resolves_meta() {
  ...
  state="$dir/state"
  dj_write_meta "$state/herdr-w7.meta" "window=default:w1:p2" "backend=herdr"
  ...
  (
    ...
    dj_backend_busy_state() {
      [ "$1" = herdr ] || fail "expected herdr busy backend, got $1"
      [ "$2" = "default:w1:p2" ] || fail "expected herdr busy target, got $2"
      printf 'idle'
    }
    dj_backend_capture herdr default:w1:p2 40 >/dev/null
    [ "$(dj_backend_busy_state herdr default:w1:p2)" = idle ] || fail "herdr busy stub did not report idle"
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_idle_busy_record_clears_stale() {
  ...
  state="$dir/state"
  dj_write_meta "$state/herdr-footer.meta" "window=default:w1:p4" "backend=herdr" "harness=claude"
  printf 'working\n' > "$state/herdr-footer.status"
  gen=$("$ROOT/bin/dj-busy-event.sh" arm "$state" herdr-footer)
  "$ROOT/bin/dj-busy-event.sh" apply "$state" herdr-footer busy --gen "$gen" \
  ...
  (
    ...
    dj_backend_capture herdr default:w1:p4 40 >/dev/null
    [ "$(dj_backend_busy_state herdr default:w1:p4)" = idle ] || fail "herdr busy stub did not report idle"
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_resumed_stale_cleared() {
  ...
  state="$dir/state"
  dj_write_meta "$state/herdr-busy.meta" "window=default:w1:p3" "backend=herdr"
  ...
  (
    ...
    dj_backend_capture herdr default:w1:p3 40 >/dev/null
    [ "$(dj_backend_busy_state herdr default:w1:p3)" = busy ] || fail "herdr busy stub did not report busy"
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_orca_persistent_stale_resolves_terminal() {
  ...
  state="$dir/state"
  dj_write_meta "$state/orca-w8.meta" "window=dj-orca-w8" "terminal=term-orca-w8" "backend=orca"
  ...
  (
    ...
    dj_backend_capture orca term-orca-w8 40 >/dev/null
    [ "$(dj_backend_busy_state orca term-orca-w8)" = idle ] || fail "Orca busy stub did not report idle"
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_PANE_ALIVE=1 DJ_FAKE_TMUX_SENT="$sent" \
    DJ_FAKE_TMUX_CAPTURE="$capture" DJ_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
    || fail "escalate_flush failed"
  grep -F 'JARVIS_OP: v1 away-supervisor: ' "$sent" >/dev/null \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_PANE_ALIVE=1 DJ_FAKE_TMUX_SENT="$sent" \
    DJ_FAKE_TMUX_CAPTURE="$capture" DJ_ESCALATE_BATCH_SECS=90 DJ_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  DJ_STATE_OVERRIDE="$state" DJ_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:dj-x" || fail "stale: not recognized as wake"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" DJ_FAKE_TMUX_PANE_ALIVE=1 DJ_FAKE_TMUX_SENT="$sent" \
    DJ_FAKE_TMUX_CAPTURE="$capture" DJ_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_marker_detection() {
  local marker_hex
  marker_hex=$(printf '%s' "$DJ_INJECT_MARK" | od -An -tx1 | tr -d ' \n')
  [ "$marker_hex" = e281a3 ] \
    || fail "DJ_INJECT_MARK must use terminal-safe U+2063 bytes, got $marker_hex"
  [ "$DJ_OPERATIONAL_PREFIX" = "${DJ_INJECT_MARK}JARVIS_OP: " ] \
  ...
  message_is_injection "${DJ_OPERATIONAL_PREFIX}Supervisor escalate: done" \
    || fail "operationally-prefixed message not detected as injection"
  message_is_injection "${DJ_INJECT_MARK}Supervisor escalate: done" \
  ...
  state="$dir/state"
  afk_enter "$state"
  should_exit_afk "$state" "${DJ_INJECT_MARK}escalate" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${DJ_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  ...
  dj_operational_input_encode away-supervisor "Supervisor escalate: done" encoded \
  ...
  stripped=$(strip_injection_marker "${DJ_OPERATIONAL_PREFIX}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
    || fail "landed untyped operational prefix not stripped: '$stripped'"
  stripped=$(strip_injection_marker "${DJ_INJECT_MARK}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$DJ_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_requires_proven_empty_prompt() {
  ...
  for prompt in '❯' '›'; do
    printf 'output\noutput\n%s \n' "$prompt" > "$capture"
    if PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=2 \
...
# The safety fix at the tmux classifier (task dj-composer-shellglyph-safety): a
...
test_tmux_composer_state_bare_shell_is_unknown() {
  ...
  for g in '$' '%' '#' '>'; do
    printf 'output\noutput\n%s \n' "$g" > "$capture"
    out=$(PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=2 \
      dj_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "bare shell prompt '$g' must classify unknown (dead shell, unsafe), got '$out'"
  done
  pass "dj_tmux_composer_state: a bare shell prompt (\$/%/#/>) reads unknown, never empty (dead-shell injection safety)"
}
...
test_tmux_composer_state_bordered_and_agent_rows_are_empty() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf '╭────────────────────────╮\n│ >                      │\n╰────────────────────────╯\n' > "$capture"
  out=$(PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=1 \
    dj_tmux_composer_state "fakepane")
  [ "$out" = empty ] || fail "a bordered '│ > │' composer should read empty, got '$out'"
  printf '%s\n' "❯ " > "$capture"
  out=$(PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=0 \
    dj_tmux_composer_state "fakepane")
  ...
  pass "dj_tmux_composer_state: a bordered composer box and bare agent glyphs (❯/›) still read empty"
}
...
test_tmux_composer_state_requires_matching_box_borders() {
  ...
  pass "dj_tmux_composer_state: only matching edge borders form a composer box"
}
...
test_pane_input_pending_preserves_bright_placeholder_like_draft() {
  ...
  capture="$dir/pane.txt"
  printf '╭────────────────╮\n│ custom idle>   │\n╰────────────────╯\n' > "$capture"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=1 \
    DJ_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-dup-s10" "$state")
...
test_afk_nonterminal_working_merged_keeps_wedge_aging() {
  ...
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "$win" "$state")
  case "$out" in
    self\|*transient*) ;;
...
test_afk_genuine_done_still_terminal_stale() {
  ...
  state="$dir/state"
  printf 'done: PR https://example.com/pull/76 checks green; stage 1 of 4 ready for jarvis merge\n' \
    > "$state/stage1-w2.status"
  out=$(DJ_STATE_OVERRIDE="$state" classify_stale "sess:dj-stage1-w2" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in '>' '❯' ''; do
    ...
    if PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=1 \
      pane_input_pending "fakepane"; then
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" DJ_FAKE_COMPOSER="$dir/composer" DJ_FAKE_SENT="$sent" \
    dj_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, dj-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" DJ_FAKE_COMPOSER="$dir/composer" DJ_FAKE_SENT="$sent" \
    DJ_FAKE_SWALLOW="$dir/.swallow" DJ_FAKE_PERSIST_SWALLOW=1 \
    dj_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_COMPOSER="$dir/composer" DJ_FAKE_SENT="$sent" \
    DJ_FAKE_SWALLOW="$dir/.swallow" DJ_FAKE_PERSIST_SWALLOW=1 DJ_INJECT_CONFIRM_SLEEP=0.05 \
    DJ_ESCALATE_BATCH_SECS=99999 DJ_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_COMPOSER="$dir/composer" DJ_FAKE_SENT="$sent" \
    DJ_ESCALATE_BATCH_SECS=99999 DJ_MAX_DEFER_SECS=60 DJ_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_COMPOSER="$dir/composer" DJ_FAKE_SENT="$sent" \
    DJ_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_PANE_ALIVE=1 DJ_FAKE_TMUX_SENT="$sent" \
    DJ_FAKE_TMUX_CAPTURE="$capture" DJ_FAKE_TMUX_CURSOR_Y=0 \
    DJ_ESCALATE_BATCH_SECS=99999 DJ_MAX_DEFER_SECS=300 housekeeping "$state"
...
# the DJ_WEDGE_ALARM_EXEC seam, which tests/wake-helpers.sh forces to a recorder
# ($DJ_WEDGE_ALARM_LOG logs "<channel>\t<summary>"); the daemon also defaults
...
#!/usr/bin/env bash
printf '%s\n' "${DJ_FAKE_UNAME:-Darwin}"
SH
  # Fakes keep command discovery deterministic on any CI host.
  cat > "$fakebin/osascript" <<'SH'
#!/usr/bin/env bash
printf '%s\n' osascript >> "${DJ_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
printf '%s\n' herdr >> "${DJ_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
...
test_wedge_alarm_library_mode_defaults_to_discard() {
  ...
  # DJ_WEDGE_ALARM_EXEC to "discard", so a sourced context (every test) cannot
  # fire a real notification even if it forgets to stub. Checked in a clean
  # subshell that first unsets this harness's recorder.
  local out
  # shellcheck disable=SC2016  # $1/$DJ_WEDGE_ALARM_EXEC must expand in the child, not here
  out=$(env -u DJ_WEDGE_ALARM_EXEC bash -c '. "$1"; printf "%s" "${DJ_WEDGE_ALARM_EXEC:-UNSET}"' _ "$DAEMON")
  [ "$out" = discard ] \
    || fail "sourcing the daemon did not default the notifier seam to discard (got: $out)"
  pass "library mode: sourcing the daemon defaults DJ_WEDGE_ALARM_EXEC to discard (no test can fire a real notification)"
}
...
test_wake_helpers_replace_inherited_notifier_override() {
  ...
  cat > "$unsafe" <<'SH'
#!/usr/bin/env bash
printf '%s\n' invoked >> "${DJ_WEDGE_ALARM_UNSAFE_LOG:?}"
SH
  chmod +x "$unsafe"
  DJ_WEDGE_ALARM_EXEC="$unsafe" DJ_WEDGE_ALARM_UNSAFE_LOG="$unsafe_log" \
    DJ_WEDGE_ALARM_LOG="$alert_log" DJ_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_discard_seam_fires_nothing() {
  ...
  PATH="$dir/fakebin:$PATH" DJ_WEDGE_ALARM_LOG="$log" DJ_WEDGE_ALARM_EXEC=discard \
    DJ_WEDGE_ALARM_CHANNEL=$'osascript\nherdr\n'"$channel" \
...
test_wedge_alarm_direct_notifiers_honor_discard_seam() {
  ...
  PATH="$dir/fakebin:$PATH" DJ_WEDGE_ALARM_REAL_LOG="$real_log" DJ_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_osascript "away-mode WEDGED 900s"
  PATH="$dir/fakebin:$PATH" DJ_WEDGE_ALARM_REAL_LOG="$real_log" DJ_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_herdr "away-mode WEDGED 900s"
  DJ_WEDGE_ALARM_EXEC=discard wedge_alarm_via_command "$command" "away-mode WEDGED 900s"
...
test_wedge_alarm_osascript_channel_selected() {
  ...
  DJ_WEDGE_ALARM_LOG="$log" DJ_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_herdr_channel_selected() {
  ...
  DJ_WEDGE_ALARM_LOG="$log" DJ_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_command_channel_receives_summary() {
  ...
  DJ_WEDGE_ALARM_EXEC='' DJ_WEDGE_ALARM_CHANNEL="$chan" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_command_failure_hides_configured_command() {
  ...
  LOG="$daemon_log" DJ_WEDGE_ALARM_EXEC='' DJ_WEDGE_ALARM_CHANNEL="command:exit 73 # $secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_unknown_channel_hides_configured_directive() {
  ...
  secret="https://alerts.example.invalid/hook?token=private-wedge-token"
  LOG="$daemon_log" DJ_WEDGE_ALARM_CHANNEL="webhook:$secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  for directives in $'osascript\noff' $'off\nosascript'; do
    : > "$log"
    DJ_WEDGE_ALARM_LOG="$log" DJ_WEDGE_ALARM_CHANNEL="$directives" \
      wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_darwin_selects_osascript() {
  ...
  PATH="$dir/fakebin:$PATH" DJ_WEDGE_ALARM_LOG="$log" DJ_FAKE_UNAME=Darwin DJ_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_non_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" DJ_WEDGE_ALARM_LOG="$log" DJ_FAKE_UNAME=Linux DJ_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_config_file_multi_channel() {
  ...
  DJ_WEDGE_ALARM_LOG="$log" DJ_CONFIG_OVERRIDE="$cfgdir" \
...
test_wedge_alarm_failing_channel_degrades_gracefully() {
  ...
  DJ_WEDGE_ALARM_LOG="$log" DJ_WEDGE_ALARM_FAIL=osascript \
    DJ_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_hung_channel_times_out_and_falls_through() {
  ...
  start=$SECONDS
  LOG="$daemon_log" DJ_WEDGE_ALARM_EXEC='' DJ_WEDGE_ALARM_TIMEOUT_SECS=1 \
    DJ_WEDGE_ALARM_CHANNEL=$'command:sleep 30\n'"$channel" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
SH
  chmod +x "$blocker"
  start=$SECONDS
  LOG="$daemon_log" DJ_WEDGE_ALARM_EXEC="$blocker" DJ_WEDGE_ALARM_TIMEOUT_SECS=1 \
    DJ_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  elapsed=$((SECONDS - start))
...
test_inject_wedge_alarm_fires_active_alert_on_non_tmux_backend() {
  ...
  DJ_WEDGE_ALARM_LOG="$log" DJ_STATE_OVERRIDE="$state" \
    DJ_WEDGE_ALARM_CHANNEL=osascript DJ_SUPERVISOR_BACKEND=herdr \
...
test_inject_wedge_alarm_throttles_when_marker_cannot_be_written() {
  ...
  chmod u-w "$state"
  WEDGE_ALARM_LAST_EPOCH=0
  LOG="$daemon_log" DJ_WEDGE_ALARM_LOG="$log" DJ_MAX_DEFER_SECS=600 \
    DJ_WEDGE_ALARM_CHANNEL=osascript DJ_SUPERVISOR_BACKEND=herdr \
    inject_wedge_alarm "$state" 30600
  LOG="$daemon_log" DJ_WEDGE_ALARM_LOG="$log" DJ_MAX_DEFER_SECS=600 \
    DJ_WEDGE_ALARM_CHANNEL=osascript DJ_SUPERVISOR_BACKEND=herdr \
...
test_fm_send_reports_delivered_unconfirmed_submit() {
  ...
  # remains pending, dj-send must return its documented delivered-unconfirmed status and prevent
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_STATE_OVERRIDE="$dir/state" DJ_FAKE_COMPOSER="$dir/composer" \
    DJ_SEND_SLEEP=0.05 "$ROOT/bin/dj-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "dj-send exited non-zero on a clean submit: $(cat "$err")"
  ...
  printf '╭─────╮\n│ >   │\n╰─────╯\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_STATE_OVERRIDE="$dir/state" DJ_FAKE_COMPOSER="$dir/composer" \
    DJ_FAKE_SWALLOW="$dir/.swallow" DJ_FAKE_PERSIST_SWALLOW=1 DJ_SEND_SLEEP=0.05 \
    "$ROOT/bin/dj-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    rc=0
  else
    rc=$?
  fi
  [ "$rc" -eq 3 ] || fail "dj-send returned $rc instead of delivered-unconfirmed exit 3: $(cat "$err")"
  grep -F 'submission is unconfirmed' "$err" >/dev/null \
    || fail "dj-send did not explain the pending confirmation: $(cat "$err")"
  grep -F 'do not retype or blindly resend' "$err" >/dev/null \
    || fail "dj-send did not prevent a duplicate resend: $(cat "$err")"
  if grep -F 'error:' "$err" >/dev/null; then
    fail "dj-send mislabeled delivered-unconfirmed as an error: $(cat "$err")"
  fi
  pass "dj-send returns 3 with a non-error no-resend warning when confirmation stays pending"
}
...
test_fm_send_exits_nonzero_on_initial_send_failure() {
  ...
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_STATE_OVERRIDE="$dir/state" DJ_FAKE_COMPOSER="$dir/composer" \
    DJ_FAKE_SEND_FAIL=1 DJ_SEND_SLEEP=0.05 \
    "$ROOT/bin/dj-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "dj-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "dj-send did not explain initial send failure: $(cat "$err")"
  pass "dj-send exits non-zero when initial text send fails"
}
...
test_fm_send_exits_nonzero_on_unproven_submit() {
  ...
  if PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_STATE_OVERRIDE="$dir/state" DJ_FAKE_COMPOSER="$dir/composer" \
    DJ_FAKE_SWALLOW="$dir/.swallow" DJ_FAKE_PERSIST_SWALLOW=1 DJ_SEND_SLEEP=0.05 \
    "$ROOT/bin/dj-send.sh" sess:win '修复' >/dev/null 2>"$err"; then
    fail "dj-send exited zero when submit proof remained pending-unproven"
  fi
  grep -F 'verdict=pending-unproven' "$err" >/dev/null \
    || fail "dj-send did not preserve the unproven-submit verdict: $(cat "$err")"
  pass "dj-send exits non-zero unless delivery is proven empty"
}

# --- herdr backend-awareness (dj-turnend-guard-h6-adjacent transport fix) ----
...
test_discover_supervisor_backend_precedence() {
  local out
  out=$(DJ_SUPERVISOR_BACKEND=herdr TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "explicit DJ_SUPERVISOR_BACKEND override was not honored: $out"

  out=$(DJ_SUPERVISOR_BACKEND='' TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = tmux ] || fail "TMUX_PANE should win over HERDR_ENV (tmux nested in herdr resolves to tmux): $out"

  out=$(DJ_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "HERDR_ENV=1 with HERDR_PANE_ID present should resolve to herdr: $out"

  if out=$(DJ_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_backend); then
...
test_discover_supervisor_target_herdr() {
  local out
  out=$(DJ_SUPERVISOR_TARGET=explicit:target TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = "explicit:target" ] || fail "explicit DJ_SUPERVISOR_TARGET override was not honored: $out"

  out=$(DJ_SUPERVISOR_TARGET='' TMUX_PANE='%3' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = '%3' ] || fail "TMUX_PANE should win over herdr markers: $out"

  out=$(DJ_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION='' discover_supervisor_target)
  [ "$out" = "default:w1:p9" ] || fail "herdr target should default HERDR_SESSION to 'default': $out"

  out=$(DJ_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION=iso1 discover_supervisor_target)
  [ "$out" = "iso1:w1:p9" ] || fail "herdr target should use an explicit HERDR_SESSION: $out"

  if out=$(DJ_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_target); then
    fail "bare fallback should return non-zero"
  fi
  [ "$out" = "jarvis:0" ] || fail "bare fallback should still print jarvis:0: $out"

  pass "discover_supervisor_target: override > TMUX_PANE > herdr '<session>:<pane-id>' composition > jarvis:0 fallback"
}
...
test_pane_is_busy_herdr_native_busy_state() {
  ...
  (
    dj_backend_busy_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected busy_state args: $1 $2"; printf 'busy'; }
    dj_backend_capture() { fail "capture should not be consulted when busy_state is conclusive"; }
    DJ_STATE_OVERRIDE="$dir/state" DJ_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr \
...
test_primary_busy_guard_is_harness_scoped() {
  (
    dj_backend_busy_state() { printf 'unknown'; }
    dj_backend_capture() { printf 'esc interrupt\n'; }
    if DJ_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr; then
      fail "OpenCode's rendered signature must not classify a Claude primary busy"
    fi
    DJ_DAEMON_PRIMARY_HARNESS=opencode pane_is_busy "default:w1:p2" herdr \
...
test_pane_is_busy_defaults_to_tmux_when_backend_omitted() {
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_CAPTURE="$capture" DJ_STATE_OVERRIDE="$dir/state" DJ_DAEMON_PRIMARY_HARNESS=grok pane_is_busy "fakepane" \
...
test_pane_input_pending_herdr_dispatch() {
  (
    dj_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
  ...
  (
    dj_backend_composer_state() { printf 'empty'; }
  ...
  (
    dj_backend_composer_state() { printf 'future-state'; }
  ...
  pass "pane_input_pending: dispatches through dj_backend_composer_state for backend=herdr"
}
...
test_inject_msg_herdr_busy_guard_defers() {
  ...
  afk_enter "$state"
  (
    dj_backend_target_exists() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected target_exists args: $1 $2"; return 0; }
    pane_is_busy() { return 0; }
    dj_backend_composer_state() { fail "composer_state should not be consulted once the busy-guard already deferred"; }
    dj_backend_send_text_submit() { fail "send_text_submit should not run when the busy-guard defers"; }
    if DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_composer_guard_defers() {
  ...
  afk_enter "$state"
  (
    dj_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    dj_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
    dj_backend_send_text_submit() { fail "send_text_submit should not run when the composer-guard defers"; }
    if DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_pane_gone_defers() {
  ...
  afk_enter "$state"
  (
    dj_backend_target_exists() { return 1; }
    pane_is_busy() { fail "busy guard should not be consulted once the pane-exists check already failed"; }
    dj_backend_send_text_submit() { fail "send_text_submit should not run when the pane does not exist"; }
    if DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:gone" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state" \
...
# Safety-critical (task dj-composer-shellglyph-safety): the away-mode injector
...
test_inject_msg_defers_on_dead_shell_unknown() {
  ...
  (
    dj_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    dj_backend_composer_state() { printf 'unknown'; }
    dj_backend_send_text_submit() { fail "send_text_submit must NOT run when the composer is a dead shell (unknown)"; }
    if DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_defers_on_unrecognized_composer_state() {
  ...
  (
    ...
    pane_is_busy() { return 1; }
    dj_backend_composer_state() { printf 'future-state'; }
    dj_backend_send_text_submit() { fail "send_text_submit must not run for an unrecognized composer state"; }
    if DJ_SUPERVISOR_BACKEND=herdr DJ_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
