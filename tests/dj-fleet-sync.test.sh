#!/usr/bin/env bash
# Behavior tests for dj-fleet-sync.sh drift handling.
#
# dj-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
# jarvis home, the jarvis checkout itself - so it must be skipped by name
...
# (fetch_with_packed_refs_lock_guard, backed by bin/dj-lock-lib.sh's shared
...
TMP_ROOT=$(dj_test_tmproot dj-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated DJ_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/dj-fleet-sync.sh" "$@" 2>/dev/null
}

# build_enclosing_home <name>: an DJ_HOME that is itself nested inside another git
# repository - jarvis's own layout, where projects/ sits inside the jarvis
...
# lsof shims mirror tests/dj-teardown.test.sh: no-holder (provably free), a live
...
# (DJ_FLEET_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
run_sync_guarded() {
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/dj-fleet-sync.sh" "$@" >"$outf" 2>"$errf"
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  out=$(DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  DJ_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  DJ_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  DJ_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_transient_packed_refs_lock_self_clears() {
  ...
  DJ_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  DJ_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
