...
PROOF="$ROOT/bin/dj-test-isolation-proof.sh"
RUNNER="$ROOT/bin/dj-test-run.sh"

assert_present "$PROOF" "bin/dj-test-isolation-proof.sh is missing"
[ -x "$PROOF" ] || fail "bin/dj-test-isolation-proof.sh must be executable"
...
test_candidates_exclude_serial_classes() {
  ...
  listed=$("$PROOF" --list)
  for banned in \
    tests/dj-test-isolation-proof.test.sh \
    ...
    tests/dj-afk-inject-e2e.test.sh \
    tests/dj-pi-primary-live-e2e.test.sh \
    tests/dj-pr-check-security.test.sh \
    tests/dj-backend-cmux-smoke.test.sh; do
...
test_list_exclusions_documents_reasons() {
  ...
  printf '%s\n' "$out" | grep -Fq 'dj-watcher-lock.test.sh' \
    || fail "exclusions must document watcher-lock serial reason"
  printf '%s\n' "$out" | grep -Fq 'dj-backend-herdr-smoke.test.sh' \
...
test_family_map_labels_this_contract() {
  ...
  printf '%s\n' "$fam" | grep -Fq 'tests/dj-test-isolation-proof.test.sh' \
    || fail "dj-test-isolation-proof.test.sh must map to pure-contract-unit"
