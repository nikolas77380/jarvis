...
# runner (bin/dj-procevent-when.sh).
#
...
TMP_ROOT=$(dj_test_tmproot dj-procevent-when-tests)
export DJ_PROCEVENT_CLAIM_ROOT="$TMP_ROOT/claims"

pe()   { DJ_HOME="$1" "$ROOT/bin/dj-procevent.sh" "${@:2}"; }
when() { DJ_HOME="$1" "$ROOT/bin/dj-procevent-when.sh" "${@:2}"; }
...
mode=$(PATH="${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/dj-pr-lib.sh"; dj_pr_file_mode "$2"' _ "$ROOT" "$H/state/when/when-arm-test.spec")
...
pass "a repeatedly erroring condition wakes jarvis instead of firing"
...
SH
chmod +x "$NOISY"
DJ_WHEN_OUTPUT_TAIL_BYTES=128 when "$H" arm bounded-output --stable 1 \
  --condition true --action "$NOISY" "$NOISY_READY" >/dev/null
DJ_WHEN_OUTPUT_TAIL_BYTES=128 pe "$H" reconcile >/dev/null
...
printf 'all dj-procevent-when tests passed\n'
