...
# bin/dj-harness.sh checks verified ENV markers before ancestry. Muse is
...
HARNESS="$ROOT/bin/dj-harness.sh"
TMP_ROOT=$(dj_test_tmproot dj-muse-harness)
...
case "${1:-}" in
  show-environment)
    [ "${DJ_FAKE_WORKER_META_KEY:-}" = present ] || exit 1
  ...
  send-keys)
    ...
    for arg in "$@"; do
      if [ "$prev" = -l ]; then
        printf '%s\n' "$arg" >> "$DJ_FAKE_LAUNCH_LOG"
        if [ "${DJ_FAKE_EXECUTE_MUSE_LAUNCH:-}" = 1 ]; then
          case "$arg" in
            *"$DJ_FAKE_MUSE_EXECUTABLE"*) (cd "$DJ_FAKE_PANE_PATH" && bash -c "$arg") ;;
          esac
        fi
        break
      fi
...
#!/usr/bin/env bash
set -u
[ -n "${DJ_FAKE_HARNESS_RESULT:-}" ] || exit 0
exec "$DJ_FAKE_MUSE_VERSIONED" -c 'result=$($DJ_FAKE_HARNESS_PROBE); printf "%s" "$result" > "$DJ_FAKE_HARNESS_RESULT"'
SH
...
run_muse_spawn() {  # <home> <proj> <wt> <fakebin> <id> [extra args...]
  ...
  DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    ...
    DJ_FAKE_LAUNCH_LOG="$home/launch.log" \
    DJ_FAKE_MUSE_EXECUTABLE="$fakebin/muse" \
    DJ_FAKE_MUSE_VERSIONED="$fakebin/muse-bin-test-version" \
    DJ_FAKE_HARNESS_PROBE="$HARNESS" \
    DJ_FAKE_EXECUTE_MUSE_LAUNCH="${DJ_FAKE_EXECUTE_MUSE_LAUNCH:-}" \
    DJ_FAKE_HARNESS_RESULT="${DJ_FAKE_HARNESS_RESULT:-}" \
    DJ_FAKE_WORKER_META_KEY="${DJ_TEST_MUSE_WORKER_KEY-present}" \
    META_API_KEY="${DJ_TEST_MUSE_KEY-test-key}" \
    XDG_CONFIG_HOME="${DJ_TEST_MUSE_CONFIG_HOME-$home/xdgconfig}" \
    XDG_DATA_HOME="${DJ_TEST_MUSE_DATA_HOME-$home/xdgdata}" \
    PATH="$fakebin:$PATH" \
...
# dj-harness.sh from a child of it.
#
...
test_detects_versioned_process_ancestor() {
  ...
  for bin in muse-bin-0.1.0-R708.1 muse-bin-9.9.9-RZZZ.9 muse; do
    ...
    [ "$out" = muse ] || fail "dj-harness.sh under process '$bin' reported '$out', expected muse"
  done
...
test_detection_is_anchored() {
  ...
  for bin in musescore amuse notmuse-bin muse-binary muse-bind; do
    ...
    out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u DJ_PI_HARNESS -u GROK_AGENT \
      -u CURSOR_AGENT -u CURSOR_INVOKED_AS \
      "$dir/$bin" -c "r=\$(\"$HARNESS\"); printf '%s' \"\$r\"")
    [ "$out" != muse ] || fail "dj-harness.sh misdetected unrelated process '$bin' as muse"
  done
...
EOF
  ...
  out=$(CLAUDECODE=1 PI_CODING_AGENT=true GROK_AGENT=1 DJ_PI_HARNESS=pi-signed \
    CURSOR_AGENT=1 CURSOR_INVOKED_AS=cursor-agent \
    DJ_FAKE_EXECUTE_MUSE_LAUNCH=1 DJ_FAKE_HARNESS_RESULT="$result" \
...
EOF
  ...
  # The captain accepted muse's self-update risk, so jarvis must not pin it.
...
EOF
  ...
  out=$(DJ_TEST_MUSE_KEY='' DJ_TEST_MUSE_WORKER_KEY='' run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
...
$rec
EOF
  out=$(DJ_TEST_MUSE_KEY='caller-only-secret' DJ_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off)
  status=$?
  [ "$status" -ne 0 ] || fail "muse spawn accepted a caller-only META_API_KEY"
  assert_contains "$out" "set for dj-spawn but cannot be proven present" \
...
EOF
  ...
  DJ_TEST_MUSE_KEY='' DJ_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
...
EOF
  ...
  out=$(cd "$caller" && DJ_TEST_MUSE_KEY='' DJ_TEST_MUSE_WORKER_KEY='' \
    DJ_TEST_MUSE_CONFIG_HOME=cfg DJ_TEST_MUSE_DATA_HOME=data \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off)
  status=$?
...
# dialect rejects the model-reawakening handlers a jarvis primary needs, so a
...
test_spawn_refuses_secondmate() {
  ...
  printf 'charter\n' > "$home/data/$id/brief.md"
  out=$(cd "$case_dir" && DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_PROJECTS_OVERRIDE="$home/projects" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_SPAWN_NO_GUARD=1 TMUX="fake,1,0" META_API_KEY=test-key \
    PATH="$fakebin:$PATH" \
...
EOF
  ...
  prior=$(printf '%s\n' "$prior" | sed 's://*:/:g')
  DJ_TEST_MUSE_DATA_HOME="$case_dir/xdgdata" \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off >/dev/null \
...
make_send_case() {  # <name> <harness>
  ...
  home="$case_dir/home"
  fakebin=$(dj_fakebin "$case_dir/fake")
  id="send-$name"
  mkdir -p "$home/state"
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
...
case "${1:-}" in
  ...
  has-session) exit 0 ;;
  list-panes|list-windows) printf 'dj-send:0\n'; exit 0 ;;
  send-keys)
    shift
    printf '%s\n' "$*" >> "$DJ_FAKE_KEY_LOG"
    [ "${DJ_FAKE_KEY_FAIL:-}" = "$*" ] && exit 1
    exit 0
    ;;
esac
exit 0
SH
  ...
  dj_write_meta "$home/state/$id.meta" \
    "window=dj-send:0" "endpoint_task_id=$id" "worktree=$case_dir" \
...
run_send_key() {  # <home> <fakebin> <id> <key> <keylog>
  DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$1" DJ_STATE_OVERRIDE="$1/state" \
    DJ_FAKE_KEY_LOG="$5" PATH="$2:$PATH" \
    "$ROOT/bin/dj-send.sh" "$3" --key "$4" 2>&1
}
...
EOF
  keylog="$case_dir/keys.log"
  : > "$keylog"
  out=$(DJ_FAKE_KEY_FAIL='-t dj-send:0 C-u' run_send_key "$home" "$fakebin" "$id" Escape "$keylog")
  status=$?
...
classify_muse() {  # <state-dir> <id>
  (
    ...
    dj_busy_classify tmux fake:0 muse "$2" "$1"
  )
}
...
test_muse_trusts_no_record_sources() {
  ...
  out=$(
    # shellcheck source=bin/dj-busy-lib.sh
    . "$ROOT/bin/dj-busy-lib.sh"
    dj_busy_sources_for_harness muse
  )
