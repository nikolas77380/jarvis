#!/usr/bin/env bash
# tests/dj-classify-corr-token.test.sh - a status line may carry the correlation
# token bin/dj-pending-reply-lib.sh embeds in a marked request and a secondmate
# echoes back (bin/dj-brief.sh), between the verb and the rest of the line. Every
...
# hard as they defend the fix: only the exact token a jarvis library writes is
...
# through the REAL bin/dj-wake-drain.sh, the classifier predicates through the
# library's own sourced entry points (as tests/dj-watch-triage.test.sh does), and
...
TMP_ROOT=$(dj_test_tmproot dj-classify-corr-token-tests)
...
drain_open() {  # <state> <out>
  local state=$1 out=$2
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed over $state"
...
test_token_is_read_through_in_every_position_it_is_written_in() {
  ...
  # bin/dj-secondmate-report.sh helper writes. Every one is a real observed shape.
...
test_daemon_and_crew_state_case_arms_read_through_the_token() {
  ...
  # cannot be proven through status_is_*: bin/dj-supervise-daemon.sh matches
  # working|resolved|captain-held to take the transient-stale path instead of
  # the terminal one, and bin/dj-crew-state.sh's map_log_state maps each verb to
...
test_pending_reply_escalation_matching_is_unaffected() {
  # bin/dj-pending-reply-lib.sh filters a status file by verb when looking for
...
test_incremental_and_whole_file_folds_agree_over_correlated_lines() {
  ...
  while [ "$round" -lt 6 ]; do
    ...
    whole=$(DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; status_open_decisions "$2"' _ \
      "$ROOT/bin/dj-classify-lib.sh" "$status")
    inc=$(DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; status_open_decisions_incremental "$2"' _ \
      "$ROOT/bin/dj-classify-lib.sh" "$status")
    [ "$whole" = "$inc" ] \
...
test_the_real_writers_produce_tokens_this_library_reads() {
  ...
  local i=0 corr
  while [ "$i" -lt 5 ]; do
    corr=$(bash -c '. "$1"; dj_pending_reply_new_id' _ "$ROOT/bin/dj-pending-reply-lib.sh")
    [ -n "$corr" ] || fail "the correlation library produced an empty id"
    token=$(bash -c '. "$1"; dj_pending_reply_corr_token "$2"' _ \
      "$ROOT/bin/dj-pending-reply-lib.sh" "$corr")
