#!/usr/bin/env bash
# tests/dj-control-herdr-smoke.test.sh - real-herdr smoke test for the agent
# lifecycle control plane (bin/dj-control.sh).
#
# tmux is the control plane's reference backend and is covered hermetically in
# tests/dj-control.test.sh. herdr is the OTHER backend whose recovery-grade
...
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/dj-control-herdr.XXXXXX")
...
CONTAINER_RAW=$(dj_backend_herdr_container_ensure "$WT") || fail "container_ensure failed"
CONTAINER=${CONTAINER_RAW%%$'\t'*}
SEEDED_TAB_ID=${CONTAINER_RAW#*$'\t'}
WORKSPACE_ID=${CONTAINER#*:}
TASK_IDS=$(dj_backend_herdr_create_task "$CONTAINER" "dj-hsmoke" "$WT" "$SEEDED_TAB_ID") \
...
run_control() {
  env DJ_HOME="$HOME_DIR" HERDR_SESSION="$SESSION" \
    DJ_CONTROL_POLL=0.2 DJ_CONTROL_EXIT_WAIT=2 \
    "$ROOT/bin/dj-control.sh" "$@" 2>&1
}
...
herdr pane report-agent "$PANE_ID" --source dj-control-smoke --agent dj-control-smoke-agent \
  --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the task pane"

STATE=$(dj_backend_agent_state herdr "$SESSION:$PANE_ID")
...
dj_backend_herdr_kill "$SESSION:$PANE_ID" 2>/dev/null || true
