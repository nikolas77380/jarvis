...
#
# The run-wrapper cases drive the REAL bin/dj-session-start.sh against a
...
if [ "${DJ_SESSIONSTART_TEST_HARNESS:-0}" != 1 ]; then
  HARNESS_FIXTURE=$(mktemp -d "${TMPDIR:-/tmp}/dj-sessionstart-harness.XXXXXX") || exit 1
  ln -s /bin/bash "$HARNESS_FIXTURE/codex" || exit 1
  # shellcheck disable=SC2016 # Expand in the fixture shell, not this parent.
  DJ_SESSIONSTART_TEST_HARNESS=1 "$HARNESS_FIXTURE/codex" \
    -c '"$@"; rc=$?; :; exit "$rc"' _ "$0" "$@"
  HARNESS_STATUS=$?
...
TMP_ROOT=$(dj_test_tmproot dj-sessionstart-nudge)
NUDGE="$ROOT/bin/dj-sessionstart-nudge.sh"
RUN="$ROOT/bin/dj-sessionstart-run.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/dj-operational-input.sh"
NUDGE_TEXT="Run \`bin/dj-session-start.sh\` now, exactly once, before executing any other instructions."
dj_operational_input_encode session-start "$NUDGE_TEXT" NUDGE_LINE \
...
run_nudge() {
  local root=$1
  DJ_GATE_REFUSE_BYPASS=0 DJ_ROOT_OVERRIDE="$root" DJ_HOME="$root" "$NUDGE"
}
...
test_genuine_primary_nudges() {
  ...
  pass "dj-sessionstart-nudge: a genuine primary gets one explicitly marked instruction line"
}

test_gate_env_is_silent() {
  local root="$TMP_ROOT/gate-env"
  make_primary "$root"
  expect_silent_zero "gate env nudge" env NO_MISTAKES_GATE=1 DJ_GATE_REFUSE_BYPASS=0 \
    DJ_ROOT_OVERRIDE="$root" DJ_HOME="$root" "$NUDGE"
  pass "dj-sessionstart-nudge: NO_MISTAKES_GATE is silent"
}
...
test_linked_secondmate_primary_nudges() {
  ...
  pass "dj-sessionstart-nudge: a marked linked secondmate home is a primary"
}
...
test_missing_state_is_silent() {
  ...
  pass "dj-sessionstart-nudge: a checkout without state is silent"
}
...
test_owned_lock_is_silent() {
  ...
  pass "dj-sessionstart-nudge: a lock holder in process ancestry is already run"
}

test_opencode_plugin_delivers_exact_nudge_once() {
  local root="$TMP_ROOT/opencode-primary" out status=0
  make_primary "$root"
  cp "$ROOT/bin/dj-sessionstart-nudge.sh" "$ROOT/bin/dj-primary-scope-lib.sh" \
    "$ROOT/bin/dj-gate-refuse-lib.sh" "$ROOT/bin/dj-operational-input.sh" "$root/bin/"
  chmod +x "$root/bin/dj-sessionstart-nudge.sh"
  out=$(PLUGIN="$ROOT/.opencode/plugins/dj-primary-sessionstart-nudge.js" \
...
# dj-session-start.sh can execute: a git repo on main so the tangle check
...
RUN_PATH=${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
...
run_hook() {  # <root> [args...]
  ...
  env -u CLAUDECODE -u PI_CODING_AGENT -u DJ_PI_HARNESS -u GROK_AGENT \
    DJ_GATE_REFUSE_BYPASS=0 DJ_ROOT_OVERRIDE="$root" DJ_HOME="$root" PATH="$RUN_PATH" "$RUN" "$@"
}
...
# Every run-tier assertion keys off the digest banner, which dj-session-start.sh
...
test_run_startup_runs_the_full_digest() {
  ...
  assert_not_contains "$out" "JARVIS_OP" "a run-tier open also emitted the nudge instruction"
...
test_run_clear_and_compact_reemit() {
  ...
  for source in clear compact; do
    ...
    assert_not_contains "$out" "JARVIS_OP" "a $source open also emitted the nudge instruction"
  done
...
test_pi_startup_classifies_cli_continuations() {
  ...
  cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$fixture/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$fixture/.pi/extensions/lib/"
  cat > "$fixture/bin/dj-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${DJ_HOME:?}/state/sources"
SH
  cat > "$fixture/bin/dj-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fixture/bin/"*.sh

  out=$(EXT="$fixture/.pi/extensions/dj-primary-turnend-guard.ts" \
    DJ_HOME="$fixture" DJ_ROOT_OVERRIDE="$fixture" \
...
test_pi_large_sessionstart_digest_is_delivered_loudly() {
  ...
  cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$fixture/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$fixture/.pi/extensions/lib/"
  cp "$ROOT/bin/dj-sessionstart-run.sh" "$ROOT/bin/dj-sessionstart-nudge.sh" \
    "$ROOT/bin/dj-primary-scope-lib.sh" "$ROOT/bin/dj-gate-refuse-lib.sh" \
    "$ROOT/bin/dj-hook-host-lib.sh" \
    "$ROOT/bin/dj-operational-input.sh" "$fixture/bin/"
  cat > "$fixture/bin/dj-session-start.sh" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  out=$(EXT="$fixture/.pi/extensions/dj-primary-turnend-guard.ts" \
    DJ_HOME="$fixture" DJ_ROOT_OVERRIDE="$fixture" DJ_GATE_REFUSE_BYPASS=1 \
    node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
const handlers = new Map();
const messages = [];
...
if (!content.includes("JARVIS_OP: v1 session-start:")) throw new Error("operational provenance was lost");
JS
  ) || status=$?
...
test_run_reads_source_from_the_hook_payload() {
  ...
  assert_contains "$out" "JARVIS_OP" "a resume hook payload did not delegate to the nudge"
...
test_run_gate_and_scope_are_silent() {
  ...
  make_run_primary "$root"
  expect_silent_zero "gate env run" env NO_MISTAKES_GATE=1 DJ_GATE_REFUSE_BYPASS=0 \
    DJ_ROOT_OVERRIDE="$root" DJ_HOME="$root" PATH="$RUN_PATH" "$RUN" --source startup
  assert_absent "$root/state/.lock" "a gate agent's session open still took the fleet lock"

  dj_git_worktree "$base" "$linked" dj/run-linked
