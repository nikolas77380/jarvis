...
# (.pi/extensions/dj-branch-supervision.ts): wake dispatch acceptance and
# gating, the two-stage noise filter's second stage (verdict-driven delivery
# into main), store-first durability through the real bin/dj-branch-outcome.sh,
...
TMP_ROOT=$(dj_test_tmproot dj-pi-branch-extension)
EXT="$ROOT/.pi/extensions/dj-branch-supervision.ts"
...
install_pi_branch_extension_fixture() {
  ...
  cp "$EXT" "$repo/.pi/extensions/dj-branch-supervision.ts"
...
// live guard (tests/dj-pi-branch-live-e2e.test.sh) is what proves the real
...
const home = process.env.DJ_HOME;
const realRoot = process.env.DJ_ROOT_OVERRIDE;
...
if (!process.env.DJ_TEST_SKIP_LOCK) {
...
function dispatch(message, projects, heartbeat, eligible) {
  ...
  bus.emit("dj-branch-supervision:dispatch", offer);
  return offer;
}
...
function outcomeScript(args) {
  const result = spawnSync("bash", [`${realRoot}/bin/dj-branch-outcome.sh`, ...args], {
    encoding: "utf8",
    env: { ...process.env, DJ_HOME: home, DJ_STATE_OVERRIDE: `${home}/state` },
  });
  if (result.status !== 0) throw new Error(`dj-branch-outcome.sh ${args.join(" ")} failed: ${result.stderr}`);
...
test_branch_dispatch_two_stage_filter_and_prefix_contract() {
  ...
  PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
...
if (!wakePrompt.includes("JARVIS SUPERVISION WAKE: signal: task-9 done")) {
...
if (JSON.stringify(session.options.tools) !== JSON.stringify(["read", "bash", "dj_branch_report"])) {
...
if (hooked.env.DJ_SUPERVISION_ACTOR !== "branch") throw new Error("branch bash does not inject the branch actor");
if (hooked.env.DJ_LEASE_HOLDER_PID !== String(process.ppid)) throw new Error("branch bash does not pin the verified session-lock holder pid");
...
if (!rewriteA.prompt_cache_key.startsWith("dj-branch-")) throw new Error(`unexpected cache key: ${rewriteA.prompt_cache_key}`);
...
const report = session.options.customTools.find((tool) => tool.name === "dj_branch_report");
...
if (sentToMain[0].message.customType !== "dj-branch-merge") throw new Error("merge note has the wrong custom type");
...
// renderer.
const outcomesTool = mainTools.find((tool) => tool.name === "dj_branch_outcomes");
if (!outcomesTool) throw new Error("dj_branch_outcomes was not registered on main");
const renderTheme = {
...
if (calmOffCall.constructor.name !== "Box" || calmOffCall.paddingX !== 1 || calmOffCall.paddingY !== 1) {
  throw new Error("dj_branch_outcomes changed its ordinary shell rendering");
}
if (calmOffResult.constructor.name !== "Container" || calmOffCall.children[0]?.text !== "dj_branch_outcomes" || calmOffCall.children[1]?.text !== "OUTCOME_DUMP") {
  throw new Error("dj_branch_outcomes changed its ordinary call or result rendering");
}
pi.events.emit("jarvis:calm-presentation", { active: true, stockExportRendering: false });
...
if (calmOnCall.constructor.name !== "Container" || calmOnCall.render(100).length !== 0 || calmOnResult.constructor.name !== "Container" || calmOnResult.render(100).length !== 0) {
  throw new Error("dj_branch_outcomes remained visible while Calm was on");
}
pi.events.emit("jarvis:calm-presentation", { active: false, stockExportRendering: false });
if (outcomesTool.renderCall({}, renderTheme, renderContext).constructor.name !== "Box" || outcomesTool.renderResult(stockResult, { expanded: false, isPartial: false }, renderTheme, renderContext).constructor.name !== "Container") {
  throw new Error("dj_branch_outcomes did not restore ordinary rendering when Calm was turned off");
}
pi.events.emit("jarvis:calm-presentation", { active: true, stockExportRendering: true });
...
if (!exportCallFellBack || !exportResultFellBack) {
  throw new Error("dj_branch_outcomes replaced Pi stock export rendering");
}
...
if (listedText.split("\n").length !== 2 || !listedText.includes("checks green")) {
  throw new Error(`dj_branch_outcomes did not read the store: ${listedText}`);
}
if (!renderers.has("dj-branch-merge")) throw new Error("merge-note renderer missing");
const assertRenderedNote = (note, glyph) => {
  const fgCalls = [];
  const rendered = renderers.get("dj-branch-merge")(
    { content: note },
    { expanded: false },
    {
      fg(color, text) {
...
test_branch_cache_key_is_per_home_stable() {
  ...
  install_pi_branch_extension_fixture "$repo"
  probe() {
    PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$1" DJ_ROOT_OVERRIDE="$ROOT" \
...
test_branch_default_on_heartbeat_afk_and_fallback() {
  ...
  install_pi_branch_extension_fixture "$repo"
  cp "$ROOT/bin/dj-lease.sh" "$ROOT/bin/dj-lease-lib.sh" "$ROOT/bin/dj-wake-lib.sh" "$ROOT/bin/dj-wake-grant.sh" "$broken/bin/"
  cat > "$broken/bin/dj-branch-prompt.sh" <<'SH'
#!/usr/bin/env bash
echo "synthetic generator failure" >&2
exit 1
SH
  chmod +x "$broken/bin/dj-branch-prompt.sh"
  PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
const heartbeatReport = heartbeatSession.options.customTools.find((tool) => tool.name === "dj_branch_report");
...
EOF
  ...
  PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$TMP_ROOT/gating-home-2" DJ_ROOT_OVERRIDE="$broken" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
if (!fallback.includes("JARVIS WATCHER WAKE: signal: first wake")) throw new Error(`fallback lost the wake: ${fallback}`);
...
const drain = spawnSync("bash", [`${realRoot}/bin/dj-wake-drain.sh`], {
  encoding: "utf8",
  env: { ...process.env, DJ_HOME: home, DJ_STATE_OVERRIDE: `${home}/state`, DJ_ROOT_OVERRIDE: realRoot },
});
...
if (!String(mainUserMessages[0].content).includes("JARVIS WATCHER WAKE: signal: interrupted main claim")) {
...
const entries = [
  ...
  { type: "message", message: { role: "user", content: "⁣JARVIS_OP: v1 watcher: operational injection" } },
  { type: "message", message: { role: "toolResult", content: "tool output stays in main" } },
  { type: "custom", message: { role: "custom", customType: "dj-branch-merge", content: "merged note" } },
...
if (mirrored.some((m) => m.customType !== "dj-main-mirror")) throw new Error("mirror used the wrong custom type");
...
test_replacement_activation_cleans_leases_and_retries_failure() {
  ...
  cat > "$fakebin/bash" <<'SH'
#!/bin/sh
if [ "$1" = "$DJ_TEST_LEASE_SCRIPT" ] && [ ! -e "$DJ_TEST_FAIL_MARKER" ]; then
  : > "$DJ_TEST_FAIL_MARKER"
  exit 7
fi
exec "$DJ_TEST_REAL_BASH" "$@"
SH
  chmod +x "$fakebin/bash"
  PATH="$fakebin:$PATH" PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_TEST_REAL_BASH="$real_bash" DJ_TEST_LEASE_SCRIPT="$ROOT/bin/dj-lease.sh" \
    DJ_TEST_FAIL_MARKER="$home/state/release-failed-once" DRIVER_PRELUDE="$DRIVER_PRELUDE" \
...
test_cold_start_activates_after_lock_acquisition() {
  ...
  install_pi_branch_extension_fixture "$repo"
  PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_TEST_SKIP_LOCK=1 DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
// the fleet lock (dj-sessionstart-run.sh acquires it later). Ownership must
...
if (!mainUserMessages[0].content.includes("JARVIS WATCHER WAKE: signal: queued wake")) {
...
const oldReport = oldSession.options.customTools.find((tool) => tool.name === "dj_branch_report");
...
try {
  oldBash.__options.spawnHook({
    command: "bin/dj-lease.sh claim task-stale --actor branch",
    cwd: home,
    env: {},
  });
} catch {
...
test_secondary_session_stays_inert() {
  ...
  PLUGIN="$repo/.pi/extensions/dj-branch-supervision.ts" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_TEST_SKIP_LOCK=1 DJ_TEST_LOCK_PID=$foreign_pid DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { dispatch, home }; })()`);
const { dispatch, home } = globalThis.__t;
import { existsSync, writeFileSync } from "node:fs";
writeFileSync(`${home}/state/.lock`, `${process.env.DJ_TEST_LOCK_PID}\n`);
...
replacementBus.emit("dj-branch-supervision:dispatch", offer);
...
# Direct unit coverage of dj-branch-dispatch.ts's classification, independent
...
test_branch_dispatch_classifies_main_only_rows_and_writes_the_eligible_snapshot() {
  ...
  LIB="$repo/.pi/extensions/lib/dj-branch-dispatch.ts" DJ_HOME="$home" GRANT="$ROOT/bin/dj-wake-grant.sh" \
    node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
import { readFileSync, writeFileSync } from "node:fs";
...
const state = `${process.env.DJ_HOME}/state`;
const project = `${process.env.DJ_HOME}/projects/approved`;
...
writeFileSync(
  ...
  [
    "1\t1\tcheck\tx-inbox\tcheck: pending x mention",
    "1\t2\tsignal\ttask-a.status\tsignal: task-a.status",
    "1\t3\tstale\tfm-window\tstale: dj-window",
  ].join("\n"),
);
...
writeFileSync(
  ...
  [
    "1\t1\theartbeat\theartbeat\theartbeat",
    "1\t2\tinvented\tsomething\tinvented: not a kind dj_wake_append emits",
  ].join("\n"),
);
...
test_outcomes_tool_uses_stock_execution_and_export_consumers() {
  ...
  package_dir=${DJ_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
  ...
  cp "$EXT" "$fixture/.pi/extensions/dj-branch-supervision.ts"
  cp "$ROOT/.pi/extensions/lib/dj-branch-dispatch.ts" "$fixture/.pi/extensions/lib/dj-branch-dispatch.ts"
  cp "$ROOT/.pi/extensions/lib/dj-calm-visibility.ts" "$fixture/.pi/extensions/lib/dj-calm-visibility.ts"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$fixture/.pi/extensions/lib/dj-operational-input.ts"
  ...
  out=$(cd "$fixture" && EXT="$fixture/.pi/extensions/dj-branch-supervision.ts" PI_PACKAGE_DIR="$package_dir" node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
...
const actualDefinition = tools.find((tool) => tool.name === "dj_branch_outcomes");
if (!actualDefinition) throw new Error("dj_branch_outcomes was not registered");
...
const stockRow = new ToolExecutionComponent("dj_branch_outcomes", "stock", args, { showImages: false }, stockDefinition, ui, process.cwd());
const actualRow = new ToolExecutionComponent("dj_branch_outcomes", "actual", args, { showImages: false }, actualDefinition, ui, process.cwd());
...
const stockCall = stockHtml.renderCall("stock-html", "dj_branch_outcomes", args);
const actualCall = actualHtml.renderCall("actual-html", "dj_branch_outcomes", args);
const stockResult = stockHtml.renderResult("stock-html", "dj_branch_outcomes", result.content, result.details, false);
const actualResult = actualHtml.renderResult("actual-html", "dj_branch_outcomes", result.content, result.details, false);
...
JS
  ...
  pass "dj_branch_outcomes hides through ToolExecutionComponent while Calm-off and HTML export stay stock"
}
