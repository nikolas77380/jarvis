#!/usr/bin/env bash
# tests/dj-watch-arm.test.sh - the arm layer's cycle-close contract when the arm
...
# bin/dj-watch.sh holds the singleton, a real bin/dj-watch-arm.sh attaches to it,
...
TMP_ROOT=$(dj_test_tmproot dj-watch-arm-tests)
...
start_seed_watcher() {  # <state> <fakebin> <watch-out>
  ...
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_POLL=5 DJ_SIGNAL_GRACE=1 \
...
start_attached_arm() {  # <state> <fakebin> <arm-out> <confirm-timeout>
  ...
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_ARM_ATTACH_POLL=0.1 \
    DJ_ARM_CONFIRM_TIMEOUT="$confirm" "$WATCH_ARM" > "$armout" &
  ARM_PID=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
write_remote_delta() {  # <result-path> <status-line>
  ...
  {
    printf 'schema=dj-remote-delta.v1\n'
    printf 'status=delta\n'
...
start_rearm_arm() {  # <home> <state> <fakebin> <arm-out> [predecessor-arm-pid]
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" \
    DJ_POLL=1 DJ_SIGNAL_GRACE=0 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    DJ_WATCH_PREDECESSOR_ARM_PID="$predecessor" \
...
test_attached_arm_reports_the_delivered_wake_after_drain() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "drain failed"
...
test_rearm_resurfaces_durable_queue_and_remote_open_decision() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" DJ_DATA_OVERRIDE="$home/data" \
    "$ROOT/bin/dj-procevent-remote-reply.sh" ingest ios "$result" >/dev/null \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/baseline-drain.out" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drainout" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/decision-only-drain.out" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replayed-decision-drain.out" \
...
test_delivery_gap_wake_is_recovered_once() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/first-drain.out" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/gap-drain.out" \
...
test_interrupted_handling_is_redrained_on_rearm() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$WATCH_ARM" --handling-delivered "$handling_generation" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/interrupted-drain.out" \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replay-drain.out" \
...
test_malformed_marker_is_quarantined_once() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/recovery-drain.out" \
...
test_recovery_consumption_serializes_queue_publication() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" \
...
test_handling_window_close_keeps_the_acknowledgement_valid() {
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" 2> "$dir/drain.err" \
    || fail "handling drain did not present the durable wake"
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/remaining-drain.out" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "${pair%%$'\t'*}" \
...
test_moved_generation_acknowledgement_is_self_healing() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$first_sequence" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through 999 \
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/redrain.out" \
