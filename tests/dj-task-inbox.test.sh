#!/usr/bin/env bash
# tests/dj-task-inbox.test.sh - the per-task steering inbox
# (bin/dj-task-inbox-lib.sh) and the watcher's re-ring ladder.
#
...
#   5. A real dj-watch.sh subprocess re-rings the doorbell for an unhandled
#      aged message on an idle pane WITHOUT waking jarvis, waits on a busy
...
TMP_ROOT=$(dj_test_tmproot dj-task-inbox)
...
inbox_lib() {  # <state> <function> [args...]
  ...
  DJ_STATE_OVERRIDE="$state" bash -c '
    ...
    shift 2
    "$fn" "$@"
  ' _ "$ROOT/bin/dj-task-inbox-lib.sh" "$@"
}

# A fake tmux for the watcher cases: capture-pane replays DJ_FAKE_TMUX_CAPTURE,
# display-message yields a numeric cursor row, and every literal send-keys is
# logged to DJ_SEND_LOG so a doorbell ring is observable.
...
case "${1:-}" in
  send-keys)
    ...
    if [ "$literal" = 1 ]; then
      printf '%s\n' "${1:-}" >> "${DJ_SEND_LOG:-/dev/null}"
      if [ -n "${DJ_ACK_RECORD:-}" ] && [ -f "$DJ_ACK_RECORD" ]; then
        mv "$DJ_ACK_RECORD" "${DJ_ACK_RECORD%/*}/handled/"
      fi
    fi
    exit 0 ;;
  display-message)
    ...
    printf 'fakepane\n'; exit 0 ;;
  capture-pane)
    if [ -n "${DJ_FAKE_TMUX_CAPTURE:-}" ] && [ -f "$DJ_FAKE_TMUX_CAPTURE" ]; then
      cat "$DJ_FAKE_TMUX_CAPTURE"
    else
      printf '╭────╮\n│    │\n╰────╯\n'
    fi
    exit 0 ;;
...
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  ...
  shift 3
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" \
    DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_FAKE_CREW_STATE='state: working · source: run-step · validating (running)' \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    DJ_TASK_INBOX_GRACE_SECS=1 \
...
test_write_is_durable_and_exact() {
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "$text") \
  ...
  inbox_lib "$state" dj_task_inbox_body "$rec" > "$actual" \
    || fail "record body could not be read"
  cmp -s "$expected" "$actual" \
    || fail "record body did not preserve trailing and blank-line bytes"
  rec2=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "no trailing newline") \
  ...
  inbox_lib "$state" dj_task_inbox_body "$rec2" > "$actual2" \
    || fail "second record body could not be read"
  cmp -s "$expected2" "$actual2" \
    || fail "record body added a trailing newline"
  doorbell=$(inbox_lib "$state" dj_task_inbox_doorbell_line "$rec")
  doorbell2=$(inbox_lib "$state" dj_task_inbox_doorbell_line "$rec2")
  [ "$doorbell" = "$doorbell2" ] \
  ...
  assert_contains "$doorbell" "Jarvis instruction waiting" "doorbell should be self-describing"
  case "$doorbell" in
...
test_idempotent_write_dedups_exact_body() {
  ...
  r1=$(inbox_lib "$state" dj_task_inbox_write_idempotent "$state" t1 "$text") \
  ...
  r2=$(inbox_lib "$state" dj_task_inbox_write_idempotent "$state" t1 "$text") \
  ...
  r3=$(inbox_lib "$state" dj_task_inbox_write_idempotent "$state" t1 $'re-runnable steer\nsecond line changed') \
  ...
  r4=$(inbox_lib "$state" dj_task_inbox_write_idempotent "$state" t1 "$text") \
...
test_idempotent_write_follows_concurrent_ack() {
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write_idempotent "$state" t1 "$text") \
    || fail "race fixture write failed"
  result=$(DJ_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    eval "$(declare -f dj_task_inbox_body | sed "1s/dj_task_inbox_body/_original_fm_task_inbox_body/")"
    dj_task_inbox_body() {
      candidate=$1
      case "$candidate" in
        */handled/*) ;;
    ...
    dj_task_inbox_write_idempotent "$2" t1 "$3"
  ' _ "$ROOT/bin/dj-task-inbox-lib.sh" "$state" "$text") \
...
test_handled_mv_dedups_by_sequence() {
  ...
  r1=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "first")
  r2=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "second")
  [ "$r2" = "$state/t1.inbox/002.msg" ] || fail "second record should be 002.msg, got $r2"
  oldest=$(inbox_lib "$state" dj_task_inbox_oldest_unhandled "$state" t1)
  ...
  if inbox_lib "$state" dj_task_inbox_oldest_unhandled "$state" t1 >/dev/null; then
  ...
  r3=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "third")
...
test_concurrent_writers_never_clobber() {
  ...
  for i in 1 2 3 4 5 6; do
    inbox_lib "$state" dj_task_inbox_write "$state" t1 "steer number $i" >/dev/null &
    pids+=($!)
  done
  for i in "${pids[@]}"; do
...
test_ladder_writes_ignore_vanished_inbox() {
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "retired task")
  rm -rf "$state/t1.inbox"
  inbox_lib "$state" dj_task_inbox_record_ring "$state" t1 "$rec" \
    || fail "ring bookkeeping should ignore a concurrently removed inbox"
  inbox_lib "$state" dj_task_inbox_record_escalated "$state" t1 "$rec" \
...
test_ring_ladder_policy() {
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "do the thing")
  # Within grace: quiet.
  action=$(DJ_TASK_INBOX_GRACE_SECS=3600 inbox_lib "$state" dj_task_inbox_due_action "$state" t1)
  [ "$action" = quiet ] || fail "a fresh unhandled message inside grace should be quiet, got: $action"
  # Past grace: one ring is due.
  age_path "$rec"
  action=$(DJ_TASK_INBOX_GRACE_SECS=60 inbox_lib "$state" dj_task_inbox_due_action "$state" t1)
  ...
  action=$(DJ_TASK_INBOX_GRACE_SECS=60 DJ_TASK_INBOX_RING_MAX=3 inbox_lib "$state" dj_task_inbox_due_action "$state" t1)
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "next thing")
  age_path "$rec"
  action=$(DJ_TASK_INBOX_GRACE_SECS=60 DJ_TASK_INBOX_RING_MAX=3 inbox_lib "$state" dj_task_inbox_due_action "$state" t1)
...
setup_watch_case() {  # <name> -> echoes case dir; state in <dir>/state
  ...
  dj_write_meta "$dir/state/t1.meta" "window=sess:dj-t1" "kind=ship" "harness=grok"
  printf '%s\n' "$dir"
}
...
test_watcher_rerings_idle_pane_quietly() {
  ...
  rec=$(inbox_lib "$state" dj_task_inbox_write "$state" t1 "please continue")
  age_path "$rec"
  watch_bg "$state" "$dir/fakebin" "$out" \
    DJ_SEND_LOG="$log" DJ_FAKE_TMUX_CAPTURE="$(idle_capture "$dir")" \
    DJ_TASK_INBOX_RING_MAX=99
  pid=$!
  local i=0
  while [ "$i" -lt 100 ]; do
    grep -qF 'Jarvis instruction waiting' "$log" 2>/dev/null && break
    kill -0 "$pid" 2>/dev/null || break
    sleep 0.1
    i=$((i + 1))
  done
  grep -qF "Jarvis instruction waiting: list $state/t1.inbox/*.msg" "$log" \
    || { kill "$pid" 2>/dev/null; fail "the watcher never re-rang the doorbell:"$'\n'"$(cat "$log")"; }
  kill -0 "$pid" 2>/dev/null \
    || fail "a healthy re-ring must not wake jarvis (watcher exited):"$'\n'"$(cat "$out")"
  [ ! -s "$state/.wake-queue" ] \
  ...
  pass "watcher: an unhandled aged message on an idle pane re-rings without waking jarvis, and the ack silences it"
}
...
test_watcher_waits_on_busy_pane() {
  ...
  age_path "$rec"
  watch_bg "$state" "$dir/fakebin" "$out" \
    DJ_SEND_LOG="$log" DJ_FAKE_TMUX_CAPTURE="$dir/busy.capture" \
    DJ_BUSY_REGEX=BUSYTOKEN DJ_TASK_INBOX_RING_MAX=99
  pid=$!
  sleep 4
  kill "$pid" 2>/dev/null; wait "$pid" 2>/dev/null
...
test_watcher_ack_silences_unwritable_ladder() {
  ...
  watch_bg "$state" "$dir/fakebin" "$out" \
    DJ_SEND_LOG="$log" DJ_FAKE_TMUX_CAPTURE="$(idle_capture "$dir")" \
    DJ_ACK_RECORD="$rec" DJ_TASK_INBOX_RING_MAX=99
  pid=$!
  while [ "$i" -lt 100 ]; do
  ...
  kill "$pid" 2>/dev/null; wait "$pid" 2>/dev/null
  rings=$(grep -cF 'Jarvis instruction waiting' "$log" || true)
...
test_watcher_escalates_once_after_budget() {
  ...
  grep -qF 'unread jarvis instruction' "$state/.wake-queue" \
  ...
  [ "$(grep -cF 'unread jarvis instruction' "$state/.wake-queue")" = 1 ] \
