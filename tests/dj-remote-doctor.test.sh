#!/usr/bin/env bash
# tests/dj-remote-doctor.test.sh - the remote second-mate readiness gate.
#
# Drives the real bin/dj-remote-doctor.sh against a controlled account fixture:
...
TMP_ROOT=$(dj_test_tmproot dj-remote-doctor)
LABEL=dev.jarvis.herdr.dj-remote
INTERACTIVE_LABEL=dev.jarvis.herdr
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
JOB_LABEL=dev.jarvis.remote-job
CASE_N=0
DOCTOR_WORKER_PID=
trap 'if [ -n "$DOCTOR_WORKER_PID" ]; then kill "$DOCTOR_WORKER_PID" 2>/dev/null || true; fi; dj_test_cleanup || true' EXIT
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$DJ_FAKE_LAUNCHCTL_LOG"
domain=${2:-}
label=${domain##*/}
loaded="$DJ_FAKE_STATE/loaded-$label"
case "${1:-}" in
  print)
    case "$domain" in
      */dev.jarvis.herdr.dj-remote)
        [ -f "$loaded" ] || exit 113
        cat "$loaded"
        ;;
      */dev.jarvis.herdr)
        [ -f "$DJ_FAKE_STATE/interactive-loaded" ] || exit 113
        printf 'interactive default job\n'
        ;;
      */*/*) [ -f "$loaded" ] || exit 113; cat "$loaded" ;;
      *) [ -f "$DJ_FAKE_STATE/gui-session" ] || exit 113 ;;
    esac
    exit 0
    ;;
  bootout)
    [ ! -f "$DJ_FAKE_STATE/bootout-fail" ] || { printf 'Boot-out failed: operation not permitted\n' >&2; exit 6; }
    case "$domain" in
      */dev.jarvis.herdr.dj-remote) rm -f "$loaded" ;;
      */dev.jarvis.herdr) rm -f "$DJ_FAKE_STATE/interactive-loaded" ;;
      *) rm -f "$loaded" ;;
    esac
    exit 0
    ;;
  bootstrap)
    ...
    [ -f "$DJ_FAKE_STATE/gui-session" ] || { printf 'Bootstrap failed: 5: Input/output error\n' >&2; exit 5; }
    ...
    [ ! -f "$loaded" ] || { printf 'Bootstrap failed: service already loaded\n' >&2; exit 5; }
    case "$label" in
      dev.jarvis.remote-job)
        cat > "$loaded" <<EOF
path = $DJ_FAKE_JOB_PLIST
program = $DJ_FAKE_JOB_WORKER
...
stdout path = $DJ_FAKE_LAUNCH_AGENT_LOG
stderr path = $DJ_FAKE_LAUNCH_AGENT_LOG
properties = keepalive | runatload | inferred program
EOF
        [ -f "$DJ_FAKE_STATE/bootstrap-does-not-start" ] || printf 'true\n' > "$DJ_FAKE_HERDR_RUNNING"
        ;;
    esac
    exit 0
    ;;
  kickstart)
    [ ! -f "$DJ_FAKE_STATE/kickstart-fail" ] || { printf 'Kickstart failed: service unavailable\n' >&2; exit 6; }
    case "$label" in
      dev.jarvis.remote-job) : ;;
      *)
        if [ -f "$DJ_FAKE_STATE/kickstart-delay" ]; then
          cp "$DJ_FAKE_STATE/kickstart-delay" "$DJ_FAKE_STATE/herdr-delay"
        else
          printf 'true\n' > "$DJ_FAKE_HERDR_RUNNING"
        fi
        ;;
...
#!/usr/bin/env bash
set -u
running=$(cat "$DJ_FAKE_HERDR_RUNNING" 2>/dev/null || printf 'false')
case "${1:-} ${2:-}" in
  "status --json")
    if [ -f "$DJ_FAKE_STATE/herdr-delay" ]; then
      delay=$(cat "$DJ_FAKE_STATE/herdr-delay")
      if [ "$delay" -gt 0 ]; then
        printf '%s\n' "$((delay - 1))" > "$DJ_FAKE_STATE/herdr-delay"
        running=false
      else
        rm -f "$DJ_FAKE_STATE/herdr-delay"
        printf 'true\n' > "$DJ_FAKE_HERDR_RUNNING"
        running=true
      fi
    fi
...
doctor() {
  ...
  DOCTOR_OUT=$(
    HOME="$CASE_HOME" \
    DJ_HOME="$CASE_PROJECT_HOME" \
    PATH="$CASE_HOME/.local/bin:$CASE_BIN:$BASE_PATH" \
    DJ_FAKE_STATE="$CASE_STATE" \
    DJ_FAKE_LAUNCHCTL_LOG="$CASE_LAUNCHCTL_LOG" \
    DJ_FAKE_FORBIDDEN_LOG="$CASE_FORBIDDEN_LOG" \
    DJ_FAKE_HERDR_RUNNING="$CASE_HERDR_RUNNING" \
    DJ_FAKE_HERDR_BIN="$CASE_BIN/herdr" \
    DJ_FAKE_PLIST="$CASE_PLIST" \
    DJ_FAKE_JOB_PLIST="$CASE_JOB_PLIST" \
    DJ_FAKE_JOB_WORKER="$ROOT/bin/dj-remote-job-worker.sh" \
    DJ_FAKE_LAUNCH_AGENT_LOG="$CASE_HOME/Library/Logs/$LABEL.log" \
    DJ_REMOTE_JOB_PLATFORM_OVERRIDE="${CASE_PLATFORM_OVERRIDE-}" \
    DJ_REMOTE_JOB_ACTIVE="${CASE_REMOTE_JOB_ACTIVE-1}" \
    "$ROOT/bin/dj-remote-doctor.sh" "$@" 2>&1
  )
  DOCTOR_RC=$?
  set -e
}
...
assert_grep "<string>$LABEL</string>" "$CASE_PLIST" "the written plist does not carry the Jarvis label"
assert_grep '<string>server</string>' "$CASE_PLIST" "the written plist does not run a herdr server"
assert_grep '<string>dj-remote</string>' "$CASE_PLIST" "the written plist does not pin the remote-secondmate session"
assert_no_grep '<string>default</string>' "$CASE_PLIST" "the written plist pins the interactive default session"
assert_grep "<string>$JOB_LABEL</string>" "$CASE_JOB_PLIST" "the worker plist does not carry the Jarvis label"
assert_grep '<string>Aqua</string>' "$CASE_JOB_PLIST" "the worker plist is not Aqua-scoped"
assert_grep "$ROOT/bin/dj-remote-job-worker.sh" "$CASE_JOB_PLIST" "the worker plist does not use the configured code root"
...
cmp -s "$CASE_STATE/interactive-before.plist" "$CASE_INTERACTIVE_PLIST" \
  || fail "the dj-remote repair rewrote the interactive default launch agent"
assert_present "$CASE_STATE/interactive-loaded" "the dj-remote repair unloaded the interactive default launch agent"
assert_no_grep "gui/$(id -u)/$INTERACTIVE_LABEL$" "$CASE_LAUNCHCTL_LOG" \
  "the dj-remote repair inspected or controlled the interactive default launch agent"
assert_no_dangerous_calls "the repair reached for auto-login, FileVault, or the keychain"
pass "--fix installs the dedicated dj-remote launch agent without touching default"
...
assert_grep '# Jarvis remote tool wrapper v1' "$CASE_HOME/.local/bin/tasks-axi" \
  "the generated wrapper is not marked Jarvis-owned"
...
assert_contains "$DOCTOR_OUT" 'fix required-treehouse=failed:' \
  "the non-Jarvis wrapper refusal was not reported"
...
HOME="$CASE_HOME" DJ_ROOT_OVERRIDE="$ROOT" DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  "$ROOT/bin/dj-remote-job-worker.sh" > "$CASE_STATE/worker.out" 2> "$CASE_STATE/worker.err" &
DOCTOR_WORKER_PID=$!
for _ in $(seq 1 100); do
  [ -f "$CASE_HOME/.jarvis/remote-job/worker.ready" ] && break
  sleep 0.05
done
assert_present "$CASE_HOME/.jarvis/remote-job/worker.ready" "the stale-identity fixture worker did not start"
printf 'stale-worker-identity\n' > "$CASE_HOME/.jarvis/remote-job/worker.identity"
doctor
expect_code 1 "$DOCTOR_RC" "doctor accepted a live worker with stale code identity"
assert_contains "$DOCTOR_OUT" 'check remote-job-worker=fixable: the running remote job worker does not match the current Jarvis code' \
...
DOCTOR_WORKER_PID=$(cat "$CASE_HOME/.jarvis/remote-job/worker.pid")
...
printf '#!/usr/bin/env bash\n' > "$REMOTE_ROOT/bin/dj-remote-entrypoint.sh"
export DJ_ROOT_OVERRIDE="$REMOTE_ROOT"
doctor
...
[ "$(readlink "$CASE_HOME/.local/bin/dj-remote-entrypoint.sh")" = "$REMOTE_ROOT/bin/dj-remote-entrypoint.sh" ] \
  || fail "the entrypoint symlink does not point at this code root"
printf 'not a symlink\n' > "$CASE_HOME/.local/bin/other"
rm -f "$CASE_HOME/.local/bin/dj-remote-entrypoint.sh"
printf 'operator wrapper\n' > "$CASE_HOME/.local/bin/dj-remote-entrypoint.sh"
doctor --fix
assert_contains "$DOCTOR_OUT" 'check entrypoint-link=human:' "an operator-owned entrypoint file was not left to the operator"
[ "$(cat "$CASE_HOME/.local/bin/dj-remote-entrypoint.sh")" = 'operator wrapper' ] \
