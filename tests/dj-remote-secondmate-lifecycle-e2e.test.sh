...
TMP_ROOT=$(dj_test_tmproot dj-remote-secondmate-e2e)
mkdir -p "$TMP_ROOT"
...
FAKEBIN=$(dj_fakebin "$TMP_ROOT/fake")
...
cleanup() {
  ...
  DJ_HOME="$PARENT" DJ_PROCEVENT_CLAIM_ROOT="$CLAIMS" \
    "$ROOT/bin/dj-procevent.sh" sweep-home >/dev/null 2>&1 || true
...
case "\${1:-}" in
  ...
  list-windows)
    [ -f "\$state" ] || exit 0
    name=\$(cut -d'|' -f1 "\$state")
    case "\$*" in *'#{session_name}:#{window_name}'*) printf 'jarvis:%s\n' "\$name" ;; *) printf '%s\n' "\$name" ;; esac
    exit 0
    ;;
  new-window)
    name=; cwd=
...
REMOTE_ORIGIN="$TMP_ROOT/jarvis-origin.git"
...
#!/usr/bin/env bash
count=$(cat "$DJ_FAKE_SSH_COUNT" 2>/dev/null || echo 0)
printf '%s\n' "$((count + 1))" > "$DJ_FAKE_SSH_COUNT"
while [ "$#" -gt 0 ]; do
...
[ "$entry" = dj-remote-entrypoint.sh ] || exit 92
cd "$DJ_FAKE_REMOTE_CWD" || exit 93
argv_b64=$4
...
$command_fields
EOF
case "${DJ_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  inherit-partial:dj-remote-inherit.sh:config/crew-harness) exit 255 ;;
  inherit-block:dj-remote-inherit.sh:data/captain-shared.md)
    cat > "$DJ_FAKE_INHERIT_PAYLOAD"
    touch "$DJ_FAKE_INHERIT_ENTERED"
    while [ ! -f "$DJ_FAKE_INHERIT_RELEASE" ]; do sleep 0.02; done
    "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@" < "$DJ_FAKE_INHERIT_PAYLOAD"
    exit $?
    ;;
esac
# The readiness gate is answered here rather than by the real doctor, which
# would inspect and repair the RUNNER's own account. tests/dj-remote-doctor.test.sh
...
if [ "$command_name" = dj-remote-doctor.sh ]; then
  printf '%s %s\n' "${DJ_FAKE_SSH_MODE:-normal}" "${_command_action:--}" >> "$DJ_FAKE_DOCTOR_LOG"
  case "${DJ_FAKE_SSH_MODE:-normal}" in
    ...
    doctor-fix-unknown)
      ...
      printf 'check launchagent=fixable: no Jarvis herdr launch agent\n'
...
if [ "${DJ_FAKE_SSH_MODE:-normal}" = doctor-fixable ] \
  && [ "$command_name" = dj-remote-secondmate-control.sh ] \
  && [ "$_command_action" = state ] \
  && [ ! -f "$DJ_FAKE_DOCTOR_REPAIRED" ]; then
  printf 'unreadable\n'
  exit 0
fi
case "${DJ_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  launch-nonherdr-route:dj-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    printf 'schema=dj-remote-secondmate-control.v1\n'
    printf 'backend=tmux\n'
    printf 'target=jarvis:dj-ios\n'
    printf 'harness=codex\n'
    exit 0
    ;;
  launch-default-session-route:dj-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    printf 'schema=dj-remote-secondmate-control.v1\n'
    ...
    exit 0
    ;;
  provision-block-fail:dj-remote-home-provision.sh:*)
    touch "$DJ_FAKE_SEED_ENTERED"
    while [ ! -f "$DJ_FAKE_SEED_RELEASE" ]; do sleep 0.02; done
    exit 1
    ;;
  launch-block:dj-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    touch "$DJ_FAKE_LAUNCH_ENTERED"
    while [ ! -f "$DJ_FAKE_LAUNCH_RELEASE" ]; do sleep 0.02; done
    ;;
esac
case "${DJ_FAKE_SSH_MODE:-normal}" in
  unreachable) exit 255 ;;
  ambiguous) "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@"; exit 255 ;;
  *) exec "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@" ;;
esac
SH
...
publish_healthy_watcher_identity() { # <state> <home> <watch-script>
  ...
  identity=$(DJ_HOME="$PARENT" DJ_STATE_OVERRIDE="$PARENT/state" /bin/bash -c \
    '. "$1"; dj_pid_identity "$2"' _ "$ROOT/bin/dj-wake-lib.sh" "$$") \
...
remote_env() {
  DJ_HOME="$PARENT" \
  DJ_ROOT_OVERRIDE="$REMOTE_ROOT" \
  DJ_PROCEVENT_CLAIM_ROOT="$CLAIMS" \
  DJ_SSH_BIN="$FAKEBIN/fake-ssh" \
  DJ_FAKE_SSH_COUNT="$SSH_COUNT" \
  DJ_FAKE_REMOTE_ENTRYPOINT="$REMOTE_ROOT/bin/dj-remote-entrypoint.sh" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  DJ_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" \
  DJ_FAKE_SSH_MODE="${DJ_FAKE_SSH_MODE:-normal}" \
  DJ_FAKE_REMOTE_CWD="$TMP_ROOT" \
  DJ_FAKE_SEED_ENTERED="$TMP_ROOT/seed.entered" \
  DJ_FAKE_SEED_RELEASE="$TMP_ROOT/seed.release" \
  DJ_FAKE_DOCTOR_LOG="$DOCTOR_LOG" \
  DJ_FAKE_DOCTOR_REPAIRED="$TMP_ROOT/doctor.repaired" \
  DJ_FAKE_INHERIT_ENTERED="$TMP_ROOT/inherit.entered" \
  DJ_FAKE_INHERIT_RELEASE="$TMP_ROOT/inherit.release" \
  DJ_FAKE_INHERIT_PAYLOAD="$TMP_ROOT/inherit.payload" \
  DJ_FAKE_LAUNCH_ENTERED="$TMP_ROOT/launch.entered" \
  DJ_FAKE_LAUNCH_RELEASE="$TMP_ROOT/launch.release" \
  DJ_SEND_SETTLE=0 DJ_SEND_SLEEP=0 DJ_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
SH
chmod +x "$FAKEBIN/git"
printf 'schema=dj-remote-home-provision.v1\nid_b64=%s\ncharter_b64=%s\nproject_count=0\n' \
  "$(printf ios | base64 | tr -d '\n')" \
  "$(printf 'Concurrent provisioning charter.\n' | base64 | tr -d '\n')" \
  > "$TMP_ROOT/provision.manifest"
PATH="$FAKEBIN:$PATH" DJ_HOME="$TMP_ROOT/concurrent-home" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" \
  "$REMOTE_ROOT/bin/dj-remote-home-provision.sh" < "$TMP_ROOT/provision.manifest" \
...
[ "$(cat "$TMP_ROOT/concurrent-home/.dj-secondmate-home")" = ios ] \
...
if [ "${DJ_TEST_PROVISION_ONLY:-0}" = 1 ]; then
  echo "ALL TESTS PASSED"
  exit 0
fi

mkdir -p "$TMP_ROOT/seed-parent/data" "$TMP_ROOT/seed-parent/state"
DJ_SECONDMATE_CHARTER='Failing seed charter.' DJ_SECONDMATE_SCOPE='failed seed' \
  DJ_FAKE_SSH_MODE=provision-block-fail seed_env "$ROOT/bin/dj-remote-home-seed.sh" \
...
DJ_SECONDMATE_CHARTER='Successful seed charter.' DJ_SECONDMATE_SCOPE='successful seed' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-keep remote-mac "$REMOTE_ROOT" \
...
assert_present "$TMP_ROOT/seed-keep-home/.dj-secondmate-home" "serialized seed lost its published remote home"
pass "remote seed rollback preserves serialized competing routes"

: > "$DOCTOR_LOG"
if DJ_SECONDMATE_CHARTER='Unknown readiness charter.' DJ_SECONDMATE_SCOPE='unknown readiness' \
  DJ_FAKE_SSH_MODE=doctor-fix-unknown seed_env "$ROOT/bin/dj-remote-home-seed.sh" \
...
: > "$DOCTOR_LOG"
if DJ_SECONDMATE_CHARTER='Unready host charter.' DJ_SECONDMATE_SCOPE='unready host' \
  DJ_FAKE_SSH_MODE=doctor-human seed_env "$ROOT/bin/dj-remote-home-seed.sh" \
...
out=$(DJ_SECONDMATE_CHARTER='Repairable host charter.' DJ_SECONDMATE_SCOPE='repairable host' \
  DJ_FAKE_SSH_MODE=doctor-fixable seed_env "$ROOT/bin/dj-remote-home-seed.sh" \
...
assert_present "$TMP_ROOT/seed-repair-home/.dj-secondmate-home" "the repaired host was never provisioned"
...
# Seeding must not need a copy of the project in this home: jarvis names the
...
dj_git_init_commit "$TMP_ROOT/seed-parent/projects/resident"
git init -q --bare "$TMP_ROOT/beta.git"
dj_git_init_commit "$TMP_ROOT/beta-src"
...
if DJ_SECONDMATE_CHARTER='Unsupplied origin charter.' DJ_SECONDMATE_SCOPE='unsupplied origin' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-noorigin remote-mac "$REMOTE_ROOT" \
...
if DJ_SECONDMATE_CHARTER='Unsafe origin charter.' DJ_SECONDMATE_SCOPE='unsafe origin' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-unsafe remote-mac "$REMOTE_ROOT" \
...
if DJ_SECONDMATE_CHARTER='Local-only charter.' DJ_SECONDMATE_SCOPE='local only' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-localonly remote-mac "$REMOTE_ROOT" \
...
if DJ_SECONDMATE_CHARTER='Unregistered charter.' DJ_SECONDMATE_SCOPE='unregistered' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-unregistered remote-mac "$REMOTE_ROOT" \
...
out=$(DJ_SECONDMATE_CHARTER='Own beta delivery on the build Mac.' \
  DJ_SECONDMATE_SCOPE='beta delivery and validation' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-noclone remote-mac "$REMOTE_ROOT" \
...
if DJ_HOME="$TMP_ROOT/unsafe-origin-home" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" \
  "$REMOTE_ROOT/bin/dj-remote-home-provision.sh" < "$TMP_ROOT/unsafe-origin.manifest" \
...
# Jarvis is a shared template, so seeding must carry a project origin from any
...
forge_project() { # <project> <origin-url>
  ...
  tab=$(printf '\t')
  dj_git_init_commit "$TMP_ROOT/forge-src-$project"
  ...
  git -C "$TMP_ROOT/forge-src-$project" \
    -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm origin
...
out=$(DJ_SECONDMATE_CHARTER='Own delivery for projects hosted anywhere.' \
  DJ_SECONDMATE_SCOPE='multi-forge delivery' \
  seed_env "$ROOT/bin/dj-remote-home-seed.sh" seed-forge remote-mac "$REMOTE_ROOT" \
  "$FORGE_HOME" \
...
out=$(DJ_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
  DJ_SECONDMATE_SCOPE='iOS implementation and Xcode validation' \
  remote_env "$ROOT/bin/dj-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$REMOTE_HOME" alpha)
...
assert_present "$REMOTE_HOME/.dj-secondmate-home" "remote provisioning did not publish the identity marker"
...
if DJ_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
  DJ_SECONDMATE_SCOPE='iOS implementation and Xcode validation' \
  remote_env "$ROOT/bin/dj-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$TMP_ROOT/other-home" alpha \
  >/dev/null 2>&1; then
...
if printf 'complete' | DJ_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/dj-remote-inherit.sh" \
...
DJ_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/dj-remote-inherit.sh" \
...
if DJ_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/dj-remote-inherit.sh" \
...
EOF
remote_env "$ROOT/bin/dj-home-seed.sh" validate >/dev/null || fail "mixed local and remote registry validation failed"
...
if DJ_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate \
...
out=$(remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate)
...
assert_grep 'remote_herdr_session=dj-remote' "$PARENT/state/ios.meta" "parent metadata omitted the pinned remote Herdr session"
assert_grep 'remote_target=dj-remote:' "$PARENT/state/ios.meta" "parent metadata did not record an dj-remote endpoint"
assert_grep 'herdr_session=dj-remote' "$REMOTE_HOME/state/parent-route/ios.meta" "remote metadata did not record the pinned Herdr session"
assert_grep '--session dj-remote' "$HERDR_LOG" "remote launch did not target the dj-remote session"
...
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$ROOT/bin/dj-watch.sh"
[ "$(remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh state ios)" = alive ] \
...
[ "$(remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh observe ios)" = idle ] \
...
[ "$(remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh state ios 2>/dev/null)" = unverified ] \
  || fail "legacy default-session metadata was not classified unverified"
if remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh route ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh send ios probe >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh key ios Enter >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh capture ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh observe ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh retire ios --force >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh launch ios codex - - herdr >/dev/null 2>&1; then
...
' "$TMP_ROOT/remote-ios-before-default-session.meta" > "$remote_route_meta"
[ "$(remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh state ios 2>/dev/null)" = unverified ] \
  || fail "mismatched dj-remote target was not classified unverified"
cmp -s "$TMP_ROOT/herdr-before-default-session.log" "$HERDR_LOG" \
  || fail "mismatched dj-remote target caused a Herdr operation"
...
set +e
DJ_FAKE_SSH_MODE=launch-nonherdr-route remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate \
...
set +e
DJ_FAKE_SSH_MODE=launch-default-session-route remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate \
...
assert_grep "remote launch returned Herdr session 'default', expected 'dj-remote'" "$TMP_ROOT/spawn-default-session-route.out" \
...
set +e
remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh launch ios codex - - herdr \
...
This file is main-authoritative and maintained by the main jarvis.
...
EOF
DJ_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate \
...
EOF
remote_env "$ROOT/bin/dj-config-push.sh" > "$TMP_ROOT/spawn-concurrent-push.out" 2>&1 &
spawn_config_push=$!
sleep 0.2
...
set +e
DJ_FAKE_SSH_MODE=ambiguous remote_env "$ROOT/bin/dj-send.sh" dj-ios \
...
assert_grep 'Jarvis instruction waiting' "$HERDR_LOG" "the remote doorbell never rang"
CORR=$(newest_remote_inbox_corr)
[ -n "$CORR" ] || fail "remote send did not carry a correlation token"
assert_grep "DJ_PENDING_REPLY_EXISTING_CORR=$CORR" "$TMP_ROOT/send.err" "ambiguous remote send did not print its correlation-reusing command"
...
remote_env "$ROOT/bin/dj-procevent.sh" start "$SID" >/dev/null \
  || fail "remote reply source did not capture the correlated answer"
RESULT="$PARENT/state/procevent-inbox/$SID.1.result"
remote_env "$ROOT/bin/dj-procevent-remote-reply.sh" handle ios 1 "$RESULT" >/dev/null \
...
printf 'grok\n' > "$PARENT/config/crew-harness"
set +e
DJ_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/dj-config-push.sh" \
...
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$REMOTE_ROOT/bin/dj-watch.sh"
remote_env "$ROOT/bin/dj-bootstrap.sh" > "$TMP_ROOT/config-partial-retry.out" \
...
remote_env "$ROOT/bin/dj-procevent-remote-reply.sh" handle ios 2 "$PARTIAL_CONFIG_RESULT" >/dev/null \
...
EOF
DJ_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/dj-config-push.sh" \
...
EOF
remote_env "$ROOT/bin/dj-bootstrap.sh" > "$TMP_ROOT/config-concurrent-second.out" 2>&1 &
config_second=$!
sleep 0.2
...
if remote_env "$ROOT/bin/dj-config-push.sh" > "$TMP_ROOT/config-push-fail.out" 2>&1; then
...
chmod 755 "$REMOTE_HOME/state/parent-route/ios.inbox"
remote_env "$ROOT/bin/dj-config-push.sh" > "$TMP_ROOT/config-push-retry.out" \
...
remote_env "$ROOT/bin/dj-procevent-remote-reply.sh" handle ios 3 "$CONFIG_RESULT" >/dev/null \
...
resolve_ios_pending() {
  ...
  for pending_record in "$PARENT/state/pending-replies"/*; do
    ...
    remote_env "$ROOT/bin/dj-procevent-remote-reply.sh" handle ios "$pending_seq" "$pending_result" >/dev/null \
...
SNAPSHOT=$(remote_env "$ROOT/bin/dj-fleet-snapshot.sh" --json)
...
REMOTE_SEED="$TMP_ROOT/jarvis-seed"
...
UPDATE_OUT=$(remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh update ios)
...
rm -f "$TMP_ROOT/doctor.repaired"
: > "$DOCTOR_LOG"
[ "$(DJ_FAKE_SSH_MODE=doctor-fixable remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-secondmate-control.sh state ios)" = unreadable ] \
...
BOOT_REPAIRED=$(DJ_FAKE_SSH_MODE=doctor-fixable remote_env "$ROOT/bin/dj-bootstrap.sh")
[ "$(cat "$DOCTOR_LOG")" = 'doctor-fixable -
doctor-fixable --fix
...
BOOT_LEGACY=$(remote_env "$ROOT/bin/dj-bootstrap.sh")
...
BOOT_UNAVAILABLE=$(DJ_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/dj-bootstrap.sh")
...
UNAVAILABLE=$(DJ_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/dj-fleet-snapshot.sh" --json)
...
# A sibling remote secondmate workspace shares dj-remote and must survive every
# refusal and the eventual successful retirement of ios.
# This fixture overrides DJ_ROOT for transport, so teardown's root-owned guard
# sees the fixture root rather than the source script path used by dj-send.
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$REMOTE_ROOT/bin/dj-watch.sh"
resolve_ios_pending
SIBLING_CREATE=$("$REMOTE_ROOT/bin/herdr" workspace create --cwd "$REMOTE_ROOT" \
  --label 2ndmate-macos --no-focus --session dj-remote)
...
if remote_env "$ROOT/bin/dj-teardown.sh" ios >/dev/null 2>&1; then
...
remote_env "$ROOT/bin/dj-bootstrap.sh" >/dev/null \
...
retired_wake_corr=$(DJ_HOME="$PARENT" bash -c '
  . "$1"
  dj_pending_reply_create "$2" "$2/state" ios "New routed work is in your backlog."
' _ "$ROOT/bin/dj-pending-reply-lib.sh" "$PARENT") \
  || fail "could not seed remote receiver wake retirement state"
retired_wake_rec="$PARENT/state/pending-replies/$retired_wake_corr"
DJ_HOME="$PARENT" bash -c '
  . "$1"
  dj_pending_reply_set "$2" phase resolved
  dj_pending_reply_set "$2" delivered_epoch 1
' _ "$ROOT/bin/dj-pending-reply-lib.sh" "$retired_wake_rec" \
...
DJ_HOME="$PARENT" /bin/bash -c '
  . "$1"
  dj_lock_acquire_wait "$2"
  touch "$3"
  while [ ! -f "$4" ]; do sleep 0.02; done
  dj_lock_release "$2"
' _ "$ROOT/bin/dj-wake-lib.sh" "$handoff_lock" "$TMP_ROOT/handoff.entered" \
...
DJ_FAKE_SSH_MODE=launch-block remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate \
...
remote_env "$ROOT/bin/dj-teardown.sh" ios > "$TMP_ROOT/teardown-serialized.out" 2>&1 &
teardown_pid=$!
sleep 0.2
...
' "$HERDR_STATE" >/dev/null \
  || fail "remote retirement removed the sibling secondmate workspace or pane from dj-remote"
assert_no_grep 'session stop' "$HERDR_LOG" "remote retirement stopped the shared dj-remote session"
assert_no_grep 'server stop' "$HERDR_LOG" "remote retirement stopped the shared dj-remote server"
