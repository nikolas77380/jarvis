#!/usr/bin/env bash
# tests/dj-secondmate-lifecycle-e2e.test.sh - the happy-path secondmate operator
...
# lease-specific paths live in dj-secondmate-safety.test.sh.
#
...
#   - a bare `dj-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(dj_test_tmproot dj-secondmate-lifecycle)
...
setup_world() {
  ...
  dj_git_init_commit "$HOME_DIR/projects/alpha"
  dj_git_init_commit "$HOME_DIR/projects/beta"
  dj_git_init_commit "$HOME_DIR/projects/gamma"
  dj_git_add_origin "$HOME_DIR/projects/alpha" "$TMP_ROOT/remotes/alpha.git"
  dj_git_add_origin "$HOME_DIR/projects/beta" "$TMP_ROOT/remotes/beta.git"
  dj_git_add_origin "$HOME_DIR/projects/gamma" "$TMP_ROOT/remotes/gamma.git"
...
EOF
  ...
  DJ_SECONDMATE_SCOPE='customer onboarding from brief' \
...
phase_seed() {
  local out
  out=$(PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" \
    "$ROOT/bin/dj-home-seed.sh" design "$SUB" alpha beta gamma) \
    || fail "seed failed"
  SUB_ABS=$(cd "$SUB" && pwd -P)

  assert_contains "$out" "home=$SUB_ABS" "seed did not report the subhome"
  assert_present "$SUB/.dj-secondmate-home" "seed did not mark the subhome"
  ...
  [ "$(DJ_HOME="$SUB" "$ROOT/bin/dj-project-mode.sh" alpha)" = "direct-PR on" ] \
    || fail "alpha delivery mode not preserved in the subhome"
  [ "$(DJ_HOME="$SUB" "$ROOT/bin/dj-project-mode.sh" beta)" = "direct-PR off" ] \
    || fail "beta delivery mode not preserved in the subhome"
  DJ_HOME="$HOME_DIR" "$ROOT/bin/dj-home-seed.sh" validate >/dev/null || fail "registry validation failed after seed"
...
phase_spawn() {
  : > "$LOG"
  PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" DJ_CONFIG_OVERRIDE="$HOME_DIR/parent-config" \
    DJ_FAKE_TMUX_LOG="$LOG" DJ_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/dj-spawn.sh" design "$SUB" codex --secondmate >/dev/null \
  ...
  assert_grep "DJ_HOME='$SUB_ABS'" "$LOG" "secondmate launch did not set DJ_HOME to the subhome"
  assert_grep 'DJ_ROOT_OVERRIDE= DJ_STATE_OVERRIDE= DJ_DATA_OVERRIDE= DJ_PROJECTS_OVERRIDE=' "$LOG" "launch did not clear operational overrides"
  assert_grep 'DJ_CONFIG_OVERRIDE=' "$LOG" "launch did not clear the config override"
...
phase_send() {
  : > "$LOG"
  printf '❯\n' > "$PANE"
  # The meta window (jarvis:dj-design) must win over a foreign same-named
  # window returned by list-windows.
  PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" DJ_FAKE_TMUX_WINDOW="other-session:dj-design" \
    DJ_FAKE_TMUX_LOG="$LOG" DJ_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/dj-send.sh" dj-design 'route this work' >/dev/null 2>&1 \
    || fail "dj-send failed for a bare jarvis window with home metadata"
  # design is a kind=secondmate target, so the durable inbox record carries the
  # from-jarvis marker and original payload. The terminal receives only the
  ...
  assert_contains "$body" '[dj-from-jarvis]' "the inbox request was not marked as from-jarvis"
  assert_contains "$body" 'route this work' "the original request text did not survive the marker"
  assert_grep 'send-keys -t jarvis:dj-design -l Jarvis instruction waiting:' "$LOG" "send did not ring the window recorded in this home's meta"
  assert_no_grep 'route this work' "$LOG" "send typed the payload instead of only the doorbell"
  assert_no_grep 'send-keys -t other-session:dj-design' "$LOG" "send targeted a foreign same-named window"
  pass "send: a bare dj-<id> secondmate enqueues a marked request and rings the meta window"
}
...
EOF
  local out before
  out=$(PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" DJ_FAKE_TMUX_LOG="$LOG" \
    DJ_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/dj-backlog-handoff.sh" design feat-x feat-y) \
  ...
  PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" DJ_FAKE_TMUX_LOG="$LOG" \
    DJ_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/dj-backlog-handoff.sh" design feat-x feat-y >/dev/null 2>&1 \
...
phase_recovery() {
  ...
  PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" DJ_FAKE_TMUX_LOG="$LOG" DJ_FAKE_TMUX_CAPTURE="$PANE" \
    "$ROOT/bin/dj-spawn.sh" design "echo relaunch" --secondmate >/dev/null 2>&1 \
