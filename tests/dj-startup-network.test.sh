#!/usr/bin/env bash
# tests/dj-startup-network.test.sh - behavior tests for bin/dj-startup-network.sh,
...
TMP_ROOT=$(dj_test_tmproot dj-startup-network-tests)
DJ_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap dj_test_cleanup EXIT

# new_world <name>: an DJ_HOME plus a fake code root whose bin/ is a real
# jarvis bin/ except for dj-bootstrap.sh, which is replaced by a scriptable
# stand-in. The stage's contract is about WHEN and WHETHER the network half runs
# and how its result is published; bin/dj-bootstrap.sh's own behavior is owned by
# tests/dj-bootstrap.test.sh, so pinning it here would duplicate that owner and
...
new_world() {
  ...
  rm -f "$root/bin/dj-bootstrap.sh"
  cat > "$root/bin/dj-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
# Scriptable stand-in: records how it was invoked, then behaves as the test asks.
set -u
printf 'network=%s detect_only=%s\n' \
  "${DJ_BOOTSTRAP_NETWORK:-all}" "${DJ_BOOTSTRAP_DETECT_ONLY:-0}" \
  >> "${DJ_FAKE_BOOTSTRAP_LOG:?}"
# The real sweeps record their elapsed times through dj-timing-lib.sh, which
# reaches them as an exported DJ_TIMING_LOG. Recording the same way here proves
...
# is owned by tests/dj-bootstrap.test.sh.
if [ -n "${DJ_TIMING_LOG:-}" ]; then
  . "$(dirname "$0")/dj-timing-lib.sh"
  dj_timing_record phase "${DJ_FAKE_TIMING_PHASE:-gh-auth}" \
    "$(( $(dj_timing_now_ms) - 1500 ))" "${DJ_FAKE_TIMING_DETAIL:-}"
fi
[ -z "${DJ_FAKE_BOOTSTRAP_SLEEP:-}" ] || sleep "$DJ_FAKE_BOOTSTRAP_SLEEP"
[ -z "${DJ_FAKE_BOOTSTRAP_OUT:-}" ] || printf '%s\n' "$DJ_FAKE_BOOTSTRAP_OUT"
exit "${DJ_FAKE_BOOTSTRAP_RC:-0}"
SH
  chmod +x "$root/bin/dj-bootstrap.sh"
  cat > "$root/bin/ps" <<'SH'
#!/usr/bin/env bash
pid=
previous=
...
if [ "$pid" = "${DJ_FAKE_HARNESS_PID:-}" ]; then
  case "$*" in
...
EOF
  ...
  pass "dj-startup-network: wait fails when no deferred stage publishes before its deadline"
}

run_stage() {  # <home> <root> <args...>
  local home=$1 root=$2
  shift 2
  PATH="$root/bin:$PATH" DJ_FAKE_HARNESS_PID="${DJ_FAKE_HARNESS_PID_OVERRIDE:-$$}" \
    DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" "$root/bin/dj-startup-network.sh" "$@"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=10 \
  ...
  assert_contains "$pending" "$root/bin/dj-startup-network.sh report" \
  ...
  pass "dj-startup-network: start returns immediately and never holds the caller's stdout open"
}
...
EOF
  ...
  sleep 30 &
  claimant=$!
  DJ_SESSION_START_TIMEOUT=15 DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_OUT='acknowledged result' \
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_OUT='MISSING: some-tool (install: brew install some-tool)' \
  ...
  pass "dj-startup-network: exactly one of the digest and the wake reports each actionable result"
}
...
EOF
  ...
  DJ_SESSION_START_TIMEOUT=4 DJ_FAKE_BOOTSTRAP_LOG="$log" \
    DJ_FAKE_BOOTSTRAP_OUT='MISSING: some-tool (install: brew install some-tool)' \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  pass "dj-startup-network: a claimant crash after publication still surfaces the result"
}
...
EOF
  ...
  DJ_SESSION_START_TIMEOUT=4 DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_OUT='unpublishable result' \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  pass "dj-startup-network: a report-publication failure is failed, diagnosed, and still wakes"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" \
    DJ_FAKE_BOOTSTRAP_OUT='BOOTSTRAP_INFO: fixture completed benign work' \
    run_stage "$home" "$root" run --locked 0
  [ ! -s "$home/state/.wake-queue" ] \
  ...
  pass "dj-startup-network: silent and explicitly informational successes never queue a main-blocking wake"
}
...
EOF
  ...
  pass "dj-startup-network: an actionable state=done report still queues a wake"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" run --locked 1 --lock-pid 111111
  ...
  : > "$log"
  printf '%s\n' $$ > "$home/state/.lock"
  DJ_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "dj-startup-network: manual callers cannot forge mutation authority"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=20 DJ_STARTUP_NETWORK_TIMEOUT=2 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  run_stage "$home" "$root" harvest --pid $$ >/dev/null
  DJ_STARTUP_NETWORK_TIMEOUT=2 run_stage "$home" "$root" wait 10 >/dev/null \
  ...
  assert_contains "$report" "dj-startup-network.sh run --locked 1" \
  ...
  pass "dj-startup-network: an aggregate bound turns a wedged sweep into an actionable line"
}
...
phases=probe,sweeps
EOF
  assert_contains "$(DJ_STARTUP_NETWORK_TIMEOUT=10 run_stage "$home" "$root" report)" \
  ...
  pass "dj-startup-network: an abandoned run reports as needing a rerun, never as in progress forever"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=6 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  await_worker_record "$home"
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=6 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "dj-startup-network: a second start never launches a competing worker"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=5 \
  ...
  pass "dj-startup-network: start atomically reserves the generation harvest observes"
}
...
EOF
  ...
  DJ_FAKE_HARNESS_PID_OVERRIDE="$next_owner" DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=1 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "dj-startup-network: a new lock owner gets a distinct worker generation"
}
...
EOF
  ...
  started=$(date +%s)
  rc=0
  out=$(PATH="$root/bin:$PATH" DJ_FAKE_HARNESS_PID="$next_owner" \
    DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" "$root/bin/dj-lock.sh" 2>&1) || rc=$?
  elapsed=$(( $(date +%s) - started ))
  ...
  out=$(PATH="$root/bin:$PATH" DJ_FAKE_HARNESS_PID="$next_owner" \
    DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" "$root/bin/dj-lock.sh" 2>&1) \
  ...
  pass "dj-startup-network: fleet-lock takeover cannot overlap a mutating sweep"
}
...
test_records_share_one_origin_so_offsets_form_a_timeline() {
  ...
  (
    # shellcheck source=bin/dj-timing-lib.sh
    . "$ROOT/bin/dj-timing-lib.sh"
    unset DJ_TIMING_EPOCH_MS
    DJ_TIMING_LOG=$log
    export DJ_TIMING_LOG
    base=$(dj_timing_now_ms)
    dj_timing_record phase first "$base"
    dj_timing_record phase second "$(( base + 5000 ))"
    dj_timing_record phase third "$(( base + 9000 ))"
  )
  ...
  pass "dj-startup-network: timing records share one origin so their offsets form a timeline"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_OUT='sweep finding' \
    DJ_FAKE_TIMING_PHASE=fleet-sync DJ_FAKE_TIMING_DETAIL=dotfiles-private \
  ...
  assert_not_contains "$harvest_out" "slowest:" \
    "the timings leaked into the session-start digest section"
  pass "dj-startup-network: timings are durable and printed only on demand"
}
...
EOF
  ...
  DJ_STARTUP_NETWORK_TIMEOUT=1 DJ_SESSION_START_TIMEOUT=2 \
    DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_BOOTSTRAP_SLEEP=20 \
    DJ_FAKE_TIMING_PHASE=secondmate-liveness DJ_FAKE_TIMING_DETAIL='mate-a@host-one' \
    run_stage "$home" "$root" run --locked 1
  ...
  pass "dj-startup-network: a timed-out run still publishes the partial timings it recorded"
}
...
EOF
  ...
  DJ_FAKE_BOOTSTRAP_LOG="$log" DJ_FAKE_TIMING_PHASE=secondmate-sync \
    DJ_FAKE_TIMING_DETAIL="ssh -i /key host	v1	forged	0	9999
GITHUB_TOKEN=ghp_supersecretvalue" \
  ...
  pass "dj-startup-network: the timing artifact cannot carry a command line or forge records"
}
...
test_a_bounded_run_still_publishes_the_timings_it_managed_to_record
test_the_timing_artifact_cannot_carry_a_command_line_or_forge_records
echo "# dj-startup-network.test.sh: all assertions passed"
