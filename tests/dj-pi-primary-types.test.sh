#!/usr/bin/env bash
# Strict no-emit contract check for the tracked Jarvis Pi extensions.
set -u
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/dj-pi-primary-types.XXXXXX")
cleanup() {
  rm -rf "$TMP_ROOT"
}
trap cleanup EXIT
...
cp "$ROOT/.pi/extensions/dj-branch-supervision.ts" "$TMP_ROOT/dj-branch-supervision.ts"
cp "$ROOT/.pi/extensions/dj-calm.ts" "$TMP_ROOT/dj-calm.ts"
cp "$ROOT/.pi/extensions/dj-primary-pi-watch.ts" "$TMP_ROOT/dj-primary-pi-watch.ts"
cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$TMP_ROOT/dj-primary-turnend-guard.ts"
cp "$ROOT/.pi/extensions/lib/dj-branch-dispatch.ts" "$TMP_ROOT/lib/dj-branch-dispatch.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-assistant-layout.ts" "$TMP_ROOT/lib/dj-calm-assistant-layout.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-operational-user-layout.ts" "$TMP_ROOT/lib/dj-calm-operational-user-layout.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-visibility.ts" "$TMP_ROOT/lib/dj-calm-visibility.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-working-ship.ts" "$TMP_ROOT/lib/dj-calm-working-ship.ts"
cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$TMP_ROOT/lib/dj-operational-input.ts"
