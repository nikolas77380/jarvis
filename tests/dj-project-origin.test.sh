#!/usr/bin/env bash
# tests/dj-project-origin.test.sh - which project origins seeding accepts.
#
# Jarvis supplies a project's origin instead of discovering it from a local
...
. "$ROOT/bin/dj-project-origin-lib.sh"

TMP_ROOT=$(dj_test_tmproot dj-project-origin)
FAKEBIN=$(dj_fakebin "$TMP_ROOT/fake")

accepts() {
  dj_project_origin_safe "$1" || fail "refused an ordinary clone URL: $1"
}
refuses() {
  ! dj_project_origin_safe "$1" || fail "accepted an origin git must never be handed: $1"
}

dj_git_init_commit "$TMP_ROOT/source"
git clone --quiet --bare "$TMP_ROOT/source" "$TMP_ROOT/source.git"

# Jarvis is a shared template, so acceptance is decided by structure alone.
