...
# tests/dj-backend-herdr.test.sh's stateful fake: workspace create seeds one
...
case "${1:-} ${2:-}" in
  ...
  "agent get")
    ...
    ;;
  "session list"*)
    printf '{"sessions":[{"name":"default","running":true,"socket_path":"%s"},{"name":"dj-remote","running":true,"socket_path":"%s"}]}\n' "$SOCKET" "$SOCKET" ;;
esac
exit 0
SH
  chmod +x "$script"
