...
SNAPSHOT="$ROOT/bin/dj-fleet-snapshot.sh"
VIEW="$ROOT/bin/dj-fleet-view.sh"
TMP_ROOT=$(dj_test_tmproot dj-fleet-snapshot)
...
EOF
  ...
  # (bin/dj-busy-lib.sh), which is what the snapshot's current-state read
  # consults; rendered pane text is no longer a state source.
  fixture_gen=$("$ROOT/bin/dj-busy-event.sh" arm "$home/state" ship-task)
  "$ROOT/bin/dj-busy-event.sh" apply "$home/state" ship-task busy --gen "$fixture_gen" \
    --source claude-hook --event user-prompt-submit
  dj_write_meta "$home/state/scout-task.meta" \
    "window=jarvis:dj-scout-task" \
  ...
  dj_write_meta "$home/state/secondmate-task.meta" \
    "window=jarvis:dj-secondmate-task" \
  ...
  dj_write_meta "$home/state/cmux-task.meta" \
    "backend=cmux" \
...
test_empty_fleet_json() {
  ...
  out=$(DJ_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
...
test_fixture_snapshot_json() {
  ...
  printf '%s' "$out" | jq -e '
    ...
    | .secondmate_projects == ["alpha","beta","gamma"]
      ...
      and (.actions.watch | contains("do not routinely dj-peek"))
...
## Done
EOF
  dj_write_meta "$home/state/worker.meta" \
    "window=jarvis:dj-worker" "worktree=$home/projects/worker" "project=alpha" \
...
test_event_hints_follow_reconciled_current_state() {
  ...
  dj_write_meta "$home/state/active-decision.meta" \
    "window=jarvis:dj-active-decision" \
  ...
  dj_write_meta "$home/state/active-blocked.meta" \
    "window=jarvis:dj-active-blocked" \
  ...
  dj_write_meta "$home/state/stale-decision.meta" \
    "window=jarvis:dj-stale-decision-ship-task" \
    "worktree=$home/projects/stale-decision" \
    "project=alpha" \
    "harness=claude" \
    "kind=ship" \
    "mode=ship"
  hint_gen=$("$ROOT/bin/dj-busy-event.sh" arm "$home/state" stale-decision)
  "$ROOT/bin/dj-busy-event.sh" apply "$home/state" stale-decision busy --gen "$hint_gen" \
    --source claude-hook --event user-prompt-submit
  printf 'needs-decision: already answered\n' > "$home/state/stale-decision.status"
  dj_write_meta "$home/state/stale-blocked.meta" \
    "window=jarvis:dj-stale-blocked-ship-task" \
    "worktree=$home/projects/stale-blocked" \
    "project=alpha" \
    "harness=claude" \
    "kind=ship" \
    "mode=ship"
  hint_gen=$("$ROOT/bin/dj-busy-event.sh" arm "$home/state" stale-blocked)
  "$ROOT/bin/dj-busy-event.sh" apply "$home/state" stale-blocked busy --gen "$hint_gen" \
    --source claude-hook --event user-prompt-submit
...
EOF
  ...
  dj_write_meta "$home/state/bold-task.meta" \
    "window=jarvis:dj-bold-task" \
  ...
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_DATA_OVERRIDE="$data" DJ_PROJECTS_OVERRIDE="$projects" \
    DJ_SNAPSHOT_NOW=2026-07-14T00:00:00Z "$SNAPSHOT" --json)
  ...
  view=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_DATA_OVERRIDE="$data" DJ_PROJECTS_OVERRIDE="$projects" "$VIEW")
...
test_view_renders_snapshot() {
  ...
  write_fixture "$home"
  fakebin=$(make_fakebin "$home")
  view=$(PATH="$fakebin:$PATH" DJ_HOME="$home" "$VIEW")
  ...
  assert_contains "$view" "bin/dj-send.sh dj-secondmate-task" \
  ...
  assert_not_contains "$view" "dj-peek.sh dj-secondmate-task" \
    "view must not tell jarvis to routinely peek secondmates"
...
test_view_renders_dead_secondmate_agent_status() {
  ...
  dj_write_meta "$home/state/dead-secondmate.meta" \
    "window=jarvis:dj-dead-secondmate" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
    "mode=secondmate" \
...
# durable keyed fold (dj-classify-lib.sh) keeps it open until an explicit resolution.
test_open_decision_survives_later_unrelated_event() {
  local home fakebin out
  home=$(make_home masking)
  mkdir -p "$home/secondmate-home"
  dj_write_meta "$home/state/masked-decision.meta" \
    "window=jarvis:dj-masked-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_secondmate_open_decision_survives_live_endpoint() {
  ...
  mkdir -p "$home/secondmate-home"
  dj_write_meta "$home/state/active-secondmate.meta" \
    "window=jarvis:dj-active-secondmate" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_open_decision_transfers_to_captain_hold() {
  ...
  mkdir -p "$home/secondmate-home"
  dj_write_meta "$home/state/transferred-decision.meta" \
    "window=jarvis:dj-transferred-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_open_decision_clears_on_keyed_resolution() {
  ...
  mkdir -p "$home/secondmate-home"
  dj_write_meta "$home/state/resolved-decision.meta" \
    "window=jarvis:dj-resolved-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_parked_scout_decision_stays_pending() {
  ...
  dj_write_meta "$home/state/parked-scout.meta" \
    "window=jarvis:dj-parked-scout" \
