...
if [ "${DJ_PI_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_PI_LIVE_E2E=1 to run the isolated interactive Pi regression"
  exit 0
fi
...
LEGACY_START='Run `bin/dj-session-start.sh` now, exactly once, before executing any other instructions.'
...
# shellcheck disable=SC2016 # Backticks are literal prompt markup.
START_NEAR_MISS='Captain quote: Run `bin/dj-session-start.sh` now, exactly once, before executing any other instructions.'
dj_operational_input_encode watcher "CURRENT_AHOY_WATCHER_BODY" CURRENT_WATCHER \
  || fail "could not construct current Ahoy watcher fixture"
QUOTED_CURRENT="Captain quote: $CURRENT_WATCHER"
ASCII_ONLY='JARVIS_OP: v1 watcher: captain-authored text'
...
run_native_ahoy_regressions() {
  ...
  cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$AHOY_PROJECT/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$AHOY_PROJECT/.pi/extensions/lib/"
  cp \
  ...
  chmod +x "$AHOY_PROJECT/bin/dj-sessionstart-nudge.sh"
  # shellcheck disable=SC2016 # Variables expand in the generated script, not this test shell.
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    'set -u' \
    'file="${DJ_HOME:?}/state/session-start-count"' \
    'count=0' \
    '[ ! -f "$file" ] || count=$(sed -n "1p" "$file")' \
    'count=$((count + 1))' \
    'printf "%s\n" "$count" > "$file"' \
    'printf "SESSION_START_DONE count=%s\n" "$count"' \
    > "$AHOY_PROJECT/bin/dj-session-start.sh"
  chmod +x "$AHOY_PROJECT/bin/dj-session-start.sh"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  printf '%s\n' \
    '---' \
    'name: bearings' \
  ...
  first_out=$(
    cd "$AHOY_PROJECT" &&
      DJ_HOME="$first_home" pi --print --approve --no-session --no-context-files --no-extensions \
        -e .pi/extensions/dj-primary-turnend-guard.ts \
  ...
  later_out=$(
    cd "$AHOY_PROJECT" &&
      DJ_HOME="$later_home" pi --print --approve --no-session --no-context-files --no-extensions \
        -e .pi/extensions/dj-primary-turnend-guard.ts \
        --no-skills --skill .agents/skills \
        --model openai-codex/gpt-5.6-sol --thinking low \
...
cp "$ROOT/.pi/extensions/dj-calm.ts" "$PROJECT/.pi/extensions/dj-calm.ts"
cp "$ROOT/.pi/extensions/dj-primary-pi-watch.ts" "$PROJECT/.pi/extensions/dj-primary-pi-watch.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-assistant-layout.ts" "$PROJECT/.pi/extensions/lib/dj-calm-assistant-layout.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-operational-user-layout.ts" "$PROJECT/.pi/extensions/lib/dj-calm-operational-user-layout.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-visibility.ts" "$PROJECT/.pi/extensions/lib/dj-calm-visibility.ts"
cp "$ROOT/.pi/extensions/lib/dj-calm-working-ship.ts" "$PROJECT/.pi/extensions/lib/dj-calm-working-ship.ts"
...
cp "$ROOT/bin/dj-watch-arm.sh" "$PROJECT/bin/dj-watch-arm.sh"
cp "$ROOT/bin/dj-operational-input.sh" "$PROJECT/bin/dj-operational-input.sh"
cp "$ROOT/bin/dj-supervision-instructions.sh" "$PROJECT/bin/dj-supervision-instructions.sh"
chmod +x "$PROJECT/bin/dj-operational-input.sh"
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config"

"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env DJ_HOME='$HOME_DIR' DJ_ROOT_OVERRIDE='$PROJECT' DJ_POLL=1 DJ_SIGNAL_GRACE=0 DJ_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$DJ_HOME/state/.lock\"; pi --approve --no-session --no-context-files --no-extensions -e .pi/extensions/dj-calm.ts -e .pi/extensions/dj-primary-turnend-guard.ts -e .pi/extensions/dj-primary-pi-watch.ts --model openai-codex/gpt-5.6-sol --thinking low; rc=\$?; printf \"PI_EXIT=%s\\n\" \"\$rc\"; sleep 300'"
...
send_prompt "Start supervision with dj_watch_arm_pi and never use bash to arm supervision. After the watcher wake arrives, run bin/dj-wake-drain.sh and reply exactly HANDLED."
...
foreground_arm='$ bin/dj-watch-arm.sh'
