#!/usr/bin/env bash
# tests/dj-trace-context-lib.test.sh - unit tests for the native, default-off
# W3C trace-context library (bin/dj-trace-context-lib.sh) plus structural checks
# that bin/dj-spawn.sh wires it in at the pre-launch injection seam and that the
...
dj_trace_context_valid "$VALID" || fail "a conformant traceparent must validate"
pass "dj_trace_context_valid accepts a conformant W3C traceparent"
...
for bad in \
  ...
  '' ; do
  if dj_trace_context_valid "$bad"; then
    fail "invalid traceparent wrongly accepted: '$bad'"
  fi
done
pass "dj_trace_context_valid rejects all-zero ids, ff version, uppercase, wrong length, missing field, shell metacharacters, and empty"
...
dollar='$'
dj_trace_context_valid "${dollar}(touch pwned-$$)" && fail "command-substitution-shaped value wrongly accepted"
...
t=$(dj_trace_context_hex 16)
[ "${#t}" -eq 32 ] || fail "16-byte hex must be 32 chars, got ${#t}"
case "$t" in *[!0-9a-f]*) fail "trace hex is not lowercase hex: $t" ;; esac
s=$(dj_trace_context_hex 8)
[ "${#s}" -eq 16 ] || fail "8-byte hex must be 16 chars, got ${#s}"
[ "$(dj_trace_context_hex 8)" != "$(dj_trace_context_hex 8)" ] || fail "hex must be fresh per call"
pass "dj_trace_context_hex yields exact-length lowercase hex, distinct per call"

# --- root mint ---------------------------------------------------------------

ROOT_TP=$(dj_trace_context_mint)
dj_trace_context_valid "$ROOT_TP" || fail "root mint must be a valid traceparent: $ROOT_TP"
...
pass "dj_trace_context_mint starts a valid sampled root trace"
...
SECOND_TP=$(dj_trace_context_mint)
dj_trace_context_valid "$SECOND_TP" || fail "second mint must be a valid traceparent: $SECOND_TP"
...
# --- minted-root shape --------------------------------------------------------
# A jarvis-MINTED root is exactly the fixed 55-char W3C form with random ids
# and no free-form field where jarvis could originate a prompt, path, or
...
pass "a minted root is the fixed 55-char W3C form (hex and hyphens only), so jarvis originates no free-form content in the carrier"

# --- enablement precedence ---------------------------------------------------

WORK=$(dj_test_tmproot dj-trace-context)
CFG_ON="$WORK/cfg-on"; CFG_OFF="$WORK/cfg-off"
mkdir -p "$CFG_ON" "$CFG_OFF"
: > "$CFG_ON/trace-context"

unset DJ_TRACE_CONTEXT
dj_trace_context_enabled "$CFG_OFF" && fail "absent config/trace-context must be off by default"
dj_trace_context_enabled "$CFG_ON" || fail "present config/trace-context must enable"
DJ_TRACE_CONTEXT=off dj_trace_context_enabled "$CFG_ON" && fail "DJ_TRACE_CONTEXT=off must override a present file"
DJ_TRACE_CONTEXT=on dj_trace_context_enabled "$CFG_OFF" || fail "DJ_TRACE_CONTEXT=on must override an absent file"
DJ_TRACE_CONTEXT=1 dj_trace_context_enabled "$CFG_OFF" || fail "DJ_TRACE_CONTEXT=1 must enable"
DJ_TRACE_CONTEXT=maybe dj_trace_context_enabled "$CFG_ON" && fail "a non-truthy DJ_TRACE_CONTEXT must disable"
DJ_TRACE_CONTEXT='' dj_trace_context_enabled "$CFG_ON" || fail "empty DJ_TRACE_CONTEXT must defer to a present file (enabled)"
DJ_TRACE_CONTEXT='' dj_trace_context_enabled "$CFG_OFF" && fail "empty DJ_TRACE_CONTEXT must defer to an absent file (disabled)"
pass "enablement is default-off; DJ_TRACE_CONTEXT overrides with truthy/other precedence, and unset or empty defers to config/trace-context"

SESSION_DIR="$WORK/session-state"
SESSION_STATE="$SESSION_DIR/.trace-context-effective"
mkdir -p "$SESSION_DIR"
printf '101\n' > "$SESSION_DIR/.lock"
DJ_TRACE_CONTEXT=off dj_trace_context_session_start "$CFG_ON" "$SESSION_STATE"
[ "$(dj_trace_context_session_effective "$SESSION_STATE")" = off ] \
  || fail "session state must freeze an env-off override over a present config file"
DJ_TRACE_CONTEXT=on dj_trace_context_session_start "$CFG_OFF" "$SESSION_STATE"
[ "$(dj_trace_context_session_effective "$SESSION_STATE")" = on ] \
...
out=$(dj_trace_context_resolve "$CFG_OFF" "$NOMETA"); rc=$?
...
out=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_OFF" "$NOMETA")
dj_trace_context_valid "$out" || fail "enabled resolve must mint a valid traceparent: $out"
pass "resolve mints a valid traceparent when enabled"

# --- secondmate home-session boundary ---------------------------------------
# dj-spawn launches every Secondmate with the primary session's non-empty frozen
# DJ_TRACE_CONTEXT decision. The Secondmate resolves it at its own session start.
...
unset TRACEPARENT
frozen_off=$(DJ_TRACE_CONTEXT=off dj_trace_context_resolve "$CFG_ON" "$WORK/sm-frozen-off.meta")
[ -z "$frozen_off" ] || fail "a Secondmate launched off must stay disabled even after the config file appears: $frozen_off"
frozen_on=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_OFF" "$WORK/sm-frozen-on.meta")
dj_trace_context_valid "$frozen_on" || fail "a Secondmate launched on must stay enabled even while the config file is absent: $frozen_on"
[ "${frozen_on:3:32}" != "${PRIMARY_TP:3:32}" ] || fail "an enabled Secondmate without an ambient carrier must start a new root"
routed_a=$(TRACEPARENT="$PRIMARY_TP" DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$WORK/sm-routed-a.meta")
routed_b=$(TRACEPARENT="$PRIMARY_TP" DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$WORK/sm-routed-b.meta")
dj_trace_context_valid "$routed_a" || fail "resolving under an ambient TRACEPARENT must still mint a valid carrier (a='$routed_a')"
dj_trace_context_valid "$routed_b" || fail "resolving under an ambient TRACEPARENT must still mint a valid carrier (b='$routed_b')"
...
out=$(TRACEPARENT='00-ffffffffffffffffffffffffffffffff-1111111111111111-01' \
  DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$REC_META")
...
out=$(dj_trace_context_resolve "$CFG_OFF" "$REC_META")
...
out=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$CORRUPT_META")
dj_trace_context_valid "$out" || fail "a corrupt recorded value must be re-minted to a valid one"
...
out=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$NOMETA")
dj_trace_context_valid "$out" || fail "resolve must yield a single valid carrier per call"
...
dj_trace_context_hex() { return 1; }
ef_mint=$(dj_trace_context_mint); ef_mint_rc=$?
ef_res=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CFG_ON" "$NOMETA"); ef_res_rc=$?
...
assert_no_grep 'sleep' "$ROOT/bin/dj-trace-context-lib.sh" "trace-context lib must not sleep on the spawn path"
assert_no_grep 'timeout' "$ROOT/bin/dj-trace-context-lib.sh" "trace-context lib must not depend on an external timeout"
assert_no_grep 'command:' "$ROOT/bin/dj-trace-context-lib.sh" "trace-context lib must not run an arbitrary command provider"
dj_trace_context_resolve "$CFG_OFF" "$NOMETA" >/dev/null || fail "resolve must return 0 when off"
...
LIB_CODE=$(sed 's/#.*$//' "$ROOT/bin/dj-trace-context-lib.sh")
...
case " $DJ_INHERITABLE_CONFIG " in
  *" trace-context "*) : ;;
  *) fail "config/trace-context must be in DJ_INHERITABLE_CONFIG so secondmate homes stay traced" ;;
esac
pass "config/trace-context is inherited into secondmate homes, keeping the nested chain enabled end to end"

echo "# dj-trace-context-lib.test.sh: all assertions passed"
