...
# PRIMARY jarvis checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/dj-spawn.sh
# (before launching a secondmate) and bin/dj-bootstrap.sh (a startup sweep of
...
TMP_ROOT=$(dj_test_tmproot dj-secondmate-sync)
export DJ_BACKEND=tmux

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY jarvis repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps dj-guard quiet for the spawn path.
...
# the tracked .gitignore and commit it on main. The marker (.dj-secondmate-home)
# is jarvis-generic, written by bin/dj-home-seed.sh into every seeded home; once
...
case "$*" in
  list-windows*)
    sed -n 's/^window=[^:]*://p' "${DJ_HOME:?}"/state/*.meta
    exit 0
    ;;
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.dj-secondmate-home"
  ...
  info_line=$(printf '%s\n' "$out" | grep '^BOOTSTRAP_INFO: nudged dj-sm-instr ' || true)
  [ -n "$info_line" ] || fail "no BOOTSTRAP_INFO nudge line emitted (got: $out)"
  assert_contains "$info_line" "jarvis was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
    "successful nudge report should include the exact message sent"
  assert_not_contains "$out" "NUDGE_SECONDMATES:" "successful nudge must not leave a jarvis action item"
  ...
  # The nudge rides dj-send's durable inbox plane: the marked message lands in
  # the secondmate task's steering-inbox record while only the doorbell is typed.
  assert_contains "$(cat "$w/home/state/sm-instr.inbox/001.msg")" "[dj-from-jarvis]" \
    "nudge send should use the marked dj-send secondmate path"
  assert_contains "$(cat "$w/home/state/sm-instr.inbox/001.msg")" "jarvis was updated to the latest - please re-read your AGENTS.md" \
    "nudge send should enqueue the exact re-read message"
  assert_contains "$(cat "$log")" "Jarvis instruction waiting" \
...
test_bootstrap_nudge_send_uses_state_override() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$w/home" DJ_ROOT_OVERRIDE="$w/main" \
    DJ_STATE_OVERRIDE="$override_state" DJ_SEND_SETTLE=0 DJ_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)

  assert_contains "$out" "BOOTSTRAP_INFO: nudged dj-sm-instr with" \
    "nudge send should resolve dj-sm-instr through the effective state dir"
  assert_not_contains "$out" "NUDGE_SECONDMATES:" \
    "effective-state nudge should not fail through DJ_HOME/state"
  assert_contains "$(cat "$override_state/sm-instr.inbox/001.msg")" "[dj-from-jarvis]" \
  ...
  pass "T8a bootstrap nudge send respects DJ_STATE_OVERRIDE"
}
...
test_bootstrap_nudge_retry_rejects_malformed_marker_id() {
  ...
  printf '../escape\n' > "$evil/.dj-secondmate-home"
  mkdir -p "$w/home/state/.secondmate-nudge-pending"
  marker="$w/home/state/.secondmate-nudge-pending/bad.pending"
  {
    printf 'id=../escape\n'
    printf 'selector=dj-../escape\n'
    printf 'home=%s\n' "$evil"
    printf 'commit=%s\n' "$c1"
    printf 'instructions=AGENTS.md\n'
    printf 'message=jarvis was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.\n'
  } > "$marker"
  {
    printf 'window=jarvis:dj-evil\n'
    printf 'kind=secondmate\n'
    printf 'home=%s\n' "$evil"
  } > "$w/home/escape.meta"
  fakebin=$(make_fake_toolchain "$w")
  ...
  assert_not_contains "$out" "BOOTSTRAP_INFO: nudged dj-../escape" \
  ...
  assert_absent "$log" "malformed retry marker should not invoke dj-send"
...
test_bootstrap_nudge_failure_records_retry_marker() {
  ...
  assert_grep "selector=dj-sm-instr" "$marker" "retry marker should pin the stable selector"
  assert_grep "message=jarvis was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
...
test_bootstrap_nudge_retry_is_idempotent() {
  ...
  out2=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$w/home" DJ_ROOT_OVERRIDE="$w/main" \
    DJ_SEND_SETTLE=0 "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
...
test_bootstrap_nudge_retry_refuses_changed_home() {
  ...
  printf '%s\n' sm-instr > "$other/.dj-secondmate-home"
...
# --- T8b: stale herdr nudge failures retry through current dj-<id> metadata ---
...
# (default:wA:p2), so dj-send with the printed target fell back to tmux and failed
# while dj-<id> resolved through current meta.
...
SH
  chmod +x "$spawn_stub"
  cp "$spawn_stub" "$w/main/bin/dj-spawn.sh"
  ...
  out=$(PATH="$herdrfb:$toolchain:$BASE_PATH" HERDR_ENV=1 DJ_BACKEND=herdr \
    DJ_SEND_SETTLE=0 \
    DJ_HOME="$w/home" DJ_ROOT_OVERRIDE="$w/main" \
    "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
  ...
  assert_contains "$out" "BOOTSTRAP_INFO: nudged dj-sm-instr" \
  ...
  resolved=$(bash -c '. "$0/bin/dj-backend.sh"; dj_backend_resolve_selector dj-sm-instr "$1"' "$ROOT" "$w/home/state")
  [ "$resolved" = "$fresh" ] || fail "dj-<id> should resolve through post-respawn meta, got '$resolved'"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  stale_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/dj-backend.sh"; dj_backend_source herdr; dj_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$stale" 2>/dev/null; printf '%s' "$?")
  [ "$stale_send" != 0 ] || fail "explicit stale herdr endpoint send should fail"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  fresh_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/dj-backend.sh"; dj_backend_source herdr; dj_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$fresh" 2>/dev/null; printf '%s' "$?")
  [ "$fresh_send" = 0 ] || fail "send through dj-<id>-resolved fresh endpoint should succeed"

  pass "T8b a stale herdr endpoint cannot lose a durably enqueued nudge, and dj-<id> resolves through post-respawn metadata"
}
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    DJ_ROOT_OVERRIDE="$w/main" DJ_HOME="$w/home" \
    ...
    DJ_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/dj-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>&1 || true
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    DJ_PROJECTS_OVERRIDE="$w/home/projects" DJ_CONFIG_OVERRIDE="$w/home/config" \
    DJ_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/dj-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>"$err" || true
...
# The seed marker used to leave every home permanently dirty: bin/dj-fleet-sync.sh
...
test_seed_marker_clean_when_gitignored() {
  ...
  # The exact dirtiness signal bin/dj-fleet-sync.sh reads (its line: dirty=yes when
...
# Standalone-clone homes converge through /updatejarvis's origin fetch instead.
...
echo "# all dj-secondmate-sync tests passed"
