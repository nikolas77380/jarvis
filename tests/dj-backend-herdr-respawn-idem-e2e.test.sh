#!/usr/bin/env bash
# tests/dj-backend-herdr-respawn-idem-e2e.test.sh - isolated real-herdr
# regression test for jarvis-restart idempotency against herdr's
...
# restored dj-<id> task tab came back a husk - a dead pane, or a plain
# agent-less shell in the saved cwd - and bin/backends/herdr.sh's
# dj_backend_herdr_create_task refused to spawn into it unconditionally,
...
# dj_backend_herdr_create_task now closes-and-replaces the resulting husk
...
# before. Adapter-level (dj_backend_herdr_container_ensure/create_task), not
# through the full bin/dj-spawn.sh + treehouse pipeline - mirrors
# tests/dj-backend-herdr-prune-safety-e2e.test.sh's own style, and avoids any
...
SESSION="dj-lab-respawn-idem-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/dj-herdr-respawn-idem.XXXXXX")
cleanup_all() {
...
# dj_backend_herdr_create_task is the ONE function both bin/dj-spawn.sh's
...
# only in DJ_HOME-shadowing (tests/dj-backend-herdr-workspace-per-home-e2e.test.sh),
# never in this duplicate-guard logic, which has no home-specific branching.

PROJ_CWD="$SCRATCH/proj"
mkdir -p "$PROJ_CWD"

RAW=$(dj_backend_herdr_container_ensure "$PROJ_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
WSID=${CONTAINER#*:}

CREW_LABEL="dj-respawn-crew1"
CREW_IDS=$(dj_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" "$SEEDED_TAB_ID") \
...
SM_LABEL="dj-respawn-sm1"
SM_IDS=$(dj_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
pass "repro setup: two real dj-<id> task tabs exist (crewmate-shaped and secondmate-shaped), neither with a registered agent"
...
sleep 0.5
dj_backend_herdr_server_ensure "$SESSION" || fail "the isolated session's server did not come back up after the restart"
...
RESPAWN_CREW_IDS=$(dj_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD") \
...
RESPAWN_SM_IDS=$(dj_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
herdr pane report-agent "$NEW_CREW_PANE_ID" --source dj-respawn-e2e --agent dj-respawn-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the respawned crewmate-shaped pane"

if dj_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" >/dev/null 2>&1; then
...
dj_backend_herdr_kill "$SESSION:$NEW_CREW_PANE_ID"
dj_backend_herdr_kill "$SESSION:$NEW_SM_PANE_ID"
