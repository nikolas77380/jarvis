...
TMP_ROOT=$(dj_test_tmproot dj-on)
...
cp "$ROOT/bin/dj-remote-entrypoint.sh" "$ROOT/bin/dj-remote-job-lib.sh" \
  "$ROOT/bin/dj-remote-job-worker.sh" "$REMOTE_ROOT/bin/"

cat > "$REMOTE_ROOT/bin/dj-probe-one.sh" <<'SH'
#!/usr/bin/env bash
set -u
out=$1
rc=$2
...
exit "$rc"
SH
cat > "$REMOTE_ROOT/bin/dj-probe-two.sh" <<'SH'
#!/usr/bin/env bash
printf 'home=%s\nroot=%s\nworker=%s\n' "$DJ_HOME" "$DJ_ROOT_OVERRIDE" "${DJ_REMOTE_JOB_ACTIVE:-}"
if [ -n "${TOP_SECRET:-}" ]; then printf 'secret=leaked\n'; else printf 'secret=absent\n'; fi
SH
cat > "$REMOTE_ROOT/bin/dj-probe-path.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$PATH"
SH
cat > "$REMOTE_ROOT/bin/tasks-axi" <<SH
#!/usr/bin/env bash
printf '%s\n' "\${DJ_REMOTE_JOB_ACTIVE:-absent}" >> "$TOOL_PROBE_LOG"
case "\${1:-}:\${2:-}" in
...
SH
cp "$ROOT/bin/dj-remote-doctor.sh" "$ROOT/bin/dj-tasks-axi-lib.sh" \
  "$ROOT/bin/dj-backend.sh" "$REMOTE_ROOT/bin/"
mkdir -p "$REMOTE_ROOT/bin/backends"
cp "$ROOT/bin/backends/herdr.sh" "$REMOTE_ROOT/bin/backends/herdr.sh"
cat > "$REMOTE_ROOT/bin/dj-mutate.sh" <<'SH'
#!/usr/bin/env bash
printf 'mutation\n' >> "$1"
SH
...
printf '%s\0' 'plain' 'two words' '$(touch /tmp/dj-on-injected)' '' $'line one\nline two' > "$ARGV_EXPECTED"
printf 'payload one\npayload two\n' > "$TMP_ROOT/stdin"
set +e
# shellcheck disable=SC2016 # Literal shell-looking argv is the injection probe.
dj_on ios dj-probe-one.sh "$ARGV_ACTUAL" 23 \
  'plain' 'two words' '$(touch /tmp/dj-on-injected)' '' $'line one\nline two' \
...
assert_absent /tmp/dj-on-injected "shell-looking argv was interpreted"
pass "dj-on preserves argv, stdin, stdout, stderr, and exit status without shell interpretation"
...
# this on the real ssh argv the DJ_SSH_BIN process seam captured, never on
# dj-on.sh source text.
...
pass "dj-on arms a bounded SSH dead-peer detection window by default (${DEFAULT_INTERVAL}s x $DEFAULT_COUNT = ${DEFAULT_WINDOW}s)"

: > "$SSH_LOG"
DJ_SSH_ALIVE_INTERVAL=7 DJ_SSH_ALIVE_COUNT_MAX=2 dj_on ios dj-probe-two.sh >/dev/null
OVERRIDE_ARGV=$(tail -n 1 "$SSH_LOG")
assert_contains "$OVERRIDE_ARGV" 'ServerAliveInterval=7' "DJ_SSH_ALIVE_INTERVAL override was not honored on the ssh transport"
assert_contains "$OVERRIDE_ARGV" 'ServerAliveCountMax=2' "DJ_SSH_ALIVE_COUNT_MAX override was not honored on the ssh transport"
pass "dj-on's dead-peer detection window is env-overridable"

SSH_CALLS_BEFORE_INVALID=$(cat "$SSH_COUNT")
set +e
INVALID_INTERVAL_OUT=$(DJ_SSH_ALIVE_INTERVAL=0 dj_on ios dj-probe-two.sh 2>&1)
INVALID_INTERVAL_RC=$?
INVALID_COUNT_OUT=$(DJ_SSH_ALIVE_COUNT_MAX=not-a-number dj_on ios dj-probe-two.sh 2>&1)
INVALID_COUNT_RC=$?
set -e
[ "$INVALID_INTERVAL_RC" -eq 1 ] || fail "a zero DJ_SSH_ALIVE_INTERVAL was accepted (got exit $INVALID_INTERVAL_RC)"
[ "$INVALID_COUNT_RC" -eq 1 ] || fail "a non-integer DJ_SSH_ALIVE_COUNT_MAX was accepted (got exit $INVALID_COUNT_RC)"
assert_contains "$INVALID_INTERVAL_OUT" 'DJ_SSH_ALIVE_INTERVAL must be a positive integer' "invalid interval did not explain its constraint"
assert_contains "$INVALID_COUNT_OUT" 'DJ_SSH_ALIVE_COUNT_MAX must be a positive integer' "invalid count did not explain its constraint"
[ "$(cat "$SSH_COUNT")" -eq "$SSH_CALLS_BEFORE_INVALID" ] || fail "invalid keepalive configuration launched ssh"
pass "dj-on rejects invalid dead-peer settings before launching ssh"

out=$(TOP_SECRET='must-not-cross' dj_on remote-mac dj-probe-two.sh)
assert_contains "$out" "home=$REMOTE_HOME" "remote DJ_HOME was not explicit"
...
CHILD_PATH=$(dj_on ios dj-probe-path.sh)
NVM_CHILD_DIRS=()
...
set +e
out=$(dj_on ios dj-remote-doctor.sh 2>&1)
set -e
...
# tests/dj-remote-doctor.test.sh owns the verdict against controlled fixtures.
set +e
out=$(dj_on ios dj-remote-doctor.sh 2>/dev/null)
set -e
...
dj_on ios dj-probe-two.sh >/dev/null
: > "$TOOL_PROBE_LOG"
set +e
out=$(dj_on ios dj-remote-doctor.sh 2>&1)
set -e
...
set +e
out=$(HOME="$DOCTOR_HOME" PATH="$DOCTOR_BIN:/usr/bin:/bin:/usr/sbin:/sbin" "$ROOT/bin/dj-remote-doctor.sh" 2>&1)
rc=$?
set -e
...
dj_on ios dj-probe-one.sh "$ARGV_TWO" 0 'second command' >/dev/null 2>/dev/null
cmp -s "$TMP_ROOT/argv-two-expected.bin" "$ARGV_TWO" || fail "second dynamic command did not execute"
pass "multiple dj-*.sh executables work without a command table"

for bad in '../dj-probe-one.sh' 'dj-probe-one.sh/extra' 'sh' 'dj-../../bin/sh'; do
  if dj_on ios "$bad" >/dev/null 2>&1; then
    fail "unsafe command name was accepted: $bad"
  fi
done
ln -s dj-probe-one.sh "$REMOTE_ROOT/bin/dj-symlink.sh"
if dj_on ios dj-symlink.sh >/dev/null 2>&1; then
  fail "a symlinked command was accepted"
fi
cat > "$REMOTE_ROOT/bin/dj-untracked.sh" <<'SH'
#!/usr/bin/env bash
printf 'untracked command ran\n'
SH
chmod +x "$REMOTE_ROOT/bin/dj-untracked.sh"
...
DJ_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" "$REMOTE_ROOT/bin/git" -C "$REMOTE_ROOT" ls-files --error-unmatch bin/dj-untracked.sh \
...
untracked_argv_b64=$(printf '%s\0' dj-untracked.sh | base64 | tr -d '\n')
set +e
out=$(DJ_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  DJ_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" "$REMOTE_ROOT/bin/dj-remote-entrypoint.sh" \
  1 "$untracked_root_b64" "$untracked_home_b64" "$untracked_argv_b64" 2>&1)
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
  fail "an untracked dj-*.sh executable was accepted"
fi
...
set +e
out=$(
  # shellcheck disable=SC2329 # Exported for indirect use by dj_on.
  command() {
    if [ "${1:-}" = -v ] && [ "${2:-}" = git ]; then return 1; fi
    builtin command "$@"
  }
  export -f command
  dj_on ios dj-remote-doctor.sh 2>&1
)
set -e
assert_contains "$out" 'mode=check' "the trusted doctor could not bootstrap while git was unavailable"
printf '\n' >> "$REMOTE_ROOT/bin/dj-remote-doctor.sh"
set +e
out=$(
  # shellcheck disable=SC2329 # Exported for indirect use by dj_on.
  command() {
...
cp "$ROOT/bin/dj-remote-doctor.sh" "$REMOTE_ROOT/bin/dj-remote-doctor.sh"
chmod +x "$REMOTE_ROOT/bin/dj-remote-doctor.sh"
pass "doctor bootstrap remains authenticated when git is unavailable"

if DJ_HOME="$LOCAL_HOME" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_SSH_BIN="$FAKEBIN/fake-ssh" \
  "$ROOT/bin/dj-on.sh" '-oProxyCommand=bad' dj-probe-two.sh >/dev/null 2>&1; then
...
EOF
if dj_on ios dj-probe-two.sh >/dev/null 2>&1; then
...
argv_b64=$(printf '%s\0' dj-probe-two.sh | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/dj-remote-entrypoint.sh" 2 "$root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
  fail "an incompatible transport protocol was accepted"
fi
traversal_root_b64=$(printf '%s' "$REMOTE_ROOT/../remote-root" | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/dj-remote-entrypoint.sh" 1 "$traversal_root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
...
EOF
if dj_on remote-mac dj-probe-two.sh >/dev/null 2>&1; then
...
: > "$SSH_COUNT"
set +e
DJ_FAKE_SSH_MODE=unreachable dj_on ios dj-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
...
: > "$SSH_COUNT"
set +e
DJ_FAKE_SSH_MODE=ambiguous dj_on ios dj-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
