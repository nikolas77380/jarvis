#!/usr/bin/env bash
# tests/dj-tmux-agent-liveness.test.sh - portable regression for the tmux
...
# live per-harness counterpart is tests/dj-harness-liveness-drift-live-e2e.test.sh.
#
...
SOCKET="dj-liveness-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-liveness.XXXXXX")
SESSION=liveness
...
wait_for_state() {  # <target> <expected> [tries]
  ...
  while [ "$i" -lt "$tries" ]; do
    got=$(dj_backend_agent_state tmux "$target")
  ...
  printf 'last verdict for %s was %s (expected %s); title=%s comms=[%s]\n' \
    ...
    "$(dj_backend_tmux_foreground_comms "$target" | tr '\n' ' ')" >&2
  return 1
}
...
title_classifies_agent() {  # <target>
  local name
  name=$(dj_backend_tmux_current_command "$1" 2>/dev/null)
  [ "$(dj_backend_tmux_classify_process_name "$name")" = agent ]
}
...
comms_classify_agent() {  # <target>
  ...
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    [ "$(dj_backend_tmux_classify_process_name "$name")" = agent ] && return 0
  done <<EOF
$(dj_backend_tmux_foreground_comms "$1")
EOF
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    [ "$(dj_backend_tmux_classify_process_name '' "$name")" = agent ] && return 0
  done <<EOF
$(dj_backend_tmux_foreground_argv0s "$1")
EOF
  return 1
}
...
assert_sources_disagree() {  # <target> <label>
  ...
  [ $((t + c)) -eq 1 ] || fail \
    "$2: the two name sources were expected to disagree, but title=$t comms=$c (title='$(dj_backend_tmux_current_command "$1")' comms='$(dj_backend_tmux_foreground_comms "$1" | tr '\n' ' ')')"
}
...
dj_backend_tmux_foreground_comms "$SESSION:no-such-window" >/dev/null \
  || fail "the foreground-comms read must stay best-effort for an absent window"
[ "$(dj_backend_agent_state tmux "$SESSION:no-such-window")" = missing ] \
...
cursor_anchored_verdict() {  # <target>
  local cy pane
  cy=$(dj_tmux_composer_cursor_row "$1")
  pane=$(dj_tmux_composer_capture "$1")
  dj_composer_classify_screen "$(dj_tmux_composer_caps)" "$pane" "$cy"
}

open_composer_pane cursor-idle "$LAB/bin/cursor-agent" 'Plan, search, build anything' 1
dj_tmux_pane_is_cursor "$SESSION:cursor-idle" \
...
[ "$(dj_tmux_composer_state "$SESSION:cursor-idle")" = empty ] \
...
[ "$(dj_tmux_composer_state "$SESSION:cursor-typed")" = pending ] \
...
if dj_tmux_pane_is_cursor "$SESSION:notcursor-idle"; then
  fail "a pane running a non-Cursor binary must not be identified as Cursor"
fi
[ "$(dj_tmux_composer_state "$SESSION:notcursor-idle")" = unknown ] \
...
if dj_tmux_pane_is_cursor "$SESSION:cursor-exited"; then
  fail "a pane whose Cursor process exited must not still identify as Cursor"
fi
[ "$(dj_tmux_composer_state "$SESSION:cursor-exited")" != empty ] \
