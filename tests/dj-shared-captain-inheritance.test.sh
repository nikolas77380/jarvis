...
TMP_ROOT=$(dj_test_tmproot dj-shared-captain)
...
assert_shared_readonly() {
  local path=$1
  [ "$(file_mode "$path")" = "$DJ_SHARED_CAPTAIN_MODE" ] \
    || fail "$path mode should be $DJ_SHARED_CAPTAIN_MODE, got $(file_mode "$path")"
}
...
test_first_copy_readonly_and_local_files_preserved() {
  ...
  out=$(DJ_CONFIG_INHERIT_REPORT="$report" propagate_secondmate_inheritance "$primary" "$second")
...
test_drift_quarantine_collision_and_repeated_convergence() {
  ...
  chmod "$DJ_SHARED_CAPTAIN_MODE" "$second/data/captain-shared.md"
  hash=$(dj_inherit_sha256 "$second/data/captain-shared.md")
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_CONFIG_INHERIT_REPORT="$report" \
...
EOF
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$data_override" \
    DJ_PROJECTS_OVERRIDE="$home/projects" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/dj-spawn.sh" sm "$sm" codex --secondmate >/dev/null 2>&1 || true

  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "spawn convergence point did not copy shared captain preferences from DJ_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "spawn convergence point propagates data/captain-shared.md from DJ_DATA_OVERRIDE"
}
...
EOF
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" \
    DJ_DATA_OVERRIDE="$data_override" \
  ...
  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "bootstrap convergence point did not copy shared captain preferences from DJ_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "bootstrap convergence point propagates data/captain-shared.md from DJ_DATA_OVERRIDE"
}
...
EOF
  ...
  chmod "$DJ_SHARED_CAPTAIN_MODE" "$sm/data/captain-shared.md"
  write_shared "$data_override/captain-shared.md" "changed override shared bytes"

  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" \
    DJ_DATA_OVERRIDE="$data_override" \
    "$ROOT/bin/dj-config-push.sh" 2>/dev/null)
  ...
  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "config-push convergence point did not update shared captain preferences from DJ_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "dj-config-push convergence point updates changed shared captain source bytes from DJ_DATA_OVERRIDE"
}
...
EOF
  fakebin=$(make_fake_spawn_toolchain "$w")
  add_bootstrap_compatible_tools "$fakebin"
  dj_fake_exit0 "$fakebin" pgrep

  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" \
    "$ROOT/bin/dj-session-start.sh")
...
echo "# all dj-shared-captain-inheritance tests passed"
