...
# bin/dj-spawn.sh installs under the contract owned by bin/dj-busy-lib.sh.
#
# These tests run the REAL dj-spawn against a fake tmux pane and an isolated
...
# bin/dj-busy-event.sh writer, and the real classifier are exercised together
...
TMP_ROOT=$(dj_test_tmproot dj-busy-adapter-wiring)
...
SH
  ...
  dj_fake_exit0 "$fakebin" treehouse pi opencode claude codex
  printf '%s\n' "$fakebin"
}
...
classify() {  # <harness> <id> <state-dir>
  dj_busy_classify tmux fake:w "$1" "$2" "$3"
}
...
test_pi_extension_semantic_lifecycle() {
  ...
  [ "$out" = "busy dj-spawn" ] || fail "seed after spawn must be 'busy dj-spawn', got '$out'"
  ...
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy dj-spawn" ] || fail "turn_end must stay a notification, not a state edge, got '$out'"
...
test_pi_extension_stale_incarnation_rejected() {
  ...
  "$ROOT/bin/dj-busy-event.sh" arm "$state" "$id" >/dev/null
  out=$(drive_pi_ext "$ext" settle-idle) || fail "stale settle drive failed: $out"
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy dj-spawn" ] || fail "a stale extension event must not change state, got '$out'"
...
test_opencode_plugin_semantic_lifecycle() {
  ...
  state="$HOME_DIR/state"
  plugin="$WT_DIR/.opencode/plugins/dj-busy-state.js"
...
test_claude_hooks_stale_incarnation_harmless() {
  ...
  out=$(classify claude "$id" "$state")
  [ "$out" = "busy dj-spawn" ] || fail "a stale-gen hook event must not change state, got '$out'"
...
test_codex_unverified_until_a_semantic_source_exists() {
  ...
  out=$(dj_busy_classify tmux fake:w codex "$id" "$state" '• Working (6s • esc to interrupt)')
...
echo "all dj-busy-adapter-wiring tests passed"
