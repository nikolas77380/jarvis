#!/usr/bin/env bash
# Behavior tests for dj-bootstrap.sh reporting and session-start clone refresh bounds.
#
...
# all is well. jarvis consumes the exact 'MISSING: treehouse (install: ...)',
# 'MISSING: tasks-axi (install: ...)', 'MISSING: quota-axi (install: ...)',
# 'MISSING: gh-axi (install: ...)', 'MISSING: edith-axi (install: ...)', and
...
# which edith-axi version is on PATH.
...
# scan.
# Dedicated network-phase cases pin DJ_BOOTSTRAP_NETWORK as a true partition of
...
TMP_ROOT=$(dj_test_tmproot dj-bootstrap-tests)
export DJ_BACKEND_CMUX_BUNDLE_BIN="$TMP_ROOT/no-bundled-cmux"
...
# dj_backend_name and flip a default-backend case onto a non-tmux backend. Unset
...
# treehouse's `get --help` advertises --lease only when DJ_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${DJ_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${DJ_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.31.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${DJ_FAKE_QUOTA_AXI_VERSION:-0.1.29}"
  exit 0
fi
exit 0
SH
...
make_fake_fleet_sync_root() {
  ...
  cat > "$fake_root/bin/dj-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
[ -z "${DJ_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] || : > "$DJ_FAKE_FLEET_SYNC_STARTED_MARKER"
printf '%s\n' 'alpha: synced'
printf '%s\n' 'beta: skipped: no origin remote'
exec perl -e 'sleep 300'
SH
  chmod +x "$fake_root/bin/dj-fleet-sync.sh"
  printf '%s\n' "$fake_root"
}
...
run_bootstrap_timeout_case() {
  ...
  (
    ...
    git() {
      local tries
      if [ "${DJ_FAKE_GIT_WAIT_FOR_FLEET_START:-}" = 1 ] && [ -n "${DJ_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ]; then
        tries=0
        while [ "$tries" -lt 5 ] && [ ! -e "$DJ_FAKE_FLEET_SYNC_STARTED_MARKER" ]; do
          command sleep 0.01
          tries=$((tries + 1))
        done
      fi
      if [ -n "${DJ_FAKE_GIT_SYNC_STARTED_RECORD:-}" ] && [ -n "${DJ_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] && [ -e "$DJ_FAKE_FLEET_SYNC_STARTED_MARKER" ]; then
        printf '%s\n' "$*" >> "$DJ_FAKE_GIT_SYNC_STARTED_RECORD"
      fi
      command git "$@"
    }
    export -f sleep
    ...
    if [ "$override" = __unset__ ]; then
      PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$fake_root" \
        DJ_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        DJ_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        DJ_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null
    else
      PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$fake_root" \
        DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT="$override" \
        DJ_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        DJ_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        DJ_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks quota backend mode expect notcontains; do
    ...
    # DJ_ROOT_OVERRIDE points the worktree-tangle check at the non-git home dir so
    ...
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
...
test_no_mistakes_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_FAKE_NO_MISTAKES_VERSION="$version" "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_gh_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_FAKE_GH_AXI_VERSION="$version" "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_edith_axi_min_version() {
  local label version mode case_dir fakebin out missing n
  missing='MISSING: edith-axi (install: npm install -g edith-axi && edith-axi setup hooks)'
  n=0
  while IFS='^' read -r label version mode; do
    [ -n "$label" ] || continue
    n=$((n + 1))
    case_dir="$TMP_ROOT/edith-axi-$n"
    mkdir -p "$case_dir/home/config"
    printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_FAKE_EDITH_AXI_VERSION="$version" "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
minimum edith-axi version is accepted^0.1.46^empty
newer edith-axi patch is accepted^0.1.47^empty
newer edith-axi minor is accepted^0.2.0^empty
newer edith-axi major is accepted^1.0.0^empty
the patch just below the floor reports an upgrade^0.1.45^missing
much older edith-axi minor reports an upgrade^0.0.9^missing
unparseable edith-axi version reports an upgrade^edith-axi development build^missing
ROWS
  pass "bootstrap enforces edith-axi minimum version"
}
...
test_tasks_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_quota_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
      DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_FAKE_QUOTA_AXI_VERSION="$version" "$ROOT/bin/dj-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$bash_env" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
...
test_herdr_install_requires_manual_action() {
  local out status
  out=$("$ROOT/bin/dj-bootstrap.sh" install herdr 2>&1)
  status=$?
...
test_cmux_bundled_cli_satisfies_dependency() {
  ...
  fakebin=$(make_fake_toolchain_no_tmux "$case_dir")
  dj_fake_exit0 "$case_dir/bundle" cmux
  bundle="$case_dir/bundle/cmux"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_BACKEND_CMUX_BUNDLE_BIN="$bundle" DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
...
test_treehouse_lease_check_follows_resolved_backend() {
  ...
  fakebin=$(make_fake_toolchain "$case_dir")
  rm -f "$fakebin/tmux"
  dj_fake_exit0 "$fakebin" orca
  # DJ_FAKE_TREEHOUSE_LEASE_HELP unset: the fake treehouse advertises NO --lease.
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    "$ROOT/bin/dj-bootstrap.sh")
...
test_fleet_sync_timeout_explicit_override_wins() {
  ...
  pass "bootstrap preserves DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT as an explicit override"
}
...
run_routine_bootstrap_fixture() {
  ...
  home=${fixture%%|*}
  fakebin=${fixture#*|}
  PATH="$fakebin:$BASE_PATH" DJ_BACKEND=tmux DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 \
    "$shell" "$ROOT/bin/dj-bootstrap.sh"
}
...
# DJ_BOOTSTRAP_NETWORK splits one bootstrap run into its local and network
...
SH
  ...
  all_out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
  ...
  skip_out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_BOOTSTRAP_NETWORK=skip "$ROOT/bin/dj-bootstrap.sh")
  ...
  only_out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_BOOTSTRAP_NETWORK=only "$ROOT/bin/dj-bootstrap.sh")
  ...
  [ "$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_BOOTSTRAP_NETWORK=sikp "$ROOT/bin/dj-bootstrap.sh")" = "$all_out" ] \
    || fail "an unrecognized DJ_BOOTSTRAP_NETWORK value did not fall back to the complete run"
  pass "bootstrap: DJ_BOOTSTRAP_NETWORK partitions one run into local and network halves"
}
...
test_network_sweeps_recheck_lock_ownership() {
  ...
  cat > "$fake_root/bin/dj-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${DJ_FAKE_FLEET_SYNC_STARTED_MARKER:?}"
SH
  chmod +x "$fake_root/bin/dj-fleet-sync.sh"

  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$fake_root" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_BOOTSTRAP_NETWORK=only \
    DJ_BOOTSTRAP_NETWORK_LOCK_PID=111111 DJ_FAKE_FLEET_SYNC_STARTED_MARKER="$marker" \
    "$ROOT/bin/dj-bootstrap.sh")
...
# assert_timing_record <log> <scope> <name> <detail> <msg>: one bin/dj-timing-lib.sh
...
# bin/dj-timing-lib.sh stays inert unless DJ_TIMING_LOG names a file, so an
...
test_network_phases_record_per_step_elapsed_times() {
  ...
  printf '%s\n' $$ > "$case_dir/home/state/.lock"
  fakebin=$(make_fake_toolchain "$case_dir")
  # A real clone with a real origin, so dj-fleet-sync.sh genuinely iterates it.
  dj_git_init_commit "$case_dir/home/projects/alpha"
  dj_git_add_origin "$case_dir/home/projects/alpha" "$case_dir/alpha-origin"
  ...
  dj_write_secondmate_meta "$case_dir/home/state/mate-a.meta" "$case_dir/home"

  log="$case_dir/timings.tsv"
  PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 DJ_BOOTSTRAP_NETWORK=only \
    DJ_BOOTSTRAP_NETWORK_LOCK_PID=$$ DJ_TIMING_LOG="$log" DJ_TIMING_EPOCH_MS=0 \
...
SH
  ...
  : > "$log"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TASKS_AXI_LOG="$log" DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
  ...
  : > "$log"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TASKS_AXI_LOG="$log" DJ_FAKE_TREEHOUSE_LEASE_HELP=1 \
    DJ_TASKS_AXI_COMPATIBLE=1 "$ROOT/bin/dj-bootstrap.sh")
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_FAKE_TASKS_AXI_LOG="$log" DJ_FAKE_TREEHOUSE_LEASE_HELP=1 \
    DJ_TASKS_AXI_COMPATIBLE=yes "$ROOT/bin/dj-bootstrap.sh")
  ...
  out=$(DJ_TASKS_AXI_COMPATIBLE=1 bash -c '. "$1"; printf "%s\n" "${DJ_TASKS_AXI_COMPATIBLE-unset}"' \
    _ "$ROOT/bin/dj-tasks-axi-lib.sh")
...
test_crew_dispatch_active_rules_are_verbose_bootstrap_info() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$case_dir/home" DJ_ROOT_OVERRIDE="$case_dir/home" \
    DJ_BOOTSTRAP_VERBOSE_FACTS=1 DJ_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/dj-bootstrap.sh")
