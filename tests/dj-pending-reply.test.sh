#!/usr/bin/env bash
# Parent-owned secondmate pending-reply guards (bin/dj-pending-reply-lib.sh).
#
...
#  10. dj-send secondmate path embeds corr and creates durable pending records
...
# shellcheck source=bin/dj-marker-lib.sh
. "$ROOT/bin/dj-marker-lib.sh"
# shellcheck source=bin/dj-pending-reply-lib.sh
. "$ROOT/bin/dj-pending-reply-lib.sh"

SEND="$ROOT/bin/dj-send.sh"
REPORT="$ROOT/bin/dj-secondmate-report.sh"
TMP_ROOT=$(dj_test_tmproot dj-pending-reply)

export DJ_PENDING_REPLY_GRACE_SECS=0
export DJ_SEND_SETTLE=0
...
run_send() {
  ...
  : > "$log"
  env PATH="$fb:$PATH" \
    DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" DJ_SEND_LOG="$log" DJ_SEND_SETTLE=0 \
    DJ_PENDING_REPLY_GRACE_SECS=0 \
    "$SEND" "$@" 2>/dev/null
}

phase_of() {  # <state> <corr>
  dj_pending_reply_get "$(dj_pending_reply_path "$1" "$2")" phase
}
...
# the production owner (bin/dj-task-inbox-lib.sh).
latest_record_body() {  # <home> <task>
  local rec
  rec=$(find "$1/state/$2.inbox" -maxdepth 1 -name '*.msg' 2>/dev/null | sort | tail -1)
  [ -n "$rec" ] || return 1
  bash -c '. "$1"; dj_task_inbox_body "$2"' _ "$ROOT/bin/dj-task-inbox-lib.sh" "$rec"
}
...
test_normal_correlated_reply_resolves_once() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=1000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "audit the ledger")
  dj_pending_reply_mark_delivered "$state" "$corr"
  status="$state/hibit.status"
  if dj_pending_reply_try_resolve "$state" "$corr"; then
    fail "missing status must not resolve"
  fi
  printf 'done [corr=%s]: ledger clean\n' "$corr" > "$status"
  dj_pending_reply_try_resolve "$state" "$corr" || fail "correlated status should resolve"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase should be resolved"
  # Idempotent second resolve.
  dj_pending_reply_try_resolve "$state" "$corr" || fail "second resolve must stay successful"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase must remain resolved"
  rec=$(dj_pending_reply_path "$state" "$corr")
  [ "$(dj_pending_reply_get "$rec" resolved_via)" = status ] \
...
test_completed_turn_no_report_triggers_one_recovery() {
  ...
  : > "$hook_log"
  export DJ_PENDING_REPLY_NOW=2000
  export DJ_PENDING_REPLY_SEND_HOOK='printf "%s\t%s\n" >>"'"$hook_log"'"'
  # The hook above is wrong for eval form - use a function.
  # Invoked indirectly through DJ_PENDING_REPLY_SEND_HOOK.
  # shellcheck disable=SC2329
  recovery_hook() {
    printf '%s\t%s\n' "$1" "$2" >> "$hook_log"
  }
  export -f recovery_hook
  export DJ_PENDING_REPLY_SEND_HOOK='recovery_hook'

  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "status of phase 7")
  dj_pending_reply_mark_delivered "$state" "$corr"
  # Turn completes with no parent report (the Hi Bit missed-report shape).
  dj_pending_reply_observe_busy "$state" "$corr" busy
  dj_pending_reply_observe_busy "$state" "$corr" idle
  dj_pending_reply_send_recovery "$state" "$corr" \
  ...
  if dj_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
...
test_recovery_attempt_is_never_reinjected() {
  ...
  : > "$hook_log"
  export DJ_PENDING_REPLY_NOW=2500
  recovery_fail_hook() {
    printf 'attempted\n' >> "$hook_log"
    return 1
  }
  export -f recovery_fail_hook
  export DJ_PENDING_REPLY_SEND_HOOK=recovery_fail_hook
  corr=$(dj_pending_reply_create "$home" "$state" hibit "at most once")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  if dj_pending_reply_send_recovery "$state" "$corr"; then
  ...
  [ -z "$(dj_pending_reply_get "$(dj_pending_reply_path "$state" "$corr")" recovery_sent_epoch)" ] \
  ...
  dj_pending_reply_maybe_escalate "$state" "$corr" \
  ...
  live_corr=$(dj_pending_reply_create "$home" "$state" hibit "live recovery")
  dj_pending_reply_mark_delivered "$state" "$live_corr"
  dj_pending_reply_mark_turn_completed "$state" "$live_corr" request
  live_rec=$(dj_pending_reply_path "$state" "$live_corr")
  live_pid=${BASHPID:-$$}
  live_identity=$(dj_pending_reply_pid_identity "$live_pid") \
    || fail "live sender identity should be observable"
  dj_pending_reply_set "$live_rec" recovery_attempted_epoch 2500 || fail "live attempt precommit failed"
  dj_pending_reply_set "$live_rec" recovery_sender_pid "$live_pid" || fail "live sender pid commit failed"
  dj_pending_reply_set "$live_rec" recovery_sender_identity "$live_identity" \
    || fail "live sender identity commit failed"
  dj_pending_reply_set "$live_rec" phase recovery_sending || fail "live sending phase failed"
  dj_pending_reply_tick_one "$state" "$live_corr" unknown || fail "live recovery tick failed"
  [ "$(phase_of "$state" "$live_corr")" = recovery_sending ] \
    || fail "live recovery must remain in progress without elapsed-time inference"
  corr=$(dj_pending_reply_create "$home" "$state" hibit "crashed recovery")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  rec=$(dj_pending_reply_path "$state" "$corr")
  dj_pending_reply_set "$rec" recovery_attempted_epoch 2500 || fail "attempt precommit failed"
  dj_pending_reply_set "$rec" phase recovery_sending || fail "sending phase precommit failed"
  dj_pending_reply_tick_one "$state" "$corr" unknown || fail "recovery reconciliation failed"
  [ "$(phase_of "$state" "$corr")" = escalated ] \
    || fail "interrupted recovery attempt should escalate unknown delivery"
  [ "$(dj_pending_reply_get "$rec" recovery_delivery_outcome)" = unknown ] \
    || fail "interrupted recovery should preserve unknown delivery outcome"
  [ -z "$(dj_pending_reply_get "$rec" recovery_sent_epoch)" ] \
  ...
  unset DJ_PENDING_REPLY_SEND_HOOK
...
test_recovery_reply_resolves_original() {
  ...
  export -f recovery_hook
  export DJ_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export DJ_PENDING_REPLY_NOW=3000

  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "phase 7 status")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  dj_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  printf 'done [corr=%s]: phase 7 is Done (reposted)\n' "$corr" > "$state/hibit.status"
  dj_pending_reply_try_resolve "$state" "$corr" || fail "recovery reply should resolve original"
...
test_second_missed_turn_escalates_once_and_stays_durable() {
  ...
  export -f recovery_hook
  export DJ_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export DJ_PENDING_REPLY_NOW=4000
  # Do not export STATE into the test process: dj-send resolves
  # DJ_STATE_OVERRIDE/STATE from the environment and a leak breaks later cases.

  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "why is phase 7 stuck")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  dj_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  # Recovery turn also completes with no correlated report.
  dj_pending_reply_mark_turn_completed "$state" "$corr" recovery
  dj_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation should fire"
  ...
  if dj_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  [ "$(dj_pending_reply_get "$rec" parent_status)" = "$state/hibit.status" ] \
...
seen_gate() {  # <state> <file>: 0 when every byte is already announced
  DJ_STATE_OVERRIDE="$1" bash -c '. "$1"; dj_wake_signal_seen_current "$2" "$3"' \
    _ "$ROOT/bin/dj-wake-lib.sh" "$1" "$2"
}
prime_seen() {  # <state> <file>
  DJ_STATE_OVERRIDE="$1" bash -c '
    . "$1"; sig=$(dj_wake_signal_sig "$3") || exit 1
    printf "%s" "$sig" > "$(dj_wake_signal_seen_path "$2" "$3")"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$1" "$2"
}
...
test_escalation_wakes_and_its_close_stays_quiet() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_SEND_HOOK='true'
  export DJ_PENDING_REPLY_NOW=4200
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "confirm the notarization")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  dj_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  dj_pending_reply_mark_turn_completed "$state" "$corr" recovery
  ...
  dj_pending_reply_try_resolve "$state" "$corr" || fail "correlated reply should resolve"
...
test_escalation_publication_failure_retries() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4500
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "retry escalation")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  export DJ_PENDING_REPLY_SEND_HOOK='true'
  dj_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  ...
  mkdir -p "$target"
  dj_pending_reply_set "$rec" parent_status "$target" || fail "failed to set escalation target"
  if dj_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  rmdir "$target"
  dj_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation retry should succeed"
...
test_legacy_escalation_closes_default_decision() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4725
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "legacy close")
  dj_pending_reply_mark_delivered "$state" "$corr"
  rec=$(dj_pending_reply_path "$state" "$corr")
  dj_pending_reply_set "$rec" phase escalated
  dj_pending_reply_set "$rec" escalated_epoch 4700
  ...
  dj_pending_reply_try_resolve "$state" "$corr" || fail "legacy reply should resolve its record"
  ...
  [ -n "$(dj_pending_reply_get "$rec" escalation_closed_epoch)" ] \
...
test_legacy_escalation_does_not_close_taken_default_decision() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4750
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "legacy escalation")
  dj_pending_reply_mark_delivered "$state" "$corr"
  rec=$(dj_pending_reply_path "$state" "$corr")
  dj_pending_reply_set "$rec" phase escalated
  dj_pending_reply_set "$rec" escalated_epoch 4700
  ...
  dj_pending_reply_tick "$state" || fail "legacy close retry failed"
  open=$(status_open_decisions "$state/hibit.status")
...
test_foreign_blocker_is_not_selected_as_escalation() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4775
  export DJ_PENDING_REPLY_SEND_HOOK=true
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "foreign blocker")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  dj_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  dj_pending_reply_mark_turn_completed "$state" "$corr" recovery
  dj_pending_reply_maybe_escalate "$state" "$corr" || fail "genuine escalation failed"
  rec=$(dj_pending_reply_path "$state" "$corr")
  printf 'blocked [key=release]: foreign decision pending-reply-id=%s corr=%s\n' \
    "$corr" "$corr" >> "$state/hibit.status"

  dj_pending_reply_try_resolve "$state" "$corr" || fail "correlated foreign blocker should resolve the record"
  open=$(status_open_decisions "$state/hibit.status")
...
test_concurrent_resolution_closes_escalation_once() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4800
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "concurrent resolution")
  dj_pending_reply_mark_delivered "$state" "$corr"
  rec=$(dj_pending_reply_path "$state" "$corr")
  dj_pending_reply_set "$rec" phase escalated
  dj_pending_reply_set "$rec" escalated_epoch 4750
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    dj_pending_reply_try_resolve "$state" "$corr" &
  done
  wait
...
test_concurrent_escalation_yields_to_late_reply() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=4900
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "concurrent escalation")
  dj_pending_reply_mark_delivered "$state" "$corr"
  rec=$(dj_pending_reply_path "$state" "$corr")
  dj_pending_reply_set "$rec" phase recovery_sent
  dj_pending_reply_set "$rec" recovery_turn_completed_epoch 4850
  ...
  [ -z "$(dj_pending_reply_get "$rec" escalated_epoch)" ] \
...
test_transport_success_is_not_reply_success() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=5000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "ping")
  dj_pending_reply_mark_delivered "$state" "$corr" || fail "mark delivered failed"
...
test_undelivered_records_are_scan_immutable() {
  (
    ...
    export DJ_PENDING_REPLY_NOW=5500
    corr=$(dj_pending_reply_create "$home" "$state" hibit "not delivered yet")
    rec=$(dj_pending_reply_path "$state" "$corr")
    printf 'done [corr=%s]: arrived too early\n' "$corr" > "$state/hibit.status"
    printf 'done [corr=%s]: wrong home too early\n' "$corr" > "$sm_home/state/hibit.status"
    dj_write_secondmate_meta "$state/hibit.meta" "$sm_home" "sess:dj-hibit"
    before=$(cat "$rec")
    if dj_pending_reply_try_resolve "$state" "$corr"; then
      fail "undelivered expectation must not resolve"
    fi
    dj_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
      || fail "undelivered wrong-home check should be inert"
    dj_pending_reply_tick_one "$state" "$corr" busy "$sm_home" \
      || fail "undelivered direct tick should be inert"
    dj_backend_busy_state() { fail "undelivered watcher tick must not probe the backend"; }
    dj_backend_capture() { fail "undelivered watcher tick must not capture the backend"; }
    dj_pending_reply_tick "$state" || fail "undelivered watcher tick should succeed"
    after=$(cat "$rec")
    [ "$after" = "$before" ] || fail "scan paths must not mutate an undelivered record"
    dj_pending_reply_mark_delivered "$state" "$corr" || fail "delivery marker should succeed"
    dj_pending_reply_tick_one "$state" "$corr" unknown "$sm_home" \
...
test_delivery_confirmation_fallback_reconciles() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export DJ_PENDING_REPLY_NOW=5750
    corr=$(dj_pending_reply_create "$home" "$state" hibit "confirmed delivery")
    rec=$(dj_pending_reply_path "$state" "$corr")
    dj_pending_reply_mark_delivered() { return 1; }
    if dj_pending_reply_confirm_delivery "$state" "$corr"; then
      fail "primary delivery commit failure should be reported"
    else
      rc=$?
    fi
    [ "$rc" = 2 ] || fail "durable fallback should return status 2, got $rc"
    marker=$(dj_pending_reply_delivery_confirmation_path "$state" "$corr")
    [ -f "$marker" ] || fail "delivery confirmation fallback marker should persist"
    [ -z "$(dj_pending_reply_get "$rec" delivered_epoch)" ] \
      || fail "failed primary commit should leave delivered_epoch empty"
    . "$ROOT/bin/dj-pending-reply-lib.sh"
    dj_pending_reply_tick_one "$state" "$corr" unknown \
      || fail "watcher should reconcile the delivery marker"
    [ "$(dj_pending_reply_get "$rec" delivered_epoch)" = 5750 ] \
      || fail "watcher should restore the confirmed delivery epoch"
    [ ! -e "$marker" ] || fail "reconciled delivery marker should be removed"
    prepared_corr=$(dj_pending_reply_create "$home" "$state" hibit "prepared delivery")
    prepared_rec=$(dj_pending_reply_path "$state" "$prepared_corr")
    dj_pending_reply_prepare_delivery "$state" "$prepared_corr" \
      || fail "delivery preparation should persist before transport"
    dj_pending_reply_set "$prepared_rec" grace_secs 10 \
      || fail "delivery-unknown grace fixture should persist"
    prepared_marker=$(dj_pending_reply_delivery_confirmation_path "$state" "$prepared_corr")
    [ -f "$prepared_marker" ] || fail "prepared delivery marker should persist"
    dj_pending_reply_tick_one "$state" "$prepared_corr" unknown \
    ...
    [ -z "$(dj_pending_reply_get "$prepared_rec" delivered_epoch)" ] \
    ...
    export DJ_PENDING_REPLY_NOW=5760
    dj_pending_reply_write_delivery_confirmation \
    ...
    dj_pending_reply_tick "$state" || fail "watcher should accept a late delivery report"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report should resolve escalated delivery-unknown"
    [ "$(dj_pending_reply_get "$prepared_rec" delivered_epoch)" = 5760 ] \
      || fail "late report should provide delivery evidence"
    escalations=$(grep -Fc "blocked [key=pending-reply-$prepared_corr]:" "$state/hibit.status")
    [ "$escalations" = 1 ] || fail "late report must not re-escalate delivery-unknown"
    dj_pending_reply_tick "$state" || fail "resolved late report should remain idempotent"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report resolution should remain durable"
    export DJ_PENDING_REPLY_NOW=5800
    reported_corr=$(dj_pending_reply_create "$home" "$state" hibit "reported delivery")
    reported_rec=$(dj_pending_reply_path "$state" "$reported_corr")
    dj_pending_reply_prepare_delivery "$state" "$reported_corr" \
      || fail "reported delivery attempt should persist"
    reported_marker=$(dj_pending_reply_delivery_confirmation_path "$state" "$reported_corr")
    printf 'done [corr=%s]: report proves delivery\n' "$reported_corr" >> "$state/hibit.status"
    dj_pending_reply_try_resolve "$state" "$reported_corr" \
...
test_delivery_confirmation_serializes_with_reconciliation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export DJ_PENDING_REPLY_NOW=5900
    corr=$(dj_pending_reply_create "$home" "$state" hibit "serialized delivery")
    rec=$(dj_pending_reply_path "$state" "$corr")
    calls="$home/mark-delivered.calls"
    entered="$home/mark-delivered.entered"
    release="$home/mark-delivered.release"
    dj_pending_reply_mark_delivered() {
      local pending_state=$1 pending_corr=$2 epoch=$3 pending_rec phase
      printf '%s\n' "$BASHPID" >> "$calls"
      : > "$entered"
      while [ ! -e "$release" ]; do /bin/sleep 0.01; done
      pending_rec=$(dj_pending_reply_path "$pending_state" "$pending_corr")
      dj_pending_reply_set "$pending_rec" delivered_epoch "$epoch" || return 1
      phase=$(dj_pending_reply_get "$pending_rec" phase)
      [ "$phase" != delivery_unknown ] \
        || dj_pending_reply_set "$pending_rec" phase awaiting_report
    }
    dj_pending_reply_confirm_delivery "$state" "$corr" &
    ...
    dj_pending_reply_reconcile_delivery "$state" "$corr" &
    # The background PID is consumed within this isolated test subshell.
    # shellcheck disable=SC2031
    reconcile_pid=$!
    /bin/sleep 0.1
    ...
    [ "$(dj_pending_reply_get "$rec" delivered_epoch)" = 5900 ] \
    ...
    [ ! -e "$(dj_pending_reply_delivery_confirmation_path "$state" "$corr")" ] \
...
test_unrelated_and_stale_corr_cannot_resolve() {
  ...
  # shellcheck disable=SC2031
  export DJ_PENDING_REPLY_NOW=6000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "need answer")
  dj_pending_reply_mark_delivered "$state" "$corr"
  other=$(dj_pending_reply_new_id)
...
test_restart_preserves_expectation_and_parent_destination() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=7000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "survive restart")
  dj_pending_reply_mark_delivered "$state" "$corr"
  rec=$(dj_pending_reply_path "$state" "$corr")
  parent_status=$(dj_pending_reply_get "$rec" parent_status)
  parent_home=$(dj_pending_reply_get "$rec" parent_home)
  # Simulate process restart: re-source library and re-read the same record.
  # shellcheck source=bin/dj-pending-reply-lib.sh
  . "$ROOT/bin/dj-pending-reply-lib.sh"
  [ -f "$rec" ] || fail "record must survive restart"
  [ "$(dj_pending_reply_get "$rec" parent_status)" = "$parent_status" ] \
    || fail "parent_status must be stable across restart"
  [ "$(dj_pending_reply_get "$rec" parent_home)" = "$parent_home" ] \
...
test_wrong_home_detected_not_acknowledged() {
  ...
  mkdir -p "$sm_home/state"
  export DJ_PENDING_REPLY_NOW=8000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "report to parent")
  dj_pending_reply_mark_delivered "$state" "$corr"
  ...
  rec=$(dj_pending_reply_path "$state" "$corr")
  hits=$(dj_pending_reply_get "$rec" wrong_home_hits)
...
test_unmarked_captain_input_creates_no_expectation() {
  ...
  dj_write_meta "$home/state/build.meta" \
    "window=sess:dj-build" "worktree=$home/wt" "project=$home/p" \
...
test_fm_send_marked_secondmate_creates_pending_and_embeds_corr() {
  ...
  dj_write_secondmate_meta "$home/state/hibit.meta" "$home/sm" "sess:dj-hibit"
...
test_document_pointer_resolves() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=9000
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "deep audit")
  dj_pending_reply_mark_delivered "$state" "$corr"
  printf 'done [corr=%s]: see data/hibit/report.md\n' "$corr" > "$state/hibit.status"
  dj_pending_reply_try_resolve "$state" "$corr" || fail "document pointer status should resolve"
  [ "$(dj_pending_reply_get "$(dj_pending_reply_path "$state" "$corr")" resolved_via)" = document ] \
...
test_helper_report_resolves() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=9100
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "quick answer")
  dj_pending_reply_mark_delivered "$state" "$corr"
  "$REPORT" "$state/hibit.status" "done" "$corr" "all good" \
    || fail "helper report failed"
  dj_pending_reply_try_resolve "$state" "$corr" || fail "helper report should resolve"
  [ "$(dj_pending_reply_get "$(dj_pending_reply_path "$state" "$corr")" resolved_via)" = helper ] \
...
test_busy_idle_observation_via_backend_abstraction() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=9200
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "backend turn")
  dj_pending_reply_mark_delivered "$state" "$corr"
  # Simulates Pi/Claude secondmate busy_state from dj_backend_busy_state without
  # reading conversation text (herdr native idle/busy or tmux unknown fallback).
  dj_pending_reply_observe_busy "$state" "$corr" unknown
  [ -z "$(dj_pending_reply_get "$(dj_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
    || fail "unknown busy_state must not prove turn completion"
  dj_pending_reply_observe_busy "$state" "$corr" busy
  dj_pending_reply_observe_busy "$state" "$corr" idle
  [ -n "$(dj_pending_reply_get "$(dj_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
...
test_unknown_backend_state_uses_capture_fallback() {
  ...
  for backend in tmux zellij; do
    (
      ...
      sm_home="$home/sm"
      mkdir -p "$sm_home/state"
      export DJ_PENDING_REPLY_GRACE_SECS=10
      # These fixture overrides are intentionally scoped to the isolated subshell.
      # shellcheck disable=SC2030,SC2031
      export DJ_PENDING_REPLY_NOW=10000
      corr=$(dj_pending_reply_create "$home" "$state" "hibit" "$backend fallback")
      dj_pending_reply_mark_delivered "$state" "$corr"
      dj_write_secondmate_meta "$state/hibit.meta" "$sm_home" "session:dj-hibit" alpha pi
      [ "$backend" = tmux ] || printf 'backend=%s\n' "$backend" >> "$state/hibit.meta"
      dj_backend_busy_state() { printf 'unknown'; }
      dj_backend_capture() { printf '%s' "$DJ_PENDING_TEST_CAPTURE"; }
      # Invoked indirectly through DJ_PENDING_REPLY_SEND_HOOK.
      # shellcheck disable=SC2329
      recovery_hook() { :; }
      ...
      # shellcheck disable=SC2030,SC2031
      export DJ_PENDING_REPLY_SEND_HOOK=recovery_hook
      export DJ_PENDING_TEST_CAPTURE='idle footer'
      dj_pending_reply_tick "$state"
      rec=$(dj_pending_reply_path "$state" "$corr")
      [ -z "$(dj_pending_reply_get "$rec" request_turn_completed_epoch)" ] \
        || fail "$backend fallback must not accept stale idle before grace"
      # Continue advancing the subshell-local fixture clock.
      # shellcheck disable=SC2030,SC2031
      export DJ_PENDING_REPLY_NOW=10010
      dj_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
        || fail "$backend fallback idle should trigger recovery after grace"
      export DJ_PENDING_REPLY_NOW=10011
      export DJ_PENDING_TEST_CAPTURE='Working...'
      dj_pending_reply_tick "$state"
      export DJ_PENDING_REPLY_NOW=10012
      export DJ_PENDING_TEST_CAPTURE='idle footer'
      dj_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = escalated ] \
...
test_kimi_capture_fallback_uses_recorded_harness() (
  ...
  # This fixture clock is intentionally scoped to the isolated subshell.
  # shellcheck disable=SC2030,SC2031
  export DJ_PENDING_REPLY_NOW=10020
  corr=$(dj_pending_reply_create "$home" "$state" hibit "kimi fallback")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_write_secondmate_meta "$state/hibit.meta" "$sm_home" "session:dj-hibit" alpha kimi
  dj_backend_busy_state() { printf 'unknown'; }
  dj_backend_capture() { printf '%s' "$DJ_PENDING_KIMI_CAPTURE"; }
  export DJ_PENDING_KIMI_CAPTURE=' 🌑 · Tip: ask Kimi to schedule tasks, e.g. "remind me at 5pm"'

  [ "$(dj_pending_reply_backend_observation tmux session:dj-hibit dj-hibit codex)" = fallback-idle ] \
    || fail "Kimi spinner leaked into another harness"
  export DJ_PENDING_KIMI_CAPTURE='Ctrl+c:cancel'
  [ "$(dj_pending_reply_backend_observation tmux session:dj-hibit dj-hibit kimi)" = fallback-idle ] \
    || fail "Grok's exact busy token leaked into Kimi pending-reply observation"
  export DJ_PENDING_KIMI_CAPTURE=' 🌑 · Tip: ask Kimi to schedule tasks, e.g. "remind me at 5pm"'
  dj_pending_reply_tick "$state"
  rec=$(dj_pending_reply_path "$state" "$corr")
  [ "$(dj_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
...
test_tick_skips_terminal_and_reuses_target_observation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export DJ_PENDING_REPLY_NOW=10100
    open1=$(dj_pending_reply_create "$home" "$state" hibit "first open request")
    open2=$(dj_pending_reply_create "$home" "$state" hibit "second open request")
    dj_pending_reply_mark_delivered "$state" "$open1"
    dj_pending_reply_mark_delivered "$state" "$open2"
    resolved=$(dj_pending_reply_create "$home" "$state" resolved "resolved request")
    dj_pending_reply_mark_delivered "$state" "$resolved"
    printf 'done [corr=%s]: complete\n' "$resolved" > "$state/resolved.status"
    dj_pending_reply_try_resolve "$state" "$resolved" || fail "resolved fixture should resolve"
    escalated=$(dj_pending_reply_create "$home" "$state" escalated "escalated request")
    dj_pending_reply_mark_delivered "$state" "$escalated"
    rec=$(dj_pending_reply_path "$state" "$escalated")
    dj_pending_reply_set "$rec" phase escalated || fail "escalated fixture should transition"
    mkdir -p "$home/escalated/state"
    printf 'done [corr=%s]: wrong home\n' "$escalated" > "$home/escalated/state/escalated.status"
    dj_write_secondmate_meta "$state/hibit.meta" "$home/hibit" "sess:dj-hibit"
    dj_write_secondmate_meta "$state/resolved.meta" "$home/resolved" "sess:dj-resolved"
    dj_write_secondmate_meta "$state/escalated.meta" "$home/escalated" "sess:dj-escalated"
    ...
    # shellcheck disable=SC2329
    dj_backend_capture() { fail "native busy observations should not capture"; }
    # shellcheck disable=SC2329
    dj_pending_reply_find_resolve_line() {
      local status_file=$1 corr=$2 line
      printf '%s\t%s\n' "$status_file" "$corr" >> "$scan_log"
      [ -f "$status_file" ] || return 0
      while IFS= read -r line || [ -n "$line" ]; do
        dj_pending_reply_line_resolves "$line" "$corr" || continue
        printf '%s' "$line"
        return 0
      done < "$status_file"
      return 0
    ...
    rec=$(dj_pending_reply_path "$state" "$open1")
    [ "$(dj_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the first open record"
    rec=$(dj_pending_reply_path "$state" "$open2")
    [ "$(dj_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the second open record"
    rec=$(dj_pending_reply_path "$state" "$escalated")
    snapshot=$(dj_pending_reply_get "$rec" wrong_home_scan_signature)
    ...
    [ "$(dj_pending_reply_get "$rec" wrong_home_scan_signature)" = "$snapshot" ] \
...
test_correlations_reuse_only_for_matching_open_task() {
  ...
  state="$home/state"
  dj_write_secondmate_meta "$state/domain.meta" "$home/domain" "sess:dj-domain"
  dj_write_secondmate_meta "$state/other.meta" "$home/other" "sess:dj-other"
  run_send "$fb" "$home" "$log" domain "first request" || fail "first marked send failed"
  got=$(latest_record_body "$home" domain)
  corr1=$(dj_pending_reply_extract_corr "$got")
  export DJ_PENDING_REPLY_EXISTING_CORR=$corr1
  if run_send "$fb" "$home" "$log" other "forwarded request"; then
    fail "an explicit cross-task correlation must be refused"
  fi
  unset DJ_PENDING_REPLY_EXISTING_CORR
  ...
  corr2=$(dj_pending_reply_extract_corr "$(latest_record_body "$home" other)")
  [ -n "$corr2" ] && [ "$corr2" != "$corr1" ] \
    || fail "cross-task send must receive a new correlation"
  rec=$(dj_pending_reply_path "$state" "$corr2")
  [ "$(dj_pending_reply_get "$rec" task_id)" = other ] \
    || fail "cross-task expectation must belong to the new target"
  printf 'done [corr=%s]: complete\n' "$corr1" > "$state/domain.status"
  dj_pending_reply_try_resolve "$state" "$corr1" || fail "first expectation should resolve"
  run_send "$fb" "$home" "$log" domain "${DJ_FROMFIRST_MARK}corr=${corr1} follow-up" \
    || fail "resolved-correlation follow-up failed"
  corr3=$(dj_pending_reply_extract_corr "$(latest_record_body "$home" domain)")
  [ -n "$corr3" ] && [ "$corr3" != "$corr1" ] \
    || fail "resolved correlation must not guard a new send"
  rec=$(dj_pending_reply_path "$state" "$corr3")
  [ "$(dj_pending_reply_get "$rec" task_id)" = domain ] \
...
test_tick_end_to_end_missed_then_escalate() {
  ...
  export DJ_PENDING_REPLY_SEND_HOOK='recovery_hook'
  # shellcheck disable=SC2031
  export DJ_PENDING_REPLY_NOW=9300

  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "e2e miss")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_write_secondmate_meta "$state/hibit.meta" "$sm_home" "sess:dj-hibit"
  # Override backend busy via direct tick_one (backend may be unknown in hermetic home).
  dj_pending_reply_tick_one "$state" "$corr" busy "$sm_home"
  dj_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
  ...
  export DJ_PENDING_REPLY_NOW=999999
  dj_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ -f "$(dj_pending_reply_path "$state" "$corr")" ] \
...
test_remote_repost_waits_for_the_reply_channel() {
  ...
  export DJ_PENDING_REPLY_SEND_HOOK=remote_repost_hook

  dj_write_meta "$state/ios.meta" \
    "window=dj-remote:w1:p1" "harness=claude" "kind=secondmate" "mode=secondmate" \
    "remote_host=remote-mac" "remote_root=/remote/root" "remote_backend=herdr"
  corr=$(dj_pending_reply_create "$home" "$state" "ios" "status of the iOS build")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_observe_busy "$state" "$corr" busy
  dj_pending_reply_observe_busy "$state" "$corr" idle
  rec=$(dj_pending_reply_path "$state" "$corr")
  ...
  dj_pending_reply_note_remote_channel_caught_up "$state" ios 4000
  if dj_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
  ...
  dj_pending_reply_note_remote_channel_caught_up "$state" ios \
    "$(dj_pending_reply_get "$rec" request_turn_completed_epoch)"
  dj_pending_reply_send_recovery "$state" "$corr" \
...
test_mirrored_remote_reply_never_triggers_a_repost() {
  ...
  export DJ_PENDING_REPLY_SEND_HOOK=mirrored_reply_hook

  dj_write_meta "$state/ios.meta" \
    "window=dj-remote:w1:p1" "harness=claude" "kind=secondmate" "mode=secondmate" \
    "remote_host=remote-mac" "remote_root=/remote/root" "remote_backend=herdr"
  corr=$(dj_pending_reply_create "$home" "$state" "ios" "did the build go green")
  dj_pending_reply_mark_delivered "$state" "$corr"
  dj_pending_reply_mark_turn_completed "$state" "$corr" request
  # The mirror caught up AND carried the mate's correlated answer.
  printf 'done [corr=%s]: build is green\n' "$corr" > "$state/ios.status"
  dj_pending_reply_note_remote_channel_caught_up "$state" ios 6000

  dj_pending_reply_tick_one "$state" "$corr" idle || fail "tick should succeed"
  [ "$(phase_of "$state" "$corr")" = resolved ] \
...
test_failed_send_discards_undelivered_expectation() {
  ...
  state="$home/state"
  export DJ_PENDING_REPLY_NOW=9400
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "never lands")
  # Not delivered: discard is allowed.
  dj_pending_reply_discard_undelivered "$state" "$corr" || fail "discard undelivered failed"
  [ ! -f "$(dj_pending_reply_path "$state" "$corr")" ] \
    || fail "undelivered record should be removed"
  # Delivered records must not be discarded by this path.
  corr=$(dj_pending_reply_create "$home" "$state" "hibit" "landed")
  dj_pending_reply_mark_delivered "$state" "$corr"
  if dj_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null; then
    fail "delivered record must not be discarded"
  fi
  [ -f "$(dj_pending_reply_path "$state" "$corr")" ] || fail "delivered record must remain"
