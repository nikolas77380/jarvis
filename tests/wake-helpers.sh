...
# dj-wake-drain.sh now calls dj-guard.sh to assert watcher liveness on every
# drain. dj-guard.sh's first check warns when the jarvis PRIMARY checkout
# (DJ_ROOT) sits on a feature branch; with no override DJ_ROOT resolves to the
...
# direct dj-guard.sh tests use. A per-call DJ_ROOT_OVERRIDE still wins where a
# suite sets its own (e.g. the watcher-lock guard-banner cases).
if [ -z "${DJ_ROOT_OVERRIDE:-}" ]; then
  DJ_ROOT_OVERRIDE="$(dj_test_tmproot dj-wake-tangle-root)"
  export DJ_ROOT_OVERRIDE
fi

# Wedge-alarm notifier recorder (safety seam). The away-mode wedge alarm fires a
# real OS-level desktop notification by default. Point its DJ_WEDGE_ALARM_EXEC
...
# $DJ_WEDGE_ALARM_LOG, which a test sets to its own file to assert on; unset means
# /dev/null. DJ_WEDGE_ALARM_FAIL=<channel> makes the recorder exit non-zero for
...
_fm_wedge_rec_dir=$(dj_test_tmproot dj-wedge-rec)
cat > "$_fm_wedge_rec_dir/rec" <<'REC'
#!/usr/bin/env bash
printf '%s\t%s\n' "${1:-}" "${2:-}" >> "${DJ_WEDGE_ALARM_LOG:-/dev/null}"
case " ${DJ_WEDGE_ALARM_FAIL:-} " in *" ${1:-} "*) exit 1 ;; esac
exit 0
REC
chmod +x "$_fm_wedge_rec_dir/rec"
export DJ_WEDGE_ALARM_EXEC="$_fm_wedge_rec_dir/rec"
...
append_wake() {
  local state=$1 kind=$2 key=$3 payload=$4 lib="$ROOT/bin/dj-wake-lib.sh"
  DJ_STATE_OVERRIDE="$state" bash -c '
    # shellcheck disable=SC1090,SC1091
    . "$1"
    dj_wake_append "$2" "$3" "$4"
...
if [ "${1:-}" = "list-windows" ]; then
  if [ -n "${DJ_FAKE_TMUX_WINDOW:-}" ]; then
    printf '%s\n' "${DJ_FAKE_TMUX_WINDOW#*:}"
  fi
  exit 0
fi
if [ "${1:-}" = "capture-pane" ]; then
  if [ -n "${DJ_FAKE_TMUX_CAPTURE:-}" ]; then
...
if [ "${1:-}" = "display-message" ]; then
  case "$*" in
    *pane_current_command*) printf '%s\n' "${DJ_FAKE_TMUX_CURRENT_COMMAND:-}"; exit 0 ;;
  esac
fi
exit 1
SH
...
# Install a hermetic fake dj-crew-state.sh into <fakebin> and echo its path. The
# watcher's absorb-only-when-provably-working triage calls this (via
# DJ_CREW_STATE_BIN) to read a crew's current state on no-verb signal and stale
...
# A per-id override DJ_FAKE_CREW_STATE_<sanitized-id> wins; otherwise the shared
# DJ_FAKE_CREW_STATE; otherwise an unknown verdict (NOT provably working), the
# safe default so a test that forgets to set one surfaces rather than absorbs.
make_fake_crew_state() {  # <fakebin>
  local fakebin=$1
  cat > "$fakebin/dj-crew-state.sh" <<'SH'
#!/usr/bin/env bash
set -u
id=${1:-}
key=$(printf '%s' "$id" | tr -c 'A-Za-z0-9' '_')
var="DJ_FAKE_CREW_STATE_$key"
val=${!var:-${DJ_FAKE_CREW_STATE:-}}
printf '%s\n' "${val:-state: unknown · source: none · fake default}"
exit 0
SH
  chmod +x "$fakebin/dj-crew-state.sh"
  printf '%s\n' "$fakebin/dj-crew-state.sh"
}

# Prime <file>'s .seen-* marker to its CURRENT signature through the production
# signature owner (bin/dj-wake-lib.sh), so a test can declare "everything in
...
ack_drain_err() {  # <state> <stderr-file>
  ...
  DJ_STATE_OVERRIDE="$state" "$ROOT/bin/dj-wake-drain.sh" \
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    # structural full-pane scan or dj_pane_is_busy's "-S -40" tail) return the whole capture. -e is accepted and
    ...
    [ -n "${DJ_FAKE_TMUX_CAPTURE:-}" ] || exit 0
    if [ -n "$_S" ] && [ -n "$_E" ]; then
      case "$_S$_E" in
        *[!0-9]*) cat "$DJ_FAKE_TMUX_CAPTURE" 2>/dev/null ;;
        *) sed -n "$((_S + 1)),$((_E + 1))p" "$DJ_FAKE_TMUX_CAPTURE" 2>/dev/null ;;
      esac
    else
      cat "$DJ_FAKE_TMUX_CAPTURE" 2>/dev/null
    fi
    exit 0 ;;
  send-keys)
    while [ "$#" -gt 0 ]; do
      case "$1" in
        -l) shift; [ "$#" -gt 0 ] && {
          printf '%s\n' "$1" >> "${DJ_FAKE_TMUX_SENT:-/dev/null}"
          # Reflect sent text into capture so pane_input_pending sees it as
          # pending input (text in the composer).
          [ -n "${DJ_FAKE_TMUX_CAPTURE:-}" ] && printf '%s\n' "$1" >> "$DJ_FAKE_TMUX_CAPTURE"
        } ;;
        Enter)
          # Optionally swallow Enter (file-based flag) to test the retry path.
          if [ -n "${DJ_FAKE_TMUX_SWALLOW_FILE:-}" ] && [ -f "$DJ_FAKE_TMUX_SWALLOW_FILE" ]; then
            rm -f "$DJ_FAKE_TMUX_SWALLOW_FILE"
          else
            printf '[ENTER]\n' >> "${DJ_FAKE_TMUX_SENT:-/dev/null}"
            # Enter submits: clear the last line (the typed text) from the
            # capture, simulating the composer being cleared on submit.
            if [ -n "${DJ_FAKE_TMUX_CAPTURE:-}" ] && [ -s "$DJ_FAKE_TMUX_CAPTURE" ]; then
              _tmp=$(mktemp 2>/dev/null) || _tmp="${DJ_FAKE_TMUX_CAPTURE}.tmp"
              sed '$d' "$DJ_FAKE_TMUX_CAPTURE" > "$_tmp" 2>/dev/null && mv -f "$_tmp" "$DJ_FAKE_TMUX_CAPTURE"
...
#!/usr/bin/env bash
set -u
COMPOSER="${DJ_FAKE_COMPOSER:?DJ_FAKE_COMPOSER unset}"
write_composer() {
  text=$1
  width=$((${#text} + 4))
  border=
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ "$is_enter" = 1 ]; then
      if [ -n "${DJ_FAKE_SWALLOW:-}" ] && [ -f "$DJ_FAKE_SWALLOW" ]; then
        [ "${DJ_FAKE_PERSIST_SWALLOW:-0}" = 1 ] || rm -f "$DJ_FAKE_SWALLOW"
      else
        [ -n "${DJ_FAKE_SENT:-}" ] && printf '[ENTER]\n' >> "$DJ_FAKE_SENT"
        write_composer ""
      fi
    elif [ "$lit" = 1 ]; then
      [ "${DJ_FAKE_SEND_FAIL:-0}" = 1 ] && exit 1
      [ -n "${DJ_FAKE_SENT:-}" ] && printf '%s\n' "$text" >> "$DJ_FAKE_SENT"
      write_composer "$text"
    fi
    exit 0 ;;
esac
