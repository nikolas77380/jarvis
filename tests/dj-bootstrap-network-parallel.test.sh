...
# suite drives bin/dj-bootstrap.sh's network-only phase through DJ_SSH_BIN, so
...
TMP_ROOT=$(dj_test_tmproot dj-bootstrap-network-parallel)
...
command -v python3 >/dev/null 2>&1 \
  || fail "python3 is required to decode the dj-on.sh argv payload"
...
[ -z "${DJ_TEST_EVIDENCE_FILE:-}" ] || : > "$DJ_TEST_EVIDENCE_FILE"
...
case "$host" in
  "${DJ_FAKE_SSH_UNREACHABLE_HOST:-host-bravo}")
    exit 255
    ;;
esac
case "$command_name" in
  ...
  dj-remote-secondmate-control.sh)
    case "$subcommand" in
      ...
      sync)
        case "$host" in
          "${DJ_FAKE_SSH_FAIL_HOST:-host-alpha}")
            printf 'synthetic tracked-file sync refusal for %s\n' "$host"
            exit 1
            ;;
          "${DJ_FAKE_SSH_DIRTY_HOST:-host-charlie}")
...
if [ "\$slow" -eq 1 ]; then
  ...
  sleep "\${DJ_FAKE_GIT_FETCH_SLEEP:-0.4}"
...
test_remote_probe_scheduling_keeps_per_mate_lines() { # <parallel|fallback>
  ...
  dj_fake_exit0 "$fakebin" gh treehouse tmux node
  log="$dir/probe.log"
  : > "$log"
...
#!/usr/bin/env bash
case "\$*" in *dj-bootstrap-par.XXXXXX*) exit 1 ;; esac
exec '$REAL_MKTEMP' "\$@"
SH
  ...
  dj_write_secondmate_meta "$home/state/alpha.meta" "$alpha_home"
  printf 'remote_host=host-alpha\n' >> "$home/state/alpha.meta"
  dj_write_secondmate_meta "$home/state/bravo.meta" "$bravo_home"
  printf 'remote_host=host-bravo\n' >> "$home/state/bravo.meta"
  dj_write_secondmate_meta "$home/state/charlie.meta" "$charlie_home"
  printf 'remote_host=host-charlie\n' >> "$home/state/charlie.meta"

  dj_git_init_commit "$home/projects/alpha"
  dj_git_add_origin "$home/projects/alpha" "$dir/alpha.origin.git"

  out=$(
    PATH="$fakebin:$BASE_PATH" \
    DJ_HOME="$home" \
    DJ_ROOT_OVERRIDE="$primary" \
    DJ_BOOTSTRAP_NETWORK=only \
    DJ_SSH_BIN="$fakebin/fake-ssh" \
    DJ_FAKE_SSH_LOG="$log" \
    DJ_FAKE_SSH_SLEEP=0.4 \
    DJ_FAKE_SSH_UNREACHABLE_HOST=host-bravo \
    DJ_FAKE_SSH_FAIL_HOST=host-alpha \
    DJ_FAKE_SSH_DIRTY_HOST=host-charlie \
    DJ_FAKE_GIT_FETCH_SLEEP=0.4 \
    DJ_INHERITABLE_CONFIG='' \
    DJ_FAKE_TREEHOUSE_LEASE_HELP=1 \
    "$ROOT/bin/dj-bootstrap.sh" 2>&1
  )
...
EOF
  ...
  doctor_overlap=$(starts_before_first_end "$log" 'dj-remote-doctor.sh')
  ...
  liveness_starts=$(grep -c '^START .* dj-remote-secondmate-control.sh state$' "$log" || true)
...
echo "# all dj-bootstrap-network-parallel tests passed"
