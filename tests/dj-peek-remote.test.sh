#!/usr/bin/env bash
# dj-peek remote-secondmate capture routing.
#
...
# dj-peek + dj-on executables with a stubbed ssh transport (DJ_SSH_BIN seam)
...
TMP_ROOT=$(dj_test_tmproot dj-peek-remote)
...
#!/usr/bin/env bash
cat > /dev/null
[ -z "${DJ_FAKE_REMOTE_CAPTURE:-}" ] || printf '%s\n' "$DJ_FAKE_REMOTE_CAPTURE"
exit "${DJ_FAKE_SSH_RC:-0}"
SH
  chmod +x "$fb/fake-ssh"
  cat > "$fb/tmux" <<'SH'
#!/usr/bin/env bash
printf 'tmux\n' >> "${DJ_FAKE_TMUX_TOUCHED:?}"
exit 1
SH
  chmod +x "$fb/tmux"
  printf '%s\n' "$fb"
...
test_remote_peek_reads_remote_pane() {
  ...
  out=$(env PATH="$fb:$PATH" \
    DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" \
    DJ_SSH_BIN="$fb/fake-ssh" DJ_FAKE_SSH_RC=0 \
    DJ_FAKE_REMOTE_CAPTURE='● the remote mate is mid-refactor' \
    DJ_FAKE_TMUX_TOUCHED="$touched" \
  ...
  pass "dj-peek remote: the capture routes over the remote transport, local adapters untouched"
}
...
test_remote_peek_unreachable_fails_loudly_without_death_claim() {
  ...
  env PATH="$fb:$PATH" \
    DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" \
    DJ_SSH_BIN="$fb/fake-ssh" DJ_FAKE_SSH_RC=255 \
    DJ_FAKE_TMUX_TOUCHED="$touched" \
  ...
  pass "dj-peek remote: an unreachable host fails loudly without a false death claim"
}

test_remote_peek_reads_remote_pane
test_remote_peek_unreachable_fails_loudly_without_death_claim

echo "all dj-peek-remote tests passed"
