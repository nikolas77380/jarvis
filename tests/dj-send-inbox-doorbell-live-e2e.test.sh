#!/usr/bin/env bash
# tests/dj-send-inbox-doorbell-live-e2e.test.sh - the live doorbell guard
...
# written into the stub, so per .agents/skills/jarvis-coding-guidelines
# this is proven against every INSTALLED verified harness: each is launched
# idle in an isolated tmux server, steered through the REAL dj-send (durable
...
#
# Run explicitly with DJ_SEND_INBOX_LIVE_E2E=1. This test spends a small
...
# DJ_SEND_INBOX_LIVE_HARNESSES="claude codex ..." when needed, and tune the
# per-harness wait with DJ_SEND_INBOX_LIVE_TIMEOUT (seconds, default 240).
...
if [ "${DJ_SEND_INBOX_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_SEND_INBOX_LIVE_E2E=1 to run the live steering-inbox doorbell guard"
  exit 0
fi

command -v tmux >/dev/null 2>&1 || { echo "not ok - DJ_SEND_INBOX_LIVE_E2E=1 but tmux is not installed" >&2; exit 1; }
unset NO_MISTAKES_GATE

SOCKET="dj-inbox-live-$$"
SESSION="inboxlive"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-inbox-live.XXXXXX")
LAB=$(cd "$LAB" && pwd)
TIMEOUT=${DJ_SEND_INBOX_LIVE_TIMEOUT:-240}
CHECKED=0
FAILED=0
...
# dj-send and the composer readiness read both reach tmux through bare `tmux`
...
# bin/dj-spawn.sh uses), so the doorbell-triggered shell actions need no
...
check_harness_doorbell() {  # <name>
  ...
  if ! DJ_HOME="$home" DJ_ROOT_OVERRIDE="$home" "$ROOT/bin/dj-send.sh" "$task" \
    "Jarvis live check: run exactly this shell command now: touch $acted - then follow the mv instruction you were given for this message. Reply with one short line." \
    >/dev/null 2>&1; then
    FAILED=1
    printf 'not ok - %s (%s): dj-send refused the live steer\n' "$name" "$version" >&2
    tmux -L "$SOCKET" kill-window -t "$SESSION:$win" 2>/dev/null || true
    return 0
  fi
  ...
  [ -f "$rec" ] || {
    FAILED=1
    printf 'not ok - %s (%s): dj-send left no durable inbox record\n' "$name" "$version" >&2
    tmux -L "$SOCKET" kill-window -t "$SESSION:$win" 2>/dev/null || true
    return 0
  }
  i=0
  while [ "$i" -lt "$TIMEOUT" ]; do
    ...
    if [ "$i" -eq $((TIMEOUT / 2)) ] && [ -f "$rec" ]; then
      dj_task_inbox_ring tmux "$SESSION:$win" "$rec" || true
...
HARNESSES=${DJ_SEND_INBOX_LIVE_HARNESSES:-'claude codex opencode pi grok kimi muse'}
