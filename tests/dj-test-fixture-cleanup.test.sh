...
# (dj_test_tmproot / dj_test_cleanup / dj_test_reap_orphans).
#
# The near-universal call pattern across this suite is
# `TMP_ROOT=$(dj_test_tmproot prefix)`, which forks a subshell to capture the
...
test_fixture_root_gone_after_normal_exit() {
  ...
  child_out=$(bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(dj_test_tmproot dj-test-cleanup-exit)
    printf "%s\n" "$d"
  ...
  assert_absent "$child_dir" \
    "dj_test_tmproot's fixture root survived its owning process's normal exit"
  pass "dj_test_tmproot cleans up its fixture root on normal exit"
}

test_fixture_root_gone_after_sigterm() {
  local harness dirfile child_dir pid tries
  harness=$(dj_test_tmproot dj-test-cleanup-sigterm-harness)
  dirfile="$harness/child-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(dj_test_tmproot dj-test-cleanup-term)
    printf "%s\n" "$d" > "'"$dirfile"'"
  ...
  wait "$pid" 2>/dev/null
  assert_absent "$child_dir" \
    "dj_test_tmproot's fixture root survived SIGTERM to its owning process"
  pass "dj_test_tmproot cleans up its fixture root on SIGTERM"
}

test_cleanup_registry_resists_precreation() {
  local harness shared_tmp victim
  harness=$(dj_test_tmproot dj-test-cleanup-registry-harness)
  shared_tmp="$harness/shared-tmp"
  victim="$harness/victim"
  mkdir -p "$shared_tmp" "$victim"

  TMPDIR="$shared_tmp" bash -c '
    printf "%s\n" "$1" > "$TMPDIR/.dj-test-cleanup.$$"
    . "$2"
  ' _ "$victim" "$LIB"
...
test_fixture_registration_failure_rolls_back_root() {
  ...
  harness=$(dj_test_tmproot dj-test-cleanup-registration-harness)
  failure_tmp="$harness/tmp"
  registry_dir="$harness/registry-dir"
  mkdir -p "$failure_tmp" "$registry_dir"

  if output=$(TMPDIR="$failure_tmp" DJ_TEST_CLEANUP_REGISTRY="$registry_dir" \
    dj_test_tmproot dj-test-cleanup-registration-failure 2>/dev/null); then
    fail "dj_test_tmproot succeeded after its cleanup registry rejected registration"
  fi
  [ -z "$output" ] || fail "dj_test_tmproot published an unregistered fixture root"
  for leaked_root in "$failure_tmp"/dj-test-cleanup-registration-failure.*; do
    [ ! -e "$leaked_root" ] || fail "dj_test_tmproot leaked a root after registration failed"
  done
...
test_orphan_sweep_respects_fixture_ownership() {
  ...
  harness=$(dj_test_tmproot dj-test-cleanup-orphan-harness)
  dirfile="$harness/active-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(dj_test_tmproot dj-test-cleanup-active)
    printf "%s\n" "$d" > "'"$dirfile"'"
    while :; do sleep 0.1; done
  ' &
  pid=$!
  ...
  touch -t 202001010000 "$active_dir/.dj-test-fixture"

  stale_dir=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-cleanup-stale.XXXXXX")
  printf '%s\n%s\n' "$$" reused-process-identity > "$stale_dir/.dj-test-fixture"
  touch -t 202001010000 "$stale_dir/.dj-test-fixture"
  fresh_dir=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-cleanup-fresh.XXXXXX")
  : > "$fresh_dir/.dj-test-fixture"
