#!/usr/bin/env bash
# tests/dj-classify-decision-key.test.sh - decision-key position tolerance in
# the open-decisions fold (bin/dj-classify-lib.sh). A "[key=<slug>]" token is
...
# from one handed to a durable captain-held task (bin/dj-captain-hold.sh
# diverged). Cross-drain cursor persistence and the incremental
# cost bound live in tests/dj-wake-drain-open-decisions-cursor.test.sh; the
# drain wiring lives in tests/dj-wake-drain-open-decisions.test.sh.
set -u
...
TMP_ROOT=$(dj_test_tmproot dj-classify-decision-key-tests)
...
test_resolution_closes_across_positions() {
  ...
  # Opened colon-first, closed in the documented form (what dj-send's
