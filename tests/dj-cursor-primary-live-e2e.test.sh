#!/usr/bin/env bash
# Opt-in live guard for Cursor Agent CLI as a jarvis PRIMARY.
#
...
#
# tests/dj-cursor-primary.test.sh and the Cursor cases in
# tests/dj-tmux-agent-liveness.test.sh are the portable regressions that run
...
#
# Isolation: a throwaway jarvis home under a temp dir, a private tmux socket,
...
if [ "${DJ_CURSOR_PRIMARY_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_CURSOR_PRIMARY_LIVE_E2E=1 to run the live Cursor primary guard"
  exit 0
fi
...
CURSOR_BIN=${DJ_CURSOR_BIN:-$(command -v cursor-agent || true)}
[ -n "$CURSOR_BIN" ] && [ -x "$CURSOR_BIN" ] \
  || fail "cursor-agent not found; install it or set DJ_CURSOR_BIN. This guard refuses to pass without checking the real harness."
...
SOCKET="dj-cursor-primary-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-cursor-primary.XXXXXX")
HOME_DIR="$LAB/home"
MARKER="DJ_CURSOR_LIVE_MARKER_$$"
...
"$REAL_TMUX" -L "$SOCKET" new-session -d -s primary -x 220 -y 60 -c "$HOME_DIR" \
  "cd '$HOME_DIR' && DJ_HOME='$HOME_DIR' DJ_HEARTBEAT=30 DJ_HEARTBEAT_MAX=30 exec '$CURSOR_BIN' --trust --yolo --workspace '$HOME_DIR'" \
...
wait_for_pane "JARVIS_OP: v1 watcher:" 300 "a watcher wake delivered as a stop-hook follow-up"
...
sleep 5
LIVE_PARKS=$(pgrep -f "$HOME_DIR/bin/dj-turnend-guard-cursor.sh" 2>/dev/null | wc -l | tr -d ' ')
[ "${LIVE_PARKS:-0}" -le 1 ] \
...
export DJ_STATE_OVERRIDE="$HOME_DIR/state"
export DJ_SUPERVISOR_TARGET=primary
export DJ_SUPERVISOR_BACKEND=tmux
export DJ_DAEMON_PRIMARY_HARNESS=cursor
. "$HOME_DIR/bin/dj-supervise-daemon.sh"
composer=\$(dj_backend_composer_state tmux primary)
printf 'composer=%s\n' "\$composer"
