...
# across bin/dj-spawn.sh, bin/dj-promote.sh, and bin/dj-project-mode.sh.
#
# A ship task's delivery mode and yolo posture are jarvis's decision at intake,
...
PROJECT_MODE="$ROOT/bin/dj-project-mode.sh"
TMP_ROOT=$(dj_test_tmproot dj-task-delivery)
...
run_spawn() {  # <home> <fakebin> <spawn-args...>
  ...
  DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    ...
    DJ_PROJECTS_OVERRIDE="$TMP_ROOT/projects-unused" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_SPAWN_NO_GUARD=1 DJ_BACKEND=tmux PATH="$fakebin:$PATH" \
    "$SPAWN" "$@" 2>&1
}
...
ROWS
  pass "dj-spawn: a ship spawn requires a valid explicit mode and yolo before anything is created"
}
...
EOF
  ...
  pass "dj-spawn: scout and secondmate spawns refuse ship delivery flags"
}
...
EOF
  ...
  pass "dj-spawn: the brief's recorded mode and the spawn's explicit mode must agree"
}
...
ROWS
  pass "dj-spawn: a rigor downgrade against the registered posture is announced, never blocked"
}
...
EOF
  ...
  pass "dj-spawn: a scout spawn resolves no delivery posture from the registry"
}
...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  write_scout_meta() {
    printf 'window=dj-promote-d1\nkind=scout\nworktree=/tmp/wt\n' > "$meta"
  }

  write_scout_meta
  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 2>&1)
  status=$?
  ...
  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode direct-PR 2>&1)
  status=$?
  ...
  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode no-mistakes-prod-only --yolo off 2>&1)
  status=$?
  ...
  out=$(DJ_HOME="$home" DJ_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode direct-PR --yolo on 2>&1)
  status=$?
  ...
  pass "dj-promote: promotion requires the delivery contract and records it exactly once"
}
...
test_promote_requires_and_records_the_delivery_contract
test_project_mode_maps_the_conditional_policy
echo "# all dj-task-delivery tests passed"
