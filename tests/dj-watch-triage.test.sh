#!/usr/bin/env bash
# tests/dj-watch-triage.test.sh - the always-on wake triage built into
# bin/dj-watch.sh and the shared classifier (bin/dj-classify-lib.sh). The watcher
# now absorbs the benign majority of wakes in bash and exits ONLY on an actionable
# wake, so jarvis's LLM re-arms once per actionable event instead of once per
# wake. These tests cover the classifier predicates as pure functions, then drive
# a real dj-watch.sh subprocess to assert the behavioral contract:
...
#
# Daemon-side classification/injection lives in dj-daemon.test.sh; watcher/lock
# liveness in dj-watcher-lock.test.sh; the durable-queue safety matrix in
# dj-wake-queue.test.sh.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/dj-classify-lib.sh"

WATCH="$ROOT/bin/dj-watch.sh"
DRAIN="$ROOT/bin/dj-wake-drain.sh"

TMP_ROOT=$(dj_test_tmproot dj-watch-triage-tests)

ack_stopped_cycle() {  # <state>
  local state=$1 err sequence generation
  err="$state/.test-cycle-drain.err"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || return 1
  sequence=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation [A-Za-z0-9._-][A-Za-z0-9._-]*$/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through [0-9][0-9]* --recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  rm -f "$err"
  [ -n "$sequence" ] && [ -n "$generation" ] || return 1
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$sequence" \
    --recovery-generation "$generation"
}

# Common watcher knobs: tight poll/grace, no check or heartbeat cadence unless a
# test overrides them, so a test only exercises the path it targets. DJ_CREW_STATE_BIN
# points at the case's hermetic fake dj-crew-state.sh (installed by make_case) so the
# absorb-only-when-provably-working triage reads a canned verdict; a test fixes that
# verdict via DJ_FAKE_CREW_STATE in its environment before calling watch_bg.
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  local state=$1 fakebin=$2 out=$3
  shift 3
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$@" "$WATCH" > "$out" &
}
...
# A fixed wait_live budget only proves the process is still ALIVE: dj-watch.sh
...
# watcher takes that long to decide, but because dj-watch.sh does bounded
...
# fallback (which writes a partial filesystem dump on Linux; see dj-watch.sh).
file_mtime() {
...
# fire on a pre-existing status (mirrors dj-watch.sh's stat_sig exactly).
seen_sig() {
...
record_pi_busy() {  # <state-dir> <id>
  ...
  gen=$("$ROOT/bin/dj-busy-event.sh" arm "$state" "$id")
  "$ROOT/bin/dj-busy-event.sh" apply "$state" "$id" busy --gen "$gen" \
    --source pi-ext --event agent-start
}

reap() { kill "$1" 2>/dev/null || true; wait "$1" 2>/dev/null || true; }

# --- pure classifier predicates (dj-classify-lib.sh) ------------------------
...
test_stale_is_terminal_classifier() {
  ...
  printf 'done: ready in branch dj/x\n' > "$state/term.status"
  stale_is_terminal "sess:dj-term" "$state" || fail "terminal stale status not classified terminal"
  dj_write_meta "$state/herdr-term.meta" "window=default:w1:p2" "backend=herdr"
  ...
  stale_is_terminal "sess:dj-nonterm" "$state" && fail "non-terminal stale classified terminal"
  stale_is_terminal "sess:dj-missing" "$state" && fail "stale with no status classified terminal"
...
test_classifier_primitives() {
  ...
  [ "$(window_to_task "sess:dj-fix-login-k3")" = "fix-login-k3" ] || fail "window_to_task did not strip session+dj- prefix"
  ...
  DJ_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "custom-verb: x" || fail "DJ_CAPTAIN_RE override not honored"
  DJ_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "done: x" && fail "DJ_CAPTAIN_RE override did not replace the default verb set"
  DJ_CAPTAIN_RE='merged|custom-verb:' status_is_captain_relevant "working: rebased onto merged #76" \
    && fail "DJ_CAPTAIN_RE override bypassed working: suppression"
  DJ_CAPTAIN_RE='checks green|custom-verb:' status_is_captain_relevant "paused: checks green pending approval" \
    && fail "DJ_CAPTAIN_RE override bypassed paused: suppression"
  DJ_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "custom-verb: x" \
...
# benign (absorb) ONLY when dj-crew-state.sh reports the crew as working from an
...
# fake dj-crew-state.sh (DJ_CREW_STATE_BIN) returns a canned verdict per case.
...
# crew_absorb_class: the single dj-crew-state.sh read that returns BOTH absorb
...
test_crew_absorb_class_classifier() {
  ...
  DJ_FAKE_CREW_STATE='state: paused · source: status-log · awaiting upstream'
...
test_crew_worktree_written_since_classifier() {
  ...
  # Only .git churn (what jarvis's own read-only git commands touch): pruned.
  ...
  # A secondmate records a provisioned jarvis home, not a code tree, and such a
  ...
  printf 'window=test:dj-sm2\nkind=ship\nworktree=%s\n' "$home" > "$state/sm2.meta"
  ! crew_worktree_written_since sm2 "$state" "$anchor" \
    || fail "a marked jarvis home reported crew write evidence"
...
# DJ_WORKTREE_WRITE_PRUNE is a skip list, so clearing it skips nothing and is the
...
test_empty_write_prune_widens_the_probe() {
  ...
  : > "$anchor"
  set_mtime "$(( $(date +%s) - 120 ))" "$anchor"
  printf 'window=test:dj-e\nkind=ship\nworktree=%s\n' "$wt" > "$state/e.meta"
  saved=$DJ_WORKTREE_WRITE_PRUNE
  DJ_WORKTREE_WRITE_PRUNE=''
  ...
  DJ_WORKTREE_WRITE_PRUNE=$saved
  ! crew_worktree_written_since e "$state" "$anchor" \
    || fail "the default prune list stopped keeping .git out of the probe"
  pass "an empty DJ_WORKTREE_WRITE_PRUNE widens the probe to the whole depth-bounded tree instead of disabling it"
}
...
test_empty_write_prune_from_the_environment_widens_the_probe() {
  ...
  : > "$anchor"
  set_mtime "$(( $(date +%s) - 120 ))" "$anchor"
  printf 'window=test:dj-wenv\nkind=ship\nworktree=%s\n' "$wt" > "$state/wenv.meta"
  # The one thing written since the anchor sits exactly where the DEFAULT list prunes.
  printf 'pack\n' > "$wt/.git/objects/fresh"
  env -u DJ_WORKTREE_WRITE_PRUNE \
    bash -c '. "$1"; crew_worktree_written_since wenv "$2" "$3"' _ \
    "$ROOT/bin/dj-classify-lib.sh" "$state" "$anchor" \
    && fail "the default skip list let .git churn count as write evidence"
  DJ_WORKTREE_WRITE_PRUNE='' \
    bash -c '. "$1"; crew_worktree_written_since wenv "$2" "$3"' _ \
    "$ROOT/bin/dj-classify-lib.sh" "$state" "$anchor" \
    || fail "an empty DJ_WORKTREE_WRITE_PRUNE in the environment fell back to the default skip list instead of widening the probe"
  pass "an empty DJ_WORKTREE_WRITE_PRUNE exported into the environment prunes nothing, widening the probe"
}
...
SH
  ...
  started=$(date +%s)
  PATH="$slowbin:$PATH" DJ_WORKTREE_WRITE_TIMEOUT=1 \
    bash -c '. "$1"; crew_worktree_written_since slow "$2" "$3"' _ \
    "$ROOT/bin/dj-classify-lib.sh" "$state" "$anchor" \
...
test_signal_crew_provably_working_classifier() {
  ...
  export DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh"
  export DJ_FAKE_CREW_STATE_a='state: working · source: run-step · running'
  export DJ_FAKE_CREW_STATE_b='state: done · source: run-step · run passed'
  ...
  unset DJ_FAKE_CREW_STATE_a DJ_FAKE_CREW_STATE_b
...
test_secondmate_status_signal_never_absorbed_classifier() {
  ...
  export DJ_FAKE_CREW_STATE_sm='state: working · source: run-step · running'
  ...
  export DJ_FAKE_CREW_STATE_crew='state: working · source: run-step · running'
  printf 'kind=ship\n' > "$state/crew.meta"
  ...
  unset DJ_FAKE_CREW_STATE_sm DJ_FAKE_CREW_STATE_crew
...
test_provably_working_signal_absorbed() {
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
...
test_turn_ended_provably_working_absorbed() {
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: pane · harness busy'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_turn_ended_not_working_surfaced() {
  ...
  export DJ_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced turn-end failed"
...
test_working_note_not_working_surfaced() {
  ...
  status_file="$state/task.status"
  printf 'working: compiling step 2\n' > "$status_file"
  # A non-no-mistakes crew (no run) whose pane went idle: dj-crew-state falls back
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: status-log · working: compiling step 2'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced working: note failed"
...
test_secondmate_status_note_surfaced_despite_busy_agent() {
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: run-step · running'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced note failed"
...
test_self_announced_close_does_not_rewake_but_next_note_does() {
  ...
  rc=0
  DJ_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    dj_wake_status_append_self_announced "$2" "$3" "resolved [key=k1]: answered: closed by this home"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$state" "$status_file" || rc=$?
  [ "$rc" -eq 0 ] || fail "the bookkeeping close was not self-announced (rc=$rc)"
  export DJ_FAKE_CREW_STATE='state: unknown · source: none · idle worker'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_actionable_signal_surfaced() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the actionable signal failed"
...
test_terminal_stale_surfaced() {
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the terminal stale failed"
...
# log gets no new entry once jarvis hands it to a no-mistakes validation
...
test_stale_terminal_status_overridden_by_active_run() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:dj-validating"
  printf 'no-mistakes axi run: validating...' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/validating.meta"
  # The crew reported done BEFORE jarvis triggered no-mistakes validation;
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_STALE_ESCALATE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$out"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_STALE_ESCALATE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: run-step · ci running'
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the wedge escalation failed"
...
test_nonterminal_stale_not_working_surfaced() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the immediate stale failed"
...
test_nonterminal_stale_paused_absorbed_then_resurfaced() {
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  # crew_absorb_class reads the declared pause from dj-crew-state.sh.
  export DJ_FAKE_CREW_STATE='state: paused · source: status-log · holding for the upstream tool release'

  # Phase A: a fresh pause (status file just written) under a high re-surface
  # threshold is absorbed - no wake, no wedge timer.
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_FAKE_TMUX_CURRENT_COMMAND=zsh \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_FAKE_TMUX_CURRENT_COMMAND=zsh \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the paused re-surface failed"
  grep "$(printf '\tstale\t')" "$drain_out" | grep -F "$window" >/dev/null || fail "paused re-surface was not queued"
  pass "a declared pause is absorbed on first sight, then re-surfaced as a recheck past the threshold, never wedge-escalated"
}

# A captain-held crew can leave a stable backend endpoint after its agent exits.
# dj-crew-state then authoritatively reports stopped rather than paused, but the
...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  ...
  while [ "$round" -le 6 ]; do
    PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
      DJ_FAKE_TMUX_CURRENT_COMMAND=zsh DJ_FAKE_CREW_STATE='state: stopped · source: pane · bare shell' \
      DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
      DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" >> "$out" &
    pid=$!
    if wait_poll_cycle "$state" "$pid"; then
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_FAKE_TMUX_CURRENT_COMMAND=grok DJ_FAKE_CREW_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_FAKE_TMUX_CURRENT_COMMAND=grok DJ_FAKE_CREW_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_STALE_ESCALATE_SECS=240 DJ_PAUSE_RESURFACE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"
...
test_secondmate_paused_resurfaces_in_normal_mode() {
  ...
  window="test:dj-secondmate-held"
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  export DJ_FAKE_CREW_STATE='state: paused · source: status-log · awaiting the upstream release'
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_secondmate_captain_held_resurfaces_in_normal_mode() {
  ...
  window="test:dj-secondmate-hold"
...
test_secondmate_nonpaused_stale_remains_suppressed() {
  ...
  window="test:dj-secondmate-working"
...
test_secondmate_unpause_clears_pause_tracking() {
  ...
  out="$dir/watch.out"; statusf="$state/secondmate-resumed.status"; window="test:dj-secondmate-resumed"
...
test_nonterminal_stale_pause_transitions_reclassify_unchanged_hash() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-transition"
  printf 'idle awaiting external\n' > "$capture_file"
...
test_nonterminal_paused_rechecks_authoritative_state() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-pause-recheck"
  printf 'idle awaiting external\n' > "$capture_file"
...
test_paused_authoritative_working_preserves_wedge_timer() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-paused-working"
  printf 'idle awaiting external\n' > "$capture_file"
...
# DJ_WEDGE_DEMAND_INSPECT_COUNT consecutive escalations on the SAME pane, the
...
test_busy_pane_below_turn_age_bound_is_absorbed() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-fresh"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_BUSY_TURN_MAX_SECS=999 DJ_STALE_ESCALATE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_busy_pane_stable_hash_escalates_past_turn_age_bound() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-stable"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$out"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_busy_pane_changing_hash_escalates_past_turn_age_bound() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-ticking"
...
test_busy_pane_turn_end_touch_resets_age() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-reset"
  printf 'Working...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_BUSY_TURN_MAX_SECS=3600 DJ_STALE_ESCALATE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_busy_pane_repeated_escalation_reaches_demand_deep_inspection() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-demand-inspect"
  printf 'Working...' > "$capture_file"
...
test_busy_declared_pause_is_rechecked_not_wedge_escalated() {
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=1 DJ_PAUSE_RESURFACE_SECS=999 \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=1 DJ_PAUSE_RESURFACE_SECS=240 \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=999 DJ_PAUSE_RESURFACE_SECS=999 \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    DJ_BUSY_TURN_MAX_SECS=1 DJ_STALE_ESCALATE_SECS=240 DJ_PAUSE_RESURFACE_SECS=999 \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
# Behavioral proof that the production default (no DJ_BUSY_TURN_MAX_SECS override
...
test_busy_pane_default_turn_age_bound_is_3600s() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:dj-busy-default"
  printf 'Working...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_STALE_ESCALATE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_wedge_escalation_deferred_while_worktree_is_written() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:dj-writing"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_STALE_ESCALATE_SECS=240 \
    DJ_PAUSE_RESURFACE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the stalled-crew escalation failed"
...
test_write_deferral_resurfaces_on_the_bounded_cadence() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:dj-churn"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_STALE_ESCALATE_SECS=240 \
    DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the write-deferral recheck failed"
...
# The worktree recorded for a secondmate is a provisioned jarvis home, and that
...
test_secondmate_home_supervision_churn_is_not_write_evidence() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:dj-mate"; home="$dir/mate-home"
  mkdir -p "$home/state"
  printf 'sm-mate\n' > "$home/.dj-secondmate-home"
  printf 'Working... (12.3s)' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_STALE_ESCALATE_SECS=240 DJ_BUSY_TURN_MAX_SECS=1 DJ_PAUSE_RESURFACE_SECS=999 \
    DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the mate escalation failed"
...
test_timer_repair_drops_a_finished_write_deferral_chain() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:dj-chain-repair"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_STALE_ESCALATE_SECS=240 DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_terminal_first_sight_drops_a_finished_write_deferral_chain() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:dj-chain-firstsight"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'no-mistakes axi run: validating...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_STALE_ESCALATE_SECS=999 DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$state/.writing-since-$key"
  set_mtime "$back" "$state/.writing-since-$key"
  DJ_FAKE_CREW_STATE='state: unknown · source: none · no run, no busy pane'
  : > "$out"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" \
    DJ_STALE_ESCALATE_SECS=999 DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
...
SH
  ...
  export DJ_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 DJ_WATCH_TRIAGE_LOG_MAX_BYTES=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
# Run the runner against a case home. DJ_ROOT_OVERRIDE (exported by the shared
...
pe_case() {  # <dir> <command>...
  local dir=$1
  shift
  (unset DJ_ROOT_OVERRIDE
   DJ_PROCEVENT_CLAIM_ROOT="$dir/claims" DJ_HOME="$dir" "$ROOT/bin/dj-procevent.sh" "$@")
}
...
# The watcher, scoped by DJ_HOME rather than DJ_STATE_OVERRIDE, so the
# per-cycle reconcile it launches resolves the same home's state.
procevent_watch_bg() {  # <dir> <out>
  local dir=$1 out=$2
  PATH="$dir/fakebin:$PATH" DJ_HOME="$dir" DJ_PROCEVENT_CLAIM_ROOT="$dir/claims" \
    DJ_CREW_STATE_BIN="$dir/fakebin/dj-crew-state.sh" \
    DJ_POLL=0.2 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
}
...
test_procevent_captured_result_surfaces_proactively() {
  ...
  beacon_age=$(DJ_STATE_OVERRIDE="$state" bash -c \
    '. "$1/bin/dj-wake-lib.sh"; dj_path_age "$2"' _ "$ROOT" "$state/.last-watcher-beat")
...
test_procevent_unacknowledged_result_redrains_until_handled() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "drain after the first process-event wake failed"
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$sequence" --recovery-generation "$generation" \
...
test_procevent_marker_keys_are_injective() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "marker identity fixture drain failed"
...
case "$dest" in
  */.seen-procevent-*)
    case "${DJ_MARKER_MV_MODE:-}" in
      pause)
        printf '1\n' > "$DJ_MARKER_MV_READY"
        while [ ! -e "$DJ_MARKER_MV_RELEASE" ]; do sleep 0.02; done
        ;;
...
test_procevent_surface_serializes_with_drain() {
  ...
  install_marker_mv_fault "$dir"
  DJ_MARKER_MV_MODE=pause DJ_MARKER_MV_READY="$ready" DJ_MARKER_MV_RELEASE="$release" \
    procevent_watch_bg "$dir" "$out"
  pid=$!
  wait_numeric_file "$ready" 100 || fail "the watcher never reached its marker commit boundary"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" &
  drain_pid=$!
...
test_procevent_surface_crash_boundaries() {
  ...
  PATH="$dir/fakebin:$PATH" DJ_HOME="$dir" DJ_PROCEVENT_CLAIM_ROOT="$dir/claims" \
    DJ_CREW_STATE_BIN="$dir/fakebin/dj-crew-state.sh" DJ_POLL=0.2 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$fifo" &
  pid=$!
  wait "$reader" || true
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  install_marker_mv_fault "$dir"
  DJ_MARKER_MV_MODE=kill-before procevent_watch_bg "$dir" "$out"; pid=$!
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  install_marker_mv_fault "$dir"
  DJ_MARKER_MV_MODE=kill-after procevent_watch_bg "$dir" "$out"; pid=$!
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out.replay.drain" 2> "$replay_err" \
...
test_procevent_marker_failure_exits_and_replays() {
  ...
  install_marker_mv_fault "$dir"
  DJ_MARKER_MV_MODE=fail procevent_watch_bg "$dir" "$out"
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "marker-failure fixture drain failed"
...
test_heartbeat_no_change_absorbed() {
  ...
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  # DJ_HEARTBEAT, which need not be the first completed cycle, so wait for the
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  # fleet-scan backstop must catch it and wake jarvis.
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the backstop heartbeat failed"
...
test_beacon_stays_fresh_while_absorbing() {
  ...
  reap "$pid"
  pass "the liveness beacon stays fresh while the watcher absorbs benign wakes (dj-guard never false-alarms)"
}
...
test_afk_present_reverts_watcher_to_one_shot() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
...
test_afk_paused_changed_pane_hands_off_plain_stale() {
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    DJ_FAKE_CREW_STATE='state: paused · source: status-log · awaiting the upstream tool release' \
    DJ_STATE_OVERRIDE="$state" DJ_CREW_STATE_BIN="$fakebin/dj-crew-state.sh" DJ_PAUSE_RESURFACE_SECS=240 DJ_POLL=0.2 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after AFK paused stale failed"
