...
CHECK="$ROOT/bin/dj-doc-audience-check.sh"
INVENTORY="$ROOT/docs/documentation-audiences.json"
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/dj-doc-audiences.XXXXXX")
...
test_repository_inventory_passes() {
  ...
  assert_contains "$out" "dj-doc-audience-check: ok surfaces=" \
...
Observed version 1.2.3 on branch `dj/example`.
MD
