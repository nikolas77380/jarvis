#!/usr/bin/env bash
# Behavior tests for dj-spawn.sh concrete dispatch profile flags.
#
# These tests drive dj-spawn through meta writing and launch construction with a
...
# command jarvis would run without starting any real harness.
set -u
...
TMP_ROOT=$(dj_test_tmproot dj-spawn-dispatch-profile)
...
set -u
if [ "${1:-}" = --help ]; then
  if [ "${DJ_FAKE_PI_VERSION:-0.84.0}" = 0.82.0 ]; then
    printf '%s\n' 'Pi 0.82.0' 'Options: --help'
  else
    printf '%s\n' "Pi ${DJ_FAKE_PI_VERSION:-0.84.0}" 'Options: --help --tui-mode <mode>'
  fi
fi
exit 0
SH
...
SH
  ...
  dj_fake_exit0 "$fakebin" treehouse
...
if [ "${1:-}" = --list-models ]; then
  [ "${DJ_FAKE_CURSOR_LIST_STATUS:-0}" -eq 0 ] || exit "${DJ_FAKE_CURSOR_LIST_STATUS}"
  printf '%b\n' "${DJ_FAKE_CURSOR_MODELS:-Available models\ncursor-grok-4.5-high - Grok 4.5 High}"
fi
exit 0
SH
...
run_spawn() {
  ...
  shift 4
  : > "$launchlog"
  # CLAUDE_CONFIG_DIR is forwarded onto claude launches by dj-spawn, so pin it
  # explicitly (empty by default) instead of leaking the invoking shell's value,
  # which would make launch assertions depend on the developer's environment.
  # A test opts in to the set case via DJ_TEST_CLAUDE_CONFIG_DIR.
  DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    ...
    CLAUDE_CONFIG_DIR="${DJ_TEST_CLAUDE_CONFIG_DIR:-}" \
    DJ_FAKE_LAUNCH_LOG="$launchlog" DJ_FAKE_PI_VERSION="${DJ_TEST_PI_VERSION:-0.84.0}" \
    DJ_FAKE_CURSOR_MODELS="${DJ_TEST_CURSOR_MODELS:-}" \
    DJ_FAKE_CURSOR_LIST_STATUS="${DJ_TEST_CURSOR_LIST_STATUS:-0}" \
...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions \"\$('${ROOT}/bin/dj-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/brief.md')\""
...
test_relative_home_overrides_launch_with_absolute_cross_process_paths() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    CDPATH="$CASE_DIR/cdpath" DJ_ROOT_OVERRIDE='' DJ_HOME=home \
      DJ_STATE_OVERRIDE=home/state DJ_DATA_OVERRIDE=home/data \
      DJ_PROJECTS_OVERRIDE=home/projects DJ_CONFIG_OVERRIDE=home/config \
      DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' DJ_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
  ...
  assert_contains "$launch" "-e '$home_real/state/$id.pi-ext.ts'" \
    "relative DJ_STATE_OVERRIDE leaked into Pi's cross-process extension path"
  assert_contains "$launch" "< '$home_real/data/$id/brief.md'" \
    "relative DJ_DATA_OVERRIDE leaked into the cross-process brief path"
...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    DJ_ROOT_OVERRIDE='' DJ_HOME=home \
      DJ_STATE_OVERRIDE='' DJ_DATA_OVERRIDE='' \
      DJ_PROJECTS_OVERRIDE=home/projects DJ_CONFIG_OVERRIDE=home/config \
      DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' DJ_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
      GROK_HOME=home/grok-home PATH="$FAKEBIN_DIR:$PATH" \
  ...
  status=$?
  expect_code 0 "$status" "spawn with relative DJ_HOME defaults should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "-e '$home_real/state/$relative_id.pi-ext.ts'" \
    "relative DJ_HOME leaked into Pi's default cross-process extension path"
  assert_contains "$launch" "< '$home_real/data/$relative_id/brief.md'" \
    "relative DJ_HOME leaked into the default cross-process brief path"

  linked_home="$CASE_DIR/home-link"
  ln -s "$HOME_DIR" "$linked_home"
  : > "$LAUNCH_LOG"
  out=$(
    DJ_ROOT_OVERRIDE='' DJ_HOME="$linked_home" \
      DJ_STATE_OVERRIDE='' DJ_DATA_OVERRIDE='' \
      DJ_PROJECTS_OVERRIDE="$linked_home/projects" DJ_CONFIG_OVERRIDE="$linked_home/config" \
      DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' DJ_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
  ...
  status=$?
  expect_code 0 "$status" "spawn with absolute symlink-spelled DJ_HOME defaults should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "-e '$linked_home/state/$absolute_id.pi-ext.ts'" \
    "absolute DJ_HOME spelling changed in Pi's default cross-process extension path"
  assert_contains "$launch" "< '$linked_home/data/$absolute_id/brief.md'" \
    "absolute DJ_HOME spelling changed in the default cross-process brief path"
  pass "DJ_HOME defaults resolve relative paths and preserve absolute spellings"
}
...
test_absolute_override_spelling_is_preserved_in_launch_paths() {
  ...
  out=$(
    DJ_ROOT_OVERRIDE='' DJ_HOME="$linked_home" \
      DJ_STATE_OVERRIDE="$linked_home/state" DJ_DATA_OVERRIDE="$linked_home/data" \
      DJ_PROJECTS_OVERRIDE="$linked_home/projects" DJ_CONFIG_OVERRIDE="$linked_home/config" \
      DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' DJ_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
      GROK_HOME="$linked_home/grok-home" PATH="$FAKEBIN_DIR:$PATH" \
  ...
  assert_contains "$launch" "-e '$linked_home/state/$id.pi-ext.ts'" \
    "absolute DJ_STATE_OVERRIDE spelling changed in Pi's cross-process extension path"
  assert_contains "$launch" "< '$linked_home/data/$id/brief.md'" \
    "absolute DJ_DATA_OVERRIDE spelling changed in the cross-process brief path"
...
test_unresolvable_relative_overrides_fail_loudly() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    DJ_ROOT_OVERRIDE='' DJ_HOME=missing-home \
      DJ_STATE_OVERRIDE='' DJ_DATA_OVERRIDE='' \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative home should fail"
  assert_contains "$out" "DJ_HOME directory cannot be resolved: missing-home" \
    "spawn did not name the unresolvable DJ_HOME"

  out=$(
    cd "$CASE_DIR" || exit 1
    DJ_ROOT_OVERRIDE='' DJ_HOME=home \
      DJ_STATE_OVERRIDE=missing-state DJ_DATA_OVERRIDE=home/data \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative state override should fail"
  assert_contains "$out" "DJ_STATE_OVERRIDE directory cannot be resolved: missing-state" \
    "spawn did not name the unresolvable DJ_STATE_OVERRIDE"

  out=$(
    cd "$CASE_DIR" || exit 1
    DJ_ROOT_OVERRIDE='' DJ_HOME=home \
      DJ_STATE_OVERRIDE=home/state DJ_DATA_OVERRIDE=missing-data \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative data override should fail"
  assert_contains "$out" "DJ_DATA_OVERRIDE directory cannot be resolved: missing-data" \
    "spawn did not name the unresolvable DJ_DATA_OVERRIDE"
...
test_grok_omits_invalid_max_reasoning_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "grok --always-approve --model 'grok-4' \"\$('${ROOT}/bin/dj-operational-input.sh' encode launch-brief < " \
...
test_cursor_failed_catalog_probe_does_not_block_spawn() {
  ...
  DJ_TEST_CURSOR_LIST_STATUS=124 \
    out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" \
...
test_pi_threads_model_and_max_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "DJ_PI_HARNESS=pi '$FAKEBIN_DIR/pi' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
    "pi launch did not force the regular TUI while threading the requested model and max thinking level"
  assert_not_contains "$launch" "DJ_JARVIS_PI_LAUNCH_BRIEF=" \
    "pi launch still exports the removed Calm input-reroute binding"
  assert_contains "$launch" "dj-operational-input.sh' encode launch-brief" \
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "DJ_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
  ...
  assert_contains "$(cat "$HOME_DIR/state/$id.busy-state")" "state=busy source=dj-spawn" \
...
test_pi_tui_mode_probe_is_safe_for_old_and_new_pi() {
  ...
  for harness in pi pi-signed; do
    for version in 0.82.0 0.84.0; do
      ...
      out=$(DJ_TEST_PI_VERSION="$version" \
      ...
      assert_not_contains "$launch" "DJ_PI_HARNESS=$harness $harness" \
...
test_pi_signed_missing_binary_refuses_before_endpoint_or_metadata() {
  ...
  out=$(DJ_ROOT_OVERRIDE='' DJ_HOME="$HOME_DIR" \
    ...
    DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
    DJ_FAKE_LAUNCH_LOG="$LAUNCH_LOG" PATH="$FAKEBIN_DIR:/usr/bin:/bin:/usr/sbin:/sbin" \
    "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1)
  status=$?
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "DJ_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular -e '$sm/.pi/extensions/dj-primary-turnend-guard.ts' -e '$sm/.pi/extensions/dj-primary-pi-watch.ts'" \
...
test_claude_forwards_jarvis_config_dir_when_set() {
  local rec id out status launch
  id=profile-claude-cfgdir-z17
  rec=$(make_spawn_case profile-claude-cfgdir claude "$id")
  read_case_record "$rec"

  out=$(DJ_TEST_CLAUDE_CONFIG_DIR="/opt/test/claude-work" \
    run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  ...
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='/opt/test/claude-work' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude" \
    "claude launch did not forward jarvis's CLAUDE_CONFIG_DIR to the crewmate pane"
  pass "claude forwards jarvis's CLAUDE_CONFIG_DIR so the crewmate uses the same credential store"
}
...
test_claude_omits_config_dir_prefix_when_unset() {
  ...
  # default path where dj-spawn adds no prefix.
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 0 "$status" "claude spawn without CLAUDE_CONFIG_DIR should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_not_contains "$launch" "CLAUDE_CONFIG_DIR=" \
    "claude launch must not add a config-dir prefix when jarvis has no CLAUDE_CONFIG_DIR set"
  pass "claude omits the config-dir prefix when jarvis runs with the single-store default"
}
...
echo "# all dj-spawn-dispatch-profile tests passed"
