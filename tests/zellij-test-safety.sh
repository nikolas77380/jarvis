...
# cleanup ever touching the machine's real "jarvis" session (the default
# session name bin/backends/zellij.sh uses for actual jarvis task tabs) or
...
# "jarvis" session name, or a caller reaching for the fleet-wide
...
# explicitly, that name is NOT "jarvis" (the real default), and it is
...
# is NOT the literal "jarvis" default session name, and IS currently
...
zellij_refuse_if_unsafe() {  # <name>
  ...
  if [ "$name" = jarvis ]; then
    echo "zellij safety guard: refusing - name is literally 'jarvis' (the real default session a live fleet may use)" >&2
    return 1
  fi
...
# never-"jarvis" session running rather than risk the wrong target.
