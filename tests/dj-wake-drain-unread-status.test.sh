#!/usr/bin/env bash
# tests/dj-wake-drain-unread-status.test.sh - drain must surface every still-
...
TMP_ROOT=$(dj_test_tmproot dj-wake-drain-unread-status-tests)
...
prime_cursor() {  # <state> <status-file>
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>/dev/null \
...
test_incident_note_answer_buried_under_routine_note_surfaces_both() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on the incident shape"
...
test_already_presented_notes_are_not_replayed() {
  ...
  printf 'note: captain said use REST not RPC\n' >> "$status"
  printf 'note: re-read acknowledgement\n' >> "$status"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "first drain of unread notes failed"
  grep -F 'captain said use REST not RPC' "$out" >/dev/null \
    || fail "setup error: first drain did not surface the answer note"

  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "second drain after presentation failed"
...
test_brand_new_note_after_presentation_is_surfaced() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain of the first note failed"
  grep -F 'task3 note: first answer' "$out" >/dev/null \
    || fail "setup error: first note was not presented"

  printf 'note: follow-up after ack\n' >> "$status"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain of the brand-new note failed"
...
test_signal_annotation_surfaces_every_unread_note_not_only_the_newest() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" \
...
test_pending_reply_resolution_surfaces_once() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a pending-reply resolution"
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "second drain after pending-reply presentation failed"
...
test_unread_output_over_cap_remains_recoverable() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed for unread output over the former cap"
...
test_open_decisions_fold_is_unchanged() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a buried needs-decision plus a note"
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after resolving the keyed decision"
  if grep -F 'OPEN DECISIONS' "$out" >/dev/null; then
...
test_routine_working_lines_stay_silent_on_the_empty_queue() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with only routine working/done lines"
