...
TMP_ROOT=$(dj_test_tmproot dj-remote-handoff)
mkdir -p "$TMP_ROOT"
...
#   rm: cannot remove '/tmp/dj-remote-handoff.XXXXXX': Directory not empty
...
trap dj_remote_handoff_teardown EXIT
printf 'fixture\n' > "$REMOTE_ROOT/AGENTS.md"
cp "$ROOT/bin/dj-remote-entrypoint.sh" "$ROOT/bin/dj-remote-job-lib.sh" \
  "$ROOT/bin/dj-remote-job-worker.sh" "$ROOT/bin/dj-remote-file.sh" \
  "$ROOT/bin/dj-backlog-receive.sh" "$ROOT/bin/dj-tasks-axi-lib.sh" \
  "$ROOT/bin/dj-wake-lib.sh" "$REMOTE_ROOT/bin/"
...
case "${DJ_FAKE_SSH_MODE:-normal}:$command_name" in
  *:dj-remote-secondmate-control.sh)
    printf '%s\n' "$command_name" >> "$DJ_FAKE_REMOTE_WAKE_LOG"
    [ "${DJ_FAKE_REMOTE_WAKE_RC:-0}" -eq 0 ] || printf 'remote receiver wake failed\n' >&2
    exit "${DJ_FAKE_REMOTE_WAKE_RC:-0}"
    ;;
  unreachable:*) exit 255 ;;
  serialize:dj-backlog-receive.sh)
    if mkdir "$DJ_FAKE_SERIALIZE_ONCE" 2>/dev/null; then
      touch "$DJ_FAKE_SERIALIZE_ENTERED"
      while [ ! -f "$DJ_FAKE_SERIALIZE_RELEASE" ]; do sleep 0.02; done
    fi
    exec "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@"
    ;;
  after-put:dj-remote-file.sh)
    "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  after-receive:dj-backlog-receive.sh)
    "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  *) exec "$DJ_FAKE_REMOTE_ENTRYPOINT" "$@" ;;
...
handoff_env() {
  ...
  DJ_FAKE_REMOTE_WAKE_LOG="$WAKE_LOG" \
  DJ_FAKE_REMOTE_WAKE_RC="${DJ_FAKE_REMOTE_WAKE_RC:-0}" \
  DJ_FAKE_SERIALIZE_ONCE="$TMP_ROOT/serialize.once" \
  DJ_FAKE_SERIALIZE_ENTERED="$TMP_ROOT/serialize.entered" \
  DJ_FAKE_SERIALIZE_RELEASE="$TMP_ROOT/serialize.release" \
...
if printf 'complete' | DJ_HOME="$REMOTE" "$REMOTE_ROOT/bin/dj-remote-file.sh" \
...
DJ_HOME="$REMOTE" "$REMOTE_ROOT/bin/dj-remote-file.sh" \
...
if DJ_HOME="$REMOTE" "$REMOTE_ROOT/bin/dj-remote-file.sh" \
...
: > "$SSH_COUNT"
set +e
DJ_FAKE_SSH_MODE=after-receive handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios ios-a ios-b \
...
out=$(handoff_env "$ROOT/bin/dj-backlog-handoff.sh" --resume-pending)
assert_contains "$out" 'received: ios moved=0 already=2' "retry did not classify already-delivered keys idempotently"
[ "$(grep -cF dj-remote-secondmate-control.sh "$WAKE_LOG")" -eq 1 ] \
...
: > "$SSH_COUNT"
set +e
DJ_FAKE_SSH_MODE=after-put handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios transfer-cut \
...
handoff_env "$ROOT/bin/dj-backlog-handoff.sh" --resume-pending >/dev/null \
...
DJ_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios serialized-a \
...
DJ_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios serialized-b \
...
handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios stale-lock-item >/dev/null \
...
set +e
DJ_FAKE_SSH_MODE=unreachable handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios pending-offline \
  > "$TMP_ROOT/offline.out" 2>&1
rc=$?
set -e
[ "$rc" -ne 0 ] || fail "offline handoff claimed success"
bootstrap_out=$(DJ_HOME="$PARENT" DJ_ROOT_OVERRIDE="$ROOT" DJ_BACKEND=tmux \
  DJ_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/dj-bootstrap.sh" 2>&1)
...
set +e
DJ_FAKE_REMOTE_WAKE_RC=1 handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios remote-wake-fail \
...
wakes_before=$(grep -cF dj-remote-secondmate-control.sh "$WAKE_LOG")
set +e
PATH="$RM_FAKEBIN:$PATH" DJ_REAL_RM="$REAL_RM" \
  DJ_FAIL_RM_PATH="$PARENT/data/handoff/ios.outbox.md" \
  handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios cleanup-retry \
...
wakes_after=$(grep -cF dj-remote-secondmate-control.sh "$WAKE_LOG")
...
handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios after-cleanup >/dev/null \
  || fail "fresh handoff did not converge an older confirmed cleanup failure"
[ "$(grep -cF dj-remote-secondmate-control.sh "$WAKE_LOG")" -eq $((wakes_after + 1)) ] \
...
DJ_HOME="$PARENT" /bin/bash -c '
  ...
  dj_lock_release "$3"
  dj_lock_release "$2"
' _ "$ROOT/bin/dj-wake-lib.sh" "$registry_lock" "$handoff_lock" \
...
handoff_env "$ROOT/bin/dj-backlog-handoff.sh" ios route-race \
...
: > "$SSH_COUNT"
fresh_out=$(DJ_HOME="$FRESH" DJ_ROOT_OVERRIDE="$ROOT" DJ_BACKEND=tmux \
  DJ_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/dj-bootstrap.sh" 2>&1)
