#!/usr/bin/env bash
# tests/dj-harness-liveness-drift-live-e2e.test.sh - opt-in drift guard proving
...
# tests/dj-tmux-agent-liveness.test.sh pins the classifier logic in CI. Run this
# guard after any harness upgrade and before trusting refreshed evidence.
set -u

if [ "${DJ_HARNESS_LIVENESS_DRIFT:-0}" != 1 ]; then
  echo "skip: set DJ_HARNESS_LIVENESS_DRIFT=1 to run the installed-harness liveness drift guard"
  exit 0
fi
...
SOCKET="dj-liveness-drift-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-liveness-drift.XXXXXX")
SESSION=drift
...
# Kimi is not required to be on PATH; mirror bin/dj-spawn.sh's own resolution
# order so this guard covers the same binary jarvis would actually launch.
resolve_harness_binary() {  # <harness>
  ...
  # verified owner dj-spawn uses, so an unrelated executable named `agent` is
  # rejected here exactly as it would be at launch.
  if [ "$harness" = cursor ]; then
    dj_cursor_resolve_binary 2>/dev/null && return 0
    return 1
  fi
  return 1
}
...
for harness in claude codex opencode pi pi-signed grok kimi cursor muse; do
  ...
  # same flag dj-spawn passes for the same reason.
  launch_args=""
