#!/usr/bin/env bash
# Regression tests for dj-guard's watcher-down banner deduplication.
#
# The first stale command in one DJ_HOME must print the full actionable watcher
# banner.
...
TMP_ROOT=$(dj_test_tmproot dj-guard-stale-banner)
...
make_guard_case() {
  ...
  dj_write_meta "$home/state/task.meta" "window=jarvis:dj-task" "kind=ship"
  printf '%s\n' "$dir"
}
...
record_live_watcher() {
  ...
  home=$(case_home "$dir")
  identity=$(DJ_STATE_OVERRIDE="$home/state" bash -c '. "$1"; dj_pid_identity "$2"' _ "$ROOT/bin/dj-wake-lib.sh" "$pid") || return 1
...
run_guard_case() {
  local dir=$1
  DJ_ROOT_OVERRIDE="$(case_root "$dir")" \
    DJ_HOME="$(case_home "$dir")" \
    DJ_GUARD_GRACE=999 \
    DJ_SUPERVISION_MODEL=persistent \
...
run_guard_case_autoarm() {
  ...
  DJ_ROOT_OVERRIDE="$(case_root "$dir")" \
    DJ_HOME="$(case_home "$dir")" \
    DJ_GUARD_GRACE=999 \
    DJ_SUPERVISION_MODEL=autoarm \
    "$ROOT/bin/dj-guard.sh" 2>&1
}

# The Pi extension model: .pi/extensions/dj-primary-pi-watch.ts tears the watcher
...
run_guard_case_extension() {
  ...
  DJ_ROOT_OVERRIDE="$(case_root "$dir")" \
    DJ_HOME="$(case_home "$dir")" \
    DJ_GUARD_GRACE=999 \
    DJ_SUPERVISION_MODEL=extension \
    "$ROOT/bin/dj-guard.sh" 2>&1
}
...
record_pi_extension_session() {
  ...
  for pair in \
    "dj-primary-pi-watch.ts:.pi-watch-extension-loaded:watch" \
    "dj-primary-turnend-guard.ts:.pi-turnend-extension-loaded:turnend"; do
    source=${pair%%:*}
    ...
    if [ "$drift" = "${pair##*:}" ]; then
      ...
      version=$(DJ_STATE_OVERRIDE="$home/state" bash -c '. "$1"; dj_pi_extension_version "$2"' \
        _ "$ROOT/bin/dj-wake-lib.sh" "$root/.pi/extensions/$source") || return 1
    fi
...
test_first_stale_call_prints_full_banner() {
  ...
  pass "dj-guard stale banner: first stale call prints the full actionable banner"
}
...
test_repeated_same_episode_prints_reminder_only() {
  ...
  pass "dj-guard stale banner: repeated same-episode calls print a concise reminder only"
}
...
test_fresh_beacon_without_live_watcher_stays_alarm() {
  ...
  pass "dj-guard stale banner: a fresh beacon without a live watcher remains unhealthy"
}
...
test_x_mode_without_live_watcher_stays_alarm() {
  ...
  pass "dj-guard stale banner: X-mode polling without a live watcher remains unhealthy"
}
...
test_healthy_recovery_rearms_next_stale_episode() {
  ...
  pass "dj-guard stale banner: healthy recovery rearms the next stale episode"
}
...
test_concurrent_same_episode_prints_one_full_banner() {
  ...
  pass "dj-guard stale banner: concurrent same-episode calls claim exactly one full banner"
}
...
test_home_isolation() {
  ...
  pass "dj-guard stale banner: deduplication is isolated per DJ_HOME"
}
...
test_queued_wake_warning_stays_independent() {
  ...
  pass "dj-guard stale banner: queued-wake warning remains independent"
}
...
test_read_only_before_writable_does_not_consume_full_banner() {
  ...
  pass "dj-guard stale banner: read-only before writable does not consume full banner"
}
...
test_read_only_during_episode_observes_without_mutating_marker() {
  ...
  pass "dj-guard stale banner: read-only during episode observes without mutating marker"
}
...
test_healthy_read_only_does_not_clear_marker() {
  ...
  pass "dj-guard stale banner: healthy read-only does not clear marker"
}
...
test_read_only_never_mutates_stale_banner_state_files() {
  ...
  pass "dj-guard stale banner: read-only never mutates stale-banner state files"
}
...
test_autoarm_fresh_beacon_without_watcher_is_healthy() {
  ...
  pass "dj-guard stale banner: auto-arm fresh beacon without a live watcher is healthy"
}
...
test_autoarm_stale_beacon_alarms_with_correct_reason() {
  ...
  pass "dj-guard stale banner: auto-arm stale beacon alarms with the true reason"
}
...
test_autoarm_stale_episode_is_stable() {
  ...
  pass "dj-guard stale banner: auto-arm stale episode stays one episode across calls"
}
...
test_persistent_no_watcher_banner_names_missing_process() {
  ...
  pass "dj-guard stale banner: persistent no-watcher banner names the true reason"
}
...
test_persistent_no_watcher_episode_survives_beacon_touch() {
  ...
  pass "dj-guard stale banner: a no-watcher episode survives a beacon mtime change"
}
...
test_extension_handoff_with_live_session_is_healthy() {
  ...
  pass "dj-guard stale banner: extension-owned hand-off with a live session is healthy"
}
...
test_extension_handoff_with_empty_lock_is_healthy() {
  ...
  pass "dj-guard stale banner: extension-owned empty lock is genuinely unheld"
}
...
test_extension_held_unhealthy_locks_stay_alarm() {
      ...
      *)
        ...
        case "$case_name" in
          dead-pid)
            ...
            ;;
          wrong-home)
            printf '%s\n' "$home/other" > "$home/state/.watch.lock/dj-home"
            ;;
          wrong-path)
            printf '%s\n' "$home/bin/not-dj-watch.sh" > "$home/state/.watch.lock/watcher-path"
            ;;
          identity-mismatch)
  ...
  pass "dj-guard stale banner: held unhealthy extension locks stay loud"
}
...
test_extension_without_ownership_evidence_stays_alarm() {
  ...
  pass "dj-guard stale banner: extension model without ownership evidence stays loud"
}
...
test_extension_ownership_needs_every_signal() {
  ...
  pass "dj-guard stale banner: every extension-ownership signal is load-bearing"
}
...
test_extension_stale_beacon_alarms_despite_live_session() {
  ...
  pid=$!
  record_pi_extension_session "$dir" "$pid" || fail "could not record the Pi extension session"
  out=$(DJ_ROOT_OVERRIDE="$(case_root "$dir")" \
    DJ_HOME="$home" \
    DJ_GUARD_GRACE=1 \
    DJ_SUPERVISION_MODEL=extension \
    "$ROOT/bin/dj-guard.sh" 2>&1)
  ...
  pass "dj-guard stale banner: extension model still alarms on a genuinely stale beacon"
}
...
test_extension_handoff_keeps_queued_wake_warning() {
  ...
  pass "dj-guard stale banner: queued-wake warning survives the extension hand-off tolerance"
}
...
test_persistent_model_ignores_pi_extension_evidence() {
  ...
  pass "dj-guard stale banner: persistent primaries ignore Pi extension evidence"
}
...
test_extension_live_watcher_is_healthy_without_ownership_evidence() {
  ...
  pass "dj-guard stale banner: extension model stays silent for a live watcher"
}

# The cases above pin the model. This one takes the end-user path instead: no
# DJ_SUPERVISION_MODEL at all, so bin/dj-harness.sh must route a Pi primary to the
# extension model on its own. Without that routing the tolerance would never reach
# a real Pi home. The foreign markers are cleared because dj-harness.sh tests them
...
test_pi_harness_routes_itself_to_the_extension_model() {
  ...
  for harness in pi pi-signed; do
    ...
    [ "$harness" = pi ] || pi_env+=(DJ_PI_HARNESS=pi-signed)
    ...
    record_pi_extension_session "$dir" "$pid" || fail "could not record the Pi extension session"
    touch "$home/state/.last-watcher-beat"
    out=$(env -u CLAUDECODE -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GROK_AGENT -u DJ_SUPERVISION_MODEL \
      "${pi_env[@]}" \
      DJ_ROOT_OVERRIDE="$(case_root "$dir")" \
      DJ_HOME="$home" \
      DJ_GUARD_GRACE=999 \
  ...
  pass "dj-guard stale banner: Pi and pi-signed primaries route themselves to the extension model"
}
