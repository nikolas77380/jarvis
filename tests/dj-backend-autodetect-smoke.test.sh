#!/usr/bin/env bash
# tests/dj-backend-autodetect-smoke.test.sh - real herdr smoke test for runtime
# backend AUTO-DETECTION (bin/dj-backend.sh's dj_backend_detect, wired into
# dj_backend_name between config/backend and the tmux default).
#
# Unlike tests/dj-backend-herdr.test.sh (fake herdr CLI) and
# tests/dj-backend-herdr-smoke.test.sh (real herdr, adapter primitives called
# directly), this suite drives the REAL bin/dj-spawn.sh and bin/dj-teardown.sh
# end to end, because auto-detection is a dj-spawn-TIME decision, not an
# adapter primitive - it has to be proven where dj_backend_name is actually
# called. The real spawn runs in a helper-provisioned, per-run named Herdr lab
# session, with a scratch DJ_HOME and scratch local-only project. Concurrent
...
# is covered as a fast, deterministic fake-tmux dj-spawn.sh test in
# tests/dj-backend.test.sh (test_spawn_autodetect_nesting_resolves_tmux_silently).
...
# by dj_backend_detect's own unit coverage plus that fake-tmux dj-spawn test.
#
# Safety (2026-07-02 incident): every test-owned Herdr operation goes through
# bin/dj-herdr-lab.sh, which appends the named session flag and verifies the
...
# The old dj-spawn bug that originally motivated this fixture shape was fixed in
# dj-spawn-symlink-guard-s8: dj-spawn.sh now normalizes PROJ_ABS and observed
# backend cwd reads before the worktree-discovery comparison.
# The dedicated regression is
# tests/dj-backend.test.sh:test_spawn_symlinked_project_prefix_avoids_false_refusal.
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-backend-autodetect-smoke.XXXXXX")
HERDR_LAB_HELPER="$ROOT/bin/dj-herdr-lab.sh"
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name dj-autodetect-smoke-concurrency-h3) || {
  rm -rf "$TMP_ROOT"
...
# --- scratch world: DJ_HOME with NO backend config, one throwaway project ---
...
git -C "$PROJ" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm initial
...
env -u TMUX -u DJ_BACKEND PATH="$PATH" HERDR_ENV=1 \
  DJ_ROOT_OVERRIDE="$ROOT" DJ_STATE_OVERRIDE="$STATE" DJ_DATA_OVERRIDE="$DATA" \
  DJ_CONFIG_OVERRIDE="$CONFIG" DJ_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" \
  DJ_SPAWN_NO_GUARD=1 \
  "$ROOT/bin/dj-spawn.sh" "$ID" "$PROJ" "sh -c 'echo autodetect-smoke-ok'" --mode no-mistakes --yolo off \
  >"$OUT_FILE" 2>"$ERR_FILE"
status=$?
[ "$status" -eq 0 ] || fail "dj-spawn.sh did not succeed auto-detecting herdr"$'\n'"--- stdout ---"$'\n'"$(cat "$OUT_FILE")"$'\n'"--- stderr ---"$'\n'"$(cat "$ERR_FILE")"

assert_contains_local "$(cat "$ERR_FILE")" "NOTICE" \
  "dj-spawn.sh did not print the auto-detect notice to stderr when selecting herdr"
assert_contains_local "$(cat "$ERR_FILE")" "EXPERIMENTAL herdr backend" \
  "dj-spawn.sh's auto-detect notice did not flag herdr as experimental"
pass "real herdr: dj-spawn.sh auto-detects herdr from HERDR_ENV=1 (no explicit config) and prints the loud notice"

META="$STATE/$ID.meta"
[ -f "$META" ] || fail "dj-spawn.sh did not write a meta file for $ID"
...
DJ_ROOT_OVERRIDE="$ROOT" DJ_STATE_OVERRIDE="$STATE" DJ_DATA_OVERRIDE="$DATA" \
  ...
  "$ROOT/bin/dj-teardown.sh" "$ID" >"$TEARDOWN_OUT" 2>&1
status=$?
[ "$status" -eq 0 ] || fail "dj-teardown.sh failed for the auto-detected herdr task"$'\n'"$(cat "$TEARDOWN_OUT")"
[ -f "$META" ] && fail "dj-teardown.sh did not remove $META"
if "$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane get "$PANE" >/dev/null 2>&1; then
  fail "dj-teardown.sh did not close the auto-detected herdr pane"
fi
WT=
