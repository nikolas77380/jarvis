...
# Bearings before Jarvis owned remediation. The shared script now stops,
...
TMP_ROOT=$(dj_test_tmproot dj-afk-return-tests)

install_runner() {  # <case-dir>
  local dir=$1
  mkdir -p "$dir/bin" "$dir/home/state" "$dir/home/data" "$dir/home/config"
  cp "$ROOT/bin/dj-afk-return.sh" "$dir/bin/"
  cp "$ROOT/bin/dj-wake-lib.sh" "$dir/bin/"
  cp "$ROOT/bin/dj-classify-lib.sh" "$dir/bin/"
  # dj-timeout-lib.sh: the shared hard bound dj-classify-lib.sh sources for the
  # wedge detector's bounded worktree write probe.
  cp "$ROOT/bin/dj-timeout-lib.sh" "$dir/bin/"
  cat > "$dir/bin/dj-afk-launch.sh" <<'SH'
#!/usr/bin/env bash
[ "${1:-}" = stop ] || exit 2
printf 'stop\n' >> "$DJ_HOME/stop.log"
rm -f "$DJ_HOME/state/.afk"
if [ -e "$DJ_HOME/state/.fail-terminal-stop-once" ]; then
  rm -f "$DJ_HOME/state/.fail-terminal-stop-once"
  exit 1
fi
rm -f "$DJ_HOME/state/.afk-daemon-terminal"
SH
  cat > "$dir/bin/dj-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
file="$DJ_HOME/state/.fake-drain"
if [ "${1:-}" = --ack-through ]; then
  [ "${3:-}" = --recovery-generation ] && [ "${4:-}" = fixture-generation ] || exit 2
  printf '%s\n' "$2" >> "$DJ_HOME/state/.fake-drain-acks"
  : > "$file"
  exit 0
fi
if [ -s "$file" ]; then
  ...
  printf 'WAKE_ACK_REQUIRED: after handling completes run bin/dj-wake-drain.sh --ack-through %s --recovery-generation fixture-generation\n' "$sequence" >&2
fi
SH
  chmod +x "$dir/bin/"*.sh
}
...
run_return() {  # <case-dir> <mode>
  local dir=$1 mode=$2
  DJ_HOME="$dir/home" DJ_STATE_OVERRIDE="$dir/home/state" "$dir/bin/dj-afk-return.sh" "$mode" 2>&1
}
...
ack_return() {  # <case-dir> <return-output>
  ...
  DJ_HOME="$dir/home" DJ_STATE_OVERRIDE="$dir/home/state" \
    "$dir/bin/dj-wake-drain.sh" --ack-through "$sequence" --recovery-generation "$generation"
}

seed_live_blocker() {  # <case-dir> <backend> <key>
  local dir=$1 backend=$2 key=$3 target
  case "$backend" in
    tmux) target='synthetic:dj-repair-task' ;;
    herdr) target='dj-lab-synthetic:w1:p2' ;;
  esac
...
kind=ship
EOF
  printf 'blocked [key=%s]: jarvis can refresh the synthetic token\n' "$key" > "$dir/home/state/repair-task.status"
}
...
test_return_gate_orders_catchup_before_bearings() {
  ...
  assert_contains "$out" 'jarvis-actionable blocker: repair-task [key=synthetic-dependency]' "return output did not assign blocker remediation to Jarvis"
  ...
  set +e
  out=$(DJ_HOME="$dir/home" DJ_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/dj-bearings-snapshot.sh" --json 2>&1)
  rc=$?
  set -e
...
test_captain_decision_does_not_masquerade_as_jarvis_blocker() {
  local dir out
  dir="$TMP_ROOT/captain-decision"
  install_runner "$dir"
  cat > "$dir/home/state/decision-task.meta" <<'EOF'
window=synthetic:dj-decision-task
backend=tmux
kind=ship
EOF
  ...
  out=$(run_return "$dir" begin) || fail "approval decision should not be treated as a jarvis blocker: $out"
  assert_contains "$out" 'catch-up wake:' "approval decision notification was not surfaced in catch-up"
  [ ! -e "$dir/home/state/.afk-return-catchup" ] || fail "approval decision incorrectly opened a jarvis blocker gate"
  pass "needs-decision remains reportable without masquerading as a jarvis-actionable blocker"
}
...
test_evidence_publication_failure_preserves_wake_for_redrain() {
  ...
  set +e
  DJ_HOME="$dir/home" DJ_STATE_OVERRIDE="$dir/home/state" \
    "$dir/bin/dj-afk-return.sh" begin 3< "$dir/read-only-output" >&3 2> "$dir/failed.err"
  rc=$?
  set -e
...
test_away_reentry_refuses_pending_return_gate() {
  ...
  set +e
  out=$(DJ_HOME="$dir/home" DJ_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/dj-afk-launch.sh" start-native 2>&1)
  rc=$?
  set -e
