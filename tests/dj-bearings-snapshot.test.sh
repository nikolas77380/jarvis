#!/usr/bin/env bash
# Behavior tests for the bearings projection wrapper over dj-fleet-snapshot.sh.
...
BEARINGS="$ROOT/bin/dj-bearings-snapshot.sh"
TMP_ROOT=$(dj_test_tmproot dj-bearings)
# Keep disposable homes outside the snapshot's fixture repo boundary even when
# TMPDIR is inside an isolated source worktree.
DJ_ROOT_OVERRIDE="$TMP_ROOT/fixture-root"
mkdir -p "$DJ_ROOT_OVERRIDE"
export DJ_ROOT_OVERRIDE
...
case "${1:-}" in
  ...
  capture-pane)
    case "$*" in
      *dj-domain-alpha*) printf 'stale terminal summary: Phase 7 started\n> \n' ;;
      *) printf 'all quiet\n> \n' ;;
    esac
    ;;
esac
...
if [ "${FAKE_GH_MANY:-0}" = 1 ]; then
  cat <<'JSON'
[{"number":1,"title":"One","url":"https://github.com/acme/repo/pull/1","headRefName":"dj/one","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":2,"title":"Two","url":"https://github.com/acme/repo/pull/2","headRefName":"dj/two","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":3,"title":"Three","url":"https://github.com/acme/repo/pull/3","headRefName":"dj/three","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]}]
JSON
  exit 0
fi
cat <<'JSON'
[{"number":9,"title":"Ship the thing","url":"https://github.com/kunchenguid/firstmate/pull/9","headRefName":"dj/ship-task","reviewDecision":"APPROVED","mergeable":"MERGEABLE","statusCheckRollup":[{"conclusion":"SUCCESS","status":"COMPLETED"}]}]
JSON
SH
  cat > "$fb/gh-axi" <<'SH'
#!/usr/bin/env bash
...
record_claude_state() {  # <state-dir> <id> <busy|idle>
  ...
  "$ROOT/bin/dj-busy-event.sh" apply "$state" "$id" "$semantic_state" --gen "$gen" \
...
write_fixture() {  # <home>
  ...
  printf '# Jarvis fixture\n' > "$mate/AGENTS.md"
  printf 'mate\n' > "$mate/.dj-secondmate-home"
  printf -- '- mate - fixture domain (home: %s; scope: fixture work; projects: jarvis; added 2026-07-11)\n' \
    "$mate" > "$home/data/secondmates.md"
  cat > "$home/data/backlog.md" <<EOF
## In flight
- [ ] ship-task - Ship the thing (repo: jarvis) (kind: ship) (since 2026-07-11)
- [ ] scout-x - Investigate the thing data/scout-x/report.md (repo: jarvis) (kind: scout) (since 2026-07-11)

## Queued
- [ ] live-gate - Real queued work blocked-by: ship-task (repo: jarvis) (kind: ship)
- [ ] dead-gate - Old conditional work (repo: jarvis) (kind: scout)
  NOT REQUIRED - superseded 2026-07-11; kept as reference only.

## Done
- [x] done-a - Landed thing https://github.com/kunchenguid/firstmate/pull/7 (repo: jarvis) (kind: ship) (merged 2026-07-10)
EOF
  printf '# Scout X\n' > "$home/data/scout-x/report.md"
  dj_write_meta "$home/state/ship-task.meta" \
    "window=jarvis:dj-ship-task" \
  ...
  dj_write_meta "$home/state/scout-x.meta" \
    "window=jarvis:dj-scout-x" \
    "worktree=$home/projects/ship-wt" \
    "project=jarvis" \
    "harness=claude" \
    "kind=scout" \
  ...
  dj_write_meta "$home/state/mate.meta" \
    "window=jarvis:dj-mate" \
    "worktree=$mate" \
    "project=$mate" \
    "harness=codex" \
    "kind=secondmate" \
  ...
  dj_write_meta "$home/state/external-wait.meta" \
    "window=jarvis:dj-external-wait" \
    "worktree=$home/projects/ship-wt" \
    "project=jarvis" \
    "harness=claude" \
    "kind=ship" \
...
## In flight
- [ ] mate - Decide subscription order (repo: jarvis) (kind: ship) (since 2026-07-11)

## Queued
- [ ] mate-decision-race - Choose subscription order (repo: jarvis) (kind: captain) (hold: captain choice pending) (hold-kind: captain)

## Done
- [x] mate-landed - Secondmate-managed fix https://github.com/kunchenguid/firstmate/pull/50 (repo: jarvis) (kind: ship) (merged 2026-07-11)
EOF
  mkdir -p "$mate/projects/mate"
  dj_write_meta "$mate/state/mate.meta" \
    "window=jarvis:dj-mate" "worktree=$mate/projects/mate" "project=jarvis" \
...
run() {  # <home> <fakebin> <args...>
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_BEARINGS_NOW=2026-07-11T18:00:00Z NET_LOG="$home/net.log" "$BEARINGS" "$@"
}
...
write_domain_alpha_fixture() {  # <parent-home> <secondmate-home>
  ...
  printf -- '- domain-alpha - sample rollout (home: %s; scope: sample rollout and legal release; projects: sample; added 2026-07-13)\n' \
    "$mate" > "$home/data/secondmates.md"
  dj_write_secondmate_meta "$home/state/domain-alpha.meta" "$mate" "jarvis:dj-domain-alpha" sample
...
test_domain_alpha_stale_parent_event_does_not_become_current_work() {
  ...
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_NOW_EPOCH=1783792800 DJ_SNAPSHOT_TERMINAL_LINES=2 DJ_SNAPSHOT_TERMINAL_BYTES=64 \
    NET_LOG="$home/net.log" FAKE_GH_FAIL=1 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
...
SH
  ...
  canonical=$(PATH="$fakebin:$PATH" STAT_LOG="$stat_log" DJ_HOME="$home" \
    DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z DJ_SNAPSHOT_NOW_EPOCH=1783792800 \
    "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.records[] | select(.id == "domain-alpha")
    | .provenance.selected == "structured-home"
...
test_parent_activity_evidence_is_bounded_and_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_PARENT_ACTIVITY_LINES=4 DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    DJ_SNAPSHOT_PARENT_ACTIVITIES=2 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.records[] | select(.id == "domain-alpha")
  ...
  json=$(DJ_SNAPSHOT_PARENT_ACTIVITY_LINES=4 DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    DJ_SNAPSHOT_PARENT_ACTIVITIES=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
EOF
  dj_write_meta "$mate/state/phase8.meta" \
    "window=jarvis:dj-phase8" "worktree=$mate/projects/phase8" "project=sample" \
...
append_landed_row() {  # <secondmate-home> <id> <title> <date>
  printf -- '- [x] %s - %s (repo: jarvis) (kind: ship) (merged %s)\n' \
...
write_parent_secondmate_event() {  # <parent> <id> <home> <note>
  dj_write_secondmate_meta "$1/state/$2.meta" "$3" "jarvis:dj-$2" sample
...
test_oversized_secondmate_summary_stays_strict_unknown() {
  ...
  dj_write_secondmate_meta "$home/state/oversized.meta" "$mate" "jarvis:dj-oversized" sample
...
EOF
  ...
  fakebin=$(make_fakebin "$home")
  json=$(DJ_SNAPSHOT_SECONDMATE_MAX_BYTES=512 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_secondmate_and_child_bounds_are_disclosed() {
  ...
  while [ "$i" -le 3 ]; do
    ...
    dj_write_meta "$mate/state/$child.meta" \
      "window=jarvis:dj-$child" "worktree=$mate/projects/$child" "project=sample" \
      "harness=claude" "kind=ship" "mode=no-mistakes"
  ...
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_SECONDMATES=2 DJ_SNAPSHOT_SECONDMATE_CHILDREN=2 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
  ...
  json=$(DJ_SNAPSHOT_SECONDMATES=2 DJ_SNAPSHOT_SECONDMATE_CHILDREN=2 DJ_BEARINGS_SECONDMATES=1 \
  ...
  expanded=$(DJ_SNAPSHOT_SECONDMATE_CHILDREN=2 DJ_BEARINGS_SECONDMATES=1 \
...
test_parent_decision_is_untrusted_contradiction_only() {
  ...
  dj_write_secondmate_meta "$home/state/authority.meta" "$mate" "jarvis:dj-authority" sample
...
test_parent_evidence_reconciles_by_verb_and_key() {
  ...
  dj_write_secondmate_meta "$home/state/hold.meta" "$hold" "jarvis:dj-hold" sample
  dj_write_secondmate_meta "$home/state/blocked.meta" "$blocked" "jarvis:dj-blocked" sample
  dj_write_secondmate_meta "$home/state/decision.meta" "$decision" "jarvis:dj-decision" sample
...
## Done
EOF
  dj_write_meta "$decision/state/$child.meta" \
    "window=jarvis:dj-$child" "worktree=$decision/projects/$child" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  dj_write_meta "$mate/state/parked.meta" \
    "window=jarvis:dj-parked" "worktree=$mate/projects/parked" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  dj_write_meta "$mate/state/done.meta" \
    "window=jarvis:dj-done" "worktree=$mate/projects/done" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
  dj_write_meta "$mate/state/failed.meta" \
    "window=jarvis:dj-failed" "worktree=$mate/projects/failed" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
test_registry_unavailability_and_bounds_are_explicit() {
  ...
  dj_write_secondmate_meta "$home/state/hidden.meta" "$mate" "jarvis:dj-hidden" sample
  ...
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_REGISTRY_RECORDS=2 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
  ...
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_REGISTRY_LINES=2 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_REGISTRY_BYTES=100 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_REGISTRY_BYTES="$((boundary - 1))" "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  json=$(DJ_SNAPSHOT_REGISTRY_RECORDS=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  dj_write_secondmate_meta "$home/state/z-hidden.meta" "$mate" "jarvis:dj-z-hidden" sample
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    DJ_SNAPSHOT_REGISTRY_RECORDS=3 "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry.complete == false
...
test_default_is_bounded_and_local_only() {
  ...
  local canon; canon=$(PATH="$fakebin:$PATH" DJ_HOME="$home" "$ROOT/bin/dj-fleet-snapshot.sh" --json)
  ...
  printf '%s' "$json" | jq -e '.schema == "dj-bearings.v1"' >/dev/null || fail "json schema wrong"
...
## Queued
- [ ] work-gate - Captain-gated ship work (repo: jarvis) (kind: ship) (hold: captain go needed) (hold-kind: captain)
- [ ] later-call - Deferred captain call (repo: jarvis) (kind: captain) (hold: revisit with the captain) (hold-kind: captain) (hold-until: 2026-08-01)
- [ ] due-call - Due captain call (repo: jarvis) (kind: captain) (hold: overdue captain choice) (hold-kind: captain) (hold-until: 2026-07-11)
- [ ] parked-call - Prose-parked captain call (repo: jarvis) (kind: ship) (hold: DEFERRED by captain revisit later) (hold-kind: captain)
- [ ] external-gate - Externally held work (repo: jarvis) (kind: ship) (hold: upstream release pending) (hold-kind: external)

## Done
- [x] answered-call - Answered captain question (repo: jarvis) (kind: captain) (done 2026-07-10) (hold: captain choice pending) (hold-kind: captain)
- [x] shipped-work - Ordinary landed work (repo: jarvis) (kind: ship) (merged 2026-07-10)
EOF
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_perl_fallback_bounds_github_call() {
  ...
  started=$(date +%s)
  json=$(PATH="$fakebin:$toolbin" DJ_HOME="$home" DJ_BEARINGS_NOW=2026-07-11T18:00:00Z \
    DJ_BEARINGS_PR_TIMEOUT=1 NET_LOG="$home/net.log" FAKE_GH_SLEEP=1 "$BEARINGS" --include-prs --json)
...
write_large_fixture() {  # <home> <count>
  ...
  while [ "$i" -le "$count" ]; do
    ...
    dj_write_meta "$home/state/$id.meta" \
      "window=jarvis:dj-$id" \
...
test_section_caps_and_expansion_flags() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(DJ_BEARINGS_IN_FLIGHT=2 DJ_BEARINGS_DECISIONS=2 DJ_BEARINGS_GATES=2 \
    DJ_BEARINGS_REPORTS=2 DJ_BEARINGS_RECORDED_PRS=2 DJ_BEARINGS_UNHEALTHY=2 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  expanded=$(DJ_BEARINGS_IN_FLIGHT=2 DJ_BEARINGS_DECISIONS=2 DJ_BEARINGS_GATES=2 \
    DJ_BEARINGS_REPORTS=2 DJ_BEARINGS_RECORDED_PRS=2 DJ_BEARINGS_UNHEALTHY=2 \
...
test_pr_repository_cap_and_expansion() {
  ...
  fakebin=$(make_fakebin "$home"); : > "$home/net.log"
  json=$(DJ_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --json)
  ...
  : > "$home/net.log"
  expanded=$(DJ_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --all-pr-repos --json)
...
test_per_repository_pr_cap_is_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(DJ_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs --json)
  toon=$(DJ_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs)
  printf '%s' "$json" | jq -e '
    (.candidate_prs | length) == 2
    and (.prs | test("2 shown, at least 3 open; capped in 1 repo"))
    and ([.omitted[] | select(.surface == "candidate_prs showing 2 of at least 3; capped in 1 repo(s)" and .reveal == "raise DJ_BEARINGS_PR_LIMIT")] | length) == 1
...
# The E.D.I.T.H.-103 defect, end to end: a COMPLETED scout that raised a decision and
...
test_completed_scout_report_not_pending() {
  ...
  fakebin=$(make_fakebin "$home")
  mkdir -p "$home/projects/lav-wt" "$home/data/edith-103"
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.[]; .id == "edith-103") | not)
      and (.reports | any(.[]; .id == "edith-103"))
...
test_all_landed_keeps_complete_global_order() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(DJ_BEARINGS_LANDED=1 run "$home" "$fakebin" --json --all-landed)
  actual=$(printf '%s' "$json" | jq -r '.landed[] | "\(.owner)/\(.id)"')
...
test_landed_bounded_and_disclosed() {
  ...
  i=1
  while [ "$i" -le 12 ]; do
    printf -- '- [x] mate-landed-%02d - Secondmate fix %02d (repo: jarvis) (kind: ship) (merged 2026-06-%02d)\n' \
      "$i" "$i" "$((13 - i))" >> "$mate/data/backlog.md"
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(DJ_BEARINGS_LANDED=20 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_live_blocker_is_not_charted_queue_work() {
  ...
  printf 'blocked [key=synthetic-dependency]: jarvis can refresh the synthetic token\n' > "$home/state/ship-task.status"
...
test_captains_call_anti_leak() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  canonical=$(PATH="$fakebin:$PATH" DJ_HOME="$home" "$ROOT/bin/dj-fleet-snapshot.sh" --json)
...
test_main_orphan_in_flight_is_disclosed_not_invented() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] only-orphan - Structured in flight without meta (repo: jarvis) (kind: ship) (since 2026-07-11)
...
## In flight
this current row is not structured
- [ ] structured-ship - Visible structured sibling (repo: jarvis) (kind: ship) (since 2026-07-11)

## Queued
another free-form note without checkbox
- [ ] structured-queued - Structured queued (repo: jarvis) (kind: ship)

## Done
EOF
  dj_write_meta "$home/state/structured-ship.meta" \
    "window=jarvis:dj-structured-ship" \
...
test_main_orphan_counterfactual_meta_clears_inventory_warning() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] orphan-ship - Gains meta in counterfactual (repo: jarvis) (kind: ship) (since 2026-07-11)
- [ ] visible-ship - Already live (repo: jarvis) (kind: ship) (since 2026-07-11)

## Queued
- [ ] queued-ship - Ordinary queue (repo: jarvis) (kind: ship)

## Done
EOF
  dj_write_meta "$home/state/visible-ship.meta" \
    "window=jarvis:dj-visible-ship" \
  ...
  dj_write_meta "$home/state/orphan-ship.meta" \
    "window=jarvis:dj-orphan-ship" \
    "worktree=$home/projects/orphan-ship" \
    "project=jarvis" \
    "harness=codex" \
    "kind=ship" \
...
## Done
EOF
  dj_write_meta "$hibit/state/hibit-worker.meta" \
    "window=jarvis:dj-hibit-worker" "worktree=$hibit/projects/worker" "project=hibit" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  dj_write_meta "$wheel/state/wheel-worker.meta" \
    "window=jarvis:dj-wheel-worker" "worktree=$wheel/projects/worker" "project=wheelhouse" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
EOF
  dj_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=jarvis:dead-sshhip-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=codex" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  dj_write_meta "$ha/state/prep.meta" \
    "window=jarvis:dj-prep" "worktree=$ha/projects/prep" "project=home-assistant" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: ship)' \
  ...
  dj_write_meta "$wheel/state/production-observation.meta" \
    "window=jarvis:dj-production-observation" "worktree=$wheel/projects/worker" "project=wheelhouse" \
  ...
  dj_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=jarvis:dj-unreadable-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
test_main_captain_readiness_matches_secondmate_projection() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] observation - Held observation (repo: jarvis) (kind: scout) (hold: watch production) (hold-kind: external)
- [ ] prep - Prepare canary (repo: jarvis) (kind: ship)

## Queued
- [ ] review - Security review (repo: jarvis) (kind: ship)
- [ ] captain-run - Run captain canary blocked-by: prep blocked-by: review (repo: jarvis) (kind: captain) (hold: captain runs canary) (hold-kind: captain)

## Done
EOF
  dj_write_meta "$home/state/prep.meta" \
    "window=jarvis:dj-prep" "worktree=$home/projects/prep" "project=jarvis" \
    "harness=codex" "kind=ship" "mode=no-mistakes"
  printf 'working: preparing main canary\n' > "$home/state/prep.status"
  dj_write_meta "$home/state/observation.meta" \
    "window=jarvis:dj-observation" "worktree=$home/projects/observation" "project=jarvis" \
...
## Done
- [x] prep - Prepare canary (repo: jarvis) (kind: ship) (done 2026-07-22)
EOF
...
## Done
- [x] prep - Prepare canary (repo: jarvis) (kind: ship) (done 2026-07-22)
- [x] review - Security review (repo: jarvis) (kind: ship) (done 2026-07-22)
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
