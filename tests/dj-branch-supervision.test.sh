...
# (bin/dj-branch-prompt.sh), the append-only outcome store
# (bin/dj-branch-outcome.sh), the per-task lease contract (bin/dj-lease.sh,
# bin/dj-lease-lib.sh), the lease and role-partition guards wired into the
...
TMP_ROOT=$(dj_test_tmproot dj-branch-supervision)
...
test_branch_prompt_is_byte_stable_and_above_cache_floor() {
  ...
  out_a=$(cd "$TMP_ROOT" && DJ_HOME="$home_a" TZ=UTC "$ROOT/bin/dj-branch-prompt.sh") \
    || fail "branch prompt generator failed for home A"
  out_b=$(cd / && DJ_HOME="$home_b" TZ=Australia/Eucla "$ROOT/bin/dj-branch-prompt.sh") \
    || fail "branch prompt generator failed for home B"
  sleep 1
  out_c=$("$ROOT/bin/dj-branch-prompt.sh") || fail "branch prompt generator failed on re-run"
...
test_outcome_store_is_append_only_with_cursor_reads() {
  ...
  seq1=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" append \
  ...
  seq2=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" append \
...
PY
  ...
  snapshot=$(cat "$store")
  DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" mark-read --through 1 || fail "mark-read failed"
  unread=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" unread) || fail "unread failed"
  case "$unread" in
    '{"seq":2,'*) ;;
  ...
  replay=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" startup-replay) || fail "startup-replay failed"
  ...
  [ -z "$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" startup-replay)" ] \
  ...
  snapshot=$(cat "$store")
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" append \
...
test_outcome_startup_replay_preserves_silence() {
  ...
  replay=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" startup-replay) || fail "mixed startup replay failed"
  ...
  [ -z "$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" unread)" ] \
  ...
  replay=$(DJ_HOME="$home" "$ROOT/bin/dj-branch-outcome.sh" startup-replay) || fail "legacy startup replay failed"
...
test_lease_exclusivity_release_stale_and_sweep() {
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-1 --actor branch \
    || fail "branch claim failed"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-1) || fail "check missed a held lease"
  case "$out" in
    "branch $$ "*" live") ;;
    *) fail "check misreported the lease: $out" ;;
  esac
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=main DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-1 2>&1)
  status=$?
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-1 \
    || fail "same-actor refresh was refused"

  # Release by the calling holder; release of an unheld lease stays a silent no-op.
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release task-1 --actor branch || fail "release failed"
  DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-1 >/dev/null && fail "released lease still reported"
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release task-1 --actor branch || fail "idempotent release failed"
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=main DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-dead \
    || fail "stale lease blocked a live claim"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-dead)
  case "$out" in "main $$ "*) ;; *) fail "stale lease was not taken over: $out" ;; esac
  printf 'branch\t999999\t123\n' > "$home/state/.lease-task-dead2"
  DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" sweep || fail "sweep failed"
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim backlog --actor branch \
    || fail "backlog lease claim failed"
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=main DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim backlog 2>&1)
...
test_mutating_scripts_refuse_the_other_actors_lease() {
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-held --actor branch \
    || fail "fixture lease claim failed"

  # dj-control: refused while the branch holds the lease; the ordinary no-task
  # error (a different failure) proves pass-through once the lease is gone.
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-control.sh" task-held interrupt 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "leased dj-control exited $status, not 6: $out"
  assert_contains "$out" "leased to the branch supervision actor" "dj-control refusal lost the holder"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-control.sh" task-unheld interrupt 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "unleased dj-control still hit the lease refusal"
  assert_contains "$out" "no task 'task-unheld'" "unleased dj-control lost its ordinary error"

  # dj-teardown: same refusal shape before any teardown work.
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-teardown.sh" task-held 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "leased dj-teardown exited $status, not 6: $out"
  assert_contains "$out" "teardown (dj-teardown) refused" "dj-teardown refusal lost its action label"

  # The same lease refuses the BRANCH actor when MAIN holds it - the guard is
  # symmetric, not a branch-only fence.
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release task-held --actor branch
  DJ_HOME="$home" DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-held --actor main \
    || fail "main fixture claim failed"
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-control.sh" task-held interrupt 2>&1)
  status=$?
  ...
  pass "dj-control and dj-teardown refuse the other actor's live lease and pass through otherwise"
}
...
test_main_owned_actions_refuse_the_branch_actor() {
  ...
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-pr-merge.sh" task-x https://github.com/o/r/pull/1 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch dj-pr-merge exited $status, not 6: $out"
  assert_contains "$out" "the supervision branch never performs this action" "pr-merge refusal lost the partition wording"

  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-merge-local.sh" task-x 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch dj-merge-local exited $status, not 6: $out"

  out=$(DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" DJ_SUPERVISION_ACTOR=branch \
    "$ROOT/bin/dj-spawn.sh" task-new --mode no-mistakes --yolo off 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch dj-spawn exited $status, not 6: $out"
  assert_contains "$out" "new-task spawn (dj-spawn) refused" "spawn refusal lost its action label"

  # The same calls as MAIN fail on their ORDINARY validation instead - the
  # partition guard never fires for the main actor.
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-merge-local.sh" task-x 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "main dj-merge-local hit the partition refusal"
  assert_contains "$out" "no meta for task task-x" "main dj-merge-local lost its ordinary error"
...
test_home_without_branch_is_untouched() {
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-control.sh" task-any interrupt 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "no-branch home hit a lease refusal in dj-control"
  assert_contains "$out" "no task 'task-any'" "no-branch dj-control lost its ordinary error"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-pr-merge.sh" 2>&1)
  status=$?
  [ "$status" -eq 2 ] || fail "no-branch dj-pr-merge usage error changed: $status: $out"
  ...
  out=$(STATE="$home/state" bash -c '. "$1"; dj_lease_guard task-reused "probe"; dj_lease_forbid_branch "probe"; echo silent-pass' _ "$ROOT/bin/dj-lease-lib.sh" 2>&1)
  ...
  out=$(env -u PI_CODING_AGENT -u DJ_SUPERVISION_ACTOR CLAUDECODE=1 STATE="$home/state" bash -c '. "$1"; dj_lease_guard task-reused "probe"; echo silent-pass' _ "$ROOT/bin/dj-lease-lib.sh" 2>&1)
...
test_lease_liveness_binds_to_the_session_lock() {
  ...
  printf 'branch\t%s\t123\n' "$PPID" > "$home/state/.lease-task-reused"
  printf '%s\n' "$$" > "$home/state/.lock"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-reused) || fail "check missed the leftover lease"
  case "$out" in
    *" stale") ;;
  ...
  DJ_HOME="$home" DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-current --actor main \
    || fail "claim under the current lock holder failed"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-current)
  case "$out" in
    "main $$ "*" live") ;;
  ...
  printf '%sjunk\n' "$$" > "$home/state/.lock"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-current) || fail "check missed the lease under a malformed lock"
  case "$out" in
    *" stale") ;;
    *) fail "a malformed lock proved lease liveness: $out" ;;
  esac
  DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" sweep || fail "sweep under malformed lock failed"
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "$DJ_TEST_LEASE_PATH" ] && mkdir "$DJ_TEST_GATE.once" 2>/dev/null; then
  : > "$DJ_TEST_GATE.ready"
  while [ ! -e "$DJ_TEST_GATE.release" ]; do sleep 0.01; done
fi
exec "$DJ_TEST_REAL_MV" "$@"
SH
  chmod +x "$fakebin/mv"

  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ \
    DJ_TEST_REAL_MV="$real_mv" DJ_TEST_LEASE_PATH="$home/state/.lease-task-race" DJ_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/dj-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  branch_pid=$!
  while [ ! -e "$home/state/gate.ready" ]; do sleep 0.01; done
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SUPERVISION_ACTOR=main DJ_LEASE_HOLDER_PID=$$ \
    DJ_TEST_REAL_MV="$real_mv" DJ_TEST_LEASE_PATH="$home/state/.lease-task-race" DJ_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/dj-lease.sh" claim task-race --actor main >/dev/null 2>&1 &
  main_pid=$!
  sleep 0.1
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "$DJ_TEST_STALE_PATH" ] && mkdir "$DJ_TEST_GATE.once" 2>/dev/null; then
  : > "$DJ_TEST_GATE.ready"
  while [ ! -e "$DJ_TEST_GATE.release" ]; do sleep 0.01; done
fi
exec "$DJ_TEST_REAL_RM" "$@"
SH
  chmod +x "$fakebin/rm"

  PATH="$fakebin:$PATH" STATE="$home/state" DJ_TEST_REAL_RM="$real_rm" \
    DJ_TEST_STALE_PATH="$home/state/.lease-task-race" DJ_TEST_GATE="$home/state/gate" \
    bash -c '. "$1"; dj_lease_guard task-race "probe"' _ "$ROOT/bin/dj-lease-lib.sh" &
  guard_pid=$!
  while [ ! -e "$home/state/gate.ready" ]; do sleep 0.01; done
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ \
    DJ_TEST_REAL_RM="$real_rm" DJ_TEST_STALE_PATH="$home/state/.lease-task-race" DJ_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/dj-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  claim_pid=$!
  sleep 0.1
  : > "$home/state/gate.release"
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-lease.sh" check task-race) || fail "guard deleted the newer lease claim"
  case "$out" in
    "branch $$ "*" live") ;;
...
test_guard_holds_exclusivity_through_mutation() {
  ...
  PI_CODING_AGENT=true STATE="$home/state" DJ_TEST_READY="$home/operation-ready" \
    DJ_TEST_RELEASE="$home/operation-release" bash -c '
      . "$1"
      dj_lease_guard task-race "probe"
      trap "dj_lease_guard_release" EXIT
      : > "$DJ_TEST_READY"
      while [ ! -e "$DJ_TEST_RELEASE" ]; do sleep 0.01; done
    ' _ "$ROOT/bin/dj-lease-lib.sh" &
  operation_pid=$!
  while [ ! -e "$home/operation-ready" ]; do sleep 0.01; done

  PI_CODING_AGENT=true DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ \
    "$ROOT/bin/dj-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  claim_pid=$!
  sleep 0.2
...
test_claim_refuses_the_other_actors_name_loudly() {
  ...
  mkdir -p "$home/state"
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ \
    "$ROOT/bin/dj-lease.sh" claim task-z --actor main 2>&1)
  status=$?
...
test_release_actor_drops_only_that_actors_leases() {
  ...
  mkdir -p "$home/state"
  printf '%s\n' "$$" > "$home/state/.lock"
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-a --actor branch \
    || fail "branch claim failed"
  DJ_HOME="$home" DJ_LEASE_HOLDER_PID=$$ "$ROOT/bin/dj-lease.sh" claim task-b --actor main \
    || fail "main claim failed"
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release task-b --actor main 2>&1)
  status=$?
  ...
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release-actor --actor main 2>&1)
  status=$?
  ...
  DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-lease.sh" release-actor --actor branch || fail "release-actor failed"
...
test_branch_cannot_force_teardown_or_directly_relaunch() {
  ...
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-teardown.sh" task-x --force 2>&1)
  status=$?
  ...
  out=$(DJ_HOME="$home" DJ_SUPERVISION_ACTOR=branch "$ROOT/bin/dj-teardown.sh" task-x 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "ordinary branch teardown hit the forced-discard refusal: $out"

  # A branch relaunch is legal only through dj-control's owned transaction,
  # never by direct dj-spawn invocation.
  out=$(DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" DJ_SUPERVISION_ACTOR=branch \
    "$ROOT/bin/dj-spawn.sh" task-x --relaunch 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "direct branch relaunch exited $status, not 6: $out"
  assert_contains "$out" "must relaunch through dj-control" "relaunch refusal lost its wording"
  pass "the branch cannot force a teardown or bypass dj-control for a relaunch"
}
