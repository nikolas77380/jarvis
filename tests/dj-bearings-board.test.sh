#!/usr/bin/env bash
# Behavior tests for bin/dj-bearings-board.sh: fail-closed payload validation,
...
BOARD="$ROOT/bin/dj-bearings-board.sh"
TMP_ROOT=$(dj_test_tmproot dj-bearings-board)
...
test_build_refuses_malformed_payloads_before_touching_the_board() {
  ...
  printf '{"schema":"dj-bearings-board.v2"}\n' > "$data"
  set +e; out=$(run_board "$home" build "$data" 2>&1); rc=$?; set -e
  [ "$rc" -ne 0 ] || fail "a wrong-schema payload was accepted"
  assert_contains "$out" "dj-bearings-board.v1" "the schema refusal did not name the contract: $out"
...
## Done
EOF
  dj_write_meta "$home/state/$origin.meta" "project=$home/projects/sample" "kind=scout"
...
test_build_refuses_a_template_without_exactly_one_slot() {
  ...
  set +e
  out=$(DJ_BEARINGS_BOARD_TEMPLATE="$home/broken-template.html" run_board "$home" build "$data" 2>&1)
  rc=$?
  set -e
