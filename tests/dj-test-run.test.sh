#!/usr/bin/env bash
# Contract tests for bin/dj-test-run.sh - the single owner of behavior suite
...
# focused scheduler checks, not the complete Jarvis suite.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

RUNNER="$ROOT/bin/dj-test-run.sh"

assert_present "$RUNNER" "bin/dj-test-run.sh is missing"
[ -x "$RUNNER" ] || fail "bin/dj-test-run.sh must be executable"
...
test_family_selection() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/dj-test-run.test.sh' \
    || fail "pure-contract-unit must include dj-test-run.test.sh"
...
test_single_script_selection() {
  local listed
  listed=$("$RUNNER" --list tests/dj-lint.test.sh)
  [ "$listed" = "tests/dj-lint.test.sh" ] \
    || fail "single-script list expected tests/dj-lint.test.sh, got: $listed"
...
test_changed_file_selection_is_conservative() {
  ...
  listed=$("$RUNNER" --list tests/dj-brief.test.sh)
  [ "$listed" = "tests/dj-brief.test.sh" ] \
...
init_changed_fixture_repo() {
  ...
  cp "$RUNNER" "$repo/bin/dj-test-run.sh"
  chmod +x "$repo/bin/dj-test-run.sh"
  for script in \
    dj-brief.test.sh \
    dj-ask-user-authority.test.sh \
    dj-cd-pretool-check.test.sh \
    dj-daemon.test.sh \
    dj-backend-herdr-smoke.test.sh \
    dj-secondmate-safety.test.sh \
    dj-session-start.test.sh \
    dj-afk-pi-herdr-return-e2e.test.sh \
    dj-backend.test.sh \
    dj-pr-merge.test.sh \
    dj-pi-watch-extension.test.sh \
    dj-afk-return.test.sh \
    dj-bearings-snapshot.test.sh \
    dj-backend-cmux.test.sh \
    dj-backend-zellij.test.sh \
    dj-backend-orca.test.sh; do
    printf '#!/usr/bin/env bash\n# tests/lib.sh\n' >"$repo/tests/$script"
    chmod +x "$repo/tests/$script"
  done
  : >"$repo/tests/lib.sh"
  : >"$repo/tests/dj-backend-herdr-eventwait.test.py"
  : >"$repo/bin/dj-supervisor-target-lib.sh"
  : >"$repo/bin/unmapped-source.sh"
  printf '# .claude/settings.json\n# .pi/extensions/dj-primary-turnend-guard.ts\n' \
    >>"$repo/tests/dj-cd-pretool-check.test.sh"
  printf '# .pi/extensions/dj-primary-pi-watch.ts\n' >>"$repo/tests/dj-pi-watch-extension.test.sh"
  mkdir -p "$repo/.agents/skills/example" "$repo/.claude" "$repo/.pi/extensions" "$repo/src"
  : >"$repo/.agents/skills/example/SKILL.md"
  : >"$repo/.claude/settings.json"
  : >"$repo/.pi/extensions/dj-primary-pi-watch.ts"
  : >"$repo/.pi/extensions/dj-primary-turnend-guard.ts"
...
test_changed_dependency_selection_and_unmapped_failure() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-changed.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"

  printf '\n' >>"$repo/tests/lib.sh"
  listed=$(cd "$repo" && bin/dj-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/dj-pr-merge.test.sh" "shared helper selects pr-forge dependents"
  assert_contains "$listed" "tests/dj-secondmate-safety.test.sh" "shared helper selects secondmate dependents"
  assert_contains "$listed" "tests/dj-bearings-snapshot.test.sh" "shared helper selects snapshot dependents"
  git -C "$repo" add tests/lib.sh
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm helper-change

  printf '\n' >>"$repo/tests/dj-backend-herdr-eventwait.test.py"
  listed=$(cd "$repo" && bin/dj-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/dj-backend-herdr-smoke.test.sh" "eventwait test selects Herdr coverage"
  assert_contains "$listed" "tests/dj-backend.test.sh" "eventwait test selects backend coverage"
  git -C "$repo" add tests/dj-backend-herdr-eventwait.test.py
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm eventwait-change

  printf '\n' >>"$repo/bin/dj-supervisor-target-lib.sh"
  listed=$(cd "$repo" && bin/dj-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/dj-daemon.test.sh" "supervisor target selects daemon coverage"
  assert_contains "$listed" "tests/dj-afk-return.test.sh" "supervisor target selects afk coverage"
  git -C "$repo" add bin/dj-supervisor-target-lib.sh
  ...
  printf '\n' >>"$repo/.pi/extensions/dj-primary-turnend-guard.ts"
  listed=$(cd "$repo" && bin/dj-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/dj-ask-user-authority.test.sh" "skill source selects pure contract coverage"
  assert_contains "$listed" "tests/dj-cd-pretool-check.test.sh" "Claude and Pi source selects hook coverage"
  assert_contains "$listed" "tests/dj-pi-watch-extension.test.sh" "Pi source selects watcher coverage"
  git -C "$repo" add .agents .claude .pi
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm non-bin-source-change

  printf '\n' >>"$repo/src/unmapped.ts"
  set +e
  (cd "$repo" && bin/dj-test-run.sh --list --changed --base HEAD) >"$tmp/out" 2>"$tmp/err"
  rc=$?
  set -e
...
test_empty_selection_emits_summary() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-empty.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  printf 'documentation only\n' >"$repo/README.md"
  out=$(cd "$repo" && bin/dj-test-run.sh --changed --base HEAD --json "$tmp/artifacts/timing.json" 2>"$tmp/err") \
    || fail "empty valid changed selection must pass"
  [ "$out" = "DJ_TEST_SUMMARY total=0 failed=0 skipped_gate=0 duration_ms=0" ] \
...
test_timing_markers_and_json() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-timing.XXXXXX")
...
SH
  ...
  begin_n=$(grep -c '^DJ_TEST_BEGIN ' "$out" || true)
  end_n=$(grep -c '^DJ_TEST_END ' "$out" || true)
  [ "$begin_n" -eq 1 ] || fail "expected one DJ_TEST_BEGIN, got $begin_n"
  [ "$end_n" -eq 1 ] || fail "expected one DJ_TEST_END, got $end_n"
  grep -Eq '^DJ_TEST_BEGIN .+ family=unclassified expected_gate_skip=none$' "$out" \
    || fail "BEGIN line missing family/expected_gate_skip: $(grep '^DJ_TEST_BEGIN' "$out")"
  grep -Eq '^DJ_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=false$' "$out" \
    || fail "END line missing exit/duration/gate_skip: $(grep '^DJ_TEST_END' "$out")"
  summary=$(grep '^DJ_TEST_SUMMARY ' "$out" || true)
  ...
  grep -q '^DJ_TEST_SLOWEST rank=1 ' "$out" \
    || fail "expected DJ_TEST_SLOWEST rank=1"
...
test_aggregate_exit_behavior() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-agg.XXXXXX")
...
SH
  ...
  grep -q 'DJ_TEST_SUMMARY total=2 failed=1' "$tmp/out.txt" \
    || fail "summary should report total=2 failed=1: $(grep DJ_TEST_SUMMARY "$tmp/out.txt")"
  # All-green stays 0.
  set +e
...
test_gate_skip_accounting() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-skip.XXXXXX")
...
SH
  ...
  grep -Eq '^DJ_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$out" \
    || fail "END must mark gate_skip=true: $(grep '^DJ_TEST_END' "$out")"
  grep -q 'DJ_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$out" \
    || fail "summary must count skipped_gate=1: $(grep DJ_TEST_SUMMARY "$out")"
  python3 -c '
import json, sys
doc = json.load(open(sys.argv[1]))
...
test_fail_on_gate_skip_token() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-fail-skip.XXXXXX")
  skip_f="$tmp/skip.test.sh"
  out="$tmp/out.txt"
  cat >"$skip_f" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  grep -q 'DJ_TEST_SUMMARY total=1 failed=1' "$out" \
    || fail "summary must report failed=1 under fail-on-gate-skip: $(grep DJ_TEST_SUMMARY "$out")"
...
test_exclude_family() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/dj-backend-herdr-smoke.test.sh' \
    && fail "exclude-family real-herdr-gated left a real-herdr script"
  printf '%s\n' "$listed" | grep -Fq 'tests/dj-lint.test.sh' \
...
test_portable_shard_union_and_coverage_guard() {
  ...
  printf '%s\n' "$s1" "$s2" "$serial" | grep -Fq 'tests/dj-backend-herdr-smoke.test.sh' \
    && fail "portable lanes must not include real-herdr-gated smoke"
  printf '%s\n' "$herdr" | grep -Fq 'tests/dj-backend-herdr-smoke.test.sh' \
    || fail "herdr family must include smoke"
  out=$("$RUNNER" --check-coverage)
  assert_contains "$out" "DJ_TEST_COVERAGE ok" "coverage guard success marker"
  all_count=$("$RUNNER" --list --all | wc -l | tr -d ' ')
...
test_portable_serial_shard_lane_refusals() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-shard-lane.XXXXXX")
...
test_jobs_requires_proven_isolated() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-jobs.XXXXXX")
  set +e
  ...
  set +e
  "$RUNNER" --jobs 2 tests/dj-watcher-lock.test.sh >"$tmp/out2" 2>"$tmp/err2"
  rc=$?
  set -e
...
test_jobs_parallel_scheduler_and_failure_propagation() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-jobs-sched.XXXXXX")
  repo="$tmp/repo"
  runner="$repo/bin/dj-test-run.sh"
  evidence="$tmp/evidence"
  fake_bin="$tmp/fake-bin"
  a=tests/dj-brief.test.sh
  b=tests/dj-composer-lib.test.sh
  c=tests/dj-lint.test.sh
  d=tests/dj-supervision-instructions.test.sh
...
SH
  ...
  begin_n=$(grep -c '^DJ_TEST_BEGIN ' "$tmp/out" || true)
  end_n=$(grep -c '^DJ_TEST_END ' "$tmp/out" || true)
  [ "$begin_n" -eq 3 ] || fail "expected 3 BEGIN markers, got $begin_n"
  [ "$end_n" -eq 3 ] || fail "expected 3 END markers, got $end_n"
  grep -q 'DJ_TEST_SUMMARY total=3 failed=0' "$tmp/out" \
    || fail "summary missing for jobs run: $(grep DJ_TEST_SUMMARY "$tmp/out")"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
...
SH
  ...
  grep -q 'DJ_TEST_SUMMARY total=2 failed=1' "$tmp/out4" \
    || { rm -rf "$tmp"; fail "jobs failure summary wrong: $(grep DJ_TEST_SUMMARY "$tmp/out4")"; }
...
SH
  ...
  grep -q 'DJ_TEST_SUMMARY total=1 failed=1' "$tmp/out5" \
    || { rm -rf "$tmp"; fail "parallel stderr hard-fail summary wrong: $(grep DJ_TEST_SUMMARY "$tmp/out5")"; }

  "$runner" --jobs 2 "$d" >"$tmp/out6" 2>"$tmp/err6" \
    || { rm -rf "$tmp"; fail "ordinary parallel stderr gate skip must remain successful"; }
  grep -Eq '^DJ_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr gate skip was not recorded"; }
  grep -q 'DJ_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr skip summary wrong: $(grep DJ_TEST_SUMMARY "$tmp/out6")"; }
...
test_aggregate_json() {
  local tmp a b
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-test-run-aggjson.XXXXXX")
  cat >"$tmp/a.json" <<'JSON'
{
  "run_id": "a",
...
JSON
  ...
  assert_contains "$out" "DJ_TEST_AGGREGATE lanes=2 total=3 failed=1" "aggregate summary line"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
