#!/usr/bin/env bash
# Tests for bin/dj-pr-merge.sh: the one path jarvis uses to merge a task's
# PR, which must always record pr= and any available pr_head= into the task's
# meta before merging so dj-teardown.sh's landed-check has a PR reference to
# verify against, even on repos with no PR CI where the usual "checks green"
# dj-pr-check.sh trigger never fires.
#
# Matrix:
...
TMP_ROOT=$(dj_test_tmproot dj-pr-merge-tests)
BASE_PATH=$PATH
...
make_case() {
  ...
  dj_write_meta "$case_dir/state/task-x1.meta" \
    ...
    "kind=ship" \
    "mode=no-mistakes"
  # No worktree/project on disk; dj-pr-check.sh tolerates a worktree it cannot
...
# headRefOid for dj-pr-check.sh's pr_head lookup. Args: case_dir head_sha
add_gh_mocks() {
...
#!/usr/bin/env bash
printf 'GITLAB_HOST=%s %s\n' "${GITLAB_HOST-<unset>}" "$*" >> "$DJ_TEST_GLAB_LOG"
case_dir=$(dirname "$DJ_TEST_GLAB_JSON")
case "${1:-} ${2:-}" in
  "mr view")
    [ ! -e "$case_dir/glab-view-fails" ] || exit 1
    cat "$DJ_TEST_GLAB_JSON"
    exit 0
    ;;
  "mr merge")
...
run_pr_merge() {
  ...
  DJ_TEST_GH_AXI_LOG="$case_dir/gh-axi.log" \
  DJ_TEST_GLAB_LOG="$case_dir/glab.log" \
  DJ_TEST_GLAB_JSON="$case_dir/mr.json" \
...
test_records_pr_and_head_before_merging() {
  ...
  expect_code 0 "$rc" "records-before-merge: dj-pr-merge should succeed"
  ...
  pass "dj-pr-merge records pr= and pr_head= before invoking gh-axi pr merge"
}
...
test_merge_failure_propagates_after_recording() {
  ...
  expect_code 1 "$rc" "merge-fails: dj-pr-merge should propagate the gh-axi merge failure"
  assert_grep 'pr=https://github.com/example/repo/pull/13' "$case_dir/state/task-x1.meta" \
    "merge-fails: pr= should already be recorded even though the merge itself failed"
  pass "dj-pr-merge propagates a real merge failure without silently succeeding"
}
...
test_extra_merge_args_forwarded() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/15 -- --squash --delete-branch \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "extra-args: dj-pr-merge failed"

  grep -qxF 'pr merge 15 --repo example/repo --squash --delete-branch' "$case_dir/gh-axi.log" \
    || fail "extra-args: extra gh-axi pr merge flags were not forwarded"
  pass "dj-pr-merge forwards extra flags to gh-axi pr merge after the -- separator"
}
...
test_missing_meta_refuses_before_merge() {
  ...
  expect_code 1 "$rc" "missing-meta: dj-pr-merge should refuse"
  assert_grep 'error: task metadata is unavailable' "$case_dir/stderr" \
    "missing-meta: refusal did not explain missing meta"
  [ ! -s "$case_dir/gh-axi.log" ] || fail "missing-meta: gh-axi pr merge was invoked"
  assert_absent "$case_dir/state/missing-x1.check.sh" \
    "missing-meta: dj-pr-check should not arm a poll for an unknown task"
  pass "dj-pr-merge refuses before merging when task meta is missing"
}
...
test_malformed_url_refuses_before_merge() {
  ...
  expect_code 2 "$rc" "malformed-url: dj-pr-merge should refuse a malformed merge request URL"
  ...
  pass "dj-pr-merge refuses malformed PR URLs before calling gh-axi"
}
...
test_rejects_unsafe_url_segments_before_recording() {
  ...
  expect_code 1 "$rc" "unsafe-url-segment: dj-pr-merge should refuse unsafe owner/repo characters"
  ...
  pass "dj-pr-merge refuses unsafe PR URL segments before recording state"
}
...
test_repo_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "repo-override: dj-pr-merge should refuse repo override flags"
  ...
  pass "dj-pr-merge refuses repo override args before recording state"
}
...
test_bundled_repo_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "bundled-repo-override: dj-pr-merge should refuse a bundled repo override"
  assert_grep 'extra merge arguments must not override the repository' "$case_dir/stderr" \
  ...
  expect_code 1 "$rc" "bundled-repo-override-gitlab: dj-pr-merge should refuse a bundled instance override"
  assert_grep 'extra merge arguments must not override the repository' "$case_dir/stderr" \
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/8 -- -d \
    > "$case_dir/stdout" 2> "$case_dir/stderr" \
    || fail "bundled-non-repo-cluster: dj-pr-merge refused a short flag that overrides nothing"
  ...
  pass "dj-pr-merge refuses a bundled short-option repo override and forwards other short flags"
}
...
test_explicit_merge_method_not_overridden() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/22 -- --merge \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "explicit-merge-method: dj-pr-merge failed"

  grep -qxF 'pr merge 22 --repo example/repo --merge' "$case_dir/gh-axi.log" \
    || fail "explicit-merge-method: caller --merge was not forwarded without an extra default --squash"
  pass "dj-pr-merge does not add default --squash when the caller passes an explicit merge method"
}
...
test_method_equals_merge_method_not_overridden() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/23 -- --method=merge \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "method-equals-merge-method: dj-pr-merge failed"

  grep -qxF 'pr merge 23 --repo example/repo --method=merge' "$case_dir/gh-axi.log" \
    || fail "method-equals-merge-method: caller --method=merge was not forwarded without an extra default --squash"
  pass "dj-pr-merge respects --method=<value> as an explicit merge method"
}
...
test_parses_pr_url_for_gh_axi() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/my-org/my-repo/pull/126 \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "url-parsing: dj-pr-merge failed"

  grep -qxF 'pr merge 126 --repo my-org/my-repo --squash' "$case_dir/gh-axi.log" \
    || fail "url-parsing: gh-axi pr merge was not invoked as number + --repo + default --squash"
  pass "dj-pr-merge parses a GitHub PR URL into gh-axi number and --repo arguments"
}
...
test_gitlab_url_resolves_and_merges() {
  ...
  pass "dj-pr-merge merges a GitLab merge request through glab instead of refusing it"
}
...
test_gitlab_host_comes_from_the_url() {
  ...
  pass "dj-pr-merge takes the GitLab instance from the URL rather than assuming one"
}
...
test_gitlab_imposes_no_merge_method() {
  ...
  pass "dj-pr-merge imposes no merge method on GitLab, leaving the project's own one"
}
...
test_gitlab_extra_args_forwarded() {
  ...
  pass "dj-pr-merge forwards extra flags to glab mr merge after the -- separator"
}
...
test_gitlab_merge_failure_propagates() {
  ...
  pass "dj-pr-merge propagates a real glab merge failure without silently succeeding"
}
...
test_gitlab_each_condition_refuses_independently() {
  ...
  for spec in "$@"; do
    ...
    expect_code 1 "$rc" "gitlab-refuse-$name: dj-pr-merge should refuse"
  ...
  pass "dj-pr-merge refuses on each GitLab pre-merge condition independently"
}
...
test_gitlab_reports_every_failing_condition() {
  ...
  expect_code 1 "$rc" "gitlab-refuse-all: dj-pr-merge should refuse"
  for expected in \
  ...
  pass "dj-pr-merge reports every failing GitLab condition, not only the first"
}
...
test_gitlab_stale_recorded_head_is_reported() {
  ...
  # dj-pr-check.sh rewrites the metadata, which drops a head it cannot resolve
  ...
  pass "dj-pr-merge reports a stale recorded head and verifies the live one"
}
...
test_gitlab_unreadable_state_refuses() {
  ...
  for name in view-fails not-an-object split-value; do
    ...
    expect_code 1 "$rc" "gitlab-unreadable-$name: dj-pr-merge should refuse"
  ...
  pass "dj-pr-merge refuses an unreadable GitLab merge request state rather than merging blind"
}
...
test_gitlab_invalid_head_refuses() {
  ...
  expect_code 1 "$rc" "gitlab-invalid-head: dj-pr-merge should refuse"
  ...
  pass "dj-pr-merge refuses a GitLab head commit it cannot validate"
}
...
test_gitlab_missing_tool_refuses_before_recording() {
  ...
  for tool in glab jq; do
    ...
    expect_code 1 "$rc" "gitlab-no-$tool: dj-pr-merge should refuse"
  ...
  pass "dj-pr-merge refuses before recording anything when glab or jq is absent"
}
...
test_gitlab_head_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "gitlab-head-override: dj-pr-merge should refuse a caller head override"
  ...
  pass "dj-pr-merge refuses a GitLab head override before recording state"
}
...
test_github_still_forwards_sha_arg() {
  ...
  # --sha is rejected only where the head is jarvis's to determine. GitHub's
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/44 -- --sha abc123 \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "github-sha-arg: dj-pr-merge failed"
  ...
  pass "dj-pr-merge leaves GitHub extra-arg handling unchanged, including --sha"
}
