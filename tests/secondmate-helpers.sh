...
# suites (dj-secondmate-lifecycle-e2e and dj-secondmate-safety).
#
...
# A fake tmux (window ops are logged to DJ_FAKE_TMUX_LOG, list-windows returns
# DJ_FAKE_TMUX_WINDOW, capture-pane echoes DJ_FAKE_TMUX_CAPTURE) plus a fake
# treehouse (durable lease of DJ_FAKE_TREEHOUSE_HOME, recording the lease holder
# to DJ_FAKE_TREEHOUSE_LEASE_FILE; `return` removes the target and lease unless
# DJ_FAKE_TREEHOUSE_RETURN_FAIL is set). Echoes the fakebin dir.
make_fake_tmux() {
...
case "${1:-}" in
  get)
    ...
    if [ -n "${DJ_FAKE_TREEHOUSE_HOME:-}" ]; then
      mkdir -p "$DJ_FAKE_TREEHOUSE_HOME"
      [ -n "${DJ_FAKE_TREEHOUSE_LEASE_FILE:-}" ] && printf '%s\n' "$holder" > "$DJ_FAKE_TREEHOUSE_LEASE_FILE"
      printf 'leased worktree for %s\n' "${holder:-unknown}" >&2
      printf '%s\n' "$DJ_FAKE_TREEHOUSE_HOME"
    fi
    exit 0
    ;;
  return)
    ...
    [ -z "${DJ_FAKE_TREEHOUSE_RETURN_FAIL:-}" ] || exit 17
    [ -n "${DJ_FAKE_TREEHOUSE_LEASE_FILE:-}" ] && rm -f "$DJ_FAKE_TREEHOUSE_LEASE_FILE"
...
# DJ_FAKE_NO_MISTAKES_LOG and fails for the project named DJ_FAKE_NO_MISTAKES_FAIL_PROJECT.
...
#!/usr/bin/env bash
set -eu
printf '%s\t%s\n' "$PWD" "${1:-}" >> "$DJ_FAKE_NO_MISTAKES_LOG"
if [ "$(basename "$PWD")" = "${DJ_FAKE_NO_MISTAKES_FAIL_PROJECT:-}" ]; then
  exit 1
fi
case "${1:-}" in
  init) touch .no-mistakes-init ;;
...
# Make a directory look like a minimal jarvis home (AGENTS.md + bin/).
mark_jarvis_home() {
  local home=$1
  mkdir -p "$home/bin"
  printf '# Jarvis\n' > "$home/AGENTS.md"
...
# A jarvis home that is also a real git repo (so it can host detached
# worktrees for teardown/lease tests).
make_jarvis_git_root() {
  local home=$1
  mkdir -p "$home/bin"
  printf '# Jarvis\n' > "$home/AGENTS.md"
  cat > "$home/bin/dj-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$home/bin/dj-guard.sh"
  git -C "$home" init -q
  git -C "$home" add AGENTS.md bin/dj-guard.sh
  git -C "$home" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm initial
}
...
scaffold_secondmate_charter() {
  ...
  shift 3
  DJ_HOME="$home" DJ_SECONDMATE_CHARTER="$charter" "$ROOT/bin/dj-brief.sh" "$id" --secondmate "$@" >/dev/null
}

# Make a directory look like a genuine seeded secondmate home (for handoff tests).
seed_secondmate_home_marker() {
  local home=$1 id=$2
  mark_jarvis_home "$home"
  mkdir -p "$home/data"
