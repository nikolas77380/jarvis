#!/usr/bin/env bash
# Parity guard for jarvis's shell-lint definition.
#
# bin/dj-lint.sh must be the single owner that BOTH CI
...
# ShellCheck retired in 0.11.0. dj-lint.sh now pins one exact version and both
...
LINT="$ROOT/bin/dj-lint.sh"
INSTALLER="$ROOT/bin/dj-install-shellcheck.sh"
...
# dj_install_stub_uname <fakebin>: uname -s / uname -m from DJ_TEST_UNAME_S/M.
dj_install_stub_uname() {
  local fakebin=$1
  cat > "$fakebin/uname" <<'SH'
#!/usr/bin/env bash
case "${1:-}" in
  -s) printf '%s\n' "${DJ_TEST_UNAME_S:-Linux}" ;;
  -m) printf '%s\n' "${DJ_TEST_UNAME_M:-x86_64}" ;;
  *) printf '%s\n' "${DJ_TEST_UNAME_S:-Linux}" ;;
esac
SH
...
# dj_install_stub_curl <fakebin>: log the URL, fail CURL_FAIL_UNTIL times, then
# write an empty file at -o. CURL_COUNT and CURL_URL_LOG are paths the stub
# updates when invoked.
dj_install_stub_curl() {
  local fakebin=$1
  cat > "$fakebin/curl" <<'SH'
#!/usr/bin/env bash
count=0
...
# dj_install_stub_hasher <fakebin> <name>: sha256sum or shasum stub that prints
# SHA256_STUB_HASH and records the invocation on HASHER_LOG. shasum requires -a 256.
dj_install_stub_hasher() {
...
dj_install_stub_tar_shellcheck() {
  local fakebin=$1
  cat > "$fakebin/tar" <<'SH'
#!/usr/bin/env bash
while [ "$#" -gt 0 ]; do
...
test_help_reports_the_complete_interface() {
  local help
  help=$("$LINT" --help) || fail "dj-lint.sh --help failed"
  assert_contains "$help" "--telemetry" "dj-lint.sh --help omitted --telemetry"
  assert_contains "$help" "--required-version" "dj-lint.sh --help omitted --required-version"
  assert_contains "$help" "--list-files" "dj-lint.sh --help omitted --list-files"
  assert_contains "$help" "--help" "dj-lint.sh --help omitted --help"
  assert_contains "$help" "--fast" "dj-lint.sh --help omitted --fast"
  pass "dj-lint.sh --help reports the complete executable interface"
}
...
test_list_files_reports_the_shell_inventory() {
  ...
  # inventory check independent of dj-lint.sh's own changed-file mode below.
  ...
  [ "$(printf '%s\n' "$listed" | LC_ALL=C sort)" = "$expected" ] \
    || fail "dj-lint.sh --list-files did not return the complete shell inventory"
  pass "dj-lint.sh --list-files reports the complete shell inventory"
}

# dj_lint_stub_git <fakebin-dir>: install a git stub for the changed-file mode
# tests below. Its answers are driven by env vars the caller sets before
# invoking dj-lint.sh, so those tests can steer git state without depending on
# this worktree's actual branch, remotes, or history:
#   DJ_TEST_GIT_INSIDE_WORKTREE  1 (default) or 0
#   DJ_TEST_GIT_BRANCH           branch name for `rev-parse --abbrev-ref HEAD`
#   DJ_TEST_GIT_HAS_ORIGIN_MAIN  1 (default) or 0
#   DJ_TEST_GIT_HAS_MAIN         1 (default) or 0
#   DJ_TEST_GIT_MERGE_BASE_OK    1 (default) or 0
#   DJ_TEST_GIT_MERGE_BASE       merge-base value to print when OK
#   DJ_TEST_GIT_DIFF_FILE        path to a file of NUL-separated changed paths
dj_lint_stub_git() {
  local fakebin=$1
  cat > "$fakebin/git" <<'SH'
#!/usr/bin/env bash
case "$*" in
  "rev-parse --is-inside-work-tree")
    [ "${DJ_TEST_GIT_INSIDE_WORKTREE:-1}" = 1 ] || exit 1
    printf 'true\n'
    exit 0
    ;;
  "rev-parse --abbrev-ref HEAD")
    printf '%s\n' "${DJ_TEST_GIT_BRANCH:-feature}"
    exit 0
    ;;
  "rev-parse --verify -q origin/main")
    [ "${DJ_TEST_GIT_HAS_ORIGIN_MAIN:-1}" = 1 ] && exit 0 || exit 1
    ;;
  "rev-parse --verify -q main")
    [ "${DJ_TEST_GIT_HAS_MAIN:-1}" = 1 ] && exit 0 || exit 1
    ;;
  "merge-base "*)
    if [ "${DJ_TEST_GIT_MERGE_BASE_OK:-1}" = 1 ]; then
      printf '%s\n' "${DJ_TEST_GIT_MERGE_BASE:-fakebase123}"
      exit 0
    fi
    exit 1
    ;;
  "diff --name-only --diff-filter=ACMR -z "*)
    if [ -n "${DJ_TEST_GIT_DIFF_FILE:-}" ] && [ -f "$DJ_TEST_GIT_DIFF_FILE" ]; then
      cat "$DJ_TEST_GIT_DIFF_FILE"
    fi
    exit 0
    ;;
  *)
...
# dj_lint_write_diff_file <file> <path>...: writes NUL-separated changed paths
# in the shape `git diff --name-only -z` produces, for DJ_TEST_GIT_DIFF_FILE.
dj_lint_write_diff_file() {
  local file=$1
  shift
  printf '%s\0' "$@" > "$file"
}

# dj_lint_stub_shellcheck <fakebin-dir> <log-file>: install a ShellCheck stub
...
# them, so changed-file mode tests can assert exactly which files dj-lint.sh
# selected without depending on real ShellCheck findings. When
# DJ_TEST_MODE_LOG is set, it records the effective analysis mode, treating
# ShellCheck's default as full analysis.
dj_lint_stub_shellcheck() {
...
if [ -n "\${DJ_TEST_MODE_LOG:-}" ]; then
  printf '%s\n' "\$mode" >> "\$DJ_TEST_MODE_LOG"
fi
[ "\$#" -eq 0 ] || shift
printf '%s\n' "\$@" >> "$log"
exit 0
...
test_fast_mode_disables_extended_analysis() {
  ...
  tmp=$(dj_test_tmproot dj-lint-fast-mode)
  fakebin=$(dj_fakebin "$tmp")
...
SH
  chmod +x "$fixture"
  dj_lint_stub_shellcheck "$fakebin" "$log"

  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' DJ_LINT_JOBS=1 \
    DJ_TEST_MODE_LOG="$mode_log" "$LINT" --fast --telemetry "$telemetry" "$fixture" 2>&1) \
  ...
  pass "dj-lint.sh --fast disables ShellCheck extended analysis"
}

test_ci_defaults_to_full_analysis() {
  local tmp fakebin log mode_log fixture out
  tmp=$(dj_test_tmproot dj-lint-ci-analysis)
  fakebin=$(dj_fakebin "$tmp")
  fixture="$tmp/fixture.sh"
  log="$tmp/shellcheck.log"
  mode_log="$tmp/mode.log"
...
SH
  ...
  out=$(PATH="$fakebin:$PATH" CI=true GITHUB_ACTIONS=true DJ_LINT_FAST=1 DJ_LINT_JOBS=1 \
    DJ_TEST_MODE_LOG="$mode_log" "$LINT" "$fixture" 2>&1) \
    || fail "CI full lint mode failed"$'\n'"$out"
  [ "$(cat "$mode_log")" = on ] \
    || fail "CI default did not keep full ShellCheck analysis"
  pass "dj-lint.sh keeps full ShellCheck analysis by default in CI"
}

test_ci_rejects_explicit_fast_mode() {
  local tmp fakebin log fixture out rc
  tmp=$(dj_test_tmproot dj-lint-ci-reject-fast)
  fakebin=$(dj_fakebin "$tmp")
  fixture="$tmp/fixture.sh"
  log="$tmp/shellcheck.log"
  cat > "$fixture" <<'SH'
...
SH
  ...
  rc=0
  out=$(PATH="$fakebin:$PATH" CI=true GITHUB_ACTIONS=true DJ_LINT_JOBS=1 \
  ...
  pass "dj-lint.sh rejects explicit --fast mode in CI"
}
...
test_fast_mode_catches_a_real_lint_defect() {
  ...
  tmp=$(dj_test_tmproot dj-lint-fast-bad)
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
foo() {
...
SH
  ...
  pass "dj-lint.sh --fast catches an ordinary shell lint defect"
}

test_changed_mode_lints_only_the_changed_file() {
  local tmp fakebin log diff_file out target
  tmp=$(dj_test_tmproot dj-lint-changed)
  fakebin=$(dj_fakebin "$tmp")
  dj_lint_stub_git "$fakebin"
  log="$tmp/shellcheck.log"
  dj_lint_stub_shellcheck "$fakebin" "$log"
  diff_file="$tmp/diff.nul"
  target="bin/dj-install-shellcheck.sh"
  dj_lint_write_diff_file "$diff_file" "$target" "README.md"
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' DJ_LINT_JOBS=1 \
    DJ_TEST_GIT_BRANCH=feature \
    DJ_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" 2>&1) \
    || fail "changed-mode lint run failed"$'\n'"$out"
  [ "$(cat "$log")" = "$target" ] \
    || fail "changed-mode lint did not run ShellCheck on exactly the changed file"$'\n'"logged: $(cat "$log")"
  pass "dj-lint.sh changed mode lints only the changed canonical file"
}

test_ci_forces_full_lint_even_with_empty_diff() {
  local listed expected
  # No git stub: CI=true must short-circuit dj-lint.sh's mode selection before
  ...
  pass "dj-lint.sh forces a full lint in CI even when the local diff would be empty"
}

test_main_branch_forces_full_lint() {
  local tmp fakebin listed expected
  tmp=$(dj_test_tmproot dj-lint-main-full)
  fakebin=$(dj_fakebin "$tmp")
  dj_lint_stub_git "$fakebin"
  ...
  listed=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' \
    DJ_TEST_GIT_BRANCH=main "$LINT" --list-files)
  expected=$(find bin bin/backends tests -maxdepth 1 -type f -name '*.sh' -print | LC_ALL=C sort)
  [ "$(printf '%s\n' "$listed" | LC_ALL=C sort)" = "$expected" ] \
    || fail "dj-lint.sh did not force a full lint when HEAD is on main"
  pass "dj-lint.sh forces a full lint when HEAD is on main"
}

test_explicit_path_bypasses_changed_logic() {
  local tmp fakebin log out target
  tmp=$(dj_test_tmproot dj-lint-explicit-override)
  fakebin=$(dj_fakebin "$tmp")
  dj_lint_stub_git "$fakebin"
  log="$tmp/shellcheck.log"
  dj_lint_stub_shellcheck "$fakebin" "$log"
...
test_zero_changed_files_exits_clean() {
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' DJ_TEST_GIT_BRANCH=feature \
    DJ_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" 2>&1) || rc=$?
  ...
  pass "dj-lint.sh exits 0 with a note when the local branch has no changed lint targets"
}

test_list_files_respects_changed_mode() {
  local tmp fakebin diff_file listed
  tmp=$(dj_test_tmproot dj-lint-list-changed)
  fakebin=$(dj_fakebin "$tmp")
  dj_lint_stub_git "$fakebin"
  diff_file="$tmp/diff.nul"
  ...
  dj_lint_write_diff_file "$diff_file" \
    "tests/dj-lint.test.sh" "docs/README.md" "bin/definitely-not-real-file.sh"
  ...
  listed=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' DJ_TEST_GIT_BRANCH=feature \
    DJ_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" --list-files)
  [ "$listed" = "tests/dj-lint.test.sh" ] \
    || fail "--list-files did not report the would-be changed set in changed mode"$'\n'"got: $listed"
  pass "dj-lint.sh --list-files reports the would-be changed set in changed mode"
}

test_pins_an_explicit_version() {
  [ -n "$REQUIRED" ] || fail "dj-lint.sh --required-version printed nothing"
  ...
  assert_contains "$REQUIRED" "0.11.0" "dj-lint.sh must pin ShellCheck 0.11.0"
  pass "dj-lint.sh pins an explicit ShellCheck version ($REQUIRED)"
}

test_installer_retries_transient_download_failure() {
  local tmp fakebin destination out
  tmp=$(dj_test_tmproot dj-shellcheck-download)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"

  dj_install_stub_uname "$fakebin"
  dj_install_stub_curl "$fakebin"
  dj_install_stub_hasher "$fakebin" sha256sum
  dj_install_stub_tar_shellcheck "$fakebin"
  dj_install_stub_sleep "$fakebin"
  ...
  out=$(CURL_COUNT="$tmp/curl-count" CURL_FAIL_UNTIL=3 \
    ...
    DJ_TEST_UNAME_S=Linux DJ_TEST_UNAME_M=x86_64 \
...
test_installer_selects_platform_archive_url_and_checksum() {
  ...
  tmp=$(dj_test_tmproot dj-shellcheck-platform)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"
  ...
  while IFS=$'\t' read -r uname_s uname_m archive sha; do
    ...
    out=$(CURL_URL_LOG="$url_log" SHA256_STUB_HASH="$sha" \
      DJ_TEST_UNAME_S="$uname_s" DJ_TEST_UNAME_M="$uname_m" \
      PATH="$fakebin:$PATH" "$INSTALLER" "$destination" 2>&1) \
...
test_installer_rejects_wrong_checksum() {
  ...
  tmp=$(dj_test_tmproot dj-shellcheck-badsum)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_falls_back_to_shasum() {
  ...
  tmp=$(dj_test_tmproot dj-shellcheck-shasum)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"
  ...
  dj_install_stub_uname "$fakebin"
  dj_install_stub_curl "$fakebin"
  dj_install_stub_hasher "$fakebin" shasum
  dj_install_stub_tar_shellcheck "$fakebin"
  dj_install_stub_sleep "$fakebin"
...
test_installer_prefers_sha256sum_over_shasum() {
  ...
  tmp=$(dj_test_tmproot dj-shellcheck-sha256sum-pref)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"
  hasher_log="$tmp/hasher.log"
...
test_installer_rejects_unsupported_platform() {
  local tmp fakebin destination out rc
  tmp=$(dj_test_tmproot dj-shellcheck-unsupported)
  fakebin=$(dj_fakebin "$tmp")
  destination="$tmp/bin"
  ...
  rc=0
  out=$(DJ_TEST_UNAME_S=FreeBSD DJ_TEST_UNAME_M=amd64 \
    PATH="$fakebin:$PATH" "$INSTALLER" "$destination" 2>&1) || rc=$?
  ...
  rc=0
  out=$(DJ_TEST_UNAME_S=Linux DJ_TEST_UNAME_M=ppc64le \
    PATH="$fakebin:$PATH" "$INSTALLER" "$destination" 2>&1) || rc=$?
...
test_missing_shellcheck_fails_closed() {
  ...
  tmp=$(dj_test_tmproot dj-lint-noshellcheck)
  fakebin=$(dj_fakebin "$tmp")
  ...
  assert_contains "$out" "dj-install-shellcheck.sh" \
...
test_rejects_wrong_shellcheck_version() {
  ...
  tmp=$(dj_test_tmproot dj-lint-ver)
  fakebin=$(dj_fakebin "$tmp")
  cat > "$fakebin/shellcheck" <<'SH'
#!/usr/bin/env bash
if [ "$1" = "--version" ]; then
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "dj-lint.sh accepted a shellcheck version other than the pin"$'\n'"$out"
  assert_contains "$out" "$REQUIRED" "dj-lint.sh did not name the required version on mismatch"
  assert_contains "$out" "0.9.9" "dj-lint.sh did not report the resolved (wrong) version"
  pass "dj-lint.sh refuses to lint under a non-pinned ShellCheck version"
}
...
test_catches_a_real_lint_defect() {
  ...
  local tmp bad out rc
  tmp=$(dj_test_tmproot dj-lint-bad)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  rc=0
  out=$("$LINT" "$bad" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "dj-lint.sh passed a known-bad fixture"$'\n'"$out"
  assert_contains "$out" "SC1007" "dj-lint.sh did not report the expected ShellCheck finding"
  pass "dj-lint.sh catches a real lint defect the old no-op gate passed"
}
...
test_ignores_ambient_shellcheck_opts() {
  ...
  local tmp bad out rc
  tmp=$(dj_test_tmproot dj-lint-opts)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "dj-lint.sh allowed ambient SHELLCHECK_OPTS to hide a finding"$'\n'"$out"
  assert_contains "$out" "SC1007" "dj-lint.sh did not neutralize ambient SHELLCHECK_OPTS"
  pass "dj-lint.sh ignores ambient ShellCheck options"
}
...
test_clean_fixture_passes() {
  ...
  local tmp good rc
  tmp=$(dj_test_tmproot dj-lint-good)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  cat > "$good" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -eq 0 ] || fail "dj-lint.sh flagged a clean fixture (exit $rc)"
  pass "dj-lint.sh passes a clean fixture"
}
...
test_jobs_are_deterministic_and_complete() {
  ...
  tmp=$(dj_test_tmproot dj-lint-jobs)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  bad_a="$tmp/bad-a.sh"
  bad_b="$tmp/bad-b.sh"
...
SH
  ...
  rc_clean_1=0
  out_clean_1=$(DJ_LINT_JOBS=1 "$LINT" "$good" 2>&1) || rc_clean_1=$?
  rc_clean_2=0
  out_clean_2=$(DJ_LINT_JOBS=2 "$LINT" "$good" 2>&1) || rc_clean_2=$?
  ...
  rc_fail_1=0
  out_fail_1=$(DJ_LINT_JOBS=1 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_1=$?
  rc_fail_2=0
  out_fail_2=$(DJ_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2=$?
  rc_fail_2b=0
  out_fail_2b=$(DJ_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2b=$?
  ...
  rc_bad_jobs=0
  DJ_LINT_JOBS=3 "$LINT" "$good" >/dev/null 2>&1 || rc_bad_jobs=$?
  [ "$rc_bad_jobs" -eq 2 ] || fail "the lint owner must reject unbounded worker counts"

  telemetry_out=$(DJ_LINT_JOBS=2 DJ_LINT_TELEMETRY="$telemetry" "$LINT" "$good" 2>&1) \
  ...
  mkdir -p "$cleanup_tmp"
  cleanup_out=$(TMPDIR="$cleanup_tmp" DJ_LINT_JOBS=2 "$LINT" "$good" 2>&1) \
    || fail "cleanup fixture lint failed"
  [ "$cleanup_out" = "$out_clean_2" ] || fail "cleanup fixture changed routine diagnostics"
  [ -z "$(find "$cleanup_tmp" -mindepth 1 -maxdepth 1 -name 'dj-lint.*' -print -quit)" ] \
...
test_worker_trees_stop_on_signal() {
  ...
  tmp=$(dj_test_tmproot dj-lint-signal)
  mkdir -p "$tmp"
  fakebin=$(dj_fakebin "$tmp")
  fixture="$tmp/good.sh"
  cat > "$fixture" <<'SH'
...
SH
  ...
  for jobs in 1 2; do
    for telemetry in off on; do
      ...
      PATH="$fakebin:$PATH" TMPDIR="$lint_tmp" DJ_LINT_JOBS="$jobs" \
        DJ_LINT_TELEMETRY="$telemetry_file" DJ_TEST_SHELLCHECK_PID="$pid_file" \
      ...
      [ -z "$(find "$lint_tmp" -mindepth 1 -maxdepth 1 -name 'dj-lint.*' -print -quit)" ] \
...
test_seeded_module_boundary_parity() {
  ...
  tmp=$(mktemp -d "$ROOT/.dj-lint-parity.XXXXXX")
  if [ "${#DJ_TEST_CLEANUP_DIRS[@]}" -eq 0 ]; then
    trap dj_test_cleanup EXIT
  fi
  DJ_TEST_CLEANUP_DIRS+=("$tmp")
  rel=${tmp#"$ROOT/"}
...
SH
  ...
  rc=0
  out=$(DJ_LINT_JOBS=2 "$LINT" "$dispatcher" "$adapter" "$owner" "$test_root" 2>&1) || rc=$?
