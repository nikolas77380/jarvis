#!/usr/bin/env bash
# Focused safety tests for bin/dj-herdr-session-cleanup.sh.
...
TMP_ROOT=$(dj_test_tmproot dj-herdr-session-cleanup)
DJ_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap dj_test_cleanup EXIT

export DJ_HOME="$TMP_ROOT/home"
export DJ_STATE_OVERRIDE="$DJ_HOME/state"
export DJ_CONFIG_OVERRIDE="$DJ_HOME/config"
mkdir -p "$DJ_STATE_OVERRIDE" "$DJ_CONFIG_OVERRIDE"
touch "$DJ_CONFIG_OVERRIDE/herdr-presentation-spaces"
printf '%s\n' herdr > "$DJ_CONFIG_OVERRIDE/backend"
FAKEBIN=$(dj_fakebin "$TMP_ROOT")
dj_fake_exit0 "$FAKEBIN" herdr
export PATH="$FAKEBIN:$PATH"
export DJ_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1
# shellcheck source=/dev/null
. "$ROOT/bin/dj-herdr-session-cleanup.sh"
unset DJ_HERDR_SESSION_CLEANUP_SOURCE_ONLY

# The idle-shell proof now lives in the backend as
# dj_backend_herdr_pane_idle_shell_pid; prove it still reads Linux argv
...
argv_pid=$(
  ...
  DJ_HERDR_PS_BIN="$FAKE_PS" dj_backend_herdr_pane_idle_shell_pid test w2:p1
...
if (
  ...
  DJ_HERDR_PS_BIN="$FAKE_PS" DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    dj_backend_herdr_pane_idle_shell_pid test w2:p1
) >/dev/null 2>&1; then
...
dj_backend_name() { printf herdr; }
dj_backend_herdr_session() { printf test; }
dj_backend_herdr_presentation_session_lock_path() { printf '%s/presentation.lock' "$TMP_ROOT"; }
dj_lock_try_acquire() {
  printf '%s\n' "$1" >> "$LOCK_LOG"
  mkdir "$1" 2>/dev/null
}
dj_lock_release() { rm -rf -- "$1"; }
dj_backend_herdr_pane_idle_shell_pid() { [ ! -e "$FIXTURE_DIR/process-unsafe" ] && printf '67\n'; }
...
fixture_workspaces() {
  ...
  printf '[{"workspace_id":"w1","label":"jarvis","focused":%s,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},' \
...
fixture_tabs() {
  ...
  while [ "$i" -le "$count" ]; do
    [ "$i" -eq 1 ] || printf ','
    printf '{"tab_id":"%s:t%s","workspace_id":"%s","focused":false,"label":"dj-task"}' "$WS" "$i" "$WS"
    i=$((i + 1))
  done
  printf ']'
}
...
dj_backend_herdr_cli() {
  ...
  if [ -e "$FIXTURE_DIR/closed" ]; then
    case "$first $second" in
      ...
      "workspace list") printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1}]}}'; return 0 ;;
    esac
  fi
  if [ -e "$FIXTURE_DIR/race" ] && [ -e "$FIXTURE_DIR/snapshotted" ] && [ "$first $second" = "workspace list" ]; then
    printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},{"workspace_id":"w2","label":"renamed","focused":false,"active_tab_id":"w2:t1","tab_count":1,"pane_count":1}]}}'
    return 0
  fi
  title=$(cat "$FIXTURE_DIR/title")
  tabs=$(cat "$FIXTURE_DIR/tabs")
...
write_v1() { # <id> [token]
  ...
  } > "$DJ_STATE_OVERRIDE/$id.herdr-presentation"
}
...
reset_fixture() {
  rm -rf "$FIXTURE_DIR" "$TMP_ROOT"/*.lock "${DJ_STATE_OVERRIDE:?}/"*
  mkdir -p "$FIXTURE_DIR"
  : > "$LOCK_LOG"; : > "$CLOSE_LOG"
...
assert_preserved() { # <case>
  ...
  [ -f "$DJ_STATE_OVERRIDE/$ID.herdr-presentation" ] && had_journal=1
  dj_herdr_session_cleanup >/dev/null 2>&1
  if [ "$had_journal" -eq 1 ]; then
    [ -f "$DJ_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "$name retired the journal"
  fi
...
reset_fixture
dj_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$DJ_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "positive cleanup kept the journal"
[ "$(wc -l < "$CLOSE_LOG" | tr -d ' ')" = 1 ] || fail "positive cleanup did not close exactly once"
[ "$(sed -n '1p' "$LOCK_LOG")" = "$DJ_STATE_OVERRIDE/.spawn-$ID.lock" ] || fail "task lock was not acquired first"
...
reset_fixture; printf 'version=1\ntask_id=%s\nprojection_id=short\n' "$ID" > "$DJ_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "malformed journal"
...
reset_fixture; rm -f "$DJ_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "zero journal match"
reset_fixture; write_v1 dj-task; assert_preserved "multiple journal matches"
reset_fixture; rm -f "$DJ_STATE_OVERRIDE/$ID.herdr-presentation"; write_cross_home_v2; assert_preserved "cross-home journal"
reset_fixture; write_v2 "$DJ_HOME" w9 "$TAB" "$PANE"; assert_preserved "v2 workspace binding mismatch"
reset_fixture; write_v2 "$DJ_HOME" "$WS" w9:t1 "$PANE"; assert_preserved "v2 tab binding mismatch"
reset_fixture; write_v2 "$DJ_HOME" "$WS" "$TAB" w9:p1; assert_preserved "v2 pane binding mismatch"
reset_fixture; write_v2 "$DJ_HOME" "$WS" "$TAB" "$PANE"
dj_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$DJ_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "matching v2 cleanup kept the journal"
...
reset_fixture; : > "$DJ_STATE_OVERRIDE/$ID.meta"; assert_preserved "current task metadata"
...
cat > "$INTEGRATION_ROOT/bin/dj-herdr-session-cleanup.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "${DJ_HOME:?}" >> "${DJ_HERDR_CLEANUP_TRACE:?}"
SH
chmod +x "$INTEGRATION_ROOT/bin/dj-herdr-session-cleanup.sh"
printf '%s\n' manual > "$INTEGRATION_ROOT/home/config/backlog-backend"
DJ_HOME="$INTEGRATION_ROOT/home" DJ_HERDR_CLEANUP_TRACE="$TRACE" DJ_BOOTSTRAP_DETECT_ONLY=1 \
  "$INTEGRATION_ROOT/bin/dj-bootstrap.sh" >/dev/null 2>&1
[ ! -e "$TRACE" ] || fail "detect-only bootstrap ran stale projection cleanup"
DJ_HOME="$INTEGRATION_ROOT/home" DJ_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/dj-bootstrap.sh" >/dev/null 2>&1
...
cat > "$INTEGRATION_ROOT/bin/dj-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'lock acquired'
SH
chmod +x "$INTEGRATION_ROOT/bin/dj-lock.sh"
DJ_HOME="$INTEGRATION_ROOT/home" DJ_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
  DJ_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/dj-session-start.sh" >/dev/null 2>&1 \
...
cat > "$INTEGRATION_ROOT/bin/dj-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'error: another live jarvis session holds the lock' >&2
exit 1
SH
chmod +x "$INTEGRATION_ROOT/bin/dj-lock.sh"
DJ_HOME="$INTEGRATION_ROOT/home" DJ_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
...
printf 'all dj-herdr-session-cleanup tests passed\n'
