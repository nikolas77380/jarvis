...
# bin/dj-herdr-lab.sh against a named non-default lab session. Only the
...
if [ "${DJ_HERDR_VERSION_FLOOR_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_HERDR_VERSION_FLOOR_LIVE_E2E=1 to run the real-release Herdr version-floor guard"
  exit 0
fi
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-herdr-version-floor.XXXXXX")
ORIGINAL_PATH=$PATH
LAB_SESSION=$("$LAB_HELPER" name dj-herdr-version-floor)
cleanup() {
  local status=$?
  rm -rf "$TMP_ROOT"
  exit "$status"
