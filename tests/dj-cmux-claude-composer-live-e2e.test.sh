...
# Run explicitly with DJ_CMUX_CLAUDE_COMPOSER_LIVE=1; it creates and cleans up
# only one exact dj-test- workspace through the normal scout lifecycle.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TASK="dj-test-cmux-claude-composer-$$"
LAB=
SPAWNED=0
...
cleanup() {
  [ "$SPAWNED" -eq 0 ] || {
    ...
    DJ_HOME="$LAB" "$ROOT/bin/dj-decision-hold.sh" complete "$TASK" --none >/dev/null 2>&1 || true
    DJ_HOME="$LAB" "$ROOT/bin/dj-teardown.sh" "$TASK" >/dev/null 2>&1 || true
  }
  [ -z "$LAB" ] || rm -rf -- "$LAB"
}

if [ "${DJ_CMUX_CLAUDE_COMPOSER_LIVE:-0}" != 1 ]; then
  echo "skip: set DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 to run the real cmux Claude composer drift guard"
  exit 0
fi

command -v claude >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but Claude Code is not installed"
command -v cmux >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but cmux is not installed"
command -v jq >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but jq is not installed"
command -v treehouse >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but treehouse is not installed"
command -v python3 >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but python3 is not installed"
cmux ping >/dev/null 2>&1 || fail "DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 but the cmux socket is unavailable"

LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-cmux-claude-composer.XXXXXX") || fail "could not create an isolated cmux Claude lab"
trap cleanup EXIT
...
DJ_HOME="$LAB" "$ROOT/bin/dj-brief.sh" "$TASK" comms --scout || fail "could not scaffold the Claude probe brief"
...
Then append exactly `needs-decision [key=probe-decision]: awaiting codeword` to that file and stop to wait for a jarvis message.
When you receive a jarvis message containing `ALBATROSS`, append `done: received ALBATROSS` to that status file and stop.
Do not change project files or make a commit.'''))
PY

DJ_HOME="$LAB" "$ROOT/bin/dj-spawn.sh" "$TASK" "$LAB/projects/comms" --scout --harness claude --model haiku --backend cmux \
...
dj_backend_source cmux || fail "could not source the cmux adapter"
TARGET=$(awk -F= '/^window=/{print $2}' "$LAB/state/$TASK.meta")
[ -n "$TARGET" ] || fail "the cmux probe did not record its endpoint"

for _ in $(seq 1 45); do
  CAPTURE=$(dj_backend_cmux_capture "$TARGET" 200 "$TASK" 2>/dev/null || true)
  case "$CAPTURE" in
    *'Yes, I trust this folder'*) DJ_HOME="$LAB" "$ROOT/bin/dj-send.sh" "$TASK" --key Enter || fail "could not accept Claude's folder-trust prompt" ;;
  esac
...
COMPOSER=$(dj_backend_cmux_composer_state "$TARGET" "$TASK")
...
DJ_SEND_SETTLE=0 DJ_HOME="$LAB" "$ROOT/bin/dj-send.sh" "$TASK" --resolve-key probe-decision ALBATROSS \
...
CAPTURE=$(dj_backend_cmux_capture "$TARGET" 200 "$TASK")
