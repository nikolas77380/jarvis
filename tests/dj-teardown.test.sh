#!/usr/bin/env bash
# Tests for bin/dj-teardown.sh's landed-work safety and stale-lock recovery.
#
...
#   (o) dj-pr-check rerun after HEAD moved                      -> no stale pr_head
#   (p) dj-pr-check when local HEAD lags                        -> record remote PR head
#   (q) no-mistakes + NO pr= recorded, PR discovered by branch  -> ALLOW  (yolo/no-CI merge)
#
# Also covers backlog teardown-lock-race: a git index.lock left in the worktree by a
# killed crew process (bin/dj-teardown.sh's teardown_treehouse_return).
...
TMP_ROOT=$(dj_test_tmproot dj-teardown-tests)
...
#   $CASE/state/        - jarvis state dir (with a fresh watcher beacon)
...
#   $CASE/project/      - clone of origin; acts as the jarvis project dir
...
exit 0
SH
  # Default hermetic no-mistakes stub: `axi status` answers DJ_FAKE_AXI_STATUS
  ...
  # DJ_FAKE_NM_ABORT_LOG when set. This keeps every case hermetic - without it,
  ...
  # path override DJ_FAKE_AXI_STATUS/DJ_FAKE_NM_ABORT_LOG before run_teardown.
  ...
  axi)
    ...
    case "${1:-}" in
      status)
        ...
        if [ -n "${DJ_FAKE_NM_ABORT_LOG:-}" ] \
           && grep -Fxq "abort --run $run_id" "$DJ_FAKE_NM_ABORT_LOG" 2>/dev/null \
           && [ "${DJ_FAKE_NM_ABORT_NOOP:-0}" != 1 ]; then
          if [ "${DJ_FAKE_NM_NOT_FOUND_AFTER_ABORT:-0}" = 1 ]; then
            printf 'error: "run \\"%s\\" not found"\n' "$run_id" >&2
            exit 1
          elif [ "${DJ_FAKE_NM_EMPTY_AFTER_ABORT:-0}" = 1 ]; then
            exit 0
          elif [ -n "${DJ_FAKE_AXI_STATUS_AFTER_ABORT:-}" ]; then
            printf '%s\n' "$DJ_FAKE_AXI_STATUS_AFTER_ABORT"
          else
            printf 'run:\n  id: "%s"\n  outcome: cancelled\n' "$run_id"
          fi
        else
      ...
      abort)
        shift
        [ -z "${DJ_FAKE_NM_ABORT_LOG:-}" ] || printf 'abort %s\n' "$*" >> "$DJ_FAKE_NM_ABORT_LOG"
        exit 0 ;;
    esac
    ;;
esac
...
SH
  ...
  git -C "$case_dir/project" worktree add -q -b dj/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so dj-guard stays quiet.
...
write_meta() {
  ...
  dj_write_meta "$case_dir/state/task-x1.meta" \
    "window=jarvis:dj-task-x1" \
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/dj-task-x1 is visible from the worktree (shared object db).
  git -C "$case_dir/wt" push -q fork dj/task-x1
...
# dj-teardown.sh's own retry-then-stale-cleanup logic (teardown_treehouse_return
# in bin/dj-teardown.sh) rather than hand-simulating that logic in the test.
...
run_teardown() {
  ...
  DJ_STATE_OVERRIDE="$case_dir/state" \
  DJ_CONFIG_OVERRIDE="$case_dir/config" \
  PATH="$case_dir/fakebin:${DJ_TEARDOWN_TEST_PATH:-$PATH}" \
    "$TEARDOWN" task-x1 "$@"
}
...
test_no_mistakes_origin_remote_allows() {
  ...
  git -C "$case_dir/wt" push -q origin dj/task-x1
...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  # recorded in meta at all (dj-pr-check.sh was never run, e.g. a yolo merge on
  ...
  # never pushing dj/task-x1 at all, so no refs/remotes/origin/dj/task-x1
...
test_squash_merged_pr_allows_replayed_unpushed_patch() {
  ...
  git -C "$case_dir/wt" push -q origin "$parent_head:refs/heads/dj/task-x1"
  git -C "$case_dir/project" fetch -q origin dj/task-x1
  wt_commit_file "$case_dir" feature.txt hello "add feature"
  append_pr_meta_url "$case_dir"
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "dj-pr-check does not refresh PR head after HEAD moves"
}
...
test_pr_check_records_remote_head_when_local_lags() {
  ...
  pass "dj-pr-check records the remote PR head when the local worktree lags"
}
...
test_non_linked_index_lock_path_is_checked_from_worktree() {
  ...
  git -C "$case_dir/wt" checkout -q -b dj/task-x1
  write_meta "$case_dir" no-mistakes ship
...
test_transient_index_lock_clears_after_first_attempt_and_retry_succeeds() {
  ...
  DJ_TREEHOUSE_RETURN_LOCK_RETRIES=2 \
  DJ_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=0 \
  DJ_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_empty_retry_wait_uses_default_without_aborting() {
  ...
  set +e
  TREEHOUSE_ATTEMPT_FILE="$attempt_file" \
  DJ_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  DJ_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  DJ_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS='' \
  DJ_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_fractional_legacy_retry_wait_refuses_without_arithmetic_error() {
  ...
  DJ_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  DJ_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  DJ_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0.1 \
  DJ_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_teardown_missing_busy_sidecar_completes() {
  ...
  write_meta "$case_dir" local-only ship
  gen=$("$ROOT/bin/dj-busy-event.sh" arm "$case_dir/state" task-x1)
...
case "\${1:-} \${2:-}" in
  ...
  "status --json")
    ...
    ;;
  "session list")
    if [ "\${DJ_FAKE_HERDR_SESSION_LIST_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
    else
      printf '%s\n' '{"sessions":[{"name":"default","running":true,"socket_path":"$case_dir/herdr.sock"}]}'
    fi
    ;;
  "pane close")
    : > "\${DJ_FAKE_HERDR_CLOSED:?}"
    ;;
  "pane get")
    if [ "\${DJ_FAKE_HERDR_PANE_GET_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
      exit 0
    fi
    if [ -e "\${DJ_FAKE_HERDR_CLOSED:?}" ]; then
...
SH
  ...
  lock=$(DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" PATH="$case_dir/fakebin:$PATH" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path default' "$ROOT") \
  ...
  rc=0
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" \
  ...
  if [ "$(git -C "$case_dir/wt" rev-parse --abbrev-ref HEAD 2>/dev/null)" != "dj/task-x1" ]; then
  ...
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
...
test_herdr_flat_teardown_refuses_records_on_unparseable_presence() {
  ...
  : > "$case_dir/state/task-x1.status"
  rc=0
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" DJ_FAKE_HERDR_PANE_GET_GARBAGE=1 \
    DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
...
SH
  ...
  case "$mode" in
    missing-adapter|missing-parser|missing-explicit-close-helper)
      ...
      if [ "$mode" = missing-adapter ]; then
        ...
        sed -i.bak 's/^dj_backend_herdr_explicit_close_pane_confirmed()/dj_backend_herdr_explicit_close_pane_confirmed_unavailable()/' \
          "$case_dir/test-root/bin/backends/herdr.sh"
        rm -f "$case_dir/test-root/bin/backends/herdr.sh.bak"
      else
        sed -i.bak 's/^dj_backend_herdr_parse_target()/dj_backend_herdr_parse_target_unavailable()/' \
          "$case_dir/test-root/bin/backends/herdr.sh"
        rm -f "$case_dir/test-root/bin/backends/herdr.sh.bak"
      fi
      teardown_bin="$case_dir/test-root/bin/dj-teardown.sh"
      ;;
  esac
  rc=0
  DJ_ROOT_OVERRIDE="$ROOT" DJ_STATE_OVERRIDE="$case_dir/state" DJ_CONFIG_OVERRIDE="$case_dir/config" \
    DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" \
    DJ_FAKE_HERDR_SESSION_LIST_GARBAGE="$([ "$mode" = unresolvable-lock ] && printf 1 || printf 0)" \
    PATH="$case_dir/fakebin:$PATH" \
  ...
  [ "$(git -C "$case_dir/wt" rev-parse --abbrev-ref HEAD 2>/dev/null)" = "dj/task-x1" ] \
...
configure_secondmate_with_herdr_child() {  # <case-dir>
  ...
  printf '%s\n' task-x1 > "$home/.dj-secondmate-home"
  printf '%s\n' "home=$home" >> "$case_dir/state/task-x1.meta"
  dj_write_meta "$home/state/child-herdr.meta" \
...
case "\${1:-} \${2:-}" in
  ...
  "pane get")
    if [ -e "\${DJ_FAKE_HERDR_CLOSED:?}" ]; then
      if [ "\${DJ_FAKE_HERDR_PRESENCE_UNKNOWN:-0}" = 1 ]; then
        printf '%s\n' 'not-json'
      else
        printf '%s\n' '{"error":{"code":"pane_not_found"}}' >&2
        exit 1
    ...
    ;;
  "pane close") : > "\${DJ_FAKE_HERDR_CLOSED:?}" ;;
esac
SH
  chmod +x "$case_dir/fakebin/herdr"
}
...
SH
  ...
  rc=0
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" \
    DJ_FAKE_HERDR_SESSION_LIST_GARBAGE=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
configure_secondmate_with_tmux_children() {  # <case-dir>
  ...
  for child in child-a child-b; do
    ...
    git -C "$case_dir/project" worktree add -q -b "dj/$child" "$child_wt" main
    dj_write_meta "$home/state/$child.meta" \
...
SH
  ...
  ROOT="$ROOT" LOCK="$lock" READY="$ready" RELEASE="$release" \
    ...
    export DJ_STATE_OVERRIDE="$HOME_STATE"
    . "$ROOT/bin/dj-wake-lib.sh"
    dj_lock_try_acquire "$LOCK" || exit 1
    : > "$READY"
...
test_forced_secondmate_herdr_child_retains_records_when_close_unconfirmed() {
  ...
  rc=0
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" DJ_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
configure_nested_secondmate_with_herdr_grandchild() {  # <case-dir>
  ...
  printf '%s\n' task-x1 > "$home/.dj-secondmate-home"
  printf '%s\n' nested-sm > "$nested_home/.dj-secondmate-home"
  printf '%s\n' "home=$home" >> "$case_dir/state/task-x1.meta"
  dj_write_meta "$home/state/nested-sm.meta" \
    "window=jarvis:dj-nested-sm" \
    ...
    "mode=local-only" \
    "home=$nested_home"
  dj_write_meta "$nested_home/state/grandchild-herdr.meta" \
...
case "${1:-} ${2:-}" in
  "workspace list")
    if [ -e "${DJ_FAKE_HERDR_RESTORED:?}" ]; then
    ...
    elif [ -e "${DJ_FAKE_HERDR_CLOSED:?}" ]; then
      ...
      printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","active_tab_id":"w1:t2","label":"jarvis/task-x1 · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w2","active_tab_id":"w2:t2","label":"2ndmate-bravo","focused":true},{"workspace_id":"w3","active_tab_id":"w3:t1","label":"2ndmate-alpha","focused":false}]}}'
    fi
    ;;
  "tab list")
    case "$*" in
  ...
  "session list")
    ...
    ;;
  "pane close")
    if [ "${DJ_FAKE_HERDR_CLOSE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${DJ_FAKE_HERDR_CLOSED:?}"
    ;;
  ...
  "tab get")
    ...
    ;;
  "tab focus")
    if [ "${DJ_FAKE_HERDR_RESTORE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${DJ_FAKE_HERDR_RESTORED:?}"
...
test_herdr_projection_teardown_retires_journal_only_after_confirmed_close() {
  ...
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" DJ_FAKE_HERDR_RESTORED="$restored" \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
test_herdr_projection_teardown_retains_journal_when_close_unconfirmed() {
  ...
  local rc=0
  DJ_FAKE_HERDR_LOG="$log" DJ_FAKE_HERDR_CLOSED="$closed" DJ_FAKE_HERDR_RESTORED="$restored" DJ_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
...
# path (bin/dj-teardown.sh), never by matching its source text. ------------

# A parked-at-a-gate `axi status` TOON payload for <branch>/<head>, matching
# the shape no-mistakes actually emits (see tests/dj-crew-state.test.sh's
# run_parked fixture, the same shape bin/dj-crew-state.sh's own tests pin).
...
test_parked_own_run_is_aborted_before_teardown() {
  ...
  local rc=0
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/task-x1 "$head")" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_mismatched_run_after_abort_refuses_unconfirmed() {
  ...
  rc=0
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/task-x1 "$head" 01RUN)" \
  DJ_FAKE_AXI_STATUS_AFTER_ABORT="$(parked_axi_status_toon dj/task-x1 "$head" 02RUN)" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_empty_status_after_abort_refuses_unconfirmed() {
  ...
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/task-x1 "$head")" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
  DJ_FAKE_NM_EMPTY_AFTER_ABORT=1 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_not_found_status_after_abort_confirms_completion() {
  ...
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/task-x1 "$head")" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
  DJ_FAKE_NM_NOT_FOUND_AFTER_ABORT=1 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_another_branchs_parked_run_is_never_touched() {
  ...
  # teardown.
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/some-other-task deadbeef)" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_own_autonomous_run_is_left_alone() {
  ...
  rc=0
  DJ_FAKE_AXI_STATUS="$(running_axi_status_toon dj/task-x1 "$head")" \
  DJ_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  ...
  rc=0
  DJ_TEARDOWN_TEST_PATH="$path_without_lsof" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${DJ_FAKE_REUSED_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  count=0
  [ ! -f "$DJ_FAKE_PS_COUNT" ] || count=$(cat "$DJ_FAKE_PS_COUNT")
  count=$((count + 1))
  printf '%s\n' "$count" > "$DJ_FAKE_PS_COUNT"
...
SH
  ...
  rc=0
  DJ_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  DJ_FAKE_REUSED_PID="$pid" DJ_FAKE_PS_COUNT="$case_dir/ps-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_exec_changed_process_is_still_reaped() {
  ...
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${DJ_FAKE_EXEC_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  out=$("$REAL_PS_FOR_TEST" "$@") || exit $?
  [ -e "$DJ_FAKE_EXEC_MARKER" ] || : > "$DJ_FAKE_EXEC_MARKER"
  printf '%s\n' "$out"
  exit 0
fi
exec "$REAL_PS_FOR_TEST" "$@"
...
#!/usr/bin/env bash
count=0
[ ! -f "$DJ_FAKE_LSOF_COUNT" ] || count=$(cat "$DJ_FAKE_LSOF_COUNT")
count=$((count + 1))
printf '%s\n' "$count" > "$DJ_FAKE_LSOF_COUNT"
if [ "$count" -eq 2 ]; then
  i=0
  while [ "$i" -lt 100 ]; do
    [ ! -e "$DJ_FAKE_EXEC_DONE" ] || break
    sleep 0.01
    i=$((i + 1))
  done
fi
...
SH
  ...
  rc=0
  DJ_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  DJ_FAKE_EXEC_PID="$pid" DJ_FAKE_EXEC_MARKER="$marker" \
  DJ_FAKE_EXEC_DONE="$done_flag" DJ_FAKE_LSOF_COUNT="$case_dir/lsof-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${DJ_FAKE_PERSISTENT_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
...
SH
  ...
  rc=0
  DJ_PROC_ROOT_OVERRIDE="$case_dir/no-proc" DJ_FAKE_PERSISTENT_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${DJ_FAKE_EXITED_PID:-}" ]; then
  exit 1
fi
exec "$REAL_PS_FOR_TEST" "$@"
SH
...
EOF
  ...
  rc=0
  DJ_PROC_ROOT_OVERRIDE="$case_dir/no-proc" DJ_FAKE_EXITED_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  ...
  rc=0
  DJ_FAKE_AXI_STATUS="$(parked_axi_status_toon dj/task-x1 "$head")" \
  DJ_FAKE_NM_ABORT_LOG="$abort_log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
