...
RECON="$ROOT/bin/dj-inactive-reconcile.sh"
DRAIN="$ROOT/bin/dj-wake-drain.sh"
WATCH="$ROOT/bin/dj-watch.sh"
TMP_ROOT=$(dj_test_tmproot dj-inactive-reconcile)
...
make_tools() { # <world>
  ...
  fake="$world/fakebin"
  mkdir -p "$fake"
  cat > "$fake/dj-crew-state.sh" <<'SH'
#!/usr/bin/env bash
printf 'state: %s · source: fake\n' "${DJ_FAKE_CREW_STATE:-unknown}"
SH
  cat > "$fake/tmux" <<'SH'
#!/usr/bin/env bash
case "${1:-}" in
...
bind_secondmate() { # <local|remote>
  ...
  if [ "$route" = local ]; then
    cat > "$MATE/.dj-secondmate-parent" <<EOF
schema=dj-secondmate-parent.v1
route=local
parent_home=$MAIN
EOF
  else
...
write_child() { # <home> <id> <status> [spawn-gen]
  ...
  dj_write_meta "$home/state/$id.meta" \
    "window=jarvis:dj-$id" "worktree=$home/projects/$id" "project=alpha" \
...
write_mate_meta() {
  dj_write_secondmate_meta "$MAIN/state/mate.meta" "$MATE"
...
run_reconcile() { # <home> [--startup]
  ...
  PATH="$WORLD/fakebin:$PATH" DJ_ROOT_OVERRIDE="$WORLD/root" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_INACTIVE_RECONCILE_SECS=60 DJ_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/dj-crew-state.sh" \
    DJ_FORGE_LOG="$WORLD/forge.log" "$RECON" scan ${option:+"$option"}
}
...
test_main_direct_terminal_presentation_receipt() {
  ...
  DJ_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup
  ...
  err="$WORLD/drain.err"
  DJ_HOME="$MAIN" DJ_STATE_OVERRIDE="$MAIN/state" "$DRAIN" >/dev/null 2> "$err"
  ...
  DJ_HOME="$MAIN" DJ_STATE_OVERRIDE="$MAIN/state" "$DRAIN" --ack-through "$seq" --recovery-generation "$generation"
...
test_local_secondmate_reports_terminal_child() {
  ...
  DJ_FAKE_CREW_STATE='done' run_reconcile "$MATE" --startup
...
test_local_secondmate_rejects_relative_parent_home() {
  ...
  printf 'schema=dj-secondmate-parent.v1\nroute=local\nparent_home=relative-parent\n' \
    > "$MATE/.dj-secondmate-parent"
  write_child "$MATE" child 'failed: terminal'
  (cd "$WORLD" && DJ_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup)
...
test_invalid_secondmate_marker_blocks_routing() {
  ...
  for kind in malformed symlink; do
    ...
    if [ "$kind" = malformed ]; then
      ...
      printf 'mate\n' > "$target"
      ln -s "$target" "$MATE/.dj-secondmate-home"
    fi

    out=$(DJ_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup)
    printf '%s\n' "$out" | grep -Fq 'inactive terminal outcomes remain unreconciled: invalid .dj-secondmate-home marker' \
...
test_reused_task_id_reports_each_incarnation() {
  ...
  DJ_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup
...
test_relaunch_cannot_replace_metadata_during_state_snapshot() {
  ...
  write_child "$MATE" child 'failed: terminal' spawn-old
  cat > "$WORLD/fakebin/dj-crew-state.sh" <<'SH'
#!/usr/bin/env bash
: > "${DJ_RACE_WORLD:?}/state-started"
while [ ! -e "$DJ_RACE_WORLD/state-release" ]; do sleep 0.05; done
printf 'state: failed · source: fake\n'
SH
  chmod +x "$WORLD/fakebin/dj-crew-state.sh"

  DJ_RACE_WORLD="$WORLD" run_reconcile "$MATE" --startup &
  recon_pid=$!
  i=0
  ...
  DJ_HOME="$MATE" DJ_STATE_OVERRIDE="$MATE/state" bash -c '
    . "$1/bin/dj-wake-lib.sh"
    meta="$DJ_STATE_OVERRIDE/child.meta"
    lock=$(dj_meta_lock_path "$meta")
    dj_lock_acquire_wait "$lock"
    awk '\''{ sub(/^spawn_gen=.*/, "spawn_gen=spawn-new"); print }'\'' "$meta" > "$meta.tmp"
    mv "$meta.tmp" "$meta"
    printf "working: replacement active\n" > "$DJ_STATE_OVERRIDE/child.status"
    : > "$2/meta-updated"
...
test_nonterminal_and_captain_held_states_do_not_report() {
  ...
  for state in working paused parked unknown; do
    ...
    DJ_FAKE_CREW_STATE="$state" run_reconcile "$MAIN" --startup
...
test_watcher_hook_and_idle_secondmate_exemption() {
  ...
  out="$WORLD/watch.out"
  PATH="$WORLD/fakebin:$PATH" DJ_HOME="$MAIN" DJ_STATE_OVERRIDE="$MAIN/state" \
    DJ_INACTIVE_RECONCILE_SECS=60 DJ_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/dj-crew-state.sh" \
    DJ_FORGE_LOG="$WORLD/forge.log" DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    DJ_FAKE_CREW_STATE='done' "$WATCH" > "$out" 2>&1 &
  pid=$!
  i=0
  while [ "$i" -lt 40 ]; do
  ...
  PATH="$WORLD/fakebin:$PATH" DJ_HOME="$MAIN" DJ_STATE_OVERRIDE="$MAIN/state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$WORLD/idle.out" 2>&1 &
...
SH
  ...
  started=$(date +%s)
  DJ_INACTIVE_RECONCILE_BUDGET_SECS=1 run_reconcile "$MAIN" --startup
...
test_full_scan_budget_includes_wake_lock_wait() {
  ...
  DJ_HOME="$MAIN" DJ_STATE_OVERRIDE="$MAIN/state" bash -c '
    . "$1/bin/dj-wake-lib.sh"
  ...
  started=$(date +%s)
  DJ_INACTIVE_RECONCILE_BUDGET_SECS=1 DJ_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup
  elapsed=$(( $(date +%s) - started ))
  reap "$holder"
...
test_notice_recovery_does_not_duplicate_wake() {
  ...
  err="$WORLD/drain.err"
  DJ_HOME="$MATE" DJ_STATE_OVERRIDE="$MATE/state" "$DRAIN" >/dev/null 2> "$err"
  seq=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation .*/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  DJ_HOME="$MATE" DJ_STATE_OVERRIDE="$MATE/state" "$DRAIN" --ack-through "$seq" --recovery-generation "$generation"
  DJ_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup
