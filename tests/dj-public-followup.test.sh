...
#
# The failure this suite pins: jarvis promises a public final reply in an X or
...
PF="$ROOT/bin/dj-public-followup.sh"
EMIT="$ROOT/bin/dj-public-followup-emit.sh"
POLL="$ROOT/bin/dj-x-poll.sh"
TEARDOWN="$ROOT/bin/dj-teardown.sh"
PROMOTE="$ROOT/bin/dj-promote.sh"
SESSION_START="$ROOT/bin/dj-session-start.sh"
TMP_ROOT=$(dj_test_tmproot dj-public-followup)
...
# make_home <name> [relay-on|relay-off]: a jarvis home with its own backlog.
# relay-off omits .env entirely, which is exactly what a home that never opted
# into the myjarvis relay looks like.
...
## Done
EOF
  [ "$relay" = relay-off ] || printf 'DJX_PAIRING_TOKEN=test-token\n' > "$home/.env"
  make_fake_curl "$home" >/dev/null
  dj_fake_exit0 "$home/fakebin" tmux treehouse no-mistakes gh gh-axi
  printf '%s\n' "$home"
}
...
run_pf() {  # <home> <args...>
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" FAKE_CURL_LOG="${FAKE_CURL_LOG:-}" \
...
seed_commitment() {
  ...
  jq -n '{type:"pr-merged", project:"jarvis",
  ...
  DJ_HOME="$home" bash -c \
    ". '$ROOT/bin/dj-x-lib.sh'; fmx_context_registry_set '$home/state' '$request' '$platform' 1900" \
...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n '{type:"report-ready", project:"jarvis",
  ...
  DJ_HOME="$home" bash -c \
    ". '$ROOT/bin/dj-x-lib.sh'; fmx_context_registry_set '$home/state' '$request' discord 2000" \
...
test_restart_e2e_delivers_exactly_once() {
  ...
  printf '%s\n' fmdev > "$child/.dj-secondmate-home"
  dj_write_meta "$home/state/fmdev.meta" "kind=secondmate" "home=$child"
  seed_commitment "$home" pf-restart req-restart discord secondmate:fmdev work-code-q1
  dj_write_meta "$child/state/work-code-q1.meta" \
...
test_dry_run_does_not_close_commitment() {
  ...
  DJX_DRY_RUN=1 FAKE_CURL_LOG="$log" expect_failure \
...
test_late_receipt_closes_the_exact_attempt_without_reposting() {
  ...
  dj_write_meta "$home/state/work-late.meta" \
...
test_typed_terminal_clear_only_removes_legacy_link() {
  ...
  out=$(PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" "$ROOT/bin/dj-x-followup.sh" --clear work-clear) \
...
test_outward_delivery_stays_with_the_owning_home() {
  ...
  printf '%s\n' child > "$child/.dj-secondmate-home"
  dj_write_meta "$owner/state/child.meta" "kind=secondmate" "home=$child"
  seed_commitment "$owner" pf-own req-own discord secondmate:child work-child
  dj_write_meta "$child/state/work-child.meta" \
  ...
  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" FAKE_CURL_LOG="$log" \
    expect_failure "a home without relay consent must not deliver a public reply" \
    "$PF" deliver pf-own
  assert_contains "$EXPECT_OUT" "has not opted into the myjarvis relay" \
...
test_delivery_requires_registration_before_posting() {
  ...
  dj_write_meta "$home/state/work-missing.meta" \
...
test_secondmate_teardown_requires_parent_binding() {
  ...
  printf '%s\n' mate > "$child/.dj-secondmate-home"
  seed_commitment "$parent" pf-teardown req-teardown x secondmate:mate work-child
  dj_write_meta "$parent/state/mate.meta" "kind=secondmate" "home=$child"
  dj_write_meta "$child/state/work-child.meta" \
    "window=jarvis:dj-work-child" "endpoint_task_id=work-child" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only"

  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
  ...
  DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" validate >/dev/null \
...
# The three tests below exercise the durable .dj-secondmate-parent record a real
# bin/dj-home-seed.sh seed now writes next to .dj-secondmate-home (dj-remote-sm-
...
# DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME env var: a restart that dropped that prefix made
...
# the real bin/dj-teardown.sh cleanup path against a home real dj-home-seed.sh
# produced, never a hand-crafted marker.

assert_local_secondmate_parent_record() {
  local child=$1 parent=$2
  cmp -s "$child/.dj-secondmate-parent" <(
    printf 'schema=dj-secondmate-parent.v1\nroute=local\nparent_home=%s\n' "$parent"
...
test_local_secondmate_seed_publishes_parent_before_identity() {
  ...
  fakebin=$(dj_fakebin "$TMP_ROOT/seed-publication-fake")
...
case "$destination" in
  */.dj-secondmate-home)
    touch "$DJ_TEST_PUBLISH_ENTERED"
    wait_count=0
    while [ ! -f "$DJ_TEST_PUBLISH_RELEASE" ]; do
...
SH
  chmod +x "$fakebin/mv"
  PATH="$fakebin:$PATH" DJ_HOME="$parent" \
    DJ_SECONDMATE_CHARTER='Local publication-order regression charter.' \
    DJ_TEST_REAL_MV="$real_mv" DJ_TEST_PUBLISH_ENTERED="$entered" \
    DJ_TEST_PUBLISH_RELEASE="$release" \
    "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects > "$manifest_out" 2>&1 &
  seed_pid=$!
  wait_count=0
  while [ ! -f "$entered" ]; do
  ...
  assert_absent "$child/.dj-secondmate-home" \
  ...
  assert_present "$child/.dj-secondmate-home" \
...
test_secondmate_teardown_resolves_parent_from_durable_record_when_env_lost() {
  ...
  DJ_SECONDMATE_CHARTER='Durable-record regression charter.' \
    DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  make_fake_curl "$child" >/dev/null
  dj_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  ...
  # No DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME at all here: a restart of the secondmate
...
test_secondmate_teardown_durable_record_missing_parent_registration_still_refuses() {
  ...
  DJ_SECONDMATE_CHARTER='Durable-record missing-registration regression charter.' \
    DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
...
test_secondmate_teardown_durable_record_with_unknown_field_succeeds() {
  ...
  DJ_SECONDMATE_CHARTER='Durable-record clean-cleanup regression charter.' \
    DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  ...
  dj_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  assert_local_secondmate_parent_record "$child" "$parent_resolved"
  printf 'some_future_field=value\n' >> "$child/.dj-secondmate-parent"
  parent_alias="$TMP_ROOT/teardown-durable-clean-parent-alias"
  ln -s "$parent" "$parent_alias"
  dj_write_meta "$parent/state/mate.meta" "kind=secondmate" "home=$child"
  dj_git_init_commit "$child/projects/worktree"
  printf 'manual\n' > "$child/config/backlog-backend"
  dj_write_meta "$child/state/work-clean.meta" \
    "window=jarvis:dj-work-clean" "endpoint_task_id=work-clean" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=ship" "mode=local-only"

  rc=0
  out=$(PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
    DJ_CONFIG_OVERRIDE="$child/config" DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent_alias" \
...
test_secondmate_teardown_rejects_conflicting_live_and_durable_parent_bindings() {
  ...
  DJ_SECONDMATE_CHARTER='Durable-record conflict regression charter.' \
    DJ_HOME="$durable_parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  ...
  dj_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  assert_local_secondmate_parent_record "$child" "$parent_resolved"
  dj_write_meta "$durable_parent/state/mate.meta" "kind=secondmate" "home=$child"
  dj_git_init_commit "$child/projects/worktree"
  printf 'manual\n' > "$child/config/backlog-backend"
  dj_write_meta "$child/state/work-conflict.meta" \
    "window=jarvis:dj-work-conflict" "endpoint_task_id=work-conflict" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=ship" "mode=local-only"

  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
    DJ_CONFIG_OVERRIDE="$child/config" DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$live_parent" \
...
test_secondmate_teardown_rejects_unsafe_durable_parent_records() {
  ...
  for case_name in symlink invalid-route duplicate-route remote-parent-home local-parent-host; do
    ...
    DJ_SECONDMATE_CHARTER='Unsafe durable-record regression charter.' \
      DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    ...
    dj_write_meta "$child/state/work-child.meta" \
      "window=jarvis:dj-work-child" "endpoint_task_id=work-child" \
      "worktree=$child" "project=$child" "kind=ship" "mode=local-only"
    parent_record="$child/.dj-secondmate-parent"
    case "$case_name" in
      symlink)
        mv "$parent_record" "$parent_record.valid"
        ln -s .dj-secondmate-parent.valid "$parent_record"
        ;;
      invalid-route)
        printf 'schema=dj-secondmate-parent.v1\nroute=garbage\n' > "$parent_record"
        ;;
...
test_secondmate_teardown_rejects_nul_bearing_durable_parent_record() {
  ...
  DJ_SECONDMATE_CHARTER='Durable-record NUL regression charter.' \
    DJ_HOME="$parent" "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  ...
  record="$child/.dj-secondmate-parent"
  {
    printf 'schema=dj-secondmate-parent.v1\nroute=local\n'
  ...
  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
    DJ_CONFIG_OVERRIDE="$child/config" \
...
test_relay_disabled_unmarked_teardown_skips_public_path() {
  ...
  dj_git_init_commit "$home/projects/worktree"
...
SH
  ...
  dj_write_meta "$home/state/work-disabled.meta" \
    "window=jarvis:dj-work-disabled" "endpoint_task_id=work-disabled" \
  ...
  out=$(PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    ...
    DJ_CONFIG_OVERRIDE="$home/config" FAKE_TASKS_AXI_LOG="$tasks_log" \
...
test_relay_disabled_parent_allows_marked_child_teardown() {
  ...
  dj_git_init_commit "$child/projects/worktree"
  printf '%s\n' disabled-mate > "$child/.dj-secondmate-home"
  printf -- '- disabled-mate - synthetic (home: %s; scope: synthetic; projects: ; added 2026-07-30)\n' \
    "$child" > "$parent/data/secondmates.md"
  dj_write_meta "$parent/state/disabled-mate.meta" "kind=secondmate" "home=$child"
...
SH
  ...
  dj_write_meta "$child/state/work-disabled.meta" \
    "window=jarvis:dj-work-disabled" "endpoint_task_id=work-disabled" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=ship" "mode=local-only"
  ...
  out=$(PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
    DJ_CONFIG_OVERRIDE="$child/config" \
    DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" FAKE_TASKS_AXI_LOG="$tasks_log" \
    "$TEARDOWN" work-disabled 2>&1) || rc=$?
...
test_secondmate_parent_binding_matches_literal_id() {
  ...
  printf '%s\n' 'mate.id' > "$child/.dj-secondmate-home"
  ...
  dj_write_meta "$parent/state/mate.id.meta" "kind=secondmate" "home=$child"
  dj_write_meta "$child/state/work-literal.meta" \
    "window=jarvis:dj-work-literal" "endpoint_task_id=work-literal" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only"

  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" \
    DJ_CONFIG_OVERRIDE="$child/config" DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" \
...
test_cleanup_refuses_while_a_public_reply_is_owed() {
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-task \
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-task >/dev/null 2>&1 || rc=$?
...
# The hard acceptance criterion. A home that never opted into the myjarvis
...
SH
  ...
  for cmd in "consume" "pending" "guard-work main any-task" "retire anything"; do
    ...
    out=$(PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
      DJ_STATE_OVERRIDE="$home/state" FAKE_TASKS_AXI_LOG="$tasks_log" "$PF" $cmd 2>&1) || rc=$?
  ...
  # shellcheck disable=SC1091
  . "$ROOT/bin/dj-public-followup-lib.sh"
  dj_pf_has_open_loops "$home/state" \
    && fail "a relay-disabled home must not grow an open-loop registry"
  dj_pf_has_delivered_open_loops "$home/state" \
    && fail "a relay-disabled home must not grow a delivered open-loop registry"

  rc=0
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" FAKE_TASKS_AXI_LOG="$tasks_log" "$PF" active || rc=$?
...
test_relay_poll_stays_inert_and_surfaces_once() {
  ...
  out=$(PATH="$off/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$off" \
    DJ_STATE_OVERRIDE="$off/state" "$POLL" 2>&1)
  ...
  out=$(PATH="$on/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$on" \
    DJ_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  ...
  first=$(PATH="$on/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$on" \
    DJ_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  ...
  second=$(PATH="$on/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$on" \
    DJ_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  assert_not_contains "$second" "public-followup" \
    "an unchanged pending set must not wake jarvis again every cycle"
...
test_session_start_surfaces_only_when_owed() {
  ...
  out=$(PATH="$off/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$off" \
    DJ_STATE_OVERRIDE="$off/state" DJ_DATA_OVERRIDE="$off/data" \
    DJ_CONFIG_OVERRIDE="$off/config" "$SESSION_START" 2>&1)
  ...
  out=$(PATH="$on/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$on" \
    DJ_STATE_OVERRIDE="$on/state" DJ_DATA_OVERRIDE="$on/data" \
    DJ_CONFIG_OVERRIDE="$on/config" "$SESSION_START" 2>&1)
...
test_dropped_baton_now_surfaces_open_loop() {
  ...
  DJ_SECONDMATE_CHARTER='Baton repro charter.' DJ_HOME="$parent" \
    "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null || fail "seed failed"
  child=$(cd "$child" && pwd -P)
  make_fake_curl "$child" >/dev/null
  dj_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  ...
  dj_write_meta "$child/state/pi-rearm-loop-fix-r1.meta" \
    "window=jarvis:dj-pi-rearm-loop-fix-r1" "endpoint_task_id=pi-rearm-loop-fix-r1" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only"

  PATH="$parent/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$parent" \
    DJ_STATE_OVERRIDE="$parent/state" "$PF" guard-work secondmate:mate pi-rearm-loop-fix-r1 \
  ...
  PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_DATA_OVERRIDE="$child/data" FAKE_CURL_LOG="$log" \
...
test_control_registered_followon_is_guarded() {
  ...
  DJ_SECONDMATE_CHARTER='Baton control charter.' DJ_HOME="$parent" \
    "$ROOT/bin/dj-home-seed.sh" mate "$child" --no-projects >/dev/null || fail "seed failed"
  child=$(cd "$child" && pwd -P)
  make_fake_curl "$child" >/dev/null
  dj_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
...
SH
  ...
  command=$(printf '%s\n' "$out" | awk '
    index($0, "/bin/dj-public-followup-emit.sh") { capture=1 }
  ...
  command=${command/"$ROOT/bin/dj-public-followup-emit.sh"/"$parent/fakebin/record-emit"}
...
test_rechain_claims_delivered_source_once() {
  ...
  DJX_NOW_OVERRIDE=1787539200 run_pf "$home" rechain public-final-claim-b \
    --from public-final-claim-a --work-home main --work-id ship-claim-b \
    --expected pr-merged > "$home/rechain-b.out" 2>&1 &
  pid_b=$!
  DJX_NOW_OVERRIDE=1787539200 run_pf "$home" rechain public-final-claim-c \
...
test_retire_after_secondmate_home_removal() {
  ...
  mkdir -p "$child/state"
  printf 'mate\n' > "$child/.dj-secondmate-home"
  dj_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$child"
  log="$home/curl.log"; : > "$log"
...
test_retire_refuses_unbound_existing_secondmate() {
  ...
  dj_write_meta "$child/state/scout-unbound.meta" "status=working" "x_request=req-unbound-mate"
...
test_retire_refuses_reassigned_secondmate_home() {
  ...
  printf 'mate\n' > "$original/.dj-secondmate-home"
  dj_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$original"
  log="$home/curl.log"; : > "$log"
  ...
  printf 'mate\n' > "$replacement/.dj-secondmate-home"
  dj_write_meta "$replacement/state/scout-reassigned.meta" \
    "status=working" "x_request=req-unrelated-replacement"
  dj_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$replacement"
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  jq -n '{type:"pr-merged", project:"jarvis", required_deliverables:["pr_url"],
...
test_pending_skips_concurrent_retirement() {
  ...
  DJ_RACE_HOME="$home" DJ_RACE_ROOT="$ROOT" bash -c '
    . "$DJ_RACE_ROOT/bin/dj-public-followup-lib.sh"
    dj_pf_registry_lock_acquire "$DJ_RACE_HOME/state" pf-race || exit 1
    : > "$DJ_RACE_HOME/lock-ready"
    while [ ! -e "$DJ_RACE_HOME/release-lock" ]; do sleep 0.02; done
    sleep 0.1
    mkdir -p "$DJ_RACE_HOME/state/public-followup/retired"
    printf "reason=concurrent close\nretired_at=2026-08-01T00:00:00Z\n" \
      > "$DJ_RACE_HOME/state/public-followup/retired/pf-race"
    chmod 600 "$DJ_RACE_HOME/state/public-followup/retired/pf-race"
    rm -f "$DJ_RACE_HOME/state/public-followup/registry/pf-race"
    dj_pf_registry_lock_release "$DJ_RACE_HOME/state" pf-race
  ' &
  locker_pid=$!
...
test_retention_creates_no_false_teardown_refusal() {
  ...
  dj_write_meta "$home/state/ship-retain.meta" \
    "window=jarvis:dj-ship-retain" \
    "worktree=$home/projects/gone" \
    "project=$home/projects/sample" \
    "harness=codex" \
    "kind=ship" \
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-retain \
...
test_expiry_escalation_uses_now_override() {
  ...
  now_expired=$((exp + 60))
  out=$(DJX_NOW_OVERRIDE="$now_expired" run_pf "$home" pending)
  ...
  out=$(DJX_NOW_OVERRIDE="$now_closing" run_pf "$home" pending)
  ...
  DJX_NOW_OVERRIDE="$now_expired" expect_failure "rechain past expiry must refuse" \
  ...
  pass "expiry escalation is pinned by DJX_NOW_OVERRIDE"
}
...
test_x_request_teardown_warns_when_final_unposted() {
  ...
  dj_write_meta "$home/state/linked-task.meta" \
    "window=jarvis:dj-linked-task" \
    "worktree=$home/projects/gone" \
    "project=$home/projects/sample" \
    "kind=ship" \
    "mode=local-only" \
  ...
  PATH="$home/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" linked-task \
    > "$home/td.out" 2> "$home/td.err" || rc=$?
...
test_secondmate_promotion_uses_teardown_parent_resolution() {
  ...
  printf '%s\n' mate > "$child/.dj-secondmate-home"
  printf 'schema=dj-secondmate-parent.v1\nroute=local\nparent_home=%s\n' \
    "$stale" > "$child/.dj-secondmate-parent"
  ...
  dj_write_meta "$child/state/promote-conflict.meta" \
    "window=jarvis:dj-promote-conflict" "kind=scout"
  out=$(PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" \
  ...
  rm -f "$child/.dj-secondmate-parent"
  dj_write_meta "$child/state/promote-legacy.meta" \
    "window=jarvis:dj-promote-legacy" "kind=scout"
  out=$(PATH="$child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$child" \
    DJ_STATE_OVERRIDE="$child/state" DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" \
    "$PROMOTE" promote-legacy --mode local-only --yolo off 2>&1) \
    || fail "legacy parent recovery must not block promotion: $out"
  assert_contains "$out" "next: DJ_HOME=" \
  ...
  printf '%s\n' remote-mate > "$remote_child/.dj-secondmate-home"
  printf 'schema=dj-secondmate-parent.v1\nroute=remote\nparent_host=remote.example\n' \
    > "$remote_child/.dj-secondmate-parent"
  printf 'DJX_PAIRING_TOKEN=child-local-token\n' > "$remote_child/.env"
  dj_write_meta "$remote_child/state/promote-remote.meta" \
    "window=jarvis:dj-promote-remote" "kind=scout"
  out=$(PATH="$remote_child/fakebin:$PATH" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$remote_child" \
    DJ_STATE_OVERRIDE="$remote_child/state" \
