...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-herdr-session-cleanup-e2e.XXXXXX")
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name dj-herdr-session-start-stale-projection-cleanup-r1)
...
production_process_proof() {
  DJ_HOME="$HOME_DIR" DJ_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
    DJ_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1 PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" \
    bash -c '. "$1"; dj_backend_herdr_pane_idle_shell_pid "$2" "$3" >/dev/null' \
      _ "$ROOT/bin/dj-herdr-session-cleanup.sh" "$HERDR_LAB_SESSION" "$PANE"
}
focus_snapshot() {
...
DJ_HOME="$HOME_DIR" DJ_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" "$ROOT/bin/dj-herdr-session-cleanup.sh" \
