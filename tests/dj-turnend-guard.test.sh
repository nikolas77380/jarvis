...
#
# Two layers:
#   PREDICATE  - bin/dj-supervision-lib.sh, the shared beacon/status computation
#                used by dj-guard.sh and by the hook's banner details.
#   HOOK       - bin/dj-turnend-guard.sh, the shared primary hook predicate that
...
. "$ROOT/bin/dj-supervision-lib.sh"

TMP_ROOT=$(dj_test_tmproot dj-turnend-guard)
...
# --- PREDICATE: bin/dj-supervision-lib.sh -----------------------------------

test_predicate_healthy_no_inflight() {
  local state="$TMP_ROOT/pred-empty/state"
  mkdir -p "$state"
  if dj_supervision_unhealthy "$state" 300; then
    fail "predicate reported unhealthy with zero in-flight tasks"
  fi
  [ "$DJ_SUP_IN_FLIGHT" -eq 0 ] || fail "expected zero in-flight, got $DJ_SUP_IN_FLIGHT"
  pass "dj_supervision_unhealthy: false with no state/*.meta at all"
}
...
test_predicate_unhealthy_no_beacon() {
  ...
  mkdir -p "$state"
  : > "$state/task1.meta"
  dj_supervision_unhealthy "$state" 300 || fail "predicate did not fire: in-flight task, beacon never seen"
  [ "$DJ_SUP_IN_FLIGHT" -eq 1 ] || fail "expected 1 in-flight, got $DJ_SUP_IN_FLIGHT"
  [ "$DJ_SUP_WATCHER_FRESH" = false ] || fail "beacon absent must not read as fresh"
  [ "$DJ_SUP_BEACON_DESC" = never ] || fail "beacon description should be 'never', got $DJ_SUP_BEACON_DESC"
  pass "dj_supervision_unhealthy: true with in-flight task and no beacon ever"
}
...
test_predicate_unhealthy_stale_beacon() {
  ...
  dj_supervision_unhealthy "$state" 300 || fail "predicate did not fire: in-flight task, beacon far outside grace"
  [ "$DJ_SUP_WATCHER_FRESH" = false ] || fail "an ancient beacon must not read as fresh"
  pass "dj_supervision_unhealthy: true with in-flight task and a beacon far outside the grace window"
}
...
test_predicate_healthy_fresh_beacon() {
  ...
  [ "$DJ_SUP_WATCHER_FRESH" = true ] || fail "a beacon touched just now must read as fresh"
  pass "dj_supervision_unhealthy: false with in-flight task and a fresh beacon"
}

test_predicate_queue_pending_flag() {
  local state="$TMP_ROOT/pred-queue/state"
  mkdir -p "$state"
  dj_supervision_status "$state" 300
  [ "$DJ_SUP_QUEUE_PENDING" = false ] || fail "empty/absent wake queue must not read as pending"
  printf 'record\n' > "$state/.wake-queue"
  dj_supervision_status "$state" 300
  [ "$DJ_SUP_QUEUE_PENDING" = true ] || fail "a non-empty wake queue must read as pending"
  pass "dj_supervision_status: DJ_SUP_QUEUE_PENDING tracks state/.wake-queue"
}
...
test_predicate_x_mode_needs_supervision() {
  ...
  dj_supervision_needed "$state" 300 || fail "X-mode relay poll did not register as supervision need"
  [ "$DJ_SUP_IN_FLIGHT" -eq 0 ] || fail "X-mode relay poll must not count as an in-flight task"
  [ "$DJ_SUP_NEEDED" = true ] || fail "X-mode relay poll must set DJ_SUP_NEEDED"
  dj_supervision_unhealthy "$state" 300 || fail "X-mode relay poll with no beacon must be unhealthy"
  pass "dj_supervision_needed: X-mode relay poll needs supervision"
}
...
test_predicate_source_needs_supervision() {
  ...
  dj_supervision_unhealthy "$state" 300 || fail "registered source with no beacon must be unhealthy"
  [ "$DJ_SUP_IN_FLIGHT" -eq 0 ] || fail "a process-event source must not count as a task"
  [ "$DJ_SUP_SOURCES" -eq 1 ] || fail "expected one registered process-event source"
  pass "dj_supervision_unhealthy: source-only home needs supervision"
}

# --- HOOK: bin/dj-turnend-guard.sh ------------------------------------------
#
# Each scenario gets its own directory carrying a copy of the two guard scripts
# under bin/, so the hook (invoked by absolute path) resolves its own DJ_ROOT to
# that scenario dir regardless of the test's cwd.

install_guard_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/dj-turnend-guard.sh" "$dir/bin/dj-turnend-guard.sh"
  cp "$ROOT/bin/dj-turnend-guard-grok.sh" "$dir/bin/dj-turnend-guard-grok.sh"
  cp "$ROOT/bin/dj-operational-input.sh" "$dir/bin/dj-operational-input.sh"
  cp "$ROOT/bin/dj-supervision-instructions.sh" "$dir/bin/dj-supervision-instructions.sh"
  cp "$ROOT/bin/dj-harness.sh" "$dir/bin/dj-harness.sh"
  cp "$ROOT/bin/dj-primary-scope-lib.sh" "$dir/bin/dj-primary-scope-lib.sh"
  cp "$ROOT/bin/dj-supervision-lib.sh" "$dir/bin/dj-supervision-lib.sh"
  cp "$ROOT/bin/dj-wake-lib.sh" "$dir/bin/dj-wake-lib.sh"
  cp "$ROOT/bin/dj-hook-host-lib.sh" "$dir/bin/dj-hook-host-lib.sh"
  mkdir -p "$dir/docs"
  cp -R "$ROOT/docs/supervision-protocols" "$dir/docs/supervision-protocols"
  chmod +x "$dir/bin/dj-turnend-guard.sh" "$dir/bin/dj-turnend-guard-grok.sh" "$dir/bin/dj-operational-input.sh" "$dir/bin/dj-supervision-instructions.sh" "$dir/bin/dj-harness.sh"
}

mark_codex_hook_root() {
  local dir=$1
  mkdir -p "$dir/.codex"
  printf '{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"dj-turnend-guard.sh"}]}]}}\n' > "$dir/.codex/hooks.json"
}
...
# Same shape as primary, plus the .dj-secondmate-home marker bin/dj-home-seed.sh
...
# A genuine linked `git worktree` of a base repo - the shape bin/dj-spawn.sh
# always hands crewmate/scout tasks working on jarvis itself. git-dir and
# git-common-dir differ here, unlike a plain checkout.
make_crewmate_worktree_dir() {
  local base=$1 dir=$2
  dj_git_worktree "$base" "$dir" dj/turnend-guard-test-branch
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf '%s\n' "$dir"
...
# .dj-secondmate-home marker, so the marker force-include never fires for it and
# it stays exempt through the linked-worktree git-dir test.
make_secondmate_child_worktree_dir() {
  local home=$1 dir=$2
  git -C "$home" worktree add --quiet -b dj/turnend-secondmate-child "$dir"
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf '%s\n' "$dir"
...
# valid .dj-secondmate-home marker. This is the production topology the plain
...
make_secondmate_linked_home_dir() {
  local base=$1 dir=$2
  dj_git_worktree "$base" "$dir" dj/turnend-secondmate-linked-home
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf 'sm-linked-1\n' > "$dir/.dj-secondmate-home"
...
run_hook() {
  ...
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s}' "$stop_active" | CLAUDECODE=1 DJ_HOME="$home" bash "$dir/bin/dj-turnend-guard.sh" 2>&1
}
...
watcher_identity() {
  local dir=$1 pid=$2
  DJ_STATE_OVERRIDE="$dir/state" bash -c '. "$1"; dj_pid_identity "$2"' _ "$dir/bin/dj-wake-lib.sh" "$pid"
}
...
record_watcher_lock() {
  ...
  printf '%s\n' "$root" > "$dir/state/.watch.lock/dj-home"
  printf '%s\n' "$bin_dir/dj-watch.sh" > "$dir/state/.watch.lock/watcher-path"
...
test_hook_silent_when_no_work_in_flight() {
  ...
  pass "dj-turnend-guard: silent no-op with nothing in flight"
}
...
test_hook_blocks_when_fresh_beacon_has_no_live_lock() {
  ...
  pass "dj-turnend-guard: blocks when a fresh beacon has no live watcher lock"
}
...
test_hook_blocks_source_only_home() {
  ...
  pass "dj-turnend-guard: non-Claude path blocks a source-only home"
}
...
test_hook_blocks_when_dead_lock_has_fresh_beacon() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "dj-turnend-guard: blocks on a dead watcher lock even when the beacon is fresh"
}
...
test_hook_silent_with_live_lock_and_fresh_beacon() {
  ...
  pass "dj-turnend-guard: silent no-op with a live watcher lock and fresh beacon"
}
...
test_hook_non_claude_health_ignores_claude_budget_contention() {
  ...
  while IFS='|' read -r harness payload; do
    out=$(printf '%s' "$payload" | DJ_HOME="$home" bash "$dir/bin/dj-turnend-guard.sh" 2>&1); status=$?
...
EOF
  ...
  pass "dj-turnend-guard: healthy non-Claude harness paths ignore Claude episode contention"
}
...
test_hook_blocks_with_live_lock_and_stale_beacon() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "dj-turnend-guard: blocks on a live watcher lock with an ancient beacon"
}
...
test_hook_blocks_when_unhealthy_in_primary() {
  ...
  pass "dj-turnend-guard: blocks with the exact required reason in the primary when unhealthy"
}

test_hook_blocks_from_fm_home_state() {
  local dir home out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-dj-home")
  home="$TMP_ROOT/hook-dj-home-op"
  mkdir -p "$home/state"
  : > "$home/state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 DJ_HOME="$home" bash "$dir/bin/dj-turnend-guard.sh" 2>&1); status=$?
  expect_code 2 "$status" "hook must inspect the active DJ_HOME state dir"
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "dj-turnend-guard: blocks from active DJ_HOME state, not only repo-root state"
}
...
test_hook_x_mode_reason_sources_cadence() {
  ...
  pass "dj-turnend-guard: X-mode repair reason sources the cadence config"
}
...
test_hook_x_mode_only_blocks_in_default_mode() {
  ...
  pass "dj-turnend-guard: X-mode-only supervision remains guarded in default mode"
}

test_hook_ignores_repo_state_when_fm_home_set() {
  local dir home out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-dj-home-ignore-root")
  home="$TMP_ROOT/hook-dj-home-quiet"
  mkdir -p "$home/state"
  : > "$dir/state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | DJ_HOME="$home" bash "$dir/bin/dj-turnend-guard.sh" 2>&1); status=$?
  expect_code 0 "$status" "hook must ignore repo-root state when DJ_HOME selects another state dir"
  [ -z "$out" ] || fail "hook produced output from stale repo-root state despite DJ_HOME: $out"
  pass "dj-turnend-guard: ignores stale repo-root state when DJ_HOME is set"
}
...
test_hook_uses_state_override() {
  ...
  : > "$state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" bash "$dir/bin/dj-turnend-guard.sh" 2>&1); status=$?
  expect_code 2 "$status" "hook must let DJ_STATE_OVERRIDE win over DJ_HOME/state"
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "dj-turnend-guard: uses DJ_STATE_OVERRIDE ahead of DJ_HOME/state"
}
...
test_hook_loop_guard_allows_retry() {
  ...
  pass "dj-turnend-guard: stop_hook_active=true always allows the stop (never blocks twice in one turn)"
}

# A secondmate's OWN home runs a primary jarvis session and must be guarded
# exactly like the main primary. This was the guard's proven blind spot: the
# .dj-secondmate-home marker used to early-exit here, so an overnight secondmate
...
test_hook_blocks_in_secondmate_own_home() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  assert_contains "$out" "TURN WOULD END BLIND" "block banner must read as an alarm"
  pass "dj-turnend-guard: blocks a blind turn end in a secondmate's own home (.dj-secondmate-home no longer excludes it)"
}
...
test_hook_silent_in_idle_secondmate_home() {
  ...
  pass "dj-turnend-guard: idle-by-default - silent in a secondmate home with nothing in flight"
}
...
test_hook_secondmate_loop_guard_allows_retry() {
  ...
  pass "dj-turnend-guard: stop_hook_active=true allows the stop in a secondmate home (never blocks twice in one turn)"
}
...
test_hook_secondmate_reinvoke_recovery_loop() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "dj-turnend-guard: secondmate deferred-death recovery - silent while watched, forces re-arm once the watcher exits"
}
...
test_hook_silent_in_secondmate_child_worktree() {
  ...
  pass "dj-turnend-guard: inert in a secondmate's own child worktree (linked git worktree) even when unhealthy"
}
...
test_hook_blocks_in_treehouse_leased_secondmate_home() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  assert_contains "$out" "TURN WOULD END BLIND" "block banner must read as an alarm"
  pass "dj-turnend-guard: blocks a blind turn end in a treehouse-leased LINKED secondmate home (marker force-include)"
}
...
test_hook_exempts_linked_worktree_with_stray_marker() {
  ...
  pass "dj-turnend-guard: an invalid (empty) marker cannot spoof inclusion; linked worktree stays exempt"
}
...
test_hook_exempts_linked_worktree_with_non_ascii_marker() {
  ...
  pass "dj-turnend-guard: a non-ASCII marker cannot spoof inclusion; linked worktree stays exempt"
}
...
test_hook_silent_in_crewmate_worktree() {
  ...
  pass "dj-turnend-guard: inert in a crewmate/scout task worktree (linked git worktree) even when unhealthy"
}
...
test_hook_runs_fast() {
  ...
  pass "dj-turnend-guard: runs well under the generous timing margin (${elapsed_s}s)"
}
...
test_grok_adapter_forces_one_resume_when_unhealthy() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-adapter-fakebin")
...
EOF
  ...
  out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  assert_contains "$(cat "$log")" 'JARVIS_OP: v1 turn-end-guard: TURN WOULD END BLIND' "grok adapter must retain the typed guard kind"
  pass "dj-turnend-guard-grok: forces one explicitly marked same-session resume when the shared predicate blocks"
}
...
test_grok_adapter_loop_guard_skips_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-adapter-loop-fakebin")
...
EOF
  chmod +x "$fakebin/grok"
  out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" GROK_TURNEND_GUARD_ACTIVE=1 bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "dj-turnend-guard-grok: legacy environment loop guard prevents a nested resume loop"
}
...
test_grok_adapter_native_false_blocks_without_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-native-false-bin")
  log="$TMP_ROOT/grok-native-false.log"
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "dj-turnend-guard-grok: native false delegates blocking feedback with zero resume processes"
}
...
test_grok_adapter_native_true_allows_without_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-native-true-bin")
  log="$TMP_ROOT/grok-native-true.log"
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "dj-turnend-guard-grok: native true remains bounded and starts no resume process"
}
...
test_grok_adapter_snake_case_native_and_camel_precedence() {
  ...
  : > "$dir/state/task1.meta"
  out=$(printf '%s' '{"sessionId":"native","stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true,"stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  expect_code 0 "$status" "camelCase true must win over snake_case false"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false,"stop_hook_active":true}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  expect_code 2 "$status" "camelCase false must win over snake_case true"
  pass "dj-turnend-guard-grok: both spellings are typed and camelCase has deterministic precedence"
}
...
test_grok_adapter_invalid_inputs_start_neither_path() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-invalid-bin")
  ...
  for payload in \
    ...
    '{"sessionId":"x","sessionId":"y"}'
  do
    out=$(printf '%s' "$payload" | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$TMP_ROOT/missing-grok-root" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "dj-turnend-guard-grok: malformed, invalidly typed, and missing-prerequisite payloads start neither path"
}
...
test_grok_adapter_missing_jq_and_no_supervision_allow() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(dj_fakebin "$TMP_ROOT/grok-nojq-bin")
  ...
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/dj-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "dj-turnend-guard-grok: missing jq and no-supervision-needed stops stay silent and bounded"
}
...
#
# bin/dj-subagent-pretool-check.sh is the deliberate exception: Grok has no
...
test_tracked_claude_entries_inert_under_grok() {
  ...
  mkdir -p "$dir/bin"
  for script in dj-turnend-guard.sh dj-claude-stop-autoarm.sh dj-sessionstart-run.sh \
    dj-arm-pretool-check.sh dj-cd-pretool-check.sh dj-subagent-pretool-check.sh; do
  ...
  while IFS= read -r cmd; do
    ...
    if [ "$target" = dj-subagent-pretool-check.sh ]; then
...
test_codex_hook_uses_process_pwd_when_payload_cwd_is_outside_root() {
  ...
  mkdir -p "$outside"
  cat > "$dir/bin/dj-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'guard=%s\n' "$0"
cat
EOF
  chmod +x "$dir/bin/dj-turnend-guard.sh"
  payload=$(jq -cn --arg cwd "$outside" '{cwd:$cwd,stop_hook_active:false}')
  out=$(printf '%s' "$payload" | (cd "$dir" && bash -c "$command") 2>&1); status=$?
  expect_code 0 "$status" "codex hook must execute successfully when payload cwd is outside the jarvis root"
  assert_contains "$out" "guard=$expected_root/bin/dj-turnend-guard.sh" "codex hook must use the hook process root"
...
test_codex_hook_ignores_nested_git_root_guard() {
  ...
  cat > "$nested/bin/dj-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'nested guard executed\n'
exit 99
EOF
  chmod +x "$nested/bin/dj-turnend-guard.sh"
  cat > "$dir/bin/dj-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'guard=%s\n' "$0"
cat
EOF
  ...
  assert_contains "$out" "guard=$expected_root/bin/dj-turnend-guard.sh" "codex hook must keep using the outer jarvis guard"
...
test_opencode_plugin_anchors_guard_to_worktree() {
  ...
  plugin="$ROOT/.opencode/plugins/dj-primary-turnend-guard.js"
  ...
  cat > "$worktree_dir/bin/dj-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null
printf 'guard-fired\n' >&2
exit 2
EOF
  chmod +x "$worktree_dir/bin/dj-turnend-guard.sh"
...
if (!promptBody.startsWith("\u2063JARVIS_OP: v1 turn-end-guard: ")) {
...
test_pi_extension_injects_once_per_logical_agent_run() {
  ...
  ext="$repo/.pi/extensions/dj-primary-turnend-guard.ts"
  log="$TMP_ROOT/pi-logical-run-guard.log"
  mkdir -p "$repo/.pi/extensions/lib" "$repo/bin" "$home/state"
  cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$ext"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$repo/.pi/extensions/lib/dj-operational-input.ts"
  cp "$ROOT/bin/dj-operational-input.sh" "$repo/bin/dj-operational-input.sh"
  cat > "$repo/bin/dj-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
cat >/dev/null
printf 'guard\n' >> "${DJ_GUARD_LOG:?}"
...
exit 2
SH
  cat > "$repo/bin/dj-arm-pretool-check.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$repo/bin/dj-turnend-guard.sh" "$repo/bin/dj-arm-pretool-check.sh"
  out=$(PLUGIN="$ext" DJ_HOME="$home" DJ_GUARD_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
const pi = {
  ...
  async sendUserMessage(message, options) {
    prompts += 1;
    if (!message.startsWith("\u2063JARVIS_OP: v1 turn-end-guard: ")) throw new Error(`untyped operational prompt: ${message}`);
...
const guardRuns = readFileSync(process.env.DJ_GUARD_LOG, "utf8").trim().split("\n").length;
...
SH
  chmod +x "$repo/bin/dj-turnend-guard.sh" "$repo/bin/dj-arm-pretool-check.sh"
  out=$(PLUGIN="$ext" DJ_HOME="$home" node --input-type=module 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
run_hook_claude() {
  local dir=$1 stop_active=$2 home
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s,"session_id":"sess-claude-mode"}' "$stop_active" | CLAUDECODE=1 DJ_HOME="$home" bash "$dir/bin/dj-turnend-guard.sh" --claude 2>&1
}
...
install_integrated_autoarm() {
  local dir=$1
  cp "$ROOT/bin/dj-claude-stop-autoarm.sh" "$dir/bin/dj-claude-stop-autoarm.sh"
  cp "$ROOT/bin/dj-primary-scope-lib.sh" "$dir/bin/dj-primary-scope-lib.sh"
  cp "$ROOT/bin/dj-supervision-lib.sh" "$dir/bin/dj-supervision-lib.sh"
  cp "$ROOT/bin/dj-wake-lib.sh" "$dir/bin/dj-wake-lib.sh"
  cp "$ROOT/bin/dj-hook-host-lib.sh" "$dir/bin/dj-hook-host-lib.sh"
  cp "$ROOT/bin/dj-session-lock-lib.sh" "$dir/bin/dj-session-lock-lib.sh"
  cp "$ROOT/bin/dj-cursor-lib.sh" "$dir/bin/dj-cursor-lib.sh"
  cp "$ROOT/bin/dj-lock.sh" "$dir/bin/dj-lock.sh"
  chmod +x "$dir/bin/dj-claude-stop-autoarm.sh" "$dir/bin/dj-lock.sh"
...
run_integrated_autoarm() {
  local dir=$1 home
  home=$(cd "$dir" && pwd)
  # shellcheck disable=SC2016 # the fake harness expands DJ_HOME inside its child shell.
  printf '{"session_id":"sess-claude-mode","stop_hook_active":false}\n' \
    | DJ_HOME="$home" "$dir/fake-claude" -c '
        printf "%s\n" "$$" > "$DJ_HOME/state/.lock"
        "$DJ_HOME/bin/dj-claude-stop-autoarm.sh"
      ' 2>&1
}

write_integrated_failed_arm() {
  local dir=$1
  cat > "$dir/bin/dj-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'watcher: FAILED - persistent fixture failure\n'
exit 1
SH
  chmod +x "$dir/bin/dj-watch-arm.sh"
}
...
test_hook_claude_mode_reblocks_stop_hook_active_when_unhealthy() {
  ...
  : > "$dir/state/task1.meta"
  out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=200 run_hook_claude "$dir" true); status=$?
  ...
  pass "dj-turnend-guard --claude: re-blocks a loop-guarded stop while unhealthy and unclaimed (incident regression)"
}
...
test_hook_claude_mode_reblocks_x_mode_without_tasks() {
  ...
  pass "dj-turnend-guard --claude: X-mode-only homes re-block when auto-arm recovery is absent"
}
...
test_hook_claude_mode_allows_when_autoarm_owner_alive() {
  ...
  pass "dj-turnend-guard --claude: a live arming epoch advances once and repeated observation is idempotent"
}
...
test_hook_claude_mode_repeated_failed_to_arming_interleavings_reach_fail_open() {
  ...
  assert_contains "$out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "arming interleavings stalled before the bounded fail-open"
  assert_present "$dir/state/.claude-autoarm-failure-alarmed" "arming interleavings did not consume the one-time alarm"
  pass "dj-turnend-guard --claude: repeated failed-to-arming races make bounded monotonic progress"
}
...
test_hook_claude_mode_terminal_boundary_excludes_starting_owner() {
  ...
  cat > "$fakebin/cat" <<'SH'
#!/usr/bin/env bash
if [ "$1" = "$DJ_TERMINAL_ROLE_PATH" ] \
  && [ "$(/bin/cat "$1" 2>/dev/null || true)" = terminal-check ] \
  && (set -C; : > "$DJ_TERMINAL_ONCE") 2>/dev/null; then
  printf 'ready\n' > "$DJ_TERMINAL_READY"
  IFS= read -r _ < "$DJ_TERMINAL_RELEASE"
fi
exec /bin/cat "$@"
SH
  chmod +x "$fakebin/cat"
  (
    printf '{"stop_hook_active":true,"session_id":"sess-claude-mode"}' \
      | PATH="$fakebin:$PATH" \
        DJ_TERMINAL_ROLE_PATH="$dir/state/.claude-autoarm.lock/role" \
        DJ_TERMINAL_READY="$ready" \
        DJ_TERMINAL_RELEASE="$release" \
        DJ_TERMINAL_ONCE="$once" \
        CLAUDECODE=1 DJ_HOME="$dir" bash "$dir/bin/dj-turnend-guard.sh" --claude \
          > "$guard_out" 2>&1
    printf '%s\n' "$?" > "$guard_status"
  ) &
  guard_pid=$!
  ...
  assert_contains "$(cat "$guard_out")" 'JARVIS SUPERVISION IS GENUINELY DOWN' "terminal boundary did not produce the one-time alarm"
  assert_absent "$dir/state/.claude-autoarm.lock" "terminal boundary left its owner lock behind"
  pass "dj-turnend-guard --claude: terminal owner boundary excludes a concurrent start without deadlock"
}
...
test_hook_claude_mode_allows_on_fresh_rewake_epoch() {
  ...
  pass "dj-turnend-guard --claude: fresh rewake epoch prevents a duplicate continuation for the same event"
}
...
test_hook_claude_mode_blocks_on_abandoned_autoarm_claim() {
  ...
  pass "dj-turnend-guard --claude: an abandoned auto-arm claim no longer allows a blind stop (incident regression)"
}
...
test_hook_claude_mode_blocks_on_pid_reused_arming_claim() {
  ...
  identity=$(dj_test_pid_identity "$$") || fail "could not compute a claim pid-identity"
  ...
  pass "dj-turnend-guard --claude: a claim whose pid was reused stops counting as recovery even while its entry reads arming"
}
...
test_hook_claude_mode_terminal_fail_open_clears_abandoned_claim() {
  ...
  assert_contains "$out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "an abandoned claim suppressed the episode's attended alarm"
  ...
  pass "dj-turnend-guard --claude: the terminal path clears an abandoned claim instead of stepping aside silently"
}
...
test_hook_claude_mode_preserves_fresh_failed_progression() {
  ...
  pass "dj-turnend-guard --claude: fresh failed epochs preserve and advance monotonic fail-open progression"
}
...
test_hook_claude_mode_integrated_monotonic_fail_open() {
  ...
  guard_out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); guard_status=$?
  ...
  for i in 1 2 3 4; do
    ...
    if [ "$i" -lt 4 ]; then
      ...
      assert_not_contains "$guard_out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "fail-open fired before the bounded progression ended"
    else
      expect_code 0 "$guard_status" "the bounded failure progression must reach the attended fail-open"
      assert_contains "$guard_out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "the integrated fail-open alarm is missing"
  ...
  assert_not_contains "$guard_out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "the attended alarm repeated in the same episode"
  ...
  guard_out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); guard_status=$?
  ...
  pass "dj-turnend-guard --claude: integrated fresh failures reach one bounded fail-open, stop continuation, and reset on recovery"
}
...
test_hook_claude_mode_recovery_contention_is_not_ordinary_allow() {
  ...
  pass "dj-turnend-guard --claude: reset contention preserves all episode state until retry"
}
...
test_hook_claude_mode_concurrent_recovery_resets_are_idempotent() {
  ...
  auto_pid=$!
  (DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false > "$dir/guard.out"; printf '%s\n' "$?" > "$dir/guard.status") &
  guard_pid=$!
  wait "$auto_pid"
  wait "$guard_pid"
  kill "$pid" 2>/dev/null || true
  ...
  pass "dj-turnend-guard --claude: concurrent auto-arm and guard resets are idempotent and deadlock-free"
}
...
test_hook_claude_mode_stale_rewake_epoch_blocks() {
  ...
  pass "dj-turnend-guard --claude: stale rewake epoch does not allow a blind stop"
}
...
test_hook_claude_mode_budget_without_verified_failure_keeps_blocking() {
  ...
  : > "$dir/state/task1.meta"
  for i in 1 2 3 4; do
    out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); status=$?
  ...
  pass "dj-turnend-guard --claude: budget exhaustion alone cannot permit a blind stop"
}
...
test_hook_claude_mode_verified_failure_alarm_is_loud_and_once() {
  ...
  seed_claude_failure "$dir"
  seed_claude_budget "$dir" 3
  out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); status=$?
  expect_code 0 "$status" "verified failure with exhausted budget must take the bounded attended fail-open"
  assert_contains "$out" 'JARVIS SUPERVISION IS GENUINELY DOWN' "bounded fail-open alarm was not unmistakable"
  ...
  assert_not_contains "$out" 'dj-watch-arm.sh' "bounded fail-open alarm assigned a manual watcher launch"
  assert_present "$dir/state/.claude-autoarm-failure-alarmed" "bounded fail-open did not consume the episode alarm"
  out2=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); status2=$?
  expect_code 2 "$status2" "a consumed attended alarm must make later unhealthy stops block again"
  assert_not_contains "$out2" 'JARVIS SUPERVISION IS GENUINELY DOWN' "attended failure alarm repeated in one episode"
  pass "dj-turnend-guard --claude: verified fail-open is loud, bounded, attended, and non-repeating"
}
...
test_hook_claude_mode_fail_open_requires_notice_and_failure_epoch() {
  ...
  out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$no_notice" true); status=$?
  ...
  out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$notice_only" true); status=$?
  expect_code 2 "$status" "a consumed notice without an exhausted failure epoch must remain blocking"
  pass "dj-turnend-guard --claude: fail-open requires both exhausted retries and consumed notice"
}
...
test_hook_claude_mode_away_mode_never_uses_stop_autoarm_fail_open() {
  ...
  pass "dj-turnend-guard --claude: away ownership excludes the Stop-autoarm fail-open"
}
...
test_hook_claude_mode_allow_resets_budget() {
  ...
  pass "dj-turnend-guard --claude: positive watcher recovery resets failure episode state"
}
...
test_hook_claude_mode_waits_for_late_claim() {
  ...
  helper=$!
  out=$(DJ_CLAUDE_AUTOARM_SYNC_WAIT_MS=3000 run_hook_claude "$dir" false); status=$?
  ...
  pass "dj-turnend-guard --claude: bounded claim wait avoids a token-consuming forced continuation"
}
...
test_hook_claude_mode_secondmate_reblocks_like_primary() {
  ...
  pass "dj-turnend-guard --claude: secondmate home re-blocks unclaimed and allows auto-arm-claimed stops"
}
