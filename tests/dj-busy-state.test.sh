#!/usr/bin/env bash
# Behavior tests for the semantic busy-state contract (bin/dj-busy-lib.sh and
# its only writer bin/dj-busy-event.sh).
#
...
. "$ROOT/bin/dj-busy-lib.sh"

TMP_ROOT=$(dj_test_tmproot dj-busy-state)
EV="$ROOT/bin/dj-busy-event.sh"
...
test_arm_seeds_busy_spawn() {
  ...
  out=$(dj_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy dj-spawn" ] || fail "seed should classify 'busy dj-spawn', got '$out'"
  pass "arm mints a gen sidecar and seeds busy dj-spawn at seq=1"
}
...
test_apply_advances_seq_and_source() {
  ...
  seq=$(dj_busy_record_read "$state" t1 | awk '{print $4}')
...
test_apply_current_gen_reset() {
  ...
  "$EV" apply "$state" t1 idle --current-gen --source dj-interrupt --event interrupt \
    || fail "apply --current-gen failed"
  out=$(dj_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "idle dj-interrupt" ] || fail "expected 'idle dj-interrupt', got '$out'"
  "$EV" apply "$state" t1 unknown --current-gen --source dj-recovery --event relaunch \
    || fail "apply unknown failed"
  out=$(dj_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "unknown dj-recovery" ] || fail "expected 'unknown dj-recovery', got '$out'"
  pass "jarvis-owned interrupt and recovery events bind to the current gen"
}
...
test_retire_serializes_and_rejects_stale_gen() {
  ...
  out=$(dj_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy dj-spawn" ] || fail "stale retirement changed the new incarnation, got '$out'"
...
# dj-teardown.sh died there after returning the worktree, leaving state/<id>.meta
...
test_stale_lock_broken_under_gnu_stat() {
  ...
  gen=$("$EV" arm "$state" t1)
  fakebin=$(dj_fakebin "$TMP_ROOT/gnu-stat-lock")
...
test_stale_gen_event_rejected() {
  ...
  out=$(dj_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy dj-spawn" ] || fail "stale event must not change the record, got '$out'"
...
test_missing_record_unknown_not_idle() {
  ...
  for h in claude opencode pi pi-signed; do
    out=$(dj_busy_classify tmux w1 "$h" t1 "$state")
    [ "$out" = "unknown missing" ] || fail "$h with no record must be 'unknown missing', got '$out'"
  done
  out=$(dj_busy_classify tmux w1 codex t1 "$state")
...
test_source_mismatch_cross_adapter() {
  ...
  out=$(dj_busy_classify tmux w1 grok t1 "$state")
...
Ctrl+c:cancel'
  for h in claude opencode pi pi-signed; do
    out=$(dj_busy_classify tmux w1 "$h" t1 "$state" "$tail")
    [ "$out" = "unknown missing" ] || fail "$h must never classify from footer text, got '$out'"
  done
  out=$(dj_busy_classify tmux w1 codex t1 "$state" "$tail")
...
test_grok_regex_isolated() {
  ...
  out=$(dj_busy_classify tmux w1 grok t1 "$state" 'thinking hard
Ctrl+c:cancel')
  [ "$out" = "busy grok-regex" ] || fail "grok busy tail must classify 'busy grok-regex', got '$out'"
  out=$(dj_busy_classify tmux w1 grok t1 "$state" 'done.
> ')
  ...
  out=$(dj_busy_classify tmux w1 grok t1 "$state" '• Working (6s • esc to interrupt)')
...
test_codex_unverified_gate() {
  ...
  [ -z "$(dj_busy_sources_for_harness codex)" ] \
...
test_kimi_unverified_gate() {
  ...
  out=$(dj_busy_classify tmux w1 kimi t1 "$state")
  [ "$out" = "unknown kimi-unverified" ] || fail "unverified kimi must classify unknown, got '$out'"
  out=$(dj_busy_classify tmux w1 kimi t1 "$state" '🌒 · thinking')
...
test_cursor_ignores_rendered_and_native_signals() {
  ...
  out=$(dj_busy_classify tmux w1 cursor t1 "$state" 'Working')
  [ "$out" = "unknown cursor-transcript" ] \
    || fail "cursor must not classify from its rendered footer, got '$out'"
  out=$(dj_busy_classify tmux w1 cursor t1 "$state" 'ctrl+c to stop')
  [ "$out" = "unknown cursor-transcript" ] \
  ...
  # shellcheck disable=SC2329 # invoked indirectly through dj_busy_classify
  dj_backend_busy_state() { printf '%s' busy; }
  out=$(dj_busy_classify herdr s:p cursor t1 "$state")
  [ "$out" = "unknown cursor-transcript" ] \
    || fail "cursor must not borrow herdr's native busy verdict, got '$out'"
  unset -f dj_backend_busy_state
  # The fold is a PULL source: nothing is armed, so no stored record is trusted.
  [ -z "$(dj_busy_sources_for_harness cursor)" ] \
...
test_dead_endpoint_overrides() {
  ...
  gen=$("$EV" arm "$state" t1)
  # shellcheck disable=SC2329 # invoked indirectly through dj_busy_classify_live
  dj_backend_target_exists() { return 1; }
  out=$(dj_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "dead endpoint-gone" ] || fail "gone endpoint must classify dead, got '$out'"
  # shellcheck disable=SC2329 # invoked indirectly through dj_busy_classify_live
  dj_backend_target_exists() { return 0; }
  out=$(dj_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "busy dj-spawn" ] || fail "live endpoint must fall through to the record, got '$out'"
  out=$(dj_busy_classify_live tmux '' claude t1 "$state")
  [ "$out" = "unknown no-target" ] || fail "empty target must classify unknown, got '$out'"
  unset -f dj_backend_target_exists
...
test_herdr_native_busy_only() {
  ...
  # shellcheck disable=SC2329 # invoked indirectly through dj_busy_classify
  dj_backend_busy_state() { printf '%s' "$FAKE_NATIVE"; }
  FAKE_NATIVE=busy
  out=$(dj_busy_classify herdr s:p claude t1 "$state")
...
test_record_read_leaves_caller_shell_intact() {
  ...
  out=$(bash -c '
    set -f
    . "$1/bin/dj-busy-lib.sh"
    set -- keepme second
    dj_busy_record_read "$2" t1 >/dev/null
    printf "%s|%s|%s" "$1" "$#" "$-"
  ' _ "$ROOT" "$state")
  case "$out" in
    keepme\|2\|*f*) : ;;
...
test_boolean_view_never_promotes_unknown() {
  ...
  gen=$("$EV" arm "$state" t1)
  dj_busy_is_busy tmux w1 claude t1 "$state" || fail "busy record must read busy"
  "$EV" apply "$state" t1 idle --gen "$gen" --source claude-hook --event stop
  if dj_busy_is_busy tmux w1 claude t1 "$state"; then
...
echo "all dj-busy-state tests passed"
