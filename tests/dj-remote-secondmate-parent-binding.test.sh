#!/usr/bin/env bash
# tests/dj-remote-secondmate-parent-binding.test.sh - regression coverage for the
# dj-remote-sm-cleanup-parent-binding-s1 scout report: finished-worker cleanup
...
# child the remote code checkout as its parent home (bin/dj-spawn.sh's sole
# writer of DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME receives DJ_HOME=$DJ_ROOT from
# bin/dj-remote-secondmate-control.sh's host-local launch), and that path can
# never carry the parent's real state or registry.
#
# The fix (report section 7, captain-approved same-machine scope): a durable
# .dj-secondmate-parent record, written once at seeding next to the
# .dj-secondmate-home identity marker, names this home's route to its parent as
# "local" or "remote". bin/dj-teardown.sh's cleanup gate reads it and treats a
...
#
# This drives the REAL remote route (dj-remote-home-seed.sh -> dj-on.sh ->
# dj-remote-entrypoint.sh -> the host-local dj-remote-secondmate-control.sh ->
# the real bin/dj-spawn.sh --secondmate) across the repo's own deterministic SSH
# boundary and Herdr fixture, then runs the real bin/dj-teardown.sh for a
...
TMP_ROOT=$(dj_test_tmproot dj-remote-parent-binding)
mkdir -p "$TMP_ROOT"
...
PUBLISH_FAKEBIN=$(dj_fakebin "$TMP_ROOT/publication-fake")
...
case "$destination" in
  */.dj-secondmate-home)
    ...
    while [ ! -f "$DJ_TEST_PUBLISH_RELEASE" ]; do sleep 0.02; done
    ;;
esac
...
PATH="$PUBLISH_FAKEBIN:$PATH" DJ_HOME="$PUBLISH_HOME" DJ_ROOT_OVERRIDE="$ROOT" \
  DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_PUBLISH_ENTERED="$PUBLISH_ENTERED" \
  DJ_TEST_PUBLISH_RELEASE="$PUBLISH_RELEASE" \
  "$ROOT/bin/dj-remote-home-provision.sh" < "$PUBLISH_MANIFEST" >/dev/null 2>&1 &
PUBLISH_PID=$!
publish_wait=0
...
cmp -s "$PUBLISH_HOME/.dj-secondmate-parent" <(
...
assert_absent "$PUBLISH_HOME/.dj-secondmate-home" \
...
PUBLISH_PID=
assert_present "$PUBLISH_HOME/.dj-secondmate-home" \
...
printf 'DJX_PAIRING_TOKEN=repro-token\n' > "$PARENT/.env"
...
DJ_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
...
assert_present "$REMOTE_HOME/.dj-secondmate-parent" \
  "real remote provisioning must write a durable parent record"
cmp -s "$REMOTE_HOME/.dj-secondmate-parent" <(
  printf 'schema=dj-secondmate-parent.v1\nroute=remote\nparent_host=remote-mac\n'
) || fail "real remote provisioning must write the exact durable remote parent record"

remote_env "$ROOT/bin/dj-spawn.sh" ios --secondmate >/dev/null \
  || fail "real remote secondmate launch failed"

DELIVERED_LINE=$(grep -F 'DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME' "$HERDR_LOG" | tail -1 || true)
DELIVERED=$(printf '%s\n' "$DELIVERED_LINE" | tr ' ' '\n' \
  | sed -n "s/^DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME='\{0,1\}\([^']*\)'\{0,1\}\$/\1/p" | tail -1)
...
write_child_meta() {
  dj_write_meta "$REMOTE_HOME/state/work-child.meta" \
...
run_child_teardown() { # <extra env assignments...>
  ...
  out=$(env "$@" PATH="$TMP_ROOT/childfake:$PATH" \
    DJ_HOME="$REMOTE_HOME" DJ_STATE_OVERRIDE="$REMOTE_HOME/state" \
    DJ_DATA_OVERRIDE="$REMOTE_HOME/data" DJ_CONFIG_OVERRIDE="$REMOTE_HOME/config" \
    "$REMOTE_ROOT/bin/dj-teardown.sh" work-child 2>&1) || rc=$?
...
# state (a captain who also runs Jarvis directly on the build Mac). Before
# the fix this refused; the durable record now makes it out of scope.
printf 'DJX_PAIRING_TOKEN=remote-host-token\n' > "$REMOTE_ROOT/.env"
run_child_teardown DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$DELIVERED"
...
# Case C-equivalent: DJX_PAIRING_TOKEN exported directly in the process
# environment, simulating the remote host's own login-shell export reaching the
# agent's pane. dj_pf_relay_active's environment-wins rule would make this look
# identical to a genuine same-home commitment; the fix must tell them apart by
# reading only $DJ_HOME/.env, never the process environment, once the durable
# record says the parent is remote.
run_child_teardown DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME="$DELIVERED" DJX_PAIRING_TOKEN=ambient-login-token
[ "$CHILD_TEARDOWN_RC" -eq 0 ] \
...
printf 'DJX_PAIRING_TOKEN=child-own-token\n' > "$REMOTE_HOME/.env"
run_child_teardown
