#!/usr/bin/env bash
# dj-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# popup settles lets it swallow the Enter, so the line never submits. dj-send
# absorbs this by pausing `settle` seconds AFTER typing and BEFORE the (retried)
# Enter - the first sleep dj_tmux_submit_core makes. These tests pin the
...
#
# The popup-settle is the FIRST sleep recorded: dj_tmux_submit_core types the text,
# then `sleep "$settle"`, then the Enter-retry loop (sleep 0.4 each) and finally
# dj-send's own post-submit DJ_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from dj-send-settle.test.sh which pins
# the TAIL (post-submit) pause. The retried Enter in dj_tmux_submit_core remains the
...
TMP_ROOT=$(dj_test_tmproot dj-send-popup-settle)

# Same stub shape as dj-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into DJ_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. DJ_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. DJ_ROOT_OVERRIDE points at a non-repo dir so
# dj-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  if [ "$harness" = --explicit ]; then
    ...
    dj_write_meta "$home/state/$meta_id.meta" "window=sess:win" "harness=$harness"
  fi
  : > "$log"
  env DJ_SEND_SETTLE=0 PATH="$fb:$PATH" \
  ...
  pass "dj-send popup-settle: $label -> ${expected}s"
}
...
rides_inbox() {  # <label> <harness> <message>
  ...
  dir="$TMP_ROOT/case-$RANDOM"; mkdir -p "$dir/state"
  fb=$(make_stubs "$dir"); log="$dir/sleep.log"; home="$dir"
  dj_write_meta "$home/state/popupcase.meta" "window=sess:win" "harness=$harness"
  : > "$log"
  env DJ_SEND_SETTLE=0 PATH="$fb:$PATH" \
    DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" DJ_SLEEP_LOG="$log" \
    "$SEND" dj-popupcase "$msg" 2>/dev/null; rc=$?
  expect_code 0 "$rc" "$label: send should succeed"
  ...
  pass "dj-send popup-settle: $label -> inbox plane, fast doorbell"
}
...
# task id, not only by the legacy `dj-<id>` window label.
