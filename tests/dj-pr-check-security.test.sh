...
. "$ROOT/bin/dj-pr-lib.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/dj-x-lib.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/dj-check-lib.sh"

PR_CHECK="$ROOT/bin/dj-pr-check.sh"
PR_MERGE="$ROOT/bin/dj-pr-merge.sh"
MIGRATE="$ROOT/bin/dj-pr-check-migrate.sh"
POLL="$ROOT/bin/dj-pr-poll.sh"
WATCH="$ROOT/bin/dj-watch.sh"
TEARDOWN="$ROOT/bin/dj-teardown.sh"
REGISTER="$ROOT/bin/dj-check-register.sh"
TMP_ROOT=$(dj_test_tmproot dj-pr-check-security)
...
ack_watcher_cycle() {  # <state>
  ...
  DJ_STATE_OVERRIDE="$state" "$ROOT/bin/dj-wake-drain.sh" >/dev/null 2> "$err" || return 1
  sequence=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation [A-Za-z0-9._-][A-Za-z0-9._-]*$/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through [0-9][0-9]* --recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  rm -f "$err"
  [ -n "$sequence" ] && [ -n "$generation" ] || return 1
  DJ_STATE_OVERRIDE="$state" "$ROOT/bin/dj-wake-drain.sh" --ack-through "$sequence" \
...
make_case() {
  ...
  cat > "$fake_root/bin/dj-guard.sh" <<'SH'
#!/usr/bin/env bash
printf 'guard\n' >> "$DJ_TEST_GUARD_LOG"
SH
  chmod +x "$fake_root/bin/dj-guard.sh"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$DJ_TEST_GH_LOG"
case " $* " in
  *" headRefOid "*) printf '%s\n' "${DJ_TEST_GH_HEAD:-0123456789abcdef0123456789abcdef01234567}" ;;
  *" state "*)
    [ "${DJ_TEST_GH_FAIL:-0}" = 0 ] || exit 1
    [ "${DJ_TEST_GH_SLEEP:-0}" = 0 ] || sleep "$DJ_TEST_GH_SLEEP"
    printf '%s\n' "${DJ_TEST_GH_STATE:-OPEN}"
    ;;
esac
SH
...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$DJ_TEST_GLAB_LOG"
[ "${DJ_TEST_GLAB_FAIL:-0}" = 0 ] || exit 1
[ "${DJ_TEST_GLAB_SLEEP:-0}" = 0 ] || sleep "$DJ_TEST_GLAB_SLEEP"
printf 'title:\tfixture merge request\nstate:\t%s\nauthor:\tsomeone\n' "${DJ_TEST_GLAB_STATE:-opened}"
SH
...
write_manual_poll_pair() {
  ...
  dj_pr_url_parse "$url" || fail "manual poll fixture URL was invalid"
  provider=$DJ_PR_PROVIDER
  host=$DJ_PR_HOST
  path=$DJ_PR_PATH
  number=$DJ_PR_NUMBER
...
start_ambiguous_pending_repair() {
  ...
  set +e
  DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
write_watcher_lock() {
  ...
  identity=$(DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; dj_pid_identity "$2"' _ "$ROOT/bin/dj-wake-lib.sh" "$pid")
  [ -n "$identity" ] || fail "could not capture fake older-watcher identity"
  printf '%s\n' "$pid" > "$state/.watch.lock/pid"
  printf '%s\n' "$home" > "$state/.watch.lock/dj-home"
...
assert_valid_migration_marker() {
  ...
  grep -qxF dj-pr-check-migration-v1 "$marker" || fail "migration marker bytes were not exact"
...
assert_valid_scan_marker() {
  ...
  grep -qxF dj-pr-check-migration-scan-v1 "$marker" || fail "migration scan marker bytes were not exact"
...
run_check_entry() {
  local dir=$1
  shift
  DJ_ROOT_OVERRIDE="$dir/root" DJ_HOME="$dir/home" \
    DJ_TEST_GUARD_LOG="$dir/guard.log" DJ_TEST_GH_LOG="$dir/gh.log" \
    DJ_TEST_GH_AXI_LOG="$dir/gh-axi.log" DJ_TEST_GLAB_LOG="$dir/glab.log" \
...
test_parser_matrix() {
  ...
  while IFS='|' read -r url owner repo number; do
    [ -n "$url" ] || continue
    dj_pr_url_parse "$url" || fail "parser rejected canonical URL"
    [ "$DJ_PR_URL" = "$url" ] || fail "parser changed canonical URL"
    [ "$DJ_PR_OWNER" = "$owner" ] || fail "parser returned wrong owner"
    [ "$DJ_PR_REPO" = "$repo" ] || fail "parser returned wrong repository"
    [ "$DJ_PR_NUMBER" = "$number" ] || fail "parser returned wrong PR number"
  done <<'EOF'
...
EOF
  while IFS='|' read -r url host path number; do
    [ -n "$url" ] || continue
    dj_pr_url_parse "$url" || fail "parser rejected a canonical merge request URL"
    [ "$DJ_PR_PROVIDER" = gitlab ] || fail "parser did not tag a merge request URL as gitlab"
    [ "$DJ_PR_URL" = "$url" ] || fail "parser changed a canonical merge request URL"
    [ "$DJ_PR_HOST" = "$host" ] || fail "parser returned wrong GitLab host"
    [ "$DJ_PR_PATH" = "$path" ] || fail "parser returned wrong GitLab project path"
    [ "$DJ_PR_NUMBER" = "$number" ] || fail "parser returned wrong merge request number"
    [ -z "$DJ_PR_OWNER" ] && [ -z "$DJ_PR_REPO" ] \
...
EOF
  dj_pr_url_parse https://github.com/a/b/pull/1 || fail "parser rejected canonical URL"
  [ "$DJ_PR_PROVIDER" = github ] || fail "parser did not tag a pull request URL as github"
  [ "$DJ_PR_HOST" = github.com ] || fail "parser returned wrong GitHub host"
  [ "$DJ_PR_PATH" = a/b ] || fail "parser returned wrong GitHub project path"
  for row in "${INVALID_URLS[@]}"; do
    ! dj_pr_url_parse "$row" || fail "parser accepted a rejected raw-byte URL class"
  done
  for id in -task task- task--a Task-a task_a task.a; do
    dj_pr_task_id_valid "$id" || fail "task ID validator rejected a safe lifecycle-compatible slug"
  done
  dj_task_id_creation_valid _noncanonical \
  ...
  dj_pr_task_id_valid "$id" || fail "operational validator rejected a path-safe legacy task ID"
  ! dj_task_id_creation_valid "$id" || fail "creation validator accepted an overlong task ID"
...
test_invalid_entrypoints_have_zero_side_effects() {
  ...
  for value in "${UNSAFE_LIFECYCLE_IDS[@]}"; do
    before=$(state_snapshot "$dir/home/state")
    set +e
    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" DJ_TEST_GUARD_LOG="$dir/guard.log" \
...
test_valid_recording_and_merge_derivation() {
  ...
  DJ_TEST_GH_HEAD=$expected run_check_entry "$dir" task-a https://github.com/my-org/repo_name.with-dots/pull/37 \
  ...
  [ "$(dj_pr_file_link_count "$dir/home/state/task-a.check.sh")" = 1 ] \
    && [ "$(dj_pr_file_link_count "$dir/home/state/task-a.pr-poll")" = 1 ] \
    && [ "$(dj_pr_file_link_count "$dir/home/state/task-a.pr-poll-registration")" = 1 ] \
    || fail "published poll artifacts were not single-link files"
  dj_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
  ...
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged-watch.out" 2> "$dir/merged-watch.err"
  rc=$?
  set -e
  ...
  dj_pr_poll_artifacts_valid "$dir/home/state" Task_A.1 "$POLL" \
...
SH
  ...
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
...
SH
    ...
    DJ_HOME="$dir/home" "$ROOT/bin/dj-x-link.sh" "$id" req-legacy \
    ...
    dj_pr_poll_artifacts_valid "$dir/home/state" "$id" "$POLL" \
...
run_watcher_bounded() {
  local home=$1 fakebin=$2 check_interval=${DJ_TEST_CHECK_INTERVAL:-0} watch_root=${DJ_TEST_WATCH_ROOT:-$ROOT}
  shift 2
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 10; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env DJ_HOME="$home" DJ_ROOT_OVERRIDE="$watch_root" DJ_CHECK_INTERVAL="$check_interval" DJ_CHECK_TIMEOUT=1 \
      DJ_POLL=0.02 DJ_HEARTBEAT=999999 DJ_SIGNAL_GRACE=0 PATH="$fakebin:$BASE_PATH" "$WATCH" "$@"
}
...
test_rejected_metacharacter_bytes_are_inert() {
  ...
  for family in "${families[@]}"; do
    ...
    dj_pr_poll_prepare "$dir/home/state" safe-check github https://github.com/o/r/pull/99 github.com o/r 99 "$POLL" \
      || fail "could not prepare bounded watcher poll"
    dj_pr_poll_publish_prepared || fail "could not publish bounded watcher poll"

    set +e
    DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
    rc=$?
    set -e
  ...
  DJ_TEST_GH_STATE=OPEN run_check_entry "$dir" task-a https://github.com/o/r/pull/1 >/dev/null 2>/dev/null \
...
run_poll() {
  local dir=$1
  DJ_TEST_GH_LOG="$dir/gh.log" DJ_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$dir/fakebin:$BASE_PATH" \
...
test_static_poll_contract() {
  ...
  make_poll_fixture "$dir"
  set +e
  out=$(DJ_STATE_OVERRIDE="$dir/home/state" DJ_CHECK_TIMEOUT=1 DJ_TEST_GH_LOG="$dir/gh.log" \
    DJ_TEST_GH_SLEEP=3 PATH="$dir/fakebin:$BASE_PATH" \
  ...
  dj_pr_poll_prepare "$dir/home/state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
    || fail "could not prepare authenticated watcher poll"
  dj_pr_poll_publish_prepared || fail "could not publish authenticated watcher poll"
  rm -f "$dir/home/state/.last-check"
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
...
SH
  ...
  ! find "$dir/home/state" -name '.dj-pr-poll-*' -print | grep . >/dev/null \
...
SH
    ...
    direct_pid=$!
    i=0
    while [ "$i" -lt 100 ] && ! find "$dir/home/state" -name '.dj-pr-poll-check.*' -print | grep . >/dev/null; do
      sleep 0.01
      i=$((i + 1))
    done
...
test_migration_excludes_older_watcher_before_scan() {
  ...
  dj_write_meta "$state/task-a.meta" \
    'window=dj-task-a' \
...
SH
  ...
  set +e
  DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_migration_initializes_fresh_state() {
  ...
  set +e
  DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
  rc=$?
  set -e
...
test_private_artifact_paths_refuse_symlinks_and_directories() {
  ...
  for artifact in task-a.pr-poll task-a.pr-poll-registration task-a.check.sh; do
    for kind in regular dangling directory; do
      ...
      state="$dir/home/state"
      dj_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
        || fail "could not stage poll symlink refusal fixture"
      destination="$state/$artifact"
      make_private_symlink "$dir" "$destination" "$kind"
      if dj_pr_poll_publish_prepared; then
...
#!/usr/bin/env bash
last=${!#}
"${DJ_TEST_REAL_MV:?}" "$@" || exit $?
[ "$last" = "${DJ_TEST_FINAL_PATH:?}" ] || exit 0
case "${DJ_TEST_FINAL_ACTION:?}" in
  type)
    rm -f -- "$last"
    ln -s "${DJ_TEST_FAULT_LINK_TARGET:?}" "$last"
    ;;
  mode) "${DJ_TEST_REAL_CHMOD:?}" 0644 "$last" ;;
  content) printf 'faulted final bytes\n' > "$last" ;;
  device) : > "${DJ_TEST_FAULT_GATE:?}" ;;
  *) exit 2 ;;
esac
SH
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${DJ_TEST_FINAL_PATH:-}" ] && [ -e "${DJ_TEST_FAULT_GATE:-/nonexistent}" ]; then
  case " $* " in
    *" %d "*) printf '%s\n' 999999; exit 0 ;;
  esac
fi
exec "${DJ_TEST_REAL_STAT:?}" "$@"
SH
...
test_postrename_poll_validation_revokes_and_retries() {
  ...
  for artifact in data registration check; do
    for action in type mode device content; do
      ...
      dj_pr_poll_publish_prepared || fail "could not publish prior poll"
      write_poll_meta "$state" task-a https://github.com/o/r/pull/2
      dj_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/2 github.com o/r 2 "$POLL" \
      ...
      if DJ_TEST_FINAL_PATH="$destination" DJ_TEST_FINAL_ACTION="$action" \
        DJ_TEST_FAULT_LINK_TARGET="$link_target" DJ_TEST_FAULT_GATE="$gate" \
        DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_REAL_STAT="$REAL_STAT" DJ_TEST_REAL_CHMOD="$REAL_CHMOD" \
        PATH="$dir/fakebin:$BASE_PATH" dj_pr_poll_publish_prepared; then
      ...
      PATH="$BASE_PATH" dj_pr_poll_publish_prepared || fail "poll retry did not recover after final validation fault"
      dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "poll retry did not publish a valid pair"
    done
  done
...
for arg in "$@"; do
  case "$arg" in
    *"${DJ_TEST_MV_MATCH:?}"*) matched=1 ;;
  esac
done
if [ "$matched" -eq 1 ]; then
  case "${DJ_TEST_MV_ACTION:?}" in
    fail) exit 1 ;;
    signal)
      kill -TERM "$PPID"
      sleep 0.1
...
test_marker_and_diagnostic_rename_fail_closed() {
  ...
  for action in fail signal; do
    ...
    set +e
    DJ_TEST_MV_MATCH=.dj-pr-check-migration. DJ_TEST_MV_ACTION="$action" DJ_TEST_REAL_MV="$REAL_MV" \
      DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.dj-pr-check-migration.*' -print | grep . >/dev/null \
    ...
    install_mv_fault "$dir"
    set +e
    DJ_TEST_MV_MATCH=.dj-pr-check-log. DJ_TEST_MV_ACTION="$action" DJ_TEST_REAL_MV="$REAL_MV" \
      DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
    rc=$?
    set -e
    ...
    ! find "$state" -name '.dj-pr-check-log.*' -print | grep . >/dev/null \
...
test_postrename_marker_and_diagnostic_validation_retries() {
  ...
  for artifact in marker diagnostic obligation; do
    for action in type mode device content; do
      ...
      install_final_publication_fault "$dir"
      set +e
      DJ_TEST_FINAL_PATH="$destination" DJ_TEST_FINAL_ACTION="$action" \
        DJ_TEST_FAULT_LINK_TARGET="$link_target" DJ_TEST_FAULT_GATE="$gate" \
        DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_REAL_STAT="$REAL_STAT" DJ_TEST_REAL_CHMOD="$REAL_CHMOD" \
        DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err"
      rc=$?
...
last=${!#}
case "$last" in
  ${DJ_TEST_CHMOD_MATCH:?}) exit 0 ;;
esac
exec "${DJ_TEST_REAL_CHMOD:?}" "$@"
SH
...
test_quarantine_validation_and_retry_contract() {
  ...
  set +e
  DJ_TEST_CHMOD_MATCH="$state/.pr-check-quarantine" DJ_TEST_REAL_CHMOD="$REAL_CHMOD" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
  ...
  install_chmod_noop_fault "$dir"
  set +e
  DJ_TEST_CHMOD_MATCH="$state/.pr-check-quarantine/task-a.check.*" DJ_TEST_REAL_CHMOD="$REAL_CHMOD" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
"${DJ_TEST_REAL_MV:?}" "$@" || exit $?
case "$last" in
  */.pr-check-quarantine/task-a.check.*) : > "${DJ_TEST_FAULT_GATE:?}" ;;
esac
SH
  cat > "$dir/fakebin/stat" <<'SH'
#!/usr/bin/env bash
...
SH
  chmod +x "$dir/fakebin/mv" "$dir/fakebin/stat"
  set +e
  DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_REAL_STAT="$REAL_STAT" DJ_TEST_FAULT_GATE="$dir/device-fault" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_REAL_CP="$REAL_CP" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_FAULT_LINK_TARGET="$external" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" >/dev/null 2>/dev/null
  rc=$?
  set -e
...
test_ambiguous_failure_accepts_validated_replacement() {
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" \
    || fail "replacement registration did not publish a valid poll pair"

  DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-retry.out" 2> "$dir/migrate-retry.err" \
  ...
  for case_name in copied-pair copied-registration metadata-mismatch task-mismatch forged-registration partial-publication; do
    ...
    case "$case_name" in
      ...
      copied-registration)
        ...
        dj_pr_poll_prepare "$donor" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare donor registration fixture"
        dj_pr_poll_publish_prepared || fail "could not publish donor registration fixture"
        ...
        ;;
      metadata-mismatch)
        dj_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare metadata-mismatch fixture"
        dj_pr_poll_publish_prepared || fail "could not publish metadata-mismatch fixture"
        write_poll_meta "$state" task-a https://github.com/o/r/pull/11
        ;;
      task-mismatch)
        dj_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/10 github.com o/r 10 "$POLL" \
          || fail "could not prepare task-mismatch fixture"
        dj_pr_poll_publish_prepared || fail "could not publish task-mismatch fixture"
      ...
      forged-registration)
        write_manual_poll_pair "$state"
        printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
          dj-pr-poll-registration-v2 task-a github https://github.com/o/r/pull/10 github.com o/r 10 \
    ...
    set +e
    DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/retry.out" 2> "$dir/retry.err"
    rc=$?
    set -e
...
test_complete_single_link_validation() {
  ...
  set +e
  DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" --checks-safe > "$dir/retry.out" 2> "$dir/retry.err"
  rc=$?
  set -e
  ...
  set +e
  DJ_HOME="$dir/home" "$REGISTER" custom > "$dir/register.out" 2> "$dir/register.err"
  rc=$?
  set -e
...
SH
  ...
  set +e
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$BASE_PATH" \
...
test_failed_outcomes_block_every_retry_until_repaired() {
  ...
  for classification in canonical ambiguous; do
    ...
    set +e
    DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
    rc=$?
    set -e
    ...
    printf '%s\n' dj-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
    chmod 0600 "$state/.pr-check-migration-v1"

    set +e
    DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err"
    rc=$?
    set -e
    ...
    rmdir "$state/task-a.pr-poll"
    DJ_HOME="$dir/home" PATH="$BASE_PATH" "$MIGRATE" > "$dir/migrate-3.out" 2> "$dir/migrate-3.err" \
    ...
    if [ "$classification" = canonical ]; then
      ...
      dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical repaired retry did not arm a valid poll pair"
    else
...
test_canonical_publication_failure_recovers_only_on_retry() {
  ...
  set +e
  DJ_TEST_FINAL_PATH="$destination" DJ_TEST_FINAL_ACTION=mode \
    DJ_TEST_FAULT_LINK_TARGET="$link_target" DJ_TEST_FAULT_GATE="$gate" \
    DJ_TEST_REAL_MV="$REAL_MV" DJ_TEST_REAL_STAT="$REAL_STAT" DJ_TEST_REAL_CHMOD="$REAL_CHMOD" \
    DJ_HOME="$dir/home" PATH="$dir/fakebin:$BASE_PATH" "$MIGRATE" > "$dir/migrate-1.out" 2> "$dir/migrate-1.err"
  rc=$?
  set -e
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical publication retry did not arm a valid pair"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  [ ! -e "$pending" ] && [ ! -L "$pending" ] \
...
test_obligation_namespace_compatibility() {
  ...
  dj_write_meta "$state/_noncanonical.meta" \
    'window=jarvis:dj-_noncanonical' \
...
SH
  ...
  DJ_HOME="$dir/home" "$MIGRATE" > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  printf 'legacy quarantined bytes\n' \
    > "$state/.pr-check-quarantine/_noncanonical.check.abc123"
  printf 'dj-pr-check-migration-scan-v1\n' > "$state/.pr-check-migration-scan-v1"
  printf 'dj-pr-check-migration-v1\n' > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-quarantine/"* \
    "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
  DJ_HOME="$dir/home" "$MIGRATE" --checks-safe > "$dir/migrate.out" 2> "$dir/migrate.err" \
  ...
  state="$dir/home/state"
  dj_write_meta "$state/foo.diagnostic.bar.meta" \
    'window=dj-foo.diagnostic.bar' \
  ...
  dj_pr_poll_artifacts_valid "$state" foo.diagnostic.bar "$POLL" \
...
test_nonexecuting_migration() {
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "canonical migration did not leave a validated armed poll"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  ...
  DJ_HOME="$dir/home" "$MIGRATE" > "$dir/migrate-2.out" 2> "$dir/migrate-2.err" \
  ...
  snap_before=$(state_snapshot "$state")
  DJ_HOME="$dir/home" "$MIGRATE" >/dev/null 2>/dev/null || fail "completed migration rerun failed"
  snap_after=$(state_snapshot "$state")
  [ "$snap_after" = "$snap_before" ] || fail "completed migration changed a later custom check"

  dir=$(make_case migration-x-linked)
  state="$dir/home/state"
  dj_write_meta "$state/task-x.meta" \
    'window=dj-task-x' \
    'pr=https://github.com/o/r/pull/12' \
  ...
  dj_pr_poll_artifacts_valid "$state" task-x "$POLL" || fail "X-linked migration did not arm a valid pair"
  ...
  state="$dir/home/state"
  dj_write_meta "$state/task-b.meta" \
    'window=dj-task-b' \
    'pr=https://github.com/o/r/pull/10' \
...
test_historical_x_shim_transition_matrix() {
  ...
  for marker_kind in unmarked completed safe-scan; do
    ...
    cat > "$dir/root/bin/dj-x-poll.sh" <<SH
#!/usr/bin/env bash
touch '$executed'
SH
    chmod 0700 "$dir/root/bin/dj-x-poll.sh"
    ...
    case "$marker_kind" in
      completed)
        ...
        ;;
      safe-scan)
        printf '%s\n' dj-pr-check-migration-scan-v1 > "$state/.pr-check-migration-scan-v1"
        chmod 0600 "$state/.pr-check-migration-scan-v1"
        ;;
    esac

    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" "$MIGRATE" >/dev/null 2> "$dir/migrate.err" \
...
SH
  ...
  set +e
  DJ_TEST_CHECK_INTERVAL=999999 DJ_TEST_WATCH_ROOT="$dir/root" \
...
SH
    ...
    set +e
    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" "$MIGRATE" --checks-safe \
...
test_direct_registration_refreshes_v1_x_shim() {
  ...
  for marker_kind in unmarked completed safe-scan; do
    ...
    state="$dir/home/state"
    shim="$state/x-watch.check.sh"
    dj_write_meta "$state/task-a.meta" 'window=dj-task-a'
    write_v1_x_shim "$shim" "$dir/home" "$dir/root"
    chmod 0755 "$shim"
    case "$marker_kind" in
      completed)
    ...
    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" "$MIGRATE" --checks-safe >/dev/null \
...
test_bootstrap_migrates_before_other_mutations() {
  ...
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
    "$ROOT/bin/dj-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err" \
...
test_bootstrap_isolates_incomplete_poll_migration() {
  ...
  dj_pr_poll_prepare "$state" z-healthy github https://github.com/o/r/pull/13 github.com o/r 13 "$POLL" \
    || fail "could not prepare healthy poll for migration isolation"
  dj_pr_poll_publish_prepared || fail "could not publish healthy poll for migration isolation"
  dj_write_meta "$state/secondmate-a.meta" \
    'window=jarvis:dj-secondmate-a' \
    'kind=secondmate' \
    'harness=codex' \
    'backend=tmux'
  printf 'DJX_PAIRING_TOKEN=test-token\n' > "$dir/home/.env"
  mkdir -p "$dir/home/projects"
  dj_fake_exit0 "$fakebin" curl jq
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
case " $* " in
  *' list-windows '*) printf 'dj-secondmate-a\n' ;;
  *' display-message '*) printf 'node\n' ;;
esac
SH
  cat > "$dir/root/bin/dj-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${DJ_TEST_FLEET_MARKER:?}"
printf 'alpha: recovered: continued after isolated migration failure\n'
SH
  cat > "$dir/root/bin/dj-x-poll.sh" <<'SH'
#!/usr/bin/env bash
: > "${DJ_TEST_X_POLL_MARKER:?}"
SH
  chmod +x "$fakebin/tmux" "$dir/root/bin/dj-fleet-sync.sh" "$dir/root/bin/dj-x-poll.sh"

  set +e
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" DJ_TEST_FLEET_MARKER="$fleet_marker" \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/dj-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err"
  rc=$?
  set -e
  ...
  set +e
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" DJ_TEST_X_POLL_MARKER="$x_poll_marker" \
    DJ_TEST_GH_STATE=MERGED DJ_POLL=0 DJ_CHECK_INTERVAL=0 DJ_SIGNAL_GRACE=0 \
  ...
  DJ_HOME="$dir/home" "$REGISTER" b-custom > "$dir/register.out" \
  ...
  set +e
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$dir/root" DJ_TEST_X_POLL_MARKER="$x_poll_marker" \
    DJ_TEST_GH_STATE=OPEN DJ_POLL=0 DJ_CHECK_INTERVAL=0 DJ_SIGNAL_GRACE=0 \
...
test_custom_snapshot_cleanup_on_signal() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'trap "" TERM' \
    'printf "%s\n" "$$" > "$DJ_TEST_CUSTOM_CHILD_PID"' 'while :; do sleep 1; done' \
    > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
...
SH
  ...
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" DJ_POLL=0 DJ_CHECK_INTERVAL=0 \
    DJ_SIGNAL_GRACE=0 DJ_TEST_CUSTOM_CHILD_PID="$child_pid_file" \
  ...
  find "$state" -maxdepth 1 -name '.dj-custom-check.*' -print | grep . >/dev/null \
  ...
  ! find "$state" -maxdepth 1 -name '.dj-check-output.*' -print | grep . >/dev/null \
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; open my $ready, ">", $ENV{DJ_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; select undef, undef, undef, 4; open my $sentinel, ">", $ENV{DJ_TEST_DESCENDANT_SENTINEL} or die $!; print {$sentinel} "late\n"; close $sentinel; select undef, undef, undef, 1' &
printf '%s\n' "$!" > "$DJ_TEST_DESCENDANT_PID"
while [ ! -s "$DJ_TEST_DESCENDANT_READY" ]; do sleep 0.01; done
: > "$DJ_TEST_DIRECT_DONE"
SH
    chmod 0700 "$state/custom.check.sh"
    DJ_HOME="$dir/home" "$REGISTER" custom >/dev/null \
...
SH
    ...
    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" DJ_POLL=0.1 DJ_CHECK_INTERVAL=999999 \
      DJ_CHECK_TIMEOUT=10 DJ_HEARTBEAT=999999 DJ_SIGNAL_GRACE=0 \
      DJ_CHECK_FORCE_FALLBACK="$force_fallback" DJ_TEST_DESCENDANT_READY="$ready" \
      DJ_TEST_DESCENDANT_SENTINEL="$sentinel" DJ_TEST_DESCENDANT_PID="$child_pid_file" \
      DJ_TEST_DIRECT_DONE="$direct_done" PATH="$fakebin:$BASE_PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    watcher_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
...
test_teardown_removes_poll_artifacts() {
  ...
  fakebin="$dir/fakebin"
  dj_write_meta "$dir/home/state/task-a.meta" \
    'window=jarvis:dj-task-a' \
    'endpoint_task_id=task-a' \
    "worktree=$dir/missing-worktree" \
    "project=$dir/project" \
...
SH
  ...
  dj_pr_poll_snapshot_capture "$dir/home/state" task-a "$POLL" \
    || fail "could not snapshot teardown receipt fixture"
  dj_pr_poll_retirement_publish "$dir/home/state" task-a "$POLL" merged \
...
SH
    ...
    touch "$dir/home/state/.last-watcher-beat"
    set +e
    DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" DJ_FAKE_TMUX_LOG="$dir/tmux.log" \
...
test_gitlab_merge_watch() {
  ...
  dj_pr_poll_prepare "$state" task-a gitlab "$url" gitlab.example group/subgroup/project 7 "$POLL" \
    || fail "could not prepare a GitLab poll"
  dj_pr_poll_publish_prepared || fail "could not publish a GitLab poll"
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" \
...
7" ] || fail "published GitLab sidecar bytes were not exact"
  ...
  # Only an exact merged state wakes jarvis. Every other reading, including
  ...
  for value in opened closed locked '' not-a-state MERGED merged-but-not; do
    out=$(DJ_TEST_GLAB_STATE="$value" run_poll "$dir")
    [ -z "$out" ] || fail "GitLab poll emitted for a non-merged state"
  done
  out=$(DJ_TEST_GLAB_STATE=merged run_poll "$dir")
  [ "$out" = merged ] || fail "GitLab poll did not emit exactly one merged line"
  out=$(DJ_TEST_GLAB_FAIL=1 run_poll "$dir")
...
EOF
  ...
  out=$(DJ_TEST_GLAB_STATE=merged DJ_TEST_GH_LOG="$dir/gh.log" DJ_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$noglab" \
    bash "$state/task-a.check.sh")
  ...
  set +e
  out=$(DJ_ROOT_OVERRIDE="$dir/root" DJ_HOME="$dir/home" \
    DJ_TEST_GUARD_LOG="$dir/guard.log" PATH="$noglab" \
...
seed_canonical_poll() {
  ...
  state="$dir/home/state"
  dj_pr_url_parse "$url" || fail "retirement fixture URL was invalid"
  provider=$DJ_PR_PROVIDER
  host=$DJ_PR_HOST
  path=$DJ_PR_PATH
  number=$DJ_PR_NUMBER
  dj_pr_poll_prepare "$state" "$id" "$provider" "$url" "$host" "$path" "$number" "$template" \
    || fail "could not prepare retirement fixture"
  dj_pr_poll_publish_prepared || fail "could not publish retirement fixture"
  printf '%s\n' dj-pr-check-migration-scan-v1 > "$state/.pr-check-migration-scan-v1"
  printf '%s\n' dj-pr-check-migration-v1 > "$state/.pr-check-migration-v1"
  chmod 0600 "$state/.pr-check-migration-scan-v1" "$state/.pr-check-migration-v1"
}
...
add_stop_custom_check() {
  ...
  DJ_HOME="$dir/home" "$REGISTER" z-stop >/dev/null \
...
test_merged_poll_retires_once() {
  ...
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-1.out" 2> "$dir/watch-1.err"
  rc=$?
  set -e
  ...
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-2.out" 2> "$dir/watch-2.err"
  rc=$?
  set -e
...
# surfaced (e.g. bin/dj-pr-check.sh re-armed after the fact). The per-task
# merge-notified marker (bin/dj-pr-lib.sh) is what stops that re-registration
...
test_merged_poll_reregistration_after_notification_is_absorbed() {
  ...
  # Re-registration: dj-pr-check.sh re-armed for a task whose PR is already
...
test_different_merged_pr_for_same_task_is_not_absorbed() {
  ...
  dj_pr_poll_merge_already_notified "$state" task-a github github.com o/r 2 \
    || fail "the marker was not advanced to the different PR identity"
  ! dj_pr_poll_merge_already_notified "$state" task-a github github.com o/r 1 \
...
test_persistent_secondmate_retirement_is_poll_only() {
  ...
  state="$dir/home/state"
  dj_write_meta "$state/domain.meta" \
    'window=session:dj-domain' \
...
test_retirement_crash_recovery() {
  ...
  DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; dj_wake_append check "$2" "$3"' _ \
    "$ROOT/bin/dj-wake-lib.sh" "$state/task-a.check.sh" "check: $state/task-a.check.sh: merged" \
    || fail "could not seed post-queue crash"
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/recovery.out" 2> "$dir/recovery.err" \
  ...
  DJ_HOME="$dir/home" DJ_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/dj-wake-drain.sh" > "$dir/drain.out" 2>/dev/null
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt crash fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish crash receipt"
  add_stop_custom_check "$dir"
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  rc=$?
  set -e
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot partial removal fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish partial removal receipt"
  rm -f "$state/task-a.check.sh"
  add_stop_custom_check "$dir"
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  ...
  assert_poll_absent "$state" task-a
  dj_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "completed retirement was not idempotent"

  dir=$(make_case retirement-after-registration-removal)
  state="$dir/home/state"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot sidecar-removal fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish sidecar-removal receipt"
  ...
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt-only fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt-only fixture"
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-dj-pr-poll.sh"
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$historical_poll" \
    || fail "could not snapshot pre-update retirement fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$historical_poll" merged \
    || fail "could not publish pre-update retirement receipt"
  add_stop_custom_check "$dir"
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/template-recovery.out" 2> "$dir/template-recovery.err" \
...
test_external_merge_transition_retires_only_terminal_poll() {
  ...
  for label in open-green open-red closed-unmerged forge-error malformed; do
    ...
    case "$label" in
      open-green|open-red)
        DJ_TEST_GH_STATE=OPEN run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      closed-unmerged)
        DJ_TEST_GH_STATE=CLOSED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      forge-error)
        DJ_TEST_GH_FAIL=1 run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      malformed)
        DJ_TEST_GH_STATE=NOT_A_FORGE_STATE run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
    esac
    rc=$?
    set -e
  ...
  set +e
  DJ_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged.out" 2> "$dir/merged.err"
  rc=$?
  set -e
...
test_retirement_refuses_replacement_and_nonterminal_results() {
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot replacement fixture"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/7
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/7
  before=$(state_snapshot "$state")
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged \
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll was not left canonical"

  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot nonterminal fixture"
  for result in OPEN CLOSED ERROR 'merged extra'; do
    dj_pr_poll_retirement_publish "$state" task-a "$POLL" "$result" \
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot rearm-race fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish rearm-race receipt"
  ...
  dj_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "stale receipt did not yield to a canonical replacement"
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll lost canonical provenance"

  dir=$(make_case retirement-template-update-rearm-race)
  state="$dir/home/state"
  historical_poll="$dir/historical-dj-pr-poll.sh"
  current_poll="$dir/current-dj-pr-poll.sh"
  cp "$POLL" "$historical_poll"
  ...
  replacement_registration=$(shasum -a 256 "$state/task-a.pr-poll-registration")
  replacement_meta=$(shasum -a 256 "$state/task-a.meta")
  dj_pr_poll_retirement_recover_one "$state" task-a "$current_poll" \
  ...
  dj_pr_poll_artifacts_valid "$state" task-a "$current_poll" || fail "updated replacement poll lost canonical provenance"

  dir=$(make_case custom-merged-not-retired)
  state="$dir/home/state"
  printf '#!/usr/bin/env bash\nprintf "merged\\n"\n' > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
  DJ_HOME="$dir/home" "$REGISTER" custom >/dev/null || fail "could not register merged custom check"
  set +e
...
test_retirement_queue_failure_and_receipt_tampering() {
  ...
  before=$(poll_artifact_snapshot "$state" task-a)
  set +e
  DJ_TEST_GH_LOG="$dir/gh.log" DJ_TEST_GH_STATE=MERGED \
    run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  ...
  dj_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt tamper fixture"
  dj_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt tamper fixture"
  printf 'extra\n' >> "$state/task-a.pr-poll-retirement"
  before=$(state_snapshot "$state")
  dj_pr_poll_retirement_recover_one "$state" task-a "$POLL" \
...
test_gitlab_merged_poll_retires() {
  ...
  set +e
  DJ_TEST_GLAB_STATE=merged run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
