#!/usr/bin/env bash
# Behavior tests for bin/dj-crew-state.sh - the deterministic crew-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. dj-crew-state
...
#       fake dj-crew-state.sh verdict): cross-branch attribution via the runs
...
CREW_STATE="$ROOT/bin/dj-crew-state.sh"
TMP_ROOT=$(dj_test_tmproot dj-crew-state)
...
make_repo_on_branch() {  # <dir> <branch>
  ...
  # Real worktree HEAD for run head-binding (fixtures read DJ_FAKE_RUN_HEAD).
  DJ_FAKE_RUN_HEAD=$(git -C "$dir" rev-parse HEAD)
  export DJ_FAKE_RUN_HEAD
}
...
case "${1:-}" in
  axi)
    ...
    case "${1:-}" in
      status)
        shift
        if [ "${1:-}" = --run ]; then printf '%s\n' "${DJ_FAKE_AXI_STATUS_RUN:-}"
        else printf '%s\n' "${DJ_FAKE_AXI_STATUS:-}"; fi ;;
      logs)
        printf '%s\n' "${DJ_FAKE_CI_LOGS:-}" ;;
    esac
    ;;
...
case "${1:-}" in
  display-message)
    [ "${DJ_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    printf '%%1\n' ;;
  capture-pane)
    [ "${DJ_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${DJ_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\n%s\n' "${DJ_FAKE_BUSY_TEXT:-esc to interrupt}"
...
case "${1:-}" in
  ...
  pane)
    case "${2:-}" in
      read)
        [ "${DJ_FAKE_HERDR_MISSING:-0}" = 1 ] && exit 1
        if [ "${DJ_FAKE_HERDR_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
        else printf 'all quiet\n> \n'; fi
        exit 0 ;;
    esac ;;
  agent)
    case "${2:-}" in
      get)
        [ -n "${DJ_FAKE_HERDR_AGENT_STATUS:-}" ] || exit 1
        printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$DJ_FAKE_HERDR_AGENT_STATUS"
        exit 0 ;;
    esac ;;
esac
exit 0
...
# Run the helper for one case dir. DJ_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" DJ_STATE_OVERRIDE="$1/state" "$CREW_STATE" "$2"
}
...
arm_idle_record() {  # <state-dir> <id>
  ...
  "$ROOT/bin/dj-busy-event.sh" apply "$state" "$id" idle --gen "$gen" \
...
reset_fakes() {
  ...
  DJ_FAKE_BUSY=0
  DJ_FAKE_BUSY_TEXT=
  DJ_FAKE_TMUX_MISSING=0
  DJ_FAKE_HERDR_BUSY=0
  DJ_FAKE_HERDR_MISSING=0
  DJ_FAKE_HERDR_AGENT_STATUS=""
  DJ_FAKE_CI_LOGS=""
  export DJ_FAKE_AXI_STATUS DJ_FAKE_AXI_STATUS_RUN DJ_FAKE_RUNS_LIST DJ_FAKE_BUSY DJ_FAKE_BUSY_TEXT DJ_FAKE_TMUX_MISSING
  export DJ_FAKE_HERDR_BUSY DJ_FAKE_HERDR_MISSING DJ_FAKE_HERDR_AGENT_STATUS DJ_FAKE_CI_LOGS
}
...
test_active_run_is_authoritative() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-a
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-a.meta" "window=fm:dj-feat-a" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_running dj/feat-a)"
...
test_stale_needs_decision_superseded() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-b
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-b.meta" "window=fm:dj-feat-b" "worktree=$d/wt" "kind=ship"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  DJ_FAKE_AXI_STATUS="$(run_fixing dj/feat-b)"
...
test_stale_blocked_superseded() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-bb
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-bb.meta" "window=fm:dj-feat-bb" "worktree=$d/wt" "kind=ship"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  DJ_FAKE_AXI_STATUS="$(run_running dj/feat-bb)"
...
test_genuine_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-c
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-c.meta" "window=fm:dj-feat-c" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  DJ_FAKE_AXI_STATUS="$(run_parked dj/feat-c)"
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cs
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cs.meta" "window=fm:dj-feat-cs" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  DJ_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running dj/feat-cs)"
...
test_gate_block_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cb
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cb.meta" "window=fm:dj-feat-cb" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  DJ_FAKE_AXI_STATUS="$(run_parked_in_gate_block dj/feat-cb)"
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-ci
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-ci.meta" "window=fm:dj-feat-ci" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-ci)"
...
# merge/close. dj-crew-state must surface this as done, not "validating
# (running)", so a green PR is never silently absorbed as still-in-progress.
test_ci_monitoring_checks_green_surfaces_done() {
  reset_fakes
  local d; d=$(new_case ci-green)
  make_repo_on_branch "$d/wt" dj/feat-cigreen
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cigreen.meta" "window=fm:dj-feat-cigreen" "worktree=$d/wt" "kind=ship"
  # No status-log line at all: the crew never reported its own checks-green line.
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cigreen)"
  DJ_FAKE_CI_LOGS=$(cat <<'EOF'
...
test_top_level_ci_checks_green_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-topcigreen
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-topcigreen.meta" "window=fm:dj-feat-topcigreen" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_top_level_ci dj/feat-topcigreen)"
  DJ_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_ci_monitoring_no_checks_terminal_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cinochecks
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cinochecks.meta" "window=fm:dj-feat-cinochecks" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cinochecks)"
  DJ_FAKE_CI_LOGS="no CI checks reported - still monitoring until merged or closed"
...
test_ci_monitoring_green_then_rearm_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cirearm
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cirearm.meta" "window=fm:dj-feat-cirearm" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cirearm)"
  DJ_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
...
test_ci_monitoring_no_checks_yet_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cinochecksyet
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cinochecksyet.meta" "window=fm:dj-feat-cinochecksyet" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cinochecksyet)"
  DJ_FAKE_CI_LOGS=$(cat <<'EOF'
...
test_ci_monitoring_still_waiting_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-ciwait
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-ciwait.meta" "window=fm:dj-feat-ciwait" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-ciwait)"
  DJ_FAKE_CI_LOGS="CI checks running, waiting for results..."
...
test_ci_monitoring_green_then_new_issue_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cirelapse
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cirelapse.meta" "window=fm:dj-feat-cirelapse" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cirelapse)"
  DJ_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
...
test_ci_ready_done_log_relapse_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cireadyrelapse
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cireadyrelapse.meta" "window=fm:dj-feat-cireadyrelapse" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cireadyrelapse.status"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/feat-cireadyrelapse)"
  DJ_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
CI checks running, waiting for results...
...
test_ci_fixing_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-cifixing
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-cifixing.meta" "window=fm:dj-feat-cifixing" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cifixing.status"
  DJ_FAKE_AXI_STATUS="$(run_ci_fixing dj/feat-cifixing)"
  DJ_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_top_level_fixing_ci_running_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-topfixingci
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-topfixingci.meta" "window=fm:dj-feat-topfixingci" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_fixing_ci_running dj/feat-topfixingci)"
  DJ_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_top_level_fixing_done_log_stays_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-topfixing
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-topfixing.meta" "window=fm:dj-feat-topfixing" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-topfixing.status"
  DJ_FAKE_AXI_STATUS="$(run_fixing dj/feat-topfixing)"
  DJ_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_terminal_passed() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-d
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-d.meta" "window=fm:dj-feat-d" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_passed dj/feat-d)"
...
test_terminal_failed() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-e
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-e.meta" "window=fm:dj-feat-e" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_failed dj/feat-e)"
...
test_cross_branch_attribution_via_runs_list() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-f
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-f.meta" "window=fm:dj-feat-f" "worktree=$d/wt" "kind=ship"
  # The repo-wide active/most-recent run belongs to a different crew's branch.
  DJ_FAKE_AXI_STATUS="$(run_running dj/other-crew)"
  # Real `no-mistakes runs` shape: plain text, newest-first, no run id, no
  # quoting - "<status> <branch> <short-sha> <date> [<pr-url>]".
  DJ_FAKE_RUNS_LIST="$(cat <<EOF
  running    dj/other-crew aaaaaaa  2026-07-02 22:10
  running    dj/feat-f ${short}  2026-07-02 22:05
EOF
)"
...
test_cross_branch_attribution_picks_most_recent_row() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-fq
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-fq.meta" "window=fm:dj-feat-fq" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_running dj/other-crew)"
  DJ_FAKE_RUNS_LIST="$(cat <<EOF
  running    dj/other-crew aaaaaaa  2026-07-02 22:10
  running    dj/feat-fq ${short}  2026-07-02 21:50
  completed  dj/feat-fq bbbbbbb  2026-07-02 20:00  https://github.com/o/r/pull/1
EOF
)"
...
test_coarse_run_does_not_probe_other_branch_ci_log_for_ready_status() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-coarseready
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-coarseready.meta" "window=fm:dj-feat-coarseready" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/4 checks green\n' > "$d/state/feat-coarseready.status"
  DJ_FAKE_AXI_STATUS="$(run_ci_monitoring dj/other-crew)"
  DJ_FAKE_RUNS_LIST="$(cat <<EOF
  running    dj/other-crew aaaaaaa  2026-07-02 22:10
  running    dj/feat-coarseready ${short}  2026-07-02 22:05
EOF
)"
  DJ_FAKE_CI_LOGS="CI checks running, waiting for results..."
...
test_other_branch_run_ignored() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-g
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-g.meta" "window=fm:dj-feat-g" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  DJ_FAKE_AXI_STATUS="$(run_running dj/some-other)"
  DJ_FAKE_RUNS_LIST="$(cat <<'EOF'
  running    dj/some-other aaaaaaa  2026-07-02 22:10
EOF
)"
  DJ_FAKE_BUSY=0
...
test_no_run_busy_pane() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-h
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-h.meta" "window=fm:dj-feat-h" "worktree=$d/wt" "kind=ship" "harness=claude"
  # No matching run anywhere. The busy verdict comes from the crew's own
  # semantic lifecycle record (bin/dj-busy-lib.sh), not from rendered text.
  DJ_FAKE_AXI_STATUS=""
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$d/state" feat-h)
  "$ROOT/bin/dj-busy-event.sh" apply "$d/state" feat-h busy --gen "$gen" \
...
test_no_run_footer_text_alone_is_not_working() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-h2
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-h2.meta" "window=fm:dj-feat-h2" "worktree=$d/wt" "kind=ship" "harness=claude"
  DJ_FAKE_AXI_STATUS=""
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_BUSY=1
...
test_no_run_grok_uses_isolated_fallback() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-h3
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-h3.meta" "window=fm:dj-feat-h3" "worktree=$d/wt" "kind=ship" "harness=grok"
  DJ_FAKE_AXI_STATUS=""
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_BUSY=1
  DJ_FAKE_BUSY_TEXT='Ctrl+c:cancel'
  export DJ_FAKE_BUSY_TEXT
...
test_no_run_herdr_unknown_uses_backend_capture() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-herdr
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-herdr.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  DJ_FAKE_AXI_STATUS=""
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_TMUX_MISSING=1
  DJ_FAKE_HERDR_BUSY=1
  DJ_FAKE_HERDR_AGENT_STATUS=working
...
test_no_run_herdr_idle_agent_status_outranked_by_record() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-herdr-idle
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-herdr-idle.meta" "window=default:w1:p3" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  ...
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_TMUX_MISSING=1
  DJ_FAKE_HERDR_AGENT_STATUS=idle
  DJ_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$d/state" feat-herdr-idle)
  "$ROOT/bin/dj-busy-event.sh" apply "$d/state" feat-herdr-idle busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_no_run_herdr_idle_agent_status_and_idle_record_stays_idle() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-herdr-stopped
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-herdr-stopped.meta" "window=default:w1:p4" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  ...
  DJ_FAKE_HERDR_AGENT_STATUS=idle
  DJ_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$d/state" feat-herdr-stopped)
  "$ROOT/bin/dj-busy-event.sh" apply "$d/state" feat-herdr-stopped idle --gen "$gen" \
    --source claude-hook --event stop
...
test_no_run_idle_pane_uses_log() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-i
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-i.meta" "window=fm:dj-feat-i" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_uses_keyed_log() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-keyed
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-keyed.meta" "window=fm:dj-feat-keyed" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_paused() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-pause
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-pause.meta" "window=fm:dj-feat-pause" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_custom_paused_verb() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-custom-pause
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-custom-pause.meta" "window=fm:dj-feat-custom-pause" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'awaiting: vendor maintenance window\n' > "$d/state/feat-custom-pause.status"
  DJ_FAKE_AXI_STATUS=""
  DJ_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-custom-pause
  local out; out=$(DJ_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
  ...
  out=$(DJ_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
...
# state=unknown with resolution prose. The one-owner keyed fold in dj-classify-lib.sh
...
test_no_run_idle_secondmate_resolved_event_not_state() {
  ...
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/mate.meta" "window=fm:dj-mate" "worktree=$d/wt" "kind=secondmate" "home=$d/wt"
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-dead
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-dead.meta" "window=fm:dj-feat-dead" "worktree=$d/wt" "kind=ship"
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-dead-done
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-dead-done.meta" "window=fm:dj-feat-dead-done" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  DJ_FAKE_AXI_STATUS="$(run_passed dj/feat-dead-done)"
  DJ_FAKE_TMUX_MISSING=1   # the crew's window has closed
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-dead-act
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-dead-act.meta" "window=fm:dj-feat-dead-act" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_running dj/feat-dead-act)"
  DJ_FAKE_TMUX_MISSING=1
...
test_no_timeout_uses_perl_bound() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-timeout
  make_fakebin "$d" >/dev/null
  calls_file="$d/no-mistakes.calls"
  : > "$calls_file"
  cat > "$d/fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${DJ_FAKE_NM_CALLS:-/dev/null}"
while :; do :; done
SH
  chmod +x "$d/fakebin/no-mistakes"
  toolbin=$(make_no_timeout_toolbin "$d")
  dj_write_meta "$d/state/feat-timeout.meta" "window=fm:dj-feat-timeout" "worktree=$d/wt" "kind=ship" \
    "harness=claude"
  DJ_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$d/state" feat-timeout)
  "$ROOT/bin/dj-busy-event.sh" apply "$d/state" feat-timeout busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  start=$SECONDS
  out=$(DJ_FAKE_NM_CALLS="$calls_file" PATH="$d/fakebin:$toolbin" DJ_STATE_OVERRIDE="$d/state" DJ_CREW_STATE_NM_TIMEOUT=1 "$CREW_STATE" feat-timeout)
...
test_scout_skips_run_lookup() {
  ...
  make_repo_on_branch "$d/wt" dj/scout-j
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/scout-j.meta" "window=fm:dj-scout-j" "worktree=$d/wt" "kind=scout" \
    "harness=claude"
  # Even if a run existed on this branch, a scout must not read it.
  DJ_FAKE_AXI_STATUS="$(run_running dj/scout-j)"
  DJ_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/dj-busy-event.sh" arm "$d/state" scout-j)
  "$ROOT/bin/dj-busy-event.sh" apply "$d/state" scout-j busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/gone-k.meta" "window=fm:dj-gone-k" "worktree=$d/no-such-worktree" "kind=ship"
  local out rc
...
# "worktree gone". These cases drive the real helper over the real dj-on.sh
# route with a stubbed ssh transport (DJ_SSH_BIN seam): the stub prints
# DJ_FAKE_REMOTE_STATE_OUT as the remote endpoint's recovery-grade state and
# exits DJ_FAKE_SSH_RC.

setup_remote_case() {  # <name> -> echoes case dir with remote meta + registry
  local d
  d=$(new_case "$1")
  mkdir -p "$d/data" "$d/fakebin"
  dj_write_meta "$d/state/rsm.meta" \
    "window=remote:rsm" \
    ...
    "remote_herdr_session=dj-remote" \
    "remote_target=dj-remote:w1:p1"
...
#!/usr/bin/env bash
cat > /dev/null
[ -z "${DJ_FAKE_REMOTE_STATE_OUT:-}" ] || printf '%s\n' "$DJ_FAKE_REMOTE_STATE_OUT"
exit "${DJ_FAKE_SSH_RC:-0}"
SH
...
run_remote_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" DJ_HOME="$1" DJ_STATE_OVERRIDE="$1/state" \
    DJ_SSH_BIN="$1/fakebin/fake-ssh" "$CREW_STATE" "$2"
}
...
test_remote_alive_with_log_uses_status_log() {
  ...
  out=$(DJ_FAKE_REMOTE_STATE_OUT=alive DJ_FAKE_SSH_RC=0 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "dj-crew-state remote: alive endpoint falls through to the routed status log"
}
...
test_remote_alive_idle_is_healthy_not_gone() {
  ...
  pass "dj-crew-state remote: an idle alive endpoint reads alive, never gone or dead"
}
...
test_remote_unreachable_is_unknown_remote_not_dead() {
  ...
  make_fakebin "$d" >/dev/null
  printf 'working: refactoring the quota adapter\n' > "$d/state/rsm.status"
  out=$(DJ_FAKE_SSH_RC=255 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "dj-crew-state remote: an unreachable host reads unknown-remote, never gone or dead"
}
...
test_remote_dead_reports_remote_verdict() {
  ...
  make_fakebin "$d" >/dev/null
  out=$(DJ_FAKE_REMOTE_STATE_OUT=dead DJ_FAKE_SSH_RC=0 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "dj-crew-state remote: the remote host's own dead verdict is reported truthfully"
}
...
# (k) crew_is_provably_working end-to-end over the REAL dj-crew-state.sh (not a
# canned fake verdict, unlike tests/dj-watch-triage.test.sh's classifier
...
test_provably_working_via_runs_list_fallback() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-provable
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-provable.meta" "window=fm:dj-feat-provable" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_AXI_STATUS="$(run_running dj/other-crew)"
  DJ_FAKE_RUNS_LIST="$(cat <<EOF
  running    dj/other-crew aaaaaaa  2026-07-02 22:10
  running    dj/feat-provable ${short}  2026-07-02 22:05
EOF
)"
  PATH="$d/fakebin:$PATH" DJ_STATE_OVERRIDE="$d/state" crew_is_provably_working feat-provable \
...
test_not_provably_working_when_stopped() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-stopped
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/feat-stopped.meta" "window=fm:dj-feat-stopped" "worktree=$d/wt" "kind=ship"
...
EOF
  ...
  DJ_FAKE_BUSY=0
  PATH="$d/fakebin:$PATH" DJ_STATE_OVERRIDE="$d/state" crew_is_provably_working feat-stopped \
...
test_historical_same_branch_rewritten_head_not_current() {
  ...
  make_repo_on_branch "$d/wt" dj/todo-flag
  ...
  git -C "$d/wt" branch -q -M dj/todo-flag
  new_head=$(git -C "$d/wt" rev-parse HEAD)
  [ "$old_head" != "$new_head" ] || fail "rewrite did not produce a new head"
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/wishlist.meta" "window=fm:dj-wishlist" "worktree=$d/wt" "kind=ship" "harness=claude"
  ...
  DJ_FAKE_RUN_HEAD="$old_head"
  DJ_FAKE_AXI_STATUS="$(run_parked dj/todo-flag)"
  DJ_FAKE_BUSY=0
...
test_active_run_descendant_fix_head_remains_current() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-pipeline
  ...
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/pipe.meta" "window=fm:dj-pipe" "worktree=$d/wt" "kind=ship"
  DJ_FAKE_RUN_HEAD="$fix_head"
  DJ_FAKE_AXI_STATUS="$(run_fixing dj/feat-pipeline)"
...
test_local_advanced_past_run_head_invalidates() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-adv
  run_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" commit -q --allow-empty -m 'local stage-2 work after prior run'
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/adv.meta" "window=fm:dj-adv" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: stage 2 implementation in progress\n' > "$d/state/adv.status"
  DJ_FAKE_RUN_HEAD="$run_head"
  DJ_FAKE_AXI_STATUS="$(run_parked dj/feat-adv)"
  DJ_FAKE_BUSY=0
...
test_missing_run_head_falls_back_to_current_state() {
  ...
  make_repo_on_branch "$d/wt" dj/feat-no-head
  make_fakebin "$d" >/dev/null
  dj_write_meta "$d/state/no-head.meta" "window=fm:dj-no-head" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: current stage still in progress\n' > "$d/state/no-head.status"
  DJ_FAKE_AXI_STATUS=$(run_parked dj/feat-no-head | grep -v '^  head:')
  DJ_FAKE_RUNS_LIST=""
  DJ_FAKE_BUSY=0
...
echo "all dj-crew-state tests passed"
