#!/usr/bin/env bash
# tests/dj-trace-context-spawn.test.sh - spawn-path integration regressions for
...
TMP_ROOT=$(dj_test_tmproot dj-trace-context-spawn)
...
case "${1:-}" in
  ...
  list-windows)
    [ -z "${DJ_FAKE_DUPLICATE_WINDOW:-}" ] || printf '%s\n' "$DJ_FAKE_DUPLICATE_WINDOW"
    exit 0
    ;;
  has-session|new-session|new-window|kill-window) exit 0 ;;
  send-keys)
    if [ "${DJ_FAKE_TRACEPARENT_SEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${DJ_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${DJ_FAKE_TRACE_METADATA_APPEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
          "export TRACEPARENT="*)
            chmod a-w "$DJ_FAKE_META_PATH"
            ;;
        esac
      done
    fi
...
# Hermetic against an ambient DJ_TRACE_CONTEXT: `env -u` unsets it so enablement
...
run_spawn() {
  ...
  env -u DJ_TRACE_CONTEXT \
    ...
    DJ_FAKE_TRACEPARENT_SEND_FAIL="${DJ_FAKE_TRACEPARENT_SEND_FAIL:-0}" \
    DJ_FAKE_TRACEPARENT_SEND_UNSAFE="${DJ_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" \
    DJ_FAKE_TRACE_METADATA_APPEND_FAIL="${DJ_FAKE_TRACE_METADATA_APPEND_FAIL:-0}" \
    DJ_FAKE_META_PATH="$home/state/$1.meta" \
    DJ_FAKE_LAUNCH_LOG="$launchlog" PATH="$fakebin:$PATH" \
    "$SPAWN" "$@" --mode no-mistakes --yolo off 2>&1
}

# Same, but with an explicit DJ_TRACE_CONTEXT override, to prove the env decides.
run_spawn_tc() {
  local tc=$1 home=$2 wt=$3 fakebin=$4 launchlog=$5
  shift 5
  : > "$launchlog"
  env DJ_TRACE_CONTEXT="$tc" \
    DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_PROJECTS_OVERRIDE="$home/projects" DJ_CONFIG_OVERRIDE="$home/config" \
    DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$wt" TMUX="fake,1,0" \
...
start_trace_session() {
  ...
  printf '%s\n' "$$" > "$home/state/.lock"
  if [ -n "$tc" ]; then
    DJ_TRACE_CONTEXT="$tc" dj_trace_context_session_start \
      "$home/config" "$home/state/.trace-context-effective"
  else
    (
      unset DJ_TRACE_CONTEXT
      dj_trace_context_session_start \
        "$home/config" "$home/state/.trace-context-effective"
    )
  fi
}
...
# Two-level primary -> secondmate -> worker regression for the DJ_TRACE_CONTEXT
# effective override. Drives bin/dj-spawn.sh TWICE against real homes and a real
...
run_two_level() {
  ...
  # Seed the secondmate home so validate_jarvis_home_for_spawn accepts it.
  mkdir -p "$sm/bin" "$sm/data"
  printf '# Jarvis\n' > "$sm/AGENTS.md"
  printf 'sm-%s\n' "$name" > "$sm/.dj-secondmate-home"
  printf 'charter\n' > "$sm/data/charter.md"
  ...
  : > "$smlog"
  env DJ_TRACE_CONTEXT="$penv" \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$prim" \
    DJ_STATE_OVERRIDE="$prim/state" DJ_DATA_OVERRIDE="$prim/data" \
    DJ_PROJECTS_OVERRIDE="$prim/projects" DJ_CONFIG_OVERRIDE="$prim/config" \
    DJ_SPAWN_NO_GUARD=1 CLAUDECODE=1 TMUX="fake,1,0" \
    DJ_FAKE_LAUNCH_LOG="$smlog" PATH="$smfake:$PATH" \
    "$SPAWN" "$sm_id" "$sm" --secondmate >/dev/null 2>&1 || true

  # Extract the EXACT env the primary put on the secondmate: the normalized
  # DJ_TRACE_CONTEXT in the launch prefix, and the TRACEPARENT carrier (if any).
  TL_ENV_TC=$(grep -o 'DJ_TRACE_CONTEXT=[a-z]*' "$smlog" | head -1 | cut -d= -f2)
  ...
  wproj="$base/wproj"
  wwt="$base/wwt"
  dj_git_worktree "$wproj" "$wwt" "wt-$name"
  ...
  : > "$wlog"
  env DJ_TRACE_CONTEXT="$TL_ENV_TC" TRACEPARENT="$TL_CARRIER" \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$sm" \
    DJ_STATE_OVERRIDE="$sm/state" DJ_DATA_OVERRIDE="$sm/data" \
    DJ_PROJECTS_OVERRIDE="$sm/projects" DJ_CONFIG_OVERRIDE="$sm/config" \
    DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$wwt" TMUX="fake,1,0" \
    DJ_FAKE_LAUNCH_LOG="$wlog" PATH="$wfake:$PATH" \
...
test_enabled_records_and_injects_identical_carrier_before_launch() {
  ...
  dj_trace_context_valid "$mtp" || fail "enabled spawn must record a valid traceparent= in meta (got '$mtp')"
  itp=$(injected_traceparent "$LAUNCH_LOG")
  dj_trace_context_valid "$itp" || fail "enabled spawn must inject a valid TRACEPARENT export (got '$itp')"
...
test_disabled_writes_and_injects_neither() {
  ...
  read_case_record "$rec"
  # No config/trace-context and no DJ_TRACE_CONTEXT: default-off.
...
test_failed_delivery_omits_metadata_and_still_launches() {
  ...
  out=$(DJ_FAKE_TRACEPARENT_SEND_FAIL=1 \
...
test_unsafe_delivery_refuses_to_append_launch() {
  ...
  out=$(DJ_FAKE_TRACEPARENT_SEND_UNSAFE=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_failed_metadata_append_unsets_carrier_and_still_launches() {
  ...
  out=$(DJ_FAKE_TRACE_METADATA_APPEND_FAIL=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_duplicate_secondmate_spawn_does_not_converge_trace_context() {
  ...
  mkdir -p "$sm/bin" "$sm/data"
  printf '# Jarvis\n' > "$sm/AGENTS.md"
  printf '%s\n' "$id" > "$sm/.dj-secondmate-home"
  printf 'charter\n' > "$sm/data/charter.md"
  fake=$(make_spawn_fakebin "$base/fake")

  out=$(env -u DJ_TRACE_CONTEXT \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$prim" \
    DJ_STATE_OVERRIDE="$prim/state" DJ_DATA_OVERRIDE="$prim/data" \
    DJ_PROJECTS_OVERRIDE="$prim/projects" DJ_CONFIG_OVERRIDE="$prim/config" \
    DJ_SPAWN_NO_GUARD=1 CLAUDECODE=1 TMUX="fake,1,0" \
    DJ_FAKE_DUPLICATE_WINDOW="dj-$id" DJ_FAKE_LAUNCH_LOG="$log" \
...
test_relaunch_reuses_recorded_carrier() {
  ...
  dj_trace_context_valid "$first" || fail "first spawn must record a valid carrier (got '$first')"
...
test_session_start_freezes_env_override_and_ignores_later_edits() {
  ...
  meta="$HOME_DIR/state/$CASE_ID.meta"
  ! grep -q '^traceparent=' "$meta" || fail "session-frozen off must ignore a later DJ_TRACE_CONTEXT=on"
  ...
  meta="$HOME_DIR/state/$CASE_ID.meta"
  dj_trace_context_valid "$(meta_traceparent "$meta")" \
    || fail "session-frozen on must ignore a later DJ_TRACE_CONTEXT=off"
...
test_secondmate_env_on_file_absent_keeps_nested_worker_enabled() {
  ...
  [ "$TL_ENV_TC" = on ] || fail "the primary must deliver DJ_TRACE_CONTEXT=on to the secondmate (got '$TL_ENV_TC')"
  dj_trace_context_valid "$TL_CARRIER" || fail "an enabled primary must mint a carrier for the secondmate (got '$TL_CARRIER')"
  dj_trace_context_valid "$TL_WORKER_TP" \
...
# DJ_TRACE_CONTEXT=off is not a real kill switch. Before the fix the copied file
...
test_secondmate_env_off_file_present_keeps_nested_worker_disabled() {
  ...
  [ "$TL_ENV_TC" = off ] || fail "the primary must deliver DJ_TRACE_CONTEXT=off to the secondmate (got '$TL_ENV_TC')"
...
test_two_routed_tasks_through_one_secondmate_root_distinct_traces() {
  ...
  dj_git_worktree "$proj_a" "$wt_a" wt-routed-a
  dj_git_worktree "$proj_b" "$wt_b" wt-routed-b
  ...
  dj_trace_context_valid "$tp_a" || fail "routed task A must record a valid carrier (got '$tp_a')"
  dj_trace_context_valid "$tp_b" || fail "routed task B must record a valid carrier (got '$tp_b')"
...
# carrier and the delivered DJ_TRACE_CONTEXT snapshot are always derived from ONE
...
# (DJ_TRACE_CONTEXT unset), which is exactly where the two-read correction matters
...
test_secondmate_carrier_and_snapshot_share_one_decision() {
  ...
  [ "$TL_ENV_TC" = on ] || fail "a file-enabled secondmate must snapshot DJ_TRACE_CONTEXT=on (got '$TL_ENV_TC')"
  dj_trace_context_valid "$TL_CARRIER" \
  ...
  [ "$TL_ENV_TC" = off ] || fail "a file-disabled secondmate must snapshot DJ_TRACE_CONTEXT=off (got '$TL_ENV_TC')"
  [ -z "$TL_CARRIER" ] \
    || fail "a file-disabled secondmate must inject no carrier, consistent with the off snapshot (got '$TL_CARRIER')"
  pass "secondmate carrier and DJ_TRACE_CONTEXT snapshot always agree, both derived from one frozen decision (file-decided path)"
}
...
echo "# all dj-trace-context-spawn tests passed"
