#!/usr/bin/env bash
# Behavioral regressions for ask-user authority instructions generated by dj-brief.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

BRIEF="$ROOT/bin/dj-brief.sh"
TMP_ROOT=$(dj_test_tmproot dj-ask-user-authority)
...
test_primary_and_secondmate_instruction_generation() {
  ...
  assert_grep "Jarvis applies \`ask-user-authority\` and obtains any required captain decision" "$ship" \
    "generated implementation brief bypasses the primary authority owner"
  assert_grep "silently bypass jarvis's authority check and any required captain escalation" "$ship" \
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" DJ_SECONDMATE_CHARTER='Handle sample work.' \
