#!/usr/bin/env bash
# Behavior tests for dj-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. DJ_SPAWN_NO_GUARD=1 keeps them off the
...
TMP_ROOT=$(dj_test_tmproot dj-spawn-batch)
export DJ_BACKEND=tmux

# Clear ambient jarvis overrides so the behavior test owns its environment.
run_spawn() {
  DJ_ROOT_OVERRIDE='' \
    DJ_HOME='' \
    DJ_STATE_OVERRIDE='' \
    DJ_DATA_OVERRIDE='' \
    DJ_PROJECTS_OVERRIDE='' \
    DJ_CONFIG_OVERRIDE='' \
    DJ_SPAWN_NO_GUARD=1 \
    "$SPAWN" "$@" 2>&1
}
...
# A projects/ path is resolved through the jarvis home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(DJ_ROOT_OVERRIDE='' DJ_STATE_OVERRIDE='' DJ_DATA_OVERRIDE='' DJ_CONFIG_OVERRIDE='' \
        DJ_HOME="$home" DJ_PROJECTS_OVERRIDE="$projects" DJ_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex --mode no-mistakes --yolo off 2>&1)
    else
      mkdir -p "$home/projects/alpha"
      out=$(DJ_ROOT_OVERRIDE='' DJ_STATE_OVERRIDE='' DJ_DATA_OVERRIDE='' DJ_PROJECTS_OVERRIDE='' DJ_CONFIG_OVERRIDE='' \
        DJ_HOME="$home" DJ_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex --mode no-mistakes --yolo off 2>&1)
    fi
    status=$?
...
DJ_HOME scopes projects/|no|nope-home-z7
DJ_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the jarvis home for single-task spawn"
}
