...
#
# A confused no-mistakes gate agent runs inside a jarvis checkout, adopts the
# captain identity from AGENTS.md, and reaches for dj-spawn/dj-send/dj-teardown.
# bin/dj-gate-refuse-lib.sh is the jarvis capability-removal half: sourced at
...
# A normal jarvis session (real primary, real crew worktree) has NEITHER
...
#
# Finally, assert jarvis's TRACKED .no-mistakes.yaml parses and sets
...
run_guard_lib() {
  ...
  (
    cd "$cwd" || exit 111
    unset NO_MISTAKES_GATE DJ_GATE_REFUSE_BYPASS
    case "$marker" in
...
test_helper_env_marker_refuses() {
  ...
  pass "dj-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set"
}
...
test_helper_empty_env_marker_refuses() {
  ...
  pass "dj-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set empty"
}
...
test_helper_path_backstop_refuses() {
  ...
  pass "dj-gate-refuse-lib: refuses from a gate worktree via git-common-dir (marker unset)"
}
...
test_helper_normal_is_noop() {
  ...
  pass "dj-gate-refuse-lib: no-op for a normal session (neither signal, set -eu clean)"
}

# --- dj-spawn ---------------------------------------------------------------

# A fake tmux/treehouse so dj-spawn resolves the crew worktree from a controlled
# pane path and completes without a live terminal (mirrors tests/dj-tangle-guard).
make_spawn_fakebin() {
  local dir=$1 fakebin
...
run_spawn() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u DJ_GATE_REFUSE_BYPASS \
...
test_spawn_refuses_and_admits() {
  ...
  dj_git_add_origin "$proj" "$TMP/spawn-origin.git"
  ...
  pass "dj-spawn: refuses on marker and gate-worktree backstop; a normal crew spawn is unaffected"
}

# --- dj-send ----------------------------------------------------------------

# A fake tmux that logs send-keys to DJ_TMUX_LOG and reports live endpoints
# (mirrors tests/dj-send-strict), so a successful send is observable and a
...
run_send() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u DJ_GATE_REFUSE_BYPASS \
      "PATH=$fakebin:$PATH" "DJ_HOME=$home" "DJ_ROOT_OVERRIDE=$home" \
      "DJ_TMUX_LOG=$log" "DJ_SEND_SETTLE=0" "$@" \
...
test_send_refuses_and_admits() {
  ...
  # env-marker refuse.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" dj-lane-ok "hello captain" NO_MISTAKES_GATE=1); rc=$?
  ...
  : > "$log"
  out=$(run_send "$GATE_WT" "$home" "$fakebin" "$log" dj-lane-ok "hello captain"); rc=$?
  ...
  # no-regression.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" dj-lane-ok "hello captain"); rc=$?
  ...
  [ "$(bash -c '. "$1"; dj_task_inbox_body "$2"' _ "$ROOT/bin/dj-task-inbox-lib.sh" \
  ...
  assert_contains "$(cat "$log")" "target=sess:dj-lane-ok literal=1 arg=Jarvis instruction waiting" \
    "send: normal steer should ring the durable inbox doorbell"
  pass "dj-send: refuses on marker and gate-worktree backstop; a normal steer uses the inbox"
}
...
# refused one leaves the task untouched (mirrors tests/dj-teardown make_case).
make_teardown_case() {
...
SH
  ...
  dj_write_meta "$case_dir/state/task-x1.meta" \
    "window=jarvis:dj-task-x1" "endpoint_task_id=task-x1" \
...
run_teardown() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u DJ_GATE_REFUSE_BYPASS \
      "DJ_ROOT_OVERRIDE=$ROOT" "DJ_STATE_OVERRIDE=$case_dir/state" \
      "DJ_CONFIG_OVERRIDE=$case_dir/config" "PATH=$case_dir/fakebin:$PATH" "$@" \
...
test_teardown_refuses_and_admits() {
  ...
  pass "dj-teardown: refuses on marker and gate-worktree backstop; a normal teardown is unaffected"
}
