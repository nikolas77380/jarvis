...
TMP_ROOT=$(dj_test_tmproot dj-calm-pi-extension)
EXT="$ROOT/.pi/extensions/dj-calm.ts"
ASSISTANT_LAYOUT="$ROOT/.pi/extensions/lib/dj-calm-assistant-layout.ts"
OPERATIONAL_USER_LAYOUT="$ROOT/.pi/extensions/lib/dj-calm-operational-user-layout.ts"
VISIBILITY="$ROOT/.pi/extensions/lib/dj-calm-visibility.ts"
WORKING_SHIP="$ROOT/.pi/extensions/lib/dj-calm-working-ship.ts"
WATCH_EXT="$ROOT/.pi/extensions/dj-primary-pi-watch.ts"
OPERATIONAL_INPUT="$ROOT/bin/dj-operational-input.sh"
PI_OPERATIONAL_INPUT="$ROOT/.pi/extensions/lib/dj-operational-input.ts"
PI_PACKAGE_DIR=${DJ_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
TMUX_SOCKET="dj-calm-$$"
TMUX_SESSION="dj-calm-e2e"
...
# .pi/extensions/dj-calm.ts) instead of relying on version inference, so a version
...
find_chrome() {
  local candidate
  if [ -n "${DJ_CHROME_BIN:-}" ] && [ -x "$DJ_CHROME_BIN" ]; then
    printf '%s\n' "$DJ_CHROME_BIN"
    return 0
  fi
  for candidate in \
...
test_home_resolution() {
  ...
  mkdir -p \
    ...
    "$fixture/override" \
    "$fixture/launch-cwd"
  cp "$EXT" "$fixture/project/.pi/extensions/dj-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/project/.pi/extensions/lib/dj-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/project/.pi/extensions/lib/dj-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/project/.pi/extensions/lib/dj-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/project/.pi/extensions/lib/dj-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/project/.pi/extensions/lib/dj-operational-input.ts"
  ...
  out=$(cd "$fixture/launch-cwd" && \
    EXT="$fixture/project/.pi/extensions/dj-calm.ts" \
...
delete process.env.DJ_HOME;
delete process.env.DJ_CONFIG_OVERRIDE;
process.env.DJ_ROOT_OVERRIDE = process.env.OVERRIDE_HOME;
...
if (readFileSync(`${process.env.OVERRIDE_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm ignored DJ_ROOT_OVERRIDE when DJ_HOME was unset");
}

delete process.env.DJ_ROOT_OVERRIDE;
calm = registerCalm();
calm.sessionStart({ reason: "startup" }, context);
await calm.calmCommand.handler("", context);
if (readFileSync(`${process.env.EXTENSION_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not derive the Jarvis home from its extension path");
}
...
const assistant = await import("./.pi/extensions/lib/dj-calm-assistant-layout.ts");
const operational = await import("./.pi/extensions/lib/dj-calm-operational-user-layout.ts");
...
process.env.DJ_HOME = process.env.HOME_OFF;
const offRun = fakePi();
...
process.env.DJ_HOME = process.env.HOME_ON;
const onRun = fakePi();
...
// captain-accepted, documented bound on the gate-at-load fix (see dj-calm.ts's file
...
test_rendering_and_session_lifecycle() {
  ...
  cp "$EXT" "$fixture/dj-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/dj-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/dj-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/dj-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/dj-calm-working-ship.ts"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$fixture/lib/dj-operational-input.ts"
  cp "$ROOT/.pi/extensions/lib/dj-branch-dispatch.ts" "$fixture/lib/dj-branch-dispatch.ts"
  cp "$WATCH_EXT" "$fixture/dj-primary-pi-watch.ts"
...
#!/usr/bin/env bash
printf '%s\n' "${1-}" >>"$DJ_OPERATIONAL_INPUT_CALLS"
exec "$DJ_OPERATIONAL_INPUT_OWNER" "$@"
SH
  chmod +x "$fixture/operational-input-probe.sh"

  output_file="$fixture/node-output"
  (cd "$fixture" && EXT="$fixture/dj-calm.ts" WATCH_EXT="$fixture/dj-primary-pi-watch.ts" DJ_HOME="$fixture/home" DJ_OPERATIONAL_INPUT_SCRIPT="$fixture/operational-input-probe.sh" DJ_OPERATIONAL_INPUT_OWNER="$OPERATIONAL_INPUT" DJ_OPERATIONAL_INPUT_CALLS="$fixture/operational-input-calls" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module) >"$output_file" 2>&1 <<'JS'
import { readFileSync, writeFileSync } from "node:fs";
import { fileURLToPath, pathToFileURL } from "node:url";

// dj-calm.ts derives its own identity the same way (fileURLToPath(import.meta.url)),
...
extension.default(pi);
const visibility = await import(`${pathToFileURL(`${process.cwd()}/lib/dj-calm-visibility.ts`).href}?policy=${Date.now()}`);
const operationalInput = await import(`${pathToFileURL(`${process.cwd()}/lib/dj-operational-input.ts`).href}?input=${Date.now()}`);

// Registration is gated on config/calm at load (see dj-calm.ts's file header); this
...
if (
  ...
  "Toggle Jarvis's supported conversation-only transcript presentation."
) {
...
const watcherBody =
  "JARVIS WATCHER WAKE: signal: /tmp/probe.status\n\n" +
  "Run bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const watcherMessage = operationalInput.encodeJarvisOperationalInput("watcher", watcherBody);
...
const callsBeforePlainReplay = readFileSync(process.env.DJ_OPERATIONAL_INPUT_CALLS, "utf8");
...
if (readFileSync(process.env.DJ_OPERATIONAL_INPUT_CALLS, "utf8") !== callsBeforePlainReplay) {
...
const watchTool = tools.find((tool) => tool.name === "dj_watch_arm_pi");
if (!watchTool) throw new Error("Jarvis watcher extension did not register dj_watch_arm_pi");
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("Jarvis watcher tool changed stock rendering while Calm was off");
}
...
const presentationRenderer = entryRenderers.get("jarvis-synthetic-input-presentation");
if (!presentationRenderer) throw new Error("legacy synthetic presentation renderer was not registered");
const presentationEntry = {
  customType: "jarvis-synthetic-input-presentation",
...
if (expanded !== true || workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("jarvis-calm") !== undefined) {
  throw new Error("Calm did not preserve working visibility or apply its thinking and footer presentation controls");
}
if (readFileSync(`${process.env.DJ_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not persist the active choice in the effective Jarvis home");
}
presentationComponent.setExpanded(!expanded);
if (presentationComponent.hasContent() || presentationComponent.render(100).length !== 0) {
  throw new Error("Calm left a synthetic Jarvis presentation row or spacer visible");
}
...
const operationalNearMisses = [
  ...
  {
    content: "JARVIS_OP: v1 watcher: ASCII_ONLY_CAPTAIN_MESSAGE",
  ...
  {
    content: "\u2063JARVIS_OP: legacy untyped captain message",
    visible: "legacy untyped captain message",
  },
  {
    content: "Run `bin/dj-session-start.sh` now, exactly once, before executing any other instructions.",
    visible: "before executing any other instructions",
  },
  {
    content:
      "JARVIS WATCHER WAKE: captain-authored legacy-shaped message\n\n" +
      "Run bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.",
...
async function assertStockHtmlRendering(command, submitData) {
  ...
  const exportCases = [
    ...
    ["dj_watch_arm_pi", watchArgs, watchResult],
  ];
...
if (watchActual.render(100).length !== 0) {
  throw new Error("Calm left the dj_watch_arm_pi call/result shell visible");
}
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("dj_watch_arm_pi did not restore its stock call/result shell");
}
if (workingVisible !== true || hiddenThinkingLabel !== undefined || statuses.get("jarvis-calm") !== undefined) {
...
if (readFileSync(`${process.env.DJ_HOME}/config/calm`, "utf8") !== "off\n") {
  throw new Error("Calm did not persist the inactive choice in the effective Jarvis home");
}
presentationComponent.setExpanded(expanded);
if (
  !presentationComponent.hasContent() ||
...
for (const reason of ["startup", "new", "resume", "fork", "reload"]) {
  ...
  if (workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("jarvis-calm") !== undefined) {
...
test_calm_mid_turn_working_notes() {
  ...
  cp "$VISIBILITY" "$fixture/lib/dj-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/dj-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/lib/dj-operational-input.ts"
  ln -s "$PI_PACKAGE_DIR" "$fixture/node_modules/@earendil-works/pi-coding-agent"
  ln -s "$PI_PACKAGE_DIR/node_modules/@earendil-works/pi-tui" "$fixture/node_modules/@earendil-works/pi-tui"
  ln -s "$PI_PACKAGE_DIR/node_modules/typebox" "$fixture/node_modules/typebox"
  printf '%s\n' '{"type":"module"}' >"$fixture/package.json"
  ...
  output_file="$fixture/node-output"
  (cd "$fixture" && EXT="$fixture/dj-calm.ts" DJ_HOME="$fixture/home" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module) >"$output_file" 2>&1 <<'JS'
...
const visibility = await import(pathToFileURL(`${process.cwd()}/lib/dj-calm-visibility.ts`).href);
const calmPreferencePath = `${process.env.DJ_HOME}/config/calm`;
const components = [];
const ui = {
  getEditorText: () => "",
  getToolsExpanded: () => false,
...
test_operational_followup_turn_e2e() {
  ...
  dj_git_init_commit "$project"
  cp "$EXT" "$project/.pi/extensions/dj-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$project/.pi/extensions/lib/dj-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$project/.pi/extensions/lib/dj-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$project/.pi/extensions/lib/dj-calm-visibility.ts"
  cp "$WORKING_SHIP" "$project/.pi/extensions/lib/dj-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$project/.pi/extensions/lib/dj-operational-input.ts"
...
import { encodeJarvisOperationalInput } from "./.pi/extensions/lib/dj-operational-input.ts";
...
const EXACT_WATCHER_INPUT =
  "\u2063JARVIS_OP: v1 watcher: JARVIS WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status\n\n" +
  "Run bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
function monitorInput(suffix: "ONE" | "TWO"): string {
  ...
  return encodeJarvisOperationalInput("watcher", `MONITOR_${label}_${suffix}`);
}
...
TS
  ...
  run_followup_case() {
    ...
    if [ "$calm_state" = absent ]; then
      ...
      rm -f "$home/config/calm"
      extensions='-e ./.pi/extensions/dj-calm.ts -e ./followup-e2e.ts'
    else
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env DJ_HOME='$home' PI_CODING_AGENT_DIR='$config' DJ_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions $extensions $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
    ...
    if [ "$calm_state" = on ]; then
      ...
      if [ "$label" = exact_watcher ]; then
        assert_not_contains "$pane" "JARVIS WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status" \
          "Pi exact watcher case rendered the Calm-hidden authoritative payload"
        assert_not_contains "$pane" "Run bin/dj-wake-drain.sh first and handle the queued wake." \
...
const expectedOperationalTexts = Array.from({ length: expected }, (_, index) => {
  ...
  return label === "exact_watcher" && suffix === "ONE"
    ...
    : label === "legacy_away" && suffix === "ONE"
      ...
      : `\u2063JARVIS_OP: v1 watcher: MONITOR_${label}_${suffix}`;
});
...
JS
  ...
  replay_exact_case() {
    ...
    printf '%s\n' on >"$home/config/calm"
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env DJ_HOME='$home' PI_CODING_AGENT_DIR='$config' DJ_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions -e ./.pi/extensions/dj-calm.ts -e ./followup-e2e.ts --session '$exact_session'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
...
const text = (content) => typeof content === "string"
  ? content
  : (content ?? []).filter((item) => item.type === "text").map((item) => item.text).join("");
const exact = "\u2063JARVIS_OP: v1 watcher: JARVIS WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status\n\nRun bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
TS
  ...
  start_geometry_pi() {
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 100 -y 44 \
      "cd '$project' && env DJ_HOME='$home' PI_CODING_AGENT_DIR='$config' PI_OFFLINE=1 pi --approve --no-context-files --no-prompt-templates --no-extensions -e ./.pi/extensions/dj-calm.ts -e ./geometry-provider.ts $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
  }
...
test_working_ship_geometry_and_lifecycle() {
  ...
  out=$(cd "$fixture" && EXT="$fixture/dj-calm.ts" DJ_HOME="$fixture/home" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
...
const ship = await import(
  `${pathToFileURL(`${process.cwd()}/lib/dj-calm-working-ship.ts`).href}?ship=${Date.now()}`
);
const {
...
test_interactive_terminal_e2e() {
  ...
  cp "$VISIBILITY" "$project/.pi/extensions/lib/dj-calm-visibility.ts"
  cp "$WORKING_SHIP" "$project/.pi/extensions/lib/dj-calm-working-ship.ts"
  cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$project/.pi/extensions/lib/dj-operational-input.ts"
  cp "$ROOT/.pi/extensions/lib/dj-branch-dispatch.ts" "$project/.pi/extensions/lib/dj-branch-dispatch.ts"
  cp "$WATCH_EXT" "$project/.pi/extensions/dj-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$project/.pi/extensions/dj-primary-turnend-guard.ts"
  cp \
    "$ROOT/bin/dj-sessionstart-run.sh" \
    "$ROOT/bin/dj-sessionstart-nudge.sh" \
    "$ROOT/bin/dj-primary-scope-lib.sh" \
    "$ROOT/bin/dj-gate-refuse-lib.sh" \
    "$ROOT/bin/dj-operational-input.sh" \
    "$project/bin/"
  ...
  cat >"$project/bin/dj-session-start.sh" <<'SH'
#!/usr/bin/env bash
printf 'CALM_E2E_SESSION_START_DIGEST\n'
exit 0
SH
  chmod +x "$project/bin/"*.sh
  cat >"$project/.pi/extensions/dj-calm-e2e-inject.ts" <<'TS'
import {
  type AssistantMessage,
  createAssistantMessageEventStream,
} from "@earendil-works/pi-ai";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { encodeJarvisOperationalInput } from "./lib/dj-operational-input.ts";
  ...
  pi.registerCommand("calm-inject-e2e", {
    ...
    handler: async (args, ctx) => {
      const fixtures = new Map([
        ...
        ["from-jarvis", "corr=0123456789abcdef CURRENT_FROM_JARVIS_E2E"],
        ["launch-brief", "CURRENT_LAUNCH_BRIEF_E2E"],
      ] as const);
      const kind = args.trim() as Parameters<typeof encodeJarvisOperationalInput>[0];
      ...
      await pi.sendUserMessage(encodeJarvisOperationalInput(kind, body), {
        deliverAs: "followUp",
      });
    },
  });
...
{"type":"message","id":"a0000007","parentId":"a0000006","timestamp":"$now","message":{"role":"assistant","content":[{"type":"thinking","thinking":"third internal reasoning block"},{"type":"toolCall","id":"call_watch_e2e","name":"dj_watch_arm_pi","arguments":{}}],"api":"anthropic-messages","provider":"anthropic","model":"claude-sonnet-4-5","usage":{"input":2,"output":1,"cacheRead":0,"cacheWrite":0,"totalTokens":3,"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0,"total":0}},"stopReason":"toolUse","timestamp":7}}
{"type":"message","id":"a0000008","parentId":"a0000007","timestamp":"$now","message":{"role":"toolResult","toolCallId":"call_watch_e2e","toolName":"dj_watch_arm_pi","content":[{"type":"text","text":"watcher: started Pi extension arm child 1"}],"details":{"ok":true,"message":"watcher: started Pi extension arm child 1"},"isError":false,"timestamp":8}}
{"type":"custom","id":"a0000009","parentId":"a0000008","timestamp":"$now","customType":"jarvis-synthetic-input-presentation","data":{"content":"JARVIS WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","kind":"watcher"}}
{"type":"custom_message","id":"a0000010","parentId":"a0000009","timestamp":"$now","customType":"jarvis-synthetic-input","content":"JARVIS WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/dj-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","display":false,"details":{"kind":"watcher"}}
{"type":"message","id":"a0000011","parentId":"a0000010","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"JARVIS WATCHER WAKE: can you explain this phrase?"}],"timestamp":11}}
{"type":"message","id":"a0000012","parentId":"a0000011","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Captain quote: \u2063JARVIS_OP: v1 watcher: QUOTED_CURRENT_NEAR_MISS"}],"timestamp":12}}
{"type":"message","id":"a0000013","parentId":"a0000012","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"JARVIS_OP: v1 watcher: ASCII_ONLY_NEAR_MISS"}],"timestamp":13}}
{"type":"message","id":"a0000014","parentId":"a0000013","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Ordinary captain text before \u2063JARVIS_OP: v1 watcher: EMBEDDED_CURRENT_NEAR_MISS"}],"timestamp":14}}
...
JSON
  ...
  tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 180 -y 44 \
    "cd '$project' && env DJ_HOME='$home' PI_CODING_AGENT_DIR='$config' DJ_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-skills --no-prompt-templates --no-context-files --session '$session_file'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 30"
  ...
  assert_contains "$(cat "$default_snapshot")" "dj_watch_arm_pi" "Calm-off transcript did not show the Jarvis watcher tool"
  assert_contains "$(cat "$default_snapshot")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" "Calm-off transcript did not show the synthetic Jarvis presentation row"
  assert_contains "$(cat "$default_snapshot")" "Thinking..." "reasoning fixture did not render Pi's collapsed thinking label"
  assert_contains "$(cat "$default_snapshot")" "dj-calm.ts" "project-local Pi calm extension did not auto-load"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$default_snapshot")" 'Run `bin/dj-session-start.sh` now' \
  ...
  while [ "$active_screen_wait" -lt 120 ]; do
    ...
    # first-ever activation are a separate, documented exception (see dj-calm.ts's
    ...
    if ! grep -Fq "Thinking..." "$hidden_snapshot" &&
      ...
      grep -Fq "JARVIS WATCHER WAKE: can you explain this phrase?" "$hidden_snapshot" &&
  ...
  # dj-calm.ts's file header and docs/calm.md): the alternative was letting Calm
  ...
  assert_not_contains "$(cat "$hidden_snapshot")" "dj_watch_arm_pi" "/calm left the Jarvis watcher tool call shell in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "watcher: started Pi extension arm child" "/calm left the Jarvis watcher tool result in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" "/calm left a synthetic Jarvis user-role presentation in the transcript"
  ...
  assert_contains "$(cat "$hidden_snapshot")" "JARVIS WATCHER WAKE: can you explain this phrase?" "/calm hid a genuine near-miss user prompt"
  for near_miss in \
  ...
  for fixture in \
    ...
    "turn-end-guard|CURRENT_TURN_END_E2E" \
    "away-supervisor|CURRENT_AWAY_E2E" \
    "from-jarvis|CURRENT_FROM_JARVIS_E2E" \
    "launch-brief|CURRENT_LAUNCH_BRIEF_E2E"
  do
    kind=${fixture%%|*}
    needle=${fixture#*|}
...
const nativeSessionStart = entries.find((entry) =>
  entry.type === "custom_message" &&
  entry.customType === "jarvis-sessionstart-nudge"
);
if (
  !nativeSessionStart ||
  nativeSessionStart.display !== false ||
  nativeSessionStart.details?.kind !== "session-start" ||
  !nativeSessionStart.content?.startsWith("\u2063JARVIS_OP: v1 session-start: ")
) {
...
const expected = new Map([
  ...
  ["CURRENT_FROM_JARVIS_E2E", "from-jarvis"],
...
for (const [needle, kind] of expected) {
  ...
  const exactEnvelope = kind === "from-jarvis"
    ? text?.startsWith("[dj-from-jarvis]\u2063corr=0123456789abcdef ")
    : text?.startsWith(`\u2063JARVIS_OP: v1 ${kind}: `);
...
JS
  ...
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$active_hidden_snapshot")" 'Run `bin/dj-session-start.sh` now' \
    "Calm showed the native session-start operational input"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
    CURRENT_FROM_JARVIS_E2E \
...
if (!serialized.includes("jarvis-synthetic-input") || !serialized.includes("/tmp/probe.status")) process.exit(1);
const synthetic = entries.find((entry) => entry.type === "custom_message" && entry.customType === "jarvis-synthetic-input");
...
if (messages.includes("[jarvis-synthetic-input]")) process.exit(1);
for (const current of ["CURRENT_WATCHER_E2E", "CURRENT_TURN_END_E2E", "CURRENT_AWAY_E2E", "CURRENT_FROM_JARVIS_E2E", "CURRENT_LAUNCH_BRIEF_E2E"]) {
  if (!messages.includes(current)) process.exit(1);
}
if (!tree.includes("jarvis-synthetic-input") || !tree.includes("/tmp/probe.status")) process.exit(1);
JS
  ...
  assert_not_contains "$(cat "$export_settled_snapshot")" "dj_watch_arm_pi" \
    "/export left the Jarvis watcher tool call shell in the Calm transcript"
  assert_not_contains "$(cat "$export_settled_snapshot")" "watcher: started Pi extension arm child" \
    "/export left the Jarvis watcher tool result in the Calm transcript"
  assert_not_contains "$(cat "$export_settled_snapshot")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" \
    "/export left a synthetic Jarvis user-role presentation in the Calm transcript"
  ...
  assert_contains "$(cat "$restored_snapshot")" "dj_watch_arm_pi" "second /calm did not restore the Jarvis watcher tool shell"
  assert_contains "$(cat "$restored_snapshot")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" "second /calm did not restore the synthetic Jarvis user row"
  for restored in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  assert_not_contains "$(cat "$boat_frame_one")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" "the real provider wait restored a hidden operational row"
  ...
  assert_not_contains "$(cat "$restarted_snapshot")" "dj_watch_arm_pi" "restart/resume reset Calm and restored the Jarvis watcher tool"
  assert_not_contains "$(cat "$restarted_snapshot")" "JARVIS WATCHER WAKE: signal: /tmp/probe.status" "restart/resume reset Calm and restored a legacy presentation row"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
