...
# (bin/dj-claude-stop-autoarm.sh, docs/watcher-continuity.md).
#
...
# through the real dj-lock.sh path. The arm wrapper is a per-test fixture, so no
# real watcher, model, or fleet state is touched.
# shellcheck disable=SC2016 # single quotes are deliberate: $DJ_HOME expands inside the fake harness child, and grep needles are literal strings
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(dj_test_tmproot dj-claude-stop-autoarm)
...
make_crewmate_worktree_dir() {
  local base=$1 dir=$2
  dj_git_worktree "$base" "$dir" dj/autoarm-test-branch
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_autoarm_scripts "$dir"
  printf '%s\n' "$dir"
...
# Arm fixture variants, installed per test as <dir>/bin/dj-watch-arm.sh.
write_arm_fixture() {
  local dir=$1 kind=$2
  case "$kind" in
    actionable)
...
test_inert_without_session_lock() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | DJ_HOME="$dir" bash "$dir/bin/dj-claude-stop-autoarm.sh" 2>&1); status=$?
...
test_reclaims_stale_session_lock_before_arming() {
  ...
  pass "auto-arm: a demonstrably dead recorded session owner is reclaimed through dj-lock.sh before arming"
}
...
test_inert_when_lock_held_by_other_harness() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | DJ_HOME="$dir" "$FAKE_CLAUDE" -c '"$DJ_HOME/bin/dj-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_stale_lock_recovery_preserves_afk_and_need_gates() {
  ...
  out=$(printf '%s\n' '{"session_id":"stale-afk"}' | DJ_HOME="$afk_dir" "$FAKE_CLAUDE" -c '"$DJ_HOME/bin/dj-claude-stop-autoarm.sh"' 2>&1); status=$?
  ...
  out=$(printf '%s\n' '{"session_id":"stale-idle"}' | DJ_HOME="$idle_dir" "$FAKE_CLAUDE" -c '"$DJ_HOME/bin/dj-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_actionable_close_rewakes_with_reason() {
  ...
  assert_contains "$out" "jarvis watcher wake" "rewake must carry the wake banner"
  assert_contains "$out" "stale: fixture-win actionable" "rewake must carry the arm's reason line"
  assert_contains "$out" "bin/dj-wake-drain.sh" "rewake must direct the drain-first protocol"
  assert_contains "$out" "do NOT run bin/dj-watch-arm.sh" "rewake must forbid a duplicate model re-arm"
...
test_actionable_close_with_live_successor_rewakes_once() {
  ...
  [ "$(printf '%s\n' "$out" | grep -c '^jarvis watcher wake')" -eq 1 ] \
...
test_failed_close_rewakes_with_failure_banner() {
  ...
  assert_not_contains "$out" "bin/dj-watch-arm.sh" "failure rewake must not create a manual arm loop"
...
test_single_flight_admits_exactly_one_owner() {
  ...
  DJ_HOME="$dir" "$FAKE_CLAUDE" -c '
    printf "%s\n" "$$" > "$DJ_HOME/state/.lock"
    printf "%s\n" "{\"session_id\":\"s\"}" | "$DJ_HOME/bin/dj-claude-stop-autoarm.sh" >/dev/null 2>"$DJ_HOME/state/err1" &
    p1=$!
    printf "%s\n" "{\"session_id\":\"s\"}" | "$DJ_HOME/bin/dj-claude-stop-autoarm.sh" >/dev/null 2>"$DJ_HOME/state/err2" &
    p2=$!
    wait "$p1"; echo $? > "$DJ_HOME/state/rc1"
    wait "$p2"; echo $? > "$DJ_HOME/state/rc2"
  '
...
record_autoarm_owner_identity() {
  ...
  identity=$(dj_test_pid_identity "$pid") || return 1
  [ -n "$identity" ] || return 1
...
test_abandoned_owner_claim_is_reclaimed_and_rearms() {
  ...
  assert_contains "$out" "jarvis watcher wake" "the reclaimed cycle must still translate its wake"
...
test_fm_lock_status_still_works_with_shared_lib() {
  local out
  out=$(DJ_HOME="$TMP_ROOT/lock-status-home" bash "$ROOT/bin/dj-lock.sh" status 2>&1)
  assert_contains "$out" "lock: free" "dj-lock.sh status must keep working after the session-lock lib extraction"
  pass "dj-lock: shared session-lock lib preserves the status path"
}
