...
# tests/dj-cursor-primary-live-e2e.test.sh.
#
# Three facts in this area come from the vendor, not from Jarvis, so a stub
...
#       (bin/dj-startup-network.sh), so a harness that reaps the hook's process
...
# tests/dj-sessionstart-nudge.test.sh pins that routing portably with real
# processes and no harness. This guard covers only what CI cannot see.
#
# It swaps a RECORDER in for bin/dj-sessionstart-run.sh inside a throwaway lab
...
if [ "${DJ_SESSIONSTART_HOOK_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_SESSIONSTART_HOOK_LIVE_E2E=1 to run the live session-open hook regression"
  exit 0
fi
...
LAB="${TMPDIR:-/tmp}/dj-sessionstart-hook-live-e2e.$$"
SOCKET="dj-ss-hook-$$"
CHECKED=0
ABSENT=
...
# --- lab ---------------------------------------------------------------------
#
# A Jarvis-shaped checkout carrying the harness's own TRACKED registration,
...
make_lab() {  # <harness> -> echoes lab dir
  ...
  for stub in dj-turnend-guard.sh dj-claude-stop-autoarm.sh dj-arm-pretool-check.sh \
    dj-cd-pretool-check.sh dj-subagent-pretool-check.sh; do
  ...
  # tests/dj-bootstrap.test.sh already owns.
  ln -sf "$ROOT/bin/dj-startup-network.sh" "$lab/bin/dj-startup-network.sh"
  ln -sf "$ROOT/bin/dj-timeout-lib.sh" "$lab/bin/dj-timeout-lib.sh"
  ln -sf "$ROOT/bin/dj-wake-lib.sh" "$lab/bin/dj-wake-lib.sh"
  ln -sf "$ROOT/bin/dj-session-lock-lib.sh" "$lab/bin/dj-session-lock-lib.sh"
  cat > "$lab/bin/dj-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
...
set -u
sleep 6
printf 'detached worker survived the hook\n' > "${DJ_LIVE_DETACH_MARKER:?}"
exit 0
SH
  chmod +x "$lab/bin/dj-bootstrap.sh"

  cat > "$lab/bin/dj-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
...
printf '%s\n' "$source" >> "$record"
# Exactly what bin/dj-session-start.sh does after taking the lock.
if [ -n "${DJ_LIVE_DETACH_MARKER:-}" ]; then
  "$(dirname "$0")/dj-startup-network.sh" start --locked 0 --harvest-pid $$ >/dev/null 2>&1 || true
fi
printf 'FMHOOKTOKEN-%s-%s-%s\n' "$source" "$(grep -c . "$record" | tr -d ' ')" "${DJ_LIVE_NONCE:?}"
exit 0
SH
  chmod +x "$lab/bin/dj-sessionstart-run.sh"
  ...
  case "$harness" in
    ...
    pi)
      ...
      cp "$ROOT/.pi/extensions/dj-primary-turnend-guard.ts" "$lab/.pi/extensions/"
      cp "$ROOT/.pi/extensions/lib/dj-operational-input.ts" "$lab/.pi/extensions/lib/"
      cp "$ROOT/bin/dj-operational-input.sh" "$lab/bin/"
...
probe_process_opens() {  # <harness> <version> <lab> <expect-resume> <cold-argv...> -- <resume-argv...>
  ...
  out=$( cd "$lab" && DJ_LIVE_RECORD="$record" DJ_LIVE_NONCE="$LIVE_NONCE" DJ_ROOT_OVERRIDE="$lab" DJ_HOME="$lab" \
    DJ_LIVE_DETACH_MARKER="$marker" \
  ...
  : > "$record"
  ( cd "$lab" && DJ_LIVE_RECORD="$record" DJ_LIVE_NONCE="$LIVE_NONCE" DJ_ROOT_OVERRIDE="$lab" DJ_HOME="$lab" \
...
probe_context_reset() {  # <harness> <version> <lab> <clear-command> <launch-argv...>
  ...
  tmux -L "$SOCKET" new-session -d -s "$session" -c "$lab" -x 200 -y 50 \
    -e DJ_LIVE_RECORD="$record" -e DJ_ROOT_OVERRIDE="$lab" -e DJ_HOME="$lab" \
    -e DJ_LIVE_NONCE="$LIVE_NONCE" \
    "$*" \
  ...
  case "$harness" in
    ...
    pi)
      probe_process_opens pi "$version" "$lab" resume \
        pi -p -e "$lab/.pi/extensions/dj-primary-turnend-guard.ts" --no-context-files --no-tools \
        -- pi -p -c -e "$lab/.pi/extensions/dj-primary-turnend-guard.ts" --no-context-files --no-tools
      probe_context_reset pi "$version" "$lab" /new \
        pi -e "$lab/.pi/extensions/dj-primary-turnend-guard.ts" --no-context-files
      ;;
  esac
  CHECKED=$((CHECKED + 1))
done
...
echo "# dj-sessionstart-hook-live-e2e.test.sh: all live assertions passed"
