#!/usr/bin/env bash
# Regression tests for dj-spawn's pooled-worktree base refresh.
#
...
TMP_ROOT=$(dj_test_tmproot dj-spawn-pool-base-freshen)
...
set -u
case "$*" in
  *"#{pane_current_path}"*) printf '%s\n' "${DJ_FAKE_PANE_PATH:?DJ_FAKE_PANE_PATH unset}"; exit 0 ;;
esac
case "${1:-}" in
...
make_case() {
  ...
  git -C "$project" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm initial
  ...
  git -C "$publisher" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm advance-main
...
run_spawn() {
  ...
  DJ_ROOT_OVERRIDE='' DJ_HOME="$HOME_DIR" \
    ...
    DJ_SPAWN_NO_GUARD=1 TMUX="fake,1,0" DJ_FAKE_PANE_PATH="$POOL_DIR" \
...
test_stale_pool_base_refreshes_before_branching() {
  ...
  git -C "$POOL_DIR" checkout --quiet -b "dj/$id"
...
echo "# all dj-spawn-pool-base-freshen tests passed"
