...
# bin/dj-harness.sh checks verified ENV markers before ancestry. A suite run
...
KIMI_HOOK="$ROOT/bin/dj-kimi-turnend-hook.sh"
TMP_ROOT=$(dj_test_tmproot dj-kimi-harness)
KIMI_RUNTIME_TASK_TMP=
PYTHON_BIN=$(command -v python3) || fail "test needs python3"
PYTHON_BIN_DIR=$(dirname "$PYTHON_BIN")
JQ_BIN=$(command -v jq) || fail "test needs jq"
BASE_PATH=${DJ_TEST_BASE_PATH:-$PYTHON_BIN_DIR:/usr/bin:/bin:/usr/sbin:/sbin}
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$DJ_FAKE_TMUX_CALL_LOG"
state=$(cat "$DJ_FAKE_KIMI_STATE" 2>/dev/null || true)
fake_screen() {
  case "$state" in
    ready)
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ -n "$literal" ]; then
      case "$literal" in
        *' --auto')
          printf '%s\n' "$literal" >> "$DJ_FAKE_LAUNCH_LOG"
          printf 'launched\n' > "$DJ_FAKE_KIMI_STATE"
          ;;
        *)
          printf '%s\n' "$literal" >> "$DJ_FAKE_POINTER_LOG"
    ...
    case " $* " in
      *' Enter '*)
        case "$state" in
          launched)
            if [ "${DJ_FAKE_KIMI_READY:-yes}" = yes ]; then
              printf 'ready\n' > "$DJ_FAKE_KIMI_STATE"
            fi
            ;;
          pointer-typed)
            if [ "${DJ_FAKE_KIMI_DELIVERY:-yes}" = yes ]; then
              if [ "${DJ_FAKE_KIMI_SWALLOW_FIRST:-no}" = yes ] \
                 && [ ! -f "$DJ_FAKE_KIMI_SWALLOWED" ]; then
                : > "$DJ_FAKE_KIMI_SWALLOWED"
              else
                printf 'delivered\n' > "$DJ_FAKE_KIMI_STATE"
              fi
            else
...
make_spawn_case() {
  ...
  dj_git_worktree "$proj" "$wt" "wt-$name"
...
run_spawn() {
  ...
  shift 6
  HOME="$home" DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    ...
    DJ_FAKE_LAUNCH_LOG="$case_dir/launch.log" \
    DJ_FAKE_POINTER_LOG="$case_dir/pointer.log" \
    DJ_FAKE_KIMI_STATE="$case_dir/kimi.state" \
    DJ_FAKE_KIMI_SWALLOWED="$case_dir/kimi.swallowed" \
    DJ_FAKE_KIMI_SWALLOW_FIRST="${DJ_FAKE_KIMI_SWALLOW_FIRST:-no}" \
    DJ_FAKE_TMUX_CALL_LOG="$case_dir/tmux-calls.log" \
    DJ_FAKE_BRIEF_REAL="$(cd "$home/data/$id" && pwd -P)/brief.md" \
    DJ_KIMI_READY_POLLS=2 DJ_KIMI_DELIVERY_POLLS=2 DJ_KIMI_POLL_INTERVAL=0 \
...
test_kimi_launch_then_send_is_verified() {
  ...
  out=$(DJ_FAKE_KIMI_SWALLOW_FIRST=yes run_spawn \
  ...
  assert_grep 'BEGIN JARVIS KIMI TURN-END HOOK' "$HOME_DIR/.kimi-code/config.toml" \
    "kimi spawn did not install its guarded global hook region"
  assert_grep 'token=' "$WT_DIR/.dj-kimi-turnend" "kimi spawn did not write its token pointer"
  assert_present "$HOME_DIR/state/$id.kimi-turnend-token" "kimi spawn did not record its token"
  pass "dj-spawn: kimi launches, delivers its brief, and registers a guarded turn-end token"
}
...
EOF
  ...
  count=$(grep -c '^# BEGIN JARVIS KIMI TURN-END HOOK' "$config")
  [ "$count" -eq 1 ] || fail "idempotent install left $count Jarvis regions"

  HOME="$home" "$KIMI_HOOK" remove || fail "Kimi hook removal failed"
  cp "$config" "$stripped"
  cmp -s "$original" "$stripped" \
    || fail "config with the Jarvis region excised was not byte-identical to the original"
  assert_absent "$home/.kimi-code/dj-turn-end.sh" "removal left the Jarvis hook script"
  assert_absent "$home/.kimi-code/dj-turn-end.d" "removal left the Jarvis registry"
...
test_kimi_hook_fails_closed_on_missing_malformed_or_partial_config() {
  ...
  assert_absent "$missing/.kimi-code/dj-turn-end.sh" "missing config refusal wrote the hook script"
  ...
  assert_absent "$malformed/.kimi-code/dj-turn-end.sh" "malformed config refusal wrote the hook script"

  printf '# BEGIN JARVIS KIMI TURN-END HOOK\n' > "$partial/.kimi-code/config.toml"
  cp "$partial/.kimi-code/config.toml" "$partial/before"
  rc=0
  out=$(HOME="$partial" "$KIMI_HOOK" install 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "partial Jarvis marker was accepted"
...
test_kimi_hook_install_refuses_without_jq() {
  ...
  assert_absent "$home/.kimi-code/dj-turn-end.sh" "missing-jq refusal wrote the hook script"
  assert_absent "$home/.kimi-code/dj-turn-end.d" "missing-jq refusal wrote the registry"
...
test_kimi_hook_is_silent_and_requires_registered_workspace_token() {
  ...
  hook="$HOME_DIR/.kimi-code/dj-turn-end.sh"
  target="$HOME_DIR/state/$id.turn-ended"
  token=$(sed -n 's/^token=//p' "$WT_DIR/.dj-kimi-turnend")
  assert_present "$HOME_DIR/.kimi-code/dj-turn-end.d/$token" "Kimi registry token is missing"
  ...
  rm "$target"
  fakebin=$(dj_fakebin "$CASE_DIR/no-jq")
  ln -s "$(command -v bash)" "$fakebin/bash"
  out=$(printf '{"hook_event_name":"Stop","session_id":"crew","cwd":"%s","stop_hook_active":false}\n' "$WT_DIR" \
  ...
  pass "Kimi hook stays silent and inert without a Jarvis registry token"
}
...
test_kimi_spawn_refuses_unsafe_global_config_before_pane_creation() {
  ...
  pass "dj-spawn: unsafe Kimi global config refuses before pane creation"
}
...
test_kimi_teardown_removes_pointer_and_registry_token() {
  ...
  HOME="$HOME_DIR" DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$HOME_DIR" \
    DJ_STATE_OVERRIDE="$HOME_DIR/state" DJ_DATA_OVERRIDE="$HOME_DIR/data" \
    DJ_PROJECTS_OVERRIDE="$HOME_DIR/projects" DJ_CONFIG_OVERRIDE="$HOME_DIR/config" \
    DJ_SPAWN_NO_GUARD=1 PATH="$FAKEBIN_DIR:$BASE_PATH" \
    "$TEARDOWN" "$id" --force >/dev/null 2>&1 || fail "Kimi teardown failed"
  assert_absent "$WT_DIR/.dj-kimi-turnend" "Kimi token pointer survived teardown"
  assert_absent "$HOME_DIR/.kimi-code/dj-turn-end.d/$token" "Kimi registry token survived teardown"
  assert_absent "$HOME_DIR/state/$id.kimi-turnend-token" "Kimi token state survived teardown"
  pass "dj-teardown: Kimi task pointer and registry token are removed"
}
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  dj_fake_exit0 "$(dirname "$fallback")" kimi
  out=$(run_spawn "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id")
  rc=$?
  ...
  pass "dj-spawn: Kimi fallback expands the active HOME"
}
...
test_kimi_missing_binary_refuses_before_pane_creation() {
  ...
  pass "dj-spawn: missing Kimi executable refuses before pane creation"
}
...
test_kimi_unconfirmed_delivery_fails_loudly() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(DJ_FAKE_KIMI_DELIVERY=no run_spawn \
  ...
  pass "dj-spawn: kimi treats a silent pointer drop as a failed spawn"
}
...
test_kimi_readiness_gate_precedes_pointer() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(DJ_FAKE_KIMI_READY=no run_spawn \
    "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id") || rc=$?
  ...
  pass "dj-spawn: kimi never sends the brief pointer before an observable ready signal"
}
...
SH
  ...
  out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u DJ_PI_HARNESS -u GROK_AGENT \
    ...
    PATH="$fakebin:$BASE_PATH" DJ_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/dj-harness.sh")
  [ "$out" = kimi ] || fail "kimi ancestry detection returned '$out'"
  out=$(env -u CURSOR_AGENT -u CURSOR_INVOKED_AS \
    CLAUDECODE=1 PATH="$fakebin:$BASE_PATH" DJ_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/dj-harness.sh")
  [ "$out" = claude ] || fail "verified env-marker precedence changed, got '$out'"
  pass "dj-harness: markerless kimi is detected by ancestry after env-marker precedence"
}

test_kimi_session_lock_identity() {
  local home fakebin out
  home="$TMP_ROOT/session-lock-home"
  fakebin=$(dj_fakebin "$TMP_ROOT/session-lock-fake")
  mkdir -p "$home/state"
  cat > "$fakebin/ps" <<'SH'
#!/usr/bin/env bash
case "$*" in
...
SH
  ...
  DJ_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/dj-lock.sh" \
    || fail "dj-lock did not acquire from Kimi ancestry"
  case "$(cat "$home/state/.lock")" in
    ''|*[!0-9]*) fail "dj-lock did not record the Kimi harness ancestor" ;;
  esac
  printf '%s\n' "$$" > "$home/state/.lock"
  out=$(DJ_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/dj-lock.sh" status)
  assert_contains "$out" "lock: held by live harness pid" \
    "dj-lock did not recognize Kimi as a live holder"
  pass "dj-lock recognizes Kimi ancestry and live lock holders"
}

test_kimi_busy_signature_is_scoped_to_spinner_lines() {
  local capture
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-tmux-lib.sh"
  unset DJ_BUSY_REGEX
  ...
  for phase in 🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘; do
    ...
    dj_pane_is_busy fake kimi || fail "Kimi spinner phase $phase was not recognized as busy"
  done
  printf 'ordinary response ending with 🌕\n│ > │\n' > "$capture"
  if dj_pane_is_busy fake kimi; then
  ...
  if dj_pane_is_busy fake codex; then
...
test_watcher_never_classifies_kimi_from_its_spinner() (
  ...
  unset DJ_BUSY_REGEX
  DJ_HOME="$TMP_ROOT/watch-home"
  DJ_STATE_OVERRIDE="$state"
  export DJ_HOME DJ_STATE_OVERRIDE
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-watch.sh"
  ...
  if window_is_busy fake "$busy_capture"; then
    fail "dj-watch classified a Kimi task busy from its spinner instead of unknown"
  fi
  [ "$(dj_busy_classify tmux fake kimi kimi-watch "$state" "$busy_capture")" = "unknown kimi-unverified" ] \
    || fail "a Kimi task must classify unknown kimi-unverified"
  printf 'window=fake\nharness=codex\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
    fail "dj-watch applied Kimi's spinner to a recorded Codex task"
  fi
  printf 'window=fake\nharness=grok\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
  ...
  pass "dj-watch classifies Kimi as unknown rather than from its spinner, and Grok's fallback stays isolated"
)

test_kimi_bordered_prompt_needs_no_override() {
  local out
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-composer-lib.sh"
  out=$(dj_composer_classify_content 1 '>')
  [ "$out" = empty ] || fail "kimi's bordered bare > composer should read empty, got '$out'"
  out=$(dj_composer_classify_content 0 '>')
