#!/usr/bin/env bash
# tests/dj-composer-matrix-live-e2e.test.sh - the live composer-matrix guard
# (live-harness-optin family; task dj-composer-thin-adapter-refactor-r1).
#
# The shared composer classifier's shape catalogue (bin/dj-composer-lib.sh) is
# built entirely from vendor-rendered signals, so per
# .agents/skills/jarvis-coding-guidelines it must be proven against the
...
# isolated tmux server and requires the real dj_tmux_composer_state to reach
...
#
# Run explicitly with DJ_COMPOSER_MATRIX_LIVE=1. No prompt is ever submitted
...
if [ "${DJ_COMPOSER_MATRIX_LIVE:-0}" != 1 ]; then
  echo "skip: set DJ_COMPOSER_MATRIX_LIVE=1 to run the live composer-matrix guard"
  exit 0
fi

command -v tmux >/dev/null 2>&1 || { echo "not ok - DJ_COMPOSER_MATRIX_LIVE=1 but tmux is not installed" >&2; exit 1; }

SOCKET="dj-cmx-live-$$"
SESSION="cmxlive"
ZELLIJ_SESSION="dj-cmx-live-zj-$$"
CHECKED=0
FAILED=0
...
SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/dj-cmx-live.XXXXXX")
...
check_harness_idle_empty() {  # <name> <launch-cmd...>
  local name=$1 win="hx-$1" verdict='' i=0 budget=${DJ_COMPOSER_MATRIX_LIVE_POLLS:-45} version dismissed=0 startup_screen
  shift
  ...
  while [ "$i" -lt "$budget" ]; do
    verdict=$(dj_tmux_composer_state "$SESSION:$win")
    [ "$verdict" = empty ] && break
    i=$((i + 1))
...
sleep 1
verdict=$(dj_tmux_composer_state "$SESSION:strictblank")
if [ "$verdict" = unknown ]; then
  if dj_pane_input_pending "$SESSION:strictblank"; then
    CHECKED=$((CHECKED + 1))
...
if command -v zellij >/dev/null 2>&1; then
  ...
  export DJ_ROOT_OVERRIDE="$ROOT"
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-backend.sh"
  dj_backend_source zellij 2>/dev/null \
  ...
  ZJ_BG=$!
  i=0
  while [ "$i" -lt 10 ] && ! dj_backend_zellij_session_exists "$ZELLIJ_SESSION"; do
    i=$((i + 1))
    sleep 0.5
  done
  dj_backend_zellij_session_exists "$ZELLIJ_SESSION" \
    || fail "zellij ($zj_version): probe session setup failed"
  panes=$(dj_backend_zellij_cli "$ZELLIJ_SESSION" action list-panes --json 2>/dev/null) \
  ...
  dj_backend_zellij_send_literal "$target" 'while sleep 1; do date; done' \
    || fail "zellij ($zj_version): clock probe setup write failed"
  dj_backend_zellij_send_key "$target" Enter \
    || fail "zellij ($zj_version): clock probe setup submit failed"
  sleep 2
  probe='# audit-probe-never-submitted'
  dj_backend_zellij_send_literal "$target" "$probe" \
    || fail "zellij ($zj_version): false-positive probe write failed"
  sleep 0.5
  probe_capture=$(dj_backend_zellij_capture "$target" 40 2>/dev/null) \
  ...
  verdict=$(dj_composer_submit_retry_core dj_backend_zellij_send_key dj_backend_zellij_composer_state \
