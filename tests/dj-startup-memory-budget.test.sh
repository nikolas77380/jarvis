...
TMP_ROOT=$(dj_test_tmproot dj-startup-memory-budget)
BUDGET="$ROOT/bin/dj-startup-memory-budget.sh"
BOOTSTRAP="$ROOT/bin/dj-bootstrap.sh"
CONFIG_PUSH="$ROOT/bin/dj-config-push.sh"
...
new_bootstrap_world() {
  ...
  printf '%s\n' '# Jarvis test root' > "$root/AGENTS.md"
...
expect_rejected_read() {
  ...
  set +e
  out=$(DJ_HOME="$home" "$BUDGET" read 2>&1)
  rc=$?
  set -e
...
test_safe_parser_rejects_ambiguous_and_unsafe_values() {
  ...
  printf '42\n' > "$home/config/startup-memory-budget"
  [ "$(DJ_HOME="$home" "$BUDGET" read)" = 42 ] || fail "valid positive decimal budget was rejected"
...
test_budget_accounting_reports_all_three_files_and_safe_failure() {
  ...
  out=$(DJ_HOME="$home" "$BUDGET" report)
  ...
  set +e
  out=$(DJ_HOME="$home" "$BUDGET" report 2>&1)
  rc=$?
  set -e
...
run_config_push() {
  ...
  PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$root" DJ_SEND_SETTLE=0 \
    DJ_FAKE_TMUX_LOG="$log" "$CONFIG_PUSH"
}
...
test_primary_budget_converges_with_exact_reread_and_safe_failures() {
  ...
  assert_contains "$(<"$log")" "Jarvis instruction waiting: list " \
...
echo '# all dj-startup-memory-budget tests passed'
