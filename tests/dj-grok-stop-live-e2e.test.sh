...
if [ "${DJ_GROK_STOP_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_GROK_STOP_LIVE_E2E=1 with DJ_GROK_NATIVE_BIN and DJ_GROK_LEGACY_BIN"
  exit 0
fi
...
NATIVE_BIN=${DJ_GROK_NATIVE_BIN:-}
LEGACY_BIN=${DJ_GROK_LEGACY_BIN:-}
AUTH=${DJ_GROK_AUTH_FILE:-$HOME/.grok/auth.json}
REAL_TMUX=$(command -v tmux || true)
ACTIVE_LAB=

[ -x "$NATIVE_BIN" ] || fail "DJ_GROK_NATIVE_BIN must be an exact executable path"
[ -x "$LEGACY_BIN" ] || fail "DJ_GROK_LEGACY_BIN must be an exact executable path"
[ -f "$AUTH" ] || fail "DJ_GROK_AUTH_FILE must name the already-managed auth artifact"
...
cleanup_exact_cell() {
  ...
  DJ_E2E_TMUX_SOCKET_ID="$lab/dedicated.sock" PATH="$lab/bin:$PATH" \
...
run_cell() {  # <native|legacy> <exact-binary>
  ...
  tmp_base=${TMPDIR:-/tmp}
  tmp_base=${tmp_base%/}
  lab=$(mktemp -d "$tmp_base/dj-grok-stop-$kind.XXXXXX") || return 1
  session="$kind-e2e"
  target="dj-$kind"
  ACTIVE_LAB=$lab
  umask 077
...
[ "\${DJ_E2E_TMUX_SOCKET_ID:-}" = "\$expected" ] || exit 92
...
#!/usr/bin/env bash
printf 'active=%s' "${GROK_TURNEND_GUARD_ACTIVE:-}" >> "${DJ_GROK_E2E_ROOT:?}/resume-invocations.log"
printf ' <%s>' "$@" >> "${DJ_GROK_E2E_ROOT:?}/resume-invocations.log"
printf '\n' >> "${DJ_GROK_E2E_ROOT:?}/resume-invocations.log"
exec "${DJ_GROK_E2E_BIN:?}" "$@"
EOF
  ...
  if [ "$kind" = native ]; then
    cat > "$lab/project/.grok/hooks/00-dj-stop-live-e2e.json" <<EOF
...
EOF
    ...
    cp "$ROOT/.grok/hooks/dj-primary-turnend-guard.json" "$lab/grok-home/hooks/dj-primary-turnend-guard.json"
    rm -f "$lab/project/.grok/hooks/dj-primary-turnend-guard.json"
    cat > "$lab/grok-home/hooks/00-dj-stop-live-e2e.json" <<EOF
{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"bash -lc 'cat >> \"$lab/payloads.jsonl\"'","timeout":30}]}]}}
EOF
...
env -u TMUX -u TMUX_PANE HOME='$lab/home' GROK_HOME='$lab/grok-home' GROK_AGENT=1 \
  DJ_HOME='$lab/fmhome' DJ_ROOT_OVERRIDE='$lab/project' DJ_GROK_E2E_ROOT='$lab' \
  DJ_GROK_E2E_BIN='$binary' DJ_E2E_TMUX_SOCKET_ID='$lab/dedicated.sock' PATH='$lab/bin':"\$PATH" \
...
EOF
  ...
  i=0
  while [ "$i" -lt "${DJ_GROK_STOP_LIVE_TIMEOUT:-600}" ] && [ ! -f "$lab/done" ]; do
    sleep 1
    i=$((i + 1))
  done
