...
#
# bin/dj-remote-job-reap-orphans.sh is a machine-wide sweep by design, so these
...
TMP_ROOT=$(dj_test_tmproot dj-remote-job-orphan-reap)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
REAPER="$ROOT/bin/dj-remote-job-reap-orphans.sh"
...
# --- a real worker fixture, launched exactly the way dj-on's Linux start does -

# build_remote_root <dir>: a minimal but genuine Jarvis code root carrying
# the real worker and job library.
build_remote_root() {
  local root=$1
  mkdir -p "$root/bin"
  cp "$ROOT/bin/dj-remote-job-lib.sh" "$ROOT/bin/dj-remote-job-worker.sh" "$root/bin/"
  chmod +x "$root/bin"/*.sh
  printf 'fixture\n' > "$root/AGENTS.md"
...
start_worker() {
  ...
  pid=$(
    ...
    export DJ_REMOTE_JOB_ORPHAN_GRACE_SECONDS=1
    # shellcheck source=bin/dj-remote-job-lib.sh
    . "$ROOT/bin/dj-remote-job-lib.sh"
    dj_remote_job_start_linux_worker "$root" "$account_home" >&2 || exit 1
    deadline=$(( $(date +%s) + 10 ))
    while [ "$(date +%s)" -lt "$deadline" ]; do
      pid=$(pgrep -f "^/bin/bash $root/bin/dj-remote-job-worker.sh\$" | head -n 1)
...
cat > "$CASE2/remote-root/bin/dj-remote-job-worker.sh" <<'SH'
#!/bin/bash
...
SH
chmod +x "$CASE2/remote-root/bin/dj-remote-job-worker.sh"
printf 'fixture\n' > "$CASE2/remote-root/AGENTS.md"

set -m
"$CASE2/remote-root/bin/dj-remote-job-worker.sh" >/dev/null 2>&1 &
STALE=$!
set +m
track "$STALE"
