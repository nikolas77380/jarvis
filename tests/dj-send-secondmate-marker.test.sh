#!/usr/bin/env bash
# dj-send from-jarvis marker for secondmate targets.
#
# A secondmate is itself a jarvis, so a request relayed to it lands in its own
# chat - which the main jarvis never reads (the only channel back is the terse
# status file). dj-send therefore prepends a from-jarvis marker
# (bin/dj-marker-lib.sh) when, and only when, the resolved target is a task
...
TMP_ROOT=$(dj_test_tmproot dj-send-marker)

# A fake tmux that (a) records the literal text of every `send-keys -l` to
# DJ_SEND_LOG and (b) lets dj-send's submit path reach a clean "empty" verdict.
# display-message yields a numeric cursor_y; capture-pane returns an empty
# bordered composer so dj_tmux_composer_state reads "empty" (submit landed) on the
...
# run_send <fakebin> <home> <send-log> -- <dj-send args...>
# Runs dj-send.sh with the stubs on PATH against the given home (which holds
# state/<id>.meta). DJ_ROOT_OVERRIDE points at the same non-repo home so
# dj-guard's tangle check stays silent; guard noise goes to stderr (discarded).
# DJ_SEND_SETTLE=0 keeps the run fast. Truncates the log first; returns dj-send's
# exit code.
run_send() {
...
# owner (bin/dj-task-inbox-lib.sh). Command substitution strips trailing
# newlines, so byte-exact trailing assertions read the raw record instead.
record_body() {  # <record-path>
  bash -c '. "$1"; dj_task_inbox_body "$2"' _ "$ROOT/bin/dj-task-inbox-lib.sh" "$1"
}
...
test_secondmate_target_is_marked() {
  ...
  case "$(cat "$log")" in
    *"$DJ_FROMFIRST_MARK"*) fail "the marker must ride the record, never the typed doorbell" ;;
  esac
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-pending-reply-lib.sh"
  corr=$(dj_pending_reply_extract_corr "$got")
  [ -f "$(dj_pending_reply_path "$home/state" "$corr")" ] \
    || fail "marked secondmate send should create a parent pending-reply record"
  pass "dj-send: a kind=secondmate target gets the from-jarvis marker and corr prepended"
}
...
test_exact_secondmate_task_id_is_marked() {
  ...
  already_marked="${DJ_FROMFIRST_MARK}corr=${corr} already routed"
  ...
  case "$got" in
    "${DJ_FROMFIRST_MARK}corr=${corr} already routed") : ;;
    *) fail "exact secondmate send altered already-correlated content"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$got" | od -An -tx1)" ;;
  esac
  pass "dj-send: an exact kind=secondmate task id is marked with corr exactly once"
}
...
test_crewmate_target_is_not_marked() {
  ...
  run_send "$fb" "$home" "$log" "dj-build" "fix the test"; rc=$?
  ...
  pass "dj-send: exact-id and stable-label kind=ship selectors are sent unmarked"
}
...
test_explicit_window_is_not_marked() {
  ...
  # metadata must not make dj-send guess the caller's intent and mark it.
  dj_write_secondmate_meta "$home/state/win.meta" "$home" "other:win"
  ...
  pass "dj-send: explicit endpoints stay unmarked with or without local metadata"
}
...
test_key_path_is_not_marked() {
  ...
  home=$(setup_home key)
  dj_write_secondmate_meta "$home/state/domain.meta" "$home" "sess:dj-domain"
  run_send "$fb" "$home" "$log" "dj-domain" --key Escape; rc=$?
  ...
  pass "dj-send: the --key path carries no marker (no literal text is typed)"
}

test_marker_is_label_plus_invisible_separator() {
  local separator hex
  separator=$(printf '\342\201\243')
  [ "$DJ_FROMFIRST_MARK" = "[dj-from-jarvis]$separator" ] \
    || fail "marker is not the expected label + U+2063 sequence"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$DJ_FROMFIRST_MARK" | od -An -tx1)"
  hex=$(printf '%s' "$DJ_FROMFIRST_MARK" | od -An -tx1 | tr -d ' \n')
  case "$hex" in
    *e281a3) : ;;
    *) fail "marker does not end in UTF-8 U+2063 bytes e2 81 a3; bytes were: $hex" ;;
  esac
  dj_message_from_jarvis "${DJ_FROMFIRST_MARK}do the work" \
    || fail "detector should recognize a marked message"
  dj_message_from_jarvis "do the work" \
    && fail "direct captain input must remain unmarked"
  dj_message_from_jarvis "[dj-from-jarvis]do the work" \
    && fail "detector must reject the label without U+2063"
  pass "dj-send: the marker is '[dj-from-jarvis]' + terminal-safe U+2063, while direct captain text stays unmarked"
}

test_marker_transformation_is_idempotent() {
  local once twice
  dj_message_mark_from_jarvis "do the work" once
  dj_message_mark_from_jarvis "$once" twice
  [ "$once" = "$twice" ] \
    || fail "already-marked content was double-prefixed"$'\n'"--- once ---"$'\n'"$(printf '%s' "$once" | od -An -tx1)"$'\n'"--- twice ---"$'\n'"$(printf '%s' "$twice" | od -An -tx1)"
  [ "$once" = "${DJ_FROMFIRST_MARK}do the work" ] \
    || fail "marker transformation did not prefix bare content exactly once"
  pass "dj-marker: from-jarvis transformation is idempotent"
}
...
test_marked_send_preserves_trailing_newlines() {
  ...
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-pending-reply-lib.sh"
  corr=$(dj_pending_reply_extract_corr "$(record_body "$home/state/domain.inbox/001.msg")")
  [ -n "$corr" ] || fail "marked send should embed a corr id"
  dj_pending_reply_embed_corr "$payload" "$corr" expected_message
  ...
  pass "dj-send: marked secondmate payload preserves trailing newline bytes in its record"
}
