...
CASCADE="$ROOT/bin/dj-stow-cascade.sh"
TMP_ROOT=$(dj_test_tmproot dj-stow-cascade)
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
FAKEBIN=$(dj_fakebin "$TMP_ROOT/fakebin")
BASE_PATH=${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}

# A fake ssh standing in for the whole remote transport. It decodes dj-on.sh's
...
case "$command" in
  dj-startup-memory-budget.sh)
    cat "$DJ_FAKE_REMOTE_BUDGET"
    ;;
  dj-remote-secondmate-control.sh)
    printf '%s\n' "${DJ_FAKE_REMOTE_AGENT_STATE:-alive}"
    ;;
  *) exit 91 ;;
esac
SH
...
test_every_registered_home_is_enumerated_exactly_once() {
  ...
  dj_write_secondmate_meta "$primary/state/enum-a.meta" "$a"
  dj_write_secondmate_meta "$primary/state/enum-a-stale.meta" "$a"
...
test_transport_routes_by_placement_and_liveness() {
  ...
  dj_write_secondmate_meta "$primary/state/live-local.meta" "$live" 'jarvis:dj-live-local' alpha claude
  dj_write_secondmate_meta "$primary/state/remote-live.meta" "$remote" 'dj-remote:dj-remote-live' alpha claude
  printf 'role=secondmate\neffective_budget_tokens=7500\ntotal_estimated_tokens=100\nbudget_status=within-budget\n' \
    > "$TMP_ROOT/remote-budget.txt"

  set +e
  out=$(run_cascade "$primary" \
    DJ_FAKE_TMUX_WINDOW='dj-live-local' \
    DJ_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    DJ_FAKE_REMOTE_AGENT_STATE=alive)
  set -e
  ...
  out=$(run_cascade "$primary" \
    DJ_FAKE_TMUX_WINDOW='dj-live-local' \
    DJ_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    DJ_FAKE_REMOTE_AGENT_STATE=dead)
  set -e
  s=$(stanza "$out" remote-live)
...
test_a_slow_remote_is_bounded_and_the_rest_still_report() {
  ...
  dj_write_secondmate_meta "$primary/state/bounded-remote.meta" "$remote" 'dj-remote:dj-bounded-remote'
  printf 'role=secondmate\n' > "$TMP_ROOT/remote-budget.txt"

  started=$(date +%s)
  set +e
  out=$(run_cascade "$primary" \
    DJ_STOW_CASCADE_TIMEOUT=2 \
    DJ_FAKE_SSH_MODE=hang \
    DJ_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt")
  rc=$?
  set -e
