...
HERDR_LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/dj-herdr-lab.sh}
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-herdr-presentation.XXXXXX")
...
export DJ_BACKEND_HERDR_WORKSPACE_MOVER="$FAKEBIN/herdr-workspace-mover"
...
HERDR_LAB_SESSION=$(PATH="$HERDR_ORIGINAL_PATH" \
  "$HERDR_LAB_HELPER" name dj-herdr-presentation-projection)
...
# defect is demonstrated by tests/dj-backend-herdr-focus-flash-e2e.test.sh);
...
make_project() {  # <dir>
  ...
  git -C "$dir" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm initial
...
spawn_task() {  # <id> <home> <project>
  ...
  DJ_GATE_REFUSE_BYPASS=1 DJ_SPAWN_NO_GUARD=1 DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/dj-spawn.sh" "$id" "$project" "sh -c 'sleep 120'" --mode no-mistakes --yolo off --backend herdr
}
...
spawn_secondmate_task() {
  local id=$1 home=$2
  DJ_GATE_REFUSE_BYPASS=1 DJ_SPAWN_NO_GUARD=1 DJ_HOME="$HOME_DIR" DJ_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/dj-spawn.sh" "$id" "$home" "sh -c 'sleep 120'" --secondmate --backend herdr
}

teardown_task() {  # <id> <home>
  local id=$1 home=$2
  DJ_GATE_REFUSE_BYPASS=1 DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_STATE_OVERRIDE="$home/state" DJ_DATA_OVERRIDE="$home/data" \
    DJ_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/dj-teardown.sh" "$id" --force
}
...
session_presentation_lock_path() {
  PATH="$FAKEBIN:$PATH" HERDR_SESSION="$HERDR_LAB_SESSION" bash -c '
    ...
    dj_backend_herdr_presentation_session_lock_path "$1"
...
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config" \
  ...
  "$HOME_DIR/data/order-fail" "$HOME_DIR/data/dj-hibit-resume-r1" \
...
# Keep one ordinary primary task live so the durable jarvis workspace is
...
JARVIS_WSID=$(grep '^herdr_workspace_id=' "$ANCHOR_META" | cut -d= -f2-)
[ -n "$JARVIS_WSID" ] || fail "anchor metadata did not record the jarvis workspace"
...
if [ "$FLOOR_VERDICT" = 0 ]; then
  ...
  [ -n "$DEFAULT_ON_WSID" ] && [ "$DEFAULT_ON_WSID" != "$JARVIS_WSID" ] \
    || fail "an unconfigured home reused the flat jarvis workspace instead of projecting"
  ...
  [ "$DEFAULT_ON_WSID" = "$JARVIS_WSID" ] \
    || fail "an unconfigured home did not land in the flat jarvis workspace on below-floor herdr $FLOOR_VERSION (got '${DEFAULT_ON_WSID:-<empty>}')"
...
printf '%s' "$PROJECTED_TABS" | jq -e --arg tab "$PROJECTED_TAB" \
  '.result.tabs[0].tab_id == $tab and .result.tabs[0].label == "dj-shape"' >/dev/null 2>&1 \
  || fail "projected workspace's only tab was not the normal dj-shape task tab"
...
PATH="$FAKEBIN:$PATH" DJ_HOME="$HOME_DIR" bash -c '
  . "$0/bin/dj-wake-lib.sh"
  . "$0/bin/backends/herdr.sh"
  lock=$1
  dj_lock_acquire_wait "$lock"
  dj_backend_herdr_projection_cleanup_exact "$2" "$3" "$4"
...
[ "$LOCK_CONTENTION_WSID" = "$JARVIS_WSID" ] \
  || fail "bounded lock contention did not use the ordinary flat jarvis workspace"
...
EXPECTED_LABELS=$(printf 'jarvis\n%s\n%s\n2ndmate-alpha\n2ndmate-bravo' "$PROJECTED_LABEL" "$CREATED_LABELS")
ACTUAL_LABELS=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[].label')
[ "$ACTUAL_LABELS" = "$EXPECTED_LABELS" ] || fail "workspace order was not jarvis, stable primary block, secondmates: $ACTUAL_LABELS"
PRIMARY_IDS=$(printf '%s' "$ORDER_LIST" | jq -r '
  .result.workspaces[]
  | select((.label | startswith("└ ")) or (.label | startswith("jarvis/")))
  | .workspace_id
')
...
DJ_BACKEND_HERDR_WORKSPACE_MOVER="$FAIL_MOVER" \
...
for ROUND in 1 2 3; do
  ...
  WAVE_EXPECTED=$(printf 'jarvis\n%s\n2ndmate-alpha\n2ndmate-bravo' "$WAVE_LABELS")
  WAVE_ACTUAL=$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "jarvis" or (.label | startswith("└ ")) or (.label | startswith("2ndmate-"))) | .label')
...
# ------------------------------------------------------------------
# Multi-home topology: real secondmate DJ_HOME spawn paths, inheritance,
...
printf 'alpha\n' > "$SECOND_HOME_A/.dj-secondmate-home"
printf 'bravo\n' > "$SECOND_HOME_B/.dj-secondmate-home"
touch "$SECOND_HOME_A/state/.last-watcher-beat" "$SECOND_HOME_B/state/.last-watcher-beat"
# Ensure the secondmate homes look like gitignored jarvis homes so inheritance
...
git -C "$SECOND_HOME_A" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm init
git -C "$SECOND_HOME_B" -c user.name='Jarvis Tests' -c user.email='tests@example.invalid' commit -qm init
mkdir -p "$SECOND_HOME_A/bin"
printf '# Jarvis secondmate fixture\n' > "$SECOND_HOME_A/AGENTS.md"
...
printf '%s' "$CROSS_LIST" | jq -e '
  ([.result.workspaces[].label] | index("jarvis")) as $fm
...
case "$PCW_LABEL" in $'└ pcw · p:'*|jarvis) ;; *) fail "cross-home primary label wrong: $PCW_LABEL" ;; esac
...
# Exercise both the leading dj- identity style seen in Hi Bit work and the
# project-name identity style used by Wheelhouse work.
for RESTART_ID in dj-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  EXPECTED_CONCISE=${RESTART_ID#dj-}
  ...
  if [ "$RESTART_ID" = dj-hibit-resume-r1 ]; then
...
LEGACY_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "jarvis/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" --no-focus) \
...
FLAT_TAB_OUT=$(lab tab create --workspace "$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "2ndmate-alpha") | .workspace_id' | head -1)" --cwd "$PROJECT_DIR" --label dj-flat-legacy-tab --no-focus) \
...
[ "$(lab workspace get "$LEGACY_WSID" | jq -r '.result.workspace.label')" = "jarvis/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" ] \
...
dj_backend_herdr_projection_journal_create "$MISSING_STATE" missing1 >/dev/null
MISSING_JOURNAL=$(dj_backend_herdr_projection_journal_path "$MISSING_STATE" missing1)
START=$(log_line_count)
dj_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$MISSING_JOURNAL" missing1 \
...
RENAMED_TOKEN=$(dj_backend_herdr_projection_journal_create "$RENAMED_STATE" renamed1)
RENAMED_JOURNAL=$(dj_backend_herdr_projection_journal_path "$RENAMED_STATE" renamed1)
RENAMED_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "jarvis/renamed1 · p:$RENAMED_TOKEN" --no-focus)
...
START=$(log_line_count)
dj_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$RENAMED_JOURNAL" renamed1 \
...
DUP_TOKEN=$(dj_backend_herdr_projection_journal_create "$DUP_STATE" duplicate1)
DUP_JOURNAL=$(dj_backend_herdr_projection_journal_path "$DUP_STATE" duplicate1)
DUP1=$(lab workspace create --cwd "$PROJECT_DIR" --label "jarvis/duplicate1 · p:$DUP_TOKEN" --no-focus)
...
START=$(log_line_count)
dj_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1 \
...
lab pane report-agent "$DUP1_PANE" --source dj-projection-e2e --agent test-agent --state idle >/dev/null \
  || fail "could not register the duplicate-live-agent risk fixture"
START=$(log_line_count)
if dj_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1; then
