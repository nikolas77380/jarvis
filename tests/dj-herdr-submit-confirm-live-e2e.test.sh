...
# and requires dj_backend_herdr_send_text_submit to report empty for a landed
# idle steer. It fails naming the harness and version rather than degrading
# quietly.
#
# Run explicitly with DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 after a Herdr or Claude
# upgrade, and before trusting a refreshed docs/verification/runtime-backends.md
# "Herdr submit confirmation" entry.
# Every Herdr call, including adapter calls, is routed through bin/dj-herdr-lab.sh.
set -u
...
if [ "${DJ_HERDR_SUBMIT_CONFIRM_LIVE:-0}" != 1 ]; then
  echo "skip: set DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 to run the live Herdr submit-confirmation guard"
  exit 0
fi

command -v herdr >/dev/null 2>&1 || fail "DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 but herdr is not installed"
command -v jq >/dev/null 2>&1 || fail "DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 but jq is not installed"
command -v claude >/dev/null 2>&1 || fail "DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 but Claude Code is not installed"
[ -x "$LAB_HELPER" ] || fail "DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 but the Herdr lab helper is not executable at $LAB_HELPER"
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-herdr-submit-confirm-live.XXXXXX")
...
WS_JSON=$(lab workspace create --cwd "$ROOT" --label dj-submitlive --no-focus) \
...
verdict=$(dj_backend_herdr_send_text_submit "$TARGET" "Reply with exactly $TOKEN and nothing else." 3 0.4 0.4) \
...
[ "$CHECKED" -gt 0 ] || fail "DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 checked no harness"
