#!/usr/bin/env bash
# tests/dj-backend-herdr.test.sh - fake-herdr-CLI unit tests for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/dj-backend-design-d7 (herdr-addendum.md). Mirrors tests/dj-backend.test.sh's
...
# The real-binary smoke test lives in tests/dj-backend-herdr-smoke.test.sh,
...
TMP_ROOT=$(dj_test_tmproot dj-backend-herdr-tests)
export DJ_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0

# make_herdr_fakebin: a `herdr` stub that logs every invocation (one line,
# unit-separated args, to $DJ_HERDR_LOG) and returns the canned response for
# that call read from $DJ_HERDR_RESPONSES/<n>.out, consumed IN ORDER (call 1
...
if [ "${1:-}" = status ] && [ "${2:-}" = --json ] && [ "${DJ_HERDR_SCRIPT_STATUS:-0}" != 1 ]; then
...
# repeatedly and the "one persistent jarvis workspace, no orphans"
...
# ($DJ_FAKE_HERDR_STATE) mutated with real jq. Modeled behaviors, all
...
# a pane with no registered agent). Every call is logged to $DJ_HERDR_LOG in
...
# Used to exercise dj_backend_herdr_workspace_prune_seeded_default_tab's
...
# herdr_case <name> -> sets up DJ_HERDR_LOG/DJ_HERDR_RESPONSES/fb for one test,
...
test_version_check_accepts_current_protocol() {
  ...
  PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_version_check' "$ROOT"
  status=$?
  ...
  pass "dj_backend_herdr_version_check: accepts the current protocol (14)"
}
...
test_version_check_refuses_old_protocol() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_version_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_version_check: refuses an old protocol loudly"
}
...
test_version_check_refuses_missing_herdr() {
  ...
  pass "dj_backend_herdr_version_check: refuses loudly when herdr is not installed"
}

# --- workspace_label: per-jarvis-HOME resolution (P3, herdr-sm-spaces-k4) -

test_workspace_label_primary_home_no_marker() {
  local home
  home="$TMP_ROOT/primary-home-no-marker"; mkdir -p "$home"
  out=$( DJ_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "jarvis" ] || fail "a primary home (no .dj-secondmate-home marker) should resolve to label 'jarvis', got '$out'"
  pass "dj_backend_herdr_workspace_label: a primary home (no marker) resolves to 'jarvis'"
}

test_workspace_label_secondmate_home_uses_marker_id() {
  local home
  home="$TMP_ROOT/secondmate-home"; mkdir -p "$home"
  printf 'sshhip-h7\n' > "$home/.dj-secondmate-home"
  out=$( DJ_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "2ndmate-sshhip-h7" ] || fail "a secondmate home should resolve to '2ndmate-<id>', got '$out'"
  pass "dj_backend_herdr_workspace_label: a secondmate home (.dj-secondmate-home) resolves to '2ndmate-<id>'"
}
...
test_workspace_label_secondmate_marker_trims_whitespace() {
  ...
  pass "dj_backend_herdr_workspace_label: trims whitespace around the marker's secondmate id"
}
...
test_workspace_label_empty_marker_falls_back_to_primary() {
  ...
  : > "$home/.dj-secondmate-home"
  out=$( DJ_HOME="$home" bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_label' "$ROOT" )
  [ "$out" = "jarvis" ] || fail "an empty/unreadable marker should fall back to 'jarvis', got '$out'"
  pass "dj_backend_herdr_workspace_label: an empty marker file falls back to the primary label 'jarvis'"
}

test_workspace_label_different_secondmates_get_different_labels() {
  local home1 home2 out1 out2
  home1="$TMP_ROOT/secondmate-a"; mkdir -p "$home1"; printf 'alpha-a1\n' > "$home1/.dj-secondmate-home"
  home2="$TMP_ROOT/secondmate-b"; mkdir -p "$home2"; printf 'bravo-b2\n' > "$home2/.dj-secondmate-home"
  out1=$( DJ_HOME="$home1" bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_label' "$ROOT" )
  out2=$( DJ_HOME="$home2" bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_label' "$ROOT" )
  ...
  pass "dj_backend_herdr_workspace_label: two different secondmate homes get two different, non-colliding labels"
}

# --- dj_backend_herdr_cli: session targeting (2026-07-02 incident fix) -------

test_cli_helper_sets_env_and_appends_trailing_session_flag() {
  local dir log resp fb
  dir="$TMP_ROOT/cli-helper"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_cli fmtest workspace list' "$ROOT"
  expect_code 0 $? "dj_backend_herdr_cli should succeed"
  assert_contains "$(cat "$log")" "HERDR_SESSION=fmtest"$'\x1f''workspace'$'\x1f''list' \
    "dj_backend_herdr_cli did not set the HERDR_SESSION env var"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''list'$'\x1f''--session'$'\x1f''fmtest' \
    "dj_backend_herdr_cli did not append a trailing --session <name> flag (the fix for the env-var-alone routing bug)"
  pass "dj_backend_herdr_cli: sets HERDR_SESSION AND appends a trailing --session flag on every call"
}
...
# every process it manages a pane for, so a jarvis or secondmate agent's own
...
test_launcher_identity_absent_without_a_herdr_pane() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
  ...
  pass "dj_backend_herdr_launcher_identity: a jarvis not running inside herdr has no launcher workspace to inherit"
}
...
test_launcher_identity_absent_when_herdr_env_alone_is_set() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" HERDR_ENV=1 \
    \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
  expect_code 2 "$status" "HERDR_ENV=1 alone is a backend-selection marker, not a parent binding"
  pass "dj_backend_herdr_launcher_identity: HERDR_ENV=1 without a pane id selects the backend but binds no parent"
}
...
test_launcher_identity_resolves_the_exact_pane_tab_and_workspace() {
  ...
  printf '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/dj-herdr-unit/fmtest.sock"}]}\n' > "$resp/1.out"
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w7","label":"jarvis"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/dj-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest || exit 1
      printf "%s|%s|%s" "$DJ_BACKEND_HERDR_LAUNCHER_PANE_ID" "$DJ_BACKEND_HERDR_LAUNCHER_TAB_ID" "$DJ_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID"' "$ROOT" )
  [ "$out" = 'w7:p3|w7:t3|w7' ] \
  ...
  pass "dj_backend_herdr_launcher_identity: resolves the launcher's exact workspace even when a same-labeled workspace sorts first"
}
...
test_launcher_identity_refuses_a_pane_from_another_session_name() {
  ...
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    ...
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_launcher_identity: refuses a launcher pane that names a different herdr session"
}
...
test_launcher_identity_refuses_a_missing_server_socket() {
  ...
  pass "dj_backend_herdr_launcher_identity: refuses a claimed pane without exact server identity"
}
...
test_launcher_identity_refuses_a_pane_from_another_server_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/dj-herdr-unit/other.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_launcher_identity: refuses a launcher pane whose injected socket belongs to another herdr server"
}
...
test_launcher_identity_refuses_an_unreadable_pane() {
  ...
  pass "dj_backend_herdr_launcher_identity: refuses when the launcher's own pane no longer resolves"
}
...
test_launcher_identity_refuses_a_pane_and_tab_that_disagree() {
  ...
  pass "dj_backend_herdr_launcher_identity: refuses when the launcher's pane and tab disagree about their workspace"
}
...
test_launcher_identity_refuses_a_workspace_missing_from_the_session() {
  ...
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/dj-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  ...
  pass "dj_backend_herdr_launcher_identity: refuses when the launcher's workspace is gone from its own session"
}
...
test_workspace_ensure_prefers_the_launcher_over_the_first_label_match() {
  ...
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/dj-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_ensure fmtest /tmp' "$ROOT" )
  ...
  pass "dj_backend_herdr_workspace_ensure: places a worker in the launcher's exact workspace, not the first same-labeled one"
}
...
test_workspace_ensure_refuses_an_ambiguous_label_with_no_launcher() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w7","label":"jarvis"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_ensure fmtest /tmp' "$ROOT" 2>&1 )
  status=$?
  expect_code 3 "$status" "two same-labeled home workspaces with no launcher identity must refuse"
  assert_contains "$out" "labeled 'jarvis'" "the ambiguity refusal did not name the duplicated label"
  ...
  pass "dj_backend_herdr_workspace_ensure: refuses to guess between two same-labeled home workspaces"
}
...
test_workspace_ensure_other_home_ignores_the_launcher_identity() {
  ...
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_ensure fmtest /tmp other-home' "$ROOT" )
  ...
  pass "dj_backend_herdr_workspace_ensure: a --secondmate container resolves that home's own workspace, not the launcher's"
}
...
test_container_ensure_refuses_an_ambiguous_home_label() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /tmp' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "container_ensure must fail when the home workspace is ambiguous"
  assert_contains "$out" "labeled 'jarvis'" "container_ensure buried the specific ambiguity it refused"
  assert_not_contains "$out" "failed to ensure herdr workspace" "container_ensure added a generic message over the specific one"
  pass "dj_backend_herdr_container_ensure: surfaces the exact ambiguous-placement refusal instead of a generic failure"
}
...
test_container_ensure_starts_server_and_workspace() {
  ...
  # 5: workspace list -> empty (no "jarvis" workspace yet)
  ...
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"jarvis"},"tab":{"tab_id":"w1:t9"},"root_pane":{"pane_id":"w1:p9"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /tmp' "$ROOT" )
  ...
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''jarvis' \
    "container_ensure did not create the jarvis workspace with the given cwd"
  pass "dj_backend_herdr_container_ensure: version-gates, starts the server, ensures the jarvis workspace, echoes session:workspace_id + the seeded default tab id"
}
...
test_container_ensure_reuses_existing_workspace() {
  ...
  printf '{"client":{"version":"0.7.1","protocol":14}}\n' > "$resp/1.out"
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w9","label":"jarvis"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w9\t' ] || fail "container_ensure should reuse the existing jarvis workspace id with an EMPTY seeded-tab field (an ADOPTED workspace is never a prune candidate), got '$out'"
  assert_not_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create' "container_ensure should not create a workspace that already exists"
  pass "dj_backend_herdr_container_ensure: reuses an existing jarvis workspace without recreating it, and reports no seeded default tab (adopted, not created)"
}

test_create_task_refuses_duplicate_label() {
  local dir log resp fb out status
  dir="$TMP_ROOT/dup-task"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-dup1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_create_task: refuses a duplicate tab label (herdr's own tab create has no uniqueness check)"
}
...
# panes) across a server restart, including a reboot. A restored dj-<id> task
...
test_create_task_refuses_duplicate_label_when_agent_live() {
  ...
  pass "dj_backend_herdr_create_task: a same-labeled tab with a live (even idle) registered agent still refuses exactly as before"
}
...
test_create_task_refuses_when_any_duplicate_label_is_live() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-mixed1","workspace_id":"w1"},{"tab_id":"w1:t3","label":"dj-mixed1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-mixed1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_create_task: scans every same-labeled tab and refuses if any duplicate is live"
}
...
test_create_task_closes_and_replaces_dead_pane_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-husk1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"dj-husk1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-husk1 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''dj-husk1' \
  ...
  pass "dj_backend_herdr_create_task: closes and replaces a same-labeled tab whose pane is dead (pane_not_found)"
}
...
test_create_task_closes_and_replaces_no_agent_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-husk2","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"dj-husk2","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-husk2 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''dj-husk2' \
    "create_task did not create the replacement tab"
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''close'$'\x1f''w1:t2' "create_task did not close the no-agent husk's tab"
  pass "dj_backend_herdr_create_task: closes and replaces a same-labeled tab whose pane is alive but hosts no registered agent (a restored plain shell)"
}
...
test_create_task_closes_all_duplicate_husks_after_replacement() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-husk-many","workspace_id":"w1"},{"tab_id":"w1:t3","label":"dj-husk-many","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/5.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t4","label":"dj-husk-many","workspace_id":"w1"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-husk-many /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  pass "dj_backend_herdr_create_task: closes every confirmed same-labeled husk only after creating the replacement"
}
...
test_create_task_refuses_when_preexisting_husk_tab_remains() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '1\n' > "$resp/6.exit"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-stale-husk","workspace_id":"w1"},{"tab_id":"w1:t3","label":"dj-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-stale-husk /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_create_task: refuses success when a preexisting husk tab remains after replacement"
}
...
test_create_task_refuses_when_agent_state_ambiguous() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-ambig1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-ambig1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_create_task: refuses (fail-safe) rather than guessing when the duplicate's agent state cannot be classified confidently"
}
...
test_create_task_husk_replacement_creates_before_closing() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"dj-order1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"error":{"code":"pane_not_found","message":"pane w1:p2 not found"}}\n' > "$resp/3.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/4.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"dj-order1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-order1 /tmp/proj' "$ROOT" ) \
  ...
  pass "dj_backend_herdr_create_task: creates the replacement tab BEFORE closing the husk tab, never the reverse"
}
...
test_create_task_creates_and_parses_ids() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task fmtest:w1 dj-newtask /tmp/proj' "$ROOT" )
  [ "$out" = "w1:t2 w1:p2" ] || fail "create_task should echo '<tab_id> <pane_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''dj-newtask' \
  ...
  pass "dj_backend_herdr_create_task: creates a tab and parses tab_id/pane_id from the JSON response, prunes nothing when no seeded tab id is given"
}
...
test_container_ensure_creates_with_no_focus_flag() {
  ...
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[]}}\n' > "$resp/3.out"
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"jarvis"},"tab":{"tab_id":"w1:t1"},"root_pane":{"pane_id":"w1:p1"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w1\tw1:t1' ] || fail "container_ensure should still echo '<session>:<workspace_id>\\t<seeded_default_tab_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''jarvis'$'\x1f''--no-focus' \
    "container_ensure's workspace create did not pass --no-focus (focus-safety: never steal the captain's attention on spawn)"
  pass "dj_backend_herdr_container_ensure: workspace create passes --no-focus"
}
...
test_container_ensure_uses_secondmate_home_label() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HOME="$home" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /tmp' "$ROOT" )
  ...
  pass "dj_backend_herdr_container_ensure: creates the workspace under the SECONDMATE home's own label, not 'jarvis'"
}
...
test_create_task_creates_with_no_focus_flag() {
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''dj-newtask'$'\x1f''--no-focus' \
    "create_task's tab create did not pass --no-focus"
  pass "dj_backend_herdr_create_task: tab create passes --no-focus"
}
...
# dj_backend_herdr_presentation_enabled is the one gate bin/dj-spawn.sh consults
...
presentation_enabled_verdict() {  # <config-dir> <fakebin> [state-dir] [session] -> "on"/"off"
  HERDR_SESSION="${4:-}" PATH="$2:$PATH" bash -c '
    ...
    if dj_backend_herdr_presentation_enabled "$1" "$2"; then printf "on\n"; else printf "off\n"; fi
  ' "$ROOT" "$1" "${3:-}"
}
...
release_floor_verdict() {  # <protocol> <version> -> above|below|indeterminate
  bash -c '
    . "$0/bin/backends/herdr.sh"
    status=0
    dj_backend_herdr_release_floor_verdict "$1" "$2" || status=$?
    case "$status" in
      0) printf "above\n" ;;
      1) printf "below\n" ;;
...
test_presentation_preference_reports_three_distinct_states() {
  ...
  preference() {
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_preference "$1"' "$ROOT" "$1" 2>/dev/null
  }
...
test_projection_journal_is_atomic_and_uses_128_bit_token() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(dj_backend_herdr_projection_journal_create "$1" task-p1) || exit 1
    parsed=$(dj_backend_herdr_projection_journal_token "$1/task-p1.herdr-presentation" task-p1) || exit 1
    printf "%s\n%s\n" "$token" "$parsed"
    dj_backend_herdr_projection_journal_create "$1" task-p1 >/dev/null 2>&1
  ' "$ROOT" "$state" 2>&1)
  status=$?
...
test_projection_journal_v2_binds_and_advances_exact_endpoint() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(dj_backend_herdr_projection_journal_create "$1" dj-hibit-r1) || exit 1
    journal="$1/dj-hibit-r1.herdr-presentation"
    home=$(dj_backend_herdr_projection_home_identity "$2") || exit 1
    label=$(dj_backend_herdr_projection_workspace_label dj-hibit-r1 "$token")
    dj_backend_herdr_projection_journal_bind \
      "$journal" dj-hibit-r1 "$home" lab-session w2 w2:t2 w2:p2 w1 jarvis "$label" dj-dj-hibit-r1 || exit 1
    dj_backend_herdr_projection_journal_snapshot "$journal" dj-hibit-r1 || exit 1
    printf "%s|%s|%s|%s|%s|%s|%s\n" \
      "$DJ_BACKEND_HERDR_JOURNAL_VERSION" \
      "$DJ_BACKEND_HERDR_JOURNAL_HOME" \
      "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" \
      "$DJ_BACKEND_HERDR_JOURNAL_TAB_ID" \
      "$DJ_BACKEND_HERDR_JOURNAL_PANE_ID" \
      "$DJ_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" \
      "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL"
    dj_backend_herdr_projection_journal_replace_endpoint \
      "$journal" dj-hibit-r1 w2:t2 w2:p2 w2:t3 w2:p3 || exit 1
    dj_backend_herdr_projection_journal_snapshot "$journal" dj-hibit-r1 || exit 1
    printf "%s|%s\n" "$DJ_BACKEND_HERDR_JOURNAL_TAB_ID" "$DJ_BACKEND_HERDR_JOURNAL_PANE_ID"
  ' "$ROOT" "$state" "$home") || fail "version 2 projection journal binding failed"
  token=$(sed -n 's/^projection_id=//p' "$state/dj-hibit-r1.herdr-presentation")
  ...
  [ "$(wc -l < "$state/dj-hibit-r1.herdr-presentation" | tr -d '[:space:]')" = 12 ] \
    || fail "version 2 projection journal must have exactly 12 fields"
  printf 'pane_id=duplicate\n' >> "$state/dj-hibit-r1.herdr-presentation"
  if bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_journal_snapshot "$1" dj-hibit-r1' \
    "$ROOT" "$state/dj-hibit-r1.herdr-presentation"; then
...
test_projection_create_uses_exact_response_ids_and_leaves_one_task_pane() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"dj-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"dj-task-p2","workspace_id":"w9"}]}}\n' > "$resp/10.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      dj_backend_herdr_projection_focus_snapshot() { printf "captain-ws\tcaptain-tab"; }
      dj_backend_herdr_projection_focus_restore() { return 0; }
      token=$(dj_backend_herdr_projection_journal_create "$1" task-p2) || exit 1
      label=$(dj_backend_herdr_projection_workspace_label task-p2 "$token")
      dj_backend_herdr_projection_create_task /tmp/proj "$label" dj-task-p2 || exit 1
      printf "%s %s %s %s %s\n" \
        "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
        "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
        "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID" \
        "$DJ_BACKEND_HERDR_PROJECTION_TAB_ID" \
        "$DJ_BACKEND_HERDR_PROJECTION_PANE_ID"
  ...
  token=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_journal_token "$1" task-p2' "$ROOT" "$journal") \
...
test_projection_create_never_closes_a_concurrent_same_label_tab() {
  ...
  printf '{"result":{"workspace":{"workspace_id":"w9"},"tab":{"tab_id":"w9:t1"},"root_pane":{"pane_id":"w9:p1"}}}\n' > "$resp/1.out"
  printf '{"result":{"tab":{"tab_id":"w9:t2"},"root_pane":{"pane_id":"w9:p2"}}}\n' > "$resp/2.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"dj-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"dj-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p1","tab_id":"w9:t1"},{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/4.out"
  printf '{"error":{"code":"agent_not_found"}}\n' > "$resp/5.out"
  printf '{"result":{"pane":{"pane_id":"w9:p1","tab_id":"w9:t1","workspace_id":"w9"}}}\n' > "$resp/6.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"dj-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"dj-task-p2","workspace_id":"w9"}]}}\n' > "$resp/7.out"
  printf '{"error":{"code":"pane_not_found"}}\n' > "$resp/9.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"dj-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"dj-task-p2","workspace_id":"w9"}]}}\n' > "$resp/10.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "captain-ws\tcaptain-tab"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_create_task /tmp/proj label dj-task-p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_focus_snapshot_requires_exact_workspace_and_tab() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot fmtest' "$ROOT") \
...
test_projection_close_restores_exact_prior_focus() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w9:p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_rechecks_required_agent_state_at_boundary() {
  ...
  out=$(ROOT="$ROOT" LOG="$log" bash -c '
    ...
    dj_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }
    dj_backend_herdr_pane_agent_state() { printf live; }
    dj_backend_herdr_cli() {
      printf "%s\n" "$*" >> "$LOG"
      case "$2 $3" in
    ...
    set +e
    dj_backend_herdr_projection_close_pane_focus_preserving fmtest w9:p2 no-agent
    rc=$?
    set -e
    printf "%s:%s" "$rc" "$DJ_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE"
  ')
  status=${out%%:*}
...
# the pane-death close fixtures. The mover appends to $DJ_FAKE_MOVER_LOG and
# exits 9 unless $DJ_FAKE_MOVER_RESPONSE names a readable response file.
...
SH
  cat > "$dir/mover" <<'SH'
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$DJ_FAKE_MOVER_LOG"
calls=$(wc -l < "$DJ_FAKE_MOVER_LOG" | tr -d ' ')
if [ "$calls" -ge 2 ] && [ -f "${DJ_FAKE_MOVER_RESPONSE_2:-}" ]; then
  cat "$DJ_FAKE_MOVER_RESPONSE_2"
  exit 0
fi
if [ -f "$DJ_FAKE_MOVER_RESPONSE" ]; then
  cat "$DJ_FAKE_MOVER_RESPONSE"
  exit 0
fi
exit 9
SH
...
test_projection_close_emptying_after_focus_uses_pane_death_without_move() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/no-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_emptying_before_focus_repositions_then_uses_pane_death() {
  ...
  make_death_lab "$dir" "$bgpid"
  printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false},{"workspace_id":"w1","focused":false}]}}' > "$dir/mover-response"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/mover-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w1:p1' "$ROOT" 2>&1)
  status=$?
  kill "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
test_projection_close_emptying_last_workspace_needs_no_move() {
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    ...
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/no-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w3:p1' "$ROOT" 2>&1)
  status=$?
  kill "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
test_projection_close_plain_without_move_requires_structured_removal() {
  ...
  out=$(ROOT="$ROOT" LOG="$log" bash -c '
    . "$ROOT/bin/backends/herdr.sh"
    dj_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }
    dj_backend_herdr_emptying_close_plan() { printf "plain\n"; }
    dj_backend_herdr_projection_focus_restore() { return 0; }
    dj_backend_herdr_cli() {
      printf "%s\n" "$*" >> "$LOG"
      case "$2 $3" in
    ...
    dj_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2
  ' 2>&1)
  status=$?
...
test_projection_close_busy_pane_falls_back_to_plain_close() {
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/no-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_transient_prompt_helper_settles_then_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/no-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=3 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2' "$ROOT" 2>&1)
  status=$?
  kill "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
assert_projection_close_failed_removal_rolls_back_the_reposition() {
  ...
  make_death_lab "$dir" "$bgpid"
  printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false},{"workspace_id":"w1","focused":false}]}}' > "$dir/mover-response"
  printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","focused":false},{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false}]}}' > "$dir/mover-response-2"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/mover-response" \
    DJ_FAKE_MOVER_RESPONSE_2="$dir/mover-response-2" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_close_pane_focus_preserving fmtest w1:p1' "$ROOT" 2>&1)
  status=$?
  kill -KILL "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
test_kill_emptying_non_focused_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_HERDR_PS_BIN="$dir/ps" DJ_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    DJ_FAKE_MOVER_LOG="$dir/mover.log" DJ_FAKE_MOVER_RESPONSE="$dir/no-response" \
    DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 DJ_FAKE_LOCK_LOG="$lock_log" \
    DJ_FAKE_LOCK_HELD="$lock_held" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      dj_backend_herdr_target_ready() { dj_backend_herdr_parse_target "$1"; }
      dj_backend_herdr_presentation_session_lock_path() { printf "%s" "$DJ_FAKE_LOCK_HELD.lock"; }
      dj_lock_try_acquire() {
        printf "acquire\n" >> "$DJ_FAKE_LOCK_LOG"
        : > "$DJ_FAKE_LOCK_HELD"
      }
      dj_lock_release() {
        [ -e "$DJ_FAKE_LOCK_HELD" ] || return 1
        rm -f "$DJ_FAKE_LOCK_HELD"
        printf "release\n" >> "$DJ_FAKE_LOCK_LOG"
      }
      eval "$(declare -f dj_backend_herdr_cli | sed "1s/dj_backend_herdr_cli/dj_backend_herdr_cli_locked/")"
      dj_backend_herdr_cli() {
        [ -e "$DJ_FAKE_LOCK_HELD" ] || return 97
        dj_backend_herdr_cli_locked "$@"
      }
      dj_backend_herdr_kill fmtest:w2:p2
    ' "$ROOT" 2>&1)
  status=$?
  ...
  pass "dj_backend_herdr_kill: one session lock covers the focus-safe emptying removal"
}
...
test_kill_focused_workspace_stays_plain_close() {
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    ...
    bash -c '
      . "$0/bin/backends/herdr.sh"
      dj_backend_herdr_presentation_session_lock_path() { printf "/tmp/dj-herdr-test-lock"; }
      dj_lock_try_acquire() { return 0; }
      dj_lock_release() { return 0; }
      dj_backend_herdr_kill fmtest:w2:p2
    ' "$ROOT" 2>&1)
  status=$?
  ...
  kill "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
  pass "dj_backend_herdr_kill: killing the focused workspace's tab keeps the legitimate plain close"
}
...
test_kill_refuses_when_presentation_lock_is_unavailable() {
  ...
  for mode in unresolved contended; do
    ...
    out=$(ROOT="$ROOT" MODE="$mode" CLI_LOG="$dir/cli.log" ATTEMPTS="$dir/attempts" bash -c '
      . "$ROOT/bin/backends/herdr.sh"
      dj_backend_herdr_target_ready() { dj_backend_herdr_parse_target "$1"; }
      dj_backend_herdr_presentation_session_lock_path() {
        [ "$MODE" = contended ] || return 1
        printf "/tmp/dj-herdr-contended-test-lock"
      }
      dj_lock_try_acquire() {
        printf "x\n" >> "$ATTEMPTS"
        return 1
  ...
  pass "dj_backend_herdr_kill: unavailable session locks defer every pane close"
}
...
test_endpoint_confirmed_gone_gates_on_structured_presence() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    dj_backend_herdr_cli() { printf "%s\n" "$DJ_FAKE_PRESENCE_RESPONSE"; return "${DJ_FAKE_PRESENCE_STATUS:-0}"; }
    check() {  # <label> <response> <status> <mode> <expected-rc>
      DJ_FAKE_PRESENCE_RESPONSE=$2 DJ_FAKE_PRESENCE_STATUS=$3
      rc=0
      dj_backend_herdr_endpoint_confirmed_gone fmtest:w2:p2 "$4" || rc=$?
    ...
    rc=0
    dj_backend_herdr_endpoint_confirmed_gone malformed-target strict || rc=$?
    [ "$rc" = 1 ] || printf "MISMATCH malformed-target: rc=%s expected=1\n" "$rc"
    rc=0
    dj_backend_herdr_endpoint_confirmed_gone "" || rc=$?
...
test_projection_seeded_prune_refuses_active_tab() {
  ...
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9","focused":true},{"tab_id":"w9:t2","label":"dj-task","workspace_id":"w9","focused":false}]}}' > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_prune_seeded_default_tab fmtest w9 w9:t1 focus-preserving' "$ROOT" 2>&1)
  status=$?
...
test_projection_label_builder_uses_corner_and_strips_owner_prefixes() {
  ...
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_workspace_label task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "primary child label was wrong: $primary"
  secondmate=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_workspace_label secondmate-child-demo '"$token" "$ROOT")
  [ "$secondmate" = "└ secondmate-child-demo · p:$token" ] \
    || fail "secondmate child label was wrong: $secondmate"
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_workspace_label jarvis/task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "jarvis/ owner prefix was not stripped: $primary"
  secondmate=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_workspace_label 2ndmate-fmdev-f2/child '"$token" "$ROOT")
  [ "$secondmate" = "└ child · p:$token" ] \
    || fail "2ndmate owner prefix was not stripped: $secondmate"
  primary=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_workspace_label dj-task-p2 '"$token" "$ROOT")
  [ "$primary" = "└ task-p2 · p:$token" ] \
    || fail "presentation dj- owner prefix was not stripped: $primary"
...
test_projection_order_moves_only_exact_new_workspace_and_preserves_relative_order() {
  ...
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":false},{"workspace_id":"w2","label":"jarvis/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w3","label":"2ndmate-alpha","focused":false},{"workspace_id":"w4","label":"2ndmate-bravo","focused":true},{"workspace_id":"w5","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$DJ_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"jarvis","focused":false},{"workspace_id":"w2","label":"jarvis/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w5","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false},{"workspace_id":"w3","label":"2ndmate-alpha","focused":false},{"workspace_id":"w4","label":"2ndmate-bravo","focused":true}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t2"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_order_best_effort fmtest w5 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_secondmate_parent_block() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  # jarvis, primary child, 2ndmate-A, A-child legacy, 2ndmate-B, human, NEW for A
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"└ primary · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"2ndmate-alpha"},{"workspace_id":"w4","label":"2ndmate-alpha/old · p:AbCdEfGhIjKlMnOpQrStU1"},{"workspace_id":"w5","label":"2ndmate-bravo"},{"workspace_id":"wH","label":"human-notes"},{"workspace_id":"w6","label":"└ new-a · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$DJ_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"└ primary · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"2ndmate-alpha"},{"workspace_id":"w4","label":"2ndmate-alpha/old · p:AbCdEfGhIjKlMnOpQrStU1"},{"workspace_id":"w6","label":"└ new-a · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"w5","label":"2ndmate-bravo"},{"workspace_id":"wH","label":"human-notes"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "w5\tw5:t1"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_order_best_effort fmtest w6 2ndmate-alpha' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_foreign_legacy_child_is_read_only() {
  ...
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"2ndmate-alpha"},{"workspace_id":"w3","label":"2ndmate-bravo/foreign · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w4","label":"└ new-alpha · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$DJ_FAKE_MOVER_CALLED"
exit 0
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w4 2ndmate-alpha' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_allows_intervening_parent_child_block() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"jarvis/old · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w3","label":"2ndmate-alpha"},{"workspace_id":"w4","label":"2ndmate-bravo"},{"workspace_id":"w5","label":"└ bravo-child · p:QqWwEeRrTtYyUuIiOoPpAa"},{"workspace_id":"w6","label":"└ new-first · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$DJ_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"jarvis/old · p:AbCdEfGhIjKlMnOpQrStUv"},{"workspace_id":"w6","label":"└ new-first · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"w3","label":"2ndmate-alpha"},{"workspace_id":"w4","label":"2ndmate-bravo"},{"workspace_id":"w5","label":"└ bravo-child · p:QqWwEeRrTtYyUuIiOoPpAa"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t1"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_order_best_effort fmtest w6 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_human_spaces_never_move_targets() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; mover_log="$dir/mover.log"
  : > "$log"; : > "$mover_log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"wH1","label":"notes"},{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"wH2","label":"scratch"},{"workspace_id":"w2","label":"2ndmate-alpha"},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
#!/usr/bin/env bash
printf '%s\t%s\t%s\n' "$1" "$2" "$3" >> "$DJ_FAKE_MOVER_LOG"
printf '%s\n' '{"id":"dj-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"wH1","label":"notes"},{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"},{"workspace_id":"wH2","label":"scratch"},{"workspace_id":"w2","label":"2ndmate-alpha"}]}}'
SH
  chmod +x "$mover"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "w2\tw2:t1"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_order_best_effort fmtest w3 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_failure_warns_without_cleanup_or_spawn_failure() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":true},{"workspace_id":"w2","label":"2ndmate-alpha","focused":false},{"workspace_id":"w3","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016 # $defs is a literal JSON Schema key.
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
  printf '%s\n' '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/fmtest.sock"}]}' > "$resp/4.out"
...
SH
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }; dj_backend_herdr_projection_focus_restore() { return 0; }; dj_backend_herdr_projection_order_best_effort fmtest w3 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_ambiguous_existing_block_is_read_only() {
  ...
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":true},{"workspace_id":"w2","label":"2ndmate-alpha","focused":false},{"workspace_id":"w3","label":"jarvis/old · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w4","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$DJ_FAKE_MOVER_CALLED"
exit 0
SH
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w4 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_anchors_the_parent_by_exact_id() {
  ...
  layout='{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis","focused":false},{"workspace_id":"w7","label":"jarvis","focused":false},{"workspace_id":"wH","label":"human-notes","focused":false},{"workspace_id":"w8","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}'
...
SH
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w8 jarvis' "$ROOT" 2>&1)
  status=$?
  [ "$status" -eq 0 ] || fail "ambiguous projection ordering must not fail the spawn"
...
SH
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w8 jarvis w7' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_missing_parent_is_read_only() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$DJ_FAKE_MOVER_CALLED"
exit 0
SH
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w2 2ndmate-missing' "$ROOT" 2>&1)
  status=$?
...
test_presentation_session_lock_path_is_shared_across_homes() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path_a=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
    || fail "session lock path resolution failed for home A"
  path_b=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
  ...
  case "$path_a" in
    /tmp/jarvis-herdr-presentation/order-*.lock) ;;
  ...
  path_other=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path other' "$ROOT") \
  ...
  if [ -L /tmp ] || [ "$(cd /tmp && pwd -P)" != /tmp ]; then
    : > /tmp/dj-herdr-lock-canon-$$.sock
    printf '%s\n' '{"sessions":[{"name":"canon","running":true,"socket_path":"/tmp/dj-herdr-lock-canon-'"$$"'.sock"}]}' > "$resp/4.out"
    printf '%s\n' "{\"sessions\":[{\"name\":\"canon\",\"running\":true,\"socket_path\":\"$(cd /tmp && pwd -P)/dj-herdr-lock-canon-$$.sock\"}]}" > "$resp/5.out"
    path_tmp=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
      || fail "lock path with /tmp socket failed"
    path_private=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
      || fail "lock path with canonical socket failed"
    rm -f /tmp/dj-herdr-lock-canon-$$.sock
...
test_presentation_session_lock_path_rejects_malformed_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_presentation_session_lock_path fmtest' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_rejects_malformed_socket() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"wH","label":"2ndmate-alpha"},{"workspace_id":"w2","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  printf '%s\n' '{"client":{"version":"0.7.4","protocol":16},"server":{"running":true}}' > "$resp/2.out"
  # shellcheck disable=SC2016
  printf '%s\n' '{"schemas":{"request":{"oneOf":[{"properties":{"method":{"const":"workspace.move"}}}],"$defs":{"WorkspaceMoveParams":{"required":["workspace_id","insert_index"],"properties":{"insert_index":{"type":"integer"}}}}}}}' > "$resp/3.out"
...
SH
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_HERDR_SCRIPT_STATUS=1 \
    DJ_BACKEND_HERDR_WORKSPACE_MOVER="$mover" DJ_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_order_best_effort fmtest w2 jarvis' "$ROOT" 2>&1)
  status=$?
...
test_projection_reclaim_refusal_matrix_is_non_mutating() {
  ...
  token=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(dj_backend_herdr_projection_journal_create "$1" refusal-r1) || exit 1
    label=$(dj_backend_herdr_projection_workspace_label refusal-r1 "$token")
    dj_backend_herdr_projection_journal_bind \
      "$1/refusal-r1.herdr-presentation" refusal-r1 "$2" fmtest \
      w2 w2:t2 w2:p2 w1 jarvis "$label" dj-refusal-r1 || exit 1
    printf "%s" "$token"
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_journal_create "$1" refusal-r1 >/dev/null' \
  ...
  out=$(ROOT="$ROOT" JOURNAL="$journal" LEGACY="$legacy" HOME_A="$home" HOME_B="$other_home" MUTATIONS="$mutation_log" \
    bash -c '
      ...
      run_case() {
        ...
        dj_backend_herdr_projection_live_binding_matches() {
          [ "$mode" != ambiguous ]
        }
        dj_backend_herdr_pane_agent_state() {
          case "$mode" in
            live) printf live ;;
        ...
        dj_backend_herdr_projection_focus_snapshot() {
          [ "$mode" != focus-unknown ] || return 1
          printf "w1\tw1:t1"
        }
        set +e
        dj_backend_herdr_projection_reclaim_task \
          fmtest "$journal" refusal-r1 "$home" w2 w2:t2 w2:p2 jarvis dj-refusal-r1 /tmp/project \
          >/dev/null 2>&1
        rc=$?
        set -e
        printf "%s:%s\n" "$mode" "$rc"
...
test_projection_reclaim_replaces_only_exact_husk_and_advances_binding() {
  ...
  token=$(bash -c '
    ...
    label=$(dj_backend_herdr_projection_workspace_label dj-hibit-r1 "$token")
    dj_backend_herdr_projection_journal_bind \
      "$1/dj-hibit-r1.herdr-presentation" dj-hibit-r1 "$2" fmtest \
      w2 w2:t2 w2:p2 w1 jarvis "$label" dj-dj-hibit-r1 || exit 1
    printf "%s" "$token"
  ' "$ROOT" "$state" "$home_real") || fail "could not create exact reclaim journal fixture"
  journal="$state/dj-hibit-r1.herdr-presentation"
  label="└ hibit-r1 · p:$token"
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w0\",\"label\":\"jarvis\",\"focused\":false,\"active_tab_id\":\"w0:t1\"},{\"workspace_id\":\"w1\",\"label\":\"jarvis\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/1.out"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t2","label":"dj-dj-hibit-r1"}]}}' > "$resp/2.out"
  ...
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w1\",\"label\":\"jarvis\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/6.out"
  ...
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t2","label":"dj-dj-hibit-r1"},{"tab_id":"w2:t3","label":"dj-dj-hibit-r1"}]}}' > "$resp/20.out"
  : > "$resp/21.out"
  ...
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t3","label":"dj-dj-hibit-r1"}]}}' > "$resp/27.out"
  ...
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      dj_backend_herdr_projection_reclaim_task \
        fmtest "$1" dj-hibit-r1 "$2" w2 w2:t2 w2:p2 jarvis dj-dj-hibit-r1 /tmp/project || exit 1
      printf "%s %s" "$DJ_BACKEND_HERDR_PROJECTION_TAB_ID" "$DJ_BACKEND_HERDR_PROJECTION_PANE_ID"
...
test_projection_recovery_is_read_only_and_refuses_live_duplicate_risk() {
  ...
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  token=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_journal_create "$1" task-p3' "$ROOT" "$state")
  journal="$state/task-p3.herdr-presentation"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis/task-p3 · p:%s"},{"workspace_id":"w2","label":"copy/task-p3 · p:%s"}]}}\n' "$token" "$token" > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_recovery_allows_flat fmtest "$1" task-p3' "$ROOT" "$journal" \
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis/task-p3 · p:%s"}]}}\n' "$token" > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p1","tab_id":"w1:t1"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p1"}}}\n' > "$resp/3.out"
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/4.out"
  out=$(PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_projection_recovery_allows_flat fmtest "$1" task-p3' "$ROOT" "$journal" 2>&1)
  status=$?
...
test_workspace_find_matches_only_this_homes_own_label() {
  ...
  # A workspace list carrying BOTH the primary's "jarvis" space and this
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"2ndmate-bravo-b2"},{"workspace_id":"w3","label":"2ndmate-alpha-a1"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HOME="$home" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_workspace_find fmtest' "$ROOT" )
  [ "$out" = "w2" ] || fail "workspace_find should have matched this home's own label (2ndmate-bravo-b2 -> w2), got '$out'"
  pass "dj_backend_herdr_workspace_find: matches only THIS home's own label among several coexisting workspaces"
}
...
test_list_live_scoped_to_this_homes_workspace_only() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"jarvis"},{"workspace_id":"w2","label":"2ndmate-bravo-b2"}]}}\n' > "$resp/1.out"
  # 2: tab list --workspace w2 (this secondmate's own tabs only)
  printf '{"result":{"tabs":[{"tab_id":"w2:t1","label":"dj-secondmatetask"}]}}\n' > "$resp/2.out"
  # 3: pane_for_tab's `pane list --workspace w2`
  printf '{"result":{"panes":[{"pane_id":"w2:p1","tab_id":"w2:t1"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HOME="$home" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_list_live fmtest' "$ROOT" )
  ...
  pass "dj_backend_herdr_list_live: scoped to this home's own workspace, never a sibling home's"
}
...
test_parse_target() {
  ( . "$ROOT/bin/backends/herdr.sh"
    dj_backend_herdr_parse_target "default:w1:p2" || exit 1
    [ "$DJ_BACKEND_HERDR_SESSION" = default ] || { echo "session mismatch: $DJ_BACKEND_HERDR_SESSION" >&2; exit 1; }
    [ "$DJ_BACKEND_HERDR_PANE" = "w1:p2" ] || { echo "pane mismatch: $DJ_BACKEND_HERDR_PANE" >&2; exit 1; }
  ) || fail "dj_backend_herdr_parse_target did not split session:pane on the first colon only"
  pass "dj_backend_herdr_parse_target: splits '<session>:<pane_id>' on the FIRST colon (pane_id itself contains one)"
}

test_normalize_key() {
  ( . "$ROOT/bin/backends/herdr.sh"
    [ "$(dj_backend_herdr_normalize_key Enter)" = enter ] || exit 1
    [ "$(dj_backend_herdr_normalize_key Escape)" = escape ] || exit 1
    [ "$(dj_backend_herdr_normalize_key C-c)" = ctrl+c ] || exit 1
    [ "$(dj_backend_herdr_normalize_key ctrl+c)" = ctrl+c ] || exit 1
  ) || fail "dj_backend_herdr_normalize_key did not map jarvis's key vocabulary to herdr's verified names"
  pass "dj_backend_herdr_normalize_key: Enter/Escape/C-c map to herdr's verified enter/escape/ctrl+c"
}
...
test_capture_calls_pane_read() {
  ...
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_capture default:w1:p2 250' "$ROOT" )
  ...
  pass "dj_backend_herdr_capture: calls 'pane read <pane> --source recent --lines N' with the session set"
}
...
test_capture_works_around_small_lines_bug() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_capture default:w1:p2 2' "$ROOT" )
  ...
  pass "dj_backend_herdr_capture: works around the verified small-N '--lines' bug by over-fetching and trimming locally"
}
...
test_capture_preserves_pane_read_failure() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_capture default:w1:p2 2' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "dj_backend_herdr_capture: ensures the session and preserves pane read failure"
}
...
test_send_key_normalizes_and_targets_pane() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_key default:w1:p2 Escape' "$ROOT"
  ...
  pass "dj_backend_herdr_send_key: normalizes the key and targets the right pane"
}
...
test_kill_is_best_effort() {
  ...
  PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '
      ...
      dj_lock_try_acquire() { return 0; }
      dj_lock_release() { return 0; }
      dj_backend_herdr_kill default:w1:p2
    ' "$ROOT"
  ...
  pass "dj_backend_herdr_kill: calls pane close and stays best-effort on failure"
}
...
test_current_path_reads_cwd() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_current_path default:w1:p2' "$ROOT" )
  ...
  pass "dj_backend_herdr_current_path: reads pane foreground_cwd (the live running process), not the frozen creation-time cwd"
}
...
test_busy_state_working_maps_to_busy() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_busy_state default:w1:p2' "$ROOT" )
  ...
  pass "dj_backend_herdr_busy_state: working -> busy"
}
...
test_busy_state_done_and_blocked_map_to_idle() {
  ...
  pass "dj_backend_herdr_busy_state: done -> idle, blocked -> idle (surfaced like a stale pane, not suppressed as busy)"
}
...
test_busy_state_unknown_on_no_agent() {
  ...
  pass "dj_backend_herdr_busy_state: unparseable/absent agent state reports unknown, the regex-fallback cue"
}
...
test_composer_state_bare_prompt_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_composer_state default:w1:p2' "$ROOT" )
  [ "$out" = empty ] || fail "a bare prompt glyph should read as empty, got '$out'"
  pass "dj_backend_herdr_composer_state: a bare '❯' composer row reads empty"
}
...
test_composer_state_styled_placeholder_draft_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: bright placeholder-like text stays pending rather than being mistaken for an idle ghost"
}
...
test_composer_state_real_text_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: real composer text reads pending"
}
...
test_composer_state_popup_placeholder_fill_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: a slash-command popup's argument-hint placeholder still reads pending (the incident fix)"
}
...
test_composer_state_unknown_on_capture_failure() {
  ...
  pass "dj_backend_herdr_composer_state: reports unknown when the pane cannot be captured"
}
...
test_composer_state_unknown_when_no_composer_row_found() {
  ...
  pass "dj_backend_herdr_composer_state: reports unknown for bare shell prompts with no composer row"
}
...
test_composer_state_pi_separator_idle_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_composer_state lab:w1:p2' "$ROOT" )
  ...
  pass "dj_backend_herdr_composer_state: a native idle Pi separator composer reads empty"
}
...
# verdict: the away-mode injection guard (bin/dj-supervise-daemon.sh) and
# dj-send's pre-type refusal both proceed ONLY on an affirmative `empty`.
test_composer_state_pi_parked_prompt_is_not_empty() {
  ...
  pass "dj_backend_herdr_composer_state: a blocked pi pane parked on a prompt is not an empty composer"
}
...
test_composer_state_pi_separator_real_text_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: real Pi composer text remains pending"
}
...
test_composer_state_pi_incomplete_separator_below_stale_generic_is_unknown() {
  ...
  pass "dj_backend_herdr_composer_state: an incomplete lower Pi separator cannot inherit a stale empty row"
}
...
test_composer_state_pi_separator_requires_safe_native_identity() {
  ...
  pass "dj_backend_herdr_composer_state: Pi separators never authorize working, non-Pi, unreadable, or over-tall targets"
}
...
# bin/dj-supervise-daemon.sh's dj_backend_herdr_send_text_submit to never
...
test_composer_state_claude_unbordered_prompt_is_empty() {
  ...
  pass "dj_backend_herdr_composer_state: a real-claude unbordered '❯' prompt row (no border box in view) reads empty"
}
...
test_composer_state_claude_unbordered_prompt_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: a real-claude unbordered '❯ <text>' prompt row reads pending"
}
...
test_composer_state_bare_prompt_below_stale_bordered_banner_wins() {
  ...
  pass "dj_backend_herdr_composer_state: a live unbordered prompt row below a stale bordered decorative box still wins (not misread as the box's own row)"
}
...
test_composer_state_claude_dim_prompt_suggestion_ghost_is_empty() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_composer_state default:w1:p3' "$ROOT" )
  ...
  pass "dj_backend_herdr_composer_state: claude's dim prompt-suggestion ghost (the overnight wedge shape) reads empty"
}
...
test_composer_state_claude_dim_ghost_row_with_real_text_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: real typed text on the same claude prompt row still reads pending"
}
...
test_composer_state_grok_dark_truecolor_placeholder_is_empty() {
  ...
  pass "dj_backend_herdr_composer_state: grok's dark-truecolor placeholder (the TRUECOLOR gap) reads empty"
}
...
test_composer_state_grok_bright_truecolor_real_text_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: grok's real bright typed input still reads pending"
}
...
test_composer_state_codex_bare_prompt_glyph_is_empty() {
  ...
  pass "dj_backend_herdr_composer_state: a real-codex unbordered '›' prompt row reads empty"
}
...
test_composer_state_codex_faint_suggestion_is_empty() {
  ...
  pass "dj_backend_herdr_composer_state: a faint real-codex ghost suggestion reads empty"
}
...
test_composer_state_codex_non_faint_same_text_is_pending() {
  ...
  pass "dj_backend_herdr_composer_state: non-faint codex prompt text still reads pending"
}

# --- wait_for_working: the native agent-state poll-and-classify primitive ---
# Direct unit coverage for dj_backend_herdr_wait_for_working, the helper
# dj_backend_herdr_send_text_submit now uses instead of composer scraping
...
test_wait_for_working_returns_busy_on_first_poll() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_for_working default w1:p2 1 5' "$ROOT" )
  ...
  pass "dj_backend_herdr_wait_for_working: reports 'busy' immediately on the first poll, without spending the rest of the budget"
}
...
test_wait_for_working_catches_a_slow_transition_mid_window() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_for_working default w1:p2 0.03 3' "$ROOT" )
  ...
  pass "dj_backend_herdr_wait_for_working: a slow transition landing on a later sample within one window is still caught (robust against the 'slow transition' failure direction)"
}
...
test_wait_for_working_samples_budget_endpoint_without_final_sleep() {
  ...
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_SLEEP_LOG="$sleep_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$DJ_SLEEP_LOG"; }; dj_backend_herdr_wait_for_working default w1:p2 0.5 6' "$ROOT" )
  ...
  pass "dj_backend_herdr_wait_for_working: spreads six samples across the full budget endpoint without a final trailing sleep"
}
...
test_send_text_submit_applies_herdr_minimum_confirm_budget() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/9.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_SLEEP_LOG="$sleep_log" DJ_BACKEND_HERDR_SUBMIT_POLLS=6 DJ_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6 \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$DJ_SLEEP_LOG"; }; dj_backend_herdr_send_text_submit default:w1:p2 "hello captain" 1 0.4 0' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: applies the herdr minimum confirmation budget before polling agent-state"
}
...
test_wait_for_working_returns_idle_when_never_busy_but_readable() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_for_working default w1:p2 0.02 2' "$ROOT" )
  [ "$out" = idle ] || fail "wait_for_working should report idle when the target was legibly read but never busy, got '$out'"
  pass "dj_backend_herdr_wait_for_working: reports 'idle' (readable, genuinely not yet working) when 'busy' never appears"
}
...
test_wait_for_working_returns_unknown_when_never_readable() {
  ...
  pass "dj_backend_herdr_wait_for_working: reports 'unknown' (a hard read failure, not a timing race) only when EVERY poll in the window fails"
}
...
test_wait_for_working_treats_blocked_as_submit_active() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_for_working default w1:p2 0.01 1' "$ROOT" )
  [ "$out" = busy ] || fail "wait_for_working should treat a post-Enter blocked state as submit-active, got '$out'"
  pass "dj_backend_herdr_wait_for_working: treats blocked as submit-active for confirmation without changing watcher busy-state semantics"
}
...
# DJ_BACKEND_HERDR_SUBMIT_POLLS=1 pins most tests
...
test_send_text_submit_detects_landed_send() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "hello captain" 3 0.01 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: reports 'empty' once agent_status reports working after one Enter, without ever reading the composer"
}
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "hello captain" 2 0.01 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: reports 'pending' when agent_status stays idle and the composer still holds unsent text after retried Enters (swallowed)"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "/compact" 3 0.01 1.2' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: a slash-command popup's placeholder fill on Enter #1 never flips agent_status to working, so it does not short-circuit as submitted; Enter #2 is retried and lands it"
}
...
test_send_text_submit_confirms_blocked_after_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "needs approval" 3 0.01 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: a post-Enter blocked state confirms delivery without retrying into the prompt"
}
...
test_send_text_submit_preexisting_working_pending_is_queued_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "hello captain" 1 0.01 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: native working + proven pending after retries reports empty (queued Enter)"
}
...
test_send_text_submit_preexisting_working_does_not_confirm_failed_enter() {
  ...
  pass "dj_backend_herdr_send_text_submit: a failed Enter cannot borrow preexisting working state as queued-delivery proof"
}
...
test_send_text_submit_idle_baseline_does_not_confirm_failed_enter() {
  ...
  pass "dj_backend_herdr_send_text_submit: a failed Enter cannot borrow a later native transition as delivery proof"
}
...
test_send_text_submit_idle_native_empty_composer_confirms_delivery() {
  ...
  pass "dj_backend_herdr_send_text_submit: idle native agent-state plus empty composer reports empty (landed Claude turn)"
}
...
test_send_text_submit_idle_native_pending_plus_rendered_busy_is_queued() {
  ...
  pass "dj_backend_herdr_send_text_submit: idle native baseline uses a rendered busy footer to confirm a queued Enter"
}
...
test_composer_state_cursor_midturn_row_reads_pending() {
  ...
  pass "dj_backend_herdr_composer_state: cursor's mid-turn placeholder-plus-busy-token row reads pending (why delivery needs a separate signal)"
}
...
test_rendered_busy_state_reads_the_cursor_busy_token() {
  ...
  printf '1\n' > "$resp/3.exit"
  fb=$(make_herdr_fakebin "$dir")
  idle_out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
  busy_out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
  fail_out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_rendered_busy_state default:w1:p2' "$ROOT" )
  ...
  pass "dj_backend_herdr_rendered_busy_state: busy/idle/unknown from the rendered footer, with an unreadable pane never reading idle"
}
...
test_send_text_submit_confirms_never_idle_native_state_via_footer_transition() {
  ...
  pass "dj_backend_herdr_send_text_submit: a rendered-footer idle-to-busy transition confirms delivery when native agent-state never reports idle"
}
...
test_send_text_submit_never_idle_native_state_keeps_pending_without_a_transition() {
  ...
  pass "dj_backend_herdr_send_text_submit: an already-busy footer baseline is never accepted as proof that this Enter landed"
}
...
test_send_text_submit_confirms_despite_codex_idle_tip_composer() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "reply with just OK" 3 0.01 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: confirms submission via native agent-state alone, immune to a codex-style dynamic idle-tip composer that would have misread as 'pending' under the old composer-based confirmation"
}

# Companion regression for the pre-injection empty-box guard itself
# (bin/dj-supervise-daemon.sh's pane_input_pending): a real Codex idle
...
test_composer_state_codex_dynamic_idle_tip_reads_empty_when_faint() {
  ...
  pass "dj_backend_herdr_composer_state: a faint real-codex dynamic idle-tip composer row reads empty"
}

# Regression guard for the PRE-injection empty-box guard itself
# (bin/dj-supervise-daemon.sh's pane_input_pending, dispatched via
# dj_backend_composer_state -> dj_backend_herdr_composer_state): this task
...
test_composer_state_guard_still_refuses_real_pending_text_after_submit_confirmation_change() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/dj-backend.sh"; dj_backend_composer_state herdr default:w1:p2' "$ROOT" )
  ...
  pass "dj_backend_composer_state (herdr): the pre-injection empty-box guard still refuses a genuinely non-empty composer, unaffected by the submit-confirmation change"
}
...
test_send_text_submit_slow_transition_within_one_enter_needs_no_extra_enter() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=3 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "hello captain" 3 0.03 0.01' "$ROOT" )
  ...
  pass "dj_backend_herdr_send_text_submit: a slow transition landing on a later sample within one Enter's budget is confirmed WITHOUT sending a needless extra Enter"
}
...
test_send_text_submit_send_failed() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" DJ_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_send_text_submit default:w1:p2 "x" 2 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "send_text_submit should report send-failed when the literal send itself fails, got '$out'"
  pass "dj_backend_herdr_send_text_submit: reports 'send-failed' when the literal send-text call itself errors"
}
...
test_send_text_submit_unknown_on_capture_failure() {
  ...
  pass "dj_backend_herdr_send_text_submit: reports 'unknown' when the post-Enter agent-get read fails (never retries past an unreadable target)"
}
...
test_send_text_submit_unknown_on_composer_capture_failure() {
  ...
  pass "dj_backend_herdr_send_text_submit: an unreadable composer stops Enter retries after native status stays idle"
}

# --- dj-backend.sh dispatch wiring -------------------------------------------

test_dispatch_routes_herdr_backend() {
  dj_backend_validate herdr 2>/dev/null || fail "dj_backend_validate should accept herdr (P2 adds it to DJ_BACKEND_KNOWN)"
  pass "dj_backend_validate: herdr is a known backend (P2)"
}

test_dispatch_busy_state_unknown_for_tmux() {
  # shellcheck source=bin/dj-backend.sh
  . "$ROOT/bin/dj-backend.sh"
  [ "$(dj_backend_busy_state tmux 'sess:win')" = unknown ] \
    || fail "dj_backend_busy_state should report unknown for tmux (no native agent-state primitive; watcher falls back to regex)"
  pass "dj_backend_busy_state: tmux (no native primitive) always reports unknown, preserving the P1 regex-only path"
}

test_dispatch_composer_state_routes_by_backend() {
  # dj_backend_composer_state (the generic per-backend composer/pending-input
  # classifier the away-mode daemon dispatches through - bin/dj-supervise-daemon.sh's
  ...
  # wrappers over the shared dj_composer_classify_screen - and report unknown
  # for an unrecognized backend name.
  # Sourced-guards are pre-set so dj_backend_source no-ops and these stubs are
  ...
  (
    ...
    dj_tmux_composer_state() { [ "$1" = "sess:win" ] || fail "tmux composer_state got wrong target: $1"; printf 'pending'; }
    dj_backend_herdr_composer_state() { [ "$1" = "default:w1:p2" ] || fail "herdr composer_state got wrong target: $1"; printf 'empty'; }
    dj_backend_orca_composer_state() { [ "$1" = "term-1" ] || fail "orca composer_state got wrong target: $1"; printf 'empty'; }
    dj_backend_zellij_composer_state() { [ "$1" = "sess:7" ] || fail "zellij composer_state got wrong target: $1"; printf 'empty'; }
    [ "$(dj_backend_composer_state tmux sess:win)" = pending ] || fail "composer_state did not dispatch to the tmux classifier"
    [ "$(dj_backend_composer_state herdr default:w1:p2)" = empty ] || fail "composer_state did not dispatch to the herdr classifier"
    [ "$(dj_backend_composer_state orca term-1)" = empty ] || fail "composer_state did not dispatch to the orca classifier"
    [ "$(dj_backend_composer_state zellij sess:7)" = empty ] || fail "composer_state did not dispatch to the zellij classifier"
    [ "$(dj_backend_composer_state bogus x)" = unknown ] || fail "composer_state should report unknown for an unrecognized backend"
  ) || fail "composer_state dispatch subshell failed"
  pass "dj_backend_composer_state dispatches every backend to its named thin classifier, unknown for unrecognized backends"
}
...
test_scripts_route_explicit_target_through_meta_backend() {
  ...
  dj_write_meta "$state/herdr-stale.meta" "window=default:w1:p2" "backend=herdr"
...
SH
  ...
  out=$( PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$neutral" DJ_STATE_OVERRIDE="$state" \
    DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    "$ROOT/bin/dj-peek.sh" default:w1:p2 5 2>/dev/null )
  [ "$out" = "captured herdr pane" ] || fail "dj-peek did not capture through herdr for an explicit metadata-matched target, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''read'$'\x1f''w1:p2' \
    "dj-peek did not route the explicit stale target through herdr capture"

  : > "$log"
  PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$neutral" DJ_HOME="$neutral" DJ_STATE_OVERRIDE="$state" \
    DJ_HERDR_LOG="$log" DJ_HERDR_RESPONSES="$resp" \
    "$ROOT/bin/dj-send.sh" default:w1:p2 --key Escape >/dev/null 2>&1
  expect_code 0 $? "dj-send --key should route an explicit metadata-matched target through herdr"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''send-keys'$'\x1f''w1:p2'$'\x1f''escape' \
    "dj-send did not route the explicit stale target through herdr send-key"

  pass "dj-peek/dj-send: explicit stale targets matching metadata use the recorded backend"
}
...
test_workspace_ensure_prunes_default_tab() {
  ...
  raw=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /proj' "$ROOT" ) \
  ...
  ids=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "dj-prunetest" "$seeded" ) \
...
EOF
  ...
  jq -r --arg w "$wsid" '[.tabs[]|select(.workspace_id==$w)][0].label' "$state" | grep -qx 'dj-prunetest' \
  ...
  pass "dj_backend_herdr_create_task: prunes exactly the seeded default tab container_ensure identified, once the first real task tab exists"
}
...
test_repeated_cycles_reuse_one_workspace_no_orphans() {
  ...
  for i in 1 2 3; do
    ...
    ids=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "dj-cycle$i" "$seeded" ) \
      || fail "cycle $i: create_task failed"
    read -r _ pane <<EOF
$ids
EOF
    [ -n "$pane" ] || fail "cycle $i: create_task returned no pane id"
    PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '
        . "$0/bin/backends/herdr.sh"
        dj_backend_herdr_presentation_session_lock_path() { printf "/tmp/dj-herdr-cycle-test-lock"; }
        dj_lock_try_acquire() { return 0; }
        dj_lock_release() { return 0; }
        dj_backend_herdr_kill "$1"
      ' "$ROOT" "fmtest:$pane" \
      || fail "cycle $i: kill failed"
  done
  # exactly one jarvis workspace survives three spawn/teardown cycles
  wscount=$(jq -r '[.workspaces[]|select(.label=="jarvis")]|length' "$state")
  [ "$wscount" = 1 ] || fail "expected exactly 1 jarvis workspace after 3 cycles, got $wscount: $(jq -c '.workspaces' "$state")"
  ...
  # zero tabs remain: every dj- task tab torn down AND the default tab pruned
  ...
  pass "herdr repeated spawn/teardown: one persistent jarvis workspace reused, zero orphans, default tab pruned, create ran once"
}

# --- created-vs-adopted default-tab-prune safety (2026-07-02 self-kill fix) -
#
# Root cause and fix are documented at
# dj_backend_herdr_workspace_prune_seeded_default_tab in bin/backends/herdr.sh
...
test_adopted_workspace_never_prunes_default_tab() {
  # An ADOPTED workspace (dj_backend_herdr_workspace_find matched a
  ...
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"jarvis"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{}}' > "$state"
  raw=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  ...
  ids=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "dj-adopttest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  pass "dj_backend_herdr_create_task: an ADOPTED workspace's pre-existing tab is never pruned (the created-vs-adopted gate)"
}
...
test_label_collision_startup_workspace_leaves_live_tab_alone() {
  ...
  # directly inside a directory named "jarvis", so herdr auto-derives that
  # workspace's DISPLAYED label from the cwd basename - "jarvis" - byte-
  # identical to the primary jarvis home's own derived label, with no
  # --label ever passed and no jarvis involvement at all. That workspace's
  ...
  fb=$(make_herdr_statefake "$dir")
  # Mimic a bare `herdr workspace create --cwd <dir-named-jarvis>` (no
  ...
  # alone, from jarvis's own freshly-seeded default tab. Its pane hosts a
  # live agent (agent_status=working), exactly like the captain's own pane.
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"jarvis"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{"w1:p1":"working"}}' > "$state"
  raw=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  ...
  ids=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "dj-collisiontest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  pass "dj_backend_herdr_create_task: the label-collision startup-workspace scenario (2026-07-02 incident) leaves the captain's live tab untouched"
}
...
test_prune_refuses_a_working_agent_pane_defense_in_depth() {
  ...
  ids=$( PATH="$fb:$PATH" DJ_HERDR_LOG="$log" DJ_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "dj-busytest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  pass "dj_backend_herdr_workspace_prune_seeded_default_tab: refuses to close the seeded default tab when its pane reports a working agent (defense in depth)"
}

# --- native event push: normalize / policy-routing / dedupe / wait ----------
#
# These exercise the herdr subscriber (dj_backend_herdr_wait_transition and its
...
# tests/dj-backend-herdr-eventwait-smoke.test.sh.

# make_herdr_eventfake: a herdr stub answering exactly the calls the event path
# makes - `session list --json` (echoes one session, name DJ_FAKE_SESSION_NAME,
# socket DJ_FAKE_SOCKET), `status --json`, and `agent get <pane>` (per-pane
# status read from $DJ_FAKE_AGENT_DIR/<key>.status, else agent_not_found).
...
case "$cmd $sub" in
  ...
  "session list")
    printf '{"sessions":[{"name":"%s","running":true,"default":false,"socket_path":"%s"}]}\n' \
      "${DJ_FAKE_SESSION_NAME:-default}" "${DJ_FAKE_SOCKET:-/tmp/dj-fake.sock}" ;;
  "agent get")
    if [ -n "${DJ_FAKE_READER_READY_FILE:-}" ] && [ ! -e "$DJ_FAKE_READER_READY_FILE" ]; then
      exit 9
    fi
    pane=${3:-}
    key=$(printf '%s' "$pane" | tr ':/.' '___')
    f="${DJ_FAKE_AGENT_DIR:-/tmp}/$key.status"
    if [ -f "$f" ]; then
      printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$(cat "$f")"
    else
      printf '{"error":{"code":"agent_not_found"}}\n' >&2
...
# the socket, streams the TAB-separated lines in $DJ_FAKE_READER_LINES to stdout
# (one projected event per line: pane_id\tworkspace_id\tagent_status\tagent),
# then exits $DJ_FAKE_READER_EXIT (default 0). A non-zero exit with no lines
...
if [ -n "${DJ_FAKE_READER_READY_FILE:-}" ]; then
  : > "$DJ_FAKE_READER_READY_FILE"
fi
printf '%s\n' "${DJ_FAKE_READER_ACK:-@subscribed}"
if [ -n "${DJ_FAKE_READER_LINES:-}" ] && [ -f "$DJ_FAKE_READER_LINES" ]; then
  cat "$DJ_FAKE_READER_LINES"
fi
exit "${DJ_FAKE_READER_EXIT:-0}"
SH
  chmod +x "$path"
  printf '%s\n' "$path"
}
...
test_normalize_event_leaves_from_empty() {
  local rec
  rec=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_normalize_event wG:pQ wG blocked claude' "$ROOT")
  [ "$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_pane_id "$1"' "$ROOT" "$rec")" = "wG:pQ" ] \
    || fail "normalize_event pane_id wrong: $rec"
  [ "$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_from_status "$1"' "$ROOT" "$rec")" = "" ] \
    || fail "normalize_event should leave from_status empty (herdr carries no previous status): $rec"
  [ "$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_to_status "$1"' "$ROOT" "$rec")" = "blocked" ] \
    || fail "normalize_event to_status wrong: $rec"
  pass "dj_backend_herdr_normalize_event routes through the shared record with an empty from_status"
}

test_escalation_marker_keys_like_watcher() {
  local m
  m=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_escalation_marker /st default:wG:pQ' "$ROOT")
  [ "$m" = "/st/.herdr-escalated-default_wG_pQ" ] \
    || fail "escalation marker key must match the watcher's tr ':/.' '___' scheme, got '$m'"
  pass "dj_backend_herdr_escalation_marker keys the dedupe marker exactly like the watcher's .stale-<key>"
}
...
test_apply_transition_blocked_requires_commit_to_dedupe() {
  ...
  rec=$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_record wG:pQ wG "" blocked claude' "$ROOT")
  marker="$state/.herdr-escalated-default_wG_pQ"
  out=$(bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"); rc=$?
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_commit_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"
  ...
  pass "dj_backend_herdr_apply_transition: blocked dedupe starts only after explicit commit"
}
...
test_apply_transition_working_clears_marker() {
  ...
  marker="$state/.herdr-escalated-default_wG_pQ"
  blocked=$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_record wG:pQ wG "" blocked claude' "$ROOT")
  working=$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_record wG:pQ wG "" working claude' "$ROOT")
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_commit_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$blocked"
  [ -e "$marker" ] || fail "setup: committed blocked edge should have set the marker"
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$working"; rc=$?
  ...
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$blocked" >/dev/null; rc=$?
  [ "$rc" = 0 ] || fail "a re-block after a working clear must re-fire (return 0), got $rc"
  pass "dj_backend_herdr_apply_transition: a working edge clears the marker so the next ->blocked re-escalates"
}
...
test_clear_transition_removes_task_marker() {
  ...
  marker="$state/.herdr-escalated-default_wG_pQ"
  : > "$marker"
  bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_clear_transition "$1" "$2"' "$ROOT" "$state" default:wG:pQ
  [ ! -e "$marker" ] || fail "clear_transition must remove the marker owned by a torn-down pane"
  pass "dj_backend_herdr_clear_transition removes task-owned dedupe state"
}
...
test_apply_transition_defer_and_fallback_are_noops() {
  ...
  for s in idle "done" unknown ""; do
    local rec
    rec=$(bash -c '. "$0/bin/dj-transition-lib.sh"; dj_transition_record wG:pQ wG "" "$1" claude' "$ROOT" "$s")
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_apply_transition "$1" "$2" "$3"' "$ROOT" "$state" default "$rec"; rc=$?
  ...
  pass "dj_backend_herdr_apply_transition: idle/done (defer) and unknown/empty (fallback) take no fast action"
}

test_wait_transition_no_panes_returns_2() {
  local rc
  bash -c '. "$0/bin/backends/herdr.sh"; DJ_BACKEND_HERDR_EVENTS_FORCE=1 dj_backend_herdr_wait_transition default 1 /tmp/st' "$ROOT"; rc=$?
  [ "$rc" = 2 ] || fail "wait_transition with no pane windows must return 2 (fall back to sleep), got $rc"
  pass "dj_backend_herdr_wait_transition: a home with no herdr panes falls back to polling (rc 2)"
}
...
test_wait_transition_not_capable_returns_2() {
  ...
  rc=$(PATH="$fb:$PATH" DJ_BACKEND_HERDR_EVENTS_FORCE=0 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  [ "$rc" = 2 ] || fail "wait_transition must return 2 when events are below capability (fail closed to poll), got $rc"
  pass "dj_backend_herdr_wait_transition: below-capability protocol/schema falls back to polling (rc 2)"
}
...
test_wait_transition_reconcile_blocked_returns_record() {
  ...
  out=$(PATH="$fb:$PATH" TMPDIR="$temp" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ' "$ROOT" "$state"); rc=$?
  ...
  pass "dj_backend_herdr_wait_transition: reconnect level-reconcile returns an uncommitted blocked pane"
}
...
test_wait_transition_subscribes_before_reconcile() {
  ...
  rc=$(PATH="$fb:$PATH" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" DJ_FAKE_READER_READY_FILE="$ready" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  [ "$rc" = 1 ] || fail "subscription must be acknowledged before reconciliation begins, got $rc"
  pass "dj_backend_herdr_wait_transition: subscribes before reconnect level-reconcile"
}
...
test_wait_transition_reconcile_dedupes_when_marked() {
  ...
  pass "dj_backend_herdr_wait_transition: a still-blocked, already-escalated pane is not re-delivered on reconnect"
}
...
test_wait_transition_stream_blocked_returns_record() {
  ...
  marker="$state/.herdr-escalated-sess_wG_pQ"
  out=$(PATH="$fb:$PATH" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 2 "$1" sess:wG:pQ' "$ROOT" "$state"); rc=$?
  ...
  pass "dj_backend_herdr_wait_transition: a streamed ->blocked edge returns the record sub-poll"
}
...
test_wait_transition_stream_absorb_clears_then_timeout() {
  ...
  rc=$(PATH="$fb:$PATH" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 2 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  ...
  pass "dj_backend_herdr_wait_transition: streamed working clears the marker, idle/done are deferred (clean timeout)"
}
...
test_wait_transition_reader_failure_returns_2() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  rc=$(PATH="$fb:$PATH" TMPDIR="$temp" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" DJ_FAKE_READER_EXIT=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
  ...
  pass "dj_backend_herdr_wait_transition: a reader/subscribe failure falls back to polling (rc 2)"
}
...
test_wait_transition_bad_ack_returns_2_and_cleans_up() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  result=$(PATH="$fb:$PATH" TMPDIR="$temp" DJ_BACKEND_HERDR_EVENTS_FORCE=1 DJ_FAKE_SESSION_NAME=sess DJ_FAKE_SOCKET="$dir/x.sock" DJ_FAKE_AGENT_DIR="$agent" \
    DJ_BACKEND_HERDR_EVENT_READER="$reader" DJ_FAKE_READER_LINES="$lines" DJ_FAKE_READER_ACK=invalid \
    /bin/bash -c '. "$0/bin/backends/herdr.sh"; dj_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; rc=$?; [ -e /dev/fd/9 ] && fd_open=yes || fd_open=no; printf "%s %s\n" "$rc" "$fd_open"' "$ROOT" "$state")
  ...
  pass "dj_backend_herdr_wait_transition: Bash 3.2-safe bad-ack path closes fd 9 and removes its FIFO"
}
...
test_wait_transition_clean_timeout_returns_1() {
  ...
  pass "dj_backend_herdr_wait_transition: stock macOS Bash clean timeout closes fd 9 and returns 1"
}
