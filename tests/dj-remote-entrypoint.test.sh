#!/usr/bin/env bash
# dj-remote-entrypoint.sh installs as a PATH symlink under ~/.local/bin
# (docs/remote-secondmates.md). SCRIPT_DIR must resolve to the real bin/
# directory so it can source its sibling dj-remote-job-lib.sh, not to the
...
TMP_ROOT=$(dj_test_tmproot dj-remote-entrypoint)
REAL_BIN="$TMP_ROOT/real-root/bin"
LOCAL_BIN="$TMP_ROOT/local-bin"
mkdir -p "$REAL_BIN" "$LOCAL_BIN"
cp "$ROOT/bin/dj-remote-entrypoint.sh" "$ROOT/bin/dj-remote-job-lib.sh" "$REAL_BIN/"
chmod +x "$REAL_BIN/dj-remote-entrypoint.sh"
ln -s "$REAL_BIN/dj-remote-entrypoint.sh" "$LOCAL_BIN/dj-remote-entrypoint.sh"
...
test_symlink_invocation_resolves_sibling_lib() {
  ...
  code=$(run_entrypoint "$LOCAL_BIN/dj-remote-entrypoint.sh" "$out" "$err")
  ...
  assert_no_grep 'No such file or directory' "$err" \
    "invoking dj-remote-entrypoint.sh through a symlink failed to source its sibling lib"
  ...
  pass "dj-remote-entrypoint.sh invoked via a PATH symlink resolves SCRIPT_DIR to the real bin/ directory"
}
...
test_direct_invocation_still_works() {
  ...
  code=$(run_entrypoint "$REAL_BIN/dj-remote-entrypoint.sh" "$out" "$err")

  expect_code 64 "$code" "direct invocation exit code"
  assert_grep 'remote entrypoint expects protocol, root, home, and argv' "$err" \
    "direct invocation did not reach argument validation"
  pass "dj-remote-entrypoint.sh invoked directly still resolves SCRIPT_DIR correctly"
}
