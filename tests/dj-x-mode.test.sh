#!/usr/bin/env bash
# Behavior tests for X mode: the relay poll client (dj-x-poll.sh), the answer
# poster (dj-x-reply.sh), and bootstrap's .env-presence activation.
#
...
TMP_ROOT=$(dj_test_tmproot dj-x-mode-tests)
...
make_sample_image() {
  local path=$1
  case "$path" in
    *.png) printf '\211PNG\r\n\032\njarvis-test-png' > "$path" ;;
    *.jpg|*.jpeg) printf '\377\330\377jarvis-test-jpeg' > "$path" ;;
    *.gif) printf 'GIF89ajarvis-test-gif' > "$path" ;;
    *.webp) printf 'RIFF....WEBPjarvis-test-webp' > "$path" ;;
    *) printf 'jarvis-test-image' > "$path" ;;
  esac
}
...
assert_no_private_artifact_temps() {
  ...
  [ -d "$dir" ] || return 0
  leftovers=$(find "$dir" -name '*.dj-x.*' -print 2>/dev/null)
...
test_poll_no_token_is_hard_noop() {
  ...
  # No .env, no DJX_PAIRING_TOKEN: must exit 0 with no output and touch nothing.
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_PAIRING_TOKEN='' \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll is a hard no-op without a token (inert default)"
}
...
test_poll_empty_env_token_overrides_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-dotenv\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_PAIRING_TOKEN='' \
  ...
  pass "dj-x-poll treats an explicitly empty env token as configured"
}
...
test_poll_204_is_silent() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-204\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_POLL_CODE=204 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll stays silent on HTTP 204 (the common case)"
}
...
test_poll_auth_error_reports_once() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-auth\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_POLL_CODE=401 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll surfaces auth/config errors once and clears on recovery"
}
...
test_poll_error_private_publication_rejects_unsafe_paths() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-linked-state FAKE_POLL_CODE=401 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-linked-marker FAKE_POLL_CODE=401 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  ln "$marker" "$hardlink"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-hard-marker FAKE_POLL_CODE=401 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll diagnostic markers use private guarded publication"
}
...
test_poll_question_stashes_and_marks() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-q\n' > "$home/.env"
  ...
  pass "dj-x-poll stashes the question and prints the compact marker"
}
...
test_poll_mentions_wake_once_per_durable_offer() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-offer\n' > "$home/.env"
  body='{"request_id":"req-repeat","platform":"discord","reply_max_chars":1900,"text":"status?"}'
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000030 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_DISMISS_CODE=200 "$ROOT/bin/dj-x-dismiss.sh" req-repeat); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000060 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  rm -f "$marker"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000090 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000120 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700604921 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll wakes once per durable request offer across inbox cleanup"
}
...
test_poll_offer_claim_failure_reports_once() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-claim-failure FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-claim-failure FAKE_POLL_CODE=204 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll retains offer claim diagnostics until recovery"
}
...
test_poll_preserves_conversation_context() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-c\n' > "$home/.env"
  ...
  pass "dj-x-poll preserves in_reply_to conversation context in the inbox"
}
...
SH
  ...
  pass "dj-x-poll reports inbox commit failures without emitting a mention wake"
}
...
test_poll_inbox_private_publication_rejects_unsafe_paths() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-linked FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-public FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-linkdest FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  ln "$dest" "$hardlink"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-hard FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  fakebin=$(make_fake_curl "$home")
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_PAIRING_TOKEN=tok-ok FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll publishes inbox records only through private guarded artifacts"
}
...
test_poll_rejects_unsafe_request_id() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-e\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
  ...
  pass "dj-x-poll rejects an unsafe request_id (path-traversal guard)"
}
...
test_reply_success_posts_request_bound_only() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-r\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-7" "Aye, charting a couple of fixes."); rc=$?
  ...
  pass "dj-x-reply posts a request-bound answer and echoes only the request_id"
}
...
SH
  chmod +x "$fakebin/curl"
  printf 'DJX_PAIRING_TOKEN=tok-clean\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_AUTH_FILE_LOG="$log" \
    "$ROOT/bin/dj-x-reply.sh" "req-clean" "Hello." 2>"$home/err"); rc=$?
  ...
  pass "dj-x-reply cleans up auth header temp files on interrupted posts"
}
...
test_reply_usage_error() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-reply.sh" "only-one" >/dev/null 2>"$err"; rc=$?
  expect_code 2 "$rc" "reply usage error exit"
  assert_grep "--image <path>" "$err" "reply usage must mention --image"
  pass "dj-x-reply rejects missing arguments with a usage error"
}

test_reply_help_mentions_image() {
  local home out rc
  home="$TMP_ROOT/reply-help"; mkdir -p "$home"
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-reply.sh" --help); rc=$?
  ...
  pass "dj-x-reply --help makes image support discoverable"
}
...
test_reply_whitespace_text_rejected() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-space" "   " 2>"$err"); rc=$?
  ...
  pass "dj-x-reply rejects whitespace-only reply text"
}

test_bootstrap_activates_on_env_token() {
  local home out sum1 sum2 n
  home="$TMP_ROOT/boot-on"; mkdir -p "$home"
  printf 'DJX_PAIRING_TOKEN=tok-boot\n' > "$home/.env"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
  ...
  assert_grep "dj-x-poll.sh" "$home/state/x-watch.check.sh" "the shim must exec the poll script"
  assert_present "$home/config/x-mode.env" "bootstrap must drop the cadence config"
  assert_grep "export DJ_CHECK_INTERVAL=30" "$home/config/x-mode.env" "cadence must be 30s"
  # Cadence inheritance: sourcing the config exports the 30s interval to a child,
  # exactly how dj-watch-arm.sh's forked watcher inherits it.
  local inherited
  # shellcheck source=/dev/null
  inherited=$( . "$home/config/x-mode.env" && bash -c 'echo "${DJ_CHECK_INTERVAL:-300}"' )
  [ "$inherited" = "30" ] \
    || fail "sourcing the cadence config must export DJ_CHECK_INTERVAL=30 to a child"
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" >/dev/null 2>&1
...
test_bootstrap_relative_home_writes_absolute_poll_shim() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-relative\n' > "$home/.env"
  out=$(
    cd "$root" || exit 1
    CDPATH="$root/cdpath" DJ_HOME=home "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null
  )
  assert_contains "$out" "FMX: X mode on" "relative-home bootstrap must announce X mode"
  quoted_home=$(printf '%q' "$home")
  assert_grep "export DJ_HOME=$quoted_home" "$home/state/x-watch.check.sh" \
    "relative DJ_HOME leaked into the durable X-mode poll shim"
  pass "bootstrap ignores CDPATH when writing absolute DJ_HOME into the durable X-mode poll shim"
}
...
SH
  ...
  printf 'DJX_PAIRING_TOKEN=tok-missing\n' > "$home/.env"
  out=$(PATH="$fakebin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$home" \
    "$BASH" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
...
test_bootstrap_does_not_announce_when_arm_fails() {
  ...
  out=$(DJ_HOME="$home" DJ_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
...
test_bootstrap_does_not_follow_x_artifact_symlinks() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-linked\n' > "$home/.env"
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" 2>"$home/bootstrap.err")
...
test_poll_empty_text_is_silent() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-t\n' > "$home/.env"
  ...
  pass "dj-x-poll requires a non-empty question before waking"
}
...
test_reply_text_file_and_stdin() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-1" --text-file "$home/reply.txt"); rc=$?
  ...
  # stdin form.
  log="$home/stdin.log"
  out=$(printf '%s' 'reply via stdin' | PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" \
    DJX_RELAY_URL="https://relay.test" FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-2" -); rc=$?
  expect_code 0 "$rc" "reply stdin exit"
  data=$(grep '^data=' "$log" | tail -1 | sed 's/^data=//')
  [ "$(printf '%s' "$data" | jq -r .text)" = 'reply via stdin' ] \
    || fail "reply via stdin must send the piped text"
  pass "dj-x-reply accepts the reply via --text-file and stdin (safe, unexpanded)"
}
...
test_bootstrap_opt_out_cleanup() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-out\n' > "$home/.env"
  DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" >/dev/null 2>&1
  ...
  printf 'DJX_PAIRING_TOKEN=\n' > "$home/.env"
  out=$(CLAUDECODE=1 DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
  ...
  assert_not_contains "$out" "bin/dj-watch-arm.sh --restart" "opt-out remediation must not hardcode a background-arm restart"
...
SH
  chmod +x "$fakebin/rm"
  printf 'DJX_PAIRING_TOKEN=\n' > "$home/.env"
  out=$(PATH="$fakebin:$PATH" DJ_HOME="$home" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null)
...
test_reply_dry_run_records_not_posts() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-d\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/dj-x-reply.sh" "req-1" "Aye, a couple of fixes underway." 2>"$home/err"); rc=$?
  ...
  pass "dj-x-reply dry-run records the would-be reply and never posts"
}
...
test_reply_dry_run_needs_no_token() {
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-2" "preview without creds" 2>/dev/null); rc=$?
  ...
  pass "dj-x-reply dry-run works without a token"
}
...
test_reply_dry_run_from_env_file() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  # DJX_DRY_RUN read from .env (not just the environment).
  printf 'DJX_PAIRING_TOKEN=tok-d\nFMX_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" "$ROOT/bin/dj-x-reply.sh" "req-3" "from dotenv" 2>/dev/null); rc=$?
  ...
  pass "dj-x-reply honors DJX_DRY_RUN from .env"
}
...
test_reply_empty_env_dry_run_overrides_env_file() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-d\nFMX_DRY_RUN=1\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_DRY_RUN='' FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-5" "empty env disables dry run" 2>/dev/null); rc=$?
  ...
  pass "dj-x-reply lets an explicitly empty dry-run env override .env"
}
...
test_reply_dry_run_fails_when_outbox_unwritable() {
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-4" "preview text" 2>"$err"); rc=$?
  ...
  pass "dj-x-reply dry-run fails when it cannot record the preview"
}
...
test_reply_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-x "preview text" 2>"$err"); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-x "preview text" 2>/dev/null); rc=$?
  ...
  assert_no_private_artifact_temps "$home/state/x-outbox"
  pass "dj-x-reply dry-run publishes outbox records only through private guarded artifacts"
}

test_split_thread_lib() {
  # shellcheck source=/dev/null
  . "$ROOT/bin/dj-x-lib.sh"
...
test_reply_single_no_texts() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-s "Short and sweet." 2>/dev/null)
  ...
  pass "dj-x-reply keeps a concise reply as a single unnumbered tweet"
}
...
test_reply_thread_dry_run() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_X_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/dj-x-reply.sh" req-t "$long" 2>/dev/null)
  ...
  pass "dj-x-reply auto-splits a long reply into a numbered thread (texts[])"
}
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-discord "$reply" 2>/dev/null)
  ...
  pass "dj-x-reply uses the Discord inbox platform budget instead of the X tweet budget"
}
...
test_reply_x_inbox_still_uses_x_budget() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-x "$long" 2>/dev/null)
  ...
  pass "dj-x-reply keeps numeric X requests on the X tweet budget"
}
...
test_reply_inbox_explicit_limit_wins() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-limit "$long" 2>/dev/null)
  ...
  pass "dj-x-reply prefers an explicit relay-provided reply limit"
}
...
test_reply_rejects_unsafe_inbox_context_reads() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-linked-dir "$reply" 2>/dev/null); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-linked-file "$reply" 2>/dev/null); rc=$?
  ...
  ln "$dest" "$hardlink"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-hardlink "$reply" 2>/dev/null); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-public-dir "$reply" 2>/dev/null); rc=$?
  ...
  pass "dj-x-reply rejects unsafe inbox context artifacts"
}
...
test_reply_max_chars_floor_clamps_to_minimum() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_X_REPLY_MAX_CHARS=49 \
    "$ROOT/bin/dj-x-reply.sh" req-floor "$long" 2>/dev/null)
  ...
  pass "dj-x-reply clamps a below-floor max to 50 characters"
}
...
test_reply_thread_live_posts_texts() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-th\n' > "$home/.env"
  # 50 is the configured minimum per-tweet budget; the text is well over it so it
  # must split into a multi-tweet thread.
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_X_REPLY_MAX_CHARS=50 FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" req-l "alpha bravo charlie delta echo foxtrot golf hotel india juliet kilo lima mike november oscar papa quebec romeo")
  ...
  pass "dj-x-reply posts a thread payload (texts[]) to the relay"
}
...
test_reply_image_live_posts_image_object() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-img\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-img" --image "$img" "Here is the illustration."); rc=$?
  ...
  pass "dj-x-reply --image posts an image object on answer"
}
...
test_reply_image_live_streams_payload_file() {
  ...
  printf 'DJX_PAIRING_TOKEN=tok-img-stream\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-img-stream" --image "$img" "Here is the illustration."); rc=$?
  ...
  pass "dj-x-reply streams large image payloads outside curl argv"
}
...
test_reply_image_thread_dry_run_records_compact_marker() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 DJX_X_REPLY_MAX_CHARS=50 \
    FAKE_CURL_LOG="$log" \
    "$ROOT/bin/dj-x-reply.sh" "req-img-dry" --image "$img" \
  ...
  pass "dj-x-reply dry-run records compact image metadata for threaded replies"
}
...
test_reply_image_dry_run_cleans_payload_temp_files() {
  ...
  make_sample_image "$img"
  out=$(PATH="$BASE_PATH" TMPDIR="$tmpdir" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-img-temp-clean" --image "$img" "Here is the image." \
    2>"$home/err"); rc=$?
  ...
  leftovers=$(find "$tmpdir" -type f -name 'dj-x-reply.*' -print)
  [ -z "$leftovers" ] || fail "reply temp files must be cleaned (left: $leftovers)"
  pass "dj-x-reply cleans image and payload temp files"
}
...
test_reply_image_path_errors_are_clear() {
  ...
  err="$home/err.txt"
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-missing" --image "$home/missing.png" "text" 2>"$err"); rc=$?
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-badtype" --image "$img" "text" 2>"$err"); rc=$?
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" "req-noarg" --image 2>"$err"); rc=$?
  ...
  pass "dj-x-reply --image rejects missing and unsupported image paths clearly"
}
...
test_reply_followup_live_posts_to_followup_endpoint() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-fu\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-7" --followup "Done, captain - the fix has shipped."); rc=$?
  ...
  pass "dj-x-reply --followup posts to /connector/followup with the same request-bound body"
}
...
test_reply_followup_409_marker_exits_distinctly() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-reply.sh" "req-409-marker" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  pass "dj-x-reply maps a followup_unavailable follow-up 409 to exit 9"
}
...
test_reply_followup_409_without_marker_still_exits_distinctly() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-reply.sh" "req-409-bare" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-reply.sh" "req-409-other" --followup "Late follow-up." 2>"$err"); rc=$?
  ...
  pass "dj-x-reply maps every follow-up 409 to exit 9 even without the marker"
}
...
test_reply_answer_409_is_generic_failure() {
  ...
  fakebin=$(make_fake_curl "$home")
  err="$home/err.txt"
  printf 'DJX_PAIRING_TOKEN=tok-answer\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_ANSWER_CODE=409 FAKE_ANSWER_BODY='{"error":"followup_unavailable"}' \
    "$ROOT/bin/dj-x-reply.sh" "req-answer-409" "Normal answer." 2>"$err"); rc=$?
  ...
  pass "dj-x-reply treats answer-endpoint 409 as a generic failure"
}
...
test_reply_followup_image_live_posts_image_object() {
  ...
  make_sample_image "$img"
  expected=$(base64 < "$img" | tr -d '\n\r')
  printf 'DJX_PAIRING_TOKEN=tok-fu-img\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-fu-img" --followup --image "$img" \
  ...
  pass "dj-x-reply --followup --image posts an image object"
}
...
test_reply_followup_flag_position_is_flexible() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-fp\n' > "$home/.env"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=280 \
    FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-a" --text-file "$home/reply.txt" --followup); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" "req-a" --text-file "$home/reply.txt"); rc=$?
  ...
  pass "dj-x-reply --followup is accepted in any position and leaves the answer path default"
}

test_reply_followup_dry_run_marks_endpoint() {
  local home out rc
  home="$TMP_ROOT/reply-followup-dry"; mkdir -p "$home"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=280 \
    "$ROOT/bin/dj-x-reply.sh" "req-d" --followup "Shipped - all green." 2>"$home/err"); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" "req-ans" "Aye." 2>/dev/null)
  ...
  pass "dj-x-reply --followup dry-run marks the endpoint without changing the answer path"
}
...
test_reply_followup_thread_dry_run() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_X_REPLY_MAX_CHARS=50 \
    DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=50 \
    "$ROOT/bin/dj-x-reply.sh" req-ft --followup "$long" 2>/dev/null)
  ...
  pass "dj-x-reply --followup auto-splits a long follow-up into a marked thread"
}
...
test_reply_followup_image_dry_run_marks_endpoint_and_compacts_image() {
  ...
  make_sample_image "$img"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_REPLY_PLATFORM=x DJX_REPLY_MAX_CHARS=280 \
    "$ROOT/bin/dj-x-reply.sh" "req-fu-img-dry" --followup --image "$img" "Done with art." \
    2>"$home/err"); rc=$?
  ...
  pass "dj-x-reply followup dry-run keeps endpoint marker and compact image metadata"
}
...
# poll records a durable per-request reply-context registry, dj-x-reply resolves
...
test_poll_records_context_registry_from_relay_platform() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-reg\n' > "$home/.env"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700000000 \
    FAKE_POLL_CODE=200 FAKE_POLL_BODY="$body" "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  pass "dj-x-poll records the durable per-request reply context from the relay payload"
}
...
test_context_registry_private_publication_rejects_unsafe_paths() {
  ...
  out=$(DJX_NOW_OVERRIDE=1700000000 bash -c '. "$1/bin/dj-x-lib.sh"; fmx_context_registry_set "$2/state" req-x x 280' _ "$ROOT" "$home")
  rc=$?
...
test_context_registry_prunes_expired_records() {
  ...
  out=$(DJX_NOW_OVERRIDE=1700000000 bash -c \
    '. "$1/bin/dj-x-lib.sh"; fmx_context_registry_get "$2" req-keep' _ "$ROOT" "$home/state")
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-retention\n' > "$home/.env"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_FOLLOWUP_MAX_AGE_SECS=100 DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_FOLLOWUP_MAX_AGE_SECS=999999999 DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=204 \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
...
test_context_registry_preserves_first_seen_timestamp() {
  ...
  mkdir -p "$home"
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-first-seen\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 \
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000100 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 \
    FAKE_POLL_BODY='{"request_id":"req-repeat","platform":"x","reply_max_chars":280,"text":"q"}' \
    "$ROOT/bin/dj-x-poll.sh"); rc=$?
...
test_context_registry_retention_starts_on_successful_live_answer() {
  ...
  mkdir -p "$home"
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-answer-window\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    DJX_RELAY_URL="https://relay.test" FAKE_POLL_CODE=200 \
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000200 \
    DJX_RELAY_URL="https://relay.test" FAKE_ANSWER_CODE=500 \
    "$ROOT/bin/dj-x-reply.sh" req-answer-window "Working on it." 2>/dev/null); rc=$?
  [ "$rc" -ne 0 ] || fail "the failed answer fixture must fail"
  [ "$(jq -r .recorded_at "$reg")" = "1700000000" ] \
    || fail "a failed answer must not refresh context retention"
  out=$(DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000300 DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-reply.sh" req-answer-window "Working on it." 2>/dev/null); rc=$?
  expect_code 0 "$rc" "answer-window dry-run exit"
  [ "$(jq -r .recorded_at "$reg")" = "1700000000" ] \
    || fail "an answer dry-run must not refresh context retention"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700604900 \
    DJX_RELAY_URL="https://relay.test" FAKE_ANSWER_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" req-answer-window "Working on it."); rc=$?
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700605000 \
    DJX_RELAY_URL="https://relay.test" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-reply.sh" req-answer-window --followup "Still working."); rc=$?
...
test_regression_discord_followup_survives_inbox_cleanup() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-rc\n' > "$home/.env"
  ...
  # 2. The acknowledgement drains the inbox file (djx-respond step 2f).
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-disc --followup - <<<"$reply" 2>/dev/null); rc=$?
...
test_regression_x_followup_still_splits_after_cleanup() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-rx\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-xs --followup - <<<"$reply" 2>/dev/null); rc=$?
...
test_regression_unresolved_followup_fails_safe() {
  ...
  err="$home/err.txt"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-none --followup - <<<"$reply" 2>"$err"); rc=$?
  ...
  log="$home/curl.log"
  err="$home/err.txt"
  printf 'DJX_PAIRING_TOKEN=tok-fs\n' > "$home/.env"
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    FAKE_CURL_LOG="$log" FAKE_REQCTX_CODE=404 \
    "$ROOT/bin/dj-x-reply.sh" req-live-none --followup - <<<"$reply" 2>"$err"); rc=$?
...
test_followup_partial_registry_uses_relay_budget_live() {
  ...
  fakebin=$(make_fake_curl "$home")
  log="$home/curl.log"
  printf 'DJX_PAIRING_TOKEN=tok-rf\n' > "$home/.env"
...
TXT
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-reply.sh" req-relay --followup - <<<"$reply"); rc=$?
...
test_regression_concurrent_requests_keep_own_platform() {
  ...
  fakebin=$(make_fake_curl "$home")
  printf 'DJX_PAIRING_TOKEN=tok-cc\n' > "$home/.env"
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-cd --followup - <<<"$discord_reply" 2>/dev/null); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-reply.sh" req-cx --followup - <<<"$x_reply" 2>/dev/null); rc=$?
...
test_dismiss_clears_context_registry() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-dismiss.sh" req-dis 2>/dev/null); rc=$?
  ...
  pass "dj-x-dismiss clears the durable per-request context (a dismissed mention gets no follow-up)"
}

# --- dj-x-dismiss: drop a mention at the relay without replying ---------------
...
test_dismiss_success_posts_request_only() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-dismiss.sh" "req-9"); rc=$?
  ...
  pass "dj-x-dismiss posts a request-bound dismiss and echoes only the request_id"
}
...
test_dismiss_dry_run_records_not_posts() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_DRY_RUN=1 FAKE_CURL_LOG="$log" \
    "$ROOT/bin/dj-x-dismiss.sh" "req-1" 2>"$home/err"); rc=$?
  ...
  pass "dj-x-dismiss dry-run records the would-be body and never posts"
}
...
test_dismiss_dry_run_needs_no_token() {
  ...
  # No token at all: dry-run still previews (it neither authenticates nor posts).
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-dismiss.sh" "req-2" 2>/dev/null); rc=$?
  ...
  pass "dj-x-dismiss dry-run works without a token"
}
...
test_dismiss_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  err="$home/err.txt"
  ln -s "$home/external" "$home/state/x-outbox"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-dismiss.sh" req-x 2>"$err"); rc=$?
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 "$ROOT/bin/dj-x-dismiss.sh" req-x 2>/dev/null); rc=$?
  ...
  assert_no_private_artifact_temps "$home/state/x-outbox"
  pass "dj-x-dismiss dry-run publishes outbox records only through private guarded artifacts"
}
...
test_dismiss_non_2xx_fails() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    ...
    "$ROOT/bin/dj-x-dismiss.sh" "req-9" 2>"$err"); rc=$?
  ...
  pass "dj-x-dismiss exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "dj-x-dismiss exits non-zero on a transport failure"
}
...
test_dismiss_unsafe_request_id_rejected() {
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" DJX_DRY_RUN=1 \
    "$ROOT/bin/dj-x-dismiss.sh" "../evil" 2>"$err"); rc=$?
  ...
  pass "dj-x-dismiss rejects an unsafe request_id (path-traversal guard)"
}

test_dismiss_usage_error() {
  local home rc
  home="$TMP_ROOT/dismiss-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-dismiss.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss missing-arg usage exit"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-dismiss.sh" req-1 extra >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss extra-arg usage exit"
  pass "dj-x-dismiss rejects missing or extra arguments with a usage error"
}

# --- dj-x-link: task <-> X-request association in meta -----------------------
...
test_link_records_request_and_timestamp() {
  ...
  # recording, not platform resolution, so dj-x-link's no-platform warning to
  # stderr is expected and dropped.
  out=$(DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/dj-x-link.sh" fix-login-k3 req-42 2>/dev/null); rc=$?
  ...
  DJ_HOME="$home" DJX_NOW_OVERRIDE=1700009999 "$ROOT/bin/dj-x-link.sh" fix-login-k3 req-99 >/dev/null 2>&1
  ...
  pass "dj-x-link records and refreshes the X-request link without disturbing meta"
}
...
test_link_records_discord_platform_for_followups() {
  ...
  DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/dj-x-link.sh" fix-discord req-discord-follow >/dev/null; rc=$?
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" fix-discord - <<<"$reply" 2>/dev/null); rc=$?
  ...
  pass "dj-x-link records Discord platform context so follow-ups keep the Discord budget"
}
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" fix-after-cleanup - <<<"$reply" 2>/dev/null); rc=$?
  ...
  pass "dj-x-link resolves the platform by request_id so a post-cleanup link keeps the Discord budget"
}

# Criterion 2 loud-warning branch: when the inbox and relay cannot resolve both
# axes, the link is still recorded but dj-x-link warns loudly and every follow-up
# is refused.
test_link_warns_loudly_when_platform_unresolvable() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  printf 'DJX_PAIRING_TOKEN=tok-unresolved\n' > "$home/.env"
  # No inbox, and the relay cannot resolve the request (404).
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700000000 FAKE_REQCTX_CODE=404 \
    "$ROOT/bin/dj-x-link.sh" fix-unresolvable req-unresolvable 2>"$err"); rc=$?
  ...
  err="$home/fu-err.txt"
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" fix-unresolvable - <<<"$reply" 2>"$err"); rc=$?
  ...
  pass "dj-x-link warns loudly and the follow-up is held (not wrongly split) when the platform is unknown"
}
...
test_link_carry_count_and_ts_preserve_followup_binding() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  DJ_HOME="$home" DJX_NOW_OVERRIDE=1700999999 \
    "$ROOT/bin/dj-x-link.sh" successor-task req-carry \
  ...
  pass "dj-x-link paired carry flags preserve a prior task's follow-up binding onto a successor"
}
...
test_link_recovery_relink_carries_discord_context_after_inbox_drain() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  DJ_HOME="$home" DJX_NOW_OVERRIDE=1700999999 \
    "$ROOT/bin/dj-x-link.sh" successor-discord req-discord-recovery \
...
TXT
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" successor-discord - <<<"$reply" 2>/dev/null); rc=$?
  ...
  pass "dj-x-link recovery relink preserves Discord platform context after inbox drain"
}
...
test_link_carry_count_validation() {
  ...
  err="$home/err.txt"
  printf 'window=w\nkind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-count abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-ts abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-count >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link --carry-count missing value exit"
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-count 1 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-platform discord >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-x-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 --carry-max 49 >/dev/null 2>"$err"; rc=$?
  ...
  pass "dj-x-link rejects malformed or unpaired carry flags"
}
...
test_meta_rewrites_do_not_depend_on_tmpdir() {
  ...
  printf 'window=w\nkind=ship\n' > "$meta"
  out=$(TMPDIR="$badtmp" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/dj-x-link.sh" fix-meta-k4 req-local 2>/dev/null); rc=$?
  ...
  out=$(TMPDIR="$badtmp" DJ_HOME="$home" DJX_NOW_OVERRIDE=1700000001 DJX_FOLLOWUP_MAX_AGE_SECS=0 \
    "$ROOT/bin/dj-x-followup.sh" --check fix-meta-k4 2>/dev/null); rc=$?
...
test_link_rejects_unsafe_and_missing() {
  ...
  printf 'kind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" "../evil" req-1 >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link unsafe task id exit"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" ok "../../etc/x" >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" no-such req-1 >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" ok >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link missing arg exit"
  pass "dj-x-link rejects unsafe ids, missing meta, and missing arguments"
}
...
test_link_missing_task_without_secondmates_stays_plain() {
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" no-such req-1 >/dev/null 2>"$err"; rc=$?
  ...
  assert_no_grep "dj-public-followup.sh register" "$err" \
    "a home with no second mates must not be pointed at the promised-final path"
  pass "dj-x-link keeps the plain missing-task error when no second mate is registered"
}
...
test_link_refuses_secondmate_routed_task_with_promised_final_pointer() {
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$main" "$ROOT/bin/dj-x-link.sh" routed-k1 req-routed 2>"$err"); rc=$?
  ...
  assert_grep "dj-public-followup.sh register" "$err" \
  ...
  out=$(PATH="$BASE_PATH" DJ_HOME="$main" DJX_NOW_OVERRIDE=1700000000 \
    "$ROOT/bin/dj-x-link.sh" local-k1 req-local 2>/dev/null); rc=$?
  ...
  pass "dj-x-link refuses a second-mate-routed task and points at the promised-final path"
}
...
test_link_missing_task_with_secondmates_points_at_promised_final() {
  ...
  printf '# Second mates\n\n- sm-far - owns the far domain (host: box; root: /srv/fm; home: /srv/dj/home; scope: far things; projects: far; added 2026-01-01)\n' \
    > "$home/data/secondmates.md"
  err="$TMP_ROOT/link-remote-secondmate.err"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-link.sh" unknown-k1 req-unknown >/dev/null 2>"$err"; rc=$?
  ...
  pass "dj-x-link points an unlocatable task at the promised-final path when second mates exist"
}

# --- dj-x-followup: detect, post up to 3 follow-ups, manage the link --------
...
mk_linked_task() { # <home> <id> <request_id> <link-epoch> [starting-count]
  ...
  printf 'window=w\nworktree=/wt\nkind=ship\nmode=no-mistakes\nyolo=off\n' > "$meta"
  DJ_HOME="$home" DJX_NOW_OVERRIDE="$ts" "$ROOT/bin/dj-x-link.sh" "$id" "$rid" \
...
test_followup_check_states() {
  ...
  out=$(DJ_HOME="$home" DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" --check task-a); rc=$?
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" --check plain 2>/dev/null); rc=$?
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" --check nope 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check missing-meta exit"
  pass "dj-x-followup --check reports postable / not-linked correctly"
}
...
test_followup_check_expired_prunes_link() {
  ...
  out=$(DJ_HOME="$home" DJX_NOW_OVERRIDE=$((1700000000 + 8*86400)) \
    "$ROOT/bin/dj-x-followup.sh" --check task-e 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup --check prunes a link past the 7-day window"
}
...
test_followup_check_cap_reached_prunes_link() {
  ...
  out=$(DJ_HOME="$home" DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" --check task-cap 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup --check prunes a link that already reached the follow-up cap"
}
...
test_followup_post_increments_counter_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-followup.sh" task-p --text-file "$home/reply.txt"); rc=$?
  ...
  pass "dj-x-followup posts a follow-up, increments the counter, and keeps the link under the cap"
}
...
test_followup_post_final_clears_link_immediately() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-followup.sh" task-final --final - <<<"Shipped - all green."); rc=$?
  ...
  pass "dj-x-followup --final clears the link after one post regardless of the remaining count"
}
...
test_followup_post_cap_reached_clears_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-followup.sh" task-cap3 - <<<"Third and final update."); rc=$?
  ...
  pass "dj-x-followup clears the link once the third follow-up reaches the cap"
}
...
test_followup_post_forwards_image_to_reply_client() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_CURL_LOG="$log" FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-followup.sh" task-img --image "$img" --final --text-file "$home/reply.txt"); rc=$?
  ...
  pass "dj-x-followup --image forwards the attachment through dj-x-reply --followup"
}
...
test_followup_post_failure_keeps_link() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=500 \
    "$ROOT/bin/dj-x-followup.sh" task-f - <<<"retry me" 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup keeps the link and counter when the post fails"
}
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=200 FAKE_CURL_TOUCH_AFTER_POST="$flag" \
    FAKE_MV_FAIL_AFTER_FLAG="$flag" FAKE_MV_FAILED_ONCE="$mvflag" \
    "$ROOT/bin/dj-x-followup.sh" task-rf - <<<"posted but local state write fails" 2>"$err"); rc=$?
  ...
  pass "dj-x-followup tombstones the link when a post-success counter write fails"
}
...
test_followup_post_relay_rejection_degrades_gracefully() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=1700003600 FAKE_FOLLOWUP_CODE=409 \
    "$ROOT/bin/dj-x-followup.sh" task-409 - <<<"rejected by relay" 2>"$err"); rc=$?
  ...
  pass "dj-x-followup treats a relay cap/window rejection as an already-exhausted link, not a retry"
}
...
test_followup_post_expired_skips_and_clears() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" DJ_HOME="$home" DJX_RELAY_URL="https://relay.test" \
    DJX_NOW_OVERRIDE=$((1700000000 + 8*86400)) FAKE_FOLLOWUP_CODE=200 \
    "$ROOT/bin/dj-x-followup.sh" task-x - <<<"too late" 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup skips silently and clears the link past the 7-day window"
}
...
test_followup_post_not_linked_is_noop() {
  ...
  printf 'kind=ship\n' > "$home/state/plain.meta"
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" plain - <<<"nothing to do" 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup is a no-op for a task with no X link"
}
...
test_followup_post_dry_run_increments_counter_keeps_link() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" task-d - <<<"Shipped in dry run." 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup dry-run records the follow-up and increments the counter, keeping the link"
}
...
test_followup_post_dry_run_final_clears_link() {
  ...
  out=$(DJ_HOME="$home" DJX_DRY_RUN=1 DJX_NOW_OVERRIDE=1700003600 \
    "$ROOT/bin/dj-x-followup.sh" task-df --final - <<<"Shipped in dry run, for real this time." 2>/dev/null); rc=$?
  ...
  pass "dj-x-followup dry-run --final clears the link just as a live post would"
}
...
test_followup_usage_errors() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" --check >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup --check no-id exit"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" some-task >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup post no-text-source exit"
  out=$(PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" --help); rc=$?
  ...
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" "../evil" --text-file /dev/null >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup unsafe-id exit"
  PATH="$BASE_PATH" DJ_HOME="$home" "$ROOT/bin/dj-x-followup.sh" some-task --image >/dev/null 2>"$err"; rc=$?
  ...
  pass "dj-x-followup rejects malformed invocations"
}
