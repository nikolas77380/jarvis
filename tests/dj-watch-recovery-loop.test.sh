...
TMP_ROOT=$(dj_test_tmproot dj-watch-recovery-loop)
...
install_pi_watch_extension_fixture() {
  ...
  mkdir -p \
    ...
    "$repo/bin"
  cp "$ROOT/.pi/extensions/dj-primary-pi-watch.ts" "$repo/.pi/extensions/dj-primary-pi-watch.ts"
...
SH
  ...
  out=$(
    PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" \
      DJ_STATE_OVERRIDE="$home/state" PATH="$fakebin:$PATH" \
...
const lockPid = existsSync(`${process.env.DJ_HOME}/state/.watch.lock/pid`)
  ? readFileSync(`${process.env.DJ_HOME}/state/.watch.lock/pid`, "utf8").trim()
  : "";
...
const marker = readFileSync(`${process.env.DJ_HOME}/state/.watcher-down`, "utf8").trim();
...
test_handling_successor_does_not_go_blind() {
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" \
    DJ_POLL=1 DJ_SIGNAL_GRACE=0 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=600 \
    DJ_WATCH_HANDLING_SUCCESSOR=1 "$WATCH" > "$out" 2>&1 &
  child=$!
  now=0
  while [ "$now" -lt 40 ]; do
