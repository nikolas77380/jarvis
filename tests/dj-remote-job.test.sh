...
TMP_ROOT=$(dj_test_tmproot dj-remote-job)
mkdir -p "$TMP_ROOT"
...
# tests/dj-remote-job-orphan-reap.test.sh pins. Stop the whole worker tree.
cleanup_remote_job_fixture() {
  ...
  if [ -f "$STATE_ROOT/worker.pid" ]; then
    dj_remote_job_stop_worker_tree "$(cat "$STATE_ROOT/worker.pid")" || true
  fi
  rm -rf -- "$TMP_ROOT"
}
trap cleanup_remote_job_fixture EXIT

cp "$ROOT/bin/dj-remote-job-lib.sh" "$ROOT/bin/dj-remote-job-worker.sh" \
  "$ROOT/bin/dj-remote-delta-read.sh" "$REMOTE_ROOT/bin/"
printf 'fixture\n' > "$REMOTE_ROOT/AGENTS.md"
cat > "$REMOTE_ROOT/bin/dj-probe-job.sh" <<'SH'
#!/bin/bash
set -u
printf 'home=%s\nroot=%s\nactive=%s\npath=%s\n' "$DJ_HOME" "$DJ_ROOT_OVERRIDE" "${DJ_REMOTE_JOB_ACTIVE:-}" "$PATH"
printf 'args:'
printf ' <%s>' "$@"
printf '\n'
if [ -n "${TOP_SECRET:-}" ]; then printf 'secret=leaked\n'; else printf 'secret=absent\n'; fi
while IFS= read -r line || [ -n "$line" ]; do printf 'stdin=%s\n' "$line"; done
exit "${DJ_PROBE_EXIT:-0}"
SH
cat > "$REMOTE_ROOT/bin/dj-timeout-job.sh" <<'SH'
#!/bin/bash
sleep 3
SH
cat > "$REMOTE_ROOT/bin/dj-delay-job.sh" <<'SH'
#!/bin/bash
sleep "$1"
printf 'ran\n' > "$2"
SH
cat > "$REMOTE_ROOT/bin/dj-touch-job.sh" <<'SH'
#!/bin/bash
printf 'ran\n' > "$1"
SH
cat > "$REMOTE_ROOT/bin/dj-shutdown-job.sh" <<'SH'
#!/bin/bash
trap '' HUP INT TERM
printf 'started\n' > "$1"
sleep 3
printf 'ran\n' > "$2"
SH
cat > "$REMOTE_ROOT/bin/dj-output-job.sh" <<'SH'
#!/bin/bash
set -e
...
DEFAULT_BOUNDS=$(
  unset DJ_REMOTE_JOB_QUEUE_TIMEOUT
  unset DJ_REMOTE_JOB_TIMEOUT
  # shellcheck disable=SC2030 # This source intentionally initializes subshell-only defaults.
  DJ_REMOTE_JOB_STATE_ROOT="$DEFAULT_STATE"
  export DJ_REMOTE_JOB_STATE_ROOT
  # shellcheck source=bin/dj-remote-job-lib.sh
  . "$ROOT/bin/dj-remote-job-lib.sh"
  dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-probe-job.sh </dev/null >/dev/null
  printf '%s %s\n' \
    "$(cat "$DEFAULT_STATE/jobs/$DJ_REMOTE_JOB_ID/queue_deadline")" \
    "$(cat "$DEFAULT_STATE/jobs/$DJ_REMOTE_JOB_ID/timeout")"
)
...
export DJ_REMOTE_JOB_STATE_ROOT="$STATE_ROOT"
export DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux
# shellcheck disable=SC2031 # The sourced defaults above were confined to DEFAULT_BOUNDS.
export DJ_REMOTE_JOB_QUEUE_TIMEOUT=5
# shellcheck disable=SC2031 # The sourced defaults above were confined to DEFAULT_BOUNDS.
export DJ_REMOTE_JOB_TIMEOUT=5
# shellcheck source=bin/dj-remote-job-lib.sh
. "$ROOT/bin/dj-remote-job-lib.sh"
...
case ":$DJ_REMOTE_JOB_OPERATOR_PATH:" in
...
dj_remote_job_compose_operator_path "$ACCOUNT_HOME" >/dev/null
NVM_SELECTED=$(PATH="$DJ_REMOTE_JOB_OPERATOR_PATH" node)
...
# tests/dj-on.test.sh reconstructs.
...
HOME="$ACCOUNT_HOME" PATH="$RUNTIME_BIN:/usr/bin:/bin:/usr/sbin:/sbin" DJ_FAKE_PERL_LOG="$FAKE_PERL_LOG" \
  DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux DJ_REMOTE_JOB_TIMEOUT=5 \
  "$REMOTE_ROOT/bin/dj-remote-job-worker.sh" > "$TMP_ROOT/worker.out" 2> "$TMP_ROOT/worker.err" &
...
TOP_SECRET=must-not-cross dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-probe-job.sh 'two words' '$(not executed)' < "$TMP_ROOT/stdin" > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
[ "$(file_mode "$JOB_DIR")" = 700 ] \
  || fail "staged job directory is not mode 0700"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "the completed probe did not preserve exit status"
OUT=$(<"$DJ_REMOTE_JOB_STDOUT")
assert_contains "$OUT" "home=$REMOTE_HOME" "the worker did not pass the staged DJ_HOME"
...
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the completed job could not be reaped"
...
DJ_REMOTE_JOB_TIMEOUT=10
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-delay-job.sh 4 "$ACTIVE_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
for _ in $(seq 1 100); do
  [ "$(dj_remote_job_read_state "$JOB_DIR" 2>/dev/null || true)" = running ] && break
  sleep 0.05
done
[ "$(dj_remote_job_read_state "$JOB_DIR" 2>/dev/null || true)" = running ] \
...
for _ in $(seq 1 40); do
  dj_remote_job_probe "$ACCOUNT_HOME" && break
  sleep 0.05
done
dj_remote_job_probe "$ACCOUNT_HOME" || fail "the active worker did not refresh its readiness heartbeat"
dj_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$(cat "$STATE_ROOT/worker.pid")" = "$ACTIVE_WORKER_PID" ] \
  || fail "ensure replaced a healthy worker during an active job"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "the active job did not complete after the readiness probe"
assert_present "$ACTIVE_SIDE_EFFECT" "the active job was interrupted by the concurrent readiness check"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the active readiness job could not be reaped"
pass "active jobs keep the worker ready for concurrent requests"

OLD_WORKER_PID=$(cat "$STATE_ROOT/worker.pid")
printf '\n' >> "$REMOTE_ROOT/bin/dj-remote-job-worker.sh"
dj_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" \
  || fail "$DJ_REMOTE_JOB_ERROR"
NEW_WORKER_PID=$(cat "$STATE_ROOT/worker.pid")
[ "$NEW_WORKER_PID" != "$OLD_WORKER_PID" ] || fail "ensure retained a worker running stale code"
dj_remote_job_worker_identity_matches "$REMOTE_ROOT" "$ACCOUNT_HOME" \
...
OLD_WORKER_PGID=$(dj_remote_job_process_pgid "$OLD_WORKER_PID") \
  || fail "the worker replacement fixture could not resolve its process group"
dj_remote_job_ensure_worker "$RELOCATED_ROOT" "$ACCOUNT_HOME" \
  || fail "$DJ_REMOTE_JOB_ERROR"
NEW_WORKER_PID=$(cat "$STATE_ROOT/worker.pid")
...
dj_remote_job_stage "$ACCOUNT_HOME" "$RELOCATED_ROOT" "$REMOTE_HOME" dj-probe-job.sh < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "the relocated worker rejected its configured code root"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the relocated-root probe could not be reaped"
dj_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" || fail "$DJ_REMOTE_JOB_ERROR"
NEW_WORKER_PID=$(cat "$STATE_ROOT/worker.pid")
...
DJ_REMOTE_JOB_TIMEOUT=1
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-timeout-job.sh < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 124 ] || fail "the worker did not terminate an over-time job"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the timed-out job could not be reaped"
pass "the worker enforces the job timeout and publishes its result"

QUEUED_SIDE_EFFECT="$TMP_ROOT/queued-side-effect"
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-timeout-job.sh < /dev/null > /dev/null
FIRST_JOB_ID=$DJ_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(dj_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
  sleep 0.05
done
[ "$(dj_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] \
  || fail "the blocking job did not begin running"
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-touch-job.sh "$QUEUED_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
printf '%s\n' "$(dj_remote_job_read_deadline "$FIRST_JOB_DIR")" > "$STATE_ROOT/jobs/$JOB_ID/queue_deadline"
dj_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 124 ] || fail "an expired queued job did not publish a timeout result"
assert_absent "$QUEUED_SIDE_EFFECT" "the worker executed a queued job after its durable deadline"
dj_remote_job_reap "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "the blocking job could not be reaped"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the expired queued job could not be reaped"
...
DJ_REMOTE_JOB_QUEUE_TIMEOUT=5
DJ_REMOTE_JOB_TIMEOUT=3
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-delay-job.sh 1.8 "$FIRST_DELAYED_SIDE_EFFECT" < /dev/null > /dev/null
FIRST_JOB_ID=$DJ_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(dj_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
...
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-delay-job.sh 1.8 "$SECOND_DELAYED_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "queue time consumed the second job's execution timeout"
assert_present "$SECOND_DELAYED_SIDE_EFFECT" "the queued job did not receive its full execution timeout"
dj_remote_job_reap "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "the first delayed job could not be reaped"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the second delayed job could not be reaped"
...
DJ_REMOTE_JOB_QUEUE_TIMEOUT=60
DJ_REMOTE_JOB_TIMEOUT=40
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 30 < /dev/null > /dev/null
POLL_JOB_ID=$DJ_REMOTE_JOB_ID
POLL_JOB_DIR="$STATE_ROOT/jobs/$POLL_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(dj_remote_job_read_state "$POLL_JOB_DIR" 2>/dev/null || true)" = running ] && break
  sleep 0.05
done
[ "$(dj_remote_job_read_state "$POLL_JOB_DIR" 2>/dev/null || true)" = running ] \
  || fail "the long-poll job did not begin running"
PREEMPT_BEGAN=$(date +%s)
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-touch-job.sh "$PREEMPT_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
PREEMPT_ELAPSED=$(( $(date +%s) - PREEMPT_BEGAN ))
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "the short command behind a long poll did not complete"
...
dj_remote_job_wait "$ACCOUNT_HOME" "$POLL_JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq "$DJ_REMOTE_JOB_PREEMPTED_EXIT" ] \
  || fail "a preempted long poll was not distinguished from an elapsed window"
[ ! -s "$DJ_REMOTE_JOB_STDOUT" ] || fail "a preempted long poll published partial stdout"
[ ! -s "$DJ_REMOTE_JOB_STDERR" ] || fail "a preempted long poll published partial stderr"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the short command could not be reaped"
dj_remote_job_reap "$ACCOUNT_HOME" "$POLL_JOB_ID" || fail "the preempted poll could not be reaped"
...
DJ_REMOTE_JOB_TIMEOUT=10
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 5 < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 0 ] || fail "the re-armed poll after preemption did not complete"
OUT=$(<"$DJ_REMOTE_JOB_STDOUT")
...
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the re-armed poll could not be reaped"
rm -f -- "$REMOTE_HOME/$REPLY_LOG_REL"
pass "a poll re-armed after preemption reads the same cursor with nothing lost"

DJ_REMOTE_JOB_TIMEOUT=15
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 6 < /dev/null > /dev/null
FIRST_JOB_ID=$DJ_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(dj_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
...
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 1 < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
POLL_PAIR_ELAPSED=$(( $(date +%s) - POLL_PAIR_BEGAN ))
[ "$DJ_REMOTE_JOB_EXIT" -eq 75 ] || fail "the first sibling poll did not close its own window"
[ "$POLL_PAIR_ELAPSED" -ge 4 ] || fail "a queued sibling poll preempted a running poll"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 75 ] || fail "the queued sibling poll did not run after the first window"
dj_remote_job_reap "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "the first sibling poll could not be reaped"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the queued sibling poll could not be reaped"
DJ_REMOTE_JOB_QUEUE_TIMEOUT=5
...
DJ_REMOTE_JOB_TIMEOUT=5
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-shutdown-job.sh "$STARTED" "$SHUTDOWN_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
for _ in $(seq 1 100); do
  [ -f "$STARTED" ] && break
  sleep 0.05
...
HOME="$ACCOUNT_HOME" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux DJ_REMOTE_JOB_TIMEOUT=1 \
  "$REMOTE_ROOT/bin/dj-remote-job-worker.sh" >> "$TMP_ROOT/worker.out" 2>> "$TMP_ROOT/worker.err" &
for _ in $(seq 1 100); do
  [ -f "$STATE_ROOT/worker.ready" ] && break
  sleep 0.05
...
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 125 ] || fail "the interrupted job did not publish an unknown-completion result"
sleep 3
assert_absent "$SHUTDOWN_SIDE_EFFECT" "the active command mutated after worker shutdown"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the interrupted job could not be reaped"
...
DJ_REMOTE_JOB_TIMEOUT=5
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-shutdown-job.sh "$CRASH_STARTED" "$CRASH_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
for _ in $(seq 1 100); do
...
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 125 ] || fail "worker crash recovery did not publish unknown completion"
sleep 3
assert_absent "$CRASH_SIDE_EFFECT" "an orphaned command mutated after worker crash recovery"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the crash-recovered job could not be reaped"
dj_remote_job_probe "$ACCOUNT_HOME" || fail "the restarted worker did not remain ready"
...
[ "$DJ_REMOTE_JOB_EXIT" -eq 124 ] || fail "the pre-execution deadline did not publish a timeout result"
...
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the pre-execution timeout leaked output readers or FIFOs"
rm -f -- "$ACCOUNT_HOME/.local/bin/git"
pass "pre-execution validation obeys the job timeout"

dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-output-job.sh < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 23 ] || fail "bounded output changed the command exit status"
OUTPUT_BYTES=$(LC_ALL=C wc -c < "$DJ_REMOTE_JOB_STDOUT" | tr -d ' ')
[ "$OUTPUT_BYTES" -le "$DJ_REMOTE_JOB_MAX_BYTES" ] || fail "the worker retained output beyond its byte bound"
ERROR_BYTES=$(LC_ALL=C wc -c < "$DJ_REMOTE_JOB_STDERR" | tr -d ' ')
[ "$ERROR_BYTES" -le "$DJ_REMOTE_JOB_MAX_BYTES" ] || fail "the worker retained stderr beyond its byte bound"
dj_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || fail "the bounded-output job could not be reaped"
...
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" dj-touch-job.sh "$SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
rm -f -- "$JOB_DIR/argv"
...
dj_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" || fail "$DJ_REMOTE_JOB_ERROR"
dj_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$DJ_REMOTE_JOB_ERROR"
[ "$DJ_REMOTE_JOB_EXIT" -eq 126 ] || fail "the worker accepted a symlinked argv record"
...
DJ_REMOTE_JOB_TIMEOUT=5
dj_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  dj-shutdown-job.sh "$QUARANTINE_STARTED" "$QUARANTINE_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$DJ_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
for _ in $(seq 1 100); do
...
dj_remote_job_probe "$ACCOUNT_HOME" && fail "quarantined worker ownership still reported ready"
set +e
HOME="$ACCOUNT_HOME" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux "$REMOTE_ROOT/bin/dj-remote-job-worker.sh" \
...
set +e
HOME="$RECOVERY_HOME" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_REMOTE_JOB_STATE_ROOT="$RECOVERY_STATE" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux "$REMOTE_ROOT/bin/dj-remote-job-worker.sh" \
...
HOME="$REPEAT_HOME" DJ_ROOT_OVERRIDE="$REMOTE_ROOT" DJ_REMOTE_JOB_STATE_ROOT="$REPEAT_STATE" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux "$REMOTE_ROOT/bin/dj-remote-job-worker.sh" --serve \
...
# A child that stays up for DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS clears the
...
cp "$ROOT/bin/dj-remote-job-lib.sh" "$RESTART_ROOT/bin/"
cp "$ROOT/bin/dj-remote-job-worker.sh" "$RESTART_ROOT/bin/dj-remote-job-supervisor-under-test.sh"
printf 'fixture\n' > "$RESTART_ROOT/AGENTS.md"
cat > "$RESTART_ROOT/bin/dj-remote-job-worker.sh" <<'SH'
#!/bin/bash
set -u
[ "${1:-}" = --serve ] || exit 2
printf '%s\n' "${BASHPID:-$$}" >> "$DJ_TEST_SUPERVISOR_CHILD_LOG"
sleep "$DJ_TEST_SUPERVISOR_CHILD_SECONDS"
exit "$DJ_TEST_SUPERVISOR_CHILD_STATUS"
SH
chmod +x "$RESTART_ROOT/bin"/*.sh
HOME="$RESTART_HOME" DJ_ROOT_OVERRIDE="$RESTART_ROOT" \
  DJ_REMOTE_JOB_STATE_ROOT="$RESTART_STATE" DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS=1 DJ_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS=3 \
  DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS=0 DJ_TEST_SUPERVISOR_CHILD_LOG="$RESTART_CHILD_LOG" \
  DJ_TEST_SUPERVISOR_CHILD_SECONDS=1.1 DJ_TEST_SUPERVISOR_CHILD_STATUS=1 \
  "$RESTART_ROOT/bin/dj-remote-job-supervisor-under-test.sh" \
