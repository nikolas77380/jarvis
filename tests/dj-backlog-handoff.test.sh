#!/usr/bin/env bash
# tests/dj-backlog-handoff.test.sh - full item-block handoff (header + indented body).
#
...
TMP_ROOT=$(dj_test_tmproot dj-backlog-handoff)
HANDOFF_FAKEBIN=$(make_fake_tmux "$TMP_ROOT/default-fake")
export PATH="$HANDOFF_FAKEBIN:$PATH"
export DJ_FAKE_TMUX_WINDOW='jarvis:dj-design'
export DJ_FAKE_TMUX_LOG="$TMP_ROOT/default-tmux.log"
export DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/default-fake/pane.txt"
export DJ_SEND_SETTLE=0 DJ_SEND_SLEEP=0 DJ_SEND_RETRIES=1
...
inbox_body_stream() { # <state-dir> <task-id>
  ...
  for rec in "$1/$2.inbox"/*.msg; do
    [ -f "$rec" ] || continue
    bash -c '. "$1"; dj_task_inbox_body "$2"' _ \
      "$ROOT/bin/dj-task-inbox-lib.sh" "$rec"
  done
}
...
doorbell_count() { # <backend-log>
  grep -cF 'Jarvis instruction waiting:' "$1" 2>/dev/null || true
}
...
EOF
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$PATH" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/live-wake-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/live-wake-fake/pane.txt" \
    DJ_SEND_SETTLE=0 DJ_SEND_SLEEP=0 DJ_SEND_RETRIES=1 \
    "$ROOT/bin/dj-backlog-handoff.sh" design wake-item > "$out" 2>&1 \
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$PATH" \
    ...
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/live-wake-fake/pane.txt" \
    DJ_SEND_SETTLE=0 DJ_SEND_SLEEP=0 DJ_SEND_RETRIES=1 \
    "$ROOT/bin/dj-backlog-handoff.sh" design wake-item > "$TMP_ROOT/live-wake-rerun.out" 2>&1 \
...
EOF
  ...
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design retry-item 2>&1) || rc=$?
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design retry-item > "$TMP_ROOT/retry-wake.out" 2>&1 \
...
SH
  ...
  out=$(PATH="$rejectbin:$basebin:$PATH" DJ_BASE_TMUX="$basebin/tmux" \
    DJ_HOME="$home" DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/known-fail-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/known-fail-fake/pane.txt" \
    "$ROOT/bin/dj-backlog-handoff.sh" design known-fail 2>&1) || rc=$?
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design known-fail \
...
#!/usr/bin/env bash
if [ "${1:-}" = send-keys ]; then
  touch "$DJ_RECONCILE_RACE_ENTERED"
  while [ ! -f "$DJ_RECONCILE_RACE_RELEASE" ]; do sleep 0.02; done
  exit 1
fi
exec "$DJ_BASE_TMUX" "$@"
SH
  ...
  PATH="$blockbin:$basebin:$PATH" DJ_BASE_TMUX="$basebin/tmux" DJ_HOME="$home" \
    DJ_RECONCILE_RACE_ENTERED="$TMP_ROOT/reconcile-race.entered" \
    DJ_RECONCILE_RACE_RELEASE="$TMP_ROOT/reconcile-race.release" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/reconcile-race-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/reconcile-race-fake/pane.txt" \
    "$ROOT/bin/dj-backlog-handoff.sh" design reconcile-race \
  ...
  corr=$(cut -d: -f2- "$home/state/.backlog-handoff-design.wake-pending")
  DJ_PENDING_REPLY_NOW=9999999999 bash -c '
    . "$1"
    dj_pending_reply_reconcile_delivery "$2" "$3"
  ' _ "$ROOT/bin/dj-pending-reply-lib.sh" "$home/state" "$corr" \
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design reconcile-race \
...
SH
  ...
  set +e
  DJ_REAL_TASKS_AXI="$real_tasks" PATH="$fakebin:$PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-backlog-handoff.sh" design crash-item > "$TMP_ROOT/move-crash.out" 2>&1
  rc=$?
  set +e
...
EOF
  rc=0
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design unrelated-move \
  ...
  : > "$TMP_ROOT/default-tmux.log"
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design crash-item \
...
SH
  ...
  set +e
  DJ_REAL_TASKS_AXI="$real_tasks" PATH="$fakebin:$PATH" DJ_HOME="$home" \
    "$ROOT/bin/dj-backlog-handoff.sh" design pre-move-crash > "$TMP_ROOT/pre-move-crash.out" 2>&1
  rc=$?
  set -e
...
EOF
  rc=0
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design unrelated-ready \
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design pre-move-crash \
...
#!/usr/bin/env bash
for arg in "$@"; do
  if [ "$arg" = "$DJ_CONFIRM_WAKE_MARKER" ] \
    && mkdir "$DJ_CONFIRM_CRASH_ONCE" 2>/dev/null; then
    kill -KILL "$PPID"
    exit 0
  fi
done
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" DJ_REAL_RM="$real_rm" \
    DJ_CONFIRM_CRASH_ONCE="$TMP_ROOT/confirm-crash.once" \
    DJ_CONFIRM_WAKE_MARKER="$home/state/.backlog-handoff-design.wake-pending" \
    DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design confirm-crash \
...
## Done
EOF
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design after-crash \
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" DJ_REAL_MV="$real_mv" DJ_HOME="$home" \
    "$ROOT/bin/dj-backlog-handoff.sh" design attempt-crash \
  ...
  rc=0
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design attempt-crash 2>&1) || rc=$?
...
#!/usr/bin/env bash
case "$*" in
  *"Jarvis instruction waiting:"*)
    if mkdir "$DJ_BLOCK_WAKE_ONCE" 2>/dev/null; then
      touch "$DJ_BLOCK_WAKE_ENTERED"
      while [ ! -f "$DJ_BLOCK_WAKE_RELEASE" ]; do sleep 0.02; done
    fi
    ;;
esac
exec "$DJ_BASE_TMUX" "$@"
SH
  ...
  PATH="$blockbin:$basebin:$PATH" DJ_HOME="$home" DJ_BASE_TMUX="$basebin/tmux" \
    DJ_BLOCK_WAKE_ONCE="$TMP_ROOT/concurrent.once" \
    DJ_BLOCK_WAKE_ENTERED="$TMP_ROOT/concurrent.entered" \
    DJ_BLOCK_WAKE_RELEASE="$TMP_ROOT/concurrent.release" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/concurrent-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/concurrent-fake/pane.txt" \
    "$ROOT/bin/dj-backlog-handoff.sh" design concurrent-a > "$TMP_ROOT/concurrent-a.out" 2>&1 &
  first=$!
  i=0
...
EOF
  PATH="$blockbin:$basebin:$PATH" DJ_HOME="$home" DJ_BASE_TMUX="$basebin/tmux" \
    ...
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/concurrent-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/concurrent-fake/pane.txt" \
    "$ROOT/bin/dj-backlog-handoff.sh" design concurrent-b > "$TMP_ROOT/concurrent-b.out" 2>&1 &
  second=$!
  sleep 0.2
...
SH
  chmod +x "$blockbin/tmux"
  PATH="$blockbin:$basebin:$PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_BASE_TMUX="$basebin/tmux" DJ_BLOCK_WAKE_ENTERED="$TMP_ROOT/teardown-race.entered" \
    DJ_BLOCK_WAKE_RELEASE="$TMP_ROOT/teardown-race.release" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/teardown-race-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-race-fake/pane.txt" \
    "$ROOT/bin/dj-backlog-handoff.sh" design teardown-race > "$TMP_ROOT/teardown-race-handoff.out" 2>&1 &
  handoff=$!
  i=0
  ...
  PATH="$basebin:$PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/teardown-race-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-race-fake/pane.txt" \
    "$ROOT/bin/dj-teardown.sh" design --force > "$TMP_ROOT/teardown-race-teardown.out" 2>&1 &
  teardown=$!
  sleep 0.3
...
test_local_teardown_preserves_wake_when_home_removal_fails() {
  ...
  corr=$(DJ_HOME="$home" bash -c '
    . "$1"
    dj_pending_reply_create "$2" "$2/state" design "New routed work is in your backlog."
  ' _ "$ROOT/bin/dj-pending-reply-lib.sh" "$home") \
...
SH
  ...
  set +e
  PATH="$rm_bin:$fakebin:$PATH" DJ_REAL_RM="$real_rm" DJ_FAIL_HOME="$fail_home" \
    DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/teardown-home-fail-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-home-fail-fake/pane.txt" \
    "$ROOT/bin/dj-teardown.sh" design --force > "$TMP_ROOT/teardown-home-fail.out" 2>&1
  rc=$?
  set -e
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_FAKE_TMUX_WINDOW='jarvis:dj-design' \
    DJ_FAKE_TMUX_LOG="$TMP_ROOT/teardown-home-fail-tmux.log" \
    DJ_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-home-fail-fake/pane.txt" \
    "$ROOT/bin/dj-teardown.sh" design --force > "$TMP_ROOT/teardown-home-retry.out" 2>&1 \
...
seed_public_commitment() {
  ...
  DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" "$ROOT/bin/dj-public-followup.sh" register \
...
EOF
  ...
  local out rc=0
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design promised-item plain-item 2>&1) || rc=$?
...
EOF
  ...
  local out rc=0
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design quiet-item 2>&1) || rc=$?
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design body-item >/dev/null \
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design section-tail >/dev/null \
...
test_body_moves_when_last_lines_of_file() {
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design eof-item >/dev/null \
...
test_eof_body_before_seeded_destination_section_keeps_boundary() {
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design seeded-eof-item >/dev/null \
...
test_untouched_eof_line_preserves_terminator() {
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design move-item >/dev/null \
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design idem-item >/dev/null \
  ...
  local out
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design idem-item 2>&1) \
...
EOF
  ...
  if out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design malformed-body 2>&1); then
...
test_indented_heading_is_not_section_boundary() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] ha-codex-fast-default-4e - harness default work (repo: jarvis)
...
- [ ] next-item - after the trap (repo: jarvis)
EOF

  local expected_block
  expected_block=$(extract_item_block "$home/data/backlog.md" ha-codex-fast-default-4e)

  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design ha-codex-fast-default-4e >/dev/null \
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design multi-para >/dev/null \
  ...
  dest_after=$(cat "$sub/data/backlog.md")
  local out
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" design multi-para 2>&1) \
...
EOF
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" "$id" paren-item >/dev/null \
...
EOF
  ...
  local out rc=0
  out=$(DJ_HOME="$home" "$ROOT/bin/dj-backlog-handoff.sh" "$id" orphan-item 2>&1) || rc=$?
