#!/usr/bin/env bash
# tests/dj-afk-launch.test.sh - the script-owned, backend-aware away-daemon
# launch (bin/dj-afk-launch.sh) and the away-mode stale-artifact lifecycle fixes
# (bin/dj-afk-start.sh). Two layers:
#
...
#   byte-identical via the dj-herdr-lab.sh fleet-state tripwire; the tmux path
#   uses uniquely-named throwaway sessions killed by exact name. A harmless
#   sleeper replaces the real daemon (DJ_AFK_LAUNCH_ENTRY) so the test observes
#   only the terminal lifecycle.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LAUNCH="$ROOT/bin/dj-afk-launch.sh"
START="$ROOT/bin/dj-afk-start.sh"

FAILED=0
fail() { printf 'not ok - %s\n' "$1" >&2; FAILED=1; }
pass() { printf 'ok - %s\n' "$1"; }

SLEEPER=$(mktemp "${TMPDIR:-/tmp}/dj-afk-sleeper.XXXXXX")
...
# ---------------------------------------------------------------------------
# UNIT 1: dj_afk_clear_stale_artifacts removes exactly the three stale artifacts.
# ---------------------------------------------------------------------------
unit_clear_stale() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-clear.XXXXXX")
  mkdir -p "$st/state"
  ...
  # Source dj-afk-start.sh inside a child bash (it sets `set -eu` and would
  # otherwise leak that into this test shell) and call the clear helper.
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" \
    bash -c '. "$1"; dj_afk_clear_stale_artifacts "$2"' _ "$START" "$st/state"
...
unit_relative_paths_are_absolute_before_daemon_launch() {
  ...
  root=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-relative-home.XXXXXX")
  mkdir -p "$root/home/state" "$root/cdpath/home/state"
  home=$(cd "$root/home" && pwd -P)
  state="$home/state"
  out=$(
    cd "$root" || exit 1
    CDPATH="$root/cdpath" DJ_HOME=home DJ_STATE_OVERRIDE=home/state \
      bash -c '. "$1"; printf "%s\n%s\n" "$DJ_HOME" "$DJ_AFK_LAUNCH_STATE"' _ "$LAUNCH"
  )
  if [ "$out" = "$home"$'\n'"$state" ]; then
  ...
  out=$(DJ_HOME="$linked_home" DJ_STATE_OVERRIDE="$linked_home/state" \
    bash -c '. "$1"; printf "%s\n%s\n" "$DJ_HOME" "$DJ_AFK_LAUNCH_STATE"' _ "$LAUNCH")
  ...
  out=$(
    cd "$root" || exit 1
    DJ_HOME=missing-home "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "DJ_HOME directory cannot be resolved: missing-home" >/dev/null; then
    pass "launcher paths: unresolved relative DJ_HOME fails loudly"
  else
    fail "launcher paths: unresolved relative DJ_HOME did not name the bad input ($out)"
  fi
  out=$(
    cd "$root" || exit 1
    DJ_HOME=home DJ_STATE_OVERRIDE=missing-state "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "DJ_STATE_OVERRIDE directory cannot be resolved: missing-state" >/dev/null; then
    pass "launcher paths: unresolved relative DJ_STATE_OVERRIDE fails loudly"
  else
    fail "launcher paths: unresolved relative DJ_STATE_OVERRIDE did not name the bad input ($out)"
  fi
  rm -rf "$root"
}
...
unit_fresh_vs_refresh() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-refresh.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  : > "$st/state/.subsuper-inject-wedged"
  ...
  ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$sleep_pid" > "$lock/pid-identity" 2>/dev/null ) || true
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" "$START" >/dev/null 2>&1
...
# ---------------------------------------------------------------------------
# UNIT 3: exit ordering - dj_afk_launch_stop SIGTERMs the daemon WHILE .afk is
# still present (so its flush is not a no-op), and clears .afk last.
# ---------------------------------------------------------------------------
unit_stop_ordering() {
  local st lock marker daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-stop.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  ...
  ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$daemon_pid" > "$lock/pid-identity" 2>/dev/null ) || true
  printf 'none\t-\tnative\n' > "$st/state/.afk-daemon-terminal"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1
...
unit_stop_rejects_reused_pid() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-pid-reuse.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  sleep 600 &
  sleeper_pid=$!
...
unit_failed_start_rolls_back_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-failed-start.XXXXXX")
  mkdir -p "$st/state"
  printf 'pending\n' > "$st/state/.subsuper-escalations"
  printf 'wedged\n' > "$st/state/.subsuper-inject-wedged"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_SUPERVISOR_TARGET=unused \
    DJ_SUPERVISOR_BACKEND=unsupported "$LAUNCH" start >/dev/null 2>&1; then
...
unit_concurrent_start_serialized() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-concurrent.XXXXXX")
  cap_session="dj-afk-concurrent-cap-$$"
  ...
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_SUPERVISOR_TARGET="$cap_pane" \
    DJ_SUPERVISOR_BACKEND=tmux DJ_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 & first=$!
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_SUPERVISOR_TARGET="$cap_pane" \
    DJ_SUPERVISOR_BACKEND=tmux DJ_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 & second=$!
...
unit_lock_initialization_grace() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-lock-init.XXXXXX")
  ...
  (
    ...
    if [ -d "$st/state/.afk-launch.lock" ]; then
      ...
      ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$$" > "$st/state/.afk-launch.lock/pid-identity" 2>/dev/null ) || true
  ...
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
unit_signal_exits_with_lock_cleanup() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-signal.XXXXXX")
  marker="$st/resumed"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_afk_launch_start() { sleep 30; }
    dj_afk_launch_main start
    : > "$2"
  ' _ "$LAUNCH" "$marker" &
  child=$!
...
unit_herdr_partial_create_recovery() {
  local st recorded
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-herdr-partial.XXXXXX")
  recorded="$st/recorded"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_AFK_LAUNCH_ENTRY=/bin/true \
    DJ_AFK_LAUNCH_LABEL=afk-exact-label RECORDED="$recorded" bash -c '
    . "$1"
    dj_backend_source() { return 0; }
    dj_backend_herdr_server_ensure() { return 0; }
    ...
    dj_afk_launch_record_write() { printf "%s:%s:%s" "$1" "$2" "$3" > "$RECORDED"; }
    dj_afk_launch_create_herdr lab:captain herdr
  ' _ "$LAUNCH"
...
unit_herdr_error_with_exact_ids_closes_exact() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-herdr-error-exact.XXXXXX")
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_backend_source() { return 0; }
    dj_backend_herdr_server_ensure() { return 0; }
...
unit_herdr_run_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-herdr-run-fail.XXXXXX")
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_backend_source() { return 0; }
    dj_backend_herdr_server_ensure() { return 0; }
...
unit_record_failure_closes_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-record-fail.XXXXXX")
  closed="$st/closed"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    dj_afk_launch_record_write() { return 1; }
    dj_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    ! dj_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_rolls_back_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-not-ready.XXXXXX")
  closed="$st/closed"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    dj_afk_launch_wait_ready() { return 1; }
    dj_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    dj_afk_launch_terminal_absent() { [ -e "$CLOSED" ]; }
    ! dj_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-not-ready-unconfirmed.XXXXXX")
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_afk_launch_wait_ready() { return 1; }
    dj_afk_launch_close_terminal() { return 1; }
    dj_afk_launch_terminal_absent() { return 1; }
    ! dj_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_tmux_absence_distinguishes_probe_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-tmux-probe.XXXXXX")
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() { printf "%s" "can'\''t find session: exact-session" >&2; return 1; }
    dj_afk_launch_terminal_absent tmux exact-session
...
unit_native_lifecycle() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-native.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" "$LAUNCH" start-native >/dev/null 2>&1 \
...
unit_native_entry_preserves_prepared_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-native-entry.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  : > "$st/state/.subsuper-escalations"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_AFK_STATE_PREPARED=1 bash -c '
    . "$1"
    DJ_AFK_DAEMON=/bin/true
    dj_afk_start_main
...
unit_close_failure_preserves_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-close-fail.XXXXXX")
  mkdir -p "$st/state"
...
unit_record_publication_atomic() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-record-atomic.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\told-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    mv() { return 1; }
    ! dj_afk_launch_record_write tmux new-session owned
  ' _ "$LAUNCH" \
...
unit_malformed_record_fails_closed() {
  local st acted
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-record-malformed.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  acted="$st/acted"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" ACTED="$acted" bash -c '
    . "$1"
    dj_afk_launch_close_terminal() { : > "$ACTED"; }
    ! dj_afk_launch_reconcile
  ' _ "$LAUNCH" \
...
unit_stop_malformed_record_fails_closed() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-stop-malformed.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
...
unit_tmux_planned_record_and_collision() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-tmux-plan.XXXXXX")
  mkdir -p "$st/state"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      if [ "$1" = new-session ]; then
        [ -s "$DJ_AFK_LAUNCH_RECORD" ] || return 9
        printf "%s" "$4" > "$DJ_HOME/created-name"
        return 1
      fi
      [ "$1" != kill-session ] || : > "$DJ_HOME/killed"
      return 1
    }
    ! dj_afk_launch_create_tmux captain:0 tmux
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-tmux-unique.XXXXXX")
  mkdir -p "$st/state"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      [ "$1" != new-session ] || { printf "%s" "$4" > "$DJ_HOME/created-name"; return 1; }
      [ "$1" != kill-session ] || : > "$DJ_HOME/killed"
      return 1
    }
    ! dj_afk_launch_create_tmux captain:0 tmux
...
unit_stop_validates_before_signal() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-stop-validate.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  ...
  ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$sleeper_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1 || true
...
unit_lock_requires_complete_metadata() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-lock-metadata.XXXXXX")
  mkdir -p "$st/state"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_pid_identity() { return 1; }
    ! dj_afk_launch_lock_acquire
...
unit_stop_surfaces_afk_removal_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-stop-remove.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    rm() { local last=${!#}; [ "$last" != "$DJ_AFK_LAUNCH_STATE/.afk" ]; }
    ! dj_afk_launch_stop
  ' _ "$LAUNCH"; then
...
unit_stop_confirms_daemon_exit() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-stop-live.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  : > "$st/state/.afk"
  printf 'none\t-\tnative\n' > "$st/state/.afk-daemon-terminal"
  ...
  ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    seq() { printf "1\n"; }
    sleep() { :; }
    kill() {
      command kill "$@"
      if [ "$1" = -TERM ]; then
        rm -rf "$DJ_AFK_LAUNCH_STATE/.supervise-daemon.lock"
      fi
    }
    ! dj_afk_launch_stop
...
unit_refresh_validates_record() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-refresh-record.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  sleep 30 & daemon_pid=$!
  printf '%s' "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid"
  ( . "$ROOT/bin/dj-wake-lib.sh"; dj_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" DJ_SUPERVISOR_TARGET=unused \
    DJ_SUPERVISOR_BACKEND=tmux bash -c '
      . "$1"
      ! dj_afk_launch_start && ! dj_afk_launch_start_native
...
unit_clear_failure_aborts_entry() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-clear-fail.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_afk_launch_reconcile() { return 0; }
    dj_afk_clear_stale_artifacts() { return 1; }
...
unit_confirmed_absence_succeeds() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-confirmed-absent.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\texact-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    dj_afk_launch_close_terminal() { return 1; }
    dj_afk_launch_terminal_absent() { return 0; }
    dj_afk_launch_reconcile
...
unit_incomplete_restore_retains_backup() {
  local st backup
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-restore-fail.XXXXXX")
  mkdir -p "$st/state"
  ...
  if DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    cp() { return 1; }
    ! dj_afk_launch_restore_backup "$2" 1
...
unit_flag_write_failure_aborts() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-flag-fail.XXXXXX")
  mkdir -p "$st/state"
  DJ_HOME="$st" DJ_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
e2e_herdr() {
  ...
  SESSION="dj-lab-afk-launch-e2e-$$"
  export HERDR_SESSION="$SESSION"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-e2e-home.XXXXXX")
  E2E_HERDR_CLEANUP() {
    # shellcheck disable=SC2031 # Cleanup reads the caller's resolved target; it does not reassign it.
    DJ_HOME="$home_tmp" DJ_STATE_OVERRIDE="$home_tmp/state" \
      DJ_SUPERVISOR_TARGET="$target" DJ_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1 || true
    herdr_safe_stop_and_delete "$SESSION" >/dev/null 2>&1 || true
    rm -rf "$home_tmp" 2>/dev/null || true
  }
  dj_herdr_lab_prepare "$SESSION" || { fail "herdr e2e: could not prepare isolated lab session"; return 0; }
  dj_backend_source herdr || { E2E_HERDR_CLEANUP; fail "herdr e2e: dj_backend_source herdr failed"; return 0; }
  dj_backend_herdr_server_ensure "$SESSION" || { E2E_HERDR_CLEANUP; fail "herdr e2e: lab server did not start"; return 0; }

  out=$(dj_backend_herdr_cli "$SESSION" workspace create --cwd "$ROOT" --label captain --no-focus 2>/dev/null)
  ...
  before=$(dj_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_before=$(dj_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')

  DJ_HOME="$home_tmp" DJ_STATE_OVERRIDE="$home_tmp/state" \
    DJ_SUPERVISOR_TARGET="$target" DJ_SUPERVISOR_BACKEND=herdr DJ_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1

  during=$(dj_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_during=$(dj_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
  dtgt=$(cut -f2 "$home_tmp/state/.afk-daemon-terminal" 2>/dev/null || true)
  dtab=$(dj_backend_herdr_cli "$SESSION" pane get "${dtgt#*:}" 2>/dev/null | jq -r '.result.pane.tab_id // empty')
  ...
  DJ_HOME="$home_tmp" DJ_STATE_OVERRIDE="$home_tmp/state" \
    DJ_SUPERVISOR_TARGET="$target" DJ_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1

  after=$(dj_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_after=$(dj_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
...
e2e_tmux() {
  ...
  cap_session="dj-afk-launch-cap-$$"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/dj-afk-tmux-home.XXXXXX")
  ...
  DJ_HOME="$home_tmp" DJ_STATE_OVERRIDE="$home_tmp/state" \
    DJ_SUPERVISOR_TARGET="$cap_pane" DJ_SUPERVISOR_BACKEND=tmux DJ_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1
  ...
  DJ_HOME="$home_tmp" DJ_STATE_OVERRIDE="$home_tmp/state" \
    DJ_SUPERVISOR_TARGET="$cap_pane" DJ_SUPERVISOR_BACKEND=tmux "$LAUNCH" stop >/dev/null 2>&1
