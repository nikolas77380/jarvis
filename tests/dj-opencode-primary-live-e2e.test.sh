...
# DJ_HOME. Existing OpenCode credentials stay in their managed store.
set -u

if [ "${DJ_OPENCODE_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_OPENCODE_LIVE_E2E=1 to run the interactive OpenCode continuity regression"
  exit 0
fi
...
dj_operational_input_encode session-start "$LEGACY_START" CURRENT_START \
...
run_ahoy_transcript_regressions() {
  ...
  cp "$ROOT/.opencode/plugins/dj-primary-sessionstart-nudge.js" \
...
run_native_ahoy_regressions() {
  ...
  (
    cd "$AHOY_PROJECT" &&
      OPENCODE_DB="$first_db" DJ_HOME="$first_home" \
  ...
  startup_text=$(sqlite3 -json "$first_db" \
    "select json_extract(p.data,'$.text') text from message m join part p on p.message_id=m.id where json_extract(m.data,'$.role')='user' and json_extract(p.data,'$.text') like '%JARVIS_OP:%' order by m.time_created limit 1;" \
    | jq -r '.[0].text')
  ...
  "$TMUX" -L "$SOCKET" new-session -d -s "$native_session" -c "$AHOY_PROJECT" \
    "env OPENCODE_DB='$later_db' DJ_HOME='$later_home' OPENCODE_DISABLE_AUTOUPDATE=1 OPENCODE_DISABLE_LSP_DOWNLOAD=1 OPENCODE_CONFIG_CONTENT='{\"permission\":{\"*\":\"allow\"}}' opencode --auto"
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  (
    cd "$AHOY_PROJECT" &&
      OPENCODE_DB="$later_db" DJ_HOME="$later_home" \
        OPENCODE_DISABLE_AUTOUPDATE=1 OPENCODE_DISABLE_LSP_DOWNLOAD=1 \
        OPENCODE_CONFIG_CONTENT='{"permission":{"*":"allow"}}' \
...
cp "$ROOT/.opencode/plugins/dj-primary-watch-arm.js" "$PROJECT/.opencode/plugins/dj-primary-watch-arm.js"
cp "$ROOT/.opencode/plugins/lib/dj-operational-input.js" "$PROJECT/.opencode/plugins/lib/dj-operational-input.js"
...
# shellcheck disable=SC2016 # The model, not this test shell, expands DJ_HOME.
PROMPT='Use the terminal to run `printf ready > "$DJ_HOME/state/opencode-model-initial"`, then respond briefly. If a later watcher wake arrives, run bin/dj-wake-drain.sh, then run `printf handled > "$DJ_HOME/state/opencode-model-handled"`. Never run or request any watcher arm command.'
"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env OPENCODE_CONFIG_CONTENT='{\"permission\":{\"*\":\"allow\"}}' DJ_HOME='$HOME_DIR' DJ_ROOT_OVERRIDE='$PROJECT' DJ_POLL=1 DJ_SIGNAL_GRACE=0 DJ_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$DJ_HOME/state/.lock\"; opencode --auto; rc=\$?; printf \"OPENCODE_EXIT=%s\\n\" \"\$rc\"; sleep 300'"
...
if printf '%s\n' "$pane" | grep -Fq '$ bin/dj-watch-arm.sh'; then
