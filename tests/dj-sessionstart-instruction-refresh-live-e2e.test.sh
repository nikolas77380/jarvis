...
# compaction. It runs an isolated tmux server, throwaway Jarvis checkout,
# and scratch DJ_HOME, so it never drives the caller's Pi session or fleet.
#
...
if [ "${DJ_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 to run the isolated real-Pi instruction-refresh regression"
  exit 0
fi

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TMUX_SOCKET="dj-sessionstart-instruction-refresh-$$"
TMUX_SESSION="instruction-refresh"
LAB=${TMPDIR:-/tmp}
LAB="${LAB%/}/dj-sessionstart-instruction-refresh-live-e2e.$$"
PROJECT="$LAB/project"
HOME_DIR="$LAB/home"
...
TEST_REF=${DJ_SESSIONSTART_INSTRUCTION_REFRESH_REF:-HEAD}
TEST_COMMIT=$(git -C "$ROOT" rev-parse --verify "$TEST_REF^{commit}" 2>/dev/null) || {
  printf 'not ok - could not resolve isolated test ref %s\n' "$TEST_REF" >&2
  exit 2
}
EXPECTATION=${DJ_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT:-updated}
case "$EXPECTATION" in
  updated|stale) ;;
  *) printf 'not ok - expected DJ_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT=updated or stale, got: %s\n' "$EXPECTATION" >&2; exit 2 ;;
esac
...
mkdir -p "$LAB"
git clone --quiet --no-hardlinks "$ROOT" "$PROJECT" || fail "could not create isolated Jarvis checkout"
...
mv "$PROJECT/bin/dj-sessionstart-run.sh" "$PROJECT/bin/.dj-sessionstart-run.real.sh"
cat > "$PROJECT/bin/dj-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
set -o pipefail
set -u
state="${DJ_HOME:?}/state"
printf 'argv=%s pi=%s root=%s home=%s\n' "$*" "${PI_CODING_AGENT:-absent}" "${DJ_ROOT_OVERRIDE:-absent}" "${DJ_HOME:-absent}" \
  >> "$state/.sessionstart-e2e-sources"
"$(dirname "$0")/.dj-sessionstart-run.real.sh" "$@" | tee -a "$state/.sessionstart-e2e-output"
exit "${PIPESTATUS[0]}"
SH
chmod +x "$PROJECT/bin/dj-sessionstart-run.sh"
...
tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -c "$PROJECT" -x 220 -y 55 \
  -e "DJ_HOME=$HOME_DIR" -e "DJ_ROOT_OVERRIDE=$PROJECT" -e "DJ_GATE_REFUSE_BYPASS=1" \
  pi --no-tools -e "$PROJECT/.pi/extensions/dj-primary-turnend-guard.ts" \
...
echo "# dj-sessionstart-instruction-refresh-live-e2e.test.sh: all live assertions passed"
