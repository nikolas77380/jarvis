#!/usr/bin/env bash
# End-to-end remote reply relay through dj-on and the process-event runner.
set -u
...
TMP_ROOT=$(dj_test_tmproot dj-remote-reply)
mkdir -p "$TMP_ROOT"
...
cleanup() {
  ...
  if [ -f "$TMP_ROOT/remote-jobs/worker.pid" ]; then
    ...
    dj_remote_job_stop_worker_tree "$worker_pid" || true
  fi
  rm -rf -- "$TMP_ROOT"
}
trap cleanup EXIT
...
remote_env() {
  ...
  DJ_FAKE_REMOTE_ENTRYPOINT="$ROOT/bin/dj-remote-entrypoint.sh" \
  DJ_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  DJ_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" \
  DJ_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
ADAPTER="$ROOT/bin/dj-procevent-remote-reply.sh"
...
remote_env "$ROOT/bin/dj-procevent.sh" start "$SID" > "$TMP_ROOT/start-one.out" 2>&1 &
RUNNER=$!
...
DJ_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/dj-wake-lib.sh"
  dj_wake_signal_seen_current "$2/state" "$2/state/ios.status"
...
reconcile_out=$(remote_env "$ROOT/bin/dj-procevent.sh" reconcile)
...
PENDING_CORR=$(dj_pending_reply_create "$PARENT" "$PARENT/state" ios 'audit the release chain')
[ -n "$PENDING_CORR" ] || fail "could not create the parent pending-reply record"
dj_pending_reply_mark_delivered "$PARENT/state" "$PENDING_CORR" \
...
[ "$(dj_pending_reply_get "$PARENT/state/pending-replies/$PENDING_CORR" phase)" = resolved ] \
...
remote_env "$ROOT/bin/dj-procevent.sh" start "$SID" >/dev/null 2>&1 \
...
ESCALATED_CORR=$(dj_pending_reply_create "$PARENT" "$PARENT/state" ios 'confirm the notarization')
[ -n "$ESCALATED_CORR" ] || fail "could not create the pending-reply record to escalate"
dj_pending_reply_mark_delivered "$PARENT/state" "$ESCALATED_CORR" \
  || fail "could not mark the escalating request delivered"
dj_pending_reply_mark_turn_completed "$PARENT/state" "$ESCALATED_CORR" request
DJ_PENDING_REPLY_SEND_HOOK=true \
  dj_pending_reply_send_recovery "$PARENT/state" "$ESCALATED_CORR" \
  || fail "the one automatic recovery repost was not sent"
dj_pending_reply_mark_turn_completed "$PARENT/state" "$ESCALATED_CORR" recovery
dj_pending_reply_maybe_escalate "$PARENT/state" "$ESCALATED_CORR" \
...
[ "$(dj_pending_reply_get "$PARENT/state/pending-replies/$ESCALATED_CORR" phase)" = resolved ] \
  || fail "the correlated reply left its escalated request unresolved"
dj_pending_reply_tick "$PARENT/state" || fail "supervision tick failed"
...
unset DJ_PENDING_REPLY_GRACE_SECS
...
for _ in $(seq 1 100); do
  for job in "$TMP_ROOT"/remote-jobs/jobs/job-*; do
    [ -d "$job" ] || continue
    if [ "$(dj_remote_job_read_state "$job" 2>/dev/null || true)" = running ]; then
      running_poll=$job
      break 2
    fi
  done
...
remote_env "$ROOT/bin/dj-on.sh" ios dj-remote-file.sh get data/reply/report.md 262144 >/dev/null
set +e
wait "$PREEMPTED_SOURCE"
preempted_rc=$?
set -e
[ "$preempted_rc" -eq "$DJ_REMOTE_JOB_PREEMPTED_EXIT" ] \
...
set +e
DJ_REMOTE_REPLY_WAIT_SECONDS=1 remote_env "$ADAPTER" source ios >/dev/null 2>&1
quiet_rc=$?
set -e
[ "$quiet_rc" -eq 75 ] || fail "a quiet reply window exited with an unexpected status: $quiet_rc"
watermark_after=$(date +%s)
caught_up=$(DJ_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/dj-pending-reply-lib.sh"
  dj_pending_reply_remote_channel_epoch "$2/state" ios
' _ "$ROOT" "$PARENT")
...
DJ_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/dj-wake-lib.sh"
  sig=$(dj_wake_signal_sig "$2/state/ios.status") || exit 1
  printf "%s" "$sig" > "$(dj_wake_signal_seen_path "$2/state" "$2/state/ios.status")"
...
remote_env "$ROOT/bin/dj-procevent.sh" start "$SID" > "$TMP_ROOT/start-two.out" 2>&1 &
RUNNER=$!
