#!/usr/bin/env bash
# tests/dj-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in dj-watcher-lock.test.sh; daemon classification/injection in
# dj-daemon.test.sh.
set -u
...
GRANT="$ROOT/bin/dj-wake-grant.sh"

TMP_ROOT=$(dj_test_tmproot dj-wake-tests)
...
test_concurrent_append_and_drain() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pids="$pids $!"
  for pid in $pids; do
    wait "$pid" || fail "concurrent append/drain subprocess failed"
  done
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" 2> "$dir/drain-two.err" || fail "final drain failed"
...
test_signal_catchup_without_running_watcher() {
  ...
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2> "$drain_err" || fail "drain after first signal failed"
...
test_stale_enqueue_before_suppressor() {
  ...
  printf 'done: ready in branch dj/stale\n' > "$state/stale.status"
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" DJ_STATE_OVERRIDE="$state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after stale wake failed"
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # fake dj-crew-state.sh the watcher reads via DJ_CREW_STATE_BIN.)
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$window" DJ_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    DJ_STALE_ESCALATE_SECS=999 DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after the immediate stale wake failed"
...
SH
  ...
  DJ_STATE_OVERRIDE="$state" "$ROOT/bin/dj-check-register.sh" task >/dev/null \
    || fail "could not register queue custom check"
  PATH="$fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_POLL=1 DJ_SIGNAL_GRACE=1 DJ_CHECK_INTERVAL=0 DJ_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after check wake failed"
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:dj-task' 'stale: s:dj-task' || fail "stale append failed"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" 2> "$dir/drain-one.err" &
  pid1=$!
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" 2> "$dir/drain-two.err" &
  pid2=$!
  ...
  leftover=$(DJ_STATE_OVERRIDE="$state" "$DRAIN" | awk -F '\t' 'NF == 5 { count++ } END { print count + 0 }')
...
test_drain_dedupes_obvious_duplicates() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "dedupe drain failed"
...
# watcher liveness via dj-guard.sh: a lapsed re-arm chain then surfaces even on a
...
case "${1:-}" in
  ...
  capture-pane) cat "${DJ_FAKE_TMUX_CAPTURE:-/dev/null}" ;;
...
SH
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_STATE_OVERRIDE="$state" DJ_FAKE_TMUX_WINDOW='jarvis:dj-mate' \
    DJ_FAKE_TMUX_LOG="$dir/tmux.log" DJ_FAKE_TMUX_CAPTURE="$dir/fake-tmux/pane.txt" \
    DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 3 > "$out" 2> "$dir/watch.err" || true
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" 2> "$dir/drain.err" \
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    ...
    DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 > "$dir/watch-second.out" 2> "$dir/watch-second.err" || true
  [ ! -s "$state/.wake-queue" ] || {
    stall_count=$(grep -c 'secondmate-wake-loop-mate-' "$state/.wake-queue" || true)
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    ...
    DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 > "$dir/watch-empty.out" 2> "$dir/watch-empty.err" || true
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_STATE_OVERRIDE="$state" DJ_FAKE_TMUX_WINDOW='jarvis:dj-mate' \
    DJ_FAKE_TMUX_LOG="$dir/tmux.log" DJ_FAKE_TMUX_CAPTURE="$dir/fake-tmux/pane.txt" \
    DJ_SECONDMATE_WAKE_STALL_SECS=60 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 > "$dir/watch-healthy.out" 2> "$dir/watch-healthy.err" || true
...
SH
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    DJ_STATE_OVERRIDE="$state" DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 \
    DJ_SIGNAL_GRACE=0 DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 \
...
test_acknowledged_stall_publication_survives_pre_marker_crash() {
  ...
  PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
    ...
    DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
    DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
    "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 > "$out" 2> "$dir/watch.err" || true
...
test_empty_prefix_mate_preserves_other_mate_receipt() {
  ...
  printf 'ios\n' > "$empty/.dj-secondmate-home"
  printf 'ios-ui\n' > "$stalled/.dj-secondmate-home"
  printf 'window=jarvis:dj-ios\nkind=secondmate\nhome=%s\n' "$empty" > "$state/ios.meta"
  printf 'window=jarvis:dj-ios-ui\nkind=secondmate\nhome=%s\n' "$stalled" > "$state/ios-ui.meta"
  ...
  while [ "$round" -le 2 ]; do
    PATH="$fakebin:$PATH" DJ_HOME="$dir" DJ_ROOT_OVERRIDE="$ROOT" \
      DJ_STATE_OVERRIDE="$state" DJ_FAKE_TMUX_WINDOW='' \
      DJ_FAKE_TMUX_LOG="$dir/tmux.log" DJ_FAKE_TMUX_CAPTURE="$dir/fake-tmux/pane.txt" \
      DJ_SECONDMATE_WAKE_STALL_SECS=1 DJ_POLL=1 DJ_SIGNAL_GRACE=0 \
      DJ_CHECK_INTERVAL=999999 DJ_HEARTBEAT=999999 \
      "$ROOT/bin/dj-watch-checkpoint.sh" --seconds 2 \
...
test_drain_asserts_watcher_liveness() {
  ...
  state="$dir/state"
  err="$dir/drain.err"
  printf 'window=test:dj-x\nkind=ship\n' > "$state/x.meta"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || fail "drain failed while asserting liveness"
  ...
  : > "$err"
  identity=$(DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; dj_pid_identity "$2"' _ "$ROOT/bin/dj-wake-lib.sh" "$$") \
    || fail "could not identify the live watcher fixture"
  mkdir "$state/.watch.lock"
  printf '%s\n' "$$" > "$state/.watch.lock/pid"
  printf '%s\n' "$dir" > "$state/.watch.lock/dj-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  touch "$state/.last-watcher-beat"
  DJ_HOME="$dir" DJ_STATE_OVERRIDE="$state" DJ_GUARD_GRACE=300 "$DRAIN" >/dev/null 2> "$err" \
...
if [ "${1:-}" = -MFcntl=:DEFAULT ]; then
  for arg in "$@"; do
    if [ "$arg" = "${DJ_WAKE_ENRICH_SWAP_PATH:-}" ]; then
      rm -f "$arg"
      ln -s "$DJ_WAKE_ENRICH_SWAP_TARGET" "$arg"
      break
    fi
  done
fi
exec "$DJ_WAKE_ENRICH_REAL_PERL" "$@"
SH
  ...
  append_wake "$state" stale test:dj-task "stale: test:dj-task" || fail "stale wake append failed"
  append_wake "$state" heartbeat heartbeat heartbeat || fail "heartbeat wake append failed"

  DJ_STATE_OVERRIDE="$state" bash -c '. "$1"; dj_wake_print_deduped "$2"' _ \
    "$ROOT/bin/dj-wake-lib.sh" "$state/.wake-queue" > "$expected"
  PATH="$dir/fakebin:$PATH" DJ_STATE_OVERRIDE="$state" DJ_WAKE_ENRICH_SWAP_PATH="$state/task.status" \
    DJ_WAKE_ENRICH_SWAP_TARGET="$outside" DJ_WAKE_ENRICH_REAL_PERL="$perl_bin" "$DRAIN" > "$out" \
...
test_enrichment_preserves_all_unread_lines_and_status_file_failures() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" \
...
test_slow_annotation_does_not_block_append_and_deleted_file_fails_open() {
  ...
  DJ_STATE_OVERRIDE="$state" DJ_WAKE_ENRICH_TEST_DELAY=3 "$DRAIN" > "$out1" &
  pid=$!
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "follow-up drain after concurrent append failed"
...
# the real bin/dj-wake-drain.sh, independent of the Pi SDK. This is the core
...
test_branch_actor_scoped_ack_never_swallows_a_main_owned_row() {
  ...
  append_wake "$state" stale "dj-window" "stale: dj-window" || fail "stale append failed"

  # The extension's own job (dj-branch-dispatch.ts) is granting exactly the
  # two task-local rows; this test drives the bash consume contract those
  # sequence numbers gate, independent of the Pi SDK.
  DJ_STATE_OVERRIDE="$state" "$GRANT" activate "$$" actor-scope || fail "branch owner activation failed"
  DJ_STATE_OVERRIDE="$state" "$GRANT" publish actor-scope 2 3 || fail "branch grant publication failed"

  out="$dir/branch-drain.out"
  err="$dir/branch-drain.err"
  DJ_STATE_OVERRIDE="$state" DJ_SUPERVISION_ACTOR=branch "$DRAIN" > "$out" 2> "$err" \
  ...
  DJ_STATE_OVERRIDE="$state" DJ_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "$sequence" --recovery-generation "$generation" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "main drain failed: $(cat "$err")"
...
test_main_drain_excludes_rows_already_granted_to_branch() {
  ...
  append_wake "$state" check "some-poll.check.sh" "check: some-poll.check.sh: merged" \
    || fail "main-only append failed"
  append_wake "$state" signal "task-a.status" "signal: task-a" || fail "signal append failed"
  DJ_STATE_OVERRIDE="$state" "$GRANT" activate "$$" main-excludes || fail "branch owner activation failed"
  DJ_STATE_OVERRIDE="$state" "$GRANT" publish main-excludes 2 || fail "branch grant publication failed"
...
test_branch_grant_refuses_rows_already_claimed_by_main() {
  ...
  append_wake "$state" signal "task-a.status" "signal: task-a" || fail "signal append failed"
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" \
    || fail "main presentation failed"
  DJ_STATE_OVERRIDE="$state" "$GRANT" activate "$$" branch-refuses || fail "branch owner activation failed"
  rc=0
  DJ_STATE_OVERRIDE="$state" "$GRANT" publish branch-refuses 1 || rc=$?
...
test_actor_filter_precedes_same_key_deduplication() {
  ...
  DJ_STATE_OVERRIDE="$state" "$GRANT" activate "$$" actor-dedup || fail "branch owner activation failed"
  DJ_STATE_OVERRIDE="$state" "$GRANT" publish actor-dedup 1 || fail "branch grant publication failed"
  append_wake "$state" signal "task-a.status" "signal: main version" || fail "main row append failed"

  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" || fail "main drain failed"
  [ "$(awk -F '\t' '$3 == "signal" { print $2 }' "$dir/main.out")" = 2 ] \
    || fail "main did not present its same-key claimed row"
  DJ_STATE_OVERRIDE="$state" DJ_SUPERVISION_ACTOR=branch "$DRAIN" > "$dir/branch.out" 2> "$dir/branch.err" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$main_sequence" --recovery-generation "$main_generation" \
    || fail "main same-key acknowledgement failed"
  DJ_STATE_OVERRIDE="$state" DJ_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "$branch_sequence" --recovery-generation "$branch_generation" \
...
test_main_reclaims_a_grant_whose_branch_owner_exited() {
  ...
  sleep 30 &
  owner=$!
  DJ_STATE_OVERRIDE="$state" "$GRANT" activate "$owner" stale-owner || {
    kill "$owner" 2>/dev/null || true
    fail "branch owner activation failed"
  }
  DJ_STATE_OVERRIDE="$state" "$GRANT" publish stale-owner 1 || {
    kill "$owner" 2>/dev/null || true
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" || fail "main reclaim drain failed"
  grep -Fq "$(printf '\tsignal\ttask-a.status\t')" "$dir/main.out" \
...
test_branch_actor_without_eligible_snapshot_refuses() {
  ...
  if DJ_STATE_OVERRIDE="$state" DJ_SUPERVISION_ACTOR=branch "$DRAIN" >/dev/null 2>"$dir/err"; then
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${DJ_TEST_PUBLISH_MARKER:-}" ]; then
  exit 1
fi
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" DJ_TEST_REAL_MV="$real_mv" DJ_TEST_PUBLISH_MARKER="$state/.watcher-down" \
  ...
  PATH="$fakebin:$PATH" DJ_TEST_REAL_MV="$real_mv" \
...
test_legacy_generationless_wake_is_adopted() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/first.out" 2> "$dir/first.err" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replay.out" 2> "$dir/replay.err" \
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/after-ack.out" 2> "$dir/after-ack.err" \
...
test_stale_recovery_generation_cannot_touch_a_newer_episode() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$newer_sequence" \
...
test_recovery_ack_failure_is_reported() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/initial.out" 2> "$dir/initial.err" \
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${DJ_TEST_ACK_MARKER:-}" ]; then
  exit 1
fi
exec "$DJ_TEST_REAL_MV" "$@"
SH
  ...
  set +e
  PATH="$fakebin:$PATH" DJ_TEST_REAL_MV="$real_mv" DJ_TEST_ACK_MARKER="$state/.watcher-down" \
    DJ_STATE_OVERRIDE="$state" "$DRAIN" --ack-through 0 --recovery-generation "$generation" \
...
test_interruption_before_and_after_raw_commit() {
  ...
  DJ_STATE_OVERRIDE="$state" DJ_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT=5 "$DRAIN" > "$before_out" &
  pid=$!
  i=0
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$replay_out" 2> "$dir/replay.err" || fail "restored pre-commit wake did not drain"
  ...
  DJ_STATE_OVERRIDE="$state" DJ_WAKE_ENRICH_TEST_DELAY=5 "$DRAIN" > "$after_out" &
  pid=$!
  ...
  wait "$pid"
  set -e
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$empty_out" 2> "$dir/after-replay.err" \
...
# The guarded self-announced status append (dj_wake_status_append_self_announced)
...
test_self_announced_append_guards() {
  ...
  run_wake_lib dj_wake_status_append_self_announced "$state" "$status" \
...
test_self_held_lock_reclaims_instead_of_deadlocking() {
  ...
  DJ_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    lock="$2/.fixture.lock"
    dj_lock_acquire_wait "$lock" || exit 10
    dj_lock_try_acquire "$lock" || exit 11
    dj_lock_release "$lock"
    [ ! -e "$lock" ] && [ ! -L "$lock" ] || exit 12
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$state" || rc=$?
  ...
  DJ_STATE_OVERRIDE="$state" bash -c '
    ...
    dj_lock_acquire_wait "$lock" || exit 10
    ( dj_lock_try_acquire "$lock" && exit 13; exit 0 ) || exit 13
    dj_lock_release "$lock"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$state" || rc=$?
...
test_historical_annotation_skips_announced_status() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "drain failed"
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "second drain failed"
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "third drain failed"
