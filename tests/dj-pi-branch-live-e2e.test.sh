...
# createAgentSession surface, the custom bash and dj_branch_report tool
...
if [ "${DJ_PI_BRANCH_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_PI_BRANCH_LIVE_E2E=1 to run the real-SDK Pi branch regression"
  exit 0
fi
...
PI_PACKAGE_DIR=${DJ_PI_PACKAGE_DIR:-"$(npm root -g)/@earendil-works/pi-coding-agent"}
if [ ! -f "$PI_PACKAGE_DIR/package.json" ]; then
  fail "Pi package absent: the live branch guard needs @earendil-works/pi-coding-agent installed (DJ_PI_PACKAGE_DIR to override)"
fi
PI_VERSION=$(jq -r '.version' "$PI_PACKAGE_DIR/package.json" 2>/dev/null || printf 'unknown')

TMP_ROOT=$(dj_test_tmproot dj-pi-branch-live)
repo="$TMP_ROOT/repo"
home="$TMP_ROOT/home"
...
cp "$ROOT/.pi/extensions/dj-branch-supervision.ts" "$repo/.pi/extensions/dj-branch-supervision.ts"
...
const home = resolve(process.env.DJ_HOME);
...
if (!fallback.includes("JARVIS WATCHER WAKE: signal: live-sdk probe")) {
...
bus.emit("dj-branch-supervision:dispatch", pinOffer);
...
cat > "$modeldir/models.json" <<'JSON'
{
  "providers": {
    "dj-live-fake": {
      ...
      "apiKey": "dj-live-placeholder",
      "models": [
        { "id": "dj-live-a", "name": "fm live a", "contextWindow": 8192, "maxTokens": 512 },
        { "id": "dj-live-b", "name": "fm live b", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
JSON
PI_PACKAGE_DIR="$PI_PACKAGE_DIR" PI_CODING_AGENT_DIR="$modeldir" DJ_LIVE_SESSIONS="$TMP_ROOT/model-sessions" \
...
const first = registry.find("dj-live-fake", "dj-live-a");
const second = registry.find("dj-live-fake", "dj-live-b");
...
if (created.session.model?.id !== "dj-live-a") {
...
if (repinned.session.model?.id !== "dj-live-b") {
...
if (unpinned.session.model?.id !== "dj-live-a") {
  ...
  "providers": {
    "dj-live-fake": {
      ...
      "models": [
        ...
        { "id": "dj-live-shallow", "name": "fm live shallow", "contextWindow": 8192, "maxTokens": 512, "reasoning": true },
        { "id": "dj-live-plain", "name": "fm live plain", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
JSON
PI_PACKAGE_DIR="$PI_PACKAGE_DIR" PI_CODING_AGENT_DIR="$effortdir" DJ_LIVE_SESSIONS="$TMP_ROOT/effort-sessions" \
...
const deep = registry.find("dj-live-fake", "dj-live-deep");
const shallow = registry.find("dj-live-fake", "dj-live-shallow");
const plain = registry.find("dj-live-fake", "dj-live-plain");
...
if (clampThinkingLevel(shallow, "dj-not-a-level") !== "off") {
