#!/usr/bin/env bash
# tests/dj-backend-herdr-workspace-per-home-e2e.test.sh - mandatory ISOLATED
# end-to-end real-herdr test for the P3 "workspace-per-home" pass (AGENTS.md
# task herdr-sm-spaces-k4). Drives the REAL bin/dj-spawn.sh and
# bin/dj-teardown.sh (not just adapter primitives), because the requirement
...
# too - only exists at dj-spawn.sh's own home-shadowing logic (the herdr case
# arm) and at dj_backend_herdr_workspace_label's DJ_HOME read; neither is
# exercised by the adapter-primitive smoke test.
#
# Mirrors tests/dj-backend-autodetect-smoke.test.sh's isolated-session
# convention: a private throwaway HERDR_SESSION (never the captain's
# default), scratch DJ_HOME(s), and scratch local-only projects.
#
...
#   - a primary-shaped home (no .dj-secondmate-home marker) spawning a
#     crewmate into the "jarvis" workspace
#   - a secondmate-shaped home (with .dj-secondmate-home) getting its own
#     labeled workspace when the PRIMARY spawns it (dj-spawn.sh's DJ_HOME
#     shadow for --secondmate)
#   - a crewmate spawned FROM that secondmate-shaped home (the secondmate
#     running its OWN dj-spawn.sh) landing in the secondmate's own workspace -
...
command -v treehouse >/dev/null 2>&1 || { echo "skip: treehouse not found (required by dj-spawn.sh)"; exit 0; }
...
# tests/dj-backend-autodetect-smoke.test.sh.
# dj-spawn no longer needs this as a symlink workaround: dj-spawn-symlink-guard-s8
# canonicalized project and backend cwd comparisons in the worktree-discovery
# poll.
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/dj-herdr-e2e.XXXXXX")
SESSION="dj-lab-herdr-e2e-$$"
...
# --- 1. primary-shaped home: a crewmate spawns into the "jarvis" space ---

CM1_OUT="$TMP_ROOT/cm1.out"; CM1_ERR="$TMP_ROOT/cm1.err"
DJ_SPAWN_NO_GUARD=1 DJ_HOME="$PRIMARY_HOME" DJ_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/dj-spawn.sh" cm1 "$PROJ1" "sh -c 'echo primary-crew-ok'" --mode no-mistakes --yolo off --backend herdr \
  >"$CM1_OUT" 2>"$CM1_ERR"
rc=$?
...
sleep 1
CM1_CAPTURE=$(dj_backend_herdr_capture "$SESSION:$CM1_PANE" 30) || fail "capture failed on cm1's pane"
...
[ "$CM1_WS_LABEL" = "jarvis" ] || fail "a primary-shaped home's crewmate should land in the 'jarvis' workspace, got '$CM1_WS_LABEL'"
pass "real herdr E2E: the primary-shaped home's crewmate landed in the 'jarvis' workspace"

# --- 2. the PRIMARY spawns a secondmate: its tab lands in the SECONDMATE's own space ---
# (dj-spawn.sh's herdr case arm shadows DJ_HOME to the secondmate's home for
# exactly this call - AGENTS.md task herdr-sm-spaces-k4, requirement 3.)

SM_OUT="$TMP_ROOT/sm.out"; SM_ERR="$TMP_ROOT/sm.err"
DJ_SPAWN_NO_GUARD=1 DJ_HOME="$PRIMARY_HOME" DJ_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/dj-spawn.sh" e2esm1 "$SM_HOME" "sh -c 'echo secondmate-launch-ok'" --secondmate --backend herdr \
  >"$SM_OUT" 2>"$SM_ERR"
rc=$?
...
DJ_SPAWN_NO_GUARD=1 DJ_HOME="$SM_HOME" DJ_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/dj-spawn.sh" cm2 "$PROJ2" "sh -c 'echo sm-crew-ok'" --mode no-mistakes --yolo off --backend herdr \
  >"$CM2_OUT" 2>"$CM2_ERR"
rc=$?
...
pass "real herdr E2E: a crewmate spawns successfully FROM a secondmate-shaped home's own dj-spawn.sh process"

sleep 1
CM2_CAPTURE=$(dj_backend_herdr_capture "$SESSION:$CM2_PANE" 30) || fail "capture failed on cm2's pane"
...
PRIMARY_LIVE=$(DJ_HOME="$PRIMARY_HOME" dj_backend_herdr_list_live "$SESSION")
assert_contains_local "$PRIMARY_LIVE" "dj-cm1" "the primary home's list_live did not see its own task"
assert_not_contains_local "$PRIMARY_LIVE" "dj-e2esm1" "the primary home's list_live must not see the secondmate's own task"
assert_not_contains_local "$PRIMARY_LIVE" "dj-cm2" "the primary home's list_live must not see the secondmate-owned crewmate's task"
...
assert_contains_local "$SM_LIVE" "dj-e2esm1" "the secondmate home's list_live did not see its own task"
assert_contains_local "$SM_LIVE" "dj-cm2" "the secondmate home's list_live did not see the crewmate spawned from it"
assert_not_contains_local "$SM_LIVE" "dj-cm1" "the secondmate home's list_live must not see the primary's task"
...
DJ_ROOT_OVERRIDE="$ROOT" DJ_STATE_OVERRIDE="$PRIMARY_HOME/state" DJ_DATA_OVERRIDE="$PRIMARY_HOME/data" \
  DJ_CONFIG_OVERRIDE="$PRIMARY_HOME/config" \
  "$ROOT/bin/dj-teardown.sh" cm1 >"$TD1_OUT" 2>&1
rc=$?
[ "$rc" -eq 0 ] || fail "dj-teardown.sh failed for the primary-shaped crewmate cm1"$'\n'"$(cat "$TD1_OUT")"
[ -f "$CM1_META" ] && fail "dj-teardown.sh did not remove cm1's meta"
if herdr pane get "$CM1_PANE" --session "$SESSION" >/dev/null 2>&1; then
  fail "dj-teardown.sh did not close cm1's pane"
fi
...
DJ_ROOT_OVERRIDE="$ROOT" DJ_STATE_OVERRIDE="$SM_HOME/state" DJ_DATA_OVERRIDE="$SM_HOME/data" \
  DJ_CONFIG_OVERRIDE="$SM_HOME/config" \
  "$ROOT/bin/dj-teardown.sh" cm2 >"$TD2_OUT" 2>&1
rc=$?
[ "$rc" -eq 0 ] || fail "dj-teardown.sh failed for the secondmate-owned crewmate cm2"$'\n'"$(cat "$TD2_OUT")"
[ -f "$CM2_META" ] && fail "dj-teardown.sh did not remove cm2's meta"
if herdr pane get "$CM2_PANE" --session "$SESSION" >/dev/null 2>&1; then
  fail "dj-teardown.sh did not close cm2's pane"
fi
if ! herdr pane get "$SM_PANE" --session "$SESSION" >/dev/null 2>&1; then
...
dj_backend_herdr_kill "$SESSION:$SM_PANE"
