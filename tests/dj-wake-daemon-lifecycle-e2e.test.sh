#!/usr/bin/env bash
# tests/dj-wake-daemon-lifecycle-e2e.test.sh - the watcher + supervise-daemon
...
# dj-watch.sh runs plus the daemon's own functions. The captain-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (dj-daemon.test.sh, dj-watcher-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(dj_test_tmproot dj-wake-daemon-e2e)
...
# always-on standalone triage is covered by dj-watch-triage.test.sh. fakebin
...
test_routine_then_terminal_after_restart() {
  ...
  DJ_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2> "$drain_err" \
    || fail "drain after routine signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
    || fail "routine signal was not queued"
  DJ_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_PANE_ALIVE=1 DJ_FAKE_TMUX_SENT="$sent" \
    DJ_FAKE_TMUX_CAPTURE="$dir/pane.txt" DJ_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_stale_pane_transient_persistent_resume() {
  ...
  case "$(DJ_STATE_OVERRIDE="$state" classify_stale "$win" "$state")" in
    self\|*) : ;;
  ...
  PATH="$fakebin:$PATH" DJ_FAKE_TMUX_WINDOW="$win" DJ_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
  ...
  # own semantic busy-state record (bin/dj-busy-lib.sh), not rendered pane text.
  stale_marker_record "$win" "$state"
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  printf 'Working...\n' > "$dir/pane.txt"
  dj_write_meta "$state/stale-w2.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  resumed_gen=$("$ROOT/bin/dj-busy-event.sh" arm "$state" stale-w2)
  "$ROOT/bin/dj-busy-event.sh" apply "$state" stale-w2 busy --gen "$resumed_gen" \
