...
TMP_ROOT=$(dj_test_tmproot dj-procevent-tests)
export DJ_PROCEVENT_CLAIM_ROOT="$TMP_ROOT/claims"
...
pe() { DJ_HOME="$1" "$ROOT/bin/dj-procevent.sh" "${@:2}"; }
...
procevent_teardown() {
  ...
  for entry in ${PE_TRACKED[@]+"${PE_TRACKED[@]}"}; do
    ...
    seen+="$home"$'\n'
    DJ_HOME="$home" "$ROOT/bin/dj-procevent.sh" sweep-home >/dev/null 2>&1 || true
  done
  dj_test_cleanup
}
...
hold_source_lock() {  # <source-id> <ready-file> <release-file>
  ...
  DJ_HOME="$TMP_ROOT/lock-helper-home" bash -c '
    . "$1/bin/dj-pr-lib.sh"
    . "$1/bin/dj-wake-lib.sh"
    . "$1/bin/dj-procevent-lib.sh"
    dj_procevent_source_lock_acquire "$2" || exit 1
    trap "dj_procevent_source_lock_release \"$2\"" EXIT
    printf "ready\n" > "$3"
    while [ ! -e "$4" ]; do
...
hold_source_lock_then_handle() {  # <home> <source-id> <sequence> <ready-file> <release-file>
  ...
  DJ_HOME="$home" bash -c '
    ...
    dj_procevent_mark_handled "$3/state" "$2" "$7"
...
sup=$(PATH="${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/dj-supervision-lib.sh"; dj_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$IDLE/state")
...
sup=$(PATH="${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/dj-supervision-lib.sh"; dj_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$H1/state")
...
wait_for "$DJ_PROCEVENT_CLAIM_ROOT/src-one.claim" || fail "reconcile never claimed the registered source"
...
mode=$(PATH="${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
...
direct_leader=$(sed -n '2p' "$DJ_PROCEVENT_CLAIM_ROOT/direct-src.claim")
...
wait_for "$DJ_PROCEVENT_CLAIM_ROOT/shared-src.claim" || fail "shared-group start never claimed its source"
...
private_mode=$(PATH="${DJ_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/dj-pr-lib.sh"; dj_pr_file_mode "$2"' _ "$ROOT" "$HPRIVATE/state/procevent-inbox/private-src.1.handled")
...
cat > "$ADAPTER_ROOT/bin/dj-procevent-endnow.sh" <<'SH'
#!/usr/bin/env bash
...
exit 2
SH
cat > "$ADAPTER_ROOT/bin/dj-procevent-openended.sh" <<'SH'
#!/usr/bin/env bash
# Fixture adapter with no terminal knowledge at all: nothing ever ends it.
exit 2
SH
cat > "$ADAPTER_ROOT/bin/dj-procevent-applying.sh" <<'SH'
#!/usr/bin/env bash
case "${1-}" in
  autohandle)
    printf '%s %s\n' "$2" "$3" >> "$DJ_HOME/state/applied"
    "$DJ_PROCEVENT_UNDER_TEST" handled "$2" "$3" >/dev/null
    ;;
  *) exit 2 ;;
esac
SH
cat > "$ADAPTER_ROOT/bin/dj-procevent-selfann.sh" <<'SH'
#!/usr/bin/env bash
# Fixture adapter that declares a durable downstream announcement of its own.
# DJ_HOME/state/selfann-fail makes its application fail so the fallback
# publication path stays provable.
case "${1-}" in
  self-announcing) exit 0 ;;
  autohandle)
    [ ! -e "$DJ_HOME/state/selfann-fail" ] || exit 1
    printf '%s %s\n' "$2" "$3" >> "$DJ_HOME/state/applied"
    "$DJ_PROCEVENT_UNDER_TEST" handled "$2" "$3" >/dev/null
    ;;
  *) exit 2 ;;
...
SH
chmod +x "$ADAPTER_ROOT/bin/dj-procevent-endnow.sh" "$ADAPTER_ROOT/bin/dj-procevent-openended.sh" \
  "$ADAPTER_ROOT/bin/dj-procevent-applying.sh" "$ADAPTER_ROOT/bin/dj-procevent-selfann.sh"

pe_adapter() {  # <home> <command>...: run the runner against the fixture adapters
  local home=$1
  shift
  DJ_ROOT_OVERRIDE="$ADAPTER_ROOT" DJ_PROCEVENT_UNDER_TEST="$ROOT/bin/dj-procevent.sh" \
    DJ_HOME="$home" "$ROOT/bin/dj-procevent.sh" "$@"
}
...
DJ_HOME="$HPUBLISH" DJ_PROCEVENT_UNDER_TEST="$ROOT/bin/dj-procevent.sh" \
  "$ADAPTER_ROOT/bin/dj-procevent-applying.sh" autohandle publish-src 1 \
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/ends-src.claim" "terminal retirement releases this runner's own claim"
...
replace_old_pid=$!
wait_for "$DJ_PROCEVENT_CLAIM_ROOT/replace-src.claim" || fail "the old registration was never claimed"
...
FAIL_RM_BIN=$(dj_fakebin "$TMP_ROOT/retire-fail-bin")
...
assert_present "$DJ_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
# Simulate a replacement Jarvis session: reconcile runs cold, as it would on
...
runner_pid=$(sed -n '2p' "$DJ_PROCEVENT_CLAIM_ROOT/shared-src.claim" 2>/dev/null)
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/shared-src.claim" "retire releases the claim"
...
CLAIM="$DJ_PROCEVENT_CLAIM_ROOT/stale-src.claim"
mkdir -p "$DJ_PROCEVENT_CLAIM_ROOT"
...
printf '%s\n%s\ncross-home-token\ncross-home-identity\n%s\n' \
  "$HC_OLD" "999999" "$HC_OLD_STATE/procevent" > "$DJ_PROCEVENT_CLAIM_ROOT/cross-home-src.claim"
...
orphan_leader=$(sed -n '2p' "$DJ_PROCEVENT_CLAIM_ROOT/orphan-src.claim")
...
case "$orphan_out" in
  *"started=1"*)
    ...
    wait_for "$DJ_PROCEVENT_CLAIM_ROOT/orphan-src.claim" \
    ...
    ;;
  *"started=0"*)
    [ -e "$DJ_PROCEVENT_CLAIM_ROOT/orphan-src.claim" ] \
...
awk 'NR == 3 { print "replacement-token"; next } { print }' \
  "$DJ_PROCEVENT_CLAIM_ROOT/torn-src.claim" > "$TMP_ROOT/torn-next.claim"
...
mv "$TMP_ROOT/torn-next.claim" "$DJ_PROCEVENT_CLAIM_ROOT/torn-src.claim"
: > "$TORN_RELEASE"
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/retire-start-src.claim" "retirement cannot leave a late claim"
...
printf '%s\n%s\nreused-token\nnot-the-live-process-identity\n' \
  "$HI" "$innocent_pid" > "$DJ_PROCEVENT_CLAIM_ROOT/reused-src.claim"
chmod 0600 "$DJ_PROCEVENT_CLAIM_ROOT/reused-src.claim"
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/reused-src.claim" "retirement releases the exact reused-pid claim"
...
IDENTITY_FAKEBIN=$(dj_fakebin "$TMP_ROOT/identity-tools")
...
identity_status=0
identity_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" DJ_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proc" \
...
assert_present "$DJ_PROCEVENT_CLAIM_ROOT/identity-src.claim" "uncertain retirement preserves claim generation"
...
sweep_pid_one=$(sed -n '2p' "$DJ_PROCEVENT_CLAIM_ROOT/sweep-one.claim")
sweep_pid_two=$(sed -n '2p' "$DJ_PROCEVENT_CLAIM_ROOT/sweep-two.claim")
...
assert_present "$DJ_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep preflight does not release claims"
...
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep releases the first claim"
assert_absent "$DJ_PROCEVENT_CLAIM_ROOT/sweep-two.claim" "home sweep releases a claim with no registration"
...
assert_present "$DJ_PROCEVENT_CLAIM_ROOT/foreign-src.claim" "home sweep preserves a foreign-home claim"
[ "$(sed -n '1p' "$DJ_PROCEVENT_CLAIM_ROOT/foreign-src.claim")" = "$HN" ] || fail "home sweep changed foreign claim ownership"
...
sweep_status=0
sweep_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" DJ_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-sweep-proc" \
...
assert_present "$DJ_PROCEVENT_CLAIM_ROOT/sweep-uncertain.claim" "uncertain home sweep preserves the claim"
...
sup=$(bash -c '. "$1/bin/dj-supervision-lib.sh"; dj_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$HV/state")
...
guard_out=$(DJ_ROOT_OVERRIDE="$TMP_ROOT/guard-root" DJ_HOME="$HS" DJ_GUARD_GRACE=1 \
  "$ROOT/bin/dj-guard.sh" 2>&1)
...
for adapter in remote-reply when; do
  ! "$ROOT/bin/dj-procevent-$adapter.sh" silent "$SIL" >/dev/null 2>&1 \
...
runner_help=$("$ROOT/bin/dj-procevent.sh" --help 2>&1 || true)
