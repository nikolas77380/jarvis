#!/usr/bin/env bash
# Behavior tests for bin/dj-brief.sh.
#
...
TMP_ROOT=$(dj_test_tmproot dj-brief)
...
test_script_parses() {
  local out rc
  out=$(bash -n "$ROOT/bin/dj-brief.sh" 2>&1); rc=$?
  expect_code 0 "$rc" "bash -n bin/dj-brief.sh must parse cleanly (got: $out)"
  [ -z "$out" ] || fail "bash -n bin/dj-brief.sh emitted unexpected output: $out"
  pass "dj-brief.sh: bash -n succeeds"
}
...
test_no_heredoc_in_command_substitution() {
  ...
  no_heredoc_in_command_substitution "$ROOT/bin/dj-brief.sh" \
    || fail "dj-brief.sh wraps a heredoc in a command substitution (breaks Bash 3.2 parsing)"
  pass "dj-brief.sh: no heredoc is nested inside a command substitution (Bash 3.2 parse-safe)"
}
...
test_help_includes_entire_header() {
  local help
  help=$("$ROOT/bin/dj-brief.sh" --help)
  assert_contains "$help" "Refuses to overwrite an existing brief." "dj-brief.sh --help omitted its header terminator"
  pass "dj-brief.sh: --help renders the complete header"
}

# Registry with one project per delivery mode. dj-brief.sh no longer reads it -
...
# dj-brief.sh must exit 0 and produce a brief with no unreplaced shell
...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    id=${id_mode%%:*}
    mode=${id_mode##*:}
    DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" some-proj --mode "$mode" >/dev/null 2>&1; status=$?
    expect_code 0 "$status" "dj-brief.sh $id --mode $mode should exit 0"
  ...
  pass "dj-brief.sh: no-mistakes/direct-PR/local-only briefs generate cleanly"
}

# A ship task's delivery mode is jarvis's per-task decision, so a missing or
...
test_ship_mode_is_required_and_closed_set() {
  ...
  while IFS='|' read -r label flag expect; do
    ...
    out=$(DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "brief-required-$id" some-proj $flag 2>&1)
    status=$?
...
ROWS
  pass "dj-brief.sh: ship --mode is required and closed-set validated"
}
...
test_ship_mode_is_explicit_not_registry() {
  ...
  write_registry "$home"
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" brief-explicit-a5 direct-proj --mode no-mistakes >/dev/null 2>&1 \
    || fail "explicit no-mistakes brief on a direct-PR project should scaffold"
  brief="$home/data/brief-explicit-a5/brief.md"
  grep -qx "Delivery contract: mode=no-mistakes" "$brief" \
    || fail "registered direct-PR posture overrode the explicit --mode"
  assert_grep "Jarvis will then instruct you to run /no-mistakes" "$brief" \
    "explicit no-mistakes brief did not render the pipeline definition of done"

  # An unregistered project is not a blocker either, because nothing is looked up.
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" brief-explicit-a6 never-registered --mode local-only >/dev/null 2>&1 \
  ...
  pass "dj-brief.sh: the explicit ship mode wins over the registered posture"
}

# yolo is jarvis's merge authority and never reaches the worker, and a scout
...
test_delivery_flags_are_refused_where_they_do_not_apply() {
  ...
  while IFS='|' read -r label args expect; do
    ...
    out=$(DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" $args 2>&1)
    status=$?
    [ "$status" -ne 0 ] || fail "$label: expected a non-zero exit"
...
ROWS
  pass "dj-brief.sh: --yolo and scout/secondmate --mode are refused, never silently dropped"
}
...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" direct-proj --mode direct-PR >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "The configured merge authority decides whether to merge the PR; jarvis relays the outcome." "$brief" \
    "direct-PR brief lost configured merge authority"
  assert_no_grep "The captain reviews and merges the PR" "$brief" \
    "direct-PR brief hard-coded captain-only authority"
  id="brief-local-authority-a4"
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" local-proj --mode local-only >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "The configured merge authority approves the ready branch, then jarvis merges it into local \`main\` through the guarded fast-forward path." "$brief" \
    "local-only brief lost configured merge authority and guarded landing"
  assert_no_grep "The captain approves the ready branch" "$brief" \
    "local-only brief hard-coded captain-only authority"
  assert_no_grep "Jarvis then reviews your branch diff" "$brief" \
  ...
  pass "dj-brief.sh: faster paths use configured authority without stacked review"
}
...
test_no_mistakes_dod_wording() {
  ...
  mkdir -p "$home/data"
  id="brief-wording-b1"
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" some-proj --mode no-mistakes >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  ...
  # The apostrophe in "jarvis's authority check" is now structurally safe
  # (no `$(...)` wrapper around the heredoc), so it renders verbatim instead of
  # being reworded or escaped away. test_no_heredoc_in_command_substitution
  # guards the structure that makes it safe.
  assert_grep "jarvis's authority check" "$brief" \
    "no-mistakes DOD lost the apostrophe prose that the structural fix makes parse-safe"
  pass "dj-brief.sh: no-mistakes DOD keeps its apostrophe prose, now parse-safe"
}
...
test_ship_project_memory_wording() {
  ...
  pass "dj-brief.sh: ship project-memory wording carries the AGENTS.md authoring bar"
}
...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" jarvis --mode no-mistakes --herdr-lab >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_present "$brief" "Herdr lab brief was not scaffolded"
  assert_grep "# Herdr isolation - HARD SAFETY CONTRACT" "$brief" \
    "Herdr lab brief missing its hard safety contract"
  assert_grep "HERDR_LAB_HELPER='$ROOT/bin/dj-herdr-lab.sh'" "$brief" \
    "Herdr lab brief must bind the absolute Jarvis helper path"
  ...
  pass "dj-brief.sh: --herdr-lab emits the complete hard safety contract"
}

test_herdr_lab_contract_quotes_foreign_jarvis_path() {
  local home id brief foreign_root helper
  home="$TMP_ROOT/herdr-lab-foreign-home"
  foreign_root="$TMP_ROOT/jarvis helper's root"
  mkdir -p "$home/data"
  id="brief-herdr-lab-foreign-d2"
  helper=$(printf '%s' "$foreign_root/bin/dj-herdr-lab.sh" | sed "s/'/'\\\\''/g")
  helper="'$helper'"
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$foreign_root" "$ROOT/bin/dj-brief.sh" "$id" foreign --scout --herdr-lab >/dev/null 2>&1
  brief="$home/data/$id/brief.md"
  assert_grep "HERDR_LAB_HELPER=$helper" "$brief" \
    "Herdr lab brief must shell-quote an absolute Jarvis helper path"
  assert_no_grep "bin/dj-herdr-lab.sh name $id" "$brief" \
    "Herdr lab brief must not invoke a worktree-relative helper"
  pass "dj-brief.sh: --herdr-lab uses its quoted Jarvis-owned helper path"
}
...
test_herdr_lab_omission_is_loud_for_ship_and_scout() {
  ...
  for kind in ship scout; do
    ...
    if [ "$kind" = scout ]; then
      DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" jarvis --scout >/dev/null 2>&1
    else
      DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" "$id" jarvis --mode no-mistakes >/dev/null 2>&1
    fi
    brief="$home/data/$id/brief.md"
  ...
  pass "dj-brief.sh: ship and scout scaffolds make omitted Herdr intent fail-visible"
}

# Regression (issue #2575): AGENTS.md section 11 and this script's own help tell
# jarvis to replace EVERY `{TASK}` placeholder. The unguarded Herdr gate used
...
test_documented_global_replace_leaves_the_herdr_gate_intact() {
  ...
  pass "dj-brief.sh: the documented {TASK} fill cannot corrupt the Herdr safety gate"
}
...
test_secondmate_no_projects_charter() {
  ...
  # a domain whose subject is the jarvis repo itself (no clones needed).
  DJ_HOME="$home" DJ_SECONDMATE_CHARTER='jarvis self-development' \
    DJ_SECONDMATE_SCOPE='jarvis repo work' \
    "$ROOT/bin/dj-brief.sh" fdev --secondmate --no-projects >/dev/null 2>&1; status=$?
  ...
  DJ_HOME="$home" DJ_SECONDMATE_CHARTER='x' "$ROOT/bin/dj-brief.sh" oops --secondmate >/dev/null 2>&1; status=$?
  ...
  DJ_HOME="$home" DJ_SECONDMATE_CHARTER='x' "$ROOT/bin/dj-brief.sh" oops2 --secondmate --no-projects alpha >/dev/null 2>&1; status=$?
  expect_code 1 "$status" "--no-projects combined with a project list must fail"

  # --no-projects applies only to secondmate charters, never a ship/scout brief.
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" oops3 somerepo --no-projects >/dev/null 2>&1; status=$?
  expect_code 1 "$status" "--no-projects on a ship brief must fail"

  pass "dj-brief.sh: --no-projects scaffolds a project-less charter and guards misuse"
}
...
test_secondmate_marked_request_reporting_contract() {
  ...
  mkdir -p "$home/data"
  DJ_HOME="$home" DJ_CLASSIFY_PAUSED_VERB=paused \
    DJ_SECONDMATE_CHARTER='Handle routed domain work.' \
    "$ROOT/bin/dj-brief.sh" marked-request-reporting --secondmate --no-projects >/dev/null 2>&1
  ...
  pass "dj-brief.sh: marked requests avoid generic acknowledgements and preserve material reporting"
}
...
test_secondmate_directory_paths_are_absolute_and_output_is_stable() {
  ...
  DJ_HOME="$home" DJ_SECONDMATE_CHARTER=x \
    "$ROOT/bin/dj-brief.sh" relative-home --secondmate --no-projects >/dev/null 2>&1
  baseline="$root/absolute-home-charter"
  cp "$brief" "$baseline"
  rm -f "$brief"
  (
    cd "$root" || exit 1
    CDPATH="$root/cdpath" DJ_HOME=home DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" relative-home --secondmate --no-projects >/dev/null 2>&1
  )
  cmp -s "$baseline" "$brief" \
    || fail "relative DJ_HOME changed charter bytes compared with the same absolute home"
  assert_grep ">> '$home/state/relative-home.status'" "$brief" \
    "relative DJ_HOME did not render an absolute secondmate status path"

  brief="$home/data/relative-state/brief.md"
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state_override" DJ_SECONDMATE_CHARTER=x \
    "$ROOT/bin/dj-brief.sh" relative-state --secondmate --no-projects >/dev/null 2>&1
  baseline="$root/absolute-state-charter"
  cp "$brief" "$baseline"
  rm -f "$brief"
  (
    cd "$root" || exit 1
    CDPATH="$root/cdpath" DJ_HOME="$home" DJ_STATE_OVERRIDE=state-override DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" relative-state --secondmate --no-projects >/dev/null 2>&1
  )
  cmp -s "$baseline" "$brief" \
    || fail "relative DJ_STATE_OVERRIDE changed charter bytes compared with the same absolute state directory"
  assert_grep ">> '$state_override/relative-state.status'" "$brief" \
    "relative DJ_STATE_OVERRIDE did not render an absolute secondmate status path"

  brief="$data_override/relative-data/brief.md"
  DJ_HOME="$home" DJ_DATA_OVERRIDE="$data_override" DJ_SECONDMATE_CHARTER=x \
    "$ROOT/bin/dj-brief.sh" relative-data --secondmate --no-projects >/dev/null 2>&1
  baseline="$root/absolute-data-charter"
  cp "$brief" "$baseline"
  rm -f "$brief"
  (
    cd "$root" || exit 1
    CDPATH="$root/cdpath" DJ_HOME="$home" DJ_DATA_OVERRIDE=data-override DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" relative-data --secondmate --no-projects >/dev/null 2>&1
  )
  cmp -s "$baseline" "$brief" \
    || fail "relative DJ_DATA_OVERRIDE changed charter bytes compared with the same absolute data directory"
  assert_grep ">> '$home/state/relative-data.status'" "$brief" \
    "relative DJ_DATA_OVERRIDE changed the absolute default status path"

  err="$root/unresolved.err"
  (
    cd "$root" || exit 1
    DJ_HOME=missing-home DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" unresolved-home --secondmate --no-projects >/dev/null 2>"$err"
  ); status=$?
  expect_code 1 "$status" "an unresolved relative DJ_HOME must fail"
  assert_grep "DJ_HOME directory cannot be resolved: missing-home" "$err" \
    "unresolved relative DJ_HOME did not fail loudly"

  (
    cd "$root" || exit 1
    DJ_HOME="$home" DJ_STATE_OVERRIDE=missing-state DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" unresolved-state --secondmate --no-projects >/dev/null 2>"$err"
  ); status=$?
  expect_code 1 "$status" "an unresolved relative DJ_STATE_OVERRIDE must fail"
  assert_grep "DJ_STATE_OVERRIDE directory cannot be resolved: missing-state" "$err" \
    "unresolved relative DJ_STATE_OVERRIDE did not fail loudly"

  (
    cd "$root" || exit 1
    DJ_HOME="$home" DJ_DATA_OVERRIDE=missing-data DJ_SECONDMATE_CHARTER=x \
      "$ROOT/bin/dj-brief.sh" unresolved-data --secondmate --no-projects >/dev/null 2>"$err"
  ); status=$?
  expect_code 1 "$status" "an unresolved relative DJ_DATA_OVERRIDE must fail"
  assert_grep "DJ_DATA_OVERRIDE directory cannot be resolved: missing-data" "$err" \
    "unresolved relative DJ_DATA_OVERRIDE did not fail loudly"

  pass "dj-brief.sh: relative directory inputs ignore CDPATH, render stable absolute charter paths, or fail loudly"
}
...
test_herdr_lab_contract_applies_to_scouts_but_not_secondmates() {
  ...
  mkdir -p "$home/data"
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" herdr-scout jarvis --scout --herdr-lab >/dev/null 2>&1
  brief="$home/data/herdr-scout/brief.md"
  assert_grep "# Herdr isolation - HARD SAFETY CONTRACT" "$brief" \
    "scout --herdr-lab brief missing the contract"

  DJ_HOME="$home" DJ_SECONDMATE_CHARTER=ops "$ROOT/bin/dj-brief.sh" herdr-secondmate --secondmate jarvis --herdr-lab >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "secondmate --herdr-lab must be rejected"
  assert_absent "$home/data/herdr-secondmate/brief.md" \
    "rejected secondmate --herdr-lab still wrote a brief"
  pass "dj-brief.sh: Herdr lab contract covers scouts and rejects secondmate misuse"
}
...
test_pause_verb_override_renders_all_brief_scaffolds() {
  ...
  for kind in ship scout secondmate; do
    ...
    case "$kind" in
      ship)
        DJ_HOME="$home" DJ_CLASSIFY_PAUSED_VERB=awaiting \
          "$ROOT/bin/dj-brief.sh" "$id" jarvis --mode no-mistakes >/dev/null 2>&1
        ;;
      scout)
        DJ_HOME="$home" DJ_CLASSIFY_PAUSED_VERB=awaiting \
          "$ROOT/bin/dj-brief.sh" "$id" jarvis --scout >/dev/null 2>&1
        ;;
      secondmate)
        DJ_HOME="$home" DJ_CLASSIFY_PAUSED_VERB=awaiting \
          "$ROOT/bin/dj-brief.sh" "$id" --secondmate --no-projects >/dev/null 2>&1
        ;;
    esac
    brief="$home/data/$id/brief.md"
  ...
  pass "dj-brief.sh: custom pause verb renders in every scaffold"
}
...
test_scout_and_secondmate_load_decision_hold_policy() {
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/dj-brief.sh" sample-investigation sample --scout >/dev/null 2>&1
  ...
  DJ_HOME="$home" DJ_ROOT_OVERRIDE="$ROOT" DJ_SECONDMATE_CHARTER='sample reviews' \
    "$ROOT/bin/dj-brief.sh" sample-mate --secondmate --no-projects >/dev/null 2>&1
  ...
  pass "dj-brief.sh: investigation and visual-review completions load the shared decision policy"
}

# Scout and secondmate paths still scaffold well-formed briefs.
test_scout_and_secondmate_scaffold() {
  local brief
  DJ_HOME="$BRIEF_HOME" "$ROOT/bin/dj-brief.sh" brief-scout-q6 alpha --scout >/dev/null 2>&1 \
    || fail "dj-brief.sh scout scaffold exited non-zero"
  ...
  DJ_SECONDMATE_CHARTER='Supervise the alpha domain.' \
    ...
    || fail "dj-brief.sh secondmate scaffold exited non-zero"
  brief="$BRIEF_HOME/data/brief-sm-q6/brief.md"
  assert_present "$brief" "secondmate charter was not scaffolded"
  assert_grep "persistent second mate" "$brief" \
    "secondmate charter must declare its role"
  pass "dj-brief: scout and secondmate code paths still scaffold well-formed briefs"
}
