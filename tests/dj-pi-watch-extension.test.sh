...
TMP_ROOT=$(dj_test_tmproot dj-pi-watch-extension)
EXT="$ROOT/.pi/extensions/dj-primary-pi-watch.ts"
...
install_pi_watch_extension_fixture() {
  ...
  cp "$EXT" "$repo/.pi/extensions/dj-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/lib/dj-branch-dispatch.ts" "$repo/.pi/extensions/lib/dj-branch-dispatch.ts"
  cp "$ROOT/.pi/extensions/lib/dj-calm-visibility.ts" "$repo/.pi/extensions/lib/dj-calm-visibility.ts"
  ...
  chmod +x "$repo/bin/dj-operational-input.sh"
...
test_pi_extension_reports_external_healthy_watcher() {
  ...
  plugin="$repo/.pi/extensions/dj-primary-pi-watch.ts"
  cat > "$repo/bin/dj-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
...
const pi = {
  ...
  registerCommand(name, options) {
    if (name === "dj-watch-arm-pi") handler = options.handler;
  },
  registerTool() {},
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
...
if (!prompt.startsWith("\u2063JARVIS_OP: v1 watcher: ")) {
  console.error(`untyped operational follow-up: ${prompt}`);
  process.exit(1);
}
if (!prompt.includes("JARVIS WATCHER WAKE")) {
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" node --input-type=module 2>&1 <<'EOF'
import { writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  registerTool(candidate) {
    if (candidate.name === "dj_watch_arm_pi") tool = candidate;
  },
...
if (tool.label !== "Arm jarvis watcher") throw new Error(`unexpected label: ${tool.label}`);
...
while [ ! -e "$DJ_STOP_FILE" ]; do sleep 0.02; done
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_STOP_FILE="$stop" node --input-type=module 2>&1 <<'EOF'
...
for (let i = 0; i < 100 && !existsSync(process.env.DJ_ARM_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (!existsSync(process.env.DJ_ARM_LOG)) throw new Error("initial arm child did not start");
await new Promise((resolve) => setTimeout(resolve, 100));
const rows = readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n");
if (rows.length !== 1) throw new Error(`redundant call spawned ${rows.length} arm children`);
writeFileSync(process.env.DJ_STOP_FILE, "stop\n");
EOF
)
  status=$?
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_WATCH_REARM_RETRY_BASE_MS=10000 DJ_WATCH_REARM_RETRY_MAX_MS=10000 node --input-type=module 2>&1 <<'EOF'
...
printf 'arm=%s predecessor=%s\n' "$$" "${DJ_WATCH_PREDECESSOR_ARM_PID:-none}" >> "${DJ_ARM_LOG:?}"
count=$(grep -c '^arm=' "$DJ_ARM_LOG")
if [ "$count" -eq 1 ]; then
  printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
...
const pi = {
  ...
  sendUserMessage: async () => {
    rowsAtDelivery = existsSync(process.env.DJ_ARM_LOG)
      ? readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n").filter((row) => row.startsWith("arm=")).length
      : 0;
...
for (let i = 0; i < 250; i += 1) {
  const rows = existsSync(process.env.DJ_ARM_LOG)
    ? readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n")
    : [];
...
await new Promise((resolve) => setTimeout(resolve, 100));
const stableRows = readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n");
...
async function runScenario(withAcceptor) {
  writeFileSync(process.env.DJ_ARM_LOG, "");
  const offers = [];
  let mainPrompt = "";
  let tool = null;
  ...
  if (withAcceptor) {
    bus.on("dj-branch-supervision:dispatch", (offer) => {
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
writeFileSync(`${process.env.DJ_HOME}/state/branch-offer.meta`, "project=/projects/approved\nwindow=dj-branch-offer\n");
writeFileSync(`${process.env.DJ_HOME}/state/.wake-queue`, "1\t1\tsignal\tbranch-offer.status\tsignal: branch-offer synthetic wake\n");
...
if (!declined.mainPrompt.includes("JARVIS WATCHER WAKE")) {
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
writeFileSync(`${process.env.DJ_HOME}/state/.wake-queue`, "1\t1\theartbeat\theartbeat\theartbeat\n");
const mod = await import(pathToFileURL(process.env.PLUGIN).href);
mod.default(pi);
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
writeFileSync(
  `${process.env.DJ_HOME}/state/.wake-queue`,
...
SH
  ...
  while IFS='|' read -r label reason; do
    ...
    out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_STOP_FILE="$stop" \
      DJ_TEST_REASON="$reason" node --input-type=module 2>&1 <<'EOF'
import { writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
writeFileSync(
  `${process.env.DJ_HOME}/state/.wake-queue`,
  "1\t1\tsignal\ttask-a.status\tsignal: task-a.status\n2\t2\tcheck\tmain-only\t" + process.env.DJ_TEST_REASON + "\n",
);
const mod = await import(pathToFileURL(process.env.PLUGIN).href);
mod.default(pi);
...
if (!prompt.includes(`JARVIS WATCHER WAKE: ${process.env.DJ_TEST_REASON}`)) {
...
#!/usr/bin/env bash
printf 'arm=%s\n' "$$" >> "${DJ_ARM_LOG:?}"
count=$(wc -l < "$DJ_ARM_LOG" | tr -d '[:space:]')
if [ "$count" -eq 1 ]; then
  printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
  printf 'heartbeat\n'
  exit 0
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (!prompt.includes("JARVIS WATCHER WAKE: heartbeat\n\nwatcher: FAILED")) {
...
if (!prompt.includes("JARVIS WATCHER WAKE")) throw new Error(`missing follow-up: ${prompt}`);
if (!prompt.includes("handling delivery confirmation was rejected")) {
  throw new Error(`failed handshake was swallowed: ${prompt}`);
}
if ((prompt.match(/JARVIS WATCHER WAKE/g) || []).length !== 1) {
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_PI_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  sendUserMessage: async (message) => {
    prompt += message;
    rowsAtPrompt = existsSync(process.env.DJ_ARM_LOG)
      ? readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n").length
      : 0;
  },
};
...
trap '' TERM INT
printf 'arm=%s\n' "$$" >> "${DJ_ARM_LOG:?}"
while [ ! -e "$DJ_RELEASE_FILE" ]; do sleep 0.1; done
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_RELEASE_FILE="$release" DJ_PI_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_ARM_RETIRE_TIMEOUT_MS=20 DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
writeFileSync(process.env.DJ_RELEASE_FILE, "release\n");
...
if [ "$count" -eq 2 ]; then
  trap 'printf "retired\\n" > "${DJ_UNRETIRED_RETIRE_FILE:?}"' TERM INT
  printf 'ready\n' > "${DJ_UNRETIRED_READY_FILE:?}"
  while [ ! -e "$DJ_RELEASE_FILE" ]; do sleep 0.02; done
  [ "$DJ_LATE_KIND" = actionable ] && printf 'signal: late wake\n'
  exit 0
fi
printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
SH
    chmod +x "$repo/bin/dj-watch-arm.sh"
    out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_UNRETIRED_READY_FILE="$ready" DJ_UNRETIRED_RETIRE_FILE="$retired" DJ_RELEASE_FILE="$release" DJ_STOP_FILE="$stop" DJ_LATE_KIND="$kind" DJ_PI_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_ARM_RETIRE_TIMEOUT_MS=20 DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
writeFileSync(process.env.DJ_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  if (rows().length >= 3 && (process.env.DJ_LATE_KIND !== "actionable" || prompts.some((message) => message.includes("late wake")))) break;
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (rows().length !== 3) throw new Error(`late close did not restore one successor: ${rows().join(" | ")}`);
if (process.env.DJ_LATE_KIND === "actionable") {
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_STOP_FILE="$stop" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" DJ_RELEASE_FILE="$release" node --input-type=module 2>&1 <<'EOF'
...
const lock = `${process.env.DJ_HOME}/state/.lock`;
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_ARM_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
const assertMissingLock = (result, label) => {
  ...
  if (!result.details.message.includes("bin/dj-session-start.sh") || !result.details.message.includes("re-arm")) {
    throw new Error(`${label} missing reclaim and re-arm guidance: ${result.details.message}`);
  }
  if (result.details.message.includes("held by another jarvis session")) {
...
try {
  ...
  if (liveOther.details.message !== "watcher: read-only - session lock is held by another jarvis session") {
...
if (existsSync(process.env.DJ_ARM_LOG)) throw new Error("watcher arm ran without lock ownership");
writeFileSync(lock, `${process.pid}\n`);
...
for (let i = 0; i < 250 && !existsSync(process.env.DJ_ARM_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.DJ_ARM_LOG)) throw new Error("owned lock did not run the watcher arm");
EOF
)
  status=$?
...
marker=$(mktemp "${DJ_MARKER_ROOT:?}/arm.XXXXXX") || exit 1
cleanup() { rm -f "$marker"; }
trap cleanup EXIT
trap 'exit 0' TERM INT
printf 'watcher: started pid=%s\n' "$$"
printf '%s\n' "$$" > "${DJ_CHILD_PID_FILE:?}"
printf '%s\n' "$marker" > "${DJ_CHILD_MARKER_FILE:?}"
printf 'arm pid=%s marker=%s\n' "$$" "$marker" >> "${DJ_ARM_LOG:?}"
while :; do sleep 0.2; done
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_CHILD_PID_FILE="$child_pid_file" DJ_CHILD_MARKER_FILE="$child_marker_file" DJ_MARKER_ROOT="$marker_root" DJ_ARM_LOG="$arm_log" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
function currentArm() {
  try {
    return {
      pid: readFileSync(process.env.DJ_CHILD_PID_FILE, "utf8").trim(),
      marker: readFileSync(process.env.DJ_CHILD_MARKER_FILE, "utf8").trim(),
    };
  } catch {
    return { pid: "", marker: "" };
  }
...
function liveArmPids() {
  if (!existsSync(process.env.DJ_ARM_LOG)) return [];
  return readFileSync(process.env.DJ_ARM_LOG, "utf8")
    .trim()
    .split(/\n/)
    .filter(Boolean)
    .map((line) => {
...
writeFileSync(process.env.DJ_ARM_LOG, `arm pid=${activeChild} marker=${startupMarker}\n`, { flag: "a" });
...
test_pi_process_exit_cleanup_stops_arm_child() {
  ...
  cat > "$repo/bin/dj-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
trap 'printf "%s\n" "$$" >> "$DJ_CLEANUP_LOG"; exit 0' TERM
printf '%s\n' "$$" > "$DJ_CHILD_PID_FILE"
while :; do sleep 1; done
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" DJ_HOME="$home" DJ_ROOT_OVERRIDE="$repo" DJ_CLEANUP_LOG="$cleanup_log" DJ_CHILD_PID_FILE="$pid_file" node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
for (let i = 0; i < 250 && !existsSync(process.env.DJ_CHILD_PID_FILE); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.DJ_CHILD_PID_FILE)) throw new Error("arm child did not start");
const firstChild = readFileSync(process.env.DJ_CHILD_PID_FILE, "utf8").trim();
await handlers.get("session_shutdown")?.({ type: "session_shutdown" }, {});
await handlers.get("session_start")?.({ type: "session_start" }, {});
await tool.execute("tool-call-replacement", {}, undefined, undefined, {});
for (let i = 0; i < 250; i += 1) {
  const currentChild = readFileSync(process.env.DJ_CHILD_PID_FILE, "utf8").trim();
  if (currentChild !== firstChild) break;
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (readFileSync(process.env.DJ_CHILD_PID_FILE, "utf8").trim() === firstChild) {
...
test_opencode_plugin_package_boundary_is_explicit_esm() {
  ...
  plugin="$fixture/plugins/dj-primary-watch-arm.js"
  ...
  cp "$ROOT/.opencode/plugins/dj-primary-watch-arm.js" "$plugin"
  cp "$ROOT/.opencode/plugins/lib/dj-operational-input.js" "$fixture/plugins/lib/dj-operational-input.js"
...
test_opencode_primary_watch_plugin_uses_effective_state_home() {
  ...
  plugin="$ROOT/.opencode/plugins/dj-primary-watch-arm.js"
  ...
  cat > "$repo/bin/dj-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'home=%s root=%s\n' "${DJ_HOME:-}" "${DJ_ROOT_OVERRIDE:-}" >> "${DJ_ARM_LOG:?}"
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" node 2>&1 <<'EOF'
...
if (!existsSync(process.env.DJ_ARM_LOG)) {
  console.error("watch arm did not run");
  process.exit(1);
}
const text = readFileSync(process.env.DJ_ARM_LOG, "utf8");
const expectedRoot = realpathSync(process.env.WORKTREE);
if (!text.includes(`home=${process.env.DJ_HOME}`) || !text.includes(`root=${expectedRoot}`)) {
  console.error(text);
  process.exit(1);
}
EOF
  ...
  status=$?
  expect_code 0 "$status" "OpenCode watch plugin must use DJ_HOME state outside the repo root"
  [ -z "$out" ] || fail "OpenCode effective-state test printed output: $out"
  pass "OpenCode watcher plugin uses the effective DJ_HOME state"
}
...
test_opencode_primary_watch_plugin_sources_effective_config() {
  ...
  git init -q "$repo"
  : > "$repo/AGENTS.md"
  printf 'export DJ_POLL=7\n' > "$home/config/x-mode.env"
  cat > "$repo/bin/dj-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'poll=%s\n' "${DJ_POLL:-missing}" >> "${DJ_ARM_LOG:?}"
printf 'watcher: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" node 2>&1 <<'EOF'
...
EOF
  ...
  status=$?
  expect_code 0 "$status" "OpenCode watch plugin must source DJ_HOME config outside the repo root"
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, "999999\n");
...
const refusal = await globalThis.__jarvisOpenCodeWatchArm.ensureArmed("session-test", client);
...
test_opencode_watch_arm_coordinator_respects_primary_scope() {
  ...
  dj_git_worktree "$base" "$repo" dj/opencode-coordinator
  mkdir -p "$repo/bin" "$home/state" "$home/config"
  : > "$repo/AGENTS.md"
  : > "$home/state/task.meta"
  cat > "$repo/bin/dj-watch-arm.sh" <<'SH'
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
const status = await globalThis.__jarvisOpenCodeWatchArm.ensureArmed("session-test", client);
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_STOP_FILE="$stop" node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if [ "$count" -eq 2 ]; then
  ...
  trap 'printf "retired\\n" > "${DJ_PRE_READY_RETIRED_FILE:?}"; exit 0' TERM INT
  while [ ! -e "$DJ_PRE_READY_RELEASE_FILE" ]; do sleep 0.02; done
  exit 0
fi
printf 'watcher: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_PRE_READY_RELEASE_FILE="$release" DJ_PRE_READY_RETIRED_FILE="$retired" DJ_STOP_FILE="$stop" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (existsSync(process.env.DJ_PRE_READY_RETIRED_FILE)) throw new Error("pre-ready actionable successor was retired before its close");
writeFileSync(process.env.DJ_PRE_READY_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  const successorRows = readFileSync(process.env.DJ_ARM_LOG, "utf8").trim().split("\n");
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_OPENCODE_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_RELEASE_FILE="$release" DJ_OPENCODE_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_ARM_RETIRE_TIMEOUT_MS=20 DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
    chmod +x "$repo/bin/dj-watch-arm.sh"
    out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_UNRETIRED_READY_FILE="$ready" DJ_UNRETIRED_RETIRE_FILE="$retired" DJ_RELEASE_FILE="$release" DJ_STOP_FILE="$stop" DJ_LATE_KIND="$kind" DJ_OPENCODE_ARM_READY_TIMEOUT_MS="$ARM_READY_TIMEOUT_MS" DJ_WATCH_ARM_RETIRE_TIMEOUT_MS=20 DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_STOP_FILE="$stop" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_WATCH_REARM_RETRY_BASE_MS=5 DJ_WATCH_REARM_RETRY_MAX_MS=10 DJ_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/dj-watch-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_RELEASE_FILE="$release" node 2>&1 <<'EOF'
import { spawn } from "node:child_process";
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
test_opencode_watch_arm_coordinates_with_turnend_guard() {
  ...
  arm_plugin="$ROOT/.opencode/plugins/dj-primary-watch-arm.js"
  guard_plugin="$ROOT/.opencode/plugins/dj-primary-turnend-guard.js"
...
exit 2
SH
  chmod +x "$repo/bin/dj-watch-arm.sh" "$repo/bin/dj-turnend-guard.sh"
  out=$(ARM_PLUGIN="$arm_plugin" GUARD_PLUGIN="$guard_plugin" WORKTREE="$repo" DJ_HOME="$home" DJ_ARM_LOG="$log" DJ_GUARD_LOG="$guard_log" node 2>&1 <<'EOF'
import { existsSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (existsSync(process.env.DJ_GUARD_LOG)) {
...
writeFileSync(`${process.env.DJ_HOME}/state/.lock`, `${process.pid}\n`);
await guardHooks.event({ event: { type: "session.idle", properties: { sessionID: "session-test" } } });
for (let i = 0; i < 250 && !existsSync(process.env.DJ_GUARD_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.DJ_ARM_LOG)) {
  console.error("watch arm did not run");
...
if (!readFileSync(process.env.DJ_ARM_LOG, "utf8").includes("args=--restart")) {
