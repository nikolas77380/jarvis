...
# dj-spawn.sh --secondmate launch, captures Pi's before_agent_start prompt bytes,
# and proves both sides of the routing boundary:
#   - exact task id through explicit DJ_HOME receives exactly one marker;
#   - direct terminal input remains unmarked.
#
# Every Herdr call, including calls made inside the production backend adapter,
# is routed through bin/dj-herdr-lab.sh. The PATH shim strips only the adapter's
...
if [ "${DJ_SEND_MARKER_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set DJ_SEND_MARKER_HERDR_E2E=1 to run the real Pi/Herdr secondmate-marker regression"
  exit 0
fi
...
SESSION=$("$LAB_HELPER" name dj-send-secondmate-marker-v7)
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/dj-send-marker-herdr-e2e.XXXXXX")
...
ID='marker-pi-sm'
REQUEST='DJ_MARKER_HERDR_E2E exact-id request'
DIRECT='DJ_MARKER_HERDR_DIRECT captain input'
...
printf '%s\n' "$ID" > "$SECOND_HOME/.dj-secondmate-home"
...
CAPTURE_EXTENSION="$TMP_ROOT/dj-send-marker-capture.ts"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" DJ_GATE_REFUSE_BYPASS=1 DJ_HOME="$SENDER_HOME" HERDR_SESSION="$SESSION" \
  "$ROOT/bin/dj-spawn.sh" "$ID" "$SECOND_HOME" --secondmate --harness pi --backend herdr >/dev/null

META="$SENDER_HOME/state/$ID.meta"
[ -f "$META" ] || fail "real secondmate spawn did not write exact-id metadata"
[ "$(dj_meta_get "$META" kind)" = secondmate ] || fail "real secondmate metadata did not record kind=secondmate"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" DJ_GATE_REFUSE_BYPASS=1 DJ_HOME="$SENDER_HOME" \
  "$ROOT/bin/dj-send.sh" "$ID" "$REQUEST" >/dev/null
wait_for_prompt "$REQUEST" || fail "real Pi did not receive the exact-id dj-send request"
GOT=$(jq -r --arg needle "$REQUEST" 'select(.prompt | contains($needle)) | .prompt' "$CAPTURE" | tail -1)
[ "$GOT" = "${DJ_FROMFIRST_MARK}${REQUEST}" ] \
...
pass "real Pi/Herdr: exact-id DJ_HOME send delivers exactly one from-jarvis marker"
wait_for_idle || fail "real Pi did not become idle after the exact-id capture"

# Direct terminal input bypasses dj-send's metadata-routed transformation and
...
if dj_message_from_jarvis "$GOT"; then
  fail "direct captain input was classified as from-jarvis"
fi
