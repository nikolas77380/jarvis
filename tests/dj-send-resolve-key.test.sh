#!/usr/bin/env bash
# dj-send answerer-closes (--resolve-key) behavior.
#
...
# dj-send's --resolve-key removes that writer-dependency at its source: the
# ANSWERING jarvis closes the decision in this home's own ledger at answer
# time - for a local target that is ENQUEUE time, because the durable inbox
# write is delivery to the task's record. These tests drive the real dj-send
# executable over stubbed transports and assert closure through the real
# consumer (dj-wake-drain.sh's OPEN DECISIONS section), never through source
# text:
...
TMP_ROOT=$(dj_test_tmproot dj-send-resolve-key)

# Stub tmux: logs literal typed text to DJ_SEND_LOG and lets the submit path
# reach a clean "empty" verdict (numeric cursor_y, empty bordered composer).
# DJ_FAKE_TMUX_SEND_FAIL=1 makes send-keys fail so the delivery-failure leg can
...
case "${1:-}" in
  send-keys)
    [ "${DJ_FAKE_TMUX_SEND_FAIL:-0}" = 1 ] && exit 1
    shift
    literal=0
    while [ $# -gt 0 ]; do
      case "$1" in
...
SH
  chmod +x "$fb/sleep"
  # Stub ssh transport for the remote-secondmate legs, selected via DJ_SSH_BIN.
  # Records the full remote invocation and exits DJ_FAKE_SSH_RC (default 0).
  cat > "$fb/fake-ssh" <<'SH'
#!/usr/bin/env bash
cat > /dev/null
printf '%s\n' "$*" >> "$DJ_SSH_LOG"
...
# run_send <fakebin> <home> <send-log> <dj-send args...>: run the real dj-send
...
test_answer_send_closes_open_decision() {
  ...
  dj_write_meta "$home/state/t1.meta" "window=sess:dj-t1" "kind=ship"
  ...
  assert_contains "$(cat "$log")" "Jarvis instruction waiting" "the doorbell should be rung for the answer"
  grep -F 'resolved [key=api-shape]: answered: go with REST' "$home/state/t1.status" >/dev/null \
    || fail "dj-send did not append the closing resolved line:"$'\n'"$(cat "$home/state/t1.status")"

  out=$(drain_out "$home")
  if printf '%s' "$out" | grep -F 'OPEN DECISIONS' >/dev/null; then
    fail "the answered decision still lists as open: $out"
  fi
  pass "dj-send --resolve-key: the answer send itself closes the open decision"
}
...
# gate the watcher's signal scan consumes (bin/dj-wake-lib.sh).
test_answer_close_is_self_announced() {
  ...
  dj_write_meta "$home/state/t9.meta" "window=sess:dj-t9" "kind=ship"
  printf 'needs-decision [key=port-choice]: 8080 or 9090\n' > "$home/state/t9.status"
  DJ_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"
    sig=$(dj_wake_signal_sig "$3") || exit 1
    printf "%s" "$sig" > "$(dj_wake_signal_seen_path "$2" "$3")"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$home/state" "$home/state/t9.status" \
  ...
  DJ_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"; dj_wake_signal_seen_current "$2" "$3"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$home/state" "$home/state/t9.status" \
    || fail "the answerer's own close was left to re-wake this same home"

  printf 'done: worker finished after the answer\n' >> "$home/state/t9.status"
  if DJ_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"; dj_wake_signal_seen_current "$2" "$3"
  ' _ "$ROOT/bin/dj-wake-lib.sh" "$home/state" "$home/state/t9.status"; then
    fail "a later worker line after the self-announced close was swallowed"
  fi
  pass "dj-send --resolve-key: the close never re-wakes its own home, later lines still do"
}
...
test_colon_first_key_position_is_answerable() {
  ...
  dj_write_meta "$home/state/t8.meta" "window=sess:dj-t8" "kind=ship"
  ...
  pass "dj-send --resolve-key: a colon-first stated key is open under that key and answerable"
}
...
test_answer_starts_work_never_orphans() {
  ...
  pass "dj-send --resolve-key: an answer that starts a workstream leaves no orphaned decision"
}
...
test_routine_steer_never_closes() {
  ...
  dj_write_meta "$home/state/t3.meta" "window=sess:dj-t3" "kind=ship"
  ...
  pass "dj-send: a send without --resolve-key never closes a decision, and working/done still cannot"
}
...
test_not_open_key_refuses_before_send() {
  ...
  dj_write_meta "$home/state/t4.meta" "window=sess:dj-t4" "kind=ship"
  ...
  pass "dj-send --resolve-key: a key that is not open refuses loudly before anything is sent"
}
...
test_failed_ring_still_closes_at_enqueue() {
  ...
  dj_write_meta "$home/state/t5.meta" "window=sess:dj-t5" "kind=ship"
  printf 'blocked [key=creds]: need the deploy token\n' > "$home/state/t5.status"

  : > "$log"
  env PATH="$fb:$PATH" DJ_FAKE_TMUX_SEND_FAIL=1 \
    DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" DJ_SEND_LOG="$log" DJ_SEND_SETTLE=0 \
  ...
  pass "dj-send --resolve-key: the close happens at enqueue, surviving a failed doorbell"
}
...
test_failed_enqueue_does_not_close() {
  ...
  pass "dj-send --resolve-key: a failed enqueue never closes the decision"
}
...
test_multiple_keys_close_together() {
  ...
  dj_write_meta "$home/state/t6.meta" "window=sess:dj-t6" "kind=ship"
  {
  ...
  pass "dj-send --resolve-key: one answer closes each named key and only those"
}
...
test_local_secondmate_answer_marked_and_closed() {
  ...
  fb=$(make_stubs "$dir"); log="$dir/send.log"
  home=$(setup_home sm)
  dj_write_secondmate_meta "$home/state/domain.meta" "$home" "sess:dj-domain"
  printf 'needs-decision [key=fleet-split]: shard by team or by repo\n' > "$home/state/domain.status"

  run_send "$fb" "$home" "$log" dj-domain --resolve-key fleet-split "shard by team"; rc=$?
  expect_code 0 "$rc" "a secondmate answer send should succeed"
  got=$(bash -c '. "$1"; dj_task_inbox_body "$2"' _ "$ROOT/bin/dj-task-inbox-lib.sh" \
    "$home/state/domain.inbox/001.msg")
  case "$got" in
    "$DJ_FROMFIRST_MARK"corr=*) : ;;
    *) fail "the secondmate answer's record lost its from-jarvis marker/corr framing: $got" ;;
  esac
  ...
  case "$closing" in
    *"$DJ_FROMFIRST_SEPARATOR"*) fail "the closing line leaked marker bytes" ;;
  esac
  ...
  pass "dj-send --resolve-key: a marked local-secondmate answer closes with the plain answer text"
}

# Remote secondmate: the answer crosses the (stubbed) ssh transport through the
# real dj-on.sh + registry route, while the close is the SAME local ledger
...
test_remote_secondmate_answer_closes_locally() {
  ...
  : > "$log"
  env PATH="$fb:$PATH" \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" DJ_SEND_LOG="$log" DJ_SEND_SETTLE=0 \
    DJ_SSH_BIN="$fb/fake-ssh" DJ_SSH_LOG="$ssh_log" DJ_FAKE_SSH_RC=0 \
  ...
  assert_grep 'dj-remote-entrypoint.sh' "$ssh_log" \
  ...
  pass "dj-send --resolve-key: a remote-secondmate answer closes the same local ledger, transport-only difference"
}
...
# though the key was right there on the line. This drives the real dj-send
...
test_remote_reply_corr_tag_does_not_block_resolve_key() {
  ...
  pass "dj-send --resolve-key: a remote reply's leading [corr=...] tag no longer blocks closing its stated key"
}
...
test_remote_transport_failure_does_not_close() {
  ...
  env PATH="$fb:$PATH" \
    DJ_ROOT_OVERRIDE="$ROOT" DJ_HOME="$home" DJ_SEND_LOG="$log" DJ_SEND_SETTLE=0 \
    DJ_SSH_BIN="$fb/fake-ssh" DJ_SSH_LOG="$ssh_log" DJ_FAKE_SSH_RC=1 \
  ...
  pass "dj-send --resolve-key: a failed remote transport never closes the decision"
}
...
test_flag_misuse_refuses() {
  ...
  dj_write_meta "$home/state/t7.meta" "window=sess:dj-t7" "kind=ship"
  printf 'needs-decision [key=k]: choose\n' > "$home/state/t7.status"

  # --resolve-key with --key (both orders) is refused: an answer is text.
  : > "$log"
  env PATH="$fb:$PATH" DJ_ROOT_OVERRIDE="$home" DJ_HOME="$home" DJ_SEND_LOG="$log" DJ_SEND_SETTLE=0 \
  ...
  pass "dj-send --resolve-key: --key, empty message, explicit targets, and malformed keys refuse loudly"
}
