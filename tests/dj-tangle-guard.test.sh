...
#
# Jarvis is a treehouse-pooled git repo of itself: linked worktrees and
# secondmate homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (DJ_ROOT) is a normal checkout on a real branch. The "tangle"
...
#            dj-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - dj-guard and dj-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the dj-guard banner,
# the dj-bootstrap problem line, the brief assertion ordering, and the dj-spawn
...
. "$ROOT/bin/dj-tangle-lib.sh"

TMP_ROOT=$(dj_test_tmproot dj-tangle-guard)
...
make_repo() {
  ...
  dj_git_add_origin "$dir" "$dir.origin.git"
  printf '%s\n' "$dir"
}

# --- shared lib: branch classification --------------------------------------

# dj_primary_tangle_branch is the whole scoping decision: a NAMED non-default
...
test_lib_classification() {
  ...
  while IFS='|' read -r label state branch expect; do
    ...
    out=$(dj_primary_tangle_branch "$repo" || true)
    [ "$out" = "$expect" ] || fail "$label: expected tangle='$expect', got '$out'"
  done <<'ROWS'
on the default branch is healthy|default||
on a feature branch is the tangle|feature|dj/readme-restructure-d3|dj/readme-restructure-d3
...
ROWS
  ...
  out=$(dj_primary_tangle_branch "$TMP_ROOT" || true)
  [ -z "$out" ] || fail "non-git dir wrongly reported a tangle: '$out'"
  pass "dj_primary_tangle_branch: feature branch alarms; default/detached/non-git stay silent"
}

# --- GUARD 2a: dj-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  DJ_ROOT_OVERRIDE="$1" DJ_HOME="$1" "$ROOT/bin/dj-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "dj-guard: bordered tangle banner fires only for a feature branch and suppresses repair commands in read-only mode"
}

# --- GUARD 2b: dj-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  DJ_ROOT_OVERRIDE="$1" DJ_HOME="$1" "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  git -C "$repo" checkout -q -B dj/tangle-bb2
  out=$(run_bootstrap "$repo" | grep '^TANGLE:' || true)
  assert_contains "$out" "dj/tangle-bb2" "bootstrap did not report the tangled branch"
  assert_contains "$out" "checkout main" "bootstrap TANGLE line lacked the restore remediation"
  out=$(DJ_ROOT_OVERRIDE="$repo" DJ_HOME="$repo" DJ_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/dj-bootstrap.sh" 2>/dev/null | grep '^TANGLE:' || true)
  assert_contains "$out" "dj/tangle-bb2" "detect-only bootstrap did not report the tangled branch"
  ...
  pass "dj-bootstrap: TANGLE problem line fires only for a feature branch and suppresses repair commands in detect-only mode"
}
...
test_brief_assertion_precedes_branch() {
  ...
  mkdir -p "$home/data"
  DJ_HOME="$home" "$ROOT/bin/dj-brief.sh" tangle-brief-cc3 alpha --mode no-mistakes >/dev/null 2>&1
  ...
  br=$(grep -n 'git checkout -b dj/' "$brief" | head -1 | cut -d: -f1)
  if [ -z "$iso" ] || [ -z "$br" ]; then
  ...
  pass "dj-brief: ship brief asserts worktree isolation before the branch step"
}

# --- GUARD 1b: dj-spawn isolation abort -------------------------------------

# A fake tmux that reports DJ_FAKE_PANE_PATH as the post-`treehouse get` pane cwd
...
run_spawn() {
  ...
  DJ_ROOT_OVERRIDE='' DJ_HOME="$home" \
    ...
    DJ_SPAWN_NO_GUARD=1 DJ_FAKE_PANE_PATH="$pane" TMUX="fake,1,0" \
    PATH="$fakebin:$PATH" \
    "$ROOT/bin/dj-spawn.sh" "$id" "$proj" codex --mode no-mistakes --yolo off 2>&1
}
...
test_spawn_isolation_abort() {
  ...
  pass "dj-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}

# --- GUARD 1c: dj-spawn tmux window construction ----------------------------

# The prevention guard also depends on dj-spawn building robust tmux commands
...
#     are disabled so the dj-<id> name survives treehouse cd'ing into the worktree;
...
#     display-message fall back to the active client's window and misread jarvis's
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "new-window -dP -F #{window_id} -t jarvis: -n dj-rec-win-gg7" "$rec" \
    "new-window must append at the session (trailing colon) and capture the window id"
  assert_no_grep "new-window -dP -F #{window_id} -t jarvis -n" "$rec" \
  ...
  pass "dj-spawn: appends windows by session-colon, pins the name, and targets the window id"
}
