...
if [ "${DJ_MUSE_SIGNALS_LIVE:-0}" != 1 ]; then
  echo "skip: set DJ_MUSE_SIGNALS_LIVE=1 to run the real Muse signal drift guard"
  exit 0
fi

[ -x "$MUSE_BIN" ] || fail "DJ_MUSE_SIGNALS_LIVE=1 but no real muse executable is installed on PATH"
[ -x "$REAL_TMUX" ] || fail "DJ_MUSE_SIGNALS_LIVE=1 but tmux is not installed"
command -v node >/dev/null 2>&1 || fail "node is required to inspect Muse's serialized session protocol"

LAB=$(mktemp -d "${TMPDIR:-/tmp}/dj-muse-signals.XXXXXX") || fail "could not create the isolated Muse lab"
trap cleanup EXIT
...
"$REAL_TMUX" -L "$SOCKET" new-window -d -t "$SESSION:" -n muse -c "$WORKSPACE" -- \
  ...
  "$MUSE_BIN" --provider echo --yolo "jarvis Muse signal drift guard" \
  || fail "could not launch Muse with the echo provider"

SESSION_LOG=
for _ in $(seq 1 150); do
  SESSION_LOG=$(dj_busy_muse_matching_logs "$LAB/data/muse/sessions" "$WORKSPACE" 2>/dev/null | head -1)
  [ -z "$SESSION_LOG" ] || break
  sleep 0.2
done
...
RUN_STATE=
for _ in $(seq 1 150); do
  RUN_STATE=$(dj_busy_muse_run_state "$SESSION_LOG" 2>/dev/null || true)
  [ "$RUN_STATE" = busy ] && break
  sleep 0.2
done
[ "$RUN_STATE" = busy ] || fail "dj_busy_muse_run_state never observed the real echo turn in flight"
...
[ "$RUN_STATE" = settled ] || fail "dj_busy_muse_run_state did not settle the real echo turn"
...
for _ in $(seq 1 100); do
  COMPOSER_STATE=$(dj_tmux_composer_state "$TARGET")
