...
# Usage: . bin/dj-procevent-lib.sh   (requires dj-pr-lib.sh and dj-wake-lib.sh)
#
# The runner lets jarvis learn that a registered long-polling source produced
...
# and handling - until `dj-procevent.sh handled` records it. It proves nothing
# about the source side of the handoff. In particular the currently published
# `edith-axi poll` destructively clears feedback before returning it, so a
# result lost between that clearing and this runner reading the process output
# is unrecoverable. A Jarvis wrapper cannot close that window, and marking a
...
dj_procevent_claim_root() {
  printf '%s\n' "${DJ_PROCEVENT_CLAIM_ROOT:-${XDG_STATE_HOME:-$HOME/.local/state}/jarvis/procevent-claims}"
}

dj_procevent_registry_dir() { printf '%s\n' "$1/procevent"; }
dj_procevent_inbox_dir()    { printf '%s\n' "$1/procevent-inbox"; }
...
dj_procevent_source_id_valid() {
  local id=${1-}
  dj_task_id_path_safe "$id" || return 1
  [ "${#id}" -le 64 ]
}

dj_procevent_adapter_valid() {
  local a=${1-}
  case "$a" in
    ''|*[!a-z0-9-]*) return 1 ;;
  esac
...
# dj_procevent_any_registered <state>
dj_procevent_any_registered() {
  local reg rec
  reg=$(dj_procevent_registry_dir "$1")
  [ -d "$reg" ] || return 1
...
dj_procevent_claim_path() {
  printf '%s/%s.claim\n' "$(dj_procevent_claim_root)" "$1"
}

dj_procevent_source_lock_path() {
  printf '%s/%s.lock\n' "$(dj_procevent_claim_root)" "$1"
}

dj_procevent_source_lock_acquire() {
  local id=$1 root
  dj_procevent_source_id_valid "$id" || return 1
  root=$(dj_procevent_claim_root)
  (umask 077; mkdir -p "$root") || return 1
  [ -d "$root" ] && [ ! -L "$root" ] || return 1
  dj_lock_acquire_wait "$(dj_procevent_source_lock_path "$id")"
}

dj_procevent_source_lock_release() {
  dj_lock_release "$(dj_procevent_source_lock_path "$1")"
}

dj_procevent_registration_publish_locked() {  # <state> <adapter> <source-id> <argv...>
  local state=$1 adapter=$2 id=$3 reg dest tmp arg
  shift 3
  dj_procevent_adapter_valid "$adapter" || return 1
  dj_procevent_source_id_valid "$id" || return 1
  [ "$#" -ge 1 ] || return 1
  for arg in "$@"; do
    case "$arg" in *$'\n'*) return 1 ;; esac
...
dj_procevent_claim_load_locked() {  # <source-id>
  local claim home pid token identity reg_dir reg_identity terminal extra
  claim=$(dj_procevent_claim_path "$1")
  ...
  DJ_PROCEVENT_CLAIM_HOME=$home
  DJ_PROCEVENT_CLAIM_PID=$pid
  DJ_PROCEVENT_CLAIM_TOKEN=$token
  DJ_PROCEVENT_CLAIM_IDENTITY=$identity
  DJ_PROCEVENT_CLAIM_REG_DIR=$reg_dir
  DJ_PROCEVENT_CLAIM_REG_IDENTITY=$reg_identity
  DJ_PROCEVENT_CLAIM_TERMINAL=$terminal
}

# dj_procevent_group_alive <pid>
...
dj_procevent_pid_state() {
  local pid=$1 expected=$2 actual
  if ! dj_pid_alive "$pid"; then
    dj_procevent_group_alive "$pid" && return 3
    return 1
  fi
  if actual=$(dj_pid_identity "$pid" 2>/dev/null); then
    [ "$actual" = "$expected" ] && return 0
    return 1
  fi
  dj_pid_alive "$pid" || { dj_procevent_group_alive "$pid" && return 3; return 1; }
  return 2
}
...
dj_procevent_claim_state_locked() {
  local claim registration current_identity
  claim=$(dj_procevent_claim_path "$1")
  [ -e "$claim" ] || return 1
  dj_procevent_claim_load_locked "$1" || return 2
  if [ "$DJ_PROCEVENT_CLAIM_TERMINAL" = terminal ] && [ -n "$DJ_PROCEVENT_CLAIM_REG_IDENTITY" ]; then
    registration="$DJ_PROCEVENT_CLAIM_REG_DIR/$1.source"
    current_identity=$(dj_pr_file_identity "$registration" 2>/dev/null || true)
    [ "$current_identity" = "$DJ_PROCEVENT_CLAIM_REG_IDENTITY" ] && return 4
  fi
  dj_procevent_pid_state "$DJ_PROCEVENT_CLAIM_PID" "$DJ_PROCEVENT_CLAIM_IDENTITY"
}

# dj_procevent_claim_acquire_locked <source-id> <home> <pid> <registration>
# 0 acquired, 1 error, 2 held by a live owner (possibly another home).
dj_procevent_claim_acquire_locked() {
  ...
  case "$reg_dir" in /*) ;; *) return 1 ;; esac
  reg_identity=$(dj_pr_file_identity "$registration" 2>/dev/null) || return 1
  identity=$(dj_pid_identity "$pid" 2>/dev/null) || return 1
  root=$(dj_procevent_claim_root)
  claim=$(dj_procevent_claim_path "$id")
  status=0
    ...
    case "$claim_state" in
      ...
      1)
        if [ -f "$claim" ] && [ ! -L "$claim" ]; then
          old_home=$DJ_PROCEVENT_CLAIM_HOME
          old_token=$DJ_PROCEVENT_CLAIM_TOKEN
          old_reg_dir=$DJ_PROCEVENT_CLAIM_REG_DIR
...
dj_procevent_claim_mark_terminal_locked() {
  local id=$1 home=$2 pid=$3 token=$4 claim root tmp
  claim=$(dj_procevent_claim_path "$id")
  dj_procevent_claim_load_locked "$id" \
    && [ "$DJ_PROCEVENT_CLAIM_HOME" = "$home" ] \
    && [ "$DJ_PROCEVENT_CLAIM_PID" = "$pid" ] \
    && [ "$DJ_PROCEVENT_CLAIM_TOKEN" = "$token" ] \
    && [ -n "$DJ_PROCEVENT_CLAIM_REG_IDENTITY" ] || return 1
  root=$(dj_procevent_claim_root)
  tmp=$(umask 077; mktemp "$root/.claim.XXXXXX") || return 1
  if printf '%s\n%s\n%s\n%s\n%s\n%s\nterminal\n' \
    "$DJ_PROCEVENT_CLAIM_HOME" "$DJ_PROCEVENT_CLAIM_PID" "$DJ_PROCEVENT_CLAIM_TOKEN" \
    "$DJ_PROCEVENT_CLAIM_IDENTITY" "$DJ_PROCEVENT_CLAIM_REG_DIR" \
    "$DJ_PROCEVENT_CLAIM_REG_IDENTITY" > "$tmp" \
    && chmod 0600 "$tmp" \
    && mv -f -- "$tmp" "$claim"; then
    return 0
  else
...
# dj_procevent_claim_release_locked <source-id> <home> <pid> <token>
dj_procevent_claim_release_locked() {
  local id=$1 home=$2 pid=$3 token=$4 claim
  dj_procevent_source_id_valid "$id" || return 1
  claim=$(dj_procevent_claim_path "$id")
  [ -e "$claim" ] || return 0
  if dj_procevent_claim_load_locked "$id" \
    && [ "$DJ_PROCEVENT_CLAIM_HOME" = "$home" ] \
    && [ "$DJ_PROCEVENT_CLAIM_PID" = "$pid" ] \
    && [ "$DJ_PROCEVENT_CLAIM_TOKEN" = "$token" ]; then
    rm -f -- "$claim"
    return $?
  fi
  return 1
...
# dj_procevent_capture <state> <source-id> <adapter> <output-file>
...
# dj_procevent_event_line <adapter> <source-id> <sequence>
...
# dj_procevent_handled_marker <state> <source-id> <sequence>
dj_procevent_handled_marker() {
  printf '%s/%s.%s.handled\n' "$(dj_procevent_inbox_dir "$1")" "$2" "$3"
}

# dj_procevent_is_handled <state> <source-id> <sequence>
dj_procevent_is_handled() {
  local marker; marker=$(dj_procevent_handled_marker "$1" "$2" "$3")
  [ -f "$marker" ] && [ ! -L "$marker" ]
}

# dj_procevent_mark_handled <state> <source-id> <sequence>
...
dj_procevent_mark_handled() {
  local state=$1 id=$2 seq=$3 inbox result adapter_file marker tmp
  dj_procevent_source_id_valid "$id" || return 2
  ...
  marker=$(dj_procevent_handled_marker "$state" "$id" "$seq")
  [ ! -L "$marker" ] || return 2
...
# dj_procevent_result_source_id <result-path>
dj_procevent_result_source_id() {
  local base=${1##*/}
  base=${base%.result}
  printf '%s\n' "${base%.*}"
}

dj_procevent_result_sequence() {
  local base=${1##*/}
  base=${base%.result}
  printf '%s\n' "${base##*.}"
}

dj_procevent_result_adapter() {
