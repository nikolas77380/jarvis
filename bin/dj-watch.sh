...
#                          jarvis hands it to a no-mistakes validation. A declared
...
#                          count in the reason; at DJ_WEDGE_DEMAND_INSPECT_COUNT
...
#   stale: <window> (unread jarvis instruction: ...)
...
#                          successful attempts never wake jarvis
#                          (bin/dj-task-inbox-lib.sh owns the ladder policy)
...
#                          DJ_SECONDMATE_WAKE_STALL_SECS; observation is read-only
...
. "$SCRIPT_DIR/dj-push-transition-lib.sh"
...
# expectations created by dj-send on marked secondmate requests. The tick is
...
# Steering-inbox loss detection: bin/dj-task-inbox-lib.sh owns the record,
...
WATCH_PATH="$SCRIPT_DIR/dj-watch.sh"
WATCHER_DOWNTIME_MARKER="$STATE/.watcher-down"
WATCHER_STALE_GRACE=${DJ_WATCHER_STALE_GRACE:-${DJ_GUARD_GRACE:-300}}
...
# bin/dj-wake-lib.sh (dj_wake_signal_sig, dj_wake_signal_seen_path), shared
# with the drain's annotation staleness check and this home's own bookkeeping
# writers' guarded self-announced append.

POLL=${DJ_POLL:-15}                   # seconds between cycles
HEARTBEAT=${DJ_HEARTBEAT:-600}        # base seconds between heartbeat scans
HEARTBEAT_MAX=${DJ_HEARTBEAT_MAX:-7200}  # heartbeat backoff cap
CHECK_INTERVAL=${DJ_CHECK_INTERVAL:-300}  # seconds between *.check.sh sweeps
CHECK_TIMEOUT=${DJ_CHECK_TIMEOUT:-30}     # seconds allowed per *.check.sh
SIGNAL_GRACE=${DJ_SIGNAL_GRACE:-30}   # seconds to linger after a signal so trailing
                                      # signals (a status write, then the same turn's
                                      # turn-end hook) coalesce into one wake
# Busy state is decided by the semantic contract in bin/dj-busy-lib.sh, which
...
# than wake jarvis's LLM for each, this watcher classifies every wake in bash
...
# dj-crew-state.sh); a crew that stopped its turn with no running pipeline and no
...
# (dj-classify-lib.sh) backs the away-mode daemon; while state/.afk exists the
# daemon owns triage, so this watcher reverts to one-shot (enqueue + exit on every
# wake) and never double-triages - and never runs the costly provably-working read.
STALE_ESCALATE_SECS=${DJ_STALE_ESCALATE_SECS:-240}  # idle secs before a provably-working stale escalates as a possible wedge
...
BUSY_TURN_MAX_SECS=${DJ_BUSY_TURN_MAX_SECS:-3600}
...
SECONDMATE_WAKE_STALL_SECS=${DJ_SECONDMATE_WAKE_STALL_SECS:-60}
...
PAUSE_RESURFACE_SECS=${DJ_PAUSE_RESURFACE_SECS:-$DJ_PAUSE_RESURFACE_SECS_DEFAULT}
# Consecutive event-path failures (dj_backend_wait_transition returning 2 -
...
EVENT_CAP_FAIL_MAX=${DJ_EVENT_CAP_FAIL_MAX:-3}
# Per-process memo for the push-capability probe (dj_backend_events_capable runs
...
# the semantic busy-state contract (bin/dj-busy-lib.sh). Only an exact busy
...
window_is_busy() {  # <window> <tail40>
  ...
  if [ -n "$task" ] && [ -f "$meta" ]; then
    verdict=$(dj_busy_classify_meta "$meta" "$task" "$STATE" "$tail40")
  else
    verdict=$(dj_busy_classify "$(window_backend "$w")" "$w" "$(window_harness "$w")" \
...
window_kind() {
  local w=$1 meta kind
  meta=$(dj_backend_meta_for_window "$w" "$STATE" 2>/dev/null || true)
  if [ -n "$meta" ]; then
...
# glob and produces nothing. When the ladder (dj_task_inbox_due_action, the
...
inbox_steer_check() {  # <window> <task>
  ...
  action=$(dj_task_inbox_due_action "$STATE" "$task") || return 0
  verb=${action%% *}
  [ "$verb" != quiet ] || return 0
  rec=${action#* }
  count=
  ...
  tail40=$(dj_backend_capture "$(window_backend "$w")" "$w" 40 "$(window_label "$w")" 2>/dev/null) || tail40=
  ...
  case "$verb" in
    ring)
      ring_rc=0
      dj_task_inbox_ring "$(window_backend "$w")" "$w" "$rec" "$(window_label "$w")" || ring_rc=$?
      if ! dj_task_inbox_record_ring "$STATE" "$task" "$rec"; then
        if [ ! -f "$rec" ]; then
          dj_task_inbox_due_action "$STATE" "$task" >/dev/null || true
          return 0
        fi
        if [ -d "${rec%/*}" ]; then
          ...
          dj_wake_append stale "$w" "$reason" || exit 1
          wake "$reason"
        fi
      fi
      ...
      ;;
    escalate)
      reason="stale: $w (unread jarvis instruction: $rec still unhandled after $count doorbell delivery attempts with an idle pane; inspect the worker)"
      if [ ! -d "${rec%/*}" ] || [ ! -f "$rec" ]; then
        dj_task_inbox_due_action "$STATE" "$task" >/dev/null || true
        return 0
      fi
      dj_wake_append stale "$w" "$reason" || exit 1
      if ! dj_task_inbox_record_escalated "$STATE" "$task" "$rec"; then
...
secondmate_wake_stall_tick() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    home=$(dj_meta_get "$meta" home)
    [ -n "$home" ] || continue
    [ -f "$home/.dj-secondmate-home" ] && [ ! -L "$home/.dj-secondmate-home" ] || continue
    [ "$(cat "$home/.dj-secondmate-home" 2>/dev/null || true)" = "$task" ] || continue
...
EOF
    ...
    queued=$(dj_wake_queued_keys check)
    if ! printf '%s\n' "$queued" | grep -Fx "$notify_key" >/dev/null 2>&1; then
      dj_wake_append check "$notify_key" "$reason" || return 1
    fi
    dj_wake_secondmate_stall_receipt_write "$task" "$row_key" || return 1
    dj_wake_secondmate_stall_marker_write "$task" "$row_key" || return 1
    wake "$reason"
  done
  return 0
}

# Consecutive wedge-escalation count for a window past DJ_WEDGE_DEMAND_INSPECT_COUNT
...
# below).
DJ_WEDGE_DEMAND_INSPECT_COUNT=${DJ_WEDGE_DEMAND_INSPECT_COUNT:-3}
...
resurface_absorbed() {  # <window> <throttle-marker> <age> <reason>
  ...
  dj_wake_append stale "$win" "$reason" || exit 1
  date +%s > "$throttle"
  wake "$reason"
}
...
# dj-classify-lib.sh). The pane and the run step both say nothing is happening;
  ...
  case "$since" in
    ...
    *)
      ...
      if [ "$age" -ge "$STALE_ESCALATE_SECS" ]; then
        ...
        if [ "$n" -ge "$DJ_WEDGE_DEMAND_INSPECT_COUNT" ]; then
...
# After dj-crew-state has fallen back to stopped or unknown, paused classification is
...
pause_state_class() {  # <window> <task>
  ...
  if [ -e "$STATE/.paused-$key" ] && [ "$(age_of "$recheck_file")" -lt "$STALE_ESCALATE_SECS" ]; then
    if [ "$kind" != secondmate ]; then
      agent_alive=$(dj_backend_agent_alive "$(window_backend "$win")" "$win" 2>/dev/null) || agent_alive=unknown
...
surface_nonterminal_stale() {  # <window> <hash>
  ...
  key=$(window_key "$win")
  dj_wake_append stale "$win" "stale: $win" || exit 1
  printf '%s' "$h" > "$STATE/.stale-$key"
...
scan_signals() {
  ...
  for f in "$STATE"/*.status "$STATE"/*.turn-ended; do
    [ -e "$f" ] || continue
    sig=$(dj_wake_signal_sig "$f") || continue
    [ -n "$sig" ] || continue
    sf=$(dj_wake_signal_seen_path "$STATE" "$f")
...
# Deliver a durably queued process-event result to jarvis. Publication is
# owned by bin/dj-procevent.sh - by the runner at capture time and by reconcile's
...
# happens to wake jarvis, which is exactly the missed delivery this repairs.
...
procevent_surface_queued() {
  ...
  done < <(dj_wake_queued_keys_locked check)
  ...
  DJ_WAKE_POST_OUTPUT_ACTION=procevent_surface_after_output
  wake "$reason"
}
...
run_check_process() {
  local c=$1
  shift
  if [ "${DJ_CHECK_FORCE_FALLBACK:-0}" != 1 ] && command -v timeout >/dev/null 2>&1; then
    exec timeout "$CHECK_TIMEOUT" bash "$c" "$@"
  elif [ "${DJ_CHECK_FORCE_FALLBACK:-0}" != 1 ] && command -v gtimeout >/dev/null 2>&1; then
    exec gtimeout "$CHECK_TIMEOUT" bash "$c" "$@"
  else
    # shellcheck disable=SC2016  # single quotes are deliberate: Perl expands its own variables.
    exec perl -e 'my $t = shift; my $owned = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0) unless $owned; exec @ARGV } my $group = $owned ? getpgrp(0) : $pid; my $stop = sub { $SIG{HUP} = $SIG{INT} = $SIG{TERM} = "IGNORE"; kill "TERM", -$group; select undef, undef, undef, 0.2; kill "KILL", -$group; waitpid $pid, 0; exit 124 }; local $SIG{ALRM} = $stop; local $SIG{HUP} = $stop; local $SIG{INT} = $stop; local $SIG{TERM} = $stop; alarm $t; waitpid $pid, 0; exit($? >> 8)' "$CHECK_TIMEOUT" "${DJ_CHECK_OWNED_GROUP:-0}" bash "$c" "$@"
  fi
}
...
DJ_ACTIVE_CHECK_PID=
DJ_ACTIVE_CHECK_PGID=
DJ_CHECK_OUTPUT=
DJ_CHECK_RESULT=
DJ_CHECK_SIGNAL_PENDING=

dj_check_output_cleanup() {
  [ -z "$DJ_CHECK_OUTPUT" ] || rm -f -- "$DJ_CHECK_OUTPUT"
  DJ_CHECK_OUTPUT=
}

dj_active_check_stop() {
  local pid=${DJ_ACTIVE_CHECK_PID:-} pgid=${DJ_ACTIVE_CHECK_PGID:-} i
...
run_check_capture() {
  ...
  dj_check_output_cleanup
  DJ_CHECK_RESULT=
  DJ_CHECK_OUTPUT=$(mktemp "$STATE/.dj-check-output.XXXXXX") || return 1
  chmod 0600 "$DJ_CHECK_OUTPUT" || { dj_check_output_cleanup; return 1; }
  DJ_CHECK_SIGNAL_PENDING=
  trap 'DJ_CHECK_SIGNAL_PENDING=1' HUP INT TERM
  set -m
  ( DJ_CHECK_OWNED_GROUP=1 run_check_process "$@" ) > "$DJ_CHECK_OUTPUT" 2>/dev/null &
  DJ_ACTIVE_CHECK_PID=$!
  DJ_ACTIVE_CHECK_PGID=$DJ_ACTIVE_CHECK_PID
  set +m
  pgid=$(ps -o pgid= -p "$DJ_ACTIVE_CHECK_PID" 2>/dev/null | tr -d '[:space:]')
  trap 'exit 1' HUP INT TERM
  if [ -n "$pgid" ] && [ "$pgid" != "$DJ_ACTIVE_CHECK_PGID" ]; then
    dj_active_check_stop || true
    dj_check_output_cleanup
    return 1
  fi
  [ -z "$DJ_CHECK_SIGNAL_PENDING" ] || exit 1
  wait "$DJ_ACTIVE_CHECK_PID" 2>/dev/null || true
  DJ_ACTIVE_CHECK_PID=
  dj_active_check_stop || return 1
  DJ_CHECK_RESULT=$(cat "$DJ_CHECK_OUTPUT" 2>/dev/null || true)
  dj_check_output_cleanup
}

# Surfaced-marker bookkeeping for the heartbeat backstop is owned by
# dj-push-transition-lib.sh because push and poll paths must write one format.
...
# any captain-relevant status has NOT already been surfaced to jarvis (its
...
# jarvis, this normally finds nothing and the heartbeat is absorbed; it
...
event_wait_or_sleep() {
  ...
  while IFS= read -r w; do
    b=$(window_backend "$w")
    dj_backend_has_push "$b" || continue
  ...
  # Memoized capability probe (dj_backend_events_capable runs a heavy schema
  ...
  if [ "$_event_cap_key" != "$first_backend:$first_session" ]; then
    ...
    if dj_backend_events_capable "$first_backend" "$first_session"; then
      _event_cap_ok=1
    else
      _event_cap_ok=0
    fi
  ...
  rec=$(DJ_BACKEND_EVENTS_CAPABILITY_CONFIRMED=1 dj_backend_wait_transition "$first_backend" "$first_session" "$POLL" "$STATE" "${windows[@]}")
  rc=$?
  case "$rc" in
    0)
      _event_cap_fails=0
...
"$SCRIPT_DIR/dj-pr-check-migrate.sh" --checks-safe || {
  echo "watcher: PR check migration blocked; refusing to execute state checks" >&2
  exit 1
}

if ! dj_lock_try_acquire "$WATCH_LOCK"; then
  BEAT="$STATE/.last-watcher-beat"
  if [ -n "${DJ_LOCK_HELD_PID:-}" ]; then
    if [ -e "$BEAT" ]; then
      beat_age=$(dj_path_age "$BEAT")
      if [ "$beat_age" -ge "$WATCHER_STALE_GRACE" ]; then
        echo "watcher: lock held by live pid $DJ_LOCK_HELD_PID but heartbeat is stale for ${beat_age}s (>${WATCHER_STALE_GRACE}s); inspect or stop that watcher before re-arming." >&2
        exit 1
      fi
    elif [ "$(dj_path_age "$WATCH_LOCK")" -ge "$WATCHER_STALE_GRACE" ]; then
      echo "watcher: lock held by live pid $DJ_LOCK_HELD_PID but no heartbeat exists; inspect or stop that watcher before re-arming." >&2
      exit 1
    fi
    echo "watcher: already running pid $DJ_LOCK_HELD_PID"
  else
...
if [ -n "${DJ_LOCK_RECOVERED_PID:-}" ]; then
  WATCHER_RECOVERY_PENDING=1
fi
if [ "${DJ_WATCH_HANDLING_SUCCESSOR:-0}" != 1 ]; then
  if ! dj_recovery_marker_reopen_announced "$WATCHER_DOWNTIME_MARKER"; then
    echo "watcher: recovery state could not be reopened safely; retaining stale lock evidence" >&2
    exit 1
  fi
fi
if ! dj_recovery_marker_arm_check "$WATCHER_DOWNTIME_MARKER"; then
  echo "watcher: recovery state could not be consumed safely; retaining stale lock evidence" >&2
  exit 1
fi
if [ "${DJ_WATCH_HANDLING_SUCCESSOR:-0}" = 1 ]; then
  WATCHER_RECOVERY_PENDING=0
elif [ "$DJ_RECOVERY_MARKER_ACTION" = recover ]; then
  WATCHER_RECOVERY_PENDING=1
fi
watcher_cleanup() {
  ...
  if [ "$(cat "$WATCH_LOCK/pid" 2>/dev/null || true)" = "${WATCHER_PID:-}" ]; then
    ...
    if [ "${WATCHER_RECOVERY_PENDING:-0}" -eq 1 ] \
      && [ "${DJ_WATCH_DELIVERED_REASON:-}" = "check: rearm-resurface" ]; then
      transition=release-lock-existing
    fi
  fi
  dj_active_check_stop || cleanup_status=1
  dj_check_output_cleanup
  dj_custom_check_snapshot_cleanup
  if [ "$owns_lock" -eq 1 ] \
    && ! dj_recovery_transition "$WATCHER_DOWNTIME_MARKER" "$transition" "$WATCH_LOCK" downtime; then
...
trap 'exit 1' HUP INT TERM
# This watcher's own pid, as recorded in the lock by dj_lock_claim (which writes
# ${BASHPID:-$$} from this same main shell). Read directly, never via a command
# substitution, so it matches the stored holder pid for the self-eviction check.
WATCHER_PID=${BASHPID:-$$}
printf '%s\n' "$DJ_HOME" > "$WATCH_LOCK/dj-home" || true
printf '%s\n' "$WATCH_PATH" > "$WATCH_LOCK/watcher-path" || true
# shellcheck disable=SC2034 # Consumed by wake() in the separately linted transition owner.
DJ_WATCH_DELIVERY_PID=$WATCHER_PID
DJ_WATCH_DELIVERY_IDENTITY=$(dj_pid_identity "$WATCHER_PID" 2>/dev/null || true)
printf '%s\n' "$DJ_WATCH_DELIVERY_IDENTITY" > "$WATCH_LOCK/pid-identity" 2>/dev/null || true
...
if ! dj_pr_poll_retirement_recover_all "$STATE" "$SCRIPT_DIR/dj-pr-poll.sh"; then
  reason="check: rejected unauthenticated PR poll retirement receipts:$DJ_PR_POLL_RETIREMENT_REJECTED"
  dj_wake_append check pr-poll-retirement "$reason" || exit 1
  touch "$STATE/.last-check"
  wake "$reason"
fi

# Shared by both the first-notification and already-notified paths below so
# the retirement sequence (bin/dj-pr-lib.sh) is stated once.
retire_merged_pr_poll() {  # <id>
  local id=$1
  if dj_pr_poll_retirement_publish "$STATE" "$id" "$SCRIPT_DIR/dj-pr-poll.sh" merged; then
    dj_pr_poll_retirement_recover_one "$STATE" "$id" "$SCRIPT_DIR/dj-pr-poll.sh" \
...
resurface_after_downtime() {
  ...
  if [ "$WATCHER_RECOVERY_PENDING" -ne 1 ]; then
    ...
    [ "$DJ_RECOVERY_MARKER_ACTION" = recover ] || return 0
  fi
...
while :; do
  ...
  # Liveness beacon for dj-guard.sh: a fresh mtime here means a watcher is
  ...
  dj_pending_reply_tick "$STATE" || true
  ...
  if [ -d "$STATE/procevent" ]; then
    DJ_HOME="$DJ_HOME" "$SCRIPT_DIR/dj-procevent.sh" reconcile >/dev/null 2>&1 || true
  fi
  ...
  # was created, so quiet cycles never wake jarvis or consume model tokens.
  inactive_out=
  if inactive_out=$(DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/dj-inactive-reconcile.sh" scan 2>/dev/null); then
  ...
  # Slow per-task checks (jarvis writes these, e.g. a merged-PR poll).
  ...
  if [ "$(age_of "$STATE/.last-check")" -ge "$CHECK_INTERVAL" ]; then
    ...
    for c in "$STATE"/*.check.sh; do
      ...
      if [ "$(basename "$c")" = x-watch.check.sh ]; then
        if fmx_poll_shim_valid "$c" "$DJ_HOME" "$DJ_ROOT" \
          && [ -f "$DJ_ROOT/bin/dj-x-poll.sh" ] && [ ! -L "$DJ_ROOT/bin/dj-x-poll.sh" ]; then
          DJ_HOME="$DJ_HOME" run_check_capture "$DJ_ROOT/bin/dj-x-poll.sh" || exit 1
          out=$DJ_CHECK_RESULT
        else
        ...
        if dj_pr_poll_snapshot_capture "$STATE" "$id" "$SCRIPT_DIR/dj-pr-poll.sh"; then
          is_pr_poll=1
          provider=$DJ_PR_POLL_SNAPSHOT_PROVIDER
          url=$DJ_PR_POLL_SNAPSHOT_URL
          host=$DJ_PR_POLL_SNAPSHOT_HOST
          path=$DJ_PR_POLL_SNAPSHOT_PATH
          number=$DJ_PR_POLL_SNAPSHOT_NUMBER
          run_check_capture "$SCRIPT_DIR/dj-pr-poll.sh" --validated \
            "$provider" "$url" "$host" "$path" "$number" || exit 1
          out=$DJ_CHECK_RESULT
        elif dj_custom_check_snapshot_prepare "$STATE" "$id"; then
          custom_snapshot=$DJ_CUSTOM_CHECK_SNAPSHOT
      ...
      if [ -n "$out" ]; then
        ...
        if [ "$is_pr_poll" -eq 1 ] && [ "$out" = merged ] \
          && dj_pr_poll_merge_already_notified "$STATE" "$id" \
            "$provider" "$host" "$path" "$number"; then
          # This exact merge was already surfaced to main once for this task
          # (dj_pr_poll_merge_mark_notified below records that at first
          # notification, and it survives a later re-registered poll for the
          # same, already-merged task - bin/dj-pr-lib.sh owns why). A repeat
        ...
        dj_wake_append check "$c" "$reason" || exit 1
        if [ "$is_pr_poll" -eq 1 ] && [ "$out" = merged ]; then
          dj_pr_poll_merge_mark_notified "$STATE" "$id" \
    ...
    if [ -n "$rejected_checks" ]; then
      ...
      dj_wake_append check unauthenticated-state-checks "$reason" || exit 1
      touch "$STATE/.last-check"
      wake "$reason"
    fi
    touch "$STATE/.last-check"
  ...
  # costs a full jarvis turn each. The re-scan also picks up a newer
...
EOF
    ...
    if afk_present || signal_reason_is_actionable $files || ! signal_crew_provably_working $files; then
      while IFS=$(printf '\t') read -r sf sig f; do
        [ -n "$sf" ] || continue
        dj_wake_append signal "$(basename "$f")" "$reason" || exit 1
      done <<EOF
$pending
EOF
      while IFS=$(printf '\t') read -r sf sig f; do
...
EOF
  ...
  while IFS= read -r w; do
    ...
    tail40=$(dj_backend_capture "$(window_backend "$w")" "$w" 40 "$(window_label "$w")" 2>/dev/null) || continue
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        # jarvis. Detection itself is unchanged from above.
        if [ "$kind" = secondmate ]; then
          ...
          if [ "$(cat "$sf" 2>/dev/null || true)" != "$h" ]; then
            dj_wake_append stale "$w" "stale: $w" || exit 1
            printf '%s' "$h" > "$sf"
            wake "stale: $w"
          fi
          ...
          # new entry once jarvis hands it to a no-mistakes validation
          ...
          # authoritative source dj-crew-state.sh itself already prioritizes
          ...
          #     Surface immediately so jarvis inspects the inconclusive state
  ...
  if [ "$(age_of "$STATE/.last-heartbeat")" -ge "$hb" ]; then
    ...
    # every heartbeat.
    if afk_present; then
      dj_wake_append heartbeat heartbeat heartbeat || exit 1
