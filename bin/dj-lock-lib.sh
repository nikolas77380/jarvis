...
#
# ONE owner for the staleness proof that dj-teardown.sh (a worktree index.lock)
# and dj-fleet-sync.sh (a clone's .git/packed-refs.lock) both rely on: a lock is
...
# Diagnostics print to stderr prefixed by ${DJ_LOCK_LOG_PREFIX:-dj-lock} so each
# caller's output stays recognizable.

dj_lock_log() {
  echo "${DJ_LOCK_LOG_PREFIX:-dj-lock}: $*" >&2
}
...
# dj_lock_lsof_holder <target>: 0 a process holds it, 1 provably none, 2 lsof
...
dj_lock_lsof_holder() {
  ...
  if [ -n "$output" ]; then
    while IFS= read -r line; do
      dj_lock_log "lsof check failed: $line"
    done <<< "$output"
  else
    dj_lock_log "lsof check failed for $target with exit $status"
  fi
  return 2
}

# dj_lock_has_live_holder <lock> <dir>: 0 if a live process holds $lock or the
...
dj_lock_has_live_holder() {
  local lock=$1 dir=$2 status
  command -v lsof >/dev/null 2>&1 || return 0
  if [ -n "$lock" ]; then
    if dj_lock_lsof_holder "$lock"; then
      return 0
    else
      status=$?
  ...
  if [ -n "$dir" ]; then
    if dj_lock_lsof_holder "$dir"; then
      return 0
    else
      status=$?
      [ "$status" -eq 1 ] || return 0
...
# dj_lock_age <lock>: prints the lock's mtime age in whole seconds, or fails.
dj_lock_age() {
  local lock=$1 m now
  m=$(dj_lock_path_mtime "$lock") || return 1
...
# dj_lock_is_provably_stale <lock> <dir> <min_age_secs>: THE proof. Returns 0 iff
...
dj_lock_is_provably_stale() {
  local lock=$1 dir=$2 min_age=$3 age
  [ -n "$lock" ] && [ -e "$lock" ] || return 1
  dj_lock_has_live_holder "$lock" "$dir" && return 1
  if ! age=$(dj_lock_age "$lock"); then
    dj_lock_log "cannot read mtime for git lock $lock; leaving it in place"
    return 1
  fi
  [ "$age" -ge "$min_age" ]
}
