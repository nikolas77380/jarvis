...
# Usage: dj-remote-inherit-push.sh <secondmate-id> <generation>
#
# The item set is derived from the ONE declared owner
# (DJ_INHERITABLE_CONFIG in bin/dj-config-inherit-lib.sh), the same declaration
# the receiving bin/dj-remote-inherit.sh enforces, so the two implementations in
# one code revision cannot drift silently. Different local and remote revisions
# fail closed as documented by that owner. DJ_CONFIG_INHERIT_LIVE=1 marks a live
...
[ "$#" -eq 2 ] || { echo "usage: dj-remote-inherit-push.sh <secondmate-id> <generation>" >&2; exit 2; }
ID=$1
GENERATION=$2
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/dj-remote-inherit-push.XXXXXX") || die "cannot create inheritance staging directory"
trap 'rm -rf -- "$TMP"' EXIT
EMPTY="$TMP/empty"
: > "$EMPTY"
EMPTY_HASH=$(sha256_file "$EMPTY") || die "cannot hash empty inheritance payload"

ITEMS=$(dj_config_inherit_items)
while IFS= read -r rel; do
  [ -n "$rel" ] || continue
  if [ "${DJ_CONFIG_INHERIT_LIVE:-0}" = 1 ]; then
    case "$rel" in
      config/*)
        if dj_config_inherit_item_session_scoped "${rel#config/}"; then
          printf 'unchanged: %s\n' "$rel"
          continue
        fi
        ;;
  ...
  if [ -e "$source" ] || [ -L "$source" ]; then
    ...
    "$SCRIPT_DIR/dj-on.sh" "$ID" dj-remote-inherit.sh put "$rel" "$bytes" "$hash" "$GENERATION" < "$snapshot"
  else
    # This loop's heredoc is its control stream, not remote command input.
    "$SCRIPT_DIR/dj-on.sh" "$ID" dj-remote-inherit.sh absent "$rel" 0 "$EMPTY_HASH" "$GENERATION" < /dev/null
  fi
done <<EOF
$ITEMS
EOF
