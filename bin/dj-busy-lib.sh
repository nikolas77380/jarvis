#!/usr/bin/env bash
# dj-busy-lib.sh - the ONE owner of jarvis's semantic busy-state contract.
#
...
# by bin/dj-busy-event.sh (the only writer):
#
...
# wiring is armed (dj-spawn, or a documented recovery re-arm). Every event
...
# newer record.
#
# Semantic sources written by adapters (dj_busy_sources_for_harness owns the
...
#                    dj_busy_codex_semantic_source
#   kimi-wire, kimi-hook  reserved: standalone Kimi, gated by dj_busy_kimi_verified
# Jarvis-owned sources accepted for every converted adapter:
#   dj-spawn         the launch-brief turn seeded at spawn
#   dj-interrupt     the legacy Claude dj-send --key Escape idle event
#   dj-recovery      a documented recovery reset after relaunch
...
#
# Classification (dj_busy_classify): busy | idle | unknown | dead, always
# with the producing source as the second token. Precedence:
#   1. dead endpoint (dj_busy_classify_live only) -> dead endpoint-gone
...
# another adapter. The delivery guards in bin/dj-composer-lib.sh match rendered
...
# cleared. See dj_busy_muse_run_state for the fold.
#
...
# cleared. See dj_busy_cursor_turn_state for the fold. Cursor's rendered
# `ctrl+c to stop` footer is deliberately not a state source here.
#
# Codex negotiation (dj_busy_codex_appserver_observable,
# dj_busy_codex_hooks_verified): the approved contract prefers Codex's
...
# falling back to idle, and dj-spawn installs no Codex busy wiring.
...
# unknown kimi-unverified and dj-spawn wires no Kimi busy events. Kimi's
...
# real turn on a jarvis-launched worker including the interrupt path,
...
# and land the wiring in dj-spawn behind this same gate in the same change.
DJ_BUSY_KIMI_VERIFIED_VERSIONS=""

dj_busy_kimi_verified() {
  [ -n "$DJ_BUSY_KIMI_VERIFIED_VERSIONS" ]
}

# dj_busy_codex_appserver_observable: capability/version negotiation for the
...
dj_busy_codex_appserver_observable() {
  return 1
}

# dj_busy_codex_hooks_verified: the sanctioned intermediate - Codex's stable
...
# jarvis-launched worker. codex-cli 0.145.0 verdict (live, 2026-07-28):
# NOT verified. Jarvis-written project hooks under <worktree>/.codex/
...
dj_busy_codex_hooks_verified() {
  return 1
}

# dj_busy_codex_semantic_source: 0 when ANY verified Codex semantic source
# exists. dj-spawn arms and wires Codex only behind this gate, and the
# classifier reports unknown codex-unverified until it opens.
dj_busy_codex_semantic_source() {
  dj_busy_codex_appserver_observable || dj_busy_codex_hooks_verified
}

dj_busy_record_path() {  # <state-dir> <id>
  printf '%s/%s.busy-state' "$1" "$2"
}

dj_busy_gen_path() {  # <state-dir> <id>
  printf '%s/%s.busy-gen' "$1" "$2"
}

# dj_busy_token_valid: conservative token charset shared by gen, source, and
# event fields. Anything else is malformed.
dj_busy_token_valid() {  # <value>
  case "${1:-}" in
...
# dj_busy_current_gen: the task's armed gen token, or failure when the busy
# contract has never been armed for this task.
dj_busy_current_gen() {  # <state-dir> <id>
  local gen_file gen
  gen_file=$(dj_busy_gen_path "$1" "$2")
  [ -f "$gen_file" ] || return 1
  IFS= read -r gen < "$gen_file" 2>/dev/null || gen=
  dj_busy_token_valid "$gen" || return 1
  printf '%s' "$gen"
}

# dj_busy_sources_for_harness: the semantic sources trusted to classify a
# task recorded with <harness>. One line, space-separated, possibly empty.
# The jarvis-owned sources are appended for every converted adapter.
...
dj_busy_sources_for_harness() {  # <harness>
  local adapter=
  case "${1:-}" in
    claude*) adapter=claude-hook ;;
    codex*)
      dj_busy_codex_semantic_source || { printf ''; return 0; }
      adapter='codex-hook codex-appserver'
      ;;
    opencode*) adapter=opencode-plugin ;;
    pi|pi-signed) adapter=pi-ext ;;
    kimi*)
      dj_busy_kimi_verified || { printf ''; return 0; }
      adapter='kimi-wire kimi-hook'
      ;;
    *) printf ''; return 0 ;;
  esac
  printf '%s dj-spawn dj-interrupt dj-recovery' "$adapter"
}

dj_busy_source_trusted() {  # <harness> <source>
  local trusted
  trusted=$(dj_busy_sources_for_harness "$1")
  case " $trusted " in
    *" $2 "*) return 0 ;;
  esac
  return 1
...
# dj_busy_record_read: parse and validate state/<id>.busy-state against the
...
dj_busy_record_read() {  # <state-dir> <id>
  local state=$1 id=$2 rec gen line extra ver f
  local r_gen='' r_seq='' r_state='' r_source='' r_event='' r_ts=''
  rec=$(dj_busy_record_path "$state" "$id")
  if [ ! -f "$rec" ]; then
    printf 'missing'
    return 1
  fi
  if ! gen=$(dj_busy_current_gen "$state" "$id"); then
  ...
  ver=${fields[0]:-}
  [ "$ver" = "$DJ_BUSY_LIB_VERSION" ] || { printf 'malformed'; return 1; }
  ...
  dj_busy_token_valid "$r_gen" || { printf 'malformed'; return 1; }
  dj_busy_token_valid "$r_source" || { printf 'malformed'; return 1; }
  dj_busy_token_valid "$r_event" || { printf 'malformed'; return 1; }
...
# dj_busy_muse_binding_path: the per-task sidecar dj-spawn writes so the
...
# predates this pane.
dj_busy_muse_binding_path() {  # <state-dir> <id>
  printf '%s/%s.muse-session' "$1" "$2"
}

dj_busy_muse_cache_path() {  # <state-dir> <id>
  printf '%s/%s.muse-session-current' "$1" "$2"
}

# dj_busy_muse_binding_field: read one field from the sidecar, or fail.
dj_busy_muse_binding_field() {  # <state-dir> <id> <key>
  local path line key=$3
  path=$(dj_busy_muse_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
...
# dj_busy_muse_matching_logs: every MAIN session log whose recorded
...
# parent's turn ended.
dj_busy_muse_matching_logs() {  # <sessions-root> <workspace-root>
  local root=$1 ws=$2
  [ -d "$root" ] || return 1
...
dj_busy_muse_binding_has_prior_log() {  # <state-dir> <id> <session-log>
  local path line
  path=$(dj_busy_muse_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
...
dj_busy_muse_cache_field() {  # <state-dir> <id> <key>
  local path line key=$3
  path=$(dj_busy_muse_cache_path "$1" "$2")
  [ -f "$path" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      "$key="*)
...
dj_busy_muse_main_log_path_valid() {  # <sessions-root> <session-log>
...
dj_busy_muse_namespace_day() {  # <sessions-root>
  printf '%s/%s' "${1%/}" "$(date '+%Y/%m/%d')"
}

dj_busy_muse_namespace_signature() {  # <day-directory>
...
dj_busy_muse_cached_session_log() {  # <state-dir> <id> <root> <binding-id>
  local cache_binding log cache_day cache_signature day signature
  [ -n "$4" ] || return 1
  cache_binding=$(dj_busy_muse_cache_field "$1" "$2" binding_id) || return 1
  [ "$cache_binding" = "$4" ] || return 1
  log=$(dj_busy_muse_cache_field "$1" "$2" session_log) || return 1
  dj_busy_muse_main_log_path_valid "$3" "$log" || return 1
  dj_busy_muse_binding_has_prior_log "$1" "$2" "$log" && return 1
  cache_day=$(dj_busy_muse_cache_field "$1" "$2" namespace_day) || return 1
  cache_signature=$(dj_busy_muse_cache_field "$1" "$2" namespace_signature) || return 1
  day=$(dj_busy_muse_namespace_day "$3") || return 1
  [ "$cache_day" = "$day" ] || return 1
  signature=$(dj_busy_muse_namespace_signature "$day") || return 1
  [ "$cache_signature" = "$signature" ] || return 1
  printf '%s' "$log"
}

dj_busy_muse_cache_session_log() {  # <state-dir> <id> <binding-id> <session-log> <namespace-day> <namespace-signature>
  local cache tmp current
  [ -n "$3" ] || return 0
  current=$(dj_busy_muse_binding_field "$1" "$2" binding_id) || return 1
  [ "$current" = "$3" ] || return 1
  cache=$(dj_busy_muse_cache_path "$1" "$2")
  tmp="$cache.tmp.$$"
  {
    printf 'binding_id=%s\n' "$3"
...
# dj_busy_muse_session_log: the one matching MAIN session log that did not
# exist when dj-spawn created this pane's binding. Multiple candidates are
# ambiguous and fail closed rather than guessing which pane owns either log.
dj_busy_muse_session_log() {  # <state-dir> <id>
  local root ws binding_id='' candidate selected='' cache namespace_day namespace_before namespace_after
  root=$(dj_busy_muse_binding_field "$1" "$2" sessions_root) || return 1
  ws=$(dj_busy_muse_binding_field "$1" "$2" workspace_root) || return 1
  binding_id=$(dj_busy_muse_binding_field "$1" "$2" binding_id 2>/dev/null || true)
  if cache=$(dj_busy_muse_cached_session_log "$1" "$2" "$root" "$binding_id"); then
    printf '%s' "$cache"
    return 0
  fi
  rm -f "$(dj_busy_muse_cache_path "$1" "$2")"
  namespace_day=$(dj_busy_muse_namespace_day "$root") || return 1
  namespace_before=$(dj_busy_muse_namespace_signature "$namespace_day") || return 1
  while IFS= read -r candidate; do
    [ -n "$candidate" ] || continue
    dj_busy_muse_binding_has_prior_log "$1" "$2" "$candidate" && continue
    [ -z "$selected" ] || return 1
    selected=$candidate
  done <<EOF
$(dj_busy_muse_matching_logs "$root" "$ws")
EOF
  [ -n "$selected" ] || return 1
  namespace_after=$(dj_busy_muse_namespace_signature "$namespace_day") || return 1
  [ "$namespace_before" = "$namespace_after" ] || return 1
  dj_busy_muse_cache_session_log "$1" "$2" "$binding_id" "$selected" \
    "$namespace_day" "$namespace_after" || return 1
  printf '%s' "$selected"
}

dj_busy_muse_run_events() {  # <session-log>
  [ -f "$1" ] || return 1
  LC_ALL=C awk '
...
# dj_busy_muse_run_state: fold one session log to busy|settled|none.
...
dj_busy_muse_run_state() {  # <session-log>
  [ -f "$1" ] || return 1
  dj_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' '
...
dj_busy_muse_active_run_id() {  # <session-log>
  [ -f "$1" ] || return 1
  dj_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' '
    $2 == "started" { open[$1] = 1 }
    $2 == "terminal" { open[$1] = 0 }
...
dj_busy_muse_run_terminal() {  # <session-log> <run-id>
  [ -f "$1" ] && [ -n "${2:-}" ] || return 1
  dj_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' -v wanted="$2" '
...
#
# dj_busy_cursor_binding_path: the per-task sidecar dj-spawn writes. It records
...
dj_busy_cursor_binding_path() {  # <state-dir> <id>
  printf '%s/%s.cursor-session' "$1" "$2"
}

dj_busy_cursor_binding_field() {  # <state-dir> <id> <key>
  local path value
  path=$(dj_busy_cursor_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
...
# dj_busy_cursor_project_dir: the project directory whose recorded
...
dj_busy_cursor_project_dir() {  # <projects-root> <workspace-root>
...
# dj_busy_cursor_transcript: the ONE transcript this pane owns, or failure.
...
dj_busy_cursor_transcript() {  # <state-dir> <id>
  local root workspace project dir conv found='' count=0 prior
  root=$(dj_busy_cursor_binding_field "$1" "$2" projects_root) || return 1
  workspace=$(dj_busy_cursor_binding_field "$1" "$2" workspace_root) || return 1
  project=$(dj_busy_cursor_project_dir "$root" "$workspace") || return 1
  prior=$(LC_ALL=C awk -F= '$1 == "prior_conversation" { sub(/^[^=]*=/, ""); print }' \
    "$(dj_busy_cursor_binding_path "$1" "$2")" 2>/dev/null)
...
# dj_busy_cursor_turn_state: fold the transcript into busy | settled | none.
...
dj_busy_cursor_turn_state() {  # <transcript>
  [ -f "$1" ] || return 1
...
# dj_busy_grok_tail_busy: the Grok-only temporary rendered-tail fallback.
# Consumes the tail on stdin; 0 when Grok's verified busy signature matches.
# DJ_BUSY_REGEX still globally overrides the signature, mirroring the
# historical operator escape hatch.
dj_busy_grok_tail_busy() {
  grep -v '^[[:space:]]*$' | tail -12 \
    | grep -qiE "${DJ_BUSY_REGEX:-${DJ_DELIVERY_GROK_BUSY_REGEX_DEFAULT:-Ctrl\\+c:cancel}}"
}

# dj_busy_classify: semantic classification for a task whose endpoint the
# caller has already established as present. Prints "<verdict> <source>":
# busy|idle|unknown plus the producing source (see header). Never probes
# process state. <tail40> is optional pre-captured plain output used only by
# the Grok arm; when absent the Grok arm captures through dj_backend_capture
# if available, else reports unknown capture-failed.
dj_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  case "$harness" in
    ...
    cursor*)
      ...
      if ! log=$(dj_busy_cursor_transcript "$state" "$id"); then
        printf 'unknown cursor-transcript'
        return 0
      fi
      case "$(dj_busy_cursor_turn_state "$log" 2>/dev/null)" in
  ...
  out=$(dj_busy_record_read "$state" "$id") && rc=0 || rc=$?
  if [ "$rc" = 0 ]; then
    r_state=${out%% *}
    out=${out#* }
    r_source=${out%% *}
    if dj_busy_source_trusted "$harness" "$r_source"; then
  ...
  # unknown here.
  if [ "$backend" = herdr ] && command -v dj_backend_busy_state >/dev/null 2>&1; then
    native=$(dj_backend_busy_state "$backend" "$target" 2>/dev/null || true)
    if [ "$native" = busy ]; then
  ...
  case "$harness" in
    muse*)
      ...
      if ! log=$(dj_busy_muse_session_log "$state" "$id"); then
        printf 'unknown muse-session-log'
        return 0
      fi
      case "$(dj_busy_muse_run_state "$log" 2>/dev/null)" in
    ...
    grok*)
      if [ -z "$tail40" ]; then
        if command -v dj_backend_capture >/dev/null 2>&1; then
          tail40=$(dj_backend_capture "$backend" "$target" 40 2>/dev/null) || {
      ...
      if printf '%s' "$tail40" | dj_busy_grok_tail_busy; then
...
# dj_busy_classify_live: dj_busy_classify behind the one process-level
# override - a gone endpoint is dead, never busy. Requires dj-backend.sh to
# be sourced for dj_backend_target_exists.
dj_busy_classify_live() {  # <backend> <target> <harness> <id> <state-dir> [expected-label]
  local backend=$1 target=$2 harness=$3 id=$4 state=$5 label=${6-}
  if [ -z "$target" ]; then
    printf 'unknown no-target'
    return 0
  fi
  if ! dj_backend_target_exists "$backend" "$target" "$label" 2>/dev/null; then
    printf 'dead endpoint-gone'
    return 0
  fi
  dj_busy_classify "$backend" "$target" "$harness" "$id" "$state"
}

# dj_busy_classify_meta: classify a task from its recorded metadata, so every
# consumer resolves backend, target, and harness the same way instead of
# re-deriving them. Requires dj-backend.sh to be sourced. <tail40> is
# optional pre-captured plain output reused by the Grok arm.
dj_busy_classify_meta() {  # <meta-file> <id> <state-dir> [tail40]
  ...
  dj_busy_classify "$backend" "$target" "$harness" "$id" "$state" "$tail40"
}

# dj_busy_is_busy: boolean view for callers that only gate on provable
...
dj_busy_is_busy() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  local verdict
  verdict=$(dj_busy_classify "$@")
  [ "${verdict%% *}" = busy ]
}
