...
#
# Run it through dj-on.sh so the fixed entrypoint invokes this readiness owner
...
# dj-remote session. Its account therefore needs the Jarvis-owned Aqua Herdr
# agent plus the sibling dev.jarvis.remote-job worker that runs normal dj-on
...
# both Jarvis-owned Aqua agents, starts the Linux workers where no Aqua agent
...
# changes FileVault, stores an account password, or replaces a non-Jarvis
...
LAUNCH_AGENT_LABEL=dev.jarvis.herdr.dj-remote
...
case "${1:-}" in
  ...
  --fix) MODE=fix; shift ;;
  --worker-tool-probe)
    [ "${DJ_REMOTE_JOB_ACTIVE:-}" = 1 ] || { printf 'error: worker tool probe requires the remote job worker\n' >&2; exit 64; }
...
herdr_adapter_load() {
  [ -z "${DJ_REMOTE_DOCTOR_HERDR_LOADED:-}" ] || return 0
  herdr_cli_available || return 1
  [ -f "$SCRIPT_DIR/dj-backend.sh" ] && [ -f "$SCRIPT_DIR/backends/herdr.sh" ] || return 1
  # shellcheck source=bin/dj-backend.sh
  . "$SCRIPT_DIR/dj-backend.sh" || return 1
  dj_backend_source herdr || return 1
  DJ_REMOTE_DOCTOR_HERDR_LOADED=1
}

herdr_server_running() {
  local running
  herdr_adapter_load || return 1
  running=$(dj_backend_herdr_cli "$HERDR_SESSION_NAME" status --json 2>/dev/null \
...
remote_job_existing_state() {
  local root
  root=${DJ_REMOTE_JOB_STATE_ROOT:-${HOME:-}/.jarvis/remote-job}
  root=$(dj_remote_job_canonical_existing_dir "$root") || return 1
  dj_remote_job_canonical_existing_dir "$root/jobs" >/dev/null || return 1
...
remote_job_probe_ok() {
  ...
  ready="$DJ_REMOTE_JOB_STATE/worker.ready"
...
remote_job_identity_ok() {
  ...
  dj_remote_job_worker_identity_matches "$DJ_ROOT" "${HOME:-}"
}

check_remote_job_worker() {
  local worker
  worker="$DJ_ROOT/bin/dj-remote-job-worker.sh"
  if [ ! -f "$worker" ] || [ -L "$worker" ] || [ ! -x "$worker" ]; then
    record remote-job-worker "human: the configured Jarvis code root has no safe remote job worker" \
      "update the remote Jarvis checkout, then rerun this command with --fix"
  ...
  if [ "$PLATFORM" = darwin ]; then
    dj_remote_job_launchagent_paths "${HOME:-}"
    if dj_remote_job_launchagent_contract_matches "$DJ_ROOT" "${HOME:-}"; then
      record remote-job-worker "ok: $DJ_REMOTE_JOB_LAUNCH_AGENT_PLIST matches the Jarvis-owned Aqua worker contract"
    else
      record remote-job-worker "fixable: $DJ_REMOTE_JOB_LAUNCH_AGENT_PLIST does not match the Jarvis-owned Aqua worker contract" \
        "rerun this command with --fix to write dev.jarvis.remote-job"
    fi
    ...
    elif dj_remote_job_launchagent_loaded "$DJ_ROOT" "${HOME:-}" "$UID_NUM"; then
      record remote-job-worker-loaded "ok: $DJ_REMOTE_JOB_LABEL is loaded in gui/$UID_NUM"
    elif check_is_ok gui-session; then
      record remote-job-worker-loaded "fixable: $DJ_REMOTE_JOB_LABEL is not loaded in gui/$UID_NUM" \
        "rerun this command with --fix to bootstrap the worker"
    else
      record remote-job-worker-loaded "human: $DJ_REMOTE_JOB_LABEL cannot be loaded because gui/$UID_NUM has no login session" \
        "close the login-session gap first; SSH cannot create an Aqua session"
    fi
  else
    local pid
    pid=$(cat "${DJ_REMOTE_JOB_STATE_ROOT:-${HOME:-}/.jarvis/remote-job}/worker.pid" 2>/dev/null || true)
    if [ "${DJ_REMOTE_JOB_ACTIVE:-}" = 1 ] ||
  ...
  if ! remote_job_probe_ok; then
    record remote-job-probe "fixable: the remote job worker has not reported a fresh probe" \
      "rerun this command with --fix to restart the worker, then rerun through dj-on.sh"
  elif ! remote_job_identity_ok; then
    set_check remote-job-worker "fixable: the running remote job worker does not match the current Jarvis code" \
...
report_required_tools() {
  ...
  for tool in "${REQUIRED_TOOLS[@]}"; do
    ...
    if [ -n "$resolved" ] && [ -x "$resolved" ]; then
      if [ "$tool" = tasks-axi ] && ! dj_tasks_axi_compatible; then
...
report_required_tools_from_worker() {
  ...
  if ! job_id=$(dj_remote_job_stage "${HOME:-}" "$DJ_ROOT" "${DJ_HOME:-}" \
    dj-remote-doctor.sh --worker-tool-probe </dev/null); then
  ...
  if ! dj_remote_job_wait "${HOME:-}" "$job_id"; then
    dj_remote_job_reap "${HOME:-}" "$job_id" 2>/dev/null || true
    set_check remote-job-probe "fixable: the remote job worker did not complete the required-tool probe" \
      "rerun this command with --fix to restart the worker"
    report_required_tools
    return 0
  fi
  probe_stdout=$DJ_REMOTE_JOB_STDOUT
  probe_stderr=$DJ_REMOTE_JOB_STDERR
  probe_exit=$DJ_REMOTE_JOB_EXIT
  MISSING=()
...
repair_tool_wrapper() { # <tool>
  ...
  target=$(dj_remote_job_manager_tool "${HOME:-}" "$tool" 2>/dev/null || true)
  [ -n "$target" ] || return 1
  wrapper="${HOME:-}/.local/bin/$tool"
  if [ -e "$wrapper" ] || [ -L "$wrapper" ]; then
    if ! wrapper_is_jarvis_owned "$wrapper"; then
      fix_report "required-$tool" failed "$wrapper exists and is not Jarvis-owned"
      return 1
    fi
  else
...
repair_required_wrappers() {
  ...
  for tool in "${HARNESS_TOOLS[@]}"; do
    dj_remote_job_manager_tool "${HOME:-}" "$tool" >/dev/null 2>&1 || continue
    repair_tool_wrapper "$tool" && return 0
  done
}

fix_remote_job_worker() {
  if dj_remote_job_ensure_worker "$DJ_ROOT" "${HOME:-}"; then
    [ "$DJ_REMOTE_JOB_REPAIRED" -eq 0 ] || fix_report remote-job-worker applied "installed or reloaded $DJ_REMOTE_JOB_LABEL"
    return 0
  fi
  fix_report remote-job-worker failed "${DJ_REMOTE_JOB_ERROR:-the remote job worker could not start}"
  return 1
}
...
check_gui_session() {
  ...
  if [ -z "$UID_NUM" ]; then
    record gui-session "human: the account uid could not be read, so its login session cannot be inspected" \
      "run 'id -u' on that account and report the failure; Jarvis cannot address gui/<uid> without it"
    return 0
  fi
  ...
  record gui-session "human: no Aqua login session exists for uid $UID_NUM" \
    "log that account in once at the console, and enable automatic login in System Settings > Users & Groups if the machine runs headless; SSH cannot create a GUI session, and Jarvis never writes an auto-login password or changes FileVault"
}
...
check_launch_agent() {
  ...
  if [ -f "$LAUNCH_AGENT_PLIST" ] && [ ! -L "$LAUNCH_AGENT_PLIST" ]; then
    if launch_agent_contract_matches; then
      record launchagent "ok: $LAUNCH_AGENT_PLIST matches the Jarvis-owned contract"
    else
      record launchagent "fixable: $LAUNCH_AGENT_PLIST does not match the current Jarvis-owned contract" \
    ...
    record launchagent "fixable: no Jarvis herdr launch agent at $LAUNCH_AGENT_PLIST" \
...
check_launch_agent_loaded() {
  ...
  if launchctl print "gui/$UID_NUM/$LAUNCH_AGENT_LABEL" >/dev/null 2>&1; then
    if launch_agent_loaded_contract_matches; then
      ...
      record launchagent-loaded "fixable: gui/$UID_NUM/$LAUNCH_AGENT_LABEL does not match the effective Jarvis-owned contract" \
...
check_entrypoint_link() {
  ...
  if [ -e "$ENTRYPOINT_LINK" ] || [ -L "$ENTRYPOINT_LINK" ]; then
    record entrypoint-link "human: $ENTRYPOINT_LINK exists but is not the symlink to $want" \
      "inspect that path yourself and replace it with 'ln -sfn $want $ENTRYPOINT_LINK' if it is stale; Jarvis never overwrites a file it did not create there"
    return 0
  fi
...
start_herdr_server() {
  ...
  if dj_backend_herdr_server_ensure "$HERDR_SESSION_NAME" >/dev/null 2>&1; then
...
link_entrypoint() {
  local want="${DJ_ROOT_OVERRIDE:-}/bin/dj-remote-entrypoint.sh"
...
printf 'mode=%s\n' "$MODE"
printf 'path=%s\n' "${PATH:-}"
if [ -n "${DJ_ROOT_OVERRIDE:-}" ] && [ "${PATH%%:*}" = "$DJ_ROOT_OVERRIDE/bin" ]; then
...
if [ "${DJ_REMOTE_JOB_ACTIVE:-}" = 1 ] || ! remote_job_identity_ok; then
  report_required_tools
else
  report_required_tools_from_worker
fi
