#!/usr/bin/env bash
# Host-local lifecycle control for the remote secondmate home selected by dj-on.
#
# Usage:
#   dj-remote-secondmate-control.sh launch <id> <harness> <model|-> <effort|-> herdr [traceparent]
#   dj-remote-secondmate-control.sh state <id>
#   dj-remote-secondmate-control.sh route <id>
#   dj-remote-secondmate-control.sh send <id> <message>
#   dj-remote-secondmate-control.sh key <id> <key>
#   dj-remote-secondmate-control.sh capture <id> [lines]
#   dj-remote-secondmate-control.sh observe <id>
#   dj-remote-secondmate-control.sh sync <id>
#   dj-remote-secondmate-control.sh update <id>
#   dj-remote-secondmate-control.sh retire <id> [--force]
#
# Remote placement ends here, but the second-mate agent always runs on the
# Herdr backend in the dedicated dj-remote session, so launch refuses any other
# selection rather than reading this home's config/backend. The interactive
# default session remains for the user's work.
# dj-spawn/dj-send/dj-teardown keep owning the local endpoint mechanics.
# The home's own workers keep their ordinary backend selection.
# bin/dj-remote-doctor.sh owns that host's readiness for Herdr.
...
# stops dj-remote or removes a sibling secondmate's workspace or panes.
#
...
# pane, and dj-spawn validates it (bin/dj-trace-context-lib.sh). Omitting it is
...
TARGET_HOME=${DJ_HOME:?DJ_HOME is required}
...
validate_home() { # <id> [allow-absent]
  ...
  [ -f "$TARGET_HOME/.dj-secondmate-home" ] && [ ! -L "$TARGET_HOME/.dj-secondmate-home" ] \
    || die "remote home is not a seeded secondmate home"
  marker=$(cat "$TARGET_HOME/.dj-secondmate-home")
  [ "$marker" = "$id" ] || die "remote home belongs to $marker, not $id"
  [ -f "$TARGET_HOME/AGENTS.md" ] && [ -d "$TARGET_HOME/bin" ] || die "remote home is not a Jarvis checkout"
}
...
remote_endpoint_load() {
  ...
  if ! dj_backend_validate_task_endpoint "$REMOTE_ENDPOINT_META" "$id" 2>/dev/null; then
    REMOTE_ENDPOINT_ERROR="remote secondmate $id endpoint metadata is invalid; refusing access until it is explicitly migrated"
    return 1
  fi
  REMOTE_ENDPOINT_BACKEND=$DJ_BACKEND_VALIDATED_BACKEND
  REMOTE_ENDPOINT_TARGET=$DJ_BACKEND_VALIDATED_TARGET
  ...
  herdr_session=$(dj_backend_meta_exact_value "$REMOTE_ENDPOINT_META" herdr_session 2>/dev/null || true)
...
state_value() { # <id>; prints recovery-grade state
  ...
  dj_backend_agent_state "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null || printf 'unreadable\n'
}

print_route() { # <id>
  local id=$1 harness traceparent
  remote_endpoint_require "$id"
  harness=$(dj_meta_get "$REMOTE_ENDPOINT_META" harness)
  traceparent=$(dj_meta_get "$REMOTE_ENDPOINT_META" traceparent)
...
cmd_launch() {
  ...
  # a remote route depends on. bin/dj-remote-doctor.sh is the readiness owner.
  ...
  if [ -f "$meta" ]; then
    remote_endpoint_require "$id"
    current=$(dj_backend_agent_state "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null || printf 'unreadable\n')
    case "$current" in
      alive)
        print_route "$id"
        return 0
        ;;
      dead)
        dj_backend_kill "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null \
  ...
  if ! out=$(HERDR_SESSION="$REMOTE_HERDR_SESSION" DJ_HOME="$DJ_ROOT" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
    DJ_STATE_OVERRIDE="$CONTROL_STATE" DJ_DATA_OVERRIDE="$CONTROL_DATA" \
    DJ_CONFIG_OVERRIDE="$TARGET_HOME/config" DJ_SKIP_SECONDMATE_INHERIT=1 \
    "$SCRIPT_DIR/dj-spawn.sh" "${ARGS[@]}" 2>&1); then
    [ -z "$out" ] || printf '%s\n' "$out" >&2
    die "remote host-local secondmate launch failed"
  fi
  [ -f "$meta" ] || die "remote launch returned without endpoint metadata"
  herdr_session=$(dj_meta_get "$meta" herdr_session)
...
cmd_send() {
  ...
  validate_home "$id"
  meta=$(meta_path "$id")
  meta_lock=$(dj_meta_lock_path "$meta") || die "remote secondmate metadata lock path is invalid"
  dj_task_inbox_lock_acquire "$meta_lock" \
  ...
  # best-effort (bin/dj-task-inbox-lib.sh owns the record and doorbell). The
  ...
  if ! rec=$(dj_task_inbox_write_idempotent "$CONTROL_STATE" "$id" "$message"); then
    dj_lock_release "$meta_lock"
  ...
  dj_task_inbox_ring "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "$rec" "dj-$id" || ring_rc=$?
  case "$ring_rc" in
...
cmd_key() {
  ...
  validate_home "$id"
  remote_endpoint_require "$id"
  DJ_HOME="$TARGET_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_STATE_OVERRIDE="$TARGET_HOME/state" \
    "$SCRIPT_DIR/dj-send.sh" "$REMOTE_ENDPOINT_TARGET" --key "$key"
}
...
cmd_capture() {
  ...
  remote_endpoint_require "$id"
  dj_backend_capture "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "$lines" "dj-$id" | head -c 65536
}
...
cmd_observe() {
  ...
  remote_endpoint_require "$id"
  harness=$(dj_meta_get "$REMOTE_ENDPOINT_META" harness)
  dj_pending_reply_backend_observation "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "dj-$id" "$harness"
  printf '\n'
}
...
cmd_sync() {
  ...
  validate_home "$id"
  target=$TARGET_HOME
  dirty=$(git -C "$target" status --porcelain 2>/dev/null | awk '$0 != "?? .dj-secondmate-home" { print; exit }')
  [ -z "$dirty" ] || die "remote secondmate checkout is dirty; sync skipped"
  head=$(git -C "$DJ_ROOT" rev-parse HEAD 2>/dev/null) || die "remote code root HEAD is unreadable"
  ...
  if ! git -C "$target" cat-file -e "$head^{commit}" 2>/dev/null; then
    git -C "$target" fetch --quiet --no-tags "$DJ_ROOT" "$head" \
...
cmd_update() {
  ...
  validate_id "$id"
  validate_home "$id"
  if ! update_out=$(DJ_HOME="$DJ_ROOT" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
    "$SCRIPT_DIR/dj-update.sh" 2>&1); then
    [ -z "$update_out" ] || printf '%s\n' "$update_out" >&2
    die "remote code root update failed"
  fi
  root_status=$(printf '%s\n' "$update_out" | grep '^jarvis:' | tail -1)
  case "$root_status" in
    'jarvis: updated '*|'jarvis: already current'*) ;;
    *)
      [ -z "$update_out" ] || printf '%s\n' "$update_out" >&2
...
cmd_retire() {
  ...
  remote_endpoint_require "$id"
  DJ_HOME="$TARGET_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_STATE_OVERRIDE="$TARGET_HOME/state" \
    DJ_CONFIG_OVERRIDE="$TARGET_HOME/config" "$SCRIPT_DIR/dj-guard.sh" || true
  if [ -n "$force" ]; then
    DJ_HOME="$DJ_ROOT" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
      DJ_STATE_OVERRIDE="$CONTROL_STATE" DJ_DATA_OVERRIDE="$CONTROL_DATA" \
      DJ_CONFIG_OVERRIDE="$TARGET_HOME/config" DJ_TEARDOWN_GUARD_DONE=1 \
      "$SCRIPT_DIR/dj-teardown.sh" "$id" --force
  else
    DJ_HOME="$DJ_ROOT" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
      DJ_STATE_OVERRIDE="$CONTROL_STATE" DJ_DATA_OVERRIDE="$CONTROL_DATA" \
      DJ_CONFIG_OVERRIDE="$TARGET_HOME/config" DJ_TEARDOWN_GUARD_DONE=1 \
      "$SCRIPT_DIR/dj-teardown.sh" "$id"
  fi
}
