...
# secondmate in its isolated jarvis home.
# Usage: dj-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
#        dj-spawn.sh <task-id> <project-dir> --scout [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
#        dj-spawn.sh <task-id> [<jarvis-home>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>] --secondmate
#   --mode and --yolo are this task's delivery contract, REQUIRED for every ship
#   spawn and refused on --scout and --secondmate spawns. Jarvis resolves both
...
#   the launch half of the control plane (bin/dj-control.sh relaunch), which
#   owns the checkpoint, the progress note, stopping the previous agent, and the
#   transaction; call dj-control rather than this flag directly unless you are
...
#   axes chosen by jarvis at intake. They are only threaded into harnesses whose
#   installed CLIs were verified to support that axis; unsupported axes are omitted
#   from that harness's launch rather than guessed.
#   --backend <name> is the explicit runtime session-provider backend for this
#   exact task only (docs/configuration.md "Runtime backend" owns when that flag
#   is authorized). Without it, the script resolves DJ_BACKEND, then
#   config/backend, then runtime auto-detection from the runtime jarvis's
#   environment: $TMUX, HERDR_ENV=1, or cmux runtime signals (via
#   bin/dj-backend.sh's dj_backend_detect, with cmux fallback details in
...
#   A herdr crewmate or scout is placed in the exact workspace of the jarvis
#   or secondmate process launching it, resolved from that process's own herdr
#   pane rather than from a workspace label (herdr enforces no label uniqueness,
#   so a label cannot tell two "jarvis" workspaces apart). A claimed parent
...
#   immediately after its owning parent (jarvis or 2ndmate-<id>) contiguous
...
#   spawns require an explicit harness so jarvis cannot silently skip dispatch
#   profile consultation. A --secondmate spawn is exempt and resolves the SECONDMATE
#   harness (config/secondmate-harness -> config/crew-harness -> own), so the
#   secondmate-vs-crewmate split is DURABLE across every respawn (recovery,
#   /updatejarvis, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|muse)
#   overrides it for this spawn (either kind). A non-flag string containing
#   whitespace is treated as a RAW launch command - the escape hatch for verifying
#   new adapters. For pi and pi-signed, dj-spawn resolves the selected executable
...
#   (dj-config-inherit-lib.sh). A successful launch clears pending inherited
#   config reread generations because the new agent reads the converged files.
#   --scout records kind=scout in the task's meta (report deliverable, scratch worktree;
#   see AGENTS.md task lifecycle); --secondmate records kind=secondmate and launches in a
#   provisioned jarvis home; the default is kind=ship.
...
#     dj-spawn.sh fix-a-k3=projects/foo add-b-q7=projects/bar [--scout]
...
#     __PITURNEND__ absolute path to .pi/extensions/dj-primary-turnend-guard.ts in a pi secondmate home
#     __PIWATCH__   absolute path to .pi/extensions/dj-primary-pi-watch.ts in a pi secondmate home
#     __OPINPUT__   absolute path to the canonical operational-input encoder
#     __WORKTREE__  absolute path to the task worktree
#     __CURSORBIN__ resolved, cursor-verified executable for a cursor launch
# Verified per-harness turn-end hooks are installed automatically where enabled; some live outside the worktree.
# Kimi uses one surgically installed Jarvis region in $HOME/.kimi-code/config.toml,
# a jarvis-owned global hook and registry, and a gitignored per-task pointer.
# grok uses a jarvis-owned global hook under ${GROK_HOME:-$HOME/.grok}/hooks
# plus a gitignored .dj-grok-turnend worktree pointer and a state token.
...
# docs/configuration.md and bin/dj-trace-context-lib.sh), the meta also records
...
#   pane export happens on the remote host (bin/dj-remote-secondmate-control.sh).
...
DJ_HOME=$(resolve_directory_input DJ_HOME "$DJ_HOME") || exit 1
if [ -n "${DJ_STATE_OVERRIDE:-}" ]; then
  DJ_STATE_OVERRIDE=$(resolve_directory_input DJ_STATE_OVERRIDE "$DJ_STATE_OVERRIDE") || exit 1
fi
if [ -n "${DJ_DATA_OVERRIDE:-}" ]; then
  DJ_DATA_OVERRIDE=$(resolve_directory_input DJ_DATA_OVERRIDE "$DJ_DATA_OVERRIDE") || exit 1
fi
STATE="${DJ_STATE_OVERRIDE:-$DJ_HOME/state}"
DATA="${DJ_DATA_OVERRIDE:-$DJ_HOME/data}"
PROJECTS="${DJ_PROJECTS_OVERRIDE:-$DJ_HOME/projects}"
CONFIG="${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}"
SUB_HOME_MARKER=".dj-secondmate-home"
...
# shellcheck source=bin/dj-secondmate-nudge-lib.sh
. "$SCRIPT_DIR/dj-secondmate-nudge-lib.sh"
# shellcheck source=bin/dj-config-inherit-lib.sh
. "$SCRIPT_DIR/dj-config-inherit-lib.sh"
# shellcheck source=bin/dj-backend.sh
. "$SCRIPT_DIR/dj-backend.sh"
# shellcheck source=bin/dj-control-lib.sh
. "$SCRIPT_DIR/dj-control-lib.sh"
# shellcheck source=bin/dj-gate-refuse-lib.sh
. "$SCRIPT_DIR/dj-gate-refuse-lib.sh"
# shellcheck source=bin/dj-busy-lib.sh
. "$SCRIPT_DIR/dj-busy-lib.sh"
# shellcheck source=bin/dj-cursor-lib.sh
. "$SCRIPT_DIR/dj-cursor-lib.sh"
# shellcheck source=bin/dj-pr-lib.sh
. "$SCRIPT_DIR/dj-pr-lib.sh"
# shellcheck source=bin/dj-trace-context-lib.sh
. "$SCRIPT_DIR/dj-trace-context-lib.sh"
# shellcheck source=bin/dj-remote-readiness-lib.sh
. "$SCRIPT_DIR/dj-remote-readiness-lib.sh"
# Fail closed before any fleet mutation: a no-mistakes gate agent must never spawn
# a direct report (see bin/dj-gate-refuse-lib.sh).
dj_refuse_if_gate_agent
# Skip the watcher guard when re-exec'd for one pair of a batch (DJ_SPAWN_NO_GUARD is
# set by the batch loop below), so the guard runs once for the batch, not once per pair.
[ -n "${DJ_SPAWN_NO_GUARD:-}" ] || "$DJ_ROOT/bin/dj-guard.sh" || true
KIND=ship
KIND_SET=0
HARNESS_ARG=
MODEL=
...
if [ "$ROLE_SET" -eq 1 ] && [ "$KIND" = secondmate ]; then
  echo "error: --role applies only to crewmate ship or scout spawns; a secondmate's scope lives in its charter" >&2
  exit 1
fi
...
if [ "$TRACEPARENT_SET" -eq 1 ]; then
  ...
  dj_trace_context_valid "$TRACEPARENT_ARG" || {
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  [ "$ROLE_SET" -eq 0 ] || { echo "error: --relaunch reuses the task's recorded brief, so its role cannot change; --role is refused" >&2; exit 1; }
else
  # Delivery contract (AGENTS.md section 7). A ship task's mode and yolo are
  # jarvis's per-task decision, so they are required and closed-set validated
...
spawn_remote_secondmate() {
  ...
  local -a launch_args
  id=${POS[0]:-}
  dj_task_id_creation_valid "$id" || { echo "error: invalid task id" >&2; return 2; }
  mkdir -p "$STATE" || { echo "error: could not create parent state directory" >&2; return 1; }
  SPAWN_TASK_LOCK="$STATE/.spawn-$id.lock"
  if ! dj_lock_try_acquire "$SPAWN_TASK_LOCK"; then
    echo "error: another spawn is already creating task $id" >&2
    return 1
  fi
  registry_lock=$(secondmate_registry_lock_path "$STATE")
  if ! dj_lock_acquire_wait "$registry_lock"; then
    dj_lock_release "$SPAWN_TASK_LOCK" || true
  ...
  if [ "$remote" != 1 ]; then
    dj_lock_release "$registry_lock" || true
    dj_lock_release "$SPAWN_TASK_LOCK" || true
    return 3
  fi
  ...
  if [ -n "$HARNESS_ARG" ]; then
    ...
    harness=$("$DJ_ROOT/bin/dj-harness.sh" secondmate)
  fi
  case "$harness" in
  ...
  if [ -z "$HARNESS_ARG" ] && [ -z "$positional" ]; then
    if [ "$MODEL_SET" -eq 0 ]; then
      model=$("$SCRIPT_DIR/dj-harness.sh" secondmate-model)
      [ -n "$model" ] || model=-
    fi
    if [ "$EFFORT_SET" -eq 0 ]; then
      effort=$("$SCRIPT_DIR/dj-harness.sh" secondmate-effort)
      [ -n "$effort" ] || effort=-
    fi
  fi
  ...
  # supervises it. bin/dj-remote-doctor.sh gates that host on the same
  ...
  if [ -e "$meta" ] || [ -L "$meta" ]; then
    if [ ! -f "$meta" ] || [ -L "$meta" ] \
      || [ "$(dj_meta_get "$meta" kind)" != secondmate ] \
      || [ "$(dj_meta_get "$meta" remote_host)" != "$host" ] \
      || [ "$(dj_meta_get "$meta" remote_root)" != "$root" ] \
      || [ "$(dj_meta_get "$meta" home)" != "$home" ]; then
      dj_lock_release "$registry_lock" || true
      dj_lock_release "$SPAWN_TASK_LOCK" || true
  ...
  rc=0
  dj_remote_readiness_ensure "$SCRIPT_DIR" "$id" || rc=$?
  if [ "$rc" -ne 0 ]; then
    dj_lock_release "$registry_lock" || true
    dj_lock_release "$SPAWN_TASK_LOCK" || true
    ...
    [ -z "$DJ_REMOTE_READINESS_OUT" ] || printf '%s\n' "$DJ_REMOTE_READINESS_OUT" >&2
    [ "$rc" -ne 255 ] || return 255
    return 1
  fi
  remote_lock=$(dj_remote_inherit_transaction_lock_path "$STATE" "$id")
  if ! dj_lock_acquire_wait "$remote_lock"; then
    dj_lock_release "$registry_lock" || true
    dj_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote secondmate $id inheritance transaction could not be locked" >&2
    return 1
  fi
  remote_generation=$(dj_remote_inherit_generation_next "$STATE" "$id" 2>/dev/null || true)
  if [ -z "$remote_generation" ]; then
    dj_lock_release "$remote_lock" || true
    dj_lock_release "$registry_lock" || true
    dj_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote secondmate $id inheritance generation could not be published" >&2
    return 1
  fi
  if "$SCRIPT_DIR/dj-remote-inherit-push.sh" "$id" "$remote_generation" >/dev/null; then
    :
  else
    rc=$?
    dj_lock_release "$remote_lock" || true
  ...
  remote_traceparent=
  if [ "$(dj_trace_context_session_effective "$STATE/.trace-context-effective")" = on ]; then
    remote_traceparent=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CONFIG" "$meta" || true)
  fi
  launch_args=("$id" "$harness" "$model" "$effort" "$backend")
  [ -z "$remote_traceparent" ] || launch_args+=("$remote_traceparent")
  if out=$("$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh launch \
  ...
  if [ "$remote_herdr_session" != dj-remote ] || [ "${remote_target%%:*}" != "$remote_herdr_session" ]; then
    dj_lock_release "$remote_lock" || true
    dj_lock_release "$registry_lock" || true
    dj_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote launch returned Herdr session '${remote_herdr_session:-missing}', expected 'dj-remote'; preserving the remote route for reconciliation" >&2
    return 1
  fi
  ...
  dj_trace_context_valid "$remote_recorded_traceparent" || remote_recorded_traceparent=
  tmp="$meta.tmp.$$"
  {
  ...
  if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
    ...
    dj_lock_release "$SPAWN_TASK_SET_LOCK"
  fi
  dj_lock_release "$remote_lock" || true
  dj_lock_release "$registry_lock" || true
  dj_lock_release "$SPAWN_TASK_LOCK" || true
  if ! "$SCRIPT_DIR/dj-procevent-remote-reply.sh" arm "$id" >/dev/null; then
...
spawn_abort_cleanup() {
  ...
  if [ "$RELAUNCH_REPLACEMENT_PENDING" = 1 ]; then
    ...
    if [ -n "$RELAUNCH_REPLACEMENT_BUSY_GEN" ]; then
      if ! "$DJ_ROOT/bin/dj-busy-event.sh" retire \
  ...
  if [ "$HERDR_PROJECTION_ABORT_CLEANUP" = 1 ]; then
    HERDR_PROJECTION_ABORT_CLEANUP=0
    dj_backend_herdr_projection_cleanup_exact \
      "$HERDR_PROJECTION_ABORT_SESSION" \
      "$HERDR_PROJECTION_ABORT_TASK_PANE" \
  ...
  if [ "$HERDR_PRESENTATION_ORDER_LOCK_HELD" = 1 ]; then
    HERDR_PRESENTATION_ORDER_LOCK_HELD=0
    dj_lock_release "$HERDR_PRESENTATION_ORDER_LOCK" || true
  fi
  if [ "$ORCA_ABORT_CLEANUP" = 1 ]; then
    ORCA_ABORT_CLEANUP=0
    if [ -n "${ORCA_TERMINAL:-}" ]; then
      dj_backend_kill orca "$ORCA_TERMINAL" 2>/dev/null || true
    fi
    if [ -n "${ORCA_WORKTREE_ID:-}" ]; then
      if ! dj_backend_remove_worktree orca "$ORCA_WORKTREE_ID" 2>/dev/null; then
  ...
  if [ "$SPAWN_META_LOCK_HELD" = 1 ]; then
    SPAWN_META_LOCK_HELD=0
    dj_lock_release "$SPAWN_META_LOCK" || true
  fi
  if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
    SPAWN_TASK_SET_LOCK_HELD=0
    dj_lock_release "$SPAWN_TASK_SET_LOCK" || true
  fi
  if [ "$SPAWN_CONTROL_LOCK_HELD" = 1 ]; then
    SPAWN_CONTROL_LOCK_HELD=0
    dj_lock_release "$SPAWN_CONTROL_LOCK" || true
  fi
  [ -z "$SPAWN_META_TMP" ] || rm -f "$SPAWN_META_TMP" 2>/dev/null || true
  if [ "$CONFIG_INHERIT_LOCK_HELD" = 1 ]; then
    CONFIG_INHERIT_LOCK_HELD=0
    dj_lock_release "$CONFIG_INHERIT_LOCK" || true
  fi
  return "$status"
}
...
spawn_herdr_presentation_order_lock_acquire() {
  ...
  lock_path=$(dj_backend_herdr_presentation_session_lock_path "$session") || return 1
  HERDR_PRESENTATION_ORDER_LOCK="$lock_path"
  attempt=0
  while [ "$attempt" -lt 50 ]; do
    if dj_lock_try_acquire "$HERDR_PRESENTATION_ORDER_LOCK"; then
...
clear_relaunch_harness_wiring() {
  ...
  harness=$(dj_control_harness_family "$harness") || harness=
  token_path=$(dj_control_harness_turnend_token_path "$harness" "$state" "$id") || return 1
  token=
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  auth_path=$(dj_control_harness_turnend_auth_path "$harness" "$token") || return 1
  if [ -n "$auth_path" ]; then
...
$(dj_control_harness_wiring_paths "$harness" "$wt" "$state" "$id")
EOF
}
...
# the DJ_ROOT path (not $0) so it works whatever cwd or relative path invoked us, and reuse
...
if [ "${#POS[@]}" -gt 0 ] && [ "${POS[0]}" != "$idpart" ] && case "$idpart" in */*) false ;; *) true ;; esac; then
  ...
  for pair in "${POS[@]}"; do
    ...
    if [ "$KIND" = secondmate ]; then
      ...
      if DJ_SPAWN_NO_GUARD=1 "$DJ_ROOT/bin/dj-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}" --scout; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    else
      if DJ_SPAWN_NO_GUARD=1 "$DJ_ROOT/bin/dj-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}"; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    fi
  done
  exit "$rc"
fi
...
# task is legitimate branch recovery (dj-control drives it through this same
# entrypoint), so only a fresh spawn refuses the branch actor (contract:
# bin/dj-lease-lib.sh; no-op in homes without a branch actor).
# shellcheck source=bin/dj-lease-lib.sh
. "$SCRIPT_DIR/dj-lease-lib.sh"
if [ "$RELAUNCH" -ne 1 ]; then
  dj_lease_forbid_branch "new-task spawn (dj-spawn)"
fi
if [ "$RELAUNCH" -eq 1 ]; then
  SPAWN_CONTROL_LOCK="$STATE/.control-$ID.lock"
  control_owner=$(cat "$SPAWN_CONTROL_LOCK/pid" 2>/dev/null || true)
  if [ "$control_owner" = "$PPID" ] && dj_pid_alive "$control_owner"; then
    SPAWN_CONTROL_PARENT=1
  elif [ "$(dj_lease_actor)" = branch ]; then
    # Role partition refinement: branch recovery relaunches only through the
    # dj-control transaction that owns the control lock, never by invoking
    # this entrypoint directly (contract: bin/dj-lease-lib.sh).
    echo "error: relaunch (dj-spawn) refused - the supervision branch must relaunch through dj-control" >&2
    exit "$DJ_LEASE_REFUSE_EXIT"
  elif dj_lock_try_acquire "$SPAWN_CONTROL_LOCK"; then
...
if [ "$RELAUNCH" -eq 0 ]; then
  ...
  # while never lifecycle-locked (bin/dj-wake-lib.sh's dj_task_set_lock_path
  # owns the evidence; bin/dj-teardown.sh holds the same lock from enumeration
  ...
  SPAWN_TASK_SET_LOCK=$(dj_task_set_lock_path "$STATE") || {
    echo "error: could not resolve the task-set lock for $STATE" >&2
    exit 1
  }
  if ! dj_lock_try_acquire "$SPAWN_TASK_SET_LOCK"; then
...
# Backend selection (data/dj-backend-design-d7): explicit --backend, else
# DJ_BACKEND env, else config/backend, else runtime auto-detection, else
# default tmux (dj_backend_name). dj_backend_validate_spawn refuses unknown or
# non-spawn-capable backends. The resolved value is
# recorded in meta only when it is NOT tmux (dj-teardown.sh and dj-watch.sh's
# window_backend/dj_backend_of_meta already treat an absent backend= as tmux),
# so the default path's meta stays byte-identical.
if [ "$RELAUNCH" -eq 0 ]; then
  if [ "$BACKEND_SET" -eq 1 ]; then
    BACKEND=$BACKEND_ARG
  else
    BACKEND=$(dj_backend_name)
  fi
  dj_backend_validate_spawn "$BACKEND" || exit 1
  dj_backend_source "$BACKEND" || exit 1
  ...
  if [ "$BACKEND" = orca ]; then
    dj_backend_orca_runtime_check || exit 1
  fi
fi
SPAWN_TASK_LOCK="$STATE/.spawn-$ID.lock"
if ! dj_lock_try_acquire "$SPAWN_TASK_LOCK"; then
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  dj_backend_validate_task_endpoint "$RELAUNCH_META" "$ID" || exit 1
  BACKEND=$DJ_BACKEND_VALIDATED_BACKEND
  RELAUNCH_TARGET=$DJ_BACKEND_VALIDATED_TARGET
  dj_backend_validate_spawn "$BACKEND" || exit 1
  dj_backend_source "$BACKEND" || exit 1
  # A relaunch must PROVE the previous agent is gone before it launches another
  # one into the same endpoint, and only tmux and herdr have a recovery-grade
  # classifier that can (bin/dj-control-lib.sh owns that capability table).
  dj_control_backend_state_verified "$BACKEND" || {
    echo "error: backend '$BACKEND' has no recovery-grade agent-state classifier, so a relaunch cannot prove the previous agent exited; refusing rather than risking two agents in one endpoint" >&2
    exit 1
  }
  RELAUNCH_STATE=$(dj_backend_agent_state "$BACKEND" "$RELAUNCH_TARGET")
  [ "$RELAUNCH_STATE" = dead ] || {
    echo "error: task $ID's endpoint reads '$RELAUNCH_STATE'; a relaunch requires a positively agent-free endpoint (stop the agent first with bin/dj-control.sh $ID exit)" >&2
    exit 1
  }
  RELAUNCH_PRIOR_HARNESS=$(dj_meta_get "$RELAUNCH_META" harness)
  KIND=$(dj_meta_get "$RELAUNCH_META" kind)
  [ -n "$KIND" ] || KIND=ship
  MODE=$(dj_meta_get "$RELAUNCH_META" mode)
  YOLO=$(dj_meta_get "$RELAUNCH_META" yolo)
  RELAUNCH_WT=$(dj_meta_get "$RELAUNCH_META" worktree)
  [ -n "$RELAUNCH_WT" ] && [ -d "$RELAUNCH_WT" ] || {
    echo "error: task $ID's recorded worktree '${RELAUNCH_WT:-none}' is missing; refusing to relaunch without the local copy its work lives in" >&2
    exit 1
  }
  if [ "$KIND" = secondmate ]; then
    JARVIS_HOME=$(dj_meta_get "$RELAUNCH_META" home)
    [ -n "$JARVIS_HOME" ] || JARVIS_HOME=$RELAUNCH_WT
  else
    PROJ=$(dj_meta_get "$RELAUNCH_META" project)
    [ -n "$PROJ" ] || {
  ...
  if [ "$BACKEND" = herdr ]; then
    HERDR_SES=$(dj_meta_get "$RELAUNCH_META" herdr_session)
    HERDR_WORKSPACE_ID=$(dj_meta_get "$RELAUNCH_META" herdr_workspace_id)
    HERDR_TAB_ID=$(dj_meta_get "$RELAUNCH_META" herdr_tab_id)
    HERDR_PANE_ID=$(dj_meta_get "$RELAUNCH_META" herdr_pane_id)
  fi
  ...
  # the caller's explicit decision, made with --harness (bin/dj-control.sh
...
launch_template() {
  ...
  case "$harness" in
    ...
    # jarvis captures the pane (see the harness-adapters skill). It is a per-launch env
    # prefix scoped to this jarvis-launched agent; it never touches the captain's
    # global config. The CLI's --prompt-suggestions flag is print/SDK-mode only and
    # does NOT suppress the interactive ghost text (verified empirically), so the env
    # var is the correct control. The dim-aware composer reader in dj-tmux-lib.sh is
    ...
    # SECOND worktree under ~/.cursor/worktrees and would break jarvis's
    ...
    cursor) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u DJ_PI_HARNESS -u CURSOR_INVOKED_AS __CURSORBIN__ --trust --yolo __MODELFLAG__--workspace __WORKTREE__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    ...
    # plugin engine is off in the default build, so jarvis folds muse's own
    # session event log instead (bin/dj-busy-lib.sh), bound by the sidecar
    # written below. Nothing to place in the template for it.
    # codex, opencode, and kimi are also markerless and share this inherited-marker hazard; changing their verified launch boundaries belongs in follow-up work.
    muse) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u DJ_PI_HARNESS XDG_CONFIG_HOME=__MUSECONFIG__ XDG_DATA_HOME=__MUSEDATA__ MUSE_EXPERIMENTAL_FOREIGN_PERSONAL_CONTEXT_KILL=on __MUSEBIN__ --yolo __MODELFLAG____EFFORTFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    *) return 1 ;;
  esac
}
...
case "$ARG3" in
  *' '*)  # raw launch command (unverified-adapter escape hatch)
  ...
  '')
    ...
    # respawn (recovery, /updatejarvis, restart) re-resolves, so
    ...
    if [ "$KIND" = secondmate ]; then
      ...
      HARNESS=$("$DJ_ROOT/bin/dj-harness.sh" crew)
...
# muse is verified as a CREWMATE/SCOUT adapter only. A secondmate is a jarvis
# instance, so it needs a primary supervision protocol; muse has none, and its
# Claude-compatible hook dialect explicitly rejects the model-reawakening and
# asyncRewake handlers that jarvis's primary turn-end supervision is built on
...
case "$HARNESS" in
  pi|pi-signed)
    ...
    LAUNCH="DJ_PI_HARNESS=$HARNESS $LAUNCH"
    ;;
  cursor)
    ...
    CURSOR_BIN=$(dj_cursor_resolve_binary) || exit 1
    if [ -n "$MODEL" ] && [ "$MODEL" != default ]; then
      if CURSOR_MODELS=$(dj_cursor_list_models "$CURSOR_BIN"); then
        if ! printf '%s\n' "$CURSOR_MODELS" | dj_cursor_catalog_has_model "$MODEL"; then
...
if [ "$KIND" = secondmate ] && [ -z "$ARG3" ]; then
  if [ "$MODEL_SET" -eq 0 ]; then
    SM_MODEL=$("$SCRIPT_DIR/dj-harness.sh" secondmate-model)
    [ -z "$SM_MODEL" ] || MODEL=$SM_MODEL
  fi
  if [ "$EFFORT_SET" -eq 0 ]; then
    SM_EFFORT=$("$SCRIPT_DIR/dj-harness.sh" secondmate-effort)
    if [ -n "$SM_EFFORT" ]; then
      case "$SM_EFFORT" in
...
muse_worker_meta_api_key_present() {
  ...
  if [ -n "${TMUX:-}" ]; then
    ...
    tmux has-session -t jarvis 2>/dev/null || return 1
    session=jarvis
  fi
...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    codex)
      ...
      ;;
    grok)
      # grok exposes both --effort and --reasoning-effort; jarvis's profile
    ...
    muse)
      ...
      # ultra is muse's max-CLASS level, so jarvis's max maps onto it - but
      ...
      # none/minimal levels sit below jarvis's shared vocabulary and are
    ...
    # to a different, non-interactive launch mode, so dj-spawn does not pass it.
  ...
  *__MUSEBIN__*)
    ...
    if ! muse_credential_present "$MUSE_AUTH_FILE"; then
      if [ -n "${META_API_KEY:-}" ]; then
        echo "error: muse has no worker-reachable credential; META_API_KEY is set for dj-spawn but cannot be proven present in the $BACKEND worker environment. Store the fleet credential at '$MUSE_AUTH_FILE' with 'muse login' or 'muse auth set --api-key-stdin'. The secret will not be copied into the launch command." >&2
      else
...
case "$LAUNCH" in
  *__KIMIBIN__*)
    ...
    if [ "$KIND" != secondmate ]; then
      "$DJ_ROOT/bin/dj-kimi-turnend-hook.sh" install || {
...
resolved_existing_dir() {
  local path=$1
  [ -d "$path" ] || { echo "error: jarvis home does not exist or is not a directory: $path" >&2; return 1; }
  cd "$path" && pwd -P
}
...
validate_jarvis_home_for_spawn() {
  local id=$1 home=$2 abs_home abs_active_home abs_root marker_id
  abs_home=$(resolved_existing_dir "$home") || return 1
  abs_active_home=$(resolved_existing_dir "$DJ_HOME")
  abs_root=$(resolved_existing_dir "$DJ_ROOT")
  if [ "$abs_home" = "/" ]; then
    echo "error: secondmate home cannot be the filesystem root: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    echo "error: secondmate home cannot be the active jarvis home: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    echo "error: secondmate home cannot be the jarvis repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    echo "error: secondmate home cannot be inside the active jarvis home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    echo "error: secondmate home cannot be inside the jarvis repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    echo "error: secondmate home cannot be an ancestor of the active jarvis home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    echo "error: secondmate home cannot be an ancestor of the jarvis repo: $home" >&2
    return 1
  fi
  validate_jarvis_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    echo "error: jarvis home $home is not a seeded secondmate home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    echo "error: jarvis home $home is marked for secondmate ${marker_id:-unknown}, expected $id" >&2
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    echo "error: $home is not a jarvis home (missing AGENTS.md)" >&2
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    echo "error: $home is not a jarvis home (missing bin/)" >&2
    return 1
  fi
  printf '%s\n' "$abs_home"
}

validate_jarvis_operational_dirs() {
  ...
  for name in data state config projects; do
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      echo "error: secondmate $name directory cannot be inside the active jarvis home: $dir" >&2
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      echo "error: secondmate $name directory cannot be inside the jarvis repo: $dir" >&2
      return 1
    fi
  done
}
...
if [ "$KIND" = secondmate ]; then
  if [ -z "$JARVIS_HOME" ] && [ -f "$STATE/$ID.meta" ]; then
    JARVIS_HOME=$(grep '^home=' "$STATE/$ID.meta" | cut -d= -f2- || true)
  fi
  if [ -z "$JARVIS_HOME" ]; then
    JARVIS_HOME=$(secondmate_registry_value "$ID" home || true)
  fi
fi

if [ "$KIND" = secondmate ]; then
  [ -n "$JARVIS_HOME" ] || { echo "error: no jarvis home supplied or registered for $ID" >&2; exit 1; }
  PROJ_ABS=$(validate_jarvis_home_for_spawn "$ID" "$JARVIS_HOME")
  if [ -e "$DATA/secondmates.md" ] || [ -L "$DATA/secondmates.md" ]; then
    if ! secondmate_registry_validate_bindings "$DATA/secondmates.md" resolve_path "$ID" "$JARVIS_HOME"; then
  ...
  if sm_primary_head=$(primary_head_commit "$DJ_ROOT"); then
  ...
  if [ "${DJ_SKIP_SECONDMATE_INHERIT:-0}" != 1 ]; then
    CONFIG_INHERIT_LOCK=$(dj_config_inherit_lock_path "$PROJ_ABS") || {
      echo "error: could not resolve secondmate inheritance lock for $PROJ_ABS" >&2
      exit 1
    }
    if ! dj_lock_acquire_wait "$CONFIG_INHERIT_LOCK"; then
    ...
    # surface into this secondmate home (dj-config-inherit-lib.sh).
    DJ_CONFIG_INHERIT_LIVE=1 \
      propagate_secondmate_inheritance "$DJ_HOME" "$PROJ_ABS" "$CONFIG" "$DATA" \
...
# dj-brief.sh records a ship brief's mode as a fixed "Delivery contract: mode=<mode>"
...
if [ "$KIND" = ship ]; then
  ...
  STANDING_MODE=$("$DJ_ROOT/bin/dj-project-mode.sh" --raw "$PROJ_NAME" 2>/dev/null | cut -d' ' -f1) || STANDING_MODE=
...
# Brief/spawn role agreement (Jarvis extension), checked before any endpoint
# exists, mirroring the delivery contract above: a spawn flag that disagrees
# with the brief's recorded role is refused, and a brief-recorded role is
# adopted when the flag is absent, so the worker's instructions and the spawn
# cannot drift apart. Untyped tasks (no flag, no recorded line) stay legal.
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  BRIEF_ROLE=$(dj_role_from_brief "$BRIEF")
  if [ "$ROLE_SET" -eq 1 ]; then
    dj_role_valid "$ROLE" || { echo "error: --role must be one of $(dj_role_list | tr -d '\n') (got '$ROLE')" >&2; exit 1; }
    if [ -n "$BRIEF_ROLE" ] && [ "$BRIEF_ROLE" != "$ROLE" ]; then
      echo "error: role mismatch for $ID: the brief says role=$BRIEF_ROLE but this spawn passed --role $ROLE; correct the flag or re-scaffold the brief so the worker's instructions and the task record agree" >&2
      exit 1
    fi
  elif [ -n "$BRIEF_ROLE" ]; then
    dj_role_valid "$BRIEF_ROLE" || { echo "error: brief for $ID records an unknown role '$BRIEF_ROLE'; re-scaffold it with a valid --role" >&2; exit 1; }
    ROLE=$BRIEF_ROLE
    ROLE_SET=1
  fi
  if [ "$ROLE_SET" -eq 1 ] && dj_role_scout_only "$ROLE" && [ "$KIND" = ship ]; then
    echo "error: role '$ROLE' is knowledge-only - its deliverable is a report, never a merge; spawn it with --scout" >&2
    exit 1
  fi
fi
...
# per-backend routing (dj_backend_resolve_selector).
...
herdr_projection_existing_meta_allows_flat() {  # <meta>
  ...
  old_backend=$(dj_backend_of_meta "$meta")
  old_target=$(dj_backend_target_of_meta "$meta")
  [ -n "$old_target" ] || {
  ...
  if [ "$old_backend" = herdr ]; then
    dj_backend_herdr_parse_target "$old_target" || {
      echo "error: existing herdr endpoint for $ID is malformed; refusing duplicate launch" >&2
      return 1
    }
    target_session=$DJ_BACKEND_HERDR_SESSION
    target_pane=$DJ_BACKEND_HERDR_PANE
    ...
    dj_backend_herdr_server_ensure "$old_session" || {
      echo "error: existing herdr endpoint for $ID could not be inspected; refusing duplicate launch" >&2
      return 1
    }
    old_state=$(dj_backend_herdr_pane_agent_state "$old_session" "$old_pane")
    case "$old_state" in
  ...
  old_state=$(dj_backend_agent_alive "$old_backend" "$old_target")
  case "$old_state" in
    dead) return 0 ;;
    alive|unknown)
...
case "$BACKEND" in
  tmux)
    SES=$(dj_backend_tmux_container_ensure)
    T="$SES:$W"
    # #134 robustness (tmux): dj_backend_tmux_create_task captures a stable window
    # id and pins the window name (automatic-rename/allow-rename off) so a captain's
    # non-default tmux config cannot rename the window away from dj-<id> once
    # treehouse cd's into the worktree. WT_TARGET carries that stable id for the
    # rename-critical worktree-detection steps below; the persisted window= handle
    # stays $T (the name form), which is safe now that rename is disabled.
    WID=$(dj_backend_tmux_create_task "$SES" "$W" "$PROJ_ABS") || exit 1
    WT_TARGET="$WID"
    ;;
  herdr)
    # dj_backend_herdr_workspace_label resolves the target workspace from
    # DJ_HOME. For every KIND except secondmate, this process's own DJ_HOME is
    ...
    # DJ_HOME - the latter needs no glue at all). A --secondmate spawn is the
    # one case that does: it is the PRIMARY's own dj-spawn.sh process
    # launching a DIFFERENT home (PROJ_ABS, already validated above as the
    # secondmate's home), so DJ_HOME here still names the primary. Shadow it
    # to PROJ_ABS for just these two calls (bash restores it automatically
    # after each prefixed simple-command call) so the secondmate's tab lands
    # in the secondmate's own workspace, not the primary's "jarvis" one.
    #
    ...
    HERDR_LABEL_HOME=$DJ_HOME
    HERDR_LAUNCHER_RELATIONSHIP=launcher-home
    if [ "$KIND" = secondmate ]; then
      HERDR_LABEL_HOME=$PROJ_ABS
      HERDR_LAUNCHER_RELATIONSHIP=other-home
    fi
    HERDR_PRESENTATION_JOURNAL=$(dj_backend_herdr_projection_journal_path "$STATE" "$ID")
    HERDR_PROJECTED=0
    if [ "$KIND" != secondmate ] && dj_backend_herdr_presentation_enabled "$CONFIG" "$STATE"; then
      HERDR_SES=$(dj_backend_herdr_session)
      HERDR_PARENT_LABEL=$(DJ_HOME="$HERDR_LABEL_HOME" dj_backend_herdr_workspace_label)
      if [ -e "$HERDR_PRESENTATION_JOURNAL" ] || [ -L "$HERDR_PRESENTATION_JOURNAL" ]; then
        dj_backend_herdr_server_ensure "$HERDR_SES" || {
        ...
        dj_backend_herdr_projection_recovery_allows_flat \
          "$HERDR_SES" "$HERDR_PRESENTATION_JOURNAL" "$ID" || exit 1
        if [ "${HERDR_RECOVERY_BACKEND:-}" = herdr ]; then
          set +e
          DJ_HOME="$HERDR_LABEL_HOME" dj_backend_herdr_projection_reclaim_task \
          ...
          case "$HERDR_RECLAIM_STATUS" in
            0)
              ...
              HERDR_TAB_ID=$DJ_BACKEND_HERDR_PROJECTION_TAB_ID
              HERDR_PANE_ID=$DJ_BACKEND_HERDR_PROJECTION_PANE_ID
        ...
        if ! dj_backend_herdr_server_ensure "$HERDR_SES"; then
          echo "warning: herdr presentation could not ensure its session server; using the ordinary flat layout without projection" >&2
        elif [ "${DJ_BACKEND_HERDR_PRESENTATION_PREFERENCE:-default}" = default ] \
          && ! dj_backend_herdr_presentation_default_supported "$STATE" "$HERDR_SES"; then
          :
          ...
          set +e
          dj_backend_herdr_launcher_identity "$HERDR_SES"
          HERDR_LAUNCHER_STATUS=$?
          set -e
          case "$HERDR_LAUNCHER_STATUS" in
            0) HERDR_PARENT_WORKSPACE_ID=$DJ_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID ;;
            2) HERDR_PARENT_WORKSPACE_ID=$(dj_backend_herdr_projection_parent_workspace_exact \
          ...
          if [ -z "$HERDR_PARENT_WORKSPACE_ID" ]; then
            ...
            HERDR_PROJECTION_ID=$(dj_backend_herdr_projection_journal_create "$STATE" "$ID") || exit 1
            HERDR_PROJECTION_LABEL=$(dj_backend_herdr_projection_workspace_label "$ID" "$HERDR_PROJECTION_ID")
            if ! DJ_HOME="$HERDR_LABEL_HOME" dj_backend_herdr_projection_create_task \
              "$PROJ_ABS" "$HERDR_PROJECTION_LABEL" "$W"; then
              if [ "${DJ_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE:-0}" = 1 ]; then
                HERDR_PROJECTION_ABORT_CLEANUP=1
                HERDR_PROJECTION_ABORT_SESSION=$DJ_BACKEND_HERDR_PROJECTION_SESSION
                HERDR_PROJECTION_ABORT_TASK_PANE=$DJ_BACKEND_HERDR_PROJECTION_PANE_ID
                HERDR_PROJECTION_ABORT_SEEDED_PANE=$DJ_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID
              fi
              exit 1
            fi
            HERDR_PROJECTED=1
            HERDR_SES=$DJ_BACKEND_HERDR_PROJECTION_SESSION
            HERDR_WORKSPACE_ID=$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID
            HERDR_SEEDED_DEFAULT_TAB_ID=$DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID
            HERDR_TAB_ID=$DJ_BACKEND_HERDR_PROJECTION_TAB_ID
            HERDR_PANE_ID=$DJ_BACKEND_HERDR_PROJECTION_PANE_ID
            HERDR_PROJECTION_ABORT_CLEANUP=1
            HERDR_PROJECTION_ABORT_SESSION=$HERDR_SES
            HERDR_PROJECTION_ABORT_TASK_PANE=$HERDR_PANE_ID
            HERDR_PROJECTION_ABORT_SEEDED_PANE=$DJ_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID
            dj_backend_herdr_projection_order_best_effort \
              "$HERDR_SES" "$HERDR_WORKSPACE_ID" "$HERDR_PARENT_LABEL" "$HERDR_PARENT_WORKSPACE_ID"
            HERDR_HOME_ID=$(dj_backend_herdr_projection_home_identity "$HERDR_LABEL_HOME" 2>/dev/null || true)
            if [ -n "$HERDR_HOME_ID" ] \
    ...
    if [ "$HERDR_PROJECTED" -ne 1 ]; then
      HERDR_CONTAINER_RAW=$(DJ_HOME="$HERDR_LABEL_HOME" dj_backend_herdr_container_ensure "$PROJ_ABS" "$HERDR_LAUNCHER_RELATIONSHIP") || exit 1
      # dj_backend_herdr_container_ensure echoes "<session>:<workspace_id>\t<seeded_default_tab_id>"
      ...
      HERDR_TASK_IDS=$(DJ_HOME="$HERDR_LABEL_HOME" dj_backend_herdr_create_task "$CONTAINER" "$W" "$PROJ_ABS" "$HERDR_SEEDED_DEFAULT_TAB_ID") || exit 1
...
EOF
    ...
    ;;
  zellij)
    ZELLIJ_SES=$(dj_backend_zellij_container_ensure) || exit 1
    ZELLIJ_TASK_IDS=$(dj_backend_zellij_create_task "$ZELLIJ_SES" "$W" "$PROJ_ABS") || exit 1
...
EOF
    ...
    ;;
  cmux)
    dj_backend_cmux_container_ensure || exit 1
    CMUX_TASK_IDS=$(dj_backend_cmux_create_task "$W" "$PROJ_ABS") || exit 1
...
EOF
  ...
  orca)
    set +e
    ORCA_WT_RAW=$(dj_backend_orca_worktree_create "$PROJ_ABS" "$W")
    ORCA_WT_STATUS=$?
    set -e
    ...
    if [ -z "$ORCA_TERMINAL" ]; then
      ORCA_TERMINAL=$(dj_backend_orca_terminal_create "$ORCA_WORKTREE_ID" "$W") || exit 1
    fi
    T="$ORCA_TERMINAL"
    ;;
esac
...
if [ "$KIND" = secondmate ]; then
  DJ_INHERITABLE_CONFIG=trace-context \
...
spawn_send_text_line() {  # <target> <text>
  case "$BACKEND" in
    tmux) dj_backend_tmux_send_text_line "$1" "$2" ;;
    herdr) dj_backend_herdr_send_text_line "$1" "$2" ;;
    zellij) dj_backend_zellij_send_text_line "$1" "$2" "$W" ;;
    orca) dj_backend_orca_send_text_line "$1" "$2" ;;
    cmux) dj_backend_cmux_send_text_line "$1" "$2" "$W" ;;
  esac
}
spawn_current_path() {  # <target>
  case "$BACKEND" in
    tmux) dj_backend_tmux_current_path "$1" ;;
    herdr) dj_backend_herdr_current_path "$1" ;;
    zellij) dj_backend_zellij_current_path "$1" "$W" ;;
    cmux) dj_backend_cmux_current_path "$1" "$W" ;;
  esac
}
spawn_send_literal() {  # <target> <text>
  case "$BACKEND" in
    tmux) dj_backend_tmux_send_literal "$1" "$2" ;;
    herdr) dj_backend_herdr_send_literal "$1" "$2" ;;
    zellij) dj_backend_zellij_send_literal "$1" "$2" "$W" ;;
    orca) dj_backend_orca_send_literal "$1" "$2" ;;
    cmux) dj_backend_cmux_send_literal "$1" "$2" "$W" ;;
  esac
}
spawn_send_key() {  # <target> <key>
  case "$BACKEND" in
    tmux) dj_backend_tmux_send_key "$1" "$2" ;;
    herdr) dj_backend_herdr_send_key "$1" "$2" ;;
    zellij) dj_backend_zellij_send_key "$1" "$2" "$W" ;;
    orca) dj_backend_orca_send_key "$1" "$2" ;;
    cmux) dj_backend_cmux_send_key "$1" "$2" "$W" ;;
  esac
}

kimi_capture() {
  dj_backend_capture "$BACKEND" "$T" 120 "$W" 2>/dev/null || true
}

# Kimi launch-readiness and delivery route their composer-emptiness half
# through the shared classifier (bin/dj-composer-lib.sh via
# dj_backend_composer_state), the same owner every steer and injection guard
...
kimi_composer_is_empty() {
  [ "$(dj_backend_composer_state "$BACKEND" "$T" "$W" 2>/dev/null)" = empty ]
}

kimi_wait_for_ready() {
  local pane i=0 max=${DJ_KIMI_READY_POLLS:-60} interval=${DJ_KIMI_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(kimi_capture)
...
kimi_wait_for_delivery() {
  local pane i=0 max=${DJ_KIMI_DELIVERY_POLLS:-40} interval=${DJ_KIMI_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(kimi_capture)
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # active client's window, which would misread jarvis's OWN pane path as the
...
# Per-task temp root: /tmp/dj-<id>/ with Go's build temp nested at gotmp/. Go won't
# create GOTMPDIR, so mkdir before it is used; dj-teardown removes the whole root.
# Nested (not a bare /tmp/dj-<id>/gotmp) so other per-task temp can live alongside
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # (bin/dj-control-lib.sh owns where those artifacts live).
...
if [ "$KIND" != secondmate ]; then
  # Arm the semantic busy-state contract (bin/dj-busy-lib.sh) for every
  # adapter with a verified semantic source. The launch brief sent below IS a
  # submitted turn, so the seed record is busy/dj-spawn. The minted gen is
  ...
  # fallback and standalone Kimi stays unknown until dj_busy_kimi_verified
  # opens, so neither is armed here.
  BUSY_GEN=
  case "$HARNESS" in
    codex*)
      if dj_busy_codex_semantic_source; then
  ...
  case "$HARNESS" in
    claude*|opencode*|pi|pi-signed)
      BUSY_GEN=$("$DJ_ROOT/bin/dj-busy-event.sh" arm "$STATE_REAL" "$ID") || {
      ...
      ;;
    kimi*)
      # Standalone Kimi stays unknown until dj_busy_kimi_verified opens on a
      # live-verified installed version (bin/dj-busy-lib.sh owns the gate and
  ...
  case "$HARNESS" in
    claude*)
      # Semantic busy-state hooks (bin/dj-busy-lib.sh): UserPromptSubmit opens
      ...
      # interrupt: dj-control preserves the adapter-owned state, while the
      # legacy dj-send --key Escape path records idle/dj-interrupt. Stop keeps
      # the turn-ended NOTIFICATION touch for the watcher. Every
      # hook command tolerates a refused event (|| true) so a stale-gen writer
      # can never break Claude's own lifecycle.
      mkdir -p "$WT/.claude"
      busy_cmd_prefix="$(shell_quote "$DJ_ROOT/bin/dj-busy-event.sh") apply $(shell_quote "$STATE_REAL") $(shell_quote "$ID")"
...
EOF
    ...
    opencode*)
      ...
      cat > "$WT/.opencode/plugins/dj-busy-state.js" <<EOF
// Jarvis semantic busy-state events + turn-end notification; written by
// dj-spawn under the contract owned by bin/dj-busy-lib.sh.
...
const busyEvent = (state, event) =>
  new Promise((resolve) => {
    execFile("$DJ_ROOT/bin/dj-busy-event.sh", [
...
EOF
      exclude_path '.opencode/plugins/dj-busy-state.js'
      ;;
    pi|pi-signed)
...
EOF
      ;;
    codex*)
      # Semantic busy-state source negotiation (bin/dj-busy-lib.sh owns the
      ...
      # jarvis-launched worker. Codex therefore classifies unknown with
    ...
    grok*)
      ...
      # jarvis cannot establish at launch without editing grok's own managed
      ...
      # lives OUTSIDE the worktree as a single jarvis-owned global hook that is a
      # guarded no-op for every non-jarvis grok session: it fires only when the
      # current workspace holds a .dj-grok-turnend token pointer that matches the
      # jarvis-owned hook registry. jarvis then drops that per-task pointer
      ...
      # touches grok's managed config - only jarvis-owned files.
      GROK_HOOKS_DIR="${GROK_HOME:-$HOME/.grok}/hooks"
      GROK_AUTH_DIR="$GROK_HOOKS_DIR/dj-turn-end.d"
      ...
      cat > "$GROK_HOOKS_DIR/dj-turn-end.sh" <<EOF
#!/usr/bin/env bash
set -u
auth_dir=$sq_grok_auth_dir
workspace=\${GROK_WORKSPACE_ROOT:-}
[ -n "\$workspace" ] || exit 0
p="\$workspace/.dj-grok-turnend"
[ -f "\$p" ] || exit 0
first=
...
exit 0
EOF
      chmod +x "$GROK_HOOKS_DIR/dj-turn-end.sh"
      hook_command=$(json_escape "bash $(shell_quote "$GROK_HOOKS_DIR/dj-turn-end.sh")")
      printf '{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"%s"}]}]}}\n' "$hook_command" > "$GROK_HOOKS_DIR/dj-turn-end.json"
      printf 'token=%s\n' "${auth_file##*/}" > "$WT/.dj-grok-turnend"
      exclude_path '.dj-grok-turnend'
      ;;
    muse*)
      ...
      # jarvis reads muse's own durable session event log instead
      # (bin/dj-busy-lib.sh owns the fold). That is a PULL
...
$(dj_busy_muse_matching_logs "$MUSE_SESSIONS_ROOT" "$WT" || true)
EOF
    ...
    cursor*)
      ...
      # there (bin/dj-busy-lib.sh owns the fold). Like muse that is a PULL
      # source with no writer, so nothing is armed and no record is seeded.
      ...
      CURSOR_PROJECTS_ROOT="${CURSOR_PROJECTS_ROOT_OVERRIDE:-$HOME/.cursor/projects}"
      {
        ...
        printf 'workspace_root=%s\n' "$WT"
        if CURSOR_PRIOR_PROJECT=$(dj_busy_cursor_project_dir "$CURSOR_PROJECTS_ROOT" "$WT" 2>/dev/null); then
    ...
    kimi*)
      ...
      # task's token pointer and the token resolves through Jarvis's private
      # registry. The installer above owns the format-preserving config edit and
      # the always-zero, silent hook script.
      KIMI_AUTH_DIR="$HOME/.kimi-code/dj-turn-end.d"
      old_umask=$(umask)
      umask 077
      ...
      printf 'token=%s\n' "${auth_file##*/}" > "$WT/.dj-kimi-turnend"
      exclude_path '.dj-kimi-turnend'
      ;;
  esac
fi

# Delivery posture recorded in meta so dj-teardown's safety check and the
...
# (dj-teardown.sh defaults an absent mode to no-mistakes, and dj-promote.sh
...
# Resolve the optional default-off W3C trace context (bin/dj-trace-context-lib.sh,
...
if [ "$TRACEPARENT_SET" -eq 1 ]; then
  ...
  SPAWN_TRACE_EFFECTIVE=$(dj_trace_context_session_effective "$STATE/.trace-context-effective")
  if [ "$SPAWN_TRACE_EFFECTIVE" = on ]; then
    SPAWN_TRACEPARENT=$(DJ_TRACE_CONTEXT=on dj_trace_context_resolve "$CONFIG" "$STATE/$ID.meta" || true)
  else
    SPAWN_TRACEPARENT=
  fi
fi
...
if [ "$RELAUNCH" -eq 1 ]; then
  SPAWN_META_LOCK=$(dj_meta_lock_path "$STATE/$ID.meta") || exit 1
  dj_lock_acquire_wait "$SPAWN_META_LOCK"
...
}
{
  ...
  # data/dj-backend-design-d7's P1 compatibility contract).
  ...
  if [ "$SPAWN_CONTROL_PARENT" = 1 ] && [ -n "${DJ_CONTROL_RELAUNCH_TX:-}" ]; then
    echo "control_relaunch_tx=$DJ_CONTROL_RELAUNCH_TX"
  fi
} > "$SPAWN_META_PATH"
if [ "$RELAUNCH" -eq 1 ]; then
  SPAWN_META_PUBLISH_STARTED=1
  ...
  SPAWN_META_PUBLISH_STARTED=0
  SPAWN_META_TMP=
  dj_lock_release "$SPAWN_META_LOCK"
  SPAWN_META_LOCK_HELD=0
fi
if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
...
sq_piturnend=$(shell_quote "$PROJ_ABS/.pi/extensions/dj-primary-turnend-guard.ts")
sq_piwatch=$(shell_quote "$PROJ_ABS/.pi/extensions/dj-primary-pi-watch.ts")
sq_opinput=$(shell_quote "$DJ_ROOT/bin/dj-operational-input.sh")
...
# inherit jarvis's current environment, so a bare `claude` in the pane falls
# back to the default ~/.claude store even when jarvis itself runs under a
# different CLAUDE_CONFIG_DIR (for example a work-vs-personal subscription split).
# Forward jarvis's own resolved store onto the claude launch so the crewmate
# uses the same credential/config jarvis is authenticated with. Only when set;
...
if [ "$KIND" = secondmate ]; then
  ...
  sq_primary_home=$(shell_quote "$DJ_HOME")
  # Keep this in step with dj_supervision_model (bin/dj-wake-lib.sh): Claude's
  ...
  # literal (never the raw DJ_TRACE_CONTEXT string) so a DJ_TRACE_CONTEXT override
  ...
  # not enable them across the launch boundary (bin/dj-trace-context-lib.sh header).
  # Reuse the single frozen decision from the carrier resolution above so the
  # injected carrier and this on/off snapshot are guaranteed to agree.
  LAUNCH="DJ_ROOT_OVERRIDE= DJ_STATE_OVERRIDE= DJ_DATA_OVERRIDE= DJ_PROJECTS_OVERRIDE= DJ_CONFIG_OVERRIDE= DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME=$sq_primary_home DJ_HOME=$sq_home DJ_TRACE_CONTEXT=$SPAWN_TRACE_EFFECTIVE DJ_SUPERVISION_MODEL=$supervision_model $LAUNCH"
fi
...
spawn_record_traceparent() {
  ...
  SPAWN_META_LOCK=$(dj_meta_lock_path "$meta") || return 1
  dj_lock_acquire_wait "$SPAWN_META_LOCK"
  SPAWN_META_LOCK_HELD=1
  ...
  SPAWN_META_TMP=
  dj_lock_release "$SPAWN_META_LOCK" || status=1
  SPAWN_META_LOCK_HELD=0
  return "$status"
}
...
if [ "$HARNESS" = kimi ]; then
  ...
  KIMI_SUBMIT_RETRIES=${DJ_KIMI_SUBMIT_RETRIES:-3}
  KIMI_SUBMIT_SLEEP=${DJ_KIMI_SUBMIT_SLEEP:-${DJ_KIMI_POLL_INTERVAL:-0.5}}
  KIMI_SUBMIT_SETTLE=${DJ_KIMI_SUBMIT_SETTLE:-0}
  KIMI_SUBMIT_VERDICT=$(dj_backend_send_text_submit \
...
if [ "$KIND" = secondmate ] && [ "${DJ_SKIP_SECONDMATE_INHERIT:-0}" != 1 ]; then
  if ! dj_config_reread_discard_pending "$PROJ_ABS" "$ID" "$DJ_HOME"; then
    if dj_config_reread_quarantine_pending "$PROJ_ABS" "$ID" "$DJ_HOME"; then
      echo "CONFIG_REREAD: secondmate $ID: quarantined pre-relaunch generations after cleanup failure (destination=$PROJ_ABS/state/.dj-inherited-config-reread-quarantine source=$DJ_HOME/state/.dj-inherited-config-reread-quarantine)" >&2
    else
      echo "CONFIG_REREAD: secondmate $ID: cleanup failed; pre-relaunch generations were force-cleared where possible (destination=$PROJ_ABS source=$DJ_HOME)" >&2
    fi
  fi
fi
