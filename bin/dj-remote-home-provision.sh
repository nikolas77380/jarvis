#!/usr/bin/env bash
# Provision the DJ_HOME selected by the fixed remote entrypoint.
#
# Usage:
#   dj-remote-home-provision.sh < manifest
#
# Manifest schema dj-remote-home-provision.v1 carries a base64 charter, the
...
# from it and re-validates it through bin/dj-project-origin-lib.sh instead of
...
# published, the durable .dj-secondmate-parent record names this home's route to its parent as
# "remote" - read by bin/dj-teardown.sh's cleanup gate so a delegated public
...
# .dj-secondmate-home marker commits the complete seed last.
...
DJ_HOME=${DJ_HOME:?DJ_HOME is required}
MAX_MANIFEST_BYTES=1048576

# shellcheck source=bin/dj-project-origin-lib.sh
. "$SCRIPT_DIR/dj-project-origin-lib.sh"
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/dj-remote-provision.XXXXXX") || die "cannot create provisioning state"
CREATED_HOME=0
CREATED_BACKLOG=0
EXISTING_HOME=0
PUBLISHED=0
...
release_provision_lock() {
  if [ "$PROVISION_LOCK_HELD" -eq 1 ]; then
    dj_lock_release "$PROVISION_LOCK"
    PROVISION_LOCK_HELD=0
  fi
}
restore_owned_file() { # <relative-path>
  local rel=$1 dest="$DJ_HOME/$1" backup="$TMP/before/$1"
  ...
  if [ "$status" -ne 0 ] && [ "$PUBLISHED" -eq 0 ]; then
    if [ "$CREATED_HOME" -eq 1 ]; then
      ...
      while IFS= read -r project; do
        [ -n "$project" ] && rm -rf -- "$DJ_HOME/projects/$project"
      done < "$CREATED_PROJECTS"
      restore_owned_file data/charter.md || true
      restore_owned_file data/projects.md || true
      restore_owned_file .dj-secondmate-home || true
      restore_owned_file .dj-secondmate-parent || true
      [ "$CREATED_BACKLOG" -eq 0 ] || rm -f -- "$DJ_HOME/data/backlog.md"
    fi
  fi
  release_provision_lock
  rm -rf -- "$TMP"
...
[ "$SCHEMA" = dj-remote-home-provision.v1 ] || die "incompatible provisioning manifest"
...
PROVISION_LOCK_STATE="$HOME_PARENT/.jarvis-provision-locks"
...
if command -v shasum >/dev/null 2>&1; then
  HOME_LOCK_KEY=$(printf '%s' "$DJ_HOME" | shasum -a 256 | awk '{print $1}')
elif command -v sha256sum >/dev/null 2>&1; then
  HOME_LOCK_KEY=$(printf '%s' "$DJ_HOME" | sha256sum | awk '{print $1}')
else
  die "no SHA-256 tool is available for provisioning serialization"
fi
DJ_STATE_OVERRIDE="$PROVISION_LOCK_STATE"
...
dj_lock_acquire_wait "$PROVISION_LOCK"
PROVISION_LOCK_HELD=1

if [ -e "$DJ_HOME" ] || [ -L "$DJ_HOME" ]; then
  [ -d "$DJ_HOME" ] && [ ! -L "$DJ_HOME" ] || die "remote home exists but is not a safe directory"
  [ -f "$DJ_HOME/AGENTS.md" ] && [ ! -L "$DJ_HOME/AGENTS.md" ] \
    && [ -d "$DJ_HOME/bin" ] && [ ! -L "$DJ_HOME/bin" ] || die "existing remote home is not a safe Jarvis checkout"
  for operational_dir in data state config projects; do
    operational_path="$DJ_HOME/$operational_dir"
  ...
  for rel in data/charter.md data/projects.md .dj-secondmate-home .dj-secondmate-parent; do
    existing="$DJ_HOME/$rel"
  ...
  if [ -f "$DJ_HOME/.dj-secondmate-home" ]; then
    [ "$(cat "$DJ_HOME/.dj-secondmate-home")" = "$ID" ] || die "existing remote home belongs to another secondmate"
  elif find "$DJ_HOME/data" "$DJ_HOME/state" "$DJ_HOME/projects" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null | grep -q .; then
    die "unmarked existing remote home contains operational data"
  fi
else
  CREATED_HOME=1
  git clone --quiet -- "$DJ_ROOT" "$DJ_HOME" || die "could not clone the remote Jarvis home"
fi
for operational_dir in data state config projects; do
  operational_path="$DJ_HOME/$operational_dir"
  if [ -e "$operational_path" ] || [ -L "$operational_path" ]; then
...
if [ -e "$DJ_HOME/data/backlog.md" ] || [ -L "$DJ_HOME/data/backlog.md" ]; then
  [ -f "$DJ_HOME/data/backlog.md" ] && [ ! -L "$DJ_HOME/data/backlog.md" ] \
    || die "remote backlog is not a safe regular file"
else
  printf '## In flight\n\n## Queued\n\n## Done\n' > "$DJ_HOME/data/backlog.md"
  CREATED_BACKLOG=1
fi
...
EOF
  ...
  dj_project_origin_safe "$ORIGIN" || die "project $NAME origin is not an accepted clone URL: $ORIGIN"
...
cp "$TMP/charter" "$DJ_HOME/data/charter.md.tmp.$$"
chmod 600 "$DJ_HOME/data/charter.md.tmp.$$"
mv -f -- "$DJ_HOME/data/charter.md.tmp.$$" "$DJ_HOME/data/charter.md"
cp "$PROJECT_REG" "$DJ_HOME/data/projects.md.tmp.$$"
mv -f -- "$DJ_HOME/data/projects.md.tmp.$$" "$DJ_HOME/data/projects.md"
{
  printf 'schema=dj-secondmate-parent.v1\n'
  printf 'route=remote\n'
  [ -z "$PARENT_HOST" ] || printf 'parent_host=%s\n' "$PARENT_HOST"
} > "$DJ_HOME/.dj-secondmate-parent.tmp.$$"
mv -f -- "$DJ_HOME/.dj-secondmate-parent.tmp.$$" "$DJ_HOME/.dj-secondmate-parent"
printf '%s\n' "$ID" > "$DJ_HOME/.dj-secondmate-home.tmp.$$"
mv -f -- "$DJ_HOME/.dj-secondmate-home.tmp.$$" "$DJ_HOME/.dj-secondmate-home"
PUBLISHED=1
release_provision_lock
trap - EXIT
rm -rf -- "$TMP"
