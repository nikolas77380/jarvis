#!/usr/bin/env bash
# dj-decision-hold.sh - transitional compatibility shim over bin/dj-captain-hold.sh.
#
# The separate "decision" concept collapsed into the one primitive the captain
# cares about: a task held for the captain. bin/dj-captain-hold.sh owns every
...
CAPTAIN_HOLD="$SCRIPT_DIR/dj-captain-hold.sh"
...
task_show() {
  (cd "$DJ_HOME" && tasks-axi show "$1" --full) 2>/dev/null
}
...
command_resolve() {
  ...
  case "$hold_body" in
    *"Resolution recorded by dj-decision-hold."*"Routed identities: "*)
      ...
      legacy_replay=1
      ;;
    *"Resolution recorded by dj-captain-hold."*)
      resolution_recorded=1
      ;;
  esac
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/dj-decision-hold-resolve.XXXXXX") \
  ...
  for dep in $routed; do
    ...
    if list_has_key "$(normalized_blocked_by "$show")" "$id"; then
      (cd "$DJ_HOME" && tasks-axi unblock "$dep" --by "$id" >/dev/null) \
