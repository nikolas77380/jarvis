#!/usr/bin/env bash
# dj-lint.sh - the single owner of jarvis's lint definition.
#
...
# The default (no explicit-path) path also runs bin/dj-lint-workflows.sh so a
...
# deterministic shard and root order after every worker finishes. DJ_LINT_JOBS=1
...
#
# Usage:
#   dj-lint.sh                         lint the context-selected file set (see above)
#   dj-lint.sh --fast [path]...       local lint with extended analysis disabled
#   dj-lint.sh <path>...               lint explicit roots with the same config
#   dj-lint.sh --jobs <1|2> [path]...  override bounded worker count
#   dj-lint.sh --telemetry <path> ...  write a quiet metrics snapshot
#   dj-lint.sh --required-version      print the ShellCheck pin
#   dj-lint.sh --list-files            print the file set that would be linted
#   dj-lint.sh --help                  print this usage
set -u

REQUIRED_SHELLCHECK=0.11.0
SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SELF="$SELF_DIR/dj-lint.sh"
ROOT="$(cd "$SELF_DIR/.." && pwd)"
cd "$ROOT" || exit 1

DJ_LINT_WORKER_SHELLCHECK_PID=
# shellcheck disable=SC2329 # Registered by the private worker's signal traps.
dj_lint_worker_stop() {
  [ -n "$DJ_LINT_WORKER_SHELLCHECK_PID" ] || return 0
  kill "$DJ_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  wait "$DJ_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  DJ_LINT_WORKER_SHELLCHECK_PID=
}

dj_lint_worker() {  # <manifest> <output-dir> <shard-index>
  ...
  if [ "${#roots[@]}" -gt 0 ]; then
    trap 'dj_lint_worker_stop; exit 129' HUP
    trap 'dj_lint_worker_stop; exit 130' INT
    trap 'dj_lint_worker_stop; exit 143' TERM
    shellcheck_args=(--norc --external-sources)
    if [ "${DJ_LINT_INTERNAL_FAST:-0}" -eq 1 ]; then
      shellcheck_args+=(--extended-analysis=false)
    fi
    "$DJ_LINT_SHELLCHECK" "${shellcheck_args[@]}" -- "${roots[@]}" > "$output.out" 2>&1 &
    DJ_LINT_WORKER_SHELLCHECK_PID=$!
    wait "$DJ_LINT_WORKER_SHELLCHECK_PID" || rc=$?
    DJ_LINT_WORKER_SHELLCHECK_PID=
    trap - HUP INT TERM
  else
    : > "$output.out"
...
if [ "${1:-}" = "--internal-worker" ]; then
  [ "${DJ_LINT_INTERNAL:-}" = 1 ] || {
    printf 'dj-lint.sh: --internal-worker is private to the lint owner.\n' >&2
    exit 2
  }
  [ "$#" -eq 4 ] && [ -n "${DJ_LINT_SHELLCHECK:-}" ] || exit 2
  dj_lint_worker "$2" "$3" "$4"
  exit $?
fi
...
dj_lint_run_workflows() {
  [ "$EXPLICIT_PATHS" -eq 0 ] || return 0
  "$SELF_DIR/dj-lint-workflows.sh"
}

JOBS=${DJ_LINT_JOBS:-2}
TELEMETRY=${DJ_LINT_TELEMETRY:-}
FAST=0
ANALYSIS_MODE=full
LIST_FILES=0
while [ "$#" -gt 0 ]; do
  case "$1" in
    --jobs)
      [ "$#" -ge 2 ] || { printf 'dj-lint.sh: --jobs requires 1 or 2.\n' >&2; exit 2; }
      JOBS=$2
      shift 2
      ;;
    --jobs=*)
      ...
      ;;
    --telemetry)
      [ "$#" -ge 2 ] || { printf 'dj-lint.sh: --telemetry requires a path.\n' >&2; exit 2; }
      TELEMETRY=$2
      shift 2
      ;;
    --telemetry=*)
...
case "$JOBS" in
  1|2) ;;
  *) printf 'dj-lint.sh: jobs must be 1 or 2, got %s.\n' "$JOBS" >&2; exit 2 ;;
esac

if [ "$FAST" -eq 1 ] && { [ "${GITHUB_ACTIONS:-}" = true ] || [ "${CI:-}" = true ]; }; then
  printf 'dj-lint.sh: --fast is local-only; CI uses full ShellCheck analysis.\n' >&2
  exit 2
fi

# dj_lint_changed_base_ref prints the ref to diff the working branch against:
...
# dj_lint_is_canonical_root tests membership in the canonical set (a direct
...
if [ "$#" -gt 0 ]; then
  ...
  if [ "${GITHUB_ACTIONS:-}" != true ] && [ "${CI:-}" != true ] \
    ...
    base_ref=$(dj_lint_changed_base_ref) || base_ref=
    merge_base=
  ...
  if [ "$full_lint" -eq 1 ]; then
    ...
    while IFS= read -r -d '' changed_path; do
      dj_lint_is_canonical_root "$changed_path" || continue
...
if [ "$LIST_FILES" -eq 1 ]; then
  [ "$#" -eq 0 ] || {
    printf 'dj-lint.sh: --list-files does not accept explicit paths.\n' >&2
    exit 2
  }
...
if ! command -v shellcheck >/dev/null 2>&1; then
  printf 'dj-lint.sh: ShellCheck not found; install ShellCheck %s with bin/dj-install-shellcheck.sh <destination-directory> and put that directory on PATH.\n' \
...
if ! PERL_BIN=$(command -v perl); then
  printf 'dj-lint.sh: perl is required for bounded worker cleanup.\n' >&2
  exit 127
fi
resolved=$("$SHELLCHECK_BIN" --version | awk '/^version:/ {print $2; exit}')
printf 'dj-lint.sh: ShellCheck %s (pinned %s)\n' "$resolved" "$REQUIRED_SHELLCHECK" >&2
if [ "$resolved" != "$REQUIRED_SHELLCHECK" ]; then
  printf 'dj-lint.sh: ShellCheck %s required for CI parity, found %s. Install %s with bin/dj-install-shellcheck.sh <destination-directory>.\n' \
    "$REQUIRED_SHELLCHECK" "$resolved" "$REQUIRED_SHELLCHECK" >&2
  exit 1
fi
if [ "$FAST" -eq 1 ]; then
  printf 'dj-lint.sh: fast local mode; ShellCheck extended analysis disabled\n' >&2
else
  printf 'dj-lint.sh: full ShellCheck extended analysis enabled\n' >&2
fi

if [ "$CHANGED_MODE" -eq 1 ] && [ "$ROOT_COUNT" -eq 0 ]; then
  printf 'dj-lint.sh: no changed lint targets\n'
  overall_rc=0
  dj_lint_run_workflows || overall_rc=$?
  exit "$overall_rc"
fi
...
if [ -n "$TELEMETRY" ]; then
  ...
  [ -d "$telemetry_parent" ] || {
    printf 'dj-lint.sh: telemetry directory does not exist: %s\n' "$telemetry_parent" >&2
    exit 2
  }
fi

TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/dj-lint.XXXXXX") || exit 1
ACTIVE_PIDS=()
...
for path in "${ROOTS[@]}"; do
  case "$path" in
    *"$TAB"*|*$'\n'*)
      printf 'dj-lint.sh: paths containing tabs or newlines are not supported: %s\n' "$path" >&2
      exit 2
      ;;
  esac
  if [ -f "$path" ]; then
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_START=$(dj_lint_shellcheck_count)
  TELEMETRY_LOAD_START=$(dj_lint_load_average)
  TELEMETRY_CPU_START=$(dj_lint_aggregate_cpu)
fi

dj_lint_run_worker() {  # <worker-index>
  ...
  if [ -n "$TELEMETRY" ] && [ -x /usr/bin/time ]; then
    if [ "$(uname)" = Darwin ]; then
      exec "$PERL_BIN" -e 'setpgrp(0, 0) or die "setpgrp: $!"; exec @ARGV or die "exec: $!"' \
        ...
        env DJ_LINT_INTERNAL=1 DJ_LINT_INTERNAL_FAST="$FAST" DJ_LINT_SHELLCHECK="$SHELLCHECK_BIN" \
...
dj_lint_start_worker() {
  dj_lint_run_worker "$1" &
  ACTIVE_PIDS+=("$!")
}

dj_lint_wait_workers() {
  local pid
...
while [ "$worker" -lt "$SHARD_COUNT" ]; do
  ...
  if [ -f "$output.rc" ]; then
    ...
    printf 'dj-lint.sh: worker produced no result for shard %s.\n' "$worker" >&2
    rc=2
  fi
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_END=$(dj_lint_shellcheck_count)
  TELEMETRY_LOAD_END=$(dj_lint_load_average)
  TELEMETRY_CPU_END=$(dj_lint_aggregate_cpu)
...
EOF
  ...
  if ! mv -f "$telemetry_tmp" "$TELEMETRY"; then
    printf 'dj-lint.sh: could not write telemetry to %s.\n' "$TELEMETRY" >&2
