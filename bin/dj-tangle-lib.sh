...
# Shared worktree-tangle guard for the jarvis-on-itself case.
# Usage: . bin/dj-tangle-lib.sh
#
# Jarvis is a treehouse-pooled git repo of itself: crewmate worktrees and
# secondmate homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root jarvis operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a crewmate spawned to work on jarvis ITSELF branching and
# committing in the primary checkout instead of its own disposable worktree,
# stranding the primary on a feature branch (e.g. dj/readme-restructure-d3).
#
# dj_primary_tangle_branch detects exactly that and nothing else: a NAMED,
