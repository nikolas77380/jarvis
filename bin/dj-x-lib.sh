#!/usr/bin/env bash
# Shared config resolution for the X-mode connector client (dj-x-poll.sh and
# dj-x-reply.sh). X mode is opt-in: a user drops a non-empty DJX_PAIRING_TOKEN
# into the jarvis home's .env. DJX_ENV_FILE can point direct client calls at
# another .env-style file, but bootstrap activation still checks $DJ_HOME/.env.
# Until then polling is a hard no-op; replies can still run in DJX_DRY_RUN
...
#   fmx_load_config            - resolve DJX_TOKEN, DJX_RELAY, DJX_DRY, DJX_MAX,
#                                and DJX_THREAD_MAX (env wins over .env)
...
# Callers must have DJ_HOME set before calling fmx_load_config.
...
fmx_private_artifact_publish_stdin() {
  ...
  dest="$dir/$base"
  tmp=$(umask 077; mktemp "$dir/.${base}.dj-x.XXXXXX" 2>/dev/null) || return 1
  if ! cat > "$tmp" \
...
fmx_private_artifact_publish_stdin_once() {
  ...
  dest="$dir/$base"
  tmp=$(umask 077; mktemp "$dir/.${base}.dj-x.XXXXXX" 2>/dev/null) || return 2
  if ! cat > "$tmp" \
    || ! chmod "$mode" "$tmp" 2>/dev/null \
    || ! fmx_single_link_file_mode_valid "$tmp" "$mode" "$device"; then
    rm -f -- "$tmp"
...
# Resolve the X-mode settings into DJX_TOKEN, DJX_RELAY, DJX_DRY, DJX_MAX,
# DJX_DISCORD_MAX, and DJX_THREAD_MAX. An explicit environment variable always
# wins over the .env file; the relay URL defaults to the production host so a
# normal user configures only the token. DJX_RELAY has any trailing slash trimmed
# so callers can append "/connector/..." cleanly.
# DJX_DRY is set to "1" when DJX_DRY_RUN is a truthy value (anything other than
# unset/empty/0/false/no/off), and "" otherwise: preview mode, where the client
# composes a reply but records it instead of posting (see dj-x-reply.sh).
fmx_load_config() {
  local env_file="${DJX_ENV_FILE:-$DJ_HOME/.env}" dry
  if [ -n "${DJX_PAIRING_TOKEN+x}" ]; then
    DJX_TOKEN=${DJX_PAIRING_TOKEN-}
  else
    DJX_TOKEN=$(fmx_env_get DJX_PAIRING_TOKEN "$env_file")
  fi
  if [ -n "${DJX_RELAY_URL+x}" ]; then
    DJX_RELAY=${DJX_RELAY_URL-}
  else
    DJX_RELAY=$(fmx_env_get DJX_RELAY_URL "$env_file")
  fi
  [ -n "$DJX_RELAY" ] || DJX_RELAY="https://myjarvis.io"
  DJX_RELAY=${DJX_RELAY%/}
  if [ -n "${DJX_DRY_RUN+x}" ]; then
    dry=${DJX_DRY_RUN-}
  else
    dry=$(fmx_env_get DJX_DRY_RUN "$env_file")
  fi
  # shellcheck disable=SC2034 # DJX_DRY is read by callers (dj-x-reply.sh) after sourcing.
  case "$(printf '%s' "$dry" | tr '[:upper:]' '[:lower:]')" in
    ''|0|false|no|off) DJX_DRY="" ;;
    *) DJX_DRY=1 ;;
  esac
  ...
  if [ -n "${DJX_X_REPLY_MAX_CHARS+x}" ]; then maxraw=${DJX_X_REPLY_MAX_CHARS-}; else maxraw=$(fmx_env_get DJX_X_REPLY_MAX_CHARS "$env_file"); fi
  case "$maxraw" in ''|*[!0-9]*) maxraw=280 ;; esac
  [ "$maxraw" -ge 50 ] 2>/dev/null || maxraw=50
  # shellcheck disable=SC2034 # DJX_MAX is read by callers (dj-x-reply.sh) after sourcing.
  DJX_MAX=$maxraw
  if [ -n "${DJX_DISCORD_REPLY_MAX_CHARS+x}" ]; then discordraw=${DJX_DISCORD_REPLY_MAX_CHARS-}; else discordraw=$(fmx_env_get DJX_DISCORD_REPLY_MAX_CHARS "$env_file"); fi
  ...
  # shellcheck disable=SC2034 # DJX_DISCORD_MAX is read by callers (dj-x-reply.sh) after sourcing.
  DJX_DISCORD_MAX=$discordraw
  if [ -n "${DJX_X_THREAD_MAX+x}" ]; then threadraw=${DJX_X_THREAD_MAX-}; else threadraw=$(fmx_env_get DJX_X_THREAD_MAX "$env_file"); fi
  case "$threadraw" in ''|*[!0-9]*) threadraw=25 ;; esac
  [ "$threadraw" -ge 1 ] 2>/dev/null || threadraw=25
  # shellcheck disable=SC2034 # DJX_THREAD_MAX is read by callers (dj-x-reply.sh) after sourcing.
  DJX_THREAD_MAX=$threadraw
}
...
# have populated DJX_TOKEN and DJX_RELAY first.
fmx_request_relay_context() {
  local rid=$1 payload_file body_file code rc ctx empty='{"platform":"","reply_max_chars":""}'
  [ -n "${DJX_TOKEN:-}" ] || { printf '%s\n' "$empty"; return 1; }
  command -v curl >/dev/null 2>&1 || { printf '%s\n' "$empty"; return 1; }
  command -v jq >/dev/null 2>&1 || { printf '%s\n' "$empty"; return 1; }
  payload_file=$(mktemp "${TMPDIR:-/tmp}/dj-x-reqctx.XXXXXX") || { printf '%s\n' "$empty"; return 1; }
  body_file=$(mktemp "${TMPDIR:-/tmp}/dj-x-reqctx-body.XXXXXX") || { rm -f "$payload_file"; printf '%s\n' "$empty"; return 1; }
...
# budget for one outbound message. X keeps the existing DJX_X_REPLY_MAX_CHARS
...
fmx_auth_header_file() {
  ...
  file=$(umask 077; mktemp "${TMPDIR:-/tmp}/dj-x-auth.XXXXXX") || return 1
  chmod 600 "$file" 2>/dev/null || { rm -f "$file"; return 1; }
  printf 'Authorization: Bearer %s\n' "$DJX_TOKEN" > "$file" || { rm -f "$file"; return 1; }
  printf '%s\n' "$file"
}
...
# The compact preview object is printed for DJX_DRY_RUN outbox records.
fmx_image_payload_file() {
  local path=$1 client=${2:-dj-x-reply} payload_file=${3:-} media_type bytes
...
fmx_post_json() (
  ...
  code=$(curl -m 10 -s -o "$body_file" -w '%{http_code}' \
    ...
    "$DJX_RELAY/connector/$endpoint" 2>/dev/null)
  rc=$?
...
# dj-x-followup.sh posts against that link (within the window, up to the cap),
# then either records the incremented count or clears the link. These helpers
# own the read/write/clear so dj-x-link.sh and dj-x-followup.sh never hand-edit
...
fmx_meta_link_set() {
  ...
  [ -f "$meta" ] || { dj_lock_release "$lock"; return 1; }
  tmp=$(fmx_meta_tmp "$meta") || { dj_lock_release "$lock"; return 1; }
  if ! { grep -vE '^x_request=|^x_request_ts=|^x_followups=|^x_platform=|^x_reply_max_chars=' "$meta" || true; } > "$tmp"; then
    rm -f "$tmp"; dj_lock_release "$lock"; return 1
  fi
  printf 'x_request=%s\n' "$rid" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  printf 'x_request_ts=%s\n' "$ts" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  printf 'x_followups=%s\n' "$followups" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  if [ -n "$platform" ]; then
    printf 'x_platform=%s\n' "$platform" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  fi
  case "$reply_max" in
    ''|*[!0-9]*) ;;
    *) printf 'x_reply_max_chars=%s\n' "$reply_max" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; } ;;
  esac
  mv -f "$tmp" "$meta" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
...
fmx_meta_followups_set() {
  ...
  printf 'x_followups=%s\n' "$n" >> "$tmp" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  mv -f "$tmp" "$meta" || { rm -f "$tmp"; dj_lock_release "$lock"; return 1; }
  dj_lock_release "$lock"
}
...
fmx_meta_link_clear() {
  ...
  lock=$(dj_meta_lock_path "$meta") || return 1
  dj_lock_acquire_wait "$lock"
  [ -f "$meta" ] || { dj_lock_release "$lock"; return 0; }
  tmp=$(fmx_meta_tmp "$meta") || { dj_lock_release "$lock"; return 1; }
  if ! { grep -vE '^x_request=|^x_request_ts=|^x_followups=|^x_platform=|^x_reply_max_chars=' "$meta" || true; } > "$tmp"; then
    rm -f "$tmp"; dj_lock_release "$lock"; return 1
  fi
