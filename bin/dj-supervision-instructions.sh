...
DJ_ROOT="${DJ_ROOT_OVERRIDE:-$REPO_ROOT}"
...
usage() {
  cat <<'EOF'
Usage: dj-supervision-instructions.sh [--harness <name>] [--read-only 0|1] [--afk 0|1] [--x-mode 0|1] [--repair-line] [--queue-pending 0|1]
...
if [ -z "$HARNESS" ]; then
  HARNESS=$("$SCRIPT_DIR/dj-harness.sh" 2>/dev/null || printf unknown)
fi
...
checkpoint_seconds=${DJ_CODEX_WATCH_CHECKPOINT:-180}
...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/dj-watch-checkpoint.sh --seconds ' "$checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool dj_watch_arm_pi, or restart Pi with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    opencode)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision by letting the OpenCode TUI plugin arm after idle; use bin/dj-watch-arm.sh only as a manual recovery probe if the plugin reports failure.'
      ;;
    grok)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/dj-watch-arm.sh as its own Grok tracked background task, never shell &.'
      ;;
    cursor)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      printf '%s\n' '- Ordinary wake: the Stop-owned auto-arm (bin/dj-claude-stop-autoarm.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: take the next foreground bin/dj-watch-checkpoint.sh checkpoint as directed below.'
      ;;
    pi|pi-signed)
    ...
    opencode)
      ...
      ;;
    grok)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/dj-watch-arm.sh Grok tracked background task as directed below.'
      ;;
    cursor)
      printf '%s\n' '- Ordinary wake: the stop-hook park (bin/dj-turnend-guard-cursor.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    *)
