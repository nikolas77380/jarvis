...
#               bin/dj-send.sh so it sweeps its own uncaptured session
...
#               generic remote write path (bin/dj-remote-file.sh put reaches
...
# one hard bound (DJ_STOW_CASCADE_TIMEOUT seconds, default 60), so a slow or
...
BUDGET_CMD=dj-startup-memory-budget.sh
...
BOUND=${DJ_STOW_CASCADE_TIMEOUT:-60}
case "$BOUND" in
  ''|*[!0-9]*) printf 'error: DJ_STOW_CASCADE_TIMEOUT must be a positive integer: %s\n' "$BOUND" >&2; exit 2 ;;
esac
[ "$BOUND" -gt 0 ] \
  || { printf 'error: DJ_STOW_CASCADE_TIMEOUT must be a positive integer: %s\n' "$BOUND" >&2; exit 2; }

die() { printf 'error: %s\n' "$1" >&2; exit 1; }
emit() { printf '%s\n' "$1"; }

TMP=$(mktemp -d "${TMPDIR:-/tmp}/dj-stow-cascade.XXXXXX") || exit 1
...
run_step() {
  local rc=0
  dj_run_timed "$BOUND" "$@" < /dev/null > "$STEP_OUT" 2>/dev/null || rc=$?
  return "$rc"
}
...
resolve_local_transport() { # <id> <resolved-home>
  ...
  [ -n "$target" ] || target=$(dj_meta_get "$meta" window)
  if [ -z "$target" ]; then
    set_transport direct 'recorded endpoint has no target'
    return 0
  fi
  case "$(dj_backend_agent_state "$backend" "$target" 2>/dev/null || printf 'unreadable')" in
...
resolve_remote_transport() { # <id>
  ...
  run_step "$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh state "$id" || rc=$?
  if [ "$rc" -eq 124 ]; then
...
if [ -e "$DJ_HOME/$SUB_HOME_MARKER" ] || [ -L "$DJ_HOME/$SUB_HOME_MARKER" ]; then
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  if [ "$remote" -eq 1 ]; then
    ...
    emit "host=$host"
    emit "home=$home"
    run_step "$SCRIPT_DIR/dj-on.sh" "$id" "$BUDGET_CMD" report || rc=$?
  else
    ...
    emit "home=$resolved"
    # Every DJ_*_OVERRIDE is restated so a caller's own override cannot leak
    # this home's memory files into the accounting of another home.
    run_step env \
      DJ_ROOT_OVERRIDE="$DJ_ROOT" \
      DJ_HOME="$resolved" \
      DJ_STATE_OVERRIDE="$resolved/state" \
      DJ_DATA_OVERRIDE="$resolved/data" \
      DJ_CONFIG_OVERRIDE="$resolved/config" \
