#!/usr/bin/env bash
# dj-inactive-reconcile.sh - bounded reconciliation of suspicious inactive terminal outcomes.
#
# Usage:
#   dj-inactive-reconcile.sh scan [--startup]
#   dj-inactive-reconcile.sh acknowledge <fingerprint>
#
...
# `scan` evaluates at most once per DJ_INACTIVE_RECONCILE_SECS (default 900,
...
# DJ_INACTIVE_RECONCILE_BUDGET_SECS deadline (default 10, valid 1..30) and
...
# captain-held. It then uses dj-crew-state.sh as the sole current-state source.
...
# append.
# In a main home, a presentation-stage record is acknowledged by dj-wake-drain
...
#
# New dj-terminal-outcome.v1 receipts contain schema, fingerprint, task_id,
...
#
# The scan reads only durable local state and dj-crew-state.sh; it never invokes
# gh, gh-axi, curl, dj-pr-check.sh, dj-pr-poll.sh, or a state *.check.sh.
set -u
export LC_ALL=C
...
CREW_STATE_BIN="${DJ_INACTIVE_CREW_STATE_BIN:-$SCRIPT_DIR/dj-crew-state.sh}"
...
DJ_INACTIVE_RECONCILE_SECS=${DJ_INACTIVE_RECONCILE_SECS:-900}
case "$DJ_INACTIVE_RECONCILE_SECS" in
  ''|*[!0-9]*|0)
    printf 'dj-inactive-reconcile: DJ_INACTIVE_RECONCILE_SECS must be a whole number from 60 to 1800\n' >&2
    exit 2
    ;;
esac
if [ "$DJ_INACTIVE_RECONCILE_SECS" -lt 60 ] || [ "$DJ_INACTIVE_RECONCILE_SECS" -gt 1800 ]; then
  printf 'dj-inactive-reconcile: DJ_INACTIVE_RECONCILE_SECS must be a whole number from 60 to 1800\n' >&2
  exit 2
fi
DJ_INACTIVE_RECONCILE_BUDGET_SECS=${DJ_INACTIVE_RECONCILE_BUDGET_SECS:-10}
case "$DJ_INACTIVE_RECONCILE_BUDGET_SECS" in
  ''|*[!0-9]*|0)
    printf 'dj-inactive-reconcile: DJ_INACTIVE_RECONCILE_BUDGET_SECS must be a whole number from 1 to 30\n' >&2
    exit 2
    ;;
esac
if [ "$DJ_INACTIVE_RECONCILE_BUDGET_SECS" -gt 30 ]; then
  printf 'dj-inactive-reconcile: DJ_INACTIVE_RECONCILE_BUDGET_SECS must be a whole number from 1 to 30\n' >&2
  exit 2
fi
...
reconcile_now() {
  case "${DJ_INACTIVE_RECONCILE_NOW:-}" in
    ''|*[!0-9]*) date +%s ;;
    *) printf '%s\n' "$DJ_INACTIVE_RECONCILE_NOW" ;;
  esac
}
...
ensure_record() { # <fingerprint> <task> <incarnation> <state> <outcome-key> <origin> <phase> <pr>
  ...
  {
    printf 'schema=dj-terminal-outcome.v1\n'
...
queue_key_exists() { # <key>
  local key=$1 queued
  queued=$(dj_wake_queued_keys check 2>/dev/null || true)
...
queue_notice_once() { # <record> <key> <payload>
  ...
  dj_wake_append check "$key" "$payload" || return 2
  record_field_set "$record" notice_emitted 1 || return 2
  printf 'actionable: %s\n' "$payload"
  return 0
}
...
home_secondmate_id() {
  local marker="$DJ_HOME/.dj-secondmate-home" id
...
report_to_parent() { # <self-id> <task> <state> <outcome-key> <fingerprint> <pr>
  ...
  parent_record="$DJ_HOME/.dj-secondmate-parent"
  dj_secondmate_parent_record_parse "$parent_record" || return 1
  case "$DJ_SECONDMATE_PARENT_ROUTE" in
    local)
      [ -n "$DJ_SECONDMATE_PARENT_HOME" ] || return 1
      destination="$DJ_SECONDMATE_PARENT_HOME/state/$self.status"
      ;;
    remote)
...
reconcile_direct_child_locked() { # <id> <meta> <secondmate-id-or-empty> <timeout>
  ...
  [ "$age" -ge "$DJ_INACTIVE_RECONCILE_SECS" ] || return 0
  state_line=$(dj_run_timed "$timeout" env DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" \
...
reconcile_direct_child() { # <id> <meta> <secondmate-id-or-empty> <timeout>
  ...
  lock=$(dj_meta_lock_path "$meta") || return 1
...
scan() {
  ...
  [ ! -L "$OUTCOME_DIR" ] || return 1
  if [ "$startup" != 1 ] && [ "$(scan_marker_age)" -lt "$DJ_INACTIVE_RECONCILE_SECS" ]; then
    return 0
  fi
  ...
  if self=$(home_secondmate_id); then
    ...
    if [ "$marker_rc" -ne 1 ]; then
      printf 'actionable: inactive terminal outcomes remain unreconciled: invalid .dj-secondmate-home marker\n'
      return 0
    fi
  fi
  deadline=$(( $(date +%s) + DJ_INACTIVE_RECONCILE_BUDGET_SECS ))
...
case "$mode" in
  scan)
    ...
    case "${2:-}" in
      '') ;;
      --startup) startup=1 ;;
      *) printf 'usage: dj-inactive-reconcile.sh scan [--startup]\n' >&2; exit 2 ;;
    esac
    ...
    if dj_run_timed $((DJ_INACTIVE_RECONCILE_BUDGET_SECS + 1)) "$0" _scan-locked "$startup"; then
      :
    elif [ "$?" -ne 124 ]; then
      exit 1
    fi
  ...
  _scan-locked)
    [ "$#" -eq 2 ] || exit 2
    dj_lock_acquire_wait "$SCAN_LOCK" || exit 1
    trap 'dj_lock_release "$SCAN_LOCK"' EXIT
    scan "$2"
    ;;
  acknowledge)
    [ "$#" -eq 2 ] || { printf 'usage: dj-inactive-reconcile.sh acknowledge <fingerprint>\n' >&2; exit 2; }
    dj_lock_acquire_wait "$SCAN_LOCK" || exit 1
    trap 'dj_lock_release "$SCAN_LOCK"' EXIT
    acknowledge "$2"
    ;;
  ...
  *)
    printf 'usage: dj-inactive-reconcile.sh scan [--startup]\n' >&2
    printf '       dj-inactive-reconcile.sh acknowledge <fingerprint>\n' >&2
    exit 2
    ;;
esac
