#!/usr/bin/env bash
# dj-lease-lib.sh - the per-task supervision lease contract (one owner).
#
# WHY. On the Pi supervision branch (docs/pi-supervision-branch.md), two LLM
# actors share one jarvis home inside one pi process: MAIN (the captain's
...
#     lease-command lock; leases never coordinate across jarvis homes.
#   - Actors: exactly "main" and "branch". The current actor is
#     $DJ_SUPERVISION_ACTOR when set, else "main". The branch's shell gets
#     DJ_SUPERVISION_ACTOR=branch injected deterministically by the Pi branch
...
#     session-lock holder, or DJ_LEASE_HOLDER_PID - see bin/dj-lease.sh), and
...
# CONFUSED-AGENT-GRADE, the same grade bin/dj-gate-refuse-lib.sh documents
...
#   - Guard semantics (dj_lease_guard): no lease, a same-actor lease, or a
#     provably stale lease passes; a live lease held by the OTHER actor
#     refuses with exit DJ_LEASE_REFUSE_EXIT. In a Pi supervision context the
#     guard retains the lease-command lock until dj_lease_guard_release, so the
...
#   - Role partition (dj_lease_forbid_branch): actions MAIN alone owns -
...
#
# Sourced by bin/dj-send.sh, bin/dj-control.sh, bin/dj-teardown.sh,
# bin/dj-pr-merge.sh, bin/dj-merge-local.sh, bin/dj-spawn.sh, and
# bin/dj-lease.sh. Callers must have $STATE resolved before calling. No side
# effects on source. set -u / set -e safe.

# Distinct from usage errors (2), the gate refusal (3), and dj-send's
# unconfirmed submit (3): recognizable as "the other supervision actor holds
# this task right now - retry after the lease clears".
DJ_LEASE_REFUSE_EXIT=6
DJ_LEASE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DJ_LEASE_GUARD_LOCK=

dj_lease_lock_helpers() {
  command -v dj_lock_acquire_wait >/dev/null 2>&1 && return 0
  # dj-wake-lib.sh is a canonical lint root in its own right and is already
  ...
  . "$DJ_LEASE_LIB_DIR/dj-wake-lib.sh"
}

# dj_lease_actor: print the current actor after validating it. Returns 1 (with
# stderr) for an unknown DJ_SUPERVISION_ACTOR value.
dj_lease_actor() {
  local actor=${DJ_SUPERVISION_ACTOR:-main}
  case "$actor" in
    main|branch) printf '%s\n' "$actor" ;;
    *)
      echo "error: unknown DJ_SUPERVISION_ACTOR '$actor' (expected main or branch)" >&2
      return 1
      ;;
  esac
}

# dj_lease_valid_id <id>: 0 iff the task/resource id is safe to embed in a
# state filename.
dj_lease_valid_id() {
  case "${1:-}" in
...
# dj_lease_read <task>: read the lease into DJ_LEASE_ACTOR/DJ_LEASE_PID/
# DJ_LEASE_EPOCH. Returns 1 when no lease file exists. A malformed lease
# (unreadable actor or pid) reads as actor "" so callers treat it as stale
# rather than blocking forever on a torn record.
dj_lease_read() {
  local file line
  file=$(dj_lease_path "$1")
  DJ_LEASE_ACTOR=
  DJ_LEASE_PID=
  DJ_LEASE_EPOCH=
  [ -e "$file" ] || return 1
  IFS= read -r line < "$file" 2>/dev/null || line=
  DJ_LEASE_ACTOR=$(printf '%s' "$line" | cut -f1)
  DJ_LEASE_PID=$(printf '%s' "$line" | cut -f2)
  # shellcheck disable=SC2034 # Consumed by sourcing callers (bin/dj-lease.sh check).
  DJ_LEASE_EPOCH=$(printf '%s' "$line" | cut -f3)
  case "$DJ_LEASE_ACTOR" in
    main|branch) ;;
    *) DJ_LEASE_ACTOR= ;;
  esac
  case "$DJ_LEASE_PID" in
    '' | *[!0-9]*) DJ_LEASE_PID= ;;
  esac
  return 0
}

# dj_lease_live <task>: 0 iff a well-formed lease exists in a Pi context, its
# recorded pid is alive, and that pid IS the current session-lock holder (see
# the staleness contract above).
dj_lease_live() {
  local lock_pid
  case "${PI_CODING_AGENT:-}:${DJ_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) ;;
    *) return 1 ;;
  esac
  dj_lease_read "$1" || return 1
  [ -n "$DJ_LEASE_ACTOR" ] || return 1
  [ -n "$DJ_LEASE_PID" ] || return 1
  kill -0 "$DJ_LEASE_PID" 2>/dev/null || return 1
  lock_pid=$(head -n 1 "$STATE/.lock" 2>/dev/null || true)
  case "$lock_pid" in ''|0|1|*[!0-9]*) return 1 ;; esac
  [ "$DJ_LEASE_PID" = "$lock_pid" ]
}

# dj_lease_clear_stale <task>: remove the lease file when it exists but is not
# live. Silent; never touches a live lease.
dj_lease_clear_stale() {
  local file
  file=$(dj_lease_path "$1")
  [ -e "$file" ] || return 0
  dj_lease_live "$1" && return 0
  rm -f -- "$file"
}

# dj_lease_guard <task> <action-label>: refuse (exit DJ_LEASE_REFUSE_EXIT) when
...
# mutation; the caller must invoke dj_lease_guard_release from its EXIT cleanup.
...
dj_lease_guard() {
  ...
  dj_lease_valid_id "$task" || return 0
  actor=$(dj_lease_actor) || exit "$DJ_LEASE_REFUSE_EXIT"
  case "${PI_CODING_AGENT:-}:${DJ_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) active=1 ;;
  esac
  [ "$active" = 1 ] || [ -e "$(dj_lease_path "$task")" ] || return 0
  dj_lease_lock_helpers
  lock="$STATE/.dj-lease-command.lock"
  ...
  if [ "$DJ_LEASE_GUARD_LOCK" != "$lock" ]; then
    dj_lock_acquire_wait "$lock"
    DJ_LEASE_GUARD_LOCK=$lock
  fi
  if ! dj_lease_live "$task"; then
    dj_lease_clear_stale "$task" || { dj_lease_guard_release; return 1; }
    if [ "$active" != 1 ]; then
      dj_lease_guard_release
    fi
    return 0
  fi
  lease_actor=$DJ_LEASE_ACTOR
...
# Release the claim/guard serialization lock retained by dj_lease_guard.
# Idempotent so callers can use it unconditionally from existing EXIT cleanup.
dj_lease_guard_release() {
  local lock=$DJ_LEASE_GUARD_LOCK
  [ -n "$lock" ] || return 0
  DJ_LEASE_GUARD_LOCK=
  dj_lock_release "$lock"
}

# dj_lease_forbid_branch <action-label>: refuse (exit DJ_LEASE_REFUSE_EXIT)
