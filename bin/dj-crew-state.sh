#!/usr/bin/env bash
# dj-crew-state.sh - deterministic read of a crew's CURRENT state.
#
# Why this exists: state/<id>.status is an append-only, best-effort EVENT LOG.
# Crews append only wake-worthy transitions (done/needs-decision/blocked/paused/failed)
# and nothing when they silently resume, so `tail -1` of that log reports the
# last EVENT, not the current STATE. After jarvis resolves a needs-decision
...
# token-tight line jarvis can read every heartbeat:
#
...
#      (dj-on.sh + dj-remote-secondmate-control.sh state). alive falls through
...
ID=${1:-}
[ -n "$ID" ] || { echo "usage: dj-crew-state.sh <id>" >&2; exit 2; }

META="$STATE/$ID.meta"
LOG="$STATE/$ID.status"
NM_TIMEOUT=${DJ_CREW_STATE_NM_TIMEOUT:-10}
...
# history every call.
DJ_CREW_STATE_RUNS_LIMIT=${DJ_CREW_STATE_RUNS_LIMIT:-200}
case "$DJ_CREW_STATE_RUNS_LIMIT" in ''|*[!0-9]*) DJ_CREW_STATE_RUNS_LIMIT=200 ;; esac
SEP=' · '
...
# Last non-empty status line; dj-classify-lib.sh owns leading-verb normalization.
log_last_line() {
  [ -f "$LOG" ] || return 1
  grep -v '^[[:space:]]*$' "$LOG" 2>/dev/null | tail -1
}
# Map a status-log verb onto a canonical state for the fallback path. `paused` is
# the deliberate-external-wait verb (dj-classify-lib.sh's DJ_CLASSIFY_PAUSED_VERB):
...
# recovery-grade state over the same dj-on.sh transport dj-send uses, then read
...
if [ -n "$REMOTE_HOST" ]; then
  if ! REMOTE_STATE=$(DJ_HOME="$DJ_HOME" "$SCRIPT_DIR/dj-on.sh" "$ID" \
    dj-remote-secondmate-control.sh state "$ID" < /dev/null 2>/dev/null); then
    REMOTE_STATE=
  fi
...
# (dj_backend_of_meta defaults absent backend= to tmux, the P1 contract): a
# herdr task is read through dj_backend_capture instead of a bare tmux probe.
TASK_BACKEND=$(dj_backend_of_meta "$META")
BACKEND_TARGET=$(dj_backend_target_of_meta "$META")
EXPECTED_LABEL="dj-$ID"
pane_readable() {  # <target>
  case "$TASK_BACKEND" in
    tmux) tmux display-message -p -t "$1" '#{pane_id}' >/dev/null 2>&1 ;;
    *) dj_backend_capture "$TASK_BACKEND" "$1" 1 "$EXPECTED_LABEL" >/dev/null 2>&1 ;;
  esac
}
# crew_busy_verdict: the crew's semantic busy state from the one contract
# owner (bin/dj-busy-lib.sh), as "<busy|idle|unknown> <source>". A converted
...
crew_busy_verdict() {  # <target>
  local tail40=''
  case "$HARNESS" in
    grok*) tail40=$(dj_backend_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40='' ;;
  esac
  dj_busy_classify "$TASK_BACKEND" "$1" "$HARNESS" "$ID" "$STATE" "$tail40"
}
...
# bin/dj-nm-run-lib.sh, shared with dj-teardown.sh's pre-teardown run abort.

trim() { dj_nm_trim "$@"; }
strip_quotes() { dj_nm_strip_quotes "$@"; }
nm_run() {  # <args...>
  dj_nm_run "$WT" "$NM_TIMEOUT" "$@"
}

# Scalar value of a TOON key in the captured run output ($RUN_OUT).
RUN_OUT=""
nm_field() {  # <key>
  dj_nm_field "$RUN_OUT" "$1"
}
...
# a separate bug in bin/dj-watch.sh's stale_is_terminal precedence - see that
...
# when the branch has no run within DJ_CREW_STATE_RUNS_LIMIT rows.
nm_runs_status_for_branch() {  # <branch>
  local branch=$1 out row st rest br sha
  out=$(nm_run runs --limit "$DJ_CREW_STATE_RUNS_LIMIT")
  [ -n "$out" ] || return 0
...
# dj_nm_head_matches_worktree in bin/dj-nm-run-lib.sh.
nm_run_head_matches_worktree() {
  local run_head
  run_head=$(strip_quotes "$(nm_field head)")
  dj_nm_head_matches_worktree "$WT" "$run_head"
}
...
nm_coarse_head_matches_worktree() {  # <short-sha>
  dj_nm_head_matches_worktree "$WT" "$1"
}
...
# run-state. A decision-closing event - resolved: (dj-classify-lib.sh's
# DJ_CLASSIFY_RESOLVE_VERB), and any future decision-only sibling - is NOT a state:
