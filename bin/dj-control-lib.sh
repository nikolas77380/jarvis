#!/usr/bin/env bash
# dj-control-lib.sh - the ONE executable owner of jarvis's agent lifecycle
# CONTROL-PLANE mechanics.
#
# Data plane vs control plane (captain-approved root architecture, 2026-07-13).
# bin/dj-send.sh is the DATA plane: conversational text for the agent to read,
...
# chat ("[dj-from-jarvis] /quit") that the agent reasons ABOUT instead of
# executing. bin/dj-control.sh is the CONTROL plane: allowlisted lifecycle
...
#      bin/dj-send.sh's --key path reads the same table rather than a second
#      copy of it.
...
#      (bin/dj-backend.sh's dj_backend_agent_state) able to PROVE that an agent
...
# command records the command's basename instead (bin/dj-spawn.sh derives
...
# family.
dj_control_harness_family() {  # <recorded-harness>
  case "${1-}" in
    pi) printf 'pi' ;;
...
# adapter only: it has no primary supervision protocol, and bin/dj-spawn.sh
...
# been stopped.
dj_control_harness_supports_kind() {  # <harness> <kind>
  local harness=${1-} kind=${2-}
  dj_control_harness_supported "$harness" || return 1
  case "$harness" in
...
dj_control_interrupt_key() {  # <harness>
  case "${1-}" in
...
dj_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
...
# above.
dj_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
...
dj_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
...
dj_control_exit_command() {  # <harness>
  case "${1-}" in
...
# Ctrl+U (bin/backends/orca.sh's dj_backend_orca_send_key).
...
# herdr implement dj_backend_agent_state; zellij, orca, and cmux report
...
# line: worktree-resident hook files and jarvis-owned state tokens only,
# never a harness's own managed config.
dj_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    muse)
      ...
      # bound to the pane by these two jarvis-owned sidecars. A relaunch
...
# The jarvis-owned global turn-end registry entry a harness mints per task.
...
# dj_control_harness_wiring_paths. Prints the registry path or nothing.
dj_control_harness_turnend_token_path() {  # <harness> <state-dir> <id>
...
dj_control_harness_turnend_auth_path() {  # <harness> <token>
  local harness=${1-} token=${2-}
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 0 ;; esac
  case "$harness" in
    grok) printf '%s\n' "${GROK_HOME:-$HOME/.grok}/hooks/dj-turn-end.d/$token" ;;
    kimi) printf '%s\n' "$HOME/.kimi-code/dj-turn-end.d/$token" ;;
    *) return 0 ;;
  esac
}
