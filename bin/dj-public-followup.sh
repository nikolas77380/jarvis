#!/usr/bin/env bash
# dj-public-followup.sh - the deterministic consumer and delivery owner for
# public commitments made through the myjarvis relay (X and Discord).
#
# THE PROBLEM THIS SOLVES: jarvis promises a public final reply, routes the
...
#   state/x-context/            the private full request context (dj-x-lib.sh).
#   bin/dj-x-reply.sh           posting to the relay, thread splitting, dry run.
#   bin/dj-public-followup-lib.sh  the activation gate and private transport.
...
# first on the authoritative activation contract (a non-empty DJX_PAIRING_TOKEN
# in $DJ_HOME/.env). Read-side and cleanup paths then use an O(1) presence check
...
#
#   dj-public-followup.sh register <obligation-id> --relation <relation-id>
...
#
#   dj-public-followup.sh brief <obligation-id>
#       Print the exact dj-public-followup-emit.sh command line the bound worker
...
#
#   dj-public-followup.sh deliver <obligation-id> [--text-file <path>]
...
#
#   dj-public-followup.sh record-posted <obligation-id> --attempt <n> --chunks <n>
...
#
#   dj-public-followup.sh guard-work <work-home-id> <work-id>
...
#
#   dj-public-followup.sh rechain <new-obligation-id> --from <delivered-id>
...
#
#   dj-public-followup.sh retire <obligation-id> --reason "<why the loop is done>" [--force]
...
# DJ_PF_RETRY_BACKOFF_SECS (default 900) sets the next-attempt time recorded with
...
RETRY_BACKOFF=${DJ_PF_RETRY_BACKOFF_SECS:-900}
case "$RETRY_BACKOFF" in ''|*[!0-9]*) RETRY_BACKOFF=900 ;; esac

usage() {
  echo "usage: dj-public-followup.sh <active|register|brief|consume|pending|deliver|record-posted|guard-work|rechain|retire> [args]" >&2
}

# The header comment IS the help text, so the two can never drift apart.
help() { sed -n '2,/^set -u$/p' "$0" | sed '$d; s/^# \{0,1\}//'; }

die() { printf 'dj-public-followup: %s\n' "$1" >&2; exit "${2:-2}"; }
...
pf_cleanup() {
  ...
  for ((i=${#PF_REGISTRY_LOCK_IDS[@]}-1; i>=0; i--)); do
    dj_pf_registry_lock_release "$STATE" "${PF_REGISTRY_LOCK_IDS[$i]}" 2>/dev/null || true
  done
  [ "${#PF_TEMP_FILES[@]}" -eq 0 ] || rm -f -- "${PF_TEMP_FILES[@]}"
}
trap pf_cleanup EXIT

now_rfc3339() { dj_pf_now_rfc3339; }
...
# same convention bin/dj-captain-hold.sh uses for typed backlog state.
tx() { (cd "$DJ_HOME" && tasks-axi "$@"); }
...
gate_or_exit() {
  dj_pf_relay_active "$DJ_HOME" || exit 0
  dj_pf_has_registrations "$STATE" || dj_pf_has_events "$STATE" || exit 0
}

# --- subcommand: active -----------------------------------------------------

cmd_active() {
  dj_pf_relay_active "$DJ_HOME" || exit 1
  dj_pf_has_registrations "$STATE" || dj_pf_has_events "$STATE" || exit 1
  exit 0
}
...
cmd_register() {
  ...
  dj_pf_relay_active "$DJ_HOME" \
    || die "this home has not opted into the myjarvis relay, so it cannot own a public commitment" 1
  require_tools

  dj_pf_slug_valid "$id"       || die "unsafe obligation id: $id"
  dj_pf_slug_valid "$relation" || die "unsafe relation id: $relation"
  dj_pf_slug_valid "$work_id"  || die "unsafe work id: $work_id"
  dj_pf_home_id_valid "$work_home" \
  ...
  [ -z "$request" ] || dj_pf_slug_valid "$request" || die "unsafe request id: $request"
  ...
  if [ -n "$request_json" ]; then
    request_context_b64=$(printf '%s' "$request_json" | dj_pf_b64_encode)
  fi
  work_home_path=
  case "$work_home" in
    secondmate:*)
  ...
  for mkdir_target in "$(dj_pf_registry_dir "$STATE")" "$(dj_pf_events_dir "$STATE")" \
                      "$(dj_pf_consumed_dir "$STATE")" "$(dj_pf_rejected_dir "$STATE")"; do
...
cmd_brief() {
  ...
  [ -n "$id" ] || { usage; exit 2; }
  dj_pf_slug_valid "$id" || die "unsafe obligation id: $id"
  dj_pf_relay_active "$DJ_HOME" || die "the relay is not active for this home" 1
  [ -f "$(dj_pf_registry_dir "$STATE")/$id" ] \
    || die "no registration for '$id' in this home" 1

  relation=$(dj_pf_registry_get "$STATE" "$id" relation_id)
  work_home=$(dj_pf_registry_get "$STATE" "$id" work_home)
  work_id=$(dj_pf_registry_get "$STATE" "$id" work_id)
  generation=$(dj_pf_registry_get "$STATE" "$id" generation)
...
(never as a sentence for someone to parse) by running exactly:
  ...
  $DJ_ROOT/bin/dj-public-followup-emit.sh \\
    --home $DJ_HOME \\
    --obligation $id \\
    --relation $relation \\
...
reject_event() {
  ...
  rejected=$(dj_pf_rejected_dir "$STATE")
...
cmd_consume() {
  ...
  events_dir=$(dj_pf_events_dir "$STATE")
  consumed_dir=$(dj_pf_consumed_dir "$STATE")
  fmx_private_artifact_dir_prepare "$consumed_dir" >/dev/null \
    || die "could not prepare the consumed-event ledger" 1
  stderr_file=$(mktemp "${TMPDIR:-/tmp}/dj-pf-consume.XXXXXX") \
  ...
  for file in "$events_dir"/*.json; do
    ...
    if ! dj_pf_slug_valid "$event_id"; then
    ...
    if [ "$(wc -c < "$file" 2>/dev/null || echo 0)" -gt "$DJ_PF_EVENT_BYTES_MAX" ]; then
      reject_event "$file" "$event_id" "event exceeds $DJ_PF_EVENT_BYTES_MAX bytes" || consume_rc=1
      continue
    fi
    ...
    # dj-public-followup-emit.sh, so it is refused before tasks-axi sees it.
    ...
    if ! dj_pf_slug_valid "$obligation"; then
    ...
    if [ "$rc" -ne 0 ]; then
      reason=$( { cat "$stderr_file" 2>/dev/null; printf '%s\n' "$out"; } \
        | grep -v '^[[:space:]]*$' | head -1 | dj_pf_clean_outcome_text | dj_pf_bound_bytes 400)
  ...
  # A fresh event must be able to wake jarvis again, so drop the surfaced
  # signature once the inbox has been worked.
  rm -f -- "$(dj_pf_root "$STATE")/$DJ_PF_SURFACED_BASENAME" 2>/dev/null || true
  return "$consume_rc"
}
...
print_window_escalation() {
  ...
  window=$(dj_pf_followup_window_class "$expires")
  case "$window" in
    expired)
...
print_open_loop() {
  ...
  request=$(dj_pf_registry_get "$STATE" "$id" request_id)
  [ -n "$request" ] || request=$(pf_field "$payload" '.public_followup.request.request_id')
  platform=$(dj_pf_registry_get "$STATE" "$id" platform)
  [ -n "$platform" ] || platform=$(pf_field "$payload" '.public_followup.request.platform')
  delivered=$(dj_pf_registry_get "$STATE" "$id" delivered_at)
  expires=$(dj_pf_registry_get "$STATE" "$id" followup_expires_at)
  [ -n "$expires" ] || expires=$(pf_field "$payload" '.public_followup.request.followup_expires_at')
  summary=$(pf_field "$payload" '.public_followup.request.public_safe_summary' | dj_pf_clean_outcome_text)
  if [ -z "$summary" ]; then
    ctx=$(dj_pf_registry_get "$STATE" "$id" request_context_b64)
    if [ -n "$ctx" ]; then
      summary=$(printf '%s' "$ctx" | dj_pf_b64_decode | jq -r '.public_safe_summary // empty' 2>/dev/null | dj_pf_clean_outcome_text)
    fi
  fi
  ...
  printf '  summary=%s\n' "$summary"
  if ! dj_pf_registry_rechainable "$STATE" "$id"; then
...
cmd_pending() {
  ...
  if ! command -v jq >/dev/null 2>&1 || ! command -v tasks-axi >/dev/null 2>&1 \
      ...
      ' >/dev/null 2>&1; then
    if dj_pf_has_registrations "$STATE"; then
      printf 'cannot read this home'\''s public commitments through tasks-axi; %s registration(s) are still recorded under state/%s/registry\n' \
        "$(dj_pf_registry_ids "$STATE" | grep -c . || true)" "$DJ_PF_DIRNAME"
      printed=1
    fi
    if dj_pf_has_events "$STATE"; then
      printf 'unconsumed terminal results are waiting; run %s/bin/dj-public-followup.sh consume\n' "$DJ_ROOT"
      printed=1
    fi
    [ "$printed" -eq 1 ] || exit 0
    return 0
  ...
  while IFS= read -r id; do
    ...
    payload=$(printf '%s' "$listing" | jq -ce --arg id "$id" \
      '(.public_followups // []) | map(select(.id == $id)) | .[0] // empty' 2>/dev/null)
    loop_state=$(dj_pf_registry_loop_state "$STATE" "$id")
    ...
    if [ "$settled" -eq 1 ]; then
      if [ "$loop_state" != delivered ]; then
        stamp_rc=0
        dj_pf_registry_stamp_delivered "$STATE" "$id" "$(now_rfc3339)" || stamp_rc=$?
        if [ "$stamp_rc" -eq 3 ] && dj_pf_retirement_receipt_exists "$STATE" "$id"; then
          continue
        fi
        [ "$stamp_rc" -eq 0 ] \
...
public_followup_registration_valid() {
  ...
  dj_pf_home_id_valid "$work_home" || return 1
  dj_pf_slug_valid "$work_id" || return 1
...
public_followup_secondmate_home() {
  ...
  [ -f "$home/.dj-secondmate-home" ] && [ ! -L "$home/.dj-secondmate-home" ] || return 2
  marker=$(sed -n '1p' "$home/.dj-secondmate-home" 2>/dev/null)
  [ "$marker" = "$id" ] || return 2
  printf '%s\n' "$home"
}
...
clear_public_followup_link() {
  ...
  case "$work_home" in
    main)
      ...
      ;;
    secondmate:*)
      work_home_path=$(dj_pf_registry_get "$STATE" "$id" work_home_path)
  ...
  DJ_HOME="$home" DJ_STATE_OVERRIDE="$state" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
    "$DJ_ROOT/bin/dj-x-followup.sh" --clear "$work_id" >/dev/null
}
...
record_error() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/dj-pf-error.XXXXXX") || return 1
  if [ -n "$next" ]; then
...
record_posted() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/dj-pf-receipt.XXXXXX") || return 1
...
mark_loop_delivered() {
  local id=$1 rc=0
  dj_pf_registry_stamp_delivered "$STATE" "$id" "$(now_rfc3339)" || rc=$?
  case "$rc" in
    0) return 0 ;;
    3) return 3 ;;
...
cmd_deliver() {
  ...
  dj_pf_slug_valid "$id" || die "unsafe obligation id: $id"
  dj_pf_relay_active "$DJ_HOME" \
    || die "this home has not opted into the myjarvis relay, so it cannot post a public reply" 1
  require_tools
  ...
  tmp_text=$(mktemp "${TMPDIR:-/tmp}/dj-pf-text.XXXXXX") || die "could not stage the reply text" 1
  PF_TEMP_FILES+=("$tmp_text")
  receipt=$(mktemp "${TMPDIR:-/tmp}/dj-pf-postreceipt.XXXXXX") || die "could not stage the post receipt" 1
  PF_TEMP_FILES+=("$receipt")
  printf '%s' "$text" > "$tmp_text"

  hash=$(dj_pf_sha256 < "$tmp_text") || die "sha256 (shasum or sha256sum) is required" 1
  ...
  rc=0
  DJX_REPLY_PLATFORM="$platform" DJ_HOME="$DJ_HOME" \
    "$DJ_ROOT/bin/dj-x-reply.sh" "$request" --followup --receipt-file "$receipt" \
...
cmd_guard_work() {
  ...
  dj_pf_relay_active "$DJ_HOME" || exit 0
  dj_pf_has_registrations "$STATE" || exit 0
  ...
  bound=$(dj_pf_registry_ids_for_work "$STATE" "$work_home" "$work_id")
  [ -n "$bound" ] || exit 0
...
cmd_rechain() {
  ...
  dj_pf_relay_active "$DJ_HOME" \
    || die "this home has not opted into the myjarvis relay, so it cannot own a public commitment" 1
  require_tools
  dj_pf_slug_valid "$new_id" || die "unsafe obligation id: $new_id"
  dj_pf_slug_valid "$from" || die "unsafe source obligation id: $from"
  dj_pf_slug_valid "$work_id" || die "unsafe work id: $work_id"
  dj_pf_home_id_valid "$work_home" \
    || die "work home must be 'main' or 'secondmate:<stable-id>', got '$work_home'"
  case "$expected" in
  ...
  expires=$(dj_pf_registry_get "$STATE" "$from" followup_expires_at)
  ...
  rechain_to=$(dj_pf_registry_get "$STATE" "$from" rechain_to)
  ...
  if [ -z "$rechain_to" ]; then
    ...
    [ ! -e "$(dj_pf_registry_dir "$STATE")/$new_id" ] \
      && [ ! -L "$(dj_pf_registry_dir "$STATE")/$new_id" ] \
      && [ ! -e "$(dj_pf_retired_dir "$STATE")/$new_id" ] \
      && [ ! -L "$(dj_pf_retired_dir "$STATE")/$new_id" ] \
    ...
    printf '%s\nrechain_to=%s\n' "$source_record" "$new_id" \
      | fmx_private_artifact_publish_stdin "$(dj_pf_registry_dir "$STATE")" "$from" 600 \
  ...
  ctx=$(dj_pf_registry_get "$STATE" "$from" request_context_b64)
  ctx_file=$(mktemp "${TMPDIR:-/tmp}/dj-pf-rechain-ctx.XXXXXX") \
    || die "could not stage the retained request context" 1
  expected_file=$(mktemp "${TMPDIR:-/tmp}/dj-pf-rechain-exp.XXXXXX") \
    || die "could not stage the expected-final document" 1
  relation_file=$(mktemp "${TMPDIR:-/tmp}/dj-pf-rechain-rel.XXXXXX") \
    || die "could not stage the relation document" 1
  PF_TEMP_FILES+=("$ctx_file" "$expected_file" "$relation_file")
  printf '%s' "$ctx" | dj_pf_b64_decode > "$ctx_file" \
  ...
  new_registry="$(dj_pf_registry_dir "$STATE")/$new_id"
  if [ -f "$new_registry" ] && [ ! -L "$new_registry" ]; then
    [ "$(dj_pf_registry_get "$STATE" "$new_id" relation_id)" = rel-1 ] \
      && [ "$(dj_pf_registry_get "$STATE" "$new_id" work_home)" = "$work_home" ] \
      && [ "$(dj_pf_registry_get "$STATE" "$new_id" work_id)" = "$work_id" ] \
      && [ "$(dj_pf_registry_get "$STATE" "$new_id" generation)" = 1 ] \
...
cmd_retire() {
  ...
  reason=$(printf '%s' "$reason" | dj_pf_clean_outcome_text)
  [ -n "$reason" ] || die "retire requires --reason \"<why the public loop is done>\"" 2
  require_tools
  pf_registry_lock_acquire "$id" \
  ...
  retired_dir=$(dj_pf_retired_dir "$STATE")
  retired_at=$(now_rfc3339)
  registry_file="$(dj_pf_registry_dir "$STATE")/$id"
  printf 'reason=%s\nretired_at=%s\n' "$reason" "$retired_at" \
