#!/usr/bin/env bash
# dj-timeout-lib.sh - the single owner of bounded command execution.
#
...
#
#   dj_timeout_mechanism
#       Prints the mechanism dj_run_timed will use on this host: "timeout",
#       "gtimeout", "perl", or "bash". Set DJ_TIMEOUT_MECHANISM_OVERRIDE=bash
#       to force the dependency-free fallback.
#
#   dj_run_timed <seconds> <command> [args...]
...
dj_timeout_mechanism() {
  if [ "${DJ_TIMEOUT_MECHANISM_OVERRIDE:-}" = bash ]; then
    printf 'bash\n'
...
dj_run_bash_timeout() {
  ...
  shift
  command_status=$(mktemp "${TMPDIR:-/tmp}/dj-bash-timeout-command.XXXXXX" 2>/dev/null) || return 124
...
dj_run_external_timeout() {
  local runner=$1 seconds=$2 status_file runner_pid runner_rc command_rc
  shift 2
  status_file=$(mktemp "${TMPDIR:-/tmp}/dj-timeout-status.XXXXXX" 2>/dev/null) || return 124
...
dj_run_timed() {  # <seconds> <command...>
  local seconds=$1
  shift
  case "$(dj_timeout_mechanism)" in
    timeout) dj_run_external_timeout timeout "$seconds" "$@" ;;
    gtimeout) dj_run_external_timeout gtimeout "$seconds" "$@" ;;
    perl)
      perl -e 'my $t = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0); exec @ARGV } local $SIG{ALRM} = sub { kill "TERM", -$pid; select undef, undef, undef, 0.2; kill "KILL", -$pid; exit 124 }; alarm $t; waitpid $pid, 0; exit($? >> 8)' \
        "$seconds" "$@"
      ;;
    bash) dj_run_bash_timeout "$seconds" "$@" ;;
    *) return 124 ;;
  esac
}
