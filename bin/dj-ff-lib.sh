...
# Shared fast-forward machinery for jarvis self-sync.
# Usage: . bin/dj-ff-lib.sh   (after DJ_ROOT and DJ_HOME are set)
#
# This is the one implementation of "advance a jarvis checkout to a base by a
# clean fast-forward, never forcing, merging, or stashing" used by every sync
# path:
#   - /updatejarvis (bin/dj-update.sh) pulls from origin: base_mode "origin".
#   - the local-HEAD secondmate sync (bin/dj-spawn.sh on launch, bin/dj-bootstrap.sh
...
# The seeded .dj-secondmate-home identity marker is gitignored too; the local
...
SUB_HOME_MARKER="${SUB_HOME_MARKER:-.dj-secondmate-home}"
# shellcheck source=bin/dj-secondmate-registry-lib.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/dj-secondmate-registry-lib.sh"
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the active jarvis home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the jarvis repo"
      return 1
    fi
  done
}
...
validate_secondmate_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$DJ_HOME") || {
    VALIDATION_ERROR="active jarvis home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$DJ_ROOT") || {
    VALIDATION_ERROR="jarvis repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
  ...
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="secondmate home cannot be the active jarvis home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="secondmate home cannot be the jarvis repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the active jarvis home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the jarvis repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the active jarvis home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the jarvis repo"
    return 1
  fi
  ...
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not a jarvis home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not a jarvis home (missing bin/)"
    return 1
  fi
...
#   origin       - fetch origin and advance to origin/<default> (the /updatejarvis
...
# dj-<id> task selector into FF_NUDGE_WINDOWS when it should be live-converged.
# Args:
...
# jarvis repo itself (DJ_ROOT) is never processed as its own secondmate, and
# each resolved home is processed at most once.
process_secondmate() {
  local id=$1 home=$2 window=${3:-} base_mode=$4 nudge_requires_instr=${5:-no} home_real dj_root_real
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  dj_root_real=$(resolve_path "$DJ_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$dj_root_real" ] || return 0
  ...
  if [ "$FF_STATUS" = "updated" ] && [ -n "$window" ]; then
    ...
    if [ "$nudge_requires_instr" = yes ] && [ -n "$FF_INSTR" ] \
      && type dj_ff_after_instruction_update >/dev/null 2>&1; then
      dj_ff_after_instruction_update "$id" "$home_real" "$window" "$FF_INSTR"
    fi
  fi
}
...
sweep_live_secondmate_metas() {
  local state=$1 base_mode=$2 nudge_requires_instr=${3:-no} registry=${4:-$DJ_HOME/data/secondmates.md} id home window meta
  [ -d "$state" ] || return 0
