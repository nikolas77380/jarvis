#!/usr/bin/env bash
# dj-branch-outcome.sh - the durable outcome store for the Pi supervision
...
#     available through the main session's dj_branch_outcomes tool.
...
#     when the session holds the lock (dj-session-start.sh owns the call site).
set -eu
...
usage() {
  echo "usage: dj-branch-outcome.sh append --task <id> --verdict routine|captain --summary <text> [--wake <text>] [--silent true|false] | unread | mark-read --through <seq> | list [--recent <n>] | startup-replay" >&2
  exit 2
}
