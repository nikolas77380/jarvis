...
#
#   2. The STATUS -> ACTION POLICY TABLE (dj_transition_policy) - the SINGLE
...
# (dj_backend_herdr_wait_transition) for the herdr producer and bin/dj-watch.sh
...
DJ_TRANSITION_FIELD_SEP=$'\t'

# dj_transition_record: THE constructor for a normalized transition record.
...
dj_transition_record() {  # <pane_id> <workspace_id> <from_status> <to_status> <agent>
  local pane_id ws from to agent
  pane_id=$(dj_transition_clean_field "${1:-}")
  ws=$(dj_transition_clean_field "${2:-}")
  from=$(dj_transition_clean_field "${3:-}")
  to=$(dj_transition_clean_field "${4:-}")
  agent=$(dj_transition_clean_field "${5:-}")
  printf '%s\t%s\t%s\t%s\t%s' "$pane_id" "$ws" "$from" "$to" "$agent"
}

# dj_transition_clean_field: collapse any TAB/CR/LF in a field value to spaces
# so a stray control char can never desync the fixed five-field record.
dj_transition_clean_field() {  # <value>
  printf '%s' "${1:-}" | LC_ALL=C tr '\t\r\n' '   '
}

# Field accessors (1-based), so consumers never hardcode the column layout.
dj_transition_field() {  # <record> <n>
  printf '%s' "$1" | cut -d"$DJ_TRANSITION_FIELD_SEP" -f"$2"
}

dj_transition_pane_id()      { dj_transition_field "$1" 1; }
dj_transition_workspace_id() { dj_transition_field "$1" 2; }
dj_transition_from_status()  { dj_transition_field "$1" 3; }
dj_transition_to_status()    { dj_transition_field "$1" 4; }
dj_transition_agent()        { dj_transition_field "$1" 5; }

# dj_transition_policy: THE single-owner status -> supervision-action table.
...
# immediate action.
dj_transition_policy() {  # <to_status> -> actionable|absorb|defer|fallback
  case "$1" in
