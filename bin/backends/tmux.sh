...
#
# Reference backend (AGENTS.md section 8; data/dj-backend-design-d7). P1 moves
# the tmux command sequences that dj-send.sh, dj-peek.sh, dj-watch.sh,
# dj-spawn.sh, and dj-teardown.sh already ran inline into named functions
...
# through bin/dj-backend.sh's dj_backend_source, never directly.
#
...
# provider, not the worktree provider, so dj-spawn.sh still drives that part
...
# already live in bin/dj-tmux-lib.sh, shared with the away-mode daemon
# (bin/dj-supervise-daemon.sh); this adapter sources that file and re-exports
...
. "$DJ_BACKEND_LIB_DIR/dj-tmux-lib.sh"
# shellcheck source=bin/dj-session-lock-lib.sh
. "$DJ_BACKEND_LIB_DIR/dj-session-lock-lib.sh"
# shellcheck source=bin/dj-cursor-lib.sh
. "$DJ_BACKEND_LIB_DIR/dj-cursor-lib.sh"

# dj_backend_tmux_resolve_bare_selector: the live-window-listing fallback for a
...
# dj-send.sh's and dj-peek.sh's own (until now duplicated) resolve().
dj_backend_tmux_resolve_bare_selector() {  # <name>
  local name=$1
  tmux list-windows -a -F '#{session_name}:#{window_name}' | grep -m1 ":$name\$" \
    || { echo "error: no window named $name" >&2; return 1; }
}

# dj_backend_tmux_capture: bounded plain-text pane capture. Mirrors
# dj-peek.sh's and dj-watch.sh's `tmux capture-pane -p -t "$T" -S -"$N"`.
dj_backend_tmux_capture() {  # <target> <lines>
  tmux capture-pane -p -t "$1" -S -"$2"
}

# dj_backend_tmux_send_key: one named key. Mirrors dj-send.sh's --key path:
# `tmux display-message -p -t "$T" '#{pane_id}' >/dev/null`, then
# `tmux send-keys -t "$T" "$2"`.
dj_backend_tmux_send_key() {  # <target> <key>
  tmux display-message -p -t "$1" '#{pane_id}' >/dev/null
  tmux send-keys -t "$1" "$2"
}

# dj_backend_tmux_send_text_submit: type <text> into <target> once, then
# submit with Enter, retried (Enter only, never retyped) until the composer
# clears. Re-exports dj_tmux_submit_core (bin/dj-tmux-lib.sh) verbatim; see
# that file for the composer-verification contract and echoed verdicts.
dj_backend_tmux_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle>
  dj_tmux_submit_core "$@"
}

# dj_backend_tmux_container_ensure: reuse the current tmux session when
# jarvis itself runs inside tmux, else ensure a dedicated detached
# "jarvis" session exists. Mirrors dj-spawn.sh's container-ensure block;
# prints the resolved session name.
dj_backend_tmux_container_ensure() {
  if [ -n "${TMUX:-}" ]; then
    tmux display-message -p '#S'
  else
    tmux has-session -t jarvis 2>/dev/null || tmux new-session -d -s jarvis
    printf 'jarvis'
  fi
}

# dj_backend_tmux_create_task: create the task's window in <proj-abs>,
# refusing an existing <window-name> in <session>. Mirrors dj-spawn.sh's
# duplicate-check-then-new-window sequence, including the exact error text
# (session:window, matching how dj-spawn.sh composed its own $T). Prints the
# created window's stable window id on stdout for the caller to target.
#
# Robustness (dj-spawn tmux window handling under a non-default captain config):
...
#     new window: the captain's tmux may rename the window away from dj-<id> once
#     treehouse cd's into the worktree, which would break name-based targeting.
# The returned window id lets callers target the window even if its name is ever
# lost, so worktree discovery cannot fall back to the active client's window.
dj_backend_tmux_create_task() {  # <session> <window-name> <proj-abs> -> prints window id
...
# dj_backend_tmux_current_path: the live pane's current working directory, or
# empty on any tmux error. Mirrors dj-spawn.sh's worktree-discovery poll:
# `tmux display-message -p -t "$T" '#{pane_current_path}'`.
dj_backend_tmux_current_path() {  # <target>
  tmux display-message -p -t "$1" '#{pane_current_path}' 2>/dev/null
}

# dj_backend_tmux_send_text_line: send one line of TEXT then Enter, with no
# composer verification - used for the fixed spawn-time commands
# (`treehouse get`, the GOTMPDIR export) that already ran this exact sequence
# inline in dj-spawn.sh. Mirrors `tmux send-keys -t "$T" "<text>" Enter`.
dj_backend_tmux_send_text_line() {  # <target> <text>
  tmux send-keys -t "$1" "$2" Enter
}

# dj_backend_tmux_send_literal: send TEXT as literal bytes with no
# submission - the caller sends Enter separately (dj-spawn.sh's launch-command
# send pauses between the literal send and Enter for the harness to settle).
# Mirrors `tmux send-keys -t "$T" -l "<text>"`.
dj_backend_tmux_send_literal() {  # <target> <text>
  tmux send-keys -t "$1" -l "$2"
}

# dj_backend_tmux_kill: remove one explicitly named task window, best-effort.
...
# dj_backend_tmux_current_command: <target>'s live foreground process name -
...
dj_backend_tmux_current_command() {  # <target>
  tmux display-message -p -t "$1" '#{pane_current_command}' 2>/dev/null
}

# dj_backend_tmux_classify_process_name: the single owner of the process-name
...
dj_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # COMPONENT, so the dj_harness_path_name fallback below never fires for it.
    muse|muse-bin-*) printf 'agent' ;;
    *claude*|*codex*|*opencode*|*grok*|*kimi*|pi|pi-signed|pi-launcher|Pi) printf 'agent' ;;
    zsh|bash|sh|dash|ash|ksh|mksh|tcsh|csh|fish) printf 'shell' ;;
    *)
      if dj_harness_path_name "$path" >/dev/null || dj_harness_path_name "$argv0" >/dev/null; then
        printf 'agent'
      ...
      # comes from the narrowed structural rule in bin/dj-cursor-lib.sh, which
      ...
      elif dj_cursor_process_matches "${path:-$argv0}" '' "$argv0"; then
        printf 'agent'
      else
        printf 'other'
      fi
...
# dj_backend_tmux_foreground_comms: the kernel-side names of every process in
...
#
# Like dj_backend_tmux_current_command this is a RAW pane read: tmux answers an
...
dj_backend_tmux_foreground_comms() {  # <target>
...
dj_backend_tmux_foreground_argv0s() {  # <target>
...
# dj_backend_tmux_agent_state: recovery-grade harness-agent state for one
# recorded target. See bin/dj-backend.sh's dj_backend_agent_state for the
...
dj_backend_tmux_agent_state() {  # <target>
  ...
  foreground=$(dj_backend_tmux_foreground_comms "$target")
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    fg_seen=1
    case "$(dj_backend_tmux_classify_process_name "$name")" in
...
EOF
  ...
  argv0s=$(dj_backend_tmux_foreground_argv0s "$target")
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    if [ "$(dj_backend_tmux_classify_process_name '' "$name")" = agent ]; then
      printf 'alive'
      return 0
    fi
  done <<EOF
...
EOF
  ...
  comm=$(dj_backend_tmux_current_command "$target") || {
    printf 'unreadable'
    return 0
  }
  if [ "$(dj_backend_tmux_classify_process_name "$comm")" = agent ]; then
    printf 'alive'
    return 0
  fi
  ...
  case "$(dj_backend_tmux_classify_process_name "$comm")" in
    shell) printf 'dead' ;;
    *) printf 'ambiguous' ;;
  esac
}
...
dj_backend_tmux_agent_alive() {  # <target>
  case "$(dj_backend_tmux_agent_state "$1")" in
    alive) printf 'alive' ;;
