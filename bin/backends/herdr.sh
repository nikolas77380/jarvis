...
#
# Design: data/dj-backend-design-d7/herdr-addendum.md ("Interface mapping",
# decisions D1-D6) and the empirical verification recorded in
# data/dj-backend-design-d7/herdr-verification-p2.md (real herdr v0.7.1,
...
# like tmux. Sourced only through bin/dj-backend.sh's dj_backend_source in
# normal operation; the unit tests source it directly, so the DJ_HOME fallback
# below keeps that path sane without dj-backend.sh's preamble.
#
...
# JARVIS HOME (the primary, and each secondmate, gets its own), ONE herdr TAB
...
# default-on projection (DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION). Projected cleanup
...
# remainder is the whole pane id - dj_backend_herdr_parse_target splits on the
# first colon only). This is the value stored in a herdr task's meta window=
# field and is what dj_backend_resolve_selector already returns unchanged for
# exact task-id, legacy dj-<id>, and explicit backend-target forms (that
...
# verification doc) uses LABEL matching (dj-<id> tab labels), never trusts a
# stored pane id blindly: dj_backend_herdr_list_live. The presentation journal
# is deliberately excluded from that path.
#
# Requires: herdr (CLI + socket), jq (JSON parsing). Bootstrap detects these
# through dj_backend_required_tools only when herdr is the resolved backend;
# this adapter also gates them again before spawning.

# DJ_HOME fallback: every real caller (dj-spawn.sh, dj-peek.sh, dj-send.sh,
# dj-teardown.sh, dj-watch.sh, dj-crew-state.sh) already sets DJ_HOME as a
# global before sourcing dj-backend.sh (which sources this file), so this
...
# default (the jarvis repo root - never a secondmate home, so
# dj_backend_herdr_workspace_label falls through to "jarvis" exactly like
# pre-P3 behavior when a test does not care about home-specific labeling).
DJ_BACKEND_HERDR_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
DJ_ROOT="${DJ_ROOT_OVERRIDE:-${DJ_ROOT:-$DJ_BACKEND_HERDR_ROOT}}"
DJ_HOME="${DJ_HOME:-${DJ_ROOT_OVERRIDE:-$DJ_ROOT}}"

# Shared composer-content classifier (empty|pending|unknown, and the fleet-wide
# dead-shell-vs-agent-composer rule). Owned by bin/dj-composer-lib.sh, reused by
# every backend so the decision cannot drift.
# shellcheck source=bin/dj-composer-lib.sh
. "$DJ_BACKEND_HERDR_ROOT/bin/dj-composer-lib.sh"

# Shared, backend-neutral normalized-transition shape and the single-owner
# status->action policy table (bin/dj-transition-lib.sh). This adapter's event
# subscriber (dj_backend_herdr_wait_transition) normalizes every
# pane.agent_status_changed edge through dj_transition_record and routes it
# through dj_transition_policy - it never re-encodes the mapping.
# shellcheck source=bin/dj-transition-lib.sh
. "$DJ_BACKEND_HERDR_ROOT/bin/dj-transition-lib.sh"

DJ_BACKEND_HERDR_MIN_PROTOCOL=14
...
# (dj_backend_herdr_events_capable). Distinct from DJ_BACKEND_HERDR_MIN_PROTOCOL
# (14): the adapter's spawn/capture/send primitives work on 14, only the push
# subscriber needs 16.
DJ_BACKEND_HERDR_MIN_EVENTS_PROTOCOL=16
...
DJ_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL=16
...
DJ_BACKEND_HERDR_MIN_PRESENTATION_PROTOCOL=19
DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION=0.8.0
...
DJ_BACKEND_HERDR_PRESENTATION_FLOOR_MARKER_PREFIX=".herdr-presentation-floor-"
...
# blocked pane. Mirrors bin/dj-watch.sh's .stale-<key> naming.
DJ_BACKEND_HERDR_ESCALATED_PREFIX=".herdr-escalated-"
# .dj-secondmate-home is written by bin/dj-home-seed.sh (AGENTS.md section 6)
# at a seeded secondmate home's root, containing exactly that secondmate's id.
# The primary jarvis home never carries this marker.
DJ_BACKEND_HERDR_SECONDMATE_MARKER=".dj-secondmate-home"
...
DJ_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX=".herdr-presentation"

# The config item a home writes to opt out of, or explicitly in to, the
# projection.
DJ_BACKEND_HERDR_PRESENTATION_CONFIG="herdr-presentation-spaces"

# dj_backend_herdr_presentation_preference <config-dir>: the single owner of
...
dj_backend_herdr_presentation_preference() {  # <config-dir>
  local config_dir=${1:-} file value
  [ -n "$config_dir" ] || { printf 'default\n'; return 0; }
  file="$config_dir/$DJ_BACKEND_HERDR_PRESENTATION_CONFIG"
...
# dj_backend_herdr_version_at_least <candidate> <floor>: numeric dotted-release
...
dj_backend_herdr_version_at_least() {  # <candidate> <floor>
...
# dj_backend_herdr_release_floor_verdict <protocol> <version>: the pure
...
dj_backend_herdr_release_floor_verdict() {  # <protocol> <version>
  local protocol=${1:-} version=${2:-} protocol_known=0 version_status=0
  case "$protocol" in
    ''|*[!0-9]*) ;;
    *)
      protocol_known=1
      [ "$protocol" -ge "$DJ_BACKEND_HERDR_MIN_PRESENTATION_PROTOCOL" ] && return 0
      ;;
  esac
  dj_backend_herdr_version_at_least "$version" "$DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION" \
    || version_status=$?
...
# dj_backend_herdr_presentation_release_supported: run the floor classifier
...
# dj_backend_herdr_release_floor_verdict, and sets
# DJ_BACKEND_HERDR_PRESENTATION_RELEASE to the identifier a caller's warning
# names. An unreadable server-running state is indeterminate rather than
# permission to substitute the client release.
dj_backend_herdr_presentation_release_supported() {  # [<session>]
  ...
  DJ_BACKEND_HERDR_PRESENTATION_RELEASE="an unreadable release"
  command -v herdr >/dev/null 2>&1 || return 2
  command -v jq >/dev/null 2>&1 || return 2
  [ -n "$session" ] || session=$(dj_backend_herdr_session)
  status=$(dj_backend_herdr_cli "$session" status --json 2>/dev/null) || return 2
  ...
  dj_backend_herdr_release_floor_verdict "$client_protocol" "$client_version" || client_verdict=$?
  ...
  case "$running" in
    true)
      ...
      dj_backend_herdr_release_floor_verdict "$server_protocol" "$server_version" || server_verdict=$?
      if [ "$server_verdict" -eq 1 ]; then
        DJ_BACKEND_HERDR_PRESENTATION_RELEASE="server version ${server_version:-unknown} (protocol ${server_protocol:-unknown})"
        return 1
      fi
      if [ "$client_verdict" -eq 1 ]; then
        DJ_BACKEND_HERDR_PRESENTATION_RELEASE="version ${client_version:-unknown} (protocol ${client_protocol:-unknown})"
        return 1
      fi
...
# dj_backend_herdr_presentation_floor_warn <state-dir> <verdict>: emit the one
...
dj_backend_herdr_presentation_floor_warn() {  # <state-dir> <verdict>
  local state_dir=${1:-} verdict=${2:-2} release=${DJ_BACKEND_HERDR_PRESENTATION_RELEASE:-an unreadable release} key marker reason tmp=""
  if [ "$verdict" -eq 1 ]; then
    reason="herdr $release is older than the $DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION floor for presentation spaces, where projected cleanup can steal the active workspace"
  else
    reason="the selected herdr release could not be read, so the $DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION floor for presentation spaces cannot be verified"
  fi
  if [ -n "$state_dir" ] && [ -d "$state_dir" ] && [ ! -L "$state_dir" ]; then
    key=${release//[^a-zA-Z0-9]/-}
    marker="$state_dir/$DJ_BACKEND_HERDR_PRESENTATION_FLOOR_MARKER_PREFIX$key"
  ...
  echo "warning: $reason; using the ordinary flat layout instead. Upgrade herdr to $DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION or newer (herdr update) to restore the projection, or write \"on\" into config/$DJ_BACKEND_HERDR_PRESENTATION_CONFIG to force it on this release." >&2
  return 0
}

# dj_backend_herdr_presentation_default_supported <state-dir> [<session>]:
...
dj_backend_herdr_presentation_default_supported() {  # <state-dir> [<session>]
  ...
  dj_backend_herdr_presentation_release_supported "$session" || verdict=$?
  [ "$verdict" -eq 0 ] && return 0
  dj_backend_herdr_presentation_floor_warn "$state_dir" "$verdict"
  return 1
}

# dj_backend_herdr_presentation_enabled <config-dir> [<state-dir>]: the one gate
# bin/dj-spawn.sh consults before projecting this home's children into
...
# DJ_BACKEND_HERDR_PRESENTATION_PREFERENCE for the new-projection boundary to
# distinguish an unconfigured default from an explicit opt-in.
dj_backend_herdr_presentation_enabled() {  # <config-dir> [<state-dir>]
  local config_dir=${1:-} state_dir=${2:-} preference
  preference=$(dj_backend_herdr_presentation_preference "$config_dir")
  # bin/dj-spawn.sh reads this out-parameter after sourcing this adapter.
  # shellcheck disable=SC2034
  DJ_BACKEND_HERDR_PRESENTATION_PREFERENCE=$preference
  case "$preference" in
    off) return 1 ;;
    on) return 0 ;;
  esac
  dj_backend_herdr_presentation_default_supported "$state_dir"
}

# dj_backend_herdr_workspace_label: the per-jarvis-HOME herdr workspace
# label (docs/herdr-backend.md "Default task container shape"). The PRIMARY home (no
# secondmate marker) resolves to the constant "jarvis", byte-identical to
...
# other secondmate's) in herdr's spaces sidebar. Read fresh from DJ_HOME on
# every call rather than cached at source time: DJ_HOME is the home's own
...
# that home. dj-spawn.sh briefly shadows DJ_HOME to a secondmate's own home
# when the PRIMARY spawns that secondmate (its own process's DJ_HOME still
# names the primary at that point) - see dj-spawn.sh's herdr case arm.
dj_backend_herdr_workspace_label() {
  local marker="$DJ_HOME/$DJ_BACKEND_HERDR_SECONDMATE_MARKER" id
  if [ -f "$marker" ]; then
...
# dj_backend_herdr_cli: run `herdr <args...>` scoped to <session>, setting
...
# dj_backend_herdr_version_check, which is intentionally session-independent
# (reads only .client.* fields).
dj_backend_herdr_cli() {  # <session> <herdr-subcommand-and-args...>
  local session=$1
  shift
  HERDR_SESSION="$session" herdr "$@" --session "$session"
}

# dj_backend_herdr_tool_check: refuse loudly if herdr or jq is missing.
dj_backend_herdr_tool_check() {
...
# dj_backend_herdr_version_check: refuse loudly on a missing/incompatible
...
dj_backend_herdr_version_check() {
  dj_backend_herdr_tool_check || return 1
  ...
  if [ "$protocol" -lt "$DJ_BACKEND_HERDR_MIN_PROTOCOL" ]; then
    echo "error: herdr protocol $protocol (version ${version:-unknown}) is older than the verified minimum $DJ_BACKEND_HERDR_MIN_PROTOCOL; update herdr (herdr update) before using backend=herdr" >&2
    return 1
  fi
  return 0
}

# dj_backend_herdr_session: resolve which named herdr session this normal
# spawn/op uses. HERDR_SESSION mirrors tmux's $TMUX ambient-selection for
# adapter workspace/tab/pane operations: an operator (or jarvis's own
...
dj_backend_herdr_session() {
  printf '%s' "${HERDR_SESSION:-default}"
}

# dj_backend_herdr_projection_id: generate a compact 128-bit base64url token.
# The token is a non-adversarial visual correlator, never destructive
# authority.
dj_backend_herdr_projection_id() {
  local token
...
dj_backend_herdr_projection_journal_path() {  # <state-dir> <task-id>
  printf '%s/%s%s' "$1" "$2" "$DJ_BACKEND_HERDR_PRESENTATION_JOURNAL_SUFFIX"
}

# dj_backend_herdr_projection_journal_create: atomically publish the
...
dj_backend_herdr_projection_journal_create() {  # <state-dir> <task-id>
...
dj_backend_herdr_projection_journal_field() {  # <journal> <key>
...
# dj_backend_herdr_projection_journal_snapshot: validate a version 1 attempt
# journal or a version 2 exact projection binding without sourcing shell code.
# Version 2 sets DJ_BACKEND_HERDR_JOURNAL_* globals for same-process callers.
dj_backend_herdr_projection_journal_snapshot() {  # <journal> <task-id>
  local journal=$1 id=$2 lines expected_label expected_task_label exact
  DJ_BACKEND_HERDR_JOURNAL_VERSION=""
  DJ_BACKEND_HERDR_JOURNAL_TASK_ID=""
  DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID=""
  DJ_BACKEND_HERDR_JOURNAL_HOME=""
  DJ_BACKEND_HERDR_JOURNAL_SESSION=""
  ...
  DJ_BACKEND_HERDR_JOURNAL_PARENT_LABEL=""
  DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL=""
  DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL=""
  [ -f "$journal" ] && [ ! -L "$journal" ] || return 1
  lines=$(wc -l < "$journal" 2>/dev/null | tr -d '[:space:]')
  DJ_BACKEND_HERDR_JOURNAL_VERSION=$(dj_backend_herdr_projection_journal_field "$journal" version) || return 1
  DJ_BACKEND_HERDR_JOURNAL_TASK_ID=$(dj_backend_herdr_projection_journal_field "$journal" task_id) || return 1
  DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID=$(dj_backend_herdr_projection_journal_field "$journal" projection_id) || return 1
  [ "$DJ_BACKEND_HERDR_JOURNAL_TASK_ID" = "$id" ] || return 1
  [ "${#DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID}" -eq 22 ] || return 1
  case "$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID" in
    *[!A-Za-z0-9_-]*) return 1 ;;
  esac
  case "$DJ_BACKEND_HERDR_JOURNAL_VERSION:$lines" in
    1:3) return 0 ;;
    2:12) ;;
    *) return 1 ;;
  esac
  DJ_BACKEND_HERDR_JOURNAL_HOME=$(dj_backend_herdr_projection_journal_field "$journal" home) || return 1
  DJ_BACKEND_HERDR_JOURNAL_SESSION=$(dj_backend_herdr_projection_journal_field "$journal" session) || return 1
  DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_ID=$(dj_backend_herdr_projection_journal_field "$journal" workspace_id) || return 1
  DJ_BACKEND_HERDR_JOURNAL_TAB_ID=$(dj_backend_herdr_projection_journal_field "$journal" tab_id) || return 1
  DJ_BACKEND_HERDR_JOURNAL_PANE_ID=$(dj_backend_herdr_projection_journal_field "$journal" pane_id) || return 1
  DJ_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID=$(dj_backend_herdr_projection_journal_field "$journal" parent_workspace_id) || return 1
  DJ_BACKEND_HERDR_JOURNAL_PARENT_LABEL=$(dj_backend_herdr_projection_journal_field "$journal" parent_label) || return 1
  DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL=$(dj_backend_herdr_projection_journal_field "$journal" workspace_label) || return 1
  DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL=$(dj_backend_herdr_projection_journal_field "$journal" task_label) || return 1
  case "$DJ_BACKEND_HERDR_JOURNAL_HOME" in
    /*) ;;
    *) return 1 ;;
  esac
  for exact in \
    ...
    "$DJ_BACKEND_HERDR_JOURNAL_TAB_ID" \
    "$DJ_BACKEND_HERDR_JOURNAL_PANE_ID" \
    "$DJ_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID"; do
    case "$exact" in
      ''|*[[:space:]]*) return 1 ;;
    esac
  done
  [ -n "$DJ_BACKEND_HERDR_JOURNAL_PARENT_LABEL" ] \
    && [ -n "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" ] \
    && [ -n "$DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL" ] || return 1
  expected_label=$(dj_backend_herdr_projection_workspace_label "$id" "$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID")
  expected_task_label="dj-$id"
  [ "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" = "$expected_label" ] \
    && [ "$DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL" = "$expected_task_label" ]
}

# dj_backend_herdr_projection_journal_token: validate and read either journal
# version's non-authoritative visual correlator.
dj_backend_herdr_projection_journal_token() {  # <journal> <task-id>
  dj_backend_herdr_projection_journal_snapshot "$1" "$2" || return 1
  printf '%s' "$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID"
}

dj_backend_herdr_projection_home_identity() {  # <home>
  local home=$1
  [ -d "$home" ] || return 1
  (cd "$home" 2>/dev/null && pwd -P)
}

dj_backend_herdr_projection_journal_write_v2() {  # <journal> <task-id> <token> <home> <session> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
...
# dj_backend_herdr_projection_journal_bind: upgrade one exact version 1
...
dj_backend_herdr_projection_journal_bind() {  # <journal> <task-id> <home> <session> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
  ...
  dj_backend_herdr_projection_journal_snapshot "$journal" "$id" || return 1
  [ "$DJ_BACKEND_HERDR_JOURNAL_VERSION" = 1 ] || return 1
  token=$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID
  dj_backend_herdr_projection_journal_write_v2 \
    "$journal" "$id" "$token" "$home" "$session" "$workspace" "$tab" "$pane" \
    "$parent_workspace" "$parent_label" "$workspace_label" "$task_label"
}

# dj_backend_herdr_projection_journal_replace_endpoint: atomically advance one
# exact version 2 binding after its old husk was replaced successfully.
dj_backend_herdr_projection_journal_replace_endpoint() {  # <journal> <task-id> <old-tab> <old-pane> <new-tab> <new-pane>
  local journal=$1 id=$2 old_tab=$3 old_pane=$4 new_tab=$5 new_pane=$6
  dj_backend_herdr_projection_journal_snapshot "$journal" "$id" || return 1
  [ "$DJ_BACKEND_HERDR_JOURNAL_VERSION" = 2 ] \
    && [ "$DJ_BACKEND_HERDR_JOURNAL_TAB_ID" = "$old_tab" ] \
    && [ "$DJ_BACKEND_HERDR_JOURNAL_PANE_ID" = "$old_pane" ] || return 1
  dj_backend_herdr_projection_journal_write_v2 \
    "$journal" "$id" "$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID" \
    "$DJ_BACKEND_HERDR_JOURNAL_HOME" "$DJ_BACKEND_HERDR_JOURNAL_SESSION" \
    "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" "$new_tab" "$new_pane" \
    "$DJ_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" "$DJ_BACKEND_HERDR_JOURNAL_PARENT_LABEL" \
    "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" "$DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL"
}

# dj_backend_herdr_projection_concise_task_label: strip redundant owner
# prefixes from a task id used only in the presentation workspace label.
# Removes jarvis/, 2ndmate-<id>/, and a presentation-level dj- owner
# prefix when present. The ordinary task tab remains dj-<id> and is not
# built by this helper.
dj_backend_herdr_projection_concise_task_label() {  # <task-id>
  local task=$1
  case "$task" in
    jarvis/*) task=${task#jarvis/} ;;
    2ndmate-*/*) task=${task#*/} ;;
  esac
  case "$task" in
    dj-*) task=${task#dj-} ;;
...
# dj_backend_herdr_projection_workspace_label: presentation-only child label.
...
dj_backend_herdr_projection_workspace_label() {  # <task-id> <projection-id>
  printf '└ %s · p:%s' "$(dj_backend_herdr_projection_concise_task_label "$1")" "$2"
}

# dj_backend_herdr_presentation_session_lock_path: one machine-private lock
# path per live named Herdr session/socket, shared across every Jarvis home
...
dj_backend_herdr_presentation_lock_namespace() {
  printf '%s' '/tmp/jarvis-herdr-presentation'
}

dj_backend_herdr_presentation_lock_namespace_mode() {
...
dj_backend_herdr_presentation_lock_namespace_uid() {
  if [ "$(uname -s 2>/dev/null)" = Darwin ]; then
...
dj_backend_herdr_presentation_lock_namespace_valid() {
  local dir=$1 expected_uid owner mode
  [ -d "$dir" ] && [ ! -L "$dir" ] || return 1
  expected_uid=$(id -u 2>/dev/null) || return 1
  owner=$(dj_backend_herdr_presentation_lock_namespace_uid "$dir") || return 1
  mode=$(dj_backend_herdr_presentation_lock_namespace_mode "$dir") || return 1
...
# dj_backend_herdr_canonical_socket_path: normalize one absolute Unix-socket
...
# proof both use it).
dj_backend_herdr_canonical_socket_path() {  # <socket-path>
...
dj_backend_herdr_presentation_session_socket_path() {  # <session>
  local session=$1 sessions socket
  [ -n "$session" ] || return 1
  sessions=$(dj_backend_herdr_cli "$session" session list --json 2>/dev/null) || return 1
  ...
  ' 2>/dev/null) || return 1
  dj_backend_herdr_canonical_socket_path "$socket"
}

dj_backend_herdr_presentation_session_lock_path() {  # <session>
  local session=$1 socket key dir hash
  [ -n "$session" ] || return 1
  socket=$(dj_backend_herdr_presentation_session_socket_path "$session") || return 1
  ...
  [ -n "$hash" ] || return 1
  key=${hash:0:32}
  dir=$(dj_backend_herdr_presentation_lock_namespace) || return 1
  [ -n "$dir" ] || return 1
  if [ ! -e "$dir" ] && [ ! -L "$dir" ]; then
    if ! mkdir -m 700 "$dir" 2>/dev/null; then
      dj_backend_herdr_presentation_lock_namespace_valid "$dir" || return 1
    fi
  fi
  dj_backend_herdr_presentation_lock_namespace_valid "$dir" || return 1
...
# dj_backend_herdr_projection_focus_snapshot: print the exact active
...
dj_backend_herdr_projection_focus_snapshot() {  # <session>
  local session=$1 list snapshot workspace tab tabs
  list=$(dj_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 1
  ...
  tabs=$(dj_backend_herdr_cli "$session" tab list --workspace "$workspace" 2>/dev/null) || return 1
...
# dj_backend_herdr_projection_focus_restore: verify that one presentation
...
dj_backend_herdr_projection_focus_restore() {  # <session> <snapshot> <operation>
  ...
  after=$(dj_backend_herdr_projection_focus_snapshot "$session") || after=
  [ "$after" != "$before" ] || return 0
  workspace=${before%%$'\t'*}
  tab=${before#*$'\t'}
  info=$(dj_backend_herdr_cli "$session" tab get "$tab" 2>/dev/null) || {
  ...
  dj_backend_herdr_cli "$session" tab focus "$tab" >/dev/null 2>&1 || {
    echo "warning: herdr presentation $operation changed focus and exact-tab restoration failed" >&2
    return 1
  }
  restored=$(dj_backend_herdr_projection_focus_snapshot "$session") || restored=
...
# dj_backend_herdr_projection_close_pane_focus_preserving: close one exact
...
# dj_backend_herdr_emptying_close_plan: reposition the doomed workspace
...
dj_backend_herdr_projection_close_pane_focus_preserving() {  # <session> <pane-id> [required-agent-state]
  ...
  DJ_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE=""
  [ -n "$pane_id" ] || return 0
  before=$(dj_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  active_tab=${before#*$'\t'}
  info=$(dj_backend_herdr_cli "$session" pane get "$pane_id" 2>/dev/null) || {
  ...
  if [ -n "$required_agent_state" ]; then
    state=$(dj_backend_herdr_pane_agent_state "$session" "$pane_id")
    DJ_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE=$state
  ...
  plan_move_record=
  if [ -n "$target_ws" ]; then
    plan=$(dj_backend_herdr_emptying_close_plan "$session" "$pane_id" "$target_ws" "$target_tab" "${before%%$'\t'*}")
    case "$plan" in
      moved$'\t'*)
  ...
  if [ "$plan" = death ]; then
    if dj_backend_herdr_death_close_pane "$session" "$pane_id" "$plan_shell_pid"; then
      close_status=0
    elif dj_backend_herdr_explicit_close_pane_confirmed "$session" "$pane_id"; then
      close_status=0
    else
      close_status=1
    fi
  ...
  if [ "$close_status" -eq 0 ] && [ -n "$plan_move_record" ]; then
    workspace_presence=$(dj_backend_herdr_workspace_presence_state "$session" "$target_ws")
  ...
  if [ "$close_status" -ne 0 ]; then
    dj_backend_herdr_emptying_move_rollback "$plan_move_record" || true
  fi
  dj_backend_herdr_projection_focus_restore "$session" "$before" "pane close" || return 2
  [ "$close_status" -eq 0 ]
}
...
# Jarvis therefore removes a doomed non-focused workspace by ending its
...
# projection is therefore floored at DJ_BACKEND_HERDR_MIN_PRESENTATION_VERSION,
...
# dj_backend_herdr_presentation_enabled, so every new projection is either on a
...
# removes and block teardown because dj-teardown treats an unconfirmed close as
...
# dj_backend_herdr_workspace_move_capable: verify that one guarded raw
...
dj_backend_herdr_workspace_move_capable() {  # <session>
  local session=$1 protocol schema
  command -v python3 >/dev/null 2>&1 || return 1
  protocol=$(dj_backend_herdr_cli "$session" status --json 2>/dev/null | jq -r '.client.protocol // empty' 2>/dev/null)
  case "$protocol" in
    ''|*[!0-9]*) return 2 ;;
  esac
  [ "$protocol" -lt "$DJ_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL" ] && return 3
  schema=$(dj_backend_herdr_cli "$session" api schema --json 2>/dev/null) || return 4
...
# dj_backend_herdr_emptying_close_plan: choose the focus-safe removal for one
...
# dj_backend_herdr_emptying_move_rollback when removal is not confirmed.
...
dj_backend_herdr_emptying_close_plan() {  # <session> <pane-id> <workspace-id> <tab-id> <focused-workspace-id>
  ...
  tabs=$(dj_backend_herdr_cli "$session" tab list --workspace "$ws_id" 2>/dev/null) || { printf 'plain\n'; return 0; }
  ...
  panes=$(dj_backend_herdr_cli "$session" pane list --workspace "$ws_id" 2>/dev/null) || { printf 'plain\n'; return 0; }
  printf '%s' "$panes" | jq -e --arg pane "$pane_id" '
    (.result.panes | type) == "array" and (.result.panes | length) == 1
    and .result.panes[0].pane_id == $pane
  ' >/dev/null 2>&1 || { printf 'plain\n'; return 0; }
  list=$(dj_backend_herdr_cli "$session" workspace list 2>/dev/null) || { printf 'plain\n'; return 0; }
  ...
  if [ "$r" -lt "$a" ] && [ "$a" -lt $((len - 1)) ]; then
    ...
    if dj_backend_herdr_workspace_move_capable "$session"; then
      capable=0
    else
      capable=$?
    fi
    ...
    socket=$(dj_backend_herdr_presentation_session_socket_path "$session") || {
    ...
    mover=${DJ_BACKEND_HERDR_WORKSPACE_MOVER:-$DJ_BACKEND_HERDR_ROOT/bin/backends/herdr-workspace-move.py}
  ...
  if shell_pid=$(dj_backend_herdr_pane_idle_shell_pid "$session" "$pane_id"); then
    printf 'death %s\n' "$shell_pid"
  else
    printf 'plain\n'
  fi
...
# dj_backend_herdr_emptying_move_rollback: restore the exact pre-move
...
dj_backend_herdr_emptying_move_rollback() {  # <move-record>
...
# dj_backend_herdr_death_close_pane: end the exact pane's proved lone idle
# shell so Herdr removes the emptied workspace through its focus-preserving
...
dj_backend_herdr_death_close_pane() {  # <session> <pane-id> <shell-pid>
  local session=$1 pane_id=$2 shell_pid=$3 ps_bin attempt max_attempts presence resampled_pid
  ps_bin=${DJ_HERDR_PS_BIN:-ps}
  case "$shell_pid" in
    ''|*[!0-9]*) return 1 ;;
  esac
  command -v "$ps_bin" >/dev/null 2>&1 || return 1
  max_attempts=${DJ_BACKEND_HERDR_DEATH_CLOSE_POLLS:-40}
  dj_backend_herdr_pid_is_bare_shell "$ps_bin" "$shell_pid" || return 1
  kill -HUP "$shell_pid" 2>/dev/null || true
  attempt=0
  while [ "$attempt" -lt "$max_attempts" ]; do
    presence=$(dj_backend_herdr_pane_presence_state "$session" "$pane_id")
  ...
  resampled_pid=$(dj_backend_herdr_pane_idle_shell_sample "$session" "$pane_id") || return 1
...
# dj_backend_herdr_pid_is_bare_shell: <pid> currently resolves to a bare
...
dj_backend_herdr_pid_is_bare_shell() {  # <ps-bin> <pid>
  local comm
...
# dj_backend_herdr_pane_idle_shell_pid: print the shell pid of <pane-id> only
...
dj_backend_herdr_pane_idle_shell_pid() {  # <session> <pane-id>
  local attempt=0 max_attempts=${DJ_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS:-10}
  while :; do
    if dj_backend_herdr_pane_idle_shell_sample "$1" "$2"; then
      return 0
    fi
    attempt=$((attempt + 1))
...
# dj_backend_herdr_pane_idle_shell_sample: one strict instantaneous
# observation for dj_backend_herdr_pane_idle_shell_pid, which owns the proof
# contract and the settle retry.
dj_backend_herdr_pane_idle_shell_sample() {  # <session> <pane-id>
  local session=$1 pane=$2 info shell_pid foreground_pgid count
  local process_pid name argv0 shell_name rows stat ps_bin
  info=$(dj_backend_herdr_cli "$session" pane process-info --pane "$pane" 2>/dev/null) || return 1
...
# dj_backend_herdr_projection_order_best_effort: place the exact workspace id
# returned by THIS projected create immediately after its owning parent's
# contiguous child block and before the next parent.
#
# <parent-label> is the owning DJ_HOME label (jarvis or 2ndmate-<id>).
...
# adjacent old-format jarvis/... or 2ndmate-<id>/... projections may extend
...
dj_backend_herdr_projection_order_best_effort() {  # <session> <created-workspace-id> <parent-label> [<parent-workspace-id>]
  ...
  list=$(dj_backend_herdr_cli "$session" workspace list 2>/dev/null) || {
  ...
  analysis=$(printf '%s' "$list" | jq -c --arg created "$created" --arg parent "$parent" --arg parent_ws "$parent_ws" '
    ...
    def is_top_level_parent:
      (.label | type) == "string"
      and ((.label == "jarvis") or (.label | test("^2ndmate-[^/]+$")));
    def is_new_child:
      (.label | type) == "string"
      and (.label | test("^└ .+ · p:[A-Za-z0-9_-]{22}$"));
    def is_legacy_child:
      (.label | type) == "string"
      and (.label | test("^(jarvis|2ndmate-[^/]+)/.+ · p:[A-Za-z0-9_-]{22}$"));
  ...
  case "$move_capable" in
    ...
    2)
      ...
      ;;
    3)
      echo "warning: herdr presentation ordering needs protocol $DJ_BACKEND_HERDR_MIN_WORKSPACE_MOVE_PROTOCOL or newer; leaving worker in Herdr's current order" >&2
      return 0
      ;;
    4)
  ...
  mover=${DJ_BACKEND_HERDR_WORKSPACE_MOVER:-$DJ_BACKEND_HERDR_ROOT/bin/backends/herdr-workspace-move.py}
  focus_before=$(dj_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace move" || true
...
# dj_backend_herdr_server_ensure: start the herdr server for <session>
...
dj_backend_herdr_server_ensure() {  # <session>
  local session=$1 running out i
  running=$(dj_backend_herdr_cli "$session" status --json 2>/dev/null | jq -r '.server.running // false' 2>/dev/null)
  [ "$running" = "true" ] && return 0
  ( dj_backend_herdr_cli "$session" server >/dev/null 2>&1 & ) || return 1
...
# dj_backend_herdr_workspace_find_all: EVERY workspace id inside <session>
# whose label equals this HOME's own label (dj_backend_herdr_workspace_label),
...
# duplicate means for them - dj_backend_herdr_workspace_ensure refuses to guess
# which one is the caller's, while the read-only recovery path below keeps its
# historical first-match behavior.
dj_backend_herdr_workspace_find_all() {  # <session>
  local session=$1 label list
  label=$(dj_backend_herdr_workspace_label)
  list=$(dj_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 0
  ...
  # ALWAYS return empty and every spawn mint a fresh "jarvis" workspace
  # (the workspace leak).
  printf '%s' "$list" | jq -r --arg want "$label" \
    '.result.workspaces[]? | select(.label == $want) | .workspace_id' 2>/dev/null
}

# dj_backend_herdr_workspace_find: this HOME's own workspace id inside
...
# exactly the defect dj_backend_herdr_workspace_ensure now refuses.
dj_backend_herdr_workspace_find() {  # <session>
  dj_backend_herdr_workspace_find_all "$1" | head -1
}

# dj_backend_herdr_launcher_identity: the EXACT herdr workspace that the
...
# manages a pane for (docs/verification/runtime-backends.md), and a jarvis
...
# uniqueness on them, so a label search cannot tell one `jarvis` workspace
...
#   DJ_BACKEND_HERDR_LAUNCHER_PANE_ID
#   DJ_BACKEND_HERDR_LAUNCHER_TAB_ID
#   DJ_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID
#
# Returns:
...
#       SELECTION marker (bin/dj-backend.sh's dj_backend_detect), never a
...
dj_backend_herdr_launcher_identity() {  # <session>
  ...
  claimed_session=$(dj_backend_herdr_session)
  ...
  claimed_socket=$(dj_backend_herdr_canonical_socket_path "$claimed_socket") || {
  ...
  session_socket=$(dj_backend_herdr_presentation_session_socket_path "$session") || {
  ...
  pane_out=$(dj_backend_herdr_cli "$session" pane get "$pane" 2>/dev/null) || {
  ...
  tab_out=$(dj_backend_herdr_cli "$session" tab get "$tab" 2>/dev/null) || {
  ...
  DJ_BACKEND_HERDR_LAUNCHER_PANE_ID=$pane
  # shellcheck disable=SC2034  # callers consume the verified binding's parts
  DJ_BACKEND_HERDR_LAUNCHER_TAB_ID=$tab
  DJ_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID=$workspace
  return 0
}

# dj_backend_herdr_workspace_prune_seeded_default_tab: close EXACTLY
# <seeded_tab_id>, the auto-created default tab id that THIS SAME
# dj_backend_herdr_workspace_ensure call captured straight from its own
...
# here never fails the caller, mirroring the dj_backend_herdr_kill `|| true`
# contract.
#
# Live-fire incident fix (2026-07-02): the prior implementation
# (dj_backend_herdr_workspace_prune_default_tabs, removed) re-derived
# "prunable" at create_task time from a pure label heuristic - exactly one
# tab, labeled "1" - run against whatever workspace dj_backend_herdr_workspace_find
...
# directory named "jarvis" produces a workspace that looks byte-identical,
# by label alone, to jarvis's own auto-created container - one tab, label
...
# dj_backend_herdr_workspace_ensure call just created carries a non-empty
# seeded_tab_id at all (see DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID below); an
...
dj_backend_herdr_workspace_prune_seeded_default_tab() {  # <session> <workspace_id> <seeded_tab_id> [focus-preserving]
  local session=$1 wsid=$2 tab_id=$3 close_mode=${4:-direct} tabs tab_count current_label pane_id agent_out agent_status
  [ -n "$tab_id" ] || return 0
  tabs=$(dj_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 0
  ...
  pane_id=$(dj_backend_herdr_pane_for_tab "$session" "$wsid" "$tab_id") || return 0
  [ -n "$pane_id" ] || return 0
  agent_out=$(dj_backend_herdr_cli "$session" agent get "$pane_id" 2>/dev/null)
  ...
  if [ "$close_mode" = focus-preserving ]; then
    dj_backend_herdr_projection_close_pane_focus_preserving "$session" "$pane_id"
  else
    dj_backend_herdr_cli "$session" pane close "$pane_id" >/dev/null 2>&1 || true
  fi
}

# dj_backend_herdr_workspace_ensure: the workspace this spawn's task tab
...
# discard them:
#   DJ_BACKEND_HERDR_WS_ID          - the resolved workspace_id (also echoed,
#                                      for callers that only need the id)
#   DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID - non-empty ONLY when THIS call just
...
#                                      (dj_backend_herdr_launcher_identity) or
#                                      a single label match
#                                      (dj_backend_herdr_workspace_find_all -
...
#                                      labeled - see
#                                      dj_backend_herdr_workspace_prune_seeded_default_tab.
...
# container being ensured belongs to the SAME jarvis home as the process
# calling this:
...
#                   (dj_backend_herdr_launcher_identity), never in whichever
...
dj_backend_herdr_workspace_ensure() {  # <session> <cwd> [<launcher-relationship>]
  local session=$1 cwd=$2 relationship=${3:-launcher-home} wsid out label matches count status
  DJ_BACKEND_HERDR_WS_ID=""
  DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID=""
  if [ "$relationship" = launcher-home ]; then
    dj_backend_herdr_launcher_identity "$session" && status=0 || status=$?
    case "$status" in
      0)
        DJ_BACKEND_HERDR_WS_ID=$DJ_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID
        printf '%s' "$DJ_BACKEND_HERDR_WS_ID"
        return 0
        ;;
      2) ;;
      *) return 3 ;;
  ...
  label=$(dj_backend_herdr_workspace_label)
  matches=$(dj_backend_herdr_workspace_find_all "$session")
  count=$(printf '%s' "$matches" | grep -c '[^[:space:]]' || true)
  if [ "$count" -gt 1 ]; then
    echo "error: ${count} herdr workspaces in session '$session' are labeled '$label' (${matches//$'\n'/ }) and this spawn has no herdr parent pane to identify which one is its own; rename or close the extras, or run jarvis inside the workspace its workers belong in" >&2
    return 3
  fi
  wsid=${matches%%$'\n'*}
  if [ -n "$wsid" ]; then
    DJ_BACKEND_HERDR_WS_ID=$wsid
    printf '%s' "$wsid"
    return 0
  fi
  out=$(dj_backend_herdr_cli "$session" workspace create --cwd "$cwd" --label "$label" --no-focus 2>/dev/null) || return 1
  wsid=$(printf '%s' "$out" | jq -r '.result.workspace.workspace_id // empty' 2>/dev/null)
  [ -n "$wsid" ] || return 1
  DJ_BACKEND_HERDR_WS_ID=$wsid
  # Herdr seeds a new workspace with one auto-created default tab jarvis
  ...
  # workspace we just created. dj_backend_herdr_create_task prunes it instead,
  # once the first real task tab exists alongside it, and only ever targets
  # this exact captured tab_id.
  DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  printf '%s' "$wsid"
}

# dj_backend_herdr_container_ensure: the full spawn-time container-ensure
...
# must be threaded through to dj_backend_herdr_create_task, which is the only
# function allowed to prune it (dj_backend_herdr_workspace_prune_seeded_default_tab).
# <launcher-relationship> is passed straight through to
# dj_backend_herdr_workspace_ensure, which owns its meaning.
dj_backend_herdr_container_ensure() {  # <cwd-for-a-fresh-workspace> [<launcher-relationship>]
  local cwd=${1:-$PWD} relationship=${2:-launcher-home} session label status
  dj_backend_herdr_version_check || return 1
  session=$(dj_backend_herdr_session)
  dj_backend_herdr_server_ensure "$session" || return 1
  dj_backend_herdr_workspace_ensure "$session" "$cwd" "$relationship" >/dev/null && status=0 || status=$?
  ...
  if [ "$status" -ne 0 ] || [ -z "$DJ_BACKEND_HERDR_WS_ID" ]; then
    label=$(dj_backend_herdr_workspace_label)
    echo "error: failed to ensure herdr workspace '$label' in session '$session'" >&2
    return 1
  fi
  printf '%s:%s\t%s' "$session" "$DJ_BACKEND_HERDR_WS_ID" "$DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID"
}

# dj_backend_herdr_pane_presence_state: classify one exact pane get response
# as dead|present|unknown from its JSON body, never from process exit status.
dj_backend_herdr_pane_presence_state() {  # <session> <pane_id>
  local session=$1 pane_id=$2 out code pid
  out=$(dj_backend_herdr_cli "$session" pane get "$pane_id" 2>&1)
...
dj_backend_herdr_workspace_presence_state() {  # <session> <workspace_id>
  local session=$1 workspace_id=$2 out matches
  out=$(dj_backend_herdr_cli "$session" workspace list 2>&1)
...
# dj_backend_herdr_explicit_close_pane_confirmed: issue one explicit close and
# succeed only when a structured follow-up proves the exact pane is gone.
dj_backend_herdr_explicit_close_pane_confirmed() {  # <session> <pane_id>
  local session=$1 pane_id=$2 presence
  dj_backend_herdr_cli "$session" pane close "$pane_id" >/dev/null 2>&1 || return 1
  presence=$(dj_backend_herdr_pane_presence_state "$session" "$pane_id")
  [ "$presence" = dead ]
}

# dj_backend_herdr_pane_agent_state: classify <pane_id> in <session> as one of
...
dj_backend_herdr_pane_agent_state() {  # <session> <pane_id>
  ...
  out=$(dj_backend_herdr_cli "$session" agent get "$pane_id" 2>&1)
  code=$(printf '%s' "$out" | jq -r '.error.code // empty' 2>/dev/null)
  if [ -n "$code" ]; then
...
# dj_backend_herdr_tab_is_husk: true (0) only for the two conservative husk
# states (dead, no-agent) dj_backend_herdr_pane_agent_state can positively
...
dj_backend_herdr_tab_is_husk() {  # <session> <pane_id>
  case "$(dj_backend_herdr_pane_agent_state "$1" "$2")" in
    dead|no-agent) return 0 ;;
    *) return 1 ;;
  esac
}

# dj_backend_herdr_agent_state: recovery-grade state for the same session-start
...
dj_backend_herdr_agent_state() {  # <target>
  local target=$1
  dj_backend_herdr_parse_target "$target" || { printf 'unreadable'; return 0; }
  case "$(dj_backend_herdr_pane_agent_state "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE")" in
    dead) printf 'missing' ;;
...
# agent verdict. The detailed state contract is owned by dj_backend_agent_state.
dj_backend_herdr_agent_alive() {  # <target>
  case "$(dj_backend_herdr_agent_state "$1")" in
    alive) printf 'alive' ;;
...
# dj_backend_herdr_create_task: create the task's tab (one pane) in
...
# panes) across a server restart, including a reboot, and a restored dj-<id>
...
# the operator to manually close each husk pane first before jarvis could
# spawn into it again. dj_backend_herdr_tab_is_husk classifies the existing
...
# This mirrors dj_backend_herdr_workspace_prune_seeded_default_tab's own
...
# dj_backend_herdr_workspace_ensure captured as DJ_BACKEND_HERDR_WS_SEEDED_TAB_ID
...
# dj_backend_herdr_workspace_prune_seeded_default_tab for the incident and
# the safety argument). An ADOPTED workspace's caller always passes an empty
# 4th arg, so this function never even queries for a prune candidate in that
# case. Echoes "<tab_id> <pane_id>" on success.
dj_backend_herdr_create_task() {  # <container> <label> <cwd> <seeded_default_tab_id>
  local container=$1 label=$2 cwd=$3 seeded_tab_id=${4:-} session wsid list dup_tabs dup dup_pane dup_tab_ids out tab_id pane_id remaining_dup_tabs
  session=${container%%:*}
  wsid=${container#*:}
  list=$(dj_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 1
  ...
  if [ -n "$dup_tabs" ]; then
    while IFS= read -r dup; do
      [ -n "$dup" ] || continue
      dup_pane=$(dj_backend_herdr_pane_for_tab "$session" "$wsid" "$dup")
      if [ -z "$dup_pane" ] || ! dj_backend_herdr_tab_is_husk "$session" "$dup_pane"; then
...
EOF
  ...
  out=$(dj_backend_herdr_cli "$session" tab create --workspace "$wsid" --cwd "$cwd" --label "$label" --no-focus 2>/dev/null) || return 1
  ...
  [ -z "$seeded_tab_id" ] || dj_backend_herdr_workspace_prune_seeded_default_tab "$session" "$wsid" "$seeded_tab_id"
  if [ -n "$dup_tab_ids" ]; then
    while IFS= read -r dup; do
      [ -n "$dup" ] || continue
      dj_backend_herdr_cli "$session" tab close "$dup" >/dev/null 2>&1 || true
    done <<EOF
$dup_tab_ids
EOF
    list=$(dj_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || {
...
# dj_backend_herdr_projection_create_task: create one disposable presentation
# workspace and its normal dj-<id> task tab without looking up, adopting, or
...
#   DJ_BACKEND_HERDR_PROJECTION_SESSION
...
#   DJ_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE
...
# cleanup authority.
dj_backend_herdr_projection_create_task() {  # <cwd> <workspace-label> <task-label>
  ...
  DJ_BACKEND_HERDR_PROJECTION_TAB_ID=""
  DJ_BACKEND_HERDR_PROJECTION_PANE_ID=""
  DJ_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE=0
  ...
  if out=$(dj_backend_herdr_cli "$session" workspace create --cwd "$cwd" --label "$workspace_label" --no-focus 2>/dev/null); then
    :
  else
    dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace create" || true
    echo "error: herdr presentation workspace create failed ambiguously; leaving its journal quarantined" >&2
    return 1
  fi
  dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "workspace create" || {
  ...
  DJ_BACKEND_HERDR_PROJECTION_SESSION=$session
  DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID=$(printf '%s' "$out" | jq -r '.result.workspace.workspace_id // empty' 2>/dev/null)
  DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  DJ_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID=$(printf '%s' "$out" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null)
  if [ -z "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" ] \
     || [ -z "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" ] \
     || [ -z "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID" ]; then
  ...
  if out=$(dj_backend_herdr_cli "$session" tab create \
    --workspace "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
    --cwd "$cwd" --label "$task_label" --no-focus 2>/dev/null); then
    :
  else
    dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "task-tab create" || true
    echo "error: herdr presentation task-tab create failed ambiguously; leaving its journal quarantined" >&2
    return 1
  fi
  dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "task-tab create" || {
    echo "error: herdr presentation task-tab create did not preserve exact active focus; leaving its journal quarantined" >&2
    return 1
  }
  DJ_BACKEND_HERDR_PROJECTION_TAB_ID=$(printf '%s' "$out" | jq -r '.result.tab.tab_id // empty' 2>/dev/null)
  DJ_BACKEND_HERDR_PROJECTION_PANE_ID=$(printf '%s' "$out" | jq -r '.result.root_pane.pane_id // empty' 2>/dev/null)
  if [ -z "$DJ_BACKEND_HERDR_PROJECTION_TAB_ID" ] || [ -z "$DJ_BACKEND_HERDR_PROJECTION_PANE_ID" ]; then
  ...
  DJ_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE=1
  focus_before=$(dj_backend_herdr_projection_focus_snapshot "$session") || {
  ...
  if ! dj_backend_herdr_workspace_prune_seeded_default_tab \
    "$session" \
    "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
    "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
  ...
  dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "seeded-tab prune" || {
    echo "error: herdr presentation seeded-tab prune did not preserve exact active focus; leaving its journal quarantined" >&2
    return 1
  }

  tabs=$(dj_backend_herdr_cli "$session" tab list --workspace "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" 2>/dev/null) || {
    echo "error: could not verify the disposable herdr presentation workspace shape" >&2
    return 1
  }
  panes=$(dj_backend_herdr_cli "$session" pane list --workspace "$DJ_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" 2>/dev/null) || {
  ...
  if [ "$tab_count" != 1 ] || [ "$pane_count" != 1 ] \
     || ! printf '%s' "$tabs" | jq -e --arg task "$DJ_BACKEND_HERDR_PROJECTION_TAB_ID" \
       --arg seeded "$DJ_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
       '.result.tabs[0].tab_id == $task and ([.result.tabs[] | select(.tab_id == $seeded)] | length) == 0' >/dev/null 2>&1 \
     || ! printf '%s' "$panes" | jq -e --arg pane "$DJ_BACKEND_HERDR_PROJECTION_PANE_ID" \
       --arg tab "$DJ_BACKEND_HERDR_PROJECTION_TAB_ID" \
...
# dj_backend_herdr_projection_cleanup_exact: same-process abort cleanup for a
# projection whose create calls returned complete exact IDs.
# It performs no lookup and never calls workspace close.
dj_backend_herdr_projection_cleanup_exact() {  # <session> <task-pane> <seeded-pane>
  local session=$1 task_pane=$2 seeded_pane=$3
  [ -z "$task_pane" ] || dj_backend_herdr_projection_close_pane_focus_preserving "$session" "$task_pane" || true
  if [ -n "$seeded_pane" ] && [ "$seeded_pane" != "$task_pane" ]; then
    dj_backend_herdr_projection_close_pane_focus_preserving "$session" "$seeded_pane" || true
  fi
}

# dj_backend_herdr_projection_parent_workspace_exact: resolve one exact parent
# workspace only when its presentation label is unique in the named session.
dj_backend_herdr_projection_parent_workspace_exact() {  # <session> <parent-label>
...
# dj_backend_herdr_projection_live_binding_matches: verify one exact projected
...
dj_backend_herdr_projection_live_binding_matches() {  # <session> <token> <workspace> <tab> <pane> <parent-workspace> <parent-label> <workspace-label> <task-label>
  ...
  ' >/dev/null 2>&1 || return 1
  panes=$(dj_backend_herdr_cli "$session" pane list --workspace "$workspace" 2>/dev/null) || return 1
...
dj_backend_herdr_projection_reclaim_rollback() {  # <session> <new-pane>
  local session=$1 new_pane=$2 state
  state=$(dj_backend_herdr_pane_agent_state "$session" "$new_pane")
  case "$state" in
    dead) return 0 ;;
    no-agent) ;;
    live|unknown) return 1 ;;
  esac
  dj_backend_herdr_projection_close_pane_focus_preserving "$session" "$new_pane" no-agent || return 1
  [ "$(dj_backend_herdr_pane_agent_state "$session" "$new_pane")" = dead ]
}

# dj_backend_herdr_projection_reclaim_task: replace one exact agent-free
...
dj_backend_herdr_projection_reclaim_task() {  # <session> <journal> <task-id> <home> <meta-workspace> <meta-tab> <meta-pane> <parent-label> <task-label> <cwd>
  ...
  DJ_BACKEND_HERDR_PROJECTION_PANE_ID=""
  dj_backend_herdr_projection_journal_snapshot "$journal" "$id" || return 1
  if [ "$DJ_BACKEND_HERDR_JOURNAL_VERSION" != 2 ]; then
    echo "warning: herdr presentation journal for $id has no exact restart binding; spawning flat" >&2
    return 2
  fi
  canonical_home=$(dj_backend_herdr_projection_home_identity "$home") || {
    echo "warning: herdr presentation home for $id could not be resolved exactly; spawning flat" >&2
    return 2
  }
  if [ "$DJ_BACKEND_HERDR_JOURNAL_HOME" != "$canonical_home" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_SESSION" != "$session" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" != "$meta_workspace" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_TAB_ID" != "$meta_tab" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_PANE_ID" != "$meta_pane" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_PARENT_LABEL" != "$parent_label" ] \
     || [ "$DJ_BACKEND_HERDR_JOURNAL_TASK_LABEL" != "$task_label" ]; then
    echo "warning: herdr presentation binding for $id does not match its exact home, endpoint, or parent; spawning flat" >&2
    return 2
  fi
  if ! dj_backend_herdr_projection_live_binding_matches \
    "$session" "$DJ_BACKEND_HERDR_JOURNAL_PROJECTION_ID" \
    "$meta_workspace" "$meta_tab" "$meta_pane" \
    "$DJ_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" "$parent_label" \
    "$DJ_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL" "$task_label"; then
    echo "warning: herdr presentation binding for $id has an ambiguous, renamed, foreign, or non-nested live shape; spawning flat" >&2
    return 2
  fi
  state=$(dj_backend_herdr_pane_agent_state "$session" "$meta_pane")
  case "$state" in
    no-agent) ;;
    dead)
  ...
  if ! out=$(dj_backend_herdr_cli "$session" tab create \
    ...
    dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "husk replacement create" || return 1
  ...
  dj_backend_herdr_projection_focus_restore "$session" "$focus_before" "husk replacement create" || return 1
  info=$(dj_backend_herdr_cli "$session" tab get "$new_tab" 2>/dev/null) || info=
  if ! printf '%s' "$info" | jq -e --arg tab "$new_tab" --arg workspace "$meta_workspace" '
    .result.tab.tab_id == $tab and .result.tab.workspace_id == $workspace
  ' >/dev/null 2>&1; then
    dj_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    echo "warning: herdr presentation reclaim for $id could not verify its replacement tab; spawning flat" >&2
    return 2
  fi
  info=$(dj_backend_herdr_cli "$session" pane get "$new_pane" 2>/dev/null) || info=
  ...
  if dj_backend_herdr_projection_close_pane_focus_preserving "$session" "$meta_pane" no-agent; then
    close_status=0
  else
    close_status=$?
  fi
  if [ "$close_status" -ne 0 ]; then
    ...
    state=$DJ_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE
    dj_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    case "$state" in
      live|unknown)
  ...
  if [ "$(dj_backend_herdr_pane_agent_state "$session" "$meta_pane")" != dead ]; then
    dj_backend_herdr_projection_reclaim_rollback "$session" "$new_pane" || return 1
    return 1
  fi
  if ! dj_backend_herdr_projection_live_binding_matches \
  ...
  if ! dj_backend_herdr_projection_journal_replace_endpoint \
  ...
  DJ_BACKEND_HERDR_PROJECTION_TAB_ID=$new_tab
  DJ_BACKEND_HERDR_PROJECTION_PANE_ID=$new_pane
  return 0
}

# dj_backend_herdr_projection_recovery_allows_flat: inspect an existing
...
dj_backend_herdr_projection_recovery_allows_flat() {  # <session> <journal> <task-id>
  local session=$1 journal=$2 id=$3 token list wsids count wsid panes pane_ids pane state
  token=$(dj_backend_herdr_projection_journal_token "$journal" "$id") || {
  ...
  while IFS= read -r wsid; do
    [ -n "$wsid" ] || continue
    panes=$(dj_backend_herdr_cli "$session" pane list --workspace "$wsid" 2>/dev/null) || {
    ...
    while IFS= read -r pane; do
      [ -n "$pane" ] || continue
      state=$(dj_backend_herdr_pane_agent_state "$session" "$pane")
      case "$state" in
        dead|no-agent) : ;;
        live|unknown)
...
# dj_backend_herdr_projection_endpoint_matches_journal: read-only correlation
...
dj_backend_herdr_projection_endpoint_matches_journal() {  # <session> <workspace-id> <journal> <task-id>
  local session=$1 workspace_id=$2 journal=$3 id=$4 token list matches
  token=$(dj_backend_herdr_projection_journal_token "$journal" "$id") || return 1
  list=$(dj_backend_herdr_cli "$session" workspace list 2>/dev/null) || return 1
...
# dj_backend_herdr_parse_target: split "<session>:<pane_id>" (pane_id itself
# contains a colon, e.g. "w1:p2") on the FIRST colon only. Sets
# DJ_BACKEND_HERDR_SESSION and DJ_BACKEND_HERDR_PANE for the caller.
dj_backend_herdr_parse_target() {  # <target>
  local target=$1
  DJ_BACKEND_HERDR_SESSION=${target%%:*}
  DJ_BACKEND_HERDR_PANE=${target#*:}
  [ -n "$DJ_BACKEND_HERDR_SESSION" ] && [ -n "$DJ_BACKEND_HERDR_PANE" ] && [ "$DJ_BACKEND_HERDR_PANE" != "$target" ]
}

dj_backend_herdr_target_ready() {  # <target>
  dj_backend_herdr_parse_target "$1" || return 1
  dj_backend_herdr_server_ensure "$DJ_BACKEND_HERDR_SESSION" || return 1
}

# dj_backend_herdr_current_path: the live FOREGROUND process's cwd, or empty on
...
# would make dj-spawn.sh's worktree-discovery poll never see the pane "leave"
...
dj_backend_herdr_current_path() {  # <target>
  dj_backend_herdr_target_ready "$1" || return 0
  dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane get "$DJ_BACKEND_HERDR_PANE" 2>/dev/null \
    | jq -r '.result.pane.foreground_cwd // empty' 2>/dev/null
}

# dj_backend_herdr_send_text_line: send one line of TEXT then submit,
# ATOMICALLY - mirrors tmux's `send-keys -t T text Enter`. Used for the fixed
# spawn-time commands (treehouse get, the GOTMPDIR export). `pane run` types
# the command and submits it in one call (verified).
dj_backend_herdr_send_text_line() {  # <target> <text>
  dj_backend_herdr_target_ready "$1" || return 1
  dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane run "$DJ_BACKEND_HERDR_PANE" "$2" >/dev/null 2>&1
}

# dj_backend_herdr_send_literal: send TEXT as literal, UNSUBMITTED input - the
...
dj_backend_herdr_send_literal() {  # <target> <text>
  dj_backend_herdr_target_ready "$1" || return 1
  dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane send-text "$DJ_BACKEND_HERDR_PANE" "$2" >/dev/null 2>&1
}

# dj_backend_herdr_normalize_key: map jarvis's key vocabulary (Enter,
# Escape, C-c, as used by dj-send.sh --key and stuck-crewmate-recovery) onto
...
dj_backend_herdr_normalize_key() {  # <key>
  case "$1" in
    Enter|enter) printf 'enter' ;;
    Escape|escape|Esc|esc) printf 'escape' ;;
    C-c|c-c|ctrl+c|Ctrl+C) printf 'ctrl+c' ;;
    # C-u clears a composer line. dj-send.sh's muse interrupt path needs it to
...
# dj_backend_herdr_send_key: one named special key. Mirrors dj-send.sh's --key
# path (tmux's `send-keys -t T key`).
dj_backend_herdr_send_key() {  # <target> <key>
  dj_backend_herdr_target_ready "$1" || return 1
  local key
  key=$(dj_backend_herdr_normalize_key "$2")
  dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane send-keys "$DJ_BACKEND_HERDR_PANE" "$key" >/dev/null 2>&1
}

# dj_backend_herdr_capture: bounded plain-text pane capture. Mirrors
# dj-peek.sh's/dj-watch.sh's `tmux capture-pane -p -t T -S -N`. --source recent
...
dj_backend_herdr_capture() {  # <target> <lines>
  dj_backend_herdr_target_ready "$1" || return 1
  local lines=${2:-200} fetch out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  out=$(dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane read "$DJ_BACKEND_HERDR_PANE" --source recent --lines "$fetch" 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}

dj_backend_herdr_capture_ansi() {  # <target> <lines>
  dj_backend_herdr_target_ready "$1" || return 1
  local lines=${2:-200} fetch out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  out=$(dj_backend_herdr_cli "$DJ_BACKEND_HERDR_SESSION" pane read "$DJ_BACKEND_HERDR_PANE" --source recent --lines "$fetch" --format ansi 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}
...
# the shared owner (bin/dj-composer-lib.sh, dj_composer_classify_screen), so
...
dj_backend_herdr_agent_identity_raw() {  # <session> <pane> -> <agent>\t<status>
  local out
  out=$(dj_backend_herdr_cli "$1" agent get "$2" 2>/dev/null) || return 1
  printf '%s' "$out" | jq -r '[.result.agent.agent // "", .result.agent.agent_status // ""] | @tsv' 2>/dev/null
}

# dj_backend_herdr_composer_identity: the native agent identity/state probe
# backing the shared classifier's separated (pi) shape - the genuine herdr
# primitive no other backend has natively.
dj_backend_herdr_composer_identity() {  # <target> -> "<agent>\t<status>"
  dj_backend_herdr_parse_target "$1" || return 1
  dj_backend_herdr_agent_identity_raw "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE"
}

# dj_backend_herdr_composer_state: thin adapter - capture plus capabilities
...
dj_backend_herdr_composer_state() {  # <target> -> empty|pending|pending-unproven|unknown
  local target=$1 cap caps verdict identity
  dj_backend_herdr_parse_target "$target" || { printf 'unknown'; return 0; }
  if cap=$(dj_backend_herdr_capture_ansi "$target" "$DJ_COMPOSER_CAPTURE_LINES" 2>/dev/null); then
    caps=$(printf 'styled=1\ncursor=0\nidentity=1\nrows=%s' "$DJ_COMPOSER_CAPTURE_LINES")
  elif cap=$(dj_backend_herdr_capture "$target" "$DJ_COMPOSER_CAPTURE_LINES"); then
    caps=$(printf 'styled=0\ncursor=0\nidentity=1\nrows=%s' "$DJ_COMPOSER_CAPTURE_LINES")
  else
    printf 'unknown'
    return 0
  fi
  verdict=$(dj_composer_classify_screen "$caps" "$cap")
  if [ "$verdict" = need-identity ]; then
    if ! identity=$(dj_backend_herdr_composer_identity "$target" 2>/dev/null) || [ -z "$identity" ]; then
      identity=probe-absent
    fi
    verdict=$(dj_composer_classify_screen "$caps" "$cap" '' "$identity")
    [ "$verdict" != need-identity ] || verdict=unknown
  fi
  printf '%s' "$verdict"
}

# dj_backend_herdr_rendered_busy_state: busy|idle|unknown from the pane's
# RENDERED busy footer, the same delivery-only signal bin/dj-tmux-lib.sh's
# dj_pane_busy_state reads, scanning the same 40-line tail folded to its last
# 12 non-blank rows. This is NOT a worker-state source: herdr's native
# agent-state (dj_backend_herdr_busy_state) stays the semantic owner, and this
...
dj_backend_herdr_rendered_busy_state() {  # <target> [harness] -> busy|idle|unknown
  local target=$1 harness=${2:-} cap visible
  cap=$(dj_backend_herdr_capture "$target" 40) || { printf 'unknown'; return 0; }
  visible=$(printf '%s' "$cap" | grep -v '^[[:space:]]*$' | tail -12)
  [ -n "$visible" ] || { printf 'unknown'; return 0; }
  if printf '%s' "$visible" | dj_busy_lines_match "$harness"; then
    printf 'busy'
  else
    printf 'idle'
  fi
...
# dj_backend_herdr_send_text_submit: type <text> into <target> once (raw,
# unsubmitted, via send_literal), then submit with a named Enter key, retried
# (Enter only, never retyped) until native agent-state, a cleared composer, or
# dj_composer_queued_enter_verdict confirms delivery. Verified hazard
...
# submission is confirmed by dj_backend_herdr_wait_for_working observing a
...
# dj_composer_queued_enter_verdict (bin/dj-composer-lib.sh).
#
...
#   - Slow transition: dj_backend_herdr_wait_for_working samples repeatedly
...
# (bin/dj-tmux-lib.sh): an idle-to-busy transition ACROSS our Enter is proof the
...
# idle. The policy is dj_composer_queued_enter_verdict; this adapter only
...
#
# dj_backend_herdr_queued_enter_busy: delivery-busy for the shared queued-Enter
...
dj_backend_herdr_queued_enter_busy() {  # <target> <allow-rendered>
  local target=$1 allow_rendered=${2:-0} raw
  raw=$(dj_backend_herdr_agent_status_raw "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE")
  case "$raw" in
    working) printf 'busy'; return 0 ;;
  esac
  if [ "$allow_rendered" = 1 ]; then
    dj_backend_herdr_rendered_busy_state "$target"
  else
    printf 'idle'
  fi
}

dj_backend_herdr_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle>
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 i=0 verdict baseline confirm_sleep
  local raw_status footer_baseline='' allow_rendered=0 enter_sent=0
  dj_backend_herdr_parse_target "$target" || { printf 'unknown'; return 0; }
  dj_backend_herdr_send_literal "$target" "$text" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  raw_status=$(dj_backend_herdr_agent_status_raw "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE")
  baseline=$(dj_backend_herdr_classify_submit_agent_status "$raw_status")
  confirm_sleep=$(dj_backend_herdr_submit_confirm_budget "$sleep_s")
  ...
  if [ "$baseline" = idle ]; then
    ...
    footer_baseline=$(dj_backend_herdr_rendered_busy_state "$target")
  fi
  while :; do
    if dj_backend_herdr_send_key "$target" Enter; then
      enter_sent=1
    ...
    if [ "$baseline" = idle ]; then
      verdict=$(dj_backend_herdr_wait_for_working "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE" \
        "$confirm_sleep" "$DJ_BACKEND_HERDR_SUBMIT_POLLS")
      case "$verdict" in
      ...
      verdict=$(dj_backend_herdr_composer_state "$target")
      case "$verdict" in
      ...
      if [ "$verdict" = pending ] && [ "$raw_status" != working ] \
        ...
        && [ "$(dj_backend_herdr_rendered_busy_state "$target")" = busy ]; then
        verdict=busy
      fi
      case "$verdict" in
        busy) printf 'empty'; return 0 ;;
    ...
    if [ "$i" -ge "$retries" ]; then
      if [ "$enter_sent" -eq 0 ]; then
        ...
        dj_composer_queued_enter_verdict "$verdict" \
          "$(dj_backend_herdr_queued_enter_busy "$target" "$allow_rendered")"
      fi
      return 0
    fi
  done
...
# dj_backend_herdr_kill: remove the task's pane, best-effort (mirrors
...
dj_backend_herdr_kill_serialized() {  # <session> <pane>
  ...
  before=$(dj_backend_herdr_projection_focus_snapshot "$session") || before=
  if [ -n "$before" ]; then
    active_tab=${before#*$'\t'}
    info=$(dj_backend_herdr_cli "$session" pane get "$pane" 2>/dev/null) || info=
    target_pane=$(printf '%s' "$info" | jq -r '.result.pane.pane_id // empty' 2>/dev/null)
    target_tab=$(printf '%s' "$info" | jq -r '.result.pane.tab_id // empty' 2>/dev/null)
    target_ws=$(printf '%s' "$info" | jq -r '.result.pane.workspace_id // empty' 2>/dev/null)
    if [ "$target_pane" = "$pane" ] && [ -n "$target_tab" ] && [ "$target_tab" != "$active_tab" ]; then
      plan=$(dj_backend_herdr_emptying_close_plan "$session" "$pane" "$target_ws" "$target_tab" "${before%%$'\t'*}")
      plan_move_record=
      case "$plan" in
        moved$'\t'*)
          plan_move_record=${plan%%$'\n'*}
      ...
      case "$plan" in
        death\ *)
          ...
          if ! dj_backend_herdr_death_close_pane "$session" "$pane" "$shell_pid" \
            && ! dj_backend_herdr_explicit_close_pane_confirmed "$session" "$pane"; then
            close_failed=1
          fi
          ;;
        *)
          dj_backend_herdr_explicit_close_pane_confirmed "$session" "$pane" || close_failed=1
          ;;
      esac
      ...
      dj_backend_herdr_projection_focus_restore "$session" "$before" "task kill" || true
      return 0
    fi
  fi
  dj_backend_herdr_explicit_close_pane_confirmed "$session" "$pane" || true
}

dj_backend_herdr_kill() {  # <target>
  dj_backend_herdr_target_ready "$1" || return 0
  local session=$DJ_BACKEND_HERDR_SESSION pane=$DJ_BACKEND_HERDR_PANE
  local lock_path attempt=0 lock_held=0
  if ! declare -F dj_lock_try_acquire >/dev/null 2>&1; then
    # shellcheck source=bin/dj-wake-lib.sh
    . "$DJ_BACKEND_HERDR_ROOT/bin/dj-wake-lib.sh"
  fi
  if lock_path=$(dj_backend_herdr_presentation_session_lock_path "$session"); then
    while [ "$attempt" -lt 50 ]; do
      if dj_lock_try_acquire "$lock_path"; then
        lock_held=1
        break
      fi
      sleep 0.1
  ...
  if [ "$lock_held" = 1 ]; then
    dj_backend_herdr_kill_serialized "$session" "$pane"
    dj_lock_release "$lock_path" || true
  else
    echo "warning: herdr task kill could not acquire its session presentation lock; refusing an unlocked pane close" >&2
  fi
}

# dj_backend_herdr_endpoint_confirmed_gone: gate durable-record removal on
# the exact recorded pane's structured presence
# (dj_backend_herdr_pane_presence_state), read-only, so a refused, skipped,
...
dj_backend_herdr_endpoint_confirmed_gone() {  # <target>
  local presence
  dj_backend_herdr_parse_target "$1" || return 1
  presence=$(dj_backend_herdr_pane_presence_state "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE")
  [ "$presence" = dead ]
}

# dj_backend_herdr_classify_agent_status: map a raw `agent get` agent_status
...
dj_backend_herdr_classify_agent_status() {  # <raw-agent_status>
  case "$1" in
    working) printf 'busy' ;;
...
dj_backend_herdr_classify_submit_agent_status() {  # <raw-agent_status>
  case "$1" in
...
# dj_backend_herdr_agent_status_raw: one `agent get` read, echoing the raw
# agent_status string (working/idle/done/blocked/...), or empty on any
# failure. Deliberately skips dj_backend_herdr_target_ready's server-ensure
# round trip (an extra `status --json` call) that dj_backend_herdr_busy_state
# pays on every call: dj_backend_herdr_wait_for_working polls this in a tight
# loop right after a caller has already parsed the target and confirmed the
# server is live (e.g. dj_backend_herdr_send_text_submit, immediately after a
# successful send-text), so re-checking server liveness on every poll would
# only add latency without adding safety.
dj_backend_herdr_agent_status_raw() {  # <session> <pane_id>
  local session=$1 pane_id=$2 out
  out=$(dj_backend_herdr_cli "$session" agent get "$pane_id" 2>/dev/null) || { printf ''; return 0; }
  printf '%s' "$out" | jq -r '.result.agent.agent_status // empty' 2>/dev/null
}

# dj_backend_herdr_busy_state: semantic busy state from herdr's native
# agent-state detection (agent.get), the "first backend where dj_session_busy_state
# gets real semantics" per the design report. See
# dj_backend_herdr_classify_agent_status for the status->busy/idle/unknown
# mapping.
dj_backend_herdr_busy_state() {  # <target>
  dj_backend_herdr_target_ready "$1" || { printf 'unknown'; return 0; }
  dj_backend_herdr_classify_agent_status \
    "$(dj_backend_herdr_agent_status_raw "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE")"
}

# dj_backend_herdr_wait_for_working: poll <session>:<pane_id>'s NATIVE
...
# DJ_BACKEND_HERDR_SUBMIT_POLLS (default 6): how many samples
# dj_backend_herdr_send_text_submit spreads across each Enter attempt's
...
DJ_BACKEND_HERDR_SUBMIT_POLLS=${DJ_BACKEND_HERDR_SUBMIT_POLLS:-6}
DJ_BACKEND_HERDR_SUBMIT_MIN_SLEEP=${DJ_BACKEND_HERDR_SUBMIT_MIN_SLEEP:-0.6}

dj_backend_herdr_submit_confirm_budget() {  # <caller-budget-seconds>
  awk -v b="${1:-0}" -v m="$DJ_BACKEND_HERDR_SUBMIT_MIN_SLEEP" 'BEGIN {
    b += 0
    m += 0
    if (b < 0) b = 0
    if (m < 0) m = 0
...
dj_backend_herdr_wait_for_working() {  # <session> <pane_id> <budget-seconds> <polls>
  ...
  for ((i = 0; i < polls; i++)); do
    ...
    raw=$(dj_backend_herdr_agent_status_raw "$session" "$pane_id")
    bs=$(dj_backend_herdr_classify_submit_agent_status "$raw")
    case "$bs" in
...
# dj_backend_herdr_pane_for_tab: the root pane id for <tab_id> in <workspace_id>
...
dj_backend_herdr_pane_for_tab() {  # <session> <workspace_id> <tab_id>
  local session=$1 wsid=$2 tab_id=$3 panes
  panes=$(dj_backend_herdr_cli "$session" pane list --workspace "$wsid" 2>/dev/null) || return 1
  printf '%s' "$panes" | jq -r --arg tab "$tab_id" \
    '.result.panes[]? | select(.tab_id == $tab) | .pane_id' 2>/dev/null | head -1
}

# dj_backend_herdr_resolve_bare_selector: the live-tab-listing fallback for an
...
dj_backend_herdr_resolve_bare_selector() {  # <name>
  ...
  while IFS= read -r session; do
    [ -n "$session" ] || continue
    tabs=$(dj_backend_herdr_cli "$session" tab list 2>/dev/null) || continue
    ...
    [ -n "$wsid" ] || continue
    pane_id=$(dj_backend_herdr_pane_for_tab "$session" "$wsid" "$tab_id") || continue
    [ -n "$pane_id" ] || continue
...
# dj_backend_herdr_list_live: recovery/orphan discovery. Lists every tab whose
# label looks like a jarvis task window (dj-<id>) in <session>'s, THIS
# HOME'S OWN workspace (dj_backend_herdr_workspace_label - never another
...
# workspace because DJ_HOME already names it - no glue needed, unlike the
# primary-spawns-a-secondmate path in dj-spawn.sh. Read-only: a session/
# workspace that does not exist yet simply lists nothing. One
# "<session>:<pane_id>\t<label>" line per live task tab.
dj_backend_herdr_list_live() {  # <session>
  local session=$1 wsid tabs tab_id label pane_id
  wsid=$(dj_backend_herdr_workspace_find "$session") || return 0
  [ -n "$wsid" ] || return 0
  tabs=$(dj_backend_herdr_cli "$session" tab list --workspace "$wsid" 2>/dev/null) || return 0
  ...
  done < <(printf '%s' "$tabs" | jq -r '.result.tabs[]? | select(.label | startswith("dj-")) | "\(.tab_id)\t\(.label)"' 2>/dev/null)
}
...
# dj_backend_herdr_wait_transition is the watcher's bounded wait primitive for
...
# dj_backend_herdr_socket_path: the control-socket path for <session>, read from
...
dj_backend_herdr_socket_path() {  # <session>
  local session=$1
...
# dj_backend_herdr_events_capable: the version/capability gate for the event
...
# reader override is configured); client protocol >= DJ_BACKEND_HERDR_MIN_EVENTS_PROTOCOL;
# and both `events.subscribe` and `pane.agent_status_changed` present in `herdr
# api schema`. DJ_BACKEND_HERDR_EVENTS_FORCE overrides the whole verdict for
...
dj_backend_herdr_events_capable() {  # <session>
  local session=$1 protocol schema
  case "${DJ_BACKEND_HERDR_EVENTS_FORCE:-}" in
    1) return 0 ;;
    0) return 1 ;;
  esac
  dj_backend_herdr_tool_check || return 1
  if [ -z "${DJ_BACKEND_HERDR_EVENT_READER:-}" ]; then
    command -v python3 >/dev/null 2>&1 || return 1
  fi
  protocol=$(herdr status --json 2>/dev/null | jq -r '.client.protocol // empty' 2>/dev/null)
  case "$protocol" in ''|*[!0-9]*) return 1 ;; esac
  [ "$protocol" -ge "$DJ_BACKEND_HERDR_MIN_EVENTS_PROTOCOL" ] || return 1
...
# dj_backend_herdr_normalize_event: THE single normalize point (report section 5
...
dj_backend_herdr_normalize_event() {  # <pane_id> <workspace_id> <agent_status> <agent>
  dj_transition_record "${1:-}" "${2:-}" "" "${3:-}" "${4:-}"
}

# dj_backend_herdr_event_reader_cmd: emit the reader argv (one word per line) for
# the raw-socket subscriber. Default: `python3 <this dir>/herdr-eventwait.py`.
# DJ_BACKEND_HERDR_EVENT_READER overrides it with a whitespace-split command so
# tests can substitute a fake reader that replays canned stream lines.
dj_backend_herdr_event_reader_cmd() {
  local word
  if [ -n "${DJ_BACKEND_HERDR_EVENT_READER:-}" ]; then
    for word in $DJ_BACKEND_HERDR_EVENT_READER; do
      printf '%s\n' "$word"
    done
    return 0
  fi
  printf 'python3\n'
  printf '%s\n' "$DJ_BACKEND_HERDR_ROOT/bin/backends/herdr-eventwait.py"
}

# dj_backend_herdr_escalation_marker: the per-pane dedupe marker path for a
# <window> ("<session>:<pane_id>"), keyed identically to the watcher's
# .stale-<key> (tr ':/.' '___'), under <state_dir>.
dj_backend_herdr_escalation_marker() {  # <state_dir> <window>
  local state=$1 window=$2 key
  key=$(printf '%s' "$window" | tr ':/.' '___')
  printf '%s/%s%s' "$state" "$DJ_BACKEND_HERDR_ESCALATED_PREFIX" "$key"
}

# dj_backend_herdr_apply_transition: route one normalized record through the
...
dj_backend_herdr_apply_transition() {  # <state_dir> <session> <record>
  local state=$1 session=$2 record=$3 pane_id to action window marker
  pane_id=$(dj_transition_pane_id "$record")
  [ -n "$pane_id" ] || return 1
  to=$(dj_transition_to_status "$record")
  action=$(dj_transition_policy "$to")
  window="$session:$pane_id"
  marker=$(dj_backend_herdr_escalation_marker "$state" "$window")
  case "$action" in
    actionable)
      if [ ! -e "$marker" ]; then
        printf '%s' "$record"
...
dj_backend_herdr_commit_transition() {  # <state_dir> <session> <record>
...
dj_backend_herdr_clear_transition() {  # <state_dir> <window>
...
# dj_backend_herdr_wait_transition: the bounded event wait. Blocks up to
...
dj_backend_herdr_wait_transition() {  # <session> <timeout_secs> <state_dir> <pane_window...>
  local session=$1 timeout=$2 state=$3
  shift 3
  local windows=("$@")
  [ "${#windows[@]}" -gt 0 ] || return 2
  if [ "${DJ_BACKEND_EVENTS_CAPABILITY_CONFIRMED:-0}" != 1 ]; then
    dj_backend_herdr_events_capable "$session" || return 2
  fi
  local sock
  sock=$(dj_backend_herdr_socket_path "$session")
  [ -n "$sock" ] || return 2
  ...
  if [ "$rc" -ne 2 ]; then
    for w in "${windows[@]}"; do
      ...
      raw=$(dj_backend_herdr_agent_status_raw "$session" "$pane_id")
      [ -n "$raw" ] || continue
      record=$(dj_backend_herdr_normalize_event "$pane_id" "" "$raw" "")
      if hit=$(dj_backend_herdr_apply_transition "$state" "$session" "$record"); then
        printf '%s' "$hit"
        rc=0
        break
      fi
  ...
  while [ "$rc" -eq 1 ] && IFS= read -r line <&9; do
    ...
    [ -n "$pane_id" ] || continue
    record=$(dj_backend_herdr_normalize_event "$pane_id" "$ws" "$status" "$agent")
    if hit=$(dj_backend_herdr_apply_transition "$state" "$session" "$record"); then
      printf '%s' "$hit"
      rc=0
      break
