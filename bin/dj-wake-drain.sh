...
# main (DJ_SUPERVISION_ACTOR unset or "main", via dj-lease-lib.sh's dj_lease_actor
# - the same actor identity dj-send.sh/dj-control.sh/dj-teardown.sh already use)
# claims every row not already granted to branch, then drains and acks only
# that claimed set. branch (DJ_SUPERVISION_ACTOR=branch, injected
...
# .pi/extensions/lib/dj-branch-dispatch.ts is the single owner of that
...
branch_grant_live_locked() {
  ...
  exec 8<&-
  [ "$version" = dj-branch-eligible-owner-v1 ] || return 1
  case "$pid" in ''|*[!0-9]*|1) return 1 ;; esac
  case "$generation" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  current=$(dj_pid_identity "$pid" 2>/dev/null) || return 1
...
claim_main_rows_locked() {
  ...
  ' "$DJ_WAKE_QUEUE" | LC_ALL=C sort -n > "$DRAIN_TMP" || return 1
...
case "${1:-}" in
  ...
  --ack-through)
    ...
    ;;
  *) echo "usage: dj-wake-drain.sh [--ack-through SEQUENCE --recovery-generation GENERATION]" >&2; exit 2 ;;
esac
...
# only when a guarded supervision script (dj-peek/dj-send/...) happens to run.
# Reuse dj-guard.sh's model-aware alarm and DJ_GUARD_GRACE instead of duplicating
...
acknowledge_inactive_outcomes() { # <mode> <newline-separated-fingerprints>
  ...
  while IFS= read -r fingerprint; do
    ...
    "$SCRIPT_DIR/dj-inactive-reconcile.sh" "$mode" "$fingerprint" || return 1
  done <<< "$fingerprints"
}
...
# dj-classify-lib.sh's status_open_decisions fold (via its cursor-backed
...
# dj-classify-lib.sh's "incremental (cursor-backed) open-decisions fold").
...
print_open_decisions_section() {
  ...
  while IFS=$(printf '\t') read -r task key verb note; do
    ...
    dj_cap_line_var "$line" $((item_bytes - 1))
...
EOF
  ...
  # bin/dj-send.sh header).
  printf "OPEN DECISIONS: close one by answering it: bin/dj-send.sh <task> --resolve-key <key> '<answer>'\n" || return 1
}
...
# with no warning at all. bin/dj-captain-hold.sh's `diverged` owns which pairs
...
print_record_divergence_section() {
  ...
  # A non-positive bound is not a bound (bin/dj-timeout-lib.sh), so a bad
  # override falls back to the default rather than disabling the deadline.
  bound=${DJ_DIVERGENCE_TIMEOUT:-20}
  ...
  diverged=$(dj_run_timed "$bound" "$SCRIPT_DIR/dj-captain-hold.sh" diverged 2>/dev/null) || return 0
  [ -n "$diverged" ] || return 0
...
EOF
  ...
  printf 'RECORD DIVERGENCE: reconcile each one - record the captain'"'"'s own words with bin/dj-captain-hold.sh answer <task> --decision-file <path>, or re-open the status decision when that resolution was not the captain'"'"'s word.\n' || return 1
}
...
print_status_presentation() {  # [<deduped-raw-rows>]
  ...
  if [ "$rc" -eq 0 ] && [ -n "$rows" ]; then
    dj_wake_print_annotations "$rows" "$snapshot" || rc=1
    if [ "$rc" -eq 0 ]; then
      annotation_manifest=$(dj_wake_annotation_manifest "$rows") || rc=1
...
if [ -n "$ACK_THROUGH" ]; then
  ...
  if [ "$ACTOR" = branch ]; then
    ...
    ' "$DJ_WAKE_QUEUE" > "$DRAIN_TMP" || exit 1
  else
    ...
    ' "$DJ_WAKE_QUEUE" > "$DRAIN_TMP" || exit 1
    dj_wake_commit_secondmate_stall_receipts_through "$ACK_THROUGH" "$MAIN_ROWS_FILE" || {
      echo "wake drain: secondmate stall receipt could not be recorded safely" >&2
      exit 1
    }
  fi
  if [ ! -s "$DRAIN_TMP" ]; then
    dj_recovery_marker_ack "$RECOVERY_MARKER" "$ACK_GENERATION"
    RECOVERY_ACK_STATUS=$?
    case "$RECOVERY_ACK_STATUS" in
      0) ;;
      3) RECOVERY_ACK_MOVED=true ;;
      *)
        echo "wake drain: recovery episode could not be retired safely; re-run bin/dj-wake-drain.sh and use the new WAKE_ACK_REQUIRED command" >&2
        exit 1
        ;;
    esac
  else
    dj_recovery_marker_snapshot "$RECOVERY_MARKER" || exit 1
    RECOVERY_MARKER_TOKEN=$DJ_RECOVERY_MARKER_TOKEN
    if [ "${RECOVERY_MARKER_TOKEN##*:}" != "$ACK_GENERATION" ]; then
      RECOVERY_ACK_MOVED=true
    fi
  fi
  if ! _fm_atomic_replace "$DRAIN_TMP" "$DJ_WAKE_QUEUE"; then
  ...
  if [ "$RECOVERY_ACK_MOVED" = true ]; then
    printf 'wake drain: acknowledged wakes through %s, but a newer recovery episode is pending; re-run bin/dj-wake-drain.sh and use the new WAKE_ACK_REQUIRED command\n' \
      "$ACK_THROUGH" >&2
  fi
  exit 0
fi
...
if [ ! -s "$DJ_WAKE_QUEUE" ]; then
  : > "$DJ_WAKE_QUEUE"
  dj_recovery_marker_snapshot "$RECOVERY_MARKER" || true
  RECOVERY_MARKER_TOKEN=$DJ_RECOVERY_MARKER_TOKEN
  case "$RECOVERY_MARKER_TOKEN" in
    pending:downtime:*|announced:downtime:*)
      dj_recovery_marker_begin_handling "$RECOVERY_MARKER" || {
  ...
  if [ "$RECOVERY_ACK_REQUIRED" = true ]; then
    printf 'WAKE_ACK_REQUIRED: after handling completes run bin/dj-wake-drain.sh --ack-through 0 --recovery-generation %s\n' "${RECOVERY_MARKER_TOKEN##*:}" >&2
  fi
  assert_watcher_liveness
  exit 0
fi
...
if [ -z "$RECOVERY_MARKER_TOKEN" ]; then
  ...
  dj_recovery_marker_publish "$RECOVERY_MARKER" downtime || {
...
awk -F '\t' -v seqs="$ACTOR_ROWS_FILE" '
  ...
  NF >= 5 && ($2 in keep)
' "$DJ_WAKE_QUEUE" > "$DRAIN_VIEW_TMP" || exit 1
RAW_ROWS=$(dj_wake_print_deduped "$DRAIN_VIEW_TMP") || exit "$?"
rm -f -- "$DRAIN_VIEW_TMP" || exit 1
DRAIN_VIEW_TMP=
ACK_THROUGH=$(printf '%s\n' "$RAW_ROWS" | awk -F '\t' '$2 ~ /^[0-9]+$/ && $2 > max { max=$2 } END { print max + 0 }') || exit 1
case "${DJ_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT:-0}" in
  0) ;;
  ''|*[!0-9]*) ;;
  *) sleep "$DJ_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT" ;;
esac
if [ -n "$RAW_ROWS" ]; then
  printf '%s\n' "$RAW_ROWS" || exit "$?"
fi
...
dj_lock_release "$DJ_WAKE_QUEUE_LOCK"
DRAIN_LOCK_HELD=false
printf 'WAKE_ACK_REQUIRED: after handling completes run bin/dj-wake-drain.sh --ack-through %s --recovery-generation %s\n' \
