...
#
# Why running beats nudging: bin/dj-sessionstart-nudge.sh can only ASK the agent
...
#
# Usage: dj-sessionstart-run.sh [--source <source>]
...
session_start_completed() {
  ...
  dj_session_lock_owned_by_self "$STATE" || return 1
...
if [ -z "$SOURCE" ] && [ ! -t 0 ]; then
  ...
  # so a Cursor-delivered payload here is the duplicate: bin/dj-sessionstart-
  ...
  if dj_hook_payload_is_foreign_host "$PAYLOAD"; then
    exit 0
  fi
...
case "$SOURCE" in
  resume|reload|fork)
    exec "$SCRIPT_DIR/dj-sessionstart-nudge.sh"
    ;;
  clear|compact)
    if session_start_completed; then
      "$SCRIPT_DIR/dj-session-start.sh" --reemit --source "$SOURCE" || true
    else
      "$SCRIPT_DIR/dj-session-start.sh" --source "$SOURCE" || true
    fi
    ;;
  *)
    "$SCRIPT_DIR/dj-session-start.sh" --source "$SOURCE" || true
