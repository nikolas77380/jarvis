#!/usr/bin/env bash
# dj-backend.sh - runtime-backend selection, meta helpers, selector resolution,
# and dispatch for jarvis's session-provider abstraction.
#
# Design: data/dj-backend-design-d7/report.md ("Backend Interface") and
# data/dj-backend-design-d7/herdr-addendum.md ("Events as the core
# abstraction"). P1 extracted the tmux command sequences that dj-send.sh,
# dj-peek.sh, dj-watch.sh, dj-spawn.sh, and dj-teardown.sh already ran inline
...
# EXPERIMENTAL spawn-capable backend behind `--backend herdr`/`DJ_BACKEND=herdr`/
# `config/backend`, and behind runtime auto-detection when jarvis itself is
# running inside herdr with no explicit backend setting; see herdr-addendum.md and
# data/dj-backend-design-d7/herdr-verification-p2.md for its empirical basis.
# P3 adds bin/backends/zellij.sh, also EXPERIMENTAL and spawn-capable, behind
# `--backend zellij`/`DJ_BACKEND=zellij`/`config/backend` - NOT behind runtime
...
# EXPERIMENTAL and spawn-capable, behind `--backend cmux`/`DJ_BACKEND=cmux`/
# `config/backend`, and behind runtime auto-detection when jarvis itself is
...
# treats that as `tmux` (dj_backend_of_meta), and dj-spawn.sh does not write
...
# task events (status-changed, went-stale, exited) that map onto jarvis's
# existing signal/stale/check/heartbeat wake vocabulary. The tmux adapter has
# no native event push, so dj-watch.sh's poll loop over the pull primitives
...
# on their own for on-demand reads (dj-peek.sh, dj-crew-state.sh).

DJ_BACKEND_SCRIPT=${BASH_SOURCE[0]:-$0}
DJ_BACKEND_LIB_DIR="$(cd "$(dirname "$DJ_BACKEND_SCRIPT")" && pwd)"
unset DJ_BACKEND_SCRIPT
DJ_BACKEND_DEFAULT_ROOT="$(cd "$DJ_BACKEND_LIB_DIR/.." && pwd)"
DJ_ROOT="${DJ_ROOT_OVERRIDE:-${DJ_ROOT:-$DJ_BACKEND_DEFAULT_ROOT}}"
DJ_HOME="${DJ_HOME:-${DJ_ROOT_OVERRIDE:-$DJ_ROOT}}"
DJ_BACKEND_CONFIG_DIR="${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}"
...
# data/dj-backend-design-d7/herdr-addendum.md) - verified against the real
# v0.7.1/protocol-14 binary (data/dj-backend-design-d7/herdr-verification-p2.md)
# but newer than tmux's long-proven default path. zellij is EXPERIMENTAL (P3;
# data/dj-backend-design-d7/report.md "Zellij Backend") - verified against the
...
DJ_BACKEND_KNOWN="tmux herdr zellij orca cmux"
DJ_BACKEND_SPAWN="tmux herdr zellij orca cmux"

# dj_backend_list_contains: whitespace-delimited membership without relying on
# shell word splitting. dj-backend.sh is normally sourced by bash scripts, but
# zsh diagnostics can source it too, so backend-name matching must stay portable.
dj_backend_list_contains() {  # <list> <name>
  local list=$1 name=$2
  case "$name" in
    *[[:space:]]*) return 1 ;;
  esac
...
dj_backend_is_known() {  # <name>
  dj_backend_list_contains "$DJ_BACKEND_KNOWN" "$1"
}

# dj_backend_detect: detect the runtime jarvis itself is CURRENTLY executing
# inside, from verified environment markers (mirrors bin/dj-harness.sh's
...
# before exec'ing the real binary - so a claude-harness jarvis launched in
...
# Callers needing the winning signal read DJ_BACKEND_DETECT_SIGNAL (set to
# TMUX, HERDR_ENV, CMUX_WORKSPACE_ID, bundle-id, or ancestry) and
# DJ_BACKEND_DETECTED after a direct (non-command-substitution) call.
DJ_BACKEND_CMUX_BUNDLE_ID="com.cmuxterm.app"

dj_backend_detect() {
  DJ_BACKEND_DETECTED=""
  DJ_BACKEND_DETECT_SIGNAL=""
  if [ -n "${TMUX:-}" ]; then
    DJ_BACKEND_DETECTED=tmux
    DJ_BACKEND_DETECT_SIGNAL=TMUX
    printf 'tmux'
    return 0
  fi
  if [ "${HERDR_ENV:-}" = "1" ]; then
    DJ_BACKEND_DETECTED=herdr
    DJ_BACKEND_DETECT_SIGNAL=HERDR_ENV
    printf 'herdr'
    return 0
  fi
  if [ -n "${CMUX_WORKSPACE_ID:-}" ]; then
    DJ_BACKEND_DETECTED=cmux
    DJ_BACKEND_DETECT_SIGNAL=CMUX_WORKSPACE_ID
    printf 'cmux'
    return 0
  fi
  if dj_backend_detect_cmux_fallback; then
    DJ_BACKEND_DETECTED=cmux
    printf 'cmux'
    return 0
  fi
...
# dj_backend_detect_cmux_fallback: the two macOS-only cmux fallback signals
# (see dj_backend_detect's header comment). Sets DJ_BACKEND_DETECT_SIGNAL to
...
dj_backend_detect_cmux_fallback() {
  [ "$(uname 2>/dev/null)" = Darwin ] || return 1
  if [ "${__CFBundleIdentifier:-}" = "$DJ_BACKEND_CMUX_BUNDLE_ID" ]; then
    DJ_BACKEND_DETECT_SIGNAL=bundle-id
    return 0
  fi
  if dj_backend_detect_cmux_app_is_ancestor; then
    DJ_BACKEND_DETECT_SIGNAL=ancestry
    return 0
  fi
  return 1
}

# dj_backend_detect_cmux_app_pid: the running cmux app's pid, resolved by
...
dj_backend_detect_cmux_app_pid() {
  command -v lsappinfo >/dev/null 2>&1 || return 1
  local out pid
  out=$(lsappinfo info -only pid -app "$DJ_BACKEND_CMUX_BUNDLE_ID" 2>/dev/null) || return 1
  pid=${out##*=}
...
# dj_backend_detect_cmux_app_is_ancestor: walk this process's parent chain and
...
dj_backend_detect_cmux_app_is_ancestor() {
  local cmux_pid pid ppid comm hops=0
  cmux_pid=$(dj_backend_detect_cmux_app_pid) || cmux_pid=""
  pid=$$
  while [ "$hops" -lt 32 ]; do
...
# dj_backend_name: resolve the ACTIVE backend for a NEW spawn, absent an
# explicit per-task override. Precedence: DJ_BACKEND env, then config/backend
# (a single word on its first non-empty line, mirroring config/crew-harness),
# then runtime auto-detection (dj_backend_detect), then default tmux. A
# per-task `--backend` flag is parsed by the caller (dj-spawn.sh) and takes
...
dj_backend_name() {
  ...
  if [ -n "${DJ_BACKEND:-}" ]; then
    printf '%s' "$DJ_BACKEND"
    return 0
  fi
  if [ -f "$DJ_BACKEND_CONFIG_DIR/backend" ]; then
  ...
  if dj_backend_detect >/dev/null; then
    ...
    if [ "$detected" = cmux ]; then
      case "$DJ_BACKEND_DETECT_SIGNAL" in
        bundle-id) marker="FALLBACK signal __CFBundleIdentifier=$DJ_BACKEND_CMUX_BUNDLE_ID; CMUX_WORKSPACE_ID absent, stripped by cmux's bundled claude wrapper" ;;
...
# dj_backend_validate: refuse an unknown backend LOUDLY. Silent on success.
dj_backend_validate() {  # <name>
  local name=$1
  if ! dj_backend_is_known "$name"; then
    echo "error: unknown backend '$name' (known: $DJ_BACKEND_KNOWN)" >&2
    return 1
  fi
  return 0
}

dj_backend_validate_spawn() {  # <name>
  local name=$1
  dj_backend_validate "$name" || return 1
  dj_backend_list_contains "$DJ_BACKEND_SPAWN" "$name" && return 0
  echo "error: backend '$name' does not support task spawning yet (spawn-supported: $DJ_BACKEND_SPAWN)" >&2
  return 1
}

# dj_backend_required_tools: the backend-SPECIFIC CLI tools a jarvis home on
# <backend> genuinely requires, beyond jarvis's universal toolchain (owned by
...
#     tool check, e.g. dj_backend_herdr_tool_check);
...
dj_backend_required_tools() {  # <backend>
  case "$1" in
...
dj_backend_required_tool_available() {  # <backend> <tool>
  local backend=$1 tool=$2 required
  required=$(dj_backend_required_tools "$backend") || return 1
  dj_backend_list_contains "$required" "$tool" || return 1
  case "$backend:$tool" in
    cmux:cmux)
      dj_backend_source cmux >/dev/null 2>&1 || return 1
      dj_backend_cmux_bin >/dev/null 2>&1
      ;;
    *) command -v "$tool" >/dev/null 2>&1 ;;
  esac
}

# dj_meta_get: the LAST value of `key=` in <meta-file>, or empty (never
# errors) if the file or key is absent. Mirrors the ad hoc `grep '^key=' |
# tail -1 | cut -d= -f2-` snippet every dj-*.sh script used to repeat inline.
dj_meta_get() {  # <meta-file> <key>
  local meta=$1 key=$2
  [ -f "$meta" ] || return 0
  grep "^$key=" "$meta" 2>/dev/null | tail -1 | cut -d= -f2- || true
}

# dj_backend_of_meta: the backend recorded in <meta-file>, defaulting to
# `tmux` when the field is absent - the P1 compatibility contract.
dj_backend_of_meta() {  # <meta-file>
  local v
  v=$(dj_meta_get "$1" backend)
  printf '%s' "${v:-tmux}"
}

dj_backend_target_of_meta() {  # <meta-file>
  local meta=$1 backend terminal window
  backend=$(dj_backend_of_meta "$meta")
  if [ "$backend" = orca ]; then
    terminal=$(dj_meta_get "$meta" terminal)
    [ -n "$terminal" ] && { printf '%s' "$terminal"; return 0; }
  fi
  window=$(dj_meta_get "$meta" window)
  [ -n "$window" ] && printf '%s' "$window"
}

# dj_backend_validate_task_endpoint: validate a task cleanup record entirely
...
# valid only when their window name itself is exactly dj-<task-id>.
# On success, sets DJ_BACKEND_VALIDATED_BACKEND and
# DJ_BACKEND_VALIDATED_TARGET. On failure, prints one refusal and returns 1.
dj_backend_meta_exact_value() {  # <meta-file> <key>
...
dj_backend_endpoint_atom_valid() {  # <value>
  case "$1" in
    ''|*[!A-Za-z0-9._@%+-]*) return 1 ;;
  esac
}

dj_backend_validate_task_endpoint() {  # <meta-file> <task-id>
  local meta=$1 id=$2 backend_count backend window worktree project binding_count binding
  local session pane recorded_session workspace tab terminal worktree_id surface
  DJ_BACKEND_VALIDATED_BACKEND=
  DJ_BACKEND_VALIDATED_TARGET=
  [ -f "$meta" ] && [ ! -L "$meta" ] || {
  ...
  case "$backend_count" in
    0) backend=tmux ;;
    1) backend=$(dj_backend_meta_exact_value "$meta" backend) || backend= ;;
    *) backend= ;;
  esac
  if [ -z "$backend" ] || ! dj_backend_is_known "$backend"; then
  ...
  case "$binding_count" in
    0) binding= ;;
    1)
      binding=$(dj_backend_meta_exact_value "$meta" endpoint_task_id) || {
  ...
  case "$backend" in
    ...
    herdr)
      ...
      recorded_session=$(dj_backend_meta_exact_value "$meta" herdr_session) || recorded_session=
      workspace=$(dj_backend_meta_exact_value "$meta" herdr_workspace_id) || workspace=
      tab=$(dj_backend_meta_exact_value "$meta" herdr_tab_id) || tab=
      pane=$(dj_backend_meta_exact_value "$meta" herdr_pane_id) || pane=
      if [ -z "$recorded_session" ] || [ -z "$workspace" ] || [ -z "$tab" ] || [ -z "$pane" ] \
        || [ "$window" != "$recorded_session:$pane" ] \
        || ! dj_backend_endpoint_atom_valid "$recorded_session" \
        || ! dj_backend_endpoint_atom_valid "$workspace" \
        || ! dj_backend_endpoint_atom_valid "${tab//:/_}" \
        || ! dj_backend_endpoint_atom_valid "${pane//:/_}"; then
    ...
    zellij)
      ...
      recorded_session=$(dj_backend_meta_exact_value "$meta" zellij_session) || recorded_session=
      tab=$(dj_backend_meta_exact_value "$meta" zellij_tab_id) || tab=
      pane=$(dj_backend_meta_exact_value "$meta" zellij_pane_id) || pane=
      case "$tab:$pane" in *[!0-9:]*) tab= ;; esac
      if [ -z "$recorded_session" ] || [ -z "$tab" ] || [ -z "$pane" ] \
        || [ "$window" != "$recorded_session:$pane" ] \
        || ! dj_backend_endpoint_atom_valid "$recorded_session"; then
    ...
    orca)
      ...
      terminal=$(dj_backend_meta_exact_value "$meta" terminal) || terminal=
      worktree_id=$(dj_backend_meta_exact_value "$meta" orca_worktree_id) || worktree_id=
      [ -n "$terminal" ] || {
      ...
      if [ "$window" != "dj-$id" ] \
        || ! dj_backend_endpoint_atom_valid "$terminal" \
        || ! dj_backend_endpoint_atom_valid "$worktree_id"; then
    ...
    cmux)
      ...
      workspace=$(dj_backend_meta_exact_value "$meta" cmux_workspace_id) || workspace=
      surface=$(dj_backend_meta_exact_value "$meta" cmux_surface_id) || surface=
      if [ -z "$workspace" ] || [ -z "$surface" ] || [ "$window" != "$workspace:$surface" ] \
        || ! dj_backend_endpoint_atom_valid "$workspace" \
        || ! dj_backend_endpoint_atom_valid "$surface"; then
  ...
  DJ_BACKEND_VALIDATED_BACKEND=$backend
  # shellcheck disable=SC2034 # Output globals are consumed by sourcing callers.
  DJ_BACKEND_VALIDATED_TARGET=$window
  return 0
}

dj_backend_meta_for_window() {  # <target> <state-dir>
...
dj_backend_task_id_for_selector() {  # <raw-target> <state-dir>
...
dj_backend_meta_for_selector() {  # <raw-target> <state-dir>
  local raw=$1 state=$2 id
  id=$(dj_backend_task_id_for_selector "$raw" "$state") || return 1
  printf '%s/%s.meta' "$state" "$id"
}

dj_backend_of_selector() {  # <raw-target> <resolved-target> <state-dir>
  local raw=$1 resolved=$2 state=$3 meta
  meta=$(dj_backend_meta_for_selector "$raw" "$state" 2>/dev/null || true)
  [ -n "$meta" ] && { dj_backend_of_meta "$meta"; return 0; }
  if [ -n "$resolved" ]; then
    meta=$(dj_backend_meta_for_window "$resolved" "$state" 2>/dev/null || true)
    [ -n "$meta" ] && { dj_backend_of_meta "$meta"; return 0; }
  fi
  printf 'tmux'
}

dj_backend_expected_label_of_selector() {  # <raw-target> <state-dir>
  local raw=$1 state=$2 id
  id=$(dj_backend_task_id_for_selector "$raw" "$state" 2>/dev/null || true)
  [ -n "$id" ] && printf 'dj-%s' "$id"
  return 0
}

# dj_backend_source: source the named backend's adapter file, once per shell.
  ...
  case "$name" in
    tmux)
      if [ -z "${_FM_BACKEND_TMUX_SOURCED:-}" ]; then
        ...
        . "$DJ_BACKEND_LIB_DIR/backends/tmux.sh" || return 1
    ...
    herdr)
      if [ -z "${_FM_BACKEND_HERDR_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$DJ_BACKEND_LIB_DIR/backends/herdr.sh" || return 1
    ...
    zellij)
      if [ -z "${_FM_BACKEND_ZELLIJ_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$DJ_BACKEND_LIB_DIR/backends/zellij.sh" || return 1
    ...
    orca)
      if [ -z "${_FM_BACKEND_ORCA_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$DJ_BACKEND_LIB_DIR/backends/orca.sh" || return 1
    ...
    cmux)
      if [ -z "${_FM_BACKEND_CMUX_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$DJ_BACKEND_LIB_DIR/backends/cmux.sh" || return 1
...
# dj_backend_resolve_selector: resolve a raw dj-send.sh/dj-peek.sh style
...
#                      this jarvis home) - backend-independent, a literal string.
...
#   "dj-<id>"          legacy task window label fallback routed through
#                      <state-dir>/<id>.meta when no exact
#                      <state-dir>/dj-<id>.meta exists.
...
dj_backend_resolve_selector() {  # <raw-target> <state-dir>
  ...
  meta=$(dj_backend_meta_for_selector "$raw" "$state" 2>/dev/null || true)
  if [ -n "$meta" ]; then
    window=$(dj_backend_target_of_meta "$meta")
  ...
  case "$raw" in
    dj-*)
      echo "error: no metadata for $raw in $state; pass session:window to target a window outside this jarvis home" >&2
      return 1
      ;;
    *)
      meta=$(dj_backend_meta_for_window "$raw" "$state" 2>/dev/null || true)
      if [ -n "$meta" ]; then
        window=$(dj_backend_target_of_meta "$meta")
        [ -n "$window" ] || { echo "error: no backend target recorded in $meta" >&2; return 1; }
        printf '%s' "$window"
      ...
      dj_backend_source tmux || return 1
      dj_backend_tmux_resolve_bare_selector "$raw"
      ;;
  esac
}
...
# rather than hand-writing `case "$backend" in tmux) dj_backend_tmux_x ;; esac`
# at every call site. Each verified backend adds its own arm here, without
# changing call sites.

# dj_backend_capture: bounded plain-text session capture.
dj_backend_capture() {  # <backend> <target> <lines> [expected-label]
  local backend=$1
  shift
  dj_backend_source "$backend" || return 1
  case "$backend" in
    tmux) dj_backend_tmux_capture "$@" ;;
    herdr) dj_backend_herdr_capture "$@" ;;
    zellij) dj_backend_zellij_capture "$@" ;;
    orca) dj_backend_orca_capture "$@" ;;
    cmux) dj_backend_cmux_capture "$@" ;;
    *) echo "error: no capture implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# dj_backend_send_key: one backend-supported named special key.
dj_backend_send_key() {  # <backend> <target> <key> [expected-label]
  local backend=$1
  shift
  dj_backend_source "$backend" || return 1
  case "$backend" in
    tmux) dj_backend_tmux_send_key "$@" ;;
    herdr) dj_backend_herdr_send_key "$@" ;;
    zellij) dj_backend_zellij_send_key "$@" ;;
    orca) dj_backend_orca_send_key "$@" ;;
    cmux) dj_backend_cmux_send_key "$@" ;;
    *) echo "error: no send-key implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# dj_backend_send_text_submit: type text once, then submit and verify,
# retrying only the submission (never retyping). Echoes the backend's
# proof-carrying verdict; callers require exact empty for confirmed delivery.
dj_backend_send_text_submit() {  # <backend> <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local backend=$1
  shift
  dj_backend_source "$backend" || return 1
  case "$backend" in
    tmux) dj_backend_tmux_send_text_submit "$@" ;;
    herdr) dj_backend_herdr_send_text_submit "$@" ;;
    zellij) dj_backend_zellij_send_text_submit "$@" ;;
    orca) dj_backend_orca_send_text_submit "$@" ;;
    cmux) dj_backend_cmux_send_text_submit "$@" ;;
    *) echo "error: no send-text implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# dj_backend_kill: remove the task's session endpoint (best-effort; a
...
dj_backend_kill() {  # <backend> <target>
  local backend=$1
  shift
  [ -n "${1:-}" ] || { echo "error: refusing empty backend kill target" >&2; return 1; }
  dj_backend_source "$backend" || return 1
  case "$backend" in
    tmux) dj_backend_tmux_kill "$@" ;;
    herdr) dj_backend_herdr_kill "$@" ;;
    zellij) dj_backend_zellij_kill "$@" ;;
    orca) dj_backend_orca_kill "$@" ;;
    cmux) dj_backend_cmux_kill "$@" ;;
    *) echo "error: no kill implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

dj_backend_remove_worktree() {  # <backend> <worktree-id>
  local backend=$1
  shift
  dj_backend_source "$backend" || return 1
  case "$backend" in
    orca) dj_backend_orca_remove_worktree "$@" ;;
    *) echo "error: backend '$backend' does not own task worktrees" >&2; return 1 ;;
  esac
}

dj_backend_worktree_path() {  # <backend> <worktree-id>
  local backend=$1
  shift
  dj_backend_source "$backend" || return 1
  case "$backend" in
    orca) dj_backend_orca_worktree_path "$@" ;;
    *) echo "error: backend '$backend' does not own task worktrees" >&2; return 1 ;;
  esac
}

# dj_backend_busy_state: semantic busy/idle/unknown for backends that expose
...
# primitive (tmux) report unknown. Callers own the fallback policy: dj-watch.sh
# uses unknown as the cue for harness-scoped pane-tail detection, while
# dj-crew-state.sh also corroborates native idle verdicts with the recorded
# harness's signature before treating a no-run crew as not busy.
dj_backend_busy_state() {  # <backend> <target>
  local backend=$1
  shift
  dj_backend_source "$backend" || { printf 'unknown'; return 0; }
  case "$backend" in
    herdr) dj_backend_herdr_busy_state "$@" ;;
    *) printf 'unknown' ;;
  esac
}

# dj_backend_composer_state: classify the composer/input area of <target> as
...
# supervisor-pane pending-input guard in bin/dj-supervise-daemon.sh, and
# dj-spawn.sh's kimi readiness/delivery checks) can ask the same question
...
# the one shared shape owner (bin/dj-composer-lib.sh,
# dj_composer_classify_screen) - so no backend can hold a private shape
# assumption; zellij's classifier reads `dump-screen --ansi`, which replaced
# its old no-classifier content-diff reporting.
dj_backend_composer_state() {  # <backend> <target> [expected-label] -> empty|pending|pending-unproven|unknown
  local backend=$1
  shift
  dj_backend_source "$backend" || { printf 'unknown'; return 0; }
  case "$backend" in
    tmux) dj_tmux_composer_state "$@" ;;
    herdr) dj_backend_herdr_composer_state "$@" ;;
    orca) dj_backend_orca_composer_state "$@" ;;
    cmux) dj_backend_cmux_composer_state "$@" ;;
    zellij) dj_backend_zellij_composer_state "$@" ;;
    *) printf 'unknown' ;;
  esac
}

# dj_backend_target_exists: cheap, READ-ONLY existence check - does the
...
# going through dj_backend_herdr_target_ready (which auto-starts the herdr
# server as a side effect via dj_backend_herdr_server_ensure - fine for an
...
# Mirrors dj-crew-state.sh's pane_readable check; exists here as one shared
# primitive so callers that only need a fast alive/dead read (recovery
# digests, the session-start fleet digest) do not re-derive it inline.
dj_backend_target_exists() {  # <backend> <target> [expected-label]
  ...
  case "$backend" in
    tmux)
      ...
      ;;
    herdr)
      dj_backend_source herdr || return 1
      session=${target%%:*}
      pane=${target#*:}
      [ -n "$session" ] && [ -n "$pane" ] && [ "$pane" != "$target" ] || return 1
      # dj_backend_herdr_cli (not a raw HERDR_SESSION-only call): verified
      ...
      # running instead. dj_backend_herdr_cli appends the required --session
      # flag on top, so this check is correctly scoped even when the caller's
      # own ambient session (e.g. the primary jarvis's default session) is
      # a DIFFERENT one than the target's.
      dj_backend_herdr_cli "$session" pane get "$pane" >/dev/null 2>&1
      ;;
    zellij)
      dj_backend_source zellij || return 1
      dj_backend_zellij_target_ready "$target" "$expected_label"
      ;;
    orca)
      dj_backend_source orca || return 1
      dj_backend_orca_capture "$target" 1 >/dev/null 2>&1
      ;;
    cmux)
      dj_backend_source cmux || return 1
      dj_backend_cmux_target_ready "$target" "$expected_label"
      ;;
    *)
      return 1
      ;;
...
# dj_backend_agent_state: the single recovery-grade agent/endpoint state
# contract. It is deliberately richer than dj_backend_target_exists's cheap
...
dj_backend_agent_state() {  # <backend> <target>
  local backend=$1 target=$2
  dj_backend_source "$backend" || { printf 'unverified'; return 0; }
  case "$backend" in
    tmux) dj_backend_tmux_agent_state "$target" ;;
    herdr) dj_backend_herdr_agent_state "$target" ;;
    *) printf 'unverified' ;;
  esac
}
...
dj_backend_agent_alive() {  # <backend> <target>
  case "$(dj_backend_agent_state "$1" "$2")" in
    alive) printf 'alive' ;;
...
#
# The watcher's event-wait splice (bin/dj-watch.sh) is backend-agnostic: it asks
# dj_backend_has_push whether a window's backend can push semantic state changes,
# and for those backends replaces its blind `sleep POLL` with a bounded wait on
# dj_backend_wait_transition. Every push-capable backend reuses the shared
# normalized-transition shape and policy table (bin/dj-transition-lib.sh); today
...
# dj_backend_has_push: 0 if <backend> exposes a native transition push stream.
dj_backend_has_push() {  # <backend>
  case "$1" in
    herdr) return 0 ;;
    *) return 1 ;;
  esac
...
# dj_backend_events_capable: 0 if <backend>'s push path is usable for <session>
...
dj_backend_events_capable() {  # <backend> <session>
  local backend=$1
  shift
  dj_backend_has_push "$backend" || return 1
  dj_backend_source "$backend" || return 1
  case "$backend" in
    herdr) dj_backend_herdr_events_capable "$@" ;;
    *) return 1 ;;
  esac
}

# dj_backend_wait_transition: bounded wait for a fresh actionable (blocked)
...
dj_backend_wait_transition() {  # <backend> <session> <timeout_secs> <state_dir> <pane_window...>
  local backend=$1
  shift
  dj_backend_has_push "$backend" || return 2
  dj_backend_source "$backend" || return 2
  case "$backend" in
    herdr) dj_backend_herdr_wait_transition "$@" ;;
    *) return 2 ;;
  esac
}

dj_backend_commit_transition() {  # <backend> <state_dir> <session> <record>
  local backend=$1
  shift
  dj_backend_has_push "$backend" || return 1
  dj_backend_source "$backend" || return 1
  case "$backend" in
    herdr) dj_backend_herdr_commit_transition "$@" ;;
    *) return 1 ;;
  esac
}

dj_backend_clear_transition() {  # <backend> <state_dir> <window>
  local backend=$1
  shift
  dj_backend_has_push "$backend" || return 0
  dj_backend_source "$backend" || return 1
  case "$backend" in
    herdr) dj_backend_herdr_clear_transition "$@" ;;
    *) return 0 ;;
  esac
}
