#!/usr/bin/env bash
# dj-install-herdr.sh - install CI's pinned, verified Herdr build.
#
...
#
# Usage:
#   dj-install-herdr.sh <destination-directory>
#
...
DJ_HERDR_CI_VERSION=0.7.4
DJ_HERDR_CI_TAG="v${DJ_HERDR_CI_VERSION}"
DJ_HERDR_CI_MIN_PROTOCOL=16
# Bounded download ceiling (bytes). The largest official 0.7.4 asset is under 20 MiB.
DJ_HERDR_CI_MAX_BYTES=25000000
DJ_HERDR_CI_REPO=ogulcancelik/herdr

die() {
  printf 'dj-install-herdr.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: dj-install-herdr.sh <destination-directory>}
...
URL="https://github.com/${DJ_HERDR_CI_REPO}/releases/download/${DJ_HERDR_CI_TAG}/${ASSET}"
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/dj-herdr.XXXXXX")
trap 'rm -rf "$TMP"' EXIT

printf 'dj-install-herdr.sh: downloading %s from %s\n' "$ASSET" "$URL" >&2
# --fail: HTTP errors; --location: follow redirects; --max-filesize: bound.
curl -fsSL --max-filesize "$DJ_HERDR_CI_MAX_BYTES" "$URL" -o "$TMP/$ASSET" \
  || die "download failed for $URL (bounded at $DJ_HERDR_CI_MAX_BYTES bytes)"
...
[ "$installed_version" = "$DJ_HERDR_CI_VERSION" ] \
  || die "installed herdr version is '${installed_version:-<empty>}', expected exact pin $DJ_HERDR_CI_VERSION"
...
[ "$protocol" -ge "$DJ_HERDR_CI_MIN_PROTOCOL" ] \
  || die "herdr protocol $protocol is below the required floor $DJ_HERDR_CI_MIN_PROTOCOL"

printf 'dj-install-herdr.sh: installed herdr %s (protocol %s) to %s\n' \
