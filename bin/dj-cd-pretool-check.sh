...
#
# A stray persistent top-level `cd projects/<clone>` in the PRIMARY jarvis
# shell silently relocates the shell, so a later jarvis-owned command (a
# backlog write, an dj-* lifecycle call, tasks-axi) runs inside a project clone
# instead of the home. This seatbelt denies such a command before it runs.
# bin/dj-cd-command-policy.mjs is the sole owner of the block/allow decision; it
# reuses the shell classifier owned by bin/dj-arm-command-policy.mjs. This
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/dj-cd-pretool-check.sh
#   bin/dj-cd-pretool-check.sh --command '<cmd>'
#
...
#
# Exit/output contract (identical shape to bin/dj-arm-pretool-check.sh):
...
usage() {
  cat <<'EOF'
Usage: dj-cd-pretool-check.sh [--command <cmd>] [--claude|--cursor]

With no --command, reads a PreToolUse-style JSON payload on stdin (Grok
toolInput.command, or Claude/Codex tool_input.command).
Fires only in the real primary jarvis checkout; it is a silent no-op in a
crewmate/scout task worktree or any non-jarvis repo.
...
if [ "$CMD_SET" -eq 0 ]; then
  ...
  . "$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/dj-hook-host-lib.sh"
...
# decoder set in bin/dj-arm-command-policy.mjs: adding any new quote/expansion
...
# Scope to a plain, non-worktree jarvis checkout, where git-dir equals
# git-common-dir. A crewmate/scout task worktree - the shape bin/dj-spawn.sh
# always hands out - is a linked git worktree where the two differ. This guard
# does not inspect .dj-secondmate-home, so it applies in a git-cloned secondmate
...
[ -f "$DJ_ROOT/AGENTS.md" ] || exit 0
[ -d "$DJ_ROOT/bin" ] || exit 0
command -v git >/dev/null 2>&1 || exit 0
GIT_DIR=$(git -C "$DJ_ROOT" rev-parse --git-dir 2>/dev/null) || exit 0
GIT_COMMON_DIR=$(git -C "$DJ_ROOT" rev-parse --git-common-dir 2>/dev/null) || exit 0
[ "$GIT_DIR" = "$GIT_COMMON_DIR" ] || exit 0

POLICY="$DJ_ROOT/bin/dj-cd-command-policy.mjs"
