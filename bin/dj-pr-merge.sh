...
# bin/dj-pr-check.sh, so teardown can verify landed work after squash merges.
# The full canonical URL is parsed by bin/dj-pr-lib.sh. A GitHub pull request is
...
# Usage: dj-pr-merge.sh <task-id> <pr-url> [-- <extra forge merge args>]
set -eu
...
# green PR and never merges (contract: bin/dj-lease-lib.sh; no-op in homes
# without a branch actor).
# shellcheck source=bin/dj-lease-lib.sh
. "$SCRIPT_DIR/dj-lease-lib.sh"
dj_lease_forbid_branch "PR merge (dj-pr-merge)"
...
PR_OWNER=$DJ_PR_OWNER
PR_REPO=$DJ_PR_REPO
PR_NUMBER=$DJ_PR_NUMBER
...
PROJECT_URL="https://$DJ_PR_HOST/$DJ_PR_PATH"
shift 2
[ "${1:-}" = "--" ] && shift
...
# The recorded head is read before bin/dj-pr-check.sh rewrites the metadata,
...
"$SCRIPT_DIR/dj-pr-check.sh" "$ID" "$URL"
...
# the merge request. Sets DJ_PR_MERGE_HEAD to the verified head on success and
...
gitlab_verify_mergeable() {
  ...
  if ! json=$(GITLAB_HOST="$DJ_PR_HOST" glab mr view "$PR_NUMBER" -R "$PROJECT_URL" -F json 2>/dev/null) \
    || [ -z "$json" ]; then
...
FIELDS
  ...
  if ! dj_pr_head_valid "$live_head"; then
...
case "$PROVIDER" in
  ...
  gitlab)
    ...
    GITLAB_HOST="$DJ_PR_HOST" glab mr merge "$PR_NUMBER" -R "$PROJECT_URL" \
      --sha "$DJ_PR_MERGE_HEAD" --yes "$@"
    ;;
  *)
    echo "error: invalid PR merge request" >&2
    exit 2
