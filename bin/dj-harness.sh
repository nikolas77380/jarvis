...
# Usage: dj-harness.sh                  print own harness: claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|muse|unknown
#        dj-harness.sh crew             print the effective CREWMATE harness
#                                        (config/crew-harness; "default" resolves to own)
#        dj-harness.sh secondmate       print the harness the PRIMARY uses to launch
...
#        dj-harness.sh secondmate-model    print the optional MODEL token from
#                                        config/secondmate-harness, or empty when absent.
#        dj-harness.sh secondmate-effort   print the optional EFFORT token from
#                                        config/secondmate-harness, or empty when absent.
...
detect_own() {
  ...
  # the verdict correct; bin/dj-spawn.sh additionally clears the foreign markers
  # at the launch boundary. Both are kept: the launch sanitization only covers
  # sessions dj-spawn started, while this ordering also covers a cursor session
  ...
  if [ "${PI_CODING_AGENT:-}" = "true" ]; then
    if [ "${DJ_PI_HARNESS:-}" = pi-signed ]; then echo pi-signed; else echo pi; fi
    return
  fi
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    ...
    argv0=$(dj_cursor_argv0_for_pid "$pid" "$comm" 2>/dev/null || true)
    if dj_cursor_process_matches "$comm" '' "$argv0"; then
      echo cursor
      return
    fi
...
# name) wins; absent or "default" mirrors jarvis's own harness.
resolve_crew() {
  local crew=
