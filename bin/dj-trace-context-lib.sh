...
# Native W3C trace-context propagation for jarvis spawns (default-off).
#
# When enabled, jarvis resolves one W3C `traceparent` carrier for a task,
# injects it into the agent's pane shell as the TRACEPARENT environment variable
# before launch (bin/dj-spawn.sh, alongside GOTMPDIR, so it reaches every spawn
...
#
# TRACEPARENT here is a jarvis CONVENTION that carries a W3C-formatted
...
#
# Usage: . bin/dj-trace-context-lib.sh
#
# Public entry points:
#   dj_trace_context_session_start <config-dir> <effective-state-file>
#     Resolves config/trace-context plus DJ_TRACE_CONTEXT once and atomically
#     writes the normalized on/off decision bound to the locked home session.
#   dj_trace_context_session_effective <effective-state-file>
...
#   dj_trace_context_resolve <config-dir> <meta-file>
...
#   DJ_TRACE_CONTEXT        env override: 1/on/true/yes enables, any other
...
#   home (DJ_INHERITABLE_CONFIG in bin/dj-config-inherit-lib.sh) and passes its
#   frozen on/off decision into the new process as a non-empty DJ_TRACE_CONTEXT
#   value in the launch prefix (bin/dj-spawn.sh). The Secondmate freezes that
...
#   that task's meta: dj-spawn's spawn_remote_secondmate resolves the carrier,
#   hands it to the configured host through dj-spawn's --traceparent, and records
...
# guarantee any collector stores a span, and jarvis emits no spans itself.
#
...
# cleared, dj-spawn refuses to append the launch command. Every carrier this lib
# yields is either a jarvis-MINTED random root that reads no prompt, path,
...
dj_trace_context_hex() {  # <byte-count>
  local bytes=$1 hex
...
dj_trace_context_enabled() {  # <config-dir>
  ...
  if [ -n "${DJ_TRACE_CONTEXT:-}" ]; then
    v=$(printf '%s' "$DJ_TRACE_CONTEXT" | tr '[:upper:]' '[:lower:]')
    case "$v" in
...
dj_trace_context_session_lock() {  # <effective-state-file>
...
dj_trace_context_session_start() {  # <config-dir> <effective-state-file>
  ...
  lock_pid=$(dj_trace_context_session_lock "$effective_file") || {
    rm -f "$effective_file" 2>/dev/null || true
    return 0
  }
  dj_trace_context_enabled "$config_dir" && value=on
...
dj_trace_context_session_effective() {  # <effective-state-file>
  ...
  current_lock=$(dj_trace_context_session_lock "$effective_file") || {
    printf '%s' off
    return 0
  }
...
dj_trace_context_recorded() {  # <meta-file>
  local meta=$1 line
  [ -f "$meta" ] || return 0
...
dj_trace_context_mint() {
  local trace span tp
  trace=$(dj_trace_context_hex 16) || return 1
  span=$(dj_trace_context_hex 8) || return 1
  tp="00-$trace-$span-01"
  dj_trace_context_valid "$tp" || return 1
  printf '%s' "$tp"
}
...
dj_trace_context_resolve() {  # <config-dir> <meta-file>
  ...
  dj_trace_context_enabled "$config_dir" || return 0
  existing=$(dj_trace_context_recorded "$meta")
  if dj_trace_context_valid "$existing"; then
    printf '%s' "$existing"
    return 0
  fi
  dj_trace_context_mint || return 0
}
