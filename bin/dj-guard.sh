...
# dj-wake-drain.sh after it empties queued wakes, and by dj-session-start.sh in
# read-only advisory mode whenever session-lock ownership was not verified.
# First, always warn if the jarvis primary checkout (DJ_ROOT) is on a named
# non-default branch, because that means jarvis-on-itself work landed in the
...
# has. Supervision health is MODEL-AWARE (dj_watcher_supervision_verdict in
# bin/dj-wake-lib.sh): under the Claude Stop auto-arm model the watcher runs only
# between turns, so mid-turn a fresh beacon with no live watcher is healthy and
# only a stale beacon (beyond DJ_GUARD_GRACE) is a genuine lapse; under the Pi
...
# per distinct down-episode in this DJ_HOME (keyed to the failing condition, not
...
queue_pending=false
READ_ONLY=${DJ_GUARD_READ_ONLY:-0}
case "$READ_ONLY" in 1|true|TRUE|yes|YES) READ_ONLY=1 ;; *) READ_ONLY=0 ;; esac
CONTINUE_LINE=${DJ_GUARD_CONTINUE_LINE:-This is a supervision warning only; the guarded operation WILL still run.}
...
# shellcheck source=bin/dj-tangle-lib.sh
...
tangle_branch=$(dj_primary_tangle_branch "$DJ_ROOT" || true)
...
# grace-based predicate (bin/dj-supervision-lib.sh). Act when work, an event
# source, or an X-mode relay poll needs supervision.
dj_supervision_status "$STATE" "$GRACE"
in_flight=$DJ_SUP_IN_FLIGHT
sources=$DJ_SUP_SOURCES
needed=$DJ_SUP_NEEDED
beacon_desc=$DJ_SUP_BEACON_DESC
dj_watcher_supervision_verdict "$STATE" "$WATCH" "$GRACE" "$DJ_HOME" "$DJ_ROOT"
watcher_healthy=$DJ_WATCHER_VERDICT_OK
watcher_down_reason=$DJ_WATCHER_VERDICT_REASON
if [ "$needed" = false ]; then
  ...
  [ "$READ_ONLY" -eq 1 ] || dj_guard_clear_stale_banner
  exit 0
fi

[ -s "$DJ_WAKE_QUEUE" ] && queue_pending=true
...
if [ "$watcher_healthy" = false ]; then
  episode_key=$(dj_guard_stale_episode_key "$watcher_down_reason")
  episode_key=${episode_key%$'\n'}
  print_full_banner=0
  if [ "$READ_ONLY" -eq 1 ]; then
    dj_guard_stale_banner_seen "$STATE" "$episode_key" || print_full_banner=1
  elif dj_guard_claim_stale_banner "$STATE" "$episode_key"; then
    print_full_banner=1
  fi
  if [ "$print_full_banner" -eq 1 ]; then
    ...
    fix=$("$SCRIPT_DIR/dj-supervision-instructions.sh" \
      --read-only "$READ_ONLY" \
      --afk "$afk" \
      --x-mode "$x_mode" \
...
if "$queue_pending"; then
  if [ "$READ_ONLY" -eq 1 ]; then
    ...
    echo "WARNING: queued wakes pending - drain them with bin/dj-wake-drain.sh before anything else." >&2
  fi
fi
exit 0
