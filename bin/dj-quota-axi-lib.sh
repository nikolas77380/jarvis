...
# Usage: . bin/dj-quota-axi-lib.sh
#
# DJ_QUOTA_AXI_MIN follows the axi-family floor policy owned beside the floor
# constants in bin/dj-bootstrap.sh.
#
# This file is the single owner of that version number. bin/dj-bootstrap.sh
...
DJ_QUOTA_AXI_MIN=0.1.29

dj_quota_axi_compatible() {
  ...
  # The floor is compared from DJ_QUOTA_AXI_MIN so bumping it needs one edit.
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$DJ_QUOTA_AXI_MIN"
