...
#
# Usage: dj-x-dismiss.sh <request_id>
#
# When jarvis decides NOT to reply to a mention (a pure acknowledgment, or any
...
#
# Preview / dry-run: with DJX_DRY_RUN set (truthy), nothing is posted. Instead the
...
usage() {
  echo "usage: dj-x-dismiss.sh <request_id>" >&2
}
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "dj-x-dismiss: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "dj-x-dismiss: jq not found" >&2; exit 1; }
...
PAYLOAD=$(jq -cn --arg rid "$REQ" '{request_id:$rid}') || {
  echo "dj-x-dismiss: failed to build request payload" >&2; exit 1; }
...
if [ -n "$DJX_DRY" ]; then
  ...
  OUTREC=$(printf '%s' "$PAYLOAD" | jq -c '. + {endpoint:"dismiss"}') || {
    echo "dj-x-dismiss: failed to build dry-run outbox record" >&2; exit 1; }
  printf '%s\n' "$OUTREC" \
    | fmx_private_artifact_publish_stdin "$outbox_dir" "$REQ.json" 600 || {
    echo "dj-x-dismiss: cannot write dry-run outbox: $outbox_dir/$REQ.json" >&2
    exit 1
  }
  ...
  printf 'dj-x-dismiss: DRY RUN - would POST to %s/connector/dismiss (recorded: state/x-outbox/%s.json)\n' \
    "$DJX_RELAY" "$REQ" >&2
  printf '%s\n' "$REQ"
  exit 0
fi
...
if [ -z "$DJX_TOKEN" ]; then
  echo "dj-x-dismiss: X mode not configured (no DJX_PAIRING_TOKEN)" >&2
  exit 1
fi
command -v curl >/dev/null 2>&1 || { echo "dj-x-dismiss: curl not found" >&2; exit 1; }
AUTH_HEADER_FILE=$(fmx_auth_header_file) || {
  echo "dj-x-dismiss: invalid DJX_PAIRING_TOKEN" >&2
  exit 1
}
...
code=$(curl -m 10 -s -o /dev/null -w '%{http_code}' \
  ...
  --data "$PAYLOAD" \
  "$DJX_RELAY/connector/dismiss" 2>/dev/null) || {
  echo "dj-x-dismiss: request to relay failed" >&2
  exit 1
}
...
case "$code" in
  2[0-9][0-9])
    ...
    printf '%s\n' "$REQ"
    ;;
  *) echo "dj-x-dismiss: relay returned HTTP $code" >&2; exit 1 ;;
esac
