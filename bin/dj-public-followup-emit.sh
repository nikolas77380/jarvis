#!/usr/bin/env bash
# dj-public-followup-emit.sh - emit ONE structured terminal work result for work
...
# TYPED result. Jarvis must never recover the source home, work id, outcome,
...
# home (bin/dj-public-followup.sh deliver).
#
# Usage:
#   dj-public-followup-emit.sh --home <owning-home> \
...
#                          `dj-public-followup.sh register`.
...
usage() {
  cat >&2 <<'EOF'
usage: dj-public-followup-emit.sh --home <owning-home> --obligation <id> --relation <id>
...
die() { printf 'dj-public-followup-emit: %s\n' "$1" >&2; exit "${2:-2}"; }
...
dj_pf_slug_valid "$OBLIGATION" || die "unsafe obligation id: $OBLIGATION"
dj_pf_slug_valid "$RELATION"   || die "unsafe relation id: $RELATION"
dj_pf_slug_valid "$WORK_ID"    || die "unsafe work id: $WORK_ID"
dj_pf_slug_valid "$OUTCOME"    || die "unsafe outcome type: $OUTCOME"
dj_pf_home_id_valid "$SOURCE_HOME" \
...
reg_mismatch relation   "$(dj_pf_registry_get "$STATE" "$OBLIGATION" relation_id)" "$RELATION"
reg_mismatch source-home "$(dj_pf_registry_get "$STATE" "$OBLIGATION" work_home)"  "$SOURCE_HOME"
reg_mismatch work-id    "$(dj_pf_registry_get "$STATE" "$OBLIGATION" work_id)"     "$WORK_ID"
reg_mismatch generation "$(dj_pf_registry_get "$STATE" "$OBLIGATION" generation)"  "$GENERATION"

case "$TEXT_MODE" in
  inline) OUTCOME_TEXT=$(printf '%s' "$TEXT_SOURCE" | dj_pf_clean_outcome_text) ;;
  file)
    if [ "$TEXT_SOURCE" = '-' ]; then
      OUTCOME_TEXT=$(dj_pf_clean_outcome_text)
    else
      [ -f "$TEXT_SOURCE" ] || die "outcome text file not found: $TEXT_SOURCE"
      OUTCOME_TEXT=$(dj_pf_clean_outcome_text < "$TEXT_SOURCE")
    fi
    ;;
esac
...
EVENT_JSON=$(jq -Sc -n \
  --argjson schema_version "$DJ_PF_EVENT_SCHEMA_VERSION" \
  ...
  --argjson outcome_max "$DJ_PF_OUTCOME_TEXT_MAX" \
...
[ "$EVENT_BYTES" -le "$DJ_PF_EVENT_BYTES_MAX" ] \
  || die "typed terminal event exceeds $DJ_PF_EVENT_BYTES_MAX bytes" 2

printf '%s\n' "$EVENT_JSON" \
  | fmx_private_artifact_publish_stdin_once "$(dj_pf_events_dir "$STATE")" "$EVENT_ID.json" 600
case $? in
  0|1) printf '%s\n' "$EVENT_ID" ;;
