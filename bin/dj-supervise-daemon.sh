#!/usr/bin/env bash
# dj-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/dj-watch.sh: runs it as a child, presents and classifies every
# durable wake after an actionable close, acknowledges only after routing, and
# either SELF-HANDLES the routine majority in bash (no jarvis turn) or
...
# signal/stale/heartbeat wakes cost zero jarvis context; only done/
...
# user message clears it and jarvis resumes full responsiveness.
# When afk is off, normal dj-watch.sh always-on triage is the active mechanism.
...
#
# IN-BAND OPERATIONAL INPUT. bin/dj-operational-input.sh constructs every
# current daemon injection as the typed away-supervisor kind after the stable
# DJ_OPERATIONAL_PREFIX. A human cannot type its leading U+2063 from a normal
# keyboard at the start of a message, and Herdr transports it as text.
# Jarvis's contract: a message that starts with the current prefix, or a
...
#     crash/restart/missed injection is recovered on the next dj-wake-drain.sh.
...
#     dj-classify-lib.sh's combined predicate - instead gets its own longer
#     PAUSE_RESURFACE_SECS recheck, never a wedge escalation.
#     Crewmates are autonomous, so a delayed stale response does not stall a
#     healthy crewmate's own progress.
#     Buffered escalation delivery also has a max-defer alarm: if a digest stays
#     undelivered past DJ_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#          DJ_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   captain-relevant escalation for matching
#                                   kinds.
#          DJ_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          DJ_PAUSE_RESURFACE_SECS  idle seconds before a declared wait (external
#                                   or captain-held) re-surfaces as a recheck
#                                   (default 3600)
#          DJ_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          DJ_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          DJ_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          DJ_BUSY_REGEX            optional rendered busy-signature override
#                                   for delivery guards and Grok's fallback
#          DJ_COMPOSER_IDLE_RE      optional shared classifier override; see
#                                   docs/configuration.md for its safety gates
#          DJ_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          DJ_WEDGE_ALARM_CHANNEL   override config/wedge-alarm with a single
...
#          DJ_WEDGE_ALARM_EXEC      notifier seam: when set, every notifier
...
#          DJ_WEDGE_ALARM_TIMEOUT_SECS seconds allowed for each notifier before
#                                   its watchdog terminates it and continues to the
#                                   next channel (default 10; invalid/zero uses the
#                                   default).
#          DJ_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/dj-tmux-lib.sh):
...
#          DJ_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          DJ_LOG_MAX_BYTES / DJ_LOG_KEEP_LINES / DJ_CRASH_*  log + crash guards
#          DJ_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing dj-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

DJ_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DJ_ROOT="${DJ_ROOT_OVERRIDE:-$(cd "$DJ_DAEMON_DIR/.." && pwd)}"
...
# corrected composer detection. Stale task rechecks use dj-backend.sh below.
# shellcheck source=bin/dj-tmux-lib.sh
. "$DJ_DAEMON_DIR/dj-tmux-lib.sh"

# shellcheck source=bin/dj-backend.sh
. "$DJ_DAEMON_DIR/dj-backend.sh"

# Canonical construction and parsing for every Jarvis operational input.
# shellcheck source=bin/dj-operational-input.sh
. "$DJ_DAEMON_DIR/dj-operational-input.sh"
...
. "$DJ_DAEMON_DIR/dj-classify-lib.sh"

# Supervisor-pane discovery (DJ_SUPERVISOR_TARGET_DEFAULT,
# DJ_SUPERVISOR_BACKEND_DEFAULT, discover_supervisor_target,
# discover_supervisor_backend). Shared with the script-owned away launcher
# (bin/dj-afk-launch.sh) so the captain-pane resolution has exactly one owner.
# shellcheck source=bin/dj-supervisor-target-lib.sh
. "$DJ_DAEMON_DIR/dj-supervisor-target-lib.sh"

# The single owner of semantic busy state for recorded tasks
# (dj_busy_classify).
# shellcheck source=bin/dj-busy-lib.sh
. "$DJ_DAEMON_DIR/dj-busy-lib.sh"

# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today. zellij, orca,
# and cmux are real backends elsewhere in jarvis (bin/dj-backend.sh) but this
...
# pane.
DJ_SUPERVISOR_SUPPORTED_BACKENDS="tmux herdr"
...
# live in bin/dj-classify-lib.sh, shared with the always-on watcher.
# Composer-empty detection, submit acknowledgement, and the harness-scoped
# supervisor-pane busy guard live in bin/dj-tmux-lib.sh.
# DJ_BUSY_REGEX also overrides Grok's isolated task-state fallback.
...
# --- presence-gating --------------------------------------------------------
# bin/dj-operational-input.sh owns the U+2063 JARVIS_OP bytes and typed
...
# Resolve the effective state dir. DJ_STATE_OVERRIDE wins (testing); otherwise
# $DJ_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${DJ_STATE_OVERRIDE:-$DJ_HOME/state}"; }

# --- portable stat (same trap as dj-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by jarvis on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes jarvis's afk-exit contract as a testable function.
...
# marker (a daemon escalation), 1 otherwise (a real user message). Jarvis's
...
message_is_injection() {  # <message-text>
  ...
  [ -n "$msg" ] || return 1
  case "$msg" in
    "$DJ_INJECT_MARK"*) return 0 ;;
  esac
  return 1
}

# strip_injection_marker: remove a current typed away envelope, the landed
# untyped JARVIS_OP prefix, or the legacy bare sentinel. Current grammar is
# delegated to its owner rather than reimplemented here.
strip_injection_marker() {  # <message-text>
  local msg=$1 body
  if dj_operational_input_body "$msg" body; then
    printf '%s' "$body"
    return
  fi
  case "$msg" in
    "$DJ_OPERATIONAL_PREFIX"*) msg=${msg#"$DJ_OPERATIONAL_PREFIX"} ;;
    "$DJ_INJECT_MARK"*) msg=${msg#"$DJ_INJECT_MARK"} ;;
  esac
  printf '%s' "$msg"
}
...
# bin/dj-supervisor-target-lib.sh (sourced above). dj_super_main below calls
# them exactly as before; the away launcher reuses the identical resolution to
# pass the captain pane in as DJ_SUPERVISOR_TARGET.

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_captain_relevant, window_to_task, and
# scan_captain_relevant_statuses come from bin/dj-classify-lib.sh (sourced above),
# the single classifier shared with bin/dj-watch.sh. The decision-string wrappers
...
# summary jarvis would otherwise have to re-read.
...
classify_stale() {  # <window> <state>
  ...
  if [ -n "$last" ] && status_is_paused_or_captain_held "$last"; then
    ...
    # (dj-classify-lib.sh owns which declarations qualify): an idle pane is
    ...
    # reuses the status line already read, no dj-crew-state.sh call, mirroring the
...
classify_check() {  # <full reason>  — check scripts print only when jarvis should wake
  printf 'escalate|%s' "$1"
}
...
migrate_watcher_pause_markers() {  # <state>
  ...
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    win=$(dj_backend_target_of_meta "$meta")
    [ -n "$win" ] || continue
...
# by bin/dj-composer-lib.sh), and future verdicts. The detector drops
...
# bin/dj-backend.sh's generic per-backend primitives rather than a hand-rolled
...
dj_daemon_primary_harness() {
  if [ -z "${DJ_DAEMON_PRIMARY_HARNESS:-}" ]; then
    DJ_DAEMON_PRIMARY_HARNESS=$("$DJ_DAEMON_DIR/dj-harness.sh" 2>/dev/null || printf 'unknown')
    [ -n "$DJ_DAEMON_PRIMARY_HARNESS" ] || DJ_DAEMON_PRIMARY_HARNESS=unknown
  fi
  printf '%s' "$DJ_DAEMON_PRIMARY_HARNESS"
}

pane_is_busy() {  # <target> [backend]
  local target=$1 backend=${2:-tmux} native tail40 harness
  harness=$(dj_daemon_primary_harness)
  native=$(dj_backend_busy_state "$backend" "$target" 2>/dev/null)
  case "$native" in
    busy) return 0 ;;
  esac
  tail40=$(dj_backend_capture "$backend" "$target" 40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -12 \
    | dj_busy_lines_match "$harness"
}

# pane_input_pending dispatches through dj_backend_composer_state and treats
...
pane_input_pending() {  # <target> [backend]
  ...
  [ "$(dj_backend_composer_state "$backend" "$target" 2>/dev/null)" != empty ]
}
...
# semantic busy-state contract (bin/dj-busy-lib.sh), 1 when it is not, and 2
...
stale_window_is_busy() {  # <window> <state>
  ...
  task=$(window_to_task "$win" "$state")
  label="dj-$task"
  tail40=$(dj_backend_capture "$backend" "$win" 40 "$label" 2>/dev/null) || return 2
  verdict=$(dj_busy_classify "$backend" "$win" "$harness" "$task" "$state" "$tail40")
  [ "${verdict%% *}" = busy ]
}
...
# non-empty, non-comment line. DJ_WEDGE_ALARM_CHANNEL overrides the file with a
...
# Print the configured channel directives, one per line. DJ_WEDGE_ALARM_CHANNEL
...
wedge_alarm_configured_channels() {
  ...
  if [ -n "${DJ_WEDGE_ALARM_CHANNEL:-}" ]; then
    printf '%s\n' "$DJ_WEDGE_ALARM_CHANNEL"
    return 0
  fi
  cfg="${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}/wedge-alarm"
  if [ -f "$cfg" ]; then
...
wedge_alarm_run_bounded() {
  ...
  shift
  timeout=${DJ_WEDGE_ALARM_TIMEOUT_SECS:-$WEDGE_ALARM_TIMEOUT_SECS_DEFAULT}
  case "$timeout" in
...
# DJ_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier: the resolved channel
...
wedge_alarm_os_notifier_override() {  # <channel> <summary>
  local channel=$1 summary=$2 rc exec_override=${DJ_WEDGE_ALARM_EXEC:-}
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    -e 'display notification (item 1 of argv) with title "jarvis: away-mode escalations WEDGED" sound name "Basso"' \
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "jarvis: away-mode escalations WEDGED" \
...
wedge_alarm_via_command() {  # <cmd> <summary>
  ...
  wedge_alarm_run_bounded command sh -c "$cmd" dj-wedge-alarm "$summary" \
...
wedge_alarm_emit() {  # <channel> <summary>
  local channel=$1 summary=$2 cmd=${3:-} rc exec_override=${DJ_WEDGE_ALARM_EXEC:-} WEDGE_ALARM_EMIT_ACTIVE=1
  case "$exec_override" in
    '') ;;
    discard) return 0 ;;
    *)
...
# an ERROR, drops a durable marker jarvis/recovery can surface, flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  max_defer="${DJ_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}"
  ...
  target="${DJ_SUPERVISOR_TARGET:-$DJ_SUPERVISOR_TARGET_DEFAULT}"
  backend="${DJ_SUPERVISOR_BACKEND:-$DJ_SUPERVISOR_BACKEND_DEFAULT}"
...
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${DJ_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${DJ_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${DJ_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
  ...
  # (2b) pause re-surface recheck. A declared wait idles by design (dj-classify-lib.sh's
  ...
  pause_secs=${DJ_PAUSE_RESURFACE_SECS:-$DJ_PAUSE_RESURFACE_SECS_DEFAULT}
  ...
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${DJ_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
...
window_for_task() {  # <task-key> [state]
  ...
  for meta in "$state"/*.meta; do
    ...
    w=$(dj_backend_target_of_meta "$meta")
    [ -n "$w" ] && { printf '%s' "$w"; return 0; }
  done
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':dj-' || true); do
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; jarvis drives the normal always-on
  ...
  dj_operational_input_encode away-supervisor "$msg" encoded || return 1
  msg=$encoded
  target="${DJ_SUPERVISOR_TARGET:-$DJ_SUPERVISOR_TARGET_DEFAULT}"
  # BACKEND-AWARE (previously a raw `tmux display-message` pane-exists probe):
  # dispatches through bin/dj-backend.sh so a herdr supervisor pane is checked
  # via the herdr adapter instead of always assuming tmux. Falls back to tmux
  # when unset (sourced/test contexts that never ran dj_super_main's startup
  # discovery), matching this function's pre-existing default assumption.
  backend="${DJ_SUPERVISOR_BACKEND:-tmux}"
  dj_backend_target_exists "$backend" "$target" || return 1
  ...
  #      composer. The shared classifier (dj_backend_composer_state ->
  #      dj_composer_classify_content, bin/dj-composer-lib.sh) reports 'pending'
  ...
  composer=$(dj_backend_composer_state "$backend" "$target" 2>/dev/null)
  ...
  # Dispatches through dj_backend_send_text_submit (bin/dj-backend.sh): for
  # backend=tmux this calls dj_backend_tmux_send_text_submit, a verbatim
  # re-export of dj_tmux_submit_core - byte-identical to calling it directly.
  retries=${DJ_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${DJ_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(dj_backend_send_text_submit "$backend" "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
...
should_force_self() {  # <reason>
  local reason=$1 skip="${DJ_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (DJ_INJECT_SKIP): $reason"
    return
  fi
  case "$reason" in
  ...
  case "$action" in
    escalate)
      ...
      mark_escalated_seen "$kind" "$arg" "$state"
      [ "${DJ_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
      ;;
    pause)
...
handle_durable_wakes() {  # <watcher-reason> <state>
  ...
  if ! "$DJ_DAEMON_DIR/dj-wake-drain.sh" > "$out" 2> "$err"; then
    cat "$err" >&2
    rm -f "$out" "$err"
    return 1
  fi
  ...
  "$DJ_DAEMON_DIR/dj-wake-drain.sh" --ack-through "$ack_through" \
    --recovery-generation "$ack_generation"
}

# --- log --------------------------------------------------------------------
# Uses LOG set by dj_super_main; harmless no-op-ish if unset (tests source fns
...
trim_log() {
  ...
  [ "$sz" -ge "${DJ_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/dj-daemon-log.XXXXXX") || return 0
  tail -n "${DJ_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/dj-daemon.test.sh).
# ============================================================================
...
dj_super_main() {
  ...
  # Export DJ_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/dj-wake-lib.sh
  DJ_STATE_OVERRIDE="$STATE" . "$DJ_DAEMON_DIR/dj-wake-lib.sh"

  local WATCH="$DJ_DAEMON_DIR/dj-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${DJ_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${DJ_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${DJ_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${DJ_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${DJ_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}
  ...
  if ! dj_lock_try_acquire "$LOCK"; then
    if [ -n "${DJ_LOCK_HELD_PID:-}" ]; then
      echo "error: another dj-supervise-daemon is already running (pid $DJ_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another dj-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"
  dj_pid_identity "${BASHPID:-$$}" > "$LOCK/pid-identity" 2>/dev/null || true

  # --- auto-discover the supervisor BACKEND (tmux vs herdr) first -----------
  # Priority: DJ_SUPERVISOR_BACKEND override > $TMUX_PANE (tmux) > $HERDR_ENV=1
  ...
  # into DJ_SUPERVISOR_BACKEND makes inject_msg/pane_is_busy/pane_input_pending
  ...
  backend_source="DJ_SUPERVISOR_BACKEND"
  if [ -z "${DJ_SUPERVISOR_BACKEND:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      backend_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      backend_source="HERDR_ENV"
    else
      backend_source="FALLBACK($DJ_SUPERVISOR_BACKEND_DEFAULT)"
    fi
  fi
  discovered_backend=$(discover_supervisor_backend) || true
  DJ_SUPERVISOR_BACKEND="$discovered_backend"
  local BACKEND="$DJ_SUPERVISOR_BACKEND"
  ...
  if ! dj_backend_list_contains "$DJ_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $DJ_SUPERVISOR_SUPPORTED_BACKENDS); set DJ_SUPERVISOR_BACKEND=tmux|herdr and DJ_SUPERVISOR_TARGET to run jarvis's own pane under a supported backend" >&2
    log "startup failed: unsupported supervisor backend '$BACKEND' (source=$backend_source)"
    dj_lock_release "$LOCK" 2>/dev/null || true
    rm -f "$PIDFILE" 2>/dev/null || true
    exit 1
  fi

  # --- auto-discover the supervisor target (the pane running jarvis) -----
  # Priority: DJ_SUPERVISOR_TARGET override > $TMUX_PANE (tmux; inherited from
  # the pane that launched the daemon, normally jarvis's own) >
  # $HERDR_PANE_ID (herdr, composed into "<session>:<pane-id>") > jarvis:0
  # fallback. Exporting the result into DJ_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="DJ_SUPERVISOR_TARGET"
  if [ -z "${DJ_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      target_source="HERDR_ENV(HERDR_PANE_ID)"
    else
      target_source="FALLBACK(jarvis:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no DJ_SUPERVISOR_TARGET, TMUX_PANE, or HERDR_ENV/HERDR_PANE_ID); falling back to '$discovered' — verify this is jarvis's pane" >&2
  fi
  DJ_SUPERVISOR_TARGET="$discovered"
  local TARGET="$DJ_SUPERVISOR_TARGET"

  # --- validate supervisor target at startup (a missing target is a typo) ---
  # Dispatches through bin/dj-backend.sh instead of a raw `tmux display-message`
  ...
  if ! dj_backend_target_exists "$BACKEND" "$TARGET"; then
    echo "error: supervisor target '$TARGET' does not resolve to a $BACKEND pane; set DJ_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; backend=$BACKEND; backend_source=$backend_source; afk=$afk_status; inject_skip='${DJ_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${DJ_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${DJ_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  migrate_watcher_pause_markers "$STATE"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/dj-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and jarvis itself is the consumer of escalations.
    ...
    # The watcher child runs on its own DJ_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${DJ_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  # execs the daemon, see bin/dj-afk-start.sh). Make it structurally impossible
  # for a sourced context to fire a real desktop notification from the wedge
  # alarm: default the DJ_WEDGE_ALARM_EXEC notifier seam to "discard" unless the
  ...
  : "${DJ_WEDGE_ALARM_EXEC:=discard}"
  export DJ_WEDGE_ALARM_EXEC
fi
