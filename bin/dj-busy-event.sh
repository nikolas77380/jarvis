#!/usr/bin/env bash
# dj-busy-event.sh - the ONLY writer of the semantic busy-state contract
# owned by bin/dj-busy-lib.sh (record format, gen binding, and classification
...
#       the record at seq=1 (default: busy, source dj-spawn, event
...
#       Claude dj-send --key Escape path (dj-interrupt) and jarvis recovery
#       paths (dj-recovery) may pass --current-gen to bind to the incarnation
#       armed right now.
#
...
usage() {
  cat >&2 <<'EOF'
usage:
  dj-busy-event.sh arm <state-dir> <id> [--state busy|idle|unknown] [--source S] [--event E]
  dj-busy-event.sh apply <state-dir> <id> <busy|idle|unknown> (--gen G | --current-gen) --source S --event E
  dj-busy-event.sh retire <state-dir> <id> (--gen G | --current-gen)
...
if [ "$CMD" != retire ]; then
  ...
  dj_busy_token_valid "$SOURCE" || { echo "error: invalid --source" >&2; exit 1; }
  dj_busy_token_valid "$EVENT" || { echo "error: invalid --event" >&2; exit 1; }
fi

REC=$(dj_busy_record_path "$STATE" "$ID")
GEN_FILE=$(dj_busy_gen_path "$STATE" "$ID")
LOCK="$REC.lock"
...
# exactly as bin/dj-watch.sh does.
...
# pair; a holder that died mid-write is broken after DJ_BUSY_LOCK_STALE_SECS.
lock_acquire() {
  ...
  while ! mkdir "$LOCK" 2>/dev/null; do
    ...
    if [ "$tries" -ge 40 ]; then
      ...
      # aborting the writer - and its caller, dj-teardown.sh - under `set -u`.
      case "$mtime" in ''|*[!0-9]*) mtime=$now ;; esac
      age=$((now - mtime))
      if [ "$age" -ge "${DJ_BUSY_LOCK_STALE_SECS:-5}" ]; then
...
if [ "$USE_CURRENT_GEN" = 1 ] && [ "$CMD" != retire ]; then
  GEN=$(dj_busy_current_gen "$STATE" "$ID") || {
    umask "$old_umask"
...
if [ "$USE_CURRENT_GEN" != 1 ] || [ "$CMD" != retire ]; then
  dj_busy_token_valid "$GEN" || { umask "$old_umask"; echo "error: invalid --gen" >&2; exit 1; }
fi

lock_acquire || { umask "$old_umask"; exit 1; }
CURRENT=$(dj_busy_current_gen "$STATE" "$ID") || {
