#!/usr/bin/env bash
# dj-install-shellcheck.sh - install CI's pinned, verified ShellCheck build.
#
...
#
# Usage:
#   dj-install-shellcheck.sh <destination-directory>
set -eu

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERSION="$("$ROOT/bin/dj-lint.sh" --required-version)"

die() {
  printf 'dj-install-shellcheck.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: dj-install-shellcheck.sh <destination-directory>}
...
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/dj-shellcheck.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
while ! curl -fsSL "$URL" -o "$TMP/$ARCHIVE"; do
  [ "$download_attempt" -lt "$DOWNLOAD_ATTEMPTS" ] || {
    printf 'dj-install-shellcheck.sh: download failed after %s attempts\n' "$DOWNLOAD_ATTEMPTS" >&2
    exit 1
  }
  printf 'dj-install-shellcheck.sh: download attempt %s failed; retrying\n' "$download_attempt" >&2
...
[ "$ACTUAL_SHA256" = "$SHA256" ] || {
  printf 'dj-install-shellcheck.sh: checksum mismatch for %s (expected %s, got %s)\n' \
