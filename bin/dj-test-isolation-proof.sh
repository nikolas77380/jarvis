#!/usr/bin/env bash
# dj-test-isolation-proof.sh - bounded concurrent isolation proof for portable
...
# portable CI shards and bounded local dj-test-run.sh --jobs for this exact set
# are owned by bin/dj-test-run.sh (docs/dj-test-portable-shards.md).
#
# It does NOT:
#   - compose production CI shard membership (dj-test-run.sh owns that partition)
#   - run real Herdr, real default-server tmux, watcher lock races, AFK, live
#     harnesses, or GUI backends
#
# Usage:
#   dj-test-isolation-proof.sh [--jobs N] [--json path] [--list]
#   dj-test-isolation-proof.sh --list-exclusions
#   dj-test-isolation-proof.sh -h | --help
#
# Options:
...
#   - TMPDIR/TMP point only at that root so mktemp/dj_test_tmproot stay private
#   - ambient DJ_HOME / DJ_*_OVERRIDE cleared so no shared home is reused
#   - no global git config mutation (snapshot before/after)
#   - no production sharding and no retry-until-green
#
# Markers (stdout):
#   DJ_ISOLATION_BEGIN <iso8601> concurrency=<n> candidates=<n>
#   DJ_ISOLATION_CANDIDATE_BEGIN <iso8601> <script> worker=<i>
#   DJ_ISOLATION_CANDIDATE_END <iso8601> <script> exit=<code> duration_ms=<n> worker=<i>
#   DJ_ISOLATION_SUMMARY total=<n> failed=<n> concurrency=<n> duration_ms=<n>
#
...
die() {
  printf 'dj-test-isolation-proof: %s\n' "$*" >&2
  exit 2
}
...
exclusion_reason() {
  case "$1" in
    ...
    dj-backend.test.sh)
      ...
      ;;
    dj-spawn-dispatch-profile.test.sh|dj-spawn-worktree-settle.test.sh|dj-trace-context-spawn.test.sh)
      printf '%s\n' 'real isolated git worktrees plus spawn settle loops; gray zone until dedicated proof'
      ;;
    dj-pr-check-security.test.sh)
    ...
    dj-teardown.test.sh)
      ...
      ;;
    dj-herdr-session-cleanup.test.sh)
    ...
    dj-wake-queue.test.sh|dj-watch-checkpoint.test.sh|dj-watch-triage.test.sh|\
    dj-watcher-lock.test.sh)
      printf '%s\n' 'watcher/wake/lock family; intentional process locks and daemon races'
      ;;
    dj-afk-inject-e2e.test.sh|dj-afk-return.test.sh|dj-afk-inject-herdr-e2e.test.sh|\
    dj-afk-launch.test.sh)
    ...
    dj-opencode-primary-live-e2e.test.sh|dj-pi-primary-live-e2e.test.sh|\
    dj-quota-array-dispatch-live-e2e.test.sh|dj-send-secondmate-marker-herdr-e2e.test.sh|\
    dj-sessionstart-instruction-refresh-live-e2e.test.sh)
      printf '%s\n' 'live harness opt-in; never default parallel CI'
      ;;
    dj-backend-autodetect-smoke.test.sh|dj-backend-herdr-eventwait-smoke.test.sh|\
    dj-backend-herdr-presentation-e2e.test.sh|dj-backend-herdr-prune-safety-e2e.test.sh|\
    dj-backend-herdr-respawn-idem-e2e.test.sh|dj-backend-herdr-smoke.test.sh|\
    dj-backend-herdr-workspace-per-home-e2e.test.sh|dj-herdr-session-cleanup-e2e.test.sh)
...
dj-spawn-worktree-settle.test.sh
dj-trace-context-spawn.test.sh
dj-pr-check-security.test.sh
dj-teardown.test.sh
dj-watcher-lock.test.sh
dj-wake-queue.test.sh
dj-afk-inject-e2e.test.sh
dj-backend-herdr-smoke.test.sh
dj-backend-cmux-smoke.test.sh
dj-pi-primary-live-e2e.test.sh
dj-quota-array-dispatch-live-e2e.test.sh
EOF
}
...
doc = {
    ...
    "dj_test_run_jobs_enabled": False,
}
...
PROOF_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/dj-isolation-proof.XXXXXX")
...
RUN_ID="dj-isolation-${RUN_STARTED_MS}-$$"
TOTAL=${#CANDIDATES[@]}
FAILED=0
AGG_RC=0

printf 'DJ_ISOLATION_BEGIN %s concurrency=%s candidates=%s\n' \
...
wait_one_slot() {
  ...
  printf 'DJ_ISOLATION_CANDIDATE_END %s %s exit=%s duration_ms=%s worker=%s\n' \
...
for script in "${CANDIDATES[@]}"; do
  ...
  printf 'DJ_ISOLATION_CANDIDATE_BEGIN %s %s worker=%s\n' \
    "$(now_iso)" "$script" "$idx"
...
if find "$PROOF_ROOT" -type f -name 'dj-arm-pretool-check-claude-stderr.*' 2>/dev/null | grep -q .; then
...
printf 'DJ_ISOLATION_SUMMARY total=%s failed=%s concurrency=%s duration_ms=%s\n' \
