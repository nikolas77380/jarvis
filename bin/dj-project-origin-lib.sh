...
# sourced by both the sending parent (bin/dj-remote-home-seed.sh) and the
# receiving host (bin/dj-remote-home-provision.sh), so an unsafe value is
...
# Jarvis is a shared template, so any host must be able to serve a project:
...
dj_project_origin_safe() { # <url>; 0 when the URL is an accepted clone URL
