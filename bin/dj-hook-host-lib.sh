...
# 2026.08.11-e8db854). A Cursor primary running in a Jarvis checkout therefore
...
# seatbelt twice. Jarvis's Cursor registration owns those events, so the
...
# Jarvis registration already covers this event.
dj_hook_payload_is_foreign_host() {  # <payload>
  local payload=${1-}
  [ -n "$payload" ] || return 1
