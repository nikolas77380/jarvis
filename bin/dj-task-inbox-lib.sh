#!/usr/bin/env bash
# dj-task-inbox-lib.sh - the per-task steering inbox: durable records plus a
# constant doorbell.
#
...
# bin/dj-send.sh writes and rings locally, the host-local remote steer leg
# (bin/dj-remote-secondmate-control.sh cmd_send) writes idempotently and rings
# on the remote host, bin/dj-watch.sh polls and re-rings, and the brief
# scaffold (bin/dj-brief.sh) tells the worker how to read and acknowledge;
# none of them restates the format.
#
# Design (captain-adopted, data/dj-send-reliability-reframe-s1/report.md): the
...
#
# Record format (dj_task_inbox_write / dj_task_inbox_body):
#   schema=dj-task-inbox.v1
#   at=<utc timestamp>
#   --
#   <exact message text; newlines are legal; a marked secondmate request keeps
#    its from-jarvis marker and corr token verbatim in this body>
#
...
#
# Re-ring ladder (dj_task_inbox_due_action): an unhandled message older than
# DJ_TASK_INBOX_GRACE_SECS is due one delivery attempt per grace period; an
# attempt may ring or be skipped to protect proven pending composer text. After
# DJ_TASK_INBOX_RING_MAX attempts without an acknowledgement it escalates. The
...
#
# dj_task_inbox_ring requires bin/dj-backend.sh's dispatch (sourced below); the
# other helpers are dependency-light. Sourced by bin/dj-send.sh, bin/dj-watch.sh,
# and tests. No side effects on source beyond its sourced libraries.
#
# Tunables (env):
#   DJ_TASK_INBOX_GRACE_SECS   default 90; delivery-attempt grace and spacing
#   DJ_TASK_INBOX_RING_MAX     default 3; delivery attempts before escalation
...
. "$_FM_TASK_INBOX_LIB_DIR/dj-wake-lib.sh"
# shellcheck source=/dev/null
. "$_FM_TASK_INBOX_LIB_DIR/dj-backend.sh"

DJ_TASK_INBOX_SCHEMA='dj-task-inbox.v1'
DJ_TASK_INBOX_GRACE_DEFAULT=90
DJ_TASK_INBOX_RING_MAX_DEFAULT=3
DJ_TASK_INBOX_LOCK_WAIT_DEFAULT=5

dj_task_inbox_grace_secs() {
  local g=${DJ_TASK_INBOX_GRACE_SECS:-$DJ_TASK_INBOX_GRACE_DEFAULT}
  case "$g" in ''|*[!0-9]*) g=$DJ_TASK_INBOX_GRACE_DEFAULT ;; esac
  printf '%s' "$g"
}

dj_task_inbox_ring_max() {
  local m=${DJ_TASK_INBOX_RING_MAX:-$DJ_TASK_INBOX_RING_MAX_DEFAULT}
  case "$m" in ''|*[!0-9]*) m=$DJ_TASK_INBOX_RING_MAX_DEFAULT ;; esac
  printf '%s' "$m"
}

dj_task_inbox_dir() {  # <state-dir> <task-id>
  printf '%s/%s.inbox' "$1" "$2"
}

dj_task_inbox_handled_dir() {  # <state-dir> <task-id>
  printf '%s/%s.inbox/handled' "$1" "$2"
}

# Numeric sequence of one record basename, or fail for a non-record name.
dj_task_inbox_seq_of() {  # <basename>
  local n=${1%.msg}
  [ "$n" != "$1" ] || return 1
...
dj_task_inbox_next_seq() {  # <inbox-dir>
  local dir=$1 max=0 d f n
  for d in "$dir" "$dir/handled"; do
    for f in "$d"/*.msg; do
      [ -e "$f" ] || continue
      n=$(dj_task_inbox_seq_of "${f##*/}") || continue
      [ "$n" -le "$max" ] || max=$n
    done
  done
  printf '%03d' "$((max + 1))"
...
dj_task_inbox_lock_acquire() {  # <lock-path>
  local lock=$1 wait=${DJ_TASK_INBOX_LOCK_WAIT_SECS:-$DJ_TASK_INBOX_LOCK_WAIT_DEFAULT}
  local deadline probe
  case "$wait" in ''|*[!0-9]*) wait=$DJ_TASK_INBOX_LOCK_WAIT_DEFAULT ;; esac
  probe=$(mktemp "${lock%/*}/.lock-probe.XXXXXX") || return 1
  rm -f "$probe" || return 1
  if [ ! -e "$lock" ] && [ ! -L "$lock" ]; then
    dj_lock_try_create "$lock" && return 0
    [ -e "$lock" ] || [ -L "$lock" ] || return 1
  fi
  deadline=$(( $(date +%s) + wait ))
  while ! dj_lock_try_acquire "$lock"; do
...
_fm_task_inbox_write_record_locked() {  # <inbox-dir> <text>
  ...
  seq=$(dj_task_inbox_next_seq "$dir")
  rec="$dir/$seq.msg"
  tmp=$(mktemp "$dir/.staging.XXXXXX") || return 1
  {
    printf 'schema=%s\n' "$DJ_TASK_INBOX_SCHEMA"
...
dj_task_inbox_write() {  # <state-dir> <task-id> <text>
  local state=$1 task=$2 text=$3 dir lock rec status=0
  dir=$(dj_task_inbox_dir "$state" "$task")
  mkdir -p "$dir/handled" || return 1
  lock="$dir/.seq.lock"
  dj_task_inbox_lock_acquire "$lock" || return 1
...
# local plane keeps plain dj_task_inbox_write: its outcome is synchronous, so
# a repeated identical local steer is a deliberate new instruction.
dj_task_inbox_write_idempotent() {  # <state-dir> <task-id> <text>
  ...
  if want=$(mktemp "$dir/.dedup.XXXXXX") && have=$(mktemp "$dir/.dedup.XXXXXX"); then
    if printf '%s' "$text" > "$want"; then
      for f in "$dir"/*.msg "$dir/handled"/*.msg; do
        ...
        if ! dj_task_inbox_body "$f" > "$have" 2>/dev/null; then
          case "$f" in
            "$dir"/*.msg)
              f="$dir/handled/${f##*/}"
              dj_task_inbox_body "$f" > "$have" 2>/dev/null || continue
              ;;
            *) continue ;;
          esac
        fi
...
dj_task_inbox_body() {  # <record-path>
  local line
  [ -f "$1" ] || return 1
...
dj_task_inbox_doorbell_line() {  # <record-path>
  local dir=${1%/*} abs
  abs=$(cd "$dir" 2>/dev/null && pwd) || abs=$dir
  printf 'Jarvis instruction waiting: list %s/*.msg and, in numeric order, read and act on each, then mv each handled file to %s/handled/.' \
    "$abs" "$abs"
}
...
dj_task_inbox_ring() {  # <backend> <target> <record-path> [expected-label]
  local backend=$1 target=$2 rec=$3 label=${4:-} line cstate verdict
  line=$(dj_task_inbox_doorbell_line "$rec")
  cstate=$(dj_backend_composer_state "$backend" "$target" "$label" 2>/dev/null) || cstate=unknown
  case "$cstate" in
    pending) return 1 ;;
  esac
  if ! verdict=$(dj_backend_send_text_submit "$backend" "$target" "$line" 1 0.4 0.3 "$label" 2>/dev/null); then
    return 2
  fi
...
dj_task_inbox_oldest_unhandled() {  # <state-dir> <task-id>
  local dir best='' best_n=0 f n
  dir=$(dj_task_inbox_dir "$1" "$2")
  for f in "$dir"/*.msg; do
    [ -e "$f" ] || continue
    n=$(dj_task_inbox_seq_of "${f##*/}") || continue
...
# a fresh ladder.
dj_task_inbox_due_action() {  # <state-dir> <task-id>
  local dir oldest base now grace max ladder rec_base count last
  dir=$(dj_task_inbox_dir "$1" "$2")
  if ! oldest=$(dj_task_inbox_oldest_unhandled "$1" "$2"); then
    rm -f "$dir/.ring-state" "$dir/.escalated" 2>/dev/null || true
    printf 'quiet'
    return 0
  fi
  base=${oldest##*/}
  grace=$(dj_task_inbox_grace_secs)
  if [ "$(dj_path_age "$oldest")" -lt "$grace" ]; then
    printf 'quiet'
    return 0
  fi
  count=0
...
dj_task_inbox_record_ring() {  # <state-dir> <task-id> <record-path>
...
dj_task_inbox_record_escalated() {  # <state-dir> <task-id> <record-path>
  local dir
  dir=$(dj_task_inbox_dir "$1" "$2")
  [ -d "$dir" ] || return 0
