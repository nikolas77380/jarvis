#!/usr/bin/env bash
# dj-control.sh - the CONTROL PLANE for a jarvis-owned agent: allowlisted
# lifecycle verbs addressed to an exact task id.
#
# Usage: dj-control.sh <task-id> interrupt
#        dj-control.sh <task-id> exit
#        dj-control.sh <task-id> relaunch [--harness <name>] [--model <name>]
#                                         [--effort <level>]
#                                         (--note <text> | --note-file <path>)
#
# Why this exists, and how it differs from dj-send.sh. bin/dj-send.sh is the
...
# (bin/dj-control-lib.sh) rather than improvised in agent prose, and a verified
# postcondition for every action. There is deliberately NO arbitrary-text and
# NO generic raw-key entry point here; dj-send remains the only way to send an
...
#              bin/dj-spawn.sh --relaunch. A failure before publication keeps
...
# endpoint, or discarding work stays with bin/dj-teardown.sh, which owns the
# landed-work test.
#
# `resume` is not a verb: it is not deterministic across the verified adapters
# (bin/dj-control-lib.sh's header owns that reasoning). `relaunch` covers the
...
# validation (bin/dj-backend.sh's dj_backend_validate_task_endpoint). A legacy
# dj-<id> label, an explicit session:window endpoint, and a bare window name
...
#   DJ_CONTROL_POLL              poll interval for postcondition waits (0.5)
#   DJ_CONTROL_SETTLE_WAIT       adapter acknowledgement wait after interrupt (5)
#   DJ_CONTROL_EXIT_WAIT         alive->dead wait after the exit command (30)
#   DJ_CONTROL_LAUNCH_WAIT       dead->alive wait after a relaunch (90)
#   DJ_CONTROL_EXIT_RETRIES      Enter retries for the exit command (3)
set -eu
...
# drive a crewmate's lifecycle (see bin/dj-gate-refuse-lib.sh).
dj_refuse_if_gate_agent

if [ -z "${DJ_HOME+x}" ] || [ -z "${DJ_HOME:-}" ]; then
  echo "error: DJ_HOME is not set; dj-control refuses to resolve a task without an explicit jarvis home" >&2
  exit 1
fi
[ -d "$DJ_HOME" ] || {
  echo "error: DJ_HOME '$DJ_HOME' is not a directory" >&2
  exit 1
}
STATE="${DJ_STATE_OVERRIDE:-$DJ_HOME/state}"
DATA="${DJ_DATA_OVERRIDE:-$DJ_HOME/data}"
[ -d "$STATE" ] || {
  echo "error: state dir '$STATE' is missing; dj-control cannot resolve tasks for DJ_HOME '$DJ_HOME'" >&2
  exit 1
}
...
POLL=${DJ_CONTROL_POLL:-0.5}
SETTLE_WAIT=${DJ_CONTROL_SETTLE_WAIT:-5}
EXIT_WAIT=${DJ_CONTROL_EXIT_WAIT:-30}
LAUNCH_WAIT=${DJ_CONTROL_LAUNCH_WAIT:-90}
EXIT_RETRIES=${DJ_CONTROL_EXIT_RETRIES:-3}
...
control_cleanup() {
  ...
  if declare -F dj_lease_guard_release >/dev/null 2>&1; then
...
if ! dj_control_verb_allowed "$VERB"; then
  {
    if [ "$VERB" = resume ]; then
...
case "$RAW_ID" in
  *:*) die "'$RAW_ID' is an explicit backend endpoint; dj-control accepts an exact task id only, so a lifecycle command can never land on an endpoint this home does not own" ;;
esac
if ! dj_task_id_creation_valid "$RAW_ID"; then
...
# live lease (contract: bin/dj-lease-lib.sh; no-op in homes without leases).
# shellcheck source=bin/dj-lease-lib.sh
. "$SCRIPT_DIR/dj-lease-lib.sh"
dj_lease_guard "$ID" "lifecycle control (dj-control)"
...
if [ ! -f "$META" ]; then
  case "$RAW_ID" in
    dj-*)
      if [ -f "$STATE/${RAW_ID#dj-}.meta" ]; then
        die "'$RAW_ID' is a window label, not a task id; pass the exact task id '${RAW_ID#dj-}'"
      fi
      ;;
  esac
  die "no task '$ID' in $STATE (dj-control resolves an exact task id only)"
fi
...
# instead, using the same `remote_host` signal bin/dj-send.sh routes on.
if [ -n "$(dj_meta_get "$META" remote_host)" ]; then
  die "task $ID is a remotely placed secondmate on $(dj_meta_get "$META" remote_host); its agent runs outside this home, so no lifecycle action here could verify that it interrupted, stopped, or came back. Drive its lifecycle on that host, and reconcile it through the secondmate recovery path rather than this plane"
fi
...
LABEL="dj-$ID"
RECORDED_HARNESS=$(dj_meta_get "$META" harness)
KIND=$(dj_meta_get "$META" kind)
WT=$(dj_meta_get "$META" worktree)
[ -n "$KIND" ] || KIND=ship

HARNESS=$(dj_control_harness_family "$RECORDED_HARNESS") \
  || die "task $ID records harness '${RECORDED_HARNESS:-none}', which has no verified control mechanics; dj-control refuses to guess an interrupt key or exit command"
dj_control_harness_supported "$HARNESS" \
  || die "task $ID records harness '${RECORDED_HARNESS:-none}', which has no verified control mechanics; dj-control refuses to guess an interrupt key or exit command"

dj_backend_validate "$BACKEND" || exit 1

# --- shared helpers ---------------------------------------------------------

agent_state() {
  dj_backend_agent_state "$BACKEND" "$T"
}

busy_verdict() {
  dj_busy_classify_meta "$META" "$ID" "$STATE"
}
...
require_state_verified_backend() {  # <verb>
  dj_control_backend_state_verified "$BACKEND" && return 0
...
send_interrupt_keys() {
  ...
  key=$(dj_control_interrupt_key "$HARNESS")
  repeat=$(dj_control_interrupt_repeat "$HARNESS")
  clear=$(dj_control_interrupt_clear_key "$HARNESS")
  ...
  [ -z "$clear" ] || dj_control_backend_supports_key "$BACKEND" "$clear" \
  ...
  while [ "$i" -lt "$repeat" ]; do
    dj_backend_send_key "$BACKEND" "$T" "$key" "$LABEL" \
      || die "interrupt key $key was not delivered to task $ID on $BACKEND"
    i=$((i + 1))
    [ "$i" -ge "$repeat" ] || sleep 0.2
  done
  [ -z "$clear" ] || dj_backend_send_key "$BACKEND" "$T" "$clear" "$LABEL" \
...
prepare_interrupt_ack() {
  INTERRUPT_ACK_SOURCE=$(dj_control_interrupt_ack_source "$HARNESS")
  INTERRUPT_ACK_LOG=
  INTERRUPT_ACK_RUN=
  case "$INTERRUPT_ACK_SOURCE" in
    muse-session-terminal)
      INTERRUPT_ACK_LOG=$(dj_busy_muse_session_log "$STATE" "$ID" 2>/dev/null || true)
      [ -n "$INTERRUPT_ACK_LOG" ] || return 0
      INTERRUPT_ACK_RUN=$(dj_busy_muse_active_run_id "$INTERRUPT_ACK_LOG" 2>/dev/null || true)
      ;;
  esac
}
...
interrupt_cancel_claim() {
  ...
  while :; do
    terminal=$(dj_busy_muse_run_terminal "$INTERRUPT_ACK_LOG" "$INTERRUPT_ACK_RUN" 2>/dev/null || true)
    case "$terminal" in
...
retire_busy_incarnation() {
  if [ -f "$STATE/$ID.busy-gen" ]; then
    "$SCRIPT_DIR/dj-busy-event.sh" retire "$STATE" "$ID" --current-gen >/dev/null 2>&1 || true
  fi
}
...
do_exit() {
  ...
  cmd=$(dj_control_exit_command "$HARNESS")
  ...
  verdict=$(dj_backend_send_text_submit "$BACKEND" "$T" "$cmd" "$EXIT_RETRIES" "$POLL" 1.2 "$LABEL") \
    ...
    exited|launching)
      if [ "$RELAUNCH_AGENT_CONFIRMED" = 1 ]; then
        ...
        echo "error: $ID's replacement is running on $TARGET_HARNESS, but transaction completion could not be persisted; its published record was retained for reconciliation" >&2
         ...
         || { [ -n "$RELAUNCH_TX" ] \
              && [ "$(dj_meta_get "$META" control_relaunch_tx)" = "$RELAUNCH_TX" ]; }; then
...
resolve_relaunch_profile() {
  PRIOR_HARNESS=$HARNESS
  PRIOR_RECORDED_HARNESS=$RECORDED_HARNESS
  PRIOR_MODEL=$(dj_meta_get "$META" model)
  PRIOR_EFFORT=$(dj_meta_get "$META" effort)
  ...
  if [ "$KIND" = secondmate ]; then
    ...
    # from jarvis's own dispatch-profile judgment at intake, and silently
    # re-resolving it would bypass that consultation.
    CONFIG_HARNESS=$("$SCRIPT_DIR/dj-harness.sh" secondmate 2>/dev/null || true)
    CONFIG_MODEL=$("$SCRIPT_DIR/dj-harness.sh" secondmate-model 2>/dev/null || true)
    CONFIG_EFFORT=$("$SCRIPT_DIR/dj-harness.sh" secondmate-effort 2>/dev/null || true)
  ...
  if [ "$HARNESS_SET" = 1 ]; then
    dj_control_harness_supported "$NEW_HARNESS" \
      || die "'$NEW_HARNESS' is not a verified harness; dj-control refuses to relaunch onto an adapter with no verified control or launch mechanics"
    TARGET_HARNESS=$NEW_HARNESS
  elif [ "$HARNESS_SET" = 0 ] && [ -n "$CONFIG_HARNESS" ]; then
    dj_control_harness_supported "$CONFIG_HARNESS" \
      || die "the configured secondmate harness '$CONFIG_HARNESS' is not verified; dj-control refuses to relaunch onto an adapter with no verified control or launch mechanics"
  ...
  dj_control_harness_supports_kind "$TARGET_HARNESS" "$KIND" \
...
safe_checkpoint() {
  ...
  if [ "$KIND" = secondmate ]; then
    ...
    marker=$(cat "$WT/.dj-secondmate-home" 2>/dev/null || true)
    [ "$marker" = "$ID" ] \
...
do_relaunch() {
  ...
  # The launch owner (dj-spawn --relaunch) clears the previous incarnation's
  ...
  if DJ_CONTROL_RELAUNCH_TX="$RELAUNCH_TX" \
      "$SCRIPT_DIR/dj-spawn.sh" "${spawn_args[@]}" >/dev/null; then
    RELAUNCH_META_PUBLISHED=1
  else
    [ "$(dj_meta_get "$META" control_relaunch_tx)" != "$RELAUNCH_TX" ] \
      || RELAUNCH_META_PUBLISHED=1
