...
# Usage: dj-send.sh <target> [--resolve-key <key>]... <text...>
#   <target> may be an exact task id, a legacy dj-<id> task label resolved
#   through this home's state/<id>.meta, or an explicit well-formed backend
#   target. dj-send refuses unresolved guesses rather than falling back to a
#   tmux window search, because a "successful" send to the wrong endpoint is
#   worse than a loud failure.
# Special keys instead of text: dj-send.sh <target> --key Enter
...
# local task, or the remote home's host-local inbox reached through dj-on.sh
...
# request onto the existing record (bin/dj-task-inbox-lib.sh), so after a lost
# transport (ssh exit 255, completion unknown) dj-send retries the same leg
# once itself. A later re-run is idempotent only through the printed
# DJ_PENDING_REPLY_EXISTING_CORR=<corr> command: it preserves the same
...
# unacknowledged message and escalating a stuck one. bin/dj-task-inbox-lib.sh
...
# bin/dj-tmux-lib.sh. Tune with DJ_SEND_RETRIES (default 3) / DJ_SEND_SLEEP
...
# dj-send-secondmate-harness-invocation-r1 owns that behavior.
#
# From-jarvis marker: when the resolved target is a task selector whose meta
# records kind=secondmate, the message uses the live-charter-compatible
# from-jarvis carrier owned by bin/dj-operational-input.sh so the secondmate
# routes its reply via its status file or a status-pointed doc instead of
# stranding it in chat the main jarvis never reads. On the inbox plane the
...
# state/pending-replies/ before delivery (bin/dj-pending-reply-lib.sh). Delivery
...
# Set DJ_PENDING_REPLY_EXISTING_CORR=<id> when re-sending a recovery request
# for an already-open expectation so a second record is not created. Direct
# unmarked captain input never creates one.
#
# Remote secondmate delivery: the send crosses dj-on.sh to a host-local leg
# (bin/dj-remote-secondmate-control.sh cmd_send) that writes the message as a
...
# locally: leg exit 0 means durably recorded (dj-send then exits 0, marks the
...
# so dj-send retries the identical leg once - safe because the remote write
...
# DJ_PENDING_REPLY_EXISTING_CORR=<id> resend command that preserves the body
...
# blocked: record in the target task's state/<id>.status. dj-send itself
...
# - because the open-decision ledger dj-wake-drain folds lives in this home's
...
# feeds bin/dj-captain-hold.sh's one keyed-answer intake for any key that names
# a captain-held task in this home - the key as a task id itself, or through
# the legacy `<task>-decision-<key>` identity for pre-collapse rows. dj-send
...
# in this home's status log per status_open_decisions (bin/dj-classify-lib.sh), or
...
#
# After a successful TYPED-plane submit dj-send pauses DJ_SEND_SETTLE seconds
...
# a crewmate (see bin/dj-gate-refuse-lib.sh).
dj_refuse_if_gate_agent

if [ -z "${DJ_HOME+x}" ] || [ -z "${DJ_HOME:-}" ]; then
  echo "error: DJ_HOME is not set; dj-send refuses to resolve targets without an explicit jarvis home" >&2
  exit 1
fi

STATE="${DJ_STATE_OVERRIDE:-$DJ_HOME/state}"
if [ ! -d "$DJ_HOME" ]; then
  echo "error: DJ_HOME '$DJ_HOME' is not a directory; dj-send cannot resolve this home's state" >&2
  exit 1
fi
if [ ! -d "$STATE" ]; then
  echo "error: state dir '$STATE' is missing; dj-send cannot resolve targets for DJ_HOME '$DJ_HOME'" >&2
  exit 1
fi
...
# shellcheck source=bin/dj-line-cap-lib.sh
. "$SCRIPT_DIR/dj-line-cap-lib.sh"
# shellcheck source=bin/dj-wake-lib.sh
. "$SCRIPT_DIR/dj-wake-lib.sh"
# shellcheck source=bin/dj-task-inbox-lib.sh
. "$SCRIPT_DIR/dj-task-inbox-lib.sh"

DJ_GUARD_CONTINUE_LINE='This is a supervision warning only; the requested message WILL still be sent.' "$SCRIPT_DIR/dj-guard.sh" || true

dj_send_id_from_meta() {  # <meta-file>
  local base
  base=${1##*/}
  printf '%s' "${base%.meta}"
}

# dj_send_clear_after_interrupt: muse RESTORES the interrupted prompt back into
...
# control-plane capability table (bin/dj-control-lib.sh) rather than a second
# copy here - the same table bin/dj-control.sh's interrupt verb reads.
dj_send_clear_after_interrupt() {  # <key>
  local key=$1 family clear
  [ "$key" = Escape ] || return 0
  family=$(dj_control_harness_family "$TARGET_HARNESS") || return 0
  clear=$(dj_control_interrupt_clear_key "$family") || return 0
  [ -n "$clear" ] || return 0
  [ "$TARGET_BACKEND" != remote ] || return 0
  if ! dj_backend_send_key "$TARGET_BACKEND" "$T" "$clear" "$EXPECTED_LABEL"; then
...
dj_send_record_interrupt() {  # <key>
  local key=$1 id gen
  [ "$key" = Escape ] || return 0
  case "$TARGET_HARNESS" in claude*) : ;; *) return 0 ;; esac
  [ -n "$TARGET_META" ] || return 0
  id=$(dj_send_id_from_meta "$TARGET_META")
  [ -f "$STATE/$id.busy-gen" ] || return 0
  gen=$(dj_meta_get "$TARGET_META" busy_gen)
  if [ -n "$gen" ]; then
    "$DJ_ROOT/bin/dj-busy-event.sh" apply "$STATE" "$id" idle \
      --gen "$gen" --source dj-interrupt --event interrupt
  else
    "$DJ_ROOT/bin/dj-busy-event.sh" apply "$STATE" "$id" idle \
      --current-gen --source dj-interrupt --event interrupt
  fi || {
...
dj_send_meta_for_key_value() {  # <state-dir> <key> <value>
  local state=$1 key=$2 value=$3 meta got
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    got=$(dj_meta_get "$meta" "$key")
    [ "$got" = "$value" ] || continue
    printf '%s' "$meta"
    return 0
  done
...
dj_send_count_colons() {  # <string>
  local s=$1 no_colons
  no_colons=${s//:/}
  printf '%s' $(( ${#s} - ${#no_colons} ))
}

dj_send_resolve_target() {  # <raw-target>
  ...
  if [ -n "$meta" ]; then
    if [ -n "$(dj_meta_get "$meta" remote_host)" ]; then
      id=$(dj_send_id_from_meta "$meta")
      RESOLVED_TARGET="remote:$id"
      TARGET_BACKEND=remote
      TARGET_META=$meta
      TARGET_HARNESS=$(dj_meta_get "$meta" harness)
      EXPECTED_LABEL="dj-$id"
      TARGET_SELECTOR=1
      TARGET_REMOTE_ID=$id
      TARGET_REMOTE_HOST=$(dj_meta_get "$meta" remote_host)
    ...
    TARGET_META=$meta
    TARGET_HARNESS=$(dj_meta_get "$meta" harness)
    EXPECTED_LABEL=$(dj_backend_expected_label_of_selector "$raw" "$STATE")
    TARGET_SELECTOR=1
    return 0
  fi
  ...
  case "$raw" in
    dj-*:*)
      # A named Herdr session may itself begin with "dj-". Keep that explicit
      # session:pane target on the validated backend-target path below rather
      # than mistaking it for an unresolved task selector.
      ;;
    dj-*)
      RESOLUTION_TRIED="meta=$STATE/$raw.meta; legacy-meta=$STATE/${raw#dj-}.meta; backend=none"
      echo "error: no metadata for $raw in $STATE (tried $RESOLUTION_TRIED); pass a well-formed explicit backend target only when targeting outside this jarvis home" >&2
      return 1
      ;;
  esac

  pane_meta=$(dj_send_meta_for_key_value "$STATE" herdr_pane_id "$raw" 2>/dev/null || true)
  if [ -n "$pane_meta" ]; then
    session=$(dj_meta_get "$pane_meta" herdr_session)
    hint="${session:-<herdr-session>}:$raw"
    id=$(dj_send_id_from_meta "$pane_meta")
    echo "error: target '$raw' matches herdr_pane_id in $pane_meta but is missing its herdr session prefix; expected <herdr-session>:<pane-id> such as '$hint' or use 'dj-$id' (tried meta=$STATE/$raw.meta; backend=herdr)" >&2
    return 1
  fi
  ...
  if [ -n "$meta" ]; then
    ...
    RESOLVED_TARGET=$target
    TARGET_BACKEND=$(dj_backend_of_meta "$meta")
    TARGET_META=$meta
    TARGET_HARNESS=$(dj_meta_get "$meta" harness)
  ...
  case "$raw" in
    *:*)
      colons=$(dj_send_count_colons "$raw")
      if [ "$colons" -ge 2 ]; then
        assumed=herdr
      else
        assumed=tmux
      fi
      if ! dj_backend_target_exists "$assumed" "$raw"; then
        echo "error: explicit target '$raw' is not a live $assumed endpoint (tried meta=$STATE/$raw.meta; metadata window/terminal lookup; backend=$assumed). Use dj-<id> for a recorded task/lane, or pass a target whose backend endpoint can be verified." >&2
        return 1
      fi
      RESOLVED_TARGET=$raw
  ...
  echo "error: target '$raw' is not resolvable (tried meta=$STATE/$raw.meta; metadata window/terminal lookup; backend=none). Use dj-$raw for a recorded task/lane, or pass a well-formed explicit backend target such as session:window." >&2
  return 1
}

RAW_TARGET=$1
dj_send_resolve_target "$RAW_TARGET" || exit 1
T=$RESOLVED_TARGET
shift
...
# untouched (contract: bin/dj-lease-lib.sh).
# shellcheck source=bin/dj-lease-lib.sh
. "$SCRIPT_DIR/dj-lease-lib.sh"
if [ -n "$TARGET_META" ]; then
  LEASE_GUARD_TASK=$(dj_send_id_from_meta "$TARGET_META")
  if [ -n "$LEASE_GUARD_TASK" ]; then
    dj_lease_guard "$LEASE_GUARD_TASK" "steer (dj-send)"
    trap 'dj_lease_guard_release' EXIT
  fi
fi
...
while :; do
  case "${1:-}" in
    --resolve-key)
      ...
      dj_send_add_resolve_key "$2" || exit 1
      shift 2
      ;;
    --resolve-key=*)
      dj_send_add_resolve_key "${1#--resolve-key=}" || exit 1
      shift
      ;;
    *) break ;;
  esac
...
if [ "$TARGET_BACKEND" != remote ]; then
  dj_backend_validate "$TARGET_BACKEND" || exit 1
fi

# Classify a from-jarvis -> secondmate request. Only a task selector resolved
# through this home's meta whose authoritative kind is secondmate is marked: the
# secondmate then routes its reply via the status path (see dj-marker-lib.sh).
...
TARGET_TASK_ID=
dj_send_known_undelivered_cleanup() {
  [ -n "$PENDING_REPLY_CORR" ] || return 0
  if [ "$PENDING_REPLY_CREATED" = 1 ]; then
    dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR"
  else
    dj_pending_reply_reset_known_undelivered "$STATE" "$PENDING_REPLY_CORR"
  fi
}
if [ -n "$TARGET_SELECTOR" ] && [ -n "$TARGET_META" ] && [ "$(dj_meta_get "$TARGET_META" kind)" = secondmate ]; then
  MARK_FROM_JARVIS=1
  TARGET_TASK_ID=$(dj_send_id_from_meta "$TARGET_META")
fi
...
# is owned by the status log: dj-captain-hold's `complete` closes that live copy
...
dj_send_hold_resolved_id() {  # <task-id> <decision-key>
  local show id state hold_kind
  command -v tasks-axi >/dev/null 2>&1 || return 1
  for id in "$2" "$1-decision-$2"; do
    show=$( (cd "$DJ_HOME" && tasks-axi show "$id" --full) 2>/dev/null ) || continue
...
if [ -n "$RESOLVE_KEYS" ]; then
  ...
  RESOLVE_TASK_ID=$(dj_send_id_from_meta "$TARGET_META")
  ...
  for k in $RESOLVE_KEYS; do
    ...
    if resolved_hold_id=$(dj_send_hold_resolved_id "$RESOLVE_TASK_ID" "$k"); then
...
# (bin/dj-wake-lib.sh) and does not wake this same session again; any
# concurrent foreign status bytes leave the watcher's wake path untouched.
dj_send_close_resolved_keys() {  # <answer-text>
  local note=$1 k line append_rc
  note=$(printf '%s' "$note" | tr '\n\r\t' '   ' | LC_ALL=C tr -d '\000-\037\177')
  for k in $RESOLVE_STATUS_KEYS; do
    line="resolved [key=$k]: answered: $note"
    dj_cap_line_var "$line"
    append_rc=0
    dj_wake_status_append_self_announced "$STATE" "$RESOLVE_STATUS_FILE" "$DJ_LINE_CAP_LINE" || append_rc=$?
...
# lines, exactly the way every other channel does. dj-send decides nothing here:
...
dj_send_feed_resolved_holds() {  # <answer-text>
  ...
  if ! printf '%s' "$lines" | "$SCRIPT_DIR/dj-captain-hold.sh" answers \
    --source "a jarvis answer sent to $RESOLVE_TASK_ID" >/dev/null 2>&1; then
    echo "error: the answer was delivered to $T, but this captain-held task could not be closed: ${RESOLVE_HOLD_KEYS}. Close it with dj-captain-hold.sh answer - do not resend the answer." >&2
    return 1
  fi
}

# Resolve the target's harness from its meta (recorded by dj-spawn), used only to
...
if [ "${1:-}" = "--key" ]; then
  ...
  key=$2
  semantic_key=$(dj_send_normalize_key "$key")
  if [ "$TARGET_BACKEND" = remote ]; then
    if ! "$SCRIPT_DIR/dj-on.sh" "$TARGET_REMOTE_ID" dj-remote-secondmate-control.sh key "$TARGET_REMOTE_ID" "$key" < /dev/null; then
      echo "error: key '$key' not sent to remote secondmate $TARGET_REMOTE_ID; completion may be unknown" >&2
      exit 1
    fi
  elif ! dj_backend_send_key "$TARGET_BACKEND" "$T" "$key" "$EXPECTED_LABEL"; then
    echo "error: key '$key' not sent to $T ($TARGET_BACKEND send failed; tried $RESOLUTION_TRIED)" >&2
    exit 1
  fi
  dj_send_clear_after_interrupt "$semantic_key" || exit 1
  dj_send_record_interrupt "$semantic_key" || exit 1
else
  MESSAGE=$*
  ...
  if [ "$MARK_FROM_JARVIS" = 1 ]; then
    # Reuse an existing correlation id for recovery resends; otherwise create a
    # durable parent expectation before delivery. Transport success never
    # resolves that expectation (see dj-pending-reply-lib.sh).
    existing_corr_explicit=0
    if [ "${DJ_PENDING_REPLY_EXISTING_CORR+x}" = x ]; then
      existing_corr_explicit=1
      existing_corr=$DJ_PENDING_REPLY_EXISTING_CORR
    else
      existing_corr=$(dj_pending_reply_extract_corr "$MESSAGE")
    fi
    if [ -n "$existing_corr" ] \
      && dj_pending_reply_corr_reusable "$STATE" "$existing_corr" "$TARGET_TASK_ID"; then
      ...
      PENDING_REPLY_CORR=$(dj_pending_reply_create "$DJ_HOME" "$STATE" "$TARGET_TASK_ID" "$MESSAGE") \
        || { echo "error: failed to create parent pending-reply expectation for $TARGET_TASK_ID" >&2; exit 1; }
      PENDING_REPLY_CREATED=1
    fi
    dj_pending_reply_embed_corr "$MESSAGE" "$PENDING_REPLY_CORR" MESSAGE
    if [ "$PENDING_REPLY_CREATED" != 1 ] \
      && dj_pending_reply_delivery_attempt_unresolved "$STATE" "$PENDING_REPLY_CORR"; then
      if [ "$TARGET_BACKEND" = remote ]; then
        if ! dj_pending_reply_reset_known_undelivered "$STATE" "$PENDING_REPLY_CORR"; then
    ...
    if ! dj_pending_reply_prepare_delivery "$STATE" "$PENDING_REPLY_CORR"; then
      [ "$PENDING_REPLY_CREATED" != 1 ] \
        || dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
  ...
  if [ "$INBOX_PLANE" = 1 ] && [ "$TARGET_BACKEND" = remote ]; then
    ...
    REMOTE_META_LOCK=$(dj_meta_lock_path "$TARGET_META") || exit 1
    if ! dj_task_inbox_lock_acquire "$REMOTE_META_LOCK"; then
    ...
    if [ -f "$TARGET_META" ]; then
      CURRENT_REMOTE_ID=$(dj_send_id_from_meta "$TARGET_META")
      CURRENT_REMOTE_HOST=$(dj_meta_get "$TARGET_META" remote_host)
    fi
    if [ "$CURRENT_REMOTE_ID" != "$TARGET_REMOTE_ID" ] \
      || [ -z "$CURRENT_REMOTE_HOST" ] \
      || [ "$CURRENT_REMOTE_HOST" != "$TARGET_REMOTE_HOST" ]; then
      dj_lock_release "$REMOTE_META_LOCK"
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    "$SCRIPT_DIR/dj-on.sh" "$TARGET_REMOTE_ID" dj-remote-secondmate-control.sh send \
    ...
    if [ "$remote_rc" -ne 0 ] && [ "$remote_completion_unknown" -eq 1 ]; then
      if [ -n "$PENDING_REPLY_CORR" ]; then
        dj_pending_reply_mark_delivery_unknown "$STATE" "$PENDING_REPLY_CORR" || true
      fi
      if [ "$remote_rc" -eq 255 ]; then
      ...
      resend_home=$(cd "$DJ_HOME" 2>/dev/null && pwd) || resend_home=$DJ_HOME
      printf 'DJ_HOME=%q ' "$resend_home" >&2
      if [ "${DJ_STATE_OVERRIDE+x}" = x ]; then
        resend_state=$(cd "$STATE" 2>/dev/null && pwd) || resend_state=$STATE
        printf 'DJ_STATE_OVERRIDE=%q ' "$resend_state" >&2
      fi
      printf 'DJ_PENDING_REPLY_EXISTING_CORR=%q %q' "$PENDING_REPLY_CORR" "$SCRIPT_DIR/dj-send.sh" >&2
      for resend_arg in "${DJ_SEND_ORIGINAL_ARGS[@]}"; do
    ...
    if [ -n "$PENDING_REPLY_CORR" ]; then
      if dj_pending_reply_confirm_delivery "$STATE" "$PENDING_REPLY_CORR"; then
        :
      else
    ...
    if [ -n "$RESOLVE_KEYS" ]; then
      dj_send_close_resolved_keys "$RESOLVE_ANSWER_TEXT" || exit 1
      dj_send_feed_resolved_holds "$RESOLVE_ANSWER_TEXT" || exit 1
    fi
    exit 0
  fi
  if [ "$INBOX_PLANE" = 1 ]; then
    INBOX_TASK_ID=$(dj_send_id_from_meta "$TARGET_META")
    INBOX_META_LOCK=$(dj_meta_lock_path "$TARGET_META") || exit 1
    if ! dj_task_inbox_lock_acquire "$INBOX_META_LOCK"; then
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    if [ -f "$TARGET_META" ]; then
      CURRENT_INBOX_TARGET=$(dj_backend_target_of_meta "$TARGET_META")
      CURRENT_INBOX_BACKEND=$(dj_backend_of_meta "$TARGET_META")
    fi
    if [ "$CURRENT_INBOX_TARGET" != "$T" ] \
      || [ "$CURRENT_INBOX_BACKEND" != "$TARGET_BACKEND" ] \
      || [ -n "$(dj_meta_get "$TARGET_META" remote_host)" ]; then
      dj_lock_release "$INBOX_META_LOCK"
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    if ! INBOX_RECORD=$(dj_task_inbox_write "$STATE" "$INBOX_TASK_ID" "$MESSAGE"); then
      dj_lock_release "$INBOX_META_LOCK"
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        dj_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    ring_rc=0
    dj_task_inbox_ring "$TARGET_BACKEND" "$T" "$INBOX_RECORD" "$EXPECTED_LABEL" || ring_rc=$?
    case "$ring_rc" in
      1) echo "dj-send: doorbell skipped (composer visibly holds pending text); the steer is durably recorded at $INBOX_RECORD and the watcher will re-ring" >&2 ;;
      2) echo "dj-send: doorbell did not reach $T; the steer is durably recorded at $INBOX_RECORD and the watcher will re-ring" >&2 ;;
    esac
    exit 0
  fi
  ...
  retries=${DJ_SEND_RETRIES:-3}
  sleep_s=${DJ_SEND_SLEEP:-0.4}
  ...
  # earlier.
  send_rc=0
  if verdict=$(dj_backend_send_text_submit "$TARGET_BACKEND" "$T" "$MESSAGE" "$retries" "$sleep_s" "$settle" "$EXPECTED_LABEL"); then
    :
  else
    send_rc=$?
  fi
  ...
  case "$verdict" in
    ...
    pending)
      ...
      # (bin/dj-pending-reply-lib.sh).
      echo "dj-send: text delivered to $T but submission is unconfirmed (verdict=pending; tried $RESOLUTION_TRIED); do not retype or blindly resend - verify with dj-peek.sh, then re-send '--key Enter' only if the composer still holds the text" >&2
      exit 3
      ;;
    *)
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
  ...
  # crewmate actually working instead of the stale idle pane. DJ_SEND_SETTLE=0
  # disables it. Scoped to this path only, never the shared submit core.
  [ "${DJ_SEND_SETTLE:-1}" = 0 ] || sleep "${DJ_SEND_SETTLE:-1}"
fi
