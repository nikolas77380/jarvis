#!/usr/bin/env bash
# Execute one tracked Jarvis command in a configured remote secondmate home.
#
# Usage:
#   dj-on.sh <secondmate-id|unambiguous-ssh-alias> <dj-command> [args...]
#
# Routes come only from remote records in data/secondmates.md. A record names an
# SSH config alias, remote Jarvis code root, and remote DJ_HOME. A host alias
...
# bin/dj-*.sh namespace. No per-command table exists.
#
# argv is encoded as one NUL-delimited stream and passed through the fixed
# dj-remote-entrypoint.sh. stdin remains the caller's stdin, stdout and stderr
...
# killed. DJ_SSH_ALIVE_INTERVAL and DJ_SSH_ALIVE_COUNT_MAX override the
...
case "$COMMAND" in
  dj-*.sh) ;;
  *) die "remote command must be a basename in the dj-*.sh namespace: $COMMAND" ;;
esac
case "$COMMAND" in */*|*..*) die "remote command must not contain a path or traversal: $COMMAND" ;; esac
LOCAL_COMMAND="$DJ_ROOT/bin/$COMMAND"
[ -f "$LOCAL_COMMAND" ] && [ ! -L "$LOCAL_COMMAND" ] && [ -x "$LOCAL_COMMAND" ] \
  || die "remote command is not a genuine tracked executable in this Jarvis checkout: $COMMAND"
git -C "$DJ_ROOT" ls-files --error-unmatch "bin/$COMMAND" >/dev/null 2>&1 \
  || die "remote command is not tracked by this Jarvis checkout: $COMMAND"
...
SSH_BIN=${DJ_SSH_BIN:-ssh}
ALIVE_INTERVAL=${DJ_SSH_ALIVE_INTERVAL:-15}
ALIVE_COUNT_MAX=${DJ_SSH_ALIVE_COUNT_MAX:-3}
case "$ALIVE_INTERVAL" in ''|*[!0-9]*) die "DJ_SSH_ALIVE_INTERVAL must be a positive integer: $ALIVE_INTERVAL" ;; esac
case "$ALIVE_COUNT_MAX" in ''|*[!0-9]*) die "DJ_SSH_ALIVE_COUNT_MAX must be a positive integer: $ALIVE_COUNT_MAX" ;; esac
[ "$ALIVE_INTERVAL" -gt 0 ] || die "DJ_SSH_ALIVE_INTERVAL must be a positive integer: $ALIVE_INTERVAL"
[ "$ALIVE_COUNT_MAX" -gt 0 ] || die "DJ_SSH_ALIVE_COUNT_MAX must be a positive integer: $ALIVE_COUNT_MAX"
...
"$SSH_BIN" \
  ...
  -- "$HOST" dj-remote-entrypoint.sh "$PROTOCOL" "$ROOT_B64" "$HOME_B64" "$ARGV_B64"
