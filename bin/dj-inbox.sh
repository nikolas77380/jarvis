#!/usr/bin/env bash
# dj-inbox.sh - the captain's out-of-band capture surface.
#
# Solves three DIFFERENT problems with three different mechanisms, because they
# are not the same problem:
#
#   note    Queue an idea for jarvis while jarvis is mid-turn and cannot
#           answer. Writes a durable record and appends ONE `check` wake, so the
#           note survives a crash and is presented at jarvis's next drain.
#           This is the only subcommand that touches jarvis's wake queue.
...
#           jarvis, the backlog, or the wake queue. A side question is not
#           fleet work and must not become fleet work.
#
# Usage:
#   dj-inbox.sh note <text>...          | dj-inbox.sh note -   (body from stdin)
#   dj-inbox.sh say  [<file.wav>]       (default: audio on stdin)
#   dj-inbox.sh status
#   dj-inbox.sh ask  <question>...
#   dj-inbox.sh list
#   dj-inbox.sh drain [--ack <id>...]
#
...
#
#   config/inbox-region     DJ_INBOX_REGION     AWS region.            required
#   config/inbox-stt-model  DJ_INBOX_STT_MODEL  speech-to-text model.  required by say
#   config/inbox-ask-model  DJ_INBOX_ASK_MODEL  side-question model.   required by ask
#   config/inbox-profile    DJ_INBOX_PROFILE    AWS profile.           optional
#
# An absent profile means the call uses whatever credentials are already in the
# environment, which is also what DJ_INBOX_PROFILE= (empty) forces.
#
...
#
# Environment:
#   DJ_HOME              operational home whose state/ and data/ are used.
#
...
# in bin/dj-voice-relay.py hands real work over to jarvis, it runs this
...
# bin/dj_voice_records.py owns the scope-controlled machine view the voice agent
# reads, because the voice agent must be able to answer without record free text
# ever reaching a model.
set -euo pipefail

# A non-interactive `ssh host dj-inbox.sh ...` does NOT get a login shell, so it
...
DJ_ROOT="$(cd "$SELF_DIR/.." && pwd)"
DJ_HOME="${DJ_HOME:-$DJ_ROOT}"
...
REGION="${DJ_INBOX_REGION:-}"
STT_MODEL="${DJ_INBOX_STT_MODEL:-}"
ASK_MODEL="${DJ_INBOX_ASK_MODEL:-}"
# Unset falls through to config; explicitly empty means "use ambient credentials".
PROFILE="${DJ_INBOX_PROFILE-$(read_setting inbox-profile)}"
...
need_region() {
  [ -n "$REGION" ] || REGION=$(require_setting inbox-region DJ_INBOX_REGION "AWS region")
}

need_stt_model() {
  need_region
  [ -n "$STT_MODEL" ] || STT_MODEL=$(require_setting inbox-stt-model \
    DJ_INBOX_STT_MODEL "speech-to-text model")
}

need_ask_model() {
  need_region
  [ -n "$ASK_MODEL" ] || ASK_MODEL=$(require_setting inbox-ask-model \
    DJ_INBOX_ASK_MODEL "side-question model")
}
...
# ambient ones win. Set DJ_INBOX_PROFILE= (empty) to force that even without env
# credentials present.
aws_call() {
...
# Append exactly one wake so jarvis picks the note up at its next drain.
...
wake_for() {
  local id=$1 summary=$2 lib="$DJ_ROOT/bin/dj-wake-lib.sh"
  if [ ! -r "$lib" ]; then
    printf 'dj-inbox: note saved but NOT announced (missing %s)\n' "$lib" >&2
    return 1
  fi
  # shellcheck source=/dev/null
  DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_HOME="$DJ_HOME" STATE="$STATE" . "$lib"
  dj_wake_append check "inbox:$id" "check: captain inbox note $id - $summary"
}
...
queue_note() {
  ...
  if wake_for "$id" "$summary"; then
    printf '  jarvis will pick this up at its next check.\n'
  else
    die "note $id is saved at $INBOX/$id.note but jarvis was NOT woken"
  fi
}
...
cmd_note() {
  local body
  if [ "$#" -eq 0 ]; then
    die "usage: dj-inbox.sh note <text>...   (or: note - to read stdin)"
  elif [ "$1" = "-" ]; then
    body=$(cat)
  else
    body="$*"
...
cmd_say() {
  ...
  raw=$(mktemp /tmp/dj-inbox-audio-XXXXXX)
  wav=$(mktemp /tmp/dj-inbox-wav-XXXXXX.wav)
...
if data[:4] == b'RIFF':
    ...
    sys.stderr.write("dj-inbox: input is WAV, passing through\n")
elif data[:4] in (b'OggS', b'fLaC') or data[:3] == b'ID3':
    sys.exit("dj-inbox: got Ogg/FLAC/MP3; re-encode to WAV first")
else:
    if len(data) % 2:
        data = data[:-1]
    w = wave.open(dst, 'wb')
    ...
    sys.stderr.write("dj-inbox: input looked like raw PCM, wrapped as 16kHz mono WAV\n")
PY
...
w=wave.open('$wav'); print(round(w.getnframes()/w.getframerate(),2))")
  printf 'dj-inbox: %ss of audio, transcribing with %s in %s\n' "$secs" "$STT_MODEL" "$REGION" >&2
...
cmd_status() {
  ...
  printf '=== jarvis status (read-only, no wake sent) ===\n'
  printf 'home     %s\n' "$DJ_HOME"
  printf 'time     %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf 'inbox    %s note(s) waiting for jarvis\n' "$pending"
...
cmd_ask() {
  [ "$#" -gt 0 ] || die "usage: dj-inbox.sh ask <question>..."
  need_ask_model
  need aws
  need python3
  local q="$*" msg
  msg=$(mktemp /tmp/dj-inbox-ask-XXXXXX.json)
  # shellcheck disable=SC2064
  trap "rm -f '$msg'" EXIT
...
cmd_drain() {
  if [ "${1:-}" = "--ack" ]; then
    shift
    [ "$#" -gt 0 ] || die "usage: dj-inbox.sh drain --ack <id>..."
  ...
  cmd_list
  printf '\nAck with: dj-inbox.sh drain --ack <id>...\n'
}
