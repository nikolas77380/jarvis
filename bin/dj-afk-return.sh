#!/usr/bin/env bash
# dj-afk-return.sh - deterministic away-mode return catch-up gate.
#
# Usage:
#   dj-afk-return.sh          Stop away mode, present catch-up, and open/check gate.
#   dj-afk-return.sh begin    Same as the default command.
#   dj-afk-return.sh check    Re-present and close the gate only after blockers resolve.
#   dj-afk-return.sh guard    Read-only refusal while away or catch-up is pending.
#
# `blocked:` is the crewmate protocol's jarvis-actionable verb. A live task's
...
# read entrypoints such as dj-bearings-snapshot.sh.
set -u
...
print_blockers() {  # <file>
  ...
  while IFS="$(printf '\t')" read -r tag id key summary; do
    ...
    printf 'jarvis-actionable blocker: %s [key=%s] %s\n' "$id" "$key" "$summary"
  done < "$file"
}
...
return_guard() {
  if [ -e "$STATE/.afk" ]; then
    printf 'dj-afk-return: away mode is still active; run bin/dj-afk-return.sh before ordinary captain work\n' >&2
    return 3
  fi
  if [ -e "$GATE" ]; then
    printf 'dj-afk-return: return catch-up is pending; remediate or durably reclassify every listed blocker, then run bin/dj-afk-return.sh check\n' >&2
...
return_reconcile() {
  ...
  if [ "$lifecycle_ok" -ne 1 ] || [ -s "$blockers" ]; then
    ...
    printf 'dj-afk-return: catch-up must finish before the captain request\n' >&2
    print_evidence "$GATE" >&2
    print_blockers "$GATE" >&2
    printf 'dj-afk-return: handle each blocker now, or close it with resolved [key=...] and append a durable reclassification reason, then run bin/dj-afk-return.sh check\n' >&2
  ...
  if ! print_evidence "$evidence"; then
    ...
    write_gate "$evidence" "$blockers" || { rm -f "$evidence" "$blockers" "$drain_err"; return 1; }
    printf 'dj-afk-return: recovery evidence could not be published; catch-up remains pending\n' >&2
    rm -f "$evidence" "$blockers" "$drain_err"
    return 3
  fi
  ...
  clear_delivery_artifacts
  rm -f "$evidence" "$blockers" "$drain_err"
  printf 'dj-afk-return: catch-up clear; ordinary captain work may proceed\n'
  return 0
}
...
main() {
  ...
  # `guard` returned above without sourcing dj-wake-lib.sh, whose initialization
  ...
  write_pending_seed || { dj_lock_release "$LOCK"; trap - EXIT; return 1; }
  return_reconcile
  rc=$?
