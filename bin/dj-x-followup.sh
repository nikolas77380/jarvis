...
#
# An X-mode mention that spawned real work is linked to its task by dj-x-link.sh
...
# done, build started, shipped, failed), jarvis composes a public-safe outcome
...
#   dj-x-followup.sh --check <task-id>
...
#       dj-x-reply.sh): clears the link and skips quietly, exactly like a
...
#       of retrying forever.
#       On dj-x-reply's fail-safe refusal (exit 8: platform or explicit budget
...
#
# Dry-run (DJX_DRY_RUN) flows through dj-x-reply.sh: the follow-up is recorded to
...
#
# The window is DJX_FOLLOWUP_MAX_AGE_SECS (default 604800, 7 days). The cap is
# DJX_FOLLOWUP_MAX_COUNT (default 3). DJX_NOW_OVERRIDE pins "now" for
# deterministic tests. Meta read/write lives in dj-x-lib.sh.
set -u
...
usage() {
  echo "usage: dj-x-followup.sh --check <task-id> | --clear <task-id> | <task-id> [--image <path>] [--final] --text-file <path> | <task-id> [--image <path>] [--final] -" >&2
}

help() {
  cat <<'EOF'
usage: dj-x-followup.sh --check <task-id>
       dj-x-followup.sh --clear <task-id>
       dj-x-followup.sh <task-id> [--image <path>] [--final] --text-file <path>
       dj-x-followup.sh <task-id> [--image <path>] [--final] -
...
MAX_AGE=${DJX_FOLLOWUP_MAX_AGE_SECS:-604800}
case "$MAX_AGE" in
  ''|*[!0-9]*) MAX_AGE=604800 ;;
esac

MAX_COUNT=${DJX_FOLLOWUP_MAX_COUNT:-3}
case "$MAX_COUNT" in
  ''|*[!0-9]*) MAX_COUNT=3 ;;
esac
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --image)
        ...
        shift
        if [ "$#" -lt 1 ] || [ -z "$1" ]; then
          echo "dj-x-followup: missing --image path" >&2
          usage
          exit 2
        fi
        TS_ARGS+=("$1")
...
case "$ID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "dj-x-followup: unsafe task id: $ID" >&2; exit 2 ;;
esac

META="$STATE/$ID.meta"
if [ "$MODE" = clear ]; then
  fmx_meta_link_clear "$META" \
    || { echo "dj-x-followup: could not clear the link in state/$ID.meta" >&2; exit 1; }
  printf '%s\n' "$ID"
  exit 0
fi
...
# a post is simply a no-op success (jarvis need not special-case it).
if [ -z "$RID" ]; then
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "dj-x-followup: $ID is not X-linked; nothing to post" >&2
  exit 0
fi

NOW=${DJX_NOW_OVERRIDE:-$(date +%s)}
case "$NOW" in
  ''|*[!0-9]*) echo "dj-x-followup: could not read the current time" >&2; exit 1 ;;
esac
...
if [ "$EXPIRED" = 1 ]; then
  fmx_meta_link_clear "$META" || echo "dj-x-followup: warning: could not clear the elapsed link in state/$ID.meta" >&2
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "dj-x-followup: $REASON for $ID; skipped and cleared the link" >&2
  exit 0
fi
...
# Post the follow-up. dj-x-reply owns text reading, thread-split, dry-run, the
...
case "$REQ_PLATFORM" in
  discord|x) REPLY_ENV+=("DJX_REPLY_PLATFORM=$REQ_PLATFORM") ;;
esac
case "$REQ_REPLY_MAX" in
  ''|*[!0-9]*) ;;
  *) REPLY_ENV+=("DJX_REPLY_MAX_CHARS=$REQ_REPLY_MAX") ;;
esac
if [ "${#REPLY_ENV[@]}" -gt 0 ]; then
  env "${REPLY_ENV[@]}" "$DJ_ROOT/bin/dj-x-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null
else
  "$DJ_ROOT/bin/dj-x-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null
fi
post_rc=$?
  ...
  0)
    ...
    if [ "$FINAL" = 1 ] || [ "$NEWCOUNT" -ge "$MAX_COUNT" ]; then
      if ! fmx_meta_link_clear "$META"; then
        echo "dj-x-followup: error: posted but could not clear the link in state/$ID.meta" >&2
        exit 1
      fi
    elif ! fmx_meta_followups_set "$META" "$NEWCOUNT"; then
      if ! fmx_meta_link_clear "$META"; then
        echo "dj-x-followup: error: posted but could not record the follow-up count or clear the link in state/$ID.meta" >&2
        exit 1
      fi
      echo "dj-x-followup: warning: posted but could not record the follow-up count in state/$ID.meta; cleared the link to avoid duplicate follow-ups" >&2
    fi
    printf '%s\n' "$RID"
    exit 0
    ;;
  8)
    # dj-x-reply.sh refused this follow-up (exit 8) because it could not
    ...
    # link here.
    echo "dj-x-followup: follow-up for $ID held: reply context lacks an authoritative platform or explicit budget; left the link in place to retry once both values are recoverable" >&2
    exit 1
    ;;
  9)
    # dj-x-reply.sh distinguishes a relay rejection of this specific follow-up
    ...
    fmx_meta_link_clear "$META" || echo "dj-x-followup: warning: could not clear the rejected link in state/$ID.meta" >&2
    echo "dj-x-followup: relay rejected the follow-up for $ID (cap or window exhausted); skipped and cleared the link" >&2
    exit 0
    ;;
  *)
    # Post failed for another reason (network, auth, transport): leave the link
    # so jarvis can retry on a later pass.
    echo "dj-x-followup: follow-up post failed for $ID; left the link in place to retry" >&2
    exit 1
    ;;
esac
