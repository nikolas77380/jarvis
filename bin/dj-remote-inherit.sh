...
#
# Usage:
#   dj-remote-inherit.sh put <allowlisted-relative-path> <bytes> <sha256> <generation> < stdin
#   dj-remote-inherit.sh absent <allowlisted-relative-path> 0 <empty-sha256> <generation>
#
...
# (DJ_INHERITABLE_CONFIG in bin/dj-config-inherit-lib.sh), so this code root's
...
HOME_REAL=$(CDPATH='' cd -- "$DJ_HOME" 2>/dev/null && pwd -P) || die "DJ_HOME is unavailable"
PARENT="$HOME_REAL/$(dirname "$REL")"
# The captain accepts this config/data parent TOCTOU within Jarvis's single-user trust boundary.
...
case "$PARENT_REAL" in "$HOME_REAL/config"|"$HOME_REAL/data") ;; *) die "inherited destination escapes DJ_HOME" ;; esac
...
BASE=$(basename "$REL")
LOCK="$PARENT_REAL/.dj-inherit-$BASE.lock"
GENERATION_FILE="$PARENT_REAL/.dj-inherit-$BASE.generation"
dj_lock_acquire_wait "$LOCK" || die "cannot lock inherited destination"
TMP=
GENERATION_TMP=
cleanup() {
  [ -z "$TMP" ] || rm -f -- "$TMP"
  [ -z "$GENERATION_TMP" ] || rm -f -- "$GENERATION_TMP"
  dj_lock_release "$LOCK" || true
}
trap cleanup EXIT
