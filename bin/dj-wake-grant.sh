...
cleanup() {
  ...
  [ "$LOCK_HELD" = false ] || dj_lock_release "$DJ_WAKE_QUEUE_LOCK"
  exit "$status"
}
trap cleanup EXIT
trap 'exit 130' INT
...
case "${1:-}" in
  activate)
    ...
    identity=$(dj_pid_identity "$pid" 2>/dev/null) || exit 1
    [ -n "$identity" ] || exit 1
    TMP=$(mktemp "$STATE/.branch-eligible-owner.tmp.XXXXXX") || exit 1
    printf '%s\n%s\n%s\n%s\n' dj-branch-eligible-owner-v1 "$pid" "$identity" "$generation" > "$TMP" || exit 1
    chmod 0600 "$TMP" || exit 1
    dj_lock_acquire_wait "$DJ_WAKE_QUEUE_LOCK"
    LOCK_HELD=true
    [ "$(dj_pid_identity "$pid" 2>/dev/null || true)" = "$identity" ] || exit 1
  ...
  deactivate)
    ...
    ;;
  *)
    echo "usage: dj-wake-grant.sh activate PID GENERATION | publish GENERATION SEQUENCE... | release GENERATION | deactivate PID GENERATION" >&2
    exit 2
    ;;
esac
