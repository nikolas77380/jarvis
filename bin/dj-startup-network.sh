#!/usr/bin/env bash
# dj-startup-network.sh - the deferred network stage of a session start.
#
...
# whole DJ_SESSION_START_TIMEOUT budget and truncate the digest outright, turning
...
#
# WHAT IS PRESERVED. Nothing is dropped. bin/dj-bootstrap.sh remains the single
# owner of every one of these sweeps and still runs all of them, unchanged, via
# its DJ_BOOTSTRAP_NETWORK=only phase. Deferral changes WHEN they run, not
...
#
# Usage: dj-startup-network.sh start --locked <0|1> --harvest-pid <pid>
...
#        dj-startup-network.sh run --locked <0|1>
...
#        dj-startup-network.sh harvest --pid <pid>
#          Print the digest's NETWORK CHECKS section and release the inline-print
#          claim. Called by bin/dj-session-start.sh, not by hand.
#        dj-startup-network.sh report
...
#        dj-startup-network.sh wait [<seconds>]
...
#                             bin/dj-bootstrap.sh produced it, plus a
...
#                             bin/dj-timing-lib.sh's tab-separated format: the
...
#
# The whole stage is bounded by DJ_STARTUP_NETWORK_TIMEOUT (default 120s), one
...
# dj-timing-lib.sh owns the per-step elapsed record this stage publishes beside
# its report. Recording is opt-in per run: it stays inert until cmd_run points
# DJ_TIMING_LOG at a file, so nothing else that sources these scripts pays for it.
# shellcheck source=bin/dj-timing-lib.sh
. "$SCRIPT_DIR/dj-timing-lib.sh"
# dj-wake-lib.sh owns both the portable lock helpers used below and the durable
...
usage() {
  sed -n '2,/^set -u$/p' "$SCRIPT_DIR/dj-startup-network.sh" | sed 's/^# \{0,1\}//; $d'
}
...
stage_budget() {
  local budget=${DJ_STARTUP_NETWORK_TIMEOUT:-120}
  case "$budget" in ''|*[!0-9]*|0) budget=120 ;; esac
  printf '%s' "$budget"
}

delivery_budget() {
  local budget=${DJ_SESSION_START_TIMEOUT:-120}
  case "$budget" in ''|*[!0-9]*|0) budget=120 ;; esac
  printf '%s' "$budget"
}
...
cmd_start() {  # <locked> <harvest-pid>
  ...
  if [ "$locked" = 1 ] && ! dj_session_lock_owned_by_self "$STATE"; then
    return 1
  fi

  dj_lock_acquire_wait "$PUBLISH_LOCK"
...
EOF
  ...
  nohup "$SCRIPT_DIR/dj-startup-network.sh" run --locked "$locked" --lock-pid "$lock_pid" \
    --generation "$generation" \
...
# rewrites this value - bin/dj-lock.sh overwrites a dead holder's pid with its
...
EOF
    ...
    if [ "$claim_live" -eq 0 ]; then
      if report_requires_wake "$state"; then
        dj_wake_append check startup-network \
          "check: startup-network: deferred startup network checks finished ($state); read them with $DJ_ROOT/bin/dj-startup-network.sh report" \
          || true
      fi
      dj_lock_release "$PUBLISH_LOCK"
      return 0
...
cmd_run() {  # <locked> <lock-pid> <generation>
  ...
  elif [ "$locked" = 1 ] && ! dj_session_lock_owned_by_self "$STATE"; then
    downgraded=1
    locked=0
  fi
  if [ "$locked" = 1 ]; then
...
EOF
  ...
  out=$(mktemp "${TMPDIR:-/tmp}/dj-startup-network.XXXXXX" 2>/dev/null) || return 1
  ...
  # Sweeps run in child processes (bin/dj-bootstrap.sh, and bin/dj-fleet-sync.sh
  # below it), so DJ_TIMING_LOG is exported and appended to by all of them.
  timings=$(mktemp "${TMPDIR:-/tmp}/dj-startup-network-timings.XXXXXX" 2>/dev/null) || timings=
  [ -z "$timings" ] || dj_timing_start "$timings"
  stage_started=$(dj_timing_now_ms)
  rc=0
  if [ "$sweep_locked" -eq 1 ]; then
    dj_lock_acquire_wait "$STATE/.lock.acquire"
    lease_held=1
    if ! lock_unchanged "$lock_pid"; then
      sweep_locked=0
      phases=probe
  ...
  if [ "$sweep_locked" -eq 1 ]; then
    dj_run_timed "$budget" env DJ_BOOTSTRAP_NETWORK=only \
      DJ_BOOTSTRAP_NETWORK_LOCK_PID="$lock_pid" \
      "$SCRIPT_DIR/dj-bootstrap.sh" >"$out" 2>&1 || rc=$?
  else
    dj_run_timed "$budget" env DJ_BOOTSTRAP_NETWORK=only DJ_BOOTSTRAP_DETECT_ONLY=1 \
      "$SCRIPT_DIR/dj-bootstrap.sh" >"$out" 2>&1 || rc=$?
  fi
  [ "$lease_held" -eq 0 ] || dj_lock_release "$STATE/.lock.acquire"
  ...
  dj_timing_record stage network-checks "$stage_started" "$phases"
  ...
  case "$rc" in
    ...
    124)
      printf 'NETWORK_CHECKS: hit the %ss bound before finishing, so %s may be incomplete; rerun %s/bin/dj-startup-network.sh run --locked %s\n' \
        "$budget" "$(phase_label "$phases")" "$DJ_ROOT" "$sweep_locked" >> "$out"
      publish "$generation" timeout "$phases" "$sweep_locked" "$started" "$rc" "$out" "$timings"
      ;;
    *)
      printf 'NETWORK_CHECKS: the deferred check worker exited %s, so %s may be incomplete; rerun %s/bin/dj-startup-network.sh run --locked %s\n' \
        "$rc" "$(phase_label "$phases")" "$DJ_ROOT" "$sweep_locked" >> "$out"
...
print_finished() {  # <state>
  ...
  if [ "$report_published" = 0 ]; then
    printf 'NETWORK_CHECKS: could not publish the deferred check report, so %s results are unavailable; rerun %s/bin/dj-startup-network.sh run --locked %s\n' \
      "$(phase_label "$phases")" "$DJ_ROOT" "$(status_get locked)"
...
print_pending() {
  ...
  printf 'The durable result is readable on demand with %s/bin/dj-startup-network.sh report; until it finishes, treat none of it as confirmed.\n' "$DJ_ROOT"
}
  ...
  case "$(status_get state)" in
    ...
    running)
      if worker_alive; then
        ...
        printf 'NETWORK_CHECKS: the deferred check worker stopped before publishing, so %s did not complete; rerun %s/bin/dj-startup-network.sh run --locked %s\n' \
          "$(phase_label "$(status_get phases)")" "$DJ_ROOT" "$(status_get locked)"
      fi
      ;;
...
case "$MODE" in
  ...
  -h|--help) usage ;;
  *)
    printf 'dj-startup-network: unknown mode: %s\n' "${MODE:-<none>}" >&2
    printf 'usage: dj-startup-network.sh start|run|harvest|report|wait\n' >&2
    exit 2
    ;;
esac
exit 0
