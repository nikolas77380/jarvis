...
# where the usual "checks green" dj-pr-check.sh trigger never fires) - by looking
...
# branch (jarvis performs that merge after configured approval) as a fallback
...
# Usage: dj-teardown.sh <task-id> [--force]
...
#   1. Retries up to DJ_TREEHOUSE_RETURN_LOCK_RETRIES times (default 3), waiting
#      DJ_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS (default 1s; falls back to the older
#      DJ_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS name when the new one is unset) between
...
#      bin/dj-lock-lib.sh's dj_lock_is_provably_stale, passing the worktree dir as the
#      companion directory and DJ_STALE_WORKTREE_LOCK_AGE_SECS (default 30s) as the age
...
#     THIS task only when its branch AND code identity (bin/dj-nm-run-lib.sh's
#     dj_nm_head_matches_worktree, the same rule bin/dj-crew-state.sh uses) both
...
#     log in a pruned no-mistakes gate worktree). bin/dj-remote-job-reap-orphans.sh
...
SUB_HOME_PARENT_MARKER=".dj-secondmate-parent"
# shellcheck source=bin/dj-tasks-axi-lib.sh
. "$SCRIPT_DIR/dj-tasks-axi-lib.sh"
...
# shellcheck source=bin/dj-lock-lib.sh
. "$SCRIPT_DIR/dj-lock-lib.sh"
# shellcheck source=bin/dj-classify-lib.sh
. "$SCRIPT_DIR/dj-classify-lib.sh"
...
# shellcheck source=bin/dj-pending-reply-lib.sh
. "$SCRIPT_DIR/dj-pending-reply-lib.sh"
# shellcheck source=bin/dj-nm-run-lib.sh
. "$SCRIPT_DIR/dj-nm-run-lib.sh"
if [ "$#" -lt 1 ] || ! dj_task_id_path_safe "$1"; then
...
# task's live lease (contract: bin/dj-lease-lib.sh; no-op in homes without
# leases).
...
# territory (contract: bin/dj-lease-lib.sh).
if [ "$FORCE" = --force ] && [ "$(dj_lease_actor)" = branch ]; then
  echo "error: forced teardown refused - the supervision branch cannot discard work" >&2
  exit "$DJ_LEASE_REFUSE_EXIT"
fi
dj_lease_guard "$ID" "teardown (dj-teardown)"
...
teardown_release_locks() {
  ...
  for ((i=${#DESCENDANT_LOCK_PATHS[@]} - 1; i >= 0; i--)); do
    dj_lock_release "${DESCENDANT_LOCK_PATHS[$i]}" || true
  done
  DESCENDANT_LOCK_PATHS=()
  if [ -n "${HANDOFF_WAKE_RETIRE_LOCK:-}" ]; then
    dj_lock_release "$HANDOFF_WAKE_RETIRE_LOCK" || true
    HANDOFF_WAKE_RETIRE_LOCK=
  fi
  if [ -n "${LOCAL_HANDOFF_LOCK:-}" ]; then
    dj_lock_release "$LOCAL_HANDOFF_LOCK" || true
    LOCAL_HANDOFF_LOCK=
  fi
  if [ -n "${LOCAL_REGISTRY_LOCK:-}" ]; then
    dj_lock_release "$LOCAL_REGISTRY_LOCK" || true
    LOCAL_REGISTRY_LOCK=
  fi
...
# down a worktree (see bin/dj-gate-refuse-lib.sh).
dj_refuse_if_gate_agent
DJ_LOCK_LOG_PREFIX=teardown
...
handoff_wake_retire_validate() {
  ...
  if [ -n "$corr" ]; then
    rec=$(dj_pending_reply_path "$STATE" "$corr")
    if [ -e "$rec" ] || [ -L "$rec" ]; then
      [ -f "$rec" ] && [ ! -L "$rec" ] \
        && [ "$(dj_pending_reply_get "$rec" task_id)" = "$ID" ] || {
        echo "REFUSED: receiver wake correlation for secondmate $ID is unsafe or belongs to another task" >&2
        return 1
      }
    fi
    confirmation=$(dj_pending_reply_delivery_confirmation_path "$STATE" "$corr")
...
handoff_wake_retire() {
  ...
  if [ -n "$corr" ]; then
    ...
    dj_lock_acquire_wait "$lock" || return 1
    rec=$(dj_pending_reply_path "$STATE" "$corr")
    confirmation=$(dj_pending_reply_delivery_confirmation_path "$STATE" "$corr")
    if { [ ! -e "$rec" ] && [ ! -L "$rec" ]; } \
      || { [ -f "$rec" ] && [ ! -L "$rec" ] \
        && [ "$(dj_pending_reply_get "$rec" task_id)" = "$ID" ]; }; then
...
handoff_wake_retire_stage_restore() {
  ...
  if [ -n "$HANDOFF_WAKE_RETIRE_CORR" ]; then
    rec=$(dj_pending_reply_path "$STATE" "$HANDOFF_WAKE_RETIRE_CORR")
    confirmation=$(dj_pending_reply_delivery_confirmation_path "$STATE" "$HANDOFF_WAKE_RETIRE_CORR")
  fi
  ...
  if [ -n "$HANDOFF_WAKE_RETIRE_LOCK" ]; then
    dj_lock_release "$HANDOFF_WAKE_RETIRE_LOCK" || return 1
    HANDOFF_WAKE_RETIRE_LOCK=
  fi
  HANDOFF_WAKE_RETIRE_STAGE=
}
...
handoff_wake_retire_stage_recover() {
  ...
  if [ -n "$corr" ]; then
    ...
    dj_lock_acquire_wait "$HANDOFF_WAKE_RETIRE_LOCK" || return 1
  fi
...
handoff_wake_retire_stage() {
  ...
  if [ -n "$corr" ]; then
    HANDOFF_WAKE_RETIRE_LOCK="$STATE/.pending-reply-$corr.lock"
    dj_lock_acquire_wait "$HANDOFF_WAKE_RETIRE_LOCK" || {
      HANDOFF_WAKE_RETIRE_LOCK=
...
remote_teardown_locks_release() {
  if [ -n "$REMOTE_REPLY_LIFECYCLE_LOCK" ]; then
    dj_lock_release "$REMOTE_REPLY_LIFECYCLE_LOCK"
    REMOTE_REPLY_LIFECYCLE_LOCK=
  fi
  if [ -n "$REMOTE_HANDOFF_LOCK" ]; then
    dj_lock_release "$REMOTE_HANDOFF_LOCK"
    REMOTE_HANDOFF_LOCK=
  fi
  if [ -n "$REMOTE_REGISTRY_LOCK" ]; then
    dj_lock_release "$REMOTE_REGISTRY_LOCK"
    REMOTE_REGISTRY_LOCK=
  fi
}
...
remote_pending_replies_cleanup() {
  ...
  (
    ...
    for rec in ./*; do
      [ -e "$rec" ] || [ -L "$rec" ] || continue
      [ -f "$rec" ] && [ ! -L "$rec" ] || exit 1
      [ "$(dj_meta_get "$rec" task_id)" = "$ID" ] && rm -f -- "$rec"
    done
  )
}
...
remote_secondmate_teardown() {
  ...
  remote_host=$(dj_meta_get "$META" remote_host)
  [ -n "$remote_host" ] || return 3
  kind=$(dj_meta_get "$META" kind)
  [ "$kind" = secondmate ] || { echo "REFUSED: remote placement metadata is valid only for a secondmate" >&2; return 1; }
  remote_root=$(dj_meta_get "$META" remote_root)
  remote_home=$(dj_meta_get "$META" home)
  ...
  if [ "$FORCE" != --force ] && [ -d "$STATE/pending-replies" ]; then
    for rec in "$STATE/pending-replies"/*; do
      [ -f "$rec" ] || continue
      task_id=$(dj_meta_get "$rec" task_id)
      [ "$task_id" = "$ID" ] || continue
      phase=$(dj_meta_get "$rec" phase)
      [ "$phase" = resolved ] || {
  ...
  "$SCRIPT_DIR/dj-procevent-remote-reply.sh" retire-quiesce-locked "$ID" "$FORCE" >/dev/null 2>&1 || {
    echo "REFUSED: remote secondmate $ID still has an unhandled captured reply" >&2
    return 1
  }
  "$DJ_ROOT/bin/dj-guard.sh" || true
  if [ "$FORCE" = --force ]; then
    if out=$("$SCRIPT_DIR/dj-on.sh" "$ID" dj-remote-secondmate-control.sh retire "$ID" --force < /dev/null 2>&1); then rc=0; else rc=$?; fi
  else
    if out=$("$SCRIPT_DIR/dj-on.sh" "$ID" dj-remote-secondmate-control.sh retire "$ID" < /dev/null 2>&1); then rc=0; else rc=$?; fi
  fi
  if [ "$rc" -ne 0 ]; then
    [ -z "$out" ] || printf '%s\n' "$out" >&2
    if [ "$rc" -eq 255 ]; then
      echo "error: remote retirement completion is unknown; preserving the route and local records for same-host reconciliation" >&2
    elif ! "$SCRIPT_DIR/dj-procevent-remote-reply.sh" arm-locked "$ID" >/dev/null 2>&1; then
  ...
  "$SCRIPT_DIR/dj-procevent-remote-reply.sh" retire-finalize-locked "$ID" "$FORCE" >/dev/null 2>&1 || {
...
# complete before dj-guard, a backend command, file removal, branch deletion,
# worktree return, registry change, or process termination can run.
dj_backend_validate_task_endpoint "$META" "$ID" || exit 1
BACKEND=$DJ_BACKEND_VALIDATED_BACKEND
T=$DJ_BACKEND_VALIDATED_TARGET
WT=$(dj_meta_get "$META" worktree)
PROJ=$(dj_meta_get "$META" project)
T_ORCA=
[ "$BACKEND" != orca ] || T_ORCA=$T
if [ "${DJ_TEARDOWN_GUARD_DONE:-0}" != 1 ]; then
  "$DJ_ROOT/bin/dj-guard.sh" || true
fi
HOME_PATH=$(grep '^home=' "$META" | cut -d= -f2- || true)
PR_URL=$(grep '^pr=' "$META" | tail -1 | cut -d= -f2- || true)
# tasktmp is recorded by dj-spawn for tasks that set up a per-task temp root
# (/tmp/dj-<id>/); absent for tasks spawned before that change, so tolerate empty.
TASK_TMP=$(grep '^tasktmp=' "$META" | cut -d= -f2- || true)
BUSY_GEN=$(dj_meta_get "$META" busy_gen)
if [ -z "$BUSY_GEN" ]; then
  BUSY_GEN=$(cat "$STATE/$ID.busy-gen" 2>/dev/null || true)
fi
ORCA_WORKTREE_ID=$(dj_meta_get "$META" orca_worktree_id)
...
public_followup_resolve_primary_home() {
  ...
  dj_pf_home_id_valid "secondmate:$id" || return 1
  ...
  [ "$(dj_meta_get "$parent_meta" kind)" = secondmate ] || return 1
  meta_home=$(dj_meta_get "$parent_meta" home)
...
if [ -f "$DJ_HOME/$SUB_HOME_MARKER" ]; then
  SECOND_MATE_ID=$(sed -n '1p' "$DJ_HOME/$SUB_HOME_MARKER")
  ...
  PARENT_ROUTE_FILE="$DJ_HOME/$SUB_HOME_PARENT_MARKER"
  PARENT_ROUTE_RECORD=absent
  PARENT_ROUTE=
  PARENT_ROUTE_HOME=
  if [ -e "$PARENT_ROUTE_FILE" ] || [ -L "$PARENT_ROUTE_FILE" ]; then
    PARENT_ROUTE_RECORD=invalid
    if dj_secondmate_parent_record_parse "$PARENT_ROUTE_FILE"; then
      PARENT_ROUTE=$DJ_SECONDMATE_PARENT_ROUTE
      PARENT_ROUTE_HOME=$DJ_SECONDMATE_PARENT_HOME
  ...
  if [ "$PARENT_ROUTE_RECORD" = invalid ]; then
    ...
    # construction (bin/dj-public-followup-emit.sh header): a parent recorded
    ...
    # dj_pf_relay_active's environment-wins rule would.
    if [ -f "$DJ_HOME/.env" ]; then
      HOME_ENV_TOKEN=$(fmx_env_get DJX_PAIRING_TOKEN "$DJ_HOME/.env")
    ...
    PUBLIC_FOLLOWUP_PARENT_UNRESOLVED=1
    PRIMARY_HOME_CANDIDATE=${DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME:-$PARENT_ROUTE_HOME}
    PARENT_BINDINGS_MATCH=1
    if [ -n "${DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME:-}" ]; then
      LIVE_PARENT_HOME=$(public_followup_canonical_home \
        "$DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME") || PARENT_BINDINGS_MATCH=0
    ...
    PRIMARY_HOME_CANDIDATE=${DJ_PUBLIC_FOLLOWUP_PRIMARY_HOME:-}
    if [ -n "$PRIMARY_HOME_CANDIDATE" ]; then
      if dj_pf_relay_active "$PRIMARY_HOME_CANDIDATE"; then
    ...
    if [ "$PUBLIC_FOLLOWUP_PARENT_RELAY_ACTIVE" = 1 ]; then
      PUBLIC_FOLLOWUP_PARENT_UNRESOLVED=1
      if dj_pf_home_id_valid "secondmate:$SECOND_MATE_ID"; then
        PUBLIC_FOLLOWUP_WORK_HOME="secondmate:$SECOND_MATE_ID"
        if PUBLIC_FOLLOWUP_HOME=$(public_followup_resolve_primary_home \
            "$PRIMARY_HOME_CANDIDATE" "$DJ_HOME" "$SECOND_MATE_ID"); then
          PUBLIC_FOLLOWUP_STATE="$PUBLIC_FOLLOWUP_HOME/state"
  ...
  if [ "$FORCE" != "--force" ] && dj_pf_relay_active "$DJ_HOME"; then
    PUBLIC_FOLLOWUP_RELAY_ACTIVE=1
  fi
elif [ "$FORCE" != "--force" ] && dj_pf_relay_active "$DJ_HOME"; then
  PUBLIC_FOLLOWUP_RELAY_ACTIVE=1
fi
...
# Where a harness's jarvis-owned global turn-end registry entry lives is
# owned by bin/dj-control-lib.sh, so teardown and the control plane's relaunch
...
remove_grok_turnend_auth() {
  ...
  token_path=$(dj_control_harness_turnend_token_path grok "$state_dir" "$id") || return 1
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  path=$(dj_control_harness_turnend_auth_path grok "$token") || return 1
  [ -n "$path" ] || return 0
  rm -f -- "$path"
}
...
remove_kimi_turnend_auth() {
  local state_dir=$1 id=$2 token_path token='' path
  token_path=$(dj_control_harness_turnend_token_path kimi "$state_dir" "$id") || return 1
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  path=$(dj_control_harness_turnend_auth_path kimi "$token") || return 1
  [ -n "$path" ] || return 0
  rm -f -- "$path"
}
...
retire_busy_state() {
  ...
  if [ -n "$gen" ]; then
    "$SCRIPT_DIR/dj-busy-event.sh" retire "$state_dir" "$id" --gen "$gen"
  elif [ -f "$state_dir/$id.busy-gen" ]; then
    "$SCRIPT_DIR/dj-busy-event.sh" retire "$state_dir" "$id" --current-gen
  fi
}

validate_pr_poll_cleanup() {
  local state_dir=$1 id=$2 quarantine state_device artifact has_artifact=0
  dj_task_id_path_safe "$id" || return 0
  ...
  state_device=$(dj_pr_file_device "$state_dir") || return 1
  for artifact in "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.check-trust"; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    if [ ! -f "$artifact" ] || [ -L "$artifact" ] \
      || [ "$(dj_pr_file_device "$artifact")" != "$state_device" ] \
      || [ "$(dj_pr_file_link_count "$artifact")" != 1 ]; then
  ...
  if [ -e "$state_dir/$id.pr-poll-retirement" ] \
    ...
    dj_pr_poll_retirement_state_valid "$state_dir" "$id" || {
  ...
  if [ "$(dj_pr_file_device "$quarantine")" != "$state_device" ] \
    || [ "$(dj_pr_file_mode "$quarantine")" != 700 ]; then
    echo "REFUSED: PR-check quarantine is not on the task state device; preserving task state." >&2
    return 1
  fi
  for artifact in "$quarantine/$id."*; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    if ! dj_pr_private_file_valid "$artifact" 600 "$state_device"; then
...
remove_pr_poll_artifacts() {
  ...
  dj_pr_poll_retirement_recover_one "$state_dir" "$id" "$SCRIPT_DIR/dj-pr-poll.sh" || return 1
  dj_pr_poll_merge_notified_remove "$state_dir" "$id" || return 1
  rm -f "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.check-trust" || return 1
  if dj_task_id_path_safe "$id"; then
    quarantine="$state_dir/.pr-check-quarantine"
...
backlog_refresh_reminder() {
  ...
  if dj_tasks_axi_backend_available "$CONFIG"; then
    case "$KIND" in
      scout)
...
STALE_WORKTREE_LOCK_AGE_SECS=${DJ_STALE_WORKTREE_LOCK_AGE_SECS:-30}
# Bounded patience window for transient index.lock after killing a crew process.
# New knobs are preferred; DJ_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS remains an alias
# for the per-attempt wait so existing tests and operators keep working.
TREEHOUSE_RETURN_LOCK_RETRIES=${DJ_TREEHOUSE_RETURN_LOCK_RETRIES:-3}
TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=${DJ_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS:-${DJ_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS:-1}}
...
# is owned by bin/dj-lock-lib.sh's dj_lock_is_provably_stale, sourced above.
...
cleanup_stale_lock_for_safety_check() {
  ...
  if dj_lock_is_provably_stale "$lock" "$dir" "$STALE_WORKTREE_LOCK_AGE_SECS"; then
    rm -f "$lock"
...
validate_worktree_teardown_safety() {
  ...
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE '^\?\? (\.claude/|\.dj-(grok|kimi)-turnend$)' | head -1 || true)
  ...
  if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
    ...
    if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
      ...
      echo "Merge the branch into local $DEFAULT first (bin/dj-merge-local.sh after the captain approves), or push to a fork/remote, or get the captain's explicit OK to discard, then --force." >&2
      return 1
    fi
  elif [ -n "$dirty" ]; then
...
NM_TEARDOWN_TIMEOUT=${DJ_TEARDOWN_NM_TIMEOUT:-10}
...
task_status_is_own_parked_run() {  # <worktree> <axi-status-output>
  ...
  [ -n "$branch" ] || return 1
  [ -n "$out" ] || return 1
  run_id=$(dj_nm_strip_quotes "$(dj_nm_field "$out" id)")
  [ -n "$run_id" ] || return 1
  run_branch=$(dj_nm_strip_quotes "$(dj_nm_field "$out" branch)")
  [ -n "$run_branch" ] && [ "$run_branch" = "$branch" ] || return 1
  run_head=$(dj_nm_strip_quotes "$(dj_nm_field "$out" head)")
  dj_nm_head_matches_worktree "$wt" "$run_head" || return 1
  outcome=$(dj_nm_strip_quotes "$(dj_nm_field "$out" outcome)")
  [ -z "$outcome" ] || return 1
  status=$(dj_nm_strip_quotes "$(dj_nm_field "$out" status)")
...
task_run_is_own_parked_run() {  # <worktree>
  ...
  out=$(dj_nm_run "$wt" "$NM_TEARDOWN_TIMEOUT" axi status)
...
# worktree (scouts and secondmates never do, mirroring bin/dj-crew-state.sh);
...
conclude_task_no_mistakes_run() {  # <worktree>
  ...
  dj_nm_run_checked "$wt" "$NM_TEARDOWN_TIMEOUT" axi abort --run "$run_id" >/dev/null 2>&1 || true
  if out=$(dj_nm_run_bounded "$wt" "$NM_TEARDOWN_TIMEOUT" axi status --run "$run_id" 2>&1); then
...
task_process_identity() {  # <pid>
  ...
  local -a stat_fields
  proc_root=${DJ_PROC_ROOT_OVERRIDE:-/proc}
...
require_orca_worktree_path_match() {
  ...
  resolved=$(dj_backend_worktree_path orca "$worktree_id") || {
...
jarvis_home_has_treehouse_slot() {
  local home=$1
  worktree_registered_for_project "$DJ_ROOT" "$home"
}
...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$DJ_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$DJ_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active jarvis home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the jarvis repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active jarvis home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the jarvis repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active jarvis home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the jarvis repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_jarvis_operational_dirs_for_removal() {
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$DJ_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active jarvis home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$DJ_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the jarvis repo" >&2
    return 1
  fi
...
validate_jarvis_home_for_removal() {
  ...
  validate_jarvis_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=
...
remove_jarvis_home() {
  ...
  [ -n "$home" ] || return 0
  [ -e "$home" ] || return 0
  abs_home_path=$(validate_jarvis_home_for_removal "$home" "$label" "$expected_id") || return 1
  [ -n "$abs_home_path" ] || return 0
  process_event_backup=$(snapshot_jarvis_home_process_events "$abs_home_path" "$label") || return 1
  if ! cleanup_jarvis_home_process_events "$abs_home_path" "$label"; then
    restore_jarvis_home_process_events "$abs_home_path" "$label" "$process_event_backup" || return $?
    return 1
  fi
  if jarvis_home_has_treehouse_slot "$abs_home_path"; then
    command -v treehouse >/dev/null 2>&1 || {
      echo "error: treehouse command not found; cannot return $label $abs_home_path" >&2
      restore_jarvis_home_process_events "$abs_home_path" "$label" "$process_event_backup" || return $?
      return 1
    }
    teardown_treehouse_return "$abs_home_path" "$DJ_ROOT" "$label" || {
...
jarvis_home_has_process_events() {
  ...
  claim_root=${DJ_PROCEVENT_CLAIM_ROOT:-${XDG_STATE_HOME:-$HOME/.local/state}/jarvis/procevent-claims}
...
snapshot_jarvis_home_process_events() {
  local home=$1 label=$2 backup path
  if ! jarvis_home_has_process_events "$home"; then
    printf '\n'
    return 0
  fi
  backup=$(umask 077; mktemp -d "${home%/*}/.dj-procevent-restore.XXXXXX") || {
...
restore_jarvis_home_process_events() {
  ...
  runner="$home/bin/dj-procevent.sh"
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    runner="$SCRIPT_DIR/dj-procevent.sh"
  fi
  if ! DJ_HOME="$home" DJ_ROOT_OVERRIDE="$DJ_ROOT" "$runner" reconcile >/dev/null; then
...
cleanup_jarvis_home_process_events() {
  local home=$1 label=$2 runner="$1/bin/dj-procevent.sh"
  jarvis_home_has_process_events "$home" || return 0
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    echo "REFUSED: $label $home has process-event state but no sweep-capable bin/dj-procevent.sh; restore the home script and rerun teardown" >&2
    return 1
  fi
  if ! DJ_HOME="$home" DJ_ROOT_OVERRIDE="$home" "$runner" sweep-home; then
...
preflight_jarvis_home_process_events() {
  local home=$1 label=$2 runner="$1/bin/dj-procevent.sh"
  jarvis_home_has_process_events "$home" || return 0
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    echo "REFUSED: $label $home has process-event state but no sweep-capable bin/dj-procevent.sh; restore the home script and rerun teardown" >&2
  ...
  if ! DJ_HOME="$home" DJ_ROOT_OVERRIDE="$home" "$runner" sweep-home --preflight >/dev/null; then
...
preflight_jarvis_home_process_event_tree() {
  ...
  if [ -d "$sub_state" ]; then
    for child_meta in "$sub_state"/*.meta; do
      ...
      preflight_jarvis_home_process_event_tree "$child_home" "child jarvis home for $child_id" || return 1
    done
  fi
  preflight_jarvis_home_process_events "$home" "$label"
}
...
collect_descendant_task_locks() {
  ...
  # without ever having been lifecycle-locked (bin/dj-wake-lib.sh's
  # dj_task_set_lock_path owns why). Taken per home, parent before child, and
  # held until this teardown exits.
  task_set_lock=$(dj_task_set_lock_path "$sub_state") || {
    echo "REFUSED: secondmate home $home has an invalid task-set lock path; forced teardown changed nothing" >&2
    return 1
  }
  if ! dj_lock_try_acquire "$task_set_lock"; then
...
preflight_descendant_task_locks() {
  ...
  # back for a parent lock. bin/dj-spawn.sh takes the same task-set lock before
  ...
  for ((i=0; i < ${#DESCENDANT_TASK_IDS[@]}; i++)); do
    ...
    if ! dj_lock_try_acquire "$meta_lock"; then
...
validate_jarvis_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    child_id=$(basename "$child_meta" .meta)
    dj_backend_validate_task_endpoint "$child_meta" "$child_id" || return 1
    validate_pr_poll_cleanup "$sub_state" "$child_id" || return 1
    child_wt=$(meta_value "$child_meta" worktree)
    child_kind=$(meta_value "$child_meta" kind)
    [ -n "$child_kind" ] || child_kind=ship
    child_backend=$(dj_backend_of_meta "$child_meta")
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      validate_jarvis_home_for_removal "$child_home" "child jarvis home" "$child_id" >/dev/null || return 1
      validate_jarvis_home_children_removal "$child_home" || return 1
...
teardown_herdr_require_prerequisites() {  # <task-id>
  ...
  if ! dj_backend_source herdr; then
  ...
  for prerequisite in \
    dj_backend_herdr_parse_target \
    dj_backend_herdr_pane_presence_state \
    dj_backend_herdr_workspace_presence_state \
    dj_backend_herdr_endpoint_confirmed_gone \
    dj_backend_herdr_explicit_close_pane_confirmed \
    dj_backend_herdr_presentation_session_lock_path; do
  ...
  if ! declare -F dj_lock_try_acquire >/dev/null 2>&1 \
    || ! declare -F dj_lock_release >/dev/null 2>&1; then
...
teardown_herdr_preflight_target() {  # <target> <task-id>
  ...
  if ! dj_backend_herdr_parse_target "$target"; then
  ...
  pane=$DJ_BACKEND_HERDR_PANE
  presence=$(dj_backend_herdr_pane_presence_state "$session" "$pane")
  case "$presence" in
    dead|present) ;;
    *)
...
FMEOF
  ...
  while [ "$attempt" -lt 50 ]; do
    if dj_lock_try_acquire "$lock_path"; then
      if ! verified_lock_path=$(dj_backend_herdr_presentation_session_lock_path "$session") \
...
preflight_jarvis_home_herdr_children() {  # <home>
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    child_id=$(basename "$child_meta" .meta)
    dj_backend_validate_task_endpoint "$child_meta" "$child_id" || return 1
    child_backend=$DJ_BACKEND_VALIDATED_BACKEND
    child_target=$DJ_BACKEND_VALIDATED_TARGET
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      preflight_jarvis_home_herdr_children "$child_home" || return 1
    fi
  done
}

cleanup_jarvis_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_backend" = orca ]; then
      ...
      child_t=$(dj_backend_target_of_meta "$child_meta")
    fi
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = herdr ]; then
        dj_backend_herdr_parse_target "$child_t" || return 1
        if ! teardown_herdr_session_lock_held "$DJ_BACKEND_HERDR_SESSION"; then
        ...
        dj_backend_herdr_kill_serialized "$DJ_BACKEND_HERDR_SESSION" "$DJ_BACKEND_HERDR_PANE" 2>/dev/null || true
        if ! dj_backend_herdr_endpoint_confirmed_gone "$child_t"; then
        ...
        ( unset DJ_ROOT_OVERRIDE; DJ_HOME=$home DJ_ROOT=$home dj_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "dj-$child_id" ) 2>/dev/null || true
      else
        dj_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "dj-$child_id" 2>/dev/null || true
      fi
    fi
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_jarvis_home_children "$child_home" || return $?
        remove_jarvis_home "$child_home" "child jarvis home" "$child_id" || return $?
      fi
    elif [ "$child_backend" = orca ]; then
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/dj-turn-end.js" \
          "$child_wt/.dj-grok-turnend" "$child_wt/.dj-kimi-turnend"
      fi
      dj_backend_remove_worktree "$child_backend" "$child_orca_worktree_id" || return 1
    elif [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
      validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
      rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/dj-turn-end.js" \
        "$child_wt/.opencode/plugins/dj-busy-state.js" \
        "$child_wt/.dj-grok-turnend" "$child_wt/.dj-kimi-turnend"
...
remove_secondmate_registry_entry() {
  ...
  [ "$acquired" -eq 0 ] || dj_lock_release "$lock"
  return "$rc"
}
...
if [ "$KIND" = secondmate ]; then
  ...
  dj_lock_acquire_wait "$LOCAL_REGISTRY_LOCK" || exit 1
  LOCAL_HANDOFF_LOCK="$STATE/.backlog-handoff-$ID.lock"
  dj_lock_acquire_wait "$LOCAL_HANDOFF_LOCK" || exit 1
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  handoff_wake_retire_stage_recover "$HOME_PATH" || exit 1
  handoff_wake_retire_validate || exit 1
  validate_jarvis_home_for_removal "$HOME_PATH" "secondmate home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_jarvis_home_children_removal "$HOME_PATH" || exit 1
    preflight_descendant_task_locks "$HOME_PATH" || exit 1
    validate_jarvis_home_children_removal "$HOME_PATH" || exit 1
    if [ "$BACKEND" = herdr ]; then
      teardown_herdr_preflight_target "$T" "$ID" || exit 1
    fi
    preflight_jarvis_home_herdr_children "$HOME_PATH" || exit 1
  fi
fi
...
if [ "$KIND" = secondmate ]; then
  preflight_jarvis_home_process_event_tree "$HOME_PATH" "secondmate home" || exit 1
fi

if [ "$KIND" = secondmate ] && [ "$FORCE" = "--force" ]; then
  cleanup_jarvis_home_children "$HOME_PATH" || exit $?
fi
...
if [ "$KIND" = scout ] && [ "$FORCE" != "--force" ]; then
  ...
  if ! DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" DJ_DATA_OVERRIDE="$DATA" \
      DJ_CONFIG_OVERRIDE="$CONFIG" "$SCRIPT_DIR/dj-captain-hold.sh" verify "$ID" >/dev/null; then
    echo "REFUSED: scout task $ID has not passed the captain-call completion gate." >&2
    echo "Inventory its report and any visual review through bin/dj-captain-hold.sh before teardown." >&2
    exit 1
  fi
fi
...
# work. Both gates live in bin/dj-public-followup-lib.sh, so a home that never
# opted into the myjarvis relay runs one [ -f ] test and nothing else here.
...
if [ "$FORCE" != "--force" ] \
  ...
  && [ "$PUBLIC_FOLLOWUP_RELAY_ACTIVE" = 1 ] \
  && dj_pf_has_registrations "$PUBLIC_FOLLOWUP_STATE"; then
  if ! PUBLIC_FOLLOWUP_BLOCKING=$(DJ_HOME="$PUBLIC_FOLLOWUP_HOME" DJ_STATE_OVERRIDE="$PUBLIC_FOLLOWUP_STATE" \
      "$SCRIPT_DIR/dj-public-followup.sh" guard-work "$PUBLIC_FOLLOWUP_WORK_HOME" "$ID" 2>/dev/null); then
    echo "REFUSED: task $ID still owes a public reply through the myjarvis relay." >&2
    printf '%s\n' "$PUBLIC_FOLLOWUP_BLOCKING" >&2
    echo "Deliver it with bin/dj-public-followup.sh deliver <obligation-id>, waive it with tasks-axi public-followup waive, or use --force after explicit discard approval." >&2
    exit 1
  fi
fi
...
if [ "$KIND" = ship ] && [ -n "$PR_URL" ] \
    ...
    && dj_pf_has_delivered_open_loops "$PUBLIC_FOLLOWUP_STATE"; then
  echo "warning: an open public loop with nothing owed is still recorded in the consent-holding home while cleaning up ship task $ID. Hand it on with bin/dj-public-followup.sh rechain or close it with retire --reason." >&2
fi
...
# dedicated process-event and jarvis-home removal machinery further below,
...
"$SCRIPT_DIR/dj-remote-job-reap-orphans.sh" >&2 || true
...
if [ "$BACKEND" = herdr ]; then
  teardown_herdr_preflight_target "$T" "$ID" || exit 1
  dj_backend_herdr_parse_target "$T" || exit 1
  TEARDOWN_HERDR_SESSION=$DJ_BACKEND_HERDR_SESSION
  TEARDOWN_HERDR_PANE=$DJ_BACKEND_HERDR_PANE
fi
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/dj-turn-end.js" \
      "$WT/.opencode/plugins/dj-busy-state.js" \
      "$WT/.dj-grok-turnend" "$WT/.dj-kimi-turnend"
  fi
  [ -z "$T_ORCA" ] || dj_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "dj-$ID" 2>/dev/null || true
  dj_backend_remove_worktree "$BACKEND" "$ORCA_WORKTREE_ID"
...
if [ "$BACKEND" = herdr ] \
  ...
  if [ -n "$HERDR_PRESENTATION_SESSION" ] \
     ...
     && dj_backend_herdr_projection_endpoint_matches_journal \
...
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  ...
  if teardown_herdr_session_lock_held "$HERDR_PRESENTATION_SESSION"; then
    ...
    dj_backend_herdr_projection_close_pane_focus_preserving \
  ...
  if teardown_herdr_session_lock_held "$TEARDOWN_HERDR_SESSION"; then
    dj_backend_herdr_kill_serialized "$TEARDOWN_HERDR_SESSION" "$TEARDOWN_HERDR_PANE" 2>/dev/null || true
  else
    echo "warning: herdr session presentation lock path is unavailable; skipping the pane close rather than closing unlocked" >&2
  fi
elif [ "$BACKEND" != orca ]; then
  dj_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "dj-$ID" 2>/dev/null || true
fi
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  if [ "$(dj_backend_herdr_pane_agent_state "$HERDR_PRESENTATION_SESSION" "$HERDR_PRESENTATION_PANE")" = dead ]; then
...
if [ "$BACKEND" = herdr ]; then
  dj_backend_source herdr || true
  if ! declare -F dj_backend_herdr_endpoint_confirmed_gone >/dev/null 2>&1; then
    echo "error: herdr endpoint confirmation is unavailable for $ID; retaining every durable task record" >&2
    exit 1
  fi
  if ! dj_backend_herdr_endpoint_confirmed_gone "$T"; then
...
if [ "$KIND" = secondmate ]; then
  ...
  if remove_jarvis_home "$HOME_PATH" "secondmate home" "$ID"; then
    :
  else
    rc=$?
    handoff_wake_retire_stage_restore \
...
dj_backend_clear_transition "$BACKEND" "$STATE" "$T" || true
# Remove the per-task temp root (/tmp/dj-<id>/, incl. its gotmp/) recorded by spawn.
...
# The steering inbox (bin/dj-task-inbox-lib.sh) is runtime state for the
...
if [ "$KIND" != scout ] && [ "$KIND" != secondmate ] && [ "$MODE" != local-only ]; then
  "$DJ_ROOT/bin/dj-fleet-sync.sh" "$PROJ" || true
fi
