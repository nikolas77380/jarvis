...
DJ_PUSH_TRANSITION_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# shellcheck source=bin/dj-wake-lib.sh
. "$DJ_PUSH_TRANSITION_LIB_DIR/dj-wake-lib.sh"
# shellcheck source=bin/dj-classify-lib.sh
. "$DJ_PUSH_TRANSITION_LIB_DIR/dj-classify-lib.sh"
# shellcheck source=bin/dj-backend.sh
. "$DJ_PUSH_TRANSITION_LIB_DIR/dj-backend.sh"
# shellcheck source=bin/dj-transition-lib.sh
. "$DJ_PUSH_TRANSITION_LIB_DIR/dj-transition-lib.sh"

TRIAGE_LOG="$STATE/.watch-triage.log"
TRIAGE_LOG_MAX_BYTES=${DJ_WATCH_TRIAGE_LOG_MAX_BYTES:-262144}
DJ_WAKE_POST_OUTPUT_ACTION=
...
DJ_WATCH_DELIVERED_REASON=
DJ_WATCH_DELIVERY_PID=
DJ_WATCH_DELIVERY_IDENTITY=
WATCH_DELIVERY_LOG="$STATE/.watch-deliveries.log"
WATCH_DELIVERY_LOCK="$STATE/.watch-deliveries.lock"
WATCH_DELIVERY_MAX_BYTES=${DJ_WATCH_DELIVERY_MAX_BYTES:-65536}
WATCH_DELIVERY_KEEP_LINES=${DJ_WATCH_DELIVERY_KEEP_LINES:-64}
...
watch_delivery_publish() {
  ...
  [ -n "$DJ_WATCH_DELIVERY_PID" ] || return 0
  [ -n "$DJ_WATCH_DELIVERY_IDENTITY" ] || return 0
  i=0
  ...
  printf '%s\t%s\t%s\n' \
    "$DJ_WATCH_DELIVERY_PID" \
    "$(watch_delivery_clean_identity "$DJ_WATCH_DELIVERY_IDENTITY")" \
  ...
  case "$size" in
    ...
    *)
      if [ "$size" -ge "$WATCH_DELIVERY_MAX_BYTES" ]; then
        tmp="$WATCH_DELIVERY_LOG.tmp.$DJ_WATCH_DELIVERY_PID"
        raw="$tmp.raw"
...
wake() {
  ...
  trap '' HUP INT TERM
  [ -z "$DJ_WAKE_POST_OUTPUT_ACTION" ] || trap '' PIPE
  if echo "$1"; then
    output_status=0
    watch_delivery_publish "$1" || true
    # shellcheck disable=SC2034 # Read by bin/dj-watch.sh's EXIT cleanup.
    DJ_WATCH_DELIVERED_REASON=$1
  else
    output_status=1
  fi
  if [ -n "$DJ_WAKE_POST_OUTPUT_ACTION" ]; then
    "$DJ_WAKE_POST_OUTPUT_ACTION" "$output_status" || true
  fi
...
handle_push_transition() {  # <backend> <session> <record>
  ...
  if status_is_paused_or_captain_held "$(last_status_line "$STATE/$task.status")"; then
    ...
    dj_backend_commit_transition "$backend" "$STATE" "$session" "$record" || exit 1
    return
  fi
  reason="stale: $window (herdr: agent $to - waiting on human, escalated immediately, not via wedge timer)"
  dj_wake_append stale "$window" "$reason" || exit 1
  dj_backend_commit_transition "$backend" "$STATE" "$session" "$record" || exit 1
