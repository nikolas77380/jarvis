...
# via a non-empty DJX_PAIRING_TOKEN (from the home's .env or the environment).
...
# Its contract is "output => wake jarvis, silence => keep sleeping", so the
...
# relay includes is preserved for djx-respond to handle with continuity; the
...
#
# Config (home .env, DJX_ENV_FILE, or env): DJX_PAIRING_TOKEN (required),
# DJX_RELAY_URL (default https://myjarvis.io). Auth: Authorization: Bearer
# <token>.
set -u
...
# Also brings in bin/dj-x-lib.sh, which this script's relay client uses.
...
# of one per cycle; bin/dj-public-followup.sh consume clears it.
if dj_pf_has_events "$STATE"; then
  PF_ROOT=$(dj_pf_root "$STATE")
  PF_SIG=$(dj_pf_events_signature "$STATE" 2>/dev/null) || PF_SIG=
  if [ -n "$PF_SIG" ] \
    && [ "$(cat "$PF_ROOT/$DJ_PF_SURFACED_BASENAME" 2>/dev/null || true)" != "$PF_SIG" ]; then
    if printf '%s\n' "$PF_SIG" \
      | fmx_private_artifact_publish_stdin "$PF_ROOT" "$DJ_PF_SURFACED_BASENAME" 600 2>/dev/null; then
...
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/dj-x-poll.XXXXXX") || exit 0
AUTH_HEADER_FILE=
...
code=$(curl -m 5 -s -o "$BODY_FILE" -w '%{http_code}' \
  ...
  "$DJX_RELAY/connector/poll" 2>/dev/null) || exit 0
...
# Semantic worthiness is decided by djx-respond, so acknowledgments can still be
...
# The offer marker outlives the inbox file, which djx-respond removes after a
