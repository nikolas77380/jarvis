...
# It is a thin transport around bin/dj-sessionstart-run.sh, which remains the
...
#
# Usage: dj-sessionstart-cursor.sh --source <source>
...
DIGEST=$("$SCRIPT_DIR/dj-sessionstart-run.sh" --source "$SOURCE" </dev/null 2>/dev/null || true)
[ -n "$DIGEST" ] || exit 0
