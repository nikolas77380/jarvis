...
#     exact dj-turnend-guard.sh scope. Child crew/scout worktrees stay inert.
...
#     the hook delegates guarded recovery to bin/dj-lock.sh and then re-verifies
...
#     forever (dj_autoarm_claim_abandoned in bin/dj-wake-lib.sh).
#   - Foreground arm: the owner runs bin/dj-watch-arm.sh in the FOREGROUND of
...
# outcome so the synchronous Stop guard (bin/dj-turnend-guard.sh --claude) can
...
AUTOARM_ATTEMPTS=${DJ_CLAUDE_AUTOARM_ATTEMPTS:-2}
...
dj_hook_payload_is_foreign_host "$PAYLOAD" && exit 0
...
need_supervision() {
  dj_supervision_needed "$STATE" "$GRACE"
}
need_supervision || exit 0

# --- stale session-lock recovery ---------------------------------------------
# Delegate the claim to dj-lock.sh so its live-owner refusal and write semantics
...
# (dj_autoarm_claim_abandoned in bin/dj-wake-lib.sh owns that proof and its
...
if ! dj_lock_try_acquire "$OWNER_LOCK"; then
  dj_autoarm_release_abandoned "$STATE" || exit 0
  dj_lock_try_acquire "$OWNER_LOCK" || exit 0
fi
...
dj_autoarm_claim_record_identity "$STATE" || true
if ! dj_lock_set_role "$OWNER_LOCK" autoarm; then
  dj_lock_release "$OWNER_LOCK"
  exit 0
fi
trap 'dj_lock_release "$OWNER_LOCK"' EXIT
...
# 30s cadence (dj-bootstrap.sh x_mode_setup contract).
...
while [ "$attempt" -lt "$AUTOARM_ATTEMPTS" ]; do
  ...
  if [ -n "$OUT" ]; then
    "$SCRIPT_DIR/dj-watch-arm.sh" >"$OUT" 2>&1 || true
  else
    "$SCRIPT_DIR/dj-watch-arm.sh" >/dev/null 2>&1 || true
  fi
  ...
  if dj_watcher_healthy "$STATE" "$SCRIPT_DIR/dj-watch.sh" "$GRACE" "$DJ_HOME"; then
    HEALTHY=1
    break
  fi
...
if [ "$HEALTHY" -eq 1 ]; then
  if dj_failure_episode_reset "$STATE"; then
    write_epoch clean
    [ -z "$OUT" ] || rm -f "$OUT" 2>/dev/null || true
    exit 0
  fi
...
if [ "$ACTIONABLE" -eq 1 ]; then
  write_epoch rewake
  {
    printf 'jarvis watcher wake - one supervision event needs a handling turn now.\n'
    [ -n "$OUT" ] && grep -E '^(signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Run bin/dj-wake-drain.sh first, handle the wake, then run its exact WAKE_ACK_REQUIRED --ack-through command. Until that post-handling acknowledgement, interruption leaves the wake durable for idempotent re-handling. This Stop hook owns watcher continuity: when the handling turn ends, the next needed cycle arms automatically - do NOT run bin/dj-watch-arm.sh after an ordinary wake.\n'
  } >&2
  [ -z "$OUT" ] || rm -f "$OUT" 2>/dev/null || true
  exit 2
fi
...
if [ ! -e "$FAILURE_NOTICE" ]; then
  write_epoch failed
  {
    printf 'jarvis watcher auto-arm FAILED - the Stop-owned automatic supervision mechanism is broken after %s bounded attempts, and no live watcher with a fresh beacon was verified.\n' "$attempt"
