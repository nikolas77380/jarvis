...
#
# Usage:
#   dj-backlog-receive.sh state/handoff/<secondmate-id>.outbox.md <bytes> <sha256> <generation>
#
# The delivered file must be a non-symlink backlog-format scratch file confined
# to DJ_HOME/state/handoff. Every item must be Queued. Keys already present in
...
[ -f "$DJ_HOME/.dj-secondmate-home" ] && [ ! -L "$DJ_HOME/.dj-secondmate-home" ] \
  || die "DJ_HOME is not a seeded secondmate home"
[ -f "$DJ_HOME/AGENTS.md" ] && [ -d "$DJ_HOME/bin" ] || die "DJ_HOME is not a Jarvis home"
HOME_REAL=$(CDPATH='' cd -- "$DJ_HOME" 2>/dev/null && pwd -P) || die "DJ_HOME cannot be resolved"
PARENT=$(dirname "$DJ_HOME/$REL")
...
dj_lock_acquire_wait "$TRANSFER_LOCK" || die "cannot lock delivered outbox"
trap 'dj_lock_release "$TRANSFER_LOCK" || true' EXIT
...
if [ "${#TO_MOVE[@]}" -gt 0 ]; then
  dj_tasks_axi_compatible || die "a compatible tasks-axi is required for atomic backlog receipt; run bin/dj-bootstrap.sh for the required version"
...
dj_lock_release "$TRANSFER_LOCK" || die "receipt succeeded but transfer lock cleanup failed"
trap - EXIT
