#!/usr/bin/env bash
# dj-captain-hold.sh - deterministic mechanics for tasks held for the captain.
#
...
# `<origin>-decision-<key>` identities through bin/dj-decision-hold.sh; those
...
# All backlog mutations run in the active DJ_HOME, which keeps main-home and
# secondmate-home ownership aligned with the work that discovered the call.
#
# Usage:
#   dj-captain-hold.sh hold <task-id> --reason <reason> \
#     [--title <title>] [--repo <repo>] [--origin <origin-id>] [--until YYYY-MM-DD]
#   dj-captain-hold.sh answer <task-id> --decision-file <path> [--release]
#   dj-captain-hold.sh answers [<legacy-origin> | --any-origin] --source <provenance>   (keyed answers on stdin)
#   dj-captain-hold.sh bind <source-id> [<legacy-origin> | --any-origin]
#   dj-captain-hold.sh unbind <source-id>
#   dj-captain-hold.sh binding <source-id>
#   dj-captain-hold.sh complete <origin-id> (--none | <task-id>...)
#   dj-captain-hold.sh verify <origin-id>
#   dj-captain-hold.sh diverged
#
...
# identity, so pre-collapse metadata written by dj-decision-hold.sh verifies
...
# Records written by the retired dj-decision-hold.sh (routed, declined,
...
captain_hold_cleanup() {
  if [ "$CAPTAIN_META_LOCK_HELD" = 1 ]; then
    dj_lock_release "$CAPTAIN_META_LOCK" || true
    CAPTAIN_META_LOCK_HELD=0
  fi
}
...
tasks_axi() {
  (cd "$DJ_HOME" && tasks-axi "$@")
}

require_tasks_axi() {
  dj_tasks_axi_compatible || fail "compatible tasks-axi is required"
...
# dj-decision-hold.sh. Both carry the same leader-then-captain-decision shape.
body_has_resolution_record() {  # <task-body>
  case "$1" in
    *"Resolution recorded by dj-captain-hold."*"Captain decision:"*) return 0 ;;
    *"Resolution recorded by dj-decision-hold."*"Captain decision:"*) return 0 ;;
  esac
  return 1
}
...
resolution_block() {  # <mode>
  printf 'Resolution recorded by dj-captain-hold.\nDecision digest: %s\nResolution mode: %s\n\nCaptain decision:\n%s\n' \
...
verify_hold_durable() {  # <task-id>
  ...
  show=$(task_show "$id") || fail "captain-held task $id is absent from $DJ_HOME/data/backlog.md"
...
resolve_entry() {  # <origin-or-empty> <entry>; prints the resolved id or fails
  ...
  if [ -n "$origin" ] && [ "$origin" != "$BINDING_ANY" ]; then
    ...
    fail "no captain-held task $entry and no legacy identity $legacy in $DJ_HOME/data/backlog.md"
  fi
  fail "no captain-held task $entry in $DJ_HOME/data/backlog.md"
}
...
write_resolution_record() {  # <task-id> <mode> <shown-body>
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/dj-captain-hold-body.XXXXXX") \
...
BINDING_SCHEMA=dj-decision-binding.v1
...
command_answers() {
  ...
  require_tasks_axi
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/dj-keyed-decision.XXXXXX") || fail "cannot stage the captain decision"
  err=$(umask 077; mktemp "${TMPDIR:-/tmp}/dj-keyed-decision-err.XXXXXX") \
  ...
  while IFS= read -r row; do
    ...
    if body_has_resolution_record "$body" \
      && { [ "$recorded_digest" = "$digest" ] \
        || { case "$body" in *"Resolution recorded by dj-decision-hold."*) true ;; *) false ;; esac \
...
command_complete() {
  ...
  [ -f "$meta" ] && has_meta=1
  if [ "$has_meta" = 1 ]; then
    CAPTAIN_META_LOCK=$(dj_meta_lock_path "$meta") || fail "could not resolve task metadata lock"
    dj_lock_acquire_wait "$CAPTAIN_META_LOCK"
    CAPTAIN_META_LOCK_HELD=1
    [ -f "$meta" ] || fail "task metadata disappeared while recording completion"
  fi
  require_tasks_axi
  origin_exists_here "$origin" || fail "origin $origin is not owned by the active home $DJ_HOME"
  if [ "$#" -eq 1 ] && [ "$1" = --none ]; then
    supplied=''
  else
    while [ "$#" -gt 0 ]; do
...
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    dj_lock_release "$CAPTAIN_META_LOCK"
    CAPTAIN_META_LOCK_HELD=0
    ...
    # guarded self-announced append (bin/dj-wake-lib.sh) and does not wake this
    # same session; an append failure still fails this command loudly.
    if [ -n "$keys" ]; then
      while IFS=$'\t' read -r key _verb _summary; do
        [ -n "$key" ] || continue
        transfer_rc=0
        dj_wake_status_append_self_announced "$STATE" "$status_file" \
...
command_diverged() {
  ...
  # tasks-axi reads $DJ_HOME's backlog, so a state dir pointed somewhere else
