#!/usr/bin/env bash
# dj-pending-reply-lib.sh - parent-owned secondmate missed-report guards.
#
# When the main jarvis delivers a marked from-jarvis request to a
...
# acknowledgement.
#
# Record location (parent DJ_HOME):
#   state/pending-replies/<corr_id>
# One more durable input, owned by bin/dj-procevent-remote-reply.sh and read
...
# keyed decision in the parent status log, and bin/dj-classify-lib.sh's fold is
# the one owner of what closes it. So this library owns both ends of that
# decision: dj_pending_reply_maybe_escalate opens it under a per-request key, and
# dj_pending_reply_close_escalation closes it once the record resolves. Resolving
...
# reserved-key rule in bin/dj-classify-lib.sh.
#
# Sourced by bin/dj-send.sh, bin/dj-watch.sh, bin/dj-secondmate-report.sh, and
# tests. No side effects on source. set -u / set -e safe.
#
# Tunables (env):
#   DJ_PENDING_REPLY_GRACE_SECS   default 120
#   DJ_PENDING_REPLY_DIR_OVERRIDE override the pending-replies directory (tests)
#   DJ_PENDING_REPLY_SEND_HOOK    optional command template for recovery delivery
#                                 (tests); receives task_id and full message as args
#   DJ_PENDING_REPLY_NOW          optional fixed epoch for deterministic tests
...
# shellcheck source=bin/dj-marker-lib.sh
. "$_FM_PENDING_REPLY_LIB_DIR/dj-marker-lib.sh"
# shellcheck source=bin/dj-backend.sh
. "$_FM_PENDING_REPLY_LIB_DIR/dj-backend.sh"
# shellcheck source=bin/dj-tmux-lib.sh
. "$_FM_PENDING_REPLY_LIB_DIR/dj-tmux-lib.sh"
# shellcheck source=bin/dj-classify-lib.sh
. "$_FM_PENDING_REPLY_LIB_DIR/dj-classify-lib.sh"

DJ_PENDING_REPLY_SCHEMA='dj-pending-reply.v1'
DJ_PENDING_REPLY_CORR_RE='corr=[A-Fa-f0-9]{16}'
DJ_PENDING_REPLY_GRACE_DEFAULT=120

dj_pending_reply_now() {
  if [ -n "${DJ_PENDING_REPLY_NOW:-}" ]; then
    printf '%s' "$DJ_PENDING_REPLY_NOW"
    return 0
  fi
  date +%s
}

dj_pending_reply_grace_secs() {
  local g=${DJ_PENDING_REPLY_GRACE_SECS:-$DJ_PENDING_REPLY_GRACE_DEFAULT}
  case "$g" in
    ''|*[!0-9]*) g=$DJ_PENDING_REPLY_GRACE_DEFAULT ;;
  esac
  printf '%s' "$g"
}

# Directory holding durable pending-reply records for <state-dir>.
dj_pending_reply_dir() {  # <state-dir>
  local state=$1
  if [ -n "${DJ_PENDING_REPLY_DIR_OVERRIDE:-}" ]; then
    printf '%s' "$DJ_PENDING_REPLY_DIR_OVERRIDE"
    return 0
  fi
  printf '%s/pending-replies' "$state"
}

dj_pending_reply_path() {  # <state-dir> <corr_id>
  printf '%s/%s' "$(dj_pending_reply_dir "$1")" "$2"
}

# Privacy-safe correlation id: 16 lowercase hex chars (64 bits of entropy).
dj_pending_reply_new_id() {
  local raw hex
...
dj_pending_reply_corr_token() {  # <corr_id>
  printf 'corr=%s' "$1"
}

# Extract the first corr=<16hex> token from free text, or empty.
dj_pending_reply_extract_corr() {  # <text>
  local text=$1
  printf '%s' "$text" | grep -oE "$DJ_PENDING_REPLY_CORR_RE" 2>/dev/null | head -1 | cut -d= -f2- | tr 'A-F' 'a-f' || true
}

# 0 if <text> carries the exact correlation token for <corr_id>.
dj_pending_reply_text_has_corr() {  # <text> <corr_id>
  local text=$1 corr=$2 token
  token=$(dj_pending_reply_corr_token "$corr")
  case "$text" in
    *"$token"*) return 0 ;;
  esac
  return 1
...
dj_pending_reply_summarize() {  # <text>
  ...
  cleaned=${cleaned#"$DJ_FROMFIRST_MARK"}
...
dj_pending_reply_get() {  # <record-path> <key>
  local rec=$1 key=$2
  [ -f "$rec" ] || return 0
  grep "^${key}=" "$rec" 2>/dev/null | tail -1 | cut -d= -f2- || true
}

dj_pending_reply_corr_reusable() {  # <state-dir> <corr_id> <task_id>
  ...
  [ -f "$rec" ] || return 1
  [ "$(dj_pending_reply_get "$rec" task_id)" = "$task_id" ] || return 1
  phase=$(dj_pending_reply_get "$rec" phase)
  case "$phase" in
    awaiting_report|recovery_sending|recovery_sent) return 0 ;;
    delivery_unknown)
      delivered=$(dj_pending_reply_get "$rec" delivered_epoch)
      [ -z "$delivered" ]
      return $?
      ;;
  esac
...
dj_pending_reply_set() {  # <record-path> <key> <value>
...
# Embed or replace a correlation token after the from-jarvis marker.
...
dj_pending_reply_embed_corr() {  # <message> <corr_id> <result-var>
  local message=$1 corr=$2 result_var=$3 body token marked existing
  [ -n "$result_var" ] || return 2
  token=$(dj_pending_reply_corr_token "$corr")
  dj_message_mark_from_jarvis "$message" marked
  body=${marked#"$DJ_FROMFIRST_MARK"}
  ...
  printf -v "$result_var" '%s' "${DJ_FROMFIRST_MARK}${token} ${body}"
}
...
dj_pending_reply_create() {  # <parent-home> <state-dir> <task_id> <request-text>
  ...
  dir=$(dj_pending_reply_dir "$state")
  mkdir -p "$dir" || return 1
  chmod 700 "$dir" 2>/dev/null || true
  corr=$(dj_pending_reply_new_id)
  [ "${#corr}" -eq 16 ] || return 1
  rec=$(dj_pending_reply_path "$state" "$corr")
  ...
  now=$(dj_pending_reply_now)
  summary=$(dj_pending_reply_summarize "$request_text")
  ...
  tmp="$dir/.${corr}.tmp.$$"
  cat > "$tmp" <<EOF
schema=$DJ_PENDING_REPLY_SCHEMA
corr_id=$corr
task_id=$task_id
...
wrong_home_scan_signature=
grace_secs=$(dj_pending_reply_grace_secs)
EOF
...
dj_pending_reply_mark_delivered() {  # <state-dir> <corr_id> [confirmed-epoch]
  ...
  delivered=$(dj_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    now=${confirmed_epoch:-$(dj_pending_reply_now)}
    dj_pending_reply_set "$rec" delivered_epoch "$now" || return 1
  fi
  if [ "$phase" = delivery_unknown ]; then
    dj_pending_reply_set "$rec" phase awaiting_report || return 1
  fi
  return 0
}

dj_pending_reply_delivery_confirmation_path() {  # <state-dir> <corr_id>
  printf '%s/.delivery-confirmed-%s' "$(dj_pending_reply_dir "$1")" "$2"
}

dj_pending_reply_write_delivery_confirmation() {  # <state-dir> <corr_id> <state> <value>
  local pending_state=$1 corr=$2 delivery_state=$3 value=$4 marker dir tmp
  marker=$(dj_pending_reply_delivery_confirmation_path "$pending_state" "$corr")
  dir=$(dirname "$marker")
  mkdir -p "$dir" || return 1
  tmp="$marker.tmp.$$"
...
dj_pending_reply_prepare_delivery() {  # <state-dir> <corr_id>
  ...
  [ -f "$marker" ] && return 0
  now=$(dj_pending_reply_now)
  dj_pending_reply_write_delivery_confirmation "$state" "$corr" attempted "$now"
}

dj_pending_reply_confirm_delivery() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE DJ_WAKE_QUEUE DJ_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
  . "$_FM_PENDING_REPLY_LIB_DIR/dj-wake-lib.sh"
...
_fm_pending_reply_confirm_delivery_locked() {  # <state-dir> <corr_id>
  ...
  marker=$(dj_pending_reply_delivery_confirmation_path "$state" "$corr")
  if ! dj_pending_reply_prepare_delivery "$state" "$corr"; then
    return 1
  fi
  now=$(dj_pending_reply_now)
  dj_pending_reply_write_delivery_confirmation "$state" "$corr" confirmed "$now" || return 1
  if dj_pending_reply_mark_delivered "$state" "$corr" "$now"; then
...
dj_pending_reply_mark_delivery_unknown() {  # <state-dir> <corr_id>
  ...
  dj_pending_reply_set "$rec" phase delivery_unknown
}
...
_fm_pending_reply_reconcile_delivery_locked() {  # <state-dir> <corr_id>
  ...
  case "$delivery_state" in
    confirmed)
      ...
      dj_pending_reply_mark_delivered "$state" "$corr" "$epoch" || return 1
      rm -f "$marker" 2>/dev/null || true
      return 0
      ;;
    attempted)
      epoch=$value
      case "$epoch" in ''|*[!0-9]*) return 1 ;; esac
      grace=$(dj_pending_reply_get "$rec" grace_secs)
      case "$grace" in ''|*[!0-9]*) grace=$(dj_pending_reply_grace_secs) ;; esac
      now=$(dj_pending_reply_now)
      age=$((now - epoch))
      [ "$age" -ge "$grace" ] || return 1
      phase=$(dj_pending_reply_get "$rec" phase)
      [ "$phase" = awaiting_report ] || return 1
      dj_pending_reply_set "$rec" phase delivery_unknown || return 1
      return 0
      ;;
  esac
  return 1
...
dj_pending_reply_reconcile_delivery() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE DJ_WAKE_QUEUE DJ_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
...
dj_pending_reply_delivery_attempt_unresolved() {  # <state-dir> <corr_id>
...
dj_pending_reply_reset_known_undelivered() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE DJ_WAKE_QUEUE DJ_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
...
dj_pending_reply_discard_undelivered() {  # <state-dir> <corr_id>
...
dj_pending_reply_line_resolves() {  # <line> <corr_id>
  local line=$1 corr=$2
  [ -n "$line" ] && [ -n "$corr" ] || return 1
  case "$line" in
    *pending-reply-missed*) return 1 ;;
  esac
  dj_pending_reply_text_has_corr "$line" "$corr"
}

# Scan a status file for a correlated resolve. Prints the matching line or empty.
dj_pending_reply_find_resolve_line() {  # <status-file> <corr_id>
  local status_file=$1 corr=$2 line
  [ -f "$status_file" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    [ -n "$line" ] || continue
    if dj_pending_reply_line_resolves "$line" "$corr"; then
      printf '%s' "$line"
      return 0
    fi
  done < "$status_file"
...
dj_pending_reply_file_signature() {  # <path>
  local path=$1
...
dj_pending_reply_status_set_signature() {  # <status-dir>
  local status_dir=$1 status_file signature
  {
    for status_file in "$status_dir"/*.status; do
      [ -f "$status_file" ] || continue
      signature=$(dj_pending_reply_file_signature "$status_file")
...
dj_pending_reply_resolve_via_of_line() {  # <line>
  local line=$1
  case "$line" in
    *data/*report*|*report.md*|*document*|*pointer*)
      printf 'document'
      ;;
    *via-helper*|*dj-secondmate-report*)
      printf 'helper'
      ;;
    *)
      printf 'status'
...
dj_pending_reply_try_resolve() {  # <state-dir> <corr_id> [status-file-override]
  # Serialized per correlation so a resolution and an escalation cannot interleave.
  # bin/dj-wake-lib.sh owns the lock primitives but assigns its own globals when
...
_fm_pending_reply_try_resolve_locked() {  # <state-dir> <corr_id> [status-file-override]
  ...
  status_file=${status_override:-$(dj_pending_reply_get "$rec" parent_status)}
  if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
    signature=$(dj_pending_reply_file_signature "$status_file")
    previous=$(dj_pending_reply_get "$rec" parent_status_scan_signature)
    [ "$signature" != "$previous" ] || return 1
  fi
  line=$(dj_pending_reply_find_resolve_line "$status_file" "$corr")
  if [ -z "$line" ]; then
    if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
      dj_pending_reply_set "$rec" parent_status_scan_signature "$signature" || return 1
    fi
    return 1
  fi
  via=$(dj_pending_reply_resolve_via_of_line "$line")
  now=$(dj_pending_reply_now)
  dj_pending_reply_set "$rec" phase resolved || return 1
  if [ -z "$delivered" ]; then
    dj_pending_reply_mark_delivered "$state" "$corr" "$now" || return 1
    rm -f "$marker" 2>/dev/null || true
  fi
  dj_pending_reply_set "$rec" resolved_epoch "$now" || return 1
  dj_pending_reply_set "$rec" resolved_via "$via" || return 1
...
dj_pending_reply_observe_busy() {  # <state-dir> <corr_id> <busy_state>
  ...
  seen=$(dj_pending_reply_get "$rec" "$field_seen")
  completed=$(dj_pending_reply_get "$rec" "$field_completed")
  case "$busy_state" in
    busy)
      if [ "$seen" != 1 ]; then
        dj_pending_reply_set "$rec" "$field_seen" 1 || return 1
      fi
      ;;
    idle)
      if [ -z "$completed" ]; then
        ...
        if [ "$seen" = 1 ] || [ "$seen" = 0 ]; then
          now=$(dj_pending_reply_now)
          dj_pending_reply_set "$rec" "$field_completed" "$now" || return 1
        fi
      fi
      ;;
    unknown)
...
dj_pending_reply_fallback_idle_eligible() {  # <record-path>
  local rec=$1 phase start seen grace now age
  phase=$(dj_pending_reply_get "$rec" phase)
  case "$phase" in
    awaiting_report)
      start=$(dj_pending_reply_get "$rec" delivered_epoch)
      seen=$(dj_pending_reply_get "$rec" turn_seen_busy)
      ;;
    recovery_sent)
      start=$(dj_pending_reply_get "$rec" recovery_sent_epoch)
      seen=$(dj_pending_reply_get "$rec" recovery_turn_seen_busy)
      ;;
    *) return 1 ;;
  esac
  [ "$seen" = 1 ] && return 0
...
# dj_pending_reply_backend_observation: one busy/idle observation of a
# SECONDMATE endpoint, without ever reading its conversation.
#
# Deliberately NOT the semantic busy-state contract (bin/dj-busy-lib.sh).
# That contract covers ordinary task workers, whose turn lifecycle jarvis
...
# category as the submit acknowledgement matcher in bin/dj-composer-lib.sh - never task
# state, and never a source consumers can confuse with semantic state.
#
# It stays harness-scoped (dj_busy_lines_match with the recorded harness, no
...
dj_pending_reply_backend_observation() {  # <backend> <target> [expected-label] [harness]
  local backend=$1 target=$2 expected_label=${3-} harness=${4-} native tail40
  native=$(dj_backend_busy_state "$backend" "$target" 2>/dev/null || printf 'unknown')
  case "$native" in
    busy|idle) printf '%s' "$native"; return 0 ;;
  esac
  tail40=$(dj_backend_capture "$backend" "$target" 40 "$expected_label" 2>/dev/null) \
    || { printf 'unknown'; return 0; }
  if printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | dj_busy_lines_match "$harness"; then
    printf 'busy'
  else
...
dj_pending_reply_busy_state_from_observation() {  # <record-path> <observation>
  local rec=$1 observation=$2
  case "$observation" in
    busy|idle|unknown) printf '%s' "$observation" ;;
    fallback-idle)
      if dj_pending_reply_fallback_idle_eligible "$rec"; then
        printf 'idle'
      else
        printf 'unknown'
      fi
...
dj_pending_reply_mark_turn_completed() {  # <state-dir> <corr_id> [which: request|recovery]
  ...
  now=$(dj_pending_reply_now)
  dj_pending_reply_set "$rec" "$field" "$now" || return 1
...
# through the asynchronous mirror in bin/dj-procevent-remote-reply.sh, so the
...
dj_pending_reply_remote_channel_watermark_path() {  # <state-dir> <task_id>
...
dj_pending_reply_note_remote_channel_caught_up() {  # <state-dir> <task_id> [epoch]
  local state=$1 task_id=$2 epoch=${3-} path dir tmp
  [ -n "$state" ] && [ -n "$task_id" ] || return 2
  case "$epoch" in ''|*[!0-9]*) epoch=$(dj_pending_reply_now) ;; esac
  path=$(dj_pending_reply_remote_channel_watermark_path "$state" "$task_id")
  dir=$(dirname "$path")
  mkdir -p "$dir" || return 1
  chmod 700 "$dir" 2>/dev/null || true
  [ ! -L "$path" ] || return 1
...
dj_pending_reply_remote_channel_epoch() {  # <state-dir> <task_id>
  local path epoch
  path=$(dj_pending_reply_remote_channel_watermark_path "$1" "$2")
...
dj_pending_reply_target_is_remote() {  # <state-dir> <task_id>
  local meta="$1/$2.meta"
  [ -f "$meta" ] || return 1
  [ -n "$(dj_meta_get "$meta" remote_host)" ]
}
...
dj_pending_reply_missing_report_is_evidence() {  # <state-dir> <task_id> <since-epoch>
  local state=$1 task_id=$2 since=$3 caught
  dj_pending_reply_target_is_remote "$state" "$task_id" || return 0
  case "$since" in ''|*[!0-9]*) return 1 ;; esac
  caught=$(dj_pending_reply_remote_channel_epoch "$state" "$task_id")
  [ -n "$caught" ] || return 1
  [ "$caught" -ge "$since" ]
}

# Build the one automatic recovery message for a pending record.
dj_pending_reply_recovery_message() {  # <record-path>
  local rec=$1 corr summary token msg
  corr=$(dj_pending_reply_get "$rec" corr_id)
  summary=$(dj_pending_reply_get "$rec" request_summary)
  token=$(dj_pending_reply_corr_token "$corr")
  ...
  dj_pending_reply_embed_corr "$msg" "$corr" msg
  printf '%s' "$msg"
}

# Deliver the recovery message once. Caller must hold phase awaiting_report with
# turn completed and grace elapsed. Uses DJ_PENDING_REPLY_SEND_HOOK when set
# (tests), otherwise invokes dj-send with DJ_PENDING_REPLY_EXISTING_CORR so a
# second expectation is not created.
dj_pending_reply_send_recovery() {  # <state-dir> <corr_id>
  ...
  phase=$(dj_pending_reply_get "$rec" phase)
  [ "$phase" = awaiting_report ] || return 1
  attempted=$(dj_pending_reply_get "$rec" recovery_attempted_epoch)
  if [ -n "$attempted" ]; then
    dj_pending_reply_reconcile_recovery "$state" "$corr" || true
    return 1
  fi
  completed=$(dj_pending_reply_get "$rec" request_turn_completed_epoch)
  ...
  age=$((now - delivered))
  [ "$age" -ge "$grace" ] || return 1
  task_id=$(dj_pending_reply_get "$rec" task_id)
  # A remote mate's report may exist and simply not have been mirrored yet.
  dj_pending_reply_missing_report_is_evidence "$state" "$task_id" "$completed" || return 1
  parent_home=$(dj_pending_reply_get "$rec" parent_home)
  msg=$(dj_pending_reply_recovery_message "$rec")
  sender_pid=${BASHPID:-$$}
  sender_identity=$(dj_pending_reply_pid_identity "$sender_pid") || return 1
  dj_pending_reply_set "$rec" recovery_sender_pid "$sender_pid" || return 1
  dj_pending_reply_set "$rec" recovery_sender_identity "$sender_identity" || return 1
  dj_pending_reply_set "$rec" recovery_attempted_epoch "$now" || return 1
  dj_pending_reply_set "$rec" phase recovery_sending || return 1
  if [ -n "${DJ_PENDING_REPLY_SEND_HOOK:-}" ]; then
    # Hook receives: task_id message
    # shellcheck disable=SC2086
    if ! eval "$DJ_PENDING_REPLY_SEND_HOOK" "$(printf '%q' "$task_id")" "$(printf '%q' "$msg")"; then
      send_status=1
    fi
  else
    if [ -z "$parent_home" ] || [ ! -d "$parent_home" ]; then
      send_status=1
    elif ! env DJ_HOME="$parent_home" DJ_PENDING_REPLY_EXISTING_CORR="$corr" \
      "$_FM_PENDING_REPLY_LIB_DIR/dj-send.sh" "$task_id" "$msg"; then
      send_status=1
    fi
  fi
  if [ "$send_status" = 0 ]; then
    dj_pending_reply_finish_recovery "$state" "$corr" confirmed
    return $?
  fi
  dj_pending_reply_finish_recovery "$state" "$corr" failed || return 1
  return 1
}

dj_pending_reply_pid_identity() {  # <pid>
...
dj_pending_reply_sender_alive() {  # <record-path>
  local rec=$1 pid expected actual
  pid=$(dj_pending_reply_get "$rec" recovery_sender_pid)
  expected=$(dj_pending_reply_get "$rec" recovery_sender_identity)
  [ -n "$expected" ] || return 1
  actual=$(dj_pending_reply_pid_identity "$pid") || return 1
  [ "$actual" = "$expected" ]
}

dj_pending_reply_finish_recovery() {  # <state-dir> <corr_id> <confirmed|failed>
  local state=$1 corr=$2 outcome=$3 rec phase now sent
  rec=$(dj_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  phase=$(dj_pending_reply_get "$rec" phase)
  [ "$phase" = recovery_sending ] || return 1
  dj_pending_reply_set "$rec" recovery_delivery_outcome "$outcome" || return 1
  if [ "$outcome" = confirmed ]; then
    sent=$(dj_pending_reply_get "$rec" recovery_sent_epoch)
    if [ -z "$sent" ]; then
      now=$(dj_pending_reply_now)
      dj_pending_reply_set "$rec" recovery_sent_epoch "$now" || return 1
    fi
    dj_pending_reply_set "$rec" recovery_turn_seen_busy 0 || return 1
    dj_pending_reply_set "$rec" recovery_turn_completed_epoch "" || return 1
    dj_pending_reply_set "$rec" phase recovery_sent || return 1
  else
    [ "$outcome" = failed ] || return 1
    dj_pending_reply_set "$rec" phase recovery_failed || return 1
  fi
}

dj_pending_reply_reconcile_recovery() {  # <state-dir> <corr_id>
  ...
  outcome=$(dj_pending_reply_get "$rec" recovery_delivery_outcome)
  case "$outcome" in
    confirmed) dj_pending_reply_finish_recovery "$state" "$corr" confirmed; return $? ;;
    failed) dj_pending_reply_finish_recovery "$state" "$corr" failed; return $? ;;
    unknown)
      dj_pending_reply_set "$rec" phase recovery_unknown || return 1
      return 0
      ;;
  esac
  dj_pending_reply_sender_alive "$rec" && return 1
  dj_pending_reply_set "$rec" recovery_delivery_outcome unknown || return 1
  dj_pending_reply_set "$rec" phase recovery_unknown || return 1
}
...
dj_pending_reply_escalation_key() {  # <corr_id>
  printf 'pending-reply-%s' "$1"
}

dj_pending_reply_escalation_payload() {  # <record-path> <kind>
...
dj_pending_reply_escalation_line() {  # <status-file> <record-path> <corr_id>
  local status_file=$1 rec=$2 corr=$3 line found='' kind payload own_key
  [ -f "$status_file" ] || return 0
  [ "$(dj_pending_reply_get "$rec" corr_id)" = "$corr" ] || return 0
  own_key=$(dj_pending_reply_escalation_key "$corr")
  while IFS= read -r line || [ -n "$line" ]; do
    [ "$(status_line_verb "$line")" = blocked ] || continue
    for kind in missed delivery-unknown recovery-delivery; do
      payload=$(dj_pending_reply_escalation_payload "$rec" "$kind") || continue
      case "$line" in
...
# bin/dj-classify-lib.sh's fold. Records that never escalated are left untouched.
dj_pending_reply_close_escalation() {  # <state-dir> <corr_id>
  # Serialized per correlation so a resolution and an escalation cannot interleave.
  # bin/dj-wake-lib.sh owns the lock primitives but assigns its own globals when
  # sourced, so they are declared local here: that contains them to this call
  # instead of leaking into every script that sources this library, without the
...
_fm_pending_reply_close_escalation_locked() {  # <state-dir> <corr_id>
  ...
  rec=$(dj_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  [ "$(dj_pending_reply_get "$rec" phase)" = resolved ] || return 0
  escalated=$(dj_pending_reply_get "$rec" escalated_epoch)
  [ -n "$escalated" ] || return 0
  closed=$(dj_pending_reply_get "$rec" escalation_closed_epoch)
  [ -z "$closed" ] || return 0
  parent_status=$(dj_pending_reply_get "$rec" parent_status)
  [ -n "$parent_status" ] || return 1
  escalation=$(dj_pending_reply_escalation_line "$parent_status" "$rec" "$corr")
  if [ -n "$escalation" ]; then
    ...
    while IFS= read -r open_line; do
      ...
      # self-announced append (bin/dj-wake-lib.sh, sourced by this function's
      ...
      close_line=$(printf 'resolved [key=%s]: pending-reply-resolved: task=%s pending-reply-id=%s via=%s' \
        "$key" "$(dj_pending_reply_get "$rec" task_id)" "$corr" \
        "$(dj_pending_reply_get "$rec" resolved_via)")
      close_rc=0
      dj_wake_status_append_self_announced "${parent_status%/*}" "$parent_status" "$close_line" \
        2>/dev/null || close_rc=$?
...
_fm_pending_reply_maybe_escalate_locked() {  # <state-dir> <corr_id>
  ...
  case "$phase" in
    recovery_sent)
      completed=$(dj_pending_reply_get "$rec" recovery_turn_completed_epoch)
      [ -n "$completed" ] || return 1
      ...
      dj_pending_reply_missing_report_is_evidence "$state" \
        "$(dj_pending_reply_get "$rec" task_id)" "$completed" || return 1
      ;;
  ...
  payload=$(dj_pending_reply_escalation_payload "$rec" "$kind") || return 1
  [ -n "$parent_status" ] || return 1
  mkdir -p "$(dirname "$parent_status")" 2>/dev/null || return 1
  line="blocked [key=$(dj_pending_reply_escalation_key "$corr")]: $payload"
  if ! grep -Fqx "$line" "$parent_status" 2>/dev/null; then
    printf '%s\n' "$line" >> "$parent_status" 2>/dev/null || return 1
  fi
  now=$(dj_pending_reply_now)
  dj_pending_reply_set "$rec" escalated_epoch "$now" || return 1
  dj_pending_reply_set "$rec" phase escalated || return 1
  return 0
}

# Detect a correlated report written under the secondmate home (wrong home)
# without treating it as acknowledgement.
dj_pending_reply_detect_wrong_home() {  # <state-dir> <corr_id> <secondmate-home>
  ...
  delivered=$(dj_pending_reply_get "$rec" delivered_epoch)
  [ -n "$delivered" ] || return 0
  snapshot=$(dj_pending_reply_status_set_signature "$sm_home/state")
  previous=$(dj_pending_reply_get "$rec" wrong_home_scan_signature)
  [ "$snapshot" != "$previous" ] || return 0
  hits=$(dj_pending_reply_get "$rec" wrong_home_hits)
  case "$hits" in ''|*[!0-9]*) hits=0 ;; esac
  sightings=$(dj_pending_reply_get "$rec" wrong_home_sightings)
  ...
  if [ "$changed" = 1 ]; then
    dj_pending_reply_set "$rec" wrong_home_sightings "$sightings" || return 1
    dj_pending_reply_set "$rec" wrong_home_hits "$hits" || return 1
  fi
  dj_pending_reply_set "$rec" wrong_home_scan_signature "$snapshot" || return 1
  return 0
}
...
dj_pending_reply_tick_one() {  # <state-dir> <corr_id> <busy_state> [secondmate-home]
  local state=$1 corr=$2 busy_state=$3 sm_home=${4-}
  local rec phase delivered
  rec=$(dj_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  dj_pending_reply_reconcile_delivery "$state" "$corr" || true
  phase=$(dj_pending_reply_get "$rec" phase)
  delivered=$(dj_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    case "$phase" in
      delivery_unknown) dj_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true ;;
      escalated) dj_pending_reply_try_resolve "$state" "$corr" >/dev/null 2>&1 || true ;;
    esac
    return 0
  fi
  ...
  case "$phase" in
    awaiting_report|recovery_sending)
      if [ -n "$(dj_pending_reply_get "$rec" recovery_attempted_epoch)" ]; then
        dj_pending_reply_reconcile_recovery "$state" "$corr" || true
        phase=$(dj_pending_reply_get "$rec" phase)
      fi
      ;;
  ...
  case "$phase" in
    ...
    escalated)
      ...
      if [ -n "$sm_home" ]; then
        dj_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" || true
      fi
      return 0
      ;;
    recovery_sending) return 0 ;;
    recovery_failed|recovery_unknown)
      dj_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true
      return 0
      ;;
  esac
  if [ -n "$sm_home" ]; then
  ...
  dj_pending_reply_observe_busy "$state" "$corr" "$busy_state" || true
  ...
  phase=$(dj_pending_reply_get "$rec" phase)
  if [ "$phase" = awaiting_report ]; then
    dj_pending_reply_send_recovery "$state" "$corr" 2>/dev/null || true
  fi
  phase=$(dj_pending_reply_get "$rec" phase)
  case "$phase" in
    recovery_sent|recovery_failed|recovery_unknown)
...
dj_pending_reply_tick() {  # <state-dir>
  ...
  for rec in "$dir"/*; do
    ...
    if [ "$phase" = resolved ]; then
      ...
      dj_pending_reply_close_escalation "$state" "$corr" || true
      continue
    fi
    dj_pending_reply_reconcile_delivery "$state" "$corr" || true
    phase=$(dj_pending_reply_get "$rec" phase)
    ...
    if [ -z "$delivered" ]; then
      case "$phase" in
        delivery_unknown|escalated)
          dj_pending_reply_tick_one "$state" "$corr" unknown "" || true
          ;;
      esac
      continue
    fi
    ...
    if [ -f "$meta" ]; then
      ...
      backend=$(dj_backend_of_meta "$meta")
      target=$(dj_backend_target_of_meta "$meta")
      sm_home=$(dj_meta_get "$meta" home)
      harness=$(dj_meta_get "$meta" harness)
      ...
      if [ -n "$target" ]; then
        ...
        if [ "$found" = 0 ]; then
          if [ -n "$remote_host" ]; then
            observation=$("$_FM_PENDING_REPLY_LIB_DIR/dj-on.sh" "$task_id" \
              dj-remote-secondmate-control.sh observe "$task_id" < /dev/null 2>/dev/null || printf 'unknown')
            case "$observation" in busy|idle|fallback-idle|unknown) ;; *) observation=unknown ;; esac
          else
            observation=$(dj_pending_reply_backend_observation "$backend" "$target" "$label" "$harness")
          fi
          observation_tasks+=("$task_id")
          observation_values+=("$observation")
        fi
        busy=$(dj_pending_reply_busy_state_from_observation "$rec" "$observation")
      fi
    fi
    dj_pending_reply_tick_one "$state" "$corr" "$busy" "$sm_home" || true
  done
  return 0
}

# True when any open (non-resolved) pending reply exists for a task.
dj_pending_reply_task_has_open() {  # <state-dir> <task_id>
  local state=$1 task_id=$2 dir rec phase tid
  dir=$(dj_pending_reply_dir "$state")
  [ -d "$dir" ] || return 1
  for rec in "$dir"/*; do
    [ -f "$rec" ] || continue
    tid=$(dj_pending_reply_get "$rec" task_id)
