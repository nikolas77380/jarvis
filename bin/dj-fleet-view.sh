#!/usr/bin/env bash
# dj-fleet-view.sh - human renderer over dj-fleet-snapshot.sh.
#
# This command intentionally does not parse fleet state itself.
# It shells out to dj-fleet-snapshot.sh --json and renders that stable
...
usage() {
  cat <<'EOF'
usage: dj-fleet-view.sh [--json]

Render a human fleet view from dj-fleet-snapshot.sh.
...
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
  --json) "$SCRIPT_DIR/dj-fleet-snapshot.sh" --json; exit $? ;;
  "") ;;
  *) usage >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "dj-fleet-view: jq not found" >&2; exit 1; }

SNAPSHOT=$("$SCRIPT_DIR/dj-fleet-snapshot.sh" --json) || exit $?
