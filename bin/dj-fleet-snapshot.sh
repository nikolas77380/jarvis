#!/usr/bin/env bash
# dj-fleet-snapshot.sh - read-only structured fleet snapshot.
#
# Output contract: `--json` prints one object with schema
# `dj-fleet-snapshot.v1`.
...
#   dj_home: resolved operational home.
...
#     current_state is parsed from bin/dj-crew-state.sh <id> and preserves
#     state, source, detail, and raw line separately.
#     paths.status_log.last_event is historical wake-event data only, never
#     current state.
#     hints.open_decisions is the keyed open-decision set returned by
#     dj-classify-lib.sh's authoritative status_open_decisions fold and reconciled
...
SNAPSHOT_NOW=${DJ_SNAPSHOT_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ -n "${DJ_SNAPSHOT_NOW_EPOCH:-}" ]; then
  SNAPSHOT_EPOCH=$DJ_SNAPSHOT_NOW_EPOCH
else
...
DJ_SNAPSHOT_SECONDMATES=${DJ_SNAPSHOT_SECONDMATES:-20}
DJ_SNAPSHOT_SECONDMATE_TIMEOUT=${DJ_SNAPSHOT_SECONDMATE_TIMEOUT:-8}
DJ_SNAPSHOT_SECONDMATE_MAX_BYTES=${DJ_SNAPSHOT_SECONDMATE_MAX_BYTES:-262144}
DJ_SNAPSHOT_SECONDMATE_CHILDREN=${DJ_SNAPSHOT_SECONDMATE_CHILDREN:-20}
DJ_SNAPSHOT_SECONDMATE_QUEUED=${DJ_SNAPSHOT_SECONDMATE_QUEUED:-20}
DJ_SNAPSHOT_SECONDMATE_DECISIONS=${DJ_SNAPSHOT_SECONDMATE_DECISIONS:-20}
DJ_SNAPSHOT_TERMINAL_LINES=${DJ_SNAPSHOT_TERMINAL_LINES:-8}
DJ_SNAPSHOT_TERMINAL_BYTES=${DJ_SNAPSHOT_TERMINAL_BYTES:-4096}
DJ_SNAPSHOT_TERMINAL_TIMEOUT=${DJ_SNAPSHOT_TERMINAL_TIMEOUT:-2}
DJ_SNAPSHOT_PARENT_ACTIVITY_LINES=${DJ_SNAPSHOT_PARENT_ACTIVITY_LINES:-256}
DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES=${DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES:-65536}
DJ_SNAPSHOT_PARENT_ACTIVITIES=${DJ_SNAPSHOT_PARENT_ACTIVITIES:-20}
DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT=${DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT:-2}
DJ_SNAPSHOT_REGISTRY_LINES=${DJ_SNAPSHOT_REGISTRY_LINES:-256}
DJ_SNAPSHOT_REGISTRY_BYTES=${DJ_SNAPSHOT_REGISTRY_BYTES:-65536}
DJ_SNAPSHOT_REGISTRY_RECORDS=${DJ_SNAPSHOT_REGISTRY_RECORDS:-40}
DJ_SNAPSHOT_REGISTRY_TIMEOUT=${DJ_SNAPSHOT_REGISTRY_TIMEOUT:-2}
validate_positive_bound() {  # <name> <value>
  case "$2" in
    ''|*[!0-9]*|0)
      printf 'dj-fleet-snapshot: %s must be a positive integer\n' "$1" >&2
      exit 2
      ;;
  esac
}
case "$DJ_SNAPSHOT_SECONDMATES" in
  ''|*[!0-9]*)
    echo "dj-fleet-snapshot: DJ_SNAPSHOT_SECONDMATES must be a non-negative integer" >&2
    exit 2
    ;;
esac
validate_positive_bound DJ_SNAPSHOT_SECONDMATE_TIMEOUT "$DJ_SNAPSHOT_SECONDMATE_TIMEOUT"
validate_positive_bound DJ_SNAPSHOT_SECONDMATE_MAX_BYTES "$DJ_SNAPSHOT_SECONDMATE_MAX_BYTES"
validate_positive_bound DJ_SNAPSHOT_SECONDMATE_CHILDREN "$DJ_SNAPSHOT_SECONDMATE_CHILDREN"
validate_positive_bound DJ_SNAPSHOT_SECONDMATE_QUEUED "$DJ_SNAPSHOT_SECONDMATE_QUEUED"
validate_positive_bound DJ_SNAPSHOT_SECONDMATE_DECISIONS "$DJ_SNAPSHOT_SECONDMATE_DECISIONS"
validate_positive_bound DJ_SNAPSHOT_TERMINAL_LINES "$DJ_SNAPSHOT_TERMINAL_LINES"
validate_positive_bound DJ_SNAPSHOT_TERMINAL_BYTES "$DJ_SNAPSHOT_TERMINAL_BYTES"
validate_positive_bound DJ_SNAPSHOT_TERMINAL_TIMEOUT "$DJ_SNAPSHOT_TERMINAL_TIMEOUT"
validate_positive_bound DJ_SNAPSHOT_PARENT_ACTIVITY_LINES "$DJ_SNAPSHOT_PARENT_ACTIVITY_LINES"
validate_positive_bound DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES "$DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES"
validate_positive_bound DJ_SNAPSHOT_PARENT_ACTIVITIES "$DJ_SNAPSHOT_PARENT_ACTIVITIES"
validate_positive_bound DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT "$DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT"
validate_positive_bound DJ_SNAPSHOT_REGISTRY_LINES "$DJ_SNAPSHOT_REGISTRY_LINES"
validate_positive_bound DJ_SNAPSHOT_REGISTRY_BYTES "$DJ_SNAPSHOT_REGISTRY_BYTES"
validate_positive_bound DJ_SNAPSHOT_REGISTRY_RECORDS "$DJ_SNAPSHOT_REGISTRY_RECORDS"
validate_positive_bound DJ_SNAPSHOT_REGISTRY_TIMEOUT "$DJ_SNAPSHOT_REGISTRY_TIMEOUT"
...
# shellcheck disable=SC1091
. "$SCRIPT_DIR/dj-ff-lib.sh"  # validate_secondmate_home: shared seeded-home boundary checks
# shellcheck source=bin/dj-timeout-lib.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/dj-timeout-lib.sh"  # dj_run_timed: the shared hard bound

usage() {
  cat <<'EOF'
usage: dj-fleet-snapshot.sh --json
       dj-fleet-snapshot.sh --secondmate-home-summary

Print a read-only structured snapshot of the jarvis fleet.
...
Cross-home reads use DJ_SNAPSHOT_SECONDMATES (default 20, 0 lifts the count
bound), DJ_SNAPSHOT_SECONDMATE_TIMEOUT, and DJ_SNAPSHOT_SECONDMATE_MAX_BYTES.
Terminal contradiction evidence uses
DJ_SNAPSHOT_TERMINAL_LINES, DJ_SNAPSHOT_TERMINAL_BYTES, and
DJ_SNAPSHOT_TERMINAL_TIMEOUT and never becomes canonical current state.
Parent activity evidence uses DJ_SNAPSHOT_PARENT_ACTIVITY_LINES,
DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES, DJ_SNAPSHOT_PARENT_ACTIVITIES, and
DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT, with truncation disclosed in the result.
The registered secondmate table uses DJ_SNAPSHOT_REGISTRY_LINES,
DJ_SNAPSHOT_REGISTRY_BYTES, DJ_SNAPSHOT_REGISTRY_RECORDS, and
DJ_SNAPSHOT_REGISTRY_TIMEOUT, with unavailability and truncation disclosed.
EOF
}
...
command -v jq >/dev/null 2>&1 || { echo "dj-fleet-snapshot: jq not found" >&2; exit 1; }
...
crew_state_json() {  # <id>
  ...
  raw=$(
    DJ_ROOT_OVERRIDE="$DJ_ROOT" \
      DJ_HOME="$DJ_HOME" \
      DJ_STATE_OVERRIDE="$STATE" \
      DJ_DATA_OVERRIDE="$DATA" \
      DJ_PROJECTS_OVERRIDE="$PROJECTS" \
      DJ_CONFIG_OVERRIDE="$CONFIG" \
      "$SCRIPT_DIR/dj-crew-state.sh" "$id" 2>/dev/null || true
  )
  raw=$(printf '%s\n' "$raw" | head -1)
  sep=' · '
  state=unknown
...
task_json_lines() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    # (dj-classify-lib.sh's status_open_decisions) so a later unrelated event can
    ...
    if [ -n "$remote_host" ]; then
      if remote_state=$(dj_run_timed "$DJ_SNAPSHOT_SECONDMATE_TIMEOUT" \
        "$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh state "$id" < /dev/null 2>/dev/null); then
        remote_rc=0
      else
        remote_rc=$?
      fi
      ...
      if [ -n "$target" ]; then
        if dj_backend_target_exists "$backend" "$target" "dj-$id" >/dev/null 2>&1; then
      ...
      if [ "$kind" = secondmate ] && [ -n "$target" ]; then
        agent_alive=$(dj_backend_agent_alive "$backend" "$target" 2>/dev/null || printf unknown)
      fi
    fi
      ...
      '{
        ...
        actions:(
          if $kind == "secondmate" then
            {send:"bin/dj-send.sh dj-\($id) \u0027<request>\u0027",
             watch:"read status/doc return channel; do not routinely dj-peek a secondmate for answers",
             return_channel_note:"Secondmate answers come back through status/doc paths after a marked dj-send request."}
          else
            {watch:"bin/dj-peek.sh dj-\($id)",
             steer:"bin/dj-send.sh dj-\($id) \u0027<instruction>\u0027",
...
secondmate_home_summary_json() {  # <backlog-json> <tasks-json>
  jq -n \
    ...
    --arg home "$DJ_HOME" \
    --argjson child_n "$DJ_SNAPSHOT_SECONDMATE_CHILDREN" \
    --argjson queued_n "$DJ_SNAPSHOT_SECONDMATE_QUEUED" \
    --argjson decisions_n "$DJ_SNAPSHOT_SECONDMATE_DECISIONS" \
    --argjson landed_n "$DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME" \
    --argjson backlog "$1" \
    --argjson tasks "$2" '
    def trunc($n):
      tostring | gsub("\\s+"; " ")
    ...
    | {
        schema:"dj-secondmate-home-summary.v1",
        generated:$generated,
        home:$home,
        valid:$valid,
        reason:$reason,
...
DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=${DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME:-10}
case "$DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME" in ''|*[!0-9]*) DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=10 ;; esac
...
JQ
  ...
  out=$(dj_run_timed "$DJ_SNAPSHOT_REGISTRY_TIMEOUT" bash -c "$script" \
    dj-secondmate-registry "$reg" "$DJ_SNAPSHOT_REGISTRY_LINES" \
    "$DJ_SNAPSHOT_REGISTRY_BYTES" "$DJ_SNAPSHOT_REGISTRY_RECORDS" "$reg" "$SNAPSHOT_NOW" \
...
BASH
  ...
  out=$(dj_run_timed "$DJ_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT" bash -c "$script" \
    dj-parent-activities "$SCRIPT_DIR/dj-classify-lib.sh" "$f" \
    "$DJ_SNAPSHOT_PARENT_ACTIVITY_LINES" "$DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES" \
    "$DJ_SNAPSHOT_PARENT_ACTIVITIES" "$SNAPSHOT_STAT_STYLE" 2>/dev/null)
  rc=$?
  if [ "$rc" -eq 0 ] && printf '%s' "$out" | jq -e '
...
terminal_evidence_json() {  # <parent-task-json> <event-note> <evidence-contradicts>
  ...
  expected=$(printf '%s' "$task" | jq -r '"dj-" + (.id // "")')
  ...
  out=$(dj_run_timed "$DJ_SNAPSHOT_TERMINAL_TIMEOUT" bash -c \
    '. "$1"; dj_backend_capture "$2" "$3" "$4" "$5" | LC_ALL=C head -c "$6"; rc=${PIPESTATUS[0]}; [ "$rc" -eq 141 ] && rc=0; exit "$rc"' \
    dj-terminal-capture "$SCRIPT_DIR/dj-backend.sh" "$backend" "$target" "$DJ_SNAPSHOT_TERMINAL_LINES" "$expected" "$DJ_SNAPSHOT_TERMINAL_BYTES" 2>/dev/null)
  rc=$?
  if [ "$rc" -ne 0 ]; then
  ...
  clean=$(printf '%s' "$out" | tail -n "$DJ_SNAPSHOT_TERMINAL_LINES" | LC_ALL=C head -c "$DJ_SNAPSHOT_TERMINAL_BYTES")
...
secondmate_current_json() {  # <parent-tasks-json>
  ...
  rows=$(printf '%s' "$union" | jq -c --argjson cap "$DJ_SNAPSHOT_SECONDMATES" '(if $cap == 0 then .records else .records[:$cap] end)[]')
  shown=$(printf '%s\n' "$rows" | grep -c . || true)
  ...
  while IFS= read -r row; do
    ...
    if [ -z "$reason" ]; then
      if [ "$remote" = true ]; then
        summary=$(dj_run_timed "$DJ_SNAPSHOT_SECONDMATE_TIMEOUT" \
          "$SCRIPT_DIR/dj-on.sh" "$id" dj-fleet-snapshot.sh --secondmate-home-summary < /dev/null 2>/dev/null)
        summary_rc=$?
      else
        summary=$(dj_run_timed "$DJ_SNAPSHOT_SECONDMATE_TIMEOUT" env \
          DJ_ROOT_OVERRIDE="$DJ_ROOT" \
          DJ_HOME="$home" \
          DJ_STATE_OVERRIDE="$home/state" \
          DJ_DATA_OVERRIDE="$home/data" \
          DJ_CONFIG_OVERRIDE="$home/config" \
          DJ_PROJECTS_OVERRIDE="$home/projects" \
          DJ_SNAPSHOT_NOW="$SNAPSHOT_NOW" \
          DJ_SNAPSHOT_NOW_EPOCH="$SNAPSHOT_EPOCH" \
          DJ_SNAPSHOT_SECONDMATE_CHILDREN="$DJ_SNAPSHOT_SECONDMATE_CHILDREN" \
          DJ_SNAPSHOT_SECONDMATE_QUEUED="$DJ_SNAPSHOT_SECONDMATE_QUEUED" \
          DJ_SNAPSHOT_SECONDMATE_DECISIONS="$DJ_SNAPSHOT_SECONDMATE_DECISIONS" \
          DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME="$DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME" \
          "$SCRIPT_DIR/dj-fleet-snapshot.sh" --secondmate-home-summary 2>/dev/null)
        summary_rc=$?
      fi
      if [ "$summary_rc" -ne 0 ]; then
        ...
        if [ "$summary_bytes" -gt "$DJ_SNAPSHOT_SECONDMATE_MAX_BYTES" ]; then
          ...
          .schema == "dj-secondmate-home-summary.v1" and .home == $home
...
BACKLOG_JSON=$(backlog_json) || { echo "dj-fleet-snapshot: backlog read failed" >&2; exit 1; }
TASKS_JSON=$(task_json_lines) || { echo "dj-fleet-snapshot: task snapshot failed" >&2; exit 1; }

if [ "$OUTPUT_MODE" = secondmate-home-summary ]; then
  secondmate_home_summary_json "$BACKLOG_JSON" "$TASKS_JSON" \
    || { echo "dj-fleet-snapshot: secondmate home summary failed" >&2; exit 1; }
  exit 0
fi

SCOUT_REPORTS_JSON=$(scout_report_lines)
MAIN_INVENTORY_JSON=$(main_inventory_json "$BACKLOG_JSON" "$TASKS_JSON") \
  || { echo "dj-fleet-snapshot: main inventory summary failed" >&2; exit 1; }
SECONDMATE_CURRENT_JSON=$(secondmate_current_json "$TASKS_JSON") \
  || { echo "dj-fleet-snapshot: registered secondmate aggregation failed" >&2; exit 1; }
SECONDMATE_LANDED_JSON=$(secondmate_landed_from_current_json "$SECONDMATE_CURRENT_JSON") \
  || { echo "dj-fleet-snapshot: secondmate landed projection failed" >&2; exit 1; }
...
jq -n \
  ...
  'def backlog_by_id($id): ($backlog.records[]? | select(.structured == true and .id == $id) | .) // null;
   ...
   {
     schema:"dj-fleet-snapshot.v1",
     generated:$generated,
     dj_home:$dj_home,
     roots:{dj_root:$dj_root,state:$state,data:$data,config:$config,projects:$projects},
     backlog:$backlog,
