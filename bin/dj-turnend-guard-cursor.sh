#!/usr/bin/env bash
# Cursor `stop` hook adapter for a jarvis PRIMARY session: the park model.
#
...
# primary supervision:
#
#   PARK      while supervision is needed, foreground bin/dj-watch-arm.sh and
...
#   - DJ_CURSOR_TURNEND_LOOP_CEILING bounds the payload's own loop_count from
#     inside, deliberately BELOW the registered loop_limit, so jarvis's bound
...
LOOP_CEILING=${DJ_CURSOR_TURNEND_LOOP_CEILING:-180}
BLOCK_BUDGET=${DJ_CURSOR_TURNEND_BLOCK_BUDGET:-3}
ARM_ATTEMPTS=${DJ_CURSOR_PARK_ATTEMPTS:-2}
POLL=${DJ_CURSOR_PARK_POLL:-2}
LOCK_ATTEMPTS=${DJ_CURSOR_LOCK_ATTEMPTS:-50}
...
lock_acquire_bounded() {  # <lock>
  ...
  while [ "$attempt" -lt "$LOCK_ATTEMPTS" ]; do
    dj_lock_try_acquire "$lock" && return 0
    attempt=$((attempt + 1))
...
emit_followup() {  # <kind> <body> [reset-budget]
  ...
  dj_operational_input_encode "$kind" "$body" encoded || exit 0
...
$reason"
  dj_operational_input_encode turn-end-guard "$body" encoded || exit 0
  response=$(jq -n --arg m "$encoded" '{followup_message:$m}' 2>/dev/null) || exit 0
...
current_session_still_ours() {
  ...
  dj_session_lock_owned_by_self "$STATE"
}
...
# case, delegated to bin/dj-lock.sh so acquisition keeps its single owner.
if ! dj_session_lock_owned_by_self "$STATE"; then
  LOCK_PID=$(cat "$STATE/.lock" 2>/dev/null || true)
  case "$LOCK_PID" in ''|*[!0-9]*) exit 0 ;; esac
  dj_harness_pid_alive "$LOCK_PID" && exit 0
  "$SCRIPT_DIR/dj-lock.sh" >/dev/null 2>&1 || exit 0
  dj_session_lock_owned_by_self "$STATE" || exit 0
fi
...
if [ "$LOOP_COUNT" -ge "$LOOP_CEILING" ]; then
  ...
  dj_supervision_needed "$STATE" "$GRACE" || exit 0
  emit_followup turn-end-guard "JARVIS SUPERVISION FOLLOW-UP CEILING REACHED - this session has taken $LOOP_COUNT consecutive hook-driven turns without a captain message, so automatic wake delivery stops here to bound the loop. Queued wakes stay durable: run bin/dj-wake-drain.sh, handle them, and run its exact WAKE_ACK_REQUIRED command. Supervision resumes automatically at the next turn end after the captain's next message."
fi

# Away mode owns the watcher and its own triage; never park and never wake.
[ -e "$STATE/.afk" ] && exit 0

if ! dj_supervision_needed "$STATE" "$GRACE"; then
  budget_reset_if_ours
  exit 0
fi
...
while [ "$attempt" -lt "$ARM_ATTEMPTS" ]; do
  ...
  if [ -n "$ARM_OUT" ]; then
    "$SCRIPT_DIR/dj-watch-arm.sh" >"$ARM_OUT" 2>&1 &
  else
    "$SCRIPT_DIR/dj-watch-arm.sh" >/dev/null 2>&1 &
  fi
  ARM_PID=$!
...
if [ "$ACTIONABLE" -eq 1 ]; then
  ...
  emit_followup watcher "jarvis watcher wake - one supervision event needs a handling turn now.
$WAKE

Run bin/dj-wake-drain.sh first, handle the wake, then run its exact WAKE_ACK_REQUIRED --ack-through command. Until that post-handling acknowledgement, interruption leaves the wake durable for idempotent re-handling. This stop hook owns watcher continuity: when the handling turn ends, the next needed cycle parks automatically - do NOT run bin/dj-watch-arm.sh after an ordinary wake." reset-budget
fi
...
# time: bin/dj-turnend-guard.sh owns the block decision and its banner for every
# harness, and --cursor tells it this is Cursor's own registration rather than
# the Claude-settings duplicate.
GUARD_ERR=$(mktemp "${TMPDIR:-/tmp}/dj-turnend-cursor.XXXXXX") || exit 0
printf '%s' "$PAYLOAD" | "$SCRIPT_DIR/dj-turnend-guard.sh" --cursor 2>"$GUARD_ERR"
GUARD_RC=$?
