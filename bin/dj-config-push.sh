...
# dj-config-inherit-lib.sh. Remote routes receive one durable marked reread nudge
...
Push the primary jarvis home's declared inherited local material into each
...
Environment overrides follow the rest of jarvis:
  DJ_HOME            active jarvis home
  DJ_ROOT_OVERRIDE  jarvis repo root
  DJ_STATE_OVERRIDE state dir
  DJ_DATA_OVERRIDE  data dir
  DJ_CONFIG_OVERRIDE config dir
EOF
}
...
case "${1:-}" in
  ...
  "")
    ;;
  *)
    echo "usage: dj-config-push.sh [--help]" >&2
    exit 2
    ;;
esac
...
records=$(mktemp "${TMPDIR:-/tmp}/dj-config-push-records.XXXXXX" 2>/dev/null) || exit 1
reports=""
...
echo "config-push: $DJ_HOME -> live secondmate homes"
...
while IFS='|' read -r id home _window meta; do
  ...
  if [ -n "$remote_host" ]; then
    ...
    remote_marker=$(dj_secondmate_nudge_marker_path "$STATE" "$id" 2>/dev/null || true)
    remote_pending=0
    if [ -f "$remote_marker" ] && [ "$(dj_meta_get "$remote_marker" remote)" = 1 ]; then remote_pending=1; fi
    if ! dj_secondmate_nudge_write "$STATE" "$id" "$home" "" remote \
      "$DJ_REMOTE_SECOND_MATE_NUDGE_MESSAGE" 1; then
      echo "  config-reread: retry marker failed"
      errors=1
      dj_lock_release "$remote_lock" || true
      continue
    fi
    if remote_out=$(DJ_CONFIG_INHERIT_LIVE=1 \
      ...
      if [ "$remote_nudge" -eq 1 ]; then
        if DJ_HOME="$DJ_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_STATE_OVERRIDE="$STATE" \
          "$SCRIPT_DIR/dj-send.sh" "dj-$id" "$DJ_REMOTE_SECOND_MATE_NUDGE_MESSAGE" >/dev/null 2>&1; then
          rm -f -- "$remote_marker"
  ...
  report=$(mktemp "${TMPDIR:-/tmp}/dj-config-push-report.XXXXXX" 2>/dev/null) || {
  ...
  reread_pending=0
  if dj_config_reread_has_pending "$home_real" || dj_config_reread_has_staged "$DJ_HOME" "$id"; then
    reread_pending=1
  fi
  if reread_out=$(DJ_HOME="$DJ_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
    ...
    if [ -n "$(dj_config_reread_changed_items "$report")" ] || [ "$reread_pending" -eq 1 ]; then
