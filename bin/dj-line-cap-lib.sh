...
# Usage: . bin/dj-line-cap-lib.sh; dj_cap_line "<line>" [<max>]
#
# ONE OWNER for the bounded-line shape both digests use. The wake digest's
# OPEN DECISIONS section (bin/dj-wake-drain.sh) and the session-start digest's
# per-task status tails (bin/dj-session-start.sh) render the same kind of
...
#
# Callers keep their own composite policy: dj-wake-drain.sh still owns the
# OPEN DECISIONS global byte cap and its "N more omitted" disclosure, and
# dj-session-start.sh still owns how many tail lines it prints per task. This
...
DJ_LINE_CAP_DEFAULT=220
DJ_LINE_CAP_SUFFIX=' [truncated]'

# dj_cap_line_var <line> [<max>]: put <line> in DJ_LINE_CAP_LINE, cut to <max>
# characters with DJ_LINE_CAP_SUFFIX in place of the tail when it is longer. A
...
dj_cap_line_var() {
  local line=$1 max=${2:-$DJ_LINE_CAP_DEFAULT} keep
  if [ "${#line}" -le "$max" ]; then
    DJ_LINE_CAP_LINE=$line
    return 0
  fi
  keep=$((max - ${#DJ_LINE_CAP_SUFFIX}))
  [ "$keep" -ge 0 ] || keep=0
  DJ_LINE_CAP_LINE="${line:0:$keep}$DJ_LINE_CAP_SUFFIX"
}

# dj_cap_line <line> [<max>]: the same cut, printed on stdout, for a caller that
