...
# Usage: . bin/dj-tasks-axi-lib.sh
#
# Compatible means tasks-axi --version reports DJ_TASKS_AXI_MIN or newer,
...
# DJ_TASKS_AXI_MIN follows the axi-family floor policy owned beside the floor
# constants in bin/dj-bootstrap.sh.
...
# `config/backlog-backend=manual` opts out of tasks-axi for routine jarvis
# backlog mutations, but validated secondmate handoffs always use `tasks-axi mv`.
# Absent or any other value keeps the default tasks-axi backend path, falling
# back to manual mutation when the tool is not compatible.
#
# This file is the single owner of DJ_TASKS_AXI_MIN. bin/dj-bootstrap.sh turns a
# failing check into the operator-facing MISSING diagnostic.
#
# COMPATIBILITY VERDICT REUSE. dj_tasks_axi_compatible costs three tasks-axi
# subprocesses, and one session start needs the same verdict twice: once in
# bin/dj-session-start.sh's backlog listing and once in the bin/dj-bootstrap.sh
...
#     in DJ_TASKS_AXI_COMPATIBLE=0|1. Sourcing this file CONSUMES that variable
...
DJ_TASKS_AXI_MIN=0.2.4

DJ_TASKS_AXI_COMPATIBLE_MEMO=${DJ_TASKS_AXI_COMPATIBLE:-}
unset DJ_TASKS_AXI_COMPATIBLE
case "$DJ_TASKS_AXI_COMPATIBLE_MEMO" in
  0|1) ;;
  *) DJ_TASKS_AXI_COMPATIBLE_MEMO= ;;
esac

dj_tasks_axi_version_parts() {
  local output
...
dj_tasks_axi_compatible() {
  ...
  if dj_tasks_axi_compatible_probe; then
    DJ_TASKS_AXI_COMPATIBLE_MEMO=1
    return 0
  fi
  DJ_TASKS_AXI_COMPATIBLE_MEMO=0
  return 1
}

dj_tasks_axi_compatible_probe() {
  local parts major minor patch extra
  local min_major min_minor min_patch min_extra
  parts=$(dj_tasks_axi_version_parts) || return 1
  [ -n "$parts" ] || return 1
  ...
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$DJ_TASKS_AXI_MIN"
  ...
  if [ "$major" -gt "$min_major" ] ||
    ...
    dj_tasks_axi_update_has_archive_body && dj_tasks_axi_mv_has_multi_id
    return $?
  fi
  return 1
}

dj_tasks_axi_update_has_archive_body() {
  local output
  command -v tasks-axi >/dev/null 2>&1 || return 1
...
dj_backlog_backend_manual() {
  local config_dir=$1
  [ "$(dj_backlog_backend_value "$config_dir")" = manual ]
}

dj_tasks_axi_backend_available() {
  local config_dir=$1
  dj_backlog_backend_manual "$config_dir" && return 1
  dj_tasks_axi_compatible
}
