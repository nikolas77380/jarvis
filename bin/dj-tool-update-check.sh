#!/usr/bin/env bash
# dj-tool-update-check.sh - report watched tooling that has an update available,
# and tooling whose update is installed but not in effect.
#
# Usage:
#   dj-tool-update-check.sh [check]
#   dj-tool-update-check.sh arm
#   dj-tool-update-check.sh disarm
#   dj-tool-update-check.sh --help
#
...
# state/tool-updates.check.sh and binds its bytes with dj-check-register.sh, so
# the watcher dispatches it on its normal DJ_CHECK_INTERVAL cadence and turns
...
# DJ_TOOL_UPDATE_INTERVAL (default 900, 0 disables the gate, otherwise 60..86400)
# and stays silent in between. Each probe is bounded by
# DJ_TOOL_UPDATE_PROBE_SECS (default 5, valid 1..30) and a whole sweep by
# DJ_TOOL_UPDATE_BUDGET_SECS (default 20, valid 1..120).
#
...
# larger than DJ_CHECK_TIMEOUT (default 30, read from this check's own
...
DJ_HOME="${DJ_HOME:-${DJ_ROOT_OVERRIDE:-$(cd "$SCRIPT_DIR/.." && pwd)}}"
STATE="${DJ_STATE_OVERRIDE:-$DJ_HOME/state}"
CONFIG="${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}/watched-tools.json"
RECORD="$STATE/.tool-updates"
CHECK_ID=tool-updates
CHECK_SHIM="$STATE/$CHECK_ID.check.sh"
CHECK_TRUST="$STATE/$CHECK_ID.check-trust"
REGISTER_BIN="$SCRIPT_DIR/dj-check-register.sh"
RECORD_SCHEMA=dj-tool-updates-v1
...
usage() {
  cat <<'EOF'
Usage:
  dj-tool-update-check.sh [check]   report watched tools needing attention (silent when current)
  dj-tool-update-check.sh arm       write and register state/tool-updates.check.sh
  dj-tool-update-check.sh disarm    remove the check shim, its trust binding, and the record
  dj-tool-update-check.sh --help    print this help
...
die_usage() {
  printf 'dj-tool-update-check: %s\n' "$1" >&2
  usage >&2
  exit 2
}

INTERVAL=${DJ_TOOL_UPDATE_INTERVAL:-900}
case "$INTERVAL" in
  ''|*[!0-9]*)
    printf 'dj-tool-update-check: DJ_TOOL_UPDATE_INTERVAL must be 0 or a whole number from 60 to 86400\n' >&2
    exit 2
    ;;
esac
...
PROBE_SECS=${DJ_TOOL_UPDATE_PROBE_SECS:-5}
case "$PROBE_SECS" in
  ''|*[!0-9]*|0)
    printf 'dj-tool-update-check: DJ_TOOL_UPDATE_PROBE_SECS must be a whole number from 1 to 30\n' >&2
    exit 2
    ;;
esac
...
BUDGET_SECS=${DJ_TOOL_UPDATE_BUDGET_SECS:-20}
case "$BUDGET_SECS" in
  ''|*[!0-9]*|0)
    printf 'dj-tool-update-check: DJ_TOOL_UPDATE_BUDGET_SECS must be a whole number from 1 to 120\n' >&2
    exit 2
    ;;
esac
...
# The smallest bound a probe can be given, because dj_run_timed treats a
...
# dj_run_timed asks its runner for -k 1, so a probe that does not stop on TERM is
...
CHECK_TIMEOUT=${DJ_CHECK_TIMEOUT:-30}
...
# PROBE_MIN_SECS, because dj_run_timed treats a non-positive bound as no bound.
probe_bound() {
  local left
...
path_hits() {
  ...
  while IFS= read -r dir; do
    ...
    identity=$(dj_pr_file_identity "$candidate" 2>/dev/null) || identity=
...
probe_output() {
  local path=$1
  shift
  dj_run_timed "$(probe_bound)" "$path" "$@" 2>&1
}
...
git_probe() {
  ...
  dj_run_timed "$(probe_bound)" git -C "$repo" "$@"
}
...
action_check() {
  ...
  if [ -n "$FINDINGS" ]; then
    ...
    dj_cap_line_var "tool updates: $FINDINGS" "$MAX_LINE"
    line=$DJ_LINE_CAP_LINE
  fi
...
shim_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by dj-tool-update-check.sh - watched tool update poll shim.' \
    '# The watcher validates these bytes, then dispatches the trusted check script.' \
    "export DJ_HOME=$(printf '%q' "$home")" \
    "exec $(printf '%q' "$SCRIPT_DIR/dj-tool-update-check.sh") check"
}
...
shim_write() {
  ...
  device=$(dj_pr_file_device "$STATE") || return 1
  [ -n "$device" ] || return 1
  dj_pr_regular_destination_on_device_or_absent "$CHECK_SHIM" "$device" || return 1
  if [ -e "$CHECK_SHIM" ] && [ "$(dj_pr_file_mode "$CHECK_SHIM")" = 700 ] \
    && [ "$(cat "$CHECK_SHIM" 2>/dev/null)" = "$want" ]; then
    return 0
  fi
  tmp=$(umask 077; mktemp "$STATE/.dj-tool-updates-check.XXXXXX" 2>/dev/null) || return 1
  SHIM_WRITE_TMP=$tmp
  if ! printf '%s\n' "$want" > "$tmp" \
    || ! chmod 0700 "$tmp" \
    || ! dj_pr_private_file_valid "$tmp" 700 "$device"; then
    rm -f -- "$tmp"
    SHIM_WRITE_TMP=
    return 1
  fi
  if ! dj_pr_regular_destination_on_device_or_absent "$CHECK_SHIM" "$device" \
  ...
  SHIM_WRITE_TMP=
  dj_pr_private_file_valid "$CHECK_SHIM" 700 "$device"
}
...
# wakes jarvis about unauthenticated state checks. So the one rule after a
...
arm_rollback() {
  ...
  if [ -n "$ARM_BACKUP" ]; then
    ...
    ARM_BACKUP=
    if dj_custom_check_registered "$STATE" "$CHECK_ID"; then
      return 0
    fi
  fi
  rm -f -- "$CHECK_SHIM"
...
arm_interrupted() {
  arm_rollback
  printf 'dj-tool-update-check: arming was interrupted, so state/%s.check.sh is not armed\n' "$CHECK_ID" >&2
  exit 1
}
...
action_arm() {
  local want home
  if [ ! -f "$CONFIG" ]; then
    printf 'dj-tool-update-check: no watched tool registry at %s\n' "$CONFIG" >&2
    return 1
  fi
  if ! config_validate || ! config_announce_patterns_usable; then
    printf 'dj-tool-update-check: %s (%s)\n' "$CONFIG_PROBLEM" "$CONFIG" >&2
    return 1
  fi
  ...
  case "$DJ_HOME" in
    /*) home=$DJ_HOME ;;
    *)
      home=$(CDPATH='' cd -- "$DJ_HOME" 2>/dev/null && pwd -P) || {
        printf 'dj-tool-update-check: cannot resolve DJ_HOME %s\n' "$DJ_HOME" >&2
        return 1
      }
      ;;
  esac
  ...
  if [ -f "$CHECK_SHIM" ] && [ ! -L "$CHECK_SHIM" ]; then
    ARM_BACKUP=$(shim_backup) || {
      printf 'dj-tool-update-check: could not save the existing %s\n' "$CHECK_SHIM" >&2
      return 1
    }
  fi
  ...
  if ! shim_write "$want"; then
    trap - HUP INT TERM
    arm_rollback
    printf 'dj-tool-update-check: could not write %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  if ! DJ_HOME="$home" "$REGISTER_BIN" "$CHECK_ID" >/dev/null; then
    trap - HUP INT TERM
    arm_rollback
    printf 'dj-tool-update-check: could not register %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  trap - HUP INT TERM
