#!/usr/bin/env bash
# dj-bearings-snapshot.sh - compact, bounded, TOON-by-default bearings projection.
#
# A thin wrapper OVER the canonical bin/dj-fleet-snapshot.sh. It does not parse
# fleet state itself: it shells out to `dj-fleet-snapshot.sh --json`, projects that
...
# secondmate_landed roll-up (dj-fleet-snapshot.sh), so merges a secondmate managed -
...
#   -h,--help        usage
#
# Output contract: `dj-bearings.v1`. Read-only; no locks, no mutation, no reports.
set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FLEET="$SCRIPT_DIR/dj-fleet-snapshot.sh"
# shellcheck source=bin/dj-timeout-lib.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/dj-timeout-lib.sh"

# Bounds (overridable for tests / large fleets).
DJ_BEARINGS_LANDED=${DJ_BEARINGS_LANDED:-6}
DJ_BEARINGS_LANDED_PER_HOME=${DJ_BEARINGS_LANDED_PER_HOME:-$DJ_BEARINGS_LANDED}
DJ_BEARINGS_IN_FLIGHT=${DJ_BEARINGS_IN_FLIGHT:-20}
DJ_BEARINGS_DECISIONS=${DJ_BEARINGS_DECISIONS:-20}
DJ_BEARINGS_SECONDMATES=${DJ_BEARINGS_SECONDMATES:-20}
DJ_BEARINGS_GATES=${DJ_BEARINGS_GATES:-20}
DJ_BEARINGS_REPORTS=${DJ_BEARINGS_REPORTS:-20}
DJ_BEARINGS_RECORDED_PRS=${DJ_BEARINGS_RECORDED_PRS:-20}
DJ_BEARINGS_UNHEALTHY=${DJ_BEARINGS_UNHEALTHY:-20}
DJ_BEARINGS_PR_REPOS=${DJ_BEARINGS_PR_REPOS:-10}
DJ_BEARINGS_PR_LIMIT=${DJ_BEARINGS_PR_LIMIT:-20}
DJ_BEARINGS_PR_TIMEOUT=${DJ_BEARINGS_PR_TIMEOUT:-20}
case "$DJ_BEARINGS_PR_TIMEOUT" in ''|*[!0-9]*|0) DJ_BEARINGS_PR_TIMEOUT=20 ;; esac
validate_bound() {  # <name> <value>
  case "$2" in ''|*[!0-9]*|0) echo "dj-bearings-snapshot: $1 must be a positive integer" >&2; exit 2 ;; esac
}
validate_bound DJ_BEARINGS_LANDED "$DJ_BEARINGS_LANDED"
validate_bound DJ_BEARINGS_LANDED_PER_HOME "$DJ_BEARINGS_LANDED_PER_HOME"
validate_bound DJ_BEARINGS_IN_FLIGHT "$DJ_BEARINGS_IN_FLIGHT"
validate_bound DJ_BEARINGS_DECISIONS "$DJ_BEARINGS_DECISIONS"
validate_bound DJ_BEARINGS_SECONDMATES "$DJ_BEARINGS_SECONDMATES"
validate_bound DJ_BEARINGS_GATES "$DJ_BEARINGS_GATES"
validate_bound DJ_BEARINGS_REPORTS "$DJ_BEARINGS_REPORTS"
validate_bound DJ_BEARINGS_RECORDED_PRS "$DJ_BEARINGS_RECORDED_PRS"
validate_bound DJ_BEARINGS_UNHEALTHY "$DJ_BEARINGS_UNHEALTHY"
validate_bound DJ_BEARINGS_PR_REPOS "$DJ_BEARINGS_PR_REPOS"
validate_bound DJ_BEARINGS_PR_LIMIT "$DJ_BEARINGS_PR_LIMIT"

usage() {
  cat <<'EOF'
usage: dj-bearings-snapshot.sh [--json] [--include-prs] [--fields <list>]
...
Compact bearings projection over dj-fleet-snapshot.sh. TOON by default.
...
landed merges this home's Done with registered secondmate homes' Done, bounded by
  a per-home cap (DJ_BEARINGS_LANDED_PER_HOME) and an overall cap (DJ_BEARINGS_LANDED),
...
Raise DJ_BEARINGS_PR_LIMIT to expand per-repository open-PR results.
EOF
}
...
command -v jq >/dev/null 2>&1 || { echo "dj-bearings-snapshot: jq not found" >&2; exit 1; }
...
"$SCRIPT_DIR/dj-afk-return.sh" guard || exit $?

NOW=${DJ_BEARINGS_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ "$ALL_LANDED" = 1 ] || [ "$ALL_SECONDMATES" = 1 ]; then
  if [ "$ALL_LANDED" = 1 ]; then
    SNAP=$(DJ_SNAPSHOT_NOW="$NOW" DJ_SNAPSHOT_SECONDMATES=0 DJ_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=0 "$FLEET" --json) || exit $?
  else
    SNAP=$(DJ_SNAPSHOT_NOW="$NOW" DJ_SNAPSHOT_SECONDMATES=0 "$FLEET" --json) || exit $?
  fi
else
  SNAP=$(DJ_SNAPSHOT_NOW="$NOW" "$FLEET" --json) || exit $?
fi
HOME_LABEL=$(printf '%s' "$SNAP" | jq -er '.dj_home | strings | split("/") | (.[-2:] | join("/"))') \
  || { echo "dj-bearings-snapshot: invalid canonical snapshot" >&2; exit 1; }
...
# bin/dj-timeout-lib.sh owns the bound itself.
gh_bounded() {  # <args...>
  dj_run_timed "$DJ_BEARINGS_PR_TIMEOUT" \
...
EOF
    ...
    pr_fetch_limit=$((DJ_BEARINGS_PR_LIMIT + 1))
    for repo in $repos; do
      if [ "$ALL_PR_REPOS" != 1 ] && [ "$nrepos" -ge "$DJ_BEARINGS_PR_REPOS" ]; then break; fi
      nrepos=$((nrepos + 1))
      ...
      [ -n "$out" ] || out='[]'
      repo_result=$(printf '%s' "$out" | jq --arg repo "$repo" --argjson limit "$DJ_BEARINGS_PR_LIMIT" '
        [ .[] | {
          num:(.number|tostring),
          repo:$repo,
          task:(if (.headRefName // "" | startswith("dj/")) then (.headRefName | ltrimstr("dj/")) else "-" end),
          url:(.url // "-"),
      ...
      [ "$returned" -gt "$DJ_BEARINGS_PR_LIMIT" ] && ncapped=$((ncapped + 1))
      npr=$((npr + cnt))
...
# --- projection: canonical snapshot -> dj-bearings.v1 model (JSON) ----------
...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  --arg prs "$PR_STATUS" \
  --arg fields "$FIELDS" \
  --argjson landed_n "$DJ_BEARINGS_LANDED" \
  --argjson landed_per_home_n "$DJ_BEARINGS_LANDED_PER_HOME" \
  --argjson in_flight_n "$DJ_BEARINGS_IN_FLIGHT" \
  --argjson decisions_n "$DJ_BEARINGS_DECISIONS" \
  --argjson secondmates_n "$DJ_BEARINGS_SECONDMATES" \
  --argjson gates_n "$DJ_BEARINGS_GATES" \
  --argjson reports_n "$DJ_BEARINGS_REPORTS" \
  --argjson recorded_prs_n "$DJ_BEARINGS_RECORDED_PRS" \
  --argjson unhealthy_n "$DJ_BEARINGS_UNHEALTHY" \
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (if (($snap.secondmate_current.truncated // 0) > 0) then {surface:("registered secondmates omitted by snapshot bound: \($snap.secondmate_current.truncated)"), reveal:"raise DJ_SNAPSHOT_SECONDMATES"} else empty end),
        (if $snap.secondmate_current.registry.input_truncated == true then {surface:"secondmate registry input truncated by bounded read", reveal:"raise DJ_SNAPSHOT_REGISTRY_LINES or DJ_SNAPSHOT_REGISTRY_BYTES"} else empty end),
        (if $snap.secondmate_current.registry.records_truncated == true then {surface:"secondmate registry records omitted by bounded read", reveal:"raise DJ_SNAPSHOT_REGISTRY_RECORDS"} else empty end),
        ...
        (([($snap.secondmate_current.records // [])[] | select(.parent_event.activity_scan.input_truncated == true or .parent_event.activity_scan.retained_truncated == true)] | length) as $n | if $n > 0 then {surface:("secondmate parent activity evidence truncated for \($n) record(s)"), reveal:"raise DJ_SNAPSHOT_PARENT_ACTIVITY_LINES, DJ_SNAPSHOT_PARENT_ACTIVITY_BYTES, or DJ_SNAPSHOT_PARENT_ACTIVITIES"} else empty end),
        ...
        (if $include_prs == 1 and $pr_rows_capped > 0 then {surface:("candidate_prs showing \($candidate_prs | length) of at least \($pr_rows_min_total); capped in \($pr_rows_capped) repo(s)"), reveal:"raise DJ_BEARINGS_PR_LIMIT"} else empty end),
        (if $include_prs == 1 then empty else {surface:"live PR discovery + checks", reveal:"--include-prs"} end) ]) }
') || { echo "dj-bearings-snapshot: projection failed" >&2; exit 1; }
...
') || { echo "dj-bearings-snapshot: TOON rendering failed" >&2; exit 1; }
printf '%s\n' "$TOON"
