#!/usr/bin/env bash
# dj-session-start.sh - one command for the whole session start.
#
...
# dj-bootstrap.sh, then separately read data/projects.md, data/secondmates.md,
# data/captain.md, data/captain-shared.md, data/learnings.md, then run
# dj-lock.sh, dj-wake-drain.sh, then read data/backlog.md, every state/*.meta,
...
#
# COMPOSITION, NOT DUPLICATION: this script calls dj-lock.sh, dj-bootstrap.sh,
# dj-wake-drain.sh, and dj-startup-network.sh as real subprocesses and prints
...
# standalone with unchanged default behavior - other flows (dj-bootstrap.sh
# install <tools> after consent, /updatejarvis, the afk daemon, existing
# tests) still call them directly. The one seam this script needed -
# bootstrap running its detect-only diagnostics without its six mutating
# sweeps - is an opt-in DJ_BOOTSTRAP_DETECT_ONLY=1 flag on dj-bootstrap.sh
...
# entire DJ_SESSION_START_TIMEOUT and truncate the digest, so a slow network
...
# harvested at step 7 without ever blocking on it. bin/dj-startup-network.sh
# owns that stage and its safety argument; bin/dj-bootstrap.sh remains the owner
...
# go dark. So on refusal, bootstrap still runs (in DJ_BOOTSTRAP_DETECT_ONLY=1
...
#     reporting surfaces (bin/dj-bearings-snapshot.sh, /ahoy), and at startup it
...
#     DJ_SESSION_START_QUEUED_LIMIT, default 20. Anything it omits is disclosed
#     with an exact remainder count and the command that shows the rest, so a
#     deep queue costs a counter rather than kilobytes.
#     (This replaces DJ_SESSION_START_BACKLOG_LIMIT, which bounded the whole
...
# truly needed.
#
# STATUS TAILS: DJ_SESSION_START_STATUS_TAIL bounds how many lines each task's
# tail prints, and bin/dj-line-cap-lib.sh bounds how long each of those lines
...
# rather than current state - bin/dj-crew-state.sh owns current state.
#
# RUNTIME BOUND: the digest is now executed on a session-open hook (see
# bin/dj-sessionstart-run.sh), which blocks session initialization while it
...
# still runs as ONE bounded child of this script (DJ_SESSION_START_TIMEOUT,
...
# records its progress in DJ_SESSION_START_STAGE_FILE, which is also the flag
...
#
# Usage: dj-session-start.sh [--reemit] [--source <source>]
...
#             ownership must be re-verified rather than assumed: dj-lock.sh already treats a lock
...
#             dj-sessionstart-run.sh. A genuine `startup` that owns the active
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --source=*)
      ...
      ;;
    -h|--help)
      sed -n '2,/^set -u$/p' "$SCRIPT_DIR/dj-session-start.sh" | sed 's/^# \{0,1\}//; $d'
      exit 0
      ;;
    *)
      printf 'dj-session-start: unknown argument: %s\n' "$1" >&2
      printf 'usage: dj-session-start.sh [--reemit] [--source <source>]\n' >&2
      exit 2
      ;;
  esac
done
...
stage() {  # <stage-name>: breadcrumb for the parent's truncation banner
  [ -n "${DJ_SESSION_START_STAGE_FILE:-}" ] || return 0
  printf '%s\n' "$1" > "$DJ_SESSION_START_STAGE_FILE" 2>/dev/null || true
}

# shellcheck source=bin/dj-timeout-lib.sh
. "$SCRIPT_DIR/dj-timeout-lib.sh"
# shellcheck source=bin/dj-session-lock-lib.sh
. "$SCRIPT_DIR/dj-session-lock-lib.sh"

if [ -z "${DJ_SESSION_START_STAGE_FILE:-}" ]; then
  SESSION_START_BUDGET=${DJ_SESSION_START_TIMEOUT:-120}
  ...
  SESSION_START_STAGE_FILE=$(mktemp "${TMPDIR:-/tmp}/dj-session-start-stage.XXXXXX" 2>/dev/null) || SESSION_START_STAGE_FILE=
  ...
  if [ "$REEMIT" -eq 1 ]; then
    if [ -n "$SESSION_SOURCE" ]; then
      dj_run_timed "$SESSION_START_BUDGET" \
        env DJ_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
        "$SCRIPT_DIR/dj-session-start.sh" --reemit --source "$SESSION_SOURCE"
    else
      dj_run_timed "$SESSION_START_BUDGET" \
        env DJ_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
        "$SCRIPT_DIR/dj-session-start.sh" --reemit
    fi
  elif [ -n "$SESSION_SOURCE" ]; then
    dj_run_timed "$SESSION_START_BUDGET" \
      env DJ_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
      "$SCRIPT_DIR/dj-session-start.sh" --source "$SESSION_SOURCE"
  else
    dj_run_timed "$SESSION_START_BUDGET" \
      env DJ_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
      "$SCRIPT_DIR/dj-session-start.sh"
  fi
  SESSION_START_RC=$?
  if [ "$SESSION_START_RC" -eq 124 ]; then
    ...
    printf '●    %s\n' "${SESSION_START_PENDING% }"
    printf '●  Rerun bin/dj-session-start.sh now to finish taking the helm. If it truncates\n'
    printf '●  again, raise DJ_SESSION_START_TIMEOUT and report the slow stage - a stage that\n'
...
PRIMARY_HARNESS=$("$SCRIPT_DIR/dj-harness.sh" 2>/dev/null || printf unknown)
...
# the backlog listing and again inside the dj-bootstrap.sh child, which reports
# an incompatible build as MISSING. Computing it once and handing it to that
# child collapses six subprocesses to three. dj-tasks-axi-lib.sh owns both reuse
# layers and the one-hop consumption rule that keeps the verdict out of any
# agent's environment.
if dj_tasks_axi_compatible; then TASKS_AXI_COMPATIBLE=1; else TASKS_AXI_COMPATIBLE=0; fi

STATUS_TAIL=${DJ_SESSION_START_STATUS_TAIL:-5}
case "$STATUS_TAIL" in ''|*[!0-9]*) STATUS_TAIL=5 ;; esac
QUEUED_LIMIT=${DJ_SESSION_START_QUEUED_LIMIT:-20}
...
# meaningful for every one of these files (captain.md absent = jarvis
    ...
    END {
      ...
      if (in_flight + queued_total + done_total == 0) {
        ...
        if (plain_total > plain_shown) {
          printf "(%d more queued - raise DJ_SESSION_START_QUEUED_LIMIT or read data/backlog.md for the rest)\n", plain_total - plain_shown
        }
      }
    }
  ' "$path"
...
print_backlog_compact() {
  ...
  if [ -f "$path" ]; then
    if [ -s "$path" ]; then
      ...
      elif dj_backlog_backend_manual "$CONFIG"; then
...
print_status_tail() {
  ...
  printf 'status tail (last %s line(s), each capped at %s characters, wake-EVENT history, not current state; full log: %s):\n' \
    "$STATUS_TAIL" "$DJ_LINE_CAP_DEFAULT" "$status"
...
agents_baseline_drifted() {  # <rebuilding-session-pid>
  ...
  current_hash=$(hash_file_sha256 "$DJ_ROOT/AGENTS.md" 2>/dev/null || true)
...
# working Jarvis delivery path arrive here. Claude fresh-reads on reset, and
...
print_agents_refresh_if_required() {  # <rebuilding-session-pid>
  ...
  if [ -f "$DJ_ROOT/AGENTS.md" ]; then
    cat <<'EOF'
The complete on-disk AGENTS.md below supersedes the instruction copy this session
started with. Apply it as the current Jarvis instruction contract.

EOF
    cat "$DJ_ROOT/AGENTS.md"
  else
...
if [ "$REEMIT" -eq 0 ] && [ "$SESSION_SOURCE" = startup ]; then
  AGENTS_START_HASH=$(hash_file_sha256 "$DJ_ROOT/AGENTS.md" 2>/dev/null || true)
fi

if [ "$REEMIT" -eq 1 ]; then
  section "SESSION START (CONTEXT RE-EMIT) - $DJ_HOME"
  ...
  section "SESSION START - $DJ_HOME"
fi
# --- 1. lock -----------------------------------------------------------
stage lock
subsection "LOCK"
LOCK_OUT=$("$SCRIPT_DIR/dj-lock.sh" 2>&1)
LOCK_RC=$?
printf '%s\n' "$LOCK_OUT"
READ_ONLY=0
...
REBUILDING_SESSION_PID=$(dj_harness_ancestry_pid 2>/dev/null || true)
...
if [ "$READ_ONLY" -eq 0 ]; then
  ...
  dj_trace_context_session_start "$CONFIG" "$STATE/.trace-context-effective"
  ...
  "$SCRIPT_DIR/dj-startup-network.sh" start \
    --locked "$NETWORK_STAGE_LOCKED" --harvest-pid $$ >/dev/null 2>&1 || true
fi

# --- 2. bootstrap --------------------------------------------------------
# DJ_BOOTSTRAP_NETWORK=skip on every path: bootstrap's own network half is what
...
if [ "$READ_ONLY" -eq 1 ]; then
  BOOT_OUT=$(DJ_BOOTSTRAP_DETECT_ONLY=1 DJ_BOOTSTRAP_NETWORK=skip \
    DJ_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" "$SCRIPT_DIR/dj-bootstrap.sh" 2>&1)
elif [ "$REEMIT" -eq 1 ]; then
  BOOT_OUT=$(DJ_BOOTSTRAP_DETECT_ONLY=1 DJ_BOOTSTRAP_LOCKED=1 DJ_BOOTSTRAP_NETWORK=skip \
    DJ_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" "$SCRIPT_DIR/dj-bootstrap.sh" 2>&1)
else
  BOOT_OUT=$(
    "$SCRIPT_DIR/dj-herdr-session-cleanup.sh" 2>&1 || true
    DJ_BOOTSTRAP_NETWORK=skip DJ_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" \
      "$SCRIPT_DIR/dj-bootstrap.sh" 2>&1
  )
fi
if [ -n "$BOOT_OUT" ]; then
  printf '%s\n' "$BOOT_OUT"
...
# The drain also runs dj-guard.sh internally on the locked path, so the
...
# dj-guard.sh directly with non-mutating advisory text, so the same alarms
...
if [ "$READ_ONLY" -eq 1 ]; then
  ...
  GUARD_OUT=$(DJ_GUARD_READ_ONLY=1 "$SCRIPT_DIR/dj-guard.sh" 2>&1)
  [ -n "$GUARD_OUT" ] && printf '%s\n' "$GUARD_OUT"
else
  INACTIVE_OUT=$(DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/dj-inactive-reconcile.sh" scan --startup 2>&1) || INACTIVE_OUT=
  ...
  if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
    DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" "$SCRIPT_DIR/dj-lease.sh" sweep 2>/dev/null || true
    BRANCH_REPLAY_OUT=$(DJ_HOME="$DJ_HOME" DJ_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/dj-branch-outcome.sh" startup-replay 2>&1) || BRANCH_REPLAY_OUT=
    if [ -n "$BRANCH_REPLAY_OUT" ]; then
      printf '%s\n' "$BRANCH_REPLAY_OUT"
    fi
  fi
  DRAIN_OUT=$("$SCRIPT_DIR/dj-wake-drain.sh" 2>&1)
  if [ -n "$DRAIN_OUT" ]; then
    printf '%s\n' "$DRAIN_OUT"
  else
...
if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
  PI_EXT="$DJ_ROOT/.pi/extensions/dj-primary-pi-watch.ts"
  PI_TURNEND_EXT="$DJ_ROOT/.pi/extensions/dj-primary-turnend-guard.ts"
  ...
  PI_WATCH_VERSION=$(dj_pi_extension_version "$PI_EXT" || printf '')
  PI_TURNEND_VERSION=$(dj_pi_extension_version "$PI_TURNEND_EXT" || printf '')
  if ! dj_pi_extension_loaded "$PI_WATCH_MARKER" "$PI_WATCH_VERSION" "$PI_LOCK" \
    || ! dj_pi_extension_loaded "$PI_TURNEND_MARKER" "$PI_TURNEND_VERSION" "$PI_LOCK"; then
...
"$SCRIPT_DIR/dj-supervision-instructions.sh" \
...
Go to a source directly only when:
  ...
  - the NETWORK CHECKS section reported its checks still IN PROGRESS and this
    turn needs their verdict (bin/dj-startup-network.sh report),
...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    if dj_backend_target_exists "$backend" "${target:-$window}" "dj-$id"; then
...
# Public commitments made through the myjarvis relay. A promise to reply in a
# public thread must survive compaction and restart, so it is surfaced from disk
# here rather than from conversation memory. dj-public-followup-lib.sh owns both
# gates: a home that never opted into the relay runs one [ -f ] test, prints no
# subsection, and never reaches dj-public-followup.sh.
if dj_pf_relay_active "$DJ_HOME" \
  && { dj_pf_has_registrations "$STATE" || dj_pf_has_events "$STATE"; }; then
  PUBLIC_FOLLOWUP=$("$SCRIPT_DIR/dj-public-followup.sh" pending 2>/dev/null) || PUBLIC_FOLLOWUP=
  if [ -n "$PUBLIC_FOLLOWUP" ]; then
    ...
    printf 'Reconcile terminal results with %s/bin/dj-public-followup.sh consume, then deliver a ready one with\n' "$DJ_ROOT"
    printf '%s/bin/dj-public-followup.sh deliver <id>. Hand a delivered loop on with rechain, or close it with\n' "$DJ_ROOT"
    printf '%s/bin/dj-public-followup.sh retire <id> --reason "...". Load djx-respond for the procedure.\n' "$DJ_ROOT"
  fi
fi
...
if [ "$READ_ONLY" -eq 1 ]; then
  ...
  "$SCRIPT_DIR/dj-startup-network.sh" harvest --pid $$ 2>&1 || true
fi
