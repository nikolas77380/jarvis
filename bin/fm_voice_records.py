#!/usr/bin/env python3
"""dj_voice_records.py - what the voice agent is allowed to know, and how it hands work over.

The voice agent answers status questions from jarvis's durable records and
...
another region, and the other writes to jarvis's wake queue.
...
nothing in a full status answer named a customer. tests/dj-voice-relay.test.sh
...
Two readings here treat that differently, on purpose.
  ...
  answer, and it is the same meaning bin/dj-inbox.sh gives "workers" in the human
...
the model cannot present it as current truth. bin/dj-crew-state.sh remains the
...
bin/dj-inbox.sh `status` is the human rendering of the same records and stays
...
Usage:
  dj_voice_records.py status [--home <dir>] [--scope counts|full]
  dj_voice_records.py queue <text>... [--home <dir>]
...
# bin/dj-brief.sh states the first six to every crewmate and bin/dj-classify-lib.sh
...
def default_home():
    """Return the operational home, matching bin/dj-inbox.sh's resolution."""
    env = os.environ.get("DJ_HOME")
    if env:
        return env
...
def state_dir(home):
    """Return the runtime state directory, resolved as bin/dj-inbox.sh resolves it.

    dj-inbox.sh reads ${DJ_STATE_OVERRIDE:-$DJ_HOME/state}, and the handover
    below queues through dj-inbox.sh with the ambient environment. A reader that
    ...
    """
    override = os.environ.get("DJ_STATE_OVERRIDE")
    if override:
        return override
...
def data_dir(home):
    ...
    Every script that sets DJ_DATA_OVERRIDE for a child sets DJ_STATE_OVERRIDE
    ...
    wrong.
    """
    override = os.environ.get("DJ_DATA_OVERRIDE")
    if override:
        return override
...
def config_dir(home):
    ...
    override = os.environ.get("DJ_CONFIG_OVERRIDE")
    if override:
        return override
...
def _last_event(state_dir, task_id):
    ...
    bin/dj-classify-lib.sh remains the owner of status-verb normalization.
...
def queue_request(text, home=None, root=None):
    ...
    return {
        ...
        "note_id": note_id,
        "queued_text": body,
        "handover": "Jarvis now owns this request and will pick it up at "
...
def main(argv):
    parser = argparse.ArgumentParser(
        prog="dj_voice_records.py", description=__doc__.splitlines()[0],
    ...
    queue = sub.add_parser("queue", help="hand a request to jarvis")
    ...
    try:
        ...
        sys.stderr.write("dj_voice_records: {}\n".format(exc))
        return 2
