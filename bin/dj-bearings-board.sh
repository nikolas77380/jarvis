...
#
# Usage:
#   dj-bearings-board.sh build <data.json>
...
# schema=dj-bearings-board.v1 and every renderer-consumed field must satisfy
# the dj-bearings-board.v1 types and item invariants below. Every fleet row and
...
#
# DJ_BEARINGS_BOARD_TEMPLATE overrides the shipped template path (tests only).
set -eu
...
TEMPLATE="${DJ_BEARINGS_BOARD_TEMPLATE:-$SCRIPT_DIR/../.agents/skills/bearings/assets/board-template.html}"
PLACEHOLDER='__FM_BEARINGS_BOARD_DATA__'
BOARD_SCHEMA=dj-bearings-board.v1
