...
# (bin/dj-watch.sh) and the away-mode daemon (bin/dj-supervise-daemon.sh) so the
# overlapping triage policy lives in one place instead of two copies that can
# drift apart.
#
# Most functions are pure, side-effect-free reads of status files: each takes
# what it needs as arguments and touches no globals beyond the optional
# DJ_CAPTAIN_RE override. Consumers layer their own dedup/marker state on top (the
# daemon keeps its escalation-digest seen-markers; the watcher keeps its .seen-*
# signatures).
#
# There are three documented exceptions. The absorb classification
# (crew_absorb_class and its working/paused wrappers) is NOT a pure status-file
# read: it reuses bin/dj-crew-state.sh, which may make a bounded no-mistakes call,
...
# Directory of this library, used to locate the sibling dj-crew-state.sh reader.
...
DJ_CREW_STATE_BIN="${DJ_CREW_STATE_BIN:-$_FM_CLASSIFY_LIB_DIR/dj-crew-state.sh}"

# dj_run_timed, the shared hard bound the worktree write probe below puts around
# its one filesystem walk. bin/dj-timeout-lib.sh owns bounded execution for this
...
. "$_FM_CLASSIFY_LIB_DIR/dj-timeout-lib.sh"
[ "$_fm_classify_nounset" = on ] || set +u
unset _fm_classify_nounset

# Captain-relevant status verbs. A status line carrying any of these is work
# jarvis must see. Lines without these verbs are no-verb signals: the watcher
# absorbs them only with positive provably-working evidence, while the daemon uses
# its away-mode classification. DJ_CAPTAIN_RE overrides the whole set when a home
...
DJ_CLASSIFY_CAPTAIN_RE_DEFAULT='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'

# The deliberate-external-wait verb. A crew (or jarvis steering it) appends
#   paused: <reason>
...
# `blocked:` (stuck, jarvis must help) an idle `paused:` pane is EXPECTED, so
...
# drift between the two consumers. DJ_CLASSIFY_PAUSED_VERB overrides it.
DJ_CLASSIFY_PAUSED_VERB_DEFAULT='paused'

# Bounded re-surface cadence for a declared pause or a verified captain hold.
# Far longer than the wedge threshold (DJ_STALE_ESCALATE_SECS, default 240s), it
# avoids nagging a deliberate wait while ensuring a forgotten hold cannot rot
# invisibly - it re-surfaces once for a recheck every window. One hour by default;
# both consumers read DJ_PAUSE_RESURFACE_SECS with this default so the cadence has
# one owner.
# shellcheck disable=SC2034 # Read by the watcher and daemon (dj-watch.sh, dj-supervise-daemon.sh), not this lib.
DJ_PAUSE_RESURFACE_SECS_DEFAULT=3600

# The resolution verb and durable-backlog-transfer verb that CLOSE a keyed
# status decision opened by needs-decision or blocked. See status_open_decisions
# below for the status-fold contract. The transfer verb is written only after
# dj-captain-hold.sh has verified the corresponding captain-held backlog item.
DJ_CLASSIFY_RESOLVE_VERB_DEFAULT='resolved'
DJ_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT='captain-held'
...
status_is_captain_relevant() {
  ...
  verb=$(status_line_verb "$line")
  case "$verb" in
    working|resolved|captain-held|"${DJ_CLASSIFY_PAUSED_VERB:-$DJ_CLASSIFY_PAUSED_VERB_DEFAULT}")
      return 1
      ;;
  esac
  if [ -z "${DJ_CAPTAIN_RE+x}" ]; then
    case "$verb" in
      done|needs-decision|blocked|failed) return 0 ;;
    esac
  fi
  printf '%s' "$line" | grep -qiE "${DJ_CAPTAIN_RE:-$DJ_CLASSIFY_CAPTAIN_RE_DEFAULT}"
}

# 0 if a status line's leading verb is the pause verb (paused: <reason>). A pure
# read of the line itself, so the daemon's classify_stale can reuse the last line
# it already read without a dj-crew-state.sh call. Matches only the verb before the
# first colon, so a reason mentioning "paused" elsewhere does not false-match.
status_is_paused() {  # <status-line>
  local line=$1 verb
  [ -n "$line" ] || return 1
  verb=$(status_line_verb "$line")
  [ "$verb" = "${DJ_CLASSIFY_PAUSED_VERB:-$DJ_CLASSIFY_PAUSED_VERB_DEFAULT}" ]
}
...
status_is_captain_held() {  # <status-line>
  ...
  [ -n "$line" ] || return 1
  verb=$(status_line_verb "$line")
  [ "$verb" = "${DJ_CLASSIFY_CAPTAIN_HELD_VERB:-$DJ_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}" ]
}
...
# once pause_state_class has admitted the wait (dj-watch.sh owns which liveness
...
# Who WRITES the closing line is owned elsewhere: the answering jarvis closes
# at answer time through dj-send's --resolve-key (bin/dj-send.sh header), and a
# worker self-closes only a blocker that cleared without an answer (bin/dj-brief.sh
...
# including the "[corr=<16 hex>]" form bin/dj-secondmate-report.sh writes. It
# does not cover the UNBRACKETED token that bin/dj-pending-reply-lib.sh writes
# (dj_pending_reply_corr_token), which a secondmate answering a marked request
# echoes on its parent status line ahead of the key tag (bin/dj-brief.sh), so a
...
#
# The token grammar is OWNED by bin/dj-pending-reply-lib.sh
# (dj_pending_reply_corr_token, DJ_PENDING_REPLY_CORR_RE). That library sources
# this one, so it cannot be sourced back here; the pattern below is a deliberate
# second statement of the SHAPE alone, and tests/dj-classify-corr-token.test.sh
...
# only what a jarvis library actually writes costs one more line here each
...
# variable, the same way bin/dj-secondmate-report.sh validates the id it is
...
DJ_CLASSIFY_RESERVED_KEY_PREFIXES_DEFAULT='pending-reply-'
...
_fm_decision_key_transition_allowed() {  # <key> <note>
  ...
  for prefix in ${DJ_CLASSIFY_RESERVED_KEY_PREFIXES:-$DJ_CLASSIFY_RESERVED_KEY_PREFIXES_DEFAULT}; do
    case "$key" in
      "$prefix"*)
        case "$note" in
          "$prefix"*:*) return 0 ;;
...
# the file, no globals beyond the optional DJ_CLASSIFY_RESOLVE_VERB override. This
...
# before any read - a cheap builtin, unlike dj_wake_latest_event's O_NOFOLLOW
...
status_open_decisions() {  # <status-file>
  ...
  resolve=${DJ_CLASSIFY_RESOLVE_VERB:-$DJ_CLASSIFY_RESOLVE_VERB_DEFAULT}
  held=${DJ_CLASSIFY_CAPTAIN_HELD_VERB:-$DJ_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
...
# (dj-captain-hold.sh complete writes it only after verifying that task), so the
...
# contradiction between the two records - see dj-captain-hold.sh's `diverged`.
#
...
# DJ_OPEN_DECISIONS_FOLD_VERSION must be bumped whenever
...
# prior cursor or the new one, never a partial one. bin/dj-wake-drain.sh calls
...
DJ_OPEN_DECISIONS_FOLD_VERSION=5
...
_fm_status_file_size() {  # <status-file>
  local f=$1
  if [ -n "${DJ_STATUS_SIZE_READER:-}" ]; then
    "$DJ_STATUS_SIZE_READER" "$f"
    return
  fi
...
_fm_status_read_span() {  # <status-file> <start-offset> <byte-length>
  ...
  if [ -n "${DJ_STATUS_SPAN_READER:-}" ]; then
    "$DJ_STATUS_SPAN_READER" "$f" "$start" "$length"
    return
  fi
...
status_open_decisions_incremental() {  # <status-file> [<captured-end-offset>]
  ...
  if [ -n "${cursor_data:-}" ]; then
      ...
      case "$first" in
        version=*)
          ...
          [ "$version" = "$DJ_OPEN_DECISIONS_FOLD_VERSION" ] || version=''
          rest=${cursor_data#*$'\n'}
          offset_line=${rest%%$'\n'*}
          case "$offset_line" in
  ...
  if [ "$offset" -lt "$size" ]; then
    ...
    [ -n "${DJ_OPEN_DECISIONS_READ_PROBE:-}" ] \
      && printf '%s\t%s\n' "$f" "$chunk_size" >> "$DJ_OPEN_DECISIONS_READ_PROBE"
    resolve=${DJ_CLASSIFY_RESOLVE_VERB:-$DJ_CLASSIFY_RESOLVE_VERB_DEFAULT}
    held=${DJ_CLASSIFY_CAPTAIN_HELD_VERB:-$DJ_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
    while IFS= read -r line || [ -n "$line" ]; do
      open=$(_fm_decision_fold_line "$open" "$line" "$resolve" "$held")
  ...
  if [ "$cursor_dirty" -eq 1 ]; then
    ...
    {
      printf 'version=%s\n' "$DJ_OPEN_DECISIONS_FOLD_VERSION"
      printf 'offset=%s\n' "$offset"
      printf 'ident=%s\n' "$cur_ident"
...
status_open_decisions_cursor_offset() {  # <status-file>
  ...
  if [ -n "${DJ_STATUS_CURSOR_SNAPSHOT_FILE:-}" ]; then
    {
      printf 'version=%s\n' "$DJ_OPEN_DECISIONS_FOLD_VERSION"
      printf 'offset=%s\n' "$offset"
      printf 'ident=%s\n' "$cur_ident"
      if [ -n "$open" ]; then printf '%s' "$open"; fi
    } > "$DJ_STATUS_CURSOR_SNAPSHOT_FILE" || return 1
  fi
  printf '%s' "$offset"
}
...
# phase outrank a structured home snapshot or dj-crew-state result.
_fm_status_open_activities_stream() {
  local line verb key note resolve held open='' stripped pause
  resolve=${DJ_CLASSIFY_RESOLVE_VERB:-$DJ_CLASSIFY_RESOLVE_VERB_DEFAULT}
  held=${DJ_CLASSIFY_CAPTAIN_HELD_VERB:-$DJ_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
  pause=${DJ_CLASSIFY_PAUSED_VERB:-$DJ_CLASSIFY_PAUSED_VERB_DEFAULT}
  while IFS= read -r line || [ -n "$line" ]; do
    stripped=${line//[[:space:]]/}
    [ -n "$stripped" ] || continue
    verb=$(status_line_verb "$line")
...
# "<session>:dj-<id>" form when no metadata state is available.
window_to_task() {
  local w=$1 state=${2:-${STATE:-${DJ_STATE_OVERRIDE:-}}} meta mw mt t
  if [ -n "$state" ]; then
...
# from bin/dj-crew-state.sh's one authoritative current-state line
...
# One dj-crew-state.sh read serves BOTH absorb reasons at once. Reading the state
# authoritatively (not the status log) is what keeps run-step precedence: a crew
# that appended paused: but then STARTED a run reports working, never paused.
# NOT a pure read: dj-crew-state.sh may make a bounded no-mistakes call, so callers
# run it only on no-verb signal and first-sighting stale paths, never every wake.
# DJ_CREW_STATE_BIN lets tests stub the verdict.
crew_absorb_class() {  # <id>
  local id=$1 line state src
  [ -n "$id" ] || { printf 'none'; return; }
  line=$("$DJ_CREW_STATE_BIN" "$id" 2>/dev/null) || true
...
# without the crew doing any work - .git first, so jarvis's own read-only git
...
DJ_WORKTREE_WRITE_PRUNE=${DJ_WORKTREE_WRITE_PRUNE-'.git node_modules .venv venv __pycache__ .mypy_cache .pytest_cache .ruff_cache .tox target dist build .next .cache vendor'}
DJ_WORKTREE_WRITE_MAXDEPTH=${DJ_WORKTREE_WRITE_MAXDEPTH:-6}
...
# gets it too.
DJ_WORKTREE_WRITE_TIMEOUT=${DJ_WORKTREE_WRITE_TIMEOUT:-10}
...
#
# A kind=secondmate task records a provisioned jarvis home, not a code tree, and
...
# that outlives DJ_WORKTREE_WRITE_TIMEOUT is killed and reported as no evidence, so
...
crew_worktree_written_since() {  # <id> <state> <anchor-file>
  ...
  if [ -e "$wt/.dj-secondmate-home" ] || [ -L "$wt/.dj-secondmate-home" ]; then
    return 1
  fi
  read -r -a names <<< "$DJ_WORKTREE_WRITE_PRUNE"
  for name in ${names[@]+"${names[@]}"}; do
    [ "${#prune[@]}" -eq 0 ] || prune+=( -o )
    prune+=( -name "$name" )
  done
  bound=$DJ_WORKTREE_WRITE_TIMEOUT
  case "$bound" in ''|*[!0-9]*|0) bound=10 ;; esac
  if [ "${#prune[@]}" -gt 0 ]; then
    hit=$(dj_run_timed "$bound" find "$wt" -xdev -maxdepth "$DJ_WORKTREE_WRITE_MAXDEPTH" \
