#!/usr/bin/env bash
# dj-doc-audience-check.sh - validate the tracked documentation audience inventory.
#
# Usage:
#   bin/dj-doc-audience-check.sh
#   bin/dj-doc-audience-check.sh --root <repo> [--inventory <path>]
#
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Jarvis documentation audiences and local links.")
    ...
    try:
        ...
        print(f"dj-doc-audience-check: {exc}", file=sys.stderr)
        return 1
    print(f"dj-doc-audience-check: ok surfaces={surfaces} local_links={links}")
    return 0
