...
#
# A jarvis primary that delegates through a harness's own delegation,
# scheduling, or background-work tool creates work with no `state/<id>.meta` and
# no `data/<id>/brief.md`. Only `bin/dj-spawn.sh` writes that metadata, and
# untracked project work contributes nothing to the in-flight branch of
# bin/dj-supervision-lib.sh or bin/dj-turnend-guard.sh. So such work is not
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/dj-subagent-pretool-check.sh
#   bin/dj-subagent-pretool-check.sh --tool '<tool-name>'
#
...
#
# Exit/output contract (identical shape to bin/dj-cd-pretool-check.sh):
#   ALLOW - exit 0 and no output.
#   DENY - exit 2, a Claude-shaped deny object on stderr, and a Grok-shaped
#          deny object on stdout unless --claude was supplied.
#   INERT - not a genuine primary home (a crewmate/scout task worktree or a
#           non-jarvis repo): exit 0 with no output, exactly like ALLOW.
#   ESCAPE - DJ_ALLOW_SUBAGENT=1 in the environment allows deliberately.
...
# work, an agent, a schedule, or an isolated workspace that jarvis would not
...
# and starts nothing that could outlive the session or escape a jarvis
# guard. Denying them stops the primary tracking its own plan while granting no
# delegation power, and the deny text would tell it to run bin/dj-brief.sh for a
...
usage() {
  cat <<'EOF'
Usage: dj-subagent-pretool-check.sh [--tool <tool-name>] [--claude]
...
Fires only in a genuine jarvis primary home; it is a silent no-op in a
crewmate/scout task worktree or any non-jarvis repo, where a worker using
delegation tools is legitimate.
Exits 0 to allow and 2 to deny, naming the real crewmate dispatch path instead.
Set DJ_ALLOW_SUBAGENT=1 in the session environment to allow deliberately.
...
[ "${DJ_ALLOW_SUBAGENT:-}" != "1" ] || exit 0

SCRIPT_DIR=$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P) || exit 0
DJ_ROOT=${DJ_ROOT_OVERRIDE:-$(CDPATH='' cd -- "$SCRIPT_DIR/.." 2>/dev/null && pwd -P)} || exit 0
...
# turn-end guard do. dj_primary_scope_matches accepts a plain checkout or a
# marked secondmate home - both operate a fleet and must dispatch through it -
# and rejects a linked task worktree, which is the shape bin/dj-spawn.sh always
...
if [ -f "$DJ_ROOT/bin/dj-scout.sh" ]; then
  ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a scout goes to bin/dj-scout.sh "<question>" [project], while authorized ship work and its bounded research go to bin/dj-brief.sh then bin/dj-spawn.sh'
else
  ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/dj-brief.sh followed by bin/dj-spawn.sh for dispatched work'
fi

REASON="[subagent-dispatch] the jarvis primary dispatches through the fleet, not the harness's own delegation tools: work started that way has no durable fleet record, leaves every jarvis guard inert, and dies with this session. Instead, $ROUTE (blocked tool: $TOOL, delegation-shaped on \"$MATCHED\"). Launch the session with DJ_ALLOW_SUBAGENT=1 for a deliberate exception."
