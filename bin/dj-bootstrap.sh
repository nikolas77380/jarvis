...
#                 "BOOTSTRAP_INFO: nudged dj-<id> with '<message>'",
#                 "SECONDMATE_LIVENESS: secondmate <id>: skipped: <reason>|respawn failed after <cause>: <reason>",
#                 "SECONDMATE_HANDOFF: secondmate <id>: pending delivery: <n> item(s)",
#                 "FMX: X mode on ..." or "FMX: X mode off ...".
#          When a RUNNING local secondmate worktree is fast-forwarded to
#          jarvis's own current default-branch commit, that update is a
...
#          via DJ_HOME=<active-home> bin/dj-send.sh dj-<id> so meta resolves the
#          current route and the standard from-jarvis marker is applied. A
...
#          recovery-grade state owned by bin/dj-backend.sh's
#          dj_backend_agent_state: skipped distinguishes an existing ambiguous
...
#          unless DJ_BOOTSTRAP_VERBOSE_FACTS=1 requests BOOTSTRAP_INFO facts.
#          A TANGLE line means the jarvis primary checkout (DJ_ROOT) is stranded
...
#          EDITH_AXI_MIN below; the per-tool owners point there. An installed
...
#          edith-axi). A compatible tasks-axi default backend is silent.
...
#          their own.
#          X mode is OPTIONAL and inert unless DJ_HOME/.env has a non-empty
#          DJX_PAIRING_TOKEN. When opted in, bootstrap requires curl+jq, writes
#          the relay poll shim and 30s cadence config, and prints an FMX line.
#          Fleet sync fetches, fast-forwards safe default-branch states, reports
#          recovered and STUCK clone drift, and prunes gone local branches; it is
#          bounded by DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT when it is a non-empty
...
#          refresh relays any completed dj-fleet-sync.sh output before the
#          aggregate timeout skip line with timeout and elapsed seconds.
#          Set DJ_FLEET_PRUNE=0 to skip branch pruning during that refresh.
#          Set DJ_BOOTSTRAP_DETECT_ONLY=1 to skip the six MUTATING sweeps
#          (PR-check migration, secondmate_sync, secondmate_liveness_sweep,
#          secondmate_handoff_resume, x_mode_setup, fleet_sync) while still
#          printing every read-only detect line
#          above; the TANGLE line switches to advisory-only wording with no
#          checkout command. Used by
#          dj-session-start.sh's read-only path when another live session holds
...
#          additive.
#          Set DJ_BOOTSTRAP_NETWORK to split this run by whether a step talks to
...
#          DJ_BOOTSTRAP_DETECT_ONLY composes with it unchanged, so `only` plus
#          detect-only is the read-only `gh auth status` probe on its own.
#          bin/dj-startup-network.sh owns the deferral: it runs the `only` phase
...
#          Set DJ_BOOTSTRAP_LOCKED=1 alongside it when the sweeps are skipped
...
# shellcheck source=bin/dj-tasks-axi-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-tasks-axi-lib.sh"
# shellcheck source=bin/dj-quota-axi-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-quota-axi-lib.sh"
# shellcheck source=bin/dj-tangle-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-tangle-lib.sh"
# shellcheck source=bin/dj-ff-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-ff-lib.sh"
# shellcheck source=bin/dj-cursor-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-cursor-lib.sh"
# shellcheck source=bin/dj-config-inherit-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-config-inherit-lib.sh"
# shellcheck source=bin/dj-secondmate-nudge-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-secondmate-nudge-lib.sh"
# shellcheck source=bin/dj-startup-memory-budget-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-startup-memory-budget-lib.sh"
# shellcheck source=bin/dj-x-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-x-lib.sh"
# shellcheck source=bin/dj-backend.sh disable=SC1091
. "$SCRIPT_DIR/dj-backend.sh"
# shellcheck source=bin/dj-remote-readiness-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-remote-readiness-lib.sh"
# dj-timing-lib.sh is inert unless DJ_TIMING_LOG names a file, which only the
# deferred network stage sets, so an ordinary bootstrap run records nothing.
# shellcheck source=bin/dj-timing-lib.sh disable=SC1091
. "$SCRIPT_DIR/dj-timing-lib.sh"
...
# safety sweep.
case "${DJ_BOOTSTRAP_NETWORK:-all}" in
  skip|only) DJ_BOOTSTRAP_NETWORK_PHASE=${DJ_BOOTSTRAP_NETWORK:-all} ;;
  *) DJ_BOOTSTRAP_NETWORK_PHASE=all ;;
esac
local_phase() { [ "$DJ_BOOTSTRAP_NETWORK_PHASE" != only ]; }
network_phase() { [ "$DJ_BOOTSTRAP_NETWORK_PHASE" != skip ]; }

network_mutation_authorized() {
  local expected=${DJ_BOOTSTRAP_NETWORK_LOCK_PID:-} current
  [ -n "$expected" ] || return 0
...
bootstrap_parallel_begin() {
  BOOTSTRAP_PAR_DIR=$(mktemp -d "${TMPDIR:-/tmp}/dj-bootstrap-par.XXXXXX") || return 1
  BOOTSTRAP_PAR_N=0
  DJ_BOOTSTRAP_PARALLEL_DIR=$BOOTSTRAP_PAR_DIR
  export DJ_BOOTSTRAP_PARALLEL_DIR
}
...
bootstrap_parallel_finish() {
  ...
  unset DJ_BOOTSTRAP_PARALLEL_DIR BOOTSTRAP_PAR_DIR BOOTSTRAP_PAR_N
}

secondmate_note_respawned() {  # <id>
  SECONDMATE_RESPAWNED_IDS="$SECONDMATE_RESPAWNED_IDS $1"
  [ -n "${DJ_BOOTSTRAP_PARALLEL_DIR:-}" ] || return 0
  printf '%s\n' "$1" > "$DJ_BOOTSTRAP_PARALLEL_DIR/respawned.$1"
}
...
fleet_sync_bootstrap_timeout() {
  local count timeout
  if [ -n "${DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT:-}" ]; then
    case "$DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT" in
      *[!0-9]*) echo 20 ;;
      *) echo "$DJ_FLEET_SYNC_BOOTSTRAP_TIMEOUT" ;;
    esac
    return 0
  fi
...
fleet_sync() {
  [ -x "$DJ_ROOT/bin/dj-fleet-sync.sh" ] || return 0
  [ -d "$PROJECTS" ] || return 0

  tmp=$(mktemp "${TMPDIR:-/tmp}/dj-fleet-sync.XXXXXX" 2>/dev/null) || return 0
  timeout=$(fleet_sync_bootstrap_timeout)
  monitor_was_on=0
  case $- in *m*) monitor_was_on=1 ;; esac
  set -m 2>/dev/null || true
  "$DJ_ROOT/bin/dj-fleet-sync.sh" >"$tmp" 2>/dev/null &
  pid=$!
...
secondmate_sync() {
  # shellcheck source=bin/dj-wake-lib.sh disable=SC1091
  ...
  # commit (dj-ff-lib.sh), while a standalone clone without it is skipped until
  # /updatejarvis refreshes it from origin. Startup sends reread nudges only
  ...
  # /updatejarvis, startup owns the live-convergence send itself because it is
  ...
  [ -d "$STATE" ] || return 0
  local primary_head
  if ! primary_head=$(primary_head_commit "$DJ_ROOT"); then
    local meta id
  ...
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  SECOND_MATE_NUDGE_MESSAGE=$DJ_SECOND_MATE_NUDGE_MESSAGE
  REMOTE_SECOND_MATE_NUDGE_MESSAGE=$DJ_REMOTE_SECOND_MATE_NUDGE_MESSAGE
  SECOND_MATE_NUDGE_PENDING_DIR="$STATE/.secondmate-nudge-pending"

  secondmate_nudge_marker_path() {
    dj_secondmate_nudge_marker_path "$STATE" "$1"
  }

  secondmate_write_nudge_marker() {
    local id=$1 home=$2 commit=$3 instr=$4 message=${5:-$SECOND_MATE_NUDGE_MESSAGE} remote=${6:-0}
    dj_secondmate_nudge_write "$STATE" "$id" "$home" "$commit" "$instr" "$message" "$remote"
  }
  ...
  secondmate_send_nudge() {
    ...
    if out=$(DJ_HOME="$DJ_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_STATE_OVERRIDE="$STATE" "$SCRIPT_DIR/dj-send.sh" "$selector" "$SECOND_MATE_NUDGE_MESSAGE" 2>&1); then
      rm -f "$marker"
  ...
  dj_ff_after_instruction_update() {
  ...
  secondmate_retry_pending_nudges() {
    ...
    for marker in "$SECOND_MATE_NUDGE_PENDING_DIR"/*.pending; do
      ...
      selector=$(dj_meta_get "$marker" selector)
      home=$(dj_meta_get "$marker" home)
      commit=$(dj_meta_get "$marker" commit)
      message=$(dj_meta_get "$marker" message)
      remote=$(dj_meta_get "$marker" remote)
      [ -n "$remote" ] || remote=0
      [ "$selector" = "dj-$id" ] || {
  ...
  local tmp line
  secondmate_retry_pending_nudges
  tmp=$(mktemp "${TMPDIR:-/tmp}/dj-secondmate-sync.XXXXXX" 2>/dev/null) || return 0
  ...
  rm -f "$tmp"
  unset -f dj_ff_after_instruction_update
  ...
  # FF_SEEN_HOMES is exactly that set, and dj-config-inherit-lib.sh owns the
  ...
  while IFS='|' read -r id home _window _meta; do
    ...
    home_lock=$(dj_config_inherit_lock_path "$home_real") || {
      echo "CONFIG_REREAD: secondmate $id: send failed: could not resolve per-home lock"
      continue
    }
    dj_lock_acquire_wait "$home_lock" || {
    ...
    if [ "$reread_skip_pending" -eq 0 ] \
      && dj_config_reread_retry_queue_is_full "$DJ_HOME" "$id"; then
      dj_config_reread_retry_pending "$id" "$home_real" || true
      if dj_config_reread_retry_queue_is_full "$DJ_HOME" "$id"; then
        echo "CONFIG_REREAD: secondmate $id: send failed: retry instruction queue is full"
        dj_lock_release "$home_lock" || true
        continue
      fi
    fi
    report=$(mktemp "${TMPDIR:-/tmp}/dj-bootstrap-inherit.XXXXXX" 2>/dev/null) || {
      echo "SECONDMATE_SYNC: secondmate $id: skipped: inheritance failed"
      dj_lock_release "$home_lock" || true
      continue
    }
    if DJ_CONFIG_INHERIT_REPORT="$report" DJ_CONFIG_INHERIT_LIVE=1 \
      propagate_secondmate_inheritance "$DJ_HOME" "$home_real" "$CONFIG" "$DATA"; then
      :
    else
      echo "SECONDMATE_SYNC: secondmate $id: skipped: inheritance failed"
    fi
    if ! reread_out=$(DJ_HOME="$DJ_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" \
      DJ_STATE_OVERRIDE="$STATE" \
      DJ_CONFIG_REREAD_SKIP_PENDING="$reread_skip_pending" \
      dj_config_send_reread_nudge "$id" "$home_real" "$report" 2>&1); then
  ...
  secondmate_sync_remote_one() {  # <id> <home> <remote-host>
    ...
    remote_lock=$(dj_remote_inherit_transaction_lock_path "$STATE" "$id" 2>/dev/null || true)
    if [ -z "$remote_lock" ] || ! dj_lock_acquire_wait "$remote_lock"; then
      echo "NUDGE_SECONDMATES: secondmate $id: send failed: cannot lock remote inheritance transaction"
      return 0
    fi
    if ! "$SCRIPT_DIR/dj-procevent-remote-reply.sh" arm "$id" >/dev/null 2>&1; then
    ...
    remote_pending=0
    if [ -f "$remote_marker" ] && [ "$(dj_meta_get "$remote_marker" remote)" = 1 ]; then remote_pending=1; fi
    ...
    nudge_needed=0
    converged=1
    if sync_out=$("$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh sync "$id" < /dev/null 2>&1); then
    ...
    if inherit_out=$(DJ_CONFIG_INHERIT_LIVE=1 \
      "$SCRIPT_DIR/dj-remote-inherit-push.sh" "$id" "$remote_generation" 2>&1); then
    ...
    if [ "$converged" -eq 1 ] && [ "$nudge_needed" -eq 1 ]; then
      if out=$(DJ_HOME="$DJ_HOME" DJ_ROOT_OVERRIDE="$DJ_ROOT" DJ_STATE_OVERRIDE="$STATE" \
        "$SCRIPT_DIR/dj-send.sh" "dj-$id" "$REMOTE_SECOND_MATE_NUDGE_MESSAGE" 2>&1); then
        rm -f "$remote_marker"
        [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" != 1 ] || echo "BOOTSTRAP_INFO: nudged remote dj-$id after convergence"
      else
        echo "NUDGE_SECONDMATES: secondmate $id: send failed: $(first_line "$out")"
      fi
  ...
  secondmate_sync_remote_one_timed() {  # <id> <home> <remote-host>
    ...
    __fm_timing_stamp=$(dj_timing_now_ms)
    secondmate_sync_remote_one "$id" "$home" "$remote_host"
    dj_timing_record secondmate convergence "$__fm_timing_stamp" "$id@$remote_host"
  }
...
# DJ_BOOTSTRAP_VERBOSE_FACTS as before; on the deferred network pass the digest
# is already out, so reporting it is what keeps the superseded record from being
# acted on.
report_relaunch() {  # <id> <cause> <where>
  [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] || ! local_phase || return 0
...
secondmate_liveness_sweep() {
  ...
  # dj_backend_agent_state. A missing tmux pane is not enough: tmux must prove
...
secondmate_liveness_one_timed() {  # <meta> <id> <label>
  ...
  dj_timing_record secondmate liveness "$__fm_timing_stamp" "$label"
}
...
secondmate_liveness_one() {  # <meta> <id>
  ...
  if [ -n "$remote_host" ]; then
    remote_rc=0
    dj_remote_readiness_ensure "$SCRIPT_DIR" "$id" || remote_rc=$?
    ...
    if [ "$remote_rc" -ne 0 ]; then
      readiness_reason=$(printf '%s\n' "$DJ_REMOTE_READINESS_OUT" \
        | awk '/^check [^=]+=(fixable|human):|^action:|^error:/ { print; exit }')
      [ -n "$readiness_reason" ] || readiness_reason=$(first_line "$DJ_REMOTE_READINESS_OUT")
    ...
    if out=$("$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh state "$id" < /dev/null 2>/dev/null); then
      remote_rc=0
    else
      remote_rc=$?
    fi
    ...
    case "$agent_state" in
      alive)
        if route_out=$("$SCRIPT_DIR/dj-on.sh" "$id" dj-remote-secondmate-control.sh route "$id" < /dev/null 2>/dev/null); then
          remote_rc=0
        else
          remote_rc=$?
        fi
        ...
        [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" != 1 ] || echo "BOOTSTRAP_INFO: remote secondmate $id already live (host=$remote_host)"
        ;;
      dead|missing)
        cause="remote endpoint $agent_state on its configured host"
        if out=$(DJ_SPAWN_NO_GUARD=1 "$DJ_ROOT/bin/dj-spawn.sh" "$id" --secondmate 2>&1); then
          secondmate_note_respawned "$id"
  ...
  agent_state=$(dj_backend_agent_state "$backend" "$target" 2>/dev/null) || agent_state=unreadable
  case "$harness" in
  ...
  case "$agent_state" in
    alive)
      if [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ]; then
    ...
    dead|missing)
      if [ "$agent_state" = dead ]; then
        ...
        dj_backend_kill "$backend" "$target" 2>/dev/null || true
      else
...
secondmate_handoff_resume() {
  ...
  "$SCRIPT_DIR/dj-backlog-handoff.sh" --resume-pending >/dev/null 2>&1 || true
}
...
install_cmd() {
  case "$1" in
    ...
    gh-axi|chrome-devtools-axi|edith-axi) echo "npm install -g $1 && $1 setup hooks" ;;
...
# dj_backend_required_tools (bin/dj-backend.sh). So a herdr/zellij/cmux home is
# never told tmux is missing, and only orca drops treehouse. A backend value with
# no verified dependency set is reported before the universal checks continue.
COMMON_TOOLS="node git gh no-mistakes gh-axi chrome-devtools-axi edith-axi tasks-axi quota-axi"
BACKEND=$(dj_backend_name)
BACKEND_VALID=1
if ! BACKEND_TOOLS=$(dj_backend_required_tools "$BACKEND"); then
  BACKEND_VALID=0
  BACKEND_TOOLS=""
fi
...
x_mode_write_if_changed() {
  ...
  tmp=$(umask 077; mktemp "$parent/.dj-x-mode.XXXXXX" 2>/dev/null) || return 1
  if ! printf '%s\n' "$content" > "$tmp" \
    || ! chmod "$mode" "$tmp" \
...
# X mode (opt-in): when this home's .env carries a non-empty DJX_PAIRING_TOKEN,
...
#                            its bytes and invokes bin/dj-x-poll.sh directly
#   config/x-mode.env      - exports DJ_CHECK_INTERVAL=30, sourced by the watcher
...
x_mode_setup() {
  ...
  env_file="$DJ_HOME/.env"
  shim="$STATE/x-watch.check.sh"
  cadence="$CONFIG/x-mode.env"

  token=
  [ -f "$env_file" ] && token=$(fmx_env_get DJX_PAIRING_TOKEN "$env_file")
  ...
  x_mode_supervision_repair() {
    local out
    out=$("$SCRIPT_DIR/dj-supervision-instructions.sh" --repair-line 2>/dev/null) \
  ...
  case "$DJ_HOME" in
    /*) shim_home=$DJ_HOME ;;
    *)
      shim_home=$(CDPATH='' cd -- "$DJ_HOME" 2>/dev/null && pwd -P) \
        || { fmx_arm_failed; return 0; }
      ;;
  esac
  shim_body=$(fmx_poll_shim_content "$shim_home" "$DJ_ROOT")
  x_mode_write_if_changed "$shim" "$shim_body" 700 || { fmx_arm_failed; return 0; }
  fmx_poll_shim_valid "$shim" "$shim_home" "$DJ_ROOT" \
    || { fmx_arm_failed; return 0; }

  cadence_body=$(cat <<'EOF'
# Auto-generated by dj-bootstrap.sh - X mode watcher cadence.
# Source this before the active harness protocol starts a watcher process so
# dj-watch.sh polls the X check every 30s. Non-X instances have no such file and
# keep the default 300s cadence.
export DJ_CHECK_INTERVAL=30
EOF
)
...
startup_memory_budget_setup() {
  ...
  if ! dj_startup_memory_budget_materialize "$CONFIG"; then
    echo "STARTUP_MEMORY_BUDGET: invalid config/$DJ_STARTUP_MEMORY_BUDGET_FILE - $DJ_STARTUP_MEMORY_BUDGET_ERROR"
  fi
}

if [ "${1:-}" = "install" ]; then
  shift
  [ $# -gt 0 ] || { echo "usage: dj-bootstrap.sh install <tool>..." >&2; exit 1; }
  for t in "$@"; do
...
if [ "${DJ_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ] && local_phase; then
  "$SCRIPT_DIR/dj-pr-check-migrate.sh" || true
  startup_memory_budget_setup
fi
...
detect_local_tools() {
  if [ "$BACKEND_VALID" -eq 0 ]; then
    echo "BACKEND_INVALID: $BACKEND (known: $DJ_BACKEND_KNOWN)"
  fi
  for t in $BACKEND_TOOLS; do
    dj_backend_required_tool_available "$BACKEND" "$t" \
  ...
  if dj_backend_list_contains "$TOOLS" treehouse \
  ...
  if command -v edith-axi >/dev/null 2>&1 && ! tool_version_at_least edith-axi "$EDITH_AXI_MIN"; then
    echo "MISSING: edith-axi (install: $(install_cmd edith-axi))"
  fi
  if command -v quota-axi >/dev/null 2>&1 && ! dj_quota_axi_compatible; then
    echo "MISSING: quota-axi (install: $(install_cmd quota-axi))"
  fi
  if command -v tasks-axi >/dev/null 2>&1 && ! dj_tasks_axi_compatible; then
    echo "MISSING: tasks-axi (install: $(install_cmd tasks-axi))"
  fi
}

detect_local_config() {
  # Worktree-tangle check: the jarvis primary checkout (DJ_ROOT) must sit on its
  # default branch, not a feature branch (see dj-tangle-lib.sh). Scoped to the
  # primary only; detached-HEAD worktrees and secondmate homes never trip it.
  tangle_branch=$(dj_primary_tangle_branch "$DJ_ROOT" 2>/dev/null || true)
  if [ -n "$tangle_branch" ]; then
    tangle_default=$(dj_default_branch "$DJ_ROOT" 2>/dev/null || echo main)
    if [ "${DJ_BOOTSTRAP_DETECT_ONLY:-0}" = 1 ] && [ "${DJ_BOOTSTRAP_LOCKED:-0}" != 1 ]; then
      ...
      echo "TANGLE: primary checkout on feature branch '$tangle_branch' (expected '$tangle_default'); the work is safe on that ref - restore the primary with: git -C $DJ_ROOT checkout $tangle_default, then re-validate the branch in a proper worktree"
    fi
  fi
  crew=
  [ -f "$CONFIG/crew-harness" ] && crew=$(tr -d '[:space:]' < "$CONFIG/crew-harness" || true)
  if [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] && [ -n "$crew" ] && [ "$crew" != "default" ]; then
  ...
  if [ "$crew" = cursor ] && ! dj_cursor_resolve_binary >/dev/null 2>&1; then
    echo "MISSING_MANUAL: cursor-agent (instructions: $(manual_install_url cursor-agent))"
  fi
  crew_dispatch_validate
  if [ "${DJ_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] \
    && ! dj_backlog_backend_manual "$CONFIG" && dj_tasks_axi_compatible; then
...
# dj-timing-lib.sh discards the record unless the caller asked for timings, and
...
if network_phase; then
  ...
  dj_timing_record phase gh-auth "$__fm_timing_stamp"
fi
local_phase && detect_local_config

if [ "${DJ_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  if network_phase && network_sweep_authorized 'project clone refresh'; then
    fleet_sync_out=$(mktemp "${TMPDIR:-/tmp}/dj-bootstrap-fleet.XXXXXX") || fleet_sync_out=
    if [ -n "$fleet_sync_out" ]; then
      (
        __fm_timing_stamp=$(dj_timing_now_ms)
        fleet_sync
        dj_timing_record phase fleet-sync "$__fm_timing_stamp"
      ) >"$fleet_sync_out" 2>&1 &
      fleet_sync_pid=$!
    else
      __fm_timing_stamp=$(dj_timing_now_ms)
  ...
  if network_phase; then
    if network_sweep_authorized 'dead-secondmate relaunch'; then
      __fm_timing_stamp=$(dj_timing_now_ms)
      secondmate_liveness_sweep
      dj_timing_record phase secondmate-liveness "$__fm_timing_stamp"
    fi
    if network_sweep_authorized 'secondmate convergence'; then
      __fm_timing_stamp=$(dj_timing_now_ms)
      secondmate_sync
      dj_timing_record phase secondmate-sync "$__fm_timing_stamp"
    fi
    if network_sweep_authorized 'pending handoff delivery'; then
      __fm_timing_stamp=$(dj_timing_now_ms)
      secondmate_handoff_resume
      dj_timing_record phase handoff-delivery "$__fm_timing_stamp"
    fi
  fi
