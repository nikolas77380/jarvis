#!/bin/bash
# Long-lived per-account worker for remote dj-on jobs.
#
# This process is launched by the Jarvis-owned dev.jarvis.remote-job
# LaunchAgent on macOS and by a detached restart supervisor on Linux. It claims
# only complete 0700 records staged by dj-remote-job-lib.sh under the fixed
# account queue, refuses symlinks and malformed records, and executes only a
# tracked non-symlink dj-*.sh under this worker's configured DJ_ROOT/bin.
#
# Each child runs under env -i with the shared filesystem-composed PATH, HOME,
# DJ_HOME, DJ_ROOT_OVERRIDE, and DJ_REMOTE_JOB_ACTIVE=1. Commands receive their
...
# lifecycle.
#
# The worker is abandoned when its configured DJ_ROOT stops being a genuine
# Jarvis checkout - the state a pruned no-mistakes gate worktree, a returned
...
# reparented to init. DJ_REMOTE_JOB_ORPHAN_GRACE_SECONDS is how long the root
...
# DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS and gives up after
# DJ_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS failed children in total, since a
# restart loop only burns CPU and grows its log without bound. A child that
# stays up for DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS clears the
...
# either. dj-on's ensure path restarts a worker that gave up.
set -u
...
DJ_REMOTE_JOB_ORPHAN_GRACE_SECONDS=$(worker_bounded_setting "${DJ_REMOTE_JOB_ORPHAN_GRACE_SECONDS:-}" 5)
DJ_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS=$(worker_bounded_setting "${DJ_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS:-}" 20)
DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS=$(worker_bounded_setting "${DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS:-}" 5)
DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS=$(worker_bounded_setting "${DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS:-}" 10)

SCRIPT_DIR=$(CDPATH='' cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)
DJ_ROOT=${DJ_ROOT_OVERRIDE:-$(CDPATH='' cd "$SCRIPT_DIR/.." && pwd -P)}
...
worker_account_home() {
  local home=${HOME:-}
  if [ -n "$home" ]; then
    dj_remote_job_canonical_existing_dir "$home" && return 0
  fi
  unset HOME
...
worker_write_heartbeat() {
  ...
  tmp=$(umask 077; mktemp "$DJ_REMOTE_JOB_STATE/.ready.XXXXXX") || return 1
...
worker_publish_pid() {
  ...
  tmp=$(umask 077; mktemp "$DJ_REMOTE_JOB_STATE/.pid.XXXXXX") || return 1
  printf '%s\n' "${BASHPID:-$$}" > "$tmp" || { rm -f -- "$tmp"; return 1; }
  chmod 600 "$tmp" || { rm -f -- "$tmp"; return 1; }
  mv -f -- "$tmp" "$pid_file"
}
...
worker_publish_identity() {
  ...
  identity=$(dj_remote_job_code_identity "$DJ_ROOT" "$account_home") || return 1
  identity_file=$(dj_remote_job_worker_identity_path)
  tmp=$(umask 077; mktemp "$DJ_REMOTE_JOB_STATE/.identity.XXXXXX") || return 1
...
worker_publish_lock_owner() {
  ...
  pid=${BASHPID:-$$}
  start=$(dj_remote_job_process_start "$pid") || return 1
  command=$(dj_remote_job_process_command "$pid") || return 1
...
worker_lock_recent() {
  local mtime now
  mtime=$(dj_remote_job_path_mtime "$WORKER_LOCK" 2>/dev/null || true)
...
worker_quarantined_execution_stopped() { # <account-home>
  ...
  dj_remote_job_regular_bounded "$WORKER_LOCK/quarantine" 256 || return 1
  dj_remote_job_lock_owner_matches_process "$account_home" && return 1
...
worker_acquire_lock() {
  ...
  while [ "$attempt" -lt 150 ]; do
    ...
    if dj_remote_job_lock_owner_matches_process "$account_home"; then return 2; fi
    if dj_remote_job_probe "$account_home" || worker_lock_recent; then
      attempt=$((attempt + 1))
      sleep 0.1
      continue
    fi
...
worker_cleanup() {
  ...
  owner_pid=$(dj_remote_job_read_single_line "$WORKER_LOCK/pid" 64 2>/dev/null || true)
  if [ -z "$owner_pid" ]; then
  ...
  pid_file=$(dj_remote_job_worker_pid_path)
  ready=$(dj_remote_job_worker_ready_path)
  identity=$(dj_remote_job_worker_identity_path)
...
worker_code_root_abandoned() {
  local waited=0
  dj_remote_job_root_is_live "$DJ_ROOT" && return 1
  while [ "$waited" -lt "$DJ_REMOTE_JOB_ORPHAN_GRACE_SECONDS" ]; do
    sleep 1
    waited=$((waited + 1))
    dj_remote_job_root_is_live "$DJ_ROOT" && return 1
  done
...
worker_read_process_id() { # <file>
  local file=$1 pid
  dj_remote_job_regular_bounded "$file" 64 || return 1
  pid=$(tr -d '\n' < "$file")
...
worker_claim_owner_alive() { # <job-dir>
  ...
  owner="$claim/owner"
  dj_remote_job_regular_bounded "$owner" 64 || return 1
  pid=$(tr -d '\n' < "$owner")
  case "$pid" in ''|*[!0-9]*) return 1 ;; esac
...
worker_read_text() { # <job-dir> <field> <max>
  ...
  dj_remote_job_regular_bounded "$job/$field" "$max" || return 1
...
worker_run_with_timeout() { # <job-dir> <seconds> <command> [args...]
  ...
  [ "$WORKER_PREEMPTED" -eq 0 ] || return "$DJ_REMOTE_JOB_PREEMPTED_EXIT"
  return "$rc"
}

worker_job_command() { # <job-dir>; the first argv element of a staged record
  local job=$1 first=
  dj_remote_job_regular_bounded "$job/argv" "$DJ_REMOTE_JOB_MAX_BYTES" || return 1
...
worker_preempting_waiter_exists() {
  ...
  for job in "$DJ_REMOTE_JOB_JOBS"/job-*; do
    ...
    dj_remote_job_command_preemptible "$command" || return 0
  done
  return 1
}
...
worker_capture_output() { # <fifo> <destination>
  ...
  {
    head -c "$DJ_REMOTE_JOB_MAX_BYTES"
    cat >/dev/null
  } < "$fifo" > "$destination"
}
...
worker_run_job() { # <account-home> <job-dir>
  ...
  root=$(dj_remote_job_canonical_existing_dir "$root") || { worker_publish_result "$job" 126; return; }
  home=$(dj_remote_job_canonical_home "$home") || { worker_publish_result "$job" 126; return; }
  [ "$root" = "$DJ_ROOT" ] || { worker_publish_result "$job" 126; return; }
  [ -f "$root/AGENTS.md" ] && [ ! -L "$root/AGENTS.md" ] &&
    [ -d "$root/bin" ] && [ ! -L "$root/bin" ] || { worker_publish_result "$job" 126; return; }
  dj_remote_job_regular_bounded "$job/argv" "$DJ_REMOTE_JOB_MAX_BYTES" || { worker_publish_result "$job" 126; return; }
  dj_remote_job_regular_bounded "$job/stdin" "$DJ_REMOTE_JOB_MAX_BYTES" || { worker_publish_result "$job" 126; return; }
  deadline=$(dj_remote_job_read_deadline "$job") || { worker_publish_result "$job" 126; return; }
  ...
  command=${argv[0]}
  case "$command" in dj-*.sh) ;; *) worker_publish_result "$job" 126; return ;; esac
  case "$command" in */*|*..*) worker_publish_result "$job" 126; return ;; esac
  if dj_remote_job_command_preemptible "$command"; then preemptible=1; fi
  ...
  child_env=(
    ...
    DJ_REMOTE_JOB_ACTIVE=1
  )
  if [ -n "${DJ_REMOTE_JOB_PLATFORM_OVERRIDE:-}" ]; then
    child_env+=("DJ_REMOTE_JOB_PLATFORM_OVERRIDE=$DJ_REMOTE_JOB_PLATFORM_OVERRIDE")
  fi
  remaining=$((deadline - $(date +%s)))
  [ "$remaining" -gt 0 ] || {
...
worker_process_once() { # <account-home>
  ...
  for job in "$DJ_REMOTE_JOB_JOBS"/job-*; do
    ...
    job=$(dj_remote_job_job_dir "$id" 2>/dev/null || true)
    [ -n "$job" ] || continue
    state=$(dj_remote_job_read_state "$job" 2>/dev/null || true)
    case "$state" in
      queued)
        worker_clear_dead_claim "$job" || continue
        queue_deadline=$(dj_remote_job_read_number "$job" queue_deadline 2>/dev/null || true)
    ...
    timeout=$(dj_remote_job_read_number "$job" timeout 2>/dev/null || true)
    ...
    dj_remote_job_write_number "$job" deadline "$deadline" || {
      worker_publish_result "$job" 125 || true
      continue
    }
    dj_remote_job_write_state "$job" running || {
      worker_publish_result "$job" 125 || true
      continue
    }
...
main() {
  ...
  DJ_ROOT=$(dj_remote_job_canonical_existing_dir "$DJ_ROOT") || { worker_error "configured DJ_ROOT is unsafe"; exit 1; }
  [ -f "$DJ_ROOT/AGENTS.md" ] && [ ! -L "$DJ_ROOT/AGENTS.md" ] || { worker_error "DJ_ROOT is not a Jarvis checkout"; exit 1; }
  dj_remote_job_prepare_state "$account_home" || { worker_error "$DJ_REMOTE_JOB_ERROR"; exit 1; }
  WORKER_LOCK=$(dj_remote_job_worker_lock_path)
  ...
  while :; do
    ...
    if worker_code_root_abandoned; then
      worker_error "configured DJ_ROOT $DJ_ROOT no longer exists; stopping the abandoned worker"
      exit 0
    fi
    worker_reap=0
    if [ "$worker_reap" -eq 0 ]; then
      dj_remote_job_reap_stale "$account_home" || true
      worker_reap=1
    fi
...
worker_supervisor_cleanup_dead_child() { # <account-home> <pid>
  ...
  recorded=$(dj_remote_job_read_single_line "$lock/pid" 64) || return 1
...
worker_supervise_linux() {
  ...
  DJ_ROOT=$(dj_remote_job_canonical_existing_dir "$DJ_ROOT") || { worker_error "configured DJ_ROOT is unsafe"; return 1; }
  [ -f "$DJ_ROOT/AGENTS.md" ] && [ ! -L "$DJ_ROOT/AGENTS.md" ] || { worker_error "DJ_ROOT is not a Jarvis checkout"; return 1; }
  dj_remote_job_prepare_state "$account_home" || { worker_error "$DJ_REMOTE_JOB_ERROR"; return 1; }
  trap worker_supervisor_shutdown HUP INT TERM
  while :; do
    if worker_code_root_abandoned; then
      worker_error "configured DJ_ROOT $DJ_ROOT no longer exists; stopping the abandoned worker supervisor"
      return 0
    fi
    started=$SECONDS
    "$SCRIPT_DIR/dj-remote-job-worker.sh" --serve &
    WORKER_SUPERVISED_PID=$!
    ...
    WORKER_SUPERVISED_PID=
    restarts=$((restarts + 1))
    if [ "$restarts" -ge "$DJ_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS" ]; then
      worker_error "remote job worker exited $restarts times; stopping the supervisor"
      return 1
    fi
    if [ $((SECONDS - started)) -ge "$DJ_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS" ]; then
      failures=0
      sleep 0.1
      continue
    fi
    failures=$((failures + 1))
    backoff=$failures
    [ "$backoff" -le "$DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS" ] ||
      backoff=$DJ_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS
    sleep "$backoff"
  done
}
...
case "${1:-}" in
  --serve)
    ...
    ;;
  '')
    if [ "$(dj_remote_job_platform)" = linux ]; then worker_supervise_linux; else main; fi
    ;;
