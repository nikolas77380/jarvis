...
# Sourced by bin/dj-spawn.sh, bin/dj-harness.sh, bin/dj-busy-lib.sh, and
...
# let jarvis launch an unrelated executable with Cursor flags.
#
...
# bin/dj-composer-lib.sh, which every backend already delegates to; an
...
DJ_CURSOR_PROBE_TIMEOUT=${DJ_CURSOR_PROBE_TIMEOUT:-10}
...
dj_cursor_path_is_cursor() {  # <path>
  local path=$1 canonical
  [ -n "$path" ] || return 1
  canonical=$(dj_cursor_canonical_path "$path") || return 1
...
dj_cursor_bounded_output() {  # <path> <args...>
  local path=$1 runner=
  shift
  ...
  [ -n "$runner" ] || return 1
  "$runner" "$DJ_CURSOR_PROBE_TIMEOUT" "$path" "$@" 2>/dev/null
}

dj_cursor_probe_is_cursor() {  # <path>
  local path=$1 out
  out=$(dj_cursor_bounded_output "$path" --help) || return 1
  [ -n "$out" ] || return 1
  case "$out" in
...
dj_cursor_verify_executable() {  # <path>
  local path=$1
  [ -n "$path" ] && [ -x "$path" ] || return 1
  case "${path##*/}" in cursor-agent) return 0 ;; esac
  dj_cursor_path_is_cursor "$path" && return 0
  dj_cursor_probe_is_cursor "$path"
}

dj_cursor_list_models() {  # <path>
  dj_cursor_bounded_output "$1" --list-models
}

dj_cursor_catalog_has_model() {  # <model>
  local wanted=$1
  awk -v wanted="$wanted" '
...
#
# Resolution order, shared by bin/dj-spawn.sh and bin/dj-remote-doctor.sh:
...
# dj_cursor_verify_executable before it is accepted, so an unrelated executable
...
dj_cursor_resolve_binary() {
  local name candidate
  for name in cursor-agent agent; do
    candidate=$(command -v "$name" 2>/dev/null || true)
    [ -n "$candidate" ] && [ -x "$candidate" ] || continue
    if dj_cursor_verify_executable "$candidate"; then
      printf '%s\n' "$candidate"
      return 0
    fi
  done
...
dj_cursor_argv0_for_pid() {  # <pid> [comm-fallback]
  local pid=$1 fallback=${2:-} proc_root=${DJ_PROC_ROOT_OVERRIDE:-/proc} argv0=
...
dj_cursor_argv0_is_cursor() {  # <argv0>
  local argv0=$1
  [ -n "$argv0" ] || return 1
  case "$argv0" in
    ''|MainThread) return 1 ;;
  ...
  dj_cursor_path_is_cursor "$argv0"
}
...
# (bin/dj-session-lock-lib.sh), harness detection (bin/dj-harness.sh), pane
# liveness (bin/backends/tmux.sh), and worker-server discovery (bin/dj-spawn.sh).
#
...
dj_cursor_process_matches() {  # <comm> <args> [argv0]
  ...
  case "$base" in
    ...
    agent|MainThread|node|node-*|node[0-9]*|python|python[0-9]*|python[0-9].[0-9]*)
      dj_cursor_argv0_is_cursor "$argv0" && return 0
      # A legacy alias may also be reported by its own path in comm.
      dj_cursor_path_is_cursor "$comm" && return 0
      return 1
      ;;
  esac
  # A version-named or otherwise renamed executable still identifies through
  # its install path.
  case "$comm" in */*) dj_cursor_path_is_cursor "$comm" && return 0 ;; esac
  return 1
}
