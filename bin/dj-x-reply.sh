#!/usr/bin/env bash
# Post jarvis's composed answer back to the relay for a pending X-mode mention.
#
# Usage: dj-x-reply.sh <request_id> [--image <path>] <text>
#        dj-x-reply.sh <request_id> [--image <path>] --text-file <path>
#        dj-x-reply.sh <request_id> [--image <path>] -
#        dj-x-reply.sh <request_id> --followup [--image <path>] ...
#        dj-x-reply.sh <request_id> ... --receipt-file <path>
#
...
# posted (bin/dj-public-followup.sh, building a typed delivery receipt) does not
...
# expansion or quote-breakage could bite. djx-respond uses them; the positional
...
# dj-x-followup.sh can tell "exhausted binding" apart from a transient post
...
# call can instead see a benign no-op 200, so dj-x-followup.sh's local
# window/cap pruning remains the primary guard.
#
# Reply platform + split budget are resolved per axis: an explicit
# DJX_REPLY_PLATFORM / DJX_REPLY_MAX_CHARS env override wins (dj-x-followup passes
...
# authoritative relay lookup by request_id (dj-x-lib.sh:fmx_resolve_reply_context).
...
# 9) rather than posting with a locally defaulted budget - jarvis holds and
# retries it.
#
# Long replies auto-split into a numbered thread. X stays within
# DJX_X_REPLY_MAX_CHARS, default 280. Discord uses
# DJX_DISCORD_REPLY_MAX_CHARS, default 1900, safely below Discord's 2000
...
# DJX_X_THREAD_MAX messages (default 25) are produced.
#
# Live post config (home .env, DJX_ENV_FILE, or env): DJX_PAIRING_TOKEN
# (required), DJX_RELAY_URL (default https://myjarvis.io). Auth:
# Authorization: Bearer <token>.
#
# Preview / dry-run: with DJX_DRY_RUN set (truthy), the reply is NOT posted.
...
reply_make_tmp_file() {
  ...
  file=$(mktemp "${TMPDIR:-/tmp}/dj-x-reply.XXXXXX") || return 1
  TMP_FILES+=("$file")
...
write_reply_receipt() {
  ...
  if ! (umask 077; jq -n --arg r "$REQ" --arg e "$ENDPOINT" --argjson c "$1" --argjson d "$2" \
    ...
    echo "dj-x-reply: warning: posted but could not write the receipt to $RECEIPT_FILE" >&2
  fi
}

usage() {
  echo "usage: dj-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] <text> | ... --text-file <path> | ... -" >&2
}

help() {
  cat <<'EOF'
usage: dj-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] <text>
       dj-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] --text-file <path>
       dj-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] -
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --image)
      shift
      if [ "$#" -lt 1 ] || [ -z "$1" ]; then
        echo "dj-x-reply: missing --image path" >&2
        usage
        exit 2
      fi
      IMAGE_PATH=$1
    ...
    --receipt-file)
      shift
      if [ "$#" -lt 1 ] || [ -z "$1" ]; then
        echo "dj-x-reply: missing --receipt-file path" >&2
        usage
        exit 2
      fi
      RECEIPT_FILE=$1
...
case "$1" in
  --text-file)
    if [ "$#" -lt 2 ]; then
      echo "usage: dj-x-reply.sh <request_id> [--followup] [--image <path>] --text-file <path>" >&2
      exit 2
    fi
    TEXT=$(cat -- "$2") || { echo "dj-x-reply: cannot read text file: $2" >&2; exit 1; }
    ;;
  -)
    TEXT=$(cat)
    ;;
...
if [ -z "$TEXT" ]; then
  echo "dj-x-reply: empty reply text" >&2
  exit 2
fi
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "dj-x-reply: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "dj-x-reply: jq not found" >&2; exit 1; }

# Resolve the reply platform + split budget. An explicit env override wins per
# axis (dj-x-followup passes recorded task-link context this way); otherwise
...
# (dj-x-lib.sh owns the resolution-order contract).
ALLOW_RELAY=0
if [ -n "${DJX_REPLY_PLATFORM:-}" ] && [ -n "${DJX_REPLY_MAX_CHARS:-}" ]; then
  REQ_PLATFORM=${DJX_REPLY_PLATFORM}
  REQ_EXPLICIT_MAX=${DJX_REPLY_MAX_CHARS}
else
  if [ "$FOLLOWUP" = 1 ] && [ -z "$DJX_DRY" ] && [ -n "$DJX_TOKEN" ]; then
    ALLOW_RELAY=1
  fi
  REPLY_CONTEXT=$(fmx_resolve_reply_context "$STATE" "$REQ" "$ALLOW_RELAY") || {
    echo "dj-x-reply: failed to resolve request platform context" >&2
    exit 1
  }
  REQ_PLATFORM=${DJX_REPLY_PLATFORM:-$(printf '%s' "$REPLY_CONTEXT" | jq -r '.platform // ""')}
  REQ_EXPLICIT_MAX=${DJX_REPLY_MAX_CHARS:-$(printf '%s' "$REPLY_CONTEXT" | jq -r '.reply_max_chars // ""')}
fi
...
if [ "$FOLLOWUP" = 1 ] && [ "$CONTEXT_RESOLVED" = 0 ]; then
  ...
  printf 'dj-x-reply: refusing follow-up for %s: could not authoritatively determine both the reply platform and explicit budget (local per-request context was incomplete%s). Hold and retry once both values are recoverable.\n' \
    "$REQ" "$relay_note" >&2
  exit 8
fi
...
if [ -n "$IMAGE_PATH" ]; then
  reply_make_tmp_file IMAGE_PAYLOAD_FILE || {
    echo "dj-x-reply: cannot create image payload temp file" >&2; exit 1; }
  IMAGE_PREVIEW=$(fmx_image_payload_file "$IMAGE_PATH" dj-x-reply "$IMAGE_PAYLOAD_FILE") || exit 1
  printf '%s' "$IMAGE_PREVIEW" | jq -e . >/dev/null 2>&1 || {
    echo "dj-x-reply: failed to build image preview" >&2; exit 1; }
fi
...
# unnumbered.
CHUNKS=$(printf '%s' "$TEXT" | fmx_split_thread "$REPLY_MAX" "$DJX_THREAD_MAX") || {
  echo "dj-x-reply: failed to split reply into a thread" >&2
  exit 1
}
N=$(printf '%s' "$CHUNKS" | jq 'length' 2>/dev/null) || N=
case "$N" in ''|*[!0-9]*) echo "dj-x-reply: failed to split reply into a thread" >&2; exit 1 ;; esac
[ "$N" -gt 0 ] || { echo "dj-x-reply: empty reply text" >&2; exit 2; }
...
reply_make_tmp_file PAYLOAD_FILE || {
  echo "dj-x-reply: cannot create request payload temp file" >&2; exit 1; }
if [ -n "$IMAGE_PAYLOAD_FILE" ]; then
  fmx_reply_payload_json "$REQ" "$CHUNKS" "$N" "$IMAGE_PAYLOAD_FILE" > "$PAYLOAD_FILE" || {
    echo "dj-x-reply: failed to build request payload" >&2; exit 1; }
else
...
if [ -n "$DJX_DRY" ]; then
  ...
  OUTREC=$(fmx_reply_outbox_json "$REQ" "$CHUNKS" "$N" "$FOLLOWUP" "$IMAGE_PREVIEW") || {
    echo "dj-x-reply: failed to build dry-run outbox record" >&2; exit 1; }
  printf '%s\n' "$OUTREC" \
    | fmx_private_artifact_publish_stdin "$outbox_dir" "$REQ.json" 600 || {
    echo "dj-x-reply: cannot write dry-run outbox: $outbox_dir/$REQ.json" >&2
    exit 1
  }
  if [ "$N" -le 1 ]; then
    printf 'dj-x-reply: DRY RUN - would POST to %s/connector/%s (recorded: state/x-outbox/%s.json): %s\n' \
      "$DJX_RELAY" "$ENDPOINT" "$REQ" "$(printf '%s' "$CHUNKS" | jq -r '.[0]')" >&2
  else
    printf 'dj-x-reply: DRY RUN - would POST a %s-tweet thread to %s/connector/%s (recorded: state/x-outbox/%s.json):\n' \
      "$N" "$DJX_RELAY" "$ENDPOINT" "$REQ" >&2
...
if [ -z "$DJX_TOKEN" ]; then
  echo "dj-x-reply: X mode not configured (no DJX_PAIRING_TOKEN)" >&2
  exit 1
fi
reply_make_tmp_file RESPONSE_BODY_FILE || {
  echo "dj-x-reply: cannot create relay response temp file" >&2; exit 1; }
code=$(fmx_post_json "$ENDPOINT" "$PAYLOAD_FILE" "$RESPONSE_BODY_FILE")
post_rc=$?
case "$post_rc" in
  0) : ;;
  127) echo "dj-x-reply: curl not found" >&2; exit 1 ;;
  3) echo "dj-x-reply: invalid DJX_PAIRING_TOKEN" >&2; exit 1 ;;
  *) echo "dj-x-reply: request to relay failed" >&2; exit 1 ;;
esac
  ...
  2[0-9][0-9])
    if [ "$FOLLOWUP" = 0 ]; then
      fmx_context_registry_set "$STATE" "$REQ" "$REQ_PLATFORM" "$REQ_EXPLICIT_MAX" 1 2>/dev/null \
        || echo "dj-x-reply: warning: could not retain reply context for $REQ" >&2
    fi
  ...
  409)
    if [ "$FOLLOWUP" = 1 ]; then
      if [ -s "$RESPONSE_BODY_FILE" ] && {
        ...
        echo "dj-x-reply: relay rejected the follow-up (confirmed followup_unavailable marker): HTTP 409" >&2
      else
        echo "dj-x-reply: relay rejected the follow-up (HTTP 409 cap/window exhaustion; marker absent)" >&2
      fi
      exit 9
    fi
    echo "dj-x-reply: relay returned HTTP $code" >&2
    exit 1
    ;;
  *) echo "dj-x-reply: relay returned HTTP $code" >&2; exit 1 ;;
esac
