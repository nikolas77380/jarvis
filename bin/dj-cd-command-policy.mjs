...
// PRIMARY jarvis shell's own working directory?
//
// A stray persistent top-level `cd projects/<clone>` silently relocates the
// primary shell, so the next jarvis-owned command (a backlog write, an
// dj-* lifecycle call, tasks-axi) runs inside a project clone instead of the
// home. This policy blocks exactly that class of command; the environmental
// scoping to the real primary checkout lives in the bin/dj-cd-pretool-check.sh
...
// bin/dj-arm-command-policy.mjs, the sole owner of jarvis's shell
...
import { Lexer, splitProgram, commandPosition } from "./dj-arm-command-policy.mjs";
import { realpathSync } from "node:fs";
import { fileURLToPath } from "node:url";

const REASONS = {
  "persistent-cd":
    "a persistent top-level directory change in the primary jarvis checkout is blocked; it would move the shell out of the home so a later jarvis-owned command runs inside a project clone. Reach the target without moving the shell - use git -C <dir> or an absolute path on the command itself - or scope the cd to a subshell like (cd <dir> && ...).",
};
