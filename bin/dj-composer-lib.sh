#!/usr/bin/env bash
# bin/dj-composer-lib.sh - the ONE fleet-wide owner of composer classification:
...
# session-provider adapter (tmux via bin/dj-tmux-lib.sh, and
# bin/backends/{herdr,orca,cmux,zellij}.sh) and by dj-spawn.sh's kimi
# launch-readiness check.
#
# WHY THIS EXISTS (tasks dj-composer-shellglyph-safety and
# dj-composer-thin-adapter-refactor-r1): the adapters each carried their own
# copy of composer shape knowledge, and every copy drifted. The audited result
# (data/dj-composer-consolidation-audit-s1) was a 5-adapter x 6-harness matrix
...
# shape is taught to dj_composer_classify_screen below, once, and every backend
...
# captures in data/dj-composer-consolidation-audit-s1/report.md and
...
# dj_composer_strip_ghost is the ONE ANSI-aware extractor of "real typed
...
# dj_composer_normalize_trim_var is the one fix: it maps every code point
...
# include guard (matching bin/dj-tmux-lib.sh).

# dj_composer_strip_ansi: drop every CSI escape sequence, leaving plain text.
...
# uses dj_composer_strip_ghost instead. Reads the styled text on stdin and prints
# plain text (stdin-only, matching dj_composer_strip_ghost). The character class
# includes ':' so an ITU colon-form SGR (38:2::r:g:b) is stripped whole, not left
# with a dangling tail.
dj_composer_strip_ansi() {
...
# guard (bin/dj-test-run.sh, live-harness-optin) is what catches a harness
# that starts drawing its composer with a character outside this property.
DJ_COMPOSER_UNICODE_SPACES=()
for _fm_composer_space_octal in \
  ...
  DJ_COMPOSER_UNICODE_SPACES+=("$_fm_composer_space_utf8")
done
unset -v _fm_composer_space_octal _fm_composer_space_utf8

# dj_composer_normalize_spaces_var: the ONE Unicode-whitespace mapping.
...
dj_composer_normalize_spaces_var() {  # <varname>
  local __fmns_name=$1 __fmns_text=${!1} __fmns_space
  for __fmns_space in "${DJ_COMPOSER_UNICODE_SPACES[@]}"; do
    __fmns_text=${__fmns_text//"$__fmns_space"/ }
  done
  printf -v "$__fmns_name" '%s' "$__fmns_text"
}

# dj_composer_normalize_trim_var: the one whitespace-normalizing trim shared by
...
dj_composer_normalize_trim_var() {  # <varname>
  local __fmnt_name=$1 __fmnt_text
  dj_composer_normalize_spaces_var "$__fmnt_name"
...
# dj_composer_strip_ghost: the ONE fleet-wide ANSI-aware extractor of "real typed
...
#     DJ_COMPOSER_GHOST_LUMA_MAX (default 128): how grok renders its placeholder
...
#     dark-foreground run. This assumes a DARK terminal theme, the jarvis
...
# Raising DJ_COMPOSER_GHOST_LUMA_MAX is not free: muse draws its `⟩` prompt glyph
...
dj_composer_strip_ghost() {
  LC_ALL=C awk -v lumamax="${DJ_COMPOSER_GHOST_LUMA_MAX:-128}" '
...
# proving a submitted Enter actually landed. Keeping them in bin/dj-tmux-lib.sh
...
# busy contract - what jarvis records and supervises on - is owned by
# bin/dj-busy-lib.sh, which forbids classifying a harness from rendered text.
...
DJ_DELIVERY_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working\.\.\.|Ctrl\+c:cancel|ctrl\+c to stop'
DJ_DELIVERY_CLAUDE_BUSY_REGEX_DEFAULT='esc to interrupt|…[[:space:]]+\([0-9]+[smh]'
DJ_DELIVERY_CODEX_BUSY_REGEX_DEFAULT='esc to interrupt'
DJ_DELIVERY_OPENCODE_BUSY_REGEX_DEFAULT='esc interrupt'
DJ_DELIVERY_PI_BUSY_REGEX_DEFAULT='Working\.\.\.'
DJ_DELIVERY_GROK_BUSY_REGEX_DEFAULT='Ctrl\+c:cancel'
...
# bin/dj-busy-lib.sh, never from this row.
DJ_DELIVERY_CURSOR_BUSY_REGEX_DEFAULT='ctrl\+c to stop'
DJ_DELIVERY_KIMI_BUSY_REGEX_DEFAULT='^[[:space:]]*(🌑|🌒|🌓|🌔|🌕|🌖|🌗|🌘)[[:space:]]+·[[:space:]]+'

dj_busy_lines_match() {  # [harness]
  local harness=${1:-} lines regex
  IFS= read -r -d '' lines || true
  if [ -n "${DJ_BUSY_REGEX:-}" ]; then
    regex=$DJ_BUSY_REGEX
  else
    case "$harness" in
      claude) regex=$DJ_DELIVERY_CLAUDE_BUSY_REGEX_DEFAULT ;;
      codex) regex=$DJ_DELIVERY_CODEX_BUSY_REGEX_DEFAULT ;;
      opencode) regex=$DJ_DELIVERY_OPENCODE_BUSY_REGEX_DEFAULT ;;
      pi|pi-signed) regex=$DJ_DELIVERY_PI_BUSY_REGEX_DEFAULT ;;
      grok) regex=$DJ_DELIVERY_GROK_BUSY_REGEX_DEFAULT ;;
      kimi) regex=$DJ_DELIVERY_KIMI_BUSY_REGEX_DEFAULT ;;
      cursor) regex=$DJ_DELIVERY_CURSOR_BUSY_REGEX_DEFAULT ;;
      '') regex=$DJ_DELIVERY_BUSY_REGEX_DEFAULT ;;
      *)
...
DJ_COMPOSER_AGENT_PROMPT_GLYPHS=$(printf '%s\n' '❯' '›' '⟩' '→')
DJ_COMPOSER_SHELL_PROMPT_GLYPHS=$(printf '%s\n' '>' '$' '%' '#')
...
# 2026.08.11-e8db854). DJ_COMPOSER_IDLE_RE overrides for an unverified harness;
# matching is case-insensitive.
DJ_COMPOSER_IDLE_RE_DEFAULT='^Type a message\.\.\.$|^Ask anything\.\.\.|^Plan, search, build anything$|^Add a follow-up$'

# Opencode draws a mode/model footer line INSIDE its left-bar composer
# ("Build · GPT-5.5 Fast OpenAI · high"). It is composer furniture, not typed
# text, and only the run's LAST row is ever matched against it.
DJ_COMPOSER_LEFTBAR_FOOTER_RE_DEFAULT='^(Build|Plan)[[:space:]]+·[[:space:]]+'
...
DJ_COMPOSER_CAPTURE_LINES=${DJ_COMPOSER_CAPTURE_LINES:-20}

# Pi allows a multi-line composer between its horizontal separators. Bound the
# structural candidate so two unrelated transcript rules with an arbitrarily
# large region between them can never be promoted into a composer.
DJ_COMPOSER_PI_MAX_LINES=${DJ_COMPOSER_PI_MAX_LINES:-8}
...
# dj_composer_leading_prompt_glyph_var: set <out-varname> to the ONE prompt
...
dj_composer_leading_prompt_glyph_var() {  # <out-varname> <content>
...
$DJ_COMPOSER_AGENT_PROMPT_GLYPHS
$DJ_COMPOSER_SHELL_PROMPT_GLYPHS
EOF
  printf -v "$__fmpg_out" '%s' ''
  return 1
}

# dj_composer_leading_agent_glyph_var: like the above but AGENT glyphs only.
# The bare-row shape must never be anchored by a shell glyph (dead-shell rule).
dj_composer_leading_agent_glyph_var() {  # <out-varname> <content>
...
dj_composer_leading_shell_glyph_var() {  # <out-varname> <content>
...
# dj_composer_classify_content: the single shared composer-content verdict.
...
# trimmed with.
dj_composer_classify_content() {  # <bordered> <content> [idle_re] [idle_case] [plain_content] [placeholder-position] [styled]
  ...
  content=$2
  dj_composer_normalize_trim_var content
  plain_content=${5:-$2}
  dj_composer_normalize_trim_var plain_content
  if [ "$bordered" != 1 ] && [ -z "$content" ] && [ -n "$plain_content" ]; then
    if _fm_composer_is_prompt_glyph "$plain_content" "$DJ_COMPOSER_AGENT_PROMPT_GLYPHS"; then
      printf 'empty'; return 0
    fi
    printf 'unknown'; return 0
  fi
  if _fm_composer_is_prompt_glyph "$content" "$DJ_COMPOSER_AGENT_PROMPT_GLYPHS"; then
    printf 'empty'; return 0
  fi
  if _fm_composer_is_prompt_glyph "$content" "$DJ_COMPOSER_SHELL_PROMPT_GLYPHS"; then
    if [ "$bordered" = 1 ]; then printf 'empty'; else printf 'unknown'; fi
    return 0
  fi
  [ -n "$content" ] || { printf 'empty'; return 0; }
  dj_composer_idle_matches "$content" "$idle_re" "$idle_case" && idle_collision=1
  if dj_composer_leading_prompt_glyph_var glyph "$content"; then
  ...
  if [ "$idle_collision" != 1 ] && [ "$styled" = 1 ] && [ -n "$plain_content" ]; then
    ...
    if dj_composer_leading_prompt_glyph_var plain_glyph "$plain_body"; then
      plain_body=${plain_body#*"$plain_glyph"}
    fi
    dj_composer_normalize_trim_var plain_body
    if [ "${#content}" -lt "${#plain_body}" ] \
       && dj_composer_idle_matches "$plain_body" "$idle_re" "$idle_case"; then
      case "$plain_body" in
...
#
# dj_composer_classify_screen <caps> <screen> [cursor_row] [identity]
...
# Row-scan results are returned through DJ_COMPOSER_SCAN_* globals (bash 3.2
...
_fm_composer_scan_screen() {  # <plain-screen> <cursor-or-empty> [extract-wrap]
  ...
  DJ_COMPOSER_SCAN_BOX_TOP=-1
  DJ_COMPOSER_SCAN_BOX_BOTTOM=-1
  DJ_COMPOSER_SCAN_BOX_AMBIG=0
  DJ_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=-1
  DJ_COMPOSER_SCAN_UNSAFE=0
  DJ_COMPOSER_SCAN_CURSOR_EDGE=0
  DJ_COMPOSER_SCAN_BARE_ROW=-1
  DJ_COMPOSER_SCAN_SHELL_ROW=-1
  DJ_COMPOSER_SCAN_LEFTBAR_START=-1
  DJ_COMPOSER_SCAN_LEFTBAR_END=-1
  DJ_COMPOSER_SCAN_PI_PAIR_FOUND=0
  DJ_COMPOSER_SCAN_PI_PAIR_VALID=0
  DJ_COMPOSER_SCAN_PI_OPEN=-1
  DJ_COMPOSER_SCAN_PI_CLOSE=-1
  DJ_COMPOSER_SCAN_PI_LAST_SEPARATOR=-1
  local leftbar_start=-1 pi_open=-1 pi_lines=0 pi_max
  pi_max=$DJ_COMPOSER_PI_MAX_LINES
  case "$pi_max" in ''|*[!0-9]*|0) pi_max=8 ;; esac
  while IFS= read -r line; do
    indent=${line%%[![:space:]]*}
    left_stripped="${line#"${line%%[![:space:]]*}"}"
    trimmed=$left_stripped
    dj_composer_normalize_trim_var trimmed
    kind=
    family=
    case "$trimmed" in
    ...
    if _fm_composer_pi_separator_row "$trimmed"; then
      DJ_COMPOSER_SCAN_PI_LAST_SEPARATOR=$row
      if [ "$pi_open" -ge 0 ]; then
        DJ_COMPOSER_SCAN_PI_PAIR_FOUND=1
        DJ_COMPOSER_SCAN_PI_OPEN=$pi_open
        DJ_COMPOSER_SCAN_PI_CLOSE=$row
        if [ "$pi_lines" -le "$pi_max" ]; then
          DJ_COMPOSER_SCAN_PI_PAIR_VALID=1
        else
          DJ_COMPOSER_SCAN_PI_PAIR_VALID=0
        fi
      fi
    ...
    case "$trimmed" in
      ...
      '┃'*)
        ...
        DJ_COMPOSER_SCAN_LEFTBAR_START=$leftbar_start
        DJ_COMPOSER_SCAN_LEFTBAR_END=$row
        ;;
      *) leftbar_start=-1 ;;
    esac
    ...
    if [ "$top" -lt 0 ] && dj_composer_leading_shell_glyph_var glyph "$trimmed"; then
      DJ_COMPOSER_SCAN_SHELL_ROW=$row
    elif dj_composer_leading_agent_glyph_var glyph "$trimmed"; then
      DJ_COMPOSER_SCAN_BARE_ROW=$row
    fi
    # Cursor safety: a cursor sitting on a structural edge row is never an
    # input row.
    if [ -n "$cy" ] && [ "$row" -eq "$cy" ] && dj_composer_row_has_edge "$trimmed"; then
      DJ_COMPOSER_SCAN_CURSOR_EDGE=1
    fi
    ...
    if [ "$kind" = top ] || { [ "$kind" = ascii ] && [ "$top" -lt 0 ]; }; then
      if [ -n "$cy" ] && [ "$top" -ge 0 ] && [ "$top" -lt "$cy" ] && [ "$cy" -le "$row" ]; then
        DJ_COMPOSER_SCAN_UNSAFE=1
      fi
      top=$row
      DJ_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=$row
      ...
      if [ "$top" -ge 0 ] && [ "$family" = "$current_family" ] \
        ...
        if [ -n "$cy" ]; then
          if [ "$top" -lt "$cy" ] && [ "$cy" -le "$row" ]; then
            DJ_COMPOSER_SCAN_BOX_TOP=$top
            DJ_COMPOSER_SCAN_BOX_BOTTOM=$row
            DJ_COMPOSER_SCAN_BOX_AMBIG=$geometry_ambiguous
          fi
        else
          DJ_COMPOSER_SCAN_BOX_TOP=$top
          DJ_COMPOSER_SCAN_BOX_BOTTOM=$row
        ...
        if [ "$DJ_COMPOSER_SCAN_INCOMPLETE_BOX_FROM" -lt 0 ]; then
          DJ_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=$row
        fi
        if [ -n "$cy" ]; then
      ...
      case "$current_family:$side_family" in
        rounded:single|light:single|heavy:heavy|double:double|ascii:ascii)
          ...
          if [ "$geometry_check" = 1 ]; then
            ...
            if content_spaces=$(dj_composer_geometry_spaces "$content_inner"); then
...
_fm_composer_titled_bottom_ok() {  # <family> <bottom-inner> <top-spaces>
  ...
  dj_composer_normalize_trim_var inner
  case "$family" in
    rounded|light) dash='─' ;;
    double) dash='═' ;;
    heavy) dash='━' ;;
...
# dj_composer_row_has_edge: 0 when the trimmed row starts or ends with a
...
dj_composer_row_has_edge() {  # <trimmed-row>
  local row=$1
  dj_composer_normalize_trim_var row
  case "$row" in
    '│'*|*'│'|'┃'*|*'┃'|'║'*|*'║'|'╭'*|*'╭'|'╮'*|*'╮'|\
    '┌'*|*'┌'|'┐'*|*'┐'|'╔'*|*'╔'|'╗'*|*'╗'|'┏'*|*'┏'|'┓'*|*'┓'|\
    '╰'*|*'╰'|'╯'*|*'╯'|'└'*|*'└'|'┘'*|*'┘'|'╚'*|*'╚'|'╝'*|*'╝'|\
...
# dj_composer_geometry_spaces: prove a box content row blank to the same width
...
dj_composer_geometry_spaces() {  # <content-inner> -> spaces
  local content=$1 glyph
  dj_composer_normalize_spaces_var content
  if dj_composer_leading_prompt_glyph_var glyph "$content"; then
    content=${content/"$glyph"/ }
  fi
...
_fm_composer_row_content() {  # <raw-row> <styled> -> content on stdout
  ...
  if [ "$styled" = 1 ]; then
    stripped=$(printf '%s\n' "$raw" | dj_composer_strip_ghost)
  else
    stripped=$(printf '%s\n' "$raw" | dj_composer_strip_ansi)
  fi
  dj_composer_normalize_trim_var stripped
  case "$stripped" in
...
_fm_composer_classify_rows() {  # <screen> <styled> <ambiguous> <first-row> <last-row>
  ...
  while [ "$row" -le "$last" ]; do
    ...
    state=$(dj_composer_classify_content 1 "$content" \
      "${DJ_COMPOSER_IDLE_RE:-$DJ_COMPOSER_IDLE_RE_DEFAULT}" insensitive "$plain" 1 "$styled")
    case "$state" in
      pending)
...
_fm_composer_classify_bare_row() {  # <screen> <styled> <row>
  ...
  content=$(_fm_composer_row_content "$raw" "$styled")
  plain=$(_fm_composer_row_content "$raw" 0)
  state=$(dj_composer_classify_content 0 "$content" \
    "${DJ_COMPOSER_IDLE_RE:-$DJ_COMPOSER_IDLE_RE_DEFAULT}" insensitive "$plain" 0 "$styled")
...
_fm_composer_wrap_region_ok() {  # <plain-screen> <glyph-row> <cursor-row>
  ...
  while [ "$row" -le "$cy" ]; do
    ...
    dj_composer_normalize_trim_var trimmed
    [ -n "$trimmed" ] || return 1
    if dj_composer_row_has_edge "$trimmed"; then return 1; fi
    if dj_composer_leading_shell_glyph_var glyph "$trimmed"; then return 1; fi
    row=$((row + 1))
  done
  return 0
}
...
_fm_composer_classify_bare_wrap() {  # <screen> <styled> <glyph-row> <cursor-row>
  ...
  while [ "$row" -le "$cy" ]; do
    raw=$(_fm_composer_screen_row "$row" "$screen")
    content=$(_fm_composer_row_content "$raw" "$styled")
    if [ "$row" -eq "$g" ] && dj_composer_leading_agent_glyph_var glyph "$content"; then
      content=${content#*"$glyph"}
    fi
    dj_composer_normalize_trim_var content
...
_fm_composer_classify_leftbar() {  # <screen> <styled> <first-row> <last-row>
  ...
  footer_re=${DJ_COMPOSER_LEFTBAR_FOOTER_RE:-$DJ_COMPOSER_LEFTBAR_FOOTER_RE_DEFAULT}
  row=$first
  while [ "$row" -le "$last" ]; do
    raw=$(_fm_composer_screen_row "$row" "$screen")
    content=$(_fm_composer_row_content "$raw" "$styled")
    ...
    if [ "$placeholder_position" = 1 ] \
       && dj_composer_idle_matches "$content" "${DJ_COMPOSER_IDLE_RE:-$DJ_COMPOSER_IDLE_RE_DEFAULT}" insensitive; then
      row=$((row + 1)); continue
    fi
    if [ "$row" -eq "$last" ] \
       && dj_composer_idle_matches "$content" "$footer_re" sensitive; then
      row=$((row + 1)); continue
    fi
    pending_seen=1
    row=$((row + 1))
...
_fm_composer_select_cursorless() {
  ...
  DJ_COMPOSER_SELECTED_KIND=
  DJ_COMPOSER_SELECTED_FIRST=-1
  DJ_COMPOSER_SELECTED_LAST=-1
  DJ_COMPOSER_SELECTED_AMBIG=0
  if [ "$DJ_COMPOSER_SCAN_BOX_BOTTOM" -ge 0 ]; then
    generic=$DJ_COMPOSER_SCAN_BOX_BOTTOM
    DJ_COMPOSER_SELECTED_KIND=box
    DJ_COMPOSER_SELECTED_FIRST=$((DJ_COMPOSER_SCAN_BOX_TOP + 1))
    DJ_COMPOSER_SELECTED_LAST=$((DJ_COMPOSER_SCAN_BOX_BOTTOM - 1))
    DJ_COMPOSER_SELECTED_AMBIG=$DJ_COMPOSER_SCAN_BOX_AMBIG
  fi
  if [ "$DJ_COMPOSER_SCAN_BARE_ROW" -gt "$generic" ]; then
    generic=$DJ_COMPOSER_SCAN_BARE_ROW
    DJ_COMPOSER_SELECTED_KIND=bare
    DJ_COMPOSER_SELECTED_FIRST=$DJ_COMPOSER_SCAN_BARE_ROW
    DJ_COMPOSER_SELECTED_LAST=$DJ_COMPOSER_SCAN_BARE_ROW
  fi
  if [ "$DJ_COMPOSER_SCAN_LEFTBAR_END" -gt "$generic" ]; then
    generic=$DJ_COMPOSER_SCAN_LEFTBAR_END
    DJ_COMPOSER_SELECTED_KIND=leftbar
    DJ_COMPOSER_SELECTED_FIRST=$DJ_COMPOSER_SCAN_LEFTBAR_START
    DJ_COMPOSER_SELECTED_LAST=$DJ_COMPOSER_SCAN_LEFTBAR_END
  fi
  if [ "$DJ_COMPOSER_SCAN_INCOMPLETE_BOX_FROM" -gt "$generic" ]; then
    DJ_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$DJ_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
     && [ "$DJ_COMPOSER_SCAN_PI_CLOSE" -gt "$generic" ] \
     && [ "$generic" -lt "$DJ_COMPOSER_SCAN_PI_OPEN" ]; then
    generic=$DJ_COMPOSER_SCAN_PI_CLOSE
    DJ_COMPOSER_SELECTED_KIND=pi
    DJ_COMPOSER_SELECTED_FIRST=$((DJ_COMPOSER_SCAN_PI_OPEN + 1))
    DJ_COMPOSER_SELECTED_LAST=$((DJ_COMPOSER_SCAN_PI_CLOSE - 1))
  fi
  if [ "$DJ_COMPOSER_SCAN_PI_PAIR_FOUND" = 0 ] \
     && [ "$DJ_COMPOSER_SCAN_PI_LAST_SEPARATOR" -gt "$generic" ]; then
    DJ_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$DJ_COMPOSER_SCAN_SHELL_ROW" -gt "$generic" ]; then
    DJ_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$DJ_COMPOSER_SELECTED_KIND" = bare ]; then
    next=$((DJ_COMPOSER_SELECTED_LAST + 1))
    while :; do
      raw=$(_fm_composer_screen_row "$next" "$plain")
      trimmed=$raw
      dj_composer_normalize_trim_var trimmed
      [ -n "$trimmed" ] || break
      dj_composer_row_has_edge "$trimmed" && break
      DJ_COMPOSER_SELECTED_LAST=$next
      next=$((next + 1))
    done
  fi
  if [ "$DJ_COMPOSER_SELECTED_KIND" = box ] \
     || [ "$DJ_COMPOSER_SELECTED_KIND" = leftbar ]; then
    boundary=$DJ_COMPOSER_SELECTED_LAST
    if [ "$DJ_COMPOSER_SELECTED_KIND" = box ]; then
      boundary=$DJ_COMPOSER_SCAN_BOX_BOTTOM
    else
      next=$((boundary + 1))
      raw=$(_fm_composer_screen_row "$next" "$plain")
      trimmed=$raw
    ...
    trimmed=$raw
    dj_composer_normalize_trim_var trimmed
    if [ -n "$trimmed" ] && ! dj_composer_row_has_edge "$trimmed"; then
      DJ_COMPOSER_SELECTED_KIND=
      return 1
    fi
  fi
  [ -n "$DJ_COMPOSER_SELECTED_KIND" ]
}

dj_composer_extract_selected_content() {  # <caps> <screen>
...
$caps
EOF
  plain=$(printf '%s\n' "$screen" | dj_composer_strip_ansi)
  _fm_composer_scan_screen "$plain" '' 1
  _fm_composer_select_cursorless "$plain" || return 1
  row=$DJ_COMPOSER_SELECTED_FIRST
  while [ "$row" -le "$DJ_COMPOSER_SELECTED_LAST" ]; do
    raw=$(_fm_composer_screen_row "$row" "$screen")
    content=$(_fm_composer_row_content "$raw" "$styled")
    placeholder_position=0
    case "$DJ_COMPOSER_SELECTED_KIND" in
      bare)
        if [ "$row" -eq "$DJ_COMPOSER_SELECTED_FIRST" ] \
           && dj_composer_leading_agent_glyph_var glyph "$content"; then
          content=${content#*"$glyph"}
        fi
        ;;
      leftbar)
        ...
        if [ -z "$content" ]; then
          :
        elif [ "$leading_blank" = 1 ] && [ "$row" -gt "$DJ_COMPOSER_SELECTED_FIRST" ]; then
          placeholder_position=1
          leading_blank=0
        else
          leading_blank=0
      ...
      box)
        if [ "$prompt_row" -lt 0 ] \
           && dj_composer_leading_prompt_glyph_var glyph "$content"; then
          prompt_row=$row
          placeholder_position=1
          if _fm_composer_is_prompt_glyph "$glyph" "$DJ_COMPOSER_SHELL_PROMPT_GLYPHS"; then
            prompt_is_shell=1
          fi
          content=${content#*"$glyph"}
    ...
    if [ -z "$content" ] \
       || { { [ "$DJ_COMPOSER_SELECTED_KIND" = leftbar ] \
              || { [ "$DJ_COMPOSER_SELECTED_KIND" = box ] && [ "$prompt_is_shell" = 1 ]; }; } \
            && [ "$placeholder_position" = 1 ] \
            && dj_composer_idle_matches "$content" "${DJ_COMPOSER_IDLE_RE:-$DJ_COMPOSER_IDLE_RE_DEFAULT}" insensitive; } \
       || { [ "$DJ_COMPOSER_SELECTED_KIND" = leftbar ] \
            && [ "$row" -eq "$DJ_COMPOSER_SELECTED_LAST" ] \
            && dj_composer_idle_matches "$content" "$footer_re" sensitive; }; then
      row=$((row + 1))
      continue
    fi
...
EOF
  ...
  if [ -n "$cy" ]; then
    ...
    if [ "$DJ_COMPOSER_SCAN_UNSAFE" = 1 ]; then
      printf 'unknown'; return 0
    fi
    if [ "$DJ_COMPOSER_SCAN_BOX_TOP" -ge 0 ]; then
      _fm_composer_classify_rows "$screen" "$styled" "$DJ_COMPOSER_SCAN_BOX_AMBIG" \
        "$((DJ_COMPOSER_SCAN_BOX_TOP + 1))" "$((DJ_COMPOSER_SCAN_BOX_BOTTOM - 1))"
      return 0
    fi
    if [ "$DJ_COMPOSER_SCAN_LEFTBAR_START" -ge 0 ] \
       && [ "$cy" -ge "$DJ_COMPOSER_SCAN_LEFTBAR_START" ] \
       && [ "$cy" -le "$DJ_COMPOSER_SCAN_LEFTBAR_END" ]; then
      _fm_composer_classify_leftbar "$screen" "$styled" \
        "$DJ_COMPOSER_SCAN_LEFTBAR_START" "$DJ_COMPOSER_SCAN_LEFTBAR_END"
      return 0
    fi
    if [ "$DJ_COMPOSER_SCAN_BARE_ROW" -ge 0 ] && [ "$cy" -eq "$DJ_COMPOSER_SCAN_BARE_ROW" ]; then
      if [ "$DJ_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
         && [ "$cy" -gt "$DJ_COMPOSER_SCAN_PI_OPEN" ] \
         && [ "$cy" -lt "$DJ_COMPOSER_SCAN_PI_CLOSE" ]; then
    ...
    # and earns its retry.
    if [ "$DJ_COMPOSER_SCAN_BARE_ROW" -ge 0 ] && [ "$cy" -gt "$DJ_COMPOSER_SCAN_BARE_ROW" ] \
       && _fm_composer_wrap_region_ok "$plain" "$DJ_COMPOSER_SCAN_BARE_ROW" "$cy"; then
      _fm_composer_classify_bare_wrap "$screen" "$styled" "$DJ_COMPOSER_SCAN_BARE_ROW" "$cy"
      return 0
    fi
    if [ "$DJ_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
       && [ "$cy" -gt "$DJ_COMPOSER_SCAN_PI_OPEN" ] \
    ...
    if [ "$DJ_COMPOSER_SCAN_CURSOR_EDGE" = 1 ]; then
      printf 'unknown'; return 0
    fi
  ...
  case "$DJ_COMPOSER_SELECTED_KIND" in
    pi)
      ...
      ;;
    box)
      _fm_composer_classify_rows "$screen" "$styled" "$DJ_COMPOSER_SELECTED_AMBIG" \
        "$DJ_COMPOSER_SELECTED_FIRST" "$DJ_COMPOSER_SELECTED_LAST"
      ;;
    bare)
      if [ "$DJ_COMPOSER_SELECTED_LAST" -gt "$DJ_COMPOSER_SELECTED_FIRST" ]; then
        _fm_composer_classify_bare_wrap "$screen" "$styled" \
          "$DJ_COMPOSER_SELECTED_FIRST" "$DJ_COMPOSER_SELECTED_LAST"
      elif [ "$DJ_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
         && [ "$DJ_COMPOSER_SCAN_BARE_ROW" -gt "$DJ_COMPOSER_SCAN_PI_OPEN" ] \
         && [ "$DJ_COMPOSER_SCAN_BARE_ROW" -lt "$DJ_COMPOSER_SCAN_PI_CLOSE" ]; then
        _fm_composer_classify_bare_pi_overlap "$screen" "$styled" "$has_identity" "$identity" \
          "$DJ_COMPOSER_SCAN_BARE_ROW"
      else
        _fm_composer_classify_bare_row "$screen" "$styled" "$DJ_COMPOSER_SCAN_BARE_ROW"
      fi
      ;;
    leftbar)
      _fm_composer_classify_leftbar "$screen" "$styled" \
...
# dj_composer_submit_retry_core: the ONE verify-and-retry-Enter submit loop
...
# dj_composer_queued_enter_verdict; no shape knowledge lives in any loop.
dj_composer_submit_retry_core() {  # <send-key-fn> <state-fn> <target> <retries> <enter-sleep> [expected-label]
...
# dj_composer_queued_enter_verdict: the ONE busy-queued-Enter policy.
...
# Adapters supply their own busy primitive (tmux: dj_pane_is_busy; herdr:
# native agent_status=working, or a rendered busy footer on an idle native
# baseline). This function does not read a pane.
dj_composer_queued_enter_verdict() {  # <composer-state> <busy|idle|unknown>
...
_fm_composer_classify_pi_rows() {  # <screen> <styled>
  ...
  row=$((DJ_COMPOSER_SCAN_PI_OPEN + 1))
  while [ "$row" -lt "$DJ_COMPOSER_SCAN_PI_CLOSE" ]; do
    raw=$(_fm_composer_screen_row "$row" "$screen")
    content=$(_fm_composer_row_content "$raw" "$styled")
    dj_composer_normalize_trim_var content
    if [ -n "$content" ]; then
...
_fm_composer_pi_verdict() {  # <screen> <styled> <has_identity> <identity>
  ...
  if [ "$agent" != pi ] || [ "$DJ_COMPOSER_SCAN_PI_PAIR_VALID" != 1 ]; then
    printf 'unknown'
    return 0
  fi
