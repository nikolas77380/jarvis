// Jarvis supervision branch for Pi (docs/pi-supervision-branch.md).
//
...
// actionable wake here (lib/dj-branch-dispatch.ts); the branch handles it with
// real tools and reports through the dj_branch_report custom tool, which
// writes the durable outcome store FIRST (bin/dj-branch-outcome.sh) and then
// merges an append-only note to main's tail. Main's captain/assistant dialog
// is mirrored into the branch as read-only dj-main-mirror context at main's
...
//
// Prefix stability (the cache contract, owner: bin/dj-branch-prompt.sh
...
// Pi start acquires the lock only when the session runs dj-session-start.sh,
...
// inside the branch's own shell. bin/dj-lease-lib.sh documents the grade and
...
import {
  ...
  JARVIS_CALM_PRESENTATION_EVENT,
} from "./lib/dj-calm-visibility.ts";
import {
  activateEligibleRowsOwner,
  deactivateEligibleRowsOwner,
  DJ_BRANCH_DISPATCH_EVENT,
  releaseEligibleRowsSnapshot,
  scopeForUnreadWake,
  writeEligibleRowsSnapshot,
  type BranchDispatchOffer,
} from "./lib/dj-branch-dispatch.ts";
import { encodeJarvisOperationalInput } from "./lib/dj-operational-input.ts";

const extensionFile = fileURLToPath(import.meta.url);
const extensionDir = dirname(extensionFile);
const root = resolve(extensionDir, "../..");
const fmHome = process.env.DJ_HOME || process.env.DJ_ROOT_OVERRIDE || root;
const fmRoot = process.env.DJ_ROOT_OVERRIDE || root;
const state = process.env.DJ_STATE_OVERRIDE || `${fmHome}/state`;
const config = process.env.DJ_CONFIG_OVERRIDE || `${fmHome}/config`;
...
const promptScript = join(fmRoot, "bin", "dj-branch-prompt.sh");
const outcomeScript = join(fmRoot, "bin", "dj-branch-outcome.sh");
const leaseScript = join(fmRoot, "bin", "dj-lease.sh");
const wakeGrantScript = join(fmRoot, "bin", "dj-wake-grant.sh");
...
const BRANCH_TOOL_NAMES = ["read", "bash", "dj_branch_report"] as const;
...
const branchCacheKey = `dj-branch-${createHash("sha256").update(fmHome).digest("hex").slice(0, 24)}`;
...
// package (tests/dj-pi-primary-types.test.sh) the moment Pi adds or removes a
// level, in either direction, so the list cannot drift into a stale Jarvis
// catalog.
...
// U+2063 operational prefix; the plain legacy form starts with JARVIS.
function isOperationalUserText(text: string): boolean {
  return text.startsWith("⁣") || /^JARVIS[ _]/.test(text);
}
...
export default function (pi: ExtensionAPI) {
  ...
  function releaseBranchLeases(expectedGeneration: number): boolean {
    ...
    try {
      const result = spawnSync("bash", [leaseScript, "release-actor", "--actor", "branch"], {
        cwd: fmRoot,
        encoding: "utf8",
        env: { ...scriptEnv, DJ_SUPERVISION_ACTOR: "branch" },
      });
  ...
  function runOutcomeScript(args: string[]): { ok: boolean; stdout: string; detail: string } {
    try {
      ...
      return {
        ok: false,
        stdout: "",
        detail: `dj-branch-outcome.sh exited ${result.status ?? "none"}: ${(result.stderr || "").trim()}`,
      };
    } catch (error) {
  ...
  // main's dj_branch_outcomes tool still reads it on demand.
  function mergeIntoMain(
    ...
    if (verdict === "captain") {
      const message = { customType: "dj-branch-merge", content: `${task}: ${summary}`, display: false };
      pi.sendMessage(message, { triggerTurn: true, deliverAs: "followUp" });
    } else {
      const message = { customType: "dj-branch-merge", content: `${MERGE_NOTE_BOAT} ${task}: ${summary}`, display: !(task === "fleet" && silent) };
      if (mainStreaming) {
  ...
  async function createBranch(branchGeneration: number): Promise<AgentSession> {
    ...
    if (prompt.status !== 0 || !prompt.stdout || prompt.stdout.length < 1024) {
      throw new Error(
        `dj-branch-prompt.sh did not produce a usable branch prompt (status=${prompt.status ?? "none"}): ${(prompt.stderr || "").trim()}`,
      );
    }
    ...
    const bashTool = createBashToolDefinition(fmRoot, {
      spawnHook: (context) => {
        ...
        return {
          ...
          // model lives in bin/dj-lease-lib.sh.
          command: `readonly DJ_SUPERVISION_ACTOR DJ_LEASE_HOLDER_PID
(
${context.command}
)`,
          env: {
            ...context.env,
            ...scriptEnv,
            DJ_SUPERVISION_ACTOR: "branch",
            DJ_LEASE_HOLDER_PID: leaseHolderPid,
          },
        };
      },
    });
  ...
  async function flushMirror(session: AgentSession, expectedGeneration: number): Promise<void> {
    ...
    while (pendingMirror.length > 0) {
      ...
      await session.sendCustomMessage(
        { customType: "dj-main-mirror", content: `[${item.tag}] ${item.text}`, display: false },
        {},
      );
  ...
  async function fallbackToMain(message: string, detail: string): Promise<void> {
    const body = `JARVIS WATCHER WAKE: ${message}\n\nRun bin/dj-wake-drain.sh first and handle the queued wake. (Supervision branch unavailable, falling back to main: ${detail})`;
    let content = body;
    try {
      ...
      content = encodeJarvisOperationalInput("watcher", body);
    } catch {
    ...
    branchChain = branchChain
      .then(async () => {
        ...
        await session.prompt(
          `JARVIS SUPERVISION WAKE: ${message}\n\nHandle this per your operating procedure and finish with dj_branch_report.`,
        );
  ...
  pi.events?.on?.(DJ_BRANCH_DISPATCH_EVENT, (data) => {
  ...
  // dialog and with no parallel Jarvis model or effort list. The effort
  // step follows the model step because the model decides which levels exist.
  pi.registerCommand?.("supervision-model", {
    description: "Pick the model and reasoning effort Jarvis's Pi supervision branch uses, or follow main's.",
  ...
  // parallel Jarvis picker catalog. Cancelling leaves the current effort
  ...
  pi.events?.on?.(JARVIS_CALM_PRESENTATION_EVENT, (data) => {
  ...
  pi.registerTool?.({
    ...
    renderCall: (_args, theme, context) => {
      ...
      shellState.call = new Text(theme.fg("toolTitle", theme.bold("dj_branch_outcomes")), 0, 0);
  ...
  pi.registerMessageRenderer?.("dj-branch-merge", (message, _options, theme) => {
