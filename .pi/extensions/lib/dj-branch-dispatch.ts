...
// DJ_BRANCH_DISPATCH_EVENT. A live, enabled branch extension calls accept()
...
// main can repair the watcher cycle (dj_watch_arm_pi lives on main).

export const DJ_BRANCH_DISPATCH_EVENT = "dj-branch-supervision:dispatch";
...
export interface UnreadWakeScope {
   ...
   * bin/dj-wake-drain.sh implements against this list). Empty whenever
   * `eligible` is false.
   */
  eligibleSeqs: string[];
  /**
...
// "Per-actor acknowledgement"). bin/dj-wake-drain.sh never reclassifies a row
...
// (dj-primary-pi-watch.ts forces every check-kind TRIGGER to main), so nothing
...
// this repo's dj_wake_append could never have produced (an unknown kind, or a
...
export function scopeForUnreadWake(state: string, heartbeat: boolean): UnreadWakeScope {
  ...
  for (const line of rows) {
    ...
    if (kind === "signal") {
      ...
      project = metadata.get(key) ?? metadata.get(key.replace(/^dj-/, "")) ?? "";
    } else {
      // A kind dj_wake_append never emits: structural corruption, not an
...
// The exact state-relative filename bin/dj-wake-drain.sh reads for a
// DJ_SUPERVISION_ACTOR=branch drain or ack (its header is the single owner of
  ...
  try {
    const result = spawnSync("bash", [grantScript, ...args], {
      ...
      env: {
        ...
        DJ_WAKE_QUEUE: `${state}/.wake-queue`,
        DJ_WAKE_QUEUE_LOCK: `${state}/.wake-queue.lock`,
      },
    });
