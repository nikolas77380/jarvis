...
// and throws if it is missing; dj-calm.ts catches that and skips only this adapter with a
...
import { calmPresentationHides } from "./dj-calm-visibility.ts";
import { classifyJarvisCurrentOperationalText } from "./dj-operational-input.ts";
...
const CALM_OPERATIONAL_USER_LAYOUT_PATCH = Symbol.for(
  "jarvis:calm-operational-user-layout:pi-0.81.1",
);
...
export function installCalmOperationalUserLayout(): void {
  ...
  const isOperationalInput = (text: string): boolean => {
    ...
    return (
      classifyJarvisCurrentOperationalText(text) !== undefined ||
  ...
  if (typeof InteractiveMode !== "function") {
    throw new Error("Jarvis Calm requires Pi InteractiveMode");
  }
  ...
  if (typeof originalAddMessageToChat !== "function") {
    throw new Error("Jarvis Calm requires Pi InteractiveMode.addMessageToChat");
  }

  const UserMessageComponent = PiCodingAgent.UserMessageComponent;
  if (typeof UserMessageComponent !== "function") {
    throw new Error("Jarvis Calm requires Pi UserMessageComponent");
  }
