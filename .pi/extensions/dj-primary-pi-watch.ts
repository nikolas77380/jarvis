// Jarvis primary watcher bridge for Pi.
//
...
const armScript = `${fmRoot}/bin/dj-watch-arm.sh`;
const marker = `${state}/.pi-watch-extension-loaded`;
const extensionVersion = `sha256:${createHash("sha256").update(readFileSync(extensionFile)).digest("hex")}`;
const retryBaseMs = positiveInteger("DJ_WATCH_REARM_RETRY_BASE_MS", 250);
const retryMaxMs = positiveInteger("DJ_WATCH_REARM_RETRY_MAX_MS", 4000);
const retryLimit = positiveInteger("DJ_WATCH_REARM_RETRY_LIMIT", 5);
// 35s on Windows so the budget stays above arm's MSYS confirm default (30s in
// bin/dj-watch-arm.sh): a slow but successful Git Bash cold start must not be
// SIGTERMed mid-confirmation. Conditioned on win32 so other platforms keep 12s.
const armReadyTimeoutMs = positiveInteger(
  "DJ_PI_ARM_READY_TIMEOUT_MS",
  process.platform === "win32" ? 35000 : 12000,
);
const armRetireTimeoutMs = positiveInteger("DJ_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call dj_watch_arm_pi again only after a later notification says the cycle is missing, failed, or unhealthy";
...
function classifyClose(stdout: string, stderr: string, code: number | null, signal: NodeJS.Signals | null): CloseClassification {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - dj-watch-arm.sh exited ${code}${combined ? `\n${combined}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
export default function (pi: ExtensionAPI) {
    ...
    try {
      const result = spawnSync(
        ...
        [armScript, "--handling-delivered", recovery.generation, "--watcher-pid", recovery.watcherPid],
        {
          cwd: fmRoot,
          encoding: "utf8",
          env: { ...process.env, DJ_HOME: fmHome, DJ_STATE_OVERRIDE: state, DJ_ROOT_OVERRIDE: fmRoot },
        },
      );
  ...
  function offerWakeToBranch(message: string): boolean {
    ...
    pi.events?.emit?.(DJ_BRANCH_DISPATCH_EVENT, offer);
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    const ownership = lockOwnership();
    if (ownership === "other") return { ok: false, message: "watcher: read-only - session lock is held by another jarvis session" };
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/dj-session-start.sh to reclaim it, then call dj_watch_arm_pi to re-arm",
      };
    }
    markLoaded();
    if (owner.child) {
    ...
    const env = {
      ...
      DJ_WATCH_ARM_SCRIPT: armScript,
      DJ_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
    };
    const armChild = spawn("bash", ["-lc", "config_dir=\"${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}\"; [ -f \"$config_dir/x-mode.env\" ] && . \"$config_dir/x-mode.env\"; exec \"$DJ_WATCH_ARM_SCRIPT\" --restart"], {
      cwd: fmRoot,
      env,
  ...
  pi.registerCommand?.("dj-watch-arm-pi", {
    description: "Arm jarvis watcher supervision through the Pi extension instead of foreground bash.",
  ...
  pi.registerTool?.({
    name: "dj_watch_arm_pi",
    label: "Arm jarvis watcher",
    description: "Start the first required Pi watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the Pi extension re-arms automatically. Never run bin/dj-watch-arm.sh through bash.",
    ...
    promptGuidelines: [
      "Call dj_watch_arm_pi only for the first required cycle or after a notification says the cycle is missing, failed, or unhealthy. Do not call it after ordinary work, turn completion, or ordinary signal, stale, check, or heartbeat handling because the Pi extension owns re-arming. Never run bin/dj-watch-arm.sh through bash.",
    ],
    ...
    renderCall: (_args, theme, context) => {
      ...
      if (calmPresentation.stockExportRendering) {
        return new Text(theme.fg("toolTitle", theme.bold("dj_watch_arm_pi")), 0, 0);
      }
      const state = context.state as WatchToolShellState;
      state.call = new Text(theme.fg("toolTitle", theme.bold("dj_watch_arm_pi")), 0, 0);
