...
import {
  classifyJarvisCurrentOperationalText,
  encodeJarvisOperationalInput,
} from "./lib/dj-operational-input.ts";
...
function runSessionstartHook(source: string): Promise<string> {
  return new Promise((resolveResult) => {
    const child = spawn(`${root}/bin/dj-sessionstart-run.sh`, ["--source", source], {
...
async function injectSessionstart(pi: ExtensionAPI, source: string): Promise<void> {
  ...
  try {
    ...
    const content = classifyJarvisCurrentOperationalText(raw)
      ? raw
      : encodeJarvisOperationalInput("session-start", raw);
    pi.sendMessage({
      customType: "jarvis-sessionstart-nudge",
      content,
      display: false,
...
function runGuard(): Promise<{ code: number; stderr: string }> {
  return new Promise((resolveResult) => {
    const child = spawn(`${root}/bin/dj-turnend-guard.sh`, {
...
// PreToolUse seatbelts (bin/dj-arm-pretool-check.sh, docs/arm-pretool-check.md;
// bin/dj-cd-pretool-check.sh, docs/cd-guard.md). Both piggyback on this same
...
function runPretoolCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("dj-arm-pretool-check.sh", command);
}

function runCdCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("dj-cd-pretool-check.sh", command);
}
