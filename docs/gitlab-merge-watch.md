...
`tests/dj-pr-check-security.test.sh` asserts that neither `bin/dj-pr-lib.sh` nor `bin/dj-pr-poll.sh` contains the string `gitlab.com` at all.
...
Its JSON would need a JSON processor, and `jq` is not one of jarvis's common tools, so the state is read from glab's own field output instead.
Only an exact `merged` wakes jarvis, so a changed output format produces no wake rather than a false merge.
...
```
$ dj-pr-check.sh e1 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
armed: state/e1.check.sh
$ dj-pr-check.sh e2 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
armed: state/e2.check.sh
$ dj-pr-check.sh e3 https://gitlab.example/group/subgroup/project/-/merge_requests/7
...
```
$ PATH="$noglab" dj-pr-poll.sh --validated $(tr '\n' ' ' < state/e1.pr-poll)
$ PATH="$noglab" dj-pr-poll.sh --validated $(tr '\n' ' ' < state/e3.pr-poll)
```
...
```
$ PATH="$noglab" dj-pr-check.sh e5 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
...
```
$ PATH="$noglab" dj-pr-check.sh e6 https://github.com/kunchenguid/firstmate/pull/750
armed: state/e6.check.sh
```

## Upgrade path from an existing armed watch

The stored record gained the provider tag, so its version moved to `dj-pr-poll-registration-v2` and a record written by the previous release no longer parses.
...
dj-pr-poll-registration-v1
$ dj-pr-check-migrate.sh --checks-safe
...
`bin/dj-pr-merge.sh` now merges a GitLab merge request through the same recording and the same guards a GitHub pull request gets.
Every run below used a throwaway `DJ_HOME`, so no live task record was touched, and a `glab` wrapper that refused any `merge` subcommand outright, so no merge could reach the forge even if a check were wrong.
...
```
$ PATH="$noglab" dj-pr-merge.sh e5 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
error: merging a GitLab merge request requires glab on PATH
$ echo $?
1
$ PATH="$nojq" dj-pr-merge.sh e6 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
...
`jq` is not one of jarvis's common tools, which is why the watch poll reads glab's field output instead.
...
```
$ dj-pr-merge.sh e1 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
armed: state/e1.check.sh
...
```
$ dj-pr-merge.sh e2 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
armed: state/e2.check.sh
...
```
$ dj-pr-merge.sh e4 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
...
The remaining refusal conditions, and the merge itself, are covered by `tests/dj-pr-merge.test.sh` against fixtures.
...
`bin/dj-pr-check.sh` records `pr_head=` only for GitHub, where `gh` exposes the head commit as a selectable field.
It is optional by design, and the other consumers already treat it that way: `bin/dj-teardown.sh` reads the head from the forge at teardown and falls back to its provider-agnostic content check, and `bin/dj-review-diff.sh` resolves the head from the remote when none is recorded.
