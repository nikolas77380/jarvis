...
`bin/dj-doc-audience-check.sh` validates exact inventory coverage, README setup routing, required owner pointers, and local link targets.
...
- `agent-runtime` is loaded or rendered as an operating contract for Jarvis agents rather than read as product documentation.

The knowledge-placement policy is owned by [`jarvis-coding-guidelines`](../.agents/skills/jarvis-coding-guidelines/SKILL.md).
