...
Jarvis agents load [`jarvis-orca`](../.agents/skills/jarvis-orca/SKILL.md) before operating or recovering this backend.
...
Select Orca with local `config/backend` containing `orca`, `DJ_BACKEND=orca` for one launch, or an explicit request to Jarvis.
It is never auto-detected.

Before any spawn mutates repository state, Jarvis requires `orca status --json` to report `reachable=true` and `state="ready"`.
...
Routine supervision uses the recorded endpoint through `bin/dj-peek.sh <id>` and `DJ_HOME=<home> bin/dj-send.sh <id> '<text>'`.
Enter and Ctrl-C are supported; Escape is not.

## Task shape and metadata

Each task has one Orca-managed git worktree and one Orca terminal.
`dj-spawn.sh` does not call Treehouse for Orca tasks.
...
`window=` remains the caller-facing Jarvis alias.
...
`dj-peek.sh` reads with `orca terminal read`.
An ordinary metadata-routed `dj-send.sh` text steer becomes a durable steering-inbox record, and only its best-effort constant doorbell passes through Orca's submit machinery.
On the typed plane, `dj-send.sh` verifies composer clearance through the fleet-wide classifier in `bin/dj-composer-lib.sh`, retrying Enter without retyping when a slash popup first fills an argument placeholder.
...
Cleanup keeps all shared Jarvis safety checks.
...
After those checks, Jarvis closes the exact terminal and releases the exact worktree with Orca's worktree command.
