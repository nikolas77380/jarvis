...
Jarvis can propagate a W3C [`traceparent`](https://www.w3.org/TR/trace-context/) to every agent it spawns so an external observer can identify each task as exactly one trace and correlate everything that task runs under that one identity.
...
This document is the rationale and current-behavior guide; `docs/configuration.md` owns the configuration schema, `bin/dj-trace-context-lib.sh`'s header owns the exact mechanics, and [`verification/trace-context.md`](verification/trace-context.md) records the repeatable test evidence.

## Why this is a source change at all

Jarvis's durable operational artifacts already let a downstream observer derive logical task identity and lifecycle.
...
When enabled, for each spawn Jarvis resolves one W3C `traceparent` carrier for the task - minted as a fresh root on the task's first spawn and reused verbatim from the meta on relaunch - and:
...
Ship and scout spawns reach that site on every spawn backend (`tmux`, `herdr`, `zellij`, `orca`, `cmux`); a Secondmate reaches it on every backend that accepts a Secondmate spawn (`tmux`, `herdr`, `zellij`), because `bin/dj-spawn.sh` rejects a Secondmate on `orca` and `cmux`.

### Remote Secondmate routes

A Secondmate on a [remote route](remote-secondmates.md) never reaches that export site in the parent's own process: the parent hands the launch to the configured host, which runs its own `bin/dj-spawn.sh` there.
...
The enablement decision travels with it exactly as on the local path: the remote home inherits `config/trace-context` as declared inherited material and the new Secondmate process receives the parent's frozen `DJ_TRACE_CONTEXT=on|off` snapshot.
...
Each locked `bin/dj-session-start.sh` run resolves that home's `config/trace-context` plus `DJ_TRACE_CONTEXT` exactly once into session-scoped effective state.
...
When the primary launches a Secondmate, local or remote, it propagates `config/trace-context` into the Secondmate home and passes the primary session's frozen decision as a non-empty `DJ_TRACE_CONTEXT=on|off` launch override.
...
Jarvis does not monitor setting drift, detect mismatches, refuse launches, or automatically stop or restart any home.
...
- **A recorded carrier keeps its flags verbatim.**
  ...
  Jarvis chooses the flag only when it mints a *root*, which is the only way a new carrier is created.
- **Cost and privacy consequence.**
  `01` records a sampling *decision*, and a conforming downstream parent-based sampler will honor it - but it does not by itself guarantee that any collector stores a span, and Jarvis emits no spans of its own; it only sets the flag on the carrier.
  ...
  An operator who wants unsampled roots or head-sampling owns that downstream or via a later, explicitly-scoped option; Jarvis does not embed a sampler.

## Safety

- **Default-off.**
  With no `config/trace-context` and no `DJ_TRACE_CONTEXT`, a fresh spawn or actual relaunch injects nothing and writes no `traceparent=` line, so the generated meta and the launch environment are unchanged.
...
- **What is and is not exposed.**
  A Jarvis-*minted* root uses a random id and reads no prompt, path, task prose, credential, or arbitrary environment key, so Jarvis never *originates* sensitive data in the carrier.
  Every carrier Jarvis injects is either such a mint or the same task's previously recorded carrier reused verbatim; ambient `TRACEPARENT` is never read, so no caller-controlled bytes enter a new carrier.
...
- **Fail-independent.**
  ...
  If the pre-launch carrier export fails, Jarvis omits the `traceparent=` metadata claim and still launches the task.
  If the backend reports that failed trace input could not be cleared, Jarvis refuses to append the launch command rather than risk launching with an unknown partial carrier.
  If recording the carrier fails after export, Jarvis unsets `TRACEPARENT` in the launch command and still launches the task, so the child never receives an identity absent from its metadata.
- **Metadata-only.**
...
Jarvis learns nothing about OpenTelemetry, any exporter, collector, storage, or UI.
