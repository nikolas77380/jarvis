...
This document owns the version-scoped feasibility evidence, Pi transcript taxonomy, and supported-API boundaries for Jarvis calm mode.
[`calm.md`](calm.md) owns the current user-facing `/calm` usage and limitation contract.

## Required extension surface

A qualifying implementation must auto-load from the trusted project, persist the toggle choice for the effective Jarvis home across Pi session starts and resumes, keep working activity visible, emit no Calm status row, redraw already-rendered controllable rows, remove supported hidden rows without gaps, restore ordinary rendering, and leave delivery, tool execution, model context, session storage, export and share operation, diagnostics, and expansion state unchanged.
...
`tests/dj-calm-pi-extension.test.sh` records the installed Pi version as evidence without gating on it and covers both newer synthetic versions and an unavailable adapter seam.
...
Pi loads project-local extensions before global or CLI-configured extensions, so Jarvis's tracked Calm extension previously won those collisions even when its persisted preference was off.
...
`tests/dj-calm-pi-extension.test.sh` covers the resulting split contract: no load-time claims while Calm is off, synchronous claims while it is already on, collision-checked first activation with a warning, preservation of a contested tool's execution, and the non-retroactive bound for rows rendered before first activation.
...
The pre-cleanup reproduction used a real isolated Pi TUI at 180 columns by 44 rows with the tracked Calm and watcher extensions, an isolated `DJ_HOME`, and a live home-owned watcher cycle.
The model called `dj_watch_arm_pi`, the real tool returned `watcher: started Pi extension arm child 1`, and a `done:` status write caused the watcher extension to inject `JARVIS WATCHER WAKE: signal: ...` followed by the stable drain instruction.
...
Adding supported empty renderer slots to a scratch copy of `dj_watch_arm_pi` likewise removed its row while the real watcher still started and the model still returned `PROBE_COMPLETE`.
...
Legacy `jarvis-synthetic-input-presentation` entries remained renderable so existing sessions preserved their stored presentation and zero-height hidden-row behavior.
...
The injected user message began with exact U+2063 plus `JARVIS_OP:` and carried the watcher status path from the durable captain screenshot followed by the blank line and stable drain instruction.
...
The adapter probes for that exact method and, per the [compatibility contract](calm.md#pi-compatibility), degrades independently with a diagnostic rather than gating on a version number.
It delegates current recognition to `bin/dj-operational-input.sh`, adds only the evidence-backed bare-U+2063 `Supervisor escalate (` presentation compatibility shape, mounts a `UserMessageComponent` subclass that preserves Pi's stock row plus leading spacer while Calm is off, and returns zero rendered lines while Calm is on.
It never intercepts the input event, rewrites the message, changes its role, filters model context, or changes session data.
Messages containing an image are left on Pi's ordinary path even when their text equals an operational envelope because Jarvis's authoritative producers are text-only.
...
The current exact marker and the narrow bare-U+2063 `Supervisor escalate (` compatibility shape hid under Calm, while quoted markers, ASCII `JARVIS_OP:` without U+2063, ordinary text before the current marker, unrelated text after U+2063, and image-bearing input remained visible.
...
`.pi/extensions/dj-calm.ts` remains the sole owner of the presentation choice and the only caller of `setWorkingVisible()`, while `.pi/extensions/lib/dj-calm-working-ship.ts` owns the sprite geometry, the bounce track, and the widget.
...
`.pi/extensions/lib/dj-calm-visibility.ts` owns only the allowlist-style transcript presentation policy.
`bin/dj-operational-input.sh` owns current cross-language operational-input construction and parsing, while the thin Pi adapter lives at `.pi/extensions/lib/dj-operational-input.ts`.
...
The established leading `[dj-from-jarvis]` plus U+2063 routing carrier remains current so running secondmate charters remain compatible.
...
## Jarvis Pi tool audit

Every tool registered or supplied by Jarvis under `.pi/extensions` has this disposition:
...
| `dj_watch_arm_pi` | Main-session custom tool in `dj-primary-pi-watch.ts` | Its complete self-rendered shell hides while Calm is active and returns unchanged when Calm is off or stock export rendering is active. |
| `dj_branch_outcomes` | Main-session custom tool in `dj-branch-supervision.ts` | Its complete self-rendered shell hides while Calm is active; the self-renderer reconstructs Pi's ordinary boxed fallback shell when visible, while stock export rendering deliberately falls through to Pi's structured fallback. |
| `dj_branch_report` | Branch-session custom tool supplied directly to `createAgentSession` | It runs only in the headless supervision session and has no main-session `ToolExecutionComponent`; successful execution writes the outcome store and merges a branch note through the separately audited delivery path, so the tool cannot emit a dump-shaped row in the captain's transcript. |
...
| `assistant-tool-call` | `ToolExecutionComponent` | Seven built-ins, `dj_watch_arm_pi`, and `dj_branch_outcomes` hidden; other arbitrary custom tools remain an unsupported boundary. |
...
For the duplicate-turn fix and the latest presentation change, the launch templates for Claude, Codex, OpenCode, Pi, and Grok and the watcher, turn-end, session-start, away-supervisor, and from-jarvis producers were re-inspected.
...
`tests/dj-calm-pi-extension.test.sh` compares wrapped and stock renderers and verifies all seven built-ins plus `dj_watch_arm_pi`; `tests/dj-pi-branch-extension.test.sh` verifies `dj_branch_outcomes` Calm toggling and export rendering.
...
`tests/dj-pi-primary-live-e2e.test.sh` also proves the working ship replaces the built-in `Working...` row while Calm is active on the credentialed provider path, and that it clears when the run settles, before continuing its ordinary watcher lifecycle.
`tests/dj-pi-primary-types.test.sh` performs strict no-emit TypeScript checking against the installed Pi declarations, currently package version 0.81.1.

The relevant commands are:

```sh
tests/dj-calm-pi-extension.test.sh
tests/dj-pi-branch-extension.test.sh
...
$ tests/dj-calm-pi-extension.test.sh
ok - Pi calm extension is presentation-only with one persisted visibility choice, no Calm status row, native working visibility, supported redraw controls, and the Jarvis watcher-tool integration
...
$ bin/dj-test-run.sh --changed --base origin/main
DJ_TEST_SUMMARY total=38 failed=0 skipped_gate=7 duration_ms=166881
DJ_TEST_SUMMARY_FAMILY family=live-harness-optin count=7 duration_ms=192 failed=0
DJ_TEST_SUMMARY_FAMILY family=pure-contract-unit count=31 duration_ms=165384 failed=0

$ tests/dj-pi-primary-live-e2e.test.sh
skip: set DJ_PI_LIVE_E2E=1 to run the isolated interactive Pi regression
```
...
dj-doc-audience-check: ok surfaces=57 local_links=160

$ bin/dj-test-run.sh --changed --base origin/main
DJ_TEST_SUMMARY total=32 failed=0 skipped_gate=7 duration_ms=196009
DJ_TEST_SUMMARY_FAMILY family=live-harness-optin count=7 duration_ms=202 failed=0
DJ_TEST_SUMMARY_FAMILY family=pure-contract-unit count=25 duration_ms=194670 failed=0
```
...
$ bin/dj-doc-audience-check.sh
dj-doc-audience-check: ok surfaces=57 local_links=163

$ bin/dj-test-run.sh --changed --base origin/main
DJ_TEST_SUMMARY total=32 failed=0 skipped_gate=7 duration_ms=386738
DJ_TEST_SUMMARY_FAMILY family=live-harness-optin count=7 duration_ms=257 failed=0
DJ_TEST_SUMMARY_FAMILY family=pure-contract-unit count=25 duration_ms=383010 failed=0
```
...
The regression is pinned by the real-terminal `/export` case in `tests/dj-calm-pi-extension.test.sh`, which now asserts the confirmation is still on screen after Calm's redraw has settled and that the redraw restored every Calm-hidden row.
...
$ bin/dj-doc-audience-check.sh
dj-doc-audience-check: ok surfaces=68 local_links=253

$ bin/dj-test-run.sh --changed --base origin/main
DJ_TEST_SUMMARY total=46 failed=0 skipped_gate=16 duration_ms=279390
DJ_TEST_SUMMARY_FAMILY family=live-harness-optin count=16 duration_ms=431 failed=0
DJ_TEST_SUMMARY_FAMILY family=pure-contract-unit count=30 duration_ms=277700 failed=0
```
