...
Talk to a voice agent that sits in front of the Jarvis. It answers questions
about what is happening from the Jarvis's own records, and when you ask for
...
------                          ------------                      ---
microphone --> dj-voice-client.py --(ssh)--> dj-voice-relay.py --> Nova Sonic 2
speaker    <-------------------------------------------------      (your region)
                                        |
                                        +--> the Jarvis's records (read)
                                        +--> dj-inbox.sh note (queue real work)
```

The two ends share one bidirectional byte stream over an SSH exec channel, so
audio and control travel together and need framing. `bin/dj_voice_frame.py` is
...
queueing half is `bin/dj-inbox.sh note`, the same surface the captain's own
...
The 0.22 seconds was the relay resolving AWS credentials again for every turn's session, which review found and fixed: `Credentials` in `bin/dj-voice-relay.py` resolves once, and every later session reuses that answer, so a reconnect costs a reconnect.
...
```
python3 -m venv ~/.dj-voice-venv
~/.dj-voice-venv/bin/pip install aws-sdk-bedrock-runtime
```
...
| --- | --- | --- |
| `config/voice-region` | `DJ_VOICE_REGION` | The Bedrock region to open the session in, required. |
| `config/voice-model` | `DJ_VOICE_MODEL` | The Nova Sonic model id, required. |
| `config/voice-profile` | `DJ_VOICE_PROFILE` | The AWS profile to export credentials from, optional: with no profile the relay uses only credentials that are already in its environment. |
| `config/voice-id` | `DJ_VOICE_ID` | The output voice, optional and `matthew` when unset. |
...
```
cd <your jarvis home>
~/.dj-voice-venv/bin/python bin/dj-voice-relay.py --self-test <clip.pcm>
```
...
```
scp <desktop>:<jarvis home>/bin/dj-voice-client.py .
scp <desktop>:<jarvis home>/bin/dj_voice_frame.py .
...
```
python3 dj-voice-client.py --host <desktop> \
  --relay <jarvis home>/bin/dj-voice-relay.py \
  --relay-python ~/.dj-voice-venv/bin/python
```

The client has no built-in idea of where the relay lives on your desktop, so `--relay` is required and `DJ_VOICE_RELAY` sets it once for a shell.
...
`tests/dj-voice-relay.test.sh` holds that boundary as an executable check, so
...
- **Doing any project work.** Real work is queued for the Jarvis and the
...
| Concern | Owner |
| --- | --- |
| Wire format between the two machines | `bin/dj_voice_frame.py` |
| The relay, the model session, the tools | `bin/dj-voice-relay.py` |
| The laptop end, capture and playback | `bin/dj-voice-client.py` |
| What may be read, and queueing real work | `bin/dj_voice_records.py` |
| The queue the handover writes to | `bin/dj-inbox.sh` |
| The boundary as an executable check | `tests/dj-voice-relay.test.sh` |
