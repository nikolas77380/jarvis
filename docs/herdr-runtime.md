...
Run each delegated Claude agent in its own Herdr tab and keep enough local metadata to inspect,
...
`.harness-state/`. A task record binds the task card, project clone, worktree, branch, Claude role
definition, unique Herdr agent name, session, workspace, tab, and pane.
...
- Spawn resolves exactly one task card, reads its `Project` and `Owner`, creates an isolated worktree
  from `projects/<project>`, creates a background Herdr tab, starts Claude with the central rules and
