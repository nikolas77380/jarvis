...
3. First cycle: arm with Grok's tracked background tool, as its own call:
   ...
   `[ -f __FM_X_MODE_ENV_SH__ ] && . __FM_X_MODE_ENV_SH__; exec bin/dj-watch-arm.sh`
...
8. Waiting is silent.
9. Never use shell `&` for jarvis supervision.
10. Never bundle the arm onto another command.
    A shell `&`, a truncating pipe, or bundling is denied automatically by the PreToolUse seatbelt (`bin/dj-arm-pretool-check.sh`) whenever this project's Grok hooks are trusted.
...
1. Run `bin/dj-wake-drain.sh` first.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/dj-watch-arm.sh` call if work remains in flight or Relay still needs polling.
...
The primary project Stop hook runs `bin/dj-turnend-guard-grok.sh` as a backstop, not the normal wake path.
...
Headless `grok -p` may wait for background process exit but does not reliably surface full auto-wake model output; do not run the primary jarvis as a one-shot headless process.
