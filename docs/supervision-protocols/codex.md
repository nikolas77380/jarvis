...
3. First cycle: run one foreground watcher checkpoint with `bin/dj-watch-checkpoint.sh --seconds "${DJ_CODEX_WATCH_CHECKPOINT:-180}"`.
...
6. Never use shell `&` or Codex background tasks for jarvis watcher supervision.
7. Do not run `bin/dj-watch-arm.sh` as Codex's normal supervision command.
   If it is ever shelled anyway, a backgrounded, piped, or bundled anti-pattern is denied automatically by the PreToolUse seatbelt (`bin/dj-arm-pretool-check.sh`) registered in `.codex/hooks.json`.
