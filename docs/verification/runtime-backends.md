...
Pi and pi-signed 0.82.0 were reverified on 2026-07-27 through real isolated `dj-spawn.sh` launches.
...
dj_backend_agent_state tmux museliv:zsh
alive
```
...
The portable regression is CI-enforced, while the real-harness drift guard is opt-in under the policy in `.agents/skills/jarvis-coding-guidelines/SKILL.md`.
...
DJ_HOME="$fixture_home" bin/dj-crew-state.sh "$task_id"
```
...
Jarvis therefore sets the exact `DJ_PI_HARNESS` selection marker on both worker launch paths, while an unmarked Pi-family process remains `pi`.
...
The current classifier matrix and its refresh guard are recorded in [Composer classification matrix](#composer-classification-matrix), with portable shape coverage in `tests/dj-composer-lib.test.sh` and `tests/dj-composer-ghost.test.sh`.
Kimi pointer delivery and OpenCode 1.18.4 busy-queue behavior remain pinned by `tests/dj-kimi-harness.test.sh`, `tests/dj-tmux-submit-busy.test.sh`, and `tests/dj-composer-lib.test.sh`.
Herdr's Claude idle-native submit confirmation is pinned by `tests/dj-backend-herdr.test.sh` and refreshed by `DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 tests/dj-herdr-submit-confirm-live-e2e.test.sh`.
...
```sh
tests/dj-teardown-endpoint-safety.test.sh
tests/dj-teardown.test.sh
tests/dj-backend-herdr.test.sh
tests/dj-backend-zellij.test.sh
tests/dj-backend-orca.test.sh
tests/dj-backend-cmux.test.sh
```

Bounded output from the incident regression:

```text
ok - dj-teardown: missing, empty, malformed, ambiguous, and task-mismatched endpoints refuse before every mutation or runtime call
...
ok - dj-teardown: dedicated-socket invalid cleanup preserves target/control and valid cleanup removes only the exact target
```
...
The shared composer classifier (`bin/dj-composer-lib.sh`, `dj_composer_classify_screen`) owns every composer shape fleet-wide; each backend contributes only a capture and a capability descriptor.
...
```sh
DJ_COMPOSER_MATRIX_LIVE=1 tests/dj-composer-matrix-live-e2e.test.sh
```
...
Kimi was not installed on the verification machine; its bordered shape is pinned by the portable byte-capture regressions in `tests/dj-composer-lib.test.sh`, which also carry the other five adapters' capability profiles for every harness under both a UTF-8 locale and `LC_ALL=C`.
...
Kimi was not installed on the verification machine; its receive path is the same one-line-plus-shell contract, and the portable ladder and enqueue regressions in `tests/dj-task-inbox.test.sh` and `tests/dj-send-inbox.test.sh` cover every harness-independent half.
...
All destructive verification used `bin/dj-herdr-lab.sh` with a non-default `dj-lab-` name and a byte-identical default-session tripwire.
No ambient `herdr server stop` command is a supported test operation.

### Submit confirmation

Measured 2026-08-19 against Herdr 0.8.0 and Claude Code 2.1.236 in an isolated `dj-lab-` session.
...
`dj_backend_herdr_send_text_submit` therefore cannot treat native idle as proof of a swallow.
The portable regressions in `tests/dj-backend-herdr.test.sh` and `tests/dj-composer-lib.test.sh` pin the verdicts: native idle plus a cleared composer is delivery, proven pending plus idle is a swallow, and proven pending plus a generating busy signal is a queued Enter.
Refresh the live Claude proof with:

```sh
DJ_HERDR_SUBMIT_CONFIRM_LIVE=1 tests/dj-herdr-submit-confirm-live-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-prune-safety-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-respawn-idem-e2e.test.sh
```
...
HERDR_ENV=1
HERDR_PANE_ID=w1:p1
HERDR_SESSION=dj-lab-dj-herdr-env-pro-65961-25535
HERDR_SOCKET_PATH=/Users/kunchen/.config/herdr/sessions/dj-lab-dj-herdr-env-pro-65961-25535/herdr.sock
HERDR_TAB_ID=w1:t1
HERDR_WORKSPACE_ID=w1
```

This complete injection shape is verified only for Herdr 0.7.5.
Jarvis requires both `HERDR_PANE_ID` and `HERDR_SOCKET_PATH` before accepting claimed launcher ancestry.
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-launcher-workspace-e2e.test.sh
```

Observed guarantees on 2026-07-30 against Herdr 0.7.5 protocol 17:

```text
ok - real herdr E2E: with one 'jarvis' workspace and no herdr parent, a crewmate still lands in this home's own workspace without stealing focus
...
ok - real herdr E2E: with two 'jarvis' workspaces, a worker spawned from inside the second one lands in that exact workspace
...
That suite's headline case runs `bin/dj-spawn.sh` inside a real Herdr pane, so the parent identity comes from Herdr's own injection rather than a composed environment.
Cross-session and contradictory bindings are covered deterministically in `tests/dj-backend-herdr.test.sh`, which can script a second server's socket without provisioning one.
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-workspace-per-home-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-presentation-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-herdr-session-cleanup-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-backend-herdr-focus-flash-e2e.test.sh
```
...
```sh
DJ_HERDR_VERSION_FLOOR_LIVE_E2E=1 tests/dj-herdr-version-floor-live-e2e.test.sh
```
...
```sh
HERDR_LAB_HELPER=bin/dj-herdr-lab.sh bin/dj-test-run.sh --lane real-herdr-gated
```
...
Direct lab probes on 2026-07-28 established the removal rules the emptying-close plan relies on, each verified with `workspace list` focus reads around one mutation in a guarded `dj-lab-` session:
...
`tests/dj-composer-ghost.test.sh`, `tests/dj-composer-lib.test.sh`, and the Herdr composer cases pin the exact captured ANSI bytes.
...
```sh
DJ_SEND_MARKER_HERDR_E2E=1 \
  tests/dj-send-secondmate-marker-herdr-e2e.test.sh
```
...
```sh
tests/dj-control-herdr-smoke.test.sh
```
...
The registry read through `herdr pane report-agent` is the same source `dj_backend_herdr_agent_state` classifies, so registering and not registering an agent on a plain shell pane exercises exactly the gate every lifecycle verb depends on, with no real agent launched.
...
```sh
DJ_AFK_PI_HERDR_E2E=1 HERDR_LAB_HELPER=bin/dj-herdr-lab.sh \
  tests/dj-afk-pi-herdr-return-e2e.test.sh
```
...
The dedicated Herdr daemon workspace topology is covered by `tests/dj-afk-launch.test.sh` and preserves the captain tab's pane count.

## Zellij

The current compatibility floor and latest verification are Zellij 0.44.0 with `jq` on macOS aarch64.
All real tests use a uniquely named session and `tests/zellij-test-safety.sh`; they never touch a session named `jarvis` or call all-session deletion.
...
```sh
tests/dj-backend-zellij.test.sh
tests/dj-backend-zellij-smoke.test.sh
```
...
Real tests use only exact `dj-test-` workspaces guarded by `tests/cmux-test-safety.sh` and never quit or relaunch the captain's app.
...
- `cmuxOnly` rejects an external Jarvis process by ancestry.
...
The app configuration writer did not retain a hand-added socket password, which is why the operator guide requires Settings and a local Jarvis password source.
...
```sh
tests/dj-backend-cmux.test.sh
tests/dj-backend-cmux-smoke.test.sh
```
...
The cmux classifier returned `empty`, and one `dj-send.sh --resolve-key <key> ALBATROSS` command - which used the typed path before ordinary task steers moved to the inbox - appended the matching `resolved` event before the worker reported completion.
The terminal capture contained exactly one submitted `❯ ALBATROSS` row.
The dated proof used this command:

```sh
DJ_CMUX_CLAUDE_COMPOSER_LIVE=1 bin/dj-test-run.sh tests/dj-cmux-claude-composer-live-e2e.test.sh
```
...
The portable classifier regression is `tests/dj-backend-cmux.test.sh`.
...
4. verify the thread appended a Jarvis status line and wrote its report;
...
Observed guarantee: a Desktop-owned thread can write Jarvis lifecycle files when the prompt provides an authorized absolute path, and create, send, read, and archive work at the Desktop host-tool layer.
The missing guarantee remains a supported shell-callable bridge that lets Jarvis perform those operations against the same visible Desktop endpoint.
...
With both markers set, `bin/dj-harness.sh` reports `cursor`; with `CLAUDECODE` alone it still reports `claude`.
...
`bin/dj-tmux-lib.sh` therefore reclassifies cursorlessly only when the pane's foreground process group is provably Cursor, so every other harness keeps the strict blank-cursor-row posture.
That supplies the genuine composer-empty proof required for away-mode escalation delivery.
A live injection through `bin/dj-supervise-daemon.sh`'s own `inject_msg` into a real Cursor pane returned 0 and the pane processed the typed `JARVIS_OP: v1 away-supervisor:` escalation.

`tests/dj-tmux-agent-liveness.test.sh` pins this with real processes and no Cursor installed: it asserts the cursor-anchored source is blind, that the composite still reads `empty` idle and `pending` with typed text, that an identical screen stays `unknown` when the pane is not Cursor, and that a stale Cursor screen over a dead shell never reads `empty`.
...
After an interrupt the aborted close was observed within seconds in some runs and not within twenty seconds in others, so `bin/dj-control-lib.sh` deliberately claims no cancellation acknowledgement for cursor.
...
| Exit | `/exit` |
| Skill invocation | `/<skill>`; cursor discovers jarvis's user-level skills, and `/no-mistakes` autocompleted with jarvis's own description and invoked the skill |
...
A throwaway scout was spawned through `bin/dj-spawn.sh --scout --backend tmux` on a real cursor worker and driven to completion:
...
4. `bin/dj-send.sh` delivered a steer through the then-current typed path and exited 0;
5. `bin/dj-control.sh <id> interrupt` cancelled a running turn;
6. `bin/dj-control.sh <id> exit` stopped the agent;
7. `bin/dj-teardown.sh` refused until the scout's report and decision gate were satisfied, then removed the session record.
...
Every step ran inside an isolated `dj-lab-` session provisioned by `bin/dj-herdr-lab.sh`, launched from a neutral parent outside any Herdr pane, with the live default session's pane count checked before, during, and after; it stayed at 7 throughout.
...
With the rendered-footer transition in place, a typed-plane `bin/dj-send.sh` invocation exited 0 and the steer executed in the pane; the same send previously exited 1 with `delivery unconfirmed; verdict=pending` on a message that had actually landed.

The rest of the lifecycle was driven end to end on that worker:

1. `bin/dj-spawn.sh --scout --backend herdr` placed the worker and it executed its brief;
...
3. `bin/dj-control.sh <id> interrupt` reported `cancel=unconfirmed` by design and the pane showed `Cancelled`, with the footer and the fold both returning to idle;
4. `bin/dj-control.sh <id> exit` stopped the agent through the slash popup and the pane returned to its shell;
5. `bin/dj-teardown.sh` refused until the scout's report and decision gate were satisfied, then removed the session record and returned the worktree.
...
Zellij, cmux, and Orca share a submit core that never consults the busy footer, so a typed-plane Cursor send there lands but `dj-send` reports delivery unconfirmed and exits non-zero; ordinary text steers ride the durable inbox and exit 0 at enqueue.
...
The portable regression is `tests/dj-cursor-harness.test.sh`, the composer captures are pinned in `tests/dj-composer-lib.test.sh`, and the Herdr submit and footer behavior is pinned in `tests/dj-backend-herdr.test.sh`.
