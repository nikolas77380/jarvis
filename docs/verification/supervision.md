...
```sh
pi -p -e .pi/extensions/dj-primary-turnend-guard.ts \
...
The first two were measured on 2026-08-05 against a throwaway Jarvis-shaped lab carrying each harness's own tracked registration with a recorder standing in for `bin/dj-sessionstart-run.sh`.
...
The interactive TUI is a known uncovered gap: Jarvis has no tracked session-open, compaction, or re-emit channel there, ships no global hook, and does not claim instruction-refresh delivery for that surface.
...
The current adapter classification and baseline mechanics are owned by [`../sessionstart-nudge.md`](../sessionstart-nudge.md#harness-transports) and the `bin/dj-session-start.sh` header.
...
It used a scratch `DJ_HOME`, a private tmux socket, and a disposable Jarvis checkout.
...
```sh
DJ_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 \
DJ_SESSIONSTART_INSTRUCTION_REFRESH_REF=origin/main \
DJ_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT=stale \
tests/dj-sessionstart-instruction-refresh-live-e2e.test.sh
...
Session start composes its digest from local reads and runs every external-network call in a worker detached by the hook (`bin/dj-startup-network.sh`), so a harness that reaped the hook's process tree would silently stop running the sweeps rather than merely delaying them.
Verified on 2026-08-06 with Claude Code 2.1.222 in a throwaway lab whose `bin/dj-bootstrap.sh` sleeps 6s before writing a marker, so the marker can exist only if the worker outlived the hook and the whole `claude -p` process.
...
The latency this buys was re-measured on 2026-08-06 against default-branch tip `8398d31`, in a throwaway home holding one remote secondmate whose host hangs 25s per SSH connection (an `DJ_SSH_BIN`-shaped stub; no real host was contacted).
Both runs used the same fixture and the same `bin/dj-session-start.sh` invocation, differing only in which checkout supplied the script:
...
Codex and Pi were not installed as run-tier labs in this measurement, so their evidence for this fact is NOT refreshed; `tests/dj-sessionstart-hook-live-e2e.test.sh` asserts it for each installed Claude, Codex exec, and Pi adapter and is the command that refreshes their record.
...
```sh
tests/dj-sessionstart-nudge.test.sh
tests/dj-session-start.test.sh
tests/dj-startup-network.test.sh
DJ_SESSIONSTART_HOOK_LIVE_E2E=1 tests/dj-sessionstart-hook-live-e2e.test.sh
DJ_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 tests/dj-sessionstart-instruction-refresh-live-e2e.test.sh
DJ_PI_LIVE_E2E=1 tests/dj-pi-primary-live-e2e.test.sh
DJ_OPENCODE_LIVE_E2E=1 tests/dj-opencode-primary-live-e2e.test.sh
```

`tests/dj-sessionstart-hook-live-e2e.test.sh` is the command that refreshes the Claude, Codex exec, and Pi table above; run it after upgrading any of those harnesses.
...
Cursor's refresh command is `DJ_CURSOR_PRIMARY_LIVE_E2E=1 tests/dj-cursor-primary-live-e2e.test.sh`, recorded under [Cursor primary park](#cursor-primary-park-2026-08-13).
...
The per-adapter semantic sources behind [`bin/dj-busy-lib.sh`](../../bin/dj-busy-lib.sh) were live-verified on 2026-07-28 against jarvis-launched workers wired exactly as `dj-spawn` writes them.
Each pass polled `state/<id>.busy-state` while a real turn ran.

| Harness | Version verified | Semantic source | Observed result |
| --- | --- | --- | --- |
| Pi | 0.82.0 | Extension `agent_start` / `agent_settled` with `ctx.isIdle()` | The spawn seed `busy source=dj-spawn`, then `busy source=pi-ext event=agent-start`, then `idle source=pi-ext event=agent-settled`; the turn-end marker was still touched. |
...
| Claude | 2.1.220 (Claude Code) | Hooks `UserPromptSubmit`, `Stop`, `StopFailure`, `SessionEnd` | `UserPromptSubmit` fired for the argv launch prompt and each steer, and `Stop` closed every completed turn. A mid-stream Escape interrupt fired no closing hook, which is why the jarvis-controlled clear exists. `StopFailure` and `SessionEnd` are wired from the four hook names present in the installed binary; only the abnormal paths they cover were not reproduced live. |
...
In this 2026-07-28 Codex 0.145.0 semantic-busy probe, Jarvis-written lifecycle project hooks under `<worktree>/.codex/hooks.json` fired for neither an interactive pane whose directory trust was granted nor `codex exec`, in both cases with `--dangerously-bypass-hook-trust`, while an untracked global probe fired in the same runs; Jarvis does not ship, install, recommend, or depend on that global path.
...
```sh
tests/dj-busy-state.test.sh
tests/dj-busy-adapter-wiring.test.sh
...
Cursor was validated as a primary on 2026-08-13 against the installed CLI on macOS 26.5.2 arm64 with tmux 3.6a, in a throwaway jarvis home on a private tmux socket, never against a live home and never with a user-scope hook.
...
```sh
DJ_CURSOR_PRIMARY_LIVE_E2E=1 tests/dj-cursor-primary-live-e2e.test.sh
```
...
The live run proved that session start acquires the fleet lock through Cursor's structural process identity in `bin/dj-cursor-lib.sh`; `tests/dj-session-lock-ancestry.test.sh` pins the same ancestry path portably.
It also proved that Cursor's `autoarm` supervision model lets the mid-turn pull guard accept a fresh beacon after the between-turn watcher closes; `tests/dj-guard-stale-banner.test.sh` pins that model-aware verdict.
...
It is therefore recorded as uncovered in the same sense as the Codex interactive TUI, and `tests/dj-cursor-primary.test.sh` asserts `preCompact` stays unregistered so it cannot return unnoticed without its own design and evidence.
...
```sh
DJ_GROK_STOP_LIVE_E2E=1 \
  DJ_GROK_NATIVE_BIN="$native_grok_0_2_112" \
  DJ_GROK_LEGACY_BIN="$official_pre_native_grok_0_2_73" \
  tests/dj-grok-stop-live-e2e.test.sh
```
...
`tests/dj-turnend-guard.test.sh` now pins every tracked `.claude/settings.json` hook entry against a real grok 1.0.0 hook environment so the inertness contract is covered deterministically rather than only by the opt-in live matrix.

The secondmate-home scope and manual-repair wake path were measured with Claude Code 2.1.207 on 2026-07-12, when a native background completion re-invoked the idle model with no human input.
The current Stop-owned main/secondmate inclusion and child-worktree exclusion are covered deterministically by `tests/dj-claude-stop-autoarm.test.sh`.
Session-lock ownership in `bin/dj-session-lock-lib.sh` is decided against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
...
`tests/dj-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
...
```sh
claude --version
DJ_CLAUDE_LIVE_E2E=1 tests/dj-claude-stop-autoarm-live-e2e.test.sh
```
...
```sh
tests/dj-turnend-guard.test.sh
tests/dj-supervision-instructions.test.sh
DJ_PI_LIVE_E2E=1 tests/dj-pi-primary-live-e2e.test.sh
DJ_GROK_STOP_LIVE_E2E=1 DJ_GROK_NATIVE_BIN="$native_grok" DJ_GROK_LEGACY_BIN="$pre_native_grok" tests/dj-grok-stop-live-e2e.test.sh
```
...
```sh
bin/dj-lint.sh
bin/dj-doc-audience-check.sh
bin/dj-test-run.sh tests/dj-claude-stop-autoarm.test.sh tests/dj-guard-stale-banner.test.sh tests/dj-turnend-guard.test.sh tests/dj-supervision-instructions.test.sh
```

Observed output:

```text
dj-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
dj-doc-audience-check: ok surfaces=61 local_links=174
DJ_TEST_SUMMARY total=4 failed=0 skipped_gate=0 duration_ms=102585
```

The model-aware pull-guard predicate correction (`bin/dj-guard.sh` no longer reports a false watcher-down mid-turn under the Claude Stop auto-arm model, where the watcher runs only between turns) was verified on 2026-08-04 with the installed ShellCheck 0.11.0 and the same isolated behavior suites.
...
```text
dj-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
dj-doc-audience-check: ok surfaces=64 local_links=188
DJ_TEST_SUMMARY total=4 failed=0 skipped_gate=0 duration_ms=80078
```

The Pi extension-model pull-guard correction (`bin/dj-guard.sh` no longer reports a false watcher-down on a Pi primary during the extension's own watcher hand-off) was verified on 2026-08-13 with the installed ShellCheck 0.11.0 and isolated behavior suites.
The guard verdict itself reads only state files and process liveness, so the portable suites are the enforcing evidence; `bin/dj-harness.sh`'s Pi marker detection, which selects the model, is exercised in the same suite through `PI_CODING_AGENT`.

```sh
bin/dj-lint.sh
bin/dj-doc-audience-check.sh
bin/dj-test-run.sh tests/dj-guard-stale-banner.test.sh tests/dj-turnend-guard.test.sh tests/dj-session-start.test.sh tests/dj-pi-watch-extension.test.sh tests/dj-watch-arm.test.sh
```
...
```text
dj-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
dj-doc-audience-check: ok surfaces=67 local_links=243
DJ_TEST_SUMMARY total=5 failed=0 skipped_gate=0 duration_ms=280160
```
...
```sh
DJ_SUPERVISION_MODEL=persistent DJ_GUARD_READ_ONLY=1 bin/dj-guard.sh
DJ_SUPERVISION_MODEL=extension DJ_GUARD_READ_ONLY=1 bin/dj-guard.sh
```
...
```sh
bin/dj-test-run.sh tests/dj-watch-triage.test.sh tests/dj-watcher-lock.test.sh tests/dj-afk-inject-e2e.test.sh tests/dj-afk-return.test.sh tests/dj-x-mode.test.sh tests/dj-backend.test.sh tests/dj-backend-tmux-smoke.test.sh tests/dj-secondmate-safety.test.sh
```

Observed output:

```text
DJ_TEST_SUMMARY total=8 failed=0 skipped_gate=0 duration_ms=617507
```

The actionable-close ordering correction was reverified on 2026-08-02 against an identity-matched live successor.

```sh
tests/dj-claude-stop-autoarm.test.sh >/dev/null && echo "dj-claude-stop-autoarm: ok"
```

Observed output:

```text
dj-claude-stop-autoarm: ok
```
...
| --- | --- | --- |
| Claude | `DJ_CLAUDE_LIVE_E2E=1 tests/dj-claude-stop-autoarm-live-e2e.test.sh` | Session start reclaimed a stale owner before two Stop-owned cycles, and a competing live owner prevented arm, rewake, epoch write, or lock replacement. |
| Codex | `DJ_CODEX_LIVE_E2E=1 tests/dj-codex-continuity-live-e2e.test.sh` | The one-second foreground checkpoint returned without switching to the arm wrapper. |
| OpenCode | `DJ_OPENCODE_LIVE_E2E=1 tests/dj-opencode-primary-live-e2e.test.sh` | A verified successor existed before prompt handling, with no model re-arm or turn-end fallback. |
| Pi | `DJ_PI_LIVE_E2E=1 tests/dj-pi-primary-live-e2e.test.sh` | One initial tool call led to extension-owned successors and clean child retirement on exit. |
| Grok | `DJ_GROK_LIVE_E2E=1 tests/dj-grok-continuity-live-e2e.test.sh` | Native task completion surfaced the actionable close and the cycle ledger recorded `reason=actionable-signal`. |
...
```sh
pi --version
tests/dj-pi-watch-extension.test.sh
...
Plain Pi and pi-signed share the same tracked `.pi/extensions/dj-primary-pi-watch.ts` path, so both inherit the generation owner; other primary harnesses are not applicable because they do not use this Pi extension lifecycle.
...
```sh
bin/dj-test-run.sh tests/dj-watch-recovery-loop.test.sh
```
...
DJ_TEST_SUMMARY total=1 failed=0 skipped_gate=0 duration_ms=59357
```
...
tests/dj-pi-watch-extension.test.sh
tests/dj-pi-primary-types.test.sh
tests/dj-watcher-lock.test.sh
tests/dj-watch-arm.test.sh
tests/dj-watch-recovery-loop.test.sh
tests/dj-wake-queue.test.sh
tests/dj-subagent-pretool-check.test.sh
tests/dj-claude-stop-autoarm.test.sh
tests/dj-turnend-guard.test.sh
```
...
/usr/bin/osascript \
  -e 'on run argv' \
  -e 'display notification (item 1 of argv) with title "JARVIS TEST - IGNORE" sound name "Basso"' \
  -e 'end run' \
  'JARVIS TEST - IGNORE (wedge-alarm channel verification)'
```
...
```sh
herdr notification show 'JARVIS TEST - IGNORE' \
  --body 'JARVIS TEST - IGNORE (wedge-alarm channel verification)' \
  --sound request
```
...
The safe command-channel contract is covered without a notification by `tests/dj-daemon.test.sh`: the summary reaches both `$1` and stdin, every channel is process-group bounded, and a failed channel falls through.
