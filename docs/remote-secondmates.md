...
Remote second mates place a whole persistent Jarvis home on another SSH-reachable host.
The primary still owns routing and supervision, while the remote home owns its own projects, backlog, and workers.
Jarvis does not support placing an individual worker remotely or failing a remote route over to a local replacement.

The remote second-mate agent itself always runs on the [Herdr backend](herdr-backend.md) in the shared `dj-remote` session, and every path that provisions or launches one refuses a host that is not ready for it.
`dj-remote` is reserved for remote fleet work and must not be used for personal work.
...
Do not enable agent forwarding for Jarvis.
`dj-on.sh` also disables agent forwarding, forwarding setup, and configured `SendEnv` patterns on every call, and arms bounded SSH dead-peer detection so a vanished host (a reboot, a dropped link) fails within a bounded window instead of hanging indefinitely; its [script header](../bin/dj-on.sh) owns the keepalive defaults and environment overrides.

Clone Jarvis on the remote host at an absolute code-root path.
Expose that clone's fixed entrypoint on the account's non-interactive SSH `PATH`, for example:

```sh
mkdir -p ~/.local/bin
ln -s /absolute/path/to/jarvis/bin/dj-remote-entrypoint.sh ~/.local/bin/dj-remote-entrypoint.sh
```

The entrypoint accepts encoded argv for genuine executable `bin/dj-*.sh` files only.
...
After setup, every other command verifies Jarvis's account-owned remote job worker, stages the encoded argv and stdin bytes, waits for its result, and relays stdout, stderr, and the exit status separately.
On macOS the worker is `dev.jarvis.remote-job`, an Aqua-scoped LaunchAgent at `~/Library/LaunchAgents/dev.jarvis.remote-job.plist` with logs under `~/Library/Logs/`.
After that bootstrap every non-doctor `dj-on.sh` target runs through that worker in the remote account's GUI session, never in the SSH process or a Herdr pane.
...
`bin/dj-remote-job-lib.sh` owns that preemption contract and distinguishes preemption from a wait window that closes with no data, so only a genuinely quiet window proves channel freshness while either outcome can re-arm without losing data.
Linux uses the same queue and worker protocol without the Aqua-session requirement.
A worker stops itself once its configured code root stops being a Jarvis checkout, so a worker started from a worktree cannot outlive that worktree, and `bin/dj-remote-job-reap-orphans.sh` clears any worker already left behind that way without ever touching one whose checkout still exists.
...
`bin/dj-remote-job-lib.sh` is the single owner of the worker `PATH` and builds it by filesystem discovery rather than by evaluating shell startup files.
...
When a required tool remains discoverable only through one of those managers, `dj-remote-doctor.sh --fix` may create a Jarvis-owned wrapper in `~/.local/bin` that executes its selected absolute target.
...
`bin/dj-remote-doctor.sh` is the single owner of what "ready for a remote second mate" means.
Check any host against it directly:

```sh
bin/dj-on.sh <secondmate-id|ssh-alias> dj-remote-doctor.sh
```
...
```sh
bin/dj-on.sh <secondmate-id|ssh-alias> dj-remote-doctor.sh --fix
```

Over the plain SSH doctor bootstrap, it writes and reloads the Jarvis-owned `dev.jarvis.remote-job` and `dev.jarvis.herdr.dj-remote` launch agents on macOS, both scoped with `LimitLoadToSessionType=Aqua` and bootstrapped in `gui/<uid>`.
It starts the same workers directly on Linux, recreates the `~/.local/bin/dj-remote-entrypoint.sh` symlink when it is absent, and creates only Jarvis-owned required-tool wrappers that it can prove resolve to a version-manager target, stopping after one harness satisfies the at-least-one requirement.
It never installs packages or overwrites a non-Jarvis file at a reserved wrapper path.
The dedicated Herdr launch agent owns only the remote-secondmate `dj-remote` server and does not inspect, rewrite, start, stop, or require the user's interactive `default` session or its `dev.jarvis.herdr` launch agent.
...
Jarvis never writes an auto-login password, never changes FileVault, and never stores an account password.
A file at `~/.local/bin/dj-remote-entrypoint.sh` that is not Jarvis's own symlink is reported for the operator to inspect and is never overwritten.
...
```sh
bin/dj-remote-home-seed.sh <id> <ssh-alias> <remote-root> <remote-home> {<project>[=<origin-url>]...|--no-projects}
```

`<remote-root>` is the remote Jarvis code clone that supplies tracked scripts.
...
[`bin/dj-project-origin-lib.sh`](../bin/dj-project-origin-lib.sh) owns which URLs are accepted; it decides on structure and safety alone, so no forge, domain, or host is privileged and a self-hosted server works exactly as a hosted one does.
...
The seed records `host:`, `root:`, and `home:` in `data/secondmates.md`, gates the host on readiness, sends a bounded manifest, and lets the remote host clone its own Jarvis home and project origins.
...
Seeding also writes a durable `.dj-secondmate-parent` record next to the home's `.dj-secondmate-home` identity marker, naming this home's route to its parent as `local` or `remote`.
The promised-public-reply subsystem is same-filesystem by construction, so a remote route can never carry a delegated public-reply promise; `bin/dj-teardown.sh`'s cleanup gate reads this record to treat a remote parent as out of scope rather than an unresolved binding.

Local secondmates keep the existing route form and need no migration.
A fleet may contain local and remote routes together.
Use `bin/dj-home-seed.sh validate` to validate either form.
...
```sh
bin/dj-spawn.sh <id> --secondmate
```

The primary resolves the verified secondmate harness and optional model and effort, runs the same readiness gate the seed runs, transfers the inherited-material allowlist, and asks the remote host to launch on Herdr in `dj-remote`.
All remote secondmates on one host share `dj-remote` and retain separate `2ndmate-<id>` workspaces inside it.
...
```sh
DJ_HOME=<primary-home> bin/dj-send.sh dj-<id> '<request>'
```

The [`dj-send.sh` header](../bin/dj-send.sh) owns the exact delivery-status contract.
...
If it remains unconfirmed, only the exact `DJ_PENDING_REPLY_EXISTING_CORR=<id>` resend command printed by `dj-send` is safe to run later because it preserves the request body and lets the remote enqueue deduplicate onto the same record; a plain rerun mints a different correlation and is not idempotent.
...
`dj-peek.sh` and `dj-crew-state.sh` route remote-secondmate reads to the endpoint's host instead of consulting local worktree or backend state.
...
The underlying `dj-on` transport never retries automatically, but `dj-send` retries its correlation-preserving steering-inbox leg exactly once.
...
```sh
bin/dj-backlog-handoff.sh <id> <item-key>...
```
...
The outbox is then copied to the remote handoff scratch directory and `dj-backlog-receive.sh` atomically ingests every destination-absent key under the remote backlog's own lock.
...
Locked startup convergence and `bin/dj-config-push.sh` transfer only the declared inherited-material allowlist.
...
`/updatejarvis` updates each remote code root from its own origin, then guardedly fast-forwards the persistent remote home to that code-root commit.
...
It closes only the retiring secondmate's panes or `2ndmate-<id>` workspace in `dj-remote`; it never stops the shared session or removes a sibling secondmate's workspace or panes.
...
```sh
bin/dj-test-run.sh tests/dj-on.test.sh
bin/dj-test-run.sh tests/dj-send-remote-delivery.test.sh
bin/dj-test-run.sh tests/dj-peek-remote.test.sh
bin/dj-test-run.sh tests/dj-crew-state.test.sh
bin/dj-test-run.sh tests/dj-remote-job.test.sh
bin/dj-test-run.sh tests/dj-remote-doctor.test.sh
bin/dj-test-run.sh tests/dj-project-origin.test.sh
bin/dj-test-run.sh tests/dj-remote-reply.test.sh
bin/dj-test-run.sh tests/dj-remote-backlog-handoff.test.sh
bin/dj-test-run.sh tests/dj-remote-secondmate-lifecycle-e2e.test.sh
bin/dj-test-run.sh tests/dj-remote-secondmate-trace-context.test.sh
```
