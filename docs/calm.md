...
It is off by default, and the last `/calm` choice persists for the effective Jarvis home across Pi session starts and resumes.
...
Calm hides collapsed thinking labels, mid-turn assistant working notes, the shells for the Pi built-in tool names Calm owns, the `dj_watch_arm_pi` and `dj_branch_outcomes` tool shells, and canonically classified Jarvis operational user rows.
...
Every hidden Jarvis input remains available to the model and in serialized session data and exported artifacts.
...
`.pi/extensions/lib/dj-calm-visibility.ts` owns the visibility policy, `.pi/extensions/lib/dj-calm-operational-user-layout.ts` owns the zero-height operational-user row adapter, and `.pi/extensions/lib/dj-calm-working-ship.ts` owns the animated working presentation.
