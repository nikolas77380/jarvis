...
The away-mode sub-supervisor (`bin/dj-supervise-daemon.sh`) buffers escalations and injects them into Jarvis's own pane.
When injection cannot confirm a submit past `DJ_MAX_DEFER_SECS`, `inject_wedge_alarm` raises a loud, rate-limited alarm so the stall never stays invisible.
...
`DJ_WEDGE_ALARM_CHANNEL` overrides the file with one directive for focused testing.
...
Every invocation is process-group bounded by `DJ_WEDGE_ALARM_TIMEOUT_SECS`, which defaults to 10 seconds, including `command:`, `osascript`, `herdr`, and the test seam.
...
Every notifier routes through `DJ_WEDGE_ALARM_EXEC` in `wedge_alarm_emit`.
...
`tests/dj-daemon.test.sh` covers directive parsing, rate limiting, timeout and process-group cleanup, argv-safe dispatch, channel fallback, and safe `command:` summary delivery.
