...
Jarvis talks to a running agent two ways, and they are not the same channel.

The **data plane** is [`bin/dj-send.sh`](../bin/dj-send.sh): conversational text for the agent to read.
For a `kind=secondmate` target it always prepends the from-jarvis routing marker, because a secondmate is itself a jarvis and its reply must come back through the status path rather than a chat nobody reads.

The **control plane** is [`bin/dj-control.sh`](../bin/dj-control.sh): allowlisted lifecycle verbs addressed to an exact task id.
...
A routing-marked `/quit` arrives as ordinary chat - `[dj-from-jarvis] /quit` - which the agent reasons about instead of executing.
...
`bin/dj-control-lib.sh` is the single executable owner of three capability tables, with no side effects, so it can be read as a contract:
...
- **Per-harness mechanics**: the key that cancels a running turn, how many times it must be delivered, whether the composer needs clearing afterwards, the command that exits the agent, and which task kinds the adapter is verified to run.
  ...
  `bin/dj-send.sh`'s `--key` path reads the composer-clear table from this owner too, rather than keeping a second copy of it.
...
`dj_control_harness_family` is the one place that prefix rule is stated, and an unrecognized value resolves to no adapter rather than being guessed into one.
...
Removing a worktree, closing an endpoint, or discarding work stays with [`bin/dj-teardown.sh`](../bin/dj-teardown.sh), which owns the landed-work test.
...
1. **Resolve the profile.**
   ...
   A ship or scout keeps the harness already recorded for it, because that harness comes from jarvis's dispatch-profile judgment at intake and must not be silently re-read from configuration.
...
5. **Launch the replacement** through its single owner, `bin/dj-spawn.sh --relaunch`, which adopts the recorded endpoint and worktree instead of creating either, clears the previous harness's per-task wiring, and arms a fresh busy generation.
...
- Targeting is exact.
  ...
  A legacy `dj-<id>` window label, an explicit `session:window` endpoint, and a record whose `endpoint_task_id` names another task are all refused.
...
- `dj-spawn --relaunch` independently refuses unless the recorded endpoint is positively agent-free and its shell is sitting in the recorded worktree, so a replacement can never join a live agent or start outside the copy holding the work.
...
Per-harness interrupt keys, repeat counts, composer clears, exit commands, and supported task kinds live in `bin/dj-control-lib.sh` and are exercised for every verified harness by `tests/dj-control.test.sh`.
The empirical basis for each adapter's value is the `harness-adapters` skill's verification record for that adapter.

## Verification

- `tests/dj-control.test.sh` - the adapter contract for every verified harness, the backend capability matrix, exact-id scoping, the closed verb list, the busy, idle, dead, and idempotent lifecycle cases, and marker non-regression, all against a stubbed session provider.
- `tests/dj-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, checkpoint refusals, and rollback after a failed launch.
- `tests/dj-control-herdr-smoke.test.sh` - the second state-verified backend against the real herdr binary, on an isolated throwaway lab session.
