...
// dj-primary-watch-arm.js (a plugin-owned child process, never a model tool
// call), so the residual risk here is the AGENT shelling `bin/dj-watch-arm.sh`
// wrong through its own bash tool - the anti-pattern bin/dj-arm-pretool-check.sh
...
export const FmPrimaryPretoolCheck = async ({ directory, worktree }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const result = await runProcess(`${root}/bin/dj-arm-pretool-check.sh`, ["--command", command]);
