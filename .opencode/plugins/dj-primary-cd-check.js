...
// the primary jarvis checkout before the agent's bash tool relocates the
// shell out of the home (see bin/dj-cd-pretool-check.sh and docs/cd-guard.md).
// This mirrors dj-primary-pretool-check.js, calling the cd-guard owner instead
...
export const FmPrimaryCdCheck = async ({ directory, worktree }) => {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const result = await runProcess(`${root}/bin/dj-cd-pretool-check.sh`, ["--command", command]);
