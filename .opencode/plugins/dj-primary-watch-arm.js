...
import { encodeJarvisOperationalInput } from "./lib/dj-operational-input.js";

const COORDINATOR_KEY = "__jarvisOpenCodeWatchArm";
...
const ARM_READY_TIMEOUT_MS = positiveInteger("DJ_OPENCODE_ARM_READY_TIMEOUT_MS", ARM_READY_TIMEOUT_DEFAULT_MS);
const ARM_RETIRE_TIMEOUT_MS = positiveInteger("DJ_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const REARM_RETRY_BASE_MS = positiveInteger("DJ_WATCH_REARM_RETRY_BASE_MS", 250);
const REARM_RETRY_MAX_MS = positiveInteger("DJ_WATCH_REARM_RETRY_MAX_MS", 4000);
const REARM_RETRY_LIMIT = positiveInteger("DJ_WATCH_REARM_RETRY_LIMIT", 5);
...
function effectivePaths(root) {
  ...
  const fmHome = process.env.DJ_HOME || process.env.DJ_ROOT_OVERRIDE || fmRoot;
...
async function isPrimaryRoot(root, home) {
  ...
  if (existsSync(`${root}/.dj-secondmate-home`)) return false;
  if (home && home !== root && existsSync(`${home}/.dj-secondmate-home`)) return false;
...
function classifyArmClose(stdout, stderr, code, signal) {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - dj-watch-arm.sh exited ${code}${combined.trim() ? `\n${combined.trim()}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
async function sendPrompt(paths, client, sessionID, text) {
  const encoded = await encodeJarvisOperationalInput(paths.root, "watcher", text);
...
function confirmHandlingDelivery(paths, recovery) {
  try {
    const result = spawnSync(
      "bash",
      [`${paths.root}/bin/dj-watch-arm.sh`, "--handling-delivered", recovery.generation, "--watcher-pid", recovery.watcherPid],
      {
        cwd: paths.root,
        encoding: "utf8",
        env: { ...process.env, DJ_HOME: paths.home, DJ_STATE_OVERRIDE: paths.state, DJ_ROOT_OVERRIDE: paths.root },
      },
    );
...
function wakePrompt(reason) {
  return `WATCHER FIRED - drain queued wakes with bin/dj-wake-drain.sh and handle the reported wake. Watcher continuity is plugin-owned.\n\n${reason}`;
}
...
function spawnArm(paths, sessionID, client, predecessorArmPid = "") {
  ...
  const env = {
    ...process.env,
    DJ_HOME: paths.home,
    DJ_ROOT_OVERRIDE: paths.root,
    DJ_CONFIG_OVERRIDE: paths.config,
    DJ_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
  };
  const armChild = spawn("bash", ["-lc", 'config_dir="${DJ_CONFIG_OVERRIDE:-$DJ_HOME/config}"; [ -f "$config_dir/x-mode.env" ] && . "$config_dir/x-mode.env"; exec "$DJ_ROOT_OVERRIDE/bin/dj-watch-arm.sh" --restart'], {
    cwd: paths.root,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
