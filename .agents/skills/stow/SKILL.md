...
<!-- maintainers: this is the jarvis-internal skill. The public, installer-facing counterpart lives at skills/stow/SKILL.md - deliberately a separate file with no shared code or environment branching. Keep them independent. -->
...
This skill writes only through the existing Jarvis ownership and write boundaries.
...
1. Run `bin/dj-startup-memory-budget.sh report` before considering a write.
...
8. Run `bin/dj-startup-memory-budget.sh report` again after the complete pass.
...
**Hard rule: the stow process never creates or writes a jarvis-repo-tracked skill.**
**Every skill stow's offload produces for a Jarvis home is user-owned and local, excluded through that active home's repository-local exclude file resolved with `git -C "$home_root" rev-parse --git-path info/exclude`; contributing a lesson to the shared tracked template is a separate deliberate captain action, never automatic.**
...
- A user-owned local skill: a directory under `.agents/skills/<freeform-name>/` whose path is appended to the active home clone's repository-local exclude file, never to a `.gitignore`.
  Resolve `home_root` to `$DJ_HOME` when it is set and otherwise to the Jarvis code root, and anchor every destination index check, exclude-path lookup, and ignore verification to that root with `git -C "$home_root"`.
  ...
  The name is freeform with no user-vs-jarvis naming convention, the skill stays per-home and untracked, and the harness still lists and JIT-loads it because skill discovery scans the filesystem and ignores git status (verified in `docs/verification/stow-memory.md`).
...
- A project's existing committed `AGENTS.md`, for project-intrinsic knowledge useful to nearly every session of that project, through a normal crewmate ship task using `bin/dj-ensure-agents-md.sh` and the project's registered delivery mode.
...
Forbidden destinations: any jarvis-repo-tracked skill per the hard rule; jarvis's own `AGENTS.md`, which is always-loaded for every fleet session; `docs/` alone, which is never agent-loaded on demand, though a skill body may point into docs for depth; and any committed surface for private content.
...
3. Migrate an approved pinned candidate outside this pass.
   Resolve `home_root` to `$DJ_HOME` when it is set and otherwise to the Jarvis code root, then re-validate the approved local-skill destination under that root for both index absence with `git -C "$home_root"` and filesystem collision absence.
...
3. **Write within the existing boundaries.**
   ...
   - In a primary home, curate shared captain preferences only under the existing primary-authoritative shared-preference contract.
     In a secondmate home, route a newly discovered shared preference to the main jarvis through marked status or a document pointer instead of editing the inherited file.
   - Project-intrinsic knowledge never goes directly into a project's `AGENTS.md`.
     Route it through a normal ship task so a crewmate records it with `bin/dj-ensure-agents-md.sh` and the project's delivery path.
   - Knowledge general to every Jarvis user belongs in this repo's shared tracked material through the normal branch, no-mistakes, PR, and captain-merge path.
...
Run `bin/dj-stow-cascade.sh` once the primary's own pass is done.
...
- `agent` - send the marked request with `bin/dj-send.sh dj-<id> "<request>"` so the live secondmate performs its own `/stow`, including the uncaptured knowledge that exists only in its session.
...
Every Jarvis-home skill that migration produces is user-owned and local under the destinations hard rule, while an approved project-level destination is produced and shipped through that project's registered delivery path, never by stow.
Changing jarvis's tracked `.agents/skills/` or public `skills/` remains a deliberately scoped Jarvis repository task through its pipeline, never a stow product.
