...
description: >-
  ...
  Use before arming a long-polling source jarvis owns, before registering a
...
The runner exists so a blocking external process never holds jarvis's conversational turn.
Jarvis registers a source, keeps working, and is woken when that process completes.
...
```sh
bin/dj-captain-hold.sh bind <source-id>
```
...
A configured remote secondmate reply source is armed and handled through `bin/dj-procevent-remote-reply.sh`.
...
```sh
bin/dj-procevent-when.sh arm <name> --condition <argv>... --action <argv>...
```
...
Eligibility is a jarvis judgment made BEFORE arming, because the scripts cannot classify an argv: the action must be safe, reversible, and exact (for example `no-mistakes update --beta`, whose own guard refuses while a validation run is active).
...
: **When the adapter owns applying the result, run the adapter, not the generic acknowledgement below.** The `<adapter>` field of the wake decides this, and `remote-reply` is such an adapter: a captured delta is applied only by
  ```sh
  bin/dj-procevent-remote-reply.sh handle <secondmate-id> <sequence> <result-file>
  ```
...
The currently published `edith-axi poll` destructively clears feedback before returning it.
A result lost after that clearing and before the runner reads the process output is unrecoverable, and no jarvis wrapper can close that source-side window.
