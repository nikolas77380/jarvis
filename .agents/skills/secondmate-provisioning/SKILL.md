...
Keep the always-inline routing rules in `AGENTS.md` authoritative: route by natural-language `scope:`, local-only projects stay with the main jarvis, and secondmates are idle by default.
...
For a remote route, `host:` is an OpenSSH config alias and `root:` is that host's separate tracked Jarvis code root.
A remote second-mate agent always runs on the Herdr backend and every seed, launch, and liveness relaunch first gates its host on `bin/dj-remote-doctor.sh` readiness, so an unready host refuses with that doctor's own gap text rather than half-creating a route; the workers that second mate supervises keep the home's ordinary backend selection.
...
```sh
bin/dj-brief.sh <id> --secondmate {<project>...|--no-projects}
```

The scaffold writes a charter brief instead of a task brief.
Set `DJ_SECONDMATE_CHARTER='<charter>'` to fill the charter text and `DJ_SECONDMATE_SCOPE='<scope>'` when the routing scope differs.
If you scaffold without `DJ_SECONDMATE_CHARTER`, replace the `{TASK}` placeholder before seeding.
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the jarvis repo itself, whose home is a jarvis worktree and whose crews take pooled worktrees of the same repo.
...
```sh
bin/dj-home-seed.sh <id> <home|-> {<project>...|--no-projects}
```
...
[`docs/remote-secondmates.md`](../../../docs/remote-secondmates.md#provision-a-route) owns the rest of the operator contract, and [`bin/dj-project-origin-lib.sh`](../../../bin/dj-project-origin-lib.sh) owns the accepted origin forms.
...
`-` durably leases a fresh jarvis worktree via `treehouse get --lease` under the secondmate id.
...
`bin/dj-home-seed.sh` copies the charter into the secondmate home as `data/charter.md`.
It also writes the gitignored `.dj-secondmate-parent` durable binding before the required `.dj-secondmate-home` identity marker; the parser header in [`bin/dj-secondmate-parent-lib.sh`](../../../bin/dj-secondmate-parent-lib.sh) owns the record contract, and both files must remain in place.
`bin/dj-spawn.sh --secondmate` launches it through the secondmate harness path, resolving `config/secondmate-harness` -> `config/crew-harness` -> the primary's own harness unless an explicit per-spawn harness override is passed.
...
`bin/dj-harness.sh secondmate-model` and `bin/dj-harness.sh secondmate-effort` print the optional 2nd/3rd tokens (empty when absent, or when the file is absent/`default`/harness-only); they read only `config/secondmate-harness`, never `config/crew-harness`, which stays a bare adapter name.
For a `--secondmate` spawn, `bin/dj-spawn.sh` populates `MODEL`/`EFFORT` from those tokens only when the harness itself came from the secondmate config path for that spawn.
...
Because this resolves from the file on every spawn, the pin is durable across every respawn (recovery, `/updatejarvis`, restart) exactly like the harness axis itself - e.g. `config/secondmate-harness` containing `claude opus` keeps a secondmate pinned to Opus even if the primary's own default model later changes.
...
Before a local launch, `dj-spawn.sh --secondmate` locally fast-forwards the home to the primary jarvis checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
...
`/updatejarvis` first updates the remote code root from its own origin, then runs that guarded home sync.
...
Explicit per-spawn `--backend` and `DJ_BACKEND` remain stronger than every home's local `config/backend`, including an inherited default.
...
Its primary file header must state that the file is main-authoritative, read-only in secondmate homes, must not be edited there, and that new captain-preference discoveries are routed to the main jarvis through marked status or a document pointer.
...
Keep every `data/learnings.md` fully local by captain decision; route fleet-general machinery facts into tracked documentation through the normal jarvis repo path rather than inventing shared learnings propagation.
...
A remote home receives the same allowlisted bytes through `dj-remote-inherit.sh` and gets one marked re-read instruction after a changed transfer.
...
These config values remain defaults and rules only; they must not harden `dj-spawn` to reject a deliberate runtime choice that differs from the configured defaults.
For already-live secondmates, use `bin/dj-config-push.sh` to push a mid-session inherited local-material change without running the tracked-file fast-forward.
...
`bin/dj-home-seed.sh` refuses to copy a missing or placeholder charter.

Direct seed without a preexisting brief requires `DJ_SECONDMATE_CHARTER`.
Run `bin/dj-home-seed.sh validate` when checking registry integrity; its header owns the complete validation and refusal mechanics.
...
`local-only` projects stay with the main jarvis.
...
Scope-matching is jarvis's judgment against the secondmate's natural-language scope, not a keyword rule.
Read `data/backlog.md`, pick queued items that fit the new scope, and move them with:

```sh
bin/dj-backlog-handoff.sh <secondmate-id> <item-key>...
```
...
This delegated route remains required when `config/backlog-backend=manual`, which controls only routine jarvis backlog edits.
...
After a successful move it warns for any moved key that still owes a public relay reply bound to `main/<key>`, because that binding no longer names the home owning the work; rebind the commitment to `secondmate:<id>` through the `djx-respond` promised-final procedure, which owns those commands.
...
It refuses any destination that is not a genuine seeded jarvis home with safe operational directories and a matching `.dj-secondmate-home` marker, so a move can never land in a project.
...
If the secondmate is already running and only inherited local material changed, prefer `bin/dj-config-push.sh` over respawning.
To move a live LOCAL secondmate onto a newly pinned harness, model, or effort without a full recovery, set `config/secondmate-harness` and then relaunch it with `bin/dj-control.sh <id> relaunch`, which re-resolves that pin, stops the agent, and launches the replacement in the same home ([`docs/agent-control.md`](../../../docs/agent-control.md)).
...
The main jarvis reconciles only direct reports.
Each secondmate is a jarvis in its own home, so it runs recovery on startup and reconciles its own crewmates.
...
Run `bin/dj-teardown.sh <id>` for `kind=secondmate` only when the captain or main jarvis explicitly decides to retire that persistent second mate.
