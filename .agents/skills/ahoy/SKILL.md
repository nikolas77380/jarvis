...
0. Before anything else, check whether this session has already taken the helm: a `SESSION START` digest for this home must be visible in the session history.
   If it is not, run `bin/dj-session-start.sh` once and read its digest before producing any recap.
...
1. Inspect only conversation or session history already visible to the current Jarvis.
2. Find the most recent real captain-authored message before the current `/ahoy` invocation.
   ...
   Exclude messages that begin with the current U+2063 `JARVIS_OP:` injection prefix.
   Exclude legacy bare-marker away-mode injections only when U+2063 is immediately followed by `Supervisor escalate (`.
   Exclude the exact legacy unmarked session-start payload ``Run `bin/dj-session-start.sh` now, exactly once, before executing any other instructions.``
   Custom-role messages such as Pi's `jarvis-sessionstart-nudge` are not captain messages.
   ...
   Do not exclude an ordinary captain message merely because it begins with U+2063 followed by other text, contains ASCII `JARVIS_OP:` without a leading U+2063, quotes or embeds a current operational message after ordinary captain text, quotes or mentions the legacy session-start payload, or adds any text to that payload.
   Apply the current exclusion only when U+2063 `JARVIS_OP:` begins at the first character of the whole message: `Captain quote: ` followed by that current prefix is a captain boundary.
   Apply the legacy startup exclusion as a literal whole-message match: ``Captain quote: Run `bin/dj-session-start.sh` now, exactly once, before executing any other instructions.`` is a captain boundary.
...
5. Additionally inspect the entire session history visible to the current Jarvis before the current invocation for every explicit captain decision that remains unanswered, including decisions raised before the ordinary recap boundary.
...
8. After the normal recap, when the existing visibly open decision inventory contains decisions, begin a guided decision-clearing flow by presenting only the single open decision judged most impactful by the Jarvis.
   Make clear that impact ordering is the Jarvis's judgment rather than a mechanical score.
