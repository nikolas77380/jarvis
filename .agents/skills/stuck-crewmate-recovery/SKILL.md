...
description: >-
  Agent-only playbook for stuck or missing ordinary Jarvis direct reports.
...
Interrupt, stop, and relaunch a worker through `bin/dj-control.sh <task-id> interrupt|exit|relaunch`, which resolves the recorded runtime itself, verifies each action, and never tears down or discards anything ([`docs/agent-control.md`](../../../docs/agent-control.md)).
...
For a REMOTE secondmate, `dj-crew-state` and `dj-peek` read the actual remote endpoint over `dj-on.sh`, and `dj-send` reports a delivered-with-pending-confirmation steer as delivered (their headers own the contracts); an `unknown-remote` read or unreachable-host failure means the remote state could not be read, never that the mate is dead or the send failed.
Recover a genuinely stuck remote mate only through `bin/dj-spawn.sh <id> --secondmate`, never raw herdr pane close/kill surgery, which strands the endpoint binding.

Treat the digest's endpoint result as a presence signal, not proof that the task's work or validation run is gone.
Read the targeted current state with `bin/dj-crew-state.sh <id>` before deciding to relaunch.
...
1. Peek the pane, and check the task's steering inbox (`state/<id>.inbox/`) for unhandled `*.msg` records - a stale wake naming an unread jarvis instruction means the worker never acknowledged a durable steer, and the record itself shows exactly what was intended.
2. If the crewmate is waiting on a question its brief already answers, answer in one line via `DJ_HOME=<this-jarvis-home> bin/dj-send.sh` from an active jarvis session unless `DJ_HOME` is already set to the active jarvis home.
3. If the crewmate is confused or looping, interrupt with `DJ_HOME=<this-jarvis-home> bin/dj-control.sh <task-id> interrupt`, then redirect with one corrective line through `dj-send`.
4. If the crewmate is genuinely wedged after redirection, relaunch it with `DJ_HOME=<this-jarvis-home> bin/dj-control.sh <task-id> relaunch --note '<progress so far>'`, which stops the agent, carries the brief plus that note into a replacement in the same local copy, and restores the prior record if the replacement cannot start.
