...
name: afk
description: >-
  Enter away-mode supervision when the captain invokes /afk, says they are going afk, `state/.afk` exists, an incoming message starts with `DJ_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
  It sets a durable away-mode flag so the sub-supervisor daemon can self-handle routine wakes and escalate captain-relevant events plus bounded declared-external-wait rechecks as batched digests during walk-away stretches, then exits automatically when any real unmarked message returns jarvis to full per-wake responsiveness.
user-invocable: true
metadata:
  internal: true
---
...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     `bin/dj-afk-launch.sh start-native`, then run
     `DJ_AFK_STATE_PREPARED=1 bin/dj-afk-start.sh` through that native tool.
     ...
     If the native launch fails, run `bin/dj-afk-launch.sh stop` to roll back the prepared lifecycle.
     Do not wrap it in `nohup ... &` (Codex/herdr can reap fire-and-forget shell children after a tool call returns).
   - **Harness WITHOUT one** (e.g. pi): run `bin/dj-afk-launch.sh start`. It is
     ...
     in as `DJ_SUPERVISOR_TARGET` so the daemon injects into the captain, not its
     ...
     support").
   Both paths share `bin/dj-afk-start.sh` as the daemon entry.
   ...
   It exits immediately if the identity-backed daemon lock already names a live process, otherwise it execs `bin/dj-supervise-daemon.sh` in the foreground.
   The daemon is **presence-gated**: it injects escalations only while
   `state/.afk` exists, and stays quiet otherwise.

3. **Do not separately arm `dj-watch.sh`.** The daemon manages the watcher as
...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the captain is back.
  Run `bin/dj-afk-return.sh` before acting on the message that brought the captain back.
  ...
  If it reports a jarvis-actionable `blocked:` event, remediate it immediately through the normal lifecycle, or explicitly reclassify it with a durable reason and close its decision key with `resolved [key=...]`, then run `bin/dj-afk-return.sh check`.
...
- A message **with** the current operational prefix (`DJ_OPERATIONAL_PREFIX`, U+2063 INVISIBLE SEPARATOR followed by `JARVIS_OP: `), or a legacy bare `DJ_INJECT_MARK` daemon escalation -> stay afk and process it.
...
afk changes how aggressively jarvis surfaces things, **not who approves what**.
...
The daemon constructs every current injection as the `away-supervisor` kind owned by `bin/dj-operational-input.sh`, beginning with `DJ_OPERATIONAL_PREFIX`: `DJ_INJECT_MARK` (U+2063 INVISIBLE SEPARATOR) followed by the stable `JARVIS_OP: ` label.
The bare `DJ_INJECT_MARK` form remains accepted for legacy daemon escalations during rollout.
U+2063 has no normal keyboard keystroke and survives terminal transport as UTF-8 text.
This is how jarvis tells a daemon escalation apart from a real message in the same pane.
...
injection, dispatched through `bin/dj-backend.sh` for the supervisor's own
...
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`pending-unproven`/`unknown` verdict from `dj_backend_composer_state` and injects only when it is affirmatively `empty`.
  ...
  Each adapter contributes only capture and capability facts to the fleet-wide screen classifier in `bin/dj-composer-lib.sh`, which owns every shape and verdict.
...
If anything stays buffered past `DJ_MAX_DEFER_SECS` (default 300), the daemon
...
`dj-send.sh` uses the same primitive only on its typed plane and exits non-zero when that plane's Enter is positively swallowed; ordinary local text steers use the durable inbox and do not treat doorbell submission as delivery proof.

**Busy-queued Enter exception (opencode 1.18.4).** OpenCode keeps queued text visible while it is mid-turn, so tmux and herdr delegate the final delivery decision to `dj_composer_queued_enter_verdict` in `bin/dj-composer-lib.sh` rather than treating visible text alone as a swallowed Enter.
...
The daemon wraps `dj-watch.sh`, runs the watcher as a child, presents every durable wake after each actionable watcher close, classifies each presented record in bash, and acknowledges the presented generation only after routing completes.
It self-handles the routine majority without consuming a jarvis turn.
Captain-relevant events, plus a bounded recheck of a declared wait that remains idle, escalate to jarvis's context as one pre-read, single-line, batched digest.
The classification predicates (the captain-relevant verb set, declared-wait vocabulary, signal/stale tests, and fleet-scan) live in the shared `bin/dj-classify-lib.sh`, the same library the always-on watcher uses for its own triage when afk is off, so the two modes apply one identical policy.
...
- `signal` or `stale` for a declared wait, either a `paused:` external wait or a verified `captain-held` transfer -> self-handle and track the pause rather than a wedge.
  If it remains declared and idle past `DJ_PAUSE_RESURFACE_SECS` (default 3600s), housekeeping sends one recheck and resets the pause window.
...
- `check` -> always escalate. Check scripts print only when jarvis should wake.
- `stale` with a terminal status or bare legacy captain-relevant line -> escalate.
  Nonterminal progress remains transient even when its prose contains a legacy free-text token or its seen-status marker already matches, so record a marker and self-handle.
  If the pane is still idle past `DJ_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge.
  This bounds wedge-detection latency to the threshold plus a tick: a delay, never a loss.
  Healthy crewmates are autonomous and do not wait on jarvis mid-task.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash fleet scan
  every `DJ_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for a
  captain-relevant status line the per-wake classifier might miss.
- Unknown reason, or any uncertainty -> escalate fail-safe.

Escalations are buffered up to `DJ_ESCALATE_BATCH_SECS` (default 90s; 0 =
...
the operational prefix lets jarvis distinguish it from a real captain message.
...
- **Single-line digest** - embedded newlines are collapsed to a literal
  ...
  harness.
- **Busy and composer guards on the supervisor pane** - before injecting, the daemon runs the detected-primary-harness rendered busy guard and reads `dj_backend_composer_state` directly.
...
- The active backend passes its capture plus declarative styled, cursor, identity, and row capabilities to the shared screen classifier; all structural recognition and verdict logic remains in `bin/dj-composer-lib.sh`.
  ...
  `DJ_COMPOSER_IDLE_RE` overrides the shared idle-placeholder regex, but a match alone never bypasses the classifier's shape-specific position and ANSI de-emphasis safety gates.
  `DJ_BUSY_REGEX` overrides the rendered delivery guards plus Grok's isolated task-state fallback.
...
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  buffered past `DJ_MAX_DEFER_SECS` (default 300s), the daemon attempts one
...
- **Marker strip** - `strip_injection_marker` removes the current operational
  ...
  text jarvis sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`dj-wake-lib.sh`) instead of `flock`, which is absent on macOS.
...
Always enter through `bin/dj-afk-launch.sh`, which clears prior-session artifacts only for a fresh entry and preserves the current session's buffer on refresh.
Always exit through `bin/dj-afk-launch.sh stop`, which keeps `state/.afk` present through the daemon's shutdown flush and clears it last.
...
`DJ_INJECT_SKIP` (default `heartbeat`) force-self-handles matching kinds,
