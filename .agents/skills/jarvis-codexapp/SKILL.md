---
name: jarvis-codexapp
description: >-
  Agent-only playbook for coordinating visible Codex Desktop threads alongside Jarvis without pretending they are a selectable shell backend.
  Use before creating, reading, steering, archiving, debugging, or reviewing a Codex App visible thread for Jarvis work, and before responding to requests to make Codex App native to Jarvis.
...
Use this playbook when Jarvis work needs a visible Codex Desktop thread.
The current supported shape is Desktop host-tool choreography plus an explicit status-file return-channel check, not a `codex-app` value in `DJ_BACKEND`.

## Boundary

Codex Desktop visible threads are companion host-tool workflows, not a selectable Jarvis backend.
...
If local helper scripts exist for Codex App work, use only helpers explicitly provided by the operator or maintained by Jarvis.
...
3. Do not create projectless threads for repo work.
   If the project is absent, stop and ask for the project to be added or use a normal Jarvis backend instead.
4. Decide whether this is a real Jarvis-managed task or a visible companion thread.
...
A Desktop-owned Codex thread can append to Jarvis status files only when the prompt gives an absolute path and the Desktop permission context can write that checkout.
That makes status writes a verified return-channel requirement, not a fact to assume.

For a Jarvis-managed task, include an explicit status instruction:

```text
Append supervisor-visible status lines to <absolute-jarvis-home>/state/<task-id>.status.
Use only these prefixes for status changes: working:, needs-decision:, blocked:, paused:, done:, failed:.
Use paused: only for a deliberate known external wait that should be rechecked later, never for a blocker that needs jarvis to act.
...
Do not claim it is a complete Jarvis backend.
...
For Jarvis reconciliation, prefer concrete evidence:
...
Avoid repeating long transcripts into Jarvis docs or PR bodies.
...
If there is a real Jarvis task record, leave teardown decisions to the normal Jarvis task flow instead of this skill.
