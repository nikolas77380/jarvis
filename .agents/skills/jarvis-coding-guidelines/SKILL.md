---
name: jarvis-coding-guidelines
description: >-
  Agent-only reference for changing jarvis's shared, tracked material per AGENTS.md section 1.
  Use before editing any of that material, whether working as jarvis directly or as a crewmate briefed on a jarvis-repo task.
...
Load this before changing jarvis's shared, tracked material, as defined by `AGENTS.md` section 1.
...
1. Does the jarvis AGENT need this on every session or every turn to operate?
...
Briefs for tasks that touch jarvis's own tracked material should tell the crewmate to load this skill.
`bin/dj-brief.sh`'s `REPO` argument is a caller-supplied string with no reliable signal that it names jarvis's own repo, unlike a project registered in `data/projects.md`, so there is no clean point inside the scaffold to detect this case automatically.
Jarvis adds this skill's load instruction to jarvis-repo briefs by hand instead.
...
- A live guard in the `live-harness-optin` family (`bin/dj-test-run.sh`), env-gated and self-skipping, that exercises every INSTALLED harness for real and fails naming the harness and version.
...
Run `bin/dj-doc-audience-check.sh`; it enforces classification, README setup routing, local link targets, and owner pointers without keyword-linting legitimate evidence prose.
...
- Run `bin/dj-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, pinned shellcheck version, and pinned actionlint workflow lint) that CI and the no-mistakes pre-push gate both invoke, and it refuses to run under any other version of either linter.
