...
description: >-
  ...
  This skill is the single owner of finding-decision policy: jarvis always applies judgment, decides findings that are unambiguous toward accepted intent, and escalates only genuinely ambiguous, expanding, or destructive ones.
...
Jarvis always applies this judgment, decides any finding that is unambiguous toward the accepted design, and escalates only genuinely ambiguous, expanding, or destructive findings.

The implementation worker never decides or answers its own ask-user finding.
It stops at the finding, routes the decision to jarvis, and applies only the decision returned through the active validation gate.
...
- Fixing a concrete defect that violates an original acceptance criterion is jarvis's to decide, regardless of implementation difficulty.
