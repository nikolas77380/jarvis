...
description: >-
  Agent-only procedure for Jarvis project management.
...
Cloning or registering a project is add intake and uses the same trigger.
This skill is the single owner of Jarvis's project-management procedure.
...
Use the registry format and parser contract owned by the header of `bin/dj-project-mode.sh`.
...
Never issue a raw removal command from Jarvis.
Once that preflight confirms none of the above and the captain's approval is concrete, AGENTS.md hard rule 1's captain-approved project operation exception authorizes jarvis to remove the clone directly and update its registry entry to match.
