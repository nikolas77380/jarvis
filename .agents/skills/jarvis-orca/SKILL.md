---
name: jarvis-orca
description: Agent-only operator checklist for Jarvis's Orca runtime backend. Use when switching to Orca, spawning or supervising Orca-backed work, smoke-testing Orca backend behavior, debugging Orca task state, or reconciling Orca-backed task metadata.
...
Use this as the operator checklist for Jarvis's experimental Orca runtime backend.
...
Prefer the `bin/dj-*` helpers over raw `orca` commands.
Use raw `orca` only when the helper surface cannot answer the inspection question, and keep the recorded jarvis metadata as the task identity.

## Preflight

Work from the current jarvis home or repo root.
If `DJ_HOME` is set, remember that operational state lives under `$DJ_HOME` while the helper scripts still run from this repo's `bin/`.

Before switching or spawning against Orca:

- Confirm Orca is intentionally selected through `--backend orca`, `DJ_BACKEND=orca`, or local `config/backend`.
...
Use `bin/dj-spawn.sh` so jarvis creates the brief, worktree, terminal, metadata, status file, and watcher surface together.
...
After spawn, check the task with jarvis helpers:

- `bin/dj-peek.sh dj-<id>` for launch failures, trust dialogs, or first output.
- `state/<id>.meta` for `backend=orca`, `terminal=`, `orca_worktree_id=`, and `worktree=`.
- `bin/dj-crew-state.sh <id>` when the current run state matters.
- `bin/dj-watch.sh` whenever there are tasks in flight and this session owns supervision.

Do not manually create the Orca worktree or terminal for a normal jarvis task.
Do not manually patch metadata to make an externally-created Orca terminal look like a jarvis task.

## Supervision

Use `bin/dj-peek.sh`, `bin/dj-send.sh`, `bin/dj-crew-state.sh`, and `bin/dj-teardown.sh` for routine operation.
For steer messages, use `bin/dj-send.sh <id> '...'`; the stable `dj-<id>` alias also works, and ordinary local text steers may contain newlines because they ride the durable inbox.
...
The stable jarvis alias is `dj-<id>`.
The recorded `terminal=` and `orca_worktree_id=` fields are what backend helpers use under the hood.

If an ordinary steer fails to enqueue, or a typed-plane `dj-send` fails to submit, do not immediately repeat the instruction.
...
4. Prefer jarvis helpers for peek, send, state, and teardown.
...
Teardown remains governed by the normal jarvis landing rules.
...
2. Spawn through `bin/dj-spawn.sh`.
3. Confirm metadata records the Orca backend, terminal, Orca worktree id, and isolated worktree path.
4. Verify `bin/dj-peek.sh`, a short `bin/dj-send.sh` steer, watcher wake behavior, and `bin/dj-crew-state.sh`.
5. Tear down through `bin/dj-teardown.sh` after the task is safely disposable or landed.
