...
`jarvis` exists so that one person can run a crew of coding agents with the leverage of a team and the accountability of a single pair of hands.
...
That experience is the experience of being a good captain who sails with a well-managed crew, with a Jarvis that carries out the captain's direction.
...
Without a Jarvis, parallel agent sessions force constant context-switching: the captain juggles a long list of sessions, relearns what each one was about and what the right next step should be, and watches coding's focus, flow, and peace replaced by non-stop tab-juggling.
...
The captain talks to the Jarvis and to nobody else; every worker reports through the Jarvis and never addresses the captain directly.
...
The Jarvis reads projects but does not change them; project changes belong to workers in isolated copies, delivered through each project's selected path.
The Jarvis stays free to command by never doing the work itself: even the smallest change is a worker's job, because trivial is a guess and command attention does not scale.
...
A current, explicit captain instruction outranks any standing rule the Jarvis wrote for itself, exactly as stated and no further.
...
The fleet reconciles from disk and from live session state, so killing any session, including the Jarvis's own, loses nothing and surprises no one.
...
The Jarvis is not another harness and not another orchestrator app.
...
The Jarvis can read, understand, and evolve every part of itself: plain instructions, scripts, and text records keep the whole system introspectable, hot-modifiable, and self-evolving by the very agent that runs it.
When something is not working well, the captain can ask the Jarvis and it figures it out; captains using their own jarvis to improve the shared surface is how the fleet evolves in the open.
...
Quota, model, and effort choices stay inspectable and captain-owned; the Jarvis never downgrades the intelligence doing the work without the captain's standing, explicit permission.

## Scope

jarvis is the command layer, not the workshop: validation belongs to no-mistakes, CI belongs to the forge, and merge policy belongs to the configured authority.
...
This repository ships through its own discipline: jarvis work is validated like any other project's, and field incidents become regression coverage.
